...
***REMOVED*** Mock test - actual tests will run once numpy is available in test environment
class TestFourierAnalysisTools:
    ...
    async def test_detect_schedule_cycles_weekly_pattern(self):
        ...
        ***REMOVED*** Import here to avoid module-level import errors if numpy not installed
        try:
            from scheduler_mcp.tools.fourier_analysis_tools import detect_schedule_cycles
        except ImportError:
            pytest.skip("Numpy not available")

        ***REMOVED*** Create synthetic 7-day periodic signal (4 weeks)
        days = 28
        ...
        ***REMOVED*** Should detect ~7-day period (allow some tolerance)
    ...
    async def test_detect_schedule_cycles_too_short(self):
        ...
        ***REMOVED*** Signal too short (< 7 samples)
        signal = [1.0, 2.0, 3.0]
    ...
    async def test_analyze_harmonic_resonance_weekly_aligned(self):
        ...
        ***REMOVED*** Perfect 7-day cycle (aligned with ACGME weekly)
        days = 28
        signal = [40 + 10 * np.sin(2 * np.pi * i / 7) for i in range(days)]

        result = await analyze_harmonic_resonance(signal)

        ***REMOVED*** Should have strong 7-day alignment
    ...
    async def test_analyze_harmonic_resonance_dissonant(self):
        ...
        ***REMOVED*** 5-day cycle (not aligned with ACGME 7/14/28)
        days = 30
        signal = [40 + 10 * np.sin(2 * np.pi * i / 5) for i in range(days)]

        result = await analyze_harmonic_resonance(signal)

        ***REMOVED*** Should have weak ACGME alignment
        assert result.overall_resonance < 0.7
        ***REMOVED*** Should detect dissonant frequencies
        assert len(result.recommendations) > 0
    ...
    async def test_calculate_spectral_entropy_simple_signal(self):
        ...
        ***REMOVED*** Pure sinusoid - should have low entropy
        days = 28
        signal = [40 + 10 * np.sin(2 * np.pi * i / 7) for i in range(days)]
    ...
    async def test_calculate_spectral_entropy_complex_signal(self):
        ...
        ***REMOVED*** Multi-frequency signal - should have higher entropy
        days = 56
        signal = [
            40
        ...
        ***REMOVED*** Multi-frequency should have higher entropy than pure sinusoid
    ...
    async def test_calculate_spectral_entropy_white_noise(self):
        ...
        ***REMOVED*** Random white noise - should have high entropy
        np.random.seed(42)
        days = 56
        ...
        ***REMOVED*** White noise should have high entropy
    ...
    async def test_calculate_spectral_entropy_flat_signal(self):
        ...
        ***REMOVED*** Constant signal (no variation)
        signal = [40.0] * 28

        result = await calculate_spectral_entropy(signal)

        ***REMOVED*** Flat signal should have very low/zero entropy
