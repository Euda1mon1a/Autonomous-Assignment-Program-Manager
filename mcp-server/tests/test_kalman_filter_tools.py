...
def volatile_workload_history() -> list[float]:
    ...
    ***REMOVED*** Extreme oscillations around 60, with high CV but no clear trend direction
...
***REMOVED*** =============================================================================
***REMOVED*** KalmanFilter1D Unit Tests
***REMOVED*** =============================================================================
...
class TestKalmanFilter1D:
    ...
    def test_predict_step(self):
        ...
        ***REMOVED*** State prediction should be unchanged (random walk)
        assert x_pred == 60.0
        ***REMOVED*** Error should increase by process noise
    ...
    def test_update_step(self):
        ...
        ***REMOVED*** State should move toward measurement
        assert x_new > 60.0
        assert x_new < 65.0
        ***REMOVED*** Error should decrease after update
        assert P_new < 1.1  ***REMOVED*** Less than predicted error
        ***REMOVED*** Kalman gain should be between 0 and 1
        assert 0.0 <= K <= 1.0
    ...
    def test_filter_smooths_noisy_data(self, noisy_workload_history):
        ...
        ***REMOVED*** Compute roughness (sum of absolute differences)
        raw_roughness = sum(
        ...
        ***REMOVED*** Filtered should be smoother
    ...
    def test_predict_future(self):
        ...
        ***REMOVED*** Run some updates first
...
class TestAnalyzeWorkloadTrend:
    ...
    async def test_trend_assessment_volatile(self, volatile_workload_history):
        ...
        ***REMOVED*** After filtering, trend should be one of the valid assessments
        assert result.trend_assessment in [
            "INCREASING",
            "STABLE",
            "DECREASING",
        ...
        ***REMOVED*** The raw data was volatile, so smoothness should be positive
        ***REMOVED*** (filter reduced volatility)
    ...
    async def test_predictions_when_requested(self, stable_workload_history):
        ...
        ***REMOVED*** Prediction confidence should have proper structure
    ...
    async def test_custom_noise_parameters(self, noisy_workload_history):
        ...
        ***REMOVED*** High measurement noise = trust measurements less
        ...
        ***REMOVED*** Low measurement noise = trust measurements more
        ...
        ***REMOVED*** High R should produce smoother output
    ...
    async def test_kalman_gain_converges(self, noisy_workload_history):
        ...
        ***REMOVED*** Gain should decrease as filter converges
...
***REMOVED*** =============================================================================
***REMOVED*** detect_workload_anomalies Tests
***REMOVED*** =============================================================================
...
class TestDetectWorkloadAnomalies:
    ...
    async def test_detects_spike_anomaly(self, workload_with_spike):
        ...
        ***REMOVED*** The spike at index 5 should be detected
    ...
    async def test_no_anomalies_in_stable_data(self, stable_workload_history):
        ...
        ***REMOVED*** Stable data should have low anomaly rate
    ...
    async def test_anomalies_sorted_by_severity(self, volatile_workload_history):
        ...
        if len(result.anomalies) > 1:
            ...
            for i in range(1, len(result.anomalies)):
                ...
                ***REMOVED*** Should be sorted by severity first
    ...
    async def test_threshold_sensitivity(self, workload_with_spike):
        ...
        ***REMOVED*** Lower threshold should detect more anomalies
    ...
    async def test_overall_assessment_concerning(self):
        ...
        ***REMOVED*** Data with a few anomalies (but not too many)
        ...
        ***REMOVED*** Should be concerning if there are HIGH severity anomalies
...
***REMOVED*** =============================================================================
***REMOVED*** Utility Function Tests
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Request Validation Tests
***REMOVED*** =============================================================================
...
class TestRequestValidation:
    ...
    def test_workload_trend_request_minimum_length(self):
        ...
        with pytest.raises(ValueError):
            WorkloadTrendRequest(workload_history=[60.0])  ***REMOVED*** Too short
    ...
    def test_anomaly_request_minimum_length(self):
        ...
        with pytest.raises(ValueError):
            WorkloadAnomalyRequest(workload_history=[60.0, 61.0])  ***REMOVED*** Too short

    def test_process_noise_bounds(self):
        """Test process noise parameter bounds."""
        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], process_noise=0.0001  ***REMOVED*** Too low
            )

        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], process_noise=100.0  ***REMOVED*** Too high
            )
    ...
    def test_measurement_noise_bounds(self):
        ...
        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], measurement_noise=0.0001  ***REMOVED*** Too low
            )

        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], measurement_noise=200.0  ***REMOVED*** Too high
            )
    ...
    def test_prediction_steps_bounds(self):
        ...
        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], prediction_steps=-1  ***REMOVED*** Negative
            )

        with pytest.raises(ValueError):
            WorkloadTrendRequest(
                workload_history=[60.0, 61.0, 62.0], prediction_steps=100  ***REMOVED*** Too high
            )
    ...
    def test_anomaly_threshold_bounds(self):
        ...
        with pytest.raises(ValueError):
            WorkloadAnomalyRequest(
                ...
                anomaly_threshold_sigma=0.5,  ***REMOVED*** Too low
            )

        with pytest.raises(ValueError):
            WorkloadAnomalyRequest(
                workload_history=[60.0, 61.0, 62.0],
                anomaly_threshold_sigma=10.0,  ***REMOVED*** Too high
            )
...
class TestToolIntegration:
    ...
    async def test_trend_and_anomaly_consistency(self, noisy_workload_history):
        ...
        ***REMOVED*** Both should produce filtered workload of same length
    ...
    async def test_prediction_follows_trend(self, increasing_workload_history):
        ...
        ***REMOVED*** For increasing trend, predictions should be near or above last filtered value
