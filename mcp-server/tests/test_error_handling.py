...
from scheduler_mcp.error_handling import (
    ***REMOVED*** Exception classes
    ...
    MCPCircuitOpenError,
    MCPErrorCode,
    ***REMOVED*** Decorators and utilities
    mcp_error_handler,
    retry_with_backoff,
    RetryConfig,
...
class TestRetryLogic:
    ...
    async def test_retry_success_after_failures(self):
        ...
        @retry_with_backoff(
            RetryConfig(
                max_attempts=3,
                base_delay=0.01,  ***REMOVED*** Fast for testing
            )
        )
    ...
    async def test_retry_non_retryable_exception(self):
        ...
        ***REMOVED*** Should not retry validation errors
        assert call_count == 1
    ...
    async def test_retry_backoff_timing(self):
        ...
        @retry_with_backoff(
            RetryConfig(
                ...
                jitter=False,  ***REMOVED*** Disable jitter for predictable timing
            )
        )
        async def always_fails():
        ...
        ***REMOVED*** Verify increasing delays (approximately)
        ***REMOVED*** Attempt 0: immediate
        ***REMOVED*** Attempt 1: ~0.1s delay (base_delay * 2^0)
        ***REMOVED*** Attempt 2: ~0.2s delay (base_delay * 2^1)
        assert len(call_times) == 3

        delay_1 = call_times[1] - call_times[0]
        delay_2 = call_times[2] - call_times[1]

        ***REMOVED*** Allow some tolerance for timing
        assert 0.08 <= delay_1 <= 0.15
        assert 0.18 <= delay_2 <= 0.25


***REMOVED*** ============================================================================
***REMOVED*** Circuit Breaker Tests
***REMOVED*** ============================================================================
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_opens_after_failures(self):
        ...
        ***REMOVED*** First 3 failures should open circuit
        for i in range(3):
            with pytest.raises(MCPServiceUnavailable):
                await cb.call(failing_call)

        ***REMOVED*** Circuit should now be open
        assert cb.state.state == CircuitState.OPEN

        ***REMOVED*** Next call should be rejected immediately
    ...
    async def test_circuit_breaker_half_open_recovery(self):
        ...
        cb = CircuitBreaker(
            ...
            CircuitBreakerConfig(
                failure_threshold=2,
                success_threshold=2,
                timeout=0.1,  ***REMOVED*** Short timeout for testing
            ),
        )
        ...
        ***REMOVED*** Trigger failures to open circuit
        for i in range(2):
            with pytest.raises(MCPServiceUnavailable):
        ...
        ***REMOVED*** Circuit should move to half-open and allow calls
        ***REMOVED*** Need 2 successes to close
    ...
    async def test_circuit_breaker_reopens_from_half_open(self):
        ...
        ***REMOVED*** Try in half-open state, should fail and re-open
        with pytest.raises(MCPServiceUnavailable):
            await cb.call(failing_call)
...
***REMOVED*** ============================================================================
***REMOVED*** Error Handler Decorator Tests
***REMOVED*** ============================================================================
...
class TestErrorHandlerDecorator:
    ...
    async def test_error_handler_with_circuit_breaker(self):
        ...
        ***REMOVED*** First two failures
        for i in range(2):
            with pytest.raises(MCPServiceUnavailable):
                await may_fail()

        ***REMOVED*** Third attempt should be rejected by circuit breaker
        with pytest.raises(MCPCircuitOpenError):
            await may_fail()


***REMOVED*** ============================================================================
***REMOVED*** Metrics Tests
***REMOVED*** ============================================================================
...
class TestIntegration:
    ...
    async def test_full_error_handling_stack(self):
        ...
        ***REMOVED*** Test successful retry
        attempt_count = 0
        ***REMOVED*** First two calls fail, third succeeds
        with pytest.raises(MCPServiceUnavailable):
            await complex_operation(should_succeed=False)

        ***REMOVED*** Verify circuit breaker state is tracked
