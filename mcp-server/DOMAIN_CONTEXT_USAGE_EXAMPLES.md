***REMOVED*** Domain Context Module - Usage Examples for MCP Tools

**Purpose:** Show how to integrate domain_context.py into MCP tools for better user experience

---

***REMOVED******REMOVED*** Example 1: Enriching Constraint Violation Responses

***REMOVED******REMOVED******REMOVED*** Before (Raw violation)
```python
@mcp.tool()
...
***REMOVED******REMOVED******REMOVED*** After (With domain context)
```python
...
***REMOVED******REMOVED*** Example 2: Creating a Glossary Tool
...
async def explain_term(term: str) -> dict:
    ...
    ***REMOVED*** Check if it's an abbreviation
    ...
    ***REMOVED*** Check if it's a constraint
    ...
    ***REMOVED*** Check if it's a pattern
...
***REMOVED******REMOVED*** Example 3: PII-Safe Field Filtering
...
async def get_schedule_details(schedule_id: str) -> dict:
    ...
    ***REMOVED*** Filter out PII-sensitive fields
    safe_schedule = {}
...
***REMOVED*** {
***REMOVED***   "schedule_id": "SCH-001",
***REMOVED***   "person_id": "RES-123",
***REMOVED***   "role": "PGY-2",
***REMOVED***   "person.name": "[REDACTED - PII]",
***REMOVED***   "person.email": "[REDACTED - PII]",
...
***REMOVED******REMOVED*** Example 4: Context-Aware Error Messages
...
async def assign_shift(
    ...
    try:
        ...
        ***REMOVED*** Get constraint explanation
        explanation = explain_constraint(e.constraint_name)

        ***REMOVED*** Expand abbreviations in error message
...
***REMOVED******REMOVED*** Example 5: Pattern Detection and Explanation
...
async def explain_date_pattern(date: str) -> dict:
    ...
    ***REMOVED*** Check if it's a Wednesday
    if dt.weekday() == 2:  ***REMOVED*** Wednesday
        day_of_month = dt.day

        ***REMOVED*** 4th Wednesday check (days 22-28)
...
***REMOVED******REMOVED*** Example 6: Interactive Constraint Help
...
async def constraint_help(constraint_name: str = None) -> dict:
    ...
    """
    if constraint_name:
        ***REMOVED*** Explain specific constraint
...
***REMOVED******REMOVED*** Example 7: Role Context for Assignments
...
async def validate_assignment(
    ...
    ***REMOVED*** Check clinic slot limits
...
***REMOVED******REMOVED*** Example 8: Batch Abbreviation Expansion for Reports
...
async def generate_schedule_report(
    ...
    if expand_terms:
        ***REMOVED*** Expand all abbreviations in the report text
        report_text = raw_report["summary"]
        expanded_text = expand_abbreviations(
            report_text,
            first_occurrence_only=True  ***REMOVED*** Only expand first occurrence for readability
        )
