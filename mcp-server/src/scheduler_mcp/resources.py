...
class AssignmentInfo(BaseModel):
    ...
    assignment_id: str
    person_id: str
    person_role: str  ***REMOVED*** Role-based identifier (e.g., "Faculty", "PGY-1") - no PII
    block_name: str
    rotation: str
    start_date: date
    end_date: date
...
class ViolationInfo(BaseModel):
    ...
    person_id: str
    person_role: str  ***REMOVED*** Role-based identifier (e.g., "PGY-1", "Faculty") - no PII
...
async def get_schedule_status(
    ...
    ***REMOVED*** Import models
    ...
    db = _get_db_session()
    try:
        ***REMOVED*** Query assignments in date range with related data
        assignments_query = (
            db.query(Assignment)
        ...
        ***REMOVED*** Build assignment list
        ...
        for assignment in assignments_query:
            ...
            ***REMOVED*** Track for coverage metrics
            assignment_dates[block.date]["blocks"].append(block)
            assignment_dates[block.date]["person_ids"].add(person.id)

            ***REMOVED*** Create assignment info
            ***REMOVED*** Determine role based on person type
        ...
        ***REMOVED*** Get all dates in range for checking coverage
        ...
        while current_date <= end_date:
            if current_date in assignment_dates:
                ...
                ***REMOVED*** Check staffing levels (simplified - at least 1 faculty per day)
                if faculty_count == 0:
                    understaffed_days += 1
                elif faculty_count > 10:  ***REMOVED*** Example threshold
                    overstaffed_days += 1
            else:
                ***REMOVED*** No coverage on this day
                understaffed_days += 1
        ...
        ***REMOVED*** Count active conflicts
        active_conflicts = (
        ...
        ***REMOVED*** Get latest schedule run info
        latest_run = (
...
async def get_compliance_summary(
    ...
    ***REMOVED*** Import models and validators
    import sys
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../../backend'))
    ...
    db = _get_db_session()
    try:
        ***REMOVED*** Query all assignments in date range
        assignments = (
            db.query(Assignment)
            .join(Block, Assignment.block_id == Block.id)
            .join(Person, Assignment.person_id == Person.id)
        ...
        ***REMOVED*** Get all residents in the period
        resident_ids = set()
        ...
        ***REMOVED*** ACGME Validation: 80-hour rule
        HOURS_PER_BLOCK = 6
        MAX_WEEKLY_HOURS = 80
        ROLLING_WEEKS = 4

        for resident in residents:
            ***REMOVED*** Get resident's assignments
            ...
            ***REMOVED*** Check 80-hour rule - rolling 4-week windows
            for i, window_start in enumerate(sorted_dates):
                window_end = window_start + timedelta(days=27)  ***REMOVED*** 4 weeks - 1 day
                ...
                if avg_weekly > MAX_WEEKLY_HOURS:
                    ...
                    ***REMOVED*** Determine role for violation
                    ...
                    break  ***REMOVED*** One violation per resident is enough for summary

            ***REMOVED*** Check 1-in-7 rule - maximum 6 consecutive days
        ...
        ***REMOVED*** Check supervision ratios per block
        ...
        for block_id, personnel in by_block.items():
            ...
            ***REMOVED*** Calculate required faculty
            ...
            if len(faculty_in_block) < required_faculty:
                ...
                ***REMOVED*** Add resident IDs to affected set
                ...
                warnings_list.append(ViolationInfo(
                    ...
                    severity="warning",
                    person_id=str(block_id),  ***REMOVED*** Block ID for supervision issues
                    person_role="Block-level",  ***REMOVED*** Supervision issues affect entire blocks
        ...
        ***REMOVED*** Calculate overall compliance rate
