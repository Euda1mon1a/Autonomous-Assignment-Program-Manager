...
***REMOVED*** Try to import Starlette for health endpoint (optional for HTTP transport)
try:
    ...
    JSONResponse = None  ***REMOVED*** type: ignore
    Route = None  ***REMOVED*** type: ignore
...
***REMOVED*** Import circuit breaker tools
...
***REMOVED*** Import composite/advanced resilience tools
from .composite_resilience_tools import (
    ***REMOVED*** Creep Fatigue
    CreepFatigueResponse,
    ***REMOVED*** Recovery Distance
    RecoveryDistanceResponse,
    ***REMOVED*** Transcription Factors
    TranscriptionTriggersResponse,
    ***REMOVED*** Unified Critical Index
...
***REMOVED*** Import empirical testing tools
...
***REMOVED*** Import FRMS (Fatigue Risk Management System) tools
...
***REMOVED*** Import immune system tools (AIS - Artificial Immune System)
...
***REMOVED*** Import optimization and analytics tools (Erlang C, Six Sigma, Equity)
...
***REMOVED*** Import resilience integration tools (N-1/N-2, Defense Levels, Epidemiology)
...
***REMOVED*** Import thermodynamics tools (entropy, phase transitions, free energy)
...
***REMOVED*** Import time crystal tools
...
***REMOVED*** Import Hopfield network attractor tools (energy landscape & schedule stability)
...
***REMOVED*** Import VaR (Value-at-Risk) tools for schedule vulnerability analysis
...
***REMOVED*** Import Game Theory tools for Nash equilibrium analysis
...
***REMOVED*** Import Ecological dynamics tools (Lotka-Volterra supply/demand modeling)
...
***REMOVED*** Import Kalman filter tools for workload trend analysis
...
***REMOVED*** Import Fourier/FFT analysis tools for periodicity detection
...
***REMOVED*** Import validate_schedule tool with ConstraintService integration
...
***REMOVED*** Configure logging - MUST use stderr for STDIO transport compatibility
***REMOVED*** MCP uses stdout for JSON-RPC messages; logging to stdout corrupts the protocol
logging.basicConfig(
    level=logging.INFO,
...
***REMOVED*** Initialize FastMCP server
mcp = FastMCP(
    "Residency Scheduler",
    version="0.1.0",
    instructions=(
...
def parse_date_range(date_range: str) -> tuple[date, date]:
    ...
    ***REMOVED*** Normalize the input
    ...
    if range_lower in ("current", "today"):
        ...
        ***REMOVED*** Try to parse as explicit range "YYYY-MM-DD:YYYY-MM-DD"
        try:
            ...
            ***REMOVED*** Invalid format, fall through to default
            pass

    ***REMOVED*** Default: next 30 days
    return today, today + timedelta(days=30)


***REMOVED*** Register Resources
...
async def schedule_status_resource(
    ...
    """
    ***REMOVED*** If explicit start/end dates are provided, use them
    if start_date or end_date:
        start = date.fromisoformat(start_date) if start_date else None
        end = date.fromisoformat(end_date) if end_date else None
    else:
        ***REMOVED*** Parse date_range preset
...
async def validate_schedule_by_id_tool(
    ...
    Example:
        ***REMOVED*** Validate a schedule with default constraints
...
async def run_contingency_analysis_tool(
    ...
    request = ContingencyRequest(
        scenario=scenario,  ***REMOVED*** type: ignore
...
async def detect_conflicts_tool(
    ...
    request = ConflictDetectionRequest(
        start_date=date.fromisoformat(start_date),
        end_date=date.fromisoformat(end_date),
        conflict_types=list(conflict_types) if conflict_types else None,  ***REMOVED*** type: ignore
...
***REMOVED*** Async Task Management Tools
...
***REMOVED*** Resilience Framework Tools
...
***REMOVED*** Deployment Workflow Tools
...
async def validate_deployment_tool(
    ...
    Example:
        ***REMOVED*** Validate staging deployment
...
async def run_security_scan_tool(
    ...
    Example:
        ***REMOVED*** Run full security scan
        result = await run_security_scan_tool(
            git_ref="main",
            dry_run=False
        )

        ***REMOVED*** Check critical vulnerabilities
...
async def run_smoke_tests_tool(
    ...
    Example:
        ***REMOVED*** Run basic smoke tests on staging
...
async def rollback_deployment_tool(
    ...
    Example:
        ***REMOVED*** Rollback production to previous version
...
async def list_deployments_tool(
    ...
    Example:
        ***REMOVED*** List last 5 production deployments
...
***REMOVED*** =============================================================================
***REMOVED*** Empirical Testing Tools
***REMOVED*** =============================================================================
...
async def benchmark_solvers_tool(
    ...
    Example:
        ***REMOVED*** Compare all solvers
        result = await benchmark_solvers_tool()

        ***REMOVED*** Compare specific solvers
        result = await benchmark_solvers_tool(
...
async def ablation_study_tool(
    ...
    Example:
        ***REMOVED*** Check if tensegrity solver can be removed
...
***REMOVED*** =============================================================================
***REMOVED*** Research-to-Tool Conversions (Game Theory, Complex Systems, Signal Processing)
***REMOVED*** Time Crystal Scheduling Tools
***REMOVED*** =============================================================================
...
async def calculate_shapley_workload_tool(
    ...
    Example:
        ...
        ***REMOVED*** Dr. Smith: Shapley=35.2%, Gap=+39.5 hours (overworked)
    """
    ...
    ***REMOVED*** For now, return placeholder with structure
    ***REMOVED*** TODO: Connect to actual ShapleyValueService when DB is available
    logger.info(
...
async def detect_critical_slowing_down_tool(
    ...
    ***REMOVED*** 2. Autocorrelation at lag-1
    if n >= 2:
        ac1 = float(np.corrcoef(data[:-1], data[1:])[0, 1])
    else:
        ac1 = 0.0

    ***REMOVED*** 3. Relaxation time approximation (simplified)
    ***REMOVED*** In a real implementation, this would track perturbation recovery
    mean_util = np.mean(data)
    deviations = np.abs(np.array(data) - mean_util)
    relaxation_proxy = float(np.mean(deviations) * 24)  ***REMOVED*** hours

    ***REMOVED*** Count triggered signals
    signals = 0
    triggered = []
...
async def detect_schedule_changepoints_tool(
    ...
    Example:
        ...
        ***REMOVED*** Detects mean shift at index 3 (2025-01-04): 8.0 → 12.0 hours
    """
    import numpy as np
    ...
    ***REMOVED*** Simplified PELT detection
    if "pelt" in methods:
        min_segment = 5
        change_points = []

        ***REMOVED*** Simple variance-based segmentation
...
async def analyze_schedule_rigidity_tool(
    ...
    Example:
        ***REMOVED*** Compare two schedule versions
        ...
        print(f"Rigidity: {result.rigidity_score}")  ***REMOVED*** 0.0 - complete change
...
***REMOVED*** =============================================================================
***REMOVED*** Composite/Advanced Resilience Tools
***REMOVED*** =============================================================================
...
async def analyze_schedule_periodicity_tool(
    ...
    Example:
        ...
        ***REMOVED*** [7, 14, 28] - weekly, biweekly, ACGME 4-week
    """
...
***REMOVED*** =============================================================================
***REMOVED*** Epidemiological Burnout Modeling Tools (Tier 4)
***REMOVED*** =============================================================================
...
async def calculate_burnout_rt_tool(
    ...
    Example:
        ***REMOVED*** Calculate Rt for providers with high burnout
...
async def simulate_burnout_spread_tool(
    ...
    Example:
        ***REMOVED*** Simulate 1 year of burnout spread starting with 3 cases
...
async def simulate_burnout_contagion_tool(
    ...
    Example:
        ***REMOVED*** Simulate contagion with burnout scores for team
        scores = {
            "provider-1": 0.7,   ***REMOVED*** High burnout
            "provider-2": 0.3,   ***REMOVED*** Low burnout
            "provider-3": 0.9,   ***REMOVED*** Very high burnout - potential superspreader
            "provider-4": 0.4,   ***REMOVED*** Moderate
            "provider-5": 0.6,   ***REMOVED*** Elevated
        }
        ...
        ***REMOVED*** Identify superspreaders
        ...
        ***REMOVED*** Get intervention recommendations
...
***REMOVED*** =============================================================================
***REMOVED*** Circuit Breaker Tools (Distributed Systems Failure Isolation)
***REMOVED*** =============================================================================
...
async def test_half_open_tool(
    ...
    Example:
        ***REMOVED*** Check if database breaker is recovering
...
async def override_circuit_breaker_tool(
    ...
    Example:
        ***REMOVED*** Force close after verified recovery
        ...
        ***REMOVED*** Force open during known issues
        result = await override_circuit_breaker_tool(
        ...
        ***REMOVED*** Reset after root cause fix
        result = await override_circuit_breaker_tool(
...
***REMOVED*** =============================================================================
***REMOVED*** Optimization and Analytics Tools
***REMOVED*** =============================================================================
...
async def optimize_erlang_coverage_tool(
    ...
    Example:
        ***REMOVED*** Optimize ER orthopedic coverage
        result = await optimize_erlang_coverage_tool(
            specialty="Orthopedic Surgery",
            arrival_rate=2.5,  ***REMOVED*** 2.5 cases/hour
            service_time_minutes=30,  ***REMOVED*** 30 min per case
...
async def calculate_erlang_metrics_tool(
    ...
    Example:
        ***REMOVED*** Check metrics for 5 orthopedic surgeons
...
async def calculate_process_capability_tool(
    ...
    Example:
        ***REMOVED*** Analyze weekly work hours for ACGME compliance
...
async def calculate_equity_metrics_tool(
    ...
    Example:
        ***REMOVED*** Analyze faculty workload equity
        hours = {
            "FAC-001": 45,
            "FAC-002": 52,
            "FAC-003": 38,
...
async def generate_lorenz_curve_tool(
    ...
    Example:
        ***REMOVED*** Generate Lorenz curve for faculty hours
        hours = [45, 52, 38, 60, 42, 55, 48]
        curve = await generate_lorenz_curve_tool(hours)
        print(f"Gini coefficient: {curve.gini_coefficient:.3f}")

        ***REMOVED*** Data ready for plotting:
        ***REMOVED*** x-axis: curve.population_shares
        ***REMOVED*** y-axis: curve.value_shares
        ***REMOVED*** reference: curve.equality_line (45-degree line)
    """
    return await generate_lorenz_curve(values=values)


***REMOVED*** =============================================================================
***REMOVED*** Artificial Immune System Tools (AIS - Immunology-Inspired Resilience)
***REMOVED*** =============================================================================
...
async def check_memory_cells_tool(
    ...
    Example:
        ...
        ***REMOVED*** Find most common anomaly types
...
async def analyze_antibody_response_tool(
    ...
    Example:
        ***REMOVED*** Find best repair for current schedule
        schedule = {
            "total_blocks": 100,
            "covered_blocks": 75,  ***REMOVED*** Low coverage
            "faculty_count": 10,
            "resident_count": 25,
...
***REMOVED*** =============================================================================
***REMOVED*** Thermodynamics Tools (Entropy, Phase Transitions, Free Energy)
***REMOVED*** =============================================================================
...
async def calculate_time_crystal_objective_tool(
    ...
    Example:
        result = await calculate_time_crystal_objective_tool(
            current_assignments=[...],
            proposed_assignments=[...],
            alpha=0.3,  ***REMOVED*** 30% weight on stability
            beta=0.1    ***REMOVED*** 10% weight on fair churn distribution
        )
...
***REMOVED*** =============================================================================
***REMOVED*** Hopfield Network Attractor Tools (Energy Landscape & Schedule Stability)
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Early Warning System Tools (Cross-Disciplinary Burnout Detection)
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** FRMS (Fatigue Risk Management System) Tools
***REMOVED*** =============================================================================
...
***REMOVED*** Server lifecycle functions (called by lifespan context manager)
...
async def on_shutdown() -> None:
    ...
    ***REMOVED*** Close database connections
    ...
    ***REMOVED*** Close any open connections
    ...
    ***REMOVED*** Flush any pending metrics
    ...
    ***REMOVED*** Release any held resources
...
***REMOVED*** =============================================================================
***REMOVED*** Health Check Endpoint for HTTP Transport
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** API Key Authentication Middleware
***REMOVED*** =============================================================================
...
class APIKeyAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        ***REMOVED*** Allow health checks without auth
        if path in self.exempt_paths:
            await self.app(scope, receive, send)
            return

        ***REMOVED*** If no API key configured, allow all requests (dev mode)
        if not self.api_key:
        ...
        ***REMOVED*** Support both "Bearer <token>" and raw token
        token = None
...
Examples:
  ***REMOVED*** Run with STDIO transport (default, for Claude Desktop/IDE)
  scheduler-mcp

  ***REMOVED*** Run with HTTP transport (for Docker containers)
  MCP_TRANSPORT=http scheduler-mcp --host 0.0.0.0 --port 8080

  ***REMOVED*** Run with HTTP transport and authentication (for Render deployment)
  MCP_TRANSPORT=http MCP_API_KEY=your-secret-key scheduler-mcp --host 0.0.0.0 --port 8080

  ***REMOVED*** Run with debug logging
  LOG_LEVEL=DEBUG scheduler-mcp
    ...
    ***REMOVED*** Configure logging level
    ...
    if args.transport != "stdio":
        ...
        ***REMOVED*** Check for API key in production
    ...
    ***REMOVED*** Run the server with appropriate transport
    if args.transport == "stdio":
        mcp.run(transport="stdio")
    else:
        ***REMOVED*** For HTTP transport, use ASGI app with auth middleware
        try:
            import uvicorn
            from starlette.applications import Starlette
            from starlette.routing import Route, Mount

            ***REMOVED*** Get the MCP ASGI app
            mcp_app = mcp.http_app()

            ***REMOVED*** Create health endpoint
            health_handler = create_health_endpoint()
            routes = []
            if health_handler:
                routes.append(Route("/health", health_handler, methods=["GET"]))

            ***REMOVED*** Mount MCP app at /mcp and add health at root
            routes.append(Mount("/mcp", app=mcp_app))

            ***REMOVED*** Also mount at root for backwards compatibility
            routes.append(Mount("/", app=mcp_app))

            app = Starlette(routes=routes)

            ***REMOVED*** Wrap with auth middleware
