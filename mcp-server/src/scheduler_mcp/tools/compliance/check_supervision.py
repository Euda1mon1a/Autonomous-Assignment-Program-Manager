...
class SupervisionRatio(BaseModel):
    ...
    level: str  ***REMOVED*** PGY-1, PGY-2, PGY-3, etc.
    residents: int
    faculty: int
...
class CheckSupervisionTool(
    ...
    async def execute(
        ...
        try:
            ***REMOVED*** Check supervision via API client
            data = await client.check_supervision(
                date=request.date,
                session=request.session,
            )

            ***REMOVED*** Parse ratios
            ratios = []
            ...
            ***REMOVED*** Check overall compliance
            ...
            ***REMOVED*** Missing required data fields
            return SupervisionCheckResponse(
                date=request.date,
                session=request.session,
                overall_compliant=False,
