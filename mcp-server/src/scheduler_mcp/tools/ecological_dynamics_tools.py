...
class SystemStabilityEnum(str, Enum):
    ...
    STABLE = "stable"  ***REMOVED*** Damped oscillations, converging to equilibrium
    MARGINALLY_STABLE = "marginally_stable"  ***REMOVED*** Persistent oscillations
    UNSTABLE = "unstable"  ***REMOVED*** Growing oscillations, diverging from equilibrium
    CHAOTIC = "chaotic"  ***REMOVED*** Irregular, unpredictable behavior


class InterventionTypeEnum(str, Enum):
    """Types of interventions that can be simulated."""

    ADD_CAPACITY = "add_capacity"  ***REMOVED*** Hire residents, expand rotations
    REDUCE_DEMAND = "reduce_demand"  ***REMOVED*** Cancel elective procedures, defer clinics
    INCREASE_EFFICIENCY = "increase_efficiency"  ***REMOVED*** Reduce β (demand consumption rate)
    MOONLIGHTING = "moonlighting"  ***REMOVED*** Temporary capacity injection
    LOCUMS = "locums"  ***REMOVED*** External capacity temporary injection
    SCHEDULE_COMPRESSION = "schedule_compression"  ***REMOVED*** Increase utilization
    DEMAND_SMOOTHING = "demand_smoothing"  ***REMOVED*** Spread workload over time


class RiskLevelEnum(str, Enum):
    """Risk level based on predicted capacity crunch timing."""

    LOW = "low"  ***REMOVED*** >90 days until crunch
    MODERATE = "moderate"  ***REMOVED*** 60-90 days
    ELEVATED = "elevated"  ***REMOVED*** 30-60 days
    HIGH = "high"  ***REMOVED*** 14-30 days
    CRITICAL = "critical"  ***REMOVED*** <14 days
    IMMINENT = "imminent"  ***REMOVED*** <7 days


***REMOVED*** =============================================================================
***REMOVED*** Core Lotka-Volterra Functions
***REMOVED*** =============================================================================
...
def fit_lotka_volterra_parameters(
    ...
    if initial_guess is None:
        ***REMOVED*** Reasonable defaults for medical scheduling
        initial_guess = (0.1, 0.01, 0.01, 0.1)

    ***REMOVED*** Convert lists to numpy arrays for vectorized operations
    ...
    def residuals(params: np.ndarray) -> np.ndarray:
        ...
        ***REMOVED*** Ensure positive parameters
        ...
        ***REMOVED*** Combined residuals for both capacity and demand
    ...
    ***REMOVED*** Bounds: all parameters must be positive
    ...
    ***REMOVED*** Calculate R² for goodness of fit
    ...
    ***REMOVED*** Clamp R² to [0, 1] for Pydantic validation
...
def simulate_trajectory(
    ...
    """
    t = np.linspace(0, days, days * 10)  ***REMOVED*** 10 points per day for smooth curves
...
def classify_stability(
    ...
    ***REMOVED*** Check for chaotic behavior (high irregularity)
    ***REMOVED*** Use coefficient of variation as irregularity metric
    ...
    ***REMOVED*** Default to marginally stable (persistent oscillations)
...
async def analyze_supply_demand_cycles(
    ...
    ***REMOVED*** Classify stability
    stability = classify_stability(alpha, beta, delta, gamma, capacity_traj)

    ***REMOVED*** Build predicted trajectory
    predicted_trajectory = [
        {"day": int(day), "capacity": float(cap), "demand": float(dem)}
        for day, cap, dem in zip(t[::10], capacity_traj[::10], demand_traj[::10])  ***REMOVED*** Subsample for readability
    ]

    ***REMOVED*** Ecological interpretation
    ...
    return SupplyDemandCyclesResponse(
        ...
        confidence=r_squared,  ***REMOVED*** Use R² as confidence measure
    )
...
async def predict_capacity_crunch(
    ...
    ***REMOVED*** Find when capacity drops below threshold
...
async def find_equilibrium_point(
    ...
    ***REMOVED*** Calculate oscillation period
    period = estimate_oscillation_period(request.alpha, request.gamma)

    ***REMOVED*** Check stability (for LV, equilibrium is always marginally stable - center point)
    is_stable = True  ***REMOVED*** LV equilibrium is a center (neutral stability)
...
async def simulate_intervention(
    ...
    ***REMOVED*** Apply intervention by modifying parameters
    ...
    if request.intervention_type == InterventionTypeEnum.ADD_CAPACITY:
        ***REMOVED*** Increase alpha (capacity growth rate)
        ...
        ***REMOVED*** Increase gamma (demand decay rate)
        ...
        ***REMOVED*** Decrease beta (consumption rate)
        ...
        ***REMOVED*** Temporary capacity boost - increase initial capacity
        capacity_boost = request.current_capacity * request.intervention_magnitude
        ***REMOVED*** Will model as one-time capacity injection
        ...
        ***REMOVED*** Increase beta (more efficient consumption) and increase gamma (faster processing)
        beta_int = request.beta * (1 + request.intervention_magnitude)
        gamma_int = request.gamma * (1 + request.intervention_magnitude)
        parameter_changes["beta"] = {"before": request.beta, "after": beta_int}
        parameter_changes["gamma"] = {"before": request.gamma, "after": gamma_int}
        ...
        ***REMOVED*** Reduce delta (demand amplification) and increase gamma (decay)
    ...
    ***REMOVED*** Intervention simulation
    ...
    ***REMOVED*** Calculate effectiveness score
    ***REMOVED*** Combine capacity improvement and amplitude reduction
    ...
    effectiveness = (capacity_score * 0.6 + amplitude_score * 0.4)  ***REMOVED*** Weight capacity more
    effectiveness = max(0.0, min(1.0, effectiveness))  ***REMOVED*** Clamp to [0, 1]
