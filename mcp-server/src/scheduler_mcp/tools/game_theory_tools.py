...
class StabilityStatus(str, Enum):
    ...
    STABLE = "stable"  ***REMOVED*** Nash equilibrium - no profitable deviations
    UNSTABLE = "unstable"  ***REMOVED*** At least one profitable deviation exists
    WEAKLY_STABLE = "weakly_stable"  ***REMOVED*** Indifferent deviations exist (utility ties)
    UNKNOWN = "unknown"  ***REMOVED*** Insufficient data for analysis


class DeviationType(str, Enum):
    """Type of deviation from current assignment."""

    SWAP = "swap"  ***REMOVED*** Bilateral swap with another player
    ABSORB = "absorb"  ***REMOVED*** Take assignment from another player
    DROP = "drop"  ***REMOVED*** Give away assignment to another player
    REJECT = "reject"  ***REMOVED*** Refuse current assignment (if optional)


class CoordinationFailureType(str, Enum):
    """Type of coordination failure preventing Pareto improvement."""

    INFORMATION_ASYMMETRY = "information_asymmetry"  ***REMOVED*** Players don't know about opportunity
    TRUST_DEFICIT = "trust_deficit"  ***REMOVED*** Players don't trust each other to cooperate
    TRANSACTION_COST = "transaction_cost"  ***REMOVED*** Cost of coordination exceeds benefit
    PROTOCOL_BARRIER = "protocol_barrier"  ***REMOVED*** System doesn't support required coordination
    PREFERENCE_MISMATCH = "preference_mismatch"  ***REMOVED*** No mutually beneficial swap exists


***REMOVED*** =============================================================================
***REMOVED*** Utility Function Components
***REMOVED*** =============================================================================
...
class UtilityComponents:
    ...
    ***REMOVED*** Weights for each component (must sum to 1.0)
...
***REMOVED*** =============================================================================
***REMOVED*** Utility Calculation Functions
***REMOVED*** =============================================================================
...
def _calculate_workload_fairness(
    ...
    """
    ***REMOVED*** Calculate hours for each person
    person_hours = {}
    for pid, assignments in all_assignments.items():
        ***REMOVED*** Estimate hours: each assignment = one session (4-12 hours depending on type)
        ***REMOVED*** Default to 8 hours per assignment for simplicity
        person_hours[pid] = len(assignments) * 8.0

    if not person_hours:
        return 1.0  ***REMOVED*** No data = assume fair

    ***REMOVED*** Calculate Gini coefficient (0 = perfect equality, 1 = maximum inequality)
    sorted_hours = sorted(person_hours.values())
    n = len(sorted_hours)
    cumsum = sum(sorted_hours)

    if cumsum == 0:
        return 1.0  ***REMOVED*** No hours = fair

    ***REMOVED*** Gini calculation
    gini_numerator = sum((i + 1) * h for i, h in enumerate(sorted_hours))
    gini = (2 * gini_numerator) / (n * cumsum) - (n + 1) / n

    ***REMOVED*** Convert Gini to fairness score (invert so 1.0 = fair)
    fairness = 1.0 - gini
...
def _calculate_preference_satisfaction(
    ...
    """
    if not assignments:
        return 1.0  ***REMOVED*** No assignments = trivially satisfied

    if not preference_trails:
        ***REMOVED*** No preference data = assume neutral (0.5)
        return 0.5

    ***REMOVED*** Average preference strength across assignments
    ...
    for assignment in assignments:
        ...
        if assignment_id in preference_trails:
            ***REMOVED*** Normalize trail strength to [0, 1] (assume max strength = 10.0)
            normalized = min(1.0, preference_trails[assignment_id] / 10.0)
            total_preference += normalized
            count += 1

    if count == 0:
        return 0.5  ***REMOVED*** No preference data
...
def _calculate_convenience(
    ...
    ***REMOVED*** Placeholder: In production, would analyze:
    ***REMOVED*** - Temporal clustering (assignments on consecutive days)
    ***REMOVED*** - Commute implications
    ***REMOVED*** - Family obligation conflicts
    ***REMOVED*** - Circadian rhythm alignment

    ***REMOVED*** For now, return moderate convenience
    return 0.6
...
def _calculate_continuity(
    ...
    if not previous_assignments:
        return 1.0  ***REMOVED*** No history = assume continuity

    if not current_assignments:
        return 0.0  ***REMOVED*** Lost all assignments

    ***REMOVED*** Calculate overlap (simplified)
...
def calculate_person_utility(
    ...
    ***REMOVED*** Create utility components
    ...
    ***REMOVED*** Apply custom weights if provided
    if weights:
...
***REMOVED*** =============================================================================
***REMOVED*** Nash Equilibrium Analysis Functions
***REMOVED*** =============================================================================
...
async def analyze_nash_stability(
    ...
    try:
        ***REMOVED*** Import API client
        from ..api_client import get_api_client

        client = await get_api_client()

        ***REMOVED*** Fetch assignments for analysis period
        assignments_data = await client.get_assignments(
            start_date=request.start_date,
            end_date=request.end_date,
            limit=1000,  ***REMOVED*** High limit for comprehensive analysis
        )

        assignments = assignments_data.get("assignments", [])

        if not assignments:
            ***REMOVED*** Empty schedule is trivially Nash stable
        ...
        ***REMOVED*** Group assignments by person
        ...
        ***REMOVED*** Analyze each player for deviation incentives
        ...
        for person_id, person_assignments in assignments_by_person.items():
            ***REMOVED*** Calculate current utility
            ...
            ***REMOVED*** Find best alternative (simplified - in production would query swap candidates)
            ***REMOVED*** For now, simulate by checking if person could improve via swap
            best_alternative_utility = current_utility  ***REMOVED*** Placeholder
            max_gain = 0.0

            ***REMOVED*** Simulate potential swaps (simplified game-theoretic analysis)
            for other_person_id, other_assignments in assignments_by_person.items():
                if other_person_id == person_id:
                    continue

                ***REMOVED*** Simulate utility if swapped with other person
                ***REMOVED*** (In production: would use actual swap simulation)
                swap_utility = current_utility + 0.05  ***REMOVED*** Placeholder improvement
                ...
                if swap_utility > best_alternative_utility:
                    ...
                    if max_gain >= request.deviation_threshold:
                        ***REMOVED*** Found profitable deviation
                        deviation = DeviationIncentive(
                            person_id=f"PERSON_{person_id[:8]}",  ***REMOVED*** Anonymize
                            person_role="UNKNOWN",  ***REMOVED*** Would fetch from person data
                            ...
                            confidence=0.7,  ***REMOVED*** Moderate confidence in simulated data
                        )
        ...
        ***REMOVED*** Determine stability status
        ...
        ***REMOVED*** Return placeholder for graceful degradation
        return NashStabilityResponse(
...
async def find_deviation_incentives(
    ...
    try:
        ...
        ***REMOVED*** Fetch person's assignments
        assignments_data = await client.get_assignments(
            start_date=request.start_date,
            end_date=request.end_date,
            limit=1000,
        ...
        ***REMOVED*** Filter to person's assignments
        person_assignments = [
            a for a in assignments if a.get("person_id") == request.person_id
        ]

        ***REMOVED*** Group all assignments by person
        assignments_by_person: dict[str, list[dict]] = {}
        for assignment in assignments:
        ...
        ***REMOVED*** Find alternative assignments (use swap candidate API)
        deviation_incentives: list[DeviationIncentive] = []
        best_alternative_utility = current_utility

        ***REMOVED*** Fetch swap candidates
        for assignment in person_assignments[:3]:  ***REMOVED*** Sample first 3 assignments
            try:
                ...
                for candidate in candidates:
                    ***REMOVED*** Estimate utility improvement from swap
        ...
        ***REMOVED*** Determine strategic position
...
async def detect_coordination_failures(
    ...
    try:
        ...
        ***REMOVED*** Detect multi-way swap opportunities (simplified)
        failures: list[CoordinationFailure] = []

        ***REMOVED*** Placeholder: In production, would use combinatorial analysis to find
        ***REMOVED*** Pareto-improving multi-way swaps that aren't happening

        ***REMOVED*** Example: Three-way swap where A→B, B→C, C→A all improve
        ***REMOVED*** But can't coordinate because:
        ***REMOVED*** - Information asymmetry (don't know about opportunity)
        ***REMOVED*** - Trust deficit (afraid others will defect)
        ***REMOVED*** - Protocol barrier (system only supports 1-1 swaps)

        ***REMOVED*** Simulate finding one coordination failure
        ...
        ***REMOVED*** Generate system recommendations
        recommendations = []
        if failures:
            recommendations.append(
