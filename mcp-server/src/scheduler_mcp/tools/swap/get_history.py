...
class SwapRecord(BaseModel):
    ...
    id: str
    swap_type: str
    status: str  ***REMOVED*** pending, approved, executed, rolled_back, rejected
    person_id: str
...
class GetSwapHistoryTool(
    ...
    def validate_input(self, **kwargs: Any) -> GetSwapHistoryRequest:
        ...
        ***REMOVED*** Validate person_id if provided
        person_id = kwargs.get("person_id")
        if person_id is not None:
            person_id = validate_person_id(person_id)

        ***REMOVED*** Validate status if provided
    ...
    async def execute(
        ...
        try:
            ...
            ***REMOVED*** Get swap history via API
            ...
            ***REMOVED*** Network connectivity issues
            return GetSwapHistoryResponse(
                total_records=0,
                records=[],
            )
        except (KeyError, ValueError) as e:
            ***REMOVED*** Data parsing errors
            return GetSwapHistoryResponse(
                total_records=0,
                records=[],
            )
