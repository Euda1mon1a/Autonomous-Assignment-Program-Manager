...
def _calculate_alignment_score(
    ...
    ***REMOVED*** Calculate fractional deviation
    deviation = abs(detected_period - target_period) / target_period

    if deviation <= tolerance:
        ***REMOVED*** Linear decay within tolerance
...
async def detect_schedule_cycles(
    ...
    Example:
        ***REMOVED*** Analyze daily workload hours
        workload = [40, 45, 50, 48, 40, 0, 0, 42, 46, ...]  ***REMOVED*** Daily hours
    ...
    ***REMOVED*** Validate signal
    ...
    try:
        ***REMOVED*** Convert to numpy array and remove DC component (mean)
        ...
        ***REMOVED*** Skip DC component (index 0) and find peaks
        ...
        ***REMOVED*** Get indices of strongest peaks (excluding DC)
        power_no_dc = power_spectrum[1:]
        freqs_no_dc = freqs[1:]

        ***REMOVED*** Sort by power (descending)
        sorted_indices = np.argsort(power_no_dc)[::-1]
        top_indices = sorted_indices[:num_peaks]

        ***REMOVED*** Build period list
        ...
        for idx in top_indices:
            ...
            if freq > 0:  ***REMOVED*** Avoid division by zero
        ...
        ***REMOVED*** Determine if clear periodicity exists
        ***REMOVED*** Criteria: strongest peak has >20% of total power (excluding DC)
...
async def analyze_harmonic_resonance(
    ...
    ***REMOVED*** Calculate alignment for each ACGME period
    alignments = {
        "7d": 0.0,
        "14d": 0.0,
        "28d": 0.0,
    ...
    for period_obj in cycles.all_periods:
        for acgme_key, acgme_period in acgme_periods.items():
            ***REMOVED*** Check alignment with target period
            ...
            ***REMOVED*** Track resonant harmonics
            if alignment > 0.5:
        ...
        ***REMOVED*** Check for dissonant frequencies (strong but not aligned)
    ...
    ***REMOVED*** Overall resonance (weighted average)
    overall_resonance = (
...
async def calculate_spectral_entropy(
    ...
    ***REMOVED*** Validate signal
    ...
    try:
        ***REMOVED*** Compute power spectrum
        signal_array = np.array(signal, dtype=float)
        signal_detrended = signal_array - np.mean(signal_array)
        fft_result = np.fft.rfft(signal_detrended)
        power_spectrum = np.abs(fft_result) ** 2

        ***REMOVED*** Remove DC component
        power_no_dc = power_spectrum[1:]
