...
class Trend(BaseModel):
    ...
    metric: str
    direction: str  ***REMOVED*** increasing, decreasing, stable
    slope: float
...
class TrendAnalysisTool(BaseTool[TrendAnalysisRequest, TrendAnalysisResponse]):
    ...
    async def execute(
        ...
        try:
            ***REMOVED*** Get trend analysis via API
            ...
            ***REMOVED*** Parse trends
            trends = []
            total_anomalies = 0
            for item in data.get("trends", []):
                ***REMOVED*** Parse data points
                data_points = []
