...
class SamnPerelliLevelEnum(int, Enum):
    ...
    FULLY_ALERT = 1  ***REMOVED*** Fully alert, wide awake, extremely peppy
    VERY_LIVELY = 2  ***REMOVED*** Very lively, responsive, but not at peak
    OKAY = 3  ***REMOVED*** Okay, somewhat fresh
    A_LITTLE_TIRED = 4  ***REMOVED*** A little tired, less than fresh
    MODERATELY_TIRED = 5  ***REMOVED*** Moderately tired, let down
    EXTREMELY_TIRED = 6  ***REMOVED*** Extremely tired, very difficult to concentrate
    EXHAUSTED = 7  ***REMOVED*** Completely exhausted, unable to function effectively


class CircadianPhaseEnum(str, Enum):
    """Circadian rhythm phases affecting alertness."""

    NADIR = "nadir"  ***REMOVED*** 2-6 AM: Lowest alertness
    EARLY_MORNING = "early_morning"  ***REMOVED*** 6-9 AM: Increasing alertness
    MORNING_PEAK = "morning_peak"  ***REMOVED*** 9-12 PM: Peak alertness
    POST_LUNCH = "post_lunch"  ***REMOVED*** 12-3 PM: Post-prandial dip
    AFTERNOON = "afternoon"  ***REMOVED*** 3-6 PM: Secondary peak
    EVENING = "evening"  ***REMOVED*** 6-9 PM: Declining alertness
    NIGHT = "night"  ***REMOVED*** 9 PM-2 AM: Pre-sleep decline


class HazardLevelEnum(str, Enum):
    """Fatigue hazard levels with escalating interventions."""

    GREEN = "green"  ***REMOVED*** Normal operations
    YELLOW = "yellow"  ***REMOVED*** Advisory - enhanced monitoring
    ORANGE = "orange"  ***REMOVED*** Cautionary - schedule review
    RED = "red"  ***REMOVED*** Warning - modification required
    BLACK = "black"  ***REMOVED*** Critical - immediate intervention


class MitigationTypeEnum(str, Enum):
    """Types of fatigue mitigation actions."""

    MONITORING = "monitoring"  ***REMOVED*** Enhanced observation
    BUDDY_SYSTEM = "buddy_system"  ***REMOVED*** Pair with alert colleague
    DUTY_RESTRICTION = "duty_restriction"  ***REMOVED*** Limit to lower-risk duties
    SHIFT_SWAP = "shift_swap"  ***REMOVED*** Swap with rested colleague
    EARLY_RELEASE = "early_release"  ***REMOVED*** End shift early
    SCHEDULE_MODIFICATION = "schedule_modification"  ***REMOVED*** Modify future schedule
    MANDATORY_REST = "mandatory_rest"  ***REMOVED*** Required rest period
    IMMEDIATE_RELIEF = "immediate_relief"  ***REMOVED*** Immediate coverage by backup


class SleepDebtSeverityEnum(str, Enum):
    """Sleep debt severity classification."""

    NONE = "none"  ***REMOVED*** < 2 hours
    MILD = "mild"  ***REMOVED*** 2-5 hours
    MODERATE = "moderate"  ***REMOVED*** 5-10 hours
    SEVERE = "severe"  ***REMOVED*** 10-20 hours
    CRITICAL = "critical"  ***REMOVED*** > 20 hours


class FatigueRiskLevelEnum(str, Enum):
    """Overall fatigue risk classification."""

    MINIMAL = "minimal"  ***REMOVED*** Fully alert, no concerns
    LOW = "low"  ***REMOVED*** Minor fatigue factors
    MODERATE = "moderate"  ***REMOVED*** Some fatigue, monitoring needed
    HIGH = "high"  ***REMOVED*** Significant fatigue, intervention needed
...
async def run_frms_assessment(
    ...
    Example:
        ***REMOVED*** Get current fatigue profile
    ...
    try:
        ...
        ***REMOVED*** Parse target time
        if target_time:
            target_dt = datetime.fromisoformat(target_time)
        else:
            target_dt = datetime.utcnow()

        ***REMOVED*** Create service (without DB for now - would integrate with API client)
        service = FRMSService()
...
async def _run_frms_assessment_fallback(
    ...
    ***REMOVED*** Derive SP level from alertness
...
async def get_fatigue_score(
    ...
    Example:
        ***REMOVED*** Score after 18 hours awake, 12 hours worked
        result = await get_fatigue_score(
            hours_awake=18,
            hours_worked_24h=12,
            time_of_day_hour=4  ***REMOVED*** 4 AM circadian nadir
        )
    ...
    ***REMOVED*** Validate inputs
    ...
    try:
        ...
        ***REMOVED*** Determine safety and restrictions
        is_safe = sp_level <= 6
        duty_restrictions = []
        if sp_level >= 5:
...
async def _get_fatigue_score_fallback(
    ...
    ***REMOVED*** Calculate score based on factors
    score = 1.0
...
async def analyze_sleep_debt(
    ...
    Example:
        ***REMOVED*** Analyze past week of sleep
    ...
    try:
        ...
        ***REMOVED*** Calculate cumulative debt
        resident_id = uuid4()
        total_debt = 0.0
        deficit_days = 0
...
async def scan_team_fatigue(
    ...
    Example:
        ***REMOVED*** Get all residents at ORANGE or higher risk
    ...
    try:
        ...
        ***REMOVED*** Determine overall severity
...
async def assess_schedule_fatigue_risk(
    ...
    ***REMOVED*** Validate shift structure
