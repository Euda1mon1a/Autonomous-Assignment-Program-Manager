...
class AuditLogger:
    ...
    def log_operation(
        ...
        ***REMOVED*** In production, write to dedicated audit log file
        ***REMOVED*** For now, just log to standard logger
...
***REMOVED*** GitHub Actions API Integration
...
class GitHubActionsClient:
    ...
    def __init__(self):
        ...
        if not self.repo:
            ...
            ***REMOVED*** Try to extract from git remote
            self.repo = self._get_repo_from_git()

    def _get_repo_from_git(self) -> str:
        """Extract repository from git remote."""
        ***REMOVED*** This is a placeholder - would need to actually parse git config
    ...
    async def trigger_workflow(
        ...
        ***REMOVED*** GitHub returns 204 No Content on success
        ***REMOVED*** Need to fetch the latest run to get details
    ...
    async def get_workflow_run(self, run_id: str) -> dict[str, Any]:
        ...
        """
        if not self.token:
            ***REMOVED*** Return placeholder data
            return {
                "id": run_id,
                "status": "in_progress",
                "conclusion": None,
    ...
    async def get_workflow_logs(self, run_id: str) -> list[str]:
        ...
        Returns:
            List of log lines
        """
        ***REMOVED*** Note: GitHub API returns logs as a zip file
        ***REMOVED*** This is a simplified implementation
        ...
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
            response = await client.get(url, headers=headers)
            response.raise_for_status()
            ***REMOVED*** In production, would need to extract and parse zip file
            return response.text.split("\n")[:100]  ***REMOVED*** Return first 100 lines
...
class PermissionChecker:
    ...
    def check_permission(
        ...
        """
        ***REMOVED*** Production operations require admin token
        ...
        ***REMOVED*** Staging operations allowed with any valid token
...
***REMOVED*** Deployment State Management
...
class DeploymentStateStore:
    ...
    def create_deployment(
        ...
        ***REMOVED*** Keep only recent deployments
    ...
    def list_deployments(
        ...
        ***REMOVED*** Iterate in reverse order (most recent first)
...
async def validate_deployment(
    ...
    ***REMOVED*** Check 1: Git ref exists
    checks.append(
        DeploymentCheck(
    ...
    ***REMOVED*** Check 2: Tests passed
    if not request.skip_tests:
        ***REMOVED*** In production, would query GitHub Actions for test results
    ...
    ***REMOVED*** Check 3: Security scan
    if not request.skip_security_scan:
        ***REMOVED*** Would integrate with actual security scanning tools
    ...
    ***REMOVED*** Check 4: Database migrations
    checks.append(
        DeploymentCheck(
    ...
    ***REMOVED*** Check 5: Environment configuration
    ...
    if request.environment == Environment.PRODUCTION:
        ***REMOVED*** Extra checks for production
        config_details = {
            "secret_key_set": True,
    ...
    ***REMOVED*** Check 6: Docker images built
    checks.append(
        DeploymentCheck(
    ...
    ***REMOVED*** Determine overall validity
...
async def run_security_scan(
    ...
    ***REMOVED*** Simulate dependency scan
    if request.scan_dependencies:
        ***REMOVED*** In production, would run: npm audit, pip-audit, safety, etc.
    ...
    ***REMOVED*** Simulate SAST scan
    if request.scan_code:
        ***REMOVED*** In production, would run: bandit, semgrep, CodeQL, etc.
        pass  ***REMOVED*** No additional vulnerabilities found

    ***REMOVED*** Simulate secret detection
    if request.scan_secrets:
        ***REMOVED*** In production, would run: gitleaks, truffleHog, detect-secrets
        pass  ***REMOVED*** No secrets detected

    ***REMOVED*** Calculate severity summary
    severity_summary = {
        severity: sum(1 for v in vulnerabilities if v.severity == severity)
        for severity in VulnerabilitySeverity
    }

    ***REMOVED*** Determine if scan passed
...
async def run_smoke_tests(
    ...
    ***REMOVED*** Get environment URL
    ...
    if request.dry_run:
        ***REMOVED*** Simulate successful tests
        results = [
            DeploymentCheck(
        ...
        ***REMOVED*** Actually run smoke tests
        async with httpx.AsyncClient(timeout=request.timeout_seconds) as client:
            ***REMOVED*** Test 1: Health check
            try:
            ...
            ***REMOVED*** Additional tests for full suite
            if request.test_suite == TestSuite.FULL:
                ***REMOVED*** Test 2: Database check
                try:
...
async def promote_to_production(
    ...
    ***REMOVED*** Validate approval token
    ...
    ***REMOVED*** Verify staging smoke tests passed
    ...
    ***REMOVED*** Trigger GitHub Actions workflow
    if not request.dry_run:
        workflow_inputs = {
            "environment": "production",
            "skip_tests": "false",  ***REMOVED*** Always run tests for production
        }
    ...
    ***REMOVED*** Update deployment status
...
async def rollback_deployment(
    ...
    ***REMOVED*** Get current and previous deployments
    ...
    ***REMOVED*** Trigger rollback via GitHub Actions
    if not request.dry_run:
        workflow_inputs = {
            "environment": request.environment.value,
            "skip_tests": "true",  ***REMOVED*** Assume previous version was tested
        }
...
async def get_deployment_status(
    ...
    ***REMOVED*** Fetch checks from GitHub Actions (if available)
    checks: list[DeploymentCheck] = [
        DeploymentCheck(
    ...
    ***REMOVED*** Fetch logs (would come from GitHub Actions in production)
    logs = [
    ...
    ***REMOVED*** Health checks
    ...
    return DeploymentStatusResult(
        ...
        checks=checks,
        logs=logs[-20:],  ***REMOVED*** Last 20 log lines
