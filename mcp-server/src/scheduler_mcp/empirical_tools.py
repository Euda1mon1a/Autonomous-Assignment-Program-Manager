...
***REMOVED*** Get project root (mcp-server/src/scheduler_mcp -> project root)
...
class AblationResult(BaseModel):
    ...
    removal_impact: str = "unknown"  ***REMOVED*** none, minor, major, breaking
    recommendation: str = ""
...
class ResilienceModuleStats(BaseModel):
    ...
    tier: str = ""  ***REMOVED*** tier1, tier2, tier3
    description: str = ""
...
async def benchmark_solvers(
    ...
    ***REMOVED*** Initialize metrics for each solver
    for solver in solvers:
        result.results_by_solver[solver] = SolverMetrics(solver_name=solver)

    ***REMOVED*** Check if pytest is available and tests exist
    ...
    ***REMOVED*** Run solver benchmarks via pytest
    for solver in solvers:
        logger.info(f"Benchmarking solver: {solver}")
        start_time = time.time()

        ***REMOVED*** Run tests for this solver
        cmd = [
            "python", "-m", "pytest",
        ...
        if returncode == 0:
            ...
            ***REMOVED*** Parse failures from output
            if "failed" in stdout.lower():
                try:
                    ***REMOVED*** Extract failure count from pytest output
                    import re
    ...
    ***REMOVED*** Determine winners by metric
    if result.results_by_solver:
        ***REMOVED*** Best runtime (lowest)
        best_runtime = min(
        ...
        ***REMOVED*** Best success rate (highest)
        best_success = max(
            result.results_by_solver.items(),
...
async def benchmark_constraints(
    ...
    ***REMOVED*** Catalog constraints
    ...
    for cf in constraint_files:
        ...
        ***REMOVED*** Check if constraint has tests
        test_file = BACKEND_ROOT / "tests" / f"test_{name}.py"
        if test_file.exists():
            ***REMOVED*** Run constraint tests to check effectiveness
            cmd = [
                "python", "-m", "pytest",
                str(test_file),
                "--tb=no", "-q",
            ...
            ***REMOVED*** No tests - unknown yield
...
async def ablation_study(
    ...
    ***REMOVED*** Find what imports this module
    ...
    ***REMOVED*** Find tests that mention this module
...
async def benchmark_resilience(
    ...
    ***REMOVED*** Define known resilience modules with tiers
    known_modules = {
        ***REMOVED*** Tier 1 - Core
        ...
        ***REMOVED*** Find module file
        ...
        if module_file and module_file.exists():
            ...
            ***REMOVED*** Check for tests
            ...
            if has_tests:
                stats.detection_rate = 0.8  ***REMOVED*** Assume tested = functional
                stats.false_alarm_rate = 0.1
            else:
                stats.detection_rate = 0.5  ***REMOVED*** Unknown
                stats.false_alarm_rate = 0.3

        result.module_stats[module_name] = stats

    ***REMOVED*** Categorize by value
...
async def module_usage_analysis(
    ...
    ***REMOVED*** Get all Python modules
    ...
    ***REMOVED*** Trace imports from entry points
    reachable = set()
    to_process = []

    ***REMOVED*** Find entry point files
    ...
    ***REMOVED*** Identify hot/cold paths based on import count
