...
***REMOVED******REMOVED******REMOVED*** Utility Function
...
***REMOVED******REMOVED******REMOVED*** Coordination Failures
...
***REMOVED******REMOVED******REMOVED*** 1. `analyze_nash_stability`
...
```python
{
    "stability_status": "UNSTABLE",  ***REMOVED*** STABLE, UNSTABLE, WEAKLY_STABLE
...
***REMOVED******REMOVED******REMOVED*** 2. `find_deviation_incentives`
...
***REMOVED******REMOVED******REMOVED*** 3. `detect_coordination_failures`
...
***REMOVED******REMOVED******REMOVED*** Complements Shapley Fairness
...
***REMOVED******REMOVED*** Game Theory Insights

***REMOVED******REMOVED******REMOVED*** Nash Equilibrium Types
...
***REMOVED******REMOVED*** Example Use Cases

***REMOVED******REMOVED******REMOVED*** Use Case 1: Monthly Schedule Health Check
```bash
***REMOVED*** Check if January schedule is Nash stable
...
if not result.is_nash_equilibrium:
    ...
    ***REMOVED*** Address top deviations
    for dev in result.deviations[:5]:
        print(f"  - {dev.person_id}: {dev.description} (+{dev.utility_gain:.2f})")
```

***REMOVED******REMOVED******REMOVED*** Use Case 2: Investigate Specific Complaint
```bash
***REMOVED*** Dr. Smith is unhappy - why?
...
***REMOVED******REMOVED******REMOVED*** Use Case 3: Justify System Improvements
```bash
***REMOVED*** Find coordination failures to justify multi-way swap feature
...
***REMOVED******REMOVED******REMOVED*** Utility Calculation Details

**Workload Fairness (Gini Coefficient):**
```python
***REMOVED*** Perfect equality: Gini = 0 → Fairness = 1.0
***REMOVED*** Maximum inequality: Gini = 1 → Fairness = 0.0
fairness = 1.0 - gini_coefficient(workload_hours)
```

**Preference Satisfaction (Stigmergy):**
```python
***REMOVED*** Uses PreferenceTrailRecord data
***REMOVED*** Normalized trail strength [0, 1] (max strength = 10.0)
preference = average(trail_strengths) / 10.0
```

**Convenience (Clustering):**
```python
***REMOVED*** Placeholder - production would analyze:
***REMOVED*** - Temporal clustering (consecutive days)
***REMOVED*** - Commute implications
***REMOVED*** - Family obligations
***REMOVED*** - Circadian rhythm
convenience = 0.6  ***REMOVED*** Moderate default
```

**Continuity (Stability):**
```python
***REMOVED*** Overlap with previous period
overlap = len(current_ids & previous_ids)
continuity = overlap / len(previous_ids)
```

***REMOVED******REMOVED******REMOVED*** Custom Utility Weights
```python
***REMOVED*** Default: (0.4, 0.3, 0.2, 0.1)
result = await analyze_nash_stability_tool(
    start_date="2025-01-01",
    end_date="2025-03-31",
    utility_weights={
        "workload": 0.5,      ***REMOVED*** Emphasize workload fairness
        "preference": 0.3,
        "convenience": 0.1,
        "continuity": 0.1
    }
...
***REMOVED******REMOVED******REMOVED*** Related Tools
