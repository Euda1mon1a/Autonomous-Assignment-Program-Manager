***REMOVED*** VaR (Value-at-Risk) Analysis for Schedule Vulnerability
...
***REMOVED******REMOVED*** What is Value-at-Risk (VaR)?
...
***REMOVED******REMOVED******REMOVED*** VaR vs CVaR (Conditional VaR)
...
***REMOVED******REMOVED******REMOVED*** 1. `calculate_coverage_var_tool`
...
result = await calculate_coverage_var_tool(
    ...
    rotation_types=None,  ***REMOVED*** All rotations
    historical_days=90
)
...
***REMOVED*** "With 90% confidence, coverage won't drop more than 8.2%"
***REMOVED*** "With 95% confidence, coverage won't drop more than 12.1%"
***REMOVED*** "With 99% confidence, coverage won't drop more than 18.7%"
```
...
***REMOVED******REMOVED******REMOVED*** 2. `calculate_workload_var_tool`
...
***REMOVED******REMOVED******REMOVED*** 3. `simulate_disruption_scenarios_tool`
...
result = await simulate_disruption_scenarios_tool(
    ...
    disruption_probability=0.05,  ***REMOVED*** 5% chance per person per day
    seed=42  ***REMOVED*** For reproducibility
)
...
***REMOVED******REMOVED******REMOVED*** 4. `calculate_conditional_var_tool`
...
***REMOVED*** Check tail risk premium
...
***REMOVED******REMOVED******REMOVED*** 1. Strategic Planning

**Question**: "Should we hire an additional faculty member?"

```python
***REMOVED*** Baseline risk
...
***REMOVED*** Simulate with +1 faculty (adjust disruption probability)
with_buffer = await simulate_disruption_scenarios_tool(
    start_date="2025-01-01",
    end_date="2025-12-31",
    num_simulations=1000,
    disruption_probability=0.04  ***REMOVED*** Lower due to buffer
)
...
***REMOVED******REMOVED******REMOVED*** 2. Deployment Preparation
...
result = await simulate_disruption_scenarios_tool(
    ...
    end_date="2025-05-31",  ***REMOVED*** 4-month deployment
    disruption_types=["deployment"],
    disruption_probability=0.10,  ***REMOVED*** 10% (2 out of 20)
    num_simulations=500
)
...
***REMOVED******REMOVED******REMOVED*** 3. Flu Season Preparedness

**Question**: "How vulnerable are we to illness outbreak?"

```python
result = await simulate_disruption_scenarios_tool(
    start_date="2025-12-01",  ***REMOVED*** Flu season
    end_date="2026-02-28",
    disruption_types=["illness_cluster"],
    disruption_probability=0.08,  ***REMOVED*** 8% (cluster effect)
    num_simulations=1000
)

***REMOVED*** Check CVaR for tail risk
...
***REMOVED******REMOVED******REMOVED*** 4. Workload Equity Monitoring
...
***REMOVED******REMOVED*** Integration with Existing Tools
...
***REMOVED******REMOVED******REMOVED*** Workflow Example

```python
***REMOVED*** 1. Check deterministic contingency
n1_result = await check_n1_contingency()

***REMOVED*** 2. If contingency passes, quantify probabilistic risk
...
***REMOVED*** 3. If VaR is acceptable, check tail risk
cvar_result = await calculate_conditional_var_tool(
    start_date="2025-01-01",
    end_date="2025-12-31",
    confidence_level=0.95
...
***REMOVED*** 4. If tail risk is high, run detailed simulation
...
***REMOVED******REMOVED******REMOVED*** Risk Severity Levels
...
***REMOVED******REMOVED******REMOVED*** VaR Confidence Levels
...
***REMOVED******REMOVED******REMOVED*** CVaR Interpretation
...
***REMOVED******REMOVED******REMOVED*** Financial Risk Management
...
***REMOVED******REMOVED******REMOVED*** Statistical Methods
