***REMOVED*** Deployment Workflow Automation Tools
...
***REMOVED*** Production (use secrets management)
***REMOVED*** Store in GitHub Secrets, AWS Secrets Manager, or similar
```

---

***REMOVED******REMOVED*** GitHub Actions Integration

The tools integrate with the existing CD workflow at `.github/workflows/cd.yml`.

***REMOVED******REMOVED******REMOVED*** Workflow Dispatch

Tools trigger workflows using the GitHub Actions API:

```python
***REMOVED*** Internally calls:
...
***REMOVED******REMOVED******REMOVED*** 3. **Token Hashing**
...
result = await validate_deployment_tool(
    environment="production",
    git_ref="v1.2.3",
    dry_run=True  ***REMOVED*** Simulate without side effects
)
```
...
***REMOVED******REMOVED******REMOVED*** Standard Deployment Flow

```python
***REMOVED*** 1. Validate deployment
...
***REMOVED*** 3. Deploy to staging (handled by GitHub Actions)
...
***REMOVED******REMOVED******REMOVED*** Emergency Rollback

```python
***REMOVED*** List recent deployments
...
try:
    result = await validate_deployment_tool(
        environment="invalid",  ***REMOVED*** Not "staging" or "production"
        git_ref="main"
    )
except ValueError as e:
    print(f"Error: {e}")
    ***REMOVED*** Output: Invalid environment: invalid. Must be 'staging' or 'production'
```
...
try:
    ...
    print(f"Error: {e}")
    ***REMOVED*** Output: Invalid approval token for production deployment
```
...
```bash
***REMOVED*** With actual GitHub Actions
export GITHUB_TOKEN="ghp_..."
...
***REMOVED******REMOVED******REMOVED*** Dry-Run Testing

```python
***REMOVED*** Test without triggering actual deployments
result = await validate_deployment_tool(
    environment="production",
    git_ref="main",
    dry_run=True
...
***REMOVED******REMOVED******REMOVED*** Issue: "GITHUB_TOKEN not set"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Invalid approval token for production"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Smoke tests failed"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Deployment stuck in 'in_progress'"
...
3. **Test in staging first:**
   ...
   ***REMOVED*** Only then promote to production
   ```
