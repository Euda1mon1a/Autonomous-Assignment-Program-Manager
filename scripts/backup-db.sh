...
***REMOVED*** Database Backup Script for Residency Scheduler
***REMOVED*** Creates compressed PostgreSQL backups with rotation
***REMOVED***
***REMOVED*** Features:
***REMOVED*** - Automated backups with timestamp
***REMOVED*** - Compression (gzip)
***REMOVED*** - Automatic cleanup of old backups
***REMOVED*** - Optional S3 upload
***REMOVED*** - Email notifications (optional)
***REMOVED***
***REMOVED*** Usage:
***REMOVED***   ./scripts/backup-db.sh [options]
***REMOVED***
***REMOVED*** Options:
***REMOVED***   --docker         Use Docker container for backup
***REMOVED***   --retention N    Keep last N backups (default: 30)
***REMOVED***   --s3             Upload to S3 (requires AWS_* env vars)
***REMOVED***   --email EMAIL    Send notification email on completion
...
***REMOVED*** S3 configuration (optional)
S3_BUCKET="${S3_BUCKET:-}"
...
***REMOVED*** Perform database backup
backup_database() {
    ...
    ***REMOVED*** Check if backup file is not empty
...
capture_version_metadata() {
    ...
    ***REMOVED*** Write metadata file
    cat > "$METADATA_FILE" << EOF
***REMOVED*** Residency Scheduler Backup Metadata
***REMOVED*** Generated: $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
handle_error() {
    ...
    ***REMOVED*** Clean up incomplete backup
...
main() {
    ...
    ***REMOVED*** Optional steps (don't fail on error)
    upload_to_s3 || true
