...
***REMOVED*** Type checking imports (not loaded at runtime)
if TYPE_CHECKING:
...
***REMOVED*** =============================================================================
***REMOVED*** Absence Type Mapping (reverse of xlsx_export.py)
***REMOVED*** =============================================================================

ABBREVIATION_TO_ABSENCE_TYPE = {
    ***REMOVED*** Standard types from xlsx_export.py
    "VAC": "vacation",
    "SICK": "sick",
    "MED": "medical",
    "CONF": "conference",
    ...
    "PER": "personal",
    ***REMOVED*** Additional types from absence.py
    "BER": "bereavement",
    "BERV": "bereavement",
    ...
    ***REMOVED*** Common variations
    ...
    "T": "tdy",
    "P": "personal",
    "ABS": "vacation",  ***REMOVED*** Generic absence defaults to vacation
}

***REMOVED*** Valid absence types (from app/models/absence.py)
VALID_ABSENCE_TYPES = {
...
***REMOVED*** =============================================================================
***REMOVED*** Name Normalization and Matching
***REMOVED*** =============================================================================
...
def normalize_name(name: str) -> str:
    ...
    ***REMOVED*** Handle "Last, First" format - convert to "First Last"
    if ", " in name:
        parts = name.split(", ", 1)
        if len(parts) == 2:
            name = f"{parts[1]} {parts[0]}"

    ***REMOVED*** Normalize multiple spaces
...
def parse_date(value: Any) -> date | None:
    ...
    ***REMOVED*** Already a date
    if isinstance(value, date) and not isinstance(value, datetime):
        return value

    ***REMOVED*** datetime object
    if isinstance(value, datetime):
        return value.date()

    ***REMOVED*** Excel serial number (int or float)
    if isinstance(value, (int, float)):
        try:
            ***REMOVED*** Excel dates are days since 1899-12-30 (with a bug for 1900 leap year)
    ...
    ***REMOVED*** String parsing
    if isinstance(value, str):
        ...
        ***REMOVED*** Try various date formats
        formats = [
            "%Y-%m-%d",      ***REMOVED*** ISO format: 2024-01-15
            "%m/%d/%Y",      ***REMOVED*** US format: 01/15/2024
            "%m/%d/%y",      ***REMOVED*** Short year: 01/15/24
            "%d-%b-%Y",      ***REMOVED*** Day-Month-Year: 15-Jan-2024
            "%d-%B-%Y",      ***REMOVED*** Full month: 15-January-2024
            "%b %d, %Y",     ***REMOVED*** Month Day, Year: Jan 15, 2024
            "%B %d, %Y",     ***REMOVED*** Full month: January 15, 2024
            "%d/%m/%Y",      ***REMOVED*** European format: 15/01/2024
            "%Y/%m/%d",      ***REMOVED*** Alternative ISO: 2024/01/15
            "%d.%m.%Y",      ***REMOVED*** German format: 15.01.2024
        ]
...
***REMOVED*** =============================================================================
***REMOVED*** Excel Sheet Processing
***REMOVED*** =============================================================================
...
def detect_sheet_type(ws: Worksheet) -> str:
    ...
    """
    ***REMOVED*** Check first few rows for header patterns
    headers = []
...
def find_header_row(ws: Worksheet, keywords: list[str]) -> int | None:
    ...
    """
    for row in range(1, 20):  ***REMOVED*** Check first 20 rows
        row_text = ""
...
def import_absences_from_sheet(
    ...
    ***REMOVED*** Validate required columns
    ...
    ***REMOVED*** Process data rows
    for row in range(header_row + 1, ws.max_row + 1):
        ***REMOVED*** Get person name
        person_name = ws.cell(row=row, column=col_map["person"]).value
        if not person_name:
            continue  ***REMOVED*** Skip empty rows
        ...
        ***REMOVED*** Get absence type
        ...
        if not absence_type:
            ***REMOVED*** Try using the raw value if it's a valid type
...
def import_schedule_from_sheet(
    ...
    ***REMOVED*** Get rotation templates cache
    ...
    ***REMOVED*** Find the date header row (usually row 3)
    date_row = 3

    ***REMOVED*** Find person column (usually E)
    person_col = 5  ***REMOVED*** Column E

    ***REMOVED*** Build date mapping from headers
    date_columns = {}  ***REMOVED*** {(date, time_of_day): column}

    for col in range(6, ws.max_column + 1):
        ***REMOVED*** Row 3 should have dates
        date_cell = ws.cell(row=date_row, column=col).value
        ***REMOVED*** Row 2 should have AM/PM
    ...
    ***REMOVED*** Collect absence cells keyed by person/date so we can create Absence rows
    absence_days: dict[UUID, dict[date, str]] = {}
    person_lookup: dict[UUID, "PersonType"] = {}

    ***REMOVED*** Process person rows (usually starting at row 9)
    for row in range(9, ws.max_row + 1):
        ...
        if not person:
            ***REMOVED*** Skip gaps and empty names
        ...
        ***REMOVED*** Process each date column
        for col, (col_date, time_of_day) in date_columns.items():
            ...
            ***REMOVED*** Skip working ("W") and empty cells
            if value in ("W", ""):
                continue

            ***REMOVED*** Check if it's an absence abbreviation
            ...
            ***REMOVED*** Look up rotation template
            ...
            ***REMOVED*** Check for existing assignment
            existing = session.execute(
            ...
            ***REMOVED*** Create assignment
            if not dry_run:
                assignment = Assignment(
                    ...
                    role="primary",  ***REMOVED*** Default role
    ...
    ***REMOVED*** Create absence records for any absence codes found in the schedule grid
    for person_id, date_map in absence_days.items():
        for absence_type in set(date_map.values()):
            ***REMOVED*** Group contiguous dates of the same absence type
            dates = sorted(
...
def import_excel(
    ...
    ***REMOVED*** Build persons cache once for efficiency
...
def main():
    """CLI entry point."""
    ***REMOVED*** Lazy imports - only load when actually running
    ***REMOVED*** Add backend to path for imports
...
Absence type abbreviations:
  ...
  DEP=deployment, TDY=tdy, FEM=family_emergency, PER=personal
    ...
    ***REMOVED*** Get database URL
    settings = get_settings()
    database_url = args.database_url or str(settings.DATABASE_URL)

    ***REMOVED*** For async databases, convert to sync URL
    if database_url.startswith("postgresql+asyncpg://"):
        database_url = database_url.replace("postgresql+asyncpg://", "postgresql://")

    logger.info(f"Connecting to database...")

    ***REMOVED*** Create engine and session
        ...
        ***REMOVED*** Print summary
        ...
        for sheet_name, result in results.items():
            ...
            if result.error_messages and args.verbose:
                for msg in result.error_messages[:10]:  ***REMOVED*** Show first 10 errors
                    print(f"    - {msg}")
    ...
    ***REMOVED*** Exit with error code if there were errors
    if total_errors > 0:
        sys.exit(1)
