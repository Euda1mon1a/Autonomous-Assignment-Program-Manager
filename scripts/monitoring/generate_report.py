...
def generate_markdown_report(metrics: dict[str, Any]) -> str:
    ...
    lines.append("***REMOVED*** Performance Report")
    lines.append(f"*Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
    lines.append("")

    ***REMOVED*** System metrics
    if "system" in metrics:
        lines.append("***REMOVED******REMOVED*** System Metrics")
        lines.append("")
        sys = metrics["system"]
        lines.append(f"- **CPU Usage**: {sys.get('cpu_percent', 0):.1f}%")
        lines.append(f"- **Memory Usage**: {sys.get('memory_percent', 0):.1f}%")
    ...
    ***REMOVED*** Database metrics
    if "database" in metrics and "error" not in metrics["database"]:
        lines.append("***REMOVED******REMOVED*** Database Metrics")
        lines.append("")
        db = metrics["database"]
        lines.append(f"- **Database Size**: {db.get('database_size_mb', 0):.2f} MB")
        lines.append(f"- **Active Connections**: {db.get('active_connections', 0)}")
    ...
    ***REMOVED*** Benchmark results
    if "benchmarks" in metrics and "benchmarks" in metrics["benchmarks"]:
        lines.append("***REMOVED******REMOVED*** Benchmark Results")
        lines.append("")
...
<head>
    ...
    <style>
        ...
        h2 {{ color: ***REMOVED***666; margin-top: 30px; }}
        table {{ border-collapse: collapse; width: 100%; margin-top: 20px; }}
        th, td {{ padding: 12px; text-align: left; border-bottom: 1px solid ***REMOVED***ddd; }}
...
def main():
    ...
    ***REMOVED*** If metrics is a list, use the latest
