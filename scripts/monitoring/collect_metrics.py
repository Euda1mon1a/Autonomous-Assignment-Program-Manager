...
Usage:
    ...
    python collect_metrics.py --continuous --interval 300  ***REMOVED*** Every 5 minutes
"""
...
def collect_benchmark_metrics(benchmark_dir: Path) -> dict[str, Any]:
    ...
    ***REMOVED*** Find most recent benchmark results
    json_files = list(benchmark_dir.glob("*.json"))

    if not json_files:
        return {"error": "No benchmark results found"}

    ***REMOVED*** Group by benchmark name and get latest for each
    benchmarks = {}
    for file in json_files:
        try:
            with open(file) as f:
                ...
                ***REMOVED*** Keep only the latest for each benchmark
...
def save_metrics(metrics: dict[str, Any], output_file: Path):
    ...
    ***REMOVED*** If file exists, append to array
