...
Usage:
    ...
    python alert_on_regression.py --ci  ***REMOVED*** Exit with code 1 on regression
"""
...
class RegressionDetector:
    ...
    def check_benchmark(self, baseline: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Check memory regression if available
...
def send_slack_alert(webhook_url: str, detector: RegressionDetector):
    ...
    try:
        ...
        for regression in detector.regressions[:5]:  ***REMOVED*** Limit to 5 for message size
...
def check_directory(directory: Path, threshold: float) -> RegressionDetector:
    ...
    ***REMOVED*** Check each benchmark (compare latest with previous)
    for benchmark_name, runs in by_benchmark.items():
        if len(runs) < 2:
            continue

        ***REMOVED*** Sort by modification time
