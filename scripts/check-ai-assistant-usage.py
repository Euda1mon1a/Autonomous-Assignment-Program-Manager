...
Usage:
    python scripts/check-ai-assistant-usage.py          ***REMOVED*** Basic check
    python scripts/check-ai-assistant-usage.py --strict ***REMOVED*** Exit non-zero on issues
    python scripts/check-ai-assistant-usage.py --commits 50  ***REMOVED*** Check more history
    python scripts/check-ai-assistant-usage.py --quiet  ***REMOVED*** Only show warnings
"""
import argparse
import re
import subprocess
...
***REMOVED*** AI assistant detection patterns
AI_PATTERNS = {
    "claude": [
        r"\(claude\):",  ***REMOVED*** feat(claude):, fix(claude):
        r"claude code",
        r"generated by claude",
        r"co-authored-by:.*claude",
        r"\.claude$",  ***REMOVED*** Branch suffix
    ],
    "codex": [
        r"\(codex\):",  ***REMOVED*** feat(codex):, fix(codex):
        r"codex",
        r"co-authored-by:.*codex",
        r"\.codex$",  ***REMOVED*** Branch suffix
    ],
    "copilot": [
        r"\(copilot\):",
        r"github copilot",
...
***REMOVED*** Human consolidation patterns (these "reset" the AI tracking)
HUMAN_PATTERNS = [
    r"^consolidate:",
    r"\(human\):",
    r"^merge:",
...
def get_recent_commits(count: int) -> list[Commit]:
    ...
    ***REMOVED*** Format: hash|author|subject
...
def detect_ai_in_commit(commit: Commit) -> Optional[str]:
    ...
    ***REMOVED*** Check for human consolidation first
    for pattern in HUMAN_PATTERNS:
        if re.search(pattern, text, re.IGNORECASE):
            commit.is_human_consolidation = True
            return None

    ***REMOVED*** Check for AI patterns
...
def analyze_commits(commits: list[Commit]) -> dict:
    ...
    ***REMOVED*** Detect AI in each commit
    for commit in commits:
        detect_ai_in_commit(commit)

    ***REMOVED*** Find sequences of AI commits
    ai_sequences = []
    current_sequence = []
    last_ai = None
    ...
    for commit in commits:
        if commit.is_human_consolidation:
            ***REMOVED*** Human consolidation resets the sequence
            if current_sequence:
            ...
            if last_ai and last_ai != commit.detected_ai:
                ***REMOVED*** Different AI detected - this is a mixing point
    ...
    ***REMOVED*** Check for interleaving
    interleaved = []
    prev_ai = None
    for commit in commits:
        if commit.is_human_consolidation:
...
For more information, see:
  docs/development/ai-assistant-guardrails.md
    ...
    ***REMOVED*** Check if we're in a git repo
    code, _, _ = run_git_command(["rev-parse", "--is-inside-work-tree"])
    if code != 0:
        print("Error: Not inside a Git repository")
        return 1

    ***REMOVED*** Get current branch
    ...
    ***REMOVED*** Get and analyze commits
