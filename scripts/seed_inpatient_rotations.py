...
***REMOVED*** Inpatient rotation keywords
INPATIENT_KEYWORDS = [
    "FMIT",
    "Night Float",
    "Inpatient",
...
def load_json(filename: str) -> dict:
    ...
    ***REMOVED*** Try both sanitized and de-sanitized versions
    ...
    ***REMOVED*** Also check docs/data for backward compatibility
...
def get_inpatient_template(templates: dict, rotation_name: str) -> dict | None:
    ...
    ***REMOVED*** Map rotation names to template names
    if "fmit" in rotation_lower:
        ***REMOVED*** FMIT assignments use FMIT AM/PM templates
...
def clear_block_assignments(headers: dict, block_number: int, dry_run: bool) -> int:
    ...
    ***REMOVED*** This needs to be done via direct DB or a delete endpoint
    ***REMOVED*** For now, we'll document this as a manual step
...
def create_assignment(
    ...
    if resp.status_code in [200, 201]:
        ...
        ***REMOVED*** Duplicate - already exists
        return True
    else:
...
def seed_resident_inpatient(
    ...
    for rec in data["records"]:
        ...
        ***REMOVED*** Filter to target block
        ...
        ***REMOVED*** Only process inpatient rotations
        if not is_inpatient_rotation(rotation_name):
            continue

        ***REMOVED*** Find person in database
        ...
        ***REMOVED*** Create assignment for each half-day block
        for block in blocks:
            ***REMOVED*** Use AM or PM template based on time of day
            tod = block["time_of_day"]
            template_name = template["name"]

            ***REMOVED*** Swap AM/PM if needed
...
def seed_faculty_fmit(
    ...
    for rec in data["records"]:
        ...
        ***REMOVED*** Extract faculty name (last part after " - ")
        ...
        ***REMOVED*** Get blocks for this week
        resp = requests.get(
            f"{BASE_URL}/api/v1/blocks",
            params={
