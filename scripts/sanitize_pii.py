...
Usage:
  ...
  python scripts/sanitize_pii.py --db-export          ***REMOVED*** Sanitize from live DB
  python scripts/sanitize_pii.py --reverse mapping.json sanitized.json  ***REMOVED*** Restore
...
***REMOVED*** Ensure this directory is gitignored
MAPPING_DIR = Path(__file__).parent.parent / ".sanitize"
MAPPING_DIR.mkdir(exist_ok=True)

***REMOVED*** Fields to completely remove (OPSEC/PERSEC sensitive)
REDACT_FIELDS = {
    "deployment_orders",
    "deployment_location",
    "tdy_location",
...
***REMOVED*** Fields containing names to replace
NAME_FIELDS = {
    "first_name",
    "last_name",
    "full_name",
...
***REMOVED*** Fields containing emails to sanitize
EMAIL_FIELDS = {
    "email",
    "work_email",
    "contact_email",
...
class PIISanitizer:
    ...
    def __init__(self, seed: int = None):
        ...
        ***REMOVED*** Mapping storage
        self.name_map: dict[str, str] = {}
        self.email_map: dict[str, str] = {}
        self.date_offset: int = random.randint(-30, 30)  ***REMOVED*** Days to shift dates

        ***REMOVED*** Counters for synthetic IDs
    ...
    def _sanitize_string(self, s: str) -> str:
        ...
        ***REMOVED*** Check for SSN patterns
        s = re.sub(r'\b\d{3}-\d{2}-\d{4}\b', 'XXX-XX-XXXX', s)

        ***REMOVED*** Check for phone patterns
        s = re.sub(r'\b\d{3}[-.]?\d{3}[-.]?\d{4}\b', 'XXX-XXX-XXXX', s)

        ***REMOVED*** Check for .mil emails
        s = re.sub(
    ...
    def _sanitize_name(self, name: str, context: dict = None) -> str:
        ...
        ***REMOVED*** Check if we've seen this name before
        if name in self.name_map:
            return self.name_map[name]

        ***REMOVED*** Determine role from context
        ...
        ***REMOVED*** Generate synthetic ID based on role
    ...
    def _sanitize_email(self, email: str) -> str:
        ...
        ***REMOVED*** Generate synthetic email
    ...
    def _sanitize_date(self, date_value: Any) -> Any:
        ...
        try:
            if isinstance(date_value, str):
                ***REMOVED*** Try common date formats
    ...
    ***REMOVED*** Reverse the name and email maps
    ...
    def reverse_recurse(data, parent_key=None):
        if isinstance(data, dict):
            ...
            ***REMOVED*** Check if it's a synthetic name or email
