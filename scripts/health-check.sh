...
***REMOVED*** Health Check Script for Residency Scheduler
***REMOVED*** Verifies all services are running and healthy
***REMOVED***
***REMOVED*** Exit codes:
***REMOVED***   0 - All services healthy
***REMOVED***   1 - One or more services unhealthy
***REMOVED***   2 - Critical service failure
***REMOVED***
***REMOVED*** Usage:
***REMOVED***   ./scripts/health-check.sh [--verbose] [--docker]
***REMOVED***
***REMOVED*** Options:
***REMOVED***   --verbose    Show detailed output
***REMOVED***   --docker     Check Docker containers instead of local services
...
check_backend() {
    ...
    if command -v curl &> /dev/null; then
        ...
        if [ "$HTTP_CODE" = "200" ]; then
            ...
            ***REMOVED*** Check resilience endpoint if verbose
            if [ "$VERBOSE" = true ]; then
...
check_celery_workers() {
    ...
    if [ "$DOCKER_MODE" = true ]; then
        ...
        ***REMOVED*** Check via Celery inspect
...
check_celery_beat() {
    ...
    if [ "$DOCKER_MODE" = true ]; then
        ...
        ***REMOVED*** Check if beat process is running
...
***REMOVED*** Check Docker containers (if in Docker mode)
