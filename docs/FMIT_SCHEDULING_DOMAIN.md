***REMOVED*** FMIT Faculty Scheduling Domain Model
...
***REMOVED******REMOVED*** Hard Constraints (Must Not Violate)

***REMOVED******REMOVED******REMOVED*** 1. No Back-to-Back FMIT Weeks
...
***REMOVED******REMOVED******REMOVED*** 2. Call Cascade Rules
...
class FMITWeekAssignment:
    faculty: str
    week_start: date
    fri_pm_call: bool = True   ***REMOVED*** Always true for FMIT week
    sat_pm_call: bool = True   ***REMOVED*** Always true for FMIT week
    sun_thurs_available: bool = False  ***REMOVED*** Always false during FMIT
```

***REMOVED******REMOVED******REMOVED*** 3. PCAT/DO Cascade
...
***REMOVED******REMOVED******REMOVED*** 4. Specialty Provider Bottlenecks
...
specialty_providers: Dict[str, List[str]] = {
    ...
    "Procedures": ["Faculty_P1", "Faculty_P2"],  ***REMOVED*** Multiple = no bottleneck
}
...
***REMOVED******REMOVED*** Soft Constraints (Should Minimize)

***REMOVED******REMOVED******REMOVED*** 5. Target Week Distribution
...
class FacultyTarget:
    name: str
    target_weeks: int = 6
    role: str = "faculty"  ***REMOVED*** "chief", "pd", "adjunct"
...
***REMOVED******REMOVED******REMOVED*** 6. Alternating Week Pattern Limits

**Rule**: Week-on/week-off alternating is required (see Hard Constraint ***REMOVED***1), but too many consecutive alternating cycles creates family hardship.
...
def count_alternating_cycles(weeks: List[date]) -> int:
    ...
    for i in range(len(sorted_weeks) - 1):
        ...
        if 7 < gap <= 21:  ***REMOVED*** 2-3 week gap = alternating
            cycles += 1
    return cycles
...
***REMOVED******REMOVED*** Contextual Constraints (External Data Required)

***REMOVED******REMOVED******REMOVED*** 7. Leave and Conference Conflicts
...
class ExternalConflict:
    ...
    start_date: date
    end_date: date
    type: str  ***REMOVED*** "leave", "conference", "tdy", "training"
    source: str  ***REMOVED*** Where this data came from
...
***REMOVED******REMOVED******REMOVED*** 8. Clinic Schedule Lock Dates
...
***REMOVED******REMOVED*** Swap Operations

***REMOVED******REMOVED******REMOVED*** 9. Direct Swap (Net Zero)
...
def validate_swap(
    faculty_a: str,
    faculty_b: str,
    week_a: date,  ***REMOVED*** A's current week, B will take
    week_b: date,  ***REMOVED*** B's current week, A will take
    ...
    ***REMOVED*** Check back-to-back for B taking week_a
    ...
    ***REMOVED*** Check schedule flexibility
...
***REMOVED******REMOVED******REMOVED*** 10. Coverage Swap (Absorb Week)
...
***REMOVED******REMOVED*** Recommended Libraries and Tools

***REMOVED******REMOVED******REMOVED*** Constraint Satisfaction
...
***REMOVED******REMOVED******REMOVED*** Date/Time Handling
...
***REMOVED******REMOVED******REMOVED*** Conflict Detection Enhancements

```python
***REMOVED*** Recommended additions to xlsx_import.py
...
class SwapCandidate:
    ...
    faculty: str
    can_take_week: date
    gives_week: Optional[date]  ***REMOVED*** None if absorbing, date if 1:1 swap
    back_to_back_ok: bool
    external_conflict: Optional[str]
    flexibility: str  ***REMOVED*** "easy", "hard", "very_hard"
...
class SwapRecommendation:
    ...
    target_faculty: str
    target_week: date
    reason: str  ***REMOVED*** "excessive_alternating", "departure", "conflict"
    candidates: List[SwapCandidate]
    priority: str  ***REMOVED*** "high", "medium", "low"
...
def find_swap_candidates(
    ...
    for faculty, weeks in all_schedules.items():
        ...
        ***REMOVED*** Find potential give-back weeks (for 1:1 swap)
        give_weeks = []
        for w in weeks:
            if w > schedule_release_date:  ***REMOVED*** Only flexible weeks
    ...
    ***REMOVED*** Sort by viability
    return sorted(candidates, key=lambda c: (
        not c.back_to_back_ok,  ***REMOVED*** Back-to-back issues first (worst)
        c.external_conflict is not None,  ***REMOVED*** External conflicts second
        c.gives_week is None,  ***REMOVED*** Absorb-only third (prefer 1:1)
    ))
```

***REMOVED******REMOVED*** What Remains for Human/LLM Judgment
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Automated Conflict Detection
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Swap Recommendation Engine
...
***REMOVED******REMOVED******REMOVED*** Phase 3: LLM Integration
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Full Automation
