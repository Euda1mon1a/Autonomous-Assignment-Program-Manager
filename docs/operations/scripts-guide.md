...
- [Script Categories](***REMOVED***script-categories)
- [Operations Scripts](***REMOVED***operations-scripts)
- [Development Scripts](***REMOVED***development-scripts)
- [Deployment Scripts](***REMOVED***deployment-scripts)
- [Scheduling Scripts](***REMOVED***scheduling-scripts)
- [Usage Patterns](***REMOVED***usage-patterns)
...
***REMOVED******REMOVED******REMOVED*** Script Organization

```
scripts/
├── ops/              ***REMOVED*** Operations scripts
├── dev/              ***REMOVED*** Development scripts
├── deploy/           ***REMOVED*** Deployment scripts
└── scheduling/       ***REMOVED*** Scheduling utilities
```
...
***REMOVED******REMOVED*** Operations Scripts

***REMOVED******REMOVED******REMOVED*** health_check.py
...
***REMOVED*** JSON output for monitoring systems
python scripts/ops/health_check.py --format json --output health.json

***REMOVED*** Critical checks only (faster)
...
***REMOVED******REMOVED******REMOVED*** backup_database.py
...
**Automation:**
```bash
***REMOVED*** Cron job for daily backups at 2 AM
0 2 * * * cd /app && python scripts/ops/backup_database.py --retention-days 30
```

---

***REMOVED******REMOVED******REMOVED*** restore_database.py
...
***REMOVED******REMOVED******REMOVED*** rotate_secrets.py
...
**Usage:**
```bash
***REMOVED*** Rotate all secrets
python scripts/ops/rotate_secrets.py --secret-type all

***REMOVED*** Rotate specific secret type
...
***REMOVED*** Create backup before rotation
python scripts/ops/rotate_secrets.py --secret-type all --backup

***REMOVED*** Custom .env file location
...
**Post-Rotation Steps:**
```bash
***REMOVED*** After rotation, restart services
docker-compose restart backend
docker-compose restart celery-worker
docker-compose restart celery-beat

***REMOVED*** If Redis password changed, update Redis container
docker-compose restart redis
```

---

***REMOVED******REMOVED******REMOVED*** cleanup_old_data.py
...
***REMOVED******REMOVED******REMOVED*** export_audit_logs.py
...
***REMOVED******REMOVED******REMOVED*** validate_config.py
...
***REMOVED******REMOVED*** Development Scripts

***REMOVED******REMOVED******REMOVED*** setup_dev_env.py
...
**Usage:**
```bash
***REMOVED*** Full setup (backend + frontend + database + seed data)
...
***REMOVED*** Skip database initialization
python scripts/dev/setup_dev_env.py --skip-db

***REMOVED*** Skip test data seeding
...
***REMOVED*** Run setup script
python scripts/dev/setup_dev_env.py

***REMOVED*** Start development servers
...
***REMOVED******REMOVED******REMOVED*** generate_test_data.py
...
**Usage:**
```bash
***REMOVED*** Use preset configuration
python scripts/dev/generate_test_data.py --preset small   ***REMOVED*** 18 residents, 10 faculty
python scripts/dev/generate_test_data.py --preset medium  ***REMOVED*** 30 residents, 15 faculty
python scripts/dev/generate_test_data.py --preset large   ***REMOVED*** 50 residents, 20 faculty

***REMOVED*** Custom configuration
python scripts/dev/generate_test_data.py --residents 24 --faculty 12

***REMOVED*** Specify academic year
python scripts/dev/generate_test_data.py --academic-year 2025

***REMOVED*** Generate with assignments (experimental)
python scripts/dev/generate_test_data.py --preset medium --with-assignments

***REMOVED*** Clear existing data first
...
**Example Workflow:**
```bash
***REMOVED*** Reset test database
python scripts/dev/generate_test_data.py --preset small --clear-first

***REMOVED*** Test with larger dataset
python scripts/dev/generate_test_data.py --preset large

***REMOVED*** Generate schedule for testing
python scripts/scheduling/generate_schedule.py --block 10
```

---

***REMOVED******REMOVED******REMOVED*** seed_database.py
...
***REMOVED******REMOVED******REMOVED*** run_migrations.py
...
***REMOVED******REMOVED******REMOVED*** check_dependencies.py
...
***REMOVED******REMOVED******REMOVED*** format_code.py
...
***REMOVED******REMOVED******REMOVED*** run_quality_checks.py
...
***REMOVED******REMOVED******REMOVED*** pre_deploy_check.py
...
***REMOVED*** Skip tests (faster but less thorough)
python scripts/deploy/pre_deploy_check.py --skip-tests

***REMOVED*** Strict mode (fail on warnings)
...
***REMOVED******REMOVED******REMOVED*** post_deploy_verify.py
...
***REMOVED******REMOVED******REMOVED*** rollback.py
...
***REMOVED******REMOVED******REMOVED*** blue_green_switch.py
...
***REMOVED******REMOVED******REMOVED*** canary_promote.py
...
***REMOVED******REMOVED******REMOVED*** generate_schedule.py
...
***REMOVED*** Generate for specific block
python scripts/scheduling/generate_schedule.py --block 10

***REMOVED*** Use specific algorithm
python scripts/scheduling/generate_schedule.py \
    --start 2025-07-01 --end 2025-09-30 \
...
***REMOVED*** Dry run (don't save to database)
python scripts/scheduling/generate_schedule.py \
    --block 10 --dry-run
...
**Automation:**
```bash
***REMOVED*** Cron job to pre-generate upcoming block schedule
0 0 1 * * cd /app && python scripts/scheduling/generate_schedule.py --block 12 --algorithm cp_sat
```

---

***REMOVED******REMOVED******REMOVED*** validate_schedule.py
...
***REMOVED******REMOVED******REMOVED*** export_schedule.py
...
***REMOVED******REMOVED******REMOVED*** import_schedule.py
...
***REMOVED******REMOVED******REMOVED*** compare_schedules.py
...
***REMOVED******REMOVED******REMOVED*** optimize_schedule.py
...
```bash
***REMOVED*** Morning health check
python scripts/ops/health_check.py

***REMOVED*** Check compliance before rounds
python -m app.cli compliance check --verbose

***REMOVED*** Export today's schedule
...
```bash
***REMOVED*** Sunday night backup
python scripts/ops/backup_database.py --retention-days 30

***REMOVED*** Check for data cleanup
python -m app.cli maintenance cleanup --days 90 --confirm

***REMOVED*** Vacuum database
...
```bash
***REMOVED*** Rotate secrets (first Monday of month)
python scripts/ops/rotate_secrets.py --secret-type all --backup
...
```bash
***REMOVED*** 1. Pre-deployment verification
python scripts/deploy/pre_deploy_check.py --strict

***REMOVED*** 2. Create backup
python scripts/ops/backup_database.py --output pre_deploy_backup.dump

***REMOVED*** 3. Run migrations (if needed)
python -m app.cli schema upgrade

***REMOVED*** 4. Post-deployment verification
python scripts/ops/health_check.py

***REMOVED*** 5. Validate schedule integrity
...
```bash
***REMOVED*** Bash script example
set -e  ***REMOVED*** Exit on error

if ! python scripts/ops/health_check.py; then
    echo "Health check failed" >&2
    python scripts/ops/backup_database.py  ***REMOVED*** Emergency backup
    exit 1
fi

***REMOVED*** Continue with deployment...
```

```python
***REMOVED*** Python script example
import subprocess
import sys
...
***REMOVED******REMOVED******REMOVED*** Capturing Script Output

```bash
***REMOVED*** Redirect to log file
python scripts/ops/health_check.py > health.log 2>&1

***REMOVED*** Tee to both console and file
python scripts/ops/health_check.py 2>&1 | tee health.log

***REMOVED*** Structured logging (JSON format)
