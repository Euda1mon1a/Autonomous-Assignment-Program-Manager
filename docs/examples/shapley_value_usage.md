***REMOVED*** Shapley Value Calculator - Usage Guide
...
***REMOVED******REMOVED******REMOVED*** 1. Shapley Value (0-1)
...
***REMOVED******REMOVED******REMOVED*** 2. Marginal Contribution
...
***REMOVED******REMOVED******REMOVED*** 3. Fair Workload Target
...
***REMOVED******REMOVED******REMOVED*** 4. Equity Gap
...
***REMOVED******REMOVED******REMOVED*** Calculate Shapley Values

```python
from datetime import date
from app.services.shapley_values import get_shapley_value_service

***REMOVED*** Initialize service (in route handler with dependency injection)
service = get_shapley_value_service(db)

***REMOVED*** Calculate for Q1 2024
results = await service.calculate_shapley_values(
    faculty_ids=[fac1_id, fac2_id, fac3_id],
    start_date=date(2024, 1, 1),
    end_date=date(2024, 3, 31),
    num_samples=2000  ***REMOVED*** Higher = more accurate (default: 1000)
)

***REMOVED*** Access individual results
...
***REMOVED******REMOVED******REMOVED*** Generate Comprehensive Equity Report
...
***REMOVED******REMOVED******REMOVED*** Get Workload Adjustment Recommendations
...
suggestions = await service.suggest_workload_adjustments(
    ...
    threshold=10.0  ***REMOVED*** Only suggest if gap > 10 hours
)
...
***REMOVED******REMOVED******REMOVED*** Use Shapley Values as Fairness Constraints

```python
***REMOVED*** In schedule generator
from app.services.shapley_values import get_shapley_value_service

***REMOVED*** Calculate historical Shapley values
shapley_service = get_shapley_value_service(db)
historical_results = await shapley_service.calculate_shapley_values(
    faculty_ids=all_faculty,
    start_date=academic_year_start - timedelta(days=90),  ***REMOVED*** Previous quarter
    end_date=academic_year_start,
    num_samples=2000
)

***REMOVED*** Use as soft constraints for next period
for faculty_id, result in historical_results.items():
    ***REMOVED*** Target workload based on Shapley proportion
    target_hours = result.fair_workload_target

    ***REMOVED*** Add constraint: faculty i should get ~target_hours ± tolerance
...
***REMOVED******REMOVED******REMOVED*** Detect Systematic Inequity Over Time

```python
from datetime import timedelta

***REMOVED*** Calculate Shapley values for each quarter
quarters = [
...
***REMOVED*** Use immutable types for caching
results = await get_shapley_cached(
    tuple(sorted(faculty_ids)),  ***REMOVED*** Tuple instead of list
    start_date,
    end_date
)
```

***REMOVED******REMOVED*** Theoretical Properties
...
2. **Symmetry**: Identical players get identical payoffs
   ```python
   ***REMOVED*** If Faculty A and B have identical assignments:
...
4. **Null Player**: Zero contribution → zero Shapley value
   ```python
   ***REMOVED*** If faculty has no assignments:
   assert results[idle_faculty].shapley_value < 0.05
   ```

***REMOVED******REMOVED*** Common Patterns

***REMOVED******REMOVED******REMOVED*** 1. Quarterly Fairness Review

```python
***REMOVED*** Run at end of each quarter
...
***REMOVED*** Flag if equity gap std dev > threshold (unfair distribution)
if report.equity_gap_std_dev > 15.0:
    send_alert(f"Quarterly inequity detected: σ={report.equity_gap_std_dev:.1f} hours")
```

***REMOVED******REMOVED******REMOVED*** 2. Pre-Schedule Validation

```python
***REMOVED*** Before finalizing next block's schedule
...
***REMOVED*** Check if new schedule would create large gaps
...
***REMOVED******REMOVED******REMOVED*** 3. Faculty Burnout Detection

```python
***REMOVED*** Combine Shapley equity with resilience metrics
...
for faculty in all_faculty:
    ***REMOVED*** Check resilience health
    health = await resilience_service.get_person_health(faculty.id)

    ***REMOVED*** Check Shapley equity
    shapley_result = shapley_results[faculty.id]

    ***REMOVED*** Alert if overworked AND showing burnout signs
...
***REMOVED******REMOVED*** API Routes (Example)

```python
***REMOVED*** backend/app/api/routes/game_theory.py
