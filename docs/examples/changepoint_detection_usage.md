***REMOVED*** Change Point Detection Usage Guide
...
***REMOVED******REMOVED******REMOVED*** MCP Tool (Simplest)
...
***REMOVED******REMOVED******REMOVED*** Python Backend (Direct)

```python
from datetime import date, timedelta
from app.analytics.signal_processing import detect_schedule_changepoints

***REMOVED*** Create sample data with a regime shift
daily_hours = [8.0] * 30 + [12.0] * 30  ***REMOVED*** Shift at day 30
dates = [date(2025, 1, 1) + timedelta(days=i) for i in range(60)]

***REMOVED*** Detect change points
...
***REMOVED******REMOVED*** Understanding the Algorithms

***REMOVED******REMOVED******REMOVED*** CUSUM (Cumulative Sum)
...
***REMOVED******REMOVED******REMOVED*** PELT (Pruned Exact Linear Time)
...
***REMOVED******REMOVED******REMOVED*** 1. Detecting Policy Changes

```python
***REMOVED*** Analyze a year of workload data
daily_hours = fetch_daily_workload(year=2024)
dates = [date(2024, 1, 1) + timedelta(days=i) for i in range(365)]

results = detect_schedule_changepoints(daily_hours, dates)

***REMOVED*** Find high-confidence changes
policy_changes = [
...
***REMOVED******REMOVED******REMOVED*** 2. Staffing Level Transitions

```python
***REMOVED*** Look for upward shifts indicating reduced staffing
results = detect_schedule_changepoints(daily_hours, dates, methods=["cusum"])

staffing_reductions = [
    cp for cp in results['cusum']['change_points']
    if cp['change_type'] == 'mean_shift_upward'  ***REMOVED*** More work = fewer staff
    and cp['magnitude'] > 2.0  ***REMOVED*** Significant increase
]
...
***REMOVED******REMOVED******REMOVED*** 3. Compliance Audit Trail

```python
***REMOVED*** Generate audit report of all detected changes
results = detect_schedule_changepoints(daily_hours, dates)
...
***REMOVED*** Flag changes requiring human review
...
***REMOVED******REMOVED******REMOVED*** 4. Integration with Signal Processing Pipeline

```python
from app.analytics.signal_processing import WorkloadSignalProcessor, WorkloadTimeSeries

***REMOVED*** Full signal analysis including change points
...
***REMOVED*** Change points in context with other anomalies
...
***REMOVED******REMOVED*** Output Interpretation

***REMOVED******REMOVED******REMOVED*** Change Point Structure

```python
{
    "index": 30,                    ***REMOVED*** Position in time series
    "timestamp": "2025-01-31",      ***REMOVED*** Date of change
    "change_type": "mean_shift_upward",  ***REMOVED*** Type of change detected
    "magnitude": 4.2,               ***REMOVED*** Size of change (hours)
    "confidence": 0.85,             ***REMOVED*** How confident (0-1)
    "description": "Upward shift: 8.0 → 12.2"
}
```

***REMOVED******REMOVED******REMOVED*** Change Types
...
***REMOVED******REMOVED******REMOVED*** Confidence Interpretation
...
***REMOVED******REMOVED******REMOVED*** No Change Points Detected
...
***REMOVED******REMOVED******REMOVED*** Too Many Change Points
...
***REMOVED******REMOVED******REMOVED*** PELT Fallback Warning
...
detect_schedule_changepoints_tool(
    daily_values: list[float],  ***REMOVED*** Required: workload values
    dates: list[str],           ***REMOVED*** Required: YYYY-MM-DD dates
    methods: list[str] | None   ***REMOVED*** Optional: ["cusum", "pelt"]
) -> dict
```

***REMOVED******REMOVED******REMOVED*** Python Function
...
***REMOVED******REMOVED******REMOVED*** WorkloadSignalProcessor Methods
