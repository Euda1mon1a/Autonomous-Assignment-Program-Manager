***REMOVED*** FMIT Scheduling - Parallel Development Sessions
...
***REMOVED******REMOVED*** Terminal 1: Unit Tests (test_fmit_scheduling.py)

**Branch:** `claude/fmit-tests-unit-{SESSION_ID}`

***REMOVED******REMOVED******REMOVED*** Files OWNED (only modify these):
...
***REMOVED*** All 31 tests should pass
```
...
***REMOVED******REMOVED*** Terminal 2: CLI Tests (NEW FILE)
...
***REMOVED*** Minimum 8 tests, all passing
```
...
class TestSwapAnalyzerCLI:
    ...
    def sample_excel_path(self, tmp_path) -> Path:
        ...
        ***REMOVED*** Create minimal test Excel file
    ...
    ***REMOVED*** Add more tests...
```

---

***REMOVED******REMOVED*** Terminal 3: Integration Tests (NEW FILE)
...
***REMOVED*** Minimum 5 tests, all passing
```
...
class TestFMITSwapWorkflow:
    ...
    def realistic_schedule_bytes(self) -> bytes:
        ...
        ***REMOVED*** Header with dates (6 weeks)
        ws["A1"] = "Provider"
        ...
        ***REMOVED*** Multiple providers with different patterns
        providers = [
            ("Dr. Smith", ["FMIT", "", "FMIT", "", "FMIT", ""]),      ***REMOVED*** Alternating
            ("Dr. Jones", ["", "FMIT", "", "", "FMIT", ""]),          ***REMOVED*** Sparse
            ("Dr. Lee", ["", "", "", "FMIT", "", ""]),                ***REMOVED*** Single week
            ("Dr. Brown", ["FMIT", "FMIT", "", "", "", ""]),          ***REMOVED*** Back-to-back
        ]
    ...
    def test_full_swap_workflow(
        ...
        ***REMOVED*** Step 1: Find swap candidates
        request_data = {
        ...
        ***REMOVED*** Step 2: Verify response structure
        assert data["success"]
        assert data["target_faculty"] == "Dr. Smith"
        assert "candidates" in data

        ***REMOVED*** Step 3: Verify constraint detection
        ***REMOVED*** Dr. Brown has back-to-back weeks, should be flagged
        brown_candidate = next(
            (c for c in data["candidates"] if c["faculty"] == "Dr. Brown"),
            None
        )
        if brown_candidate:
            ***REMOVED*** Taking week 1 would create back-to-back for Brown
...
***REMOVED******REMOVED*** Terminal 4: Performance Tests (NEW FILE)
...
***REMOVED*** All performance benchmarks pass
***REMOVED*** find_swap_candidates < 1s for 50 faculty
```
...
class TestSwapFinderPerformance:
    ...
    def generate_large_schedule(self, num_faculty: int, num_weeks: int) -> ImportResult:
        ...
        base_date = date(2025, 1, 6)  ***REMOVED*** First Monday of 2025

        for i in range(num_faculty):
            provider_name = f"Dr. Faculty{i:03d}"
            provider = ProviderSchedule(name=provider_name)

            ***REMOVED*** Assign ~6 FMIT weeks per faculty (distributed)
            assigned_weeks = [
    ...
    def test_find_candidates_under_one_second(self):
        ...
        ***REMOVED*** Pick a faculty and week to search
    ...
    def test_scaling_characteristics(self, num_faculty: int):
        ...
        ***REMOVED*** Log for documentation
        print(f"\n{num_faculty} faculty: {elapsed:.3f}s, {len(candidates)} candidates")

        ***REMOVED*** Reasonable limits based on faculty count
        limit = num_faculty * 0.05  ***REMOVED*** 50ms per faculty max
...
***REMOVED******REMOVED*** Terminal 5: xlsx_import Coverage Tests (NEW FILE)
...
class TestProviderSchedule:
    ...
    def test_get_fmit_weeks(self):
        ...
        ***REMOVED*** Add slots for same week (Mon-Fri)
        for day in range(5):
            for tod in ["AM", "PM"]:
                slot = ScheduleSlot(
                    provider_name="Dr. Test",
        ...
        assert len(weeks) == 1  ***REMOVED*** All slots in same week
...
***REMOVED******REMOVED******REMOVED*** Phase 1: All 5 Terminals in Parallel (No Dependencies)
```
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Verify All Tests Pass
```bash
***REMOVED*** Run all tests together
...
```bash
***REMOVED*** 1. Create branch
git checkout -b claude/fmit-tests-{type}-{SESSION_ID}

***REMOVED*** 2. Make changes to OWNED files only

***REMOVED*** 3. Commit with descriptive message
