***REMOVED*** MCP Meta-Tools Implementation Specification
...
2. [Meta-Tool Architecture](***REMOVED***meta-tool-architecture)
3. [Meta-Tool Specifications](***REMOVED***meta-tool-specifications)
4. [Tool Definitions](***REMOVED***tool-definitions)
5. [Prompt Templates](***REMOVED***prompt-templates)
6. [Resource Management](***REMOVED***resource-management)
7. [Testing Strategy](***REMOVED***testing-strategy)
...
***REMOVED******REMOVED*** Meta-Tool Architecture

***REMOVED******REMOVED******REMOVED*** Layer Structure
...
***REMOVED******REMOVED******REMOVED*** Tool Composition DAG Pattern
...
class ExecutionStrategy(Enum):
    ...
    SEQUENTIAL = "sequential"      ***REMOVED*** One at a time, strict order
    PARALLEL = "parallel"          ***REMOVED*** All at once
    PHASED = "phased"              ***REMOVED*** Groups with barriers
    CONDITIONAL = "conditional"    ***REMOVED*** Branch based on results
...
class DAGNode:
    ...
    required: bool = True  ***REMOVED*** If True, failure aborts DAG
...
class DAGDefinition:
    ...
    timeout_seconds: float = 300.0  ***REMOVED*** 5 minutes default
    rollback_on_failure: bool = False

    def to_mermaid(self) -> str:
        """Generate Mermaid diagram of DAG."""
        ***REMOVED*** Implementation in orchestration/dag_visualizer.py
        pass
```

***REMOVED******REMOVED******REMOVED*** Error Handling and Rollback
...
class TransactionalOperation:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is not None:
            ...
            if self.rollback_on_validation_failure:
                await self._rollback()
        return False  ***REMOVED*** Don't suppress exceptions
    ...
    async def _create_snapshot(self) -> Any:
        ...
        ***REMOVED*** Implementation depends on backend
        pass
...
***REMOVED******REMOVED*** Meta-Tool Specifications

***REMOVED******REMOVED******REMOVED*** Meta-Tool 1: `full_schedule_generation`
...
**Implementation:**
```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/schedule_generation.py
...
class ScheduleGenerationReport(BaseModel):
    ...
    status: str  ***REMOVED*** "success", "validation_failed", "approval_rejected", "conflict_unresolved"
...
async def full_schedule_generation(
    ...
    try:
        ***REMOVED*** Step 1: Pre-generation validation
        ***REMOVED*** Verify date range is valid, faculty available, etc.
        ...
        ***REMOVED*** Step 2: Verify backup exists (safety gate)
        ...
        ***REMOVED*** Step 3: Generate schedule
        generation_result = await api_client.generate_schedule(
            start_date=start_date,
            end_date=end_date,
            algorithm=algorithm,
            dry_run=require_approval  ***REMOVED*** Don't apply if approval needed
        )

        report.schedule_id = generation_result["schedule_id"]

        ***REMOVED*** Step 4: Validate generated schedule
        iteration = 0
        while iteration < max_iterations:
            ...
            ***REMOVED*** Step 5: Detect and resolve conflicts
            ...
            ***REMOVED*** Auto-resolve conflicts
        ...
        ***REMOVED*** Step 6: Resilience health checks (parallel)
        import asyncio
        ...
        resilience_tasks = [
            ...
            run_contingency_analysis_resilience(
                analyze_n1=True,
                analyze_n2=False  ***REMOVED*** N-2 too slow for generation
            )
        ]
        ...
        ***REMOVED*** Step 7: Human approval gate
        if require_approval:
            ***REMOVED*** Request approval from human
            ***REMOVED*** Implementation depends on notification system
...
async def _request_approval(schedule_id: str, validation: dict, resilience: dict) -> bool:
    ...
    """
    ***REMOVED*** Implementation depends on notification/approval system
    ***REMOVED*** Could be:
    ***REMOVED*** - Email with approval link
    ***REMOVED*** - Slack message with approve/reject buttons
    ***REMOVED*** - Web UI notification
    return True
```
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 2: `emergency_coverage_repair`
...
**Implementation:**
```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/coverage_repair.py
...
class CoverageRepairReport(BaseModel):
    ...
    status: str  ***REMOVED*** "success", "partial", "failed", "manual_required"
    gaps_detected: int
    gaps_resolved: int
    gaps_remaining: int
...
async def emergency_coverage_repair(
    ...
    try:
        ***REMOVED*** Step 1: Detect coverage gaps via contingency analysis
        ...
        ***REMOVED*** Step 2: Parallel solution analysis
        solution_tasks = []

        ***REMOVED*** For each gap, find swap candidates
        for gap in gaps:
        ...
        ***REMOVED*** Also check for fallback schedules
        if use_fallback:
            solution_tasks.append(
                get_static_fallbacks(
                    start_date=absence_start,
        ...
        ***REMOVED*** Run all analyses in parallel
        solution_results = await asyncio.gather(*solution_tasks, return_exceptions=True)

        ***REMOVED*** Step 3: Rank and apply solutions
        ...
        ***REMOVED*** Collect all swap options
        all_swap_options = []
        ...
        ***REMOVED*** Try swaps first (highest priority)
        swaps_applied = 0
        for option in all_swap_options:
            ...
            ***REMOVED*** NOTE: This requires a dry-run swap validation endpoint
            ...
            ***REMOVED*** Apply swap (or request approval)
            if auto_apply_swaps:
                ...
                ***REMOVED*** Request approval
                ***REMOVED*** TODO: Integrate with approval system
                pass

        report.gaps_remaining = report.gaps_detected - report.gaps_resolved

        ***REMOVED*** Step 4: Use fallback if gaps remain
        if report.gaps_remaining > 0 and use_fallback and fallback_result:
            if not isinstance(fallback_result, Exception) and fallback_result.fallback_schedules:
                ***REMOVED*** Apply fallback schedule
        ...
        ***REMOVED*** Step 5: Load shedding (simulate only, requires approval)
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 3: `swap_workflow_complete`
...
**Implementation:**
```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/swap_workflow.py
...
class SwapWorkflowReport(BaseModel):
    ...
    status: str  ***REMOVED*** "success", "no_candidates", "validation_failed", "rejected"
...
async def swap_workflow_complete(
    ...
    try:
        ***REMOVED*** Step 1: Find swap candidate (if not specified)
        if target_person_id is None and auto_match:
            ...
            ***REMOVED*** Filter by mutual benefit if required
        ...
        ***REMOVED*** Step 2: Validate swap (dry-run)
        validation = await api_client.validate_swap(
            requester_person_id=requester_person_id,
            assignment_id=assignment_id,
        ...
        ***REMOVED*** Step 3: Check ACGME impact
        ***REMOVED*** Get assignment details to find dates
        ...
        ***REMOVED*** Step 4: Execute swap (unless dry-run)
        if not dry_run:
            ...
            ***REMOVED*** Step 5: Post-swap validation
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 4: `resilience_dashboard_snapshot`
...
**Implementation:**
```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/resilience_dashboard.py
...
class ResilienceDashboardSnapshot(BaseModel):
    ...
    overall_severity: str  ***REMOVED*** "healthy", "warning", "critical", "emergency"
    tier1: dict[str, Any] | None  ***REMOVED*** Core: Utilization, Defense, Contingency
    tier2: dict[str, Any] | None  ***REMOVED*** Strategic: Homeostasis, Blast, Le Chatelier
    tier3: dict[str, Any] | None  ***REMOVED*** Advanced: Hubs, Cognitive, Behavioral, Stigmergy
    mtf: dict[str, Any] | None    ***REMOVED*** Military compliance
    timestamp: str
    execution_time_ms: int
    errors: list[str] = []
...
async def resilience_dashboard_snapshot(
    ...
    try:
        ...
        ***REMOVED*** Barrier: Wait for Tier 2

        ***REMOVED*** Tier 3: Advanced (Parallel)
        if include_tier3:
            tier3_tasks = [
                analyze_hub_centrality(),
                assess_cognitive_load(
        ...
        ***REMOVED*** Military Compliance (Independent)
        if include_mtf:
        ...
        ***REMOVED*** Aggregate overall severity
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 5: `weekly_health_report`
...
**Implementation:**
```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/weekly_report.py
...
async def weekly_health_report(
    ...
    ***REMOVED*** Calculate dates
    ...
    try:
        ***REMOVED*** Parallel data gathering
        tasks = [
            ***REMOVED*** Past week validation
            validate_schedule(
                start_date=str(period_start),
                end_date=str(report_date)
            ),
            ***REMOVED*** Current conflicts
            detect_conflicts(
            ...
            ***REMOVED*** Resilience snapshot
            resilience_dashboard_snapshot(
                include_tier1=True,
                include_tier2=True,
                include_tier3=False  ***REMOVED*** Skip advanced for weekly report
            ),
            ***REMOVED*** Expiring credentials
            api_client.get("/api/v1/credentials/expiring", params={
                "days_ahead": lookahead_days
            }),
            ***REMOVED*** Upcoming absences
            ...
            ***REMOVED*** Critical alerts
            if validation_result.critical_issues > 0:
                report.critical_alerts.append({
                    ...
                    "details": validation_result.issues[:5]  ***REMOVED*** Top 5
                })

        ***REMOVED*** Conflict summary
        if conflicts_result:
        ...
        ***REMOVED*** Generate recommendations (if requested)
...
async def _generate_recommendations(
    ...
    ***REMOVED*** Recommendation 1: Compliance violations
    ...
    ***REMOVED*** Recommendation 2: High utilization
    ...
    ***REMOVED*** Recommendation 3: Expiring credentials
    if len(report.expiring_credentials) > 0:
        recommendations.append({
            "priority": "medium",
    ...
    ***REMOVED*** Recommendation 4: Upcoming absences
...
***REMOVED******REMOVED*** Tool Definitions

***REMOVED******REMOVED******REMOVED*** FastMCP Tool Registration

```python
***REMOVED*** mcp-server/src/scheduler_mcp/meta_tools/__init__.py
...
***REMOVED******REMOVED******REMOVED*** Input/Output Schemas

All meta-tools use Pydantic models for validation:

```python
***REMOVED*** Inputs are validated via function signatures with type hints
***REMOVED*** Outputs are validated via Pydantic BaseModel subclasses

***REMOVED*** Example validation:
...
***REMOVED******REMOVED******REMOVED*** Dependency Declarations

Meta-tools declare dependencies on atomic tools:

```python
***REMOVED*** Each meta-tool has a DEPENDENCIES constant
DEPENDENCIES = [
    "validate_schedule",
    "detect_conflicts",
    "check_utilization_threshold",
...
***REMOVED******REMOVED*** Prompt Templates

***REMOVED******REMOVED******REMOVED*** Template 1: Schedule Generation Workflow

```python
***REMOVED*** mcp-server/src/scheduler_mcp/prompts/schedule_generation.py
...
***REMOVED******REMOVED******REMOVED*** Template 2: Emergency Coverage Response
...
***REMOVED******REMOVED******REMOVED*** Template 3: Weekly Report Review
...
***REMOVED******REMOVED******REMOVED*** Dynamic Prompt Generation

```python
***REMOVED*** mcp-server/src/scheduler_mcp/prompts/generator.py
...
***REMOVED******REMOVED*** Resource Management

***REMOVED******REMOVED******REMOVED*** MCP Resources for Schedule State

```python
***REMOVED*** mcp-server/src/scheduler_mcp/resources.py (additions)
...
def register_meta_tool_resources(mcp: FastMCP):
    ...
    async def resilience_dashboard_live() -> str:
        ...
        ***REMOVED*** Generate fresh snapshot
        snapshot = await resilience_dashboard_snapshot()

        ***REMOVED*** Cache for 5 minutes
...
async def invalidate_cache_on_event(event_type: str, event_data: dict):
    ...
    for key in keys_to_invalidate:
        if "*" in key:
            ***REMOVED*** Wildcard: clear all matching keys
            prefix = key.split("*")[0]
...
***REMOVED******REMOVED******REMOVED*** Unit Tests for Meta-Tools

```python
***REMOVED*** mcp-server/tests/test_meta_tools.py
...
class TestFullScheduleGeneration:
    ...
    async def test_rollback_on_error(self, mock_api_client):
        ...
        ***REMOVED*** Verify rollback was called
...
```python
***REMOVED*** mcp-server/tests/integration/test_meta_tool_orchestration.py
...
class TestMetaToolOrchestration:
    ...
    async def test_full_schedule_generation_end_to_end(
        ...
        ***REMOVED*** This test requires:
        ***REMOVED*** 1. Test database with sample data
        ***REMOVED*** 2. Running FastAPI backend
        ***REMOVED*** 3. MCP server with meta-tools registered
        ...
        ***REMOVED*** Verify schedule exists in database
...
***REMOVED******REMOVED******REMOVED*** Mock Tool Testing

```python
***REMOVED*** mcp-server/tests/mocks/mock_tools.py
...
├── __init__.py
├── dag_executor.py          ***REMOVED*** DAG execution engine
├── parallel.py              ***REMOVED*** Parallel execution patterns
├── retry.py                 ***REMOVED*** Retry and backoff
├── circuit_breaker.py       ***REMOVED*** Circuit breaker pattern
└── transactional.py         ***REMOVED*** Transactional operations
```
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Meta-Tool Implementation (Week 2-3)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Prompt Templates & Resources (Week 3)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Integration & Testing (Week 4)
