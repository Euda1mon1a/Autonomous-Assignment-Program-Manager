***REMOVED*** Burnout Prediction Service Specification
...
4. [Prediction Models](***REMOVED***prediction-models)
5. [Database Schema](***REMOVED***database-schema)
6. [Ensemble Logic](***REMOVED***ensemble-logic)
7. [Integration Points](***REMOVED***integration-points)
8. [Implementation Plan](***REMOVED***implementation-plan)
9. [Testing Strategy](***REMOVED***testing-strategy)
10. [Performance Requirements](***REMOVED***performance-requirements)
...
***REMOVED******REMOVED******REMOVED*** 1.2 Key Innovation
...
***REMOVED******REMOVED******REMOVED*** 1.3 Success Metrics
...
***REMOVED******REMOVED******REMOVED*** 1.4 Core Models
...
***REMOVED******REMOVED******REMOVED*** 2.1 Module Structure
...
├── ensemble.py                   ***REMOVED*** Multi-model ensemble predictor
├── calibration.py               ***REMOVED*** Parameter calibration from data
├── service.py                   ***REMOVED*** Main service class
└── persistence.py               ***REMOVED*** Database operations

backend/app/schemas/burnout_prediction.py  ***REMOVED*** Pydantic schemas
backend/app/models/burnout.py             ***REMOVED*** SQLAlchemy models
backend/app/api/routes/burnout.py         ***REMOVED*** API endpoints
```

***REMOVED******REMOVED******REMOVED*** 2.2 Service Class Design
...
class BurnoutPredictionConfig:
    ...
    ***REMOVED*** Model parameters
    sn_sigma_f_prime: float = 100.0      ***REMOVED*** S-N curve fatigue strength coefficient
    sn_b: float = -0.12                  ***REMOVED*** S-N curve exponent
    sn_endurance_limit: float = 50.0     ***REMOVED*** Stress below which infinite cycles tolerated

    ***REMOVED*** Miner's Rule
    miner_critical_damage: float = 1.0   ***REMOVED*** Failure threshold
    miner_warning_threshold: float = 0.7 ***REMOVED*** Early warning at 70%

    ***REMOVED*** Larson-Miller
    lm_C: float = 20.0                   ***REMOVED*** Material constant
    lm_reference_LMP: float = 30000      ***REMOVED*** Reference parameter value

    ***REMOVED*** Paris Law
    paris_C: float = 1e-5                ***REMOVED*** Crack growth coefficient
    paris_m: float = 3.0                 ***REMOVED*** Crack growth exponent
    paris_critical_severity: float = 27.0 ***REMOVED*** MBI-EE burnout threshold

    ***REMOVED*** Ensemble weights (must sum to 1.0)
    weight_sn_curve: float = 0.20
    weight_miner: float = 0.35           ***REMOVED*** Highest weight - most practical
    weight_larson_miller: float = 0.25
    weight_paris_law: float = 0.20

    ***REMOVED*** Calibration settings
    enable_adaptive_calibration: bool = True
    calibration_learning_rate: float = 0.1
    min_calibration_samples: int = 20

    ***REMOVED*** Alert thresholds (days to burnout)
    critical_threshold: int = 30         ***REMOVED*** < 30 days = CRITICAL
    high_threshold: int = 90             ***REMOVED*** < 90 days = HIGH
    moderate_threshold: int = 180        ***REMOVED*** < 180 days = MODERATE
...
class BurnoutPredictionService:
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize ensemble predictor
        self.ensemble = EnsembleBurnoutPredictor(config=self.config)

        ***REMOVED*** Initialize calibrator
        self.calibrator = ModelCalibrator(db=db, config=self.config)

        ***REMOVED*** Load calibrated parameters if available
    ...
    async def predict_burnout(
        ...
        """
        ***REMOVED*** Implementation details below...
...
***REMOVED******REMOVED******REMOVED*** 2.3 Ensemble Predictor
...
class BurnoutPrediction:
    ...
    ***REMOVED*** Model-specific predictions (days to burnout)
    ...
    coffin_manson_regime: str              ***REMOVED*** LOW_CYCLE_FATIGUE, HIGH_CYCLE_FATIGUE, TRANSITION
    ...
    ***REMOVED*** Unified prediction (weighted ensemble)
    ...
    confidence_interval_lower: float       ***REMOVED*** 95% CI lower bound
    confidence_interval_upper: float       ***REMOVED*** 95% CI upper bound
    ...
    ***REMOVED*** Recommendations
    ...
    workload_adjustment_recommended: float ***REMOVED*** Percentage reduction suggested

    ***REMOVED*** Metadata
    models_used: int                       ***REMOVED*** How many models contributed
    prediction_quality: str                ***REMOVED*** EXCELLENT, GOOD, FAIR, POOR
...
class EnsembleBurnoutPredictor:
    ...
    def predict(
        ...
        ***REMOVED*** Implementation combines all 5 models with weighted averaging
        ***REMOVED*** Details in section 6.1
```

---

***REMOVED******REMOVED*** 3. API Endpoints

***REMOVED******REMOVED******REMOVED*** 3.1 Prediction Endpoints

***REMOVED******REMOVED******REMOVED******REMOVED*** POST /api/v1/burnout/predict
...
***REMOVED******REMOVED******REMOVED******REMOVED*** GET /api/v1/burnout/resident/{person_id}/fatigue
...
***REMOVED******REMOVED******REMOVED******REMOVED*** POST /api/v1/burnout/calibrate
...
***REMOVED******REMOVED******REMOVED******REMOVED*** GET /api/v1/burnout/dashboard
...
***REMOVED******REMOVED******REMOVED*** 3.2 Symptom Tracking Endpoints

***REMOVED******REMOVED******REMOVED******REMOVED*** POST /api/v1/burnout/symptoms
...
***REMOVED******REMOVED******REMOVED*** 3.3 Workload Recording Endpoints

***REMOVED******REMOVED******REMOVED******REMOVED*** POST /api/v1/burnout/workload
...
***REMOVED******REMOVED*** 4. Prediction Models

***REMOVED******REMOVED******REMOVED*** 4.1 S-N Curve Model
...
***REMOVED*** Predict cycles to burnout for weekend call (85% intensity)
cycles = model.cycles_to_failure(85)  ***REMOVED*** ~52 cycles
days = cycles * 7  ***REMOVED*** Convert to days (assuming weekly cycles)

***REMOVED*** Find safe workload for 3-year residency
safe_stress = model.stress_for_target_life(156)  ***REMOVED*** 3 years = 156 weeks
```

---

***REMOVED******REMOVED******REMOVED*** 4.2 Miner's Rule Model
...
***REMOVED******REMOVED******REMOVED*** 4.3 Coffin-Manson Model
...
***REMOVED******REMOVED******REMOVED*** 4.4 Larson-Miller Model
...
***REMOVED******REMOVED******REMOVED*** 4.5 Paris Law Model
...
class ParisLawParameters:
    ...
    K_IC: float = 50.0
    a_critical: float = 27.0  ***REMOVED*** MBI-EE ≥ 27 = high burnout
...
class ParisLawModel:
    ...
    def stress_intensity_range(self, crack_length: float, stress_range: float) -> float:
        ...
        Y = 1.0  ***REMOVED*** Geometry factor (simplified)
...
***REMOVED******REMOVED*** 5. Database Schema

***REMOVED******REMOVED******REMOVED*** 5.1 Core Tables

***REMOVED******REMOVED******REMOVED******REMOVED*** `burnout_predictions`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `workload_history`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `symptom_records`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `burnout_events`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `personal_calibration`
...
***REMOVED******REMOVED******REMOVED*** 5.2 Alembic Migration Example
...
def upgrade():
    ***REMOVED*** Create burnout_predictions table
    op.create_table(
        'burnout_predictions',
    ...
    ***REMOVED*** Create other tables (workload_history, symptom_records, etc.)
    ***REMOVED*** ... similar pattern


def downgrade():
    op.drop_table('burnout_predictions')
    ***REMOVED*** Drop other tables
```

---

***REMOVED******REMOVED*** 6. Ensemble Logic

***REMOVED******REMOVED******REMOVED*** 6.1 Weighted Ensemble Algorithm
...
class EnsembleBurnoutPredictor:
    ...
    def _ensemble_combine(
        ...
        ***REMOVED*** Confidence interval (±30% empirically calibrated)
    ...
    def _generate_interventions(
        ...
        ***REMOVED*** Risk-based interventions
    ...
    def _calculate_workload_adjustment(self, risk_level: str) -> float:
        ...
        adjustments = {
            "CRITICAL": -0.75,    ***REMOVED*** 75% reduction
            "HIGH": -0.50,        ***REMOVED*** 50% reduction
            "MODERATE": -0.25,    ***REMOVED*** 25% reduction
            "LOW": 0.0,           ***REMOVED*** No change
        }
...
***REMOVED******REMOVED******REMOVED*** 6.2 Confidence Interval Calculation
...
***REMOVED******REMOVED******REMOVED*** 7.1 Resilience Framework Integration

**Connect to existing resilience service:**

```python
***REMOVED*** In app/resilience/service.py
...
class ResilienceService:
    ...
    async def check_health(
        ...
        ***REMOVED*** ... existing health check logic

        ***REMOVED*** Add burnout risk assessment for all faculty
        burnout_risks = []
        ...
        ***REMOVED*** Add to report
        report.burnout_risks = burnout_risks

        ***REMOVED*** Generate additional recommendations
        if burnout_risks:
...
***REMOVED******REMOVED******REMOVED*** 7.2 Scheduling Constraints Integration

**Use predictions as soft constraints in scheduler:**

```python
***REMOVED*** In app/scheduling/constraints/burnout_protection.py
...
class BurnoutProtectionConstraint:
    ...
    def __init__(self, db: Session):
        ...
        self.weight = 100  ***REMOVED*** High priority
    ...
    async def evaluate(
        ...
        """
        ***REMOVED*** Get burnout prediction for this person
        prediction = await self.burnout_service.predict_burnout(assignment.person_id)

        ***REMOVED*** High penalty if assigning high-intensity work to high-risk person
        if prediction.risk_level == "CRITICAL":
            return 1000.0  ***REMOVED*** Strong penalty
        elif prediction.risk_level == "HIGH":
            ***REMOVED*** Scale penalty by workload intensity
        ...
        return 0.0  ***REMOVED*** No penalty for low-risk or low-intensity assignments
```

---

***REMOVED******REMOVED******REMOVED*** 7.3 Alert Integration

**Emit events for high-risk predictions:**

```python
***REMOVED*** In app/analytics/burnout_prediction/service.py
...
class BurnoutPredictionService:
    ...
    async def _emit_critical_alert(self, prediction: BurnoutPrediction):
        ...
        ***REMOVED*** Get person details
        person = await self.db.get(Person, prediction.person_id)

        ***REMOVED*** Send email to program leadership
...
***REMOVED******REMOVED******REMOVED*** 7.4 Dashboard Visualization
...
***REMOVED******REMOVED*** 8. Implementation Plan

***REMOVED******REMOVED******REMOVED*** Phase 1: Core Models (Weeks 1-2)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Symptom Tracking (Week 3)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Advanced Models (Week 4)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Service Integration (Week 5)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Validation & Refinement (Week 6)
...
***REMOVED******REMOVED******REMOVED*** Phase 6: Deployment (Week 7)
...
***REMOVED******REMOVED******REMOVED*** 9.1 Unit Tests

**Coverage Requirements**: ≥90% for all model classes

```python
***REMOVED*** backend/tests/analytics/burnout_prediction/test_sn_curve.py
...
class TestSNCurveModel:
    ...
    def test_stress_for_target_life(self, model):
        ...
        ***REMOVED*** 3-year residency = 156 weeks
    ...
    def test_parameter_calibration(self):
        ...
        config = BurnoutPredictionConfig(
            sn_sigma_f_prime=95.0,  ***REMOVED*** Personalized value
            sn_b=-0.15,             ***REMOVED*** More susceptible to fatigue
        )
        model = SNCurveModel(config)
...
***REMOVED******REMOVED******REMOVED*** 9.2 Integration Tests

```python
***REMOVED*** backend/tests/analytics/burnout_prediction/test_service_integration.py
...
class TestBurnoutPredictionServiceIntegration:
    ...
    async def test_predict_with_workload_history(self, db, sample_person):
        ...
        ***REMOVED*** Create workload history (3 months of data)
    ...
    async def test_predict_critical_risk(self, db, sample_person):
        ...
        ***REMOVED*** Create high-intensity workload history
    ...
    async def test_calibration_improves_accuracy(self, db, sample_person):
        ...
        ***REMOVED*** Record observed burnout event
        ...
        ***REMOVED*** Check that personal parameters were created
        ...
        assert cal.sigma_f_prime != 100.0  ***REMOVED*** Should be personalized
```

---

***REMOVED******REMOVED******REMOVED*** 9.3 Performance Tests

```python
***REMOVED*** backend/tests/performance/test_burnout_prediction_load.py
...
class TestBurnoutPredictionPerformance:
    ...
    async def test_prediction_latency(self, db, sample_person):
        ...
        ***REMOVED*** Measure prediction time
        start = time.time()
        prediction = await service.predict_burnout(sample_person.id)
    ...
    async def test_concurrent_predictions(self, db):
        ...
        ***REMOVED*** Create 50 people with data
        people = await self._create_test_people(db, count=50)

        ***REMOVED*** Predict concurrently
        start = time.time()
...
***REMOVED******REMOVED*** 10. Performance Requirements

***REMOVED******REMOVED******REMOVED*** 10.1 Latency Requirements
...
***REMOVED******REMOVED******REMOVED*** 10.2 Throughput Requirements
...
***REMOVED******REMOVED******REMOVED*** 10.3 Data Volume Assumptions
...
***REMOVED******REMOVED******REMOVED*** 10.4 Optimization Strategies
...
***REMOVED******REMOVED*** Appendix A: Sample Data for Testing

***REMOVED******REMOVED******REMOVED*** A.1 Workload History Sample

```python
***REMOVED*** backend/tests/fixtures/burnout_prediction.py
...
async def sample_workload_history(db, sample_person):
    ...
    ***REMOVED*** 3 months of varied workload
    workload_data = [
        ***REMOVED*** Month 1: Moderate intensity
        {"cycle_type": "clinic", "intensity": 60, "duration": 8, "unrecoverable": 0.05},
        {"cycle_type": "clinic", "intensity": 65, "duration": 8, "unrecoverable": 0.08},
        ***REMOVED*** ... repeat for 4 weeks

        ***REMOVED*** Month 2: Increasing intensity
        ...
        ***REMOVED*** Month 3: High intensity (risk building)
        {"cycle_type": "weekend_call", "intensity": 85, "duration": 28, "unrecoverable": 0.30},
        {"cycle_type": "night_float", "intensity": 75, "duration": 24, "unrecoverable": 0.20},
        {"cycle_type": "weekend_call", "intensity": 85, "duration": 28, "unrecoverable": 0.30},
        ***REMOVED*** ... repeat for 4 weeks
...
***REMOVED******REMOVED*** Appendix B: Calibration Procedure

***REMOVED******REMOVED******REMOVED*** B.1 Initial Population Calibration
...
async def calibrate_population_parameters(
    ...
    def objective(params):
        ...
        for event in burnout_events:
            ***REMOVED*** Predicted cycles using these parameters
            N_pred = (event.average_workload_percent / sigma_f_prime) ** (1 / b)

            ***REMOVED*** Actual cycles
            N_actual = event.cycles_to_burnout

            ***REMOVED*** Log-scale error (more robust for wide range of values)
            error = (np.log10(N_pred) - np.log10(N_actual)) ** 2
            errors.append(error)

        return np.sum(errors)

    ***REMOVED*** Initial guess (default parameters)
    x0 = [100.0, -0.12]

    ***REMOVED*** Bounds (reasonable ranges from materials science)
...
***REMOVED******REMOVED*** Appendix C: Validation Metrics

***REMOVED******REMOVED******REMOVED*** C.1 Model Validation Report Template
...
async def generate_validation_report(
    ...
    ***REMOVED*** Generate predictions for validation set
    predictions = []
    actuals = []

    for event in validation_set:
        ***REMOVED*** Predict burnout at T-12 weeks
        ...
        predictions.append(pred.ensemble_prediction_days / 7)  ***REMOVED*** Convert to weeks
        actuals.append(12)  ***REMOVED*** Actual was 12 weeks

    ***REMOVED*** Regression metrics
    mae = mean_absolute_error(actuals, predictions)
    rmse = np.sqrt(mean_squared_error(actuals, predictions))
    r2 = r2_score(actuals, predictions)

    ***REMOVED*** Classification metrics (binary: will burn out within 12 weeks?)
    y_true = [1] * len(validation_set)  ***REMOVED*** All did burn out
...
***REMOVED******REMOVED*** Document Metadata
