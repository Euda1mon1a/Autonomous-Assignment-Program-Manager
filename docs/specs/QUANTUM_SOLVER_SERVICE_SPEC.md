***REMOVED*** Quantum Scheduling Solver Service - Implementation Specification
...
1. [Service Architecture](***REMOVED***1-service-architecture)
2. [API Endpoints](***REMOVED***2-api-endpoints)
3. [Cloud Integration](***REMOVED***3-cloud-integration)
4. [QUBO Builder Module](***REMOVED***4-qubo-builder-module)
5. [Solver Selection Logic](***REMOVED***5-solver-selection-logic)
6. [Configuration](***REMOVED***6-configuration)
7. [Cost Management](***REMOVED***7-cost-management)
8. [Monitoring & Observability](***REMOVED***8-monitoring--observability)
9. [Deployment Architecture](***REMOVED***9-deployment-architecture)
10. [Testing Strategy](***REMOVED***10-testing-strategy)
11. [Migration Path](***REMOVED***11-migration-path)
12. [Performance Benchmarks](***REMOVED***12-performance-benchmarks)

---

***REMOVED******REMOVED*** 1. Service Architecture

***REMOVED******REMOVED******REMOVED*** 1.1 Overview
...
***REMOVED******REMOVED******REMOVED*** 1.2 Core Components

***REMOVED******REMOVED******REMOVED******REMOVED*** 1.2.1 QuantumSolverOrchestrator
...
class QuantumSolverOrchestrator:
    ...
    def _select_solver(
        ...
        budget: float
    ) -> str  ***REMOVED*** "dwave_hybrid", "aws_braket", "ibm_qaoa", "classical"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1.2.2 QUBOBuilder
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1.2.3 Cloud Quantum Clients
...
***REMOVED******REMOVED*** 2. API Endpoints

***REMOVED******REMOVED******REMOVED*** 2.1 POST /api/v1/quantum/qubo/formulate
...
async def formulate_qubo(
    ...
    ***REMOVED*** Build scheduling context
    ...
    ***REMOVED*** Analyze problem
    analyzer = ProblemAnalyzer()
    analysis = analyzer.analyze(qubo_matrix, context)

    ***REMOVED*** Store QUBO for download
...
***REMOVED******REMOVED******REMOVED*** 2.2 POST /api/v1/quantum/solve
...
async def solve_with_quantum(
    ...
    ***REMOVED*** Create orchestrator
    ...
    if request.async_mode:
        ***REMOVED*** Submit to background task
...
***REMOVED******REMOVED******REMOVED*** 2.3 GET /api/v1/quantum/job/{job_id}
...
async def get_job_status(
    ...
    ***REMOVED*** Check authorization (user can only view own jobs)
...
***REMOVED******REMOVED******REMOVED*** 2.4 POST /api/v1/quantum/simulate
...
async def simulate_quantum_solver(
    ...
    ***REMOVED*** Force classical solver
    ...
    return QuantumSolveResponse(
        ...
        objective_value=result.objective_value,
        runtime_seconds=result.runtime_seconds,
        cost_usd=0.0,  ***REMOVED*** Free simulation
        statistics=result.statistics
    )
```
...
***REMOVED******REMOVED*** 3. Cloud Integration

***REMOVED******REMOVED******REMOVED*** 3.1 D-Wave Leap Client
...
class DWaveClient:
    ...
    def _calculate_cost(self, info: dict) -> float:
        ...
        ***REMOVED*** D-Wave Leap pricing (as of 2025):
        ***REMOVED*** Hybrid: $0.00019 per second
        ***REMOVED*** QPU: $0.30 per task (1 second QPU time)

        if "qpu_access_time" in info.get("timing", {}):
            ***REMOVED*** QPU pricing
            qpu_time_us = info["timing"]["qpu_access_time"]
            tasks = max(1, qpu_time_us / 1_000_000)  ***REMOVED*** Convert us to seconds
            return tasks * 0.30
        else:
            ***REMOVED*** Hybrid pricing
            run_time_ms = info.get("run_time", 0)
            run_time_s = run_time_ms / 1000
            return run_time_s * 0.00019
```

***REMOVED******REMOVED******REMOVED*** 3.2 AWS Braket Client
...
class AWSBraketClient:
    ...
    async def solve_dwave(
        ...
        ***REMOVED*** Calculate cost (AWS Braket pricing varies by device)
    ...
    def _estimate_braket_cost(self, device, response) -> float:
        ...
        """
        base_cost = 0.30  ***REMOVED*** Per task
        shot_cost = response.info.get("num_reads", 1000) * 0.00019
        return base_cost + shot_cost
```

***REMOVED******REMOVED******REMOVED*** 3.3 IBM Quantum Client
...
class IBMQuantumClient:
    ...
    async def solve_qaoa(
        ...
        ***REMOVED*** Convert QUBO to Hamiltonian
        hamiltonian = self._qubo_to_hamiltonian(Q)

        ***REMOVED*** Select backend
        backend_service = self.service.backend(backend)

        ***REMOVED*** Create QAOA instance
        ...
        ***REMOVED*** Extract solution
        ...
        return IBMSolution(
            ...
            cost_usd=0.0  ***REMOVED*** IBM Quantum free tier for research
        )
    ...
    def _qubo_to_hamiltonian(self, Q: dict) -> PauliSumOp:
        ...
        for (i, j), coef in Q.items():
            if i == j:
                ***REMOVED*** Single-qubit term
                hamiltonian += coef * Z(i)
            else:
                ***REMOVED*** Two-qubit term
...
***REMOVED******REMOVED*** 4. QUBO Builder Module

***REMOVED******REMOVED******REMOVED*** 4.1 Architecture
...
***REMOVED******REMOVED******REMOVED*** 4.2 Core Implementation

```python
@dataclass
class PenaltyConfig:
    """Configuration for QUBO penalty weights."""
    unavailability: float = 10000.0      ***REMOVED*** Hard: Cannot assign during absence
    block_capacity: float = 1000.0       ***REMOVED*** Hard: One resident per block
    acgme_80_hour: float = 500.0         ***REMOVED*** Hard: 80-hour rule compliance
    acgme_1_in_7: float = 300.0          ***REMOVED*** Hard: 1-in-7 days off
    supervision_ratio: float = 200.0     ***REMOVED*** Hard: Faculty supervision ratios
    equity: float = 10.0                 ***REMOVED*** Soft: Workload balance
    template_balance: float = 5.0        ***REMOVED*** Soft: Rotation diversity
    coverage: float = -1.0               ***REMOVED*** Objective: Maximize assignments


@dataclass
class QUBOMatrix:
    """QUBO matrix representation."""
    Q: dict[tuple[int, int], float]      ***REMOVED*** Sparse matrix
    num_variables: int
    num_nonzero_terms: int
    variable_map: dict[tuple[int, int, int], int]  ***REMOVED*** (r, b, t) -> index
    index_to_var: dict[int, tuple[int, int, int]]  ***REMOVED*** index -> (r, b, t)
    metadata: dict
...
class QUBOBuilder:
    ...
    def build(
        ...
        """
        ***REMOVED*** Initialize empty QUBO
        Q = {}

        ***REMOVED*** Build variable mapping
        ...
        ***REMOVED*** Add objective: maximize coverage
        self._add_coverage_objective(Q, var_map)

        ***REMOVED*** Add hard constraints
    ...
    def _build_variable_mapping(
        ...
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Check availability if eliminating infeasible
                if eliminate_infeasible:
                    unavail = context.availability.get(resident.id, set())
                    if b_i in unavail:
                        continue  ***REMOVED*** Skip unavailable blocks

                for template in context.templates:
                    ***REMOVED*** Skip procedure templates if resident unqualified
                    if template.requires_procedure_credential:
                        ***REMOVED*** Check credentials here
                        continue

                    t_i = context.template_idx[template.id]

                    ***REMOVED*** Create mapping
    ...
    def _add_block_capacity_constraint(
        ...
        """
        ***REMOVED*** Group variables by block
        ...
        for b_i, indices in block_vars.items():
            ...
            ***REMOVED*** Quadratic penalty: discourage multiple assignments
            for i, idx1 in enumerate(indices):
                for idx2 in indices[i + 1:]:
                    Q[(idx1, idx2)] = Q.get((idx1, idx2), 0) + penalty

            ***REMOVED*** Linear penalty: encourage at least one assignment
            for idx in indices:
    ...
    def _add_80_hour_constraint(
        ...
        """
        HOURS_PER_BLOCK = 6
        MAX_BLOCKS_PER_WEEK = int(80 / HOURS_PER_BLOCK)  ***REMOVED*** 13
        ...
        ***REMOVED*** Group blocks by week
        ...
        for resident in context.residents:
            ...
            for week, blocks in week_blocks.items():
                ***REMOVED*** Get all variables for this resident in this week
                week_vars = []
                for block in blocks:
                    b_i = context.block_idx[block.id]
                    for template in context.templates:
                ...
                ***REMOVED*** Quadratic penalty for exceeding limit
                ***REMOVED*** Approximation: penalize pairs beyond threshold
    ...
    def _add_equity_objective(
        ...
        """
        ***REMOVED*** Group variables by resident
        ...
        for r_i, indices in resident_vars.items():
            ***REMOVED*** Quadratic penalty within same resident
            ***REMOVED*** This discourages one resident from getting too many assignments
    ...
    def decompose(
        ...
        """
        ***REMOVED*** Implementation deferred to Phase 1 of roadmap
        pass
```

***REMOVED******REMOVED******REMOVED*** 4.3 Penalty Tuning Strategy
...
***REMOVED******REMOVED*** 5. Solver Selection Logic

***REMOVED******REMOVED******REMOVED*** 5.1 Decision Tree
...
class QuantumSolverOrchestrator:
    ...
    def _select_solver(
        ...
        if num_vars < 100:
            return "classical_exact"  ***REMOVED*** CP-SAT can find optimal
...
***REMOVED******REMOVED******REMOVED*** 5.2 Cost-Benefit Analysis
...
class CostBenefitAnalysis:
    ...
    expected_quality: float  ***REMOVED*** 0-1, where 1 = optimal
    confidence: str  ***REMOVED*** "high", "medium", "low"
...
class CostBenefitAnalyzer:
    ...
    def _estimate_dwave_hybrid_cost(self, problem_size: ProblemSize) -> float:
        ...
        """
        ***REMOVED*** Empirical formula based on benchmarks
        num_vars = problem_size.num_variables
        estimated_runtime = 10 + (num_vars / 1000) * 5  ***REMOVED*** 10s base + 5s per 1000 vars
        return estimated_runtime * 0.00019
```

---

***REMOVED******REMOVED*** 6. Configuration

***REMOVED******REMOVED******REMOVED*** 6.1 Environment Variables

**Required:**

```bash
***REMOVED*** Quantum Service API Keys (at least one required)
...
```bash
***REMOVED*** Solver Preferences
QUANTUM_DEFAULT_SOLVER=auto  ***REMOVED*** auto, dwave_hybrid, classical_sa
QUANTUM_ENABLE_QPU=false     ***REMOVED*** Enable raw QPU access (advanced)
QUANTUM_ENABLE_QAOA=false    ***REMOVED*** Enable IBM QAOA (experimental)

***REMOVED*** Performance Tuning
QUANTUM_DECOMPOSE_THRESHOLD=10000  ***REMOVED*** Variables before decomposition
QUANTUM_TIMEOUT_SECONDS=300
QUANTUM_NUM_READS=1000

***REMOVED*** Monitoring
QUANTUM_ENABLE_TELEMETRY=true
QUANTUM_LOG_QUBO_MATRICES=false  ***REMOVED*** Large output, only for debugging
```

***REMOVED******REMOVED******REMOVED*** 6.2 Configuration Class
...
***REMOVED******REMOVED*** 7. Cost Management

***REMOVED******REMOVED******REMOVED*** 7.1 Budget Tracking
...
class QuantumCostTracker:
    ...
    async def record_job_cost(
        ...
        ***REMOVED*** Store in Redis
        key = f"quantum_cost:{job_id}"
        await self.redis.setex(
            key,
            86400 * 30,  ***REMOVED*** 30 day TTL
            json.dumps({
                "job_id": job_id,
                "solver": solver,
                "cost_usd": cost_usd,
...
***REMOVED******REMOVED******REMOVED*** 7.2 Cost Dashboard
...
***REMOVED******REMOVED******REMOVED*** 8.1 Metrics

**Prometheus Metrics:**

```python
***REMOVED*** backend/app/scheduling/quantum/metrics.py
...
***REMOVED******REMOVED******REMOVED*** 8.2 Logging
...
***REMOVED******REMOVED******REMOVED*** 8.3 Alerting
...
***REMOVED******REMOVED*** 9. Deployment Architecture

***REMOVED******REMOVED******REMOVED*** 9.1 Docker Compose
...
services:
  backend:
    build: ./backend
    environment:
      ***REMOVED*** Quantum API Keys (from secrets)
      - DWAVE_API_TOKEN=${DWAVE_API_TOKEN}
      - AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID}
      - AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY}
      - IBM_QUANTUM_TOKEN=${IBM_QUANTUM_TOKEN}

      ***REMOVED*** Quantum Configuration
...
***REMOVED******REMOVED******REMOVED*** 9.2 Kubernetes Deployment (Optional)
...
***REMOVED******REMOVED*** 10. Testing Strategy

***REMOVED******REMOVED******REMOVED*** 10.1 Unit Tests
...
class TestQUBOBuilder:
    ...
    def test_coverage_objective(self, scheduling_context):
        ...
        ***REMOVED*** All diagonal terms should include -1.0
        for i in range(qubo.num_variables):
            assert qubo.Q[(i, i)] <= 0  ***REMOVED*** Coverage is negative
    ...
    def test_block_capacity_constraint(self, scheduling_context):
        ...
        ***REMOVED*** Find two variables for same block
        ...
        ***REMOVED*** There should be quadratic penalty between pairs
    ...
    def test_variable_elimination(self, scheduling_context):
        ...
        ***REMOVED*** Add some unavailability
        ...
        ***REMOVED*** Build with elimination
        builder = QUBOBuilder()
        qubo = builder.build(scheduling_context, eliminate_infeasible=True)

        ***REMOVED*** Verify eliminated variables not in mapping
...
***REMOVED******REMOVED******REMOVED*** 10.2 Integration Tests
...
class TestDWaveClient:
    ...
    async def test_hybrid_solver_small_problem(self):
        ...
        ***REMOVED*** Simple 4-variable QUBO
        Q = {
        ...
        assert solution.energy < 0  ***REMOVED*** Should find negative energy
        assert solution.cost_usd > 0  ***REMOVED*** Should have non-zero cost
    ...
    async def test_cost_tracking(self):
        ...
        ***REMOVED*** Verify cost is reasonable (should be < $1 for 5 second job)
        assert 0 < solution.cost_usd < 1.0
```

***REMOVED******REMOVED******REMOVED*** 10.3 End-to-End Tests
...
class TestQuantumSolverAPI:
    ...
    async def test_formulate_solve_workflow(self, client, auth_headers):
        ...
        ***REMOVED*** Step 2: Solve (async)
        solve_request = {
            **formulate_request,
            "solver": "classical_sa",  ***REMOVED*** Use free solver for testing
            "async": True
        }
        ...
        ***REMOVED*** Step 3: Poll until complete
        max_polls = 60
...
***REMOVED******REMOVED*** 11. Migration Path

***REMOVED******REMOVED******REMOVED*** 11.1 Phase 1: Classical QUBO Foundation (Months 1-3)
...
***REMOVED******REMOVED******REMOVED*** 11.2 Phase 2: D-Wave Cloud Integration (Months 4-6)
...
***REMOVED******REMOVED******REMOVED*** 11.3 Phase 3: Advanced Quantum Features (Months 7-9)
...
***REMOVED******REMOVED*** 12. Performance Benchmarks

***REMOVED******REMOVED******REMOVED*** 12.1 Target Performance
...
***REMOVED******REMOVED******REMOVED*** 12.2 Quality Metrics
...
***REMOVED******REMOVED******REMOVED*** 12.3 Cost Targets
...
***REMOVED******REMOVED*** Appendix A: QUBO Matrix Example
...
```python
Q = {
    ***REMOVED*** Coverage objective (diagonal)
    (0,0): -1, (1,1): -1, (2,2): -1, (3,3): -1,
    (4,4): -1, (5,5): -1, (6,6): -1, (7,7): -1,

    ***REMOVED*** Block capacity constraint (quadratic penalties)
    (0,4): 1000, (1,5): 1000,  ***REMOVED*** Block 0 conflicts
    (2,6): 1000, (3,7): 1000,  ***REMOVED*** Block 1 conflicts
}
```
...
***REMOVED******REMOVED******REMOVED*** Research Papers
...
***REMOVED******REMOVED******REMOVED*** Cloud Platform Documentation
