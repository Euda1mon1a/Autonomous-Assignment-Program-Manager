***REMOVED*** Ecological Resilience Service - Implementation Specification
...
3. [Database Schema](***REMOVED***database-schema)
4. [API Endpoints](***REMOVED***api-endpoints)
5. [Analysis Modules](***REMOVED***analysis-modules)
6. [Integration Points](***REMOVED***integration-points)
7. [Implementation Phases](***REMOVED***implementation-phases)
8. [Testing Requirements](***REMOVED***testing-requirements)
9. [Monitoring & Alerts](***REMOVED***monitoring--alerts)
10. [Performance Specifications](***REMOVED***performance-specifications)
...
***REMOVED******REMOVED******REMOVED*** Technical Foundation
...
***REMOVED******REMOVED******REMOVED*** Expected Impact
...
***REMOVED******REMOVED******REMOVED*** Component Hierarchy
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Main Service Class

```python
***REMOVED*** File: backend/app/resilience/ecological_resilience.py
...
class AdaptiveCyclePhase(str, Enum):
    ...
    EXPLOITATION = "exploitation"        ***REMOVED*** r: Growth, innovation, low capital
    CONSERVATION = "conservation"        ***REMOVED*** K: Mature, high capital, rigid
    RELEASE = "release"                  ***REMOVED*** Ω: Crisis, rapid destabilization
    REORGANIZATION = "reorganization"    ***REMOVED*** α: Renewal, experimentation
    TRANSITION = "transition"            ***REMOVED*** Between phases


class ScaleLevel(str, Enum):
    """Panarchy scale hierarchy."""
    INDIVIDUAL = "individual"      ***REMOVED*** Days-weeks (residents/faculty)
    TEAM = "team"                  ***REMOVED*** Weeks-months (rotations, call teams)
    DEPARTMENTAL = "departmental"  ***REMOVED*** Months-years (program, schedule templates)
    INSTITUTIONAL = "institutional" ***REMOVED*** Years-decades (medical center, accreditation)


class DangerLevel(str, Enum):
    """Tipping point urgency classification."""
    MONITOR = "monitor"           ***REMOVED*** Early signals, watch trends
    PREPARE = "prepare"           ***REMOVED*** Multiple signals, contingency planning
    IMMEDIATE = "immediate"       ***REMOVED*** Near threshold, intervention required
...
class AdaptiveCycleMetrics:
    ...
    ***REMOVED*** Core dimensions (0.0-1.0)
    potential: float                    ***REMOVED*** Accumulated capital (expertise, relationships)
    connectedness: float                ***REMOVED*** System coupling strength
    resilience: float                   ***REMOVED*** Disturbance absorption capacity

    ***REMOVED*** Phase classification
    current_phase: AdaptiveCyclePhase
    phase_duration: timedelta          ***REMOVED*** How long in current phase
    phase_stability: float             ***REMOVED*** 0-1: Confidence in classification

    ***REMOVED*** Transition risk
    omega_risk: float                  ***REMOVED*** 0-1: Probability of entering Ω
    early_warning_signals: list[str] = field(default_factory=list)

    ***REMOVED*** Historical tracking
    time_since_last_omega: Optional[timedelta] = None
    previous_phase: Optional[AdaptiveCyclePhase] = None

    ***REMOVED*** Component metrics
...
class PanarchyState:
    ...
    ***REMOVED*** Cross-scale connections
    revolt_risk: float                  ***REMOVED*** 0-1: Bottom-up cascade probability
    memory_strength: float              ***REMOVED*** 0-1: Top-down stabilization

    ***REMOVED*** Synchronization detection
    synchronized_scales: list[ScaleLevel] = field(default_factory=list)
    cascade_warning: bool = False

    ***REMOVED*** Stability assessment
    overall_stability: float            ***REMOVED*** 0-1: Composite stability
...
class TippingPointAlert:
    ...
    distance_to_threshold: float       ***REMOVED*** 0-1, lower = closer

    ***REMOVED*** Early warning signals detected
    critical_slowing: bool
    increased_variance: bool
    skewness_shift: bool

    ***REMOVED*** Confidence and urgency
    confidence: float                   ***REMOVED*** 0-1
    urgency: DangerLevel

    ***REMOVED*** Intervention guidance
    intervention: str
    reversibility: str                  ***REMOVED*** "reversible", "limited_window", "irreversible"
...
class DiversityMetrics:
    ...
    ***REMOVED*** Basic diversity indices
    species_richness: int              ***REMOVED*** Total unique skills/specialties
    shannon_index: float               ***REMOVED*** Weighted diversity (1.5-3.5 typical)
    simpson_index: float               ***REMOVED*** Probability of different skills (0-1)
    evenness: float                    ***REMOVED*** Distribution uniformity (0-1)

    ***REMOVED*** Functional diversity
    functional_richness: int           ***REMOVED*** Number of functional groups
    response_diversity: float          ***REMOVED*** Avg substitutes per specialty

    ***REMOVED*** Stability prediction
    portfolio_effect: float            ***REMOVED*** Variance reduction from diversity (>1 = stabilizing)
    stability_score: float             ***REMOVED*** 0-1: Composite stability prediction
...
class EcologicalResilienceReport:
    ...
    ***REMOVED*** Adaptive cycle state
    adaptive_cycle: AdaptiveCycleMetrics

    ***REMOVED*** Panarchy (multi-scale)
    panarchy: PanarchyState

    ***REMOVED*** Tipping points
    ...
    ***REMOVED*** Overall assessment
    overall_risk: str                   ***REMOVED*** "low", "moderate", "high", "critical"
    priority_interventions: list[str] = field(default_factory=list)

    ***REMOVED*** Integration with existing framework
...
class EcologicalResilienceService:
    ...
    async def analyze_system_resilience(
        ...
        ***REMOVED*** Default to 90-day lookback
        if period_end is None:
            period_end = datetime.utcnow()
        if period_start is None:
            period_start = period_end - timedelta(days=90)

        ***REMOVED*** Run all analyses in parallel (async)
        ...
        ***REMOVED*** Aggregate overall risk
        ...
        ***REMOVED*** Generate priority interventions
        ...
        ***REMOVED*** Integration recommendations
    ...
    def _calculate_overall_risk(
        ...
        ***REMOVED*** Diversity contribution
        risk_score += (1.0 - diversity.stability_score) * 1.5

        ***REMOVED*** Map to categorical risk
        if risk_score < 2.0:
            return "low"
        elif risk_score < 4.0:
            return "moderate"
    ...
    def _generate_interventions(
        ...
        ***REMOVED*** Phase-specific interventions
        ...
        ***REMOVED*** Panarchy interventions
        if panarchy.cascade_warning:
            interventions.append(
    ...
    def _map_to_defense_level(
        ...
        ***REMOVED*** Ω phase = RED/BLACK
        if adaptive_cycle.current_phase == AdaptiveCyclePhase.RELEASE:
            return "RED"

        ***REMOVED*** Risk-based mapping
        risk_to_defense = {
            "low": "GREEN",
            "moderate": "YELLOW",
            "high": "ORANGE",
    ...
    async def _persist_report(self, report: EcologicalResilienceReport):
        ...
        ***REMOVED*** Import models here to avoid circular imports
        ...
        ***REMOVED*** Save adaptive cycle history
        ...
        ***REMOVED*** Save diversity snapshot
        ...
        ***REMOVED*** Save tipping point alerts
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Adaptive Cycle Tracker

```python
***REMOVED*** File: backend/app/resilience/adaptive_cycle.py
...
class AdaptiveCycleTracker:
    ...
    async def calculate_current_state(
        ...
        ***REMOVED*** Calculate three core dimensions
        ...
        ***REMOVED*** Phase stability (confidence)
    ...
    async def _calculate_potential(
        ...
        ***REMOVED*** 1. Competency progress (procedure counts)
        ***REMOVED*** Query procedure counts per resident
        ***REMOVED*** High counts = high potential
        ***REMOVED*** Normalize to 0-1 (e.g., 100+ procedures = 1.0)
        competency_score = 0.0  ***REMOVED*** TODO: Implement from procedure_credential model
        scores.append(competency_score)

        ***REMOVED*** 2. Mentorship stability
        ***REMOVED*** Query assignment stability (same faculty-resident pairings)
        ***REMOVED*** High stability = high potential
        ...
        ***REMOVED*** Calculate pairing stability (simplified)
        if len(assignments) > 0:
            unique_pairings = len(set(
                (a.person_id, a.rotation_template_id) for a in assignments
            ))
            mentorship_score = min(1.0, unique_pairings / 50.0)  ***REMOVED*** 50+ = stable
            scores.append(mentorship_score)

        ***REMOVED*** 3. Schedule template maturity
        ***REMOVED*** Days since last major schedule change
        ***REMOVED*** More mature = higher potential
        template_maturity = 0.7  ***REMOVED*** Placeholder (would query schedule_run model)
        scores.append(template_maturity)

        ***REMOVED*** 4. Documentation completeness
        ***REMOVED*** Count of documented protocols, policies
        documentation_score = 0.6  ***REMOVED*** Placeholder
        scores.append(documentation_score)

        ***REMOVED*** Average all components
    ...
    async def _calculate_connectedness(
        ...
        ***REMOVED*** 1. Network density
        ***REMOVED*** High density = high connectedness = rigid
        ***REMOVED*** Would use NetworkX to calculate from assignment graph
        network_density = 0.5  ***REMOVED*** Placeholder
        scores.append(network_density)

        ***REMOVED*** 2. Faculty substitutability
        ***REMOVED*** Low substitutability = high connectedness
        ***REMOVED*** Herfindahl index: Σ(market_share²)
        ***REMOVED*** High HHI = concentrated = rigid
        faculty_query = select(Person).where(Person.role == "FACULTY")
        faculty = (await self.db.execute(faculty_query)).scalars().all()

        if len(faculty) > 1:
            ***REMOVED*** Equal distribution = low connectedness
            ***REMOVED*** Concentrated = high connectedness
            equal_share = 1.0 / len(faculty)
            hhi = len(faculty) * (equal_share ** 2)  ***REMOVED*** Min HHI (uniform)
            max_hhi = 1.0  ***REMOVED*** All in one person
            ***REMOVED*** Normalize: 0 = perfectly distributed, 1 = monopoly
            substitutability_score = hhi / max_hhi
            scores.append(substitutability_score)

        ***REMOVED*** 3. Cross-coverage dependencies
        ***REMOVED*** Count of assignments requiring specific faculty
        dependency_score = 0.6  ***REMOVED*** Placeholder
        scores.append(dependency_score)

        ***REMOVED*** 4. Single points of failure (inverse)
        ***REMOVED*** Fewer SPOFs = lower connectedness
        spof_count = 3  ***REMOVED*** Placeholder (from hub_analysis)
        spof_score = min(1.0, spof_count / 10.0)  ***REMOVED*** 10+ SPOFs = max
    ...
    async def _calculate_resilience(
        ...
        ***REMOVED*** 1. Utilization buffer
        ***REMOVED*** Low utilization = high resilience
        utilization_rate = 0.65  ***REMOVED*** Placeholder (from utilization monitor)
        buffer_score = 1.0 - utilization_rate
        scores.append(buffer_score)

        ***REMOVED*** 2. Contingency pass rate
        ***REMOVED*** Pass N-1 and N-2 = high resilience
        n1_pass = True  ***REMOVED*** Placeholder (from contingency analyzer)
        n2_pass = False
        contingency_score = (1.0 if n1_pass else 0.0) + (1.0 if n2_pass else 0.0)
        contingency_score /= 2.0  ***REMOVED*** Normalize
        scores.append(contingency_score)

        ***REMOVED*** 3. Diversity indices
        ***REMOVED*** High diversity = high resilience
        shannon_index = 2.3  ***REMOVED*** Placeholder (from diversity analyzer)
        ***REMOVED*** Shannon typically 1.5-3.5, normalize
        diversity_score = min(1.0, shannon_index / 3.5)
        scores.append(diversity_score)

        ***REMOVED*** 4. Response diversity
        ***REMOVED*** Multiple ways to cover each service = high resilience
        response_diversity = 2.5  ***REMOVED*** Placeholder (avg substitutes per specialty)
        response_score = min(1.0, response_diversity / 5.0)  ***REMOVED*** 5+ = excellent
    ...
    def _classify_phase(
        ...
        ***REMOVED*** K: High potential AND high connectedness
        if potential > HIGH and connectedness > HIGH:
            return AdaptiveCyclePhase.CONSERVATION

        ***REMOVED*** Ω: Low potential AND low connectedness AND low resilience
        elif potential < LOW and connectedness < LOW and resilience < 0.4:
            return AdaptiveCyclePhase.RELEASE

        ***REMOVED*** α: Low potential AND low connectedness BUT moderate+ resilience
        elif potential < LOW and connectedness < LOW and resilience >= 0.4:
            return AdaptiveCyclePhase.REORGANIZATION

        ***REMOVED*** r: Growing potential, moderate connectedness, good resilience
    ...
    def _calculate_omega_risk(
        ...
        if phase != AdaptiveCyclePhase.CONSERVATION:
            ***REMOVED*** Only K phase has significant Ω risk
            return 0.0

        ***REMOVED*** Risk components
        rigidity_risk = connectedness  ***REMOVED*** Higher connectedness = higher risk
        brittleness_risk = 1.0 - resilience  ***REMOVED*** Lower resilience = higher risk

        ***REMOVED*** Combine (weighted)
    ...
    async def _detect_early_warnings(
        ...
        ***REMOVED*** Would integrate with tipping_point_detector for:
        ***REMOVED*** - Critical slowing down
        ***REMOVED*** - Increased variance
        ***REMOVED*** - Decreased resilience
    ...
    async def _get_previous_state(self) -> Optional[AdaptiveCycleMetrics]:
        ...
        ***REMOVED*** Query CyclePhaseHistory model
        ***REMOVED*** Return most recent record
        return None  ***REMOVED*** Placeholder
    ...
    async def _calculate_phase_duration(
        ...
        ***REMOVED*** Query CyclePhaseHistory for phase changes
        ***REMOVED*** Count days since last transition
        return timedelta(days=30)  ***REMOVED*** Placeholder

    async def _time_since_last_omega(self) -> Optional[timedelta]:
        """Calculate time since last Ω (crisis) event."""
        ***REMOVED*** Query CyclePhaseHistory for last RELEASE phase
        return timedelta(days=180)  ***REMOVED*** Placeholder
    ...
    def _calculate_phase_stability(
        ...
        """
        ***REMOVED*** Distance from phase boundaries
        ***REMOVED*** Clear K: P>0.8, C>0.8 → high stability
        ***REMOVED*** Near boundary: P≈0.7, C≈0.7 → low stability

        if phase == AdaptiveCyclePhase.CONSERVATION:
            ***REMOVED*** How far from K→Ω boundary?
            k_strength = min(potential, connectedness)
            return k_strength  ***REMOVED*** 0.9 = very K, 0.7 = borderline

        elif phase == AdaptiveCyclePhase.RELEASE:
            ***REMOVED*** In crisis, stability is low by definition
            return 0.3

        elif phase == AdaptiveCyclePhase.REORGANIZATION:
            ***REMOVED*** α phase is inherently uncertain
            return 0.5

        elif phase == AdaptiveCyclePhase.EXPLOITATION:
            ***REMOVED*** r phase: growing but not yet mature
            return 0.7

        else:  ***REMOVED*** TRANSITION
            return 0.4  ***REMOVED*** Low confidence in transitional states
```
...
***REMOVED******REMOVED******REMOVED*** New Models

Add to `/backend/app/models/resilience.py`:

```python
***REMOVED*** Ecological Resilience Models
...
class CyclePhaseHistory(Base):
    ...
    ***REMOVED*** Adaptive cycle position
    phase = Column(
        String(30), nullable=False
    )  ***REMOVED*** exploitation, conservation, release, reorganization
    potential = Column(Float, nullable=False)  ***REMOVED*** 0-1: Accumulated capital
    connectedness = Column(Float, nullable=False)  ***REMOVED*** 0-1: System coupling
    resilience = Column(Float, nullable=False)  ***REMOVED*** 0-1: Disturbance absorption

    ***REMOVED*** Risk assessment
    omega_risk = Column(Float, default=0.0)  ***REMOVED*** 0-1: Probability of crisis
    early_warning_count = Column(Integer, default=0)

    ***REMOVED*** Phase metadata
    phase_duration_days = Column(Integer)  ***REMOVED*** Days in current phase
    phase_stability = Column(Float)  ***REMOVED*** 0-1: Classification confidence

    ***REMOVED*** Component scores (detailed breakdown)
...
class PanarchyStateRecord(Base):
    ...
    ***REMOVED*** Cross-scale connections
    revolt_risk = Column(Float, default=0.0)  ***REMOVED*** 0-1: Bottom-up cascade
    memory_strength = Column(Float, default=0.0)  ***REMOVED*** 0-1: Top-down stability

    ***REMOVED*** Synchronization
    synchronized_scales = Column(StringArrayType())  ***REMOVED*** Scales in same phase
    cascade_warning = Column(Boolean, default=False)

    ***REMOVED*** Stability
    overall_stability = Column(Float)  ***REMOVED*** 0-1: Composite
    weakest_scale = Column(String(30))  ***REMOVED*** individual, team, etc.
...
class DiversitySnapshot(Base):
    ...
    ***REMOVED*** Basic diversity indices
    species_richness = Column(Integer, nullable=False)  ***REMOVED*** Unique skills count
    shannon_index = Column(Float, nullable=False)  ***REMOVED*** 1.5-3.5 typical
    simpson_index = Column(Float, nullable=False)  ***REMOVED*** 0-1
    evenness = Column(Float, nullable=False)  ***REMOVED*** 0-1: Distribution uniformity

    ***REMOVED*** Functional diversity
    functional_richness = Column(Integer, nullable=False)  ***REMOVED*** Functional groups
    response_diversity = Column(Float, nullable=False)  ***REMOVED*** Avg substitutes

    ***REMOVED*** Stability metrics
    portfolio_effect = Column(Float, nullable=False)  ***REMOVED*** >1 = stabilizing
    stability_score = Column(Float, nullable=False)  ***REMOVED*** 0-1: Composite
    ...
    ***REMOVED*** Detailed breakdown
    skill_distribution = Column(JSONType())  ***REMOVED*** {skill: count}
    specialty_coverage = Column(JSONType())  ***REMOVED*** {specialty: [faculty_ids]}
...
class TippingPointAlertRecord(Base):
    ...
    ***REMOVED*** Metric being monitored
    metric_name = Column(String(100), nullable=False)
    current_value = Column(Float, nullable=False)
    threshold_estimate = Column(Float, nullable=False)
    distance_to_threshold = Column(Float, nullable=False)  ***REMOVED*** 0-1

    ***REMOVED*** Early warning signals
    ...
    ***REMOVED*** Assessment
    confidence = Column(Float, nullable=False)  ***REMOVED*** 0-1
    urgency = Column(
        String(20), nullable=False
    )  ***REMOVED*** monitor, prepare, immediate

    ***REMOVED*** Guidance
    intervention = Column(String(1000), nullable=False)
    reversibility = Column(
        String(30), nullable=False
    )  ***REMOVED*** reversible, limited_window, irreversible
    estimated_time_to_threshold_days = Column(Integer)

    ***REMOVED*** Historical trend
...
class RecoveryPathRecord(Base):
    ...
    ***REMOVED*** Recovery context
    crisis_event_id = Column(GUID())  ***REMOVED*** Related ResilienceEvent
    from_phase = Column(String(30), nullable=False)  ***REMOVED*** Usually "release" (Ω)
    to_phase_target = Column(String(30), nullable=False)  ***REMOVED*** Usually "exploitation" (r)

    ***REMOVED*** Experimental templates tried
    ...
    ***REMOVED*** Convergence metrics
    coverage_rate = Column(Float)  ***REMOVED*** Current coverage
    target_coverage = Column(Float, default=0.85)  ***REMOVED*** Goal
    resident_acceptance = Column(Float)  ***REMOVED*** 0-1: Survey results
    faculty_acceptance = Column(Float)  ***REMOVED*** 0-1: Survey results
    acgme_compliance = Column(Boolean, default=True)

    ***REMOVED*** Progress
    convergence_score = Column(Float)  ***REMOVED*** 0-1: How close to stable r
    is_converged = Column(Boolean, default=False)

    ***REMOVED*** Completion
    completed_at = Column(DateTime)
    final_phase_reached = Column(String(30))
    success = Column(Boolean)  ***REMOVED*** True = reached r, False = stuck in α
...
***REMOVED******REMOVED******REMOVED*** Alembic Migration

```python
***REMOVED*** File: backend/alembic/versions/{timestamp}_add_ecological_resilience.py

"""Add ecological resilience models

Revision ID: {generated_by_alembic}
Revises: {previous_migration_id}
Create Date: {generated_by_alembic}
"""

***REMOVED*** Note: Actual values will be generated by Alembic when running:
***REMOVED*** alembic revision --autogenerate -m "Add ecological resilience models"
...
revision = 'XXXX'
down_revision = 'YYYY'  ***REMOVED*** Latest migration
branch_labels = None
depends_on = None


def upgrade():
    ***REMOVED*** Create cycle_phase_history table
    op.create_table(
        'cycle_phase_history',
    ...
    ***REMOVED*** Create index on timestamp for efficient queries
    op.create_index(
    ...
    ***REMOVED*** Create panarchy_states table
    op.create_table(
        'panarchy_states',
        sa.Column('id', GUID(), primary_key=True),
        sa.Column('timestamp', sa.DateTime, nullable=False),
    ...
    ***REMOVED*** Create diversity_snapshots table
    op.create_table(
        'diversity_snapshots',
        sa.Column('id', GUID(), primary_key=True),
    ...
    ***REMOVED*** Create tipping_point_alerts table
    op.create_table(
    ...
    ***REMOVED*** Create recovery_paths table
    op.create_table(
        'recovery_paths',
        sa.Column('id', GUID(), primary_key=True),
    ...
    **Use Cases:**
    ...
    return [
        AdaptiveCycleResponse(
            ...
            early_warning_signals=[],  ***REMOVED*** Not stored in history
    ...
    **Use Cases:**
    ...
    return [
        DiversityMetricsResponse(
            ...
            low_diversity_alerts=[],  ***REMOVED*** Reconstruct from flags
            monoculture_risks={},
...
class EcologicalResilienceReportResponse(BaseModel):
    ...
    def from_domain(cls, report):
        ...
        ***REMOVED*** Implementation would map domain objects to Pydantic models
        ***REMOVED*** Simplified for specification
        pass
```

---

***REMOVED******REMOVED*** Analysis Modules

***REMOVED******REMOVED******REMOVED*** Module Implementations (Continued in next section due to length)
...
***REMOVED******REMOVED******REMOVED*** 1. Resilience Service Integration

```python
***REMOVED*** Add to backend/app/resilience/service.py
...
***REMOVED******REMOVED******REMOVED*** 3. SPC Integration

```python
***REMOVED*** Feed ecological variance signals to SPC monitoring

from app.resilience.spc_monitoring import WorkloadControlChart

***REMOVED*** Tipping point "flickering" detection feeds SPC variance tracking
if tipping_alert.increased_variance:
    spc_chart.investigate_special_cause_variation()
```

***REMOVED******REMOVED******REMOVED*** 4. Fire Index Integration

```python
***REMOVED*** Multi-temporal alignment

from app.resilience.burnout_fire_index import BurnoutDangerRating

***REMOVED*** Adaptive cycle potential aligns with FWI buildup index (BUI)
***REMOVED*** K phase high potential = high BUI = accumulated fuel
```
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Foundation (Months 1-3)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Tipping Point Detection (Months 4-6)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Recovery Dynamics (Months 7-9)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Production Integration (Months 10-12)
...
```python
***REMOVED*** Example: tests/resilience/test_adaptive_cycle.py
...
class TestAdaptiveCycleTracker:
    ...
    async def test_classify_r_phase(self, db):
        ...
        ***REMOVED*** Mock conditions: Low P, Low C, High R
        ***REMOVED*** This should classify as r (growth/exploitation)
    ...
    async def test_classify_k_phase(self, db):
        ...
        ***REMOVED*** Mock conditions: High P, High C (mature/rigid)
        phase = tracker._classify_phase(
            potential=0.85,
            connectedness=0.90,
            resilience=0.4
    ...
    async def test_omega_risk_high_in_k_phase(self, db):
        ...
        ***REMOVED*** K phase with low resilience = high Ω risk
        ...
        assert omega_risk > 0.7  ***REMOVED*** High risk
...
```python
***REMOVED*** tests/resilience/test_ecological_integration.py
...
class TestEcologicalResilienceIntegration:
    ...
    async def test_defense_level_escalation(self, db):
        ...
        ***REMOVED*** Create Ω phase conditions
        ***REMOVED*** ...
    ...
    async def test_diversity_feeds_spc(self, db):
        ...
        ***REMOVED*** Test that low diversity triggers SPC variance investigation
        pass
```
...
```python
***REMOVED*** tests/resilience/test_ecological_performance.py
...
class TestEcologicalPerformance:
    ...
    async def test_analysis_latency(self, db):
        ...
        assert duration < 5.0  ***REMOVED*** <5 second SLA
```
...
***REMOVED******REMOVED******REMOVED*** Celery Scheduled Tasks

```python
***REMOVED*** File: backend/app/resilience/tasks.py (extend existing)
...
def ecological_daily_check():
    ...
    try:
        ...
        ***REMOVED*** Send alerts for IMMEDIATE urgency tipping points
        immediate_alerts = [
...
def diversity_weekly_update():
    ...
    try:
        ...
        ***REMOVED*** Check for low diversity alerts
...
***REMOVED*** Add to Celery beat schedule:
app.conf.beat_schedule.update({
    'ecological-daily-check': {
        'task': 'app.resilience.tasks.ecological_daily_check',
        'schedule': crontab(hour=6, minute=0),  ***REMOVED*** 6 AM daily
    },
    'diversity-weekly-update': {
        'task': 'app.resilience.tasks.diversity_weekly_update',
        'schedule': crontab(day_of_week=1, hour=8, minute=0),  ***REMOVED*** Monday 8 AM
    },
})
```
...
***REMOVED******REMOVED*** Performance Specifications

***REMOVED******REMOVED******REMOVED*** Latency Requirements
...
***REMOVED******REMOVED******REMOVED*** Python Packages

All required packages already in `requirements.txt`:

```python
numpy==2.3.5           ***REMOVED*** Array operations
scipy>=1.11.0          ***REMOVED*** Statistical functions (skewness, etc.)
networkx>=3.0          ***REMOVED*** Graph analysis for panarchy
```
...
***REMOVED******REMOVED******REMOVED*** User Guide
...
***REMOVED******REMOVED******REMOVED*** Phase 1 Sign-off
...
***REMOVED******REMOVED******REMOVED*** Phase 2 Sign-off
...
***REMOVED******REMOVED******REMOVED*** Phase 3 Sign-off
...
***REMOVED******REMOVED******REMOVED*** Phase 4 Production Release
...
***REMOVED******REMOVED******REMOVED*** Implementation Risks
...
***REMOVED******REMOVED******REMOVED*** A. Mathematical Formulas
...
***REMOVED******REMOVED******REMOVED*** B. Glossary
...
***REMOVED******REMOVED******REMOVED*** C. References
