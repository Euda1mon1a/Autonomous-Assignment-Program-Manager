***REMOVED*** Vector Database Integration Proposal
...
***REMOVED******REMOVED******REMOVED*** Estimated Investment
...
1. [Infrastructure Recommendation](***REMOVED***1-infrastructure-recommendation)
2. [Use Case 1: Documentation RAG](***REMOVED***2-use-case-1-documentation-rag)
3. [Use Case 2: Swap Matching Enhancement](***REMOVED***3-use-case-2-swap-matching-enhancement)
4. [Use Case 3: Semantic Search API](***REMOVED***4-use-case-3-semantic-search-api)
5. [Use Case 4: Schedule Similarity & Anomaly Detection](***REMOVED***5-use-case-4-schedule-similarity--anomaly-detection)
6. [Use Case 5: MCP Tool Context Augmentation](***REMOVED***6-use-case-5-mcp-tool-context-augmentation)
7. [Use Case 6: Burnout Contagion Modeling](***REMOVED***7-use-case-6-burnout-contagion-modeling)
8. [Implementation Roadmap](***REMOVED***8-implementation-roadmap)
9. [Success Metrics](***REMOVED***9-success-metrics)
10. [Risk Mitigation](***REMOVED***10-risk-mitigation)
11. [Appendix: Code Snippets](***REMOVED***11-appendix-code-snippets)

---

***REMOVED******REMOVED*** 1. Infrastructure Recommendation

***REMOVED******REMOVED******REMOVED*** Primary Choice: pgvector
...
***REMOVED******REMOVED******REMOVED*** Comparison Matrix
...
***REMOVED******REMOVED******REMOVED*** Embedding Model Choice: sentence-transformers
...
***REMOVED******REMOVED******REMOVED*** Docker-Compose Addition

```yaml
***REMOVED*** No new services required - just enable pgvector in PostgreSQL
db:
...
```txt
***REMOVED*** Add to requirements.txt
pgvector==0.3.4
...
***REMOVED******REMOVED*** 2. Use Case 1: Documentation RAG
...
***REMOVED******REMOVED******REMOVED*** Solution: Semantic Documentation Search
...
***REMOVED******REMOVED******REMOVED*** Schema Design
...
class DocumentationChunk(Base):
    ...
    chunk_type = Column(String(50))  ***REMOVED*** text, code, table
    domains = Column(ARRAY(String))  ***REMOVED*** ['api', 'acgme', 'resilience']
    ...
    def chunk_document(self, doc: DocumentMetadata) -> list[DocumentChunk]:
        ...
        for section in sections:
            if section["type"] == "code":
                ***REMOVED*** Preserve code blocks intact
                chunks.append(self._create_chunk(
                    content=section["content"],
                    metadata={"type": "code_example", "section": section["title"]}
                ))
            else:
                ***REMOVED*** Split text with overlap
...
***REMOVED******REMOVED******REMOVED*** Storage Estimates
...
***REMOVED******REMOVED******REMOVED*** MCP Tool Integration
...
***REMOVED******REMOVED*** 3. Use Case 2: Swap Matching Enhancement
...
***REMOVED******REMOVED******REMOVED*** Solution: Hybrid Scoring (Explicit + Semantic)
...
***REMOVED******REMOVED******REMOVED*** Feature Vector Design

***REMOVED******REMOVED******REMOVED******REMOVED*** Request Context Embedding (7D)
...
class SwapRequestEmbedding:
    ...
    ***REMOVED*** Temporal (1D)
    temporal_urgency: float      ***REMOVED*** 0=far future, 1=imminent

    ***REMOVED*** Burden (2D)
    burden_intensity: float      ***REMOVED*** 0=minimal, 1=extreme
    burden_trend: float          ***REMOVED*** -1=avoiding, 0=neutral, 1=seeking

    ***REMOVED*** Preference (2D)
    preference_strength: float   ***REMOVED*** 0=no preference, 1=strong
    preference_stability: float  ***REMOVED*** 0=volatile, 1=consistent

    ***REMOVED*** Pattern (2D)
    swap_frequency: float        ***REMOVED*** 0=never, 1=always swapping
    acceptance_likelihood: float ***REMOVED*** P(acceptance) from history
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Faculty Profile Embedding (10D)

```python
class FacultyProfileEmbedding:
    """10-dimensional embedding of faculty swap behavior."""

    ***REMOVED*** Behavioral role (3D: orthogonal encoding)
    role_stabilizer_axis: float   ***REMOVED*** -1=martyr, 0=neutral, 1=stabilizer
    role_engagement_axis: float   ***REMOVED*** -1=isolate, 0=neutral, 1=power_broker
    role_fairness_axis: float     ***REMOVED*** -1=evader, 0=neutral, 1=balanced

    ***REMOVED*** Responsiveness (2D)
    response_speed: float         ***REMOVED*** Median hours to respond
    response_consistency: float   ***REMOVED*** Std dev of response times

    ***REMOVED*** Partnership (3D)
    reciprocity_score: float      ***REMOVED*** Mutual vs one-way swaps
    partner_loyalty: float        ***REMOVED*** Concentration with few partners
    conflict_avoidance: float     ***REMOVED*** Accepts low-compatibility requests

    ***REMOVED*** Capacity (2D)
    capacity_utilization: float   ***REMOVED*** Current % of ideal swaps
    capacity_trend: float         ***REMOVED*** +1=taking more, -1=reducing
```

***REMOVED******REMOVED******REMOVED*** Hybrid Scoring Integration
...
def score_swap_compatibility_hybrid(
    ...
    ***REMOVED*** Phase 1: Explicit scoring (current system)
    explicit_score = self._score_explicitly(request_a, request_b)

    ***REMOVED*** Phase 2: Semantic scoring via embeddings
    ...
    ***REMOVED*** Phase 3: Adaptive weighting based on agreement
    ...
    if agreement > 0.8:
        ...
        confidence = 'low'  ***REMOVED*** Flag for human review
...
***REMOVED******REMOVED******REMOVED*** Success Prediction Model
...
class SwapSuccessPredictor:
    ...
    def predict_success_probability(
        ...
        features = np.concatenate([
            self._embed_request(request_a),  ***REMOVED*** 7D
            self._embed_request(request_b),  ***REMOVED*** 7D
            self._embed_faculty(request_a.source_faculty_id),  ***REMOVED*** 10D
            self._embed_faculty(request_b.source_faculty_id),  ***REMOVED*** 10D
        ])

        return float(self.model.predict_proba(features.reshape(1, -1))[0, 1])
```

***REMOVED******REMOVED******REMOVED*** Data Requirements
...
***REMOVED******REMOVED*** 4. Use Case 3: Semantic Search API
...
***REMOVED******REMOVED******REMOVED*** Solution: Hybrid Search Layer
...
***REMOVED******REMOVED******REMOVED*** Entity-Specific Benefits
...
***REMOVED******REMOVED******REMOVED*** Hybrid Scoring Formula

```python
def compute_hybrid_score(
    keyword_score: float,      ***REMOVED*** BM25: 0-1
    semantic_score: float,     ***REMOVED*** Cosine similarity: 0-1
    entity_type: str,
    query_length: int,
) -> float:
    """
    ...
    ***REMOVED*** Entity-specific weights
    entity_weights = {
    ...
    ***REMOVED*** Query length adaptation
    if query_length <= 2:
...
***REMOVED******REMOVED******REMOVED*** Query Expansion
...
***REMOVED******REMOVED*** 5. Use Case 4: Schedule Similarity & Anomaly Detection
...
***REMOVED******REMOVED******REMOVED*** Solution: Schedule Embeddings
...
***REMOVED******REMOVED******REMOVED*** Schedule Embedding Schema

```python
class DailyActivityVector:
    """16-dimensional feature vector for a single day."""

    ***REMOVED*** One-hot activity (9D): clinic, inpatient, procedure, conference,
    ***REMOVED***                        fmit, call, night_float, off, unknown
    ***REMOVED*** Special markers (3D): back_to_back_call, post_call, rest_day
    ***REMOVED*** Intensity (1D): 0.0 (off) to 1.0 (call)
    ***REMOVED*** Supervision (1D): 0-1 scale
    ***REMOVED*** Continuity (1D): continuous duty indicator
    ***REMOVED*** Reserved (1D): future use

class WeeklyScheduleEmbedding:
    """Weekly embedding: 7 days × 16 features = 112D."""

    daily_vectors: np.ndarray  ***REMOVED*** Shape: (7, 16)
    ...
    def embedding_vector(self) -> np.ndarray:
        return self.daily_vectors.flatten()  ***REMOVED*** (112,)

class BlockScheduleEmbedding:
    """28-day academic block: 28 × 16 = 448D."""

    daily_vectors: np.ndarray  ***REMOVED*** Shape: (28, 16)
...
***REMOVED******REMOVED******REMOVED*** Similarity Search
...
***REMOVED******REMOVED*** 6. Use Case 5: MCP Tool Context Augmentation
...
***REMOVED******REMOVED******REMOVED*** Solution: Semantic Tool Wrapper
    ...
    def _enrich_response(self, response, tool_name, contexts):
        ...
        if tool_name == "validate_schedule" and "issues" in response:
            for issue in enriched["issues"]:
                ***REMOVED*** Add constraint explanation
        ...
        ***REMOVED*** Expand abbreviations
        enriched = self._expand_abbreviations(enriched)

        return enriched
```

***REMOVED******REMOVED******REMOVED*** Priority Tools to Augment
...
***REMOVED******REMOVED******REMOVED*** Response Enrichment Example
...
***REMOVED******REMOVED*** 7. Use Case 6: Burnout Contagion Modeling
...
***REMOVED******REMOVED******REMOVED*** Solution: Behavioral Vector Embeddings + Network Analysis

**Note:** This is a research-grade feature requiring significant development and validation.

***REMOVED******REMOVED******REMOVED*** Behavioral Feature Vector (21D)
...
class BehavioralFeatureVector:
    ...
    DIMENSIONS = {
        ***REMOVED*** Workload signals (0-4)
        "recent_hours_ratio": 0,       ***REMOVED*** Current/target hours
        "workload_trend": 1,           ***REMOVED*** Slope over 4 weeks
        "high_burden_shifts": 2,       ***REMOVED*** Count of severe shifts
        "swing_shift_ratio": 3,        ***REMOVED*** % evening/night
        "recovery_deficit": 4,         ***REMOVED*** Target - actual / target

        ***REMOVED*** Behavioral signals (5-9)
        "swap_request_spike": 5,       ***REMOVED*** Recent vs baseline
        "sick_call_frequency": 6,      ***REMOVED*** Per week
        "response_delay_trend": 7,     ***REMOVED*** Hours to respond
        "coverage_decline": 8,         ***REMOVED*** From historical
        "preference_deviation": 9,     ***REMOVED*** Assigned vs preferred

        ***REMOVED*** Network position (10-13)
        "degree_centrality": 10,       ***REMOVED*** Direct connections
        "betweenness_centrality": 11,  ***REMOVED*** Bridge position
        "burden_absorption_ratio": 12, ***REMOVED*** Absorbed/offloaded
        "martyr_classification": 13,   ***REMOVED*** 0-1 score

        ***REMOVED*** Stress accumulation (14-17)
        "fwi_component": 14,           ***REMOVED*** Fire Weather Index
        "creep_stage": 15,             ***REMOVED*** Primary/secondary/tertiary
        "allostatic_load": 16,         ***REMOVED*** Physiological stress
        "sta_lta_ratio": 17,           ***REMOVED*** Seismic precursor

        ***REMOVED*** Cognitive impact (18-20)
        "decision_fatigue": 18,        ***REMOVED*** Cumulative cost
        "context_switches": 19,        ***REMOVED*** Per week
        "cognitive_state": 20,         ***REMOVED*** Fresh=1, depleted=0
    }
```

***REMOVED******REMOVED******REMOVED*** Network Embedding (Node2Vec-style)
...
class BurnoutNetworkEmbedder:
    ...
    def embed_network(
        ...
        ***REMOVED*** Generate random walks
        walks = self._generate_biased_walks(G)

        ***REMOVED*** Skip-gram embedding
        structural_emb = self._skip_gram_embedding(walks)

        ***REMOVED*** Contextualize with behavioral vectors
...
***REMOVED******REMOVED******REMOVED*** SIR Parameter Learning
...
class AdaptiveSIRLearner:
    ...
    def learn_parameters(
        ...
        ***REMOVED*** Learn which features increase transmission
        beta_params = self._learn_transmission_rates(
            historical_transitions, behavioral_vectors, G
        )

        ***REMOVED*** Learn which features slow recovery
...
***REMOVED******REMOVED******REMOVED*** Research Questions
...
***REMOVED******REMOVED******REMOVED*** Phase 0: Foundation (Weeks 1-2)
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Documentation RAG (Weeks 3-4)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Search Enhancement (Weeks 5-6)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Swap Matching (Weeks 7-8)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: MCP Augmentation (Weeks 9-10)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Schedule Embeddings (Weeks 11-12)
...
***REMOVED******REMOVED******REMOVED*** Phase 6: Burnout Research (Weeks 13+)
...
***REMOVED******REMOVED*** 10. Risk Mitigation
...
***REMOVED******REMOVED******REMOVED*** Privacy/Security Risks
...
***REMOVED******REMOVED*** 11. Appendix: Code Snippets

***REMOVED******REMOVED******REMOVED*** A. EmbeddingService (Core)

```python
***REMOVED*** backend/app/services/embedding_service.py
...
***REMOVED******REMOVED******REMOVED*** B. SemanticSearchService

```python
***REMOVED*** backend/app/services/semantic_search_service.py
...
class SemanticSearchService:
    ...
    async def hybrid_search(
        ...
        ***REMOVED*** Semantic search via pgvector
        semantic_results = await self._semantic_search(
            query_embedding, entity_type, limit * 2
        )

        ***REMOVED*** Keyword search (existing infrastructure)
        keyword_results = await self._keyword_search(
            query, entity_type, limit * 2
        )

        ***REMOVED*** Combine and rerank
...
***REMOVED******REMOVED******REMOVED*** C. DocumentationSearchTool (MCP)

```python
***REMOVED*** mcp-server/src/scheduler_mcp/doc_search_tool.py
...
***REMOVED******REMOVED******REMOVED*** D. AnomalyScore Model

```python
***REMOVED*** backend/app/models/anomaly_score.py
...
class AnomalyScore:
    ...
    ***REMOVED*** Individual scores (0-1 scale)
