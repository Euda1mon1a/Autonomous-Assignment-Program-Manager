...
1. Run Codex worktree triage:
   - `python3 .codex/scripts/codex_automation_report.py`
2. Run safety audit:
   - `.codex/scripts/codex_safety_audit.sh --hooks-only --with-scans`
...
- `.codex/setup.sh`
- `.codex/skills/`
- `.codex/scripts/codex_automation_report.py`
- `.codex/scripts/codex_automation_triage.py`
- `.codex/scripts/codex_safety_audit.sh`
- `.codex/scripts/codex_skill_audit.sh`
- `.codex/scripts/codex_daily_health.sh`
- `.codex/scripts/codex_safe_prune_plan.sh`
...
1. Add JSON output mode to `.codex/scripts/codex_safety_audit.sh` for dashboards.
2. Add optional `--diff-main` mode in worktree triage scripts for stronger cherry-pick ranking.
3. Add a single `.codex/scripts/codex_self_check.sh` entrypoint that runs triage + safety + MCP health and writes one report.
