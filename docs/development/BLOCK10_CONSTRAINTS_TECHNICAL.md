***REMOVED*** Block 10 Constraints: Technical Implementation Guide
...
***REMOVED******REMOVED******REMOVED*** Weight Hierarchy
...
***REMOVED******REMOVED******REMOVED*** PostFMITSundayBlockingConstraint
...
class PostFMITSundayBlockingConstraint(BaseConstraint):
    ...
    def check(self, assignment: Assignment, context: ScheduleContext) -> ConstraintResult:
        ***REMOVED*** If this is a Sunday call assignment
        if not self._is_sunday_call(assignment):
            return ConstraintResult(satisfied=True)

        ***REMOVED*** Check if resident completed FMIT in the prior week
...
***REMOVED******REMOVED******REMOVED*** ResidentInpatientHeadcountConstraint
...
***REMOVED******REMOVED*** Soft Constraints (Call Equity)
...
class SundayCallEquityConstraint(BaseConstraint):
    ...
    def evaluate(self, schedule: Schedule) -> float:
        ...
        ***REMOVED*** Lower variance = better equity = higher score
    ...
    def check(self, assignment: Assignment, context: ScheduleContext) -> ConstraintResult:
        ...
        if last_call:
            ...
            if days_since < self.min_days:
                return ConstraintResult(
                    satisfied=False,
                    penalty=self.weight * 2,  ***REMOVED*** Double penalty for < minimum
...
class TuesdayCallPreferenceConstraint(BaseConstraint):
    ...
    def check(self, assignment: Assignment, context: ScheduleContext) -> ConstraintResult:
        if assignment.date.weekday() != 1:  ***REMOVED*** Tuesday
            return ConstraintResult(satisfied=True)
...
***REMOVED******REMOVED*** Registration in ConstraintManager
...
class ConstraintManager:
    ...
    def create_default(cls) -> "ConstraintManager":
        ...
        ***REMOVED*** Block 10: Call equity constraints
        ...
        ***REMOVED*** Block 10: Workload constraints
    ...
    def create_resilience_aware(cls) -> "ConstraintManager":
        ***REMOVED*** Same constraints, plus resilience-specific ones
        manager = cls.create_default()
        ***REMOVED*** ... additional resilience constraints ...
        return manager
```
...
***REMOVED******REMOVED******REMOVED*** Pre-flight Script
...
***REMOVED******REMOVED******REMOVED*** CI Tests
...
***REMOVED******REMOVED*** Testing Block 10 Schedule Generation
...
***REMOVED******REMOVED******REMOVED*** Expected Results (from 2025-12-25 verification)
...
***REMOVED******REMOVED******REMOVED*** Constraints Not Registered
