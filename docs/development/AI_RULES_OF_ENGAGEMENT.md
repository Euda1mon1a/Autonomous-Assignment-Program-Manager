...
***REMOVED******REMOVED*** The Sacred Timeline: `origin/main`
...
3. **Sync Protocol**
   ```bash
   ***REMOVED*** Before starting any work
   git fetch origin main
   git checkout -b claude/new-task origin/main

   ***REMOVED*** Before creating PR
   git fetch origin main
...
***REMOVED******REMOVED*** Environment Detection: Adapt Your Behavior

**Before making any changes, AI agents MUST detect their environment and adapt accordingly.**

***REMOVED******REMOVED******REMOVED*** Step 1: Identify Your Interface Type
...
***REMOVED******REMOVED******REMOVED*** Step 2: Detect Git Context

Run these checks before any file modifications:

```bash
***REMOVED*** Check if in a git repository
git rev-parse --is-inside-work-tree

***REMOVED*** Check current branch
git branch --show-current

***REMOVED*** Check if we're on main/master
git branch --show-current | grep -E '^(main|master)$'

***REMOVED*** Check remote configuration
git remote -v

***REMOVED*** Check if there are uncommitted changes
git status --porcelain
```

***REMOVED******REMOVED******REMOVED*** Step 3: Adapt Behavior Based on Environment

***REMOVED******REMOVED******REMOVED******REMOVED*** If on `main` or `master` branch:
1. **STOP** - Do not make direct commits
2. Create a feature branch: `git checkout -b feature/descriptive-name`
3. Make changes on the feature branch only

***REMOVED******REMOVED******REMOVED******REMOVED*** If connected to a remote (origin):
1. Never `git push` directly to main
2. Always create PRs for review
3. Never use `--force` without explicit approval

***REMOVED******REMOVED******REMOVED******REMOVED*** If in Web/Suggestion Mode:
...
***REMOVED******REMOVED*** CLI/IDE: Adopt Web-Like Safety Patterns

**CLI agents should adopt the safety patterns of web applications:**

***REMOVED******REMOVED******REMOVED*** The PR-First Principle
...
***REMOVED******REMOVED******REMOVED*** Destructive Command Blocklist
...
***REMOVED******REMOVED******REMOVED*** Safe Default Commands
...
***REMOVED******REMOVED******REMOVED*** PR Creation Checklist
...
***REMOVED******REMOVED******REMOVED*** Hard Stops
...
***REMOVED******REMOVED*** Session Awareness

***REMOVED******REMOVED******REMOVED*** Parallel Session Detection
...
***REMOVED******REMOVED******REMOVED*** Session Handoff Protocol
...
***REMOVED******REMOVED*** Code Quality Patterns for LLMs

**AI agents must follow these patterns to avoid common CI failures.**

***REMOVED******REMOVED******REMOVED*** Pre-Edit Checklist
...
***REMOVED******REMOVED******REMOVED*** Common LLM Mistakes to Avoid

***REMOVED******REMOVED******REMOVED******REMOVED*** Variable Hygiene

```python
***REMOVED*** BAD - Unused variable (F841)
result = expensive_operation()
return True

***REMOVED*** GOOD - Use or remove
return expensive_operation()
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Boolean Comparisons

```python
***REMOVED*** BAD - Explicit comparison (E712)
if user.is_active == True:
    pass

***REMOVED*** GOOD - Pythonic
if user.is_active:
    pass
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Import Discipline

```python
***REMOVED*** BAD - Unused import (F401)
from typing import Optional, Dict, List
def foo() -> Dict:
    return {}

***REMOVED*** GOOD - Only what's needed
from typing import Dict
def foo() -> Dict:
    return {}
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Loop Variables

```python
***REMOVED*** BAD - Unused loop variable (B007)
...
```python
***REMOVED*** BAD - Bare except (E722)
try:
    risky_operation()
except:
    pass

***REMOVED*** GOOD - Specific exceptions
from contextlib import suppress
with suppress(ValueError, TypeError):
    risky_operation()
```

***REMOVED******REMOVED******REMOVED*** TypeScript Test Files
...
***REMOVED******REMOVED******REMOVED*** Post-Edit Verification
...
***REMOVED******REMOVED******REMOVED*** CI Failure Recovery
...
***REMOVED******REMOVED*** CI/CD Integration for AI Branches

***REMOVED******REMOVED******REMOVED*** Branch Naming Conventions
...
***REMOVED******REMOVED******REMOVED*** CI/CD Workflow Triggers

The following workflows are configured to run on AI branches:

```yaml
***REMOVED*** Workflows that trigger on claude/** branches:
- ci.yml              ***REMOVED*** Main test suite
- ci-enhanced.yml     ***REMOVED*** Matrix testing
- ci-comprehensive.yml ***REMOVED*** Full quality checks
- code-quality.yml    ***REMOVED*** Linting and formatting
```
...
on:
  push:
    branches:
      - main
      - master
      - 'claude/**'  ***REMOVED*** AI-created branches
```

***REMOVED******REMOVED******REMOVED*** Before Pushing AI Branches
...
***REMOVED******REMOVED*** Preventing Parallel Development Conflicts

***REMOVED******REMOVED******REMOVED*** The Single-Session Rule

**CRITICAL**: Only ONE AI session should work on a codebase at a time.

***REMOVED******REMOVED******REMOVED*** Pre-Session Checks

Before starting work, AI agents MUST:

```bash
***REMOVED*** 1. Fetch latest state
git fetch origin

***REMOVED*** 2. Check for other AI branches in progress
git branch -r | grep -E 'claude/|codex/|ai/' | head -10

***REMOVED*** 3. Check for recent activity
git log --oneline --all --since="1 hour ago" --author-date-order

***REMOVED*** 4. Check for uncommitted changes
git status --porcelain
```

***REMOVED******REMOVED******REMOVED*** If Parallel Work Detected
...
***REMOVED******REMOVED******REMOVED*** Session Isolation Strategies
...
***REMOVED******REMOVED******REMOVED*** Avoiding Merge Conflicts
...
***REMOVED******REMOVED******REMOVED*** Branch Cleanup
...
***REMOVED******REMOVED******REMOVED*** Operation Classification
...
***REMOVED******REMOVED******REMOVED*** Read-Only Operations (Always Safe)
...
***REMOVED******REMOVED******REMOVED*** Pre-Flight Checklist for Database Modifications

Before calling any database-modifying MCP tool:

```bash
***REMOVED*** 1. Verify recent backup exists
ls -la backups/postgres/*.sql.gz | tail -1

***REMOVED*** 2. Check backup is recent (within 2 hours)
find backups/postgres -name "*.sql.gz" -mmin -120 | head -1

***REMOVED*** 3. If no recent backup:
./scripts/backup-db.sh --docker

***REMOVED*** 4. Verify backend health
curl -s http://localhost:8000/health

***REMOVED*** 5. Confirm with user
***REMOVED*** "Backup verified at [timestamp]. Proceed with [operation]?"
```

***REMOVED******REMOVED******REMOVED*** MCP Tool Decision Tree
...
```bash
***REMOVED*** 1. Find most recent backup
...
***REMOVED******REMOVED******REMOVED*** Local Data Sources (PII - for Reimport)
