***REMOVED*** Review Patches for Claude (Session 14 - Scheduling + Airtable)

This document proposes concrete fixes for issues found in the latest code changes.

***REMOVED******REMOVED*** 1) Block `block_half` should scope to the correct academic year
...
+        settings = get_settings()
         ...
         if session is None:
             ***REMOVED*** Fallback if not attached to session - use modulo of day of year
...
-        ***REMOVED*** Find the first date of this block_number
-        first_date = session.query(func.min(Block.date)).filter(
-            Block.block_number == self.block_number
-        ).scalar()
+        ***REMOVED*** Find the first date of this block_number within the same academic year
...
+        ).scalar()
@@
-        ***REMOVED*** Calculate day within the block (0-27)
+        ***REMOVED*** Calculate day within the block (0-27)
         day_in_block = (self.date - first_date).days

         ***REMOVED*** First half = days 0-13, Second half = days 14-27
-        return 1 if day_in_block < 14 else 2
+        half = settings.BLOCK_LENGTH_DAYS // 2
+        return 1 if day_in_block < half else 2
```

***REMOVED******REMOVED*** 2) Preserved FMIT assignments should block supervision assignment
...
***REMOVED******REMOVED*** 3) Make admin credentials configurable in seed script
...
***REMOVED******REMOVED*** 4) Airtable absence import: normalize names before matching
