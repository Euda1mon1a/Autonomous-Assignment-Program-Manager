***REMOVED*** Schema Version Tracking for Backup/Restore Compatibility
...
***REMOVED******REMOVED******REMOVED*** Real Example (2025-12-25)
...
***REMOVED******REMOVED******REMOVED*** 1. Store Schema Version in ApplicationSettings

The `ApplicationSettings` model (singleton table) now tracks:

```python
***REMOVED*** backend/app/models/settings.py

class ApplicationSettings(Base):
    ***REMOVED*** ... existing fields ...

    ***REMOVED*** Schema versioning - for backup/restore compatibility detection
    ***REMOVED*** Stores current Alembic head revision to detect schema mismatches
    alembic_version = Column(String(255), nullable=True)

    ***REMOVED*** Timestamp when schema version was last updated (after migration)
    schema_timestamp = Column(DateTime, nullable=True)
```

***REMOVED******REMOVED******REMOVED*** 2. Capture Version in Backups

The backup script captures Alembic version alongside the dump:

```bash
***REMOVED*** scripts/backup-db.sh - capture_version_metadata()

capture_version_metadata() {
    METADATA_FILE="${BACKUP_FILE%.sql}.metadata"

    ***REMOVED*** Get current Alembic head
...
***REMOVED*** Residency Scheduler Backup Metadata
timestamp: $TIMESTAMP
...
***REMOVED******REMOVED******REMOVED*** 3. Validate on Restore (Future Enhancement)

Restore operations should check version compatibility:

```python
***REMOVED*** backend/app/backup/restore.py (proposed)
...
***REMOVED******REMOVED*** Migration Details

```python
***REMOVED*** backend/alembic/versions/20251225_add_schema_versioning.py
...
***REMOVED******REMOVED******REMOVED*** Creating Backups with Version Metadata
...
***REMOVED******REMOVED******REMOVED*** Checking Backup Metadata
...
***REMOVED******REMOVED******REMOVED*** Verifying Current Schema Version
...
***REMOVED******REMOVED*** Recovery Workflow

When schema mismatch is detected:

***REMOVED******REMOVED******REMOVED*** Option A: Upgrade Schema to Match Backup

```bash
***REMOVED*** Check what migrations are needed
cd backend
alembic history --verbose

***REMOVED*** Apply missing migrations
alembic upgrade head

***REMOVED*** Then restore backup
gunzip -c backups/postgres/backup.sql.gz | \
  docker-compose exec -T db psql -U scheduler -d residency_scheduler
```

***REMOVED******REMOVED******REMOVED*** Option B: Fresh Start with Current Schema

```bash
***REMOVED*** Remove old data
