***REMOVED*** N+1 Query Optimization Guide
...
***REMOVED******REMOVED******REMOVED*** Example of N+1 Problem

```python
***REMOVED*** BAD: N+1 queries
persons = await db.execute(select(Person))  ***REMOVED*** 1 query
for person in persons.scalars():
    assignments = await db.execute(         ***REMOVED*** N queries
...
```python
***REMOVED*** GOOD: 1-2 queries with eager loading
...
***REMOVED******REMOVED*** SQLAlchemy Loading Strategies

***REMOVED******REMOVED******REMOVED*** `selectinload()`
...
***REMOVED*** Query 1: SELECT * FROM persons
***REMOVED*** Query 2: SELECT * FROM assignments WHERE person_id IN (1, 2, 3, ...)
```

***REMOVED******REMOVED******REMOVED*** `joinedload()`
...
***REMOVED*** Query: SELECT * FROM assignments JOIN persons ON ...
```

***REMOVED******REMOVED******REMOVED*** `contains_eager()`
...
***REMOVED******REMOVED*** Optimized Services

***REMOVED******REMOVED******REMOVED*** 1. `assignment_service.py`
...
***REMOVED******REMOVED******REMOVED*** 2. `person_service.py`
...
async def list_people(
    db: AsyncSession,
    include_assignments: bool = False,  ***REMOVED*** NEW parameter
    ...
) -> list[Person]:
    query = select(Person)
...
***REMOVED******REMOVED******REMOVED*** 3. `block_service.py`
...
***REMOVED******REMOVED******REMOVED*** 4. `swap_request_service.py`
...
***REMOVED******REMOVED******REMOVED*** 5. `swap_executor.py`
...
async def _update_schedule_assignments(
    ...
    ***REMOVED*** N+1 OPTIMIZATION: Batch load all assignments in one query
    result = await db.execute(
        select(Assignment)
        .options(
            selectinload(Assignment.person),
...
***REMOVED******REMOVED******REMOVED*** 1. Opt-in Loading
...
async def list_items(
    db: AsyncSession,
    include_related: bool = False  ***REMOVED*** Default to lazy loading
) -> list[Item]:
    query = select(Item)
    if include_related:
...
***REMOVED******REMOVED******REMOVED*** 2. Chained Loading for Deep Relationships
...
***REMOVED******REMOVED******REMOVED*** 3. Use `.unique()` with `joinedload()`
...
***REMOVED******REMOVED******REMOVED*** 4. Profile Before Optimizing
...
- [Avoiding N+1 Queries](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html***REMOVED***what-kind-of-loading-to-use)
