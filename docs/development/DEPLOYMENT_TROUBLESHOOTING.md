***REMOVED*** Deployment Troubleshooting Guide
...
1. [Quick Diagnostic Commands](***REMOVED***quick-diagnostic-commands)
2. [Database Issues](***REMOVED***database-issues)
3. [Docker Build Issues](***REMOVED***docker-build-issues)
4. [CORS and Frontend Issues](***REMOVED***cors-and-frontend-issues)
5. [Authentication Issues](***REMOVED***authentication-issues)
6. [SQLAlchemy Model Issues](***REMOVED***sqlalchemy-model-issues)
7. [Clean Rebuild Procedure](***REMOVED***clean-rebuild-procedure)
...
***REMOVED*** View all container logs
...
***REMOVED******REMOVED******REMOVED*** Missing Database Columns
...
***REMOVED*** Apply pending migrations
docker exec scheduler-local-backend alembic upgrade head

***REMOVED*** If column still missing, check if migration exists
ls backend/alembic/versions/ | grep -i <column_name>

***REMOVED*** If no migration exists, create one (see "Creating Migrations" below)
```

***REMOVED******REMOVED******REMOVED*** Missing Tables
...
***REMOVED*** If table still missing, check model is imported in alembic/env.py
```

***REMOVED******REMOVED******REMOVED*** Creating Migrations for Missing Columns

When the model has columns that don't exist in the database:

```bash
***REMOVED*** Option 1: Auto-generate migration (may need manual review)
cd backend
alembic revision --autogenerate -m "Add missing columns to table"

***REMOVED*** Option 2: Manual migration
alembic revision -m "Add missing columns to table"
***REMOVED*** Then edit the generated file in alembic/versions/
```
...
***REMOVED******REMOVED*** Docker Build Issues

***REMOVED******REMOVED******REMOVED*** Cached Layer Prevents Dependency Installation
...
**Solution:**
```bash
***REMOVED*** Nuclear option: remove all project images and cache
...
***REMOVED******REMOVED******REMOVED*** Volume Data Persistence Issues
...
**Solution:**
```bash
***REMOVED*** Remove volumes (WARNING: destroys data!)
docker-compose -f docker-compose.local.yml down -v

***REMOVED*** Rebuild
docker-compose -f docker-compose.local.yml up -d --build

***REMOVED*** Re-seed if needed
...
***REMOVED******REMOVED*** CORS and Frontend Issues

***REMOVED******REMOVED******REMOVED*** 307 Redirect Causes CORS Errors
...
frontend:
  environment:
    NEXT_PUBLIC_API_URL: http://localhost:8000/api/v1  ***REMOVED*** Not /api
```
...
***REMOVED******REMOVED******REMOVED*** Frontend Environment Variables Not Updating
...
**Solution:**
```bash
***REMOVED*** Must rebuild frontend image, not just restart
docker-compose -f docker-compose.local.yml up -d --build frontend
```
...
***REMOVED******REMOVED******REMOVED*** Login Succeeds But User Appears Unauthenticated
...
***REMOVED******REMOVED*** SQLAlchemy Model Issues

***REMOVED******REMOVED******REMOVED*** Reserved Column Name "metadata"
...
***REMOVED*** Good
run_config = Column(JSONB())  ***REMOVED*** or config_data, settings_json, etc.
```

***REMOVED******REMOVED******REMOVED*** Ambiguous Foreign Keys
...
class APIKey(Base):
    ...
    ***REMOVED*** Explicit foreign_keys to resolve ambiguity
    owner = relationship("User", foreign_keys=[owner_id])
    created_by = relationship("User", foreign_keys=[created_by_id])
```

***REMOVED******REMOVED******REMOVED*** Stale Model Imports After Rename
...
**Solution:**
```bash
***REMOVED*** Find all references to old name
grep -r "OldModelName" backend/app/

***REMOVED*** Update each file to use new name
***REMOVED*** Common locations:
***REMOVED***   - routes/*.py
***REMOVED***   - services/*.py
***REMOVED***   - schemas/*.py
...
***REMOVED******REMOVED*** Clean Rebuild Procedure
...
***REMOVED*** 2. Remove project images
docker rmi autonomous-assignment-program-manager-backend 2>/dev/null || true
docker rmi autonomous-assignment-program-manager-frontend 2>/dev/null || true
...
***REMOVED*** 3. Optional: Remove volumes if you want fresh database
***REMOVED*** WARNING: This destroys all data!
***REMOVED*** docker-compose -f docker-compose.local.yml down -v

***REMOVED*** 4. Clean build cache
docker builder prune -af

***REMOVED*** 5. Rebuild and start
docker-compose -f docker-compose.local.yml up -d --build

***REMOVED*** 6. Wait for services to be healthy
sleep 20

***REMOVED*** 7. Apply migrations
docker exec scheduler-local-backend alembic upgrade head

***REMOVED*** 8. Verify
curl -s http://localhost:8000/health | jq .

***REMOVED*** 9. Seed data if needed
docker exec scheduler-local-backend python -m scripts.seed_people
docker exec scheduler-local-backend python -m scripts.seed_rotation_templates
...
***REMOVED******REMOVED*** Issues Fixed in Session 14 (2025-12-22)

This section documents specific issues encountered and fixed:

***REMOVED******REMOVED******REMOVED*** 1. Missing `explain_json` Column in Assignments Table
...
***REMOVED******REMOVED******REMOVED*** 2. CORS Errors from 307 Redirect
...
***REMOVED******REMOVED******REMOVED*** 3. Redis Double Password in URL
...
***REMOVED******REMOVED******REMOVED*** 4. SQLAlchemy AmbiguousForeignKeysError
...
***REMOVED******REMOVED******REMOVED*** 5. Reserved `metadata` Column Name
...
***REMOVED******REMOVED******REMOVED*** 6. Missing `freeze_horizon_days` and `freeze_scope` in ApplicationSettings
...
***REMOVED******REMOVED******REMOVED*** 7. Version Table Naming Mismatch
...
***REMOVED******REMOVED******REMOVED*** 8. PropertyModTrackerPlugin Requires Missing `*_mod` Columns
...
```python
make_versioned(
    plugins=[],  ***REMOVED*** PropertyModTrackerPlugin disabled
    options={...}
)
```
...
***REMOVED******REMOVED*** Known Harmless Warnings

***REMOVED******REMOVED******REMOVED*** bcrypt `__about__` AttributeError
...
***REMOVED******REMOVED*** Appendix: Useful Debug Logging

***REMOVED******REMOVED******REMOVED*** Backend Request/Response Logging
...
***REMOVED******REMOVED******REMOVED*** Frontend API Logging
