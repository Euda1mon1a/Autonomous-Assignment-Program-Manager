***REMOVED*** MCP Transport Configuration & Security Guide
...
1. [Transport Types Overview](***REMOVED***transport-types-overview)
2. [STDIO vs HTTP: When to Use Each](***REMOVED***stdio-vs-http-when-to-use-each)
3. [Multi-Agent Concurrency Issue](***REMOVED***multi-agent-concurrency-issue)
4. [Secure HTTP Configuration](***REMOVED***secure-http-configuration)
5. [Configuration Examples](***REMOVED***configuration-examples)
6. [Security Checklist](***REMOVED***security-checklist)
7. [Troubleshooting](***REMOVED***troubleshooting)

---

***REMOVED******REMOVED*** Transport Types Overview
...
***REMOVED******REMOVED******REMOVED*** Key Architectural Difference
...
***REMOVED******REMOVED*** STDIO vs HTTP: When to Use Each

***REMOVED******REMOVED******REMOVED*** Use STDIO When
...
***REMOVED******REMOVED******REMOVED*** Use HTTP When
...
***REMOVED******REMOVED*** Multi-Agent Concurrency Issue
...
Switch to HTTP transport for multi-agent workloads. See [Secure HTTP Configuration](***REMOVED***secure-http-configuration).

---

***REMOVED******REMOVED*** Secure HTTP Configuration

***REMOVED******REMOVED******REMOVED*** Security Threat Model
...
***REMOVED******REMOVED******REMOVED*** Secure Configuration

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Docker Compose (`docker-compose.yml`)
...
mcp-server:
  build:
    context: ./mcp-server
  ports:
    ***REMOVED*** CRITICAL: Bind to localhost ONLY - not 0.0.0.0
    - "127.0.0.1:8080:8080"
  environment:
    - MCP_TRANSPORT=http
    - MCP_HOST=0.0.0.0           ***REMOVED*** Inside container, bind all interfaces
    - MCP_PORT=8080
...
```yaml
***REMOVED*** INSECURE - exposes to all network interfaces
ports:
  - "8080:8080"           ***REMOVED*** Equivalent to 0.0.0.0:8080:8080

***REMOVED*** SECURE - localhost only
ports:
  - "127.0.0.1:8080:8080" ***REMOVED*** Only your machine can reach it
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. MCP Configuration (`.mcp.json`)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Keep STDIO as Fallback (Optional)
...
***REMOVED******REMOVED******REMOVED*** Development (Single Developer, No Agents)
...
***REMOVED******REMOVED******REMOVED*** Development (Multi-Agent Workflows)
...
***REMOVED******REMOVED******REMOVED*** Team Development (Shared Server)
...
***REMOVED******REMOVED******REMOVED*** Localhost HTTP (Development)
...
***REMOVED******REMOVED******REMOVED*** Remote HTTP (Production)
...
***REMOVED******REMOVED******REMOVED*** "Connection refused" on HTTP

**Cause:** MCP server not running or wrong port

**Fix:**
```bash
***REMOVED*** Check if server is listening
curl -s http://127.0.0.1:8080/health

***REMOVED*** Check Docker port mapping
docker compose ps
...
***REMOVED******REMOVED******REMOVED*** Agents Still Can't Access MCP Tools
