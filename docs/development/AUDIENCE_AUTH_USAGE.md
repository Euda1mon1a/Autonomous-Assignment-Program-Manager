***REMOVED*** Audience-Scoped JWT Authentication Usage Guide
...
2. [Architecture](***REMOVED***architecture)
3. [Quick Start](***REMOVED***quick-start)
4. [API Reference](***REMOVED***api-reference)
5. [Backend Integration](***REMOVED***backend-integration)
6. [Frontend Integration](***REMOVED***frontend-integration)
7. [Security Considerations](***REMOVED***security-considerations)
8. [Testing](***REMOVED***testing)
...
***REMOVED******REMOVED******REMOVED*** What is Audience-Scoped Authentication?
...
***REMOVED******REMOVED******REMOVED*** Why Use Audience Tokens?
...
***REMOVED******REMOVED******REMOVED*** Security Flow
...
***REMOVED******REMOVED******REMOVED*** Available Audiences
...
***REMOVED******REMOVED******REMOVED*** 1. Backend: Protect an Endpoint

```python
***REMOVED*** backend/app/api/routes/jobs.py
...
async def abort_job(
    ...
    ***REMOVED*** Verify token belongs to authenticated user
...
***REMOVED******REMOVED******REMOVED*** 2. Frontend: Request and Use Token
...
***REMOVED******REMOVED******REMOVED*** 3. Test the Integration

```bash
***REMOVED*** 1. Authenticate and get access token
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "admin", "password": "password"}' \
  -c cookies.txt

***REMOVED*** 2. Request audience token
...
***REMOVED*** Response:
***REMOVED*** {
***REMOVED***   "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
***REMOVED***   "audience": "jobs.abort",
***REMOVED***   "expires_at": "2025-12-29T15:30:00Z",
***REMOVED***   "ttl_seconds": 120
***REMOVED*** }

***REMOVED*** 3. Use audience token for operation
...
***REMOVED******REMOVED******REMOVED*** List Available Audiences
...
***REMOVED******REMOVED******REMOVED*** Request Audience Token
...
***REMOVED******REMOVED******REMOVED*** Revoke Audience Token
...
***REMOVED******REMOVED******REMOVED*** Protecting Endpoints

***REMOVED******REMOVED******REMOVED******REMOVED*** Option 1: Using Dependency (Recommended)
...
async def generate_schedule(
    ...
    user_id = audience_token.sub  ***REMOVED*** Access user ID
    ***REMOVED*** Execute operation
    ...
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Option 2: Manual Validation
...
***REMOVED******REMOVED******REMOVED*** Creating Tokens Programmatically

```python
from app.core.audience_auth import create_audience_token

***REMOVED*** In a service or route
def request_elevated_permission(user_id: str, operation: str):
    token_response = create_audience_token(
        user_id=user_id,
        audience=operation,
        ttl_seconds=180,  ***REMOVED*** 3 minutes
    )
    return token_response.token
```

***REMOVED******REMOVED******REMOVED*** Revoking Tokens
...
***REMOVED******REMOVED******REMOVED*** React Hook for Audience Tokens
...
***REMOVED******REMOVED******REMOVED*** Usage in Components
...
2. **Use Shortest Necessary TTL**
   ```python
   ***REMOVED*** Good: 2 minutes for quick operation
   create_audience_token(user_id, "jobs.abort", ttl_seconds=120)

   ***REMOVED*** Bad: 10 minutes for operation that takes 30 seconds
   create_audience_token(user_id, "jobs.abort", ttl_seconds=600)
   ```

3. **Verify Token Ownership**
   ```python
   ***REMOVED*** Always verify audience token belongs to authenticated user
   if audience_token.sub != str(current_user.id):
       raise HTTPException(403, "Token mismatch")
   ```
...
5. **Revoke After Use (Optional)**
   ```python
   ***REMOVED*** For maximum security, revoke immediately after operation
   revoke_audience_token(db, jti, expires_at, user_id, "completed")
   ```

***REMOVED******REMOVED******REMOVED*** Attack Scenarios and Mitigations
...
***REMOVED******REMOVED******REMOVED*** Role-Based Audience Restrictions (TODO)

```python
***REMOVED*** Future enhancement: Restrict audiences by role
ROLE_AUDIENCE_MAP = {
    "admin": ["*"],  ***REMOVED*** All audiences
    "coordinator": ["schedule.generate", "swap.execute"],
    "faculty": ["swap.execute"],
    "resident": [],  ***REMOVED*** No elevated permissions
}
...
```python
***REMOVED*** tests/test_audience_auth.py
import pytest
...
def test_wrong_audience_fails(db):
    ...
    ***REMOVED*** Create token for jobs.abort
    response = create_audience_token(user_id, "jobs.abort", ttl_seconds=120)

    ***REMOVED*** Try to use for schedule.generate
...
```python
***REMOVED*** tests/integration/test_audience_endpoints.py
import pytest
...
async def test_abort_job_workflow(client: AsyncClient, auth_headers):
    ...
    ***REMOVED*** Step 1: Request audience token
    ...
    ***REMOVED*** Step 2: Use token to abort job
    response = await client.post(
    ...
    ***REMOVED*** Step 3: Verify token cannot be reused after revocation
    ***REMOVED*** (if single-use enforcement is implemented)
```

***REMOVED******REMOVED******REMOVED*** Load Tests

```bash
***REMOVED*** k6 load test for audience token endpoint
k6 run - <<EOF
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1. "Token has expired"
...
```python
***REMOVED*** Before: Request token, then wait 5 minutes
token = create_audience_token(user_id, "jobs.abort", 120)
time.sleep(300)  ***REMOVED*** ❌ Token expired
abort_job(token)

***REMOVED*** After: Request token immediately before use
token = create_audience_token(user_id, "jobs.abort", 120)
abort_job(token)  ***REMOVED*** ✅ Token still valid
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. "Token not valid for this operation"
...
```python
***REMOVED*** Wrong: Request jobs.abort token for schedule.generate endpoint
token = create_audience_token(user_id, "jobs.abort")
generate_schedule(token)  ***REMOVED*** ❌ Audience mismatch

***REMOVED*** Right: Match audience to operation
token = create_audience_token(user_id, "schedule.generate")
generate_schedule(token)  ***REMOVED*** ✅ Correct audience
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 3. "Token has been revoked"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. "Invalid token timestamp"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. "Authorization header required"
...
***REMOVED*** View detailed token validation logs
...
***REMOVED*** {
***REMOVED***   'sub': 'user-id',
***REMOVED***   'aud': 'jobs.abort',
***REMOVED***   'exp': 1735489800,
***REMOVED***   'iat': 1735489680,
***REMOVED***   'jti': '...',
***REMOVED***   'type': 'audience'
***REMOVED*** }
```
...
```python
***REMOVED*** Using observability metrics (if configured)
...
**Prometheus**:
```promql
***REMOVED*** Audience tokens issued per minute
rate(tokens_issued{type="audience_jobs.abort"}[1m])

***REMOVED*** Authentication failures by reason
sum by (reason) (auth_failures{type=~"audience_.*"})

***REMOVED*** Token blacklist operations
