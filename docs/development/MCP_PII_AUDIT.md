...
***REMOVED******REMOVED******REMOVED*** 1. Mock Data with Fake Names (LOW RISK)

**Location:** `mcp-server/src/scheduler_mcp/tools.py`

```python
***REMOVED*** Lines 252, 273, 295, 341, 850, 889, 928, 967, 1008
...
***REMOVED******REMOVED******REMOVED*** 2. Schema Fields That Could Hold PII (MEDIUM RISK)
...
class ValidationIssue(BaseModel):
    ...
    person_name: str | None = None  ***REMOVED*** ← PII RISK
```

**Risk:** MEDIUM - Schema allows PII, even if not currently populated
**Action:** Remove `person_name` field from schema

***REMOVED******REMOVED******REMOVED*** 3. Resources with Direct Name Access (HIGH RISK if DB connected)

**Location:** `mcp-server/src/scheduler_mcp/resources.py`

```python
***REMOVED*** Line 29, 71
person_name: str  ***REMOVED*** Required field in schema

***REMOVED*** Line 217
person_name=person.name,  ***REMOVED*** Direct DB field access

***REMOVED*** Lines 442, 479
person_name=resident.name,  ***REMOVED*** Direct DB field access
```

**Risk:** HIGH if connected to database - would pull real names
**Action:** Remove `person_name` from schemas, use `person_id` only

***REMOVED******REMOVED******REMOVED*** 4. PII Detection in Error Handling (GOOD)

**Location:** `mcp-server/src/scheduler_mcp/error_handling.py:969`

```python
***REMOVED*** Shows awareness of PII fields
"ssn"  ***REMOVED*** Listed in sensitive fields
```

**Risk:** N/A - This is protective code
**Action:** Ensure this is used consistently

***REMOVED******REMOVED******REMOVED*** 5. Sanitization Script Exists (GOOD)
...
***REMOVED******REMOVED*** Current Data Flow (Unsafe)
...
***REMOVED******REMOVED*** Recommended Data Flow (Safe)
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Schema Sanitization (MCP Layer)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: API Layer Sanitization (Backend)
...
***REMOVED******REMOVED*** GitHub Protection (Already in Place)
