***REMOVED*** Resilience API Reference
...
***REMOVED******REMOVED******REMOVED*** POST /crisis/activate
...
***REMOVED******REMOVED******REMOVED*** POST /crisis/deactivate
...
***REMOVED******REMOVED******REMOVED*** POST /fallback/activate
...
***REMOVED******REMOVED******REMOVED*** POST /fallback/deactivate
...
***REMOVED******REMOVED******REMOVED*** GET /fallback/list
...
***REMOVED******REMOVED******REMOVED*** GET /events/history
...
***REMOVED******REMOVED******REMOVED*** GET /vulnerabilities
...
***REMOVED******REMOVED******REMOVED*** POST /load-shedding/activate
...
***REMOVED******REMOVED******REMOVED*** POST /load-shedding/deactivate
...
***REMOVED******REMOVED******REMOVED*** 1. Faculty Emergency - Activate Crisis Mode

```bash
***REMOVED*** 1. Check current health
curl -X GET http://localhost:8000/api/resilience/health \
  -H "Authorization: Bearer <ACCESS_TOKEN>" | jq '.defense_level'

***REMOVED*** 2. Activate crisis mode
curl -X POST http://localhost:8000/api/resilience/crisis/activate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
  -d '{
...
***REMOVED*** 3. Monitor health
curl -X GET http://localhost:8000/api/resilience/health \
  -H "Authorization: Bearer <ACCESS_TOKEN>" | jq '.crisis_mode'

***REMOVED*** 4. Deactivate when resolved
curl -X POST http://localhost:8000/api/resilience/crisis/deactivate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
...
***REMOVED******REMOVED******REMOVED*** 2. Prepare Fallback Schedules

```bash
***REMOVED*** List available fallbacks
curl -X GET 'http://localhost:8000/api/resilience/fallback/list?status=ready' \
  -H "Authorization: Bearer <ACCESS_TOKEN>"

***REMOVED*** Activate fallback if needed
curl -X POST http://localhost:8000/api/resilience/fallback/activate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** 3. Load Shedding During High Utilization
...
***REMOVED*** If > 85%, activate load shedding
curl -X POST http://localhost:8000/api/resilience/load-shedding/activate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** Health Check Response
