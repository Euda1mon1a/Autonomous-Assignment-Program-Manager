...
2. [Quick Start](***REMOVED***quick-start)
3. [Tool Categories](***REMOVED***tool-categories)
4. [Common Workflows](***REMOVED***common-workflows)
5. [Tool Reference by Category](***REMOVED***tool-reference-by-category)
...
***REMOVED******REMOVED******REMOVED*** Connection Information
...
***REMOVED******REMOVED******REMOVED*** Basic Usage Pattern

All MCP tools follow a consistent pattern:

```python
***REMOVED*** 1. Import the tool
from scheduler_mcp import mcp

***REMOVED*** 2. Call the tool with parameters
result = await tool_name(
    param1="value1",
    param2="value2"
)

***REMOVED*** 3. Process the response
...
***REMOVED******REMOVED******REMOVED*** Example: Check Schedule Compliance

```python
***REMOVED*** Validate a week of schedules
...
***REMOVED******REMOVED******REMOVED*** Example: Monitor Burnout Risk

```python
***REMOVED*** Calculate Fire Danger Index for a resident
...
***REMOVED******REMOVED******REMOVED*** 1. Schedule Validation & Management (8 tools)
...
***REMOVED******REMOVED******REMOVED*** 2. Resilience Framework (12 tools)
...
***REMOVED******REMOVED******REMOVED*** 3. Early Warning System (4 tools)
...
***REMOVED******REMOVED******REMOVED*** 4. Epidemiology Tools (3 tools)
...
***REMOVED******REMOVED******REMOVED*** 5. Optimization & Analytics (3 tools)
...
***REMOVED******REMOVED******REMOVED*** 6. Composite Analytics (4 tools)
...
***REMOVED******REMOVED******REMOVED*** 7. Circuit Breaker Tools (5 tools)
...
***REMOVED******REMOVED******REMOVED*** 8. Deployment Workflow (7 tools)
...
***REMOVED******REMOVED******REMOVED*** 9. Time Crystal Scheduling (7 tools)
...
***REMOVED******REMOVED******REMOVED*** 10. Artificial Immune System (4 tools)
...
***REMOVED******REMOVED******REMOVED*** 11. Thermodynamics Tools (5 tools)
...
***REMOVED******REMOVED******REMOVED*** 12. Hopfield Network Attractors (5 tools)
...
***REMOVED******REMOVED******REMOVED*** 13. FRMS (Fatigue Risk Management) (6 tools)
...
***REMOVED******REMOVED******REMOVED*** 14. Async Task Management (4 tools)
...
***REMOVED******REMOVED******REMOVED*** 15. Empirical Testing Tools (3 tools)
...
***REMOVED******REMOVED******REMOVED*** Workflow 1: Daily Resilience Check
...
***REMOVED******REMOVED******REMOVED*** Workflow 2: Pre-Deployment Validation

Validate before deploying new schedule.

```python
***REMOVED*** 1. Validate ACGME compliance
...
***REMOVED*** 2. Check for conflicts
conflicts = await detect_conflicts_tool(
    start_date="2025-01-01",
    end_date="2025-03-31"
)

***REMOVED*** 3. Calculate process capability
...
***REMOVED******REMOVED******REMOVED*** Workflow 3: Individual Burnout Screening

Screen resident for burnout risk.

```python
***REMOVED*** 1. Fire Danger Index (multi-temporal)
...
***REMOVED*** 2. SPC Analysis (process control)
...
***REMOVED*** 3. Precursor Detection (seismic)
precursors = await detect_burnout_precursors_tool(
    resident_id="res-123",
    signal_type="swap_requests",
    time_series=[0, 1, 2, 3, 5, 8, 13]  ***REMOVED*** Fibonacci pattern
)

***REMOVED*** 4. FRMS Assessment (aviation fatigue)
frms = await run_frms_assessment_tool(
    resident_id="res-123"
)

***REMOVED*** Aggregate risk
high_risk = (
...
if high_risk:
    ***REMOVED*** Trigger intervention
    schedule_wellness_checkup("res-123")
```

***REMOVED******REMOVED******REMOVED*** Workflow 4: Schedule Optimization

Optimize schedule quality and equity.

```python
***REMOVED*** 1. Baseline entropy (flexibility)
entropy = await calculate_schedule_entropy_tool(
    start_date="2025-01-01",
    end_date="2025-03-31"
)

***REMOVED*** 2. Find stable alternatives (Hopfield)
attractors = await find_nearby_attractors_tool(
    max_distance=5
)

***REMOVED*** 3. Calculate Shapley values (fairness)
...
***REMOVED*** 4. Optimize for anti-churn (time crystal)
time_crystal = await calculate_time_crystal_objective_tool()

***REMOVED*** 5. Test recovery distance (fragility)
recovery = await calculate_recovery_distance_tool(
    max_events=20
)

***REMOVED*** Select schedule with best balance
best_schedule = min(attractors, key=lambda s: (
    0.3 * s.energy +           ***REMOVED*** Stability
    0.3 * (1 - s.equity) +     ***REMOVED*** Inequity penalty
    0.2 * s.churn_score +      ***REMOVED*** Change penalty
    0.2 * s.fragility         ***REMOVED*** Fragility penalty
))
```

***REMOVED******REMOVED******REMOVED*** Workflow 5: Emergency Response

Respond to crisis situation.

```python
***REMOVED*** 1. Check current defense level
defense = await get_defense_level_tool()

if defense.level >= 4:  ***REMOVED*** Containment or Emergency

    ***REMOVED*** 2. Calculate blast radius
    blast = await calculate_blast_radius_tool()

    ***REMOVED*** 3. Get static fallbacks
    fallbacks = await get_static_fallbacks_tool()

    ***REMOVED*** 4. Execute sacrifice hierarchy
    sacrifice = await execute_sacrifice_hierarchy_tool(
        severity_level=defense.level
    )

    ***REMOVED*** 5. Override circuit breakers if needed
    if defense.level == 5:  ***REMOVED*** Emergency
        await override_circuit_breaker_tool(
            breaker_name="critical_services",
            override_state="closed"  ***REMOVED*** Force allow
        )

    ***REMOVED*** 6. Start emergency contingency
    task = await start_background_task_tool(
        task_type="resilience_crisis_activation",
        params={"severity": defense.level}
    )

    ***REMOVED*** Alert leadership
...
***REMOVED******REMOVED******REMOVED*** 1. Error Handling
...
try:
    ...
    ***REMOVED*** Use fallback or retry
```

***REMOVED******REMOVED******REMOVED*** 2. Batch Operations

Use batch tools when processing multiple entities:

```python
***REMOVED*** ❌ DON'T: Loop individual calls
for resident in residents:
    await calculate_fire_danger_index_tool(resident.id, ...)

***REMOVED*** ✅ DO: Use batch tool
...
***REMOVED******REMOVED******REMOVED*** 3. Async Task Polling
...
while True:
    ...
    await asyncio.sleep(2)  ***REMOVED*** Poll every 2 seconds
```

***REMOVED******REMOVED******REMOVED*** 4. Caching Results
...
***REMOVED******REMOVED******REMOVED*** 5. Severity-Based Alerting
...
***REMOVED******REMOVED******REMOVED*** 6. Validation Chains
...
***REMOVED******REMOVED******REMOVED*** Tool Returns Fallback Data
...
***REMOVED******REMOVED******REMOVED*** Task Stuck in Pending
...
**Solution**:
```python
***REMOVED*** ❌ DON'T: Use ambiguous formats
start_date = "1/15/2025"

***REMOVED*** ✅ DO: Use ISO 8601
start_date = "2025-01-15"
```

***REMOVED******REMOVED******REMOVED*** Missing Required Parameters
...
def daily_resilience_check():
    ...
    ***REMOVED*** Fire danger for all residents
...
```python
***REMOVED*** Real-time dashboard data
...
***REMOVED******REMOVED******REMOVED*** Alert Pipeline
...
async def monitor_and_alert():
    ...
    while True:
        ***REMOVED*** Check multiple signals
        spc = await run_spc_analysis_tool(...)
        precursors = await detect_burnout_precursors_tool(...)
        fwi = await calculate_fire_danger_index_tool(...)
        ...
        ***REMOVED*** Route alert
        ...
        await asyncio.sleep(900)  ***REMOVED*** Every 15 minutes
```

---

***REMOVED******REMOVED*** API Response Patterns
...
***REMOVED******REMOVED******REMOVED*** Error Response
...
***REMOVED******REMOVED******REMOVED*** Degraded Response
