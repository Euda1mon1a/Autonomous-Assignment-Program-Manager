...
2. [Backend Error Handling](***REMOVED***backend-error-handling)
3. [Frontend Error Handling](***REMOVED***frontend-error-handling)
4. [Best Practices](***REMOVED***best-practices)
5. [Common Patterns](***REMOVED***common-patterns)
6. [Testing Error Handling](***REMOVED***testing-error-handling)
...
***REMOVED******REMOVED*** Backend Error Handling
...
from app.exceptions import (
    ...
    ***REMOVED*** Domain-specific exceptions
    SchedulingError,
...
***REMOVED******REMOVED******REMOVED*** Using Exceptions
...
***REMOVED*** Raise a not found error
if not person:
    raise NotFoundError(
        message="Person not found",
        ***REMOVED*** Optional context for logging
        person_id=person_id,
    )

***REMOVED*** Raise a validation error
if age < 0:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Domain-Specific Exceptions

```python
from app.exceptions import ScheduleConflictError, WorkHourViolationError

***REMOVED*** Schedule conflict
...
***REMOVED******REMOVED******REMOVED*** Creating Custom Exceptions

```python
***REMOVED*** In app/exceptions/your_domain.py
...
***REMOVED******REMOVED******REMOVED*** Error Handling in Routes
...
async def create_assignment(
    ...
    """
    ***REMOVED*** Check if person exists
...
***REMOVED******REMOVED*** Frontend Error Handling
...
***REMOVED******REMOVED******REMOVED*** Error Boundary
...
***REMOVED******REMOVED******REMOVED*** Error Toast Notifications
...
***REMOVED******REMOVED******REMOVED*** Error Handler Hook
...
***REMOVED******REMOVED******REMOVED*** Handling Specific Error Types
...
***REMOVED******REMOVED******REMOVED*** 1. Use Domain-Specific Exceptions

```python
***REMOVED*** ❌ Bad - using generic exceptions
raise ValueError("Schedule conflict")

***REMOVED*** ✅ Good - using domain-specific exception
raise ScheduleConflictError(
    message="Assignment conflicts with existing schedule",
    conflicting_assignment_id=existing.id,
)
...
***REMOVED******REMOVED******REMOVED*** 2. Provide Context

```python
***REMOVED*** ❌ Bad - no context
raise NotFoundError("Person not found")

***REMOVED*** ✅ Good - with context for debugging
raise NotFoundError(
...
***REMOVED******REMOVED******REMOVED*** 3. Don't Leak Sensitive Information

```python
***REMOVED*** ❌ Bad - leaking database details
raise DatabaseError(f"Query failed: {sql_query}")

***REMOVED*** ✅ Good - safe error message
raise DatabaseError(
...
***REMOVED******REMOVED******REMOVED*** 4. Use Type Guards on Frontend
...
***REMOVED******REMOVED******REMOVED*** 5. Handle Errors at the Right Level

```python
***REMOVED*** ❌ Bad - catching and re-raising without adding value
try:
    do_something()
except Exception as e:
    raise e

***REMOVED*** ✅ Good - add context or handle
try:
    do_something()
...
***REMOVED******REMOVED******REMOVED*** Pattern 1: Validation Errors with Field Details
...
def validate_assignment(assignment_data):
    ...
    if errors:
        ***REMOVED*** Create validation error with field details
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Retrying on Specific Errors
...
***REMOVED******REMOVED*** Testing Error Handling
...
async def test_acgme_violation_error(db):
    ...
    ***REMOVED*** Create assignments that violate 80-hour rule
    resident = await create_resident(db)

    ***REMOVED*** Add 80 hours of assignments
    await add_assignments(db, resident.id, hours=80)

    ***REMOVED*** Attempt to add more hours
...
***REMOVED******REMOVED*** Error Code Reference
...
***REMOVED******REMOVED******REMOVED*** Q: Error responses don't include stack traces in development

**A:** Make sure `DEBUG=True` in backend settings. Stack traces are only included when `DEBUG=True`.

***REMOVED******REMOVED******REMOVED*** Q: Error toasts aren't showing up

**A:** Ensure `<ErrorToastContainer />` is rendered in your app root.

***REMOVED******REMOVED******REMOVED*** Q: Type guards aren't working

**A:** Make sure you're importing from `@/lib/errors`, not constructing error objects manually.

***REMOVED******REMOVED******REMOVED*** Q: Custom exceptions aren't being caught by error handler
