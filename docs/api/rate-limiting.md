...
***REMOVED******REMOVED******REMOVED*** Default Limits
...
***REMOVED******REMOVED******REMOVED*** Endpoint-Specific Limits
...
***REMOVED******REMOVED*** Rate Limit Headers
...
***REMOVED******REMOVED*** Rate Limit Exceeded Response
...
***REMOVED******REMOVED******REMOVED*** Retry-After Header
...
***REMOVED******REMOVED******REMOVED*** Python Example
        ...
        while True:
            ...
            ***REMOVED*** Check rate limit headers
            ...
            if response.status_code == 429:
                ***REMOVED*** Rate limited - wait and retry
                retry_after = int(response.headers.get('Retry-After', 60))
                print(f"Rate limited. Waiting {retry_after} seconds...")
                time.sleep(retry_after)
                continue

            ***REMOVED*** Proactive throttling when nearing limit
            if remaining < 10:
...
***REMOVED******REMOVED******REMOVED*** JavaScript Example
...
***REMOVED******REMOVED******REMOVED*** cURL with Rate Limit Handling
...
***REMOVED******REMOVED******REMOVED*** 1. Implement Exponential Backoff
...
***REMOVED******REMOVED******REMOVED*** 2. Monitor Rate Limit Headers

Always check `X-RateLimit-Remaining` and slow down proactively:

```python
if remaining < limit * 0.1:  ***REMOVED*** Less than 10% remaining
    ***REMOVED*** Slow down requests
    time.sleep(1)
```

***REMOVED******REMOVED******REMOVED*** 3. Cache Responses
...
def get_people(last_updated=None):
    ***REMOVED*** Cache expires every 5 minutes
...
***REMOVED******REMOVED******REMOVED*** 4. Batch Operations

When possible, use batch endpoints instead of multiple individual requests:

```bash
***REMOVED*** Instead of multiple individual assignment creations
***REMOVED*** Use bulk operations when available
curl -X DELETE "http://localhost:8000/api/assignments?start_date=2024-01-01&end_date=2024-01-31"
```

***REMOVED******REMOVED******REMOVED*** 5. Use Webhooks

For real-time updates, consider webhooks instead of polling:

```python
***REMOVED*** Instead of polling every minute
while True:
    check_schedule_updates()
    time.sleep(60)

***REMOVED*** Subscribe to webhooks (when available)
subscribe_to_updates(callback_url)
```

***REMOVED******REMOVED*** Rate Limiting Tiers
...
***REMOVED******REMOVED******REMOVED*** Consistently Hitting Rate Limits
...
***REMOVED******REMOVED******REMOVED*** Sudden Rate Limit Errors
