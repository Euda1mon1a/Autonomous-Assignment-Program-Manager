***REMOVED*** Skill Update Workflow - How META_UPDATER Proposes Skill Enhancements
...
2. [Update Trigger Conditions](***REMOVED***update-trigger-conditions)
3. [Signal Detection Process](***REMOVED***signal-detection-process)
4. [GitHub PR Format for Skill Updates](***REMOVED***github-pr-format-for-skill-updates)
5. [Review and Approval Process](***REMOVED***review-and-approval-process)
6. [Merging and Activation](***REMOVED***merging-and-activation)
7. [Rollback Procedures](***REMOVED***rollback-procedures)
8. [Example: Updating SCHEDULING Skill](***REMOVED***example-updating-scheduling-skill)
9. [Monitoring Post-Update](***REMOVED***monitoring-post-update)

---

***REMOVED******REMOVED*** Overview

***REMOVED******REMOVED******REMOVED*** Purpose of Skill Updates
...
***REMOVED******REMOVED******REMOVED*** META_UPDATER Responsibility
...
***REMOVED******REMOVED*** Update Trigger Conditions

***REMOVED******REMOVED******REMOVED*** Trigger 1: Weekly Scan (Scheduled)
...
***REMOVED******REMOVED******REMOVED*** Trigger 2: Post-Incident Review
...
***REMOVED******REMOVED******REMOVED*** Trigger 3: Manual Request
...
***REMOVED******REMOVED******REMOVED*** Trigger 4: Code Change Detection
...
***REMOVED******REMOVED*** Signal Detection Process

***REMOVED******REMOVED******REMOVED*** Signal 1: Constraint Violations
...
***REMOVED******REMOVED******REMOVED*** Signal 2: Swap Failures
...
***REMOVED******REMOVED******REMOVED*** Signal 3: N-1 Fragility Increases

**What to Monitor:**
```python
***REMOVED*** Weekly resilience scan
...
***REMOVED******REMOVED******REMOVED*** Signal 4: Agent Escalations

**What to Monitor:**
```
***REMOVED*** Parse agent conversation logs for escalations
...
***REMOVED******REMOVED*** GitHub PR Format for Skill Updates

***REMOVED******REMOVED******REMOVED*** PR Title Convention
...
```markdown
***REMOVED******REMOVED*** Skill Enhancement Proposal
...
***REMOVED******REMOVED******REMOVED*** Change Summary
...
- **Related Issues:** ***REMOVED***[issue-number], ***REMOVED***[issue-number]
...
***REMOVED******REMOVED******REMOVED*** PR Labels
...
***REMOVED******REMOVED******REMOVED*** PR File Organization

```
Files changed (typical skill update):

.claude/skills/SKILL_NAME/SKILL.md                  ***REMOVED*** Main spec update
.claude/skills/SKILL_NAME/Workflows/new-workflow.md ***REMOVED*** New workflow (if applicable)
.claude/skills/SKILL_NAME/Reference/new-ref.md      ***REMOVED*** New reference (if applicable)
.claude/skills/CORE/SKILL.md                        ***REMOVED*** Updated routing (if needed)
docs/SKILL_UPDATE_WORKFLOW.md                       ***REMOVED*** Update this doc if process changes
```

---

***REMOVED******REMOVED*** Review and Approval Process

***REMOVED******REMOVED******REMOVED*** Review Phases

***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 1: Automated Checks (GitHub Actions)

```yaml
***REMOVED*** .github/workflows/skill-update-check.yml
  ...
  validate:
    ...
    steps:
      - name: Check YAML frontmatter
        run: |
          ***REMOVED*** Verify SKILL.md has valid YAML frontmatter
          python scripts/validate_skill_yaml.py

      - name: Check for TODOs
        run: |
          ***REMOVED*** Fail if TODO/FIXME found in updated skill files
          ! grep -r "TODO\|FIXME" .claude/skills/

      - name: Validate examples
        run: |
          ***REMOVED*** Run code examples to ensure they work
          python scripts/test_skill_examples.py

      - name: Check links
        run: |
          ***REMOVED*** Verify all internal links resolve
          markdown-link-check .claude/skills/**/*.md

      - name: Lint markdown
        run: |
          ***REMOVED*** Ensure consistent formatting
          markdownlint .claude/skills/**/*.md
```

**Auto-checks must pass before human review begins.**

***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 2: Specialist Agent Review
...
**Review format:**
```markdown
***REMOVED******REMOVED*** Review: [REVIEWER_NAME]
...
***REMOVED******REMOVED******REMOVED*** Concerns
- [Issues that need addressing]

***REMOVED******REMOVED******REMOVED*** Suggestions
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 3: META_UPDATER Self-Review
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 4: Human Review (Optional)
...
***REMOVED******REMOVED******REMOVED*** Approval Criteria
...
***REMOVED******REMOVED*** Merging and Activation

***REMOVED******REMOVED******REMOVED*** Merge Process
...
1. **Final Pre-Merge Check**
   ...
   ***REMOVED*** Verify skill examples
   python scripts/test_skill_examples.py

   ***REMOVED*** Check no merge conflicts
...
2. **Merge PR**
   ```bash
   ***REMOVED*** Squash merge (prefer for skill updates)
   gh pr merge [PR-number] --squash --delete-branch

   ***REMOVED*** OR merge commit (if preserving history important)
   gh pr merge [PR-number] --merge
   ```

3. **Tag Version**
   ```bash
   ***REMOVED*** If skill version bumped, tag the commit
...
**Experimental Flag:**
```yaml
***REMOVED*** In SKILL.md frontmatter
---
name: SCHEDULING
version: 2.0.0
experimental: true  ***REMOVED*** Flag for gradual rollout
...
***REMOVED******REMOVED******REMOVED*** Rollback Methods

***REMOVED******REMOVED******REMOVED******REMOVED*** Method 1: Git Revert (Preferred)

```bash
***REMOVED*** Identify commit to revert
git log --oneline .claude/skills/SKILL_NAME/

***REMOVED*** Revert the skill update commit
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 2: Version Restore

```bash
***REMOVED*** Restore previous version from git tag
git checkout skills/SKILL_NAME-v1.2.0 -- .claude/skills/SKILL_NAME/

***REMOVED*** Commit restoration
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 3: Hotfix Branch

```bash
***REMOVED*** Create hotfix branch from pre-update commit
git checkout -b hotfix/skill-SKILL_NAME [commit-before-update]

***REMOVED*** Cherry-pick any critical fixes made since update
git cherry-pick [critical-fix-commit]

***REMOVED*** Merge hotfix
git checkout main
...
***REMOVED******REMOVED******REMOVED*** Post-Rollback Actions

1. **Notify Stakeholders**
   ```markdown
   ***REMOVED******REMOVED*** Rollback Notice: SKILL_NAME v1.3.0 → v1.2.0
   ...
   **Next Steps:**
   - Root cause analysis (GitHub issue ***REMOVED***XXX)
...
2. **Create Incident Report**
   ```markdown
   ***REMOVED******REMOVED*** Incident: SKILL_NAME v1.3.0 Rollback
...
***REMOVED******REMOVED*** Example: Updating SCHEDULING Skill with New Constraint

***REMOVED******REMOVED******REMOVED*** Scenario
...
***REMOVED******REMOVED******REMOVED*** Step 1: Analyze Root Cause
...
***REMOVED******REMOVED******REMOVED*** Step 2: Draft Skill Enhancement
...
***REMOVED******REMOVED******REMOVED*** Step 3: Create GitHub PR
...
**Evidence:**
- GitHub Issue ***REMOVED***487: "Add NICU Friday Clinic to institutional constraints"
...
**Before:**
```markdown
***REMOVED******REMOVED******REMOVED*** Tier 2: Institutional Hard Constraints (HIGH)
...
- **Related Issues:** ***REMOVED***487
- **Incident Reports:** N/A
- **Code Changes:** PR ***REMOVED***512 (backend constraint validator)
...
***REMOVED******REMOVED******REMOVED*** Step 4: Review Process
...
```markdown
***REMOVED******REMOVED*** Review: SCHEDULER
...
***REMOVED******REMOVED******REMOVED*** Step 5: Merge and Activate
...
***REMOVED*** Verify activation
python scripts/validate_core_registry.py
***REMOVED*** ✅ SCHEDULING skill v1.5.0 loaded successfully
```

***REMOVED******REMOVED******REMOVED*** Step 6: Monitor Post-Update
...
***REMOVED******REMOVED*** Monitoring Post-Update
...
```bash
***REMOVED*** Count escalations mentioning the updated skill
...
***REMOVED******REMOVED******REMOVED*** Week 2-4: Trend Analysis

```python
***REMOVED*** Compare metrics pre- vs. post-update
...
***REMOVED*** If p_value < 0.05 and delta > 0: Update improved performance ✅
***REMOVED*** If p_value < 0.05 and delta < 0: Update degraded performance ❌ (consider rollback)
```

***REMOVED******REMOVED******REMOVED*** Feedback Collection

**Agent Feedback:**
```markdown
***REMOVED******REMOVED*** Skill Update Feedback: SCHEDULING v1.5.0
...
***REMOVED******REMOVED*** Summary: Skill Update Lifecycle
