...
- ~~Default admin user is auto-created on empty DB with static credentials `admin` / `admin123` and no DEBUG gating.~~ **FIXED (PR #1134, #1136):** `/initialize-admin` endpoint and `main.py` startup both use `secrets.token_urlsafe(16)`. All seed scripts use auto-bootstrap pattern.
