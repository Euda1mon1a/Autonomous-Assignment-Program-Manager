***REMOVED*** Backend Issues That Look Like Frontend Bugs

Purpose: quick map of backend/middleware/routing behaviors that commonly surface as “frontend” issues.

***REMOVED******REMOVED*** 1) /api → /api/v1 Redirect (307) Drops CORS Headers
...
***REMOVED******REMOVED*** 2) Auth Cookie Not Sent → 401 → Frontend Redirect - PARTIALLY FIXED
...
***REMOVED******REMOVED*** 3) Global Rate Limiting Masks Real Failures
...
***REMOVED******REMOVED*** 4) TrustedHostMiddleware Blocks Requests in Production-like Config
...
***REMOVED******REMOVED*** 5) Metrics Path Confusion (double prefix)
...
***REMOVED******REMOVED*** 6) Missing /auth/check Endpoint - FIXED
...
***REMOVED******REMOVED*** 7) Security Headers in Debug vs Prod
