...
2. [Architecture Decision: JWT vs Sessions](***REMOVED***architecture-decision-jwt-vs-sessions)
3. [User Model Design](***REMOVED***user-model-design)
4. [Role-Based Access Control (RBAC)](***REMOVED***role-based-access-control-rbac)
5. [JWT Token Flow](***REMOVED***jwt-token-flow)
6. [Token Storage Strategy](***REMOVED***token-storage-strategy)
7. [Backend Implementation Patterns](***REMOVED***backend-implementation-patterns)
8. [Frontend Implementation Patterns](***REMOVED***frontend-implementation-patterns)
9. [Security Considerations](***REMOVED***security-considerations)
10. [ACGME Audit Trail Requirements](***REMOVED***acgme-audit-trail-requirements)
11. [API Endpoint Specifications](***REMOVED***api-endpoint-specifications)
12. [Implementation Checklist](***REMOVED***implementation-checklist)
...
***REMOVED******REMOVED*** Architecture Decision: JWT vs Sessions

***REMOVED******REMOVED******REMOVED*** Decision: JWT with Dual Token Pattern
...
| API-first design | ✅ Native fit | ⚠️ Cookie management complexity |
...
***REMOVED******REMOVED*** User Model Design

***REMOVED******REMOVED******REMOVED*** Distinction: User vs Person
...
***REMOVED******REMOVED******REMOVED*** User Model Schema
...
class User(Base):
    ...
    ***REMOVED*** Role and permissions
    role = Column(String(50), nullable=False, default="faculty")

    ***REMOVED*** Optional link to Person (for faculty who also manage schedules)
    person_id = Column(UUID(as_uuid=True), ForeignKey("people.id"), nullable=True)

    ***REMOVED*** Account status
    is_active = Column(Boolean, default=True)
    is_verified = Column(Boolean, default=False)  ***REMOVED*** Email verified
...
***REMOVED******REMOVED******REMOVED*** Refresh Token Model

```python
***REMOVED*** backend/app/models/refresh_token.py
...
class RefreshToken(Base):
    ...
    token_hash = Column(String(255), nullable=False, index=True)  ***REMOVED*** SHA-256 hash

    ***REMOVED*** Token metadata
    device_info = Column(String(255), nullable=True)  ***REMOVED*** User-agent or device ID
    ip_address = Column(String(45), nullable=True)    ***REMOVED*** IPv6-compatible

    ***REMOVED*** Lifecycle
    issued_at = Column(DateTime, default=datetime.utcnow)
    expires_at = Column(DateTime, nullable=False)
    revoked_at = Column(DateTime, nullable=True)
    replaced_by = Column(UUID(as_uuid=True), nullable=True)  ***REMOVED*** For token rotation tracking
...
***REMOVED******REMOVED*** Role-Based Access Control (RBAC)

***REMOVED******REMOVED******REMOVED*** Role Hierarchy
...
***REMOVED******REMOVED******REMOVED*** Permission Implementation

```python
***REMOVED*** backend/app/core/permissions.py
...
***REMOVED*** Role to permissions mapping
ROLE_PERMISSIONS: dict[str, Set[Permission]] = {
    ...
    "coordinator": {
        ***REMOVED*** Inherits all faculty permissions
        Permission.USERS_READ_SELF,
        Permission.USERS_UPDATE_SELF,
...
***REMOVED******REMOVED*** JWT Token Flow

***REMOVED******REMOVED******REMOVED*** Token Specifications
...
***REMOVED******REMOVED******REMOVED*** Token Payloads
...
***REMOVED******REMOVED******REMOVED*** Authentication Flows

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Login Flow
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Authenticated Request Flow
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Token Refresh Flow (Silent Refresh)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Logout Flow
...
***REMOVED******REMOVED*** Token Storage Strategy

***REMOVED******REMOVED******REMOVED*** Decision: HttpOnly Cookies
...
| HttpOnly Cookie | ❌ No | ✅ Yes (mitigable) | ✅ Yes |
...
***REMOVED******REMOVED******REMOVED*** Cookie Configuration

```python
***REMOVED*** Access token cookie
...
***REMOVED*** Refresh token cookie
Set-Cookie: refresh_token=<jwt>;
    HttpOnly;
    Secure;
    SameSite=Lax;
    Path=/api/auth/refresh;  ***REMOVED*** Restricted path!
    Max-Age=604800
```
...
- `Max-Age`: Cookie expiration matches token expiration

---

***REMOVED******REMOVED*** Backend Implementation Patterns

***REMOVED******REMOVED******REMOVED*** Configuration Updates

```python
***REMOVED*** backend/app/core/config.py - Add to Settings class
...
COOKIE_SECURE: bool = True  ***REMOVED*** Set False for local dev without HTTPS
COOKIE_SAMESITE: str = "lax"
COOKIE_DOMAIN: str | None = None  ***REMOVED*** Set for cross-subdomain auth
```

***REMOVED******REMOVED******REMOVED*** Authentication Service

```python
***REMOVED*** backend/app/services/auth.py
...
***REMOVED******REMOVED******REMOVED*** FastAPI Dependencies

```python
***REMOVED*** backend/app/api/deps.py
...
***REMOVED******REMOVED******REMOVED*** Protected Route Example

```python
***REMOVED*** backend/app/api/routes/people.py (modified)
...
def list_people(
    ...
    ***REMOVED*** All authenticated users can list people
    ...
...
***REMOVED******REMOVED*** Frontend Implementation Patterns

***REMOVED******REMOVED******REMOVED*** Auth Context Provider
...
***REMOVED******REMOVED******REMOVED*** API Client with Token Refresh
...
***REMOVED******REMOVED******REMOVED*** Protected Route Component
...
***REMOVED******REMOVED******REMOVED*** Account Security
...
***REMOVED******REMOVED******REMOVED*** Session Management
...
***REMOVED******REMOVED******REMOVED*** HTTPS Enforcement
...
***REMOVED******REMOVED*** ACGME Audit Trail Requirements

***REMOVED******REMOVED******REMOVED*** Audit Log Model

```python
***REMOVED*** backend/app/models/audit_log.py
...
class AuditLog(Base):
    ...
    ***REMOVED*** Who
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    user_email = Column(String(255), nullable=False)  ***REMOVED*** Denormalized for history
    user_role = Column(String(50), nullable=False)    ***REMOVED*** Denormalized for history

    ***REMOVED*** What
    action = Column(String(100), nullable=False)  ***REMOVED*** e.g., "assignment.create"
    resource_type = Column(String(100), nullable=False)  ***REMOVED*** e.g., "assignment"
    resource_id = Column(UUID(as_uuid=True), nullable=True)

    ***REMOVED*** Details
    changes = Column(JSON, nullable=True)  ***REMOVED*** {field: {old: x, new: y}}
    metadata = Column(JSON, nullable=True)  ***REMOVED*** Additional context
...
***REMOVED******REMOVED******REMOVED*** Audited Actions
...
***REMOVED******REMOVED******REMOVED*** Audit Service

```python
***REMOVED*** backend/app/services/audit.py
...
***REMOVED******REMOVED*** API Endpoint Specifications
...
POST /api/auth/login
  ...
  Response: { user: User }
  Cookies: Sets access_token, refresh_token

POST /api/auth/logout
  Response: { message: "Logged out" }
  Cookies: Clears access_token, refresh_token

POST /api/auth/refresh
  Cookies: Requires refresh_token
  Response: { message: "Token refreshed" }
  Cookies: Sets new access_token, refresh_token
...
***REMOVED******REMOVED******REMOVED*** User Management Endpoints (Admin only)
...
***REMOVED******REMOVED******REMOVED*** Backend Tasks (For Sonnet)
...
***REMOVED******REMOVED******REMOVED*** Frontend Tasks (For Sonnet)
...
***REMOVED******REMOVED*** Escalation Points
