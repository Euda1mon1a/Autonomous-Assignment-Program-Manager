***REMOVED*** Clinic Scheduling Constraints
...
***REMOVED******REMOVED*** Implemented Constraints

***REMOVED******REMOVED******REMOVED*** 1. MaxPhysiciansInClinicConstraint (Hard Constraint)
...
***REMOVED******REMOVED******REMOVED*** 2. WednesdayAMInternOnlyConstraint (Hard Constraint)

**Status:** Implemented

**Rule:** Wednesday morning clinic sessions should be staffed by interns (PGY-1) only, with rare exceptions.

***REMOVED******REMOVED******REMOVED*** 3. FacultyPrimaryDutyClinicConstraint (Hard Constraint)
...
***REMOVED******REMOVED******REMOVED*** 4. FacultyDayAvailabilityConstraint (Hard Constraint)
...
***REMOVED******REMOVED******REMOVED*** 5. FacultyClinicEquitySoftConstraint (Soft Constraint)
...
***REMOVED******REMOVED*** Planned Constraints (Phase 2)

***REMOVED******REMOVED******REMOVED*** 6. ClinicContinuityConstraint (Hard Constraint)
...
***REMOVED******REMOVED******REMOVED*** 4. WednesdayPMFacultyCoverageConstraint (Hard + Soft)
...
***REMOVED******REMOVED******REMOVED*** 5. FourthWednesdayInvertedScheduleConstraint (Hard Constraint)
...
```python
***REMOVED*** Identifying 4th Wednesday of a block
def get_fourth_wednesday(block_start_date: date) -> date:
    ...
    while (current - block_start_date).days < 28:
        if current.weekday() == 2:  ***REMOVED*** Wednesday
            wednesday_count += 1
            if wednesday_count == 4:
                return current
        current += timedelta(days=1)
    return None  ***REMOVED*** Should not happen in a 4-week block
```

---

***REMOVED******REMOVED*** Deferred Constraints (Phase 3)

***REMOVED******REMOVED******REMOVED*** 6. TimeVaryingSupervisionRatioConstraint
...
***REMOVED******REMOVED******REMOVED*** Current Settings (ApplicationSettings)

```python
pgy1_supervision_ratio = "1:2"  ***REMOVED*** Static, does not vary by half
pgy2_supervision_ratio = "1:4"
pgy3_supervision_ratio = "1:4"
```

***REMOVED******REMOVED******REMOVED*** Proposed Future Settings

```python
***REMOVED*** Phase 2
clinic_max_physicians = 6
wednesday_am_intern_only = True
wednesday_am_exceptions = []  ***REMOVED*** List of exception dates

***REMOVED*** Phase 3 (deferred)
h1_pgy1_ratio = "1:2"
h2_pgy1_ratio = "1:3"
```
...
***REMOVED******REMOVED******REMOVED*** Unit Tests Required
