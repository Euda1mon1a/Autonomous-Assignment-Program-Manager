***REMOVED*** Engine Assignment Flow
...
***REMOVED******REMOVED*** Step-by-Step Flow

***REMOVED******REMOVED******REMOVED*** Step 1: Load Preserved Assignments
...
***REMOVED******REMOVED******REMOVED*** Step 2: Combine Preserved Assignments
...
***REMOVED******REMOVED******REMOVED*** Step 3: Build Context with Preserved Assignments
...
context = self._build_context(
    ...
    templates=templates,
    blocks=blocks,
    existing_assignments=preserved_assignments,  ***REMOVED*** Key: passed to context
)
```

***REMOVED******REMOVED******REMOVED*** Step 4: Run Solver
...
***REMOVED******REMOVED******REMOVED*** Step 5: Filter Solver Output
...
def _create_assignments_from_result(
    ...
    ***REMOVED*** Build set of occupied slots
    ...
    for person_id, block_id, template_id in result.assignments:
        ***REMOVED*** SKIP if this slot is already occupied by a preserved assignment
        if (person_id, block_id) in occupied_slots:
            skipped += 1
            continue

        ***REMOVED*** Create the assignment
        assignment = Assignment(...)
        self.assignments.append(assignment)
```

***REMOVED******REMOVED******REMOVED*** Step 6: Faculty Assignment with Preserved Check
...
def _assign_faculty(
    ...
    ***REMOVED*** Build occupied slots from BOTH preserved AND session assignments
    faculty_occupied_slots: set[tuple[UUID, UUID]] = set()
    faculty_ids = {f.id for f in faculty}

    ***REMOVED*** Check preserved assignments
    ...
    ***REMOVED*** Check already-created session assignments
    for a in self.assignments:
        if a.person_id in faculty_ids:
            faculty_occupied_slots.add((a.person_id, a.block_id))

    ***REMOVED*** Only assign faculty to unoccupied slots
    for faculty_member in faculty:
        for block in blocks:
            if (faculty_member.id, block.id) in faculty_occupied_slots:
                continue  ***REMOVED*** Skip - already occupied
            ***REMOVED*** ... create faculty supervision assignment
```

***REMOVED******REMOVED******REMOVED*** Step 7: Deferred Deletion

Old solver-managed assignments are deleted **after** successful solve:

```python
***REMOVED*** Old approach (dangerous):
***REMOVED*** DELETE existing assignments BEFORE solving
***REMOVED*** If solver fails, data is lost!

***REMOVED*** New approach (safe):
***REMOVED*** 1. Run solver
***REMOVED*** 2. If successful, create new assignments
***REMOVED*** 3. THEN delete old solver-managed assignments
***REMOVED*** 4. If solver fails, existing data is preserved
```

---

***REMOVED******REMOVED*** Activity Type Filters
...
***REMOVED*** When deleting old assignments, skip those in preserve_ids
...
if result.status != SolverStatus.SUCCESS:
    return ScheduleRunResult(
        status=207,  ***REMOVED*** Partial Content
        message=f"Solver failed: {result.status}",
        ...
    )

***REMOVED*** Only proceed with database changes if solver succeeded
```

---

***REMOVED******REMOVED*** Key Data Structures

***REMOVED******REMOVED******REMOVED*** `occupied_slots: set[tuple[UUID, UUID]]`

Set of (person_id, block_id) tuples representing slots that cannot be assigned.

***REMOVED******REMOVED******REMOVED*** `preserve_ids: set[UUID]`

Set of assignment IDs that should not be deleted during cleanup.

***REMOVED******REMOVED******REMOVED*** `SchedulingContext.existing_assignments`
