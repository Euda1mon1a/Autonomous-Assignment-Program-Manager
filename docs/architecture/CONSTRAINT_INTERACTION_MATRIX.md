***REMOVED*** Constraint Interaction Matrix
...
2. [Constraint Catalog](***REMOVED***constraint-catalog)
3. [Priority Hierarchy](***REMOVED***priority-hierarchy)
4. [Constraint Interactions](***REMOVED***constraint-interactions)
5. [Conflict Resolution](***REMOVED***conflict-resolution)
6. [Configuration Patterns](***REMOVED***configuration-patterns)
7. [Dependency Graph](***REMOVED***dependency-graph)
8. [Weight Tuning Guide](***REMOVED***weight-tuning-guide)
9. [Common Issues](***REMOVED***common-issues)
10. [Constraint Debugging](***REMOVED***constraint-debugging)
...
***REMOVED******REMOVED******REMOVED*** ACGME Compliance Constraints (Hard)
...
***REMOVED******REMOVED******REMOVED*** Capacity & Coverage Constraints
...
***REMOVED******REMOVED******REMOVED*** Temporal Constraints (Hard)
...
***REMOVED******REMOVED******REMOVED*** Faculty Primary Duty Constraints (Airtable-Driven)
...
***REMOVED******REMOVED******REMOVED*** Faculty Role Constraints (Hard)
...
***REMOVED******REMOVED******REMOVED*** FMIT (Family Medicine Inpatient Teaching) Constraints
...
***REMOVED******REMOVED******REMOVED*** Call Coverage Constraints (Hard)
...
***REMOVED******REMOVED******REMOVED*** Overnight Call Generation & Post-Call (Hard, Opt-In)
...
***REMOVED******REMOVED******REMOVED*** Call Equity Constraints (Soft)
...
***REMOVED******REMOVED******REMOVED*** Equity & Continuity Constraints (Soft)
...
***REMOVED******REMOVED******REMOVED*** Resilience Constraints (Soft)

***REMOVED******REMOVED******REMOVED******REMOVED*** Tier 1: Core Resilience (Enabled by Default)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Tier 2: Strategic Resilience (Disabled by Default)
...
***REMOVED******REMOVED******REMOVED*** Sports Medicine Constraints (Hard, Opt-In)
...
***REMOVED******REMOVED******REMOVED*** Faculty Preference Constraints (Soft)
...
***REMOVED******REMOVED*** Priority Hierarchy

***REMOVED******REMOVED******REMOVED*** Hard Constraint Evaluation Order
...
***REMOVED******REMOVED******REMOVED*** Soft Constraint Weight Order

Objective function: `maximize(coverage) - Σ(weight × penalty)`

```
Coverage (1000.0)           ***REMOVED*** Dominant objective
  ├─ N1Vulnerability (25.0)   ***REMOVED*** Resilience Tier 2
  ├─ UtilizationBuffer (20.0) ***REMOVED*** Resilience Tier 1
  ├─ HubProtection (15.0)     ***REMOVED*** Resilience Tier 1
  ├─ FacultyClinicEquity (15.0)
  ├─ ZoneBoundary (12.0)      ***REMOVED*** Resilience Tier 2
  ├─ SundayCallEquity (10.0)
  ├─ Equity (10.0)
  ├─ CallSpacing (8.0)
  ├─ PreferenceTrail (8.0)    ***REMOVED*** Resilience Tier 2
...
***REMOVED******REMOVED*** Constraint Interactions

***REMOVED******REMOVED******REMOVED*** Synergistic Interactions (Constraints That Work Together)
...
***REMOVED******REMOVED******REMOVED*** Competitive Interactions (Constraints That Compete)
...
***REMOVED******REMOVED******REMOVED*** Conflicting Interactions (Potential Issues)
...
***REMOVED******REMOVED******REMOVED*** Handling Infeasible Schedules

When hard constraints cannot be satisfied:

```python
***REMOVED*** Solver returns None if infeasible
result = solver.solve(context, constraints)

if result is None:
    ***REMOVED*** Common causes:
    ***REMOVED*** 1. Too many absences (AvailabilityConstraint)
    ***REMOVED*** 2. Insufficient faculty (SupervisionRatioConstraint)
    ***REMOVED*** 3. Over-constrained faculty targets (FacultyPrimaryDutyClinicConstraint)
    ***REMOVED*** 4. Conflicting FMIT/clinic requirements

    ***REMOVED*** Mitigation strategies:
    ***REMOVED*** - Reduce faculty clinic minimums
    ***REMOVED*** - Disable optional hard constraints (FMIT, Sports Medicine)
    ***REMOVED*** - Extend date range
    ***REMOVED*** - Hire more faculty
```

***REMOVED******REMOVED******REMOVED*** Debugging Constraint Conflicts
...
***REMOVED******REMOVED******REMOVED*** 1. Default Configuration (Balanced)
...
***REMOVED******REMOVED******REMOVED*** 2. Resilience-Aware Configuration (Robust)

```python
manager = ConstraintManager.create_resilience_aware(
    target_utilization=0.80,
    tier=2  ***REMOVED*** Enable all resilience constraints
)
```
...
***REMOVED******REMOVED******REMOVED*** 3. Minimal Configuration (Fast)
...
***REMOVED******REMOVED******REMOVED*** 4. Strict Configuration (Aggressive Optimization)
...
***REMOVED******REMOVED******REMOVED*** 5. FMIT-Enabled Configuration (Inpatient Focus)
...
***REMOVED******REMOVED******REMOVED*** 6. Call-Focused Configuration (Equity Priority)

```python
manager = ConstraintManager.create_default()

***REMOVED*** Increase call equity weights
for c in manager._soft_constraints:
    if c.constraint_type == ConstraintType.EQUITY and "Call" in c.name:
        c.weight *= 2  ***REMOVED*** Double call equity importance
```
...
***REMOVED******REMOVED******REMOVED*** 7. Custom Configuration Template

```python
manager = ConstraintManager()

***REMOVED*** Add only what you need
...
manager.add(EquityConstraint(weight=15.0))  ***REMOVED*** Increase equity importance
manager.add(HubProtectionConstraint(weight=20.0))  ***REMOVED*** Increase hub protection

***REMOVED*** Conditionally enable
if has_fmit_service:
    manager.add(FMITWeekBlockingConstraint())

if has_resilience_data:
    manager.add(UtilizationBufferConstraint(target_utilization=0.75))  ***REMOVED*** More conservative
```
...
***REMOVED******REMOVED******REMOVED*** Hard Constraint Dependencies
...
***REMOVED******REMOVED******REMOVED*** Soft Constraint Dependencies
...
***REMOVED******REMOVED******REMOVED*** Data Flow Dependencies
...
***REMOVED******REMOVED*** Weight Tuning Guide

***REMOVED******REMOVED******REMOVED*** Weight Scaling Principles
...
***REMOVED******REMOVED******REMOVED*** Tuning Scenarios

***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 1: Improve Workload Balance
...
***REMOVED*** Increase equity weight
for c in manager._soft_constraints:
    if c.name == "Equity":
        c.weight = 20.0  ***REMOVED*** Double from 10.0
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 2: Protect Critical Faculty
...
***REMOVED*** Increase hub protection weight
for c in manager._soft_constraints:
    if c.name == "HubProtection":
        c.weight = 30.0  ***REMOVED*** Double from 15.0
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 3: Improve Call Distribution
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 4: Reduce System Utilization (More Buffer)
...
```python
manager = ConstraintManager.create_resilience_aware(
    target_utilization=0.75  ***REMOVED*** Reduce from 0.80 to 0.75
)

***REMOVED*** Increase buffer weight
for c in manager._soft_constraints:
    if c.name == "UtilizationBuffer":
        c.weight = 40.0  ***REMOVED*** Double from 20.0
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 5: Balance Coverage vs. Equity Tradeoff
...
***REMOVED*** Option A: Reduce coverage dominance (aggressive)
for c in manager._soft_constraints:
    if c.name == "Coverage":
        c.weight = 500.0  ***REMOVED*** Reduce from 1000.0
    if c.name == "Equity":
        c.weight = 25.0  ***REMOVED*** Increase from 10.0

***REMOVED*** Option B: Keep coverage dominant, but boost equity (conservative)
for c in manager._soft_constraints:
    if c.name == "Equity":
        c.weight = 50.0  ***REMOVED*** 5× increase (still <5% of coverage weight)
```
...
***REMOVED******REMOVED******REMOVED*** Weight Tuning Best Practices
...
***REMOVED******REMOVED******REMOVED*** Issue 1: Solver Times Out / No Solution Found
...
**Diagnosis:**
```python
***REMOVED*** Run pre-flight validation
...
***REMOVED******REMOVED******REMOVED*** Issue 2: Poor Workload Balance (High Equity Penalty)
...
***REMOVED******REMOVED******REMOVED*** Issue 3: Wednesday AM Has No Residents
...
**Diagnosis:**
```python
***REMOVED*** Check PGY-1 availability
...
***REMOVED******REMOVED******REMOVED*** Issue 4: FMIT Weeks Cause Coverage Gaps
...
**Diagnosis:**
```python
***REMOVED*** Check FMIT week overlap
...
***REMOVED******REMOVED******REMOVED*** Issue 5: Resilience Constraints Have No Effect
...
***REMOVED*** Check constraint weights
...
***REMOVED******REMOVED******REMOVED*** Issue 6: Call Distribution Unfair (Some Faculty Get All Sundays)
...
**Diagnosis:**
```python
***REMOVED*** Check Sunday call distribution
...
***REMOVED******REMOVED*** Constraint Debugging

***REMOVED******REMOVED******REMOVED*** Debugging Workflow
...
***REMOVED******REMOVED******REMOVED*** Debugging Tools

***REMOVED******REMOVED******REMOVED******REMOVED*** Tool 1: Constraint Isolation

```python
***REMOVED*** Test each constraint individually
manager = ConstraintManager()
manager.add(AvailabilityConstraint())
manager.add(OnePersonPerBlockConstraint())

***REMOVED*** Add suspect constraint
manager.add(FMITWeekBlockingConstraint())
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Tool 2: Constraint Dependency Checker
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Tool 4: Constraint Conflict Detector
...
def detect_conflicts(manager, assignments, context):
    ...
    ***REMOVED*** Identify high-penalty constraints
...
***REMOVED******REMOVED******REMOVED*** Constraint Inventory
...
***REMOVED******REMOVED******REMOVED*** Default Configuration Breakdown
...
***REMOVED******REMOVED******REMOVED*** Weight Distribution (Default Config)
