***REMOVED*** Hub-Epidemiology Bridge Specification
...
***REMOVED******REMOVED******REMOVED*** 1.1 Hub Centrality Metrics (Network Theory)
...
***REMOVED******REMOVED******REMOVED*** 1.2 Epidemiological Contact Rate
...
***REMOVED******REMOVED******REMOVED*** 1.3 Bridge Formula: Centrality → Contact Rate
...
***REMOVED******REMOVED******REMOVED*** 1.4 Individual Reproduction Number
...
***REMOVED******REMOVED*** 2. Centrality → Contact Rate Mapping Implementation

***REMOVED******REMOVED******REMOVED*** 2.1 Function Specification
...
def centrality_to_contact_rate(
    ...
    Example:
        ...
        5.0  ***REMOVED*** Lowest centrality → 0.5× baseline
        >>> centrality_to_contact_rate(1.0, baseline_contacts=10)
        20.0  ***REMOVED*** Highest centrality → 2.0× baseline
        >>> centrality_to_contact_rate(0.5, baseline_contacts=10)
        12.5  ***REMOVED*** Mid centrality → 1.25× baseline
    """
    if not 0 <= centrality <= 1:
        raise ValueError(f"Centrality must be in [0, 1], got {centrality}")

    ***REMOVED*** Linear interpolation between min and max multiplier
...
***REMOVED******REMOVED******REMOVED*** 2.2 Validation Examples
...
***REMOVED******REMOVED*** 3. Super-spreader Detection Strategy

***REMOVED******REMOVED******REMOVED*** 3.1 Three-Signal Approach

Super-spreaders are identified using **three complementary signals**:

***REMOVED******REMOVED******REMOVED******REMOVED*** Signal 1: High Centrality (Structural Potential)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Signal 2: High SIR Contact Degree (Epidemiological Observation)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Signal 3: Combined Structural + State (Immediate Risk)
...
***REMOVED******REMOVED******REMOVED*** 3.2 Super-spreader Classification Matrix
...
***REMOVED******REMOVED******REMOVED*** 3.3 Implementation Function
...
class SuperSpreaderRisk(str, Enum):
    ...
    STRUCTURAL_CANDIDATE = "structural_candidate"  ***REMOVED*** High centrality, healthy
    HIGH_RISK_PRE_SPREADER = "high_risk_pre_spreader"  ***REMOVED*** High centrality, at risk
    ACTIVE_EARLY = "active_early"  ***REMOVED*** High centrality, burned out, <3 cases
    ACTIVE_CONFIRMED = "active_confirmed"  ***REMOVED*** High centrality, burned out, ≥3 cases
    EMPIRICAL_SPREADER = "empirical_spreader"  ***REMOVED*** Mid centrality, ≥3 observed cases
    ANOMALY = "anomaly"  ***REMOVED*** Low centrality but high spread (data quality issue)
    LOW_RISK = "low_risk"  ***REMOVED*** Does not meet criteria
...
class SuperSpreaderProfile:
    ...
    ***REMOVED*** Epidemiological metrics
    contact_rate: float
    individual_r0: float
    burnout_state: str  ***REMOVED*** susceptible, at_risk, burned_out, recovered

    ***REMOVED*** Observed spread
    secondary_cases: int
    close_contacts_count: int

    ***REMOVED*** Classification
    risk_level: SuperSpreaderRisk
    intervention_priority: int  ***REMOVED*** 1-5 (5 = highest)
...
***REMOVED******REMOVED*** 4. Data Flow and Integration

***REMOVED******REMOVED******REMOVED*** 4.1 System Architecture
...
***REMOVED******REMOVED******REMOVED*** 4.2 Data Flow Sequence
...
***REMOVED******REMOVED******REMOVED*** 4.3 Integration Points

***REMOVED******REMOVED******REMOVED******REMOVED*** 4.3.1 Hub Analysis → Bridge
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4.3.2 Bridge → Burnout Epidemiology
**Input:** Contact rates dictionary
**Output:** Updated SIR model with heterogeneous contact rates

```python
***REMOVED*** In burnout_epidemiology.py
class BurnoutEpidemiology:
    ...
    def get_individual_contact_rate(self, resident_id: UUID) -> float:
        ...
        """
        return self.contact_rates.get(resident_id, 10.0)  ***REMOVED*** Default baseline = 10
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 4.3.3 UCI Integration
...
```python
***REMOVED*** In unified_critical_index.py
class UnifiedCriticalIndexAnalyzer:
    def analyze_population(
        ...
        ***REMOVED*** Build network if not already done
        if not self._network:
            self.build_network(faculty, assignments)

        ***REMOVED*** NEW: Export contact rates from hub centrality
        ...
        ***REMOVED*** NEW: Initialize epidemiology module with contact rates
        ...
        ***REMOVED*** Continue with existing population analysis...
        ***REMOVED*** ...
```

---

***REMOVED******REMOVED*** 5. Enhanced UCI Integration

***REMOVED******REMOVED******REMOVED*** 5.1 Current UCI Epidemiology Score (Existing)
...
def compute_epidemiology_score(
    ...
    ***REMOVED*** Check burnout state
    is_burned_out = False
    if burnout_states:
        state = burnout_states.get(faculty_id, "susceptible")
        is_burned_out = state in ["burned_out", "at_risk"]

    ***REMOVED*** Raw score combines network position with current state
...
***REMOVED******REMOVED******REMOVED*** 5.2 Enhanced Epidemiology Score (With Bridge)
...
def compute_epidemiology_score_enhanced(
    ...
    ***REMOVED*** Composite centrality (same as hub score)
    composite = (
    ...
    ***REMOVED*** Raw score now based on R₀ (normalized to [0, 1])
    ***REMOVED*** Typical R₀ range: 0-50, normalize to [0, 1]
    raw_score = min(1.0, r0_individual / 50.0)

    ***REMOVED*** Critical if active super-spreader
...
***REMOVED******REMOVED*** 6. Test Cases and Validation

***REMOVED******REMOVED******REMOVED*** 6.1 Unit Test: Centrality to Contact Rate

```python
***REMOVED*** tests/resilience/test_hub_epidemiology_bridge.py
...
class TestIndividualR0:
    ...
    def test_r0_at_baseline(self):
        ...
        ***REMOVED*** R₀ = beta × contact_rate × (1/gamma)
        ***REMOVED*** R₀ = 0.05 × 12.5 × 50 = 31.25
    ...
    def test_r0_for_super_spreader(self):
        ...
        ***REMOVED*** contact_rate = 10 × (0.5 + 1.5 × 0.9) = 18.5
        ***REMOVED*** R₀ = 0.05 × 18.5 × 50 = 46.25
...
***REMOVED******REMOVED******REMOVED*** 6.2 Integration Test: Super-spreader Detection
...
***REMOVED******REMOVED******REMOVED*** 6.3 Cross-Validation Test
...
class TestHubEpidemiologyCrossValidation:
    ...
    def test_structural_vs_empirical_alignment(self):
        ...
        ***REMOVED*** Structural candidate (preventive)
        ...
        ***REMOVED*** Same person after burnout and spread
...
***REMOVED******REMOVED*** 7. Implementation Checklist

***REMOVED******REMOVED******REMOVED*** Phase 1: Core Bridge Functions ✓
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Integration with Existing Modules
...
***REMOVED******REMOVED******REMOVED*** Phase 3: UCI Enhancement
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Documentation
...
***REMOVED******REMOVED*** 8. Expected Impact

***REMOVED******REMOVED******REMOVED*** 8.1 Preventive Super-spreader Detection
...
***REMOVED******REMOVED******REMOVED*** 8.2 Cross-Validation and Anomaly Detection
...
***REMOVED******REMOVED******REMOVED*** 8.3 Enhanced Intervention Targeting
...
***REMOVED******REMOVED******REMOVED*** 8.4 Quantified Risk Metrics
...
***REMOVED******REMOVED*** 9. Future Extensions

***REMOVED******REMOVED******REMOVED*** 9.1 Dynamic Contact Rate Adjustment
...
def update_contact_rates_from_schedule(
    ...
    """
    ***REMOVED*** Count actual shared shifts in recent time window
    ***REMOVED*** ...
```

***REMOVED******REMOVED******REMOVED*** 9.2 Temporal Super-spreader Analysis
...
***REMOVED******REMOVED******REMOVED*** 9.3 Intervention Simulation
...
***REMOVED******REMOVED******REMOVED*** 9.4 Multi-layer Networks
...
***REMOVED******REMOVED******REMOVED*** Implementation References
...
***REMOVED******REMOVED*** 12. Approval and Status
