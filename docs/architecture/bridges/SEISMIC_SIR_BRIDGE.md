***REMOVED*** Seismic-SIR Bridge Specification
...
1. [Mathematical Foundation](***REMOVED***1-mathematical-foundation)
2. [Data Flow Diagram](***REMOVED***2-data-flow-diagram)
3. [Implementation Specification](***REMOVED***3-implementation-specification)
4. [Configuration](***REMOVED***4-configuration)
5. [Test Cases](***REMOVED***5-test-cases)
6. [Integration Points](***REMOVED***6-integration-points)
7. [Implementation Estimate](***REMOVED***7-implementation-estimate)
8. [References](***REMOVED***8-references)

---

***REMOVED******REMOVED*** 1. Mathematical Foundation

***REMOVED******REMOVED******REMOVED*** 1.1 STA/LTA to Beta Mapping
...
***REMOVED******REMOVED******REMOVED*** 1.2 Transformation Function
...
***REMOVED******REMOVED******REMOVED*** 1.3 Aggregation Across Multiple Residents
...
***REMOVED******REMOVED******REMOVED*** 1.4 Temporal Smoothing
...
***REMOVED******REMOVED******REMOVED*** 1.5 Clamping
...
***REMOVED******REMOVED*** 2. Data Flow Diagram
...
***REMOVED******REMOVED*** 3. Implementation Specification

***REMOVED******REMOVED******REMOVED*** 3.1 New Class: `SeismicSIRBridge`
...
class BetaAdjustment:
    ...
    adjustment_magnitude: float  ***REMOVED*** Percentage change
...
class SeismicSIRBridge:
    ...
    Example:
        ...
        ***REMOVED*** Process new seismic alerts
        alerts = detector.detect_precursors(...)
        bridge.update_from_alerts(alerts)

        ***REMOVED*** SIR model now uses updated β
    ...
    def _calculate_beta_delta(self) -> float:
        ...
        ***REMOVED*** Get network degrees for weighting
        network = self.epidemiology_model.network
        total_delta = 0.0
        total_weight = 0.0

        for resident_id, alert in self._active_alerts.items():
            ***REMOVED*** Transform STA/LTA to beta delta
            f_r = self._transform_sta_lta(alert.sta_lta_ratio)

            ***REMOVED*** Weight by network connectivity (super-spreaders matter more)
    ...
    def _transform_sta_lta(ratio: float) -> float:
        ...
        """
        if ratio < 2.5:
            ***REMOVED*** Below trigger threshold - no adjustment
            return 0.0

        elif ratio < 10.0:
            ***REMOVED*** Moderate range - logarithmic scaling
            return np.log2(ratio / 2.5)

        else:
            ***REMOVED*** Critical range - linear continuation
...
***REMOVED******REMOVED******REMOVED*** 3.2 New Method: `BurnoutEpidemiology.update_transmission_rate()`
...
def update_transmission_rate(self, new_beta: float):
    ...
    ***REMOVED*** Store as instance variable for use in simulations
...
def __init__(self, social_network: nx.Graph):
    ...
    ***REMOVED*** Default beta (can be overridden by bridge)
    self._current_beta: float = 0.05  ***REMOVED*** 5% per week default
```
...
def simulate_sir_spread(
    ...
    beta: float | None = None,  ***REMOVED*** Make optional
    gamma: float = 0.02,
    steps: int = 52,
) -> list[dict]:
    """
    ...
    """
    ***REMOVED*** Use current beta if not explicitly provided
    if beta is None:
        beta = self._current_beta

    ***REMOVED*** ... rest of existing code ...
```

***REMOVED******REMOVED******REMOVED*** 3.3 Integration Service
...
class SeismicSIRIntegrationService:
    ...
    async def analyze_resident_behavioral_data(
        ...
        ***REMOVED*** Detect precursors for each signal type
        ...
        ***REMOVED*** Update SIR model with new alerts
        if all_alerts:
    ...
    async def get_integrated_risk_report(self) -> dict:
        ...
        """
        ***REMOVED*** Get current beta
        current_beta = self.bridge.get_current_beta()

        ***REMOVED*** Get active seismic alerts
        active_alerts = len(self.bridge._active_alerts)

        ***REMOVED*** Calculate current Rt with updated beta
        burned_out = set(self.epidemiology_model.burnout_history.keys())  ***REMOVED*** Simplified
        epi_report = self.epidemiology_model.calculate_reproduction_number(
            burned_out_residents=burned_out
        )

        ***REMOVED*** Get recent beta adjustments
...
***REMOVED******REMOVED*** 4. Configuration

***REMOVED******REMOVED******REMOVED*** 4.1 Configuration Parameters
...
```python
***REMOVED*** Seismic-SIR Bridge Configuration
...
***REMOVED******REMOVED******REMOVED*** 4.2 Environment Variables

**.env.example**:

```bash
***REMOVED*** Seismic-SIR Bridge
SEISMIC_SIR_SENSITIVITY=0.3          ***REMOVED*** Sensitivity (0.0-1.0)
SEISMIC_SIR_SMOOTHING=0.2            ***REMOVED*** Smoothing factor (0.0-1.0)
SEISMIC_SIR_BETA_BASE=0.05           ***REMOVED*** Baseline beta
SEISMIC_SIR_UPDATE_INTERVAL_HOURS=6  ***REMOVED*** Update frequency
```

***REMOVED******REMOVED******REMOVED*** 4.3 Parameter Tuning Guide
...
def network():
    ...
    ***REMOVED*** Add edges (social connections)
...
class TestSeismicSIRBridge:
    ...
    def test_no_adjustment_below_threshold(self, bridge):
        ...
        ***REMOVED*** Create alert with ratio below threshold
        alert = SeismicAlert(
            signal_type=PrecursorSignal.SWAP_REQUESTS,
            sta_lta_ratio=2.0,  ***REMOVED*** Below 2.5 threshold
        ...
        ***REMOVED*** Beta should remain at baseline
    ...
    def test_moderate_adjustment(self, bridge):
        ...
        alert = SeismicAlert(
            signal_type=PrecursorSignal.SWAP_REQUESTS,
            sta_lta_ratio=5.0,  ***REMOVED*** Moderate elevation
            trigger_time=datetime.now(),
            severity="medium",
        ...
        ***REMOVED*** Beta should increase
        assert adjustment.beta_adjusted > 0.05
        assert adjustment.beta_adjusted < 0.10  ***REMOVED*** Not too high
    ...
    def test_critical_adjustment(self, bridge):
        ...
        alert = SeismicAlert(
            ...
            sta_lta_ratio=15.0,  ***REMOVED*** Critical elevation
            trigger_time=datetime.now(),
            severity="critical",
        ...
        ***REMOVED*** Beta should increase significantly
        assert adjustment.beta_adjusted > 0.10
        assert adjustment.adjustment_magnitude > 50  ***REMOVED*** >50% increase
    ...
    def test_multiple_alerts_aggregation(self, bridge, network):
        ...
        ***REMOVED*** Beta should reflect combined signals
    ...
    def test_smoothing_reduces_volatility(self, bridge):
        ...
        ***REMOVED*** Beta should increase but smoothed (not jump immediately to max)
        assert beta_after_second > beta_after_first
        ***REMOVED*** With smoothing_factor=0.2, change should be gradual
        beta_change = beta_after_second - beta_after_first
        assert beta_change < 0.05  ***REMOVED*** Not a huge jump

    def test_clamping_prevents_invalid_beta(self, bridge):
        """Test that beta is clamped to valid range [0.01, 0.95]."""
        ***REMOVED*** Create extreme alert
        alert = SeismicAlert(
            signal_type=PrecursorSignal.SICK_CALLS,
            sta_lta_ratio=100.0,  ***REMOVED*** Extremely high
            trigger_time=datetime.now(),
            severity="critical",
        ...
        ***REMOVED*** Beta should be clamped to max (0.95)
    ...
    def test_old_alerts_pruned(self, bridge):
        ...
        ***REMOVED*** Create old alert (8 days ago)
        ...
        ***REMOVED*** Add to active alerts manually
        bridge._active_alerts[old_alert.resident_id] = old_alert

        ***REMOVED*** Create new alert
        ...
        ***REMOVED*** Old alert should be pruned (>7 days)
    ...
    def test_transform_sta_lta_function(self, bridge):
        ...
        ***REMOVED*** Moderate range (logarithmic)
        f_5 = bridge._transform_sta_lta(5.0)
        f_10 = bridge._transform_sta_lta(10.0)
        assert f_5 > 0
        assert f_10 > f_5
        assert f_10 == pytest.approx(2.0, rel=0.01)  ***REMOVED*** log2(10/2.5) = log2(4) = 2
    ...
    def test_adjustment_history_tracking(self, bridge):
        ...
        ***REMOVED*** Create multiple alerts over time
        for i in range(5):
            alert = SeismicAlert(
                signal_type=PrecursorSignal.SWAP_REQUESTS,
                sta_lta_ratio=3.0 + i,
...
class TestSeismicSIRIntegration:
    ...
    async def test_full_pipeline(self, network):
        ...
        ***REMOVED*** Create time series data showing increasing stress
        resident_id = list(network.nodes())[0]
        time_series = [0, 1, 0, 1, 0, 1, 2, 3, 5, 7, 8, 10, 12, 15] * 3  ***REMOVED*** 42 points

        ***REMOVED*** Detect precursors
        ...
        ***REMOVED*** Update bridge
        adjustment = bridge.update_from_alerts(alerts)

        ***REMOVED*** Verify beta increased
        assert adjustment.beta_adjusted > 0.05

        ***REMOVED*** Run SIR simulation with new beta
        ...
        ***REMOVED*** Verify epidemic spreads faster with elevated beta
...
***REMOVED******REMOVED*** 6. Integration Points

***REMOVED******REMOVED******REMOVED*** 6.1 Celery Task for Periodic Updates
...
def update_beta_from_seismic_signals():
    ...
    """
    ***REMOVED*** Implementation would fetch latest behavioral data and update
    pass
```

***REMOVED******REMOVED******REMOVED*** 6.2 API Endpoint
...
***REMOVED******REMOVED******REMOVED*** 6.3 Dashboard Widget
...
***REMOVED******REMOVED*** 7. Implementation Estimate

***REMOVED******REMOVED******REMOVED*** 7.1 Effort Breakdown
...
***REMOVED******REMOVED******REMOVED*** 7.2 Files Modified
...
***REMOVED******REMOVED******REMOVED*** 7.3 Dependencies
...
***REMOVED******REMOVED******REMOVED*** 8.1 Seismology
...
***REMOVED******REMOVED******REMOVED*** 8.2 Epidemiology

- Kermack, W. O., & McKendrick, A. G. (1927). "A contribution to the mathematical theory of epidemics." *Proceedings of the Royal Society A*, 115(772), 700-721.
- Anderson, R. M., & May, R. M. (1991). *Infectious Diseases of Humans: Dynamics and Control*. Oxford University Press.

***REMOVED******REMOVED******REMOVED*** 8.3 Burnout Contagion
...
***REMOVED******REMOVED******REMOVED*** 8.4 Related Documentation
...
***REMOVED******REMOVED*** Appendix: Example Usage
...
***REMOVED*** Setup
network = nx.karate_club_graph()  ***REMOVED*** Example social network
seismic_detector = BurnoutEarlyWarning(short_window=5, long_window=30)
epi_model = BurnoutEpidemiology(social_network=network)
...
***REMOVED*** Collect behavioral data
resident_id = list(network.nodes())[0]
...
***REMOVED*** Run epidemic simulation with updated beta
initial_infected = {resident_id}
