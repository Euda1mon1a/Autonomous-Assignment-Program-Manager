***REMOVED*** Erlang-N1 Bridge Specification
...
***REMOVED******REMOVED******REMOVED*** Current State (Binary N-1)

```python
***REMOVED*** From backend/app/resilience/contingency.py
vulnerabilities = analyzer.analyze_n1(faculty, blocks, assignments, coverage_requirements)

***REMOVED*** Returns:
***REMOVED*** Vulnerability(
***REMOVED***     faculty_id=UUID('abc...'),
***REMOVED***     severity="critical",  ***REMOVED*** Based on is_unique_provider
***REMOVED***     affected_blocks=15,
***REMOVED***     is_unique_provider=True
***REMOVED*** )
```
...
***REMOVED******REMOVED******REMOVED*** Desired State (Quantified N-1 Margins)

```python
***REMOVED*** Enhanced output
margin_result = bridge.quantify_n1_margins(n1_scenarios)

***REMOVED*** Returns:
***REMOVED*** N1MarginResult(
***REMOVED***     faculty_id=UUID('abc...'),
***REMOVED***     binary_survives=True,        ***REMOVED*** Original binary result
***REMOVED***     margin_score=0.18,            ***REMOVED*** NEW: 18% margin (critical!)
***REMOVED***     offered_load_increase=0.35,   ***REMOVED*** 35% more load on survivors
***REMOVED***     wait_probability=0.42,        ***REMOVED*** 42% chance of delayed coverage
***REMOVED***     severity_classification="critical_low_margin"  ***REMOVED*** Binary pass but risky
***REMOVED*** )
```
...
***REMOVED******REMOVED******REMOVED*** 1. Erlang C Probability of Waiting
...
***REMOVED******REMOVED******REMOVED*** 2. N-1 Margin Calculation
...
***REMOVED******REMOVED******REMOVED*** 3. Offered Load Redistribution

When a faculty member becomes unavailable:

```python
***REMOVED*** Original state
A_original = λ * μ  ***REMOVED*** Total offered load
c_original = N      ***REMOVED*** Total faculty

***REMOVED*** After faculty loss
load_lost = (assignments_by_faculty[lost_id].count / total_assignments) * A_original
A_n1 = A_original   ***REMOVED*** Same total workload
c_n1 = N - 1        ***REMOVED*** One fewer server

***REMOVED*** Offered load per remaining faculty
load_per_faculty = A_n1 / c_n1
load_increase_pct = (A_n1 / c_n1) - (A_original / c_original)
```

***REMOVED******REMOVED******REMOVED*** 4. Service Level Under N-1

```python
service_level_n1 = 1 - P(wait) * exp(-(c_n1 - A_n1) * target_wait / μ)

***REMOVED*** Example target: 95% coverage within 15 minutes
target_wait = 0.25 hours  ***REMOVED*** 15 min
service_time = 2.0 hours  ***REMOVED*** Avg shift duration

***REMOVED*** If service_level_n1 < 0.95, system is degraded even though it "survives"
```

---

***REMOVED******REMOVED*** Data Flow Diagram
...
***REMOVED******REMOVED******REMOVED*** New Module: `backend/app/resilience/erlang_n1_bridge.py`
...
class N1MarginResult:
    ...
    ***REMOVED*** Original binary analysis
    binary_survives: bool
    affected_blocks: int
    is_unique_provider: bool

    ***REMOVED*** Enhanced queuing-based metrics
    margin_score: float           ***REMOVED*** 0.0-1.0 (0=critical, 1=comfortable)
    offered_load_increase: float  ***REMOVED*** Fractional increase in load per faculty
    wait_probability: float       ***REMOVED*** P(wait > 0) under N-1
    service_level_n1: float       ***REMOVED*** P(coverage within target time)

    ***REMOVED*** Interpretation
    severity_classification: str  ***REMOVED*** "comfortable", "marginal", "critical", "unstable"
...
class ErlangN1Bridge:
    ...
    def __init__(
        self,
        target_wait_time: float = 0.25,     ***REMOVED*** 15 min in hours
        avg_service_time: float = 2.0,      ***REMOVED*** 2 hour shifts
        ...
        self.thresholds = margin_thresholds or {
            "critical": 0.2,    ***REMOVED*** < 20% margin = critical
            "marginal": 0.5,    ***REMOVED*** 20-50% = marginal
            "comfortable": 0.5  ***REMOVED*** > 50% = comfortable
        }
    ...
    def quantify_n1_margins(
        ...
        ***REMOVED*** For each N-1 scenario, calculate margin
        for vuln in n1_vulnerabilities:
            ...
            ***REMOVED*** N-1 scenario parameters
            servers_n1 = total_servers - 1
            offered_load_n1 = offered_load_baseline  ***REMOVED*** Total load unchanged

            ***REMOVED*** Check for instability
            if offered_load_n1 >= servers_n1:
                ***REMOVED*** Unstable queue
                result = N1MarginResult(
                    ...
                    margin_score=-1.0,  ***REMOVED*** Negative indicates unstable
            ...
            ***REMOVED*** Calculate Erlang C metrics for N-1 scenario
            try:
                ...
                ***REMOVED*** Calculate margin score
                ***REMOVED*** Higher service level = higher margin
                margin_score = service_level

                ***REMOVED*** Classify severity based on margin
                ...
                ***REMOVED*** Check for false positives (binary pass but low margin)
                ...
                ***REMOVED*** Calculate load increase
                ...
                ***REMOVED*** Erlang calculation failed (likely instability)
    ...
    def get_prioritized_vulnerabilities(
        ...
        sorted_results = sorted(
            ...
            key=lambda r: (
                0 if r.margin_score < 0 else 1,  ***REMOVED*** Unstable first
                r.margin_score,                   ***REMOVED*** Then by margin (ascending)
                -r.affected_blocks                ***REMOVED*** Break ties by impact
            )
        )
    ...
    def _estimate_offered_load(
        ...
        ***REMOVED*** Total required coverage-hours
        total_blocks = len(assignments)

        ***REMOVED*** Assume each block is ~4 hours (half-day)
        hours_per_block = 4.0
        total_coverage_hours = total_blocks * hours_per_block

        ***REMOVED*** Total available capacity-hours
        ***REMOVED*** Assume 8 hours per day per faculty
        hours_per_faculty_per_day = 8.0
        total_capacity_hours = total_servers * days_in_period * hours_per_faculty_per_day

        ***REMOVED*** Offered load = demand / capacity in "servers" units
        ***REMOVED*** For Erlang, this needs to be in units of servers
...
***REMOVED******REMOVED******REMOVED*** N1MarginResult
...
class N1MarginResult:
    ...
    ***REMOVED*** Original binary analysis (preserved)
    binary_survives: bool         ***REMOVED*** True if system survives loss (existing metric)
    affected_blocks: int           ***REMOVED*** Number of blocks under-covered
    is_unique_provider: bool       ***REMOVED*** True if sole provider for some service

    ***REMOVED*** NEW: Queuing-based metrics
    margin_score: float            ***REMOVED*** 0.0-1.0 (service level under N-1)
                                   ***REMOVED*** < 0.0 = unstable queue
    offered_load_increase: float   ***REMOVED*** Fractional load increase per faculty (e.g., 0.15 = +15%)
    wait_probability: float        ***REMOVED*** P(wait > 0) under N-1 scenario
    service_level_n1: float        ***REMOVED*** P(coverage within target time)

    ***REMOVED*** Interpretation
    severity_classification: str   ***REMOVED*** "comfortable", "marginal", "critical", "unstable"
    recommendations: list[str]     ***REMOVED*** Action items prioritized by urgency

    ***REMOVED*** Metadata (for debugging/audit)
    servers_before: int            ***REMOVED*** Faculty count before loss
    servers_after: int             ***REMOVED*** Faculty count after loss (servers_before - 1)
    offered_load_before: float     ***REMOVED*** Baseline offered load
    offered_load_after: float      ***REMOVED*** Offered load under N-1
```
...
N1MarginResult(
    ...
    ***REMOVED*** Binary (existing)
    binary_survives=True,          ***REMOVED*** ✓ System survives (min coverage met)
    affected_blocks=8,
    is_unique_provider=False,

    ***REMOVED*** Quantified (NEW)
    margin_score=0.18,             ***REMOVED*** ⚠️  Only 18% margin - CRITICAL!
    offered_load_increase=0.23,    ***REMOVED*** 23% more load per remaining faculty
    wait_probability=0.47,         ***REMOVED*** 47% chance of delayed coverage
    service_level_n1=0.18,         ***REMOVED*** Only 18% on-time coverage
...
***REMOVED******REMOVED*** Prioritization Logic

***REMOVED******REMOVED******REMOVED*** Sorting Algorithm
...
def get_prioritized_vulnerabilities(margin_results: dict) -> list:
    ...
    return sorted(
        margin_results.values(),
        key=lambda r: (
            ***REMOVED*** 1. Unstable systems first (margin_score < 0)
            0 if r.margin_score < 0 else 1,

            ***REMOVED*** 2. Then by margin score (ascending = worst first)
            r.margin_score,

            ***REMOVED*** 3. Break ties by impact (more blocks = higher priority)
            -r.affected_blocks
        )
    )
```

***REMOVED******REMOVED******REMOVED*** Priority Tiers
...
***REMOVED******REMOVED******REMOVED*** Detection of "False Pass" Scenarios

```python
def detect_false_pass(result: N1MarginResult) -> bool:
    """Detect binary pass but critically low margin."""
    return (
        result.binary_survives and           ***REMOVED*** Binary says "ok"
        result.margin_score < 0.2            ***REMOVED*** But margin is critical
    )

***REMOVED*** Flag in recommendations
...
***REMOVED******REMOVED*** Integration with ResilienceService

***REMOVED******REMOVED******REMOVED*** New Method

```python
***REMOVED*** In backend/app/resilience/service.py
    ...
    ***REMOVED*** Detect false positives
    ...
    return {
        ...
        "prioritized_vulnerabilities": [
            ...
            for r in prioritized[:10]  ***REMOVED*** Top 10 priorities
        ],
        "margin_distribution": {
...
***REMOVED******REMOVED******REMOVED*** Test Suite: `backend/tests/resilience/test_erlang_n1_bridge.py`
...
class TestErlangN1Bridge:
    ...
    def test_comfortable_margin(self):
        ...
        ***REMOVED*** 12 faculty, losing 1 → 11 remain
        ***REMOVED*** Offered load = 7.0 (well below 11)
        ***REMOVED*** Expected: High margin, low wait probability
    ...
    def test_marginal_margin(self):
        ...
        ***REMOVED*** 10 faculty, offered load = 8.5 → tight but survivable
        ***REMOVED*** Expected: Marginal classification, moderate wait probability

        bridge = ErlangN1Bridge()
        ***REMOVED*** Setup to produce ~40% margin
        ***REMOVED*** ... (similar to above but tighter capacity)
    ...
    def test_critical_low_margin(self):
        ...
        ***REMOVED*** 8 faculty, offered load = 7.8 → very tight
        ***REMOVED*** Expected: Critical classification, high wait probability

        bridge = ErlangN1Bridge()
        ***REMOVED*** Setup to produce ~10% margin
        ***REMOVED*** ...
    ...
    def test_false_pass_detection(self):
        ...
        ***REMOVED*** Binary says "survives" (not unique provider)
        ***REMOVED*** But margin < 20% (critical)

        bridge = ErlangN1Bridge()
        binary_vuln = Vulnerability(
            faculty_id=uuid4(),
            faculty_name="Dr. Smith",
            severity="medium",              ***REMOVED*** Binary says "medium" (not critical)
            affected_blocks=5,
            is_unique_provider=False        ***REMOVED*** Binary says "survives"
        )

        ***REMOVED*** Setup to produce low margin despite binary pass
        ***REMOVED*** ...
    ...
    def test_unstable_scenario(self):
        ...
        ***REMOVED*** 5 faculty, offered load = 5.2 → unstable after losing 1 (4 < 5.2)

        bridge = ErlangN1Bridge()
        ***REMOVED*** Setup to produce instability
        ***REMOVED*** ...
    ...
    def test_prioritization_order(self):
        ...
        ***REMOVED*** Create scenarios with different margins
        scenarios = {
        ...
        ***REMOVED*** ... generate results for each

        prioritized = bridge.get_prioritized_vulnerabilities(margin_results)

        ***REMOVED*** Verify unstable comes first
        assert prioritized[0].severity_classification == "unstable"

        ***REMOVED*** Verify sorted by margin ascending
...
***REMOVED******REMOVED******REMOVED*** Example 1: Basic Analysis

```python
from app.resilience.contingency import ContingencyAnalyzer
from app.resilience.erlang_n1_bridge import ErlangN1Bridge

***REMOVED*** 1. Run binary N-1 analysis
analyzer = ContingencyAnalyzer()
binary_vulns = analyzer.analyze_n1(faculty, blocks, assignments, coverage_reqs)

***REMOVED*** 2. Enhance with quantified margins
bridge = ErlangN1Bridge(
    target_wait_time=0.25,    ***REMOVED*** 15 min acceptable wait
    avg_service_time=2.0      ***REMOVED*** 2 hour shifts
)
margin_results = bridge.quantify_n1_margins(
...
***REMOVED*** 3. Get prioritized action list
prioritized = bridge.get_prioritized_vulnerabilities(margin_results)

***REMOVED*** 4. Report top priorities
...
***REMOVED******REMOVED******REMOVED*** Example 2: Integration with Dashboard

```python
***REMOVED*** Dashboard endpoint: /api/resilience/n1-margins
...
class ErlangN1Bridge:
    def __init__(self):
        self._margin_cache = {}  ***REMOVED*** Cache recent calculations

    def quantify_n1_margins(self, ...):
        ***REMOVED*** Cache key: hash of (faculty_ids, assignment_count, blocks_count)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Multi-Faculty Losses (N-2)
...
def quantify_n2_margins(
    ...
    ***REMOVED*** Similar logic but with servers_n2 = servers - 2
```

***REMOVED******REMOVED******REMOVED*** Phase 3: Dynamic Margin Monitoring
...
def monitor_n1_margins():
    ...
    ***REMOVED*** Alert if any margin drops below threshold
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Margin-Based Scheduling

Use margins to guide schedule optimization:

```python
***REMOVED*** In scheduling solver: add soft constraint to maximize minimum margin
...
***REMOVED******REMOVED******REMOVED*** Erlang C Queuing Theory
...
***REMOVED******REMOVED******REMOVED*** Power Grid N-1 Analysis
