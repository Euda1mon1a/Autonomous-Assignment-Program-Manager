***REMOVED*** PID-Homeostasis Bridge Specification
...
4. [Three PID Controllers](***REMOVED***three-pid-controllers)
5. [Integration with Homeostasis](***REMOVED***integration-with-homeostasis)
6. [Anti-Windup Protection](***REMOVED***anti-windup-protection)
7. [Data Flow](***REMOVED***data-flow)
8. [Tuning Procedures](***REMOVED***tuning-procedures)
9. [Implementation](***REMOVED***implementation)
10. [Test Cases](***REMOVED***test-cases)
...
***REMOVED******REMOVED******REMOVED*** Problem with Current System

```python
***REMOVED*** Current: Ad-hoc correction in homeostasis.py
def _trigger_correction(self, loop, current_value, deviation, severity):
    ...
    ***REMOVED*** No mathematical basis for correction magnitude
    ***REMOVED*** No integral action to eliminate steady-state error
    ***REMOVED*** No derivative action to dampen oscillations
...
***REMOVED******REMOVED******REMOVED*** Solution: PID Control

Replace ad-hoc logic with proven PID control:

```python
***REMOVED*** New: Formal PID calculation
...
***REMOVED******REMOVED******REMOVED*** PID Control Equation
...
***REMOVED******REMOVED******REMOVED*** Three Terms Explained
...
***REMOVED******REMOVED******REMOVED*** Control Signal Interpretation
...
***REMOVED******REMOVED******REMOVED*** Stability Conditions
...
***REMOVED******REMOVED******REMOVED*** System Components
...
***REMOVED******REMOVED******REMOVED*** Design Pattern: Bridge
...
***REMOVED******REMOVED******REMOVED*** Responsibility Separation
...
***REMOVED******REMOVED*** Three PID Controllers

***REMOVED******REMOVED******REMOVED*** Controller Configurations

Each controller targets a specific process variable with custom-tuned gains.

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Utilization PID
...
class PIDConfig:
    ...
    name: str
    setpoint: float
    Kp: float  ***REMOVED*** Proportional gain
    Ki: float  ***REMOVED*** Integral gain
    Kd: float  ***REMOVED*** Derivative gain
    output_limits: tuple[float, float]  ***REMOVED*** Min, max control signal
    unit: str
    description: str

UTILIZATION_PID = PIDConfig(
    name="utilization",
    setpoint=0.75,  ***REMOVED*** 75% utilization target
    Kp=10.0,        ***REMOVED*** Strong immediate response (10% error → 100% correction)
    Ki=0.5,         ***REMOVED*** Moderate integral action (eliminate drift over ~2 weeks)
    Kd=2.0,         ***REMOVED*** Damping to prevent overshoot
    output_limits=(-0.2, 0.2),  ***REMOVED*** ±20% max correction per cycle
    unit="ratio",
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Workload Balance PID
...
WORKLOAD_PID = PIDConfig(
    ...
    setpoint=0.0,   ***REMOVED*** Target: zero imbalance (perfect balance)
    Kp=5.0,         ***REMOVED*** Moderate proportional (5% imbalance → 25% redistribution)
    Ki=0.3,         ***REMOVED*** Gentle integral (slow drift correction)
    Kd=1.0,         ***REMOVED*** Light damping (balance changes slowly)
    output_limits=(-0.15, 0.15),  ***REMOVED*** ±15% max redistribution per cycle
    unit="std_dev",
...
```python
***REMOVED*** Calculate imbalance from current workload distribution
...
error = 0.0 - imbalance  ***REMOVED*** Negative error means too much variance
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Coverage Rate PID
...
COVERAGE_PID = PIDConfig(
    name="coverage",
    setpoint=0.95,  ***REMOVED*** 95% coverage target
    Kp=12.0,        ***REMOVED*** Very strong immediate response (critical metric)
    Ki=0.8,         ***REMOVED*** Aggressive integral (eliminate gaps quickly)
    Kd=1.5,         ***REMOVED*** Moderate damping
    output_limits=(-0.1, 0.1),  ***REMOVED*** ±10% max coverage adjustment per cycle
    unit="ratio",
...
***REMOVED******REMOVED******REMOVED*** Gain Selection Summary
...
***REMOVED******REMOVED*** Integration with Homeostasis

***REMOVED******REMOVED******REMOVED*** Current Homeostasis Flow

```python
***REMOVED*** Existing: backend/app/resilience/homeostasis.py
...
class HomeostasisMonitor:
    def check_feedback_loop(self, loop_id: UUID, current_value: float) -> CorrectiveAction | None:
        ...
        ***REMOVED*** Check deviation from setpoint
        ...
        ***REMOVED*** ... (existing logic)
```

***REMOVED******REMOVED******REMOVED*** Enhanced PID Flow

```python
***REMOVED*** New: Replace _trigger_correction() with PID calculation
...
class PIDHomeostasisBridge:
    def __init__(self, homeostasis_monitor: HomeostasisMonitor):
        ...
        self.use_pid = True  ***REMOVED*** Feature flag for gradual rollout
    ...
    def check_feedback_loop_with_pid(
        ...
        ***REMOVED*** Get PID correction
        if self.use_pid and loop.setpoint.name in self.pid_bank.controllers:
            ...
            ***REMOVED*** Convert PID control signal to CorrectiveAction
            ...
            ***REMOVED*** Fallback to existing homeostasis logic
            return self.homeostasis.check_feedback_loop(loop_id, current_value)
```

***REMOVED******REMOVED******REMOVED*** Replacing Ad-Hoc Correction Calculation
...
def _pid_to_corrective_action(
    ...
    ***REMOVED*** Map control signal magnitude to severity
    severity = self._control_signal_to_severity(abs(control_signal))

    ***REMOVED*** Map setpoint + control direction to action type
    ...
    return CorrectiveAction(
        ...
        ***REMOVED*** NEW: Store PID diagnostics
        pid_diagnostics={
            "control_signal": control_signal,
            "P_term": pid_result["P"],
            "I_term": pid_result["I"],
...
def _pid_signal_to_action_type(
    ...
    ***REMOVED*** Positive control signal = need to increase metric
    ***REMOVED*** Negative control signal = need to decrease metric

    if setpoint_name == "coverage_rate":
        if control_signal > 0:
            return CorrectiveActionType.RECRUIT_BACKUP  ***REMOVED*** Increase coverage
        else:
            return CorrectiveActionType.REDUCE_SCOPE  ***REMOVED*** Reduce demand

    elif setpoint_name == "faculty_utilization":
        if control_signal > 0:
            return CorrectiveActionType.REDISTRIBUTE  ***REMOVED*** Add assignments
        else:
            return CorrectiveActionType.DEFER_ACTIVITY  ***REMOVED*** Reduce load

    elif setpoint_name == "workload_balance":
        if abs(control_signal) > 0.05:
            return CorrectiveActionType.REDISTRIBUTE  ***REMOVED*** Rebalance
        else:
...
***REMOVED******REMOVED******REMOVED*** Preserving Existing Features
...
***REMOVED******REMOVED******REMOVED*** Dual-Mode Operation (Migration)
...
class PIDHomeostasisBridge:
    def __init__(self, homeostasis_monitor: HomeostasisMonitor):
        ...
        ***REMOVED*** Feature flags for gradual rollout
        self.pid_enabled_for = {
            "coverage_rate": True,      ***REMOVED*** Start with critical metric
            "faculty_utilization": False,  ***REMOVED*** Deploy incrementally
            "workload_balance": False,     ***REMOVED*** Deploy last
        }

    def check_feedback_loop_with_pid(self, loop_id: UUID, current_value: float):
        loop = self.homeostasis.feedback_loops.get(loop_id)

        ***REMOVED*** Use PID if enabled for this setpoint
        if self.pid_enabled_for.get(loop.setpoint.name, False):
            return self._use_pid_control(loop, current_value)
        else:
            ***REMOVED*** Fallback to original homeostasis
            return self.homeostasis.check_feedback_loop(loop_id, current_value)
```
...
***REMOVED******REMOVED*** Anti-Windup Protection

***REMOVED******REMOVED******REMOVED*** The Windup Problem
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Integral Clamping (Simple, Effective)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Conditional Integration (Advanced)
...
def update(self, error: float, dt: float, output_limits: tuple[float, float]) -> float:
    ...
    ***REMOVED*** Calculate tentative control signal WITHOUT integral
    ...
    ***REMOVED*** Only integrate if we're NOT saturated
    if output_limits[0] < tentative_control < output_limits[1]:
        ...
        ***REMOVED*** Controller is saturated - don't accumulate error
        pass
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Back-Calculation Anti-Windup (Most Advanced)
...
def update(self, error: float, dt: float, output_limits: tuple[float, float]) -> float:
    ...
    ***REMOVED*** Tentative integral term
    self.integral += error * dt
    I = self.Ki * self.integral

    D = self.Kd * (error - self.previous_error) / dt

    ***REMOVED*** Calculate raw control signal
    raw_control = P + I + D
    ...
    ***REMOVED*** Back-calculation anti-windup
    if raw_control != control_signal:
        ***REMOVED*** We saturated - reduce integral by the excess
        excess = raw_control - control_signal
        Kt = 1.0 / self.Ki  ***REMOVED*** Anti-windup gain (typically 1/Ki)
...
***REMOVED******REMOVED******REMOVED*** Recommended Implementation
...
class PIDState:
    ...
    ***REMOVED*** Output limits
    ...
    def update(self, current_value: float) -> dict:
        ...
        now = datetime.now()
        dt = (now - self.previous_time).total_seconds() / 86400  ***REMOVED*** Days
        ...
        ***REMOVED*** Control signal with output limiting
        raw_control = P + I + D
...
***REMOVED******REMOVED******REMOVED*** Reset Integral on Setpoint Change
...
def set_setpoint(self, new_setpoint: float):
    ...
    if abs(new_setpoint - old_setpoint) > 0.01:
        ***REMOVED*** Significant setpoint change - reset integral
        self.integral = 0.0
...
***REMOVED******REMOVED******REMOVED*** End-to-End Flow Diagram
...
***REMOVED******REMOVED******REMOVED*** Step-by-Step Data Transformation

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 1: Metric Collection

```python
***REMOVED*** Example: Collect current metrics
...
***REMOVED*** Returns:
{
    "coverage_rate": 0.91,           ***REMOVED*** 91% of blocks covered
    "faculty_utilization": 0.82,     ***REMOVED*** 82% average utilization
    "workload_balance": 0.18,        ***REMOVED*** 0.18 std_dev in workload
    "schedule_stability": 0.93,
    "acgme_compliance": 1.0,
}
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 2: PID Update

```python
***REMOVED*** Bridge updates all PID controllers
bridge = PIDHomeostasisBridge(homeostasis_monitor)
corrections = bridge.update_all(metrics)

***REMOVED*** For utilization (target 0.75, current 0.82):
utilization_pid.update(0.82)
***REMOVED*** Returns:
{
    "error": -0.07,           ***REMOVED*** 7% over target
    "control_signal": -0.14,  ***REMOVED*** Remove ~14% of assignments
    "P": -0.70,               ***REMOVED*** Kp=10.0 × -0.07
    "I": -0.02,               ***REMOVED*** Small integral contribution
    "D": -0.04,               ***REMOVED*** Derivative dampening (if error is decreasing)
    "integral_saturated": False,
    "output_saturated": False,
}
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 3: Convert to CorrectiveAction

```python
***REMOVED*** Bridge converts PID output to CorrectiveAction
...
CorrectiveAction(
    ...
    action_type=CorrectiveActionType.DEFER_ACTIVITY,  ***REMOVED*** Reduce load
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 4: Scheduling Engine Adjustment

```python
***REMOVED*** Engine applies PID corrections to constraint weights
from app.scheduling.engine import SchedulingEngine

engine = SchedulingEngine()
await engine.adjust_weights(corrections)

***REMOVED*** For utilization correction (control_signal = -0.14):
***REMOVED*** - Reduce "maximize_utilization" weight by 14%
***REMOVED*** - Increase "balance_workload" weight by 14%
***REMOVED*** - Optionally trigger incremental reassignment
```

***REMOVED******REMOVED******REMOVED*** Feedback Loop Timing
...
***REMOVED******REMOVED*** Tuning Procedures

***REMOVED******REMOVED******REMOVED*** Why Tuning Matters
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Ziegler-Nichols Method (Classical)
...
**Example:**
```python
***REMOVED*** Relay feedback test for utilization controller
***REMOVED*** 1. Set Kp = 0, increase until oscillation
***REMOVED*** 2. Observe: Ku = 15.0, Pu = 7 days

Kp, Ki, Kd = tune_ziegler_nicholds(Ku=15.0, Pu=7.0)
***REMOVED*** Kp = 0.6 × 15.0 = 9.0
***REMOVED*** Ki = 1.2 × 15.0 / 7.0 = 2.57
***REMOVED*** Kd = 0.075 × 15.0 × 7.0 = 7.875

***REMOVED*** Initial gains: Kp=9.0, Ki=2.57, Kd=7.875
***REMOVED*** Typically need to reduce Ki and Kd by 50% for real-world use
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Manual Tuning (Trial-and-Error)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Simulation-Based Tuning (Recommended)
...
def simulate_pid_response(
    ...
    for i, measured_value in enumerate(historical_data):
        ***REMOVED*** Add disturbance
        measured_value += disturbances[i]

        ***REMOVED*** PID calculation
        error = setpoint - measured_value
        integral += error
        integral = np.clip(integral, -5.0, 5.0)  ***REMOVED*** Anti-windup
        derivative = error - previous_error

        control = Kp * error + Ki * integral + Kd * derivative
        control = np.clip(control, -0.2, 0.2)  ***REMOVED*** Output limits

        control_history.append(control)
        error_history.append(error)
        previous_error = error

    ***REMOVED*** Performance metric: Integral Squared Error (ISE)
    ise = sum(e**2 for e in error_history)

    ***REMOVED*** Penalty for control effort (avoid aggressive control)
...
def optimize_pid_gains(
    ...
    """
    ***REMOVED*** Initial guess
    x0 = [10.0, 0.5, 2.0]

    ***REMOVED*** Bounds: Kp ∈ [0, 20], Ki ∈ [0, 5], Kd ∈ [0, 10]
...
***REMOVED*** historical_utilization = [0.72, 0.78, 0.82, 0.79, 0.81, ...]
***REMOVED*** disturbances = [0.0, 0.02, -0.05, 0.01, ...]  ***REMOVED*** Absences, emergencies
***REMOVED*** optimized_gains = optimize_pid_gains(0.75, historical_utilization, disturbances)
***REMOVED*** Kp, Ki, Kd = optimized_gains
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Online Tuning (Adaptive)
...
class AdaptivePIDController:
    ...
    def __init__(self, initial_Kp, initial_Ki, initial_Kd):
        ...
        self.adaptation_rate = 0.05  ***REMOVED*** 5% adjustment per cycle
        self.performance_window = []
        self.window_size = 20

    def update(self, current_value: float) -> dict:
        """Update with adaptive tuning."""
        ***REMOVED*** Standard PID calculation
        result = super().update(current_value)

        ***REMOVED*** Track performance
        ise = result["error"] ** 2
        ...
        ***REMOVED*** Adapt gains if performance is poor
        if len(self.performance_window) == self.window_size:
            avg_ise = sum(self.performance_window) / self.window_size

            ***REMOVED*** If error is persistently high, increase Kp and Ki
            if avg_ise > 0.01:  ***REMOVED*** Threshold for "poor performance"
            ...
            ***REMOVED*** If oscillating, reduce Kp and increase Kd
    ...
    def _is_oscillating(self) -> bool:
        ...
        return sign_changes > 3  ***REMOVED*** More than 3 direction changes in 6 samples
```

***REMOVED******REMOVED******REMOVED*** Tuning Strategy by Defense Level
...
class DefenseLevelGainSchedule:
    ...
    GAIN_SCHEDULES = {
        "GREEN": {  ***REMOVED*** Normal operation - smooth, balanced control
            "utilization": {"Kp": 10.0, "Ki": 0.5, "Kd": 2.0},
            "workload": {"Kp": 5.0, "Ki": 0.3, "Kd": 1.0},
            "coverage": {"Kp": 12.0, "Ki": 0.8, "Kd": 1.5},
        },
        "YELLOW": {  ***REMOVED*** Elevated utilization - more aggressive correction
            "utilization": {"Kp": 15.0, "Ki": 1.0, "Kd": 3.0},
            "workload": {"Kp": 7.0, "Ki": 0.5, "Kd": 1.5},
            "coverage": {"Kp": 15.0, "Ki": 1.2, "Kd": 2.0},
        },
        "ORANGE": {  ***REMOVED*** High utilization - very aggressive, prioritize coverage
            "utilization": {"Kp": 20.0, "Ki": 1.5, "Kd": 4.0},
            "workload": {"Kp": 3.0, "Ki": 0.1, "Kd": 0.5},  ***REMOVED*** Deprioritize balance
            "coverage": {"Kp": 20.0, "Ki": 2.0, "Kd": 2.5},
        },
        "RED": {  ***REMOVED*** Crisis - maximum correction, coverage only
...
├── homeostasis.py              ***REMOVED*** Existing (unchanged)
├── pid_homeostasis_bridge.py   ***REMOVED*** NEW: Bridge implementation
└── pid_control.py              ***REMOVED*** NEW: PID controller classes
```

***REMOVED******REMOVED******REMOVED*** Class Definitions

***REMOVED******REMOVED******REMOVED******REMOVED*** `pid_control.py` - Core PID Implementation
...
class PIDState:
    ...
    ***REMOVED*** History for diagnostics
    ...
    ***REMOVED*** Performance tracking
    ...
    def update(self, current_value: float) -> dict:
        ...
        """
        now = datetime.now()
        dt = (now - self.previous_time).total_seconds() / 86400.0  ***REMOVED*** Convert to days

        ***REMOVED*** Prevent division by zero on first call
        if dt == 0:
            dt = 1.0 / 96.0  ***REMOVED*** Default to 15 minutes
        ...
        ***REMOVED*** Integral term with anti-windup (clamping)
        self.integral += error * dt
        ...
        ***REMOVED*** Derivative term
        if abs(self.previous_error) < 1e-9:
            ***REMOVED*** First call or previous error was zero
            derivative = 0.0
        else:
            derivative = (error - self.previous_error) / dt
        ...
        ***REMOVED*** Check for oscillations
        if self._is_oscillating():
    ...
    def _is_oscillating(self, window: int = 6) -> bool:
        ...
        ***REMOVED*** Oscillating if more than half the intervals change sign
    ...
    def _initialize_default_controllers(self):
        ...
        ***REMOVED*** Workload balance controller
        workload_config = PIDConfig(
            name="workload_balance",
            setpoint=0.0,  ***REMOVED*** Target: zero imbalance
            Kp=5.0,
            Ki=0.3,
            Kd=1.0,
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `pid_homeostasis_bridge.py` - Bridge Integration
...
class PIDHomeostasisBridge:
    ...
    def __init__(self, homeostasis_monitor: HomeostasisMonitor):
        ...
        ***REMOVED*** Feature flags for gradual rollout
        self.pid_enabled_for = {
            "coverage_rate": True,      ***REMOVED*** Start with critical metric
            "faculty_utilization": True,  ***REMOVED*** Deploy second
            "workload_balance": False,    ***REMOVED*** Deploy last
        }

        ***REMOVED*** Map homeostasis setpoint names to PID controller names
    ...
    def _get_pid_action_description(
        ...
        magnitude = abs(control) * 100  ***REMOVED*** Convert to percentage
        ...
        ***REMOVED*** Add diagnostics if saturated
    ...
    def _merge_recommendations(
        ...
        ***REMOVED*** Convert PID corrections to CorrectiveActions
        for pid_corr in pid_corrections:
            ***REMOVED*** Find corresponding feedback loop
        ...
        ***REMOVED*** Add homeostasis corrections for non-PID metrics
        ...
        ***REMOVED*** Sort by severity (CRITICAL first)
        severity_order = {
...
***REMOVED******REMOVED******REMOVED*** Unit Tests for PID Controllers
...
class TestPIDController:
    ...
    def test_proportional_response(self):
        ...
        config = PIDConfig(
            ...
            setpoint=0.75,
            Kp=10.0,
            Ki=0.0,  ***REMOVED*** Disable integral
            Kd=0.0,  ***REMOVED*** Disable derivative
            output_limits=(-1.0, 1.0)
        )
        controller = PIDState(id=uuid4(), config=config)

        ***REMOVED*** Error = 0.75 - 0.85 = -0.10
        result = controller.update(0.85)

        ***REMOVED*** P = 10.0 × -0.10 = -1.0
    ...
    def test_integral_eliminates_steady_state_error(self):
        ...
        config = PIDConfig(
            name="test",
            setpoint=0.75,
            Kp=0.0,   ***REMOVED*** Disable proportional
            Ki=1.0,   ***REMOVED*** Only integral
            Kd=0.0,
            output_limits=(-1.0, 1.0)
        )
        controller = PIDState(id=uuid4(), config=config)

        ***REMOVED*** Simulate persistent error over 5 days
        errors = []
        for day in range(5):
            controller.previous_time = datetime.now() - timedelta(days=5-day)
            result = controller.update(0.70)  ***REMOVED*** Error = +0.05
            errors.append(result["error"])

        ***REMOVED*** Integral should accumulate
        assert controller.integral > 0
        assert result["I"] > 0

        ***REMOVED*** After correction, error should decrease (in real system)
    ...
    def test_derivative_dampens_oscillation(self):
        ...
        config = PIDConfig(
            ...
            Kp=10.0,
            Ki=0.0,
            Kd=5.0,   ***REMOVED*** Strong derivative
            output_limits=(-1.0, 1.0)
        )
        controller = PIDState(id=uuid4(), config=config)

        ***REMOVED*** Step 1: Large error
        controller.previous_time = datetime.now() - timedelta(days=1)
        result1 = controller.update(0.85)  ***REMOVED*** Error = -0.10

        ***REMOVED*** Step 2: Error decreasing rapidly
        controller.previous_time = datetime.now() - timedelta(days=1)
        result2 = controller.update(0.76)  ***REMOVED*** Error = -0.01 (improving fast)

        ***REMOVED*** Derivative should oppose the change (negative D for decreasing error)
        assert result2["D"] > 0  ***REMOVED*** Positive derivative (error became less negative)

        ***REMOVED*** Total control should be reduced by derivative
    ...
    def test_anti_windup_integral_clamping(self):
        ...
        ***REMOVED*** Simulate sustained error that would cause windup
        for day in range(20):
            controller.previous_time = datetime.now() - timedelta(days=20-day)
            result = controller.update(0.50)  ***REMOVED*** Error = +0.25 (large)
    ...
    def test_output_limiting(self):
        ...
        ***REMOVED*** Large error → large control signal
        result = controller.update(0.95)  ***REMOVED*** Error = -0.20

        ***REMOVED*** P = 100 × -0.20 = -20.0 (huge)
        ***REMOVED*** But output should be clamped to -0.2
    ...
    def test_setpoint_change_resets_integral(self):
        ...
        ***REMOVED*** Accumulate some integral
        for _ in range(5):
        ...
        ***REMOVED*** Change setpoint
        controller.set_setpoint(0.80)

        ***REMOVED*** Integral should be reset
...
class TestPIDControllerBank:
    ...
    def test_ziegler_nichols_tuning(self):
        ...
        ***REMOVED*** Tune utilization controller
        Ku = 15.0
        Pu = 7.0
        ...
        ***REMOVED*** Expected: Kp = 0.6×15 = 9.0, Ki = 1.2×15/7 ≈ 2.57, Kd = 0.075×15×7 ≈ 7.88
...
***REMOVED******REMOVED******REMOVED*** Integration Tests for Bridge
...
class TestPIDHomeostasisBridge:
    ...
    def test_update_all_with_pid_enabled(self, bridge):
        ...
        ***REMOVED*** PID should generate corrections for enabled metrics
        pid_controllers = [c.controller_name for c in result["pid_corrections"]]
        assert "coverage" in pid_controllers  ***REMOVED*** Enabled by default
        assert "utilization" in pid_controllers  ***REMOVED*** Enabled by default
    ...
    def test_pid_overrides_homeostasis_for_enabled_metrics(self, bridge):
        ...
        current_values = {
            "coverage_rate": 0.85,  ***REMOVED*** Below target (0.95)
            "faculty_utilization": 0.82,
        }

        result = bridge.update_all(current_values)

        ***REMOVED*** Check that merged actions come from PID (not homeostasis) for coverage
        coverage_actions = [
        ...
        ***REMOVED*** Description should mention "PID"
    ...
    def test_enable_disable_pid(self, bridge):
        ...
        ***REMOVED*** Initially workload_balance is disabled
    ...
    def test_dual_mode_operation(self, bridge):
        ...
        ***REMOVED*** Disable PID for utilization
        bridge.disable_pid("faculty_utilization")

        current_values = {
            "coverage_rate": 0.91,  ***REMOVED*** PID enabled
            "faculty_utilization": 0.82,  ***REMOVED*** PID disabled → homeostasis
        }

        result = bridge.update_all(current_values)

        ***REMOVED*** PID should only run for coverage
        pid_controllers = [c.controller_name for c in result["pid_corrections"]]
        assert "coverage" in pid_controllers
        assert "utilization" not in pid_controllers

        ***REMOVED*** Homeostasis should handle utilization
...
class TestPIDPerformance:
    ...
    def test_step_response(self):
        ...
        ***REMOVED*** Simulate step response: setpoint changes from 0.75 to 0.80
        bank.controllers["utilization"].set_setpoint(0.80)

        ***REMOVED*** Simulate system response over 30 days
        utilization_values = []
        control_signals = []

        current_util = 0.75  ***REMOVED*** Start at old setpoint

        for day in range(30):
            ***REMOVED*** Update controller
            result = bank.update("utilization", current_util)
            control_signals.append(result["control_signal"])

            ***REMOVED*** Simulate system response (simple first-order model)
            ***REMOVED*** du/dt = control_signal (simplified)
            current_util += result["control_signal"] * 0.1  ***REMOVED*** 10% response rate
            utilization_values.append(current_util)

            ***REMOVED*** Advance time
        ...
        ***REMOVED*** Check convergence
        ...
        assert final_util == pytest.approx(0.80, abs=0.02)  ***REMOVED*** Should converge to new setpoint

        ***REMOVED*** Check for overshoot
        max_util = max(utilization_values)
        assert max_util < 0.85  ***REMOVED*** Should not overshoot by more than 5%

    def test_disturbance_rejection(self):
        """Test controller rejects disturbances."""
        bank = PIDControllerBank()

        ***REMOVED*** System at steady state
        current_coverage = 0.95

        ***REMOVED*** Simulate disturbance at day 10 (sudden absence drops coverage)
        disturbances = [0.0] * 10 + [-0.10] + [0.0] * 19  ***REMOVED*** -10% coverage drop
        ...
        for day, disturbance in enumerate(disturbances):
            ...
            ***REMOVED*** Apply control and disturbance
            current_coverage += result["control_signal"] * 0.2 + disturbance
            coverage_values.append(current_coverage)

        ***REMOVED*** Recovery time: how long to get back within 2% of setpoint
        recovery_day = None
        for day, cov in enumerate(coverage_values[11:], start=11):  ***REMOVED*** Start after disturbance
            if abs(cov - 0.95) < 0.02:
                recovery_day = day
                break

        assert recovery_day is not None
        assert recovery_day < 20  ***REMOVED*** Should recover within 10 days

    def test_steady_state_error(self):
        """Test integral term eliminates steady-state error."""
        bank = PIDControllerBank()

        ***REMOVED*** Simulate system with bias (always settles 5% below target)
        target = 0.75
        system_bias = -0.05

        current_util = 0.70  ***REMOVED*** Start below target

        for day in range(50):
            result = bank.update("utilization", current_util)

            ***REMOVED*** System response with bias
            current_util += result["control_signal"] * 0.15 + system_bias * 0.01

        ***REMOVED*** Integral term should compensate for bias
        ***REMOVED*** Final value should be close to target despite bias
...
***REMOVED******REMOVED******REMOVED*** Key Performance Indicators (KPIs)
...
***REMOVED******REMOVED******REMOVED*** Monitoring Dashboard

Create Grafana dashboard with these panels:

```yaml
***REMOVED*** grafana/dashboards/pid_control.json
...
***REMOVED******REMOVED******REMOVED*** Logging Strategy

```python
***REMOVED*** Add to backend/app/resilience/pid_homeostasis_bridge.py
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Coverage PID (Week 2)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Utilization PID (Week 3)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Workload PID (Week 4)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Deprecate Old Logic (Week 5)
