***REMOVED*** Nash Stability Bridge Specification
...
3. [Algorithm Specification](***REMOVED***algorithm-specification)
4. [Integration Points](***REMOVED***integration-points)
5. [Utility Function Design](***REMOVED***utility-function-design)
6. [Output Schema](***REMOVED***output-schema)
7. [Implementation Details](***REMOVED***implementation-details)
8. [Test Cases](***REMOVED***test-cases)
...
***REMOVED******REMOVED******REMOVED*** Nash Equilibrium Definition
...
***REMOVED******REMOVED******REMOVED*** Nash Distance Metric
...
***REMOVED******REMOVED******REMOVED*** Stability Threshold

Based on empirical swap request data:

```python
STABILITY_THRESHOLDS = {
    "STABLE": 0.05,      ***REMOVED*** < 5% of assignments have beneficial swaps
    "MARGINAL": 0.15,    ***REMOVED*** 5-15% (monitor)
    "UNSTABLE": 0.30,    ***REMOVED*** 15-30% (needs refinement)
    "CRITICAL": 0.50,    ***REMOVED*** > 30% (do not publish)
}
```

***REMOVED******REMOVED******REMOVED*** Predicted Swap Volume
...
***REMOVED******REMOVED*** Algorithm Specification

***REMOVED******REMOVED******REMOVED*** High-Level Algorithm
...
class NashStabilityAnalyzer:
    ...
    def calculate_nash_distance(
        ...
        ***REMOVED*** For each faculty member
            ...
            for other_faculty in schedule.get_all_faculty():
                ...
                for my_assignment in current_assignments:
                    for their_assignment in other_assignments:
                        ***REMOVED*** Calculate utility after swapping these two assignments
                        ...
                        ***REMOVED*** If this swap improves utility, it's a beneficial deviation
...
***REMOVED******REMOVED******REMOVED*** Detailed Steps

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 1: Build Utility Matrix
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 2: Enumerate Beneficial Swaps
...
def _enumerate_beneficial_swaps(
    ...
    for my_assignment in current_assignments:
        ...
        ***REMOVED*** Find assignments held by others that this faculty would prefer
        for other_assignment in all_assignments:
            if other_assignment in current_assignments:
                continue  ***REMOVED*** Can't swap with yourself

            other_utility = utility_matrix[(faculty_id, other_assignment.id)]

            ***REMOVED*** Would this swap improve utility?
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 3: Calculate Distance
...
def _calculate_distance(
    ...
    ***REMOVED*** Nash distance = fraction of assignments with beneficial deviations
    distance = total_beneficial_deviations / total_assignments

    return min(distance, 1.0)  ***REMOVED*** Cap at 1.0
```

***REMOVED******REMOVED******REMOVED*** Complexity Analysis
...
***REMOVED******REMOVED******REMOVED*** 1. Post-Generation Validation
...
def generate(self, ...) -> dict:
    ...
    ***REMOVED*** ... existing generation logic ...

    ***REMOVED*** After solver completes
    solver_result = solver.solve(context, timeout_seconds)

    ***REMOVED*** Create assignments from solution
    self.assignments = self._create_assignments_from_solution(
        solver_result.solution
    )

    ***REMOVED*** ===== NEW: Nash Stability Check =====
    ...
    ***REMOVED*** Validate ACGME compliance (existing)
    ...
    return {
        ...
        "nash_stability": nash_report,  ***REMOVED*** NEW
        ***REMOVED*** ... rest of result ...
    }
```

***REMOVED******REMOVED******REMOVED*** 2. Pre-Publish Warning Gate

```python
***REMOVED*** In schedule publish endpoint

@router.post("/schedules/{schedule_id}/publish")
async def publish_schedule(
    schedule_id: UUID,
    force: bool = False,  ***REMOVED*** Override warnings
    ...
    ***REMOVED*** Run Nash stability analysis
    analyzer = NashStabilityAnalyzer(db)
    nash_report = await analyzer.calculate_nash_distance(schedule)

    ***REMOVED*** Block publication if critically unstable (unless forced)
    ...
    ***REMOVED*** Warn if unstable
    if nash_report.stability_level in ["UNSTABLE", "MARGINAL"]:
        ***REMOVED*** Log warning, but allow publication
        logger.warning(
            f"Publishing unstable schedule: {nash_report.stability_level}"
        )

    ***REMOVED*** Proceed with publication
...
```python
***REMOVED*** In schedule analytics endpoint
...
***REMOVED******REMOVED******REMOVED*** 4. Swap Auto-Matcher Enhancement

```python
***REMOVED*** In SwapAutoMatcher
...
def suggest_optimal_matches_with_stability(
    ...
    ***REMOVED*** For each match, calculate Nash distance after swap
    for match in matches:
        ...
        ***REMOVED*** Boost score if swap improves stability
        if stability_improvement > 0.01:  ***REMOVED*** Meaningful improvement
            match.compatibility_score *= (1.0 + 0.3 * stability_improvement)
            match.explanation += (
                f"; Improves schedule stability by {stability_improvement:.2%}"
            )

    ***REMOVED*** Re-sort by updated scores
    return sorted(matches, key=lambda m: m.compatibility_score, reverse=True)
```

---

***REMOVED******REMOVED*** Utility Function Design

***REMOVED******REMOVED******REMOVED*** Preference-Based Utility
...
class PreferenceUtilityFunction:
    ...
    def __init__(self, db: Session):
        ...
        self.stigmergy = StigmergicScheduler()  ***REMOVED*** If available
    ...
    def _score_explicit_preference(
        ...
        ***REMOVED*** Check if week is blocked (utility = 0.0)
        if self.preference_service.is_week_blocked(faculty_id, week):
            return 0.0

        ***REMOVED*** Check if week is preferred (utility = 1.0)
        if self.preference_service.is_week_preferred(faculty_id, week):
            return 1.0

        ***REMOVED*** Neutral week (utility = 0.5)
        return 0.5
    ...
    def _score_workload_balance(
        ...
        """
        ***REMOVED*** Get faculty's current assignment count
        current_count = (
        ...
        ***REMOVED*** Get average assignment count across all faculty
        total_faculty = (
        ...
        ***REMOVED*** Calculate deviation from average
        deviation = abs(current_count - avg_per_faculty)

        ***REMOVED*** Lower deviation = higher utility
        ***REMOVED*** Normalize: deviation 0 = score 1.0, deviation > 10 = score 0.0
    ...
    def _score_timeoff_pattern(
        ...
        """
        ***REMOVED*** Check for absences near this week
        absences = (
        ...
        ***REMOVED*** Calculate distance to nearest absence
        min_distance = min(
            abs((week - absence.start_date).days)
            for absence in absences
        )

        ***REMOVED*** Closer to time off = lower utility (prefer gap)
        ***REMOVED*** 0-7 days: score 0.2 (very close, avoid)
        ***REMOVED*** 8-14 days: score 0.5 (close, neutral)
        ***REMOVED*** 15+ days: score 0.8 (far, good)
        if min_distance < 7:
            return 0.2
    ...
    def _score_historical_behavior(
        ...
        ***REMOVED*** Check historical swaps for this week number
        historical_swaps = (
        ...
        ***REMOVED*** Count how often they've swapped out of this week number
        swap_count = sum(
        ...
        total_this_week = sum(
            1 for swap in historical_swaps
            if swap.source_week.isocalendar()[1] == week_number
        ) + 1  ***REMOVED*** +1 to avoid division by zero

        ***REMOVED*** High swap rate = low utility
...
***REMOVED******REMOVED******REMOVED*** Alternative: Stigmergy-Based Utility
...
def calculate_utility_from_stigmergy(
    ...
    ***REMOVED*** Get preference trail strength
    ...
    ***REMOVED*** Find trail for this slot type
    matching_trails = [
    ...
    if matching_trails:
        ***REMOVED*** Use strongest trail
        max_strength = max(t.strength for t in matching_trails)
        return max_strength

    ***REMOVED*** Check avoidance trails (negative utility)
...
***REMOVED******REMOVED******REMOVED*** NashStabilityReport
...
***REMOVED******REMOVED******REMOVED*** API Response Integration

```python
***REMOVED*** In ScheduleEngine.generate() return value
...
├── nash_stability.py          ***REMOVED*** NEW: NashStabilityAnalyzer
├── nash_utility.py            ***REMOVED*** NEW: PreferenceUtilityFunction
├── engine.py                  ***REMOVED*** MODIFIED: Add nash_stability check
└── validator.py               ***REMOVED*** EXISTING: ACGME validator

backend/app/schemas/
├── schedule.py                ***REMOVED*** MODIFIED: Add NashStabilityReport
└── nash_stability.py          ***REMOVED*** NEW: InstabilityDetail schema

backend/tests/scheduling/
├── test_nash_stability.py     ***REMOVED*** NEW: Nash stability tests
└── test_nash_utility.py       ***REMOVED*** NEW: Utility function tests
```

***REMOVED******REMOVED******REMOVED*** Implementation Phases

***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 1: Core Algorithm (Week 1)

```python
***REMOVED*** backend/app/scheduling/nash_stability.py
...
class NashStabilityAnalyzer:
    ...
    def _utility_after_swap(self, faculty_id, assignments, swap_from, swap_to):
        ***REMOVED*** Utility calculation helper
        pass

    def _classify_stability(self, distance):
        ***REMOVED*** Stability level classification
        pass

    def _predict_swap_volume(self, distance, total_assignments):
        ***REMOVED*** Swap prediction model
        pass
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 2: Utility Function (Week 1)

```python
***REMOVED*** backend/app/scheduling/nash_utility.py

class PreferenceUtilityFunction:
    """Maps (faculty, assignment) -> utility score."""

    def calculate_utility(self, faculty_id, assignment):
        ***REMOVED*** Preference-based utility
        pass
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 3: Integration (Week 2)
...
class SchedulingEngine:
    def generate(self, ...):
        ***REMOVED*** ... existing generation ...

        ***REMOVED*** Add Nash stability check
        nash_analyzer = NashStabilityAnalyzer(self.db)
        nash_report = nash_analyzer.calculate_nash_distance(
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Phase 4: API Integration (Week 2)

```python
***REMOVED*** backend/app/api/routes/schedules.py
...
```python
***REMOVED*** backend/alembic/versions/xxx_add_nash_stability.py

def upgrade():
    ***REMOVED*** Add nash_distance column to schedules table
    op.add_column(
        'schedules',
...
***REMOVED******REMOVED******REMOVED*** Test 1: Perfectly Stable Schedule

**Setup:**
- 3 faculty members
- 3 weeks
- Each faculty gets their ***REMOVED***1 preferred week
...
def test_perfectly_stable_schedule():
    ...
    ***REMOVED*** Setup
    schedule = create_test_schedule(
        ...
        assignments=[
            ("FAC-A", "2025-01-06"),  ***REMOVED*** Gets preferred week
            ("FAC-B", "2025-01-13"),  ***REMOVED*** Gets preferred week
            ("FAC-C", "2025-01-20"),  ***REMOVED*** Gets preferred week
        ]
    )
...
***REMOVED******REMOVED******REMOVED*** Test 2: Moderately Stable Schedule
...
def test_moderately_stable_schedule():
    ...
    schedule = create_test_schedule(
        faculty=[
            ...
            ***REMOVED*** ... 8 more faculty with matched preferences ...
        ],
        assignments=[
            ("FAC-A", "2025-01-06"),  ***REMOVED*** Has blocked week
            ("FAC-B", "2025-01-13"),  ***REMOVED*** Has blocked week
            ***REMOVED*** ... 8 more with preferred weeks ...
        ]
    )
...
***REMOVED******REMOVED******REMOVED*** Test 3: Unstable Schedule
...
def test_unstable_schedule():
    ...
    ***REMOVED*** Create schedule with many conflicts
    schedule = create_conflicted_schedule(
        num_faculty=10,
        num_weeks=10,
        conflict_rate=0.5  ***REMOVED*** 50% of assignments are mismatched
    )
...
***REMOVED******REMOVED******REMOVED*** Test 4: Validation with Historical Data
...
***REMOVED******REMOVED******REMOVED*** Test 5: Utility Function Accuracy

```python
def test_utility_function_matches_preferences():
    """Test utility function correctly reflects preferences."""
    ***REMOVED*** Setup faculty with explicit preferences
    ...
    ***REMOVED*** Test preferred week (should be high utility)
    ...
    ***REMOVED*** Test blocked week (should be low/zero utility)
    ...
    ***REMOVED*** Test neutral week (should be medium utility)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Pre-Compute Utility Matrix

```python
def calculate_nash_distance_optimized(self, schedule):
    """Optimized Nash distance with pre-computed utilities."""

    ***REMOVED*** Pre-compute all utilities (200 calculations for 10 faculty × 20 assignments)
    utility_matrix = self._build_utility_matrix(schedule)

    ***REMOVED*** Now use matrix for O(1) lookup instead of O(F) calculation
    beneficial_deviations = 0

    for faculty in schedule.get_all_faculty():
        ***REMOVED*** Use matrix, not repeated utility calculations
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Early Termination
        ...
        for other_assignment in all_assignments:
            if self._is_beneficial_swap(my_assignment, other_assignment, utility_matrix):
                ...
                if count >= max_to_find:
                    return count  ***REMOVED*** Early termination

    return count
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Parallel Processing
...
def calculate_nash_distance_parallel(self, schedule):
    ...
    ***REMOVED*** Process each faculty in parallel
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Caching
...
***REMOVED******REMOVED******REMOVED*** Benchmarks
...
***REMOVED******REMOVED******REMOVED*** Game Theory Foundations
...
***REMOVED******REMOVED******REMOVED*** Mechanism Design
...
***REMOVED******REMOVED******REMOVED*** Scheduling Applications
...
***REMOVED******REMOVED******REMOVED*** Codebase Integration
...
***REMOVED******REMOVED******REMOVED*** Appendix A: Stability Threshold Calibration
...
***REMOVED******REMOVED******REMOVED*** Appendix B: Example Calculation Walkthrough
