***REMOVED*** Tool Composition Patterns
...
2. [Pattern Catalog](***REMOVED***pattern-catalog)
   - [Sequential Chain](***REMOVED***1-sequential-chain)
   - [Parallel Fan-Out](***REMOVED***2-parallel-fan-out)
   - [Conditional Branching](***REMOVED***3-conditional-branching)
   - [Retry with Fallback](***REMOVED***4-retry-with-fallback)
   - [Long-Running Task](***REMOVED***5-long-running-task)
   - [Transactional Saga](***REMOVED***6-transactional-saga)
3. [Advanced Patterns](***REMOVED***advanced-patterns)
4. [Real-World Examples](***REMOVED***real-world-examples)
5. [Anti-Patterns](***REMOVED***anti-patterns)

---

***REMOVED******REMOVED*** Overview

***REMOVED******REMOVED******REMOVED*** What is Tool Composition?
...
***REMOVED******REMOVED******REMOVED*** Available Tools (34 Total)
...
***REMOVED******REMOVED******REMOVED*** Available Resources (2 Total)
...
***REMOVED******REMOVED*** Pattern Catalog

***REMOVED******REMOVED******REMOVED*** 1. Sequential Chain
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Diagram
...
graph LR
    ...
    style A fill:***REMOVED***90EE90
    style B fill:***REMOVED***87CEEB
    style C fill:***REMOVED***DDA0DD
    style D fill:***REMOVED***FFD700
```

***REMOVED******REMOVED******REMOVED******REMOVED*** TypeScript Implementation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Python Implementation
...
async def validate_and_fix_schedule(
    ...
    """
    ***REMOVED*** Step 1: Validate schedule
    ...
    ***REMOVED*** Step 3: Find swap candidates for each conflict
    recommendations = []
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Error Handling Strategy
...
async def validate_and_fix_schedule_with_errors(
    ...
    try:
        ***REMOVED*** Step 1: Validate (with retry)
    ...
    try:
        ...
        ***REMOVED*** Graceful degradation: return validation issues only
        return {
            "status": "conflicts_unknown",
            "validation_issues": validation["issues"],
        }

    ***REMOVED*** Step 3: Continue with swap analysis...
    ***REMOVED*** (Rest of implementation)
```

---

***REMOVED******REMOVED******REMOVED*** 2. Parallel Fan-Out
...
graph TB
    ...
    style A fill:***REMOVED***FFD700
    style B fill:***REMOVED***90EE90
    style C fill:***REMOVED***87CEEB
    style D fill:***REMOVED***DDA0DD
    style E fill:***REMOVED***FFB6C1
    style F fill:***REMOVED***FFA07A
    style G fill:***REMOVED***98FB98
```
...
async def comprehensive_health_check(
    ...
    """
    ***REMOVED*** Fan-out: Launch all checks concurrently
    ...
    ***REMOVED*** Gather: Wait for all to complete
    ...
    ***REMOVED*** Aggregate: Combine into unified score
...
async def comprehensive_health_check_with_errors(
    ...
    ***REMOVED*** Gather with exception handling
    results = {}
    errors = {}
    ...
    for name, task in tasks.items():
        try:
            ...
            errors[name] = str(e)
            ***REMOVED*** Provide default/fallback value
            results[name] = get_default_result(name)

    ***REMOVED*** Calculate score with available data
...
***REMOVED******REMOVED******REMOVED*** 3. Conditional Branching
...
graph TB
    ...
    style A fill:***REMOVED***90EE90
    style B fill:***REMOVED***FFD700
    style C fill:***REMOVED***98FB98
    style D fill:***REMOVED***FFD700
    style E fill:***REMOVED***FF6B6B
    style F fill:***REMOVED***FFD700
    style G fill:***REMOVED***87CEEB
    style H fill:***REMOVED***FFA07A
    style I fill:***REMOVED***DDA0DD
```
...
async def smart_schedule_validation(schedule_id: str) -> dict:
    ...
    ***REMOVED*** Branch 1: Valid - no action needed
    if validation["is_valid"]:
        return {
    ...
    ***REMOVED*** Branch 2: Critical violations - emergency response
    if validation["critical_count"] > 0:
        ***REMOVED*** Execute sacrifice hierarchy
    ...
    ***REMOVED*** Branch 3: Auto-resolvable conflicts
    auto_resolvable = all(
    ...
    ***REMOVED*** Branch 4: Manual review required
    await notify_coordinator(
        severity="warning",
        message="Manual review required for validation issues",
        details=validation["issues"],
...
async def smart_schedule_validation_with_errors(schedule_id: str) -> dict:
    ...
    try:
        ...
        logger.error(f"Validation failed: {e}")
        ***REMOVED*** Fallback: Mark as needs review
        return {
            "status": "error",
    ...
    ***REMOVED*** Try auto-fix with fallback
    try:
        if validation["critical_count"] > 0:
            ***REMOVED*** Try emergency response
            sacrifice = await mcp.call("execute_sacrifice_hierarchy", {
                "target_level": "orange",
                "simulate_only": True,  ***REMOVED*** Simulate first for safety
            })

            ***REMOVED*** If simulation successful, apply for real
    ...
    ***REMOVED*** Fallback: Manual review
    return {
        "status": "needs_review",
        "action": "manual_intervention",
    }
...
***REMOVED******REMOVED******REMOVED*** 4. Retry with Fallback
...
graph TB
    ...
    style A fill:***REMOVED***90EE90
    style B fill:***REMOVED***87CEEB
    style C fill:***REMOVED***FFD700
    style D fill:***REMOVED***98FB98
    style E fill:***REMOVED***FFD700
    style F fill:***REMOVED***DDA0DD
    style G fill:***REMOVED***FFA07A
    style H fill:***REMOVED***FFD700
    style I fill:***REMOVED***98FB98
    style J fill:***REMOVED***FF6B6B
    style K fill:***REMOVED***FFB6C1
```
...
async def resilient_validation(
    ...
    ***REMOVED*** Primary attempt with retries
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ***REMOVED*** Exponential backoff: 2^attempt seconds
                backoff_seconds = 2 ** attempt
                await asyncio.sleep(backoff_seconds)

    ***REMOVED*** Fallback 1: Try alternative validation tool
    try:
        logger.info("Primary validation failed, trying fallback...")
    ...
    ***REMOVED*** Fallback 2: Return mock validation (degraded mode)
...
async def resilient_validation_with_circuit_breaker(
    ...
    ***REMOVED*** Check circuit breaker state
...
async def fallback_validation(schedule_id: str) -> dict:
    ...
    ***REMOVED*** Could use cached results, simplified validation, or mock data
    return {
        "source": "fallback",
...
***REMOVED******REMOVED******REMOVED*** 5. Long-Running Task
...
graph TB
    ...
    style C fill:***REMOVED***DDA0DD
    style D fill:***REMOVED***FFD700
    style E fill:***REMOVED***FFA07A
    style F fill:***REMOVED***FFB6C1
    style G fill:***REMOVED***98FB98
    style H fill:***REMOVED***FF6B6B
    style I fill:***REMOVED***FFA500
    style J fill:***REMOVED***98FB98
    style K fill:***REMOVED***FF6B6B
```
...
async def run_resilience_health_check(
    ...
    """
    ***REMOVED*** Step 1: Start background task
    ...
    ***REMOVED*** Step 2: Poll for completion
    max_poll_time = 300  ***REMOVED*** 5 minutes
    poll_interval = 2    ***REMOVED*** 2 seconds
    elapsed = 0
    ...
    while elapsed < max_poll_time:
        ...
        if status["status"] == "success":
            ...
            ***REMOVED*** Still running, continue polling
            pass

        else:
            logger.warning(f"Unknown task status: {status['status']}")

        ***REMOVED*** Wait before next poll
...
async def run_resilience_health_check_with_progress(
    ...
    while elapsed < max_poll_time:
        try:
            ...
            ***REMOVED*** Retry status check
            await asyncio.sleep(poll_interval)
            elapsed += poll_interval
            continue

        ***REMOVED*** Report progress
...
***REMOVED******REMOVED******REMOVED*** 6. Transactional Saga
...
graph TB
    ...
    style E fill:***REMOVED***FF6B6B
    style F fill:***REMOVED***FFD700
    style G fill:***REMOVED***FFB6C1
    style H fill:***REMOVED***FFA500
    style I fill:***REMOVED***FFD700
    style J fill:***REMOVED***98FB98
    style K fill:***REMOVED***FFA500
    style L fill:***REMOVED***FF6B6B
    style M fill:***REMOVED***FF6B6B
```
...
async def safe_production_deployment(
    ...
    try:
        ***REMOVED*** Step 1: Validate deployment
        ...
        ***REMOVED*** Step 3: Promote to production
        logger.info("Step 3: Promoting to production")
        ...
        ***REMOVED*** Add compensation: rollback if anything fails later
        ...
        ***REMOVED*** Execute compensations in reverse order
...
async def safe_production_deployment_with_checkpoints(
    ...
    try:
        ...
        ***REMOVED*** Save failure checkpoint
        checkpoint("failure", {"error": str(e)})

        ***REMOVED*** Attempt recovery based on last successful checkpoint
        last_checkpoint = checkpoints[-1] if checkpoints else None

        if last_checkpoint and last_checkpoint["step"] == "promotion":
            ***REMOVED*** We deployed but tests failed - rollback
...
***REMOVED******REMOVED*** Advanced Patterns

***REMOVED******REMOVED******REMOVED*** 7. Map-Reduce
...
async def analyze_all_faculty_centrality() -> dict:
    ...
    ***REMOVED*** Get all faculty IDs
    faculty_ids = await get_all_faculty_ids()

    ***REMOVED*** Map: Analyze each faculty member in parallel
    ...
    ***REMOVED*** Reduce: Aggregate into single report
...
***REMOVED******REMOVED******REMOVED*** 8. Pipeline with Transformation
...
async def deployment_pipeline(git_ref: str) -> dict:
    ...
    ***REMOVED*** Step 1: Validate (returns validation result)
    validation = await mcp.call("validate_deployment", {
        "environment": "production",
        "git_ref": git_ref,
    })

    ***REMOVED*** Transform: Extract blockers for security scan
    blockers_to_scan = [
        b for b in validation["blockers"]
        if "security" in b.lower()
    ]

    ***REMOVED*** Step 2: Security scan (if needed)
    if blockers_to_scan:
        scan = await mcp.call("run_security_scan", {
            "git_ref": git_ref,
        })

        ***REMOVED*** Transform: Convert vulnerabilities to validation format
...
***REMOVED******REMOVED******REMOVED*** 9. Circuit Breaker Pattern
...
class CircuitBreaker:
    ...
    async def call(self, tool_name: str, params: dict) -> dict:
        ...
        ***REMOVED*** Check circuit state
        ...
        try:
            ***REMOVED*** Attempt call
            result = await mcp.call(tool_name, params)

            ***REMOVED*** Success - reset on half-open
            ...
            ***REMOVED*** Open circuit if threshold reached
...
async def resilient_validation(schedule_id: str) -> dict:
    ...
    try:
        ...
        ***REMOVED*** Use fallback when circuit is open
        return await fallback_validation(schedule_id)
```
...
***REMOVED******REMOVED******REMOVED*** Example 1: Complete Schedule Health Check
...
async def complete_schedule_health_check(
    ...
    """
    ***REMOVED*** Parallel fan-out: Independent health checks
    ...
    ***REMOVED*** Conditional branching: Auto-fix if possible
    if not validation["is_valid"]:
        ***REMOVED*** Sequential chain: Detect conflicts → Find swaps
        conflicts = await mcp.call("detect_conflicts", {
        ...
        if conflicts["summary"]["auto_resolvable"] > 0:
            ***REMOVED*** Auto-fix available
            fixes = await apply_auto_fixes(conflicts)
            action = "auto_fixed"
        else:
            ***REMOVED*** Manual review needed
            action = "needs_review"
            fixes = None
    else:
        action = "no_action"
...
***REMOVED******REMOVED******REMOVED*** Example 2: Deployment Workflow with Safeguards
...
async def safe_deployment_workflow(
    ...
    try:
        ...
        ***REMOVED*** Add compensation
        compensations.append(lambda: rollback_deployment(deployment_id))

        ***REMOVED*** Step 4: Long-running smoke tests
...
***REMOVED******REMOVED******REMOVED*** Example 3: Resilience Monitoring Dashboard
...
async def resilience_monitoring_dashboard() -> dict:
    ...
    """
    ***REMOVED*** Parallel fan-out: All resilience checks
...
***REMOVED******REMOVED******REMOVED*** ❌ Anti-Pattern 1: Waterfall Chain Without Error Handling

**Bad:**

```python
async def bad_validation():
    ***REMOVED*** No error handling - any failure kills entire chain
...
async def good_validation():
    ...
    ***REMOVED*** Continue with error handling at each step...
```

***REMOVED******REMOVED******REMOVED*** ❌ Anti-Pattern 2: Sequential When Parallel Would Work

**Bad:**

```python
async def bad_health_check():
    ***REMOVED*** Sequential - takes 4x longer than necessary
...
***REMOVED******REMOVED******REMOVED*** ❌ Anti-Pattern 3: Polling Too Frequently
...
async def bad_task_polling(task_id):
    while True:
        ...
        ***REMOVED*** No sleep - hammers API constantly
```
...
async def good_task_polling(task_id):
    while True:
        ...
        if status["status"] == "success":
            return status["result"]
        await asyncio.sleep(2)  ***REMOVED*** Reasonable polling interval
```

***REMOVED******REMOVED******REMOVED*** ❌ Anti-Pattern 4: No Timeout on Polling

**Bad:**

```python
async def bad_task_wait(task_id):
    ***REMOVED*** Could wait forever
    while True:
        status = await mcp.call("get_task_status", {"task_id": task_id})
        if status["status"] == "success":
            return status
...
***REMOVED******REMOVED******REMOVED*** ❌ Anti-Pattern 5: Ignoring Idempotency

**Bad:**

```python
async def bad_deployment(git_ref):
    ***REMOVED*** Not idempotent - retrying creates duplicates
    deployment = await mcp.call("promote_to_production", {
        "staging_version": git_ref,
    })
    ***REMOVED*** If this fails and we retry, we deploy twice
    return deployment
```
...
- Explore [Real-World Examples](***REMOVED***real-world-examples) for production patterns
- Study [Anti-Patterns](***REMOVED***anti-patterns) to avoid common mistakes
