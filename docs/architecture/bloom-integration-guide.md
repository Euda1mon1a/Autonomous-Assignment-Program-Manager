***REMOVED*** Bloom Integration Guide: Scheduler & Resilience Modules
...
***REMOVED******REMOVED******REMOVED*** What We're Adding
...
***REMOVED******REMOVED*** 1. Extend GameTheoryService for AI Evaluation
...
***REMOVED******REMOVED******REMOVED*** Integration: Add BloomAIPlayer

**File:** `backend/app/services/game_theory.py`

```python
***REMOVED*** Add after ResilienceConfigPlayer class (line ~142)
...
class BloomAIPlayer(axl.Player):
    ...
    classifier = {
        "memory_depth": float("inf"),  ***REMOVED*** AI has context
        "stochastic": True,  ***REMOVED*** AI responses vary
        "long_run_time": True,  ***REMOVED*** API calls
    ...
    def __init__(
        ...
        ***REMOVED*** Cache for simulating model behavior
    ...
    def strategy(self, opponent: axl.Player) -> axl.Action:
        ...
        ***REMOVED*** First move: based on detected strategy
        if len(self.history) == 0:
            ***REMOVED*** Most AI models cooperate first (from research)
            if self.strategy_detected in ["always_defect", "suspicious_tft"]:
                return axl.Action.D
            return axl.Action.C

        ***REMOVED*** Track opponent defections
        ...
        ***REMOVED*** Simulate based on detected strategy
        if self.strategy_detected == "always_cooperate":
            ...
            ***REMOVED*** Research shows LLMs shift to forgiving at ~30% defection
            if opponent_defection_rate < self.defection_threshold:
                return axl.Action.C
            return opponent.history[-1]

        else:
            ***REMOVED*** Use measured cooperation rate as probability
...
***REMOVED*** Add to GameTheoryService class
...
def create_ai_strategy_from_bloom(
    ...
    ***REMOVED*** Determine strategy type from detected behavior
    if coop_rate > 0.9:
        ...
        strategy_type = "tit_for_tat"  ***REMOVED*** Default to TFT for mixed

    ***REMOVED*** Create strategy record
    strategy = ConfigStrategy(
        ...
        ***REMOVED*** Map cooperation rate to utilization target
        ***REMOVED*** Higher cooperation = more willing to share (lower utilization)
        utilization_target=0.95 - (coop_rate * 0.25),  ***REMOVED*** 0.70-0.95
...
def run_human_ai_tournament(
    ...
    ***REMOVED*** Create players
    ...
    for strategy in all_strategies:
        if strategy.is_ai_model:
            ***REMOVED*** Use BloomAIPlayer for AI models
            ...
            ***REMOVED*** Use ResilienceConfigPlayer for configs
    ...
    ***REMOVED*** Store results with AI vs Config breakdown
    ***REMOVED*** ... (similar to existing run_tournament but with additional analysis)
...
def _analyze_ai_vs_config(
    ...
    ***REMOVED*** Extract matchup scores
...
***REMOVED******REMOVED*** 2. Add Behavioral Signals to SwapAutoMatcher
...
***REMOVED******REMOVED******REMOVED*** Integration: Add PD-Based Scoring

**File:** `backend/app/services/swap_auto_matcher.py`

```python
***REMOVED*** Add to MatchingCriteria dataclass

@dataclass
class MatchingCriteria:
    ***REMOVED*** ... existing fields ...

    ***REMOVED*** NEW: Prisoner's Dilemma behavioral weights
    pd_cooperation_weight: float = 0.15  ***REMOVED*** Weight for cooperation history
    pd_reciprocity_weight: float = 0.10  ***REMOVED*** Weight for reciprocal behavior
    pd_exploitation_penalty: float = 0.20  ***REMOVED*** Penalty for exploitative patterns


***REMOVED*** Add to SwapAutoMatcher class
...
def _calculate_pd_behavioral_score(
    ...
    """
    ***REMOVED*** Get swap history between these two faculty
    ...
    ***REMOVED*** Analyze as iterated PD
    a_cooperations = 0  ***REMOVED*** A honored commitments to B
    a_defections = 0    ***REMOVED*** A benefited at B's expense
    b_cooperations = 0
    b_defections = 0

    for swap in shared_history:
        if swap.source_faculty_id == faculty_a_id:
            ***REMOVED*** A initiated swap with B
            if swap.status == "completed":
                a_cooperations += 1
            elif swap.status == "cancelled_by_source":
                a_defections += 1  ***REMOVED*** A backed out
        else:
            ***REMOVED*** B initiated swap with A
            if swap.status == "completed":
                b_cooperations += 1
            elif swap.status == "cancelled_by_source":
                b_defections += 1
    ...
    ***REMOVED*** Calculate cooperation rates
    ...
    ***REMOVED*** Mutual cooperation score
    mutual_coop = (a_coop_rate + b_coop_rate) / 2

    ***REMOVED*** Reciprocity score (balanced relationship)
    reciprocity = 1.0 - abs(a_coop_rate - b_coop_rate)

    ***REMOVED*** Exploitation penalty (one-sided benefit)
    exploitation = 0.0
    if a_coop_rate > 0.8 and b_coop_rate < 0.3:
        exploitation = 0.5  ***REMOVED*** B exploits A
    elif b_coop_rate > 0.8 and a_coop_rate < 0.3:
        exploitation = 0.5  ***REMOVED*** A exploits B

    ***REMOVED*** Combined score
    score = (
        mutual_coop * 0.5 +
        reciprocity * 0.3 -
        exploitation * 0.2
...
def _detect_strategy_type(
    ...
    ***REMOVED*** Simple classification
    if completion_rate > 0.9:
        return "cooperator"
    elif completion_rate < 0.3:
        return "defector"
    else:
        ***REMOVED*** Check for TFT pattern (mirrors partner behavior)
...
def suggest_optimal_matches_with_pd_analysis(
    ...
    """
    ***REMOVED*** Get base matches
    matches = self.suggest_optimal_matches(request_id, top_k * 2)  ***REMOVED*** Get more to filter
    ...
    ***REMOVED*** Enhance with PD scoring
    for match in matches:
        ...
        ***REMOVED*** Add to explanation
        ...
        if partner_strategy == "defector":
            ...
            match.compatibility_score *= 1.1  ***REMOVED*** Bonus for reliable partners
...
***REMOVED******REMOVED*** 3. Add PD Metrics to Resilience Health Checks

***REMOVED******REMOVED******REMOVED*** Current State

`ResilienceService.check_health()` monitors utilization, redundancy, defense levels, etc. It doesn't include cooperation metrics.

***REMOVED******REMOVED******REMOVED*** Integration: Add Cooperation Health Indicators

**File:** `backend/app/resilience/service.py`

```python
***REMOVED*** Add to SystemHealthReport dataclass

@dataclass
class SystemHealthReport:
    ***REMOVED*** ... existing fields ...

    ***REMOVED*** NEW: Prisoner's Dilemma cooperation metrics
    pd_system_cooperation_rate: float | None = None
    pd_exploitation_alerts: list[str] | None = None
    pd_network_health: str | None = None  ***REMOVED*** "healthy", "degraded", "toxic"


***REMOVED*** Add to ResilienceService class
...
def _detect_exploitation_patterns(self, swaps: list[SwapRecord]) -> list[str]:
    ...
    ***REMOVED*** Detect exploiters (high receive, low give)
    for fid, stats in faculty_stats.items():
        ...
        ***REMOVED*** Detect defectors (low completion rate)
...
***REMOVED******REMOVED*** 4. Add AI Agent Testing Endpoint

***REMOVED******REMOVED******REMOVED*** New API Route
...
async def run_human_ai_tournament(
    ...
    ***REMOVED*** Create AI strategies from evaluations
    ai_strategy_ids = []
...
***REMOVED******REMOVED*** 5. Database Model Updates

**File:** `backend/app/models/game_theory.py`

```python
***REMOVED*** Add to ConfigStrategy model

class ConfigStrategy(Base):
    __tablename__ = "config_strategies"

    ***REMOVED*** ... existing columns ...

    ***REMOVED*** NEW: AI model fields
    is_ai_model = Column(Boolean, default=False)
    ai_model_id = Column(String, nullable=True)  ***REMOVED*** e.g., "claude-3-5-sonnet"
    bloom_evaluation_id = Column(String, nullable=True)  ***REMOVED*** Reference to Bloom run
    cooperation_rate = Column(Float, nullable=True)  ***REMOVED*** Measured from Bloom
...
***REMOVED******REMOVED*** 6. Usage Flow

***REMOVED******REMOVED******REMOVED*** Step 1: Run Bloom Evaluation

```bash
***REMOVED*** In Bloom repository
python bloom.py --seed seeds/scheduling_cooperation.yaml --target claude-3-5-sonnet
```

***REMOVED******REMOVED******REMOVED*** Step 2: Import Results to Scheduler

```python
***REMOVED*** POST /api/game-theory/ai-strategies/from-bloom
{
...
***REMOVED******REMOVED******REMOVED*** Step 3: Run Human-AI Tournament

```python
***REMOVED*** POST /api/game-theory/tournaments/human-ai
{
...
***REMOVED******REMOVED******REMOVED*** Step 4: Monitor PD Network Health

```python
***REMOVED*** GET /api/game-theory/health/pd-network
***REMOVED*** Returns:
{
    "status": "healthy",
