***REMOVED*** MCP Orchestration Patterns
...
3. [Tool Composition DAGs](***REMOVED***tool-composition-dags)
4. [Meta-Tool Specifications](***REMOVED***meta-tool-specifications)
5. [Parallel Execution Patterns](***REMOVED***parallel-execution-patterns)
6. [MCP Prompt Templates](***REMOVED***mcp-prompt-templates)
7. [Error Recovery Flows](***REMOVED***error-recovery-flows)
8. [Implementation Guide](***REMOVED***implementation-guide)
...
***REMOVED******REMOVED******REMOVED*** Core Scheduling Tools (5)
...
***REMOVED******REMOVED******REMOVED*** Async Task Management (4)
...
***REMOVED******REMOVED******REMOVED*** Resilience Framework (13)
...
***REMOVED******REMOVED******REMOVED*** Deployment Workflow (7)
...
***REMOVED******REMOVED******REMOVED*** Empirical Testing (5)
...
***REMOVED******REMOVED******REMOVED*** Resources (2)
...
***REMOVED******REMOVED*** Tool Composition DAGs

***REMOVED******REMOVED******REMOVED*** DAG 1: Deep Schedule Audit
...
***REMOVED******REMOVED******REMOVED*** DAG 2: Conflict Resolution Pipeline
...
***REMOVED******REMOVED******REMOVED*** DAG 3: Resilience Health Check
...
***REMOVED******REMOVED******REMOVED*** DAG 4: Background Task Lifecycle
...
***REMOVED******REMOVED******REMOVED*** DAG 5: Deployment Pipeline
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 1: `deep_schedule_audit`
...
async def deep_schedule_audit(
    ...
    ***REMOVED*** Level 2: Contingency Planning
    ...
    ***REMOVED*** Level 3: Full Resilience (Parallel)
    ...
    ***REMOVED*** Level 4: Empirical (Optional, Slow)
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 2: `auto_resolve_conflicts`
...
async def auto_resolve_conflicts(
    ...
    while iteration < max_iterations:
        ***REMOVED*** Detect current conflicts
        ...
        ***REMOVED*** Process auto-resolvable conflicts
        resolutions = []
        ...
        for conflict in conflicts.conflicts:
            ...
            ***REMOVED*** Validate best candidate
            best = candidates.candidates[0]

            ***REMOVED*** Dry run validation
            validation = await validate_schedule(
                start_date, end_date
                ***REMOVED*** TODO: Backend needs dry-run swap validation endpoint
            )
...
class ConflictResolutionReport(BaseModel):
    status: str  ***REMOVED*** "success", "manual_review_required", "timeout"
    iterations: int
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 3: `full_resilience_check`
...
async def full_resilience_check(
    ...
    ***REMOVED*** Tier 1: Critical (Parallel)
    if include_tier1:
        tier1_tasks = []
    ...
    ***REMOVED*** Barrier: Wait for Tier 1 completion
    ...
    ***REMOVED*** Barrier: Wait for Tier 2 completion
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 4: `monitor_background_task`
...
class TaskMonitoringReport(BaseModel):
    status: str  ***REMOVED*** "success", "failure", "timeout", "canceled"
    task_id: str
    result: Any | None
    error: str | None
    elapsed_time: float
...
***REMOVED******REMOVED******REMOVED*** Meta-Tool 5: `safe_deployment_pipeline`
...
async def safe_deployment_pipeline(
    ...
    try:
        ...
        ***REMOVED*** Step 3: Deploy to Staging (external CI/CD)
        ***REMOVED*** Trigger GitHub Actions workflow via API
        ***REMOVED*** ...

        ***REMOVED*** Step 4: Smoke Tests (Staging)
        ...
        ***REMOVED*** Step 5: Production Promotion (requires approval)
        if environment == Environment.PRODUCTION:
            ...
            ***REMOVED*** Step 6: Smoke Tests (Production)
            if not skip_smoke_tests:
        ...
        ***REMOVED*** Rollback on any failure
        if deployment_id:
...
class DeploymentPipelineReport(BaseModel):
    status: str  ***REMOVED*** "success", "validation_failed", "security_blocked", "smoke_tests_failed", "approval_required", "failed"
...
***REMOVED******REMOVED*** Parallel Execution Patterns

***REMOVED******REMOVED******REMOVED*** Pattern 1: Independent Reads (Max Parallelism)
...
results = await asyncio.gather(
    ...
    return_exceptions=True  ***REMOVED*** Don't fail entire batch on single error
)
```
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Dependency Barriers (Phased Parallelism)
...
**Execution**:
```python
***REMOVED*** Phase 1: Tier 1 (parallel within tier)
...
***REMOVED*** Phase 2: Tier 2 (parallel within tier)
...
***REMOVED*** Phase 3: Tier 3 (parallel within tier)
...
***REMOVED******REMOVED******REMOVED*** Pattern 3: Scatter-Gather (Fan-Out, Fan-In)
...
**Execution**:
```python
***REMOVED*** Scatter: Process each conflict in parallel
conflict_tasks = [
...
***REMOVED*** Gather: Aggregate results
best_resolutions = [
...
***REMOVED******REMOVED******REMOVED*** Pattern 4: Sequential with Side Effects (No Parallelism)
...
***REMOVED*** Can't parallelize deployment steps
...
***REMOVED******REMOVED******REMOVED*** Pattern 5: Background Task + Polling (Async Monitoring)
...
***REMOVED*** User can continue other work while task runs
...
***REMOVED******REMOVED******REMOVED*** Result Aggregation
...
async def aggregate_results(tools: list[Callable]) -> AggregatedReport:
    ...
    ***REMOVED*** Collect results and errors
...
***REMOVED******REMOVED*** MCP Prompt Templates

***REMOVED******REMOVED******REMOVED*** Template 1: Schedule Review Prompt
...
***REMOVED******REMOVED*** Schedule Audit Report - [DATE]
...
***REMOVED******REMOVED******REMOVED*** Template 2: Crisis Response Prompt
...
***REMOVED******REMOVED******REMOVED*** Template 3: Deployment Approval Prompt
...
***REMOVED******REMOVED******REMOVED*** Template 4: Empirical Analysis Prompt
...
***REMOVED******REMOVED******REMOVED*** Template 5: Background Task Management Prompt
...
***REMOVED******REMOVED*** Error Recovery Flows

***REMOVED******REMOVED******REMOVED*** Flow 1: Retry with Exponential Backoff
...
async def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if e.response.status_code == 429:  ***REMOVED*** Rate limit
                retry_after = int(e.response.headers.get("Retry-After", base_delay * (2 ** attempt)))
                await asyncio.sleep(min(retry_after, max_delay))
            elif 500 <= e.response.status_code < 600:  ***REMOVED*** Server error
                if attempt == max_retries:
                    raise
                delay = min(base_delay * (2 ** attempt), max_delay)
                await asyncio.sleep(delay)
            else:
                ***REMOVED*** Don't retry client errors
                raise
```
...
***REMOVED******REMOVED******REMOVED*** Flow 2: Fallback to Alternative Tool
...
async def fallback_chain(
    ...
    Example:
        ***REMOVED*** Try backend API, fall back to mock data
    ...
    for i, tool in enumerate(tools):
        try:
            ...
            if i > 0:
                logger.warning(f"Using fallback ***REMOVED***{i}: {tool.__name__}")
            ...
            ***REMOVED*** Check if we should try next fallback
...
***REMOVED******REMOVED******REMOVED*** Flow 3: Graceful Degradation
...
async def graceful_degradation(
    ...
    ***REMOVED*** Execute all tools in parallel
    results = await asyncio.gather(
        *[tool() for _, tool in tools],
        return_exceptions=True
    )

    ***REMOVED*** Check required tools
    ...
    ***REMOVED*** Return all results (including errors)
    return {
        name: result
...
**Example**:
```python
***REMOVED*** Resilience check: Tier 1 is required, Tier 2/3 are optional
results = await graceful_degradation(
    tools=[
        ("utilization", lambda: check_utilization_threshold(...)),  ***REMOVED*** Required
        ("defense", lambda: get_defense_level(...)),  ***REMOVED*** Required
        ("contingency", lambda: run_contingency_analysis_resilience(...)),  ***REMOVED*** Required
        ("homeostasis", lambda: analyze_homeostasis(...)),  ***REMOVED*** Optional
        ("blast_radius", lambda: calculate_blast_radius(...)),  ***REMOVED*** Optional
    ],
    required_tools={"utilization", "defense", "contingency"}
)

***REMOVED*** Even if homeostasis/blast_radius fail, we get partial results
...
***REMOVED******REMOVED******REMOVED*** Flow 4: Circuit Breaker Pattern
...
class CircuitBreaker:
    ...
    async def call(self, tool: Callable) -> Any:
        ...
        """
        if self.state == "OPEN":
            ***REMOVED*** Check if we should try half-open
        ...
        try:
            ...
            ***REMOVED*** Success: reset or close
            if self.state == "HALF_OPEN":
                self.state = "CLOSED"
                self.failure_count = 0
...
**Usage**:
```python
***REMOVED*** Global circuit breaker for backend API
...
async def validate_schedule_with_breaker(start_date, end_date):
    try:
        ...
        ***REMOVED*** Use fallback (mock data)
        return mock_validate_schedule(start_date, end_date)
```

---

***REMOVED******REMOVED******REMOVED*** Flow 5: Rollback on Validation Failure
...
async def transactional_schedule_operation(
    ...
    Example:
        ***REMOVED*** Apply swap with rollback
    ...
    """
    ***REMOVED*** Take snapshot before operation
...
**Example**:
```python
***REMOVED*** Apply conflict resolution with rollback
try:
    result = await transactional_schedule_operation(
...
***REMOVED******REMOVED******REMOVED*** Error Classification
...
***REMOVED******REMOVED******REMOVED*** Python Implementation (Backend/MCP Server)
...
├── orchestration/
│   ├── __init__.py
│   ├── meta_tools.py          ***REMOVED*** Meta-tool implementations
│   ├── dag_executor.py        ***REMOVED*** DAG execution engine
│   ├── parallel.py            ***REMOVED*** Parallel execution patterns
│   ├── retry.py               ***REMOVED*** Retry and error recovery
│   └── prompts.py             ***REMOVED*** Prompt template registry
├── server.py                  ***REMOVED*** Existing MCP server
└── tools.py                   ***REMOVED*** Existing tool definitions
```

**Meta-Tool Registration**:
```python
***REMOVED*** mcp-server/src/scheduler_mcp/orchestration/meta_tools.py
...
**DAG Executor**:
```python
***REMOVED*** mcp-server/src/scheduler_mcp/orchestration/dag_executor.py
...
class DAGExecutor:
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        """
        ***REMOVED*** Topological sort
        execution_order = self._topological_sort()

        for node_name in execution_order:
            node = self.nodes[node_name]

            ***REMOVED*** Wait for dependencies
            await self._wait_for_dependencies(node)

            ***REMOVED*** Execute node
            try:
    ...
    def _topological_sort(self) -> List[str]:
        ...
        ***REMOVED*** Implementation omitted for brevity
        pass
...
***REMOVED******REMOVED******REMOVED*** TypeScript Implementation (Frontend/AI Client)

**File Structure**:
```
frontend/src/lib/mcp/
├── orchestration/
│   ├── meta-tools.ts          ***REMOVED*** Meta-tool client wrappers
│   ├── dag-executor.ts        ***REMOVED*** DAG execution (client-side)
│   ├── parallel.ts            ***REMOVED*** Parallel execution
│   ├── retry.ts               ***REMOVED*** Retry logic
│   └── prompts.ts             ***REMOVED*** Prompt templates
├── client.ts                  ***REMOVED*** Existing MCP client
└── tools.ts                   ***REMOVED*** Existing tool types
```
...
orchestration:
  ...
  ***REMOVED*** Timeouts
  timeouts:
    ...
    resilience_check: 30
    deployment_pipeline: 1800  ***REMOVED*** 30 minutes
```

---

***REMOVED******REMOVED******REMOVED*** Monitoring and Observability
...
**Logging**:
```python
***REMOVED*** Structured logging for orchestration
logger.info(
