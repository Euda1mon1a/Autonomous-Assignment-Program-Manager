***REMOVED*** ADR-009: Time Crystal Scheduling (Anti-Churn)
...
***REMOVED******REMOVED******REMOVED*** Core Concepts
...
***REMOVED******REMOVED******REMOVED*** Anti-Churn Objective
```python
def add_anti_churn_objective(model, current_assignments, previous_assignments):
    ...
    for (person, block, rotation) in current_assignments:
        ...
        ***REMOVED*** Variable = 1 if changed (either added or removed)
    ...
    ***REMOVED*** Total changes to minimize
...
def calculate_rigidity(current: Schedule, proposed: Schedule) -> float:
    ...
    if not union:
        return 1.0  ***REMOVED*** Both empty = identical

    return len(overlap) / len(union)
```

***REMOVED******REMOVED******REMOVED*** Stroboscopic Checkpoints
```python
def should_regenerate(current_date: date, last_regen: date) -> bool:
    """Only regenerate at natural boundaries."""
    ***REMOVED*** Weekly boundary (Sunday/Monday)
    if current_date.weekday() == 0 and last_regen.weekday() != 0:
        return True

    ***REMOVED*** Block boundary (based on academic calendar)
    if is_block_boundary(current_date):
        return True

    ***REMOVED*** ACGME 4-week averaging window boundary
...
***REMOVED******REMOVED******REMOVED*** Change Report
```
...
***REMOVED******REMOVED******REMOVED*** Stability Settings
```yaml
anti_churn:
  enabled: true
  weight: 10.0      ***REMOVED*** Higher = more stability preference
  min_rigidity: 0.7 ***REMOVED*** Alert if below this threshold
  report_changes: true
```
...
- [ADR-006: Swap System](../../.claude/dontreadme/synthesis/DECISIONS.md***REMOVED***adr-006-swap-system-with-auto-matching) - Swaps trigger schedule changes
