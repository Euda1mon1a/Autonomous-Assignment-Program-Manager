***REMOVED*** ADR-002: Constraint Programming (OR-Tools) for Scheduling
...
***REMOVED******REMOVED******REMOVED*** Constraint Categories
...
***REMOVED******REMOVED******REMOVED*** Constraint Variable Model
```python
from ortools.sat.python import cp_model

model = cp_model.CpModel()

***REMOVED*** Assignment variables: person p assigned to block b with rotation r
assignment = {}
for p in persons:
    for b in blocks:
        for r in rotations:
...
***REMOVED******REMOVED******REMOVED*** Hard Constraint Example (80-Hour Rule)
```python
***REMOVED*** Person cannot exceed 80 hours per week (rolling average over 4 weeks)
for person in persons:
    for week_start in week_starts:
        ...
        ***REMOVED*** Average must be <= 80
        model.Add(sum(four_week_hours) <= 80 * 4)
```

***REMOVED******REMOVED******REMOVED*** Soft Constraint Example (Workload Balance)
```python
***REMOVED*** Minimize variance in workload across persons
workloads = {}
for person in persons:
...
***REMOVED*** Minimize max - min (proxy for variance)
...
***REMOVED******REMOVED******REMOVED*** Solving with Timeout
```python
...
if status == cp_model.OPTIMAL:
    ***REMOVED*** Extract optimal solution
    pass
elif status == cp_model.FEASIBLE:
    ***REMOVED*** Solution found but may not be optimal
    pass
elif status == cp_model.INFEASIBLE:
    ***REMOVED*** No valid schedule exists - identify conflicting constraints
    pass
```
