***REMOVED*** ADR-003: MCP Server for AI Integration
...
***REMOVED******REMOVED******REMOVED*** MCP Server Configuration
```yaml
...
mcp-server:
  ...
  depends_on:
    - backend
  ports:
    - "8080:8080"  ***REMOVED*** Dev mode HTTP transport
```

***REMOVED******REMOVED******REMOVED*** Tool Registration
```python
***REMOVED*** mcp-server/scheduler_mcp/tools/scheduling.py
...
- [ADR-007: Monorepo with Docker Compose](../../.claude/dontreadme/synthesis/DECISIONS.md***REMOVED***adr-007-monorepo-with-docker-compose) - MCP server in Docker
