***REMOVED*** ADR-008: Slot-Type Invariants for Credentials
...
***REMOVED******REMOVED******REMOVED*** Invariant Catalog Structure
```python
invariant_catalog = {
    "inpatient_call": {
...
***REMOVED******REMOVED******REMOVED*** Credential Model
```python
class Credential(Base):
    ...
    id: str
    person_id: str
    credential_type: str  ***REMOVED*** "HIPAA", "BLS", "N95_Fit"
    issued_at: date
    expires_at: date
    is_valid: bool
    verification_status: str  ***REMOVED*** "verified", "pending", "expired"
    document_url: str | None  ***REMOVED*** S3 link to certificate
```

***REMOVED******REMOVED******REMOVED*** Eligibility Check
```python
class SlotTypeValidator:
    ...
    def is_eligible(
        ...
        ***REMOVED*** Check hard constraints (must pass all)
        for req in reqs.get("hard", []):
            ...
            if not cred or not cred.is_valid or cred.expires_at < assignment_date:
                return False, 0  ***REMOVED*** Hard failure

        ***REMOVED*** Check soft constraints (accumulate penalties)
        penalty = 0
...
***REMOVED******REMOVED******REMOVED*** Schedule Generation Integration
```python
class ScheduleGenerator:
    ...
    async def generate_schedule(
        ...
        ***REMOVED*** Build constraint model
        model = cp_model.CpModel()

        ***REMOVED*** For each assignment variable
        for person_id, slot_type, date in assignments:
            ...
            ***REMOVED*** Hard constraint: cannot assign if ineligible
            if not eligible:
                model.Add(assignment_var == 0)

            ***REMOVED*** Soft constraint: penalize if credentials expiring soon
            if penalty > 0:
                objective_terms.append(-penalty * assignment_var)

        ***REMOVED*** Solve with constraints
...
***REMOVED******REMOVED******REMOVED*** Common Credential Categories
