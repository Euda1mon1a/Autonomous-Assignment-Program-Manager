...
- **Faculty Precepting Constraint:** The daily solver successfully enforces strict supervision requirements. If no faculty are preloaded/scheduled for a specific half-day, the solver will correctly return an OPTIMAL schedule with 0 resident clinic assignments rather than illegally scheduling un-precepted residents. Faculty schedules must be loaded prior to resident clinic solving.
- **Solver Enhancement:** We need to transition the faculty  hard floor (currently ensuring at least 1 clinic per week) into a purely soft constraint driven by an equity/penalty model. This allows the solver to naturally drop a faculty member to 0 clinics during critical staffing shortages where all available faculty must be diverted to resident supervision (AT) or inpatient coverage, without throwing an INFEASIBLE error.
- **Solver Enhancement:** We need to transition the faculty clinic_min hard floor (currently ensuring at least 1 clinic per week) into a purely soft constraint driven by an equity/penalty model. This allows the solver to naturally drop a faculty member to 0 clinics during critical staffing shortages where all available faculty must be diverted to resident supervision (AT) or inpatient coverage, without throwing an INFEASIBLE error.
- **Call Equity Refactor:** Transition the call solver from a simple proximity penalty to a cumulative, stackable Year-To-Date (YTD) equity penalty. The solver must read historical call points, add the proposed block's points, and minimize the variance within PGY cohorts. Crucially, residents must *not* be penalized (or fall behind in the equity distribution) for approved absences (Leave/TDY) — their targets must be prorated by their available days.
- **Target Halfdays Refactor:** Update the CP-SAT Activity Solver to utilize the existing `target_halfdays` column on `rotation_activity_requirements`. Currently, the solver only strictly binds between `min_halfdays` and `max_halfdays`. By transitioning to soft targets with objective function incentives (e.g., maximizing the number of slots filled up to the target), we eliminate 'unassigned' spaces while allowing the solver to fluidly adapt to physical/faculty constraints without throwing INFEASIBLE.
