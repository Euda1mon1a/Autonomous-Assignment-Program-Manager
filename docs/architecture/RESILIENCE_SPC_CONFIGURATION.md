***REMOVED*** Statistical Process Control (SPC) Configuration Guide
...
1. [SPC System Overview](***REMOVED***spc-system-overview)
2. [Control Limit Configuration](***REMOVED***control-limit-configuration)
3. [Alert Configuration](***REMOVED***alert-configuration)
4. [Integration Points](***REMOVED***integration-points)
5. [Monitoring Rules](***REMOVED***monitoring-rules)
6. [Dashboard Configuration](***REMOVED***dashboard-configuration)
7. [Alert Response SLAs](***REMOVED***alert-response-slas)

---

***REMOVED******REMOVED*** SPC System Overview

***REMOVED******REMOVED******REMOVED*** What is SPC?
...
***REMOVED******REMOVED******REMOVED*** Frequency
...
***REMOVED******REMOVED*** Control Limit Configuration

***REMOVED******REMOVED******REMOVED*** Default Parameters

```python
***REMOVED*** Target workload: 60 hours per week
TARGET_HOURS = 60.0

***REMOVED*** Process standard deviation: 5 hours per week
SIGMA = 5.0

***REMOVED*** Calculated control limits:
UCL_3_SIGMA = 75.0   ***REMOVED*** Upper Control Limit (CRITICAL)
UCL_2_SIGMA = 70.0   ***REMOVED*** Upper Warning Limit (WARNING)
UCL_1_SIGMA = 65.0   ***REMOVED*** Upper Zone boundary
CENTERLINE = 60.0    ***REMOVED*** Target
LCL_1_SIGMA = 55.0   ***REMOVED*** Lower Zone boundary
LCL_2_SIGMA = 50.0   ***REMOVED*** Lower Warning Limit
LCL_3_SIGMA = 45.0   ***REMOVED*** Lower Control Limit (CRITICAL)
```

***REMOVED******REMOVED******REMOVED*** Rationale for Parameters
...
***REMOVED******REMOVED******REMOVED*** Customization by Program

**Military Medical Programs (High-Stress):**

```python
MILITARY_ADJUSTMENTS = {
    "target_hours": 70.0,       ***REMOVED*** Higher baseline due to TDY/deployment
    "sigma": 6.0,               ***REMOVED*** Greater variation expected
    "ucl_3_sigma": 88.0,        ***REMOVED*** Higher limits, but still <ACGME 80h
    "lcl_3_sigma": 52.0,
}
```
...
```yaml
***REMOVED*** backend/config/spc_config.yaml
...
spc_monitoring:
  ...
  specialties:
    ...
    surgery:
      target_hours: 75.0
      sigma: 6.0
      ucl_3_sigma_cap: 90.0  ***REMOVED*** Don't exceed ACGME limit
    psychiatry:
      target_hours: 50.0
      sigma: 3.5
```
...
***REMOVED******REMOVED*** Alert Configuration
...
**Default Threshold:**
```python
UCL_CRITICAL = 75.0  ***REMOVED*** hours/week
LCL_CRITICAL = 45.0  ***REMOVED*** hours/week
...
***REMOVED******REMOVED******REMOVED*** Rule 2: 2 of 3 Beyond 2σ (WARNING)

**Trigger:** In any 3-week window, 2+ weeks with hours > UCL_2σ OR < LCL_2σ (same side)

**Default Threshold:**
```python
UCL_WARNING = 70.0  ***REMOVED*** hours/week
LCL_WARNING = 50.0  ***REMOVED*** hours/week
...
***REMOVED******REMOVED******REMOVED*** Rule 3: 4 of 5 Beyond 1σ (WARNING)

**Trigger:** In any 5-week window, 4+ weeks with hours > UCL_1σ OR < LCL_1σ (same side)

**Default Threshold:**
```python
UCL_TREND = 65.0   ***REMOVED*** hours/week
LCL_TREND = 55.0   ***REMOVED*** hours/week
...
***REMOVED******REMOVED******REMOVED*** Rule 4: 8 Consecutive Same Side (INFO)
...
***REMOVED******REMOVED******REMOVED*** Data Source: Work Hour Collection
...
def validate_weekly_hours(resident_id, week_ending_date, hours):
    ...
    ***REMOVED*** Check bounds
    if not (0 <= hours <= 168):
        raise ValueError(f"Hours out of range: {hours}")

    ***REMOVED*** Check for duplicate entries
    ...
    if existing and existing != hours:
        ...
        return False  ***REMOVED*** Reject update
    ...
    ***REMOVED*** Check for anomalies
    ...
    return True  ***REMOVED*** Accept
```

***REMOVED******REMOVED******REMOVED*** Output: Alert Generation
...
def route_alert(alert):
    ...
    if alert.severity == "CRITICAL":
        ***REMOVED*** Call program director immediately
        ...
        ***REMOVED*** Email chief resident, dashboard alert
        notify_email(alert.chief_resident.email)
        notify_dashboard(alert)

    elif alert.severity == "INFO":
        ***REMOVED*** Dashboard only
        notify_dashboard(alert)
...
***REMOVED******REMOVED*** Monitoring Rules

***REMOVED******REMOVED******REMOVED*** Rule Precedence (if multiple triggered)
...
***REMOVED******REMOVED******REMOVED*** Example Multi-Rule Scenario
...
***REMOVED******REMOVED******REMOVED*** Hysteresis for SPC Alerts
...
def check_hysteresis(resident_id, rule, timestamp):
    ...
    if days_since < 7:
        ...
        return False  ***REMOVED*** Don't send alert

    return True  ***REMOVED*** Send alert
```
...
***REMOVED******REMOVED******REMOVED*** Grafana Dashboard: SPC Monitoring

**Panels to Create:**

***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 1: Alert Heatmap
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 2: Current Alerts
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 3: Process Capability Index (Cpk)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 4: Rule Distribution
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 5: Alert Response Metrics
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Panel 6: Individual Resident Trend
...
***REMOVED******REMOVED*** Alert Response SLAs

***REMOVED******REMOVED******REMOVED*** CRITICAL (Rule 1)
...
***REMOVED******REMOVED******REMOVED*** WARNING (Rules 2 & 3)
...
***REMOVED******REMOVED******REMOVED*** INFO (Rule 4)
...
***REMOVED******REMOVED*** Configuration Maintenance
...
***REMOVED******REMOVED******REMOVED*** Annual Recalibration
...
***REMOVED******REMOVED******REMOVED*** Problem: Too Many Rule 2/3 Alerts (Alert Fatigue)
...
***REMOVED******REMOVED******REMOVED*** Problem: Rule 1 Alerts for Data Entry Errors
...
***REMOVED******REMOVED******REMOVED*** Problem: No Alerts When Issues Occur
