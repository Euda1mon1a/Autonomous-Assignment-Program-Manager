***REMOVED*** Call Generation Architecture
...
***REMOVED******REMOVED*** Component Details

***REMOVED******REMOVED******REMOVED*** 1. SchedulingContext Extension
...
class SchedulingContext:
    ...
    ***REMOVED*** Call Assignment Data (for overnight call generation)
...
***REMOVED******REMOVED******REMOVED*** 2. Engine Integration
...
***REMOVED******REMOVED******REMOVED*** 3. Solver Variable Creation
...
if call_eligible:
    ...
    ***REMOVED*** IMPORTANT: Use context.blocks, NOT workday_blocks
    ***REMOVED*** workday_blocks excludes is_weekend=True, but Sunday (weekday 6)
    ***REMOVED*** requires overnight call despite being marked as weekend
    call_blocks = [
        block
...
> **Note:** Prior to PR ***REMOVED***489, this code incorrectly used `workday_blocks` which
...
***REMOVED******REMOVED******REMOVED*** 4. Solution Extraction

```python
***REMOVED*** Extract call assignments from solution
call_assignments_result = []
for (f_i, b_i, call_type), var in call.items():
    if solver.Value(var) == 1:
        faculty_id = ***REMOVED*** lookup from call_eligible
        block_id = ***REMOVED*** lookup from blocks
...
***REMOVED******REMOVED*** Database Model
...
class CallAssignment(Base):
    ...
    call_type = Column(String(50), nullable=False)  ***REMOVED*** 'overnight', 'weekend', 'backup'
...
***REMOVED*** Call equity constraints (already existed):
