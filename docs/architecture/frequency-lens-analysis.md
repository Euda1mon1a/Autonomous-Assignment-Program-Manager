***REMOVED*** Frequency Lens: Spectral Analysis for Schedule Optimization
...
2. [Why Frequency Domain Analysis?](***REMOVED***why-frequency-domain-analysis)
3. [Core Techniques](***REMOVED***core-techniques)
4. [STA/LTA Anomaly Detection](***REMOVED***stalta-anomaly-detection)
5. [Wavelet Analysis](***REMOVED***wavelet-analysis)
6. [Harmonic Analysis](***REMOVED***harmonic-analysis)
7. [Constraint Validation](***REMOVED***frequency-based-constraint-validation)
8. [Usage Examples](***REMOVED***usage-examples)
9. [Visualization Export](***REMOVED***visualization-export)
10. [Performance Considerations](***REMOVED***performance-considerations)
...
***REMOVED******REMOVED******REMOVED*** The "Lens" Metaphor
...
***REMOVED******REMOVED*** Why Frequency Domain Analysis?

***REMOVED******REMOVED******REMOVED*** Problem with Time-Domain Only
...
***REMOVED******REMOVED******REMOVED*** Frequency Domain Reveals
...
***REMOVED******REMOVED******REMOVED*** Clinical Significance
...
***REMOVED******REMOVED*** Core Techniques

***REMOVED******REMOVED******REMOVED*** 1. Fast Fourier Transform (FFT)
...
***REMOVED******REMOVED******REMOVED*** 2. Wavelet Transform
...
***REMOVED******REMOVED******REMOVED*** 3. Spectral Decomposition (STL)
...
***REMOVED******REMOVED*** STA/LTA Anomaly Detection

***REMOVED******REMOVED******REMOVED*** Origin: Seismology
...
***REMOVED******REMOVED******REMOVED*** Interpretation for Schedules
...
***REMOVED******REMOVED******REMOVED*** Types of Detected Anomalies
...
processor = WorkloadSignalProcessor(
    sta_window=5,      ***REMOVED*** 5-day short-term average
    lta_window=20,     ***REMOVED*** 20-day long-term average
    sta_lta_threshold=3.0  ***REMOVED*** Trigger at 3x ratio
)
```
...
***REMOVED******REMOVED******REMOVED*** Discrete Wavelet Transform (DWT)
...
***REMOVED******REMOVED******REMOVED*** Wavelet Selection
...
***REMOVED******REMOVED******REMOVED*** Continuous Wavelet Transform (CWT)
...
***REMOVED******REMOVED*** Harmonic Analysis

***REMOVED******REMOVED******REMOVED*** What Are Harmonics?
...
***REMOVED******REMOVED******REMOVED*** Why Harmonics Matter
...
***REMOVED******REMOVED******REMOVED*** Total Harmonic Distortion (THD)
...
***REMOVED******REMOVED*** Frequency-Based Constraint Validation

***REMOVED******REMOVED******REMOVED*** Built-in Constraints
...
***REMOVED******REMOVED******REMOVED*** Violation Detection
...
***REMOVED******REMOVED******REMOVED*** Example Violation
...
***REMOVED******REMOVED******REMOVED*** Custom Constraints

```python
custom_constraint = FrequencyConstraint(
    name="post_call_recovery",
    max_frequency=0.33,  ***REMOVED*** Max 1 cycle per 3 days
    min_period_days=3.0,
...
***REMOVED******REMOVED******REMOVED*** Basic Analysis
...
***REMOVED*** Create sample data (90 days of daily hours)
dates = [date(2025, 1, 1) + timedelta(days=i) for i in range(90)]
hours = np.random.normal(8, 2, 90)  ***REMOVED*** Simulated workload

***REMOVED*** Add weekly pattern
for i in range(90):
    if dates[i].weekday() in [5, 6]:  ***REMOVED*** Weekend
        hours[i] *= 0.5  ***REMOVED*** Reduced weekend work

***REMOVED*** Create time series
...
***REMOVED*** Initialize processor and analyze
processor = WorkloadSignalProcessor()
...
***REMOVED******REMOVED******REMOVED*** Quick Anomaly Detection

```python
from app.analytics.signal_processing import detect_schedule_anomalies

***REMOVED*** Detect anomalies with custom threshold
anomalies = detect_schedule_anomalies(
    daily_hours=[8, 8, 14, 8, 8, 16, 8, 8],  ***REMOVED*** Spike on days 3, 6
    dates=dates[:8],
    threshold=2.5  ***REMOVED*** More sensitive
)
...
***REMOVED******REMOVED******REMOVED*** Extracting Specific Patterns

```python
***REMOVED*** Get only the weekly pattern
dwt_result = processor.discrete_wavelet_transform(ts, level=4)

***REMOVED*** Reconstruct with only weekly component (level 3)
weekly_pattern = processor.wavelet_reconstruct(
    dwt_result,
    keep_levels=[2]  ***REMOVED*** Level 3 ≈ 7-8 days
)

***REMOVED*** Compare original vs weekly pattern
print(f"Weekly pattern strength: {np.std(weekly_pattern) / np.std(ts.values):.2%}")
```

***REMOVED******REMOVED******REMOVED*** Filtering Noise

```python
***REMOVED*** Apply adaptive filtering
...
***REMOVED******REMOVED******REMOVED*** Holographic Format
...
3. **Selective Analysis**: Run only needed analyses:
   ...
   result = processor.analyze_workload_patterns(
       ts,
       analysis_types=["fft", "sta_lta"]  ***REMOVED*** Skip expensive CWT
   )
   ```
...
***REMOVED******REMOVED******REMOVED*** Burnout Fire Index
...
***REMOVED*** Use spectral decomposition trend for velocity
spectral = result["spectral_decomposition"]
trend = spectral["trend"]
velocity = (trend[-7] - trend[-14]) / 7  ***REMOVED*** Trend slope over last week
...
danger = rating.calculate_burnout_danger(
    ...
    workload_velocity=velocity  ***REMOVED*** From frequency analysis
)
```
