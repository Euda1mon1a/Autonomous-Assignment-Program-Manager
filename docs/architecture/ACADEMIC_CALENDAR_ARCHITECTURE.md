***REMOVED*** Academic Calendar Architecture: The Foundational Gap
...
***REMOVED******REMOVED*** The Problem Visualized
...
***REMOVED******REMOVED******REMOVED*** Block Boundary Conflicts
...
***REMOVED******REMOVED*** The Missing Model: ResidentBlockRotation
...
***REMOVED******REMOVED******REMOVED*** Required State
...
***REMOVED******REMOVED*** Schema Design

***REMOVED******REMOVED******REMOVED*** New Model: ResidentBlockRotation
...
class ResidentBlockRotation(Base):
    ...
    ***REMOVED*** Core relationship
    person_id: Mapped[int] = mapped_column(ForeignKey("people.id"))
    rotation_template_id: Mapped[int] = mapped_column(ForeignKey("rotation_templates.id"))

    ***REMOVED*** Academic calendar position
    block_number: Mapped[int] = mapped_column()  ***REMOVED*** 1-13
    academic_year: Mapped[str] = mapped_column(String(9))  ***REMOVED*** "2025-2026"

    ***REMOVED*** For half-block rotations (NF, NICU)
    block_half: Mapped[Optional[int]] = mapped_column(nullable=True)  ***REMOVED*** 1 or 2

    ***REMOVED*** Metadata
    assigned_by: Mapped[str] = mapped_column(String(50))  ***REMOVED*** "scheduler", "manual", "swap"
...
***REMOVED******REMOVED******REMOVED*** Enhanced RotationTemplate

```python
class CalendarMode(str, Enum):
    """How a rotation's time boundaries are calculated."""
    ACADEMIC_BLOCK = "academic_block"    ***REMOVED*** Standard 28-day blocks
    BLOCK_HALF = "block_half"            ***REMOVED*** 14-day halves (NF, NICU)
    FMIT_WEEK = "fmit_week"              ***REMOVED*** Friday-Thursday weeks
    GREGORIAN_MONTH = "gregorian_month"  ***REMOVED*** For reporting/admin only
    DAILY = "daily"                      ***REMOVED*** Individual day assignments
...
class RotationTemplate(Base):
    ...
    ***REMOVED*** NEW FIELDS
    ...
    week_structure: Mapped[Optional[str]] = mapped_column(
        String(20),
        nullable=True
    )  ***REMOVED*** "FRIDAY_THURSDAY" for FMIT
```

---

***REMOVED******REMOVED*** FMIT Calendar Exception Explained
...
***REMOVED******REMOVED******REMOVED*** Current Hardcoding (fmit.py:45-67)
...
def get_fmit_week_dates(any_date: date) -> tuple[date, date]:
    ...
    if day_of_week >= 4:  ***REMOVED*** Fri, Sat, Sun
        days_since_friday = day_of_week - 4
    else:  ***REMOVED*** Mon, Tue, Wed, Thu
...
***REMOVED******REMOVED******REMOVED*** Proposed: Data-Driven Calendar Mode
...
***REMOVED******REMOVED*** Hardcoding Locations to Refactor
...
***REMOVED******REMOVED*** Two-Phase Schedule Generation

***REMOVED******REMOVED******REMOVED*** Current (One Phase)
```
Solver → assigns half-day slots → done
```

***REMOVED******REMOVED******REMOVED*** Required (Two Phases)
...
***REMOVED******REMOVED******REMOVED*** Step 1: Add New Fields to RotationTemplate
...
***REMOVED******REMOVED******REMOVED*** Step 2: Create ResidentBlockRotation Model

```bash
alembic revision --autogenerate -m "Add ResidentBlockRotation model"
```

***REMOVED******REMOVED******REMOVED*** Step 3: Add FK to Assignment (Optional)

```bash
alembic revision --autogenerate -m "Add resident_block_rotation_id to Assignment"
```

***REMOVED******REMOVED******REMOVED*** Step 4: Seed calendar_mode Values

```python
***REMOVED*** In seed script or migration
ROTATION_CALENDAR_MODES = {
    ...
    ***REMOVED*** All others default to ("academic_block", True, None)
}
```

***REMOVED******REMOVED******REMOVED*** Step 5: Refactor Engine

Replace `preserve_fmit` pattern with:
```python
***REMOVED*** Load block rotations, not individual assignments
...
for br in block_rotations:
    if br.rotation_template.is_solver_eligible:
        ...
        ***REMOVED*** Skip - these are pre-assigned
        pass
```
...
***REMOVED******REMOVED*** Why This Supersedes Block 10
...
***REMOVED******REMOVED*** For Local Claude Code
...
***REMOVED******REMOVED******REMOVED*** Execution Order
