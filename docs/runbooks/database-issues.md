***REMOVED*** Runbook: Database Issues
...
```bash
***REMOVED*** 1. Check database container status
docker compose ps db

***REMOVED*** 2. Check if PostgreSQL is accepting connections
docker compose exec db pg_isready -U scheduler

***REMOVED*** 3. Check database logs
docker compose logs --tail=100 db

***REMOVED*** 4. Check connection count
docker compose exec db psql -U scheduler -c \
  "SELECT count(*) as connections FROM pg_stat_activity;"

***REMOVED*** 5. Check long-running queries
docker compose exec db psql -U scheduler -c \
...
***REMOVED******REMOVED******REMOVED*** Database Unavailable
...
***REMOVED*** Check backend can connect
