...
***REMOVED******REMOVED*** ⚠️ DEVIATION POLICY (READ FIRST)
...
***REMOVED******REMOVED******REMOVED*** Deviation Log

| Date | Reason | Approved By | Resolution |
|------|--------|-------------|------------|
| 2025-12-26 | activity_type filter fix (PR ***REMOVED***442) | Human | Corrected "clinic" → "outpatient" |
...
***REMOVED******REMOVED******REMOVED*** What's Now Complete ✅ (Updated 2025-12-25)
...
***REMOVED******REMOVED******REMOVED*** Remaining Work (Optional Enhancements)

| Component | Status | Notes |
|-----------|--------|-------|
| **Swap Executor** | 40% | TODOs documented, DB model needed for enhanced features |

***REMOVED******REMOVED******REMOVED*** Root Causes Identified & RESOLVED ✅
...
***REMOVED******REMOVED*** 🎯 BLOCK 10 TASK QUEUE - COMPLETE ✅

> **Completed:** 2025-12-26
> **Final Status:** All tasks done, PRs merged, constraints implemented and tested

***REMOVED******REMOVED******REMOVED*** Sprint Complete: Block 10 Generation + Post-Block 10 Enhancements
...
***REMOVED******REMOVED******REMOVED*** Task Queue (Execute in Order) - ALL COMPLETE ✅

| ID | Task | Est. | Status |
|----|------|------|--------|
| **T1** | ~~Fix activity_type filter (clinic → outpatient)~~ | 30m | ✅ Merged PR ***REMOVED***443, ***REMOVED***444 |
...
| **T8** | ~~Create PR for review~~ | 15m | ✅ PRs ***REMOVED***440-***REMOVED***445 merged |

***REMOVED******REMOVED******REMOVED*** Human Tasks - ALL COMPLETE ✅

| ID | Task | Status |
|----|------|--------|
| **H1** | ~~Review and merge activity_type fix PR~~ | ✅ Merged PR ***REMOVED***443 |
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 2: Schedule Generation ✅ COMPLETE
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 3: Quality Analysis ✅ READY (No Longer Blocked)
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 4: UI/UX Testing ✅ READY
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 5: Production Readiness ✅ READY
...
***REMOVED******REMOVED*** Available Capabilities Summary

***REMOVED******REMOVED******REMOVED*** Skills (Invoke via `Skill` tool)
...
***REMOVED******REMOVED******REMOVED*** MCP Tools Status
...
***REMOVED******REMOVED******REMOVED*** Execution Tools
...
***REMOVED******REMOVED******REMOVED*** Current Status: PRODUCTION READY (v1.0.0)
...
***REMOVED******REMOVED******REMOVED*** Block 10 Context
...
***REMOVED******REMOVED*** Checkpoint System

***REMOVED******REMOVED******REMOVED*** Checkpoint 1: Data Preparation (CLAUDE CODE IDE) ✅ COMPLETE
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 2: Schedule Generation Test (CLAUDE CODE IDE) ⚠️ PARTIAL
...
***REMOVED******REMOVED******REMOVED*** Sanitized Data Export for Claude Web (No PII)
...
***REMOVED******REMOVED******REMOVED*** Existing Pattern: `preserve_fmit` (Use as Template)

**Location:** `backend/app/scheduling/engine.py:148-154, 842-857`

```python
***REMOVED*** Already implemented pattern for FMIT faculty preservation:
def generate_schedule(
    ...,
    preserve_fmit: bool = True,  ***REMOVED*** <-- Parameter exists!
):
    if preserve_fmit:
        fmit_assignments = self._load_fmit_assignments()
        preserve_ids = {a.id for a in fmit_assignments}
        ***REMOVED*** These IDs are protected from deletion during regeneration
...
├── fmit.py                  ***REMOVED*** FMIT-specific constraints
├── night_float_post_call.py ***REMOVED*** NF→PC transition rules
├── capacity.py              ***REMOVED*** OnePersonPerBlockConstraint (max 1 per block)
├── base.py                  ***REMOVED*** HardConstraint/SoftConstraint base classes
└── manager.py               ***REMOVED*** Constraint registration
```
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 3: Quality Analysis (CLAUDE WEB)
...
***REMOVED******REMOVED*** Checkpoint 3 Design Document (Claude Web - 2025-12-25)

***REMOVED******REMOVED******REMOVED*** Design 1: Inpatient Pre-Loading Mechanism
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Solution: Extend `preserve_fmit` Pattern
...
def generate_schedule(
    ...
    preserve_resident_inpatient: bool = True,  ***REMOVED*** NEW
):
    ***REMOVED*** Step 1.5a: Load FMIT faculty assignments
    if preserve_fmit:
        fmit_assignments = self._load_fmit_assignments()
        preserve_ids.update({a.id for a in fmit_assignments})

    ***REMOVED*** Step 1.5b: Load resident inpatient assignments  ***REMOVED*** NEW
...
***REMOVED******REMOVED******REMOVED******REMOVED*** New Method: `_load_resident_inpatient_assignments()`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pre-Loading from Airtable Source
...
def _preload_inpatient_from_source(self, block_number: int) -> None:
    ...
    """
    ***REMOVED*** Implementation by Claude Code (IDE) - requires DB access
    pass
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Inpatient Rotation Rules (Encoded as Constraints)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** New Constraint: `ResidentInpatientHeadcountConstraint`
...
class ResidentInpatientHeadcountConstraint(HardConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** FMIT headcount per PGY level per block
        ...
        ***REMOVED*** NF headcount per day (across all residents)
        for block in context.blocks:
            b_i = context.block_idx[block.id]
            nf_vars = []
...
***REMOVED******REMOVED******REMOVED******REMOVED*** FMIT Clinic Day Constraint
...
class FMITResidentClinicDayConstraint(HardConstraint):
    ...
    FMIT_CLINIC_DAYS = {
        1: {"weekday": 2, "time_of_day": "AM"},  ***REMOVED*** Wed AM for PGY-1
        2: {"weekday": 1, "time_of_day": "PM"},  ***REMOVED*** Tue PM for PGY-2
        3: {"weekday": 0, "time_of_day": "PM"},  ***REMOVED*** Mon PM for PGY-3
    }
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        """
        ***REMOVED*** Implementation: if resident is on FMIT and date matches their clinic day,
        ***REMOVED*** force clinic template assignment instead of FMIT template
        pass
```

---

***REMOVED******REMOVED******REMOVED*** Design 2: Call Solver Integration
...
***REMOVED******REMOVED******REMOVED******REMOVED*** New Constraint 1: `PostFMITSundayBlockingConstraint`
...
class PostFMITSundayBlockingConstraint(HardConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
            ...
            for friday_start, thursday_end in fmit_weeks:
                ***REMOVED*** Sunday after FMIT = Thursday + 3 days
    ...
    def validate(self, assignments, context):
        ...
        ***REMOVED*** Build lookup of blocked Sundays
        blocked_sundays = {}
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
        ...
        ***REMOVED*** Check call assignments
...
***REMOVED******REMOVED******REMOVED******REMOVED*** New Constraint 2: `CallSpacingConstraint`
...
class CallSpacingConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** Group blocks by week number (ISO week)
        ...
        for faculty in context.faculty:
            ...
            ***REMOVED*** For each pair of adjacent weeks
            for i in range(len(sorted_week_nums) - 1):
                week_n = sorted_week_nums[i]
                week_n1 = sorted_week_nums[i + 1]

                ***REMOVED*** Check if weeks are actually adjacent
                if week_n1 - week_n != 1:
                    continue

                ***REMOVED*** Get call vars for this faculty in week N
                week_n_vars = []
                ...
                ***REMOVED*** Get call vars for this faculty in week N+1
                week_n1_vars = []
    ...
    def validate(self, assignments, context):
        ...
        ***REMOVED*** Group call assignments by faculty and week
        ...
        ***REMOVED*** Count back-to-back occurrences
        violations = []
        total_penalty = 0.0
        ...
        for faculty_id, weeks in call_weeks_by_faculty.items():
            ...
            for i in range(len(sorted_weeks) - 1):
                if sorted_weeks[i + 1] - sorted_weeks[i] == 1:
                    ***REMOVED*** Back-to-back call weeks
...
***REMOVED******REMOVED******REMOVED*** Design 3: Constraint File Organization

***REMOVED******REMOVED******REMOVED******REMOVED*** Current Structure
```
backend/app/scheduling/constraints/
├── __init__.py
├── base.py                  ***REMOVED*** Base classes
├── manager.py               ***REMOVED*** Constraint registration
├── capacity.py              ***REMOVED*** OnePersonPerBlock, ClinicCapacity, Coverage
├── call_equity.py           ***REMOVED*** Sunday/Weekday equity, preferences
├── fmit.py                  ***REMOVED*** Faculty FMIT constraints
├── night_float_post_call.py ***REMOVED*** NF → PC transition
└── (future files)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Recommended New Files
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Updated `__init__.py`
...
from .call_equity import (
    ...
    CallSpacingConstraint,  ***REMOVED*** NEW
)
...
from .fmit import (
    ...
    PostFMITSundayBlockingConstraint,  ***REMOVED*** NEW
...
from .inpatient import (  ***REMOVED*** NEW FILE
...
***REMOVED******REMOVED******REMOVED*** Implementation Priority (for Claude Code IDE)
...
***REMOVED******REMOVED******REMOVED*** Verification Checklist (for Claude Code IDE)
...
***REMOVED******REMOVED******REMOVED*** Faculty Call Scheduling Context (for Claude Web)
...
├── SundayCallEquityConstraint     ***REMOVED*** Minimize max Sunday calls
├── WeekdayCallEquityConstraint    ***REMOVED*** Minimize max Mon-Thu calls
├── TuesdayCallPreferenceConstraint ***REMOVED*** PD/APD penalty for Tuesday
└── DeptChiefWednesdayPreferenceConstraint ***REMOVED*** Dept Chief bonus for Wednesday
```
...
***REMOVED******REMOVED******REMOVED*** Block 10 Faculty FMIT Schedule (Sanitized)
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 4: UI/UX Testing (CLAUDE CODE IDE)
...
***REMOVED******REMOVED******REMOVED*** Checkpoint 5: Production Readiness (HUMAN + CLAUDE)
...
***REMOVED******REMOVED*** Work Distribution by Agent

***REMOVED******REMOVED******REMOVED*** Claude Code (IDE) - Full Access Tasks

These tasks involve PII or database operations:

| ***REMOVED*** | Task | Effort | Notes |
|---|------|--------|-------|
...
***REMOVED******REMOVED******REMOVED*** Claude (Web) - Sanitized Data Tasks

These tasks require only anonymized data:

| ***REMOVED*** | Task | Effort | Input Required |
|---|------|--------|----------------|
...
***REMOVED******REMOVED******REMOVED*** Claude (Web) - No Data Required
...
***REMOVED******REMOVED*** Sanitized Data Requirements for Analysis

To enable Claude Code analysis without PII exposure, please provide:

***REMOVED******REMOVED******REMOVED*** 1. Schedule Metrics Export (REQUIRED)
...
***REMOVED******REMOVED******REMOVED*** 2. Workload Distribution (REQUIRED)
...
***REMOVED******REMOVED******REMOVED*** 3. Constraint Satisfaction Report (REQUIRED)
...
***REMOVED******REMOVED******REMOVED*** 4. Solver Performance (OPTIONAL but helpful)
...
***REMOVED******REMOVED******REMOVED*** 5. Anonymized Timeline (OPTIONAL)
...
***REMOVED******REMOVED*** Priority Matrix: Now vs Later

***REMOVED******REMOVED******REMOVED*** Do Now (This Session)
...
***REMOVED******REMOVED******REMOVED*** Do Next (After Checkpoint 2)
...
***REMOVED******REMOVED******REMOVED*** Backlog (v1.1.0 Scope)
...
***REMOVED******REMOVED******REMOVED*** Communication Channels
...
***REMOVED******REMOVED******REMOVED*** Parallel Work Pattern
...
***REMOVED******REMOVED******REMOVED*** Backup Commands

```bash
***REMOVED*** Before any schedule generation
./scripts/backup-db.sh --docker

***REMOVED*** Verify backup exists
ls -la backups/postgres/*.sql.gz | tail -1

***REMOVED*** Restore if needed
...
| ***REMOVED*** | Blocker | Owner | Resolution | Status |
|---|---------|-------|------------|--------|
...
***REMOVED******REMOVED******REMOVED*** Claude Code (IDE) - Full Access Commands
```bash
***REMOVED*** Seed database with Block 10 data
cd backend && python scripts/seed_data.py

***REMOVED*** Generate schedule
docker compose exec backend python -m app.scheduling.engine --block 10

***REMOVED*** Run full test suite with real DB
cd backend && pytest

***REMOVED*** Export sanitized metrics for Claude Web
python scripts/export_sanitized_metrics.py --block 10 --include-violations -o /tmp/block10_metrics.json

***REMOVED*** Test with real data
cd frontend && npm run dev  ***REMOVED*** View actual schedule
```

***REMOVED******REMOVED******REMOVED*** Claude (Web) - Sanitized Data Analysis
```
***REMOVED*** Provide these files to Claude Web for analysis:
- block10_metrics.json (coverage, workload, violations)
- Anonymized timeline CSV (if needed)
- Solver benchmark results

***REMOVED*** Claude Web can review code directly (no PII in source):
- backend/app/scheduling/*.py
- backend/app/resilience/*.py
- mcp-server/src/*.py
```

***REMOVED******REMOVED******REMOVED*** Human - Approval & Strategy
```bash
***REMOVED*** Review schedule visually
open http://localhost:3000/schedule

***REMOVED*** Approve production export
***REMOVED*** Review CHANGELOG updates
