***REMOVED*** Diagnostic Report: Faculty Outpatient Generation Issues

**Purpose:** Comprehensive diagnosis for handoff to Claude Code Web for parallel implementation

---

***REMOVED******REMOVED*** Key Documentation References
...
***REMOVED******REMOVED*** Issues Identified

***REMOVED******REMOVED******REMOVED*** Issue 1: Supervision Calculation Overcounting (CRITICAL)
...
required = ceil(pgy1_count / 2) + ceil(other_count / 4)  ***REMOVED*** <-- BUG HERE
```
...
required = (pgy1_count + 1) // 2 + (other_count + 3) // 4  ***REMOVED*** <-- BUG HERE
```

**Location 3:** `backend/app/scheduling/constraints/acgme.py` lines 555-560
```python
***REMOVED*** SupervisionRatioConstraint.calculate_required_faculty()
def calculate_required_faculty(self, pgy1_count: int, other_count: int) -> int:
    ...
    ***REMOVED*** ^-- BUG: Separate ceiling operations
```

**Correct Formula:**
```python
***REMOVED*** Fractional load approach:
supervision_load = (pgy1_count * 0.5) + (other_count * 0.25)
required = ceil(supervision_load) if supervision_load > 0 else 0

***REMOVED*** OR integer math equivalent:
supervision_units = (pgy1_count * 2) + other_count  ***REMOVED*** intern=2, resident=1
required = (supervision_units + 3) // 4 if supervision_units > 0 else 0  ***REMOVED*** 4 units per faculty
```

---

***REMOVED******REMOVED******REMOVED*** Issue 2: Adjunct Faculty Category
...
def weekly_clinic_limit(self) -> int:
    limits = {
        ...
        FacultyRole.CORE: 4,
        ***REMOVED*** ADD: FacultyRole.ADJUNCT: 0,
    }
```
...
def _get_available_faculty(self, block_start, block_end):
    ...
    ***REMOVED*** ADD: Exclude adjunct faculty
...
***REMOVED******REMOVED******REMOVED*** Issue 3: PCAT Not Being Generated (Lower Priority)
...
***REMOVED******REMOVED*** Handoff Summary for Claude Code Web

***REMOVED******REMOVED******REMOVED*** Task 1: Fix Supervision Calculation (Backend Only)
...
```python
***REMOVED*** FROM: required = ceil(pgy1_count / 2) + ceil(other_count / 4)
***REMOVED*** TO:   supervision_load = (pgy1_count * 0.5) + (other_count * 0.25)
***REMOVED***       required = ceil(supervision_load) if supervision_load > 0 else 0
```
...
***REMOVED******REMOVED******REMOVED*** Task 2: Add Adjunct Faculty Role (Backend + Frontend)
...
***REMOVED******REMOVED******REMOVED*** Task 3: PCAT Investigation (Deferred)
