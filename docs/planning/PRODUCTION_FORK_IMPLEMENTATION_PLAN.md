***REMOVED*** Production Fork Implementation Plan
...
1. [New Data Models](***REMOVED***1-new-data-models)
2. [Screener Constraint System](***REMOVED***2-screener-constraint-system)
3. [FMIT Crisis Mode (OB Turf)](***REMOVED***3-fmit-crisis-mode-ob-turf)
4. [Intern Stagger Patterns](***REMOVED***4-intern-stagger-patterns)
5. [Overnight Call Reasonableness](***REMOVED***5-overnight-call-reasonableness)
6. [Simulation Scenarios](***REMOVED***6-simulation-scenarios)
7. [Fork Strategy](***REMOVED***7-fork-strategy)
8. [Parallel Implementation Tracks](***REMOVED***8-parallel-implementation-tracks)
9. [Sequential Dependencies](***REMOVED***9-sequential-dependencies)

---

***REMOVED******REMOVED*** 1. New Data Models

***REMOVED******REMOVED******REMOVED*** 1.1 Screener Role Type
...
class ScreenerRole(str, Enum):
    ...
    """
    DEDICATED = "dedicated"     ***REMOVED*** Full-time clinic screener (MA, LVN)
    RN = "rn"                   ***REMOVED*** RN who can screen when needed
    RESIDENT = "resident"       ***REMOVED*** PGY-1 can screen in staggered model

class Person(Base):
    ***REMOVED*** ... existing fields ...

    ***REMOVED*** Screener-specific fields
    screener_role = Column(String(50))  ***REMOVED*** ScreenerRole enum
    can_screen = Column(Boolean, default=False)  ***REMOVED*** Quick check for RN fallback
    screening_efficiency = Column(Float, default=1.0)  ***REMOVED*** 1.0 = average, 1.5 = fast
```

***REMOVED******REMOVED******REMOVED*** 1.2 Clinic Session Model
...
class ClinicSession(Base):
    ...
    session_type = Column(String(20))  ***REMOVED*** 'AM' or 'PM'
    clinic_type = Column(String(50))   ***REMOVED*** 'FM', 'peds', 'OB', 'sports_med'
    ...
    ***REMOVED*** Calculated metrics
    screener_ratio = Column(Float)  ***REMOVED*** screeners per physician

    ***REMOVED*** Status
    staffing_status = Column(String(20))  ***REMOVED*** 'optimal', 'adequate', 'suboptimal', 'critical'
    ...
    def ratio_quality(self) -> str:
        ...
        if self.screener_ratio >= 2.0:
            return "optimal"      ***REMOVED*** 2-3 screeners per physician
        elif self.screener_ratio >= 1.0:
            return "adequate"     ***REMOVED*** 1:1
        elif self.screener_ratio >= 0.5:
            return "suboptimal"   ***REMOVED*** 2 physicians sharing 1 screener
        else:
            return "critical"     ***REMOVED*** No screener coverage
```

***REMOVED******REMOVED******REMOVED*** 1.3 Intern Stagger Model
...
class InternStaggerPattern(Base):
    ...
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    name = Column(String(100))  ***REMOVED*** "Standard 90min Stagger"

    ***REMOVED*** Timing
    intern_a_start = Column(Time)  ***REMOVED*** 07:30
    intern_b_start = Column(Time)  ***REMOVED*** 09:00
    overlap_duration_minutes = Column(Integer)  ***REMOVED*** 150 min overlap

    ***REMOVED*** Screener efficiency during overlap (typically 0.6-0.8)
    overlap_efficiency = Column(Float, default=0.7)

    ***REMOVED*** Which intern pairs this pattern works for
    min_intern_a_experience_weeks = Column(Integer, default=4)  ***REMOVED*** A needs some ramp-up
```

---

***REMOVED******REMOVED*** 2. Screener Constraint System

***REMOVED******REMOVED******REMOVED*** 2.1 Screener Ratio Constraint
...
class ScreenerRatioConstraint(SoftConstraint):
    ...
    weight = 15.0  ***REMOVED*** High weight - screener ratio affects patient throughput
...
class RNScreenerFallbackConstraint(SoftConstraint):
    ...
    FALLBACK_PENALTY = 3  ***REMOVED*** Small penalty per RN-as-screener session
    weight = 5.0
...
***REMOVED******REMOVED******REMOVED*** 2.2 Screener Assignment Service
...
***REMOVED******REMOVED*** 3. FMIT Crisis Mode (OB Turf)

***REMOVED******REMOVED******REMOVED*** 3.1 FMIT Continuity Turf Rules
    ...
    def evaluate(self, context: SchedulingContext) -> ConstraintResult:
        ...
        for delivery in context.continuity_deliveries:
            ...
            if defense_level in ("GREEN", "YELLOW"):
                ...
                ***REMOVED*** Degraded: FM preferred, OB acceptable
                if not self._is_fm_available(fm_faculty, delivery.date, context):
                    ***REMOVED*** Log but don't fail - OB will cover
                    violations.append(ConstraintViolation(
                        severity="warning",
            ...
            else:  ***REMOVED*** RED or BLACK
                ***REMOVED*** Crisis: All continuity turfed to OB
                violations.append(ConstraintViolation(
                    severity="info",
                    message=f"Crisis mode: All continuity turfed to OB "
                            f"(defense={defense_level})",
                ))
                break  ***REMOVED*** One message covers all
...
class FMITStaffingFloorConstraint(HardConstraint):
    ...
    FMIT_UTILIZATION_CAP = 0.20  ***REMOVED*** Max 20% on FMIT simultaneously
    ...
    def evaluate(self, context: SchedulingContext) -> ConstraintResult:
        ...
        if total_faculty < self.MINIMUM_FACULTY_FOR_FMIT:
            ***REMOVED*** Block ALL FMIT assignments
            ...
            ***REMOVED*** Check utilization cap
            max_concurrent_fmit = int(total_faculty * self.FMIT_UTILIZATION_CAP)
            max_concurrent_fmit = max(1, max_concurrent_fmit)  ***REMOVED*** At least 1 allowed
...
***REMOVED******REMOVED*** 4. Intern Stagger Patterns

***REMOVED******REMOVED******REMOVED*** 4.1 Stagger Pattern Service
...
class InternStaggerService:
    ...
    OVERLAP_EFFICIENCY = 0.7  ***REMOVED*** Each intern gets 70% screener time during overlap
    ...
    def calculate_effective_screener_coverage(
        ...
        """
        ***REMOVED*** Non-overlap periods: full efficiency
        non_overlap_hours = 1.5  ***REMOVED*** 0730-0900 for A only

        ***REMOVED*** Overlap period: reduced efficiency per intern
        overlap_hours = 2.5  ***REMOVED*** 0900-1130 shared

        effective_coverage = (
            (non_overlap_hours * 1.0) +  ***REMOVED*** Full for A
            (overlap_hours * self.OVERLAP_EFFICIENCY * 2)  ***REMOVED*** Split for both
        )

        return effective_coverage * screener_count
```

***REMOVED******REMOVED******REMOVED*** 4.2 Stagger-Aware Screener Constraint
...
class StaggeredInternScreenerConstraint(SoftConstraint):
    ...
    STAGGER_BONUS = 10  ***REMOVED*** Bonus for using stagger pattern
    weight = 8.0
    ...
    def evaluate(self, context: SchedulingContext) -> ConstraintResult:
        ...
        for session in context.clinic_sessions:
            if session.has_intern_stagger:
                ***REMOVED*** Check if stagger was necessary (low screener count)
...
***REMOVED******REMOVED*** 5. Overnight Call Reasonableness

***REMOVED******REMOVED******REMOVED*** 5.1 Call Reasonableness Constraint
...
class CallReasonablenessConstraint(SoftConstraint):
    ...
    ***REMOVED*** Days between calls for sustainability (by faculty count)
    SUSTAINABILITY_THRESHOLDS = {
        10: 7,   ***REMOVED*** 10+ faculty: q7 sustainable
        8: 6,    ***REMOVED*** 8-9 faculty: q6 sustainable
        6: 5,    ***REMOVED*** 6-7 faculty: q5 sustainable
        4: 4,    ***REMOVED*** 4-5 faculty: q4 minimum
        2: 3,    ***REMOVED*** 2-3 faculty: q3 CRITICAL (not sustainable long-term)
    }
    ...
    def evaluate(self, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Find sustainability threshold
        sustainable_interval = 7  ***REMOVED*** Default
        ...
        ***REMOVED*** Calculate actual average interval per faculty
        ...
        ***REMOVED*** Check for critical staffing
        if faculty_count < 4:
            violations.append(ConstraintViolation(
                severity="error",
            ...
            total_penalty += 100  ***REMOVED*** Heavy penalty
...
***REMOVED******REMOVED*** 6. Simulation Scenarios

***REMOVED******REMOVED******REMOVED*** 6.1 PCS Season Scenario
...
class PCSSeasonConfig:
    ...
    ***REMOVED*** Starting conditions
    initial_faculty: int = 4  ***REMOVED*** Your PCS arrival reality
    expected_faculty: int = 10  ***REMOVED*** Normal staffing level

    ***REMOVED*** Timing
    simulation_days: int = 365
    pcs_season_start_day: int = 150  ***REMOVED*** ~June 1
    pcs_season_end_day: int = 240    ***REMOVED*** ~August 31

    ***REMOVED*** Departure/arrival patterns
<<<<<<< HEAD
    departures_during_pcs: int = 3   ***REMOVED*** Faculty leaving
    arrivals_during_pcs: int = 5     ***REMOVED*** Faculty arriving (eventually)
=======
    departures_during_pcs: int = 3   ***REMOVED*** leaving
    arrivals_during_pcs: int = 5     ***REMOVED*** arriving (eventually)
>>>>>>> origin/docs/session-14-summary
    arrival_lag_days: int = 45       ***REMOVED*** Average gap between departure and replacement
    onboarding_days: int = 21        ***REMOVED*** New faculty at 50% productivity

    ***REMOVED*** Screener dynamics
    initial_screeners: int = 2
    screener_turnover_rate: float = 0.4  ***REMOVED*** 40% annual (higher than faculty)

    ***REMOVED*** Workload
    total_clinic_sessions_per_week: int = 40  ***REMOVED*** Full schedule demand
...
class PCSSeasonScenario:
    ...
    def run(self) -> PCSSeasonResult:
        ...
        ***REMOVED*** Convert to cascade config for burnout modeling
        ...
        ***REMOVED*** Run burnout cascade
        cascade_result = BurnoutCascadeScenario(cascade_config).run()

        ***REMOVED*** Overlay screener dynamics
...
class PCSSeasonResult:
    ...
    peak_call_frequency: float  ***REMOVED*** Worst-case days between calls
    screener_ratio_nadir: float  ***REMOVED*** Lowest screener:physician ratio
    fmit_gaps: int  ***REMOVED*** Number of weeks FMIT couldn't be staffed
    defense_escalations: list[tuple[int, str]]  ***REMOVED*** (day, level)
    recommendations: list[str]
```

***REMOVED******REMOVED******REMOVED*** 6.2 Monte Carlo Test Suite
...
class MonteCarloConfig:
    ...
    ***REMOVED*** Parameter ranges to test
...
class MonteCarloRunner:
    ...
    def find_critical_thresholds(self) -> dict:
        ...
        ***REMOVED*** Find minimum faculty for 95% survival
        min_faculty = None
    ...
    def _generate_threshold_recommendations(self, survival: dict) -> list[str]:
        ...
        ***REMOVED*** Find 50%, 80%, 95% survival thresholds
...
***REMOVED******REMOVED*** 7. Fork Strategy

***REMOVED******REMOVED******REMOVED*** 7.1 Repository Structure

```
your-org/residency-scheduler-[site-name]/    ***REMOVED*** Private fork
├── .github/
│   └── workflows/
│       └── deploy.yml                        ***REMOVED*** Site-specific CI/CD
├── config/
│   └── site_config.py                        ***REMOVED*** Site-specific parameters
├── data/
│   └── faculty_roster.json                   ***REMOVED*** Real faculty data (gitignored)
├── docs/
│   └── SITE_SPECIFIC.md                      ***REMOVED*** Your program's documentation
└── (all other files from upstream)
```

***REMOVED******REMOVED******REMOVED*** 7.2 Site Configuration
...
CURRENT_FACULTY_COUNT = 4  ***REMOVED*** PCS season reality
EXPECTED_SCREENER_COUNT = 6
CURRENT_SCREENER_COUNT = 2

***REMOVED*** Screener ratios
OPTIMAL_SCREENER_RATIO = 2.5  ***REMOVED*** Your efficient faculty can use 2-3
...
MINIMUM_FACULTY_FOR_FMIT = 5
OB_TURF_DEFENSE_LEVEL = "ORANGE"  ***REMOVED*** Turf continuity at this level

***REMOVED*** Intern stagger
ENABLE_INTERN_STAGGER = True
STAGGER_MINUTES = 90
MIN_INTERN_EXPERIENCE_WEEKS = 4

***REMOVED*** Call
SUNDAY_CALL_EQUITY_WEIGHT = 2.0  ***REMOVED*** Sunday counts double for equity
...
***REMOVED******REMOVED******REMOVED*** Track B: Screener Constraints
**Files to create:**
...
***REMOVED******REMOVED******REMOVED*** Track C: FMIT Crisis Mode
**Files to modify:**
...
***REMOVED******REMOVED******REMOVED*** Track D: Intern Stagger System
**Files to create:**
...
***REMOVED******REMOVED******REMOVED*** Track E: Simulation Scenarios
**Files to create:**
...
***REMOVED******REMOVED******REMOVED*** Track F: Site Fork Setup
**Actions:**
- Create private repo
- Set up CI/CD
...
***REMOVED******REMOVED*** 9. Sequential Dependencies
...
***REMOVED******REMOVED*** 10. Implementation Priority Matrix
...
***REMOVED******REMOVED*** 11. Immediate Next Steps

***REMOVED******REMOVED******REMOVED*** Session 1 (Now): Foundation
1. Create data models (Track A)
2. Run migration
3. Set up fork (Track F)

***REMOVED******REMOVED******REMOVED*** Session 2: Core Constraints
1. Screener constraints (Track B)
2. FMIT crisis mode (Track C)

***REMOVED******REMOVED******REMOVED*** Session 3: Simulation
1. PCS scenario (Track E)
2. Run with your parameters (4 faculty, 2 screeners)
3. Generate survival curves

***REMOVED******REMOVED******REMOVED*** Session 4: Optimization
...
***REMOVED******REMOVED*** 12. Documentation Backlog (For Future Sessions)
