***REMOVED*** MCP Implementation Plan: Local IDE Integration
...
***REMOVED******REMOVED******REMOVED*** 4 Work Streams
...
***REMOVED******REMOVED*** Stream 1: PII Sanitization
...
***REMOVED******REMOVED******REMOVED*** Specific Changes

```python
***REMOVED*** tools.py:35 - BEFORE
class ValidationIssue(BaseModel):
    person_id: str | None = None
    person_name: str | None = None  ***REMOVED*** REMOVE

***REMOVED*** tools.py:35 - AFTER
class ValidationIssue(BaseModel):
    person_id: str | None = None
    role: str | None = None  ***REMOVED*** Add role for context without PII
```

```python
***REMOVED*** tools.py:252 - BEFORE
ValidationIssue(
    person_id="resident-001",
    person_name="Dr. Williams",  ***REMOVED*** REMOVE
    ...
)

***REMOVED*** tools.py:252 - AFTER
ValidationIssue(
    person_id="RES-001",
    role="PGY-2",  ***REMOVED*** Add role for context
    ...
)
```
...
```bash
***REMOVED*** Must return 0 after changes
...
***REMOVED******REMOVED*** Stream 2: API Client

***REMOVED******REMOVED******REMOVED*** New File: `mcp-server/src/scheduler_mcp/api_client.py`
...
***REMOVED*** Singleton instance for tool use
...
***REMOVED******REMOVED******REMOVED*** Tool Integration Example

```python
***REMOVED*** tools.py - Update validate_schedule to use API
...
async def validate_schedule(
    ...
    ***REMOVED*** Get API client
    client = await get_api_client()

    ***REMOVED*** Check if API is available
    ...
    ***REMOVED*** Call API (returns sanitized data - no PII)
    ...
    ***REMOVED*** Transform to MCP schema
    return ScheduleValidationResult(
        is_valid=result["is_valid"],
        overall_compliance_rate=result["compliance_rate"],
        total_issues=len(result["issues"]),
        ***REMOVED*** ... map remaining fields
    )
```

---

***REMOVED******REMOVED*** Stream 3: Context Layer

***REMOVED******REMOVED******REMOVED*** New File: `mcp-server/src/scheduler_mcp/domain_context.py`
...
***REMOVED*** Import from the glossary we created
ABBREVIATIONS = {
...
def expand_abbreviations(text: str) -> str:
    ...
    for abbrev, full in ABBREVIATIONS.items():
        ***REMOVED*** Only expand if it's a standalone word
        import re
...
def enrich_violation_response(violation: dict) -> dict:
    ...
    ***REMOVED*** Expand abbreviations in message
...
***REMOVED******REMOVED*** Stream 4: IDE Connection (IMPLEMENTED 2025-12-25)

***REMOVED******REMOVED******REMOVED*** Docker Container Approach
...
***REMOVED******REMOVED******REMOVED*** Docker Compose Service (Implemented)
...
***REMOVED******REMOVED******REMOVED*** Claude Code Integration Options
...
***REMOVED******REMOVED******REMOVED*** Starting the MCP Server

```bash
***REMOVED*** Start all services including MCP server
docker-compose up -d

***REMOVED*** Development mode with hot reload
...
***REMOVED******REMOVED*** Merge Point: Integration Test

***REMOVED******REMOVED******REMOVED*** Test Script: `scripts/test-mcp-integration.sh`

```bash
***REMOVED***!/bin/bash
***REMOVED*** Integration test for MCP + FastAPI + Claude Code

set -e

echo "=== MCP Integration Test ==="

***REMOVED*** 1. Start services
echo "1. Starting services..."
docker-compose up -d backend db
sleep 10

***REMOVED*** 2. Verify FastAPI
echo "2. Checking FastAPI health..."
curl -f http://localhost:8000/health || exit 1

***REMOVED*** 3. Test MCP server starts
echo "3. Testing MCP server startup..."
cd mcp-server/src
timeout 5 python -c "from scheduler_mcp.server import mcp; print('MCP imports OK')" || exit 1

***REMOVED*** 4. Test API client
...
***REMOVED*** 5. Test no PII in responses
echo "5. Verifying no PII in MCP code..."
if grep -r "person_name" src/scheduler_mcp/*.py | grep -v "***REMOVED*** REMOVED"; then
...
***REMOVED******REMOVED******REMOVED*** Parallel Phase (Can run simultaneously)
...
***REMOVED******REMOVED******REMOVED*** Merge Phase (Sequential)
...
***REMOVED******REMOVED******REMOVED*** Total Estimated Time
...
***REMOVED******REMOVED******REMOVED*** Functional
...
```bash
***REMOVED*** Revert to previous state
git checkout HEAD~1 -- mcp-server/
git checkout HEAD~1 -- .claude/settings.json

***REMOVED*** Or reset entirely
git reset --hard HEAD~1
```

---

***REMOVED******REMOVED*** Post-Implementation
