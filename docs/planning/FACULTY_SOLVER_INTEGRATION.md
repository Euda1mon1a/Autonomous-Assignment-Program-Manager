***REMOVED*** Faculty Solver Integration Gap
...
***REMOVED******REMOVED******REMOVED*** Solver Architecture (`backend/app/scheduling/solvers.py`)
```python
***REMOVED*** Line ~176-223: Only resident variables are created
...
***REMOVED******REMOVED******REMOVED*** Wednesday Constraints Impact
```python
***REMOVED*** temporal.py add_to_cpsat - this does nothing because:
for f in context.faculty:
    f_i = context.resident_idx.get(f.id)  ***REMOVED*** Always None!
    if f_i is not None and (f_i, b_i, t_i) in template_vars:
        faculty_clinic_vars.append(...)  ***REMOVED*** Never executed
```
...
***REMOVED******REMOVED******REMOVED*** Option 1: Model Faculty in Solver (Recommended)
...
```python
***REMOVED*** In solvers.py, after resident variables
faculty_vars = {}
...
```python
***REMOVED*** In base.py SchedulingContext
faculty_idx: dict[uuid.UUID, int] = field(default_factory=dict)

***REMOVED*** Populate during context creation
...
***REMOVED*** Use faculty_vars instead of template_vars:
...
***REMOVED******REMOVED*** Scope Assessment
...
***REMOVED******REMOVED*** Validation After Fix
...
***REMOVED******REMOVED*** Current Workaround
