***REMOVED*** Slack Integration Architecture for Natural Language Coding Requests
...
2. [Architecture Overview](***REMOVED***1-architecture-overview)
3. [Slack App Configuration](***REMOVED***2-slack-app-configuration)
4. [Message Flow Patterns](***REMOVED***3-message-flow-patterns)
5. [Integration with GitHub Actions](***REMOVED***4-integration-with-github-actions)
6. [Security Considerations](***REMOVED***5-security-considerations)
7. [Implementation Guide](***REMOVED***6-implementation-guide)
8. [Performance & Scalability](***REMOVED***7-performance--scalability)
9. [Monitoring & Observability](***REMOVED***8-monitoring--observability)
10. [Implementation Checklist](***REMOVED***implementation-checklist)
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  ✅ Created issue ***REMOVED***123: Implement user authentication    │  │
│  │  🔄 PR ***REMOVED***45 opened by claude-bot                          │  │
│  │  ✅ CI checks passed                                      │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
***REMOVED******REMOVED******REMOVED*** 1.1 System Components

***REMOVED******REMOVED******REMOVED******REMOVED*** A. Slack Bot Layer
...
***REMOVED******REMOVED******REMOVED******REMOVED*** B. Intent Parser (Claude AI)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** C. GitHub Integration Service
...
class GitHubTaskRequest(BaseModel):
    ...
    task_type: str  ***REMOVED*** "issue" or "pull_request"
    title: str
    body: str
    labels: List[str]
...
class GitHubIntegrationService:
    ...
    async def create_issue(
        ...
        """
        ***REMOVED*** Add Slack context to issue body
...
"""
        ...
        try:
            ...
            ***REMOVED*** Add priority label
            issue.add_to_labels(f"priority:{task.priority}")

            ***REMOVED*** Trigger workflow if auto-implementation requested
...
***REMOVED******REMOVED******REMOVED******REMOVED*** D. Background Task Queue (Celery)
...
def process_slack_request(
    ...
    try:
        ...
        ***REMOVED*** 2. Create GitHub resource
        ...
        ***REMOVED*** 3. Post success message
        slack.post_message(
            channel_id,
            f"{emoji} Created {resource} ***REMOVED***{result['number']}: {result['url']}",
            thread_ts=thread_ts
        )

        ***REMOVED*** 4. Watch for GitHub webhook updates
...
def handle_github_webhook(event_type: str, payload: Dict):
    ...
    ***REMOVED*** Get Slack thread from issue/PR metadata
    thread_info = extract_slack_thread(payload)
    if not thread_info:
        return

    ***REMOVED*** Format message based on event type
...
***REMOVED******REMOVED******REMOVED*** 1.2 Data Flow Diagram
...
│  Bot:    "📋 Created issue ***REMOVED***123"        │
│  Bot:    "🔄 PR ***REMOVED***45 opened"             │
...
***REMOVED******REMOVED*** 2. Slack App Configuration

***REMOVED******REMOVED******REMOVED*** 2.1 OAuth Scopes

***REMOVED******REMOVED******REMOVED******REMOVED*** Bot Token Scopes

Required for bot functionality:

```
channels:history       ***REMOVED*** Read channel messages
channels:read          ***REMOVED*** List public channels
chat:write             ***REMOVED*** Post messages as bot
chat:write.public      ***REMOVED*** Post to channels bot isn't in
commands               ***REMOVED*** Register slash commands
im:history             ***REMOVED*** Read direct messages
im:read                ***REMOVED*** List DMs
im:write               ***REMOVED*** Send DMs
reactions:read         ***REMOVED*** Read message reactions
reactions:write        ***REMOVED*** Add reactions to messages
users:read             ***REMOVED*** Get user info
app_mentions:read      ***REMOVED*** Receive @mentions
```

***REMOVED******REMOVED******REMOVED******REMOVED*** User Token Scopes (Optional)

For user-impersonation features:

```
users:read             ***REMOVED*** Get user profile info
search:read            ***REMOVED*** Search messages
```

***REMOVED******REMOVED******REMOVED*** 2.2 Event Subscriptions
...
| `message.channels` | Message in subscribed channels | Monitor ***REMOVED***engineering for keywords |
...
***REMOVED******REMOVED******REMOVED*** 2.3 Slash Commands

***REMOVED******REMOVED******REMOVED******REMOVED*** `/create-issue` - Quick Issue Creation
...
async def handle_create_issue(request: Request):
    ...
    ***REMOVED*** Immediate response (Slack requires response within 3 sec)
    return {
        "response_type": "ephemeral",
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `/start-pr` - Create PR from Branch
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `/workflow-status` - Check GitHub Actions Status
...
**Example:**
```
/workflow-status          ***REMOVED*** Latest workflow
/workflow-status 123456   ***REMOVED*** Specific run
```

***REMOVED******REMOVED******REMOVED*** 2.4 Interactive Components

***REMOVED******REMOVED******REMOVED******REMOVED*** Approval Button (Modal)
...
***REMOVED******REMOVED*** 3. Message Flow Patterns

***REMOVED******REMOVED******REMOVED*** 3.1 Pattern 1: Simple Issue Creation

**User Intent:** "Create a bug report"

```
┌─────────────────────────────────────────────────────────┐
│ Slack Channel: ***REMOVED***engineering                             │
├─────────────────────────────────────────────────────────┤
│                                                         │
...
│         (3 seconds)                                     │
│         ↓                                               │
│ @bot:   📋 Created bug issue ***REMOVED***456                       │
...
async def handle_simple_issue_request(message: str, context: Dict):
    ...
    ***REMOVED*** Return formatted response
    return {
        "blocks": [
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": f"📋 Created {analysis['type']} issue ***REMOVED***{issue.number}\n{issue.url}"
                }
            },
            {
                "type": "section",
...
***REMOVED******REMOVED******REMOVED*** 3.2 Pattern 2: Complex Feature Request

**User Intent:** "Implement a feature with multiple components"

```
┌─────────────────────────────────────────────────────────┐
│ Slack Channel: ***REMOVED***product-requests                       │
├─────────────────────────────────────────────────────────┤
│                                                         │
...
***REMOVED******REMOVED******REMOVED*** 3.3 Pattern 3: Auto-Implementation with Progress Updates
...
│         • Assigned issue ***REMOVED***123 to claude-bot             │
...
│         ↓                                               │
│ @bot:   🔀 Pull Request created: ***REMOVED***789                   │
...
async def run_autonomous_implementation(issue_number: int):
    ...
    ***REMOVED*** Execute steps
    for i, step in enumerate(progress.steps):
        ...
        ***REMOVED*** Execute actual implementation
    ...
    ***REMOVED*** Create PR
    ...
    await progress._post_update(
        f"🔀 Pull Request created: ***REMOVED***{pr.number}\n{pr.url}"
    )
```

***REMOVED******REMOVED******REMOVED*** 3.4 Pattern 4: Error Handling & Escalation
...
***REMOVED******REMOVED*** 4. Integration with GitHub Actions

***REMOVED******REMOVED******REMOVED*** 4.1 Workflow: Autonomous Issue Resolver
  ...
  implement-changes:
    ...
    steps:
      ...
      - name: Execute implementation steps
        ...
        run: |
          ***REMOVED*** Run autonomous implementation script
      ...
      - name: Commit changes
        run: |
          ...
          git add .
          git commit -m "Auto-implement: ${{ steps.issue.outputs.title }} (***REMOVED***${{ inputs.issue_number }})"
        ...
        with:
          script: |
            const pr = await github.rest.pulls.create({
              owner: context.repo.owner,
              repo: context.repo.repo,
              title: `Auto-implement: Issue ***REMOVED***${{ inputs.issue_number }}`,
              head: process.env.BRANCH_NAME,
              base: 'main',
              body: `Resolves ***REMOVED***${{ inputs.issue_number }}\n\n**Autonomous Implementation**\n\nThis PR was automatically generated by the autonomous issue resolver.\n\nPlease review the changes carefully.`
            });
      ...
      - name: Post PR to Slack
        ...
        with:
          ...
          payload: |
            {
              "text": "🔀 Pull Request created: ***REMOVED***${{ steps.create-pr.outputs.pr_number }}",
              "blocks": [
                {
                  "type": "section",
                  "text": {
                    "type": "mrkdwn",
                    "text": "🔀 **Pull Request Created**\n<${{ steps.create-pr.outputs.pr_url }}|PR ***REMOVED***${{ steps.create-pr.outputs.pr_number }}>\n\nReady for review!"
                  }
                }
              ]
            }
...
***REMOVED******REMOVED******REMOVED*** 4.2 Workflow: GitHub Webhook → Slack Updates
...
async def handle_github_webhook(request: Request):
    ...
    ***REMOVED*** Extract Slack thread info from issue/PR metadata
    slack_thread = extract_slack_thread_from_payload(payload)

    if not slack_thread:
        ***REMOVED*** No Slack thread associated, skip
        return {"status": "ok"}

    ***REMOVED*** Handle different event types
...
async def handle_pr_event(payload: Dict, slack_thread: Dict):
    ...
    messages = {
        "opened": f"🔀 **Pull Request Opened**\n<{pr['html_url']}|PR ***REMOVED***{pr['number']}>\n{pr['title']}",
...
***REMOVED******REMOVED*** 5. Security Considerations

***REMOVED******REMOVED******REMOVED*** 5.1 Token Management

***REMOVED******REMOVED******REMOVED******REMOVED*** Storage Strategy
...
class TokenManager:
    ...
    def __init__(self):
        ***REMOVED*** Encryption key stored in environment (rotate quarterly)
...
***REMOVED*** Store encrypted tokens in Redis with TTL
class SlackTokenCache:
    ...
    async def store_token(
        ...
        team_id: str,
        token: str,
        ttl_seconds: int = 43200  ***REMOVED*** 12 hours
    ):
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Token Rotation

```yaml
***REMOVED*** .github/workflows/rotate-tokens.yml
name: Rotate API Tokens

on:
  schedule:
    - cron: '0 0 1 */3 *'  ***REMOVED*** Quarterly (1st of every 3rd month)
  workflow_dispatch:
  ...
  rotate-slack-token:
    ...
    steps:
      - name: Generate new Slack token
        run: |
          ***REMOVED*** Slack tokens can't be auto-rotated via API
          ***REMOVED*** This workflow creates a GitHub issue to remind admins
...
***REMOVED******REMOVED******REMOVED*** 5.2 Rate Limiting

***REMOVED******REMOVED******REMOVED******REMOVED*** Slack API Rate Limits
...
class SlackRateLimiter:
    ...
    async def check_limit(self, team_id: str) -> bool:
        ...
        ***REMOVED*** Set expiry on first request in window
        if count == 1:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** User-Based Rate Limiting
...
class UserRateLimiter:
    ...
    async def check_issue_creation_limit(self, user_id: str) -> bool:
        ...
        if count == 1:
            await self.redis.expire(key, 3600)  ***REMOVED*** 1 hour
    ...
    async def check_auto_implement_limit(self, user_id: str) -> bool:
        ...
        if count == 1:
            await self.redis.expire(key, 86400)  ***REMOVED*** 24 hours
...
***REMOVED******REMOVED******REMOVED*** 5.3 Audit Logging

***REMOVED******REMOVED******REMOVED******REMOVED*** Comprehensive Activity Log
...
class AuditLog(Base):
    ...
    ***REMOVED*** Who performed the action
    user_id = Column(String, nullable=False, index=True)  ***REMOVED*** Slack user ID
    user_email = Column(String, nullable=True)
    team_id = Column(String, nullable=False, index=True)  ***REMOVED*** Slack team ID

    ***REMOVED*** What action was performed
    action_type = Column(String, nullable=False, index=True)  ***REMOVED*** "issue_create", "pr_create", "auto_implement"
    resource_type = Column(String, nullable=False)  ***REMOVED*** "issue", "pr", "workflow"
    resource_id = Column(String, nullable=True)  ***REMOVED*** GitHub issue/PR number
    ...
    ***REMOVED*** Details (JSON)
    metadata = Column(JSON, nullable=True)

    ***REMOVED*** Result
    status = Column(String, nullable=False)  ***REMOVED*** "success", "failure", "rate_limited"
...
async def log_action(
    ...
    ***REMOVED*** Also log to external SIEM if configured
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Security Monitoring Queries
...
***REMOVED******REMOVED******REMOVED*** 5.4 Input Validation & Sanitization
...
class SlackMessageRequest(BaseModel):
    ...
    def sanitize_text(cls, v):
        ...
        ***REMOVED*** Remove script tags
        v = re.sub(r'<script[^>]*>.*?</script>', '', v, flags=re.DOTALL | re.IGNORECASE)

        ***REMOVED*** Remove SQL injection attempts
        dangerous_patterns = [
            r';\s*DROP\s+TABLE',
            r';\s*DELETE\s+FROM',
            r'UNION\s+SELECT',
...
***REMOVED*** Use in endpoint
@app.post("/slack/events")
async def handle_slack_event(request: Request):
    """Handle validated Slack event."""
    payload = await request.json()

    ***REMOVED*** Validate with Pydantic
    try:
    ...
    ***REMOVED*** Process validated data
    await process_slack_message(validated)
```

---

***REMOVED******REMOVED*** 6. Implementation Guide

***REMOVED******REMOVED******REMOVED*** 6.1 Prerequisites

```bash
***REMOVED*** Required accounts and tokens
- Slack Workspace (admin access)
- GitHub Organization (admin or owner)
- Anthropic API key (for Claude)
- Redis instance (for caching/queue)

***REMOVED*** Required secrets
...
ENCRYPTION_KEY=...  ***REMOVED*** Generate with: python -c 'from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())'
```

***REMOVED******REMOVED******REMOVED*** 6.2 Step-by-Step Setup

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 1: Create Slack App

```bash
***REMOVED*** 1. Go to https://api.slack.com/apps
***REMOVED*** 2. Click "Create New App" → "From scratch"
***REMOVED*** 3. Name: "Code Assistant Bot"
***REMOVED*** 4. Workspace: Your workspace

***REMOVED*** 5. Configure OAuth Scopes (Settings → OAuth & Permissions)
***REMOVED***    Add bot scopes from section 2.1

***REMOVED*** 6. Install app to workspace
***REMOVED***    Get SLACK_BOT_TOKEN (starts with xoxb-)

***REMOVED*** 7. Enable Events (Settings → Event Subscriptions)
***REMOVED***    Request URL: https://your-domain.com/slack/events
***REMOVED***    Subscribe to events from section 2.2

***REMOVED*** 8. Create slash commands (Settings → Slash Commands)
***REMOVED***    /create-issue → https://your-domain.com/slack/commands/create-issue
***REMOVED***    /start-pr → https://your-domain.com/slack/commands/start-pr
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 2: Deploy Backend Service
...
***REMOVED*** Add new dependencies for Slack integration
...
***REMOVED*** Terminal 1: Backend API
uvicorn app.main:app --reload

***REMOVED*** Terminal 2: Celery worker
celery -A app.celery_app worker --loglevel=info

***REMOVED*** Terminal 3: Celery beat (scheduler)
celery -A app.celery_app beat --loglevel=info

***REMOVED*** Terminal 4: Redis
redis-server
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 3: Configure GitHub Webhooks

```bash
***REMOVED*** GitHub Repository → Settings → Webhooks → Add webhook
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 4: Test Integration

```bash
***REMOVED*** 1. Test Slack event handling
...
***REMOVED*** Should return: {"challenge": "3eZbrw1aBm2rZgRNFdxV2595E9CY3gmdALWMmHkvFXO7tYXAYM8P"}

***REMOVED*** 2. Test slash command
***REMOVED*** In Slack: /create-issue bug Test issue

***REMOVED*** 3. Test mention
***REMOVED*** In Slack: @bot implement user authentication

***REMOVED*** 4. Test GitHub webhook
***REMOVED*** Create a test PR in your repo
***REMOVED*** Should see message in Slack thread
```

***REMOVED******REMOVED******REMOVED*** 6.3 Production Deployment
...
***REMOVED******REMOVED*** 7. Performance & Scalability

***REMOVED******REMOVED******REMOVED*** 7.1 Performance Targets
...
***REMOVED******REMOVED******REMOVED*** 7.2 Scaling Strategy
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Auto-Scaling Configuration

```yaml
***REMOVED*** Kubernetes HPA (Horizontal Pod Autoscaler)
...
spec:
  ...
  - type: Pods
    pods:
      ...
      target:
        type: AverageValue
        averageValue: "50"  ***REMOVED*** Scale up if queue > 50 tasks
```

***REMOVED******REMOVED******REMOVED*** 7.3 Caching Strategy
...
***REMOVED*** Layer 1: In-memory cache (LRU)
...
***REMOVED*** Layer 2: Redis cache (distributed)
...
***REMOVED******REMOVED*** 8. Monitoring & Observability

***REMOVED******REMOVED******REMOVED*** 8.1 Prometheus Metrics
...
***REMOVED******REMOVED******REMOVED*** 8.2 Grafana Dashboard
...
***REMOVED******REMOVED******REMOVED*** 8.3 Alerting Rules

```yaml
***REMOVED*** Prometheus alerting rules
groups:
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Foundation (Week 1)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Core Features (Week 2)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Autonomous Implementation (Week 3)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Security & Monitoring (Week 4)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Production Deployment (Week 5)
...
***REMOVED******REMOVED******REMOVED*** Optional Enhancements (Future)
