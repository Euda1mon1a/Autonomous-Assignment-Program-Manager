***REMOVED*** FastMCP 2.0 Implementation Plan for Residency Scheduler
...
1. [FastMCP Overview](***REMOVED***1-fastmcp-overview)
2. [Authentication Integration](***REMOVED***2-authentication-integration)
3. [FastAPI Integration](***REMOVED***3-fastapi-integration)
4. [Resource & Tool Patterns](***REMOVED***4-resource--tool-patterns)
5. [Implementation Architecture](***REMOVED***5-implementation-architecture)
6. [Testing Strategy](***REMOVED***6-testing-strategy)
7. [Implementation Roadmap](***REMOVED***7-implementation-roadmap)
8. [References](***REMOVED***8-references)

---

***REMOVED******REMOVED*** 1. FastMCP Overview

***REMOVED******REMOVED******REMOVED*** What is FastMCP 2.0?
...
***REMOVED******REMOVED******REMOVED*** Key Features and Capabilities

***REMOVED******REMOVED******REMOVED******REMOVED*** Core MCP Primitives
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Advanced Features (FastMCP 2.0 Exclusive)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Latest Features (v2.14+)

**Background Tasks (SEP-1686):**
```python
@mcp.tool(task=True)  ***REMOVED*** Enable background task support
...
***REMOVED******REMOVED******REMOVED*** Comparison with Raw MCP Protocol
...
***REMOVED******REMOVED*** 2. Authentication Integration

***REMOVED******REMOVED******REMOVED*** How FastMCP Handles Auth
...
***REMOVED******REMOVED******REMOVED*** Authentication Architecture
...
***REMOVED******REMOVED******REMOVED*** Integration with Existing JWT Tokens

FastMCP can validate JWT tokens issued by external systems (like our existing FastAPI auth):

***REMOVED******REMOVED******REMOVED******REMOVED*** Option 1: Custom JWT Validator (Recommended)
...
class ResidencySchedulerJWTAuth(AuthProvider):
    ...
    async def validate(self, credentials: str) -> AuthResult:
        ...
        try:
            ***REMOVED*** Decode and verify JWT (same logic as FastAPI)
            payload = jwt.decode(
                token,
                self.secret_key,
            ...
            ***REMOVED*** Return successful auth with user context
            return AuthResult(
                success=True,
                user_id=user_id,
                metadata={
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Option 2: OAuth Proxy Pattern
...
***REMOVED*** Create FastMCP server with OAuth
mcp = FastMCP(
...
***REMOVED******REMOVED******REMOVED*** Role-Based Access Patterns

***REMOVED******REMOVED******REMOVED******REMOVED*** Accessing User Context in Tools
...
async def create_swap_request(
    ...
    ***REMOVED*** Call existing service layer (reuse business logic)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Granular Permission Checks
...
***REMOVED*** Role -> Permission mapping
ROLE_PERMISSIONS = {
    ...
    def decorator(func):
        async def wrapper(ctx: Context, *args, **kwargs):
            ...
            if Permission.ADMIN in user_permissions:
                ***REMOVED*** Admins have all permissions
...
***REMOVED******REMOVED*** 3. FastAPI Integration

***REMOVED******REMOVED******REMOVED*** Can FastMCP Integrate with Existing FastAPI Routes?
...
***REMOVED******REMOVED******REMOVED*** Integration Pattern 1: Auto-Conversion (Fastest)

```python
from fastapi import FastAPI
from fastmcp import FastMCP

***REMOVED*** Existing FastAPI app
app = FastAPI(title="Residency Scheduler API")

***REMOVED*** ... existing routes ...

***REMOVED*** Auto-generate MCP server from FastAPI app
mcp = FastMCP.from_fastapi(
    app,
    name="Residency Scheduler MCP",
    ***REMOVED*** Only expose specific routes (optional)
    include_routes=[
        "/api/v1/schedule/*",
        "/api/v1/swaps/*",
        "/api/v1/acgme/*"
    ],
    ***REMOVED*** Exclude sensitive routes
    exclude_routes=[
        "/api/v1/auth/*",
...
***REMOVED******REMOVED******REMOVED*** Integration Pattern 2: Shared Database Sessions
...
***REMOVED*** Shared lifespan for both FastAPI and FastMCP
@asynccontextmanager
async def app_lifespan(app):
    ...
    ***REMOVED*** Initialize database pool (shared by both)
    ***REMOVED*** Already done in app.db.session
...
***REMOVED*** FastAPI app with shared lifespan
app = FastAPI(lifespan=app_lifespan)

***REMOVED*** FastMCP server
mcp = FastMCP("Residency Scheduler MCP")

***REMOVED*** Use the SAME database session factory in MCP tools
@mcp.tool()
async def get_schedule(person_id: str, start_date: str, end_date: str) -> dict:
    ...
    ***REMOVED*** Get database session from existing session factory
...
***REMOVED******REMOVED******REMOVED*** Integration Pattern 3: Middleware Compatibility
...
***REMOVED*** Custom middleware (existing)
...
***REMOVED*** IMPORTANT: Avoid CORS conflicts
***REMOVED*** FastMCP handles CORS for OAuth routes automatically
***REMOVED*** Only add CORS to FastAPI sub-app if needed:

from fastapi import APIRouter

api_router = APIRouter()
***REMOVED*** ... add routes to api_router ...

***REMOVED*** Mount as sub-app with its own CORS
api_app = FastAPI()
...
***REMOVED******REMOVED******REMOVED*** Integration Pattern 4: Combined App with Routing
...
***REMOVED*** Existing REST API routes
...
***REMOVED*** Add MCP tools (defined separately)
from app.mcp.tools import register_tools
register_tools(mcp)

***REMOVED*** Mount MCP server at /mcp endpoint
***REMOVED*** Supports both SSE and WebSocket transports
app.mount("/mcp", mcp.get_asgi_app())

***REMOVED*** Now clients can:
***REMOVED*** - Use REST API: http://localhost:8000/api/v1/schedule
***REMOVED*** - Use MCP: http://localhost:8000/mcp (SSE transport)
```
...
***REMOVED******REMOVED*** 4. Resource & Tool Patterns

***REMOVED******REMOVED******REMOVED*** Best Practices for Read-Only Resources

Resources expose read-only data to LLMs. They're URI-addressable and support templating.

***REMOVED******REMOVED******REMOVED******REMOVED*** Static Resources
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Dynamic Resources with Templates
...
async def person_current_week_schedule(person_id: str) -> str:
    ...
    ***REMOVED*** Calculate current week bounds
    today = date.today()
    ...
    ***REMOVED*** Format as readable text for LLM
    lines = [
...
async def person_month_schedule(person_id: str, year: int, month: int) -> str:
    ...
    Args:
        ...
        month: Month (1-12)
    """
    ***REMOVED*** Implementation similar to above
    pass
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Database-Backed Resources
...
***REMOVED******REMOVED******REMOVED*** Write/Action Tool Patterns
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Simple Tool Pattern
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Complex Tool with Transaction
...
async def create_swap_request(
    ...
    ***REMOVED*** Get authenticated user
    user_id = ctx.request_context.auth_result.user_id
    user_role = ctx.request_context.auth_result.metadata.get("role")
    ...
    ***REMOVED*** Check permissions
    ...
    async with async_session_maker() as db:
        try:
            ***REMOVED*** Use existing service layer (reuse business logic!)
            service = SwapService()

            ***REMOVED*** Validate swap is possible
            ...
            ***REMOVED*** Create swap request (transactional)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Background Task Tool (Long-Running Operations)
...
async def generate_schedule(
    ...
    async with async_session_maker() as db:
        ...
        ***REMOVED*** Report progress: Phase 1
        ...
        ***REMOVED*** Report progress: Phase 2
        await ctx.report_progress(
            progress=20.0,
            total=100.0,
...
async def get_faculty_workload(faculty_id: str) -> dict:
    ...
    async with async_session_maker() as db:
        ***REMOVED*** Async query with eager loading
        ...
        ***REMOVED*** Calculate workload
        ...
        return {
            ...
            "utilization_percent": (total_hours / 2080) * 100  ***REMOVED*** Annual hours
        }
```
...
***REMOVED******REMOVED*** 5. Implementation Architecture

***REMOVED******REMOVED******REMOVED*** Recommended Project Structure

```
backend/app/
├── mcp/                          ***REMOVED*** NEW: MCP server implementation
│   ├── __init__.py
│   ├── server.py                 ***REMOVED*** Main MCP server setup
│   ├── auth.py                   ***REMOVED*** JWT validator for MCP
│   ├── tools/                    ***REMOVED*** MCP tools (write operations)
│   │   ├── __init__.py
│   │   ├── schedule_tools.py     ***REMOVED*** Schedule management tools
│   │   ├── swap_tools.py         ***REMOVED*** Swap request tools
│   │   ├── acgme_tools.py        ***REMOVED*** ACGME analysis tools
│   │   └── admin_tools.py        ***REMOVED*** Admin-only tools
│   ├── resources/                ***REMOVED*** MCP resources (read operations)
│   │   ├── __init__.py
│   │   ├── schedule_resources.py ***REMOVED*** Schedule data resources
│   │   ├── acgme_resources.py    ***REMOVED*** ACGME regulation resources
│   │   └── faculty_resources.py  ***REMOVED*** Faculty/staff resources
│   └── testing/                  ***REMOVED*** MCP-specific tests
│       ├── __init__.py
│       ├── test_tools.py
│       └── test_resources.py
│
├── api/                          ***REMOVED*** Existing FastAPI routes
├── services/                     ***REMOVED*** Existing business logic (REUSE!)
├── models/                       ***REMOVED*** Existing SQLAlchemy models
├── schemas/                      ***REMOVED*** Existing Pydantic schemas
└── main.py                       ***REMOVED*** Updated to mount MCP server
```

***REMOVED******REMOVED******REMOVED*** Code Scaffolding Plan

***REMOVED******REMOVED******REMOVED******REMOVED*** Step 1: Create MCP Server Foundation
...
***REMOVED*** Import and register all tools and resources
from app.mcp.tools import register_tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 2: Create Authentication Provider
...
class ResidencySchedulerJWTAuth(AuthProvider):
    ...
    async def validate(self, credentials: str) -> AuthResult:
        ...
        ***REMOVED*** Extract token
        token = credentials.removeprefix("Bearer ").strip()

        try:
            ***REMOVED*** Decode JWT (same as FastAPI auth)
            payload = jwt.decode(
                token,
                self.secret_key,
                algorithms=[self.algorithm]
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 3: Create Tool Registry
...
def register_tools(mcp: FastMCP):
    ...
    ***REMOVED*** Tools are registered via decorators in each module
    ***REMOVED*** Just importing them registers them with the server
    pass
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 4: Create Resource Registry
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Step 5: Integrate with FastAPI Main App
...
***REMOVED*** Mount MCP server
***REMOVED*** Accessible via SSE at http://localhost:8000/mcp
app.mount("/mcp", mcp.get_asgi_app())
...
***REMOVED******REMOVED******REMOVED*** Service Layer Integration

**Key Principle:** **Reuse existing service layer** - don't duplicate business logic!

```python
***REMOVED*** MCP tool wraps existing service
from app.services.swap_service import SwapService  ***REMOVED*** Existing service
...
async def approve_swap(ctx: Context, swap_id: str) -> dict:
    ...
    async with async_session_maker() as db:
        ***REMOVED*** Reuse existing service method
        service = SwapService()
...
***REMOVED******REMOVED*** 6. Testing Strategy

***REMOVED******REMOVED******REMOVED*** Unit Testing MCP Tools

FastMCP provides **in-memory testing** - no subprocess or network overhead:

***REMOVED******REMOVED******REMOVED******REMOVED*** Basic Tool Test
...
async def test_get_person_schedule():
    ...
    ***REMOVED*** Use in-memory client (no subprocess!)
    async with Client(mcp) as client:
        ***REMOVED*** Call the tool
        ...
        ***REMOVED*** Verify result
        assert len(result) == 1
        data = result[0].text  ***REMOVED*** JSON string
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test with Authentication
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Resource Fetching
...
async def test_schedule_generation_integration():
    ...
    ***REMOVED*** Setup test data
    async with async_session_maker() as db:
        ***REMOVED*** Create test person
        person = Person(
            id="test-faculty-1",
            name="Dr. Test",
            email="test@example.com",
...
***REMOVED******REMOVED******REMOVED*** Mock Patterns
    ...
    with patch("app.services.notification_service.NotificationService") as mock_notif:
        ***REMOVED*** Setup mock
        ...
        async with Client(mcp) as client:
            ...
            ***REMOVED*** Verify notification was called
...
```
backend/tests/
├── mcp/                          ***REMOVED*** MCP-specific tests
│   ├── __init__.py
│   ├── test_auth.py             ***REMOVED*** Authentication tests
│   ├── test_tools.py            ***REMOVED*** Tool unit tests
│   ├── test_resources.py        ***REMOVED*** Resource tests
...
```bash
***REMOVED*** All MCP tests
pytest tests/mcp/

***REMOVED*** Unit tests only (fast)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Read Operations (Week 2)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Write Operations (Week 3)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Advanced Features (Week 4)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Production Readiness (Week 5)
...
***REMOVED******REMOVED******REMOVED*** Integration & Tools
...
***REMOVED******REMOVED******REMOVED*** Advanced Topics
...
***REMOVED******REMOVED*** Appendix A: Example Tool Implementation

***REMOVED******REMOVED******REMOVED*** Complete Example: Swap Request Tool
...
async def create_swap_request(
    ...
    Example:
        ...
        ```
    """
    ***REMOVED*** Extract authenticated user
    user_id = ctx.request_context.auth_result.user_id
    user_role = ctx.request_context.auth_result.metadata.get("role")
    ...
    ***REMOVED*** Check permissions
    ...
    async with async_session_maker() as db:
        try:
            ***REMOVED*** Initialize service
            service = SwapService()

            ***REMOVED*** Phase 1: Validate swap is possible
            ...
            ***REMOVED*** Phase 2: Create swap request (transactional)
            ...
            ***REMOVED*** Phase 3: Attempt auto-matching
            ...
            ***REMOVED*** Business logic error (e.g., invalid block ID)
            await db.rollback()
...
***REMOVED******REMOVED*** Appendix B: Performance Considerations

***REMOVED******REMOVED******REMOVED*** Database Query Optimization
...
async def get_faculty_schedule_with_rotations(faculty_id: str) -> dict:
    ...
    async with async_session_maker() as db:
        ...
        ***REMOVED*** result = await db.execute(select(Person).where(Person.id == faculty_id))
        ***REMOVED*** person = result.scalar_one()
        ***REMOVED*** for assignment in person.assignments:  ***REMOVED*** Triggers separate query!
        ***REMOVED***     rotation = assignment.rotation  ***REMOVED*** Another query!

        ***REMOVED*** GOOD: Single query with eager loading
        result = await db.execute(
            select(Person)
            .where(Person.id == faculty_id)
            .options(
        ...
        ***REMOVED*** Now all data is loaded, no additional queries
        return {
            "person": person.name,
            "assignments": [
                {
...
***REMOVED*** Cache static resources
...
async def get_acgme_regulation(regulation_name: str) -> str:
    ...
    ***REMOVED*** This won't hit database repeatedly
    pass

***REMOVED*** Cache with TTL for semi-static data
_cache = {}
_cache_ttl = {}
