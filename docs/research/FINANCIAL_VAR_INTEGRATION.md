***REMOVED*** Financial Value-at-Risk (VaR) Integration for Residency Scheduling
...
***REMOVED******REMOVED******REMOVED*** Cross-Disciplinary Mapping
...
1. [Background: VaR in Financial Risk Management](***REMOVED***background-var-in-financial-risk-management)
2. [VaR Calculation Methodologies](***REMOVED***var-calculation-methodologies)
3. [Expected Shortfall (CVaR) for Tail Risk](***REMOVED***expected-shortfall-cvar-for-tail-risk)
4. [Stress Testing Framework](***REMOVED***stress-testing-framework)
5. [Risk Aggregation Across Portfolios](***REMOVED***risk-aggregation-across-portfolios)
6. [Mapping VaR to Scheduling Risk](***REMOVED***mapping-var-to-scheduling-risk)
7. [Monte Carlo Simulation for Schedule Scenarios](***REMOVED***monte-carlo-simulation-for-schedule-scenarios)
8. [Risk Budgeting for Workload Distribution](***REMOVED***risk-budgeting-for-workload-distribution)
9. [Mathematical Formulations](***REMOVED***mathematical-formulations)
10. [Implementation Architecture](***REMOVED***implementation-architecture)
11. [Code Integration Points](***REMOVED***code-integration-points)
12. [Implementation Roadmap](***REMOVED***implementation-roadmap)
13. [References](***REMOVED***references)

---

***REMOVED******REMOVED*** Background: VaR in Financial Risk Management

***REMOVED******REMOVED******REMOVED*** Definition
...
***REMOVED******REMOVED******REMOVED*** Advantages
...
**Example (Scheduling)**:
```python
***REMOVED*** Historical weekly coverage shortfalls over past year (52 weeks)
historical_shortfalls = [0, 0, 1, 0, 2, 0, 0, 3, 0, 1, ...]  ***REMOVED*** hours understaffed

***REMOVED*** 95% VaR: 5th percentile (worst 5% of weeks)
var_95 = np.percentile(historical_shortfalls, 95)
***REMOVED*** Result: "We expect coverage shortfalls ≤ 4 hours on 95% of weeks"
```
...
***REMOVED******REMOVED******REMOVED*** 2. Parametric (Variance-Covariance) VaR
...
**Example (Scheduling)**:
```python
***REMOVED*** Weekly hours per resident (target = 60, σ = 8)
mu = 60
sigma = 8
confidence = 0.95

***REMOVED*** 95% VaR for weekly hours exceeding target
z_score = norm.ppf(confidence)  ***REMOVED*** 1.645 for 95%
var_95 = mu + sigma * z_score   ***REMOVED*** 60 + 8 × 1.645 = 73.16 hours

***REMOVED*** Interpretation: 95% of weeks, hours won't exceed 73.16
```
...
***REMOVED******REMOVED******REMOVED*** 3. Monte Carlo Simulation VaR
...
def monte_carlo_coverage_var(n_simulations=10000, weeks=4, confidence=0.95):
    ...
    for _ in range(n_simulations):
        ...
        for week in range(weeks):
            ***REMOVED*** Simulate case arrivals (mean=50/week, Poisson)
            cases = np.random.poisson(lam=50)

            ***REMOVED*** Simulate resident absences (10 residents, 5% absence rate)
            absences = np.random.binomial(n=10, p=0.05)

            ***REMOVED*** Simulate case durations (mean=2h, log-normal)
            durations = np.random.lognormal(mean=np.log(2), sigma=0.3, size=cases)

            ***REMOVED*** Total demand
            demand_hours = durations.sum()

            ***REMOVED*** Available capacity (10 - absences) × 60 hours/week
            capacity_hours = (10 - absences) * 60

            ***REMOVED*** Shortfall (if demand exceeds capacity)
            week_shortfall = max(0, demand_hours - capacity_hours)
            total_shortfall += week_shortfall

        shortfalls.append(total_shortfall)

    ***REMOVED*** VaR: 95th percentile of shortfalls
    var_95 = np.percentile(shortfalls, 95)
    return var_95

***REMOVED*** Result: "95% confident coverage shortfall won't exceed X hours over 4 weeks"
```
...
***REMOVED******REMOVED*** Expected Shortfall (CVaR) for Tail Risk

***REMOVED******REMOVED******REMOVED*** Motivation: Beyond VaR
...
***REMOVED*** Example
losses = np.array([0, 1, 2, 3, 5, 8, 15, 25, 40, 100])  ***REMOVED*** Sorted
var_90 = np.percentile(losses, 90)  ***REMOVED*** 40
es_90 = losses[losses >= var_90].mean()  ***REMOVED*** (40 + 100) / 2 = 70
```

***REMOVED******REMOVED******REMOVED*** Advantages of ES over VaR
...
***REMOVED******REMOVED******REMOVED*** 2025 Regulatory Context
...
***REMOVED******REMOVED******REMOVED*** Scheduling Application: Coverage CVaR
...
```python
***REMOVED*** Historical weekly shortfalls (hours)
shortfalls = [0, 0, 1, 0, 0, 2, 0, 3, 0, 0, 1, 5, 0, 8, 0, 0, 12, ...]

***REMOVED*** VaR: 95th percentile
var_95 = np.percentile(shortfalls, 95)  ***REMOVED*** 8 hours

***REMOVED*** CVaR: Average of tail (worst 5%)
tail_shortfalls = shortfalls[shortfalls >= var_95]
cvar_95 = tail_shortfalls.mean()  ***REMOVED*** 14 hours

***REMOVED*** Interpretation:
***REMOVED*** - 95% of weeks, shortfalls ≤ 8 hours (VaR)
***REMOVED*** - When shortfalls occur (5% of weeks), average = 14 hours (CVaR)
```
...
***REMOVED******REMOVED*** Stress Testing Framework
...
***REMOVED******REMOVED******REMOVED*** Federal Reserve 2025 Stress Test Scenarios
...
***REMOVED******REMOVED******REMOVED*** Stress Testing Methodologies

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Scenario Analysis
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Sensitivity Analysis
...
def sensitivity_analysis_coverage(base_residents=10, base_cases=50):
    ...
    results = []
    for residents in range(6, 15):  ***REMOVED*** Vary ±40%
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Reverse Stress Testing
...
***REMOVED******REMOVED******REMOVED*** Scheduling Stress Test Architecture
...
***REMOVED******REMOVED*** Risk Aggregation Across Portfolios

***REMOVED******REMOVED******REMOVED*** Challenge: Portfolio Effects
...
***REMOVED******REMOVED******REMOVED*** Variance-Covariance Method
...
***REMOVED*** Three rotations: ER, ICU, Clinic
weights = np.array([0.4, 0.4, 0.2])  ***REMOVED*** 40% ER, 40% ICU, 20% Clinic

***REMOVED*** Weekly workload standard deviations (hours)
sigma_er = 12
sigma_icu = 10
sigma_clinic = 5

***REMOVED*** Correlation matrix (estimated from historical data)
correlation = np.array([
    [1.0, 0.6, 0.2],   ***REMOVED*** ER correlated with ICU (0.6), weakly with Clinic (0.2)
    [0.6, 1.0, 0.3],   ***REMOVED*** ICU moderately correlated with Clinic (0.3)
    [0.2, 0.3, 1.0]    ***REMOVED*** Clinic relatively independent
])

***REMOVED*** Covariance matrix: Σ = D × R × D
***REMOVED*** where D = diag([σ_er, σ_icu, σ_clinic])
stdevs = np.array([sigma_er, sigma_icu, sigma_clinic])
covariance = np.outer(stdevs, stdevs) * correlation

***REMOVED*** Portfolio variance
portfolio_variance = weights.T @ covariance @ weights
portfolio_stdev = np.sqrt(portfolio_variance)  ***REMOVED*** ~8.9 hours

***REMOVED*** 95% VaR (assuming normality)
var_95 = 1.645 * portfolio_stdev  ***REMOVED*** ~14.6 hours
```
...
***REMOVED******REMOVED******REMOVED*** Copula-Based Aggregation (Advanced)
...
***REMOVED******REMOVED******REMOVED*** Downside Risk Approaches
...
***REMOVED*** Standard deviation: 8.2 (penalizes 58h same as 75h)
std_dev = hours.std()

***REMOVED*** Downside risk: 6.8 (only penalizes 75h, 80h, 70h)
downside = calculate_downside_risk(hours, target)
```

---

***REMOVED******REMOVED*** Mapping VaR to Scheduling Risk

***REMOVED******REMOVED******REMOVED*** Coverage VaR
...
def coverage_var(
    ...
    """
    ***REMOVED*** Aggregate to horizon (sum of weekly shortfalls)
    ...
    ***REMOVED*** VaR = percentile of aggregated distribution
    var = np.percentile(horizon_shortfalls, confidence * 100)
    return var

***REMOVED*** Usage
weekly_gaps = [0, 1, 0, 2, 0, 0, 3, ...]  ***REMOVED*** 52 weeks historical
coverage_var_4wk = coverage_var(weekly_gaps, confidence=0.95, horizon_weeks=4)
***REMOVED*** Result: "95% confident 4-week coverage gap won't exceed 8 hours"
```
...
***REMOVED******REMOVED******REMOVED*** Compliance VaR
...
***REMOVED*** Usage
history = {
    'R001': [65, 72, 68, 75, 70, 67, 71, ...],  ***REMOVED*** 52 weeks
    'R002': [60, 78, 62, 74, 69, 73, 66, ...],
    ***REMOVED*** ...
}

var_99 = acgme_80hour_var(history, confidence=0.99)
***REMOVED*** Result: {'R001': 78.3, 'R002': 79.1, ...}

***REMOVED*** Alert if any VaR > 80
...
***REMOVED******REMOVED******REMOVED*** Workload VaR
...
def workload_distribution_var(
    ...
    """
    ***REMOVED*** Calculate std dev across residents for each week
    weekly_stdevs = weekly_hours_matrix.std(axis=1)

    ***REMOVED*** VaR = percentile of std dev distribution
    var = np.percentile(weekly_stdevs, confidence * 100)
    return var

***REMOVED*** Example
hours_matrix = np.array([
    [60, 65, 58, 70, 62],  ***REMOVED*** Week 1: 5 residents
    [62, 68, 75, 60, 65],  ***REMOVED*** Week 2
    ***REMOVED*** ... 52 weeks
])

inequality_var = workload_distribution_var(hours_matrix, 0.95)
***REMOVED*** Result: "95% of weeks, workload std dev ≤ 6.2 hours"
***REMOVED*** Lower VaR = more equitable scheduling
```

---

***REMOVED******REMOVED*** Monte Carlo Simulation for Schedule Scenarios

***REMOVED******REMOVED******REMOVED*** Why Monte Carlo for Scheduling?
...
***REMOVED******REMOVED******REMOVED*** Algorithm Architecture
...
class ScheduleMonteCarloSimulator:
    ...
    def _generate_scenario(
        ...
        for week in range(self.time_horizon):
            ***REMOVED*** Simulate patient arrivals
            n_cases = np.random.poisson(
                risk_factors['case_arrival_rate']
            )

            ***REMOVED*** Simulate case durations
            ...
            ***REMOVED*** Simulate resident absences
            ...
            ***REMOVED*** Simulate emergencies
            n_emergencies = np.random.poisson(
                risk_factors['emergency_rate']
            )

            ***REMOVED*** Update scenario schedule
...
***REMOVED******REMOVED******REMOVED*** Example: 4-Week Coverage Simulation

```python
***REMOVED*** Initialize simulator
simulator = ScheduleMonteCarloSimulator(
    n_simulations=10000,
    time_horizon_weeks=4
)

***REMOVED*** Define risk factors
risk_factors = {
    'case_arrival_rate': 50,  ***REMOVED*** Cases per week (Poisson λ)
    'case_duration_params': (2.0, 0.3),  ***REMOVED*** Mean=2h, σ=0.3 (log-normal)
    'absence_probability': 0.05,  ***REMOVED*** 5% weekly absence rate
    'emergency_rate': 1.5  ***REMOVED*** 1.5 emergencies per week
}
...
***REMOVED*** Output:
***REMOVED*** Coverage Shortfall Risk (hours):
***REMOVED***   Mean: 3.2
***REMOVED***   VaR (95%): 8.4
***REMOVED***   CVaR (95%): 14.7
***REMOVED***   VaR (99%): 18.3
***REMOVED***   CVaR (99%): 26.5
```
...
***REMOVED******REMOVED*** Risk Budgeting for Workload Distribution

***REMOVED******REMOVED******REMOVED*** Concept: Allocate Risk Capacity
...
***REMOVED******REMOVED******REMOVED*** Risk Contribution Analysis
...
def calculate_risk_contributions(weights, covariance_matrix):
    ...
    ***REMOVED*** Marginal contributions
    ...
    ***REMOVED*** Percentage contributions
...
covariance = np.array([
    [144, 72, 12],   ***REMOVED*** ER variance=144, cov(ER,ICU)=72, cov(ER,Clinic)=12
    [72, 100, 15],   ***REMOVED*** ICU variance=100
    [12, 15, 25]     ***REMOVED*** Clinic variance=25
])

risk_contributions = calculate_risk_contributions(weights, covariance)
***REMOVED*** Result: [52%, 41%, 7%]
...
***REMOVED******REMOVED******REMOVED*** Risk Parity Approach
...
def risk_parity_objective(weights, covariance_matrix):
    ...
    ***REMOVED*** Target: equal contributions (1/N each)
    n = len(weights)
    target_contrib = portfolio_stdev / n

    ***REMOVED*** Penalize deviations from equal contribution
...
def risk_parity_portfolio(covariance_matrix, constraints=None):
    ...
    ***REMOVED*** Initial guess: equal weight
    init_weights = np.ones(n) / n

    ***REMOVED*** Constraints: weights sum to 1, all non-negative
    if constraints is None:
        constraints = [
            {'type': 'eq', 'fun': lambda w: w.sum() - 1},  ***REMOVED*** Sum = 1
        ]
    bounds = [(0, 1) for _ in range(n)]  ***REMOVED*** 0 ≤ w_i ≤ 1
...
***REMOVED*** Optimize rotation allocation
...
***REMOVED*** Result: ER=28%, ICU=32%, Clinic=40%
***REMOVED*** (Reduces high-variance ER, increases low-variance Clinic)
```

***REMOVED******REMOVED******REMOVED*** Risk Budget Allocation
...
def allocate_risk_budget(
    ...
    ***REMOVED*** Build covariance matrix
    ...
    ***REMOVED*** Constraints
    constraint_list = [
        {'type': 'eq', 'fun': lambda w: w.sum() - 1},  ***REMOVED*** Sum = 1
        {'type': 'ineq', 'fun': risk_constraint}  ***REMOVED*** Variance ≤ budget
    ]

    ***REMOVED*** Bounds (min/max allocation per rotation)
    if constraints:
    ...
    ***REMOVED*** Objective: Maximize "coverage quality" (placeholder—could be procedure count)
    ***REMOVED*** Here: uniform allocation subject to risk constraint
    init_weights = np.ones(n) / n

    result = minimize(
        fun=lambda w: -w.sum(),  ***REMOVED*** Dummy objective (feasibility problem)
        x0=init_weights,
        method='SLSQP',
        bounds=bounds,
...
***REMOVED******REMOVED*** Mathematical Formulations

***REMOVED******REMOVED******REMOVED*** Parametric VaR (Normal Distribution)
...
***REMOVED******REMOVED******REMOVED*** Expected Shortfall
...
***REMOVED******REMOVED******REMOVED*** Marginal VaR
...
***REMOVED******REMOVED******REMOVED*** Monte Carlo VaR Convergence
...
├── __init__.py
├── var_calculator.py          ***REMOVED*** Core VaR calculation engines
├── expected_shortfall.py      ***REMOVED*** CVaR/ES methods
├── stress_testing.py          ***REMOVED*** Scenario analysis framework
├── monte_carlo.py             ***REMOVED*** MC simulation engine
├── risk_aggregation.py        ***REMOVED*** Portfolio VaR, correlation
├── risk_budgeting.py          ***REMOVED*** Risk parity, allocation
└── models.py                  ***REMOVED*** Data classes (VaRResult, StressScenario)
```

***REMOVED******REMOVED******REMOVED*** Core Classes

```python
***REMOVED*** var_calculator.py
class VaRCalculator:
...
class VaRResult:
    ...
    var: float
    confidence: float
    method: str  ***REMOVED*** 'historical', 'parametric', 'monte_carlo'
    metadata: dict
...
class StressTestResult:
    ...
    severity: str  ***REMOVED*** 'mild', 'moderate', 'severe', 'catastrophic'
```
...
def calculate_schedule_var():
    ...
    ***REMOVED*** Trigger alerts if VaR exceeds thresholds
...
def generate_schedule_with_risk_constraints(
    residents: list,
    rotations: list,
    max_coverage_var: float = 10.0  ***REMOVED*** hours
) -> Schedule:
    """Generate schedule respecting risk budget."""

    ***REMOVED*** Monte Carlo pre-assessment
    ...
    ***REMOVED*** Check risk budget
    if risk_forecast.coverage_var_95 > max_coverage_var:
        ***REMOVED*** Adjust schedule (add float, reduce variance)
...
***REMOVED******REMOVED******REMOVED*** 1. Database Schema Extensions

```python
***REMOVED*** backend/app/models/resilience_metrics.py
class VaRMetric(Base):
    __tablename__ = "var_metrics"

    id: Mapped[UUID] = mapped_column(primary_key=True, default=uuid4)
    metric_type: Mapped[str]  ***REMOVED*** 'coverage', 'compliance', 'workload'
    confidence_level: Mapped[float]  ***REMOVED*** 0.95, 0.99
    var_value: Mapped[float]
    cvar_value: Mapped[float]  ***REMOVED*** Expected Shortfall
    calculation_method: Mapped[str]  ***REMOVED*** 'historical', 'parametric', 'monte_carlo'
...
class StressTestResult(Base):
    ...
    severity: Mapped[str]  ***REMOVED*** 'mild', 'moderate', 'severe', 'catastrophic'
...
***REMOVED******REMOVED******REMOVED*** 2. API Schema (Pydantic)

```python
***REMOVED*** backend/app/schemas/var_metrics.py
...
class RiskDashboardResponse(BaseModel):
    ...
    risk_contributions: dict[str, float]  ***REMOVED*** {rotation_name: risk_pct}
```

***REMOVED******REMOVED******REMOVED*** 3. Frontend Integration (Next.js/React)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Monte Carlo Simulation (Weeks 3-4)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Risk Aggregation & Budgeting (Weeks 5-6)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Dashboard & Visualization (Weeks 7-8)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Advanced Features (Weeks 9-12)
...
***REMOVED******REMOVED******REMOVED*** Web Resources
