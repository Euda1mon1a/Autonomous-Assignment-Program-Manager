***REMOVED*** Network Resilience Deep Dive: Graph Theory for Schedule Analysis
...
1. [Network Topology Analysis](***REMOVED***1-network-topology-analysis)
2. [Robustness Metrics](***REMOVED***2-robustness-metrics)
3. [Centrality Measures](***REMOVED***3-centrality-measures)
4. [Community Detection](***REMOVED***4-community-detection)
5. [Dynamic Networks](***REMOVED***5-dynamic-networks)
6. [NetworkX Integration](***REMOVED***6-networkx-integration)
7. [References](***REMOVED***7-references)

---

***REMOVED******REMOVED*** 1. Network Topology Analysis

***REMOVED******REMOVED******REMOVED*** 1.1 Small-World Networks

***REMOVED******REMOVED******REMOVED******REMOVED*** Core Principle
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Application to Medical Scheduling
...
***REMOVED******REMOVED******REMOVED******REMOVED*** NetworkX Implementation
...
def analyze_small_world_properties(
    ...
    """
    ***REMOVED*** Global clustering coefficient
    clustering = nx.average_clustering(G)

    ***REMOVED*** Average shortest path length (only for connected component)
    if nx.is_connected(G):
        avg_path_length = nx.average_shortest_path_length(G)
    else:
        ***REMOVED*** Get largest connected component
        largest_cc = max(nx.connected_components(G), key=len)
        G_cc = G.subgraph(largest_cc).copy()
        avg_path_length = nx.average_shortest_path_length(G_cc)

    ***REMOVED*** Compare to random graph with same degree distribution
    ***REMOVED*** Generate random graph with same degree sequence
    degree_sequence = [d for n, d in G.degree()]
    G_random = nx.configuration_model(degree_sequence)
    G_random = nx.Graph(G_random)  ***REMOVED*** Remove multi-edges
    ...
    ***REMOVED*** Small-world coefficient
    ***REMOVED*** σ = (C/C_random) / (L/L_random)
    ***REMOVED*** σ > 1 indicates small-world topology
...
def identify_small_world_bridges(
    ...
    for u, v in G.edges():
        ***REMOVED*** Bridge if both endpoints have low local clustering
        if clustering[u] < clustering_threshold and clustering[v] < clustering_threshold:
            bridges.append((u, v))
        ***REMOVED*** Or if removing edge would disconnect components
...
def demo_small_world_analysis():
    ...
    ***REMOVED*** Build coverage network
    G = nx.Graph()

    ***REMOVED*** Faculty nodes
    faculty = ["FAC-PD", "FAC-APD", "FAC-01", "FAC-02", "FAC-03",
               "FAC-04", "FAC-05", "FAC-06", "FAC-07", "FAC-08"]
    G.add_nodes_from(faculty)

    ***REMOVED*** Edges represent shared service coverage
    ***REMOVED*** Clustered groups: Peds (PD, APD, 01, 02), Adult (03, 04, 05),
    ***REMOVED*** Procedures (06, 07, 08)
    edges = [
        ***REMOVED*** Peds cluster (high clustering)
        ("FAC-PD", "FAC-APD"), ("FAC-PD", "FAC-01"), ("FAC-PD", "FAC-02"),
        ("FAC-APD", "FAC-01"), ("FAC-APD", "FAC-02"), ("FAC-01", "FAC-02"),

        ***REMOVED*** Adult cluster
        ("FAC-03", "FAC-04"), ("FAC-04", "FAC-05"), ("FAC-03", "FAC-05"),

        ***REMOVED*** Procedures cluster
        ("FAC-06", "FAC-07"), ("FAC-07", "FAC-08"), ("FAC-06", "FAC-08"),

        ***REMOVED*** Bridges between clusters (critical!)
        ("FAC-02", "FAC-03"),  ***REMOVED*** Peds to Adult
        ("FAC-05", "FAC-06"),  ***REMOVED*** Adult to Procedures
    ]
...
***REMOVED******REMOVED******REMOVED*** 1.2 Scale-Free Networks
...
def detect_scale_free_distribution(
    G: nx.Graph,
    degree_type: str = 'degree'  ***REMOVED*** 'degree', 'in_degree', 'out_degree'
) -> Dict[str, any]:
    """
    ...
    ***REMOVED*** Fit power law using powerlaw package (proper MLE)
    fit = powerlaw.Fit(degrees, discrete=True)

    ***REMOVED*** Get power-law exponent
    gamma = fit.power_law.alpha
    xmin = fit.power_law.xmin

    ***REMOVED*** Compare to alternative distributions
    ***REMOVED*** R > 0: power law is better fit
    ***REMOVED*** R < 0: alternative is better fit
    R_exp, p_exp = fit.distribution_compare('power_law', 'exponential')
    R_lognorm, p_lognorm = fit.distribution_compare('power_law', 'lognormal')

    ***REMOVED*** Visual test: log-log plot should be linear
    ***REMOVED*** Calculate R² for linear fit in log-log space
    ...
    ***REMOVED*** Determine if scale-free
    is_scale_free = (
        R_exp > 0 and  ***REMOVED*** Better than exponential
        R_lognorm > 0 and  ***REMOVED*** Better than lognormal
        2.0 < gamma < 3.5 and  ***REMOVED*** Reasonable exponent
        r_squared > 0.7  ***REMOVED*** Good linear fit in log-log
    )
...
***REMOVED******REMOVED******REMOVED*** 1.3 Clustering Coefficient
...
def analyze_clustering_structure(
    ...
    """
    ***REMOVED*** Local clustering for each node
    local_clustering = nx.clustering(G)

    ***REMOVED*** Global clustering (transitivity)
    global_clustering = nx.transitivity(G)

    ***REMOVED*** Average clustering
    avg_clustering = nx.average_clustering(G)

    ***REMOVED*** Identify highly-clustered nodes (potential team cores)
    high_cluster_nodes = [
        (node, coef)
    ...
    ***REMOVED*** Find triangles (3-node cliques)
    triangles = [clique for clique in nx.enumerate_all_cliques(G) if len(clique) == 3]

    ***REMOVED*** Find k-cliques (fully connected subgroups)
    cliques_4 = [c for c in nx.find_cliques(G) if len(c) == 4]
    cliques_5plus = [c for c in nx.find_cliques(G) if len(c) >= 5]

    ***REMOVED*** Clustering distribution
...
def identify_optimal_teams(
    ...
    ***REMOVED*** Find communities using Louvain algorithm
    communities = community.louvain_communities(G, seed=42)

    ***REMOVED*** Refine communities based on clustering
    teams = []

    for comm in communities:
        ***REMOVED*** Extract subgraph
        subG = G.subgraph(comm).copy()

        ***REMOVED*** If too large, split by k-core decomposition
        if len(comm) > target_size * 1.5:
            k_cores = nx.core_number(subG)
            max_k = max(k_cores.values())

            ***REMOVED*** Split into high-k core and periphery
...
***REMOVED******REMOVED*** 2. Robustness Metrics

***REMOVED******REMOVED******REMOVED*** 2.1 Percolation Theory
...
def calculate_percolation_threshold(
    G: nx.Graph,
    removal_strategy: str = 'random',  ***REMOVED*** 'random', 'degree', 'betweenness'
    num_trials: int = 100
) -> Dict[str, any]:
    """
    ...
    if removal_strategy == 'random':
        ***REMOVED*** Monte Carlo simulation
        ...
        for trial in range(num_trials):
            ...
            for i, node in enumerate(nodes):
                ...
                ***REMOVED*** Measure largest component
        ...
        ***REMOVED*** Average across trials
        avg_giant_component = np.mean(results, axis=0)

        ***REMOVED*** Find threshold (where giant component < 0.5)
        ...
        ***REMOVED*** Targeted attack: remove highest degree nodes first
        G_copy = G.copy()
        ...
        ***REMOVED*** Targeted attack: remove highest betweenness nodes first
        G_copy = G.copy()
    ...
    ***REMOVED*** Calculate theoretical threshold for random graph
...
***REMOVED******REMOVED******REMOVED*** 2.2 Attack Tolerance vs Random Failure
...
def compare_attack_tolerance(
    ...
    """
    ***REMOVED*** Run percolation for each strategy
    ...
    ***REMOVED*** Calculate robustness (area under curve)
    ...
    ***REMOVED*** Determine network type
    if V_degree > 2.0:
...
***REMOVED******REMOVED******REMOVED*** 2.3 k-Core Decomposition
...
def analyze_k_core_structure(
    ...
    """
    ***REMOVED*** Calculate core numbers for each node
    core_numbers = nx.core_number(G)

    ***REMOVED*** Find maximum k
    k_max = max(core_numbers.values()) if core_numbers else 0

    ***REMOVED*** Identify nodes in each k-shell
    k_shells = {}
...
def identify_critical_core_nodes(
    ...
    ***REMOVED*** Calculate critical score
    critical_scores = {}
    for node in G.nodes():
...
***REMOVED******REMOVED*** 3. Centrality Measures

***REMOVED******REMOVED******REMOVED*** 3.1 Comprehensive Centrality Comparison
...
def calculate_all_centrality_measures(
    ...
    ***REMOVED*** 1. Degree Centrality (already implemented)
    ***REMOVED*** Measures: Direct connections
    centrality_results['degree'] = nx.degree_centrality(G)

    ***REMOVED*** 2. Betweenness Centrality (already implemented)
    ***REMOVED*** Measures: Bottleneck/bridge importance
    centrality_results['betweenness'] = nx.betweenness_centrality(G)

    ***REMOVED*** 3. Closeness Centrality
    ***REMOVED*** Measures: Average distance to all other nodes
    ***REMOVED*** High = can quickly reach everyone
    centrality_results['closeness'] = nx.closeness_centrality(G)

    ***REMOVED*** 4. Eigenvector Centrality (already implemented)
    ***REMOVED*** Measures: Importance of neighbors
    try:
    ...
    ***REMOVED*** 5. PageRank (already implemented)
    ***REMOVED*** Measures: Probability of random walk visit
    centrality_results['pagerank'] = nx.pagerank(G)

    ***REMOVED*** 6. Katz Centrality
    ***REMOVED*** Measures: Influence via all paths (weighted by distance)
    ***REMOVED*** More stable than eigenvector for some topologies
    try:
        alpha = 1 / (2 * nx.adjacency_spectrum(G)[0])  ***REMOVED*** Ensure convergence
        centrality_results['katz'] = nx.katz_centrality(G, alpha=alpha * 0.9)
    except:
        centrality_results['katz'] = {n: 0.0 for n in G.nodes()}

    ***REMOVED*** 7. Harmonic Centrality
    ***REMOVED*** Measures: Sum of inverse distances
    ***REMOVED*** Handles disconnected components better than closeness
    centrality_results['harmonic'] = nx.harmonic_centrality(G)

    ***REMOVED*** 8. Load Centrality
    ***REMOVED*** Measures: Fraction of shortest paths through node (variant of betweenness)
    centrality_results['load'] = nx.load_centrality(G)

    ***REMOVED*** 9. Current Flow Betweenness
    ***REMOVED*** Measures: Expected flow if network were electrical circuit
    ***REMOVED*** Better for networks with multiple paths
    if G.number_of_nodes() < 100:  ***REMOVED*** Computationally expensive
        try:
    ...
    ***REMOVED*** 10. Communicability Centrality
    ***REMOVED*** Measures: Weighted sum of all walks (not just paths)
    ***REMOVED*** Accounts for redundant connections
...
def rank_nodes_by_centrality_consensus(
    ...
    """
    if weights is None:
        ***REMOVED*** Equal weights
        weights = {k: 1.0 for k in centrality_results.keys()}

    ***REMOVED*** Normalize each centrality measure to [0, 1]
    normalized = {}
    ...
    ***REMOVED*** Calculate weighted consensus
    all_nodes = set()
    ...
    ***REMOVED*** Sort by consensus score
...
***REMOVED******REMOVED*** 4. Community Detection

***REMOVED******REMOVED******REMOVED*** 4.1 Louvain Algorithm
...
def detect_communities_louvain(
    ...
    ***REMOVED*** Analyze community properties
    community_stats = []
    for i, comm in enumerate(communities):
    ...
    ***REMOVED*** Inter-community edges (bridges)
...
def compare_community_algorithms(
    ...
    ***REMOVED*** 4. Girvan-Newman (only if network is small, O(m²n) complexity)
    if G.number_of_nodes() < 50:
        gn_iterator = community.girvan_newman(G)
        ***REMOVED*** Get first level
    ...
    ***REMOVED*** Find consensus communities
    ***REMOVED*** (Use Louvain as baseline, compute similarity with others)
    best_algorithm = max(
        results.items(),
...
***REMOVED******REMOVED******REMOVED*** 4.2 Cross-Rotation Community Overlap
...
def analyze_community_stability_over_time(
    ...
    ***REMOVED*** Detect communities for each snapshot
    community_sequences = []
    for G in G_snapshots:
        communities = list(nx.community.louvain_communities(G, seed=42))
        ***REMOVED*** Convert to node->community mapping
        node_to_comm = {}
        for i, comm in enumerate(communities):
            for node in comm:
                node_to_comm[node] = i
...
***REMOVED******REMOVED*** 5. Dynamic Networks

***REMOVED******REMOVED******REMOVED*** 5.1 Temporal Network Evolution
...
def build_temporal_network(
    ...
    """
    ***REMOVED*** Convert to DataFrame for easier manipulation
    ...
    ***REMOVED*** Create snapshots
    ...
    while current <= end_date:
        ***REMOVED*** Get assignments in window [current, current + window]
        window_end = current + timedelta(days=time_window_days)
        window_df = df[(df['date'] >= current) & (df['date'] < window_end)]

        ***REMOVED*** Build bipartite graph: faculty <-> services
        G = nx.Graph()
        ...
        ***REMOVED*** Advance window
        current += timedelta(days=7)  ***REMOVED*** Weekly snapshots
...
def analyze_temporal_evolution(
    ...
    ***REMOVED*** Track metrics over time
    densities = []
    avg_clusterings = []
    avg_degrees = []
    num_components = []
    ...
    ***REMOVED*** Linear regression for trends
...
***REMOVED******REMOVED******REMOVED*** 5.2 Link Prediction
...
def predict_future_coverage_gaps(
    ...
    ***REMOVED*** Calculate multiple predictors
    ...
    for u, v in non_edges:
        ...
        ***REMOVED*** Ensemble score (weighted average)
        score = (
...
def detect_emerging_coverage_patterns(
    ...
    ***REMOVED*** Find edges that appeared in recent windows
    emerging_edges = [
        (u, v, freq)
        for (u, v), freq in edge_frequency.items()
        if freq >= lookback_windows // 2  ***REMOVED*** Appeared in at least half of windows
    ]

    ***REMOVED*** Identify emerging hubs (nodes with increasing degree)
    degree_trends = {}
    for G, ts in recent_snapshots:
    ...
    for node, degrees in degree_trends.items():
        if len(degrees) >= 2:
            ...
            if trend > 0.5:  ***REMOVED*** Increasing
...
***REMOVED******REMOVED*** 6. NetworkX Integration

***REMOVED******REMOVED******REMOVED*** 6.1 Production-Ready Patterns
...
class ScheduleNetworkAnalyzer:
    ...
    def run_comprehensive_analysis(self) -> Dict[str, any]:
        ...
        ***REMOVED*** 4. Communities
        report["communities"] = detect_communities_louvain(self.G)

        ***REMOVED*** 5. Cache for future queries
    ...
    def get_resilience_score(self) -> float:
        ...
        ***REMOVED*** Weight factors
        scores = []

        ***REMOVED*** Small-world coefficient (higher = better)
        sigma = self.analysis_cache["topology"]["small_world"].get("small_world_sigma", 0)
        scores.append(min(100, sigma * 30))  ***REMOVED*** Cap at 100

        ***REMOVED*** Modularity (higher = better)
        modularity = self.analysis_cache["communities"].get("modularity", 0)
        scores.append(modularity * 100)

        ***REMOVED*** Attack tolerance (lower vulnerability = better)
        ...
        ***REMOVED*** Density (moderate is best)
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        ***REMOVED*** Check hub vulnerability
        ...
        ***REMOVED*** Check community structure
...
***REMOVED******REMOVED******REMOVED*** 6.2 Performance Optimization
...
def optimize_network_analysis_for_large_graphs(
    ...
    ***REMOVED*** Community detection (usually fast)
...
***REMOVED******REMOVED******REMOVED*** 6.3 Visualization Patterns
...
def visualize_network_with_communities(
    ...
    ***REMOVED*** Assign colors to communities
    color_map = {}
...
***REMOVED******REMOVED******REMOVED*** Foundational Papers
...
***REMOVED******REMOVED******REMOVED*** Network Science Textbooks
...
***REMOVED******REMOVED******REMOVED*** Software and Tools
...
***REMOVED******REMOVED******REMOVED*** Medical Scheduling Applications
...
***REMOVED******REMOVED******REMOVED*** When to Use Each Analysis
...
***REMOVED******REMOVED******REMOVED*** Performance Guidelines
...
- [Burnout Epidemiology Implementation](../architecture/cross-disciplinary-resilience.md***REMOVED***3-burnout-epidemiology-sir-models)
