***REMOVED*** Exotic Control Theory for Residency Scheduling and Resilience Systems
...
***REMOVED******REMOVED*** 1. PID Controllers (Proportional-Integral-Derivative Control)

***REMOVED******REMOVED******REMOVED*** Core Principle
...
***REMOVED******REMOVED******REMOVED*** Application to Residency Scheduling

***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: Faculty Utilization Control
...
class UtilizationPIDController:
    ...
    def __init__(
        self,
        setpoint: float = 0.75,
        Kp: float = 10.0,    ***REMOVED*** Strong immediate response
        Ki: float = 0.5,     ***REMOVED*** Slow integral windup
        Kd: float = 2.0,     ***REMOVED*** Moderate damping
        dt: float = 1.0,     ***REMOVED*** Daily sampling
    ):
        ...
        ***REMOVED*** Anti-windup limits
        self.integral_max = 5.0
        self.integral_min = -5.0

        ***REMOVED*** History for analysis
    ...
    def update(self, current_utilization: float) -> dict:
        ...
        ***REMOVED*** Interpret control signal
    ...
    def _interpret_control_signal(self, signal: float, current_util: float) -> dict:
        ...
        """
        ***REMOVED*** Scale signal to assignment changes
        ***REMOVED*** Assume each assignment changes utilization by ~0.02
        ...
        if abs(signal) < 0.1:
            ...
            ***REMOVED*** Overutilized - remove assignments
            return {
                "type": "reduce_load",
            ...
            ***REMOVED*** Underutilized - can add more
            return {
                "type": "increase_load",
    ...
    def _detect_oscillation(self, window: int = 6) -> bool:
        ...
        ***REMOVED*** Oscillating if >50% sign changes
    ...
    def tune_ziegler_nichols(self, ultimate_gain: float, ultimate_period: float):
        ...
        """
        ***REMOVED*** Classic Ziegler-Nichols PID tuning
        self.Kp = 0.6 * ultimate_gain
        self.Ki = 1.2 * ultimate_gain / ultimate_period
        self.Kd = 0.075 * ultimate_gain * ultimate_period
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Workload Balance Control
...
class WorkloadBalancePIDController:
    ...
    def calculate_redistribution(
        ...
        ***REMOVED*** Convert to transfers
        ***REMOVED*** Negative control = need more balance (higher variance than setpoint)
    ...
    def _generate_transfers(
        ...
        ***REMOVED*** Identify overloaded and underloaded
        ...
        overloaded.sort(key=lambda x: -x[1])  ***REMOVED*** Highest first
        underloaded.sort(key=lambda x: x[1])   ***REMOVED*** Lowest first
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 3: Coverage Rate PID Control
...
class CoveragePIDIntegration:
    ...
    def __init__(self, homeostasis_monitor: HomeostasisMonitor):
        ...
        ***REMOVED*** Create PID controllers for each setpoint
        self.pid_controllers: dict[str, UtilizationPIDController] = {}

        ***REMOVED*** Initialize PIDs for critical setpoints
    ...
    def update_with_pid(self, current_values: dict[str, float]) -> dict:
        ...
        """
        ***REMOVED*** Run standard homeostasis check
        corrections = self.homeostasis.check_all_loops(current_values)

        ***REMOVED*** Run PID controllers
        pid_actions = {}
        ...
        ***REMOVED*** Combine recommendations
        return {
    ...
    def _get_proportional_gain(self, setpoint: Setpoint) -> float:
        ...
        ***REMOVED*** Critical setpoints need stronger immediate response
        if setpoint.is_critical:
            return 15.0
        ***REMOVED*** Tolerance determines sensitivity
    ...
    def _get_integral_gain(self, setpoint: Setpoint) -> float:
        ...
        ***REMOVED*** Critical setpoints accumulate error faster
        if setpoint.is_critical:
            return 1.0
        return 0.3
    ...
    def _get_derivative_gain(self, setpoint: Setpoint) -> float:
        ...
        ***REMOVED*** More damping for volatile metrics
        ...
        ***REMOVED*** Prioritize by urgency
        ...
        for setpoint_name, pid_result in pid_actions.items():
            ...
            if action["type"] != "maintain":
                ***REMOVED*** Add PID-based fine-tuning
                recommendations.append({
                    "source": "pid",
...
***REMOVED******REMOVED******REMOVED*** Key Parameters for Tuning
...
```python
***REMOVED*** File: backend/app/resilience/pid_control.py
...
class PIDControllerBank:
    ...
    def _initialize_default_controllers(self):
        ...
        ***REMOVED*** Utilization controller
        ...
        ***REMOVED*** Workload balance controller
        self.controllers["workload_balance"] = PIDState(
            id=uuid4(),
            name="Workload Balance PID",
            setpoint=0.15,  ***REMOVED*** Target std dev
            Kp=5.0,
            Ki=0.3,
            Kd=1.0,
        )

        ***REMOVED*** Coverage rate controller
    ...
    def update(self, process_variable: str, current_value: float) -> dict:
        ...
        now = datetime.now()
        dt = (now - controller.previous_time).total_seconds() / 86400  ***REMOVED*** Days
        ...
        ***REMOVED*** I term with anti-windup
...
***REMOVED******REMOVED*** 2. Kalman Filters (Optimal State Estimation Under Uncertainty)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: True Workload Estimation
...
class WorkloadKalmanFilter:
    ...
    def __init__(self, initial_hours: float = 60.0):
        ***REMOVED*** State: [hours, trend, seasonal]
        self.x = np.array([initial_hours, 0.0, 0.0])

        ***REMOVED*** State covariance (uncertainty in estimate)
        self.P = np.array([
            [25.0, 0.0, 0.0],   ***REMOVED*** High initial uncertainty in hours
            [0.0, 1.0, 0.0],    ***REMOVED*** Moderate uncertainty in trend
            [0.0, 0.0, 4.0],    ***REMOVED*** Moderate uncertainty in seasonal
        ])

        ***REMOVED*** State transition matrix (how state evolves)
        self.F = np.array([
            [1.0, 1.0, 1.0],   ***REMOVED*** hours += trend + seasonal
            [0.0, 0.95, 0.0],  ***REMOVED*** trend decays 5% per week
            [0.0, 0.0, 0.98],  ***REMOVED*** seasonal decays 2% per week
        ])

        ***REMOVED*** Process noise (model uncertainty)
        self.Q = np.array([
            [4.0, 0.0, 0.0],   ***REMOVED*** Hours vary randomly
            [0.0, 0.5, 0.0],   ***REMOVED*** Trend changes
            [0.0, 0.0, 1.0],   ***REMOVED*** Seasonal component varies
        ])

        ***REMOVED*** Measurement matrix (what we observe)
        self.H = np.array([
            [1.0, 0.0, 0.0],   ***REMOVED*** Self-report measures hours directly
            [1.0, 0.0, 0.0],   ***REMOVED*** System logs measure hours
            [1.0, 0.0, 0.0],   ***REMOVED*** Peer estimate measures hours
        ])

        ***REMOVED*** Measurement noise (sensor uncertainty)
        self.R = np.array([
            [16.0, 0.0, 0.0],   ***REMOVED*** Self-report: ±4 hours std dev
            [9.0, 0.0, 0.0],    ***REMOVED*** System logs: ±3 hours (more accurate)
            [25.0, 0.0, 0.0],   ***REMOVED*** Peer estimate: ±5 hours (less reliable)
        ])
    ...
    def predict(self) -> np.ndarray:
        ...
        """
        ***REMOVED*** Predict state
        x_pred = self.F @ self.x

        ***REMOVED*** Predict covariance
    ...
    def update(self, measurements: dict) -> dict:
        ...
        """
        ***REMOVED*** Prediction step
        x_pred, P_pred = self.predict()

        ***REMOVED*** Build measurement vector and observation matrix
        z_list = []
        H_list = []
        R_list = []
        ...
        if not z_list:
            ***REMOVED*** No measurements - just use prediction
            self.x = x_pred
            self.P = P_pred
            innovation = None
            kalman_gain = None
        else:
            ***REMOVED*** Have measurements - update
            z = np.array(z_list)
            H_obs = np.array(H_list)
            R_obs = np.diag(R_list)

            ***REMOVED*** Innovation (measurement residual)
        ...
        ***REMOVED*** Record history
        result = {
            ...
            "trend": self.x[1],
            "seasonal": self.x[2],
            "uncertainty": np.sqrt(self.P[0,0]),  ***REMOVED*** Standard deviation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Schedule Health State Estimation
...
class ScheduleHealthEKF:
    ...
    State Vector:
    x = [
        overall_health,      ***REMOVED*** Latent health score (0-1)
        coverage_quality,    ***REMOVED*** True coverage quality
        workload_stress,     ***REMOVED*** Underlying stress level
<<<<<<< HEAD
        morale,              ***REMOVED*** Faculty morale
=======
...
>>>>>>> origin/docs/session-14-summary
    ...
    Measurements:
    z = [
        coverage_rate,       ***REMOVED*** Observed coverage
        reported_stress,     ***REMOVED*** Self-reported stress
        satisfaction_score,  ***REMOVED*** Survey results
        swap_request_rate,   ***REMOVED*** Behavioral indicator
    ]
    """

    def __init__(self):
        ***REMOVED*** Initial state estimate (assume healthy start)
        self.x = np.array([0.8, 0.9, 0.3, 0.7])

        ***REMOVED*** Initial uncertainty
        self.P = np.eye(4) * 0.1
    ...
    def predict(self, dt: float = 1.0) -> tuple:
        ...
        """
        ***REMOVED*** Nonlinear state transition
        health = self.x[0]
        coverage = self.x[1]
        stress = self.x[2]
        morale = self.x[3]

        ***REMOVED*** Dynamics (simplified nonlinear model)
        health_next = 0.7 * health + 0.2 * coverage - 0.1 * stress
        coverage_next = 0.95 * coverage  ***REMOVED*** Slow degradation
        stress_next = 0.9 * stress + 0.1 * (1.0 - coverage)  ***REMOVED*** Accumulates from gaps
        morale_next = 0.8 * morale + 0.2 * health  ***REMOVED*** Follows health with lag

        x_pred = np.array([health_next, coverage_next, stress_next, morale_next])

        ***REMOVED*** Jacobian for linearization (EKF)
        F = np.array([
            [0.7, 0.2, -0.1, 0.0],
            [0.0, 0.95, 0.0, 0.0],
            [0.0, -0.1, 0.9, 0.0],
    ...
    def update(self, measurements: dict) -> dict:
        ...
        ***REMOVED*** Measurement function (nonlinear mapping from state to observations)
        def h(x):
            """Map state to expected measurements."""
            health, coverage, stress, morale = x
            return np.array([
                coverage,              ***REMOVED*** Coverage observed directly
                stress,                ***REMOVED*** Stress reported (with bias)
                morale * 0.8 + 0.2,    ***REMOVED*** Satisfaction correlates with morale
                stress * 1.5,          ***REMOVED*** Swap requests increase with stress
            ])

        ***REMOVED*** Measurement Jacobian
        H = np.array([
            [0.0, 1.0, 0.0, 0.0],      ***REMOVED*** coverage
            [0.0, 0.0, 1.0, 0.0],      ***REMOVED*** stress
            [0.0, 0.0, 0.0, 0.8],      ***REMOVED*** satisfaction
            [0.0, 0.0, 1.5, 0.0],      ***REMOVED*** swap rate
        ])

        ***REMOVED*** Build measurement vector (handle missing data)
        z_list = []
        H_rows = []
        R_diag = []
...
***REMOVED******REMOVED*** 3. Model Predictive Control (MPC) / Receding Horizon Optimization
...
subject to:
    x(k+1) = f(x(k), u(k))           ***REMOVED*** System dynamics
    x_min ≤ x(k) ≤ x_max             ***REMOVED*** State constraints
    u_min ≤ u(k) ≤ u_max             ***REMOVED*** Control constraints
    g(x(k), u(k)) ≤ 0                ***REMOVED*** General constraints
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: Schedule Generation as MPC
...
class MPCScheduler:
    ...
    def __init__(
        ...
        self.horizon_weeks = horizon_weeks  ***REMOVED*** How far ahead to predict
        self.control_horizon_weeks = control_horizon_weeks  ***REMOVED*** How many weeks to optimize

        ***REMOVED*** Objective function weights
        self.weights = objective_weights or {
            'coverage': 100.0,          ***REMOVED*** Must maintain coverage
            'utilization': 50.0,        ***REMOVED*** Keep utilization near target
            'workload_balance': 30.0,   ***REMOVED*** Balance workload
            'preference': 20.0,         ***REMOVED*** Honor preferences
            'continuity': 15.0,         ***REMOVED*** Maintain continuity
            'setup_cost': 10.0,         ***REMOVED*** Minimize changes
        }

        ***REMOVED*** Model of system dynamics
    ...
    def generate_schedule(
        ...
        while current_date < end_date:
            ***REMOVED*** Define current horizon
            horizon_end = min(
                current_date + timedelta(weeks=self.horizon_weeks),
                end_date
            )

            ***REMOVED*** Solve MPC problem for this horizon
            ...
            ***REMOVED*** Extract assignments for control horizon only (rest is discarded)
            ...
            ***REMOVED*** Update state with executed assignments
            current_state = self.state_model.update(current_state, executed_assignments)

            ***REMOVED*** Move horizon forward
    ...
    def _solve_mpc_step(
        ...
        ***REMOVED*** Build multi-objective function
        objective_terms = []

        ***REMOVED*** 1. Coverage objective
        ...
        ***REMOVED*** 2. Utilization objective (keep near target over horizon)
        ...
        ***REMOVED*** 3. Workload balance objective
        ...
        ***REMOVED*** 4. Preference objective (integrate stigmergy trails)
        ...
        ***REMOVED*** 5. Continuity objective (prefer stable assignments)
        ...
        ***REMOVED*** Set objective
        model.Maximize(sum(objective_terms))

        ***REMOVED*** Solve
        solver = cp_model.CpSolver()
        solver.parameters.max_time_in_seconds = 30.0  ***REMOVED*** Fast solve for real-time MPC
        status = solver.Solve(model)

        if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            ***REMOVED*** Extract assignments
            assignments = self._extract_assignments(solver, variables, residents, blocks, templates)

            ***REMOVED*** Predict end state
            ...
            ***REMOVED*** Return empty or fallback
    ...
    def _create_utilization_objective(self, model, variables, residents, blocks, current_state):
        ...
        from the 75% target.
        """
        ***REMOVED*** Predict utilization at each week in horizon
        weeks = self._group_blocks_by_week(blocks)
        penalty_vars = []

        for week_blocks in weeks:
            ***REMOVED*** Total assignments in this week
            week_assignments = [
    ...
    def _create_preference_objective(self, model, variables, residents, blocks, templates):
        ...
        ***REMOVED*** Get stigmergy preference data
        stigmergy = self.state_model.get_stigmergy_scheduler()

        bonus_vars = []

        for resident in residents:
            ***REMOVED*** Get preference trails for this resident
            ...
            ***REMOVED*** Create preference variables
            for block in blocks:
                for template in templates:
                    ...
                    ***REMOVED*** Check if this assignment matches preferences
                    pref_strength = 0.0
                    ...
                    ***REMOVED*** Net preference (positive = preferred, negative = avoided)
                    net_pref = pref_strength - avoid_strength

                    ***REMOVED*** Convert to integer for CP-SAT
                    pref_score = int(net_pref * 100)

                    ***REMOVED*** Preference bonus
...
class ScheduleStateModel:
    ...
    def predict(self, current_state: dict, planned_assignments: list, end_date: date) -> dict:
        ...
        ***REMOVED*** Start from current
        predicted_state = current_state.copy()

        ***REMOVED*** Apply assignments
        predicted_state = self.update(predicted_state, planned_assignments)

        ***REMOVED*** Model natural dynamics (stress decay, etc.)
        days_ahead = (end_date - datetime.now().date()).days
        stress_decay = 0.95 ** (days_ahead / 7)  ***REMOVED*** Decay per week
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Real-Time Schedule Adjustment
...
class EmergencyMPCReplanner:
    ...
    def replan_after_absence(
        ...
        """
        ***REMOVED*** Filter out assignments for absent faculty
        valid_assignments = [
        ...
        ***REMOVED*** Run MPC to fill gaps
        ***REMOVED*** Add penalty for changing existing assignments (stability)
        self.mpc.weights['setup_cost'] = 100.0  ***REMOVED*** High penalty for changes

        new_assignments = self.mpc.generate_schedule(
            start_date=absence_start,
            end_date=absence_end + timedelta(days=7),  ***REMOVED*** Extra week buffer
...
***REMOVED******REMOVED*** 4. Adaptive Control (Self-Tuning Systems)
...
```
e(t) = y(t) - y_m(t)    ***REMOVED*** Error between actual and reference model

θ̇ = -Γ·e·φ             ***REMOVED*** Parameter update law (gradient descent)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: Self-Tuning Constraint Weights
...
class AdaptiveConstraintWeights:
    ...
    def __init__(
        ...
        self.reference = reference_metrics  ***REMOVED*** Ideal target metrics
        self.learning_rate = learning_rate

        ***REMOVED*** History for learning
        self.weight_history: list[dict] = []
        self.performance_history: list[dict] = []

        ***REMOVED*** Adaptation gains (different for each constraint type)
        self.gains = {
            'coverage': 0.5,        ***REMOVED*** Adapt quickly - critical
            'utilization': 0.3,     ***REMOVED*** Moderate adaptation
            'preferences': 0.1,     ***REMOVED*** Adapt slowly - less critical
    ...
    def adapt_weights(self, schedule_metrics: dict) -> dict:
        ...
        ***REMOVED*** Update weights using gradient descent
        for constraint_name, weight in self.weights.items():
            ...
            ***REMOVED*** Gradient: if error is positive (underperforming), increase weight
            ***REMOVED*** If error is negative (overperforming), decrease weight
            delta = self.learning_rate * gain * error

            ***REMOVED*** Update with bounds
    ...
    def _check_convergence(self, window: int = 5, threshold: float = 0.05) -> bool:
        ...
        ***REMOVED*** Check variance of each weight
        for constraint_name in self.weights.keys():
            ...
            ***REMOVED*** Coefficient of variation
            cv = variance / mean if mean > 0 else 0

            if cv > threshold:
                return False  ***REMOVED*** Still changing significantly

        return True  ***REMOVED*** All weights stable
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Adaptive PID Tuning
...
class AdaptivePIDController:
    ...
    def __init__(self, setpoint: float):
        ...
        ***REMOVED*** Operating regimes with pre-tuned gains
        self.gain_schedules = {
            'normal': {'Kp': 10.0, 'Ki': 0.5, 'Kd': 2.0},
            'stressed': {'Kp': 15.0, 'Ki': 1.0, 'Kd': 3.0},  ***REMOVED*** More aggressive
            'crisis': {'Kp': 20.0, 'Ki': 2.0, 'Kd': 1.0},     ***REMOVED*** Fast response, less damping
            'recovery': {'Kp': 5.0, 'Ki': 0.3, 'Kd': 4.0},    ***REMOVED*** Gentle, avoid overshoot
        }

        ***REMOVED*** Current gains (start with normal)
        self.gains = self.gain_schedules['normal'].copy()
        self.current_regime = 'normal'

        ***REMOVED*** Self-tuning parameters
        ...
        ***REMOVED*** Performance tracking
        self.ise = 0.0  ***REMOVED*** Integral squared error
    ...
    def _detect_regime(self, system_state: dict) -> str:
        ...
        ***REMOVED*** Crisis: high utilization, low coverage, or active stresses
        if utilization > 0.85 or coverage < 0.80 or stresses > 2:
            return 'crisis'

        ***REMOVED*** Recovery: recently exited crisis
        if days_since_crisis < 14:
            return 'recovery'

        ***REMOVED*** Stressed: approaching limits
        if utilization > 0.75 or coverage < 0.90:
            return 'stressed'

        ***REMOVED*** Normal: healthy operation
        return 'normal'
    ...
    def _self_tune_gains(self, error: float, derivative: float):
        ...
        """
        ***REMOVED*** Calculate recent performance
        recent_error_mean = statistics.mean(self.performance_window)

        ***REMOVED*** MIT rule: adjust proportional gain based on error magnitude
        if abs(error) > 0.1:  ***REMOVED*** Only adapt if error is significant
            ***REMOVED*** If error is large and positive, need stronger response
            kp_adjustment = self.adaptation_rate * error * error
            self.gains['Kp'] = max(1.0, min(30.0, self.gains['Kp'] + kp_adjustment))

        ***REMOVED*** Adjust integral gain based on steady-state error
        if len(self.performance_window) >= 20:
        ...
        ***REMOVED*** Adjust derivative gain based on oscillation
        oscillation = self._detect_oscillation_magnitude()
        if oscillation > 0.3:  ***REMOVED*** High oscillation
            ***REMOVED*** Increase damping
            self.gains['Kd'] *= 1.1
        elif oscillation < 0.1:  ***REMOVED*** Very stable
            ***REMOVED*** Can reduce damping for faster response
            self.gains['Kd'] *= 0.95

        ***REMOVED*** Clamp all gains to safe ranges
...
***REMOVED******REMOVED*** 5. Sliding Mode Control (Robust Control Under Uncertainty)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: Robust Coverage Maintenance
...
class SlidingModeCoverageController:
    ...
    def __init__(
        ...
        lambda_param: float = 2.0,    ***REMOVED*** Sliding surface slope
        switching_gain: float = 5.0,   ***REMOVED*** Must overcome max disturbance
        boundary_layer: float = 0.02,  ***REMOVED*** Smooth chattering
    ):
        self.target = target_coverage
        self.lambda_param = lambda_param
        self.K = switching_gain
        self.phi = boundary_layer  ***REMOVED*** Boundary layer thickness

        ***REMOVED*** State
        self.previous_error = 0.0
        self.previous_time = datetime.now()

        ***REMOVED*** Chattering detection
    ...
    def update(self, current_coverage: float, dt: float = 1.0) -> dict:
        ...
        """
        ***REMOVED*** Tracking error
        error = self.target - current_coverage

        ***REMOVED*** Error derivative (finite difference)
        error_dot = (error - self.previous_error) / dt if dt > 0 else 0.0

        ***REMOVED*** Sliding surface
        s = self.lambda_param * error + error_dot

        ***REMOVED*** Equivalent control (linear part, maintains sliding)
        ***REMOVED*** For simple first-order system: u_eq = -λ·e
        u_eq = -self.lambda_param * error

        ***REMOVED*** Switching control (nonlinear part, drives to surface)
        ***REMOVED*** Use smooth sat() function instead of sign() to reduce chattering
        u_sw = -self.K * self._smooth_sat(s, self.phi)

        ***REMOVED*** Total control
        control_signal = u_eq + u_sw

        ***REMOVED*** Interpret control (how many assignments to add/remove)
        action = self._interpret_control(control_signal, current_coverage)

        ***REMOVED*** Detect chattering
        if abs(s) < self.phi:
            chattering = False
        else:
            chattering = True
    ...
    def _interpret_control(self, signal: float, current_coverage: float) -> dict:
        ...
        """
        ***REMOVED*** Scale signal to assignments (each assignment ≈ 0.01 coverage)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Crisis Mode Sliding Surface
...
class CrisisModeSlidingController:
    ...
    def __init__(self):
        ***REMOVED*** Primary surface (CRITICAL): Minimum coverage
        self.s1_controller = SlidingModeCoverageController(
            target_coverage=0.85,  ***REMOVED*** Lower target for crisis
            lambda_param=3.0,      ***REMOVED*** Fast convergence
            switching_gain=10.0,   ***REMOVED*** Strong control
        )

        ***REMOVED*** Secondary surface (HIGH): Maximum utilization
        self.s2_controller = SlidingModeUtilizationController(
            target_utilization=0.80,  ***REMOVED*** Hard limit
            lambda_param=2.0,
            switching_gain=7.0,
        )

        ***REMOVED*** Tertiary surface (MEDIUM): Workload balance
        self.s3_controller = SlidingModeBalanceController(
            target_std=0.20,  ***REMOVED*** Relaxed during crisis
            lambda_param=1.0,
            switching_gain=3.0,
        )
    ...
    def crisis_update(
        ...
        """
        ***REMOVED*** Primary: Coverage (must satisfy first)
        s1_result = self.s1_controller.update(coverage)

        ***REMOVED*** Check if on primary surface
        if not s1_result['on_sliding_surface']:
            ***REMOVED*** Not on primary surface - focus only on that
            return {
                'priority': 'CRITICAL',
                'focus': 'coverage',
        ...
        ***REMOVED*** On both primary and secondary - optimize tertiary
...
***REMOVED******REMOVED*** 6. Smith Predictor (Handling Time Delays in Feedback)
...
y(t) = G(s)·u(t - τ)     ***REMOVED*** Output delayed by τ
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 1: Schedule Quality Feedback Delay
...
class ScheduleQualitySmithPredictor:
    ...
    def __init__(
        self,
        delay_weeks: int = 3,    ***REMOVED*** How long until quality is measurable
    ...
    def update(
        ...
        ***REMOVED*** 4. Corrected estimate (Smith Predictor formula)
        ***REMOVED*** y_corrected = y_predicted + [y_delayed - y_predicted_delayed]
        corrected_quality = current_prediction + correction

        ***REMOVED*** 5. Use corrected estimate for control
        control_result = self.controller.update(corrected_quality)

        ***REMOVED*** 6. Store history
    ...
    def _default_quality_model(self, params: dict) -> float:
        ...
        ***REMOVED*** Utilization impact (optimal at 0.75)
        util_penalty = abs(util - 0.75) * 2.0
        quality -= util_penalty

        ***REMOVED*** Variance impact (lower is better)
        ...
        ***REMOVED*** Continuous days penalty (>5 days is tiring)
        if continuous > 5:
            continuous_penalty = (continuous - 5) * 0.05
            quality -= continuous_penalty

        ***REMOVED*** Clamp to [0, 1]
    ...
    def _calculate_model_accuracy(self) -> dict:
        ...
        return {
            ...
            'bias': bias,  ***REMOVED*** Systematic over/under prediction
            'rmse': rmse,
    ...
    def improve_model(self):
        ...
        ***REMOVED*** Analyze systematic errors
        ...
        if accuracy['bias'] > 0.1:
            ...
            ***REMOVED*** Could adjust model parameters here
        elif accuracy['bias'] < -0.1:
            logger.info(f"Model is over-predicting by {abs(accuracy['bias']):.3f} on average")
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Use Case 2: Delayed ACGME Violations
...
class ACGMESmithPredictor:
    ...
    def __init__(self):
        self.delay_weeks = 4  ***REMOVED*** ACGME measurement window

        ***REMOVED*** Model of how hours accumulate
        self.hours_model = HoursAccumulationModel()

        ***REMOVED*** Delayed compliance measurements
        self.delayed_compliance: deque = deque(maxlen=10)

        ***REMOVED*** Predictor state
    ...
    def predict_compliance(
        ...
        ***REMOVED*** If we predict violation, recommend action NOW
        if will_violate:
            action = {
        ...
        ***REMOVED*** Store prediction for later validation
    ...
    def validate_prediction(
        ...
        """
        ***REMOVED*** Find prediction for this window
        for pred in self.predicted_windows:
            if (pred['faculty_id'] == faculty_id and
                ...
                ***REMOVED*** Update model with this error
                self.hours_model.learn_from_error(error)

                ***REMOVED*** Record for statistics
...
class HoursAccumulationModel:
    ...
    def __init__(self):
        ***REMOVED*** Historical average hours by rotation type
        self.rotation_hours = {
            'clinic': 8.0,
            'inpatient': 12.0,
            'procedure': 10.0,
        ...
        ***REMOVED*** Variance (for uncertainty quantification)
        self.rotation_std = {
            'clinic': 1.0,
            'inpatient': 2.5,
            'procedure': 2.0,
        ...
        ***REMOVED*** Model confidence (improves with more data)
    ...
    def get_confidence(self) -> float:
        ...
        if self.prediction_count < 5:
            return 0.5  ***REMOVED*** Low confidence initially

        ***REMOVED*** Confidence based on recent errors
        if len(self.error_history) < 3:
            return 0.7

        recent_mae = statistics.mean([abs(e) for e in self.error_history[-10:]])

        ***REMOVED*** Good predictions (MAE < 5 hours) = high confidence
        if recent_mae < 5.0:
            return 0.9
    ...
    def learn_from_error(self, error: float):
        ...
        ***REMOVED*** Could do more sophisticated learning here
        ***REMOVED*** (e.g., adjust rotation_hours based on systematic bias)
```
...
***REMOVED******REMOVED*** Integration Strategy and Implementation Roadmap
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Advanced Control (Weeks 3-4)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Robustness (Weeks 5-6)
...
***REMOVED******REMOVED******REMOVED*** Database Schema Additions

```python
***REMOVED*** Add to backend/app/models/control_theory.py
...
<<<<<<< HEAD
    entity_id = Column(UUID)  ***REMOVED*** Faculty or system
=======
...
>>>>>>> origin/docs/session-14-summary
    ...
    ***REMOVED*** Estimate
    ...
    uncertainty = Column(Float)  ***REMOVED*** sqrt(P[0,0])
...
class AdaptiveWeightHistory(Base):
    ...
    ***REMOVED*** Performance that led to these weights
...
***REMOVED******REMOVED*** Comparative Analysis: When to Use Each Technique
...
***REMOVED******REMOVED******REMOVED*** Decision Tree for Selecting Control Method
...
***REMOVED******REMOVED*** Testing and Validation Plan

***REMOVED******REMOVED******REMOVED*** Unit Tests

```python
***REMOVED*** backend/tests/resilience/test_pid_control.py

def test_pid_reaches_setpoint():
    """Verify PID drives error to zero."""
    pid = UtilizationPIDController(setpoint=0.75)

    ***REMOVED*** Simulate system response
    utilization = 0.50  ***REMOVED*** Start low

    for _ in range(50):
        result = pid.update(utilization)
        ***REMOVED*** Simple plant model: utilization changes proportionally to control
        utilization += result['control_signal'] * 0.01

    ***REMOVED*** Should be close to setpoint
...
def test_kalman_reduces_noise():
    ...
    ***REMOVED*** Estimate should be smoother (lower variance) than measurements
    meas_var = np.var(measurements)
    est_var = np.var(estimates[-10:])  ***REMOVED*** Last 10 after convergence

    assert est_var < meas_var * 0.5  ***REMOVED*** At least 50% noise reduction
```
...
def test_pid_kalman_integration():
    ...
    ***REMOVED*** Simulate system with noisy measurements
    ...
    for _ in range(30):
        ...
        ***REMOVED*** Filter measurement
        filtered_result = kf.update({'self_report': measured})
        filtered_value = filtered_result['estimated_hours'] / 80.0  ***REMOVED*** Convert to utilization
    ...
    ***REMOVED*** Should converge despite noise
