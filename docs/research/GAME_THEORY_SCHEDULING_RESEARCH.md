***REMOVED*** Game Theory and Mechanism Design for Scheduling Systems
***REMOVED******REMOVED*** Research Report and Implementation Recommendations
...
1. [Nash Equilibrium in Scheduling](***REMOVED***1-nash-equilibrium-in-scheduling)
2. [Mechanism Design Fundamentals](***REMOVED***2-mechanism-design-fundamentals)
3. [Auction Theory for Shift Allocation](***REMOVED***3-auction-theory-for-shift-allocation)
4. [Cooperative Game Theory](***REMOVED***4-cooperative-game-theory)
5. [Evolutionary Game Theory](***REMOVED***5-evolutionary-game-theory)
6. [Schelling's Segregation Model](***REMOVED***6-schellings-segregation-model)
7. [Principal-Agent Problem](***REMOVED***7-principal-agent-problem)
8. [Practical Implementation Roadmap](***REMOVED***8-practical-implementation-roadmap)
9. [Integration with Existing Systems](***REMOVED***9-integration-with-existing-systems)
10. [References](***REMOVED***10-references)

---

***REMOVED******REMOVED*** 1. Nash Equilibrium in Scheduling
...
***REMOVED******REMOVED******REMOVED*** Application to Shift Scheduling

***REMOVED******REMOVED******REMOVED******REMOVED*** 1.1 Stable Schedule States
...
class NashEquilibriumAnalyzer:
    ...
    def is_schedule_stable(
        ...
        for faculty in schedule.faculty:
            ...
            ***REMOVED*** Check all possible single-swap improvements
    ...
    def calculate_nash_distance(self, schedule: Schedule) -> float:
        ...
        ***REMOVED*** Normalize by number of possible swaps
        avg_improvement = total_potential_improvement / total_possible_swaps

        ***REMOVED*** Convert to 0-1 scale (assuming max utility difference is 1.0)
        return min(avg_improvement, 1.0)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 1.2 Mixed Strategy Nash Equilibrium for Scheduling Games
...
def infer_mixed_strategy_from_trails(
    ...
    ***REMOVED*** Filter to relevant slot type
    relevant_trails = [t for t in trails if t.slot_type == slot_type]

    if not relevant_trails:
        return 0.0  ***REMOVED*** No preference data = uniform distribution

    ***REMOVED*** Aggregate trail strengths with recency weighting
    total_strength = 0.0
    total_weight = 0.0

    for trail in relevant_trails:
        ***REMOVED*** Weight by recency (exponential decay)
    ...
    ***REMOVED*** Normalize to probability
...
```python
***REMOVED*** In SwapAutoMatcher, add Nash stability check
def suggest_optimal_matches_with_stability(
    ...
    ***REMOVED*** Enhance with stability analysis
    analyzer = NashEquilibriumAnalyzer()

    for match in matches:
        ***REMOVED*** Calculate if this swap improves Nash distance
        stability_improvement = analyzer.calculate_stability_improvement(
            current_schedule=self._get_current_schedule(),
            proposed_swap=match
        )

        ***REMOVED*** Boost score for stability-improving swaps
...
***REMOVED******REMOVED*** 2. Mechanism Design Fundamentals
...
***REMOVED******REMOVED******REMOVED*** Application to Preference Elicitation

***REMOVED******REMOVED******REMOVED******REMOVED*** 2.1 Strategyproof Preference Collection
...
class StrategyproofPreferenceElicitor:
    ...
    def collect_preferences_vcg(
        ...
        """
        ***REMOVED*** Ask faculty to rank all slots (reports)
        reported_ranking = self._ask_for_ranking(faculty_id, available_slots)

        ***REMOVED*** VCG payment rule ensures truthfulness
        ***REMOVED*** Faculty pays the "externality" they impose on others
        allocation = self._vcg_allocate(reported_ranking, available_slots)

        ***REMOVED*** Calculate VCG payment (difference in others' welfare)
        payment = self._calculate_vcg_payment(faculty_id, allocation)

        ***REMOVED*** In our context, "payment" is constraint satisfaction cost
        ***REMOVED*** (e.g., working less preferred shifts, ACGME burden)
    ...
    def _vcg_allocate(
        ...
        """
        ***REMOVED*** This is a maximum weight matching problem
        ***REMOVED*** Use Hungarian algorithm or linear programming
    ...
    def _calculate_vcg_payment(
        ...
        """
        ***REMOVED*** Total welfare with this faculty
        total_with = self._calculate_social_welfare(allocation)

        ***REMOVED*** Remove this faculty and re-optimize
        ...
        ***REMOVED*** Externality = reduction in others' welfare due to your presence
        externality = total_without - (total_with - allocation[faculty_id].utility)

        return max(0, externality)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2.2 Preventing Preference Manipulation
...
class RandomSerialDictatorship:
    ...
    def allocate_shifts(
        ...
        ***REMOVED*** Randomize faculty order (uniformly)
        random_order = random.sample(faculty, len(faculty))

        for faculty_id in random_order:
            ***REMOVED*** Faculty picks their most preferred available shift
    ...
    def calculate_rsd_probabilities(
        ...
        This is ***REMOVED***P-complete in general, but can use Monte Carlo estimation
        or exact calculation for small instances.
        """
        ***REMOVED*** Monte Carlo estimation
        num_samples = 10000
        successes = 0
        ...
        for _ in range(num_samples):
            allocation = self.allocate_shifts(
                ...
                list(all_preferences.values())[0],  ***REMOVED*** All shifts
                all_preferences
            )
...
```python
***REMOVED*** In FacultyPreferenceService
def collect_preferences_strategyproof(
    ...
    """
    ***REMOVED*** Option 1: Use RSD for actual allocation
    rsd = RandomSerialDictatorship()

    ***REMOVED*** Get reported preferences (faculty has no incentive to lie)
    reported_prefs = self._ask_for_preference_ranking(faculty_id, available_weeks)

    ***REMOVED*** Store in existing FacultyPreference model
    preferences = self.get_or_create_preferences(faculty_id)

    ***REMOVED*** Convert ranking to preferred/blocked weeks
    top_k = int(len(reported_prefs) * 0.3)  ***REMOVED*** Top 30% = preferred
    bottom_k = int(len(reported_prefs) * 0.2)  ***REMOVED*** Bottom 20% = blocked
    ...
    ***REMOVED*** Record in stigmergy system for learning
    for i, week in enumerate(reported_prefs):
        strength = 1.0 - (i / len(reported_prefs))  ***REMOVED*** Linear decay
...
***REMOVED******REMOVED*** 3. Auction Theory for Shift Allocation
...
***REMOVED******REMOVED******REMOVED*** Application to Swap Markets

***REMOVED******REMOVED******REMOVED******REMOVED*** 3.1 Vickrey Auction for Priority Swaps
...
class ShiftAuctionMechanism:
    ...
    def run_vickrey_auction(
        ...
        shift: Shift,
        bidders: list[UUID],
        bids: dict[UUID, float]  ***REMOVED*** faculty_id → bid in points
    ) -> tuple[UUID, float]:
        """
        ...
        ***REMOVED*** Payment = second highest bid (Vickrey rule)
        if len(sorted_bids) > 1:
            payment = sorted_bids[1][1]
        else:
            payment = 0.0  ***REMOVED*** No competition, pay reserve price

        ***REMOVED*** Deduct points from winner
    ...
    def run_combinatorial_auction(
        ...
        """
        ***REMOVED*** Formulate as Integer Linear Program
        ***REMOVED*** Variables: x[faculty, bundle] ∈ {0, 1}
        ***REMOVED*** Objective: maximize Σ bid[faculty, bundle] * x[faculty, bundle]
        ***REMOVED*** Constraints: each shift appears in at most one winning bundle
    ...
    def _solve_wdp_ilp(
        ...
        ***REMOVED*** Create variables for each (faculty, bundle) pair
        variables = {}
        ...
        ***REMOVED*** Objective: maximize total value
        objective = solver.Objective()
        for (fid, bundle), (var, bid) in variables.items():
            objective.SetCoefficient(var, bid)
        objective.SetMaximization()

        ***REMOVED*** Constraints: each shift in at most one bundle
        for shift in shifts:
        ...
        ***REMOVED*** Solve
        ...
        if status != pywraplp.Solver.OPTIMAL:
            return {}  ***REMOVED*** No feasible solution

        ***REMOVED*** Extract winning allocations
        allocation = {}
        for (fid, bundle), (var, bid) in variables.items():
            if var.solution_value() > 0.5:  ***REMOVED*** Binary variable is 1
                allocation[fid] = set(bundle)

        return allocation
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 3.2 Double Auction for Swap Matching
...
class SwapDoubleAuction:
    ...
    def run_double_auction(
        self,
        asks: dict[UUID, tuple[Shift, float]],  ***REMOVED*** seller → (shift, ask_price)
        bids: dict[UUID, tuple[Shift, float]]   ***REMOVED*** buyer → (shift, bid_price)
        ...
        ***REMOVED*** Match for each shift type
        for shift_key in set(shifts_offered.keys()) & set(shifts_wanted.keys()):
            sellers = sorted(shifts_offered[shift_key], key=lambda x: x[1])  ***REMOVED*** Asc ask
            buyers = sorted(shifts_wanted[shift_key], key=lambda x: -x[1])   ***REMOVED*** Desc bid

            ***REMOVED*** Find equilibrium: where supply meets demand
            for i, (seller_id, ask) in enumerate(sellers):
                ...
                if bid >= ask:  ***REMOVED*** Trade possible
                    ***REMOVED*** k-Double Auction price rule (k=0.5 = median)
                    price = 0.5 * (ask + bid)
...
```python
***REMOVED*** Add to SwapAutoMatcher
class AuctionEnhancedMatcher(SwapAutoMatcher):
    ...
    def suggest_with_priority_auction(
        ...
        ***REMOVED*** Calculate "willingness to pay" from preference intensity
        source_prefs = self.preference_service.get_or_create_preferences(
            request.source_faculty_id
        )

        ***REMOVED*** If source week is blocked → high willingness to swap (high bid)
        if self.preference_service.is_week_blocked(
            request.source_faculty_id,
            request.source_week
        ):
            urgency_points = 100.0  ***REMOVED*** High priority points
        else:
            urgency_points = 10.0  ***REMOVED*** Normal priority

        ***REMOVED*** Enhance matches with auction priority
        for match in matches:
            ***REMOVED*** Calculate counter-party's ask price
            ...
            if self.preference_service.is_week_preferred(
                ...
                ask_price = 5.0  ***REMOVED*** Low ask (they want it)
            else:
                ask_price = 50.0  ***REMOVED*** High ask (reluctant)

            ***REMOVED*** Match if bid > ask
...
***REMOVED******REMOVED*** 4. Cooperative Game Theory
...
***REMOVED******REMOVED******REMOVED*** Application to Workload Distribution

***REMOVED******REMOVED******REMOVED******REMOVED*** 4.1 Shapley-Fair Workload Allocation
...
class ShapleyWorkloadAllocator:
    ...
    def calculate_shapley_values(
        ...
        ***REMOVED*** Enumerate all possible orderings (permutations)
        ***REMOVED*** For each ordering, add marginal contribution
        for perm in itertools.permutations(faculty):
            coalition = set()

            for i, faculty_id in enumerate(perm):
                ***REMOVED*** Value before adding this faculty
                value_before = capability_function(coalition)

                ***REMOVED*** Value after adding this faculty
        ...
        ***REMOVED*** Average over all permutations
    ...
    def allocate_shifts_shapley_fair(
        ...
        faculty: list[Person],
        shifts: list[Shift],
        qualifications: dict[UUID, set[str]]  ***REMOVED*** faculty → set of rotation types
        ...
        """
        ***REMOVED*** Define capability function
        def coverage_capability(coalition: set[UUID]) -> float:
            ...
            covered = 0
            for shift in shifts:
                ***REMOVED*** Check if any member can cover this shift
                can_cover = any(
        ...
        ***REMOVED*** Calculate Shapley values
        ...
        ***REMOVED*** Allocate shifts in proportion to Shapley values
        total_shapley = sum(shapley.values())

        allocation = {fid: [] for fid in faculty_ids}
        available_shifts = shifts.copy()

        ***REMOVED*** Sort faculty by Shapley value (descending)
        sorted_faculty = sorted(
            faculty_ids,
            key=lambda fid: -shapley[fid]
        )

        ***REMOVED*** Allocate using weighted random selection
        for shift in shifts:
            ***REMOVED*** Who can cover this shift?
            eligible = [
                fid for fid in sorted_faculty
                if shift.rotation_type in qualifications.get(fid, set())
                and len(allocation[fid]) < (len(shifts) / len(faculty)) * 1.5  ***REMOVED*** Load balance
            ]

            if not eligible:
                continue  ***REMOVED*** No one available

            ***REMOVED*** Select proportional to Shapley value (those who contribute more get more choice)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4.2 Core Stability for Schedule Assignments
...
def is_allocation_in_core(
    ...
    ***REMOVED*** Check all possible coalitions
    for r in range(1, n + 1):
        for coalition in itertools.combinations(faculty, r):
            coalition_set = set(coalition)

            ***REMOVED*** Current utility for coalition members
            current_utility = sum(
                utility_function(fid, allocation[fid])
                for fid in coalition_set
            )

            ***REMOVED*** Can they improve by reallocating among themselves?
            coalition_shifts = []
            for fid in coalition_set:
                coalition_shifts.extend(allocation[fid])

            ***REMOVED*** Find best reallocation within coalition
            ...
            if best_utility > current_utility:
                return False  ***REMOVED*** Coalition can improve → not in core

    return True  ***REMOVED*** No coalition can improve → in core
```

***REMOVED******REMOVED******REMOVED*** Integration with Existing System

```python
***REMOVED*** Add Shapley-based workload fairness check to resilience monitoring
class ShapleyResilienceMetric:
    ...
    def calculate_workload_fairness(
        ...
        ***REMOVED*** Get all faculty and their qualifications
        ...
        ***REMOVED*** Calculate Shapley values
        ...
        def coverage_capability(coalition: set[UUID]) -> float:
            ***REMOVED*** From resilience.py - use existing coverage metrics
        ...
        ***REMOVED*** Calculate actual workload
        actual_workload = {
            f.id: len([a for a in schedule.assignments if a.person_id == f.id])
            for f in faculty
        }

        ***REMOVED*** Compare Shapley-fair allocation vs actual
        ...
        ***REMOVED*** Calculate fairness deviation
        deviations = {
        ...
        ***REMOVED*** Fairness score (0 = perfectly fair, 1 = very unfair)
        ...
        return {
            ...
            "fairness_score": 1.0 - fairness_score,  ***REMOVED*** Convert to 0-1 where 1=fair
...
***REMOVED******REMOVED*** 5. Evolutionary Game Theory
...
***REMOVED******REMOVED******REMOVED*** Application to Preference Evolution

***REMOVED******REMOVED******REMOVED******REMOVED*** 5.1 Learning Optimal Swap Strategies
...
class EvolutionarySwapLearner:
    ...
    def __init__(self, db: Session):
        self.db = db
        self.strategy_frequencies = {}  ***REMOVED*** strategy_type → frequency
    ...
    def update_strategy_frequencies(
        ...
        ***REMOVED*** Get recent swaps
        swaps = self.db.query(SwapRecord).filter(
            SwapRecord.requested_at >= cutoff
        ).all()

        ***REMOVED*** Categorize swap strategies
        strategies = {
            "early_request": 0,      ***REMOVED*** Request >2 weeks in advance
            "late_request": 0,        ***REMOVED*** Request <1 week in advance
            "mutual_preference": 0,   ***REMOVED*** Both parties prefer swap
            "one_way": 0,            ***REMOVED*** Only one party benefits
            "emergency": 0,          ***REMOVED*** Blocked week (urgent)
            "opportunistic": 0,      ***REMOVED*** Not urgent, but preferred
        }

        success_count = {k: 0 for k in strategies.keys()}

        for swap in swaps:
            ***REMOVED*** Categorize this swap's strategy
        ...
        ***REMOVED*** Calculate fitness (success rate) for each strategy
        fitness = {}
    ...
    def suggest_optimal_strategy(
        ...
        ***REMOVED*** Filter strategies applicable to current situation
        ...
        if not applicable:
            return "one_way"  ***REMOVED*** Default

        ***REMOVED*** Select strategy proportional to frequency (evolutionary success)
    ...
    def _categorize_swap_strategy(self, swap: SwapRecord) -> str:
        ...
        ***REMOVED*** Check if it's mutual preference
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5.2 Evolutionarily Stable Scheduling Strategies
...
def find_evolutionarily_stable_strategy(
    ...
    for candidate in strategies:
        ...
        for alternative in strategies:
            ...
            ***REMOVED*** Payoff for candidate vs candidate
            u_cc = strategy_payoffs(candidate, candidate)
            ***REMOVED*** Payoff for alternative vs candidate
            u_ac = strategy_payoffs(alternative, candidate)

            ***REMOVED*** First condition: candidate must do better vs itself
            if u_ac > u_cc:
                is_ess = False
                break

            ***REMOVED*** Second condition: if tied, candidate must beat alternative
            if u_ac == u_cc:
    ...
    return None  ***REMOVED*** No ESS found
```

***REMOVED******REMOVED******REMOVED*** Integration with Existing System

```python
***REMOVED*** Enhance stigmergy with evolutionary dynamics
class EvolutionaryStigmergy(StigmergicScheduler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.evolutionary_learner = EvolutionarySwapLearner(db=None)  ***REMOVED*** Set db later
    ...
    def evolve_trail_strengths(self, time_delta_days: float = 1.0):
        ...
        """
        ***REMOVED*** Calculate fitness for each trail based on swap outcomes
        for trail in self.trails.values():
            if trail.trail_type != TrailType.SWAP_AFFINITY:
                continue

            ***REMOVED*** Count successful swaps using this trail
            ...
            ***REMOVED*** Fitness = success rate
            fitness = successful_swaps / total_swaps

            ***REMOVED*** Average fitness across all trails
            all_fitnesses = [
...
***REMOVED******REMOVED******REMOVED*** Application to Shift Preference Clustering

***REMOVED******REMOVED******REMOVED******REMOVED*** 6.1 Detecting Preference Homophily
...
class PreferenceClusteringAnalyzer:
    ...
    def detect_preference_clusters(
        ...
        """
        ***REMOVED*** Build preference similarity matrix
        similarity = {}
        for f1 in faculty:
            for f2 in faculty:
                if f1.id == f2.id:
        ...
        ***REMOVED*** Simulate Schelling dynamics
        ***REMOVED*** Start with random "neighborhood" assignments
        ...
        for iteration in range(max_iterations):
            ...
            for faculty_member in random.sample(faculty, len(faculty)):
                ...
                ***REMOVED*** Calculate satisfaction with current neighborhood
                neighbors = [
                ...
                ***REMOVED*** If unsatisfied, try to move
                if satisfaction < tolerance:
                    ***REMOVED*** Find better neighborhood
                    ...
                    for target_neighborhood in range(num_neighborhoods):
                        ...
                        ***REMOVED*** Calculate potential satisfaction there
                        potential_neighbors = [
                            f.id for f in faculty
                        ...
                        if not potential_neighbors:
                            potential_satisfaction = 1.0  ***REMOVED*** Empty = no dissimilarity
                        else:
                    ...
                    ***REMOVED*** Move if better neighborhood found
                    if best_neighborhood != current_neighborhood:
                        assignments[fid] = best_neighborhood
                        moved = True

            ***REMOVED*** Stop if no one moved (stable state)
            if not moved:
                break

        ***REMOVED*** Extract clusters
        clusters = {}
    ...
    def _calculate_preference_similarity(
        ...
        if not prefs_a or not prefs_b:
            return 0.5  ***REMOVED*** No data = neutral

        ***REMOVED*** Calculate overlap in preferred/blocked weeks
    ...
    def measure_segregation_level(
        ...
        """
        ***REMOVED*** Calculate within-cluster similarity vs between-cluster similarity
        within_similarity = []
        between_similarity = []

        for cluster_id, members in clusters.items():
            ***REMOVED*** Within-cluster
...
```python
***REMOVED*** Add clustering analysis to resilience monitoring
def analyze_preference_clustering(schedule: Schedule) -> dict[str, Any]:
    ...
    ***REMOVED*** Check cluster size balance
...
***REMOVED******REMOVED*** 7. Principal-Agent Problem
...
***REMOVED******REMOVED******REMOVED*** Application to Schedule Compliance

***REMOVED******REMOVED******REMOVED******REMOVED*** 7.1 Incentive-Compatible Schedule Acceptance
...
class IncentiveAlignmentMechanism:
    ...
    def design_incentive_compatible_allocation(
        ...
        """
        ***REMOVED*** Step 1: Allocate to maximize reported social welfare
        ...
        ***REMOVED*** Step 2: Calculate VCG "payments" (opportunity costs)
        ***REMOVED*** Faculty "pays" by accepting less preferred shifts
        payments = {}
        for fid in faculty:
            ***REMOVED*** Welfare of others with this faculty
            total_with = self._social_welfare(allocation, reported_preferences)

            ***REMOVED*** Welfare of others if this faculty wasn't there
            ...
            ***REMOVED*** This faculty's own welfare
            own_welfare = sum(
                value for shift, value in reported_preferences[fid]
                if shift in allocation[fid]
            )

            ***REMOVED*** VCG payment = externality imposed on others
            externality = others_welfare_without - (total_with - own_welfare)
            payments[fid] = max(0, externality)

        ***REMOVED*** Step 3: Verify incentive compatibility
        ***REMOVED*** For each faculty, check that lying doesn't help
        for fid in faculty:
            ***REMOVED*** Try alternative (lying) strategies
            for lie_factor in [0.5, 1.5]:  ***REMOVED*** Under/over-report values
                lying_prefs = {
                ...
                ***REMOVED*** Utility from truth-telling
                truthful_utility = sum(
                    value for shift, value in reported_preferences[fid]
                    if shift in allocation[fid]
                ) - payments[fid]

                ***REMOVED*** Verify truth-telling is better
                if lying_utility > truthful_utility + 0.01:  ***REMOVED*** Small epsilon for numerical stability
                    logger.warning(
    ...
    def implement_monitoring_mechanism(
        ...
        """
        ***REMOVED*** Estimate probability faculty is misreporting
        historical_accuracy = self._estimate_reporting_accuracy(faculty_id)

        ***REMOVED*** Expected value of monitoring = prob(lying) * benefit(catching) - cost
        prob_lying = 1.0 - historical_accuracy

        ***REMOVED*** Benefit of catching = improved allocation efficiency
    ...
    def implement_screening_mechanism(
        ...
        for fid in applicant_faculty:
            ***REMOVED*** Analyze historical behavior
            ...
            swap_rate = len(swap_history) / 365.0  ***REMOVED*** Swaps per year
...
```python
***REMOVED*** Add incentive analysis to swap auto-matcher
class IncentiveAwareSwapMatcher(SwapAutoMatcher):
    ...
    def detect_strategic_behavior(
        ...
        ***REMOVED*** Check if always swapping same week type
        ...
        ***REMOVED*** Scoring
        ...
        if immediate_requests / len(swaps) > 0.5:
            strategic_score += 0.3  ***REMOVED*** Often requests swaps right away

        if absorb_swaps / len(swaps) > 0.4:
            strategic_score += 0.3  ***REMOVED*** Many absorb swaps (reducing workload)

        if concentration > 0.7:
            strategic_score += 0.4  ***REMOVED*** Always swapping same week type
...
***REMOVED******REMOVED*** 8. Practical Implementation Roadmap
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Auction Mechanisms (Weeks 5-8)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Cooperative Mechanisms (Weeks 9-12)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Learning and Evolution (Weeks 13-16)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Integration and Testing (Weeks 17-20)
...
***REMOVED******REMOVED*** 9. Integration with Existing Systems

***REMOVED******REMOVED******REMOVED*** 9.1 Stigmergy + Game Theory Synergy
...
class PriorityPointTransaction(Base):
    ...
    id = Column(UUID, primary_key=True, default=uuid4)
    faculty_id = Column(UUID, ForeignKey("persons.id"))
    amount = Column(Float)  ***REMOVED*** Positive = earned, negative = spent
...
class StrategyPerformance(Base):
    ...
    id = Column(UUID, primary_key=True, default=uuid4)
    strategy_type = Column(String)  ***REMOVED*** e.g., "early_request", "mutual_preference"
    success_count = Column(Integer, default=0)
    total_attempts = Column(Integer, default=0)
    success_rate = Column(Float, default=0.0)
    population_frequency = Column(Float, default=0.0)  ***REMOVED*** For replicator dynamics
    last_updated = Column(DateTime, default=datetime.utcnow)
```

***REMOVED******REMOVED******REMOVED*** 9.3 API Routes

```python
***REMOVED*** New routes in /backend/app/api/routes/game_theory.py
...
***REMOVED******REMOVED******REMOVED*** Implementation Resources
