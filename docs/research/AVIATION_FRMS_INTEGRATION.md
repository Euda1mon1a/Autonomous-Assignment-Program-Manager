***REMOVED*** Aviation Fatigue Risk Management Systems (FRMS) Integration
...
1. [Regulatory Framework Mapping: FAA/EASA → ACGME](***REMOVED***regulatory-framework-mapping)
2. [Bio-Mathematical Fatigue Models](***REMOVED***bio-mathematical-fatigue-models)
3. [Circadian Rhythm Scheduling Constraints](***REMOVED***circadian-rhythm-scheduling-constraints)
4. [Fatigue Risk Metrics for Medical Shifts](***REMOVED***fatigue-risk-metrics)
5. [Aviation Crew Scheduling Algorithms](***REMOVED***aviation-crew-scheduling-algorithms)
6. [Implementation Roadmap](***REMOVED***implementation-roadmap)
7. [Code Integration Points](***REMOVED***code-integration-points)
8. [References](***REMOVED***references)

---

***REMOVED******REMOVED*** Regulatory Framework Mapping

***REMOVED******REMOVED******REMOVED*** 1.1 ICAO/FAA/EASA FRMS Framework
...
***REMOVED******REMOVED******REMOVED*** 1.2 FAA Part 117 Flight Duty Limits
...
***REMOVED******REMOVED******REMOVED*** 1.3 EASA ORO.FTL Regulations
...
***REMOVED******REMOVED******REMOVED*** 1.4 Mapping FAA/EASA → ACGME
...
***REMOVED******REMOVED*** Bio-Mathematical Fatigue Models

***REMOVED******REMOVED******REMOVED*** 2.1 The Two-Process Model of Sleep Regulation
...
***REMOVED******REMOVED******REMOVED*** 2.2 SAFTE-FAST Model
...
***REMOVED******REMOVED******REMOVED*** 2.3 Application to Medical Residents
...
```python
***REMOVED*** Resident shift effectiveness prediction
...
class ResidentFatigueModel:
    ...
    ***REMOVED*** Constants from SAFTE literature
    TAU_WAKE = 18.2  ***REMOVED*** hours, homeostatic rise time constant
    TAU_SLEEP = 4.2  ***REMOVED*** hours, homeostatic decay time constant
    CIRCADIAN_PERIOD = 24.0  ***REMOVED*** hours
    WOCL_START = 2.0  ***REMOVED*** 2:00 AM
    WOCL_END = 6.0   ***REMOVED*** 6:00 AM

    ***REMOVED*** Effectiveness thresholds
    THRESHOLD_CAUTION = 77.0  ***REMOVED*** FAA threshold
    THRESHOLD_HIGH_RISK = 70.0  ***REMOVED*** FRA threshold

    def __init__(self):
        self.sleep_reservoir = 100.0  ***REMOVED*** Start fully rested

    def update_wakefulness(self, hours_awake: float) -> float:
        """Update sleep homeostasis for time awake."""
        ***REMOVED*** Process S increases during wakefulness
        depletion_rate = hours_awake / self.TAU_WAKE
        self.sleep_reservoir -= depletion_rate * 5.0  ***REMOVED*** 5% per tau
    ...
    def update_sleep(self, sleep_hours: float, sleep_quality: float = 1.0) -> float:
        ...
        ***REMOVED*** Process S decreases during sleep
    ...
    def circadian_component(self, time_of_day: float) -> float:
        ...
        ***REMOVED*** Sinusoidal with minimum at 4:00 AM (WOCL center)
        phase_shift = 4.0  ***REMOVED*** hours
    ...
    def calculate_effectiveness(
        ...
        """
        ***REMOVED*** Homeostatic component (Process S)
        homeostatic = self.sleep_reservoir

        ***REMOVED*** Circadian component (Process C)
        circadian = self.circadian_component(time_of_day) * 100

        ***REMOVED*** Sleep inertia penalty (first 30 minutes after waking)
        inertia_penalty = 0
        if sleep_inertia_minutes > 0:
            inertia_penalty = max(0, 20 * (1 - sleep_inertia_minutes / 30))

        ***REMOVED*** Combined effectiveness (weighted average)
        effectiveness = (
            0.6 * homeostatic +  ***REMOVED*** 60% weight on sleep debt
            0.4 * circadian -    ***REMOVED*** 40% weight on circadian
            inertia_penalty
        )
    ...
    def predict_shift_effectiveness(
        ...
        """
        ***REMOVED*** Update sleep reservoir from prior rest
        ...
        ***REMOVED*** Sample effectiveness every 30 minutes during shift
...
***REMOVED******REMOVED******REMOVED*** 2.4 Psychomotor Vigilance Task (PVT) as Validation Metric
...
```python
***REMOVED*** PVT equivalents for resident tasks
PVT_TASK_MAPPING = {
    "central_line": "fine_motor_precision",      ***REMOVED*** Like RT, requires alertness
    "suturing": "sustained_attention",            ***REMOVED*** Like lapse detection
    "medication_order": "working_memory",         ***REMOVED*** Dosage calculation
    "patient_assessment": "situational_awareness", ***REMOVED*** Pattern recognition
    "emergency_response": "decision_speed"        ***REMOVED*** Like RT under pressure
}
...
def map_effectiveness_to_clinical_risk(effectiveness: float) -> dict:
    ...
    elif effectiveness >= 77:  ***REMOVED*** FAA threshold
        return {
            "risk_level": "moderate",
            "error_multiplier": 1.5,
            "supervision_required": True  ***REMOVED*** Recommend supervision
        }
    elif effectiveness >= 70:  ***REMOVED*** FRA threshold
        return {
            "risk_level": "high",
...
***REMOVED******REMOVED*** Circadian Rhythm Scheduling Constraints

***REMOVED******REMOVED******REMOVED*** 3.1 Window of Circadian Low (WOCL)
...
```python
***REMOVED*** WOCL-aware constraint for resident scheduling
class CircadianConstraint:
    ...
    def evaluate(self, assignment: Assignment) -> ConstraintViolation:
        ...
        if not self.is_procedure_shift(assignment):
            return None  ***REMOVED*** Not applicable

        shift_hour = assignment.block.start_time.hour

        ***REMOVED*** Check if shift overlaps WOCL
        if self.WOCL_START_HOUR <= shift_hour < self.WOCL_END_HOUR:
            ***REMOVED*** High-risk procedures during WOCL
    ...
    def recommend_supervision_level(
        ...
        if self.WOCL_START_HOUR <= time_of_day < self.WOCL_END_HOUR:
            ***REMOVED*** During WOCL, increase supervision
            if resident_pgy_level == 1:
                return "direct_supervision"  ***REMOVED*** Attending must be present
            else:
                return "indirect_supervision"  ***REMOVED*** Attending immediately available
        else:
            ***REMOVED*** Normal daytime hours
            if resident_pgy_level == 1:
                return "indirect_supervision"
            else:
                return "oversight"  ***REMOVED*** Attending in hospital
```

***REMOVED******REMOVED******REMOVED*** 3.2 Shift Start Time Optimization
...
def calculate_max_shift_duration(start_hour: int, base_duration: int = 12) -> int:
    ...
    """
    ***REMOVED*** Favorable start times (6 AM - 6 PM)
    if 6 <= start_hour < 18:
        return base_duration  ***REMOVED*** Full duration allowed

    ***REMOVED*** Moderately unfavorable (6 PM - 10 PM)
    elif 18 <= start_hour < 22:
        return base_duration - 2  ***REMOVED*** Reduce by 2 hours

    ***REMOVED*** Highly unfavorable (10 PM - 6 AM, includes WOCL)
    else:
        return base_duration - 4  ***REMOVED*** Reduce by 4 hours
```

***REMOVED******REMOVED******REMOVED*** 3.3 Rotating Shift Challenges
...
class RotationTransitionConstraint:
    ...
    TRANSITION_RECOVERY_DAYS = {
        ("day", "day"): 0,
        ("day", "night"): 2,      ***REMOVED*** 2 days to phase delay
        ("night", "day"): 3,      ***REMOVED*** 3 days to phase advance
        ("night", "night"): 0,
        ("evening", "day"): 1,
        ("day", "evening"): 0,
    }
...
***REMOVED******REMOVED*** Fatigue Risk Metrics for Medical Shifts

***REMOVED******REMOVED******REMOVED*** 4.1 Operational Fatigue Metrics
...
class FatigueMetrics:
    ...
    ***REMOVED*** Time-based metrics
    ...
    fatigue_risk_time: float      ***REMOVED*** Hours with effectiveness <70%
    fatigue_free_time: float      ***REMOVED*** Hours with effectiveness >85%
    wocl_hours_worked: float      ***REMOVED*** Hours during 2-6 AM

    ***REMOVED*** Sleep metrics
    total_sleep_hours: float
    sleep_debt_hours: float       ***REMOVED*** Cumulative deficit from 8h/night baseline
    shortest_sleep_period: float  ***REMOVED*** Minimum sleep in any 24h period

    ***REMOVED*** Effectiveness metrics
    mean_effectiveness: float     ***REMOVED*** Average effectiveness during duty
    min_effectiveness: float      ***REMOVED*** Lowest effectiveness during duty
    effectiveness_variance: float ***REMOVED*** Consistency of performance

    ***REMOVED*** Recovery metrics
    recovery_ratio: float         ***REMOVED*** Total sleep / total wake
    days_off: int                 ***REMOVED*** 24-hour rest periods

    def risk_score(self) -> float:
        """Calculate composite fatigue risk score (0-100)."""
        ***REMOVED*** Weighted risk components
        risk = 0.0

        ***REMOVED*** High fatigue risk time is bad
        if self.total_duty_hours > 0:
            frt_ratio = self.fatigue_risk_time / self.total_duty_hours
            risk += frt_ratio * 30  ***REMOVED*** Max 30 points

        ***REMOVED*** Sleep debt penalty
        risk += min(self.sleep_debt_hours / 10, 1.0) * 25  ***REMOVED*** Max 25 points

        ***REMOVED*** WOCL exposure penalty
        risk += min(self.wocl_hours_worked / 20, 1.0) * 20  ***REMOVED*** Max 20 points

        ***REMOVED*** Low mean effectiveness
        if self.mean_effectiveness < 85:
            risk += (85 - self.mean_effectiveness) / 85 * 15  ***REMOVED*** Max 15 points

        ***REMOVED*** Inadequate recovery
        if self.recovery_ratio < 0.4:  ***REMOVED*** Should be ~0.33 (8/16)
            risk += 10
    ...
    def acgme_violations(self) -> List[str]:
        ...
        ***REMOVED*** 80-hour rule (approximate for metric period)
...
***REMOVED******REMOVED******REMOVED*** 4.2 Real-Time Fatigue Monitoring Dashboard

```python
***REMOVED*** Dashboard endpoint for real-time fatigue monitoring
...
async def get_active_fatigue_alerts(
    threshold: float = 60.0,  ***REMOVED*** Risk score threshold
    db: AsyncSession = Depends(get_db)
):
    """
...
***REMOVED******REMOVED*** Aviation Crew Scheduling Algorithms

***REMOVED******REMOVED******REMOVED*** 5.1 Crew Pairing and Rostering Problem
...
***REMOVED******REMOVED******REMOVED*** 5.2 Column Generation Approach
...
```python
***REMOVED*** Column generation for resident rotation assignments
...
class RotationAssignmentOptimizer:
    ...
    def __init__(self, residents: List[Person], rotations: List[Rotation]):
        ...
        self.columns = []  ***REMOVED*** Generated rotation sequences

    def generate_initial_columns(self):
        """Generate initial feasible rotation sequences."""
        for resident in self.residents:
            ***REMOVED*** Create simple sequential rotation assignments
    ...
    def solve_master_problem(self) -> dict:
        ...
        ***REMOVED*** Decision variables: binary for each column (sequence)
        x = {
            i: LpVariable(f"sequence_{i}", cat="Binary")
            for i in range(len(self.columns))
        }

        ***REMOVED*** Objective: Minimize total fatigue cost
        prob += lpSum(
            x[i] * self.calculate_fatigue_cost(self.columns[i])
            for i in range(len(self.columns))
        )

        ***REMOVED*** Constraint: Each rotation must be covered exactly once
        ...
        ***REMOVED*** Constraint: Each resident assigned to at most one sequence
        for resident in self.residents:
            prob += lpSum(
                x[i] for i in range(len(self.columns))
    ...
    def generate_improving_column(self, dual_values: dict) -> Optional[RotationSequence]:
        ...
        for resident in self.residents:
            ***REMOVED*** Try different rotation orderings
    ...
    def calculate_fatigue_cost(self, sequence: RotationSequence) -> float:
        ...
        for rotation in sequence.rotations:
            ***REMOVED*** Simulate rotation and accumulate effectiveness deficits
            for shift in rotation.shifts:
                ...
                ***REMOVED*** Cost increases as effectiveness drops below 85%
                if effectiveness < 85:
...
***REMOVED******REMOVED******REMOVED*** 5.3 Fatigue-Balanced Rostering
...
def calculate_fatigue_balance_score(assignments: List[Assignment]) -> float:
    ...
    for resident in get_all_residents():
        ...
        ***REMOVED*** Calculate cumulative fatigue for this resident
        model = ResidentFatigueModel()
        total_fatigue = 0.0
        ...
        for assignment in resident_assignments:
            ...
            ***REMOVED*** Accumulate fatigue (inverse of effectiveness)
    ...
    ***REMOVED*** Return coefficient of variation (lower = more balanced)
    return np.std(fatigue_scores) / np.mean(fatigue_scores)
```

***REMOVED******REMOVED******REMOVED*** 5.4 Integration with Commercial Tools
...
```python
***REMOVED*** API integration with external fatigue model
import httpx
...
class ExternalFatigueModelClient:
    ...
    async def evaluate_schedule(
        ...
        """
        ***REMOVED*** Convert assignments to API format
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Metrics & Monitoring (Months 3-4)

**2.1 Fatigue Metrics Service**

```python
***REMOVED*** backend/app/services/fatigue_metrics_service.py
from datetime import datetime, timedelta
...
class FatigueMetricsService:
    ...
    async def calculate_metrics(
        ...
        ***REMOVED*** Fetch assignments in period
        ...
        ***REMOVED*** Initialize fatigue model
        model = ResidentFatigueModel()

        ***REMOVED*** Accumulate metrics
        ...
        for assignment in assignments:
            ***REMOVED*** Predict effectiveness throughout shift
            ...
            for timestamp, effectiveness in predictions:
                total_duty_hours += 0.5  ***REMOVED*** 30-minute intervals
        ...
        ***REMOVED*** Calculate sleep debt (assumes 8h/night baseline)
        expected_sleep = days * 8
        ***REMOVED*** Estimate actual sleep from off-duty time (simplified)
        actual_sleep = (days * 24 - total_duty_hours) * 0.5  ***REMOVED*** 50% of off time
    ...
    async def store_metrics(
        ...
        ***REMOVED*** Implementation: Insert into fatigue_metrics table
        pass
```
...
```python
***REMOVED*** Calculate fatigue metrics daily for all residents
...
async def calculate_daily_fatigue_metrics():
    ...
    async with get_db_session() as db:
        ...
        for resident in residents:
            ...
            ***REMOVED*** Send alert if high risk
            if metrics.risk_score() > 75:
                await send_fatigue_alert(resident, metrics)
```

***REMOVED******REMOVED******REMOVED*** Phase 3: Schedule Optimization (Months 5-6)
...
```python
***REMOVED*** Modify schedule generator objective function
def add_fatigue_objectives(solver, assignments, residents):
    ...
    for resident in residents:
        ...
        ***REMOVED*** Calculate predicted fatigue cost
        model = ResidentFatigueModel()
        total_cost = 0

        for assignment in resident_assignments:
            ***REMOVED*** Penalize shifts with low predicted effectiveness
            effectiveness = model.calculate_effectiveness(
            ...
            if effectiveness < 85:
                deficit = (85 - effectiveness) * 10  ***REMOVED*** Scale factor
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Advanced Features (Months 7-9)

**4.1 Wearable Integration**

```python
***REMOVED*** Integrate actual sleep data from wearables (Fitbit, Apple Watch)
class WearableDataIntegration:
    ...
    async def fetch_sleep_data(
        ...
        ***REMOVED*** Integration with Fitbit/Apple Health/Garmin APIs
        pass
    ...
    async def update_fatigue_model(
        ...
        ***REMOVED*** Adaptive learning: Improve predictions over time
        pass
```
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Validation & Rollout (Months 10-12)
...
***REMOVED******REMOVED*** Code Integration Points

***REMOVED******REMOVED******REMOVED*** 7.1 Backend Service Architecture
...
│   └── fatigue/
│       ├── __init__.py
│       ├── models.py                    ***REMOVED*** Bio-mathematical models
│       ├── metrics_service.py           ***REMOVED*** Fatigue metrics calculation
│       ├── alert_service.py             ***REMOVED*** High-risk detection
│       └── wearable_integration.py      ***REMOVED*** Sleep data sync
│
├── scheduling/
│   └── constraints/
│       ├── circadian_constraint.py      ***REMOVED*** WOCL/circadian constraints
│       ├── shift_duration_constraint.py ***REMOVED*** Time-of-day duration limits
│       └── transition_constraint.py     ***REMOVED*** Rotation transition recovery
│
├── api/routes/
│   └── fatigue.py                       ***REMOVED*** FRMS API endpoints
│
├── models/
│   ├── fatigue_metrics.py               ***REMOVED*** FatigueMetrics ORM model
│   └── shift_effectiveness.py           ***REMOVED*** ShiftEffectiveness ORM model
│
└── schemas/
    └── fatigue.py                       ***REMOVED*** Pydantic schemas for FRMS
```

***REMOVED******REMOVED******REMOVED*** 7.2 Frontend Dashboard Components
...
***REMOVED******REMOVED******REMOVED*** 7.3 Database Migrations

```python
***REMOVED*** backend/alembic/versions/YYYYMMDD_add_frms_tables.py
def upgrade():
    ***REMOVED*** Create fatigue_metrics table
    op.create_table(
        'fatigue_metrics',
        sa.Column('id', sa.UUID(), nullable=False),
        sa.Column('person_id', sa.UUID(), nullable=False),
        sa.Column('metric_date', sa.Date(), nullable=False),
        ***REMOVED*** ... (see Phase 1 SQL)
    )

    ***REMOVED*** Create shift_effectiveness table
    op.create_table(
        'shift_effectiveness',
        ***REMOVED*** ... (see Phase 1 SQL)
    )
...
***REMOVED******REMOVED******REMOVED*** 7.4 Celery Task Integration

```python
***REMOVED*** backend/app/core/celery_app.py

***REMOVED*** Add new periodic tasks
celery_app.conf.beat_schedule.update({
    'calculate-daily-fatigue-metrics': {
        'task': 'calculate_daily_fatigue_metrics',
        'schedule': crontab(hour=1, minute=0),  ***REMOVED*** Daily at 1 AM
    },
...
***REMOVED******REMOVED******REMOVED*** 7.5 Configuration Updates
...
class Settings(BaseSettings):
    ...
    ***REMOVED*** FRMS Configuration
    FRMS_ENABLED: bool = True
    FRMS_EFFECTIVENESS_THRESHOLD_CAUTION: float = 77.0  ***REMOVED*** FAA threshold
    FRMS_EFFECTIVENESS_THRESHOLD_HIGH_RISK: float = 70.0  ***REMOVED*** FRA threshold
    FRMS_WOCL_START_HOUR: int = 2  ***REMOVED*** 2:00 AM
    FRMS_WOCL_END_HOUR: int = 6    ***REMOVED*** 6:00 AM
    FRMS_ALERT_RISK_SCORE_THRESHOLD: float = 75.0

    ***REMOVED*** External fatigue model API (optional)
    FRMS_EXTERNAL_API_URL: Optional[str] = None
    FRMS_EXTERNAL_API_KEY: Optional[str] = None

    ***REMOVED*** Wearable integration (optional)
...
***REMOVED******REMOVED******REMOVED*** Aviation Regulations & Guidelines
...
***REMOVED******REMOVED******REMOVED*** Bio-Mathematical Fatigue Models
...
***REMOVED******REMOVED******REMOVED*** Circadian Rhythms & Sleep Science
...
***REMOVED******REMOVED******REMOVED*** Aviation Crew Scheduling
...
***REMOVED******REMOVED******REMOVED*** Healthcare Fatigue Management
...
***REMOVED******REMOVED*** Appendix B: Comparison Matrix
