...
```bash
***REMOVED*** Sanitize data before sharing with Claude Code IDE
python scripts/sanitize_pii.py input.json sanitized.json

***REMOVED*** Restore original data (user only - never share mapping file)
python scripts/sanitize_pii.py --reverse .sanitize/mapping_*.json sanitized.json restored.json
```

---

***REMOVED******REMOVED*** When to Use This Process
...
***REMOVED******REMOVED******REMOVED*** Step 1: Export Data

Export the data you need to share from your local database or Airtable:

```bash
***REMOVED*** Example: Export from database
...
***REMOVED*** Or use API export endpoint
curl -s http://localhost:8000/api/schedule/export?block=10 > block10_export.json
```

***REMOVED******REMOVED******REMOVED*** Step 2: Sanitize the Data
...
***REMOVED******REMOVED******REMOVED*** Step 3: Verify Sanitization

Before sharing, verify the sanitized file:

```bash
***REMOVED*** Check for any remaining PII patterns
./scripts/pii-scan.sh

***REMOVED*** Manually review the sanitized file
...
***REMOVED******REMOVED******REMOVED*** Step 4: Share with Claude Code IDE
...
***REMOVED******REMOVED******REMOVED*** Step 5: Restore Original Data (If Needed)

If you need to apply changes back to real data:

```bash
***REMOVED*** Use the mapping file to reverse sanitization
...
***REMOVED******REMOVED*** Data Classification Reference

***REMOVED******REMOVED******REMOVED*** REDACTED (Completely Removed)
...
***REMOVED******REMOVED******REMOVED*** REPLACED (With Synthetic Data)
...
***REMOVED******REMOVED******REMOVED*** PRESERVED (Safe to Share)
...
***REMOVED******REMOVED*** Important Rules
...
***REMOVED******REMOVED*** Mapping File Security
...
***REMOVED******REMOVED******REMOVED*** "File contains PII patterns" error

```bash
***REMOVED*** Run the scanner to identify issues
./scripts/pii-scan.sh

***REMOVED*** If false positive, check if it's in test data
grep -n "pattern" file.json
```

***REMOVED******REMOVED******REMOVED*** Sanitization didn't replace all names

The script uses context (role, pgy_level) to generate appropriate synthetic IDs. If names aren't being replaced:

```python
***REMOVED*** Ensure your data has role context
{
  "name": "John Smith",
  "role": "RESIDENT",  ***REMOVED*** Required for proper ID generation
  "pgy_level": 2       ***REMOVED*** Optional but helpful
}
```

***REMOVED******REMOVED******REMOVED*** Need to share data that can't be sanitized
