...
1. [Docker Build Issues](***REMOVED***docker-build-issues)
2. [Redis Connection Issues](***REMOVED***redis-connection-issues)
3. [SQLAlchemy/ORM Issues](***REMOVED***sqlalchemyorm-issues)
4. [Authentication Issues](***REMOVED***authentication-issues)
5. [Rate Limiting Issues](***REMOVED***rate-limiting-issues)
6. [GraphQL Issues](***REMOVED***graphql-issues)
7. [Database Seeding](***REMOVED***database-seeding)
8. [Quick Recovery Steps](***REMOVED***quick-recovery-steps)

---

***REMOVED******REMOVED*** Docker Build Issues

***REMOVED******REMOVED******REMOVED*** Problem: Cached Layer Prevents Dependency Installation
...
***REMOVED******REMOVED*** Redis Connection Issues

***REMOVED******REMOVED******REMOVED*** Problem: Double Password in Redis URL

**Symptom:**
```
AuthenticationError: ...
***REMOVED*** Or rate limiting middleware crashes
```
...
def redis_url_with_password(self) -> str:
    ***REMOVED*** Check if password is already in the URL
    if "@" in self.REDIS_URL and self.REDIS_URL.startswith("redis://:"):
        return self.REDIS_URL  ***REMOVED*** Already has password
...
***REMOVED******REMOVED*** SQLAlchemy/ORM Issues

***REMOVED******REMOVED******REMOVED*** Problem: Ambiguous Foreign Keys Error
...
revoked_by_id = Column(GUID(), ForeignKey("users.id"))  ***REMOVED*** Second FK to users
owner = relationship("User", backref="api_keys")  ***REMOVED*** Ambiguous!
```
...
***REMOVED******REMOVED******REMOVED*** Problem: Reserved Column Name "metadata"
...
***REMOVED******REMOVED******REMOVED*** Problem: Admin User Not Created
...
***REMOVED******REMOVED******REMOVED*** Problem: JWT Token Validation Fails
...
***REMOVED******REMOVED*** Rate Limiting Issues

***REMOVED******REMOVED******REMOVED*** Problem: Invalid Rate Limiting Strategy
...
limiter = Limiter(
    ...
    strategy="fixed-window",  ***REMOVED*** Not "fixed-window-elastic-expiry"
)
```

---

***REMOVED******REMOVED*** GraphQL Issues

***REMOVED******REMOVED******REMOVED*** Problem: Enum Not Recognized
...
```python
@strawberry.enum
class AssignmentRole(str):  ***REMOVED*** Missing Enum inheritance
    PRIMARY = "primary"
```
...
@strawberry.enum
class AssignmentRole(str, Enum):  ***REMOVED*** Must have both!
    PRIMARY = "primary"
```
...
***REMOVED******REMOVED******REMOVED*** Problem: Scripts Not Mounted in Container
...
1. **Run from host with requests:**
   ```bash
   python3 scripts/seed_people.py  ***REMOVED*** Requires requests module
   ```
...
3. **Inline execution:**
   ...
   ***REMOVED*** ... seeding code
   "
   ```

***REMOVED******REMOVED******REMOVED*** Problem: Duplicate Key Constraint
...
***REMOVED******REMOVED*** Quick Recovery Steps

***REMOVED******REMOVED******REMOVED*** Full Stack Restart

```bash
***REMOVED*** Stop everything
docker-compose -f docker-compose.local.yml down

***REMOVED*** Clean slate (optional - removes volumes)
docker-compose -f docker-compose.local.yml down -v

***REMOVED*** Fresh rebuild
docker builder prune -af
docker-compose -f docker-compose.local.yml up -d --build
...
***REMOVED******REMOVED******REMOVED*** Database Reset
...
***REMOVED******REMOVED******REMOVED*** Check Container Health
...
***REMOVED******REMOVED*** Checklist for New Deployments
...
***REMOVED******REMOVED******REMOVED*** Problem: Login Flashes and Resets
...
**Wrong:**
```yaml
***REMOVED*** docker-compose.local.yml
NEXT_PUBLIC_API_URL: http://localhost:8000  ***REMOVED*** Missing /api!
```

**Correct:**
```yaml
NEXT_PUBLIC_API_URL: http://localhost:8000/api  ***REMOVED*** Include /api
```
...
***REMOVED******REMOVED******REMOVED*** Problem: Demo Credentials Wrong
