...
4) Export canonical JSON → XLSX. (`backend/app/services/canonical_schedule_export_service.py`, `backend/app/services/json_to_xlsx_converter.py`)

**Edit a block**
- Use schedule drafts: add/modify/delete half‑day assignments in a draft, preview, publish, rollback. (`backend/app/api/routes/schedule_drafts.py`, `backend/app/services/schedule_draft_service.py`)

---

## Readiness Matrix — normal + edge + edge-of-edge
...
- ✅ People/roles exist (residents/faculty). (`backend/app/models/person.py`)
- ✅ Rotation templates (primary + secondary mid-block). (`backend/app/models/rotation_template.py`, `backend/app/models/block_assignment.py`)
- ✅ Block assignments import (CSV/Excel). (`backend/app/services/block_assignment_import_service.py`)
- ✅ Absences (blocking). (`backend/app/services/preload_service.py`)
...
- ✅ Activity requirements are mandatory for outpatient templates; missing requirements now fail fast (no auto-create). (`backend/app/scheduling/activity_solver.py`)
...
### B) Generation (CP-SAT)
- ✅ Canonical CP-SAT pipeline enforced. (`docs/scheduling/CP_SAT_CANONICAL_PIPELINE.md`, `backend/app/scheduling/engine.py`)
- ✅ Call coverage for Sun–Thu generated by solver. (`backend/app/scheduling/constraints/*call*`, `backend/app/scheduling/engine.py`)
...
- ✅ FMC physical capacity enforced at activity level (soft 6 / hard 8 per slot). Counts only FMC continuity templates (`C/CONT`) plus PROC/VAS/V1‑3/SM; CV excluded. (`backend/app/scheduling/activity_solver.py`)
...
- ✅ Last Wednesday AM/PM LEC/ADV for all. (`backend/app/services/preload_service.py`)
- ✅ Intern continuity Wed AM (PGY‑1) except exempt rotations. (`backend/app/services/preload_service.py`)
- ✅ Kapiolani (KAP) pattern. (`backend/app/services/preload_service.py`)
- ✅ LDNF Friday clinic + weekday OFF/LDNF. (`backend/app/services/preload_service.py`)
- ✅ NF/PedNF weekday OFF/NF + weekend W. (`backend/app/services/preload_service.py`)
- ✅ Hilo/OKI TDY with pre/post clinic days. (`backend/app/services/preload_service.py`)
- 🟡 Compound rotation weekend rules depend on rotation name parsing; works for common patterns but fragile to naming variants. (`backend/app/services/sync_preload_service.py`)
...
- 🟡 Block 13 (variable length): rules use block_end; patterns should still apply but not explicitly tested. (`backend/app/utils/academic_blocks.py`)
- 🟡 Hilo/OKI day-index logic assumes standard 28‑day blocks; nonstandard blocks could misalign. (`backend/app/services/preload_service.py`)
- 🟡 Re-run solver after manual edits: manual slots preserved, but any solver/template slots are wiped; must communicate to users. (`backend/app/scheduling/engine.py`)
- 🟡 Missing activity mappings → preloads skipped (logs only). No hard failure. (`backend/app/services/preload_service.py`)
...
- Draft edit on a preload slot: verify delete‑then‑add path works and is visible.
- Call edit: verify PCAT/DO regen works and is required.

---

## PD workflow (ideal end‑state)

1) Upload/confirm block assignments + absences + inpatient preloads
2) Generate block schedule (CP‑SAT)
3) Review in GUI (half‑day assignments)
4) Make edits via drafts → preview → publish
5) Export canonical JSON/XLSX

---

If you want, I can turn the Phase 1–3 items into specific tickets with owners and PR scopes.
