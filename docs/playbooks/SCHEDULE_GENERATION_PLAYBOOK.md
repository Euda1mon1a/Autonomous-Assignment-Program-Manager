***REMOVED*** Schedule Generation Playbook
...
1. [Pre-Generation Checklist](***REMOVED***pre-generation-checklist)
2. [Constraint Configuration](***REMOVED***constraint-configuration)
3. [Generation Execution](***REMOVED***generation-execution)
4. [Validation and Review](***REMOVED***validation-and-review)
5. [Publication Workflow](***REMOVED***publication-workflow)
6. [Rollback Procedures](***REMOVED***rollback-procedures)
7. [Failure Recovery](***REMOVED***failure-recovery)
8. [Post-Generation Verification](***REMOVED***post-generation-verification)
9. [Troubleshooting Guide](***REMOVED***troubleshooting-guide)
10. [Templates and Checklists](***REMOVED***templates-and-checklists)

---

***REMOVED******REMOVED*** Pre-Generation Checklist

**Timeline:** Complete this checklist 2-3 weeks before generation

***REMOVED******REMOVED******REMOVED*** Step 1: Data Validation

**Objective:** Ensure all input data is accurate and complete.

***REMOVED******REMOVED******REMOVED******REMOVED*** 1.1 Personnel Data

```bash
***REMOVED*** Command: Verify personnel in system
curl -X GET http://localhost:8000/api/admin/personnel/validation \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Expected Response:
***REMOVED*** {
***REMOVED***   "total_residents": 24,
***REMOVED***   "total_faculty": 12,
***REMOVED***   "validation_status": "passed",
***REMOVED***   "issues": []
***REMOVED*** }
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1.2 Credentials and Certifications

```bash
***REMOVED*** Command: Validate credentials
curl -X GET http://localhost:8000/api/admin/credentials/expiring \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Returns: Credentials expiring in next 90 days
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1.3 Rotation Templates

```bash
***REMOVED*** Command: List all rotation templates
curl -X GET http://localhost:8000/api/admin/rotations/templates \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Response includes: rotation_id, name, hours_per_day, type, required_credentials
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 1.4 ACGME Constraints

```bash
***REMOVED*** Command: Validate ACGME rules engine
curl -X POST http://localhost:8000/api/admin/validate/acgme-constraints \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"check_all": true}'

***REMOVED*** Expected Response: All constraints validated successfully
```
...
***REMOVED******REMOVED******REMOVED*** Step 2: Historical Data Cleanup

```bash
***REMOVED*** Command: Archive previous schedules
...
***REMOVED******REMOVED******REMOVED*** Step 3: System Health Check

```bash
***REMOVED*** Command: Full system health verification
curl -X GET http://localhost:8000/api/health/full \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Response includes:
***REMOVED*** - Database connectivity
***REMOVED*** - Redis cache status
***REMOVED*** - Celery worker health
***REMOVED*** - API response times
```
...
***REMOVED******REMOVED******REMOVED*** Pre-Generation Approval

Once all checks pass:

```bash
***REMOVED*** Document approval
...
***REMOVED******REMOVED*** Constraint Configuration

**Timeline:** Complete this 1-2 weeks before generation

***REMOVED******REMOVED******REMOVED*** Step 1: Load Constraint Profile

```bash
***REMOVED*** List available constraint profiles
...
***REMOVED***   "profiles": [
***REMOVED***     {
***REMOVED***       "id": "profile_001",
***REMOVED***       "name": "Standard Program",
***REMOVED***       "created": "2025-06-01",
***REMOVED***       "constraints": [...]
***REMOVED***     }
***REMOVED***   ]
***REMOVED*** }
...
***REMOVED******REMOVED******REMOVED*** Step 2: Configure Hard Constraints

These MUST be satisfied:

```bash
***REMOVED*** POST: Apply hard constraints
...
***REMOVED******REMOVED******REMOVED*** Step 3: Configure Soft Constraints

These are optimized but can be relaxed:

```bash
***REMOVED*** POST: Apply soft constraints with priorities
...
***REMOVED******REMOVED******REMOVED*** Step 4: Configure Resident Preferences

```bash
***REMOVED*** POST: Load preference data
...
***REMOVED******REMOVED******REMOVED*** Step 5: Review and Approve Configuration

```bash
***REMOVED*** GET: Retrieve current constraint configuration
...
***REMOVED******REMOVED*** Generation Execution

**Timeline:** Main generation (2-4 hours typical)

***REMOVED******REMOVED******REMOVED*** Step 1: Pre-Generation Backup

```bash
***REMOVED*** MANDATORY: Create database backup before generation
...
***REMOVED******REMOVED******REMOVED*** Step 2: Start Generation Job

```bash
***REMOVED*** POST: Initiate schedule generation
...
***REMOVED*** Expected Response:
***REMOVED*** {
***REMOVED***   "job_id": "gen_20251231_150000",
***REMOVED***   "status": "started",
***REMOVED***   "estimated_completion": "2025-12-31T17:00:00Z"
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED*** Step 3: Monitor Generation Progress

```bash
***REMOVED*** GET: Poll generation status
curl -X GET http://localhost:8000/api/scheduler/generate/gen_20251231_150000/status \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Response includes:
***REMOVED*** - Current block: 245/730
***REMOVED*** - Constraint violations: 0
***REMOVED*** - Time elapsed: 45 minutes
***REMOVED*** - Estimated remaining: 35 minutes
***REMOVED*** - Current allocation rate: 16.2 blocks/minute
```

**Monitoring Dashboard (Optional):**

```bash
***REMOVED*** Watch progress in real-time
watch -n 5 'curl -s http://localhost:8000/api/scheduler/generate/gen_20251231_150000/status \
  -H "Authorization: Bearer $TOKEN" | jq "."'
```

***REMOVED******REMOVED******REMOVED*** Step 4: Handle Generation Timeout

If generation approaches timeout (default 120 minutes):

```bash
***REMOVED*** Check if solver still making progress
curl -X GET http://localhost:8000/api/scheduler/generate/gen_20251231_150000/solver_status \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** If making good progress, extend timeout
curl -X PATCH http://localhost:8000/api/scheduler/generate/gen_20251231_150000/settings \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"timeout_minutes": 180}'

***REMOVED*** If stalled, consider canceling
curl -X POST http://localhost:8000/api/scheduler/generate/gen_20251231_150000/cancel \
  -H "Authorization: Bearer $TOKEN"
```

***REMOVED******REMOVED******REMOVED*** Step 5: Generation Complete

```bash
***REMOVED*** GET: Retrieve generation results
...
***REMOVED***   "blocks_scheduled": 730,
***REMOVED***   "assignments_created": 17520,
***REMOVED***   "violations": 0,
***REMOVED***   "generation_time_minutes": 87,
***REMOVED***   "schedule_quality_score": 0.94
***REMOVED*** }
```

---

***REMOVED******REMOVED*** Validation and Review

**Timeline:** Review period (3-5 days typical)

***REMOVED******REMOVED******REMOVED*** Step 1: Automated Compliance Check

```bash
***REMOVED*** POST: Comprehensive validation
...
***REMOVED*** Response includes detailed validation report
```

***REMOVED******REMOVED******REMOVED*** Step 2: ACGME Rule Verification

```bash
***REMOVED*** Generate ACGME compliance report
...
***REMOVED*** Verify:
***REMOVED*** - 80-hour rule: All residents within limit
***REMOVED*** - 1-in-7 rule: All residents have adequate rest
***REMOVED*** - Supervision ratios: All met for all days
```

***REMOVED******REMOVED******REMOVED*** Step 3: Manual Review Checklist

Create review tasks for Program Directors and Faculty:

```bash
***REMOVED*** Email notification template
...
***REMOVED******REMOVED******REMOVED*** Step 4: Address Review Feedback

```bash
***REMOVED*** Categorize feedback received
...
***REMOVED*** For each critical issue:
***REMOVED*** 1. Generate swap options
***REMOVED*** 2. Validate alternatives
***REMOVED*** 3. Present to requester for approval
***REMOVED*** 4. Document decision
```

***REMOVED******REMOVED******REMOVED*** Step 5: Approval Sign-Off

```bash
***REMOVED*** Collect formal approvals
...
***REMOVED******REMOVED*** Publication Workflow

**Timeline:** 1-2 days (after all approvals)

***REMOVED******REMOVED******REMOVED*** Step 1: Pre-Publication Preparation

```bash
***REMOVED*** Prepare communication materials
...
***REMOVED******REMOVED******REMOVED*** Step 2: Final System Verification

```bash
***REMOVED*** CRITICAL: Final verification before publication
curl -X GET http://localhost:8000/api/scheduler/pre-publication-check \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Expected results: All GREEN
***REMOVED*** - Database integrity: GREEN
***REMOVED*** - Backup status: GREEN (backup within last 1 hour)
***REMOVED*** - All constraints satisfied: GREEN
***REMOVED*** - System capacity: GREEN
***REMOVED*** - All required approvals: GREEN
```

***REMOVED******REMOVED******REMOVED*** Step 3: Publish Schedule

```bash
***REMOVED*** POST: Make schedule visible to residents and faculty
...
***REMOVED***   "published_at": "2025-12-31T18:00:00Z",
***REMOVED***   "residents_notified": 24,
***REMOVED***   "faculty_notified": 12
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED*** Step 4: Publish Notifications

```bash
***REMOVED*** Send publication notifications
...
***REMOVED*** Log notification results
curl -X GET http://localhost:8000/api/notifications/status/schedule_published \
  -H "Authorization: Bearer $TOKEN"
```

***REMOVED******REMOVED******REMOVED*** Step 5: Post-Publication Monitoring

```bash
***REMOVED*** Monitor first 24 hours for issues
watch -n 60 'curl -s http://localhost:8000/api/health/extended \
  -H "Authorization: Bearer $TOKEN" | jq ".system_load, .error_rate"'

***REMOVED*** Check for unexpected errors or high support ticket volume
...
***REMOVED******REMOVED******REMOVED*** Decision Tree: Should We Rollback?
...
***REMOVED******REMOVED******REMOVED*** Step 1: Immediate Rollback

```bash
***REMOVED*** STEP 1: Halt current schedule
...
***REMOVED*** STEP 2: Stop all ongoing operations
docker-compose exec backend python -c \
  "from app.services.scheduler import SchedulerService; \
   SchedulerService.abort_all_jobs()"

***REMOVED*** STEP 3: Verify no new assignments being made
sleep 30  ***REMOVED*** Wait for operations to drain
curl -X GET http://localhost:8000/api/scheduler/job-queue \
  -H "Authorization: Bearer $TOKEN" | jq ".active_jobs"
```

***REMOVED******REMOVED******REMOVED*** Step 2: Database Rollback

```bash
***REMOVED*** STEP 1: Identify correct backup
ls -lt backups/schedule_generation_*.sql | head -5

***REMOVED*** STEP 2: Restore from backup
BACKUP_FILE="backups/schedule_generation_20251231_150000.sql"

***REMOVED*** Stop backend to release DB connections
...
***REMOVED*** STEP 3: Verify restoration
curl -X GET http://localhost:8000/api/health \
  -H "Authorization: Bearer $TOKEN"
```

***REMOVED******REMOVED******REMOVED*** Step 3: Communication

```bash
***REMOVED*** Notify all stakeholders of rollback
...
***REMOVED*** Send to all stakeholders
curl -X POST http://localhost:8000/api/notifications/send-bulk \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** Step 4: Post-Rollback Analysis

```bash
***REMOVED*** Gather diagnostic data
cat > rollback_analysis.md << 'EOF'
***REMOVED*** Schedule Generation Rollback Analysis

***REMOVED******REMOVED*** Issue Description
...
***REMOVED******REMOVED*** Data Collected
...
***REMOVED******REMOVED*** Verification Before Retry
- [ ] Issue resolved
- [ ] System validated
...
***REMOVED******REMOVED******REMOVED*** Common Failure Scenarios

***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 1: Solver Timeout (Generation Not Completing)
...
```bash
***REMOVED*** Step 1: Check solver status
curl -X GET http://localhost:8000/api/scheduler/generate/gen_20251231_150000/solver-diagnostics \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Step 2: Check for constraint conflicts
curl -X POST http://localhost:8000/api/scheduler/validate/constraints \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"check_feasibility": true}'

***REMOVED*** Step 3: Identify problematic constraints
***REMOVED*** Look for: conflicting hard constraints, unrealistic parameters

***REMOVED*** Step 4: Relax soft constraints temporarily
...
***REMOVED*** Step 5: Retry generation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 2: Database Connection Lost
...
```bash
***REMOVED*** Step 1: Check database status
docker-compose logs db | tail -50

***REMOVED*** Step 2: Verify database is running
docker-compose ps db

***REMOVED*** Step 3: If database crashed, restart it
docker-compose restart db

***REMOVED*** Step 4: Run database integrity check
docker-compose exec db psql -U scheduler residency_scheduler \
  -c "ANALYZE;"

***REMOVED*** Step 5: Resume generation from last checkpoint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 3: ACGME Violations Found
...
```bash
***REMOVED*** Step 1: Generate detailed violation report
...
***REMOVED*** Step 2: Analyze patterns
***REMOVED*** Check if violations are systematic or isolated

***REMOVED*** Step 3: If systematic (all residents affected):
***REMOVED*** Adjust solver parameters and regenerate
curl -X POST http://localhost:8000/api/scheduler/generate \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "solver": {
      "algorithm": "constraint_programming",
      "timeout_minutes": 180,  ***REMOVED*** Increased timeout
      "parallel_workers": 2,   ***REMOVED*** Reduced for stability
      "constraint_weighting": {
        "80_hour_rule": 1000,  ***REMOVED*** Highest priority
        "one_in_seven": 500,
        "coverage": 100
      }
    }
...
***REMOVED*** Step 4: If isolated (1-3 residents affected):
***REMOVED*** Use swap executor to resolve
...
***REMOVED******REMOVED*** Post-Generation Verification

**Timeline:** Immediately after publication

***REMOVED******REMOVED******REMOVED*** Step 1: System Health Check

```bash
***REMOVED*** POST: Full system verification
curl -X POST http://localhost:8000/api/scheduler/post-publication-check \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Should show all GREEN
```

***REMOVED******REMOVED******REMOVED*** Step 2: Spot Check Assignments

```bash
***REMOVED*** Sample check: Random residents
...
***REMOVED*** Verify:
***REMOVED*** - All slots assigned
***REMOVED*** - No double-bookings
***REMOVED*** - Credentials match assignments
***REMOVED*** - Rotation types valid
```

***REMOVED******REMOVED******REMOVED*** Step 3: Coverage Verification

```bash
***REMOVED*** GET: Verify all blocks have adequate coverage
curl -X GET http://localhost:8000/api/scheduler/coverage/analysis \
  -H "Authorization: Bearer $TOKEN"

***REMOVED*** Expected: No blocks with zero coverage
```

***REMOVED******REMOVED******REMOVED*** Step 4: Performance Baseline

```bash
***REMOVED*** Record system performance metrics
...
***REMOVED******REMOVED******REMOVED*** Issue: "Constraint Satisfaction Failed"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Insufficient Coverage for Rotation"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Memory Exceeded During Generation"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Database Locking - Cannot Save Assignments"
...
***REMOVED******REMOVED******REMOVED*** Issue: "API Timeout Errors"
...
***REMOVED******REMOVED******REMOVED*** Schedule Generation Approval Checklist
...
***REMOVED******REMOVED******REMOVED*** Issue Tracking Template
