***REMOVED*** System Maintenance Playbook
...
2. [Database Backup Procedures](***REMOVED***database-backup-procedures)
3. [Database Restore Procedures](***REMOVED***database-restore-procedures)
4. [Log Management](***REMOVED***log-management)
5. [Cache Management](***REMOVED***cache-management)
6. [Health Check Verification](***REMOVED***health-check-verification)
7. [System Upgrades](***REMOVED***system-upgrades)
8. [Security Patching](***REMOVED***security-patching)
9. [Performance Optimization](***REMOVED***performance-optimization)
10. [Maintenance Schedule](***REMOVED***maintenance-schedule)

---

***REMOVED******REMOVED*** Overview

***REMOVED******REMOVED******REMOVED*** Maintenance Windows
...
***REMOVED******REMOVED*** Database Backup Procedures

***REMOVED******REMOVED******REMOVED*** Daily Backup (Automated)

**Frequency:** Every day at 11:00 PM

```bash
***REMOVED*** AUTOMATED (runs via Celery task)
***REMOVED*** Location: /backups/daily/backup_${date}.sql

***REMOVED*** Manual verification of automated backup
...
***REMOVED******REMOVED******REMOVED*** Weekly Backup (Full + Incremental)

**Frequency:** Every Sunday at 2:00 AM

```bash
***REMOVED*** Full weekly backup
docker-compose exec db pg_dump -U scheduler residency_scheduler \
  | gzip > /backups/weekly/backup_week$(date +%V).sql.gz

***REMOVED*** Record backup metadata
...
***REMOVED******REMOVED******REMOVED*** Monthly Full Backup + Archive

**Frequency:** First Sunday of each month at 2:00 AM

```bash
***REMOVED*** Create full backup
docker-compose exec db pg_dump -U scheduler residency_scheduler \
  | gzip > /backups/monthly/backup_$(date +%Y-%m).sql.gz

***REMOVED*** Create detailed metadata
...
***REMOVED*** Send to off-site storage
***REMOVED*** (Implementation depends on your infrastructure)
```

***REMOVED******REMOVED******REMOVED*** Backup Retention Policy
...
***REMOVED***!/bin/bash
***REMOVED*** Delete daily backups older than 7 days
find /backups/daily -name "backup_*.sql.gz" -mtime +7 -delete

***REMOVED*** Delete weekly backups older than 56 days
...
***REMOVED*** STEP 4: Stop backend to release database connections
docker-compose down backend

***REMOVED*** STEP 5: Restore database
echo "Starting restore (this may take 10-30 minutes)..."
gunzip -c $BACKUP_FILE | \
  docker-compose exec -T db psql -U scheduler residency_scheduler

***REMOVED*** STEP 6: Verify restore
docker-compose exec -T db psql -U scheduler residency_scheduler \
  -c "SELECT COUNT(*) as assignments FROM assignments;"

***REMOVED*** STEP 7: Restart backend
docker-compose up -d backend
sleep 30

***REMOVED*** STEP 8: Verify system
...
☐ Record count: [***REMOVED***] assignments
☐ Latest date: [Date]
...
```bash
***REMOVED*** STEP 1: Full system stop
docker-compose down

***REMOVED*** STEP 2: Backup current state for forensics
tar -czf /backups/forensics/system_state_$(date +%Y%m%d_%H%M%S).tar.gz \
  ./data ./backend ./frontend ./docker-compose.yml

***REMOVED*** STEP 3: Identify restore point
RESTORE_WEEK=$(date +%V)
RESTORE_DATE=$(date -d "7 days ago" +%Y-%m-%d)
BACKUP_FILE="/backups/weekly/backup_week${RESTORE_WEEK}.sql.gz"

***REMOVED*** STEP 4: Remove current database volume
docker volume rm residency_scheduler_db_data || true

***REMOVED*** STEP 5: Start database
docker-compose up -d db
sleep 30

***REMOVED*** STEP 6: Restore database
gunzip -c $BACKUP_FILE | \
  docker-compose exec -T db psql -U scheduler residency_scheduler

***REMOVED*** STEP 7: Restart all services
docker-compose up -d

***REMOVED*** STEP 8: Run comprehensive verification
sleep 30
...
***REMOVED******REMOVED******REMOVED*** Daily Log Rotation

**Frequency:** Daily at 1:00 AM

```bash
***REMOVED*** Manual log rotation (automated by logrotate)

***REMOVED*** Backend logs
docker-compose exec backend python -c \
  "import logging; logging.getLogger().handlers[0].doRollover()" 2>/dev/null || true

***REMOVED*** List current log files
ls -lh /var/log/scheduler/

***REMOVED*** View recent logs
...
/var/log/scheduler/*.log {
    ...
    sharedscripts
    postrotate
        ***REMOVED*** Restart logging if needed
...
ERROR MONITORING:
Recent errors in logs: [***REMOVED*** errors]
...
***REMOVED******REMOVED*** Cache Management

***REMOVED******REMOVED******REMOVED*** Redis Cache Operations
...
***REMOVED*** View cache statistics
docker-compose exec redis redis-cli INFO stats

***REMOVED*** Get memory usage
docker-compose exec redis redis-cli INFO memory

***REMOVED*** List cache keys (carefully!)
docker-compose exec redis redis-cli KEYS "*" | wc -l

***REMOVED*** Clear specific cache pattern
docker-compose exec redis redis-cli DEL "schedule:*"

***REMOVED*** Clear all cache (be careful!)
docker-compose exec redis redis-cli FLUSHDB
***REMOVED*** Follow with: "Are you sure? Yes, I understand the consequences"
```
...
***REMOVED******REMOVED******REMOVED*** Cache Clear Procedures

```bash
***REMOVED*** Safe cache clearing (gradual)

***REMOVED*** 1. Clear swap-related cache (safe any time)
docker-compose exec redis redis-cli DEL "swaps:*"

***REMOVED*** 2. Clear schedule cache (safe if no active operations)
curl -s http://localhost:8000/api/health | jq '.active_operations'
if [ $? -eq 0 ]; then
  docker-compose exec redis redis-cli DEL "schedule:*"
fi

***REMOVED*** 3. Clear report cache (safe to clear)
docker-compose exec redis redis-cli DEL "report:*"

***REMOVED*** 4. Full clear (only during maintenance window)
docker-compose exec redis redis-cli FLUSHDB

***REMOVED*** Verify cache is cleared
docker-compose exec redis redis-cli DBSIZE
***REMOVED*** Should return a low number
```

---

***REMOVED******REMOVED*** Health Check Verification

***REMOVED******REMOVED******REMOVED*** Daily Health Checks (Automated)

**Frequency:** Every 5 minutes (24/7)

```bash
***REMOVED*** GET: System health status
...
***REMOVED***   "cache": "online",
***REMOVED***   "api": "responding",
***REMOVED***   "timestamp": "2025-12-31T10:00:00Z"
***REMOVED*** }

***REMOVED*** GET: Extended health check
curl -s http://localhost:8000/api/health/extended | jq '.'

***REMOVED*** Expected metrics:
***REMOVED*** - response_time_ms: < 100
***REMOVED*** - database_query_time_ms: < 50
***REMOVED*** - cache_hit_rate: > 80%
***REMOVED*** - error_rate: < 0.1%
```
...
***REMOVED******REMOVED******REMOVED*** Weekly Deep Health Check

**Frequency:** Every Sunday at 5:00 AM

```bash
***REMOVED*** Comprehensive health verification
...
***REMOVED*** 7. Container status
echo -e "\n[Containers]"
docker-compose ps

***REMOVED*** 8. Recent errors
echo -e "\n[Recent Errors]"
...
***REMOVED******REMOVED*** System Upgrades

***REMOVED******REMOVED******REMOVED*** Dependency Upgrade Procedure

```bash
***REMOVED*** 1. Identify available updates
docker-compose exec backend pip list --outdated

***REMOVED*** 2. Check changelog for breaking changes
***REMOVED*** Review: /backend/docs/CHANGELOG.md
***REMOVED*** Search for: deprecated features, breaking changes

***REMOVED*** 3. Plan update approach
***REMOVED*** - Patch updates (1.0.0 -> 1.0.1): Can be done immediately
***REMOVED*** - Minor updates (1.0.0 -> 1.1.0): Test in staging first
***REMOVED*** - Major updates (1.0.0 -> 2.0.0): Require full testing

***REMOVED*** 4. Update in development first
***REMOVED*** Create feature branch
git checkout -b upgrade/dependencies-2025-12

***REMOVED*** Edit requirements
vim backend/requirements.txt

***REMOVED*** Test locally
cd backend
pip install -r requirements.txt
pytest

***REMOVED*** Commit and PR

***REMOVED*** 5. Deploy to production
docker-compose build backend
docker-compose up -d backend
docker-compose logs -f backend | grep -i "error"

***REMOVED*** 6. Verify functionality
curl http://localhost:8000/api/health
***REMOVED*** Run smoke tests
pytest tests/smoke/
```

***REMOVED******REMOVED******REMOVED*** Docker Image Update

```bash
***REMOVED*** 1. Pull latest images
docker-compose pull

***REMOVED*** 2. Review changes
docker images | grep -E "python|postgres|redis"

***REMOVED*** 3. Test updates
docker-compose up -d --build

***REMOVED*** 4. Run full test suite
cd backend
pytest --co  ***REMOVED*** Check if tests can be collected
pytest tests/ -x  ***REMOVED*** Run and stop on first failure

***REMOVED*** 5. If all pass, commit and continue normal operations
```

---

***REMOVED******REMOVED*** Security Patching

***REMOVED******REMOVED******REMOVED*** Critical Patch Procedure

**Timeline:** Apply within 24-48 hours

```bash
***REMOVED*** 1. Identify critical patches
***REMOVED*** Monitor: security advisories, GitHub alerts, CVE database

***REMOVED*** 2. Assess impact
***REMOVED*** - What is vulnerable?
***REMOVED*** - What is exposed?
***REMOVED*** - Is it used in production?

***REMOVED*** 3. Emergency patch process
git pull origin main  ***REMOVED*** Get latest patches
docker-compose build --no-cache backend  ***REMOVED*** Rebuild
docker-compose up -d backend  ***REMOVED*** Restart
docker-compose logs -f backend | head -50  ***REMOVED*** Verify startup

***REMOVED*** 4. Run security tests
pytest tests/security/

***REMOVED*** 5. Verify no new issues
curl http://localhost:8000/api/health
```

***REMOVED******REMOVED******REMOVED*** Regular Security Updates

**Frequency:** Monthly (or as needed)

```bash
***REMOVED*** 1. Schedule update window (during maintenance)
***REMOVED*** - Notify users in advance
***REMOVED*** - Plan 2-hour window

***REMOVED*** 2. Create backup before updates
docker-compose exec db pg_dump -U scheduler residency_scheduler > \
  /backups/pre_security_patch_$(date +%Y%m%d).sql

***REMOVED*** 3. Apply updates
docker-compose pull
...
***REMOVED******REMOVED******REMOVED*** Weekly Schedule
...
***REMOVED******REMOVED******REMOVED*** Monthly Schedule
...
***REMOVED******REMOVED******REMOVED*** Quarterly Deep Maintenance
