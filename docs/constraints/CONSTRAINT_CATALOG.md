***REMOVED*** Constraint Catalog - Residency Scheduler
...
2. [Quick Reference](***REMOVED***quick-reference)
3. [Hard Constraints](***REMOVED***hard-constraints)
4. [Soft Constraints](***REMOVED***soft-constraints)
5. [Constraint Priorities](***REMOVED***constraint-priorities)
6. [Constraint Types](***REMOVED***constraint-types)
7. [Constraint Conflicts Matrix](***REMOVED***constraint-conflicts-matrix)
8. [Parameter Reference](***REMOVED***parameter-reference)
9. [Examples](***REMOVED***examples)
10. [Testing Guide](***REMOVED***testing-guide)
11. [Debugging Guide](***REMOVED***debugging-guide)
12. [Relaxation Rules](***REMOVED***relaxation-rules)
13. [Performance Tuning](***REMOVED***performance-tuning)
...
***REMOVED******REMOVED******REMOVED*** ACGME Compliance (acgme.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Availability Constraint
...
for assignment in assignments:
    if assignment.person_id in context.availability:
        if not context.availability[person_id][block_id]["available"]:
            violation()  ***REMOVED*** Mark unavailable assignment as violation
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Eighty Hour Rule Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. One In Seven Rule Constraint
...
for resident in residents:
    for each 7-day window:
        ...
        if days_worked >= 7:
            violation()  ***REMOVED*** No 24-hour break in 7 days
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Supervision Ratio Constraint
...
***REMOVED******REMOVED******REMOVED*** Capacity Constraints (capacity.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 5. One Person Per Block Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 6. Clinic Capacity Constraint
**Type:** `CAPACITY` | **Priority:** `HIGH` | **Module:** `capacity.py`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 7. Max Physicians In Clinic Constraint
**Type:** `CAPACITY` | **Priority:** `HIGH` | **Module:** `capacity.py`
...
***REMOVED******REMOVED******REMOVED*** Call Constraints (call_coverage.py, post_call.py, overnight_call.py, night_float_post_call.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 8. Post Call Auto Assignment Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 9. Night Float Post Call Constraint
...
for assignment in night_float_assignments:
    if is_night_float(assignment):
        ***REMOVED*** Night float typically Friday-Monday
        ***REMOVED*** Post-call recovery: Mon/Tue light duty
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 10. Overnight Call Coverage Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 11. Overnight Call Generation Constraint
...
for resident in residents:
    ...
    if actual_calls < expected_calls * (current_week / 52):
        warning()  ***REMOVED*** Under-scheduled on calls (soft constraint)
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 12. Call Availability Constraint
**Type:** `CALL` | **Priority:** `HIGH` | **Module:** `call_coverage.py`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 13. Adjunct Call Exclusion Constraint
...
for assignment in call_assignments:
    ...
    if faculty.is_adjunct:
        violation()  ***REMOVED*** Adjuncts cannot take call
```
...
***REMOVED******REMOVED******REMOVED*** Faculty Assignment Constraints (primary_duty.py, faculty_role.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 14. Faculty Day Availability Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 15. Faculty Primary Duty Clinic Constraint
**Type:** `AVAILABILITY` | **Priority:** `HIGH` | **Module:** `primary_duty.py`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 16. Faculty Role Clinic Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 17. Sports Medicine Faculty Clinic Constraint
**Type:** `AVAILABILITY` | **Priority:** `HIGH` | **Module:** `faculty_role.py`
...
***REMOVED******REMOVED******REMOVED*** Temporal Constraints (temporal.py, inpatient.py, fmit.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 18. Wednesday AM Intern Only Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 19. Wednesday PM Single Faculty Constraint
**Type:** `ROTATION` | **Priority:** `HIGH` | **Module:** `temporal.py`
...
wednesday_pm_clinics = assignments.filter(
    day_of_week=2,  ***REMOVED*** Wednesday
    block='PM',
    is_clinic=True
)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 20. Inverted Wednesday Constraint
**Type:** `ROTATION` | **Priority:** `HIGH` | **Module:** `temporal.py`
...
for resident in residents:
    for each 2-week period:
        ...
        if week1_wed == week2_wed:  ***REMOVED*** Both or neither
            warning()  ***REMOVED*** Soft constraint - log for review
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 21. FMIT Mandatory Call Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 22. FMIT Staffing Floor Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 23. FMIT Week Blocking Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 24. FMIT Continuity Turf Constraint
**Type:** `ROTATION` | **Priority:** `HIGH` | **Module:** `fmit.py`
...
for fmit_resident in fmit_residents:
    for patient_cohort in continuity_patients:
        ...
        if provider_count > max_providers:
            violation()  ***REMOVED*** Too many turfs
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 25. Post FMIT Recovery Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 26. Post FMIT Sunday Blocking Constraint
**Type:** `ROTATION` | **Priority:** `HIGH` | **Module:** `fmit.py`
...
for resident in fmit_residents:
    if ends_intensive_rotation(resident):
        ...
        if has_assignment(resident, next_sunday):
            violation()  ***REMOVED*** No work assigned Sunday post-intensive
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 27. Resident Inpatient Headcount Constraint
...
***REMOVED******REMOVED*** Soft Constraints
...
***REMOVED******REMOVED******REMOVED*** Equity Constraints (equity.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 28. Equity Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 29. Continuity Constraint
...
***REMOVED******REMOVED******REMOVED*** Resilience Constraints (resilience.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 30. Hub Protection Constraint
...
if hub_score > 0.6:  ***REMOVED*** Critical hub
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 31. Utilization Buffer Constraint
...
target_util = context.target_utilization  ***REMOVED*** Default 0.80
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 32. N1 Vulnerability Constraint
...
if faculty_id in context.n1_vulnerable_faculty:
    ***REMOVED*** This faculty is critical - distribute assignments to backups
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 33. Preference Trail Constraint
...
for assignment in assignments:
    ...
    if strength < 0.3:  ***REMOVED*** Strong avoidance
        penalty += weight * priority.value * (0.3 - strength)
    elif strength > 0.7:  ***REMOVED*** Strong preference
        bonus = weight * priority.value * (strength - 0.7)  ***REMOVED*** Negative penalty
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 34. Zone Boundary Constraint
...
***REMOVED******REMOVED******REMOVED*** Preference Constraints (faculty.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 35. Faculty Preference Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 36. Faculty Clinic Equity Soft Constraint
...
***REMOVED******REMOVED******REMOVED*** Coverage Constraints (capacity.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 37. Coverage Constraint
...
***REMOVED******REMOVED******REMOVED*** Call Equity Constraints (call_equity.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 38. Weekday Call Equity Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 39. Sunday Call Equity Constraint
...
for faculty in call_eligible:
    ...
    penalty += weight * priority.value * deviation * 2.0  ***REMOVED*** Extra weight
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 40. Call Spacing Constraint
**Type:** `EQUITY` | **Priority:** `MEDIUM` | **Weight:** `1.5` | **Module:** `call_equity.py`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 41. Tuesday Call Preference Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 42. Department Chief Wednesday Preference Constraint
**Type:** `PREFERENCE` | **Priority:** `MEDIUM` | **Weight:** `1.0` | **Module:** `call_equity.py`
...
***REMOVED******REMOVED******REMOVED*** FMIT Resident Constraints (inpatient.py, sports_medicine.py)

***REMOVED******REMOVED******REMOVED******REMOVED*** 43. FMIT Resident Clinic Day Constraint
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 44. Sports Medicine Resident-Faculty Alignment Constraint
...
***REMOVED******REMOVED*** Constraint Priorities
...
***REMOVED******REMOVED******REMOVED*** Solver Impact
...
***REMOVED******REMOVED*** Constraint Types
...
***REMOVED******REMOVED*** Constraint Conflicts Matrix

***REMOVED******REMOVED******REMOVED*** Potential Conflicts
...
***REMOVED******REMOVED******REMOVED*** Conflict Resolution Strategy
...
***REMOVED******REMOVED*** Parameter Reference

***REMOVED******REMOVED******REMOVED*** Common Parameters
...
***REMOVED******REMOVED******REMOVED*** Context Data
...
***REMOVED******REMOVED******REMOVED*** Example 1: Building a Constraint Manager
...
***REMOVED*** Add constraints with chaining
manager.add(AvailabilityConstraint()) \
        .add(EightyHourRuleConstraint()) \
        .add(OneInSevenRuleConstraint())

***REMOVED*** Enable resilience constraints when data available
...
***REMOVED*** Soft constraint penalties
print(f"Total penalty: {result.penalty}")
```

***REMOVED******REMOVED******REMOVED*** Example 3: Selectively Disabling Constraints

```python
***REMOVED*** Disable soft constraints for faster solving
manager.disable("Equity") \
        .disable("Continuity") \
        .disable("PreferenceTrail")

***REMOVED*** Keep hard constraints enabled
...
***REMOVED******REMOVED******REMOVED*** Unit Testing Constraints
...
def test_eighty_hour_rule_satisfied(constraint, context):
    ...
    assignments = create_assignments(70)  ***REMOVED*** 70 hours
...
def test_eighty_hour_rule_violated(constraint, context):
    ...
    assignments = create_assignments(100)  ***REMOVED*** 100 hours
    result = constraint.validate(assignments, context)
...
def test_constraint_manager_with_multiple_violations():
    ...
    ***REMOVED*** Create schedule with multiple violations
    assignments = create_invalid_schedule()

    result = manager.validate_all(assignments, context)
    assert not result.satisfied
    assert len(result.violations) >= 2  ***REMOVED*** At least 2 violation types
```

---

***REMOVED******REMOVED*** Debugging Guide

***REMOVED******REMOVED******REMOVED*** Identifying Constraint Violations

```python
***REMOVED*** Get detailed violation info
for violation in result.violations:
...
***REMOVED******REMOVED******REMOVED*** Checking Constraint Status
...
***REMOVED*** Apply constraints with debug output
manager.apply_to_cpsat(model, variables, context)
***REMOVED*** Look for error messages in logs
```

---

***REMOVED******REMOVED*** Relaxation Rules

When solving fails due to constraint conflicts, use relaxation in this order:

***REMOVED******REMOVED******REMOVED*** Tier 1: Disable Low-Priority Soft Constraints
```python
manager.disable("PreferenceTrail")  ***REMOVED*** Weight 0.5
manager.disable("FacultyPreference")  ***REMOVED*** Low priority preferences
manager.disable("CallSpacing")  ***REMOVED*** Can tolerate tighter spacing
```

***REMOVED******REMOVED******REMOVED*** Tier 2: Reduce Medium-Priority Soft Constraints
```python
manager.disable("Continuity")  ***REMOVED*** Reduce continuity requirement
manager.disable("Equity")  ***REMOVED*** Allow some inequity
***REMOVED*** Reduce weights on resilience constraints
```

***REMOVED******REMOVED******REMOVED*** Tier 3: Relax High-Priority Soft Constraints
```python
manager.disable("UtilizationBuffer")  ***REMOVED*** Allow > 80% util
manager.disable("CallEquity")  ***REMOVED*** Allow unequal call distribution
```

***REMOVED******REMOVED******REMOVED*** Tier 4: Hard Constraint Relaxation (Last Resort)
```python
***REMOVED*** Only if absolutely necessary - discuss with human
manager.disable("PostCallAutoAssignment")  ***REMOVED*** Risk: burnout
manager.disable("SupervisionRatio")  ***REMOVED*** Risk: compliance violation
***REMOVED*** NEVER disable: Availability, 80-Hour Rule, 1-in-7 Rule
```
...
***REMOVED******REMOVED*** Constraint Registration
...
***REMOVED*** Auto-register constraint
...
***REMOVED*** Retrieve registered constraints
...
***REMOVED******REMOVED******REMOVED*** From Version 1.0 → 2.0
...
**Migration:**
```python
***REMOVED*** Old: Manual constraint creation
constraints = [
    AvailabilityConstraint(),
    EightyHourRuleConstraint(),
]

***REMOVED*** New: Using manager with chaining
manager = ConstraintManager()
