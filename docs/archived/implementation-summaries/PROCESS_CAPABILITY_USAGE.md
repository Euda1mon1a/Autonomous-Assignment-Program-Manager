***REMOVED*** Six Sigma Process Capability Analysis - Usage Guide
...
***REMOVED*** Create analyzer instance
analyzer = ScheduleCapabilityAnalyzer(min_sample_size=30)

***REMOVED*** Analyze weekly resident hours
weekly_hours = [65, 72, 58, 75, 68, 70, 62, 77, 64, 69, 71, 66]

report = analyzer.analyze_workload_capability(
    weekly_hours,
    min_hours=40.0,      ***REMOVED*** Lower spec limit
    max_hours=80.0,      ***REMOVED*** Upper spec limit (ACGME)
    target_hours=60.0    ***REMOVED*** Ideal target
)

***REMOVED*** Check results
print(f"Capability: {report.capability_status}")  ***REMOVED*** EXCELLENT, CAPABLE, MARGINAL, or INCAPABLE
print(f"Cpk: {report.cpk:.3f}")                    ***REMOVED*** Process capability index
print(f"Sigma Level: {report.sigma_level:.2f}σ")  ***REMOVED*** Quality level
```

***REMOVED******REMOVED*** Understanding the Metrics

***REMOVED******REMOVED******REMOVED*** Cp (Process Capability)
...
***REMOVED******REMOVED******REMOVED*** Cpk (Centered Process Capability)
...
***REMOVED******REMOVED******REMOVED*** Cpm (Taguchi Capability)
...
***REMOVED******REMOVED******REMOVED*** Sigma Level
...
***REMOVED******REMOVED******REMOVED*** 1. ACGME Compliance Monitoring
...
***REMOVED*** Collect 12 weeks of data
weekly_hours = [65, 72, 58, 75, 68, 70, 62, 77, 64, 69, 71, 66]

report = analyzer.analyze_workload_capability(
    weekly_hours,
    min_hours=40.0,
    max_hours=80.0,  ***REMOVED*** ACGME limit
    target_hours=60.0
)
...
***REMOVED******REMOVED******REMOVED*** 3. Process Improvement Tracking
...
***REMOVED*** Baseline (before improvement)
baseline_hours = [...]  ***REMOVED*** Old scheduling system
baseline_report = analyzer.analyze_workload_capability(baseline_hours, 40, 80)

***REMOVED*** After improvement
improved_hours = [...]  ***REMOVED*** New scheduling system
...
***REMOVED******REMOVED******REMOVED*** 4. Integration with Resilience Monitoring
...
***REMOVED******REMOVED******REMOVED*** ProcessCapabilityReport Fields

```python
report = analyzer.analyze_workload_capability(data, 40, 80)

***REMOVED*** Core indices
report.cp       ***REMOVED*** Process potential (assumes centered)
report.cpk      ***REMOVED*** Process capability (actual) ⭐ Use this for decisions
report.pp       ***REMOVED*** Process performance (long-term Cp)
report.ppk      ***REMOVED*** Process performance (long-term Cpk)
report.cpm      ***REMOVED*** Taguchi index (penalizes off-target)

***REMOVED*** Classification
report.capability_status  ***REMOVED*** "EXCELLENT", "CAPABLE", "MARGINAL", "INCAPABLE"
report.sigma_level       ***REMOVED*** Quality level (e.g., 4.5σ)

***REMOVED*** Statistics
report.sample_size       ***REMOVED*** Number of data points
report.mean              ***REMOVED*** Average value
report.std_dev          ***REMOVED*** Standard deviation
report.lsl              ***REMOVED*** Lower specification limit
report.usl              ***REMOVED*** Upper specification limit
report.target           ***REMOVED*** Target value (optional)
```

***REMOVED******REMOVED******REMOVED*** Getting a Human-Readable Summary
...
***REMOVED******REMOVED*** Decision Guidelines

***REMOVED******REMOVED******REMOVED*** When to Deploy a Schedule Generator
...
***REMOVED******REMOVED******REMOVED*** When to Investigate a Process
...
***REMOVED******REMOVED******REMOVED*** Custom Specification Limits

```python
***REMOVED*** Coverage rate analysis (0-100%)
coverage_rates = [0.98, 0.97, 0.99, 0.96, 0.98, 0.97]
report = analyzer.analyze_workload_capability(
    coverage_rates,
    min_hours=0.95,  ***REMOVED*** 95% minimum coverage
    max_hours=1.0,   ***REMOVED*** 100% maximum
    target_hours=1.0 ***REMOVED*** Aim for 100%
)
```

***REMOVED******REMOVED******REMOVED*** Individual Index Calculations

```python
***REMOVED*** Calculate just Cp
cp = analyzer.calculate_cp(data, lsl=40, usl=80)

***REMOVED*** Calculate just Cpk
cpk = analyzer.calculate_cpk(data, lsl=40, usl=80)

***REMOVED*** Calculate Cpm with specific target
cpm = analyzer.calculate_cpm(data, lsl=40, usl=80, target=60)

***REMOVED*** Convert Cpk to sigma level
sigma = analyzer.get_sigma_level(cpk)

***REMOVED*** Classify capability
status = analyzer.classify_capability(cpk)  ***REMOVED*** Returns str
```
...
```bash
***REMOVED*** Run all process capability tests
cd backend
