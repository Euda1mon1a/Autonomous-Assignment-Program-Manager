***REMOVED*** Schedule Generation Runbook
...
***REMOVED******REMOVED*** Detailed Steps

***REMOVED******REMOVED******REMOVED*** Step 1: Verify Docker Containers
...
**If NOT healthy:**
```bash
***REMOVED*** Restart unhealthy containers
docker-compose restart <container-name>

***REMOVED*** View logs for troubleshooting
docker-compose logs -f <container-name>
```

---

***REMOVED******REMOVED******REMOVED*** Step 2: Verify Database Connectivity
...
***REMOVED*** Show people breakdown by role
docker-compose exec db psql -U scheduler -d residency_scheduler -c "
...
***REMOVED******REMOVED******REMOVED*** Step 3: Run Health Check Endpoints
...
***REMOVED******REMOVED******REMOVED*** Step 4: Generate a Schedule
...
**What Claude Code should run:**
```bash
***REMOVED*** Generate with idempotency key to prevent duplicates
...
***REMOVED******REMOVED******REMOVED*** Step 5: Verify the Generated Schedule
...
***REMOVED******REMOVED******REMOVED*** Step 6: Human-Readable Schedule Summary
...
```bash
***REMOVED*** Check if postgres is accepting connections
docker-compose exec db pg_isready -U scheduler

***REMOVED*** Reset database (WARNING: destroys data)
...
```bash
***REMOVED*** Try with longer timeout
curl -X POST http://localhost:8000/api/v1/schedule/generate \
  -d '{"timeout_seconds": 600, ...}'

***REMOVED*** Or use greedy algorithm (faster, lower quality)
curl -X POST http://localhost:8000/api/v1/schedule/generate \
  -d '{"algorithm": "greedy", ...}'
```

***REMOVED******REMOVED******REMOVED*** Violations After Generation
```bash
***REMOVED*** Check specific violations
curl -s "http://localhost:8000/api/v1/schedule/validate?start_date=2026-03-10&end_date=2026-04-06"

***REMOVED*** Run conflict detection with resolution suggestions
...
***REMOVED******REMOVED******REMOVED*** Insufficient Coverage
...
***REMOVED******REMOVED******REMOVED*** Solver Hangs or Takes Too Long
...
**Diagnosis:**
```bash
***REMOVED*** Check active solver runs
...
2. **Try simpler algorithm:**
   ```bash
   ***REMOVED*** Use greedy instead of CP-SAT
   curl -X POST "http://localhost:8000/api/v1/schedule/generate" \
...
**Diagnosis:**
```bash
***REMOVED*** Check container memory usage
docker stats --no-stream backend

***REMOVED*** Check logs for memory errors
...
1. **Increase Docker memory limit:**
   ...
   ```yaml
   backend:
     mem_limit: 4g  ***REMOVED*** Increase from default
   ```
...
***REMOVED******REMOVED******REMOVED*** Schedule Validation Fails
...
**Diagnosis:**
```bash
***REMOVED*** Test minimal valid request
curl -X POST "http://localhost:8000/api/v1/schedule/generate" \
  -H "Content-Type: application/json" \
  -d '{
    "start_date": "2026-03-10",
...
***REMOVED******REMOVED******REMOVED*** Idempotency Conflicts
...
**Diagnosis:**
```bash
***REMOVED*** Check idempotency key status
...
2. **Clear old idempotency records (admin only):**
   ```bash
   ***REMOVED*** Delete idempotency records older than 24 hours
...
**Diagnosis:**
```bash
***REMOVED*** Check current user's role
...
**Diagnosis:**
```bash
***REMOVED*** Check rate limit headers
curl -I "http://localhost:8000/api/v1/schedule/generate" \
  -H "Authorization: Bearer $TOKEN"
***REMOVED*** Look for X-RateLimit-* headers
```
...
2. Use exponential backoff:
   ...
   for i in {1..5}; do
     curl -X POST "http://localhost:8000/api/v1/schedule/generate" \
       -d '{...}' && break
     sleep $((2**i))  ***REMOVED*** Wait 2, 4, 8, 16, 32 seconds
   done
   ```

***REMOVED******REMOVED******REMOVED*** MCP Server Unavailable
...
**Diagnosis:**
```bash
***REMOVED*** Check MCP server status
docker-compose ps mcp-server

***REMOVED*** Check MCP server health
curl -s "http://localhost:8080/health"  ***REMOVED*** If using HTTP transport
...
***REMOVED******REMOVED*** Post-Generation Checklist
...
***REMOVED******REMOVED*** Emergency: Abort Running Solver

If schedule generation is taking too long or behaving unexpectedly:

```bash
***REMOVED*** Find active runs
...
- [ACGME Compliance](../../CLAUDE.md***REMOVED***acgme-compliance-rules) - Compliance requirements
