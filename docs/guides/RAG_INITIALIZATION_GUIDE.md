***REMOVED*** RAG Initialization Guide
...
***REMOVED******REMOVED*** Knowledge Base Documents
...
***REMOVED******REMOVED*** Method 1: CLI Script (Recommended for Initial Setup)

***REMOVED******REMOVED******REMOVED*** Prerequisites

```bash
***REMOVED*** Ensure backend dependencies are installed
cd backend
pip install -r requirements.txt

***REMOVED*** Ensure PostgreSQL with pgvector extension is running
docker-compose up -d db

***REMOVED*** Run migrations to create rag_documents table
...
***REMOVED******REMOVED*** Method 2: Celery Tasks (Recommended for Production)

***REMOVED******REMOVED******REMOVED*** Prerequisites

```bash
***REMOVED*** Ensure Celery and Redis are running
../scripts/start-celery.sh both
```

***REMOVED******REMOVED******REMOVED*** Available Tasks

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Initialize All Embeddings

```python
from app.tasks import initialize_embeddings

***REMOVED*** Queue the task
...
print(result)
***REMOVED*** {
***REMOVED***   "timestamp": "2025-12-29T12:00:00",
***REMOVED***   "status": "completed",
***REMOVED***   "documents_processed": 6,
***REMOVED***   "documents_successful": 6,
***REMOVED***   "total_chunks": 412,
***REMOVED***   "results": [...]
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Refresh Single Document

```python
from app.tasks import refresh_single_document

***REMOVED*** Refresh just the ACGME rules
task = refresh_single_document.delay("acgme-rules.md")
result = task.get(timeout=60)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Check RAG Health
...
***REMOVED***   "total_documents": 412,
***REMOVED***   "documents_by_type": {
***REMOVED***     "acgme_rules": 87,
***REMOVED***     "scheduling_policy": 65,
***REMOVED***     ...
***REMOVED***   },
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Periodic Refresh (Scheduled)

```python
from app.tasks import periodic_refresh

***REMOVED*** This is typically called by Celery Beat on a schedule
***REMOVED*** But can be triggered manually:
task = periodic_refresh.delay()
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Clear All Embeddings
...
***REMOVED***   "timestamp": "2025-12-29T12:00:00",
***REMOVED***   "status": "completed",
***REMOVED***   "total_deleted": 412
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED*** Scheduling with Celery Beat
...
app.conf.beat_schedule = {
    ***REMOVED*** ... other schedules ...

    "refresh-rag-embeddings-weekly": {
        "task": "app.tasks.rag_tasks.periodic_refresh",
        "schedule": crontab(day_of_week=1, hour=2, minute=0),  ***REMOVED*** Monday 2 AM
    },

    "check-rag-health-daily": {
        "task": "app.tasks.rag_tasks.check_rag_health",
        "schedule": crontab(hour=6, minute=0),  ***REMOVED*** Daily at 6 AM
    },
}
```

***REMOVED******REMOVED*** Method 3: Python API (Programmatic Access)
...
***REMOVED*** Create database session
db = SessionLocal()
rag_service = RAGService(db)

try:
    ***REMOVED*** Read a document
    doc_path = Path("docs/rag-knowledge/acgme-rules.md")
    content = doc_path.read_text()

    ***REMOVED*** Ingest document
...
***REMOVED******REMOVED******REMOVED*** Issue: "pgvector extension not found"
...
***REMOVED*** Enable extension
CREATE EXTENSION IF NOT EXISTS vector;
```

***REMOVED******REMOVED******REMOVED*** Issue: "No documents found"
...
***REMOVED******REMOVED******REMOVED*** Issue: "Insufficient samples" or empty results
...
***REMOVED******REMOVED******REMOVED*** Issue: Task timeout

**Solution:**
Increase timeout for large knowledge bases:
```python
task = initialize_embeddings.delay()
result = task.get(timeout=600)  ***REMOVED*** 10 minutes
```

***REMOVED******REMOVED******REMOVED*** Issue: Memory errors during embedding

**Solution:**
Reduce batch size in embedding service or process documents individually:
```bash
***REMOVED*** Process one at a time
...
***REMOVED******REMOVED******REMOVED*** Check embeddings were created

```bash
***REMOVED*** Via CLI script
python scripts/init_rag_embeddings.py --dry-run
...
***REMOVED******REMOVED******REMOVED*** Test semantic search
...
try:
    ***REMOVED*** Search for ACGME rules
...
***REMOVED******REMOVED******REMOVED*** Embedding Model
...
***REMOVED******REMOVED******REMOVED*** Database Indexes
...
async def answer_scheduling_question(question: str):
    ...
    ***REMOVED*** Build context from knowledge base
    ...
    ***REMOVED*** Use context in LLM prompt
    prompt = f"""
...
***REMOVED******REMOVED******REMOVED*** When to re-index
...
```bash
***REMOVED*** Backup rag_documents table
