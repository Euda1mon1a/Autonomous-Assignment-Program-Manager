***REMOVED*** Boolean Algebra: The Rosetta Stone
...
***REMOVED******REMOVED*** The Unifying Pattern

***REMOVED******REMOVED******REMOVED*** What CSG Actually Does
...
***REMOVED******REMOVED******REMOVED*** What N-1/N-2 Actually Does

From `backend/app/resilience/contingency.py:305-330`:

```python
***REMOVED*** N-2: What if TWO faculty are unavailable?
...
***REMOVED******REMOVED******REMOVED*** What Blast Radius Isolation Actually Does
...
***REMOVED******REMOVED******REMOVED*** What Epidemiology Actually Does
...
***REMOVED******REMOVED******REMOVED*** What Fire Weather Index Actually Does

The CFFDRS combines **multiple temporal sets**:

```
FFMC = f(recent_workload)      ***REMOVED*** 2-week set
DMC = f(monthly_workload)       ***REMOVED*** 3-month set
DC = f(yearly_satisfaction)     ***REMOVED*** 12-month set
```
...
***REMOVED******REMOVED*** The Cross-Disciplinary Framework as Boolean Algebra
...
***REMOVED******REMOVED*** The Rosetta Stone
...
***REMOVED******REMOVED******REMOVED*** 1. Conceptual Unification
...
***REMOVED******REMOVED******REMOVED*** 2. Implementation Clarity
...
***REMOVED******REMOVED******REMOVED*** 3. New Insights
...
***REMOVED******REMOVED*** Spatial and Temporal Dimensions
...
***REMOVED******REMOVED*** Appendix A: Set Theory Foundations

*For the physician who'd rather be reading about cytokines.*

***REMOVED******REMOVED******REMOVED*** The Universal Set (U)
...
***REMOVED******REMOVED******REMOVED*** Basic Sets
...
***REMOVED******REMOVED******REMOVED*** Fundamental Operations

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Union (A ∪ B) — "OR"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Intersection (A ∩ B) — "AND"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Difference (A - B) or (A \ B) — "BUT NOT"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Complement (Aᶜ) or (¬A) — "NOT"
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Symmetric Difference (A △ B) — "XOR"
...
***REMOVED******REMOVED******REMOVED*** Set Properties
...
***REMOVED******REMOVED******REMOVED*** Cardinality
...
```python
***REMOVED*** "Do we have enough coverage?"
available_dedicated = len(set(self.dedicated_faculty) & available_faculty)
...
***REMOVED******REMOVED******REMOVED*** Partitions
...
***REMOVED******REMOVED******REMOVED*** Power Sets
...
***REMOVED******REMOVED******REMOVED*** Mapping to This Codebase
...
***REMOVED******REMOVED******REMOVED*** The Formal Definition of a Schedule
...
***REMOVED******REMOVED*** Appendix B: Matrix Representations
...
***REMOVED******REMOVED******REMOVED*** The Assignment Matrix
...
***REMOVED******REMOVED******REMOVED*** Matrix Operations = Set Operations
...
***REMOVED******REMOVED******REMOVED*** The Adjacency Matrix (Networks)
...
***REMOVED******REMOVED******REMOVED*** The Transition Matrix (Markov Chains)
...
***REMOVED******REMOVED******REMOVED*** The Constraint Matrix (Optimization)
...
***REMOVED******REMOVED******REMOVED*** Why Matrices Matter Here
...
***REMOVED******REMOVED******REMOVED*** The Connection: Sets, Matrices, Graphs
...
***REMOVED******REMOVED******REMOVED*** Matrix Example: Finding Critical Faculty

The hub analysis module computes **eigenvector centrality**. In matrix terms:

```python
***REMOVED*** Simplified version of what NetworkX does internally
import numpy as np

***REMOVED*** Adjacency matrix A
A = np.array([[0,1,1,0],
              [1,0,1,1],
              [1,1,0,0],
              [0,1,0,0]])

***REMOVED*** Eigenvector centrality = dominant eigenvector
...
***REMOVED******REMOVED*** Appendix C: Code Examples

***REMOVED******REMOVED******REMOVED*** Set Intersection (Static Stability)

```python
***REMOVED*** backend/app/resilience/static_stability.py:77-82
available_dedicated = len(set(self.dedicated_faculty) & available_faculty)
return available_dedicated >= self.minimum_coverage
```

***REMOVED******REMOVED******REMOVED*** Set Union + Difference (N-2 Contingency)

```python
***REMOVED*** backend/app/resilience/contingency.py:305-330
combined_assignments = set()
for a in assignments_by_faculty.get(fac1.id, []):
    combined_assignments.add(a.block_id)
for a in assignments_by_faculty.get(fac2.id, []):
...
***REMOVED*** Then: uncovered = combined_assignments - covered_by_others
```

***REMOVED******REMOVED******REMOVED*** Disjoint Partitioning (Burden Categories)

```python
***REMOVED*** backend/app/resilience/behavioral_network.py:777-781
...
***REMOVED*** AllFaculty = crushing ⊔ heavy ⊔ balanced ⊔ light ⊔ very_light
```

***REMOVED******REMOVED******REMOVED*** Complement (Constraint Checking)

```python
***REMOVED*** backend/app/scheduling/constraints/acgme.py:104-120
if not context.availability[person_id][block_id]["available"]:
    violations.append(...)
***REMOVED*** This is: person ∈ ¬Available
```
