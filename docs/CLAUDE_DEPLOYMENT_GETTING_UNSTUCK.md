***REMOVED*** Claude Deployment: Getting Unstuck (Docker Desktop macOS)
...
***REMOVED******REMOVED*** 0) One-Screen Triage (Crashloop)
...
***REMOVED******REMOVED*** 1) Validate Dependencies in the Running Image
...
***REMOVED******REMOVED*** 2) Check DB Migrations vs Models
...
***REMOVED******REMOVED*** 3) Confirm API Base URL (Avoid 307 CORS Redirect)
...
***REMOVED******REMOVED*** 4) Ensure Cookies Are Sent (Auth Looks Broken Otherwise)
...
***REMOVED******REMOVED*** 5) Verify Required Data Exists (People + Templates)

Schedule generation fails without people and rotation templates. Confirm both before running schedules.

Commands:
```bash
***REMOVED*** Count people (requires DB access)
docker compose -f docker-compose.local.yml exec backend python - <<'PY'
...
***REMOVED******REMOVED*** 6) Nuke-and-Rebuild (When State Is Corrupt)
...
***REMOVED******REMOVED*** 7) Docker Desktop macOS Notes (File Sync)
...
***REMOVED******REMOVED*** 8) Known Failure Patterns to Watch
...
***REMOVED******REMOVED*** 9) Minimal Health Checks
...
***REMOVED******REMOVED*** 10) When You Need a Log Bundle
