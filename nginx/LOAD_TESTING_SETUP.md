***REMOVED*** Load Testing Setup Guide
...
***REMOVED******REMOVED******REMOVED*** 1. Generate Load Test Secret Key

```bash
***REMOVED*** Generate a secure random key (32+ characters)
...
***REMOVED******REMOVED******REMOVED*** 2. Configure the Secret Key
...
map $http_x_load_test_key $load_test_mode {
    default 0;
    ***REMOVED*** Replace with your generated key
...
***REMOVED******REMOVED******REMOVED*** 3. Configure IP Whitelist (Optional)
...
geo $load_test_whitelist {
    ...
    ***REMOVED*** Default whitelisted networks (already configured)
    127.0.0.1/32 1;        ***REMOVED*** Localhost
    ::1/128 1;             ***REMOVED*** IPv6 localhost
    10.0.0.0/8 1;          ***REMOVED*** Private network
    172.16.0.0/12 1;       ***REMOVED*** Docker networks
    192.168.0.0/16 1;      ***REMOVED*** Local network

    ***REMOVED*** Add your CI/CD servers here:
    203.0.113.0/24 1;      ***REMOVED*** Example: Jenkins server subnet
    198.51.100.50/32 1;    ***REMOVED*** Example: Specific load test runner
}
```

***REMOVED******REMOVED******REMOVED*** 4. Enable Load Test Locations
...
server {
    ...
    ***REMOVED*** Add this line:
    include /etc/nginx/snippets/loadtest-locations.conf;

    ***REMOVED*** ... rest of server block ...
}
```

***REMOVED******REMOVED******REMOVED*** 5. Test Configuration Syntax

```bash
***REMOVED*** If using Docker
docker compose exec nginx nginx -t

***REMOVED*** Or directly if nginx is installed
nginx -t
```
...
***REMOVED******REMOVED******REMOVED*** 6. Reload Nginx

```bash
***REMOVED*** Docker
docker compose exec nginx nginx -s reload

***REMOVED*** Or restart the container
docker compose restart nginx
```

***REMOVED******REMOVED*** Usage Examples

***REMOVED******REMOVED******REMOVED*** Basic API Request with Load Test Key
...
***REMOVED******REMOVED******REMOVED*** Load Test with K6
...
```bash
***REMOVED*** Set the load test key as environment variable
...
***REMOVED******REMOVED******REMOVED*** Load Test with Apache Bench (ab)

```bash
***REMOVED*** 1000 requests, 10 concurrent connections
ab -n 1000 -c 10 \
   -H "X-Load-Test-Key: YOUR_SECRET_KEY_HERE" \
   http://localhost/api/loadtest/persons
```

***REMOVED******REMOVED******REMOVED*** Load Test with Locust
...
class ResidencySchedulerUser(HttpUser):
    wait_time = between(1, 3)  ***REMOVED*** Wait 1-3 seconds between requests
...
***REMOVED*** Start Locust web UI
locust -f locustfile.py --host=http://localhost

***REMOVED*** Or headless mode
locust -f locustfile.py --host=http://localhost \
       --users 100 --spawn-rate 10 --run-time 5m --headless
```

***REMOVED******REMOVED******REMOVED*** Schedule Generation Load Test

**Warning:** Schedule generation is resource-intensive. Start with low concurrency.

```bash
***REMOVED*** Test with curl
curl -X POST \
     -H "X-Load-Test-Key: YOUR_SECRET_KEY_HERE" \
...
***REMOVED*** K6 script for schedule generation
export default function() {
  const params = {
    headers: {
...
***REMOVED******REMOVED*** Monitoring During Load Tests

***REMOVED******REMOVED******REMOVED*** View Real-time Logs

```bash
***REMOVED*** Load test specific logs (JSON format)
tail -f /var/log/nginx/loadtest.log

***REMOVED*** Parse JSON for readability
tail -f /var/log/nginx/loadtest.log | jq '.'

***REMOVED*** Filter slow requests (>1 second)
tail -f /var/log/nginx/loadtest.log | jq 'select(.request_time > "1")'
```

***REMOVED******REMOVED******REMOVED*** Check Nginx Status
...
***REMOVED******REMOVED******REMOVED*** Monitor Backend Metrics

```bash
***REMOVED*** Prometheus metrics from backend
...
***REMOVED******REMOVED******REMOVED*** Monitor System Resources

```bash
***REMOVED*** CPU and memory usage
docker stats

***REMOVED*** Detailed container stats
docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}"
```

***REMOVED******REMOVED*** Rate Limit Comparison
...
***REMOVED******REMOVED******REMOVED*** 403 Forbidden Error
...
***REMOVED******REMOVED******REMOVED*** Rate Limiting During Load Test
...
2. **Check burst settings**: The burst allows temporary spikes. Increase if needed in `loadtest-locations.conf`:
   ```nginx
   limit_req zone=loadtest_api burst=500 nodelay;  ***REMOVED*** Increased from 200
   ```
...
***REMOVED******REMOVED******REMOVED*** Connection Pooling Not Working
...
2. Check if connections are being reused:
   ```bash
   ***REMOVED*** Monitor connection count
...
location /api/loadtest/schedule/generate {
    ***REMOVED*** Increase from 600s (10 min) to 1200s (20 min)
    proxy_send_timeout 1200s;
    proxy_read_timeout 1200s;

    ***REMOVED*** ... rest of config
}
```
...
***REMOVED******REMOVED*** Cleanup After Load Testing

***REMOVED******REMOVED******REMOVED*** Disable Load Test Endpoints (Recommended)

If you're not actively running load tests, disable the endpoints:

1. **Comment out the include** in `conf.d/default.conf`:
   ```nginx
   ***REMOVED*** include /etc/nginx/snippets/loadtest-locations.conf;
   ```
...
***REMOVED******REMOVED******REMOVED*** Archive Load Test Logs

```bash
***REMOVED*** Create archive directory
mkdir -p /var/log/nginx/loadtest-archive

***REMOVED*** Move old logs
...
***REMOVED******REMOVED******REMOVED*** Multiple Load Test Environments
...
***REMOVED******REMOVED******REMOVED*** Custom Rate Limits Per Team

```nginx
***REMOVED*** Define team-specific zones
...
***REMOVED*** Use in location blocks
location /api/loadtest/ {
    ***REMOVED*** Apply appropriate zone based on team
    limit_req zone=team_alpha burst=300 nodelay;
    ***REMOVED*** ... rest of config
}
```

***REMOVED******REMOVED******REMOVED*** Gradual Rate Limit Increase

For gradual ramp-up scenarios:

```nginx
***REMOVED*** Time-based rate limiting (slower during business hours)
map $hour $loadtest_rate_zone {
    default loadtest_api;           ***REMOVED*** 100 req/s
    ~^(09|10|11|12|13|14|15|16)$ loadtest_api_slow;  ***REMOVED*** 50 req/s
}
