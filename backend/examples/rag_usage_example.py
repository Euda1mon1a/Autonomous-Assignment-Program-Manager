...
async def example_ingest_acgme_rules(db: Session):
    ...
    response = await service.ingest_document(
        ...
        chunk_size=300,  ***REMOVED*** Smaller chunks for this example
        chunk_overlap=50,
    )

    print(f"✅ Status: {response.status}")
    print(f"📄 Chunks created: {response.chunks_created}")
    print(f"🆔 Chunk IDs: {response.chunk_ids[:3]}...")  ***REMOVED*** Show first 3
    print()
...
async def example_semantic_search(db: Session):
    ...
    for query in queries:
        ...
        response = await service.retrieve(
            query=query,
            top_k=3,
            min_similarity=0.3,  ***REMOVED*** Lower threshold for demo
        )
...
async def example_filtered_search(db: Session):
    ...
    response = await service.retrieve(
        ...
        doc_type="acgme_rules",  ***REMOVED*** Filter to only ACGME docs
        top_k=5,
    )
