...
def calculate_process_capability_manual_demo():
    ...
    ***REMOVED*** Sample data: weekly resident hours
    weekly_hours = [65, 72, 58, 75, 68, 70, 62, 77, 64, 69, 71, 66]

    ***REMOVED*** Specification limits (ACGME: 80 hours max, practical min: 40 hours)
    LSL = 40.0  ***REMOVED*** Lower Specification Limit
    USL = 80.0  ***REMOVED*** Upper Specification Limit
    TARGET = 60.0  ***REMOVED*** Ideal target hours

    print(f"Weekly Hours Data: {weekly_hours}")
    print(f"Specification Limits: LSL={LSL}, USL={USL}, Target={TARGET}")
    print()

    ***REMOVED*** Calculate basic statistics
    ...
    ***REMOVED*** Calculate Cp (Process Capability - assumes centered)
    ***REMOVED*** Cp = (USL - LSL) / (6σ)
    ...
    ***REMOVED*** Calculate Cpk (Process Capability accounting for centering)
    ***REMOVED*** Cpk = min[(USL - μ) / 3σ, (μ - LSL) / 3σ]
    cpu = (USL - mean) / (3 * std_dev)  ***REMOVED*** Upper capability
    cpl = (mean - LSL) / (3 * std_dev)  ***REMOVED*** Lower capability
    cpk = min(cpu, cpl)
    ...
    ***REMOVED*** Calculate Cpm (Taguchi - penalizes off-target)
    ***REMOVED*** Cpm = (USL - LSL) / (6 × √[σ² + (μ - T)²])
...
def demonstrate_good_vs_poor_scheduling():
    ...
    ***REMOVED*** Good scheduling: tight control, well-centered
    good_schedule = [
        60,
        62,
        58,
    ...
    ***REMOVED*** Poor scheduling: high variation, off-center
    poor_schedule = [
        45,
        75,
        50,
...
def demonstrate_acgme_monitoring():
    ...
    ***REMOVED*** Simulate 12 weeks of data
    weeks = [
        ("Week 1", 65),
        ("Week 2", 72),
        ("Week 3", 58),
    ...
    ***REMOVED*** Predict probability of violation
    ***REMOVED*** Approximate: For Cpk=1.33, ~66 PPM (0.0066%) outside specs
    ***REMOVED*** For Cpk=1.0, ~2700 PPM (0.27%) outside specs
    if cpk >= 1.67:
        risk = "VERY LOW (<0.001%)"
...
def demonstrate_practical_use_case():
    ...
    ***REMOVED*** Algorithm A: Simple round-robin (poor balancing)
    algo_a_hours = [55, 70, 48, 75, 52, 68, 58, 72, 50, 74, 56, 69]

    ***REMOVED*** Algorithm B: Optimized with load balancing (good)
