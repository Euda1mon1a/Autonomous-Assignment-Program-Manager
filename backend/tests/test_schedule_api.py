...
class TestGenerateSchedule:
    ...
    def test_generate_schedule_success(
        ...
        ***REMOVED*** Issue ***REMOVED***5: Now returns 200 for success, 207 for partial, 422 for failure
    ...
    def test_generate_schedule_with_pgy_filter(
        ...
        payload = {
            "start_date": start_date.isoformat(),
            "end_date": end_date.isoformat(),
            "pgy_levels": [1, 2],  ***REMOVED*** Only PGY-1 and PGY-2
            "algorithm": "greedy",
        }
    ...
    def test_generate_schedule_no_residents(
        ...
        response = client.post("/api/schedule/generate", json=payload)
        ***REMOVED*** Issue ***REMOVED***5: Failed generation now returns 422 Unprocessable Entity
    ...
    def test_generate_schedule_invalid_date_range(self, client: TestClient):
        ...
        payload = {
            ...
            "end_date": "2024-01-01",  ***REMOVED*** End before start
        }

        response = client.post("/api/schedule/generate", json=payload)
        ***REMOVED*** Should either return error or empty result
    ...
    def test_generate_schedule_prevents_double_submit(
        ...
        ***REMOVED*** Create an "in_progress" run for the same date range
        ...
        ***REMOVED*** Issue ***REMOVED***1: Should reject with 409 Conflict when generation in progress
        response = client.post("/api/schedule/generate", json=payload)
...
class TestValidateSchedule:
    ...
    def test_validate_schedule_acgme_coverage_rate(
        ...
        ***REMOVED*** Create assignments for half the weekday blocks
...
class TestGetSchedule:
    ...
    def test_get_schedule_with_assignments(
        ...
        ***REMOVED*** Check schedule structure
    ...
    def test_get_schedule_grouped_by_date(
        ...
        ***REMOVED*** Create assignments for multiple blocks
        for block in sample_blocks[:4]:  ***REMOVED*** First 2 days
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
                person_id=sample_residents[0].id,
        ...
        ***REMOVED*** Should have date keys
        for date_key in schedule:
            ***REMOVED*** Each date should have AM and PM keys
...
class TestEmergencyCoverage:
    ...
    def test_emergency_coverage_invalid_person(self, client: TestClient):
        ...
        response = client.post("/api/schedule/emergency-coverage", json=payload)
        ***REMOVED*** Should either return 404 or handle gracefully
