...
class TestOvernightCallDays:
    ...
    def test_sunday_is_call_day(self):
        ...
        sunday = date(2025, 12, 28)  ***REMOVED*** A Sunday
...
class TestOvernightCallCoverageConstraint:
    ...
    def blocks(self):
        ...
        blocks = []
        start_date = date(2025, 12, 28)  ***REMOVED*** Sunday
        for i in range(7):
    ...
    def context(self, faculty, blocks):
        ...
        return SchedulingContext(
            ...
            blocks=blocks,
            templates=[],
            call_eligible_faculty=faculty,  ***REMOVED*** All are eligible
        )
    ...
    def test_validate_with_complete_coverage(self, constraint, context, faculty, blocks):
        ...
        ***REMOVED*** Create mock assignments with one faculty per overnight call day
        assignments = []
        call_dates_assigned = set()

        for block in blocks:
            if block.date not in call_dates_assigned and is_overnight_call_day(block.date):
                ***REMOVED*** Assign first faculty to this date
                assignments.append(
                    type(
                        "MockCallAssignment",
                        (),
        ...
        ***REMOVED*** Note: Current implementation may need assignments in specific format
...
class TestAdjunctCallExclusionConstraint:
    ...
    def test_adjunct_excluded_from_call_eligible(self):
        ...
        ***REMOVED*** Filter like the engine does
...
class TestCallAssignmentModel:
    ...
    def test_create_call_assignment(self, db: Session):
        ...
        ***REMOVED*** Create a call assignment
        call = CallAssignment(
            id=uuid4(),
            date=date(2025, 12, 29),  ***REMOVED*** Monday
    ...
    def test_call_assignment_unique_constraint(self, db: Session):
        ...
        ***REMOVED*** Create first call assignment
        ...
        ***REMOVED*** Try to create duplicate - same date, person, call_type
    ...
    def test_is_weekend_or_holiday_property(self, db: Session):
        ...
        ***REMOVED*** Weekend call
        weekend_call = CallAssignment(
            date=date(2025, 12, 28),  ***REMOVED*** Sunday
            person_id=faculty.id,
            call_type="overnight",
            is_weekend=True,
            is_holiday=False,
        ...
        ***REMOVED*** Holiday call
        holiday_call = CallAssignment(
            date=date(2025, 12, 25),  ***REMOVED*** Christmas
            person_id=faculty.id,
            call_type="overnight",
            is_weekend=False,
            is_holiday=True,
...
class TestCallAssignmentService:
    ...
    def test_bulk_create_call_assignments(self, db: Session):
        ...
        ***REMOVED*** Create multiple faculty
        faculty_list = []
        for i in range(3):
            f = Person(
                id=uuid4(),
        ...
        ***REMOVED*** Create bulk assignments
        create_list = [
    ...
    def test_delete_call_assignments_in_range(self, db: Session):
        ...
        ***REMOVED*** Create assignments for a week
        for i in range(7):
...
class TestSolverSundayNightCallCoverage:
            ...
            for time_of_day in ["AM", "PM"]:
                blocks.append(
                    Block(
                        ...
                        date=block_date,
                        time_of_day=time_of_day,
                        ***REMOVED*** Weekends are 5 (Sat) and 6 (Sun)
    ...
    def test_sunday_block_has_is_weekend_true(self, week_blocks):
        ...
        assert len(sunday_blocks) == 2  ***REMOVED*** AM and PM
    ...
    def test_sunday_is_valid_overnight_call_day(self):
        ...
        assert sunday.weekday() == 6  ***REMOVED*** Confirm it's Sunday
    ...
    def test_call_blocks_include_sunday_despite_weekend_flag(self, week_blocks):
        ...
        This is the core regression test for the bug fixed in PR ***REMOVED***489.
        The solver should use explicit weekday filtering (0,1,2,3,6)
        rather than relying on is_weekend flag.
        """
        ***REMOVED*** Simulate the fixed solver logic: filter by weekday, not is_weekend
        call_blocks = [
            block
        ...
        ***REMOVED*** Get unique call dates
        call_dates = set(b.date for b in call_blocks)

        ***REMOVED*** Should include: Sun (28), Mon (29), Tue (30), Wed (31), Thu (1 Jan)
        ***REMOVED*** Should exclude: Fri (2 Jan), Sat (3 Jan)
        assert len(call_dates) == 5, f"Expected 5 call nights, got {len(call_dates)}"

        ***REMOVED*** Specifically verify Sunday is included
        sunday = date(2025, 12, 28)
    ...
    def test_context_blocks_include_all_days(self, week_blocks):
        ...
        ***REMOVED*** Verify weekends are in the full block list
        sunday = date(2025, 12, 28)
        saturday = date(2026, 1, 3)
