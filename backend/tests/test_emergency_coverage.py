...
def multiple_residents(db: Session) -> list[Person]:
    ...
    for i in range(1, 5):
        resident = Person(
            ...
            pgy_level=(i % 3) + 1,  ***REMOVED*** PGY 1, 2, 3 levels
        )
        db.add(resident)
...
class TestEmergencyAbsenceHandling:
    ...
    async def test_handle_deployment_emergency(
        ...
        ***REMOVED*** Create assignment for deployed resident
        ...
        ***REMOVED*** Verify absence was recorded
...
class TestCoverageGapIdentification:
    ...
    def test_find_affected_assignments(
        ...
        ***REMOVED*** Create multiple assignments across date range
        assignments = []
        for i in range(5):
            assignment = Assignment(
                id=uuid4(),
        ...
        ***REMOVED*** Should find assignments within date range
...
class TestEmergencyStaffingLogic:
    ...
    async def test_find_replacement_same_pgy_level(
        ...
        ***REMOVED*** Create residents at different PGY levels
        ...
        ***REMOVED*** Create assignment for PGY2
        assignment = Assignment(
            id=uuid4(),
            block_id=sample_block.id,
        ...
        ***REMOVED*** Find replacement
        replacement = await service._find_replacement(assignment)

        ***REMOVED*** Should prefer same PGY level
    ...
    async def test_find_replacement_excludes_assigned(
        ...
        ***REMOVED*** Create assignments - resident1 needs replacement, resident2 is already on same block
        ...
        ***REMOVED*** Should be the available resident, not the one already assigned
        if replacement:
    ...
    async def test_find_replacement_excludes_absent(
        ...
        ***REMOVED*** Create absence for one potential replacement
        absence = Absence(
            id=uuid4(),
        ...
        ***REMOVED*** Should be the available resident, not the absent one
        if replacement:
    ...
    async def test_find_replacement_no_candidates(
        ...
        ***REMOVED*** Create only one resident
        resident = Person(
            id=uuid4(),
        ...
        ***REMOVED*** Should return None when no candidates
...
class TestPriorityHandling:
    ...
    async def test_critical_service_creates_gap_if_no_replacement(
        ...
        ***REMOVED*** Create critical assignment
        assignment = Assignment(
            id=uuid4(),
            block_id=emergency_blocks[0].id,
            person_id=resident.id,
        ...
        ***REMOVED*** Should create coverage gap for critical service
        assert result["coverage_gaps"] > 0
        assert result["requires_manual_review"] is True
        assert result["status"] == "partial"

        ***REMOVED*** Check details for UNCOVERED status
        critical_gaps = [
    ...
    async def test_non_critical_service_cancelled_if_no_replacement(
        ...
        ***REMOVED*** Create non-critical assignment
        assignment = Assignment(
            id=uuid4(),
            block_id=emergency_blocks[0].id,
            person_id=resident.id,
        ...
        ***REMOVED*** Non-critical should be cancelled, not create gap
        assert result["coverage_gaps"] == 0
        assert result["requires_manual_review"] is False
        assert result["status"] == "success"

        ***REMOVED*** Check details for cancelled status
    ...
    async def test_mixed_critical_and_non_critical_assignments(
        ...
        ***REMOVED*** Create two residents - one to be absent, one available
        resident = Person(
            id=uuid4(),
        ...
        ***REMOVED*** Create one critical and one non-critical assignment
        ...
        ***REMOVED*** Should have replacements and/or cancellations
        assert len(result["details"]) == 2

        ***REMOVED*** Check that critical is covered or flagged
        critical_details = [
            d for d in result["details"]
...
class TestEdgeCases:
    ...
    async def test_overlapping_date_ranges(
        ...
        ***REMOVED*** Create assignments across a week
        for i in range(7):
            assignment = Assignment(
                id=uuid4(),
                block_id=emergency_blocks[i].id,
        ...
        ***REMOVED*** Should find all affected assignments
    ...
    async def test_same_day_emergency(
        ...
        ***REMOVED*** Update block to be today
        ...
        ***REMOVED*** Should handle single-day emergency
        assert result is not None
        assert "status" in result
    ...
    async def test_assignment_notes_updated_with_replacement_info(
        ...
        ***REMOVED*** Refresh assignment and check notes
        db.refresh(assignment)
    ...
    async def test_multiple_emergencies_same_block(
        ...
        ***REMOVED*** Create assignments for both residents on same block
        assignment1 = Assignment(
            id=uuid4(),
            block_id=sample_block.id,
            person_id=resident1.id,
        ...
        ***REMOVED*** Handle first emergency
        ...
        ***REMOVED*** Handle second emergency
        ...
        ***REMOVED*** Both should be handled (though may have gaps)
    ...
    async def test_person_type_matching_in_replacement(
        ...
        ***REMOVED*** Create a resident and a faculty
        resident = Person(
            id=uuid4(),
            name="Dr. Resident",
            type="resident",
        ...
        ***REMOVED*** Create assignment for resident
        assignment = Assignment(
            id=uuid4(),
            block_id=sample_block.id,
            person_id=resident.id,
        ...
        ***REMOVED*** Replacement should be same type
        if replacement:
    ...
    def test_absence_recorded_before_finding_replacements(
        ...
        ***REMOVED*** This is tested implicitly in other tests, but verify absence creation
        ***REMOVED*** happens regardless of replacement success
        pass
    ...
    async def test_result_details_structure(
        ...
        ***REMOVED*** Create both critical and non-critical assignments
        critical_assignment = Assignment(
            id=uuid4(),
            block_id=emergency_blocks[0].id,
            person_id=resident.id,
        ...
        ***REMOVED*** Verify each detail has expected fields
...
class TestIntegrationScenarios:
    ...
    async def test_full_deployment_scenario(
        ...
        ***REMOVED*** Create deployed resident and multiple available residents
        deployed = Person(
            id=uuid4(),
            name="Maj. Deployed",
            type="resident",
        ...
        ***REMOVED*** Create diverse assignments: critical, clinic, education
        assignments = [
            Assignment(
                id=uuid4(),
                block_id=emergency_blocks[0].id,
        ...
        start_date = date.today() + timedelta(days=1)
        end_date = date.today() + timedelta(days=90)  ***REMOVED*** 3-month deployment
        ...
        ***REMOVED*** Verify comprehensive handling
        assert result["status"] in ["success", "partial"]
        total_handled = (
        ...
        assert total_handled == 3  ***REMOVED*** All 3 assignments should be handled

        ***REMOVED*** Verify absence recorded as deployment
        absence = db.query(Absence).filter(
    ...
    async def test_cascading_coverage_with_limited_pool(
        ...
        ***REMOVED*** Create 3 residents, 2 need emergency coverage
        resident1 = Person(
            id=uuid4(),
            name="Dr. Emergency 1",
            type="resident",
        ...
        ***REMOVED*** Create critical assignments for both
        assignment1 = Assignment(
            id=uuid4(),
            block_id=emergency_blocks[0].id,
            person_id=resident1.id,
        ...
        ***REMOVED*** Handle first emergency - should get the available resident
        result1 = await service.handle_emergency_absence(
            person_id=resident1.id,
            start_date=start_date,
            end_date=end_date,
        ...
        ***REMOVED*** Handle second emergency - no one left
        result2 = await service.handle_emergency_absence(
            person_id=resident2.id,
            start_date=start_date,
            end_date=end_date,
        ...
        ***REMOVED*** First should potentially succeed, second should show gaps
        assert result1 is not None
        assert result2 is not None

        ***REMOVED*** At least one should require manual review
