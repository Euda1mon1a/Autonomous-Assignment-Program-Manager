...
class TestParseXlsxEndpoint:
    ...
    def test_parse_xlsx_with_empty_rows(self, client):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total_rows"] == 2  ***REMOVED*** Empty row skipped
    ...
    def test_parse_xlsx_duplicate_headers(self, client):
        ...
        xlsx_data = create_test_xlsx(
            [
                ["name", "name", "value"],  ***REMOVED*** Duplicate header
                ["A", "B", "C"],
            ]
        )
        ...
        assert response.status_code == 200
        data = response.json()
        ***REMOVED*** Second "name" should be renamed
