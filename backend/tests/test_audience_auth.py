...
class TestAudienceValidation:
    ...
    def test_validate_valid_audience(self):
        ...
        for audience in VALID_AUDIENCES:
            validate_audience(audience)  ***REMOVED*** Should not raise
...
class TestCreateAudienceToken:
    ...
    def test_create_token_success(self):
        ...
        ***REMOVED*** Verify token can be decoded
        payload = jwt.decode(
    ...
    def test_create_token_default_ttl(self):
        ...
        ***REMOVED*** Verify expiration is approximately 120 seconds from now
        now = datetime.utcnow()
        expected_expiry = now + timedelta(seconds=120)
        time_diff = abs((response.expires_at - expected_expiry).total_seconds())
        assert time_diff < 5  ***REMOVED*** Allow 5 second tolerance
...
class TestVerifyAudienceToken:
    ...
    def test_verify_wrong_audience(self, db):
        ...
        ***REMOVED*** Try to verify with different audience
    ...
    def test_verify_expired_token(self, db):
        ...
        ***REMOVED*** Create token with very short TTL
        response = create_audience_token(
            user_id=user_id,
            audience=audience,
            ttl_seconds=30,  ***REMOVED*** 30 seconds
        )

        ***REMOVED*** Wait for expiration (with clock skew tolerance)
        time.sleep(31)

        ***REMOVED*** Verify token fails
        with pytest.raises(HTTPException) as exc_info:
    ...
    def test_verify_blacklisted_token(self, db):
        ...
        ***REMOVED*** Decode to get jti
        payload = jwt.decode(
            response.token, settings.SECRET_KEY, algorithms=[ALGORITHM]
        )
        jti = payload["jti"]

        ***REMOVED*** Blacklist the token
        revoke_audience_token(
            db=db,
            jti=jti,
    ...
    def test_verify_wrong_token_type(self, db):
        ...
        ***REMOVED*** Create a regular access token (no type field)
        payload = {
            "sub": str(uuid4()),
            "aud": "jobs.abort",
            ...
            "iat": datetime.utcnow(),
            "jti": str(uuid4()),
            ***REMOVED*** No "type": "audience" field
        }
    ...
    def test_verify_missing_claims(self, db):
        ...
        ***REMOVED*** Create token missing required claims
        payload = {
            "sub": str(uuid4()),
            ***REMOVED*** Missing aud, exp, iat, jti
            "type": "audience",
        }
        token = jwt.encode(payload, settings.SECRET_KEY, algorithm=ALGORITHM)
    ...
    def test_verify_invalid_signature(self, db):
        ...
        ***REMOVED*** Tamper with token (change last character)
...
class TestRequireAudience:
    ...
    async def test_require_audience_success(self, db):
        ...
        ***REMOVED*** Call dependency with valid authorization header
    ...
    async def test_require_audience_wrong_audience(self, db):
        ...
        ***REMOVED*** Create token for different audience
        response = create_audience_token(
            user_id=user_id,
            audience="jobs.abort",
            ttl_seconds=120,
        ...
        ***REMOVED*** Try to use for different audience
...
class TestClockSkewTolerance:
    ...
    def test_verify_near_expiration_within_tolerance(self, db):
        ...
        ***REMOVED*** Create token that expires in 35 seconds
        response = create_audience_token(
            user_id=user_id,
            audience=audience,
            ttl_seconds=35,
        ...
        ***REMOVED*** Wait until just before expiration (within 30s tolerance)
        time.sleep(32)

        ***REMOVED*** Should still succeed (within clock skew tolerance)
        payload = verify_audience_token(response.token, audience, db)
        assert payload.sub == user_id
    ...
    def test_verify_past_tolerance_fails(self, db):
        ...
        ***REMOVED*** Create token with minimum TTL
        response = create_audience_token(
            user_id=user_id,
            audience=audience,
            ttl_seconds=30,
        ...
        ***REMOVED*** Wait past expiration + tolerance
        time.sleep(61)  ***REMOVED*** 30s TTL + 30s tolerance + 1s buffer
...
class TestIntegrationScenarios:
    ...
    async def test_abort_job_workflow(self, db):
        ...
        ***REMOVED*** Step 1: User requests elevated permission token
        ...
        ***REMOVED*** Step 2: User performs operation with token
        dependency = require_audience(audience)
        ...
        ***REMOVED*** Step 3: Token is revoked after use (good practice)
        ...
        ***REMOVED*** Step 4: Token cannot be reused
    ...
    def test_multiple_audiences_same_user(self, db):
        ...
        ***REMOVED*** Create tokens for different operations
        ...
        ***REMOVED*** Verify each token works for its audience
        ...
        ***REMOVED*** Verify tokens don't work for wrong audiences
        with pytest.raises(HTTPException):
...
def db():
    ...
    ***REMOVED*** Mock TokenBlacklist.is_blacklisted to track blacklisted tokens
    ...
    ***REMOVED*** Patch TokenBlacklist.is_blacklisted
