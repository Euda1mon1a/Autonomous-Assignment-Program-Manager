...
class TestAuditRepository:
    ...
    def test_get_audit_entries_with_assignment(
        ...
        ***REMOVED*** The assignment creation should have created a version entry
        entries, total = repo.get_audit_entries(
            filters={"entity_type": "assignment"}
        )

        ***REMOVED*** Note: This might be 0 if Continuum isn't fully configured in test env
        ***REMOVED*** The test validates the repository can query without errors
    ...
    def test_get_audit_entries_pagination(self, db: Session):
        ...
        ***REMOVED*** Test different page sizes
    ...
    def test_get_audit_entries_sorting(self, db: Session):
        ...
        ***REMOVED*** Test ascending sort
        entries_asc, _ = repo.get_audit_entries(
            sort_by="changed_at",
            sort_direction="asc",
        )

        ***REMOVED*** Test descending sort
    ...
    def test_mark_entries_reviewed(self, db: Session):
        ...
        ***REMOVED*** This is a placeholder that currently returns 0
        ...
        ***REMOVED*** Currently not implemented, so returns 0
        assert count == 0
...
class TestAuditService:
    ...
    def test_query_audit_logs_pagination_calculation(self, db: Session):
        ...
        ***REMOVED*** Total pages should be calculated correctly
        if result.total > 0:
    ...
    def test_get_entity_history(self, db: Session, sample_assignment: Assignment):
        ...
        ***REMOVED*** May be empty if Continuum not fully configured in tests
    ...
    def test_export_audit_logs_csv(self, db: Session):
        ...
        ***REMOVED*** Should at least have header
        assert len(csv_data) > 0
        ***REMOVED*** Verify it's valid CSV format
...
class TestAuditServiceLegacyMethods:
    ...
    def test_get_assignment_history(
        ...
        assert isinstance(history, list)
        ***REMOVED*** May be empty if Continuum not fully configured
        for entry in history:
    ...
    def test_get_recent_changes_default(self, db: Session):
        ...
        ***REMOVED*** Should look back 24 hours by default
    ...
    def test_get_recent_changes_with_model_filter(self, db: Session):
        ...
        assert isinstance(changes, list)
        for change in changes:
            if change:  ***REMOVED*** May be empty
    ...
    def test_get_changes_by_user_default(self, db: Session):
        ...
        assert isinstance(changes, list)
        ***REMOVED*** Should look back 30 days by default
...
class TestAuditIntegration:
    ...
    def test_audit_trail_on_assignment_creation(
        ...
        ***REMOVED*** Query audit logs
        ...
        assert isinstance(result, AuditLogResponse)
        ***REMOVED*** Note: May be empty if Continuum not configured in test DB
    ...
    def test_audit_trail_on_assignment_update(
        ...
        ***REMOVED*** Update the assignment
        sample_assignment.notes = "Updated notes"
        db.commit()

        ***REMOVED*** Get entity history
        history = service.get_entity_history(
            db,
            entity_type="assignment",
            entity_id=sample_assignment.id,
