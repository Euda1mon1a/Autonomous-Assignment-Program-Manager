...
def weekly_pattern_workload(sample_dates: list[date]) -> WorkloadTimeSeries:
    ...
    for i, d in enumerate(sample_dates):
        if d.weekday() < 5:  ***REMOVED*** Weekday
            hours[i] = 10.0
        else:  ***REMOVED*** Weekend
            hours[i] = 4.0
    return WorkloadTimeSeries(
        values=hours,
        dates=sample_dates,
...
def spike_workload(sample_dates: list[date]) -> WorkloadTimeSeries:
    ...
    ***REMOVED*** Add spikes at specific days
    hours[30] = 16.0  ***REMOVED*** Single spike
    hours[60:65] = 14.0  ***REMOVED*** Multi-day elevated period
    return WorkloadTimeSeries(
        values=hours,
        dates=sample_dates,
        person_id=uuid4(),
...
***REMOVED*** =============================================================================
***REMOVED*** WorkloadTimeSeries Tests
***REMOVED*** =============================================================================
...
class TestWorkloadTimeSeries:
    ...
    def test_create_with_mismatched_lengths_raises(
        ...
        with pytest.raises(ValueError, match="same length"):
            WorkloadTimeSeries(
                values=np.array([8.0] * 50),  ***REMOVED*** 50 values
                dates=sample_dates,  ***REMOVED*** 90 dates
            )
...
class TestFFTAnalysis:
    ...
    def test_fft_uniform_signal(
        ...
        ***REMOVED*** Uniform signal should have no periodicity
    ...
    def test_fft_detects_weekly_pattern(
        ...
        ***REMOVED*** Find the dominant frequency closest to 1/7 cycles/day
        weekly_freq = 1.0 / 7.0
        closest = min(
            result["dominant_frequencies"],
            key=lambda d: abs(d["frequency"] - weekly_freq),
        )
        assert abs(closest["period_days"] - 7.0) < 1.0  ***REMOVED*** Within 1 day of 7
    ...
    def test_fft_result_structure(
        ...
        ***REMOVED*** Dominant frequencies have required fields
    ...
    def test_fft_inverse_filter(
        ...
        ***REMOVED*** Filter to keep only weekly frequency
        weekly_freq = 1.0 / 7.0
        ...
        ***REMOVED*** Filtered signal should have reduced variance compared to including all frequencies
        assert np.std(filtered) < np.std(weekly_pattern_workload.values)


***REMOVED*** =============================================================================
***REMOVED*** Wavelet Transform Tests
***REMOVED*** =============================================================================
...
class TestWaveletTransform:
    ...
    def test_dwt_decomposes_signal(
        ...
        ***REMOVED*** Should have multiple detail levels
    ...
    def test_dwt_frequency_bands_mapping(
        ...
        ***REMOVED*** First levels should be high-frequency (daily)
    ...
    def test_cwt_produces_time_frequency_map(
        ...
        ***REMOVED*** Should be 2D (scales x time)
        coeffs = result["coefficients"]
        if coeffs:  ***REMOVED*** If pywt is installed
    ...
    def test_wavelet_reconstruct(
        ...
        ***REMOVED*** Reconstructed should be similar length
    ...
    def test_wavelet_reconstruct_with_level_filter(
        ...
        ***REMOVED*** Keep only level 0 (highest frequency)
        high_freq = processor.wavelet_reconstruct(dwt_result, keep_levels=[0])

        ***REMOVED*** Keep only higher levels (lower frequency)
        low_freq = processor.wavelet_reconstruct(dwt_result, keep_levels=[2, 3])

        ***REMOVED*** High frequency should have more rapid oscillations
        high_freq_std = np.std(np.diff(high_freq))
        low_freq_std = np.std(np.diff(low_freq))
        ***REMOVED*** Note: This is a heuristic check
        assert high_freq_std != low_freq_std  ***REMOVED*** They should be different


***REMOVED*** =============================================================================
***REMOVED*** STA/LTA Anomaly Detection Tests
***REMOVED*** =============================================================================
...
class TestSTALTADetector:
    ...
    def test_sta_lta_detects_spike(
        ...
        ***REMOVED*** Should detect at least one anomaly
        anomalies = result["anomalies"]
        assert len(anomalies) > 0

        ***REMOVED*** At least one should be a spike
        spike_types = [
    ...
    def test_sta_lta_custom_threshold(self, spike_workload: WorkloadTimeSeries) -> None:
        ...
        ***REMOVED*** Higher sensitivity should detect more
        sensitive_anomalies = len(result["anomalies"])

        ***REMOVED*** High threshold = less sensitive
    ...
    def test_sta_lta_window_parameters(
        ...
        ***REMOVED*** Both should produce valid results
    ...
    def test_sta_lta_anomaly_structure(
        ...
        for anomaly in result["anomalies"]:
            ...
            ***REMOVED*** Severity should be between 0 and 1
            assert 0.0 <= anomaly["severity"] <= 1.0


***REMOVED*** =============================================================================
***REMOVED*** Spectral Decomposition Tests
***REMOVED*** =============================================================================
...
class TestSpectralDecomposition:
    ...
    def test_decomposition_produces_components(
        ...
        ***REMOVED*** All components should have same length as input
    ...
    def test_decomposition_sum_equals_original(
        ...
        ***REMOVED*** Should be close to original
    ...
    def test_decomposition_detects_trend(
        ...
        ***REMOVED*** Trend should be generally increasing
        assert trend[-1] > trend[0]

        ***REMOVED*** Trend strength should be significant
    ...
    def test_decomposition_detects_seasonal(
        ...
        ***REMOVED*** Seasonal strength should be high
        assert result["statistics"]["seasonal_strength"] > 0.5

        ***REMOVED*** Residual variance should be low compared to original
    ...
    def test_decomposition_insufficient_data(self, sample_dates: list[date]) -> None:
        ...
        short_ts = WorkloadTimeSeries(
            values=np.array([8.0] * 10),  ***REMOVED*** Only 10 days
            dates=sample_dates[:10],
        )
        processor = WorkloadSignalProcessor()

        ***REMOVED*** Period = 7 would need at least 14 days
...
***REMOVED*** =============================================================================
***REMOVED*** Constraint Validation Tests
***REMOVED*** =============================================================================
...
class TestConstraintValidation:
    ...
    def test_validates_against_default_constraints(
        ...
        ***REMOVED*** Weekly pattern should not violate default constraints
        ***REMOVED*** (it has proper rest periods)
    ...
    def test_detects_rapid_alternation(
        ...
        ***REMOVED*** Should detect some violation (alternating every day)
        ***REMOVED*** This may trigger rapid_alternation or frequency-based violation
        alternation_violations = [
            v for v in violations if "alternation" in v["violation_type"].lower()
        ]
        ***REMOVED*** Note: depending on power levels, might or might not trigger
        ***REMOVED*** At minimum, the result should be a valid list
        assert isinstance(violations, list)
    ...
    def test_custom_constraint(
        ...
        custom_constraint = FrequencyConstraint(
            name="custom_test",
            max_frequency=0.1,  ***REMOVED*** Max 1 cycle per 10 days
        ...
        ***REMOVED*** Weekly pattern has 7-day period, which violates 10-day minimum
        ***REMOVED*** if the power is significant
        assert isinstance(violations, list)
...
***REMOVED*** =============================================================================
***REMOVED*** Adaptive Filtering Tests
***REMOVED*** =============================================================================
...
class TestAdaptiveFiltering:
    ...
    def test_filtering_reduces_noise(self, sample_dates: list[date]) -> None:
        ...
        ***REMOVED*** Create signal with noise
        ...
        ***REMOVED*** Filtered should be closer to clean than noisy was
    ...
    def test_quality_metrics(
        ...
        ***REMOVED*** Correlation should be high (filtered ≈ original for clean signal)
        assert metrics["correlation_with_original"] > 0.8


***REMOVED*** =============================================================================
***REMOVED*** Harmonic Analysis Tests
***REMOVED*** =============================================================================
...
class TestHarmonicAnalysis:
    ...
    def test_harmonic_analysis_detects_fundamental(
        ...
        ***REMOVED*** Should detect 7-day as fundamental
    ...
    def test_thd_calculation(
        ...
        assert thd >= 0.0  ***REMOVED*** THD should be non-negative


***REMOVED*** =============================================================================
***REMOVED*** Full Analysis Pipeline Tests
***REMOVED*** =============================================================================
...
class TestAnalysisPipeline:
    ...
    def test_full_analysis_all_types(
        ...
        ***REMOVED*** All analyses should be present
    ...
    def test_full_analysis_selected_types(
        ...
        ***REMOVED*** Selected analyses should be present
        assert "fft_analysis" in result
        assert "sta_lta_analysis" in result

        ***REMOVED*** Unselected should not be present or be None
        ***REMOVED*** (implementation may include as None or exclude)
...
class TestHolographicExport:
    ...
    def test_export_is_json_serializable(
        ...
        ***REMOVED*** Should be parseable
        parsed = json.loads(json_str)
        assert parsed["version"] == "1.0"


***REMOVED*** =============================================================================
***REMOVED*** Convenience Function Tests
***REMOVED*** =============================================================================
...
class TestConvenienceFunctions:
    ...
    def test_detect_schedule_anomalies(self, sample_dates: list[date]) -> None:
        ...
        hours = [8.0] * 90
        hours[45] = 16.0  ***REMOVED*** Add spike

        anomalies = detect_schedule_anomalies(hours, sample_dates)

        assert isinstance(anomalies, list)
        ***REMOVED*** Should detect the spike
        if len(anomalies) > 0:
...
***REMOVED*** =============================================================================
***REMOVED*** Edge Cases and Error Handling
***REMOVED*** =============================================================================
...
class TestEdgeCases:
    ...
    def test_constant_signal(self, sample_dates: list[date]) -> None:
        ...
        ts = WorkloadTimeSeries(
            values=np.full(90, 8.0),  ***REMOVED*** All same value
            dates=sample_dates,
        )
        processor = WorkloadSignalProcessor()
    ...
    def test_signal_with_nans(self, sample_dates: list[date]) -> None:
        ...
        ***REMOVED*** This might raise or handle gracefully depending on implementation
        ***REMOVED*** Current implementation doesn't explicitly handle NaN
        ts = WorkloadTimeSeries(values=values, dates=sample_dates)
        processor = WorkloadSignalProcessor()

        ***REMOVED*** FFT should handle or propagate NaN
        result = processor.fft_analysis(ts)
        ***REMOVED*** At minimum, should return a result
        assert "frequencies" in result

    def test_very_noisy_signal(self, sample_dates: list[date]) -> None:
        """Test handling of very noisy signal."""
        ts = WorkloadTimeSeries(
            values=np.random.normal(8, 5, 90),  ***REMOVED*** High variance noise
            dates=sample_dates,
        )
        processor = WorkloadSignalProcessor()
        ...
        ***REMOVED*** Should complete without error
        assert "analysis_id" in result
...
class TestIntegration:
    ...
    def test_full_workflow(self, sample_dates: list[date]) -> None:
        ...
        ***REMOVED*** Create realistic workload pattern
        hours = np.zeros(90)
        for i, d in enumerate(sample_dates):
            base = 8.0
            ***REMOVED*** Weekly pattern
            if d.weekday() < 5:
                base += 2.0
            ***REMOVED*** Add call day pattern (every 4th day)
            if i % 4 == 0:
                base += 4.0
            ***REMOVED*** Add noise
    ...
    def test_comparison_with_known_pattern(self, sample_dates: list[date]) -> None:
        ...
        ***REMOVED*** Should detect 7-day period
