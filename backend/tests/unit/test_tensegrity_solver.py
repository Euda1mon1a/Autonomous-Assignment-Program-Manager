...
class TestTensegritySolverSimpleSystems:
    ...
    def test_two_anchored_nodes(self):
        ...
        ***REMOVED*** Anchored nodes should stay at their positions
    ...
    def test_one_free_node_between_anchors(self):
        ...
        ***REMOVED*** Anchors at 8.0 and 17.0
        solver.add_node('start', initial_position=8.0, is_anchor=True)
        solver.add_node('task', initial_position=10.0)  ***REMOVED*** Free node
        solver.add_node('end', initial_position=17.0, is_anchor=True)

        ***REMOVED*** Equal force densities - should settle in middle
        solver.add_tension_element('start', 'task', force_density=1.0)
        solver.add_tension_element('task', 'end', force_density=1.0)

        solution = solver.solve()

        ***REMOVED*** Free node should be pulled to midpoint between anchors
        assert solution['start'] == 8.0
        assert solution['end'] == 17.0
    ...
    def test_asymmetric_force_densities(self):
        ...
        solver.add_node('task', initial_position=5.0)  ***REMOVED*** Free node
        solver.add_node('end', initial_position=10.0, is_anchor=True)

        ***REMOVED*** Stronger pull toward start (3x)
        solver.add_tension_element('start', 'task', force_density=3.0)
        solver.add_tension_element('task', 'end', force_density=1.0)

        solution = solver.solve()

        ***REMOVED*** Free node should be closer to start due to stronger force
        ***REMOVED*** Expected: (3*0 + 1*10) / (3+1) = 2.5
    ...
    def test_compression_element_pushes_nodes_apart(self):
        ...
        ***REMOVED*** Two free nodes with a compression strut between them
        solver.add_node('anchor_left', initial_position=0.0, is_anchor=True)
        solver.add_node('node1', initial_position=5.0)
        solver.add_node('node2', initial_position=5.0)  ***REMOVED*** Same initial position
        solver.add_node('anchor_right', initial_position=10.0, is_anchor=True)

        ***REMOVED*** Tension to anchors (pull toward center)
        ...
        ***REMOVED*** Compression between free nodes (push apart)
        solver.add_compression_element('node1', 'node2', force_density=2.0)

        solution = solver.solve()

        ***REMOVED*** Nodes should separate due to compression
...
class TestTensegritySolverComplexSystems:
    ...
    def test_shift_schedule_with_tasks(self):
        ...
        ***REMOVED*** Shift boundaries (anchored)
    ...
    def test_four_node_square_configuration(self):
        ...
        ***REMOVED*** Tension elements forming a chain
        ...
        ***REMOVED*** Free nodes should distribute evenly
        assert solution['corner1'] == 0.0
        assert solution['corner2'] == 10.0

        ***REMOVED*** With equal force densities, should be evenly spaced
    ...
    def test_multiple_anchors_no_free_nodes(self):
        ...
        ***REMOVED*** All anchors should remain at their positions
...
class TestTensegritySolverStability:
    ...
    def test_is_stable_without_solution(self):
        ...
        ***REMOVED*** Should solve internally and check stability
    ...
    def test_is_stable_rejects_negative_times(self):
        ...
        ***REMOVED*** Create a solution manually with negative values
        solver.add_node('node1', initial_position=5.0)
...
class TestTensegritySolverInternalForces:
    ...
    def test_get_internal_forces_tension_only(self):
        ...
        ***REMOVED*** Force = q * (x_end - x_start) = 2.0 * (10.0 - 0.0) = 20.0
    ...
    def test_get_internal_forces_compression_element(self):
        ...
        ***REMOVED*** Compression force should be negative (q is negated for compression)
        ***REMOVED*** Force = -q * (x2 - x1) = -1.0 * (10.0 - 0.0) = -10.0
        assert ('node1', 'node2') in forces
        assert forces[('node1', 'node2')] < 0  ***REMOVED*** Compression is negative
    ...
    def test_get_internal_forces_mixed_system(self):
        ...
        ***REMOVED*** Should have forces for both edges
...
class TestTensegritySolverUtilities:
    ...
    def test_to_schedule_basic(self):
        ...
        ***REMOVED*** Should have 4 entries
        assert len(schedule) == 4

        ***REMOVED*** Should be sorted by time
        ...
        ***REMOVED*** Anchors should be marked as fixed
    ...
    def test_visualize_basic(self):
        ...
        ***REMOVED*** Check that visualization contains expected elements
        assert 'Tensegrity Structure Visualization' in viz
        assert 'start' in viz
        assert 'task' in viz
        assert 'end' in viz
        assert 'ANCHOR' in viz  ***REMOVED*** Should mark anchored nodes
        assert 'Nodes' in viz
...
class TestTensegritySolverEdgeCases:
    ...
    def test_solve_only_free_nodes_no_constraints(self):
        ...
        ***REMOVED*** With no constraints, positions should remain at initial values
        ***REMOVED*** (zero force system)
        solution = solver.solve()

        ***REMOVED*** Should succeed with initial positions
    ...
    def test_tension_only_structure(self):
        ...
        ***REMOVED*** Should produce valid solution
    ...
    def test_mixed_tension_compression(self):
        ...
        ***REMOVED*** Create a simple tensegrity structure
        ...
        ***REMOVED*** Tension cables to anchors (pull down)
        solver.add_tension_element('base1', 'top', force_density=1.0)
        solver.add_tension_element('base2', 'top', force_density=1.0)

        ***REMOVED*** Compression strut (push up)
        ***REMOVED*** Note: In 1D, compression is less intuitive, but mathematically valid
        solver.add_compression_element('base1', 'base2', force_density=0.5)

        solution = solver.solve()

        ***REMOVED*** Should produce valid equilibrium
        assert solver.is_stable(solution)
    ...
    def test_build_force_density_matrix_structure(self):
        ...
        ***REMOVED*** Check matrix is square
        assert F.shape[0] == F.shape[1] == 3

        ***REMOVED*** Check matrix is symmetric
        F_dense = F.toarray()
        assert np.allclose(F_dense, F_dense.T)

        ***REMOVED*** Check diagonal elements (sum of incident force densities)
        assert np.isclose(F_dense[0, 0], 1.0)  ***REMOVED*** node1: only edge to node2
        assert np.isclose(F_dense[1, 1], 3.0)  ***REMOVED*** node2: edges to node1 (1) and node3 (2)
        assert np.isclose(F_dense[2, 2], 2.0)  ***REMOVED*** node3: only edge to node2

        ***REMOVED*** Check off-diagonal elements
