...
***REMOVED*** Check if ndlib is available
try:
    import ndlib
...
def simple_network():
    ...
    ***REMOVED*** Add 10 providers
    providers = [f"provider_{i}" for i in range(10)]
    G.add_nodes_from(providers)

    ***REMOVED*** Create connections (small-world-ish pattern)
    edges = [
        ("provider_0", "provider_1"),
        ("provider_0", "provider_2"),
        ("provider_0", "provider_3"),  ***REMOVED*** provider_0 is a hub
        ...
        ("provider_9", "provider_0"),  ***REMOVED*** Close the loop
    ]
...
def hub_network():
    ...
    ***REMOVED*** Central hub with many connections
    hub = "hub_provider"
    ...
    ***REMOVED*** Hub connected to all peripherals
    for peripheral in peripherals:
        G.add_edge(hub, peripheral)

    ***REMOVED*** Some peripheral-to-peripheral connections
...
def initial_burnout_mixed():
    ...
    return {
        "provider_0": 0.8,  ***REMOVED*** High burnout
        "provider_1": 0.7,  ***REMOVED*** High burnout
        "provider_2": 0.3,  ***REMOVED*** Low
        "provider_3": 0.9,  ***REMOVED*** Very high burnout
        "provider_4": 0.2,  ***REMOVED*** Low
        "provider_5": 0.4,  ***REMOVED*** Medium
        "provider_6": 0.6,  ***REMOVED*** Medium-high
        "provider_7": 0.1,  ***REMOVED*** Low
        "provider_8": 0.5,  ***REMOVED*** Medium
        "provider_9": 0.2,  ***REMOVED*** Low
    }
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Model Initialization
***REMOVED*** =============================================================================
...
def test_model_initialization(simple_network):
    ...
    assert model.model is None  ***REMOVED*** Not configured yet
    assert model.infection_rate == 0.05  ***REMOVED*** Default
    assert model.recovery_rate == 0.01  ***REMOVED*** Default
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Model Configuration
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Initial Burnout State
***REMOVED*** =============================================================================
...
def test_set_initial_burnout(simple_network, initial_burnout_mixed):
    ...
    ***REMOVED*** Check that high-burnout providers are marked as infected
    ***REMOVED*** (burnout >= 0.5 should be infected)
...
def test_simulate_basic(simple_network, initial_burnout_mixed):
    ...
    ***REMOVED*** Verify each result has expected structure
...
def test_simulate_infection_spread(simple_network, initial_burnout_mixed):
    ...
    model = BurnoutContagionModel(simple_network)
    model.configure(infection_rate=0.15, recovery_rate=0.01)  ***REMOVED*** High infection rate
    ...
    ***REMOVED*** Check that infection rate changes over time
    initial_rate = model.snapshots[0].infection_rate
    final_rate = model.snapshots[-1].infection_rate

    ***REMOVED*** With high infection rate, final should be >= initial
    ***REMOVED*** (or at least simulation should progress)
...
def test_simulate_snapshots(simple_network, initial_burnout_mixed):
    ...
    ***REMOVED*** Verify snapshot structure
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Superspreader Identification
***REMOVED*** =============================================================================
...
def test_identify_superspreaders(hub_network):
    ...
    ***REMOVED*** Set hub provider to high burnout (should be superspreader)
    burnout_scores = {
        "hub_provider": 0.9,  ***REMOVED*** High burnout + high centrality = superspreader
        "provider_0": 0.2,
        "provider_1": 0.3,
        "provider_2": 0.1,
        "provider_3": 0.2,
        "provider_4": 0.8,  ***REMOVED*** High burnout but low centrality
        "provider_5": 0.1,
        "provider_6": 0.2,
        "provider_7": 0.1,
    }
    ...
    ***REMOVED*** Hub with high burnout should be identified
    assert "hub_provider" in superspreaders

    ***REMOVED*** Total superspreaders should be reasonable
...
def test_identify_superspreaders_profiles(hub_network):
    ...
    ***REMOVED*** Should have at least one profile
    assert len(profiles) >= 1

    ***REMOVED*** Check profile structure
    profile = profiles[0]
...
def test_identify_superspreaders_no_burnout(simple_network):
    ...
    ***REMOVED*** Don't set any burnout scores
    superspreaders = model.identify_superspreaders()
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Intervention Recommendations
***REMOVED*** =============================================================================
...
def test_recommend_interventions(simple_network, initial_burnout_high):
    ...
    ***REMOVED*** Should recommend some interventions for high burnout
    assert len(interventions) > 0

    ***REMOVED*** Check intervention structure
...
def test_recommend_interventions_types(hub_network):
    ...
    ***REMOVED*** Create scenario with high burnout superspreader
    burnout_scores = {
        "hub_provider": 0.95,  ***REMOVED*** Extreme burnout
        "provider_0": 0.7,
        "provider_1": 0.8,
        "provider_2": 0.6,
        "provider_3": 0.7,
    ...
    ***REMOVED*** Should have multiple intervention types
    intervention_types = {i.intervention_type for i in interventions}

    ***REMOVED*** Should recommend workload reduction for extreme burnout
    assert InterventionType.WORKLOAD_REDUCTION in intervention_types

    ***REMOVED*** Should recommend quarantine for extreme superspreader
...
def test_recommend_interventions_no_superspreaders(simple_network, initial_burnout_low):
    ...
    ***REMOVED*** Low burnout = no superspreaders = no interventions
    assert len(interventions) == 0
...
***REMOVED*** =============================================================================
***REMOVED*** Test: Report Generation
***REMOVED*** =============================================================================
...
def test_report_risk_levels(simple_network):
    ...
    ***REMOVED*** Low burnout scenario
    low_burnout = {f"provider_{i}": 0.1 for i in range(10)}

    model = BurnoutContagionModel(simple_network)
    model.configure(infection_rate=0.02, recovery_rate=0.05)  ***REMOVED*** Low spread
    ...
    ***REMOVED*** Should be low or moderate risk
...
def test_report_warnings(simple_network, initial_burnout_high):
    ...
    model = BurnoutContagionModel(simple_network)
    model.configure(infection_rate=0.2, recovery_rate=0.01)  ***REMOVED*** High spread
    ...
    ***REMOVED*** High burnout should generate warnings
    assert len(report.warnings) > 0


***REMOVED*** =============================================================================
***REMOVED*** Test: Utility Methods
***REMOVED*** =============================================================================
...
def test_get_infection_trajectory(simple_network, initial_burnout_mixed):
    ...
    ***REMOVED*** Each point should be (iteration, rate)
...
def test_get_current_state(simple_network, initial_burnout_mixed):
    ...
    assert state["iteration"] == 14  ***REMOVED*** 0-indexed
...
def test_single_node_network():
    ...
    ***REMOVED*** Note: This test is skipped because ndlib's SIS model doesn't support
    ***REMOVED*** single-node networks well (raises ValueError during iteration)
    G = nx.Graph()
    G.add_node("provider_1")
    ...
    ***REMOVED*** Should run without error (but currently doesn't due to ndlib limitation)
...
def test_disconnected_network():
    ...
    ***REMOVED*** Component 2 (disconnected)
    G.add_edge("provider_4", "provider_5")
    ...
    ***REMOVED*** Should run without error
    results = model.simulate(iterations=20)
    assert len(results) == 20

    ***REMOVED*** Infection shouldn't spread to disconnected component
    ***REMOVED*** (though this is probabilistic, so we just verify it runs)
...
def test_all_infected_initial_state(simple_network):
    ...
    ***REMOVED*** Should still run
    assert len(results) == 20

    ***REMOVED*** Initial state should have some infected (ndlib adds randomness)
    ***REMOVED*** We expect at least some nodes to be infected given all have high burnout
    assert model.snapshots[0].infected_count >= 1

    ***REMOVED*** Over time, infection should spread since all have high burnout
    ***REMOVED*** Check that infection increases or stays high
...
def test_all_susceptible_initial_state(simple_network):
    ...
    ***REMOVED*** Initial state should have at least one seeded (since all below threshold)
    ***REMOVED*** Most should be susceptible
