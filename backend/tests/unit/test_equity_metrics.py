...
class TestGiniCoefficient:
    ...
    def test_maximum_inequality(self) -> None:
        ...
        ***REMOVED*** One person has everything, others have nothing
        values = [0.0, 0.0, 0.0, 100.0]
        result = gini_coefficient(values)
        ***REMOVED*** For n=4, max Gini = (n-1)/n = 3/4 = 0.75
    ...
    def test_maximum_inequality_large_group(self) -> None:
        ...
        values = [0.0] * 99 + [100.0]
        result = gini_coefficient(values)
        ***REMOVED*** For n=100, max Gini = (n-1)/n = 99/100 = 0.99
    ...
    def test_moderate_inequality(self) -> None:
        ...
        result = gini_coefficient(values)
        ***REMOVED*** This should be between 0 and 1, closer to 0 (relatively equitable)
    ...
    def test_high_inequality(self) -> None:
        ...
        result = gini_coefficient(values)
        ***REMOVED*** Very unequal distribution
    ...
    def test_weighted_calculation_different_weights(self) -> None:
        ...
        ***REMOVED*** Same hours but different intensity
        values = [40.0, 40.0, 40.0, 40.0]
        weights = [1.0, 1.0, 1.5, 2.0]  ***REMOVED*** Last two have higher intensity
        result = gini_coefficient(values, weights=weights)
        ***REMOVED*** Should show inequality due to intensity differences
        assert result > 0.0
    ...
    def test_weighted_calculation_compensating_inequality(self) -> None:
        ...
        ***REMOVED*** Person A: 60 hours at 1.0 intensity = 60 weighted hours
        ***REMOVED*** Person B: 40 hours at 1.5 intensity = 60 weighted hours
        values = [60.0, 40.0]
        weights = [1.0, 1.5]
        result = gini_coefficient(values, weights=weights)
        ***REMOVED*** Weighted values are equal, so should be perfect equality
    ...
    def test_mismatched_weights_length_raises_error(self) -> None:
        ...
        values = [10.0, 20.0, 30.0]
        weights = [1.0, 1.5]  ***REMOVED*** Wrong length
    ...
    def test_large_values(self) -> None:
        ...
        result = gini_coefficient(values)
        ***REMOVED*** Scale shouldn't affect Gini (it's scale-invariant)
    ...
    def test_medical_scheduling_scenario_equitable(self) -> None:
        ...
        ***REMOVED*** 5 residents with similar hours (within 10%)
        values = [165.0, 168.0, 170.0, 162.0, 165.0]
        result = gini_coefficient(values)
        ***REMOVED*** Should be very equitable (< 0.15 threshold)
        assert result < 0.15

    def test_medical_scheduling_scenario_inequitable(self) -> None:
        """Test realistic inequitable medical scheduling scenario."""
        ***REMOVED*** One resident significantly overloaded, one underutilized
        values = [80.0, 160.0, 170.0, 165.0, 280.0]
        result = gini_coefficient(values)
        ***REMOVED*** Should exceed equity threshold
        assert result > 0.15
...
class TestLorenzCurve:
    ...
    def test_lorenz_curve_perfect_equality(self) -> None:
        ...
        ***REMOVED*** Should have n+1 points (including origin)
        assert len(x_coords) == 5
        assert len(y_coords) == 5

        ***REMOVED*** First point should be origin
        assert x_coords[0] == 0.0
        assert y_coords[0] == 0.0

        ***REMOVED*** Last point should be (1, 1)
        assert x_coords[-1] == 1.0
        assert y_coords[-1] == 1.0

        ***REMOVED*** For perfect equality, Lorenz curve should equal y=x line
    ...
    def test_lorenz_curve_maximum_inequality(self) -> None:
        ...
        ***REMOVED*** Should have n+1 points
        assert len(x_coords) == 5
        assert len(y_coords) == 5

        ***REMOVED*** First 3 people have 0 cumulative share
        assert y_coords[0] == 0.0
        assert y_coords[1] == 0.0
        assert y_coords[2] == 0.0
        assert y_coords[3] == 0.0

        ***REMOVED*** Last person has 100%
    ...
    def test_lorenz_curve_moderate_distribution(self) -> None:
        ...
        ***REMOVED*** Check points are properly ordered
        ...
        ***REMOVED*** Lorenz curve should be below or on the equality line (except endpoints)
    ...
    def test_lorenz_curve_all_zeros(self) -> None:
        ...
        ***REMOVED*** Should return equality line when all values are zero
        assert np.allclose(x_coords, y_coords)
    ...
    def test_lorenz_curve_single_value(self) -> None:
        ...
        ***REMOVED*** Should have 2 points: (0, 0) and (1, 1)
    ...
    def test_lorenz_curve_unsorted_input(self) -> None:
        ...
        ***REMOVED*** Lorenz curve should sort internally
        ...
        ***REMOVED*** Results should be identical regardless of input order
...
class TestEquityReport:
    ...
    def test_equity_report_with_intensity_weights(self) -> None:
        ...
        ***REMOVED*** Provider C has high-intensity assignments
        intensity_weights = {
            "A": 1.0,
            "B": 1.0,
            "C": 2.0,
        ...
        ***REMOVED*** Should detect inequality due to intensity differences
        assert report["gini"] > 0.0
    ...
    def test_equity_report_statistics(self) -> None:
        ...
        assert report["most_overloaded"] == "D"
        assert report["most_underloaded"] == "A"
        assert report["overload_delta"] == 15.0  ***REMOVED*** 60 - 45
        assert report["underload_delta"] == 15.0  ***REMOVED*** 45 - 30
    ...
    def test_equity_report_recommendations_underload(self) -> None:
        ...
        provider_hours = {
            "A": 10.0,  ***REMOVED*** Significantly underloaded
            "B": 40.0,
            "C": 40.0,
            "D": 40.0,
        }
    ...
    def test_equity_report_recommendations_rebalancing(self) -> None:
        ...
        ***REMOVED*** Should suggest transferring hours
        recommendations_text = " ".join(report["recommendations"])
    ...
    def test_equity_report_target_gini(self) -> None:
        ...
        assert report["target_gini"] == 0.15  ***REMOVED*** Medical scheduling threshold
    ...
    def test_equity_report_mismatched_weights_raises_error(self) -> None:
        ...
        intensity_weights = {"A": 1.0, "B": 1.5}  ***REMOVED*** Missing C
    ...
    def test_equity_report_high_intensity_warning(self) -> None:
        ...
        intensity_weights = {"A": 1.0, "B": 1.0, "C": 3.0}  ***REMOVED*** C has very high intensity
    ...
    def test_equity_report_real_world_scenario(self) -> None:
        ...
        ***REMOVED*** This should be equitable (low variation)
        assert report["is_equitable"] is True
        assert report["gini"] < 0.15
    ...
    def test_equity_report_intensity_compensated_scenario(self) -> None:
        ...
        ***REMOVED*** Junior residents work more hours but at lower intensity
        ***REMOVED*** Senior residents work fewer hours but higher intensity
        provider_hours = {
            "PGY1_A": 80.0,
            "PGY1_B": 80.0,
            "PGY3_A": 60.0,
        ...
        intensity_weights = {
            "PGY1_A": 1.0,
            "PGY1_B": 1.0,
            "PGY3_A": 1.33,  ***REMOVED*** 80/60 = 1.33
            "PGY3_B": 1.33,
        }
        report = equity_report(provider_hours, intensity_weights)

        ***REMOVED*** Should be equitable when intensity-adjusted
        assert report["gini"] < 0.15
        assert report["is_equitable"] is True
