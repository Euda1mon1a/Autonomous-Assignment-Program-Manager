...
class TestGraphConstruction:
    ...
    def test_build_graph_multi_task_constraint(self):
        ...
        ***REMOVED*** Should create edges A-B, A-C, B-C (3 edges)
...
class TestRigidStructure:
    ...
    def test_triangle_is_rigid(self):
        ...
        ***REMOVED*** Minimally rigid structure has 3 DoF (rigid body motions: 2 translations + 1 rotation)
        assert result["degrees_of_freedom"] == 3
        assert result["is_rigid"] is False  ***REMOVED*** Not fully rigid (still has rigid body DoF)
    ...
    def test_square_is_over_rigid(self):
        ...
        constraints = [
            ...
            {"type": "c3", "tasks": ["C", "D"]},
            {"type": "c4", "tasks": ["D", "A"]},
            {"type": "c5", "tasks": ["A", "C"]},  ***REMOVED*** Diagonal 1
            {"type": "c6", "tasks": ["B", "D"]},  ***REMOVED*** Diagonal 2 (redundant)
        ]
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)
        result = analyzer.run_pebble_game(graph)
        ...
        ***REMOVED*** Note: edge ordering can affect which specific edge is redundant
        ***REMOVED*** but total should be 5 independent, 1 redundant
...
class TestStressedStructure:
    ...
    def test_complete_graph_is_stressed(self):
        ...
        ***REMOVED*** Create complete graph: all pairs connected
        constraints = [{"type": "c1", "tasks": ["A", "B", "C", "D"]}]  ***REMOVED*** Creates 6 edges
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)
        result = analyzer.run_pebble_game(graph)

        ***REMOVED*** 4 nodes: expect 2*4 - 3 = 5 independent edges, 1 redundant
        assert result["total_edges"] == 6
        assert result["independent_edges"] >= 5
        assert result["redundant_edges"] >= 1
        assert result["is_stressed"] is True
    ...
    def test_identify_stressed_region(self):
        ...
        ***REMOVED*** Create complete graph (all pairs)
...
class TestDegreesOfFreedom:
    ...
    def test_degrees_of_freedom_unconstrained(self):
        ...
        ***REMOVED*** 2 nodes × 2 pebbles each = 4 DoF
        assert dof == 4
    ...
    def test_degrees_of_freedom_partial(self):
        ...
        dof = analyzer.get_degrees_of_freedom()
        ***REMOVED*** Started with 6 pebbles, used 1 for edge → 5 remaining
        assert dof == 5
...
class TestRecommendations:
    ...
    def test_recommend_remove_from_stressed_region(self):
        ...
        constraints = [
            {"type": "complete", "tasks": ["A", "B", "C", "D"]}  ***REMOVED*** Creates 6 edges
        ]
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)
        analyzer.run_pebble_game(graph)
        ...
        ***REMOVED*** Should recommend removing at least one redundant edge
    ...
    def test_recommend_add_to_flexible_region(self):
        ...
        constraints = [
            {"type": "c1", "tasks": ["A", "B"]},
            {"type": "c2", "tasks": ["B", "C"]},
            {"type": "c3", "tasks": ["C", "D"]},  ***REMOVED*** 3 edges total, needs 5 for rigidity
        ]
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)
        analyzer.run_pebble_game(graph)
        ...
        ***REMOVED*** Should recommend adding constraints
        add_recommendations = [r for r in recommendations if r["action"] == "add"]
        assert len(add_recommendations) >= 1
        ***REMOVED*** Needs 2 more edges for rigidity (5 total - 3 current = 2)
    ...
    def test_no_recommendations_for_perfect_structure(self):
        ...
        ***REMOVED*** Rigid triangle is perfect - no changes needed
...
class TestComplexScenarios:
    ...
    def test_disconnected_components(self):
        ...
        analyzer = ConstraintRigidityAnalyzer()
        tasks = [
            ***REMOVED*** Component 1: Triangle (rigid)
            {"id": "A", "type": "shift"},
            {"id": "B", "type": "shift"},
            {"id": "C", "type": "shift"},
            ***REMOVED*** Component 2: Chain (flexible)
            {"id": "D", "type": "shift"},
            {"id": "E", "type": "shift"},
            {"id": "F", "type": "shift"},
        ]
        ...
        ***REMOVED*** Should identify triangle as rigid
        assert len(rigid_regions) >= 1
        ***REMOVED*** Should identify chain as flexible
        assert len(flexible_regions) >= 1
    ...
    def test_large_rigid_structure(self):
        ...
        ***REMOVED*** For 5 nodes: 2*5 - 3 = 7 edges needed for minimal rigidity
        constraints = [
            {"type": "c1", "tasks": ["A", "B"]},
            {"type": "c2", "tasks": ["B", "C"]},
            {"type": "c3", "tasks": ["C", "D"]},
        ...
        assert result["redundant_edges"] == 0
        ***REMOVED*** DoF = 2*V - E = 2*5 - 7 = 3 (rigid body motions)
        assert result["degrees_of_freedom"] == 3
        assert result["is_rigid"] is False  ***REMOVED*** Has rigid body DoF
        assert result["is_stressed"] is False
...
class TestEdgeCases:
    ...
    def test_constraint_with_nonexistent_task(self):
        ...
        analyzer = ConstraintRigidityAnalyzer()
        tasks = [{"id": "A", "type": "shift"}]
        constraints = [{"type": "c1", "tasks": ["A", "Z"]}]  ***REMOVED*** Z doesn't exist
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)

        ***REMOVED*** Should only have node A, no edges
        assert graph.number_of_nodes() == 1
        assert graph.number_of_edges() == 0
    ...
    def test_constraint_with_single_task(self):
        ...
        analyzer = ConstraintRigidityAnalyzer()
        tasks = [{"id": "A", "type": "shift"}, {"id": "B", "type": "shift"}]
        constraints = [{"type": "c1", "tasks": ["A"]}]  ***REMOVED*** Only one task
        graph = analyzer.build_constraint_graph(tasks=tasks, constraints=constraints)
    ...
    def test_pebble_game_on_single_node(self):
        ...
        ***REMOVED*** Single node keeps all pebbles
