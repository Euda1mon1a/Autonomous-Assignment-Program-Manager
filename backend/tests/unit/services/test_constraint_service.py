...
class TestConstraintManager:
    ...
    def test_create_default_manager(self):
        ...
        ***REMOVED*** Should have both hard and soft constraints
        assert len(manager.get_hard_constraints()) >= 5
        assert len(manager.get_soft_constraints()) >= 3

        ***REMOVED*** Resilience constraints should be disabled by default
    ...
    def test_create_minimal_manager(self):
        ...
        ***REMOVED*** Should have only essential constraints
        assert len(manager.constraints) == 3
...
class TestScheduleConstraintValidation:
    ...
    def test_80_hour_rule_no_violation(self, db, sample_resident):
        ...
        ***REMOVED*** Create 4 weeks of blocks (28 days)
        blocks = []
        ...
        ***REMOVED*** Create assignments: 5 days/week, AM only = 30 hours/week (within 80)
        assignments = []
        for i in range(0, 28, 7):  ***REMOVED*** Each week
            for j in range(5):  ***REMOVED*** 5 days per week
    ...
    def test_80_hour_rule_violation(self, db, sample_resident):
        ...
        ***REMOVED*** Create 4 weeks of blocks
        blocks = []
        start_date = date.today()
        for i in range(28):
        ...
        ***REMOVED*** Create assignments: 7 days/week, AM+PM = 84 hours/week (violates 80)
        assignments = []
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
    ...
    def test_1_in_7_rule_no_violation(self, db, sample_resident):
        ...
        ***REMOVED*** Create 14 days of blocks
        blocks = []
        start_date = date.today()
        for i in range(14):
            block = Block(
        ...
        ***REMOVED*** Create assignments: Work 6 days, off 1 day (compliant)
        assignments = []
        for i in range(14):
            if i % 7 != 6:  ***REMOVED*** Day 7, 14 are off
                assignment = Assignment(
                    id=uuid4(),
    ...
    def test_1_in_7_rule_violation(self, db, sample_resident):
        ...
        ***REMOVED*** Create 10 consecutive days of blocks
        blocks = []
        start_date = date.today()
        for i in range(10):
            block = Block(
        ...
        ***REMOVED*** Create assignments for all 10 consecutive days (violates 1-in-7)
        assignments = []
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
    ...
    def test_minimum_rest_between_shifts(self, db, sample_resident):
        ...
        ***REMOVED*** Create consecutive AM/PM blocks
        blocks = []
        start_date = date.today()
        for i in range(2):
            for time_of_day in ["AM", "PM"]:
        ...
        ***REMOVED*** Assign resident to consecutive blocks
        assignments = []
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
        ...
        ***REMOVED*** OnePersonPerBlock ensures no double-booking
    ...
    def test_maximum_consecutive_work_days(self, db, sample_resident):
        ...
        ***REMOVED*** Create 8 consecutive days
        blocks = []
        start_date = date.today()
        for i in range(8):
            block = Block(
        ...
        ***REMOVED*** Assign all 8 consecutive days
        assignments = []
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
        ...
        ***REMOVED*** 1-in-7 rule should catch this
        constraint = OneInSevenRuleConstraint()
        result = constraint.validate(assignments, context)
...
class TestSupervisionRatioValidation:
    ...
    def test_pgy1_supervision_ratio_compliant(
        ...
        ***REMOVED*** Assign 2 PGY-1 residents and 1 faculty (compliant 1:2 ratio)
        assignments = []
    ...
    def test_pgy1_supervision_ratio_violation(
        ...
        ***REMOVED*** Get PGY-1 residents (need to ensure they exist)
        pgy1_residents = [r for r in sample_residents if r.pgy_level == 1]

        ***REMOVED*** Create additional PGY-1 residents if needed
        ...
        ***REMOVED*** Assign 3 PGY-1 residents and only 1 faculty (violates 1:2 ratio)
        assignments = []
        ...
        ***REMOVED*** Add only 1 faculty (need 2 for 3 residents)
        faculty_assignment = Assignment(
            id=uuid4(),
            block_id=block.id,
            person_id=sample_faculty_members[0].id,
    ...
    def test_pgy23_supervision_ratio_compliant(
        ...
        ***REMOVED*** Assign 2 PGY-2/3 residents and 1 faculty (compliant 1:4 ratio)
        assignments = []
    ...
    def test_coverage_requirement_met(self, db, sample_residents, sample_blocks):
        ...
        ***REMOVED*** Assign residents to blocks
        assignments = []
        ...
        ***REMOVED*** Coverage constraint checks if blocks are assigned
        constraint = CoverageConstraint(weight=1000.0)
        result = constraint.validate(assignments, context)

        ***REMOVED*** Should have good coverage
...
class TestCustomConstraintValidation:
    ...
    def test_equity_constraint(self, db, sample_residents, sample_blocks):
        ...
        ***REMOVED*** Assign all blocks to one resident (inequitable)
        assignments = []
        ...
        ***REMOVED*** Should have high penalty for inequity
        assert result.penalty > 0
    ...
    def test_continuity_constraint(self, db, sample_resident, sample_blocks):
        ...
        ***REMOVED*** Assign resident to some blocks
        assignments = []
        ...
        ***REMOVED*** Continuity constraint should evaluate
        assert result.penalty >= 0

    def test_constraint_priority_ordering(self):
        """Test that constraints are ordered by priority."""
        manager = ConstraintManager()
        manager.add(AvailabilityConstraint())  ***REMOVED*** CRITICAL
        manager.add(EquityConstraint(weight=10.0))  ***REMOVED*** MEDIUM

        enabled = manager.get_enabled()

        ***REMOVED*** Critical constraint should have higher priority value
    ...
    def test_soft_constraint_penalty_calculation(self):
        ...
        ***REMOVED*** Penalty should be based on weight and priority
...
class TestConstraintErrorHandling:
    ...
    def test_validate_with_invalid_person_id(self, db, sample_blocks):
        ...
        ***REMOVED*** Create assignment with non-existent person
        assignment = Assignment(
            id=uuid4(),
            block_id=sample_blocks[0].id,
            person_id=uuid4(),  ***REMOVED*** Non-existent person
            role="primary",
        )
        ...
        ***REMOVED*** Should handle gracefully (person not in availability matrix)
        assert result is not None

    def test_validate_with_invalid_block_id(self, db, sample_resident):
        """Test validation handles invalid block IDs."""
        ***REMOVED*** Create assignment with non-existent block
        assignment = Assignment(
            id=uuid4(),
            block_id=uuid4(),  ***REMOVED*** Non-existent block
            person_id=sample_resident.id,
            role="primary",
        )
    ...
    def test_constraint_violation_reporting(self, db, sample_resident, sample_blocks):
        ...
        ***REMOVED*** Create many consecutive assignments to violate 1-in-7
        assignments = []
        for i in range(10):
            block = Block(
                id=uuid4(),
        ...
        ***REMOVED*** Should report violations with details
    ...
    def test_multiple_constraint_validation(self, db, sample_resident, sample_blocks):
        ...
        ***REMOVED*** Create some assignments
        assignments = []
        for block in sample_blocks[:3]:
            assignment = Assignment(
                id=uuid4(),
        ...
        ***REMOVED*** Should aggregate results from all constraints
        assert result is not None
    ...
    def test_constraint_conflict_detection(self):
        ...
        ***REMOVED*** Add constraints that might conflict
        manager.add(CoverageConstraint(weight=1000.0))  ***REMOVED*** Wants max coverage
        manager.add(EightyHourRuleConstraint())  ***REMOVED*** Limits hours

        ***REMOVED*** Both constraints should be present
        assert len(manager.constraints) == 2

        ***REMOVED*** Hard constraint should take precedence (higher priority)
        hard = manager.get_hard_constraints()
        soft = manager.get_soft_constraints()
...
class TestConstraintServicePIISanitization:
    ...
    def test_sensitive_fields_removed_from_details(self, db):
        ...
        ***REMOVED*** Sensitive fields should be removed
        ...
        ***REMOVED*** Non-sensitive fields should remain
    ...
    def test_entity_anonymization(self, db):
        ...
        ***REMOVED*** Only first 8 chars of UUID should be used
...
class TestConstraintServiceValidateSchedule:
    ...
    async def test_validate_schedule_success_no_violations(
        ...
        ***REMOVED*** Create compliant assignments (5 days/week, AM only = 30 hours/week)
        assignments = []
        for i, block in enumerate(sample_blocks[:5]):  ***REMOVED*** First 5 blocks (2.5 days)
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
                person_id=sample_residents[i % len(sample_residents)].id,
    ...
    async def test_validate_schedule_with_violations(
        ...
        ***REMOVED*** Create 10 consecutive days to violate 1-in-7 rule
        blocks = []
        start_date = date.today()
        for i in range(10):
            block = Block(
        ...
        ***REMOVED*** Assign all blocks to same resident
        assignments = []
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
        ...
        assert result.schedule_id == str(schedule_run_id)
        ***REMOVED*** May or may not be valid depending on constraints enabled
        assert result.total_issues >= 0
        assert len(result.issues) == result.total_issues
        ***REMOVED*** Verify compliance rate is less than 1.0 if violations exist
    ...
    async def test_validate_schedule_different_configs(
        ...
        ***REMOVED*** Test each configuration
    ...
    async def test_validate_schedule_issue_counts(
        ...
        ***REMOVED*** Create scenario likely to have violations
        blocks = []
        start_date = date.today()
        for i in range(28):  ***REMOVED*** 4 weeks
            for time_of_day in ["AM", "PM"]:
                block = Block(
                    id=uuid4(),
                    date=start_date + timedelta(days=i),
        ...
        ***REMOVED*** Assign all blocks to same resident (will violate 80-hour rule)
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
...
class TestConstraintServiceInternalMethods:
    ...
    def test_get_suggested_action_unknown(self, db):
        ...
        service = ConstraintService(db)
        ***REMOVED*** Use a constraint type that doesn't have a specific suggestion
    ...
    def test_get_constraint_manager_minimal(self, db):
        ...
        assert manager is not None
        ***REMOVED*** Minimal should have fewer constraints
    ...
    def test_get_constraint_manager_resilience(self, db):
        ...
        assert manager is not None
        ***REMOVED*** Resilience should have resilience-specific constraints enabled
        assert len(manager.constraints) > 0
    ...
    def test_get_constraint_manager_unknown_fallback(self, db):
        ...
        assert manager is not None
        ***REMOVED*** Should fall back to default
        assert len(manager.constraints) > 0
...
class TestConstraintServiceComplianceCalculation:
    ...
    async def test_compliance_rate_perfect(
        ...
        ***REMOVED*** Create minimal compliant assignments
        for i, block in enumerate(sample_blocks[:3]):
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
        ...
        ***REMOVED*** With minimal constraints and good schedule, should have high compliance
    ...
    async def test_compliance_rate_weighted_by_severity(
        ...
        ***REMOVED*** Assign all blocks to same resident (violations)
        for block in blocks:
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
        ...
        ***REMOVED*** If there are critical issues, compliance should be lower
    ...
    async def test_is_valid_false_with_critical_issues(
        ...
        ***REMOVED*** Create excessive consecutive days
        blocks = []
        start_date = date.today()
        for i in range(10):
            block = Block(
        ...
        ***REMOVED*** May have critical violations depending on enabled constraints
        ***REMOVED*** Just verify the logic is consistent
        if result.critical_count > 0:
            assert result.is_valid is False
        else:
            assert result.is_valid is True
