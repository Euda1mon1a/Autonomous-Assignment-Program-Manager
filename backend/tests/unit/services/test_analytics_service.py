...
***REMOVED*** ============================================================================
***REMOVED*** Fairness Metrics Tests
***REMOVED*** ============================================================================
...
class TestFairnessMetrics:
    ...
    def test_calculate_fairness_index_perfect_distribution(self):
        ...
        ***REMOVED*** Create assignments with equal distribution (5 per person, 3 people)
        assignments = []
        for person_idx in range(3):
            for _ in range(5):
                assignments.append({"person_id": str(uuid4()) if person_idx == 0 else str(uuid4())})

        ***REMOVED*** Use same person IDs for equal distribution
        ...
        assert result["value"] >= 0.95  ***REMOVED*** Very high fairness
        assert result["status"] == "good"
        assert result["details"]["min_assignments"] == 5
        assert result["details"]["max_assignments"] == 5
        assert result["details"]["std_dev"] == 0  ***REMOVED*** Perfect equality

    def test_calculate_fairness_index_unequal_distribution(self):
        """Test fairness index with unequal distribution."""
        ***REMOVED*** Create assignments with unequal distribution
        person_a = str(uuid4())
        person_b = str(uuid4())
        person_c = str(uuid4())

        assignments = (
            [{"person_id": person_a}] * 10 +  ***REMOVED*** Person A gets 10
            [{"person_id": person_b}] * 2 +   ***REMOVED*** Person B gets 2
            [{"person_id": person_c}] * 2     ***REMOVED*** Person C gets 2
        )

        result = calculate_fairness_index(assignments)

        assert result["value"] < 0.9  ***REMOVED*** Low fairness due to inequality
    ...
    def test_calculate_fairness_index_empty_assignments(self):
        ...
        assert result["value"] == 1.0  ***REMOVED*** Perfect fairness (no assignments)
        assert result["status"] == "good"
    ...
    def test_calculate_gini_coefficient_calculation(self):
        ...
        ***REMOVED*** Create known distribution
        ...
        ***REMOVED*** Should detect inequality
        assert 0.0 <= result["value"] <= 1.0
        assert result["value"] < 1.0  ***REMOVED*** Not perfect equality
        assert result["details"]["mean_assignments"] == 2.5


***REMOVED*** ============================================================================
***REMOVED*** Workload Analysis Tests
***REMOVED*** ============================================================================
...
class TestWorkloadAnalysis:
    ...
    def test_get_resident_workload_distribution(
        ...
        ***REMOVED*** Create assignments with different workloads
        for i, resident in enumerate(sample_residents):
            ***REMOVED*** Give different amounts: PGY1 gets 3, PGY2 gets 5, PGY3 gets 7
            num_blocks = 3 + (i * 2)
    ...
    def test_workload_utilization_calculation(
        ...
        ***REMOVED*** Set target blocks
        sample_resident.target_clinical_blocks = 10
        db.commit()

        ***REMOVED*** Assign 8 blocks (80% utilization)
        for i in range(8):
            assignment = Assignment(
                id=uuid4(),
        ...
        ***REMOVED*** Find the resident's data
    ...
    def test_consecutive_duty_stats_calculation(self):
        ...
        ***REMOVED*** Create assignments for 5 consecutive days, then 2 days off, then 3 consecutive days
        assignments = [
        ...
        assert result["status"] == "good"  ***REMOVED*** 5 days is within ACGME limits (<=6)
    ...
    def test_consecutive_duty_stats_acgme_violation(self):
        ...
        ***REMOVED*** Create assignments for 10 consecutive days (violates ACGME)
        assignments = [
            {"person_id": person_id, "block_date": base_date + timedelta(days=i)}
            for i in range(10)
        ]
        ...
        assert result["status"] == "warning"  ***REMOVED*** 10 days exceeds 6-day limit
    ...
    def test_weekend_holiday_distribution(
        ...
        ***REMOVED*** Create blocks including weekends and holidays
        blocks = []
        start_date = date(2025, 1, 1)

        for i in range(14):  ***REMOVED*** 2 weeks
            ...
            block = Block(
                ...
                is_holiday=(i == 0),  ***REMOVED*** Jan 1 is a holiday
            )
            db.add(block)
            blocks.append(block)
        db.commit()

        ***REMOVED*** Assign weekends/holidays unevenly
        weekend_blocks = [b for b in blocks if b.is_weekend or b.is_holiday]
        for i, block in enumerate(weekend_blocks):
            ***REMOVED*** Give all weekend/holiday shifts to first resident
            assignment = Assignment(
                id=uuid4(),
                block_id=block.id,
        ...
        ***REMOVED*** Calculate distribution
        weekend_assignments = db.query(Assignment).join(Block).filter(
            Block.is_weekend == True  ***REMOVED*** noqa: E712
        ).all()

        ***REMOVED*** All weekend assignments should go to one person
...
***REMOVED*** ============================================================================
***REMOVED*** Coverage and Compliance Tests
***REMOVED*** ============================================================================
...
class TestCoverageAndCompliance:
    ...
    def test_calculate_coverage_rate_partial_coverage(self):
        ...
        blocks = [{"id": str(uuid4())} for _ in range(10)]
        ***REMOVED*** Only cover 8 out of 10 blocks
    ...
    def test_calculate_acgme_compliance_rate_with_violations(self):
        ...
        assert result["status"] == "warning"  ***REMOVED*** Has violations but >90%
    ...
    def test_calculate_acgme_compliance_rate_critical(self):
        ...
        assert result["status"] == "critical"  ***REMOVED*** <90% compliance
        assert result["details"]["violations"] == 15


***REMOVED*** ============================================================================
***REMOVED*** Preference Satisfaction Tests
***REMOVED*** ============================================================================
...
class TestPreferenceSatisfaction:
    ...
    def test_calculate_preference_satisfaction_partial(self):
        ...
        assignments = [
            {"person_id": person_a, "rotation_template_id": rotation_x},
            {"person_id": person_a, "rotation_template_id": rotation_y},
            {"person_id": person_a, "rotation_template_id": str(uuid4())}  ***REMOVED*** Not preferred
        ]
...
class TestTrendAnalysis:
    ...
    def test_get_trend_analysis_violations(
        ...
        ***REMOVED*** Create schedule runs with different violation counts
        for i in range(5):
            run = ScheduleRun(
                id=uuid4(),
                ...
                algorithm="cp_sat",
                status="success",
                acgme_violations=i * 2,  ***REMOVED*** Increasing violations
        ...
        ***REMOVED*** Violations should increase over time
    ...
    def test_get_trend_analysis_coverage(
        ...
        ***REMOVED*** Create schedule runs with varying coverage
        for i in range(3):
            run = ScheduleRun(
                id=uuid4(),
                ...
                status="success",
                acgme_violations=0,
                total_blocks_assigned=14 * (i + 1) // 3,  ***REMOVED*** Varying coverage
                runtime_seconds=5.0,
                created_at=datetime.utcnow(),
            )
            db.add(run)
    ...
    def test_get_trend_analysis_runtime(
        ...
        ***REMOVED*** Create schedule runs with increasing runtime
        for i in range(4):
            run = ScheduleRun(
                id=uuid4(),
                ...
                acgme_violations=0,
                total_blocks_assigned=14,
                runtime_seconds=5.0 + (i * 2.0),  ***REMOVED*** Increasing runtime
                created_at=datetime.utcnow(),
            )
            db.add(run)
        db.commit()
...
class TestReportGeneration:
    ...
    def test_generate_monthly_report(
        ...
        ***REMOVED*** Generate report for current month
        today = date.today()
    ...
    def test_generate_compliance_report(
        ...
        ***REMOVED*** Create schedule runs with violations
        start_date = date.today()
        end_date = start_date + timedelta(days=7)
    ...
    def test_generate_workload_report(
        ...
        ***REMOVED*** Create residents with different workloads
        residents = []
        for i in range(3):
            resident = Person(
                id=uuid4(),
        ...
        ***REMOVED*** Assign different amounts
        for i, resident in enumerate(residents):
            num_assignments = 5 + (i * 3)  ***REMOVED*** 5, 8, 11
...
***REMOVED*** ============================================================================
***REMOVED*** Schedule Comparison Tests
***REMOVED*** ============================================================================
...
class TestScheduleComparison:
    ...
    def test_compare_schedules_basic(
        ...
        ***REMOVED*** Create two schedule runs
        run1 = ScheduleRun(
            id=uuid4(),
            start_date=date.today(),
        ...
        assert comparison["differences"]["violations_delta"] == -3  ***REMOVED*** Improvement
        assert comparison["differences"]["blocks_delta"] == 0  ***REMOVED*** Same
        assert comparison["differences"]["runtime_delta"] == 4.5  ***REMOVED*** Slower
...
***REMOVED*** ============================================================================
***REMOVED*** Analytics Engine Integration Tests
***REMOVED*** ============================================================================
...
class TestAnalyticsEdgeCases:
    ...
    def test_fairness_index_single_person(self):
        ...
        assert result["value"] == 1.0  ***REMOVED*** Perfect fairness (only one person)
        assert result["status"] == "good"
        assert result["details"]["std_dev"] == 0
    ...
    def test_coverage_rate_multiple_assignments_per_block(self):
        ...
        ***REMOVED*** Multiple assignments for same block
        assignments = (
        ...
        ***REMOVED*** Should count unique covered blocks (3 out of 5)
