...
def valid_schedule_state():
    ...
    return {
        ...
        "supervision_ratio": 0.5,  ***REMOVED*** 1:2 ratio
...
def invalid_schedule_state():
    ...
    return {
        "total_blocks": 100,
        "covered_blocks": 70,  ***REMOVED*** Low coverage
        "faculty_count": 5,
        "resident_count": 20,
        "acgme_violations": [
        ...
        "avg_hours_per_week": 85.0,  ***REMOVED*** Over limit
        "supervision_ratio": 4.0,  ***REMOVED*** Poor ratio (1:4)
        "workload_std_dev": 0.5,  ***REMOVED*** High imbalance
        "schedule_changes": 30,
        "total_assignments": 100,
        "coverage_by_type": {
            "clinic": 0.70,
...
def trained_immune_system(immune_system, valid_schedule_state):
    ...
    ***REMOVED*** Generate variations of valid schedules for training
    training_schedules = []
    for i in range(10):
        schedule = valid_schedule_state.copy()
        ***REMOVED*** Small variations
...
***REMOVED*** Test Feature Extraction
...
def test_feature_extraction_coverage(immune_system):
    ...
    ***REMOVED*** First feature should be coverage rate (90/100 = 0.9)
...
def test_feature_extraction_violations(immune_system):
    ...
    ***REMOVED*** Features should be different
    assert not np.allclose(features_clean, features_violations)


***REMOVED*** Test Detector Class
...
def test_detector_distance():
    ...
    ***REMOVED*** Distance should be 5.0 (3-4-5 triangle)
    assert abs(distance - 5.0) < 0.01


***REMOVED*** Test Training (Negative Selection)
...
def test_negative_selection_rejects_self_matching(immune_system, valid_schedule_state):
    ...
    ***REMOVED*** Extract features from valid schedule
    valid_features = immune_system.extract_features(valid_schedule_state)

    ***REMOVED*** Count how many detectors match valid schedule
    matches = sum(1 for d in immune_system.detectors if d.matches(valid_features))

    ***REMOVED*** Should be zero or very few (negative selection should prevent this)
...
def test_anomaly_detection_on_valid_schedule(trained_immune_system, valid_schedule_state):
    ...
    ***REMOVED*** Valid schedule should not be anomalous
    ***REMOVED*** (some detectors might trigger due to randomness, but generally should be False)
...
def test_anomaly_detection_on_invalid_schedule(trained_immune_system, invalid_schedule_state):
    ...
    ***REMOVED*** Invalid schedule should be detected as anomaly
    assert is_anomaly
...
def test_anomaly_score_invalid_higher_than_valid(
    ...
    ***REMOVED*** Invalid should have higher anomaly score
...
def test_detect_anomaly_returns_report(trained_immune_system, invalid_schedule_state):
    ...
    if report:  ***REMOVED*** Only check if anomaly was detected
...
def test_antibody_affinity_calculation():
    ...
    ***REMOVED*** Point at center should have affinity 1.0
...
***REMOVED*** Test Antibody Registration and Selection
...
def test_select_antibody_highest_affinity(immune_system, invalid_schedule_state):
    ...
    ***REMOVED*** Register two antibodies
    immune_system.register_antibody(
        name="antibody1",
        repair_fn=repair1,
        affinity_pattern=invalid_schedule_state,  ***REMOVED*** High affinity
        affinity_radius=2.0,
    )
    ...
    immune_system.register_antibody(
        name="antibody2",
        repair_fn=repair2,
        affinity_radius=0.1,  ***REMOVED*** Low affinity
    )

    ***REMOVED*** Select for invalid schedule
    selection = immune_system.select_antibody(invalid_schedule_state)

    assert selection is not None
    name, antibody = selection
    ***REMOVED*** Should select antibody1 (higher affinity)
...
***REMOVED*** Test Repair Application
...
def test_apply_repair_success(trained_immune_system, invalid_schedule_state):
    ...
    def fix_coverage(state):
        ...
        state = state.copy()
        state["covered_blocks"] = state["total_blocks"]  ***REMOVED*** Full coverage
        state["acgme_violations"] = []  ***REMOVED*** Clear violations
        state["avg_hours_per_week"] = 75.0  ***REMOVED*** Compliant hours
    ...
    assert result.anomaly_after < result.anomaly_before  ***REMOVED*** Should improve
...
def test_apply_repair_ineffective(trained_immune_system, invalid_schedule_state):
    ...
    def bad_repair(state):
        ...
        return state  ***REMOVED*** No change
    ...
    assert result is not None
    assert not result.successful  ***REMOVED*** Should fail
...
def test_apply_repair_updates_statistics(trained_immune_system, invalid_schedule_state):
    ...
    ***REMOVED*** Applications count should increase
    assert (
...
def test_full_workflow(immune_system, valid_schedule_state, invalid_schedule_state):
    ...
    ***REMOVED*** 2. Detect anomaly
    is_anomaly = immune_system.is_anomaly(invalid_schedule_state)
    assert is_anomaly

    ***REMOVED*** 3. Register repair antibody
    ...
    ***REMOVED*** 5. Verify repaired state is better
    ...
    ***REMOVED*** Repaired should be less anomalous (or not anomalous at all)
...
def test_multiple_repairs_improve_success_rate(
    ...
    ***REMOVED*** Apply repair multiple times
    for _ in range(5):
        trained_immune_system.apply_repair(invalid_schedule_state)

    antibody = trained_immune_system.antibodies["good_repair"]
    assert antibody.applications_count == 5
    assert antibody.success_rate > 0.5  ***REMOVED*** Should have decent success rate
