...
def small_candidates() -> list[CallCandidate]:
    ...
    for i in range(10):
        avoid_days = set()
        if i < 3:  ***REMOVED*** First 3 avoid Tuesdays
            avoid_days.add(1)
        if i >= 7:  ***REMOVED*** Last 3 prefer Wednesdays
            pref = {2: 2.0}
        else:
            pref = {}
...
def small_nights(base_date) -> list[CallNight]:
    ...
    return create_call_nights_from_dates(
        ...
        exclude_weekends=True,  ***REMOVED*** Exclude Fri/Sat
    )
...
class TestQUBOFormulation:
    ...
    def test_variable_count(self, minimal_candidates, minimal_nights):
        ...
        assert formulation.num_variables == 70  ***REMOVED*** 10 × 7
    ...
    def test_qubo_matrix_construction(self, minimal_candidates, minimal_nights):
        ...
        assert len(Q) > 0
        ***REMOVED*** Should have diagonal (linear) terms
        diagonal_terms = sum(1 for (i, j) in Q.keys() if i == j)
        assert diagonal_terms > 0
        ***REMOVED*** Should have off-diagonal (quadratic) terms
...
class TestSimulatedAnnealing:
    ...
    def test_tunneling_strength_affects_results(self, minimal_candidates, minimal_nights):
        ...
        ***REMOVED*** Low tunneling (more classical)
        ...
        ***REMOVED*** High tunneling (more quantum-like)
        ...
        ***REMOVED*** Both should find solutions (may or may not differ due to randomness)
...
class TestConstraintSatisfaction:
    ...
    def test_coverage_constraint(self, small_candidates, small_nights):
        ...
        ***REMOVED*** Count assignments per night
        night_counts = {}
        ...
        ***REMOVED*** Check each assigned night has exactly one resident
    ...
    def test_no_consecutive_calls(self, small_candidates, small_nights):
        ...
        ***REMOVED*** Check no consecutive dates
...
class TestValidator:
    ...
    def test_valid_solution_passes(self, small_candidates, small_nights):
        ...
        ***REMOVED*** Should have some assignments
        assert len(solution.assignments) > 0
        ***REMOVED*** Validator should return metrics
    ...
    def test_equity_within_tolerance(self, small_candidates, small_nights):
        ...
        validator = CallAssignmentValidator(
            small_nights,
            small_candidates,
            equity_tolerance=5.0,  ***REMOVED*** Allow some variance
        )
        is_valid, violations, metrics = validator.validate(solution.assignments)

        ***REMOVED*** Equity range should be reasonable
        equity_range = metrics["equity"]["range"]
        ***REMOVED*** With good solver, range shouldn't be extreme
        assert equity_range <= 10, f"Equity range too large: {equity_range}"


***REMOVED*** =============================================================================
***REMOVED*** SCALABILITY TESTS
***REMOVED*** =============================================================================
...
class TestScalability:
    ...
    def test_minimal_problem_70_vars(self, minimal_candidates, minimal_nights):
        ...
        assert solution.runtime_seconds < 10  ***REMOVED*** Should be fast
        assert len(solution.assignments) > 0
    ...
    def test_sweet_spot_7300_vars(self, medium_candidates, base_date):
        ...
        ***REMOVED*** With weekends excluded, should be ~260 nights
        ...
        assert solution.runtime_seconds < 300  ***REMOVED*** 5 minute max
        assert len(solution.assignments) > 0
...
***REMOVED*** =============================================================================
***REMOVED*** LANDSCAPE EXPORT TESTS
***REMOVED*** =============================================================================
...
class TestLandscapeExport:
    ...
    def test_landscape_json_export(self, minimal_candidates, minimal_nights):
        ...
        ***REMOVED*** Should be JSON serializable
        json_str = json.dumps(solution.landscape_data, default=str)
        assert len(json_str) > 0

        ***REMOVED*** Should parse back
...
class TestHelperFunctions:
    ...
    def test_create_call_nights_from_dates(self, base_date):
        ...
        ***REMOVED*** 14 days, minus Fri/Sat should give ~10 nights
        assert 8 <= len(nights) <= 12

        ***REMOVED*** All should have OVERNIGHT type
        for night in nights:
    ...
    def test_create_call_nights_with_weekends(self, base_date):
        ...
        assert len(nights) == 7  ***REMOVED*** Full week
...
class TestEdgeCases:
    ...
    def test_single_candidate_single_night(self, base_date):
        ...
        ***REMOVED*** Should assign the one resident to the one night
    ...
    def test_more_nights_than_candidates(self, base_date):
        ...
        candidates = [
            CallCandidate(
                ...
                name=f"Resident-{i}",
                pgy_level=1,
                max_calls_per_week=1,  ***REMOVED*** Very restrictive
            )
            for i in range(3)
        ]
        nights = [
        ...
        ***REMOVED*** May not cover all nights due to constraints
        assert len(solution.assignments) >= 0

    def test_high_constraint_pressure(self, base_date):
        """Test case with high constraint pressure."""
        ***REMOVED*** Create candidates that all avoid certain days
        candidates = [
            CallCandidate(
                person_id=uuid4(),
                name=f"Resident-{i}",
                pgy_level=1,
                avoid_days={0, 1, 2},  ***REMOVED*** Avoid Mon, Tue, Wed
            )
            for i in range(5)
        ]
        nights = create_call_nights_from_dates(
        ...
        ***REMOVED*** Should still find a solution (preferences are soft)
