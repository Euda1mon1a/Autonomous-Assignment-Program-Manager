...
def test_rotation_templates(db: Session) -> dict[str, RotationTemplate]:
    ...
    templates = {
        ...
        "no_location": RotationTemplate(
            ...
            activity_type="admin",
            abbreviation="ADM",
            clinic_location=None,  ***REMOVED*** No location set
            max_residents=0,
...
class TestDailyManifestBasicFunctionality:
    ...
    def test_get_manifest_for_date(
        ...
        ***REMOVED*** Verify response structure
        ...
        assert data["time_of_day"] is None  ***REMOVED*** No filter, should be None
    ...
    def test_manifest_with_no_assignments(
        ...
        ***REMOVED*** Use a date far in the future with no assignments
            ...
            ***REMOVED*** Verify assignment structure
            for time_slot, assignments in time_slots.items():
                for assignment in assignments:
                    ...
                    ***REMOVED*** Verify person structure
...
class TestDailyManifestDateFiltering:
    ...
    def test_date_parameter_required(
        ...
        ***REMOVED*** FastAPI will return 422 for missing required query parameter
    ...
    def test_filters_by_specific_date(
        ...
        ***REMOVED*** Create assignments on different dates
        ...
        ***REMOVED*** Query for the test date
        response = client.get(
            "/api/assignments/daily-manifest",
            params={"date": manifest_test_date.isoformat()},
            headers={"Authorization": f"Bearer {auth_token}"},
        ...
        ***REMOVED*** Should only have assignments from test_day
        assert data["date"] == manifest_test_date.isoformat()
        assert len(data["locations"]) == 1

        ***REMOVED*** Count total assignments across all time slots
...
***REMOVED*** ============================================================================
***REMOVED*** Time of Day Filtering Tests
***REMOVED*** ============================================================================
...
class TestDailyManifestTimeOfDayFiltering:
    ...
    def test_filter_by_am(
        ...
        ***REMOVED*** Verify only AM assignments are present
        for location in data["locations"]:
            am_assignments = location["time_slots"]["AM"]
            pm_assignments = location["time_slots"]["PM"]

            ***REMOVED*** AM should have assignments (based on populated_manifest)
            ***REMOVED*** PM should be empty
    ...
    def test_filter_by_pm(
        ...
        ***REMOVED*** Verify only PM assignments are present
        for location in data["locations"]:
            am_assignments = location["time_slots"]["AM"]
            pm_assignments = location["time_slots"]["PM"]

            ***REMOVED*** AM should be empty
            ***REMOVED*** PM should have assignments (based on populated_manifest)
    ...
    def test_no_time_filter_shows_both(
        ...
        ***REMOVED*** Find a location that has both AM and PM assignments
        found_am = False
        found_pm = False
        ...
        ***REMOVED*** Based on populated_manifest, we should have both
...
***REMOVED*** ============================================================================
***REMOVED*** Location Grouping Tests
***REMOVED*** ============================================================================
...
class TestDailyManifestLocationGrouping:
    ...
    def test_groups_by_clinic_location(
        ...
        ***REMOVED*** Should have multiple locations
        assert len(data["locations"]) >= 2

        ***REMOVED*** Verify expected locations are present
    ...
    def test_unassigned_location_for_null_clinic_location(
        ...
        ***REMOVED*** Should have an "Unassigned" location
        location_names = [loc["clinic_location"] for loc in data["locations"]]
        assert "Unassigned" in location_names

        ***REMOVED*** Verify Unassigned location has the expected assignment
        unassigned_location = next(
            loc for loc in data["locations"] if loc["clinic_location"] == "Unassigned"
        )

        ***REMOVED*** Based on populated_manifest, should have 1 faculty in PM slot
    ...
    def test_separate_time_slots_within_location(
        ...
        ***REMOVED*** Based on populated_manifest:
        ***REMOVED*** AM: 2 residents + 1 faculty = 3
        ***REMOVED*** PM: 1 resident + 1 faculty = 2
        assert len(am_assignments) == 3
        assert len(pm_assignments) == 2

        ***REMOVED*** Verify person IDs are different between slots
        am_person_ids = {a["person"]["id"] for a in am_assignments}
        pm_person_ids = {a["person"]["id"] for a in pm_assignments}

        ***REMOVED*** There should be some overlap (faculty might work both shifts)
        ***REMOVED*** but they should not be identical
        assert am_person_ids != pm_person_ids


***REMOVED*** ============================================================================
***REMOVED*** Staffing Summary Tests
***REMOVED*** ============================================================================
...
class TestDailyManifestStaffingSummary:
    ...
    def test_staffing_summary_counts_residents_correctly(
        ...
        ***REMOVED*** Based on populated_manifest:
        ***REMOVED*** AM: 2 residents (PGY1, PGY2)
        ***REMOVED*** PM: 1 resident (PGY3)
        ***REMOVED*** Total: 3 residents across both time slots
    ...
    def test_staffing_summary_counts_faculty_correctly(
        ...
        ***REMOVED*** Based on populated_manifest:
        ***REMOVED*** AM: 1 faculty
        ***REMOVED*** PM: 1 faculty (different person)
        ***REMOVED*** Total: 2 faculty across both time slots
    ...
    def test_staffing_summary_with_time_filter(
        ...
        ***REMOVED*** AM only: 2 residents + 1 faculty = 3 total
    ...
    def test_staffing_summary_identifies_residents_by_pgy_level(
        ...
        ***REMOVED*** Create a person with pgy_level and one without
        resident = Person(
            id=uuid4(),
        ...
        faculty = Person(
            ...
            pgy_level=None,  ***REMOVED*** Faculty should not have pgy_level
        )
        ...
        ***REMOVED*** Should have 1 resident and 1 faculty
    ...
    def test_staffing_summary_handles_zero_counts(
        ...
        ***REMOVED*** Create location with only residents
        resident = Person(
            id=uuid4(),
        ...
        ***REMOVED*** Should have 1 resident, 0 faculty
...
***REMOVED*** ============================================================================
***REMOVED*** Assignment Details Tests
***REMOVED*** ============================================================================
...
class TestDailyManifestAssignmentDetails:
    ...
    def test_person_details_included(
        ...
        ***REMOVED*** Get first assignment
        ...
        if am_assignments:
            ...
            assert "name" in person
            assert "pgy_level" in person
            assert person["name"]  ***REMOVED*** Should have a name
    ...
    def test_role_included_in_assignment(
        ...
        ***REMOVED*** Check that all assignments have roles
        for location in data["locations"]:
    ...
    def test_activity_name_included(
        ...
        ***REMOVED*** Check that all assignments have activity names
        for location in data["locations"]:
            for time_slot, assignments in location["time_slots"].items():
                for assignment in assignments:
                    assert "activity" in assignment
                    ***REMOVED*** Activity should be the template name or override
                    assert assignment["activity"]  ***REMOVED*** Should not be empty
    ...
    def test_activity_override_used_when_present(
        ...
        ***REMOVED*** Find the assignment with override
...
class TestDailyManifestEdgeCases:
    ...
    def test_handles_multiple_assignments_same_person(
        ...
        ***REMOVED*** Create two assignments for same person, same day, different times
        assignment_am = Assignment(
            id=uuid4(),
            block_id=test_blocks["test_day"][0].id,  ***REMOVED*** AM
            person_id=test_residents[0].id,
            rotation_template_id=test_rotation_templates["clinic_a"].id,
            role="primary",
            created_by=test_user.username,
        ...
        assignment_pm = Assignment(
            id=uuid4(),
            block_id=test_blocks["test_day"][1].id,  ***REMOVED*** PM
            person_id=test_residents[0].id,
            rotation_template_id=test_rotation_templates["clinic_b"].id,
            role="primary",
            created_by=test_user.username,
        ...
        ***REMOVED*** Both assignments should appear
    ...
    def test_handles_missing_rotation_template(
        ...
        ***REMOVED*** Create assignment without rotation_template_id
        assignment = Assignment(
            id=uuid4(),
            block_id=test_blocks["test_day"][0].id,
            person_id=test_residents[0].id,
        ...
        ***REMOVED*** Query should still work
        response = client.get(
            "/api/assignments/daily-manifest",
            params={"date": manifest_test_date.isoformat()},
            headers={"Authorization": f"Bearer {auth_token}"},
        ...
        ***REMOVED*** Should have Unassigned location
        location_names = [loc["clinic_location"] for loc in data["locations"]]
        assert "Unassigned" in location_names
    ...
    def test_date_format_validation(
        ...
        for invalid_date in invalid_dates:
            ...
            ***REMOVED*** Should return 422 for invalid date format
            assert response.status_code == 422
