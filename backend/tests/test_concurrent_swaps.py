...
class TestTransactionManagement:
    ...
    def test_transactional_context_commits_on_success(self, db: Session):
        ...
        with transactional(db):
            ...
            db.add(swap)
            ***REMOVED*** Should auto-commit

        ***REMOVED*** Verify committed (new session)
        db.expire_all()
    ...
    def test_transactional_with_savepoint(self, db: Session):
        ...
        with transactional(db):
            ...
            ***REMOVED*** Inner transaction with savepoint
            try:
                ...
                pass  ***REMOVED*** Catch inner exception

            ***REMOVED*** Outer transaction should still succeed

        ***REMOVED*** Verify outer committed, inner rolled back
        db.expire_all()
        assert (
    ...
    def test_transactional_with_retry_on_deadlock(self, db: Session):
        ...
        ***REMOVED*** This is a simulation since triggering real deadlocks is complex
        swap_id = uuid4()
        attempt_count = [0]
        ...
        def operation_with_simulated_deadlock():
            ...
            if attempt_count[0] < 3:
                ***REMOVED*** Simulate deadlock on first 2 attempts
                from sqlalchemy.exc import OperationalError

                raise OperationalError("deadlock detected", None, None)

            ***REMOVED*** Succeed on 3rd attempt
            swap = SwapRecord(
                id=swap_id,
                source_faculty_id=uuid4(),
                source_week=date.today(),
        ...
        ***REMOVED*** Should succeed after retries
...
class TestDistributedLocking:
    ...
    def test_lock_prevents_concurrent_access(self, redis_client):
        ...
        ***REMOVED*** First lock acquires
        assert lock1.acquire(blocking=False) is True

        ***REMOVED*** Second lock should fail (non-blocking)
        assert lock2.acquire(blocking=False) is False

        ***REMOVED*** Release first lock
        lock1.release()

        ***REMOVED*** Now second lock can acquire
    ...
    def test_lock_auto_expires(self, redis_client):
        ...
        ***REMOVED*** Wait for expiry
        time.sleep(1.5)

        ***REMOVED*** Another lock should be able to acquire
    ...
    def test_lock_acquisition_timeout(self, redis_client):
        ...
        ***REMOVED*** Second lock should timeout
    ...
    def test_lock_extend(self, redis_client):
        ...
        ***REMOVED*** Lock should still be valid after original TTL
        time.sleep(2.5)
        assert lock.is_locked() is True
...
class TestIdempotency:
    ...
    def test_idempotency_expiry(self, idempotency):
        ...
        ***REMOVED*** Should be duplicate initially
        assert idempotency.is_duplicate(operation_id) is True

        ***REMOVED*** Wait for expiry
        time.sleep(1.5)

        ***REMOVED*** Should no longer be duplicate
        assert idempotency.is_duplicate(operation_id) is False
...
class TestConcurrentSwapExecution:
    ...
    def test_concurrent_swap_execution_no_race(
        ...
        ***REMOVED*** Execute 5 concurrent swap attempts
        threads = []
        for _ in range(5):
        ...
        ***REMOVED*** Wait for all threads
        for thread in threads:
            thread.join()

        ***REMOVED*** Verify: should have exactly one success, rest should fail or be idempotent
        successful = [r for r in results if r.success]
        assert len(successful) >= 1, "At least one swap should succeed"

        ***REMOVED*** Check database state is consistent
        db.expire_all()
        swaps = (
            db.query(SwapRecord)
            .filter(
        ...
        ***REMOVED*** Should have limited number of swap records (transaction isolation)
    ...
    def test_concurrent_rollback_no_race(
        ...
        ***REMOVED*** Now try concurrent rollbacks
        rollback_results = []
        errors = []
        ...
        ***REMOVED*** Execute 5 concurrent rollback attempts
        threads = []
        for _ in range(5):
...
class TestRowLevelLocking:
    ...
    def test_with_for_update_prevents_concurrent_modification(
        ...
        ***REMOVED*** Create a swap record
        ...
        def modify_swap_with_lock():
            ...
            with transactional(db):
                ...
                ***REMOVED*** Simulate work while holding lock
                time.sleep(0.1)
                ...
                ***REMOVED*** Record how long we waited for lock
                lock_waits.append(time.time() - start_time)

        ***REMOVED*** Execute 3 concurrent modifications
        threads = []
        for i in range(3):
        ...
        ***REMOVED*** All modifications should have completed
        assert modification_count[0] == 3

        ***REMOVED*** Later threads should have waited (lock_wait > 0.1s)
        ***REMOVED*** Due to row locking, they execute serially
        assert any(wait > 0.1 for wait in lock_waits[1:]), (
            "Later threads should wait for lock"
        )


***REMOVED*** Fixtures for tests
@pytest.fixture
