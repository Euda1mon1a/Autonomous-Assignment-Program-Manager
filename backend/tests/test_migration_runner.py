...
class TestMigration:
    ...
    def test_migration_hashable(self):
        ...
        assert len(migration_set) == 1  ***REMOVED*** Same version, so only one in set
...
class TestMigrationLockManager:
    ...
    def test_acquire_lock_already_held(self, db: Session):
        ...
        ***REMOVED*** Manager 1 acquires lock
        lock1 = manager1.acquire_lock("test_lock", wait=False)
        assert lock1 is not None

        ***REMOVED*** Manager 2 tries to acquire same lock
    ...
    def test_acquire_lock_wait(self, db: Session):
        ...
        ***REMOVED*** Manager 1 acquires lock with short timeout
        lock1 = manager1.acquire_lock("test_lock", wait=False)
        assert lock1 is not None

        ***REMOVED*** Manually expire the lock
        lock1.expires_at = datetime.utcnow() - timedelta(seconds=1)
        db.commit()

        ***REMOVED*** Manager 2 should acquire expired lock
    ...
    def test_refresh_lock(self, db: Session):
        ...
        ***REMOVED*** Wait a bit and refresh
        time.sleep(0.1)
    ...
    def test_cleanup_expired_locks(self, db: Session):
        ...
        ***REMOVED*** Create lock with short timeout
        lock = manager.acquire_lock("test_lock", wait=False)

        ***REMOVED*** Manually expire it
        lock.expires_at = datetime.utcnow() - timedelta(seconds=1)
        db.commit()

        ***REMOVED*** Clean up expired locks
    ...
    def test_cannot_refresh_unowned_lock(self, db: Session):
        ...
        ***REMOVED*** Manager 2 tries to refresh Manager 1's lock
        with pytest.raises(MigrationLockError):
...
class TestDependencyResolver:
    ...
    def test_resolve_simple_order(self):
        ...
        ***REMOVED*** Without dependencies, order should be preserved
        ordered = resolver.resolve_order(migrations)

        assert len(ordered) == 3
        ***REMOVED*** All migrations should be present
    ...
    def test_resolve_complex_dependencies(self):
        ...
        versions = [m.version for m in ordered]
        ***REMOVED*** 001 must come first
        assert versions[0] == "001"
        ***REMOVED*** 002 and 003 must come before 004
    ...
    def test_filter_from_current(self):
        ...
        ***REMOVED*** Filter from current version 002
        filtered = resolver.filter_to_target(
...
class TestMigrationDiscovery:
    ...
    def test_discover_migrations(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ***REMOVED*** Create migration files
    ...
    def test_discover_ignores_non_matching_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ***REMOVED*** Create valid migration
            self._create_migration_file(tmpdir, "001_valid.py", revision="001")

            ***REMOVED*** Create files that should be ignored
            ...
            ***REMOVED*** Should only find the valid migration
    ...
    def test_calculate_file_checksum(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(checksum1) == 64  ***REMOVED*** SHA-256 hex digest
...
class TestMigrationRunner:
    ...
    def test_run_migrations_dry_run(self, db: Session):
        ...
        ***REMOVED*** In dry run, changes are rolled back, but functions are still called
    ...
    def test_run_migrations_with_lock(self, db: Session):
        ...
        ***REMOVED*** Lock should be released after execution
        locks = (
    ...
    def test_run_migrations_with_hooks(self, db: Session):
        ...
        assert hook_calls["pre_migration"] == 2  ***REMOVED*** Called for each migration
    ...
    def test_run_migrations_partial_failure(self, db: Session):
        ...
        ***REMOVED*** First migration succeeds, second fails
