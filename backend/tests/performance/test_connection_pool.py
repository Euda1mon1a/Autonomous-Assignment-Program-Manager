...
***REMOVED*** ============================================================================
***REMOVED*** Test Fixtures for Connection Pool Configuration
***REMOVED*** ============================================================================
...
def small_pool_engine():
    ...
    test_engine = create_engine(
        ...
        pool_size=5,
        max_overflow=5,
        pool_timeout=2,  ***REMOVED*** 2 second timeout
        pool_pre_ping=True,
...
def monitored_pool_engine():
    ...
    ***REMOVED*** Track connection events
    connection_events = {
        "checked_out": 0,
        "checked_in": 0,
        "connect": 0,
    ...
    ***REMOVED*** Attach metrics to engine for test access
...
***REMOVED*** ============================================================================
***REMOVED*** Test Class: Connection Pool Saturation
***REMOVED*** ============================================================================
...
class TestConnectionPoolStress:
    ...
    def test_pool_saturation_graceful_handling(self, small_pool_engine, small_pool_session, pool_metrics):
        ...
        """
        num_requests = 15
        hold_duration = 0.5  ***REMOVED*** Hold connection for 500ms
        ...
        def long_running_query(session_factory, request_id):
            ...
            try:
                ...
                try:
                    ***REMOVED*** Hold the connection with a long operation
                    time.sleep(hold_duration)

                    ***REMOVED*** Perform a simple query to verify connection works
        ...
        ***REMOVED*** Execute concurrent requests
        ...
        ***REMOVED*** Pool should be healthy (no connections leaked)
    ...
    def test_concurrent_celery_and_api_requests(self, small_pool_engine, small_pool_session):
        ...
        def simulate_celery_task(session_factory, task_id):
            ...
            try:
                session = session_factory()
                try:
                    ***REMOVED*** Simulate longer processing
                    time.sleep(0.3)
        ...
        ***REMOVED*** Most operations should succeed (some may timeout under high load)
        ...
        ***REMOVED*** API requests should be faster than Celery tasks
...
***REMOVED*** ============================================================================
***REMOVED*** Test Class: Connection Leak Detection
***REMOVED*** ============================================================================
...
class TestConnectionLeakDetection:
    ...
    def test_connection_leak_detection(self, monitored_pool_engine):
        ...
        ***REMOVED*** Verify all requests succeeded
        successful = sum(1 for r in results if r.get("success"))
        assert successful == num_requests, f"All {num_requests} requests should succeed"

        ***REMOVED*** Check connection metrics
        events = monitored_pool_engine._connection_events

        ***REMOVED*** Allow small margin for timing differences
        checkout_count = events["checkout"]
        checkin_count = events["checked_in"]

        ***REMOVED*** All checked-out connections should be returned (within small margin)
        ...
        ***REMOVED*** Pool should be at or near baseline
    ...
    def test_long_running_transaction_timeout(self, small_pool_engine, small_pool_session):
        ...
        def long_transaction(session_factory):
            ...
            try:
                ...
                ***REMOVED*** Hold transaction open for extended period
                time.sleep(1.0)
        ...
        ***REMOVED*** Execute mixed workload
        with ThreadPoolExecutor(max_workers=8) as executor:
            ***REMOVED*** Start long transaction
            long_tx_future = executor.submit(long_transaction, small_pool_session)

            ***REMOVED*** Give it a moment to start
            time.sleep(0.1)

            ***REMOVED*** Submit multiple quick queries while long transaction is running
            quick_futures = [
        ...
        ***REMOVED*** Long transaction should complete
        assert long_tx_result.get("success"), "Long transaction should complete successfully"

        ***REMOVED*** Most quick queries should succeed (some may timeout)
    ...
    def test_rollback_releases_connection(self, monitored_pool_engine):
        ...
        def failing_transaction(session_factory):
            ...
            session = session_factory()
            try:
                ***REMOVED*** Create invalid data to trigger failure
                person = Person(
                    id=uuid4(),
                    name="Test Person",
                    type="resident",
        ...
        ***REMOVED*** Execute multiple failing transactions
        num_failures = 50
        ...
        ***REMOVED*** All should have rolled back
        ...
        ***REMOVED*** All connections should be returned to pool
        ...
        ***REMOVED*** Should be close to num_failures (within small margin for concurrent timing)
        assert connections_returned >= num_failures - 5, \
            f"All connections should be returned: {connections_returned}/{num_failures}"


***REMOVED*** ============================================================================
***REMOVED*** Test Class: Pool Recovery
***REMOVED*** ============================================================================
...
class TestPoolRecovery:
    ...
    def test_pool_recovery_after_db_restart(self, small_pool_engine, small_pool_session):
        ...
        """
        ***REMOVED*** Initial successful query
        ...
        ***REMOVED*** Simulate database restart by disposing pool
        small_pool_engine.dispose()

        ***REMOVED*** Wait briefly
        time.sleep(0.1)

        ***REMOVED*** Attempt queries after "restart" - should reconnect automatically
        session = small_pool_session()
        try:
            result = session.execute(text("SELECT 1")).scalar()
    ...
    def test_connection_health_checks_work(self):
        ...
        """
        ***REMOVED*** Create engine with pre_ping enabled
        test_engine = create_engine(
            "sqlite:///:memory:",
            poolclass=QueuePool,
            pool_size=5,
            max_overflow=5,
            pool_pre_ping=True,  ***REMOVED*** Enable health checks
            connect_args={"check_same_thread": False},
        )
        ...
        try:
            ***REMOVED*** Execute multiple queries
            for i in range(10):
                session = TestSession()
                try:
                    result = session.execute(text("SELECT 1")).scalar()
            ...
            ***REMOVED*** All queries should succeed without stale connection errors
            ***REMOVED*** The pre_ping should have validated connections
    ...
    def test_pool_invalidation_and_recovery(self, small_pool_engine, small_pool_session):
        ...
        ***REMOVED*** Get current pool
        current_pool = small_pool_engine.pool

        ***REMOVED*** Recreate the pool (simulates invalidation)
        small_pool_engine.pool = current_pool.recreate()

        ***REMOVED*** Should be able to execute queries with new pool
        session = small_pool_session()
        try:
            result = session.execute(text("SELECT 1")).scalar()
...
***REMOVED*** ============================================================================
***REMOVED*** Test Class: Concurrent Transaction Isolation
***REMOVED*** ============================================================================
...
class TestConcurrentTransactionIsolation:
    ...
    def test_concurrent_transactions_modifying_same_data(self, small_pool_engine):
        ...
        ***REMOVED*** Execute concurrent updates
        ...
        ***REMOVED*** All updates should succeed (SQLite handles this with serialized isolation)
        successful = sum(1 for r in results if r.get("success"))
        assert successful >= 1, "At least one update should succeed"

        ***REMOVED*** Verify final state
        session = TestSession()
        try:
            person = session.query(Person).filter(Person.id == person_id).first()
    ...
    def test_read_committed_isolation_level(self, small_pool_engine):
        ...
        ***REMOVED*** Transaction 1: Update but don't commit yet
        session1 = TestSession()
        person1 = session1.query(Person).filter(Person.id == person_id).first()
        person1.name = "Updated Name (not committed)"
        session1.flush()

        ***REMOVED*** Transaction 2: Read person (should see original value)
        session2 = TestSession()
        try:
            person2 = session2.query(Person).filter(Person.id == person_id).first()

            ***REMOVED*** In SQLite with default isolation, may see original or updated depending on timing
            ***REMOVED*** Just verify no crash and data is consistent
        ...
        ***REMOVED*** Rollback transaction 1
        session1.rollback()
        session1.close()

        ***REMOVED*** Verify final state (should be original)
    ...
    def test_deadlock_detection_and_recovery(self, small_pool_engine):
        ...
        ***REMOVED*** Create two people
        ...
        def update_persons_order1(session_factory):
            ...
            try:
                ...
                time.sleep(0.2)  ***REMOVED*** Create timing gap
        ...
        ***REMOVED*** Execute concurrent updates in opposite order (potential deadlock)
        ...
        ***REMOVED*** At least one should succeed (SQLite serializes transactions)
        successful = sum(1 for r in results if r.get("success"))
        assert successful >= 1, "At least one transaction should succeed"

        ***REMOVED*** No unhandled exceptions - all should return result dict
        assert all("success" in r for r in results), "All transactions should complete (success or failure)"


***REMOVED*** ============================================================================
***REMOVED*** Test Class: Task Session Scope (Celery Pattern)
***REMOVED*** ============================================================================
...
class TestTaskSessionScope:
    ...
    def test_task_session_scope_basic_usage(self):
        ...
        with task_session_scope() as session:
            ...
            session.add(person)
            ***REMOVED*** Commit happens automatically on context exit

        ***REMOVED*** Verify person was created
        with task_session_scope() as session:
    ...
    def test_task_session_scope_concurrent_tasks(self):
        ...
        def celery_task_simulation(task_id):
            ...
            try:
                with task_session_scope() as session:
                    ...
                    session.add(person)
                    ***REMOVED*** Auto-commits on exit

                return {"success": True, "task_id": task_id}
            except Exception as e:
                return {"success": False, "task_id": task_id, "error": str(e)}

        ***REMOVED*** Execute 20 concurrent "tasks"
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
        ...
        ***REMOVED*** All tasks should succeed
        successful = sum(1 for r in results if r.get("success"))
        assert successful == 20, f"All 20 tasks should succeed, got {successful}"

        ***REMOVED*** Verify all persons were created
        with task_session_scope() as session:
...
***REMOVED*** ============================================================================
***REMOVED*** Performance Metrics Summary Test
***REMOVED*** ============================================================================
...
class TestConnectionPoolMetrics:
    ...
    def test_pool_performance_metrics(self, monitored_pool_engine, pool_metrics):
        ...
        ***REMOVED*** Performance assertions (reasonable thresholds)
        ...
        ***REMOVED*** Print metrics for analysis
        print(f"\n{'='*60}")
