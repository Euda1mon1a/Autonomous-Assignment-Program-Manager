...
Note: As of PR ***REMOVED***240, idempotency is implemented for schedule generation.
...
class TestScheduleGenerationIdempotencyLoad:
    ...
    def test_schedule_generation_100_concurrent(
        ...
        ***REMOVED*** Same idempotency key for all requests (simulating retries)
        idempotency_key = f"schedule-gen-{uuid4()}"

        ***REMOVED*** Count schedule runs before
        initial_count = db.query(ScheduleRun).count()

        ***REMOVED*** Send 100 concurrent identical requests
        def make_request():
            return client.post(
        ...
        ***REMOVED*** Execute concurrently using thread pool (since TestClient is sync)
        ...
        ***REMOVED*** All requests should succeed (200 or 207 for partial success)
        ...
        ***REMOVED*** All responses should return the same run ID
        ...
        ***REMOVED*** Database should have exactly 1 new schedule run
        final_count = db.query(ScheduleRun).count()
        assert final_count == initial_count + 1, (
            f"Expected 1 new schedule run, database has {final_count - initial_count}"
        )

        ***REMOVED*** Verify idempotency record exists and is completed
        idem_record = (
        ...
        ***REMOVED*** At least some responses should have X-Idempotency-Replayed header
        replayed_responses = [
            r for r in responses if r.headers.get("X-Idempotency-Replayed") == "true"
        ]
        ***REMOVED*** After the first request, all others should be replayed
    ...
    def test_schedule_generation_network_retry_simulation(
        ...
        ***REMOVED*** Simulate network retry scenario:
        ***REMOVED*** Request 1: Initial request
        ***REMOVED*** Request 2-5: Retries due to timeout (same key)
        ***REMOVED*** Request 6-10: More retries from different connection

        responses = []

        ***REMOVED*** Initial request
        r1 = client.post(
            "/api/schedule/generate",
            json=schedule_data,
            headers={"Idempotency-Key": idempotency_key},
        ...
        ***REMOVED*** Immediate retries (same connection)
        for _ in range(4):
            r = client.post(
                "/api/schedule/generate",
                json=schedule_data,
            ...
            ***REMOVED*** Should have replay header
            assert r.headers.get("X-Idempotency-Replayed") == "true"

        ***REMOVED*** Simulate different connection retrying (concurrent)
        from concurrent.futures import ThreadPoolExecutor
        ...
        ***REMOVED*** All requests should succeed
        assert all(r.status_code in [200, 207] for r in responses)

        ***REMOVED*** All should return same run ID
        run_ids = [r.json().get("run_id") for r in responses]
        assert len(set(run_ids)) == 1

        ***REMOVED*** Database should have exactly 1 schedule run
        schedule_runs = (
    ...
    def test_schedule_generation_idempotency_key_isolation(
        ...
        ***REMOVED*** Create 3 schedule runs with different idempotency keys
        ...
        ***REMOVED*** All should have different run IDs
        run_ids = [r.json().get("run_id") for r in responses]
        assert len(set(run_ids)) == 3

        ***REMOVED*** Each idempotency key should have its own record
        for key in idempotency_keys:
            idem_record = (
                db.query(IdempotencyRequest)
    ...
    def test_schedule_generation_idempotency_conflict_detection(
        ...
        ***REMOVED*** First request: 7-day schedule
        first_request = {
            "start_date": start_date.isoformat(),
        ...
        ***REMOVED*** Second request: Same key, different end date (conflict!)
        second_request = {
            "start_date": start_date.isoformat(),
            "end_date": (start_date + timedelta(days=13)).isoformat(),  ***REMOVED*** Different!
            "algorithm": "greedy",
        }
        ...
        ***REMOVED*** Should be rejected with 422 (conflict)
        assert r2.status_code == 422
        assert "different request parameters" in r2.json()["detail"].lower()

        ***REMOVED*** Only one schedule run should exist for this key
        idem_records = (
            db.query(IdempotencyRequest)
            .filter(IdempotencyRequest.idempotency_key == idempotency_key)
            .all()
...
class TestIdempotencyExpiry:
    ...
    def test_schedule_generation_idempotency_expiry_allows_new_creation(
        ...
        ***REMOVED*** Manually expire the idempotency record
        idem_record = (
            db.query(IdempotencyRequest)
            .filter(IdempotencyRequest.idempotency_key == idempotency_key)
            .first()
        ...
        ***REMOVED*** Set expiration to past
        idem_record.expires_at = datetime.utcnow() - timedelta(hours=1)
        db.commit()

        ***REMOVED*** Try to create another schedule with same key (should succeed after expiry)
        ***REMOVED*** Note: In production, you'd wait 24 hours. Here we manually expired it.
        ***REMOVED*** This simulates key reuse after natural expiration.

        ***REMOVED*** Modify data slightly to create different schedule
        schedule_data_2 = {
        ...
        ***REMOVED*** Should create a new schedule (idempotency key expired)
        assert r2.status_code in [200, 207]
        second_run_id = r2.json().get("run_id")

        ***REMOVED*** Should be different schedule runs
        assert first_run_id != second_run_id

        ***REMOVED*** Database should have 2 schedule runs
    ...
    def test_idempotency_cleanup_expired_records(self, db: Session):
        ...
        ***REMOVED*** Create 10 expired records
        for i in range(10):
        ...
        ***REMOVED*** Create 5 active records
        for i in range(5):
            request = IdempotencyRequest(
        ...
        ***REMOVED*** Should delete 10 expired records
        assert deleted_count == 10

        ***REMOVED*** Should have 5 active records remaining
        remaining = db.query(IdempotencyRequest).count()
        assert remaining == 5

        ***REMOVED*** All remaining should be active (not expired)
...
class TestIdempotencyServiceConcurrency:
    ...
    def test_create_request_race_condition(self, db: Session):
        ...
        def create_request():
            try:
                ...
                db.rollback()
                ***REMOVED*** Try to get existing
                existing = service.get_existing_request(idempotency_key, body_hash)
                return existing

        ***REMOVED*** Simulate concurrent creation attempts
        from concurrent.futures import ThreadPoolExecutor
        ...
        ***REMOVED*** All should succeed (either create or return existing)
        assert all(r is not None for r in results)

        ***REMOVED*** All should have same idempotency key
        assert all(r.idempotency_key == idempotency_key for r in results)

        ***REMOVED*** Database should have exactly 1 record
        records = (
            db.query(IdempotencyRequest)
            .filter(IdempotencyRequest.idempotency_key == idempotency_key)
            .all()
    ...
    def test_concurrent_mark_completed(self, db: Session):
        ...
        ***REMOVED*** Create a pending request
        request = service.create_request(
        ...
        ***REMOVED*** Concurrent completion attempts
        from concurrent.futures import ThreadPoolExecutor

        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(mark_complete) for _ in range(5)]
            for f in futures:
                f.result()  ***REMOVED*** Wait for all to complete

        ***REMOVED*** Refresh from database
        db.refresh(request)

        ***REMOVED*** Should be completed
...
class TestIdempotencyHeaderPropagation:
    ...
    def test_replayed_response_includes_header(
        ...
        ***REMOVED*** First request (original)
        r1 = client.post(
            "/api/schedule/generate",
            json=schedule_data,
            headers={"Idempotency-Key": idempotency_key},
        ...
        ***REMOVED*** Should NOT have replay header (or it should be "false")
        assert (
            "X-Idempotency-Replayed" not in r1.headers
            or r1.headers.get("X-Idempotency-Replayed") == "false"
        )

        ***REMOVED*** Second request (replayed)
        r2 = client.post(
            "/api/schedule/generate",
            json=schedule_data,
            headers={"Idempotency-Key": idempotency_key},
        ...
        ***REMOVED*** Should return same run ID
