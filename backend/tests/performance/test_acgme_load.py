...
Usage:
    ...
    ***REMOVED*** Run only performance-marked tests
    pytest -m performance

    ***REMOVED*** Run with verbose timing output
...
***REMOVED*** ============================================================================
***REMOVED*** ACGME Validation Performance Tests
***REMOVED*** ============================================================================
...
class TestACGMEPerformance:
    ...
    def test_80_hour_rule_large_dataset(
        ...
        ***REMOVED*** Verify correctness (basic sanity checks)
    ...
    def test_1_in_7_rule_large_dataset(
        ...
        ***REMOVED*** Check for 1-in-7 violations specifically
        ...
        ***REMOVED*** Our test data should be mostly compliant (residents get Sundays off)
        ***REMOVED*** Allow some violations but not excessive
        assert len(one_in_seven_violations) < len(residents) * 0.1  ***REMOVED*** < 10% violation rate
    ...
    def test_supervision_ratio_validation_under_load(
        ...
        ***REMOVED*** Check supervision violations
        ...
        ***REMOVED*** With 30 faculty and 100 residents, we should have adequate supervision
        ***REMOVED*** on most blocks (faculty are assigned 3 per weekday block in fixture)
        ***REMOVED*** Some violations are expected but should be minority
        ...
        assert violation_rate < 0.2  ***REMOVED*** < 20% blocks with violations
    ...
    def test_full_validation_medium_dataset(
        ...
        ***REMOVED*** Create 50 residents
        residents = []
        for pgy in [1, 2, 3]:
            for i in range(17):  ***REMOVED*** ~17 residents per PGY level
                resident = Person(
                    id=uuid4(),
        ...
        ***REMOVED*** Create assignments
        for resident in residents:
            for block in four_week_blocks:
                if block.date.weekday() != 6:  ***REMOVED*** Skip Sundays
    ...
    def test_small_dataset_validation_speed(
        ...
        """
        ***REMOVED*** Create small dataset
        residents = []
        for pgy in [1, 2, 3]:
            for i in range(8):  ***REMOVED*** 8 residents per level = 24 total
                resident = Person(
                    id=uuid4(),
        ...
        ***REMOVED*** Create 2 weeks of blocks
        blocks = []
...
class TestConcurrentValidation:
    ...
    def test_concurrent_validation_no_degradation(
        ...
        ***REMOVED*** Run 10 concurrent validations
        ...
        ***REMOVED*** All validations should complete successfully
        assert len(results) == 10
        assert all(r["duration"] > 0 for r in results)

        ***REMOVED*** Check for consistency (all runs should find same number of violations)
        ...
        ***REMOVED*** Log individual timings
    ...
    def test_rapid_sequential_validation(
        ...
        ***REMOVED*** First run is typically slower (cold cache)
        ***REMOVED*** Subsequent runs should be similar or faster
        assert durations[0] <= MAX_VALIDATION_TIME_100_RESIDENTS

        ***REMOVED*** Later runs should not be significantly slower (no resource leaks)
        avg_later_runs = sum(durations[1:]) / len(durations[1:])
        assert avg_later_runs <= MAX_VALIDATION_TIME_100_RESIDENTS * 1.2  ***REMOVED*** Allow 20% overhead
...
***REMOVED*** ============================================================================
***REMOVED*** Memory and Scaling Tests
***REMOVED*** ============================================================================
...
class TestValidationMemoryEfficiency:
    ...
    def test_validation_memory_efficiency(
        ...
        ***REMOVED*** Should complete in reasonable time (3x the 4-week threshold)
        ...
        ***REMOVED*** Verify results are sensible
        assert result is not None
        assert result.statistics is not None
    ...
    def test_incremental_vs_full_validation(
        ...
        ***REMOVED*** Full 4-week validation
        ...
        ***REMOVED*** Incremental 1-week validation
        ...
        ***REMOVED*** Incremental should be faster
        ...
        ***REMOVED*** Should be at least 2x faster for 4x less data
        assert speedup >= 2.0, (
            f"Incremental validation not scaling properly: {speedup:.1f}x speedup"
        )

        ***REMOVED*** Both should complete successfully
        assert full_result is not None
        assert incr_result is not None


***REMOVED*** ============================================================================
***REMOVED*** Edge Cases and Stress Tests
***REMOVED*** ============================================================================
...
class TestValidationEdgeCases:
    ...
    def test_empty_schedule_validation(self, db: Session):
        ...
        ***REMOVED*** Create residents but no assignments
        for i in range(10):
            resident = Person(
                id=uuid4(),
        ...
        ***REMOVED*** Should be very fast
        assert metrics["duration"] < 0.5  ***REMOVED*** 500ms
        assert result is not None
    ...
    def test_sparse_assignment_validation(
        ...
        ***REMOVED*** Assign only 10% of possible assignments
        assignment_count = 0
        for i, resident in enumerate(residents):
            for j, block in enumerate(four_week_blocks):
                ***REMOVED*** Only assign every 10th combination
                if (i + j) % 10 == 0:
                    assignment = Assignment(
                        id=uuid4(),
                        block_id=block.id,
        ...
        ***REMOVED*** Should still be fast with sparse data
        assert_performance(
            metrics["duration"],
            MAX_VALIDATION_TIME_100_RESIDENTS,
        ...
        ***REMOVED*** Coverage should be low
        assert result.coverage_rate < 20.0  ***REMOVED*** < 20%
