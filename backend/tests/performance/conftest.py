...
***REMOVED*** ACGME validation should complete quickly even for large datasets
...
***REMOVED*** Concurrent validation should not degrade significantly
MAX_CONCURRENT_VALIDATION_TIME = 10.0  ***REMOVED*** seconds for 10 concurrent validations

***REMOVED*** Memory should remain reasonable (tracked via monitoring, not enforced here)
MAX_MEMORY_MB = 500  ***REMOVED*** MB per validation operation


***REMOVED*** ============================================================================
***REMOVED*** Performance Measurement Utilities
***REMOVED*** ============================================================================
...
***REMOVED*** ============================================================================
***REMOVED*** Large Dataset Fixtures
***REMOVED*** ============================================================================
...
def large_resident_dataset(db: Session) -> dict:
    ...
    ***REMOVED*** Realistic PGY distribution for a large program
    pgy_distribution = [
        (1, 40),  ***REMOVED*** PGY-1: 40 residents
        (2, 35),  ***REMOVED*** PGY-2: 35 residents
        (3, 25),  ***REMOVED*** PGY-3: 25 residents
    ]
    ...
    for pgy_level, count in pgy_distribution:
        for i in range(count):
            resident = Person(
                ...
                pgy_level=pgy_level,
                target_clinical_blocks=48,  ***REMOVED*** ~12 weeks clinical
            )
            db.add(resident)
    ...
    ***REMOVED*** Organize by PGY level for easy access
    return {
        "residents": residents,
...
def large_faculty_dataset(db: Session) -> list[Person]:
    ...
    for i in range(30):
        fac = Person(
            ...
            performs_procedures=(i % 3 == 0),  ***REMOVED*** 1/3 perform procedures
...
def large_assignment_dataset(
    ...
    ***REMOVED*** Assign each resident to most blocks (simulating full clinical schedule)
    ***REMOVED*** Skip some weekend blocks to allow for 1-in-7 compliance
    for resident in residents:
        resident_block_count = 0
        consecutive_days = 0

        for i, block in enumerate(four_week_blocks):
            ***REMOVED*** Implement 1-in-7 rule: give residents at least 1 day off per week
            ***REMOVED*** Skip blocks to create rest days
            day_of_week = block.date.weekday()

            ***REMOVED*** Give each resident Sunday off (day 6)
            if day_of_week == 6:
                consecutive_days = 0
                continue

            ***REMOVED*** Occasionally skip a Saturday to provide variety
    ...
    ***REMOVED*** Add faculty supervision for weekday blocks
    faculty_idx = 0
    for block in four_week_blocks:
        if not block.is_weekend:
            ***REMOVED*** Assign 2-3 faculty per weekday block for supervision
            for _ in range(3):
...
def huge_dataset(
    ...
    ***REMOVED*** Create 12 weeks (84 days = 168 blocks)
    for day_offset in range(84):
        ...
        for time_of_day in ["AM", "PM"]:
            block = Block(
                ...
                date=current_date,
                time_of_day=time_of_day,
                block_number=1 + (day_offset // 28),  ***REMOVED*** Rotate block numbers
                is_weekend=is_weekend,
                is_holiday=False,
            )
            db.add(block)
    ...
    ***REMOVED*** Create assignments (similar pattern to large_assignment_dataset)
    assignments = []
    residents = large_resident_dataset["residents"]

    for resident in residents:
        for block in blocks:
            ***REMOVED*** Skip Sundays
...
***REMOVED*** ============================================================================
***REMOVED*** Performance Assertion Helpers
***REMOVED*** ============================================================================
