...
***REMOVED*** ============================================================================
***REMOVED*** Test StabilityMetrics Dataclass
***REMOVED*** ============================================================================
...
def test_stability_metrics_is_stable():
    ...
    ***REMOVED*** Unstable due to high churn
    ...
    ***REMOVED*** Unstable due to violations
...
def test_stability_metrics_grading():
    ...
    ***REMOVED*** Grade A - excellent stability
    ...
    ***REMOVED*** Grade F - has violations
    ...
    ***REMOVED*** Grade C - moderate stability
...
***REMOVED*** ============================================================================
***REMOVED*** Test StabilityMetricsComputer
***REMOVED*** ============================================================================
...
def test_calculate_churn_rate_with_changes(mock_assignments):
    ...
    ***REMOVED*** Create modified version - remove one, add one new
    new_assignments = mock_assignments[1:].copy()  ***REMOVED*** Remove first
    ...
    assert result["changed_count"] == 2  ***REMOVED*** 1 removed + 1 added
...
def test_calculate_n1_vulnerability(mock_assignments):
    ...
    ***REMOVED*** Should be non-zero since we have assignments
...
def test_calculate_n1_vulnerability_single_point_of_failure():
    ...
    ***REMOVED*** One person covers a unique rotation
    person1 = UUID('00000000-0000-0000-0000-000000000001')
    person2 = UUID('00000000-0000-0000-0000-000000000002')
    rotation1 = UUID('10000000-0000-0000-0000-000000000001')  ***REMOVED*** Unique to person1
    rotation2 = UUID('10000000-0000-0000-0000-000000000002')  ***REMOVED*** Shared

    assignments = [
        MockAssignment(person1, uuid4(), rotation1),  ***REMOVED*** Unique coverage
        MockAssignment(person1, uuid4(), rotation1),
        MockAssignment(person1, uuid4(), rotation1),
        MockAssignment(person1, uuid4(), rotation2),  ***REMOVED*** Shared
        MockAssignment(person2, uuid4(), rotation2),  ***REMOVED*** Shared
    ]

    computer = StabilityMetricsComputer(None)
    vulnerability = computer._calculate_n1_vulnerability(assignments)

    ***REMOVED*** Should be high due to person1 covering unique rotation
...
def test_build_dependency_graph(mock_assignments):
    ...
    ***REMOVED*** Graph should be created (or mock if NetworkX not available)
    if HAS_NETWORKX:
        import networkx as nx
        assert isinstance(graph, nx.DiGraph)
    ***REMOVED*** If no NetworkX, mock graph is returned
...
***REMOVED*** ============================================================================
***REMOVED*** Integration/Edge Cases
***REMOVED*** ============================================================================
...
***REMOVED*** ============================================================================
***REMOVED*** Test New Implementations (SQLAlchemy-Continuum Integration)
***REMOVED*** ============================================================================
...
def test_get_previous_assignments_no_history(mock_db_session, mock_assignments):
    ...
    ***REMOVED*** Mock db.execute to return no results
    ...
    ***REMOVED*** Should return empty list when no history
    assert previous == []
...
def test_get_previous_assignments_empty_input(mock_db_session):
    ...
    ***REMOVED*** Should return empty list for empty input
    assert previous == []
...
def test_count_new_violations_no_assignments(mock_db_session):
    ...
    ***REMOVED*** Should return 0 for no assignments
    assert violations == 0
...
def test_count_new_violations_error_handling(mock_db_session, mock_assignments):
    ...
    ***REMOVED*** Mock db.query to raise an exception
    ...
    ***REMOVED*** Should return 0 on error (fall back gracefully)
    assert violations == 0

    ***REMOVED*** Restore original query
...
def test_days_since_major_change_no_history(mock_db_session):
    ...
    ***REMOVED*** Mock db.execute to return empty results
    class MockExecuteResult:
        def fetchall(self):
            return []
    ...
    ***REMOVED*** Should return 0 when no history
    assert days == 0
...
def test_version_history_integration_fallback(mock_db_session, mock_assignments):
    ...
    ***REMOVED*** Mock execute to raise an exception
    ...
    ***REMOVED*** Test _get_previous_assignments
    previous = computer._get_previous_assignments(mock_assignments, None, None)
    assert previous == []  ***REMOVED*** Should fall back to empty list

    ***REMOVED*** Test _days_since_major_change
    days = computer._days_since_major_change()
    assert days == 0  ***REMOVED*** Should fall back to 0
...
def test_compute_with_version_history_fallback(mock_db_session, mock_assignments):
    ...
    ***REMOVED*** Set up mock to simulate no version history
    class MockExecuteResult:
        def fetchone(self):
            return None
        def fetchall(self):
    ...
    ***REMOVED*** Should complete successfully even without version history
    assert metrics.total_assignments == len(mock_assignments)
    assert metrics.assignments_changed == 0  ***REMOVED*** No previous version to compare
    assert metrics.churn_rate == 0.0
