...
class TestErlangBFormula:
    ...
    def test_erlang_b_known_values(self):
        ...
        ***REMOVED*** Known value: A=1.0, c=2 -> B ≈ 0.200
        result = calc.erlang_b(offered_load=1.0, servers=2)
        assert abs(result - 0.200) < 0.05

        ***REMOVED*** Known value: A=5.0, c=8 -> B ≈ 0.070 (using recursive formula)
        result = calc.erlang_b(offered_load=5.0, servers=8)
        assert abs(result - 0.070) < 0.02

        ***REMOVED*** Test that result is reasonable (low blocking with excess capacity)
        result = calc.erlang_b(offered_load=10.0, servers=15)
        assert result < 0.15  ***REMOVED*** Should be relatively low
    ...
    def test_erlang_b_increasing_servers(self):
        ...
        assert prob_8 < 0.1  ***REMOVED*** Should be quite low with 8 servers
...
class TestErlangCFormula:
    ...
    def test_erlang_c_known_values(self):
        ...
        ***REMOVED*** Erlang C formulas vary by implementation
        ***REMOVED*** Just verify results are valid probabilities
    ...
    def test_erlang_c_increasing_servers(self):
        ...
        assert prob_10 < 0.05  ***REMOVED*** Should be very low with 10 servers
...
class TestWaitProbability:
    ...
    def test_calculate_wait_probability_basic(self):
        ...
        ***REMOVED*** 2.5 calls/hour, 30 min (0.5 hr) per call, 3 servers
        ***REMOVED*** Offered load = 2.5 * 0.5 = 1.25
        ...
        assert result < 0.2  ***REMOVED*** Should be relatively low with this config

    def test_calculate_wait_probability_high_load(self):
        """Test wait probability with high load."""
        calc = ErlangCCalculator()

        ***REMOVED*** High load scenario: 8 calls/hour, 0.5 hr per call, 5 servers
        ***REMOVED*** Offered load = 8 * 0.5 = 4.0
        result = calc.calculate_wait_probability(
            arrival_rate=8.0,
            service_time=0.5,
            servers=5,
        ...
        assert result > 0.3  ***REMOVED*** Should be high with tight capacity

    def test_calculate_wait_probability_excess_capacity(self):
        """Test wait probability with excess capacity."""
        calc = ErlangCCalculator()

        ***REMOVED*** Low load: 2 calls/hour, 0.5 hr per call, 10 servers
        ***REMOVED*** Offered load = 2 * 0.5 = 1.0
        result = calc.calculate_wait_probability(
            arrival_rate=2.0,
            service_time=0.5,
            servers=10,
        ...
        assert result < 0.01  ***REMOVED*** Should be very low with excess capacity
...
class TestAverageWaitTime:
    ...
    def test_calculate_avg_wait_time_basic(self):
        ...
        assert result >= 0.0
        assert result < 0.5  ***REMOVED*** Should be less than service time
    ...
    def test_calculate_avg_wait_time_increases_with_load(self):
        ...
        ***REMOVED*** High load (still stable)
...
class TestServiceLevel:
    ...
    def test_calculate_service_level_basic(self):
        ...
        ***REMOVED*** 2.5 calls/hour, 0.5 hr service, 3 servers, 0.25 hr (15 min) target
        ...
        assert 0.0 <= result <= 1.0
        assert result > 0.8  ***REMOVED*** Should achieve good service level

    def test_calculate_service_level_zero_target(self):
        """Test service level with zero target wait."""
        calc = ErlangCCalculator()

        ***REMOVED*** Zero target means immediate answer
        result = calc.calculate_service_level(
            arrival_rate=2.5,
            service_time=0.5,
            servers=3,
        ...
        ***REMOVED*** Service level with zero wait = 1 - P(wait)
    ...
    def test_calculate_service_level_long_target(self):
        ...
        ***REMOVED*** Very long target should approach 100%
        result = calc.calculate_service_level(
            arrival_rate=2.5,
            service_time=0.5,
            servers=3,
            target_wait=10.0,  ***REMOVED*** 10 hours!
        )

        assert result > 0.99  ***REMOVED*** Nearly 100%
...
class TestOccupancy:
    ...
    def test_calculate_occupancy_basic(self):
        ...
        ***REMOVED*** Offered load = 2.5 * 0.5 = 1.25, 3 servers
        ***REMOVED*** Occupancy = 1.25 / 3 = 0.417
    ...
    def test_calculate_occupancy_zero_servers(self):
        ...
        assert result == 1.0  ***REMOVED*** Capped at 100%

    def test_calculate_occupancy_high_load(self):
        """Test occupancy with high load."""
        calc = ErlangCCalculator()

        ***REMOVED*** Offered load = 5.0 * 0.5 = 2.5, 3 servers
        ***REMOVED*** Occupancy = 2.5 / 3 = 0.833
        result = calc.calculate_occupancy(
            arrival_rate=5.0,
            service_time=0.5,
            servers=3,
    ...
    def test_calculate_occupancy_capped(self):
        ...
        ***REMOVED*** Over-capacity (would be > 1.0)
        result = calc.calculate_occupancy(
            arrival_rate=10.0,
            service_time=0.5,
            servers=3,
...
class TestOptimizeSpecialistCoverage:
    ...
    def test_optimize_basic(self):
        ...
        assert coverage.required_specialists >= 2  ***REMOVED*** At minimum
    ...
    def test_optimize_high_load(self):
        ...
        ***REMOVED*** High load (10 Erlangs) should require many specialists
    ...
    def test_optimize_low_load(self):
        ...
        ***REMOVED*** Low load should require few specialists
    ...
    def test_optimize_strict_target(self):
        ...
        coverage = calc.optimize_specialist_coverage(
            ...
            arrival_rate=5.0,
            service_time=0.5,
            target_wait_prob=0.01,  ***REMOVED*** Very strict: 1% wait probability
        )

        assert coverage.predicted_wait_probability <= 0.01
        ***REMOVED*** Stricter target should require more specialists
    ...
    def test_optimize_impossible_target(self):
        ...
        with pytest.raises(ValueError, match="Cannot meet target"):
            calc.optimize_specialist_coverage(
                specialty="Test",
                arrival_rate=50.0,  ***REMOVED*** Very high load
                service_time=1.0,
                target_wait_prob=0.001,  ***REMOVED*** Very strict
                max_servers=10,  ***REMOVED*** Low limit
            )
...
class TestCalculateMetrics:
    ...
    def test_calculate_metrics_default_target(self):
        ...
        ***REMOVED*** Default target should be half of service time
        assert isinstance(metrics, ErlangMetrics)
    ...
    def test_calculate_metrics_consistency(self):
        ...
        ***REMOVED*** Occupancy should match offered_load / servers
        expected_occupancy = (5.0 * 0.5) / 8
        assert abs(metrics.occupancy - expected_occupancy) < 0.01

        ***REMOVED*** Service level should be >= (1 - wait_probability)
...
class TestGenerateStaffingTable:
    ...
    def test_generate_staffing_table_range(self):
        ...
        assert len(table) <= 6  ***REMOVED*** Max 6 rows (5-10 inclusive)
    ...
    def test_generate_staffing_table_decreasing_wait(self):
        ...
        ***REMOVED*** Each row should have lower or equal wait probability than previous
    ...
    def test_generate_staffing_table_increasing_service_level(self):
        ...
        ***REMOVED*** Each row should have higher or equal service level than previous
        for i in range(1, len(table)):
...
class TestEdgeCases:
    ...
    def test_very_large_server_count(self):
        ...
        metrics = calc.calculate_metrics(
            arrival_rate=5.0,
            service_time=0.5,
            servers=100,  ***REMOVED*** Way more than needed
        )

        assert metrics.wait_probability < 0.0001  ***REMOVED*** Nearly zero
        assert metrics.service_level > 0.9999  ***REMOVED*** Nearly 100%

    def test_boundary_stable_queue(self):
        """Test queue at stability boundary."""
        calc = ErlangCCalculator()

        ***REMOVED*** Just under stability limit
        metrics = calc.calculate_metrics(
            arrival_rate=4.99,
            service_time=0.5,
            servers=3,  ***REMOVED*** Offered load = 2.495, just under 2.5
        )

        assert metrics.wait_probability > 0.0  ***REMOVED*** Should still calculate

    def test_negative_values_handling(self):
        """Test that negative values are handled appropriately."""
        calc = ErlangCCalculator()

        ***REMOVED*** Negative arrival rate should give zero load
        result = calc.erlang_b(offered_load=-1.0, servers=5)
        assert result == 0.0

        ***REMOVED*** Negative servers should give blocking
...
class TestRealWorldScenarios:
    ...
    def test_er_orthopedic_coverage(self):
        ...
        ***REMOVED*** Scenario: ER needs orthopedic consults
        ***REMOVED*** Average 1.5 consults/hour, 2 hours per consult
        coverage = calc.optimize_specialist_coverage(
            specialty="Orthopedic Surgery",
            arrival_rate=1.5,
            service_time=2.0,
    ...
    def test_cardiology_call_schedule(self):
        ...
        ***REMOVED*** Scenario: Cardiology call coverage
        ***REMOVED*** Average 0.8 calls/hour, 1.5 hours per case
        coverage = calc.optimize_specialist_coverage(
            specialty="Cardiology",
            arrival_rate=0.8,
            service_time=1.5,
            target_wait_prob=0.05,  ***REMOVED*** 5% max wait
        )
    ...
    def test_procedure_specialist_coverage(self):
        ...
        ***REMOVED*** Scenario: Emergent procedure coverage
        ***REMOVED*** Average 2.0 cases/hour during peak, 0.75 hour per procedure
        coverage = calc.optimize_specialist_coverage(
    ...
    def test_high_volume_clinic(self):
        ...
        ***REMOVED*** Scenario: High-volume urgent care
        ***REMOVED*** 12 patients/hour, 20 minutes (0.33 hr) per patient
        coverage = calc.optimize_specialist_coverage(
    ...
    def test_rare_specialty_coverage(self):
        ...
        ***REMOVED*** Scenario: Rare specialty consultation
        ***REMOVED*** Average 0.2 consults/hour, 3 hours per case
        coverage = calc.optimize_specialist_coverage(
        ...
        ***REMOVED*** Low volume should still require at least 1-2 specialists
        assert coverage.required_specialists >= 1
        ***REMOVED*** Offered load = 0.2 * 3.0 = 0.6 (allow floating point tolerance)
