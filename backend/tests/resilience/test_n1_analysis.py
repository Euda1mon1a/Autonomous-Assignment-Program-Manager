...
class TestN1Analyzer:
    ...
    def test_analyze_person_with_backup(self):
        ...
        assert scenario.criticality_score < 0.7  ***REMOVED*** Low criticality with backup

    def test_analyze_person_without_backup(self):
        """Test analyzing person without backup (SPOF)."""
        analyzer = N1Analyzer()

        assigned_slots = [(date(2024, 1, i), "clinic") for i in range(1, 11)]  ***REMOVED*** 10 slots
        ...
        assert scenario.criticality_score >= 0.5  ***REMOVED*** High criticality without backup
    ...
    def test_analyze_specialty_single_specialist(self):
        ...
        scenario = analyzer.analyze_specialty_failure(
            ...
            required_slots=20,
            available_specialists=["specialist1"],  ***REMOVED*** Only one!
            cross_trained=[],
        )

        assert scenario.component_type == "specialty"
        assert scenario.criticality_score >= 0.8  ***REMOVED*** Very high criticality
        assert not scenario.backup_available
    ...
    def test_analyze_specialty_multiple_specialists(self):
        ...
        assert scenario.backup_available
        assert scenario.criticality_score < 0.5  ***REMOVED*** Lower criticality with redundancy

    def test_find_single_points_of_failure(self):
        """Test finding SPOFs."""
        analyzer = N1Analyzer()

        ***REMOVED*** Create several scenarios
        ...
        analyzer.analyze_specialty_failure(
            ...
            cross_trained=[],  ***REMOVED*** SPOF
        )

        spofs = analyzer.find_single_points_of_failure(min_criticality=0.7)

        assert len(spofs) >= 2  ***REMOVED*** At least 2 SPOFs identified
    ...
    def test_calculate_redundancy_score(self):
        ...
        ***REMOVED*** High redundancy
        score_high = analyzer.calculate_redundancy_score(
            person_id="person1",
            assignments=[(date(2024, 1, i), "clinic") for i in range(1, 6)],  ***REMOVED*** 5 assignments
            available_backups=3,
        )

        ***REMOVED*** Low redundancy
        score_low = analyzer.calculate_redundancy_score(
            person_id="person2",
            assignments=[(date(2024, 1, i), "clinic") for i in range(1, 21)],  ***REMOVED*** 20 assignments
            available_backups=1,
        )
