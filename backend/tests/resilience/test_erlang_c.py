...
class TestErlangC:
    ...
    def test_stable_system_low_utilization(self):
        ...
        result = erlang.calculate(
            arrival_rate=2.0,  ***REMOVED*** 2 requests/hour
            service_rate=1.0,  ***REMOVED*** 1 per hour per server
            num_servers=5,  ***REMOVED*** 5 servers
        )

        ***REMOVED*** Utilization = λ/(c*μ) = 2/(5*1) = 0.4
        assert result.utilization == pytest.approx(0.4, abs=0.01)
        assert result.utilization < 1.0  ***REMOVED*** Stable
        assert result.prob_wait < 1.0
        assert result.avg_queue_length < 10  ***REMOVED*** Reasonable queue

    def test_high_utilization_increases_wait(self):
        """Test that high utilization increases wait time."""
        erlang = ErlangC()

        ***REMOVED*** Low utilization
        result_low = erlang.calculate(
            arrival_rate=4.0,
            service_rate=1.0,
            num_servers=10,  ***REMOVED*** ρ = 0.4
        )

        ***REMOVED*** High utilization
        result_high = erlang.calculate(
            arrival_rate=9.0,
            service_rate=1.0,
            num_servers=10,  ***REMOVED*** ρ = 0.9
        )
    ...
    def test_unstable_system(self):
        ...
        result = erlang.calculate(
            arrival_rate=10.0,
            service_rate=1.0,
            num_servers=10,  ***REMOVED*** ρ = 1.0 (exactly unstable)
        )
    ...
    def test_calculate_required_servers(self):
        ...
        ***REMOVED*** Should need at least 10 servers (8/(1*0.8) = 10)
        assert required >= 10

        ***REMOVED*** Verify the result actually meets targets
    ...
    def test_service_level_calculation(self):
        ...
        result = erlang.calculate(
            ...
            service_rate=1.0,
            num_servers=10,
            target_wait_time=0.5,  ***REMOVED*** 30 minutes
        )
    ...
    def test_compare_scenarios(self):
        ...
        ***REMOVED*** Check that more servers = lower utilization
    ...
    def test_scale_effect(self):
        ...
        ***REMOVED*** Adding servers should improve metrics
    ...
    def test_find_optimal_servers(self):
        ...
        optimal, cost = erlang.find_optimal_servers(
            arrival_rate=10.0,
            service_rate=1.0,
            cost_per_server=100.0,  ***REMOVED*** $100/hour per server
            cost_per_wait_hour=50.0,  ***REMOVED*** $50/hour customer wait cost
            max_servers=25,
        )

        assert optimal >= 11  ***REMOVED*** At least enough for stability
        assert cost > 0
    ...
    def test_littles_law(self):
        ...
        ***REMOVED*** L_q = λ * W_q (Little's Law)
    ...
    def test_80_percent_utilization_threshold(self):
        ...
        ***REMOVED*** Calculate queue length at different utilizations
        util_70 = erlang.calculate(7.0, 1.0, 10)  ***REMOVED*** 70%
        util_80 = erlang.calculate(8.0, 1.0, 10)  ***REMOVED*** 80%
        util_90 = erlang.calculate(9.0, 1.0, 10)  ***REMOVED*** 90%

        ***REMOVED*** Queue growth accelerates after 80%
        ...
        ***REMOVED*** Growth from 80-90% should be much larger than 70-80%
