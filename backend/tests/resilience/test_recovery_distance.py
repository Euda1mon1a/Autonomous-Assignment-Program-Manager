...
def block_ids(sample_date):
    ...
    return [uuid.uuid4() for _ in range(14)]  ***REMOVED*** 7 days × 2 blocks
...
def mock_assignments(faculty_ids, resident_ids, block_ids):
    ...
    ***REMOVED*** Assign faculty to blocks (round-robin)
    for i, bid in enumerate(block_ids):
...
***REMOVED*** =========================================================================
***REMOVED*** Test RecoveryDistanceCalculator - Basic Functionality
***REMOVED*** =========================================================================
...
def test_calculate_for_event_rd_zero(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Remove a faculty member who has minimal assignments
    ***REMOVED*** Since we have round-robin assignments, removing one faculty should
    ***REMOVED*** leave most blocks covered by others
    event = N1Event(
        event_type="faculty_absence",
        resource_id=faculty_ids[-1],  ***REMOVED*** Last faculty (least assignments in round-robin)
        affected_blocks=[block_ids[-1]],  ***REMOVED*** Only affects last block
    )

    ***REMOVED*** Modify schedule to have redundant coverage
    ...
    ***REMOVED*** Result depends on calculator's feasibility checking algorithm
    ***REMOVED*** Just verify valid result is returned
    assert result.recovery_distance >= 0
...
def test_calculate_for_event_needs_recovery(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Remove a faculty member with many assignments
    event = N1Event(
        event_type="faculty_absence",
        resource_id=faculty_ids[0],
        affected_blocks=block_ids[:3],  ***REMOVED*** Affects first 3 blocks
    )

    result = calculator.calculate_for_event(simple_schedule, event)

    ***REMOVED*** Should need some recovery (exact distance depends on search implementation)
    ***REMOVED*** For now, just verify it attempts recovery
...
def test_calculate_for_event_caching(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Second call (should be cached)
...
***REMOVED*** =========================================================================
***REMOVED*** Test Aggregate Metrics
***REMOVED*** =========================================================================
...
def test_calculate_aggregate_single_event(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Mean/median may be int or float depending on implementation
...
def test_aggregate_metrics_by_event_type(calculator, simple_schedule, faculty_ids, resident_ids, block_ids):
    ...
    ***REMOVED*** Event type breakdown depends on which events were actually processed
    ***REMOVED*** Just verify the structure is correct
...
def test_aggregate_breakglass_count():
    ...
    ***REMOVED*** Manually create results with known RD values
...
def test_generate_test_events_faculty_absence(calculator, simple_schedule, faculty_ids):
    ...
    ***REMOVED*** Should have one event per faculty with assignments
...
def test_generate_test_events_resident_sick(calculator, simple_schedule, resident_ids):
    ...
    ***REMOVED*** Add some resident assignments
    ...
    ***REMOVED*** Resident events may or may not be generated depending on algorithm
...
def test_infeasible_recovery_max_depth(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Create calculator with very limited depth
    limited_calc = RecoveryDistanceCalculator(max_depth=1, timeout_seconds=5.0)

    ***REMOVED*** Create event requiring multiple edits
    event = N1Event(
        event_type="faculty_absence",
        resource_id=faculty_ids[0],
        affected_blocks=block_ids[:5],  ***REMOVED*** Many blocks affected
    )

    result = limited_calc.calculate_for_event(simple_schedule, event)

    ***REMOVED*** Should respect max_depth
    assert result.search_depth_reached <= limited_calc.max_depth


def test_timeout_handling():
    """Test that search respects timeout."""
    calc = RecoveryDistanceCalculator(max_depth=10, timeout_seconds=0.001)  ***REMOVED*** Very short timeout

    ***REMOVED*** Create complex schedule
    schedule = {
        "assignments": [],
    ...
    ***REMOVED*** Should timeout gracefully
...
def test_double_booking_detection(calculator):
    ...
    ***REMOVED*** Create two assignments for same person on same block
    assignments = [
    ...
    ***REMOVED*** Should detect as infeasible
...
def test_minimum_coverage_check(calculator):
    ...
    ***REMOVED*** No assignments for this block
    assignments = []
    blocks = [type("Block", (), {"id": block_id, "date": date.today()})()]
    people = []
    ...
    ***REMOVED*** Should detect as infeasible (no coverage)
    is_feasible = calculator._is_feasible(assignments, blocks, people, schedule)
    assert is_feasible is False


***REMOVED*** =========================================================================
***REMOVED*** Test Integration Scenarios
***REMOVED*** =========================================================================
...
def test_realistic_faculty_absence_scenario(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Faculty member covers 3 blocks in a week
    event = N1Event(
        event_type="faculty_absence",
        resource_id=faculty_ids[0],
    ...
    ***REMOVED*** Should have a result
    assert isinstance(result, RecoveryResult)
    assert result.recovery_distance >= 0

    ***REMOVED*** If not feasible, should have reasonable explanation
...
def test_resilience_comparison():
    ...
    ***REMOVED*** Create two schedules with different resilience profiles
    ***REMOVED*** Schedule A: Brittle (no redundancy)
    schedule_a = {
        "assignments": [
...
def test_computation_time_tracking(calculator, simple_schedule, faculty_ids, block_ids):
    ...
    ***REMOVED*** Should have computation time recorded
...
def test_cache_performance():
    ...
    ***REMOVED*** Second call (cached)
    result2 = calc.calculate_for_event(schedule, event)
    time2 = result2.computation_time_ms

    ***REMOVED*** Cached call should be faster (or same if first was instant)
    assert time2 <= time1 or time1 < 1.0  ***REMOVED*** Allow for measurement noise
