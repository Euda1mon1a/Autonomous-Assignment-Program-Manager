...
class TestMetastabilityDetector:
    ...
    def test_detect_plateau_insufficient_data(self):
        ...
        trajectory = [100, 95, 90]  ***REMOVED*** Only 3 points

        is_plateau = detector.detect_plateau(trajectory)
        assert not is_plateau  ***REMOVED*** Not enough data
    ...
    def test_estimate_barrier_height_infeasible(self):
        ...
        ***REMOVED*** Infeasible solutions should have lower barrier
        assert barrier > 0
        assert barrier < 10  ***REMOVED*** Should be relatively low
    ...
    def test_estimate_barrier_height_feasible(self):
        ...
        state = SolverState(
            iteration=10,
            objective_value=100.0,
            constraint_violations=0,  ***REMOVED*** Feasible
        )

        barrier = detector.estimate_barrier_height(state)

        ***REMOVED*** Feasible solutions should have moderate barrier
        assert barrier > 0
    ...
    def test_recommend_strategy_continue_search(self):
        ...
        ***REMOVED*** High escape probability, low barrier
        state = MetastableState(
            objective_value=100.0,
            constraint_violations=0,
            barrier_height=2.0,
            basin_size=10,
            escape_probability=0.5,  ***REMOVED*** High
            plateau_duration=50,
            best_objective=95.0,
        )
    ...
    def test_recommend_strategy_increase_temperature(self):
        ...
        ***REMOVED*** Medium barrier, feasible
        state = MetastableState(
            objective_value=100.0,
            constraint_violations=0,
            barrier_height=4.0,
    ...
    def test_recommend_strategy_accept_optimum(self):
        ...
        ***REMOVED*** Very high barrier, feasible
        state = MetastableState(
            objective_value=100.0,
            constraint_violations=0,
            barrier_height=30.0,
    ...
    def test_recommend_strategy_infeasible_never_accept(self):
        ...
        ***REMOVED*** Very high barrier but infeasible
        state = MetastableState(
            objective_value=100.0,
            constraint_violations=3,  ***REMOVED*** Infeasible
            barrier_height=30.0,
            basin_size=50,
            escape_probability=0.0001,
            plateau_duration=200,
        ...
        strategy = detector.recommend_escape_strategy(state)
        ***REMOVED*** Should restart, not accept
        assert strategy in [
    ...
    def test_analyze_trajectory_improving(self):
        ...
        ***REMOVED*** Continuously improving trajectory
        ...
        assert not analysis.is_metastable
        assert not analysis.plateau_detected  ***REMOVED*** Still improving
        assert analysis.trajectory_length == 100
        assert analysis.best_iteration == 99  ***REMOVED*** Best at end
    ...
    def test_analyze_trajectory_metastable(self):
        ...
        ***REMOVED*** Improve then plateau
        improving = [100 - i for i in range(50)]
        plateau = [50.0] * 60  ***REMOVED*** Constant for 60 iterations
        trajectory = [
        ...
        assert analysis.is_metastable  ***REMOVED*** Feasible + plateau + stagnation
        assert analysis.metastable_state is not None
        assert analysis.best_iteration == 49  ***REMOVED*** Best at end of improvement
    ...
    def test_analyze_trajectory_infeasible_plateau_not_metastable(self):
        ...
        ***REMOVED*** Plateau detected, but not metastable due to infeasibility
        assert analysis.plateau_detected
        assert not analysis.is_metastable  ***REMOVED*** Must be feasible for metastability

    def test_estimate_basin_size(self):
        """Test basin size estimation."""
        detector = MetastabilityDetector()

        ***REMOVED*** Create trajectory with clustering around 100
        trajectory = []
        for i in range(100):
            if i < 50:
                ***REMOVED*** Early diversity
                obj = 150 - i
            else:
                ***REMOVED*** Cluster around 100 (within 1%)
        ...
        ***REMOVED*** Should find most recent 50 iterations clustered around 100
        assert basin_size > 40  ***REMOVED*** Most of the window

    def test_estimate_basin_size_small_basin(self):
        """Test basin size estimation with small basin."""
        detector = MetastabilityDetector()

        ***REMOVED*** Diverse trajectory, no clustering
        trajectory = [
        ...
        ***REMOVED*** No clustering - small basin
        assert basin_size < 5
...
class TestIntegrationFunctions:
    ...
    def test_check_metastability_not_at_interval(self):
        ...
        assert not should_intervene  ***REMOVED*** Not at interval
    ...
    def test_check_metastability_at_interval_not_metastable(self):
        ...
        ***REMOVED*** Improving trajectory
        ...
        assert not should_intervene  ***REMOVED*** Not metastable
    ...
    def test_check_metastability_at_interval_metastable(self):
        ...
        ***REMOVED*** Plateau trajectory
        ...
        assert should_intervene  ***REMOVED*** Metastable
        assert analysis is not None
...
class TestEdgeCases:
    ...
    def test_plateau_detection_single_value(self):
        ...
        is_plateau = detector.detect_plateau(trajectory)
        assert is_plateau  ***REMOVED*** All identical
    ...
    def test_escape_probability_zero_temperature(self):
        ...
        assert prob == 0.0  ***REMOVED*** Zero temperature = no escape

    def test_escape_probability_extreme_barrier(self):
        """Test escape probability with extreme barrier height."""
        detector = MetastabilityDetector()

        ***REMOVED*** Very high barrier should give near-zero probability
