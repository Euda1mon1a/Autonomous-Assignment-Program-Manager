...
class TestFeasibilitySurface:
    ...
    def test_surface_validation_shape_mismatch(self):
        ...
        demand_vals = np.linspace(0.5, 1.2, 10)
        strictness_vals = np.linspace(0.0, 1.0, 8)
        grid = np.random.rand(5, 5)  ***REMOVED*** Wrong shape
    ...
    def test_get_feasibility_in_bounds(self):
        ...
        demand_vals = np.linspace(0.5, 1.2, 10)
        strictness_vals = np.linspace(0.0, 1.0, 8)
        grid = np.ones((8, 10)) * 0.75  ***REMOVED*** Constant feasibility
        ...
        assert 0.7 <= feasibility <= 0.8  ***REMOVED*** Approximately 0.75
...
class TestCatastropheDetector:
    ...
    def test_initialization_custom_function(self):
        ...
        ***REMOVED*** Test that custom function is used
    ...
    def test_default_cusp_model(self):
        ...
        ***REMOVED*** Test in different regions
        ***REMOVED*** Low demand, low strictness -> should be feasible
        feas1 = detector.feasibility_function(0.6, 0.2)
        assert 0.0 <= feas1 <= 1.0

        ***REMOVED*** High demand, high strictness -> should be less feasible
    ...
    def test_detect_catastrophe_cusp_no_boundary(self):
        ...
        ***REMOVED*** Constant feasibility - no catastrophe
        def constant_func(demand: float, strictness: float) -> float:
            return 0.9  ***REMOVED*** Always feasible
    ...
    def test_compute_distance_to_catastrophe_far(self):
        ...
        ***REMOVED*** Mock cusp analysis with cusp at (0.9, 0.7)
        analysis = CuspAnalysis(
            cusp_exists=True,
            cusp_center=(0.9, 0.7),
            cusp_score=0.8,
    ...
    def test_compute_distance_to_catastrophe_near(self):
        ...
        ***REMOVED*** Current params near cusp
        params = ParameterState(demand=0.86, strictness=0.61)

        distance = detector.compute_distance_to_catastrophe(params, analysis)

        ***REMOVED*** Should be small distance
        assert distance < 0.3
    ...
    def test_compute_distance_no_cusp(self):
        ...
        ***REMOVED*** Should return max distance when no cusp
        assert distance == 1.0
    ...
    def test_measure_hysteresis_gap_with_points(self):
        ...
        ***REMOVED*** Create boundary points with clear hysteresis
        boundary_points = [
            (0.7, 0.5),
            (0.8, 0.5),
            (0.9, 0.5),  ***REMOVED*** Same strictness, different demands
            (0.7, 0.6),
            (0.85, 0.6),
        ]
    ...
    def test_predict_system_failure_toward_cusp(self):
        ...
        ***REMOVED*** Trajectory moving toward cusp
        trajectory = [
            ParameterState(0.7, 0.5),
            ParameterState(0.75, 0.55),
            ParameterState(0.80, 0.60),
            ParameterState(0.85, 0.65),  ***REMOVED*** Getting closer to (0.9, 0.7)
        ]
    ...
    def test_predict_system_failure_away_from_cusp(self):
        ...
        ***REMOVED*** Trajectory moving away from cusp
        trajectory = [
            ParameterState(0.85, 0.65),
            ParameterState(0.80, 0.60),
            ParameterState(0.75, 0.55),
            ParameterState(0.70, 0.50),  ***REMOVED*** Moving away from (0.9, 0.7)
        ]
    ...
    def test_predict_system_failure_critical_proximity(self):
        ...
        ***REMOVED*** Very close to cusp
        trajectory = [
            ParameterState(0.84, 0.64),
            ParameterState(0.85, 0.65),  ***REMOVED*** Almost at cusp
        ]

        prediction = detector.predict_system_failure(trajectory, analysis)

        ***REMOVED*** Should predict failure due to proximity
        assert prediction.will_fail is True
    ...
    def test_find_critical_thresholds(self):
        ...
        ***REMOVED*** All should be valid numbers
    ...
    def test_create_alert_safe_region(self):
        ...
        params = ParameterState(0.6, 0.3)  ***REMOVED*** Far from cusp
        distance = 0.8  ***REMOVED*** Large distance

        alert = detector.create_alert(params, analysis, distance)

        assert alert is None  ***REMOVED*** No alert in safe region
    ...
    def test_create_alert_warning_region(self):
        ...
        distance = 0.25  ***REMOVED*** Warning distance
    ...
    def test_create_alert_critical_region(self):
        ...
        distance = 0.08  ***REMOVED*** Critical distance
...
class TestIntegrationScenarios:
    ...
    def test_complete_workflow(self):
        ...
        ***REMOVED*** 1. Initialize detector
        detector = CatastropheDetector()

        ***REMOVED*** 2. Map parameter space
        surface = detector.map_constraint_space(resolution=(15, 15))
        assert surface is not None

        ***REMOVED*** 3. Detect cusp
        analysis = detector.detect_catastrophe_cusp(surface)
        assert analysis is not None

        ***REMOVED*** 4. Create trajectory
        trajectory = [
    ...
    def test_scenario_approaching_catastrophe(self):
        ...
        ***REMOVED*** Create surface with known cusp
        ...
        if analysis.cusp_exists and analysis.cusp_center:
            ...
            ***REMOVED*** Create trajectory approaching cusp
            trajectory = [
            ...
            ***REMOVED*** Should detect movement toward cusp
            assert prediction.trajectory_direction in [
                "toward_cusp",
                "parallel",
            ]
    ...
    def test_scenario_stable_system(self):
        ...
        ***REMOVED*** Stable trajectory in safe region
        trajectory = [
        ...
        ***REMOVED*** Should not predict failure
        assert prediction.will_fail is False
