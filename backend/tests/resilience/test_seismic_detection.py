...
class TestBurnoutEarlyWarning:
    ...
    def test_classic_sta_lta_basic(self):
        ...
        ***REMOVED*** Create signal with sudden increase
        data = np.concatenate(
            [
                np.ones(30),  ***REMOVED*** Baseline
                np.ones(10) * 5,  ***REMOVED*** Sudden increase
            ]
        )

        sta_lta = BurnoutEarlyWarning.classic_sta_lta(data, nsta=5, nlta=30)

        ***REMOVED*** Should be close to 1.0 during baseline
        assert sta_lta[29] < 1.5

        ***REMOVED*** Should increase significantly after spike
    ...
    def test_classic_sta_lta_short_data(self):
        ...
        ***REMOVED*** Should return zeros for too-short data
    ...
    def test_classic_sta_lta_zero_data(self):
        ...
        ***REMOVED*** Should return zeros when LTA is zero
        assert np.allclose(sta_lta, 0.0)
    ...
    def test_recursive_sta_lta_basic(self):
        ...
        ***REMOVED*** Should increase after spike
    ...
    def test_recursive_vs_classic_sta_lta(self):
        ...
        ***REMOVED*** Create realistic signal
        np.random.seed(42)
        ...
        ***REMOVED*** Both should detect the spike (ratio > 1.5 after spike)
        assert classic[35] > 1.5
        assert recursive[35] > 1.5

    def test_trigger_onset_basic(self):
        """Test trigger onset detection."""
        ***REMOVED*** Create STA/LTA with clear trigger
        sta_lta = np.array(
            [
                1.0,
                1.0,
                1.0,  ***REMOVED*** Below threshold
                3.0,
                4.0,
                3.5,  ***REMOVED*** Above threshold
                0.8,
                0.8,
                0.8,  ***REMOVED*** Back below
            ]
        )
        ...
        assert triggers[0][0] == 3  ***REMOVED*** Start at index 3
        assert triggers[0][1] == 6  ***REMOVED*** End at index 6
    ...
    def test_trigger_onset_multiple_triggers(self):
        ...
        sta_lta = np.array(
            [
                ...
                3.0,
                3.0,
                0.5,  ***REMOVED*** First trigger
                1.0,
                1.0,
                4.0,
                4.0,
                0.5,  ***REMOVED*** Second trigger
            ]
        )
    ...
    def test_trigger_onset_still_active(self):
        ...
        assert triggers[0][1] == 4  ***REMOVED*** End at last index
    ...
    def test_detect_precursors_basic(self):
        ...
        ***REMOVED*** Create realistic swap request pattern
        ***REMOVED*** Baseline: 0-1 per week, then increasing stress
        time_series = [0, 1, 0, 1, 0, 0, 1, 0, 1, 0] * 3 + [  ***REMOVED*** 30 days baseline
            1,
            2,
            2,
            3,
        ...
        ***REMOVED*** Should detect at least one alert
        assert len(alerts) > 0

        ***REMOVED*** Check alert properties
        alert = alerts[0]
        assert alert.signal_type == PrecursorSignal.SWAP_REQUESTS
        assert alert.resident_id == resident_id
    ...
    def test_detect_precursors_short_time_series(self):
        ...
        ***REMOVED*** Only 20 data points (less than long_window)
        ...
        ***REMOVED*** Should return empty list
        assert len(alerts) == 0
    ...
    def test_detect_precursors_no_anomaly(self):
        ...
        ***REMOVED*** Stable baseline, no spikes
        time_series = [1.0] * 50
        ...
        ***REMOVED*** Algorithm may or may not detect alerts depending on parameters
        ***REMOVED*** Just verify it returns a list
    ...
    def test_detect_precursors_severity_levels(self):
        ...
        ***REMOVED*** Should be high or critical severity due to extreme spike
        alert = alerts[0]
    ...
    def test_predict_burnout_magnitude_multiple_signals(self):
        ...
        ***REMOVED*** Multiple signals should give higher confidence
    ...
    def test_predict_burnout_magnitude_short_signals(self):
        ...
        signals = {
            PrecursorSignal.SWAP_REQUESTS: [1, 2, 3],  ***REMOVED*** Too short
            PrecursorSignal.SICK_CALLS: [1, 2],  ***REMOVED*** Too short
        }
    ...
    def test_predict_burnout_magnitude_bonus_for_multiple(self):
        ...
        ***REMOVED*** Same spike in all signals
        ...
        ***REMOVED*** Multiple signals should give higher magnitude
    ...
    def test_estimate_time_to_event_basic(self):
        ...
        ***REMOVED*** Current ratio of 5.0, growing at 1.0 per day
        ***REMOVED*** Critical threshold is 10.0
        ***REMOVED*** Should take ~5 days
    ...
    def test_estimate_time_to_event_already_critical(self):
        ...
        time_to_event = detector.estimate_time_to_event(
            sta_lta_ratio=12.0,  ***REMOVED*** Already above critical
            signal_growth_rate=1.0,
        )

        ***REMOVED*** Should return minimum (1 day)
    ...
    def test_estimate_time_to_event_negative_growth(self):
        ...
        time_to_event = detector.estimate_time_to_event(
            sta_lta_ratio=5.0,
            signal_growth_rate=-0.5,  ***REMOVED*** Improving
        )

        ***REMOVED*** Should return large value (1 year)
    ...
    def test_estimate_time_to_event_slow_growth(self):
        ...
        ***REMOVED*** Growth of 0.01 per day would take 500 days
        ***REMOVED*** Should be clamped to 365 days
        time_to_event = detector.estimate_time_to_event(
    ...
    def test_estimate_magnitude_signal_weights(self):
        ...
        ***REMOVED*** SICK_CALLS has higher weight (1.3) than RESPONSE_DELAYS (0.9)
        ...
        ***REMOVED*** Sick calls should yield higher magnitude for same ratio
    ...
    def test_estimate_magnitude_logarithmic_scale(self):
        ...
        ***REMOVED*** Doubling ratio should add ~1 to magnitude (log2 scale)
        ...
        ***REMOVED*** Should increase roughly linearly on log scale
...
class TestIntegration:
    ...
    def test_full_detection_workflow(self):
        ...
        ***REMOVED*** Simulate 60 days of data
        ***REMOVED*** Weeks 1-4: Normal (0-2 swap requests per week)
        baseline = [0, 1, 0, 1, 0, 2, 1] * 4

        ***REMOVED*** Weeks 5-6: Increasing stress (3-5 per week)
        increasing = [2, 3, 3, 4, 4, 5, 5] * 2

        ***REMOVED*** Weeks 7-8: High stress (6-8 per week)
        ...
        ***REMOVED*** Should detect anomaly
        assert len(alerts) > 0

        ***REMOVED*** Check that severity increases with ratio
        for alert in alerts:
    ...
    def test_multi_signal_magnitude_prediction(self):
        ...
        ***REMOVED*** Create correlated burnout signals
        baseline = [1.0] * 30
        stress = [5.0] * 15
        ...
        ***REMOVED*** Should predict some burnout risk (magnitude varies by algorithm)
        assert magnitude >= 0
        assert magnitude <= 10.0
    ...
    def test_early_vs_late_detection(self):
        ...
        ***REMOVED*** Gradual increase pattern
        ...
        if alerts:
            ***REMOVED*** First alert should trigger before end of series
            first_alert_context = alerts[0].context
            trigger_start = first_alert_context["trigger_start_idx"]

            ***REMOVED*** Should detect before peak stress
