...
class TestSamnPerelliScale:
    ...
    def test_duty_restrictions_by_level(self):
        ...
        ***REMOVED*** Level 3 (Okay) - should be safe for all duties
        ...
        ***REMOVED*** Level 5 (Moderately tired) - should restrict some duties
        ...
        ***REMOVED*** Level 7 (Exhausted) - should restrict most duties
    ...
    def test_is_safe_for_duty(self):
        ...
        ***REMOVED*** Level 3 should be safe for procedures
        safe, reason = is_safe_for_duty(SamnPerelliLevel.OKAY, "procedures")
        assert safe is True

        ***REMOVED*** Level 5 should not be safe for procedures
    ...
    def test_estimate_level_from_factors(self):
        ...
        ***REMOVED*** Well-rested scenario
        ...
        assert level.value <= 3  ***REMOVED*** Should be alert

        ***REMOVED*** Fatigued scenario
        level = estimate_level_from_factors(
            hours_awake=20.0,
            hours_worked_24h=14.0,
            consecutive_night_shifts=3,
            time_of_day_hour=4,  ***REMOVED*** Circadian nadir
            prior_sleep_hours=4.0,
        )
        assert level.value >= 5  ***REMOVED*** Should be tired
...
class TestSleepDebtModel:
    ...
    def test_calculate_daily_debt_adequate_sleep(self):
        ...
        assert debt < 0  ***REMOVED*** Net recovery (slept more than baseline)
    ...
    def test_calculate_daily_debt_inadequate_sleep(self):
        ...
        debt = model.calculate_daily_debt([sleep])
        assert debt > 0  ***REMOVED*** Net debt (slept less than baseline)
        assert debt == pytest.approx(2.5, rel=0.1)  ***REMOVED*** 7.5 - 5 = 2.5

    def test_effective_sleep_hours(self):
        """Test effective sleep calculation with quality factors."""
        now = datetime.now()

        ***REMOVED*** Perfect sleep
    ...
    def test_circadian_multiplier(self):
        ...
        ***REMOVED*** Nadir should have lowest multiplier
        nadir_time = datetime.now().replace(hour=4, minute=0)
        nadir_mult = model.get_circadian_multiplier(nadir_time)
        assert nadir_mult == 0.6

        ***REMOVED*** Peak should have highest multiplier
        peak_time = datetime.now().replace(hour=10, minute=0)
    ...
    def test_sleep_debt_trajectory(self):
        ...
        ***REMOVED*** Simulate poor sleep week
        ...
        ***REMOVED*** Debt should increase then decrease
        max_debt_day = max(trajectory, key=lambda t: t["cumulative_debt"])
        assert max_debt_day["day"] <= 5  ***REMOVED*** Peak before recovery days

    def test_impairment_equivalent_bac(self):
        """Test cognitive impairment BAC equivalence."""
        model = SleepDebtModel()

        ***REMOVED*** 10 hours debt should be ~0.05 BAC
...
class TestAlertnessPrediction:
    ...
    def test_predict_alertness_rested(self):
        ...
        resident_id = uuid4()
        now = datetime.now().replace(hour=10)  ***REMOVED*** Morning peak

        ***REMOVED*** Recent good sleep
        sleep = SleepOpportunity(
    ...
    def test_predict_alertness_fatigued(self):
        ...
        predictor = AlertnessPredictor()
        resident_id = uuid4()
        now = datetime.now().replace(hour=4)  ***REMOVED*** Circadian nadir

        ***REMOVED*** Long awake period
        sleep = SleepOpportunity(
    ...
    def test_shift_trajectory_prediction(self):
        ...
        ***REMOVED*** Alertness should decrease across night shifts
    ...
    def test_identify_high_risk_windows(self):
        ...
        ***REMOVED*** Create trajectory with varying alertness
        predictions = [
            AlertnessPrediction(
                resident_id=uuid4(),
...
class TestHazardThresholds:
    ...
    def test_hazard_level_info(self):
        ...
        assert len(levels) == 5  ***REMOVED*** GREEN through BLACK
    ...
    def test_acgme_risk_detection(self):
        ...
        ***REMOVED*** Approaching weekly limit
        hazard = engine.evaluate_hazard(
            resident_id=resident_id,
            alertness=0.7,
            hours_worked_week=75.0,  ***REMOVED*** 90%+ of 80-hour limit
        )
...
class TestFRMSService:
    ...
    def test_calculate_fatigue_score(self):
        ...
        assert result["samn_perelli_level"] <= 4  ***REMOVED*** Should be reasonably alert
    ...
    def test_export_temporal_constraints(self):
        ...
        ***REMOVED*** Verify ACGME integration
        assert export["acgme_integration"]["weekly_limit"] == 80.0

        ***REMOVED*** Verify scheduling constraints structure
...
class TestACGMEFRMSIntegration:
    ...
    def test_fatigue_predicts_acgme_risk(self):
        ...
        ***REMOVED*** High fatigue scenario
        hazard = engine.evaluate_hazard(
            resident_id=uuid4(),
            alertness=0.4,
            sleep_debt=15.0,
            hours_worked_week=70.0,  ***REMOVED*** Not yet at 80, but high fatigue
        )

        ***REMOVED*** Even before ACGME limit, fatigue indicates risk
    ...
    def test_frms_catches_violations_early(self):
        ...
        ***REMOVED*** Simulate resident approaching problems
        score = service.calculate_fatigue_score(
            hours_awake=18.0,
            hours_worked_24h=12.0,
            consecutive_night_shifts=4,
            time_of_day_hour=4,  ***REMOVED*** Nadir
            prior_sleep_hours=5.0,
        )

        ***REMOVED*** FRMS should flag this as concerning
    ...
    def test_circadian_constraint_validates_acgme(self):
        ...
        ***REMOVED*** Verify circadian constraints exist
...
class TestTemporalDynamics:
    ...
    def test_debt_severity_classification(self):
        ...
        ***REMOVED*** Test classification at different debt levels
    ...
    def test_recovery_time_estimation(self):
        ...
        ***REMOVED*** 10 hours debt with 9 hours sleep (1.5 hour extra)
        nights = model.estimate_recovery_time(10.0, recovery_sleep_per_night=9.0)

        ***REMOVED*** Should take ~10 nights (10 / (1.5 / 1.5))
        assert nights >= 8
        assert nights <= 15
