...
class TestSOCAvalanchePredictor:
    ...
    async def test_insufficient_data(self, predictor):
        ...
        ***REMOVED*** Only 10 data points (need 30 minimum)
        utilization = [0.75] * 10
    ...
    async def test_healthy_system(self, predictor):
        ...
        ***REMOVED*** Stable utilization around 60% with minor random noise
        np.random.seed(42)
        utilization = list(0.6 + np.random.normal(0, 0.02, 60))

        result = await predictor.detect_critical_slowing_down(utilization)

        ***REMOVED*** Warning level depends on algorithm sensitivity
        assert result.warning_level in [WarningLevel.GREEN, WarningLevel.YELLOW]
        assert not result.is_critical  ***REMOVED*** Handles np.bool_ and bool
    ...
    async def test_increasing_variance_signal(self, predictor):
        ...
        ***REMOVED*** Stable first half, increasing variance second half
        np.random.seed(42)
    ...
    async def test_high_autocorrelation_signal(self, predictor):
        ...
        ***REMOVED*** Create highly autocorrelated series (sluggish system)
        utilization = [0.7]
        for i in range(59):
            ***REMOVED*** Each value closely follows previous (AC1 will be high)
            utilization.append(utilization[-1] + np.random.normal(0, 0.01))

        result = await predictor.detect_critical_slowing_down(utilization)

        ***REMOVED*** Should detect high autocorrelation
        assert result.autocorrelation_ac1 is not None
        if result.autocorrelation_ac1 is not None:
            assert result.autocorrelation_ac1 > 0.5  ***REMOVED*** Should be quite high

    @pytest.mark.asyncio
    async def test_multiple_signals_critical(self, predictor):
        """Test critical detection with multiple signals."""
        ***REMOVED*** Create pathological series approaching criticality:
        ***REMOVED*** - High variance
        ***REMOVED*** - High autocorrelation
        ***REMOVED*** - Increasing trend
        np.random.seed(42)

        ***REMOVED*** Start stable
        base = [0.6 + np.random.normal(0, 0.02) for _ in range(20)]

        ***REMOVED*** Add increasing variance + autocorrelation
        critical = [base[-1]]
        for i in range(40):
            ***REMOVED*** High autocorrelation (follows previous)
            next_val = critical[-1] + np.random.normal(0, 0.05)
            ***REMOVED*** Increasing variance over time
        ...
        ***REMOVED*** Should trigger at least one warning signal
    ...
    async def test_recommendations_scale_with_severity(self, predictor):
        ...
        ***REMOVED*** Note: Creating guaranteed YELLOW/ORANGE/RED is tricky with real algorithms,
        ***REMOVED*** so we just verify that structure is correct
    ...
    async def test_days_lookback_parameter(self, predictor):
        ...
        ***REMOVED*** 90 days of data
        utilization = list(0.6 + np.random.normal(0, 0.01, 90))

        ***REMOVED*** Analyze only last 45 days
