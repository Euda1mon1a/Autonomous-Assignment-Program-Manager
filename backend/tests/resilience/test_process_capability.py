...
class TestScheduleCapabilityAnalyzer:
    ...
    def centered_data(self):
        ...
        return [
            62, 58, 65, 55, 68, 57, 63, 61, 59, 64,
            60, 62, 58, 66, 54, 67, 59, 61, 63, 58,
            64, 60, 57, 65, 62, 59, 61, 63, 58, 66,
            60, 62, 64, 58, 61, 59, 63, 65, 57, 60,
        ]
    ...
    def off_center_data(self):
        ...
        return [
            72, 68, 75, 65, 78, 67, 73, 71, 69, 74,
            70, 72, 68, 76, 64, 77, 69, 71, 73, 68,
            74, 70, 67, 75, 72, 69, 71, 73, 68, 76,
        ]
    ...
    def high_variation_data(self):
        ...
        return [
            45, 75, 50, 70, 48, 72, 52, 68, 55, 65,
            47, 73, 51, 69, 49, 71, 53, 67, 54, 66,
            46, 74, 50, 70, 48, 72, 52, 68, 56, 64,
        ]
...
class TestCpCalculation:
    ...
    def test_cp_perfect_process(self, analyzer):
        ...
        ***REMOVED*** If σ=1 and range=40, then Cp = 40/(6*1) = 6.67
        data = [60.0] * 10  ***REMOVED*** Will have σ close to 0, but need variation
        data = [59, 60, 61, 60, 60, 59, 60, 61, 60, 60]  ***REMOVED*** Small variation

        cp = analyzer.calculate_cp(data, lsl=40.0, usl=80.0)

        ***REMOVED*** With small variation, Cp should be very high
        assert cp > 2.0
    ...
    def test_cp_normal_process(self, analyzer, centered_data):
        ...
        ***REMOVED*** For well-controlled process, expect Cp > 1.0
        assert cp > 1.0
        assert cp < 5.0  ***REMOVED*** Reasonable upper bound
    ...
    def test_cp_high_variation(self, analyzer, high_variation_data):
        ...
        ***REMOVED*** High variation means lower Cp
        assert cp < 2.0

    def test_cp_formula_validation(self, analyzer):
        """Test Cp formula with known values."""
        ***REMOVED*** Data with known statistics
        ***REMOVED*** mean=60, σ=5, LSL=40, USL=80
        ***REMOVED*** Cp = (80-40)/(6*5) = 40/30 = 1.333
        data = [60 + x for x in [-5, -3, -1, 0, 1, 3, 5]]
        data.extend([60 + x for x in [-5, -3, -1, 0, 1, 3, 5]])  ***REMOVED*** Double it
    ...
    def test_cp_zero_variance(self, analyzer):
        ...
        ***REMOVED*** Zero variance → infinite capability (theoretically)
...
class TestCpkCalculation:
    ...
    def test_cpk_centered_process(self, analyzer, centered_data):
        ...
        ***REMOVED*** Well-centered process should have Cpk close to Cp
        cp = analyzer.calculate_cp(centered_data, lsl=40.0, usl=80.0)

        assert cpk > 0.9 * cp  ***REMOVED*** Cpk should be close to Cp
        assert cpk <= cp  ***REMOVED*** Cpk can never exceed Cp
    ...
    def test_cpk_zero_variance_inside_specs(self, analyzer):
        ...
        ***REMOVED*** Perfect centering, no variation
    ...
    def test_cpk_zero_variance_outside_specs(self, analyzer):
        ...
        data = [85.0] * 10  ***REMOVED*** Above USL

        cpk = analyzer.calculate_cpk(data, lsl=40.0, usl=80.0)

        ***REMOVED*** Outside specs with no variation → completely incapable
        assert cpk == 0.0
...
class TestPpAndPpkCalculation:
    ...
    def test_pp_equals_cp(self, analyzer, centered_data):
        ...
        ***REMOVED*** For our implementation, Pp = Cp
        assert pp == cp
    ...
    def test_ppk_equals_cpk(self, analyzer, centered_data):
        ...
        ***REMOVED*** For our implementation, Ppk = Cpk
        assert ppk == cpk
...
class TestCpmCalculation:
    ...
    def test_cpm_always_less_than_cp(self, analyzer):
        ...
        ***REMOVED*** Cpm penalizes off-target
        assert cpm <= cp
    ...
    def test_cpm_zero_variance_on_target(self, analyzer):
        ...
        ***REMOVED*** Perfect on target, no variation
...
class TestWorkloadCapabilityAnalysis:
    ...
    def test_analyze_default_target(self, analyzer, centered_data):
        ...
        ***REMOVED*** Target should default to midpoint
        assert report.target == 60.0
    ...
    def test_analyze_off_center_workload(self, analyzer, off_center_data):
        ...
        ***REMOVED*** Off-center: Cpk should be less than Cp
    ...
    def test_analyze_high_variation_workload(self, analyzer, high_variation_data):
        ...
        ***REMOVED*** High variation → lower capability
    ...
    def test_analyze_excellent_workload(self, analyzer):
        ...
        ***REMOVED*** Tightly controlled data
        data = [60 + x for x in range(-2, 3)] * 10  ***REMOVED*** 58-62 range
    ...
    def test_analyze_small_sample_warning(self, analyzer, caplog):
        ...
        small_data = [60, 65, 58, 62, 61]  ***REMOVED*** Only 5 samples
        ...
        ***REMOVED*** Should still produce report
        assert isinstance(report, ProcessCapabilityReport)

        ***REMOVED*** Should log warning about sample size
    ...
    def test_analyze_invalid_spec_limits(self, analyzer):
        ...
        with pytest.raises(ValueError, match="Invalid specification limits"):
            analyzer.analyze_workload_capability(
                data,
                min_hours=80.0,  ***REMOVED*** LSL > USL
                max_hours=40.0,
            )
...
class TestCapabilitySummary:
    ...
    def test_summary_indices_formatted(self, analyzer, sample_report):
        ...
        ***REMOVED*** Check formatting (3 decimal places)
...
class TestEdgeCases:
    ...
    def test_all_data_at_lsl(self, analyzer):
        ...
        ***REMOVED*** At spec limit, should still be technically capable
    ...
    def test_all_data_outside_usl(self, analyzer):
        ...
        ***REMOVED*** Should be incapable
        assert report.capability_status == "INCAPABLE"
        assert report.cpk == 0.0  ***REMOVED*** Zero variance outside spec

    def test_bimodal_distribution(self, analyzer):
        """Test with bimodal data distribution."""
        ***REMOVED*** Two clusters: one at 50, one at 70
        data = [50] * 15 + [70] * 15
    ...
    def test_negative_values(self, analyzer):
        ...
        ***REMOVED*** Should not crash, even if data is unusual
        report = analyzer.analyze_workload_capability(
            data,
            min_hours=0.0,
            max_hours=40.0,
    ...
    def test_very_narrow_spec_limits(self, analyzer):
        ...
        ***REMOVED*** Narrow specs with typical variation → low capability
