...
class TestCeleryTaskMetricsIntegration:
    ...
    def test_calculate_task_metrics_with_active_tasks(self, mock_celery_app, db_session):
        ...
        ***REMOVED*** Verify counts
        assert metrics.active_tasks == 3  ***REMOVED*** 2 + 1
        assert metrics.pending_tasks == 1  ***REMOVED*** scheduled
        assert metrics.total_tasks >= 4  ***REMOVED*** At least active + pending
    ...
    def test_calculate_task_metrics_with_redis_results(
        ...
        ***REMOVED*** Verify counts
        ...
        assert metrics.success_rate == 2 / 3  ***REMOVED*** 2 success out of 3 total
    ...
    def test_calculate_task_metrics_no_workers(self, mock_celery_app, db_session):
        ...
        ***REMOVED*** Should return safe defaults
        metrics = _calculate_task_metrics(db_session)
    ...
    def test_calculate_task_metrics_exception_handling(self, mock_celery_app, db_session):
        ...
        ***REMOVED*** Should return safe defaults without raising
        metrics = _calculate_task_metrics(db_session)
...
class TestCeleryRecentTasksIntegration:
    ...
    def test_get_recent_tasks_from_redis(
        ...
        ***REMOVED*** Mock inspect API (no active tasks)
        mock_inspect = MagicMock()
        mock_celery_app.control.inspect.return_value = mock_inspect
        mock_inspect.active.return_value = {}
    ...
    def test_get_recent_tasks_with_active_tasks(
        ...
        ***REMOVED*** Mock Redis (no stored results)
        mock_redis = MagicMock()
        mock_redis_class.from_url.return_value = mock_redis
        mock_redis.keys.return_value = []
        ...
        ***REMOVED*** Verify active task is included
        assert len(tasks) == 1
    ...
    def test_get_recent_tasks_limit(
        ...
        ***REMOVED*** Mock Redis with many tasks
        mock_redis = MagicMock()
        mock_redis_class.from_url.return_value = mock_redis

        ***REMOVED*** Create 20 task keys
        task_keys = [f"celery-task-meta-task-{i}".encode() for i in range(20)]
        mock_redis.keys.return_value = task_keys

        ***REMOVED*** Mock get to return valid task data
        def mock_get(key):
            return json.dumps({
                "status": "SUCCESS",
                "task_name": "test_task",
        ...
        ***REMOVED*** Get recent tasks with limit of 5
        tasks = _get_recent_tasks(db_session, limit=5)

        ***REMOVED*** Should return only 5 tasks
        assert len(tasks) == 5
    ...
    def test_get_recent_tasks_exception_handling(self, mock_celery_app, db_session):
        ...
        ***REMOVED*** Should return empty list without raising
        tasks = _get_recent_tasks(db_session, limit=10)
