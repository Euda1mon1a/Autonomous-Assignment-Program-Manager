...
class ResilienceScenarios:
    ...
    def create_n_minus_2_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create schedule with some buffer
        schedule = ScheduleFactory.create_complete_schedule(
            db, num_residents=8, num_faculty=4, num_days=7
        )

        ***REMOVED*** Remove two people (different emergencies)
        emergencies = []

        ***REMOVED*** Loss 1: Resident family emergency
        emergency1 = LeaveFactory.create_family_emergency(
            db, person=schedule["residents"][0]
        )
        emergencies.append(emergency1)

        ***REMOVED*** Loss 2: Faculty deployment
    ...
    def create_80_percent_utilization_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create schedule at exactly 80% capacity
        num_residents = 10
        num_faculty = 4

        ***REMOVED*** 80% utilization = 8 residents working, 2 buffer
        schedule = ScheduleFactory.create_complete_schedule(
            db, num_residents=num_residents, num_faculty=num_faculty, num_days=28
        )

        ***REMOVED*** Now push to 90% by adding leave for 1 resident (reduces buffer)
    ...
    def create_cascade_failure_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create tightly-coupled schedule
        schedule = ScheduleFactory.create_fmit_week_schedule(db, num_faculty=7)

        ***REMOVED*** Faculty A can't work -> Faculty B must cover double
        ***REMOVED*** Faculty B overworked -> ACGME violation -> Faculty C forced in
        ***REMOVED*** Faculty C cancels planned leave -> morale impact

        ***REMOVED*** Trigger: Faculty A deployment
        ...
        ***REMOVED*** Downstream: Faculty C had planned leave (now conflicted)
    ...
    def create_burnout_propagation_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create schedule with some residents overworked
        schedule = ScheduleFactory.create_complete_schedule(
            db, num_residents=10, num_faculty=3, num_days=28  ***REMOVED*** Understaffed
        )

        ***REMOVED*** Simulate burnout by creating consecutive weeks without breaks
    ...
    def create_blast_radius_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create two independent clinic zones
    ...
    def create_erlang_coverage_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create clinic schedule with specific coverage requirements
        ***REMOVED*** Assume 10 patients/hour arrival rate, 15 min service time

        ***REMOVED*** Optimal staffing from Erlang C would be ~3 residents
        ***REMOVED*** Test with 2 (understaffed) vs 3 (optimal) vs 4 (overstaffed)
        ...
        return {
            ...
            "arrival_rate": 10,  ***REMOVED*** patients/hour
            "service_time": 15,  ***REMOVED*** minutes
    ...
    def create_time_crystal_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create initial schedule
        initial = ScheduleFactory.create_complete_schedule(
            db, num_residents=8, num_faculty=4, num_days=28
        )

        ***REMOVED*** Simulate regeneration with minimal changes
        ***REMOVED*** (In practice, solver would try to preserve existing assignments)
    ...
    def create_recovery_distance_test(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create stable schedule
        stable = ScheduleFactory.create_complete_schedule(
            db, num_residents=8, num_faculty=4, num_days=7
        )

        ***REMOVED*** Remove one person
        lost_person = stable["residents"][0]
        emergency = LeaveFactory.create_family_emergency(db, person=lost_person)

        ***REMOVED*** Recovery = reassign lost_person's assignments to others
        ***REMOVED*** Count minimum swaps needed
