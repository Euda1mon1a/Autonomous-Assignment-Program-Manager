...
class CoverageGapScenarios:
    ...
    def create_overlapping_absences(
        ...
        ***REMOVED*** Create overlapping absences for first N residents
    ...
    def create_weekend_gap(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create minimal faculty (insufficient for 4 weekends)
    ...
    def create_deployment_coverage_gap(db: Session) -> dict:
        ...
        ***REMOVED*** Deploy one faculty member
    ...
    def create_flu_season_gap(db: Session) -> dict:
        ...
        ***REMOVED*** Create sick leave for 3 residents (same week)
        absences = []
        for resident in schedule["residents"][:3]:
            absence = LeaveFactory.create_medical_leave(
                db, person=resident, days=3  ***REMOVED*** Short but simultaneous
            )
    ...
    def create_fmit_understaffing(db: Session) -> dict:
        ...
        return ScheduleFactory.create_fmit_week_schedule(
            db, num_faculty=4  ***REMOVED*** Need 7 for daily rotation
        )
    ...
    def create_specialty_shortage(db: Session) -> dict:
        ...
        ***REMOVED*** Create residents and non-SM faculty
        ...
        ***REMOVED*** Create SM clinic template (requires SM specialty)
        sm_template = RotationFactory.create_sports_medicine_template(db)

        ***REMOVED*** Try to assign residents without SM faculty
        assignments = []
    ...
    def create_holiday_coverage_gap(db: Session) -> dict:
        ...
        ***REMOVED*** Create holiday blocks (Christmas week)
        year = date.today().year
        blocks = BlockFactory.create_holiday_blocks(db, year=year)

        ***REMOVED*** No assignments created = coverage gap
        return {
            "faculty": faculty,
            "blocks": blocks,
    ...
    def create_n_minus_1_failure(db: Session) -> dict:
        ...
        """
        ***REMOVED*** Create minimal staffing (exactly what's needed, no buffer)
        schedule = ScheduleFactory.create_complete_schedule(
            db, num_residents=4, num_faculty=1, num_days=7  ***REMOVED*** Minimal faculty
        )

        ***REMOVED*** Remove one resident (emergency absence)
    ...
    def create_maternity_leave_gap(db: Session) -> dict:
        ...
        """
        schedule = ScheduleFactory.create_complete_schedule(
            db, num_residents=6, num_faculty=3, num_days=84  ***REMOVED*** 12 weeks
        )

        ***REMOVED*** Create maternity leave for one resident
