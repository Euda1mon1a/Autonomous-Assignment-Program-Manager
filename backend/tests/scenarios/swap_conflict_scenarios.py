...
class SwapConflictScenarios:
    ...
    def create_conflicting_swap_requests(db: Session) -> dict:
        ...
        ***REMOVED*** Create conflicting swaps (same source, same week, different targets)
    ...
    def create_circular_swap_chain(db: Session) -> dict:
        ...
        ***REMOVED*** A gives week1, gets week2 from B
        ...
        ***REMOVED*** B gives week2, gets week3 from C
        ...
        ***REMOVED*** C gives week3, gets week1 from A
    ...
    def create_overlapping_absorb_requests(db: Session) -> dict:
        ...
        ***REMOVED*** Faculty 0 gives away week, multiple faculty want to absorb
        absorb_swaps = []
    ...
    def create_cascade_swap_failure(db: Session) -> dict:
        ...
        ***REMOVED*** This swap gets rejected (breaks the chain)
    ...
    def create_rollback_conflict(db: Session) -> dict:
        ...
        ***REMOVED*** Now rollback due to emergency
    ...
    def create_approval_deadlock(db: Session) -> dict:
        ...
        ***REMOVED*** B wants A's week (conflict)
    ...
    def create_equity_violation_swap(db: Session) -> dict:
        ...
        ***REMOVED*** Create faculty with already high Sunday call count
        overloaded_faculty = PersonFactory.create_faculty(db)
        overloaded_faculty.sunday_call_count = 6  ***REMOVED*** Already at max
        db.commit()

        target_faculty = PersonFactory.create_faculty(db)

        ***REMOVED*** Try to swap for another Sunday (would violate equity)
        ***REMOVED*** Source week = Sunday, target week = weekday
        source_week = date.today() + timedelta(days=(6 - date.today().weekday()))  ***REMOVED*** Next Sunday
        target_week = source_week + timedelta(days=2)  ***REMOVED*** Tuesday
