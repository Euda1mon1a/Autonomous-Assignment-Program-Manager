        ...
        ***REMOVED*** Arrange
        def traced_with_name(span_name):
            def decorator(func):
                @wraps(func)
                def wrapper(*args, **kwargs):
                    ***REMOVED*** Simulated: Create span with custom name
                    return func(*args, **kwargs)
                return wrapper
            return decorator
        ...
        ***REMOVED*** Arrange
        def traced_with_attrs(attributes):
            def decorator(func):
                @wraps(func)
                def wrapper(*args, **kwargs):
                    ***REMOVED*** Simulated: Add custom attributes to span
                    return func(*args, **kwargs)
                return wrapper
            return decorator
...
class TestDecoratorMetrics:
    ...
    def test_decorator_measures_execution_time(self):
        ...
        ***REMOVED*** Arrange
        ...
        @timed_trace
        def function():
            time.sleep(0.01)  ***REMOVED*** Sleep 10ms
            return "result"
        ...
        ***REMOVED*** Assert
        ...
        assert durations[0] >= 0.01  ***REMOVED*** Should take at least 10ms
