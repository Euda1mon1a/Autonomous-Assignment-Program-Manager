...
class TestBasicHealthEndpoint:
    ...
    def test_health_check_response_format(self, client: TestClient):
        ...
        data = response.json()
        ***REMOVED*** Ensure all expected fields are present
    ...
    def test_health_check_no_authentication_required(self, client: TestClient):
        ...
        ***REMOVED*** Call without auth headers
        response = client.get("/health")

        ***REMOVED*** Should succeed without auth
        assert response.status_code == 200
    ...
    def test_health_check_with_database(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Database should be reported as connected
        assert data["database"] == "connected"
        assert data["status"] == "healthy"

    def test_health_check_accepts_get_only(self, client: TestClient):
        """Test health endpoint only accepts GET requests."""
        ***REMOVED*** POST should not be allowed
        response = client.post("/health")
        assert response.status_code == 405

        ***REMOVED*** PUT should not be allowed
        response = client.put("/health")
        assert response.status_code == 405

        ***REMOVED*** DELETE should not be allowed
...
class TestRootEndpoint:
    ...
    def test_root_endpoint_as_health_check(self, client: TestClient):
        ...
        assert response.status_code == 200
        ***REMOVED*** A 200 response indicates the service is up
        assert response.json()["status"] == "healthy"
...
class TestResilienceHealthEndpoint:
    ...
    def test_resilience_health_components(self, client: TestClient):
        ...
        ***REMOVED*** Should include components
        assert "components" in data
        assert isinstance(data["components"], list)

        ***REMOVED*** Expected components
        expected_components = [
            "utilization_monitor",
            "defense_in_depth",
    ...
    def test_resilience_health_metrics_enabled_status(self, client: TestClient):
        ...
        ***REMOVED*** Should include metrics_enabled flag
    ...
    def test_resilience_health_degraded_on_error(
        ...
        ***REMOVED*** Mock an exception during metrics retrieval
        ...
        ***REMOVED*** Should report degraded status
...
class TestHealthCheckDatabaseConnectivity:
    ...
    def test_health_with_valid_database(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Should report database as connected
        assert data["database"] == "connected"
        assert data["status"] == "healthy"
    ...
    def test_health_database_field_type(self, client: TestClient):
        ...
        ***REMOVED*** database field should be a string
    ...
    def test_health_with_multiple_requests(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** All should report healthy status
...
class TestHealthCheckComponentStatus:
    ...
    def test_root_endpoint_reports_service_info(self, client: TestClient):
        ...
        ***REMOVED*** Should include service identification
        assert data["name"] == "Residency Scheduler API"
        assert data["version"] == "1.0.0"
        assert data["status"] == "healthy"
    ...
    def test_resilience_endpoint_reports_all_components(self, client: TestClient):
        ...
        ***REMOVED*** All 5 resilience components should be listed
        assert len(data["components"]) == 5

        ***REMOVED*** Verify each component is a string
    ...
    def test_health_check_json_serializable(self, client: TestClient):
        ...
        for endpoint in endpoints:
            ...
            ***REMOVED*** All values should be JSON-serializable types
...
class TestHealthCheckErrorScenarios:
    ...
    def test_health_endpoint_handles_exceptions_gracefully(self, client: TestClient):
        ...
        ***REMOVED*** Even if internal errors occur, health check should return a response
        response = client.get("/health")

        ***REMOVED*** Should not raise unhandled exceptions
    ...
    def test_resilience_health_error_handling(
        ...
        ***REMOVED*** Simulate metrics error
        mock_get_metrics.side_effect = RuntimeError("Metrics service down")

        response = client.get("/health/resilience")

        ***REMOVED*** Should return 200 with degraded status, not crash
        assert response.status_code == 200
        data = response.json()
    ...
    def test_health_check_timeout_resilience(self, client: TestClient):
        ...
        ***REMOVED*** Health check should be fast (< 1 second)
    ...
    def test_concurrent_health_checks(self, client: TestClient):
        ...
        ***REMOVED*** Make 10 concurrent requests
    ...
    def test_health_check_with_malformed_requests(self, client: TestClient):
        ...
        ***REMOVED*** Try with query parameters (should ignore them)
        response = client.get("/health?invalid=param")
        assert response.status_code == 200

        ***REMOVED*** Try with different content types
...
class TestHealthCheckIntegration:
    ...
    def test_health_check_consistency(self, client: TestClient):
        ...
        ***REMOVED*** Both should report healthy status
    ...
    def test_health_checks_with_database_operations(
        ...
        ***REMOVED*** Perform a database operation
        from app.models.person import Person

        people = db.query(Person).all()
        assert len(people) > 0

        ***REMOVED*** Health checks should still work
        response = client.get("/health")
        assert response.status_code == 200
        assert response.json()["status"] == "healthy"
    ...
    def test_health_checks_excluded_from_metrics(self, client: TestClient):
        ...
        ***REMOVED*** Make several health check requests
        for _ in range(5):
            client.get("/health")
            client.get("/")

        ***REMOVED*** Metrics endpoint should not count health checks
        ***REMOVED*** (This is configured in main.py with excluded_handlers)
        ***REMOVED*** Just verify metrics endpoint is accessible
        response = client.get("/metrics")
        ***REMOVED*** May return 200 or 403 depending on configuration
    ...
    def test_health_endpoints_performance_baseline(self, client: TestClient):
        ...
        ***REMOVED*** All endpoints should respond in under 500ms
    ...
    def test_health_check_response_headers(self, client: TestClient):
        ...
        for endpoint in endpoints:
            ...
            ***REMOVED*** Should have content-type header
            assert "content-type" in response.headers
            assert "application/json" in response.headers["content-type"]

            ***REMOVED*** Should not leak server information (security)
            assert (
...
class TestHealthCheckMonitoring:
    ...
    def test_health_check_provides_service_status(self, client: TestClient):
        ...
        ***REMOVED*** Status should be clear and actionable
    ...
    def test_root_endpoint_provides_version_info(self, client: TestClient):
        ...
        ***REMOVED*** Version info should be present for monitoring/debugging
        assert "version" in data
    ...
    def test_resilience_health_provides_operational_status(self, client: TestClient):
        ...
        ***REMOVED*** Should clearly indicate operational status
        assert data["status"] in ["operational", "degraded"]
    ...
    def test_health_checks_suitable_for_load_balancer(self, client: TestClient):
        ...
        ***REMOVED*** Load balancers typically use simple GET requests
        response = client.get("/health")

        ***REMOVED*** Should return 200 for healthy
        assert response.status_code == 200

        ***REMOVED*** Should be fast
        import time
        ...
        ***REMOVED*** Load balancer probes should be fast (< 100ms ideally)
        assert elapsed < 1.0  ***REMOVED*** Allow up to 1s for test environment
    ...
    def test_health_checks_suitable_for_kubernetes(self, client: TestClient):
        ...
        ***REMOVED*** Kubernetes liveness probe
        liveness_response = client.get("/health")
        assert liveness_response.status_code == 200

        ***REMOVED*** Kubernetes readiness probe
        readiness_response = client.get("/")
        assert readiness_response.status_code == 200

        ***REMOVED*** Both should have JSON responses
