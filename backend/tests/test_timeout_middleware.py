...
class TestTimeoutMiddleware:
    ...
    def test_excluded_paths_no_timeout(self, app):
        ...
        ***REMOVED*** Health endpoint should work even with very short timeout
        response = client.get("/health")

        assert response.status_code == 200
        ***REMOVED*** Excluded paths don't get timeout headers
    ...
    def test_timeout_headers_values(self, app):
        ...
        ***REMOVED*** Elapsed should be a small positive number
        elapsed = float(response.headers["X-Timeout-Elapsed"])
        assert 0 < elapsed < 1.0
        ***REMOVED*** Remaining should be close to 5.0
    ...
    def test_default_exclude_paths(self, app):
        ...
        ***REMOVED*** Health endpoint is excluded by default
        response = client.get("/health")
        assert response.status_code == 200
...
class TestTimeoutMiddlewareIntegration:
    ...
    def test_multiple_requests(self, app):
        ...
        ***REMOVED*** Quick request should succeed
        response1 = client.get("/quick")
        assert response1.status_code == 200

        ***REMOVED*** Slow request should timeout
        response2 = client.get("/slow")
        assert response2.status_code == 504

        ***REMOVED*** Another quick request should succeed
    ...
    def test_concurrent_requests(self, app):
        ...
        ***REMOVED*** Note: TestClient is synchronous, so we can't truly test concurrency
        ***REMOVED*** This is a basic check that multiple requests work
        client = TestClient(app)
