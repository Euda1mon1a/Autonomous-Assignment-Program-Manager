...
class TestCreateRotationTemplate:
    ...
    def test_create_template_minimal_fields(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Verify it was saved to database
    ...
    def test_create_template_created_at_timestamp(self, client: TestClient):
        ...
        data = response.json()
        assert "created_at" in data
        ***REMOVED*** Verify it's a valid ISO timestamp
...
class TestUpdateRotationTemplate:
    ...
    def test_update_template_empty_body(self, client: TestClient, sample_rotation_template: RotationTemplate):
        ...
        assert response.status_code == 200
        data = response.json()
        ***REMOVED*** Original values should remain
        assert data["name"] == sample_rotation_template.name
    ...
    def test_update_template_preserves_id_and_created_at(
        ...
        ***REMOVED*** created_at should not change
...
class TestDeleteRotationTemplate:
    ...
    def test_delete_template_success(self, client: TestClient, db: Session, sample_rotation_template: RotationTemplate):
        ...
        ***REMOVED*** Verify template was deleted from database
        db_template = db.query(RotationTemplate).filter(RotationTemplate.id == template_id).first()
    ...
    def test_delete_template_verify_list_updated(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Verify it's in the list
        response = client.get("/api/rotation-templates")
        assert response.status_code == 200
        ...
        ***REMOVED*** Verify it's gone from list
        response = client.get("/api/rotation-templates")
        assert response.status_code == 200
    ...
    def test_delete_template_verify_get_fails(self, client: TestClient, sample_rotation_template: RotationTemplate):
        ...
        ***REMOVED*** Verify we can get it initially
...
class TestRotationTemplateIntegration:
    ...
    def test_create_multiple_and_filter(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Filter by inpatient
        response = client.get("/api/rotation-templates?activity_type=inpatient")
        assert response.status_code == 200
        assert response.json()["total"] == 1

        ***REMOVED*** No filter (all)
        response = client.get("/api/rotation-templates")
        assert response.status_code == 200
