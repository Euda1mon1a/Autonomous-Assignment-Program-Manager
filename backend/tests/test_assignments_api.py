...
class TestAssignmentAuthenticationEnforcement:
    ...
    def test_get_assignment_requires_auth(self, client, sample_data, admin_user, admin_token):
        ...
        ***REMOVED*** Create an assignment as admin
        ...
        ***REMOVED*** Try to get without auth
    ...
    def test_update_assignment_requires_auth(self, client, sample_data, admin_user, admin_token):
        ...
        ***REMOVED*** Try to update without auth
        response = client.put(
    ...
    def test_delete_assignment_requires_auth(self, client, sample_data, admin_user):
        ...
        ***REMOVED*** Try to delete without auth
...
class TestAssignmentRoleBasedAccessControl:
    ...
    def test_faculty_can_list_assignments(self, client, sample_data, faculty_token, admin_token):
        ...
        ***REMOVED*** Create an assignment first
        client.post(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={
...
class TestACGMEValidationAtWriteTime:
    ...
    def test_create_assignment_returns_warnings(self, client, sample_data, admin_token, db):
        ...
        ***REMOVED*** Create multiple assignments for the same resident to trigger 80-hour rule
        blocks = sample_data["blocks"][:28]  ***REMOVED*** 4 weeks
        resident_id = str(sample_data["resident"].id)
        template_id = str(sample_data["template"].id)

        ***REMOVED*** Assign resident to many blocks (both AM and PM each day)
        for i, block in enumerate(blocks):
            ...
            ***REMOVED*** Only check last assignment for violations
            if i == len(blocks) - 1:
                assert response.status_code == 201
                data = response.json()
                assert "acgme_warnings" in data
                assert "is_compliant" in data
                ***REMOVED*** With 28 half-day blocks over 14 days, this should trigger violations
                ***REMOVED*** (28 * 6 hours = 168 hours / 2 weeks = 84 hours/week average)
    ...
    def test_create_assignment_with_override_reason(self, client, sample_data, admin_token, db):
        ...
        ***REMOVED*** Last one with override
        response = client.post(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={
        ...
        ***REMOVED*** If there are warnings, check that override is acknowledged
        if data.get("acgme_warnings"):
            assert any("Override acknowledged" in w for w in data["acgme_warnings"])
            ***REMOVED*** Check that it was saved to notes
...
class TestOptimisticLocking:
    ...
    def test_update_with_stale_timestamp_fails(self, client, sample_data, admin_token):
        ...
        ***REMOVED*** Try to update with stale timestamp
        response = client.put(
            f"/api/assignments/{assignment_id}",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={
                "role": "supervising",
                "updated_at": original_updated_at,  ***REMOVED*** Stale timestamp
            },
        )
...
class TestAssignmentFiltering:
    ...
    def test_filter_by_activity_type(self, client, sample_data, admin_token, db):
        ...
        ***REMOVED*** Create rotation templates with different activity types
        ...
        ***REMOVED*** Create 2 on_call, 2 clinic, 2 inpatient assignments
        ...
        ***REMOVED*** Test filtering by on_call
        response = client.get(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
        ...
        ***REMOVED*** Test filtering by clinic
        response = client.get(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
        ...
        ***REMOVED*** Test filtering by inpatient
        response = client.get(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
        ...
        ***REMOVED*** Test with no filter - should return all assignments
        response = client.get(
            "/api/assignments",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        data = response.json()
        ***REMOVED*** Should have at least 6 assignments (could have more from sample_data)
    ...
    def test_filter_by_activity_type_with_date_range(self, client, sample_data, admin_token, db):
        ...
        ***REMOVED*** Create an on_call template
        on_call_template = RotationTemplate(
            id=uuid4(),
            name="On Call",
            activity_type="on_call",
        ...
        ***REMOVED*** Create assignments spanning multiple dates
        today = date.today()
        ...
        ***REMOVED*** Test filtering by activity_type and date range
        ...
        assert response.status_code == 200
        data = response.json()
        ***REMOVED*** Should return assignments within the date range that are on_call
