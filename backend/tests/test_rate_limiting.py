...
def mock_redis():
    ...
    ***REMOVED*** Mock pipeline
    mock_pipe = MagicMock()
    mock_pipe.execute.return_value = [None, 0, None, None]  ***REMOVED*** [del_result, count, zadd_result, expire_result]
...
def real_redis_limiter():
    ...
    try:
        ...
        if limiter.redis is not None:
            ***REMOVED*** Clean up any existing test keys
...
***REMOVED*** ============================================================================
***REMOVED*** Unit Tests - RateLimiter Class
***REMOVED*** ============================================================================
...
class TestRateLimiter:
    ...
    def test_is_rate_limited_blocks_after_limit(self, rate_limiter, mock_redis):
        ...
        mock_pipe = mock_redis.pipeline.return_value
        ***REMOVED*** Simulate 5 existing requests (at limit)
    ...
    def test_is_rate_limited_fails_open_on_redis_error(self, rate_limiter, mock_redis):
        ...
        ***REMOVED*** Should allow request despite error
        assert not is_limited
    ...
    def test_get_remaining(self, rate_limiter, mock_redis):
        ...
        mock_pipe = mock_redis.pipeline.return_value
        mock_pipe.execute.return_value = [None, 2]  ***REMOVED*** 2 existing requests
...
***REMOVED*** ============================================================================
***REMOVED*** Integration Tests - Real Redis (if available)
***REMOVED*** ============================================================================
...
class TestRateLimiterWithRealRedis:
    ...
    def test_sliding_window_behavior(self, real_redis_limiter):
        ...
        max_requests = 3
        window_seconds = 2  ***REMOVED*** Short window for testing

        ***REMOVED*** Make 3 requests (at limit)
        for i in range(max_requests):
            ...
            if i < max_requests - 1:
                ...
                ***REMOVED*** The last request should be limited because we're checking BEFORE adding
                assert not is_limited, f"Request {i + 1} should still be allowed"

        ***REMOVED*** Next request should be limited
        is_limited, info = real_redis_limiter.is_rate_limited(
            key=key,
            max_requests=max_requests,
            window_seconds=window_seconds,
        ...
        ***REMOVED*** Wait for window to pass
        time.sleep(window_seconds + 0.5)

        ***REMOVED*** Should be allowed again
        is_limited, info = real_redis_limiter.is_rate_limited(
            key=key,
            max_requests=max_requests,
            window_seconds=window_seconds,
    ...
    def test_different_keys_are_independent(self, real_redis_limiter):
        ...
        ***REMOVED*** Exhaust limit for key1
        ...
        ***REMOVED*** key1 should be limited, key2 should not
...
class TestRateLimitingOnAuthEndpoints:
    ...
    def test_login_rate_limit_blocks_after_limit(self, client: TestClient):
        ...
        ***REMOVED*** Make requests up to the limit
        ...
        for i in range(max_attempts):
            ...
            ***REMOVED*** Should get 401 (unauthorized) not 429 (rate limited)
            if response.status_code != 429:
                assert response.status_code == 401, f"Attempt {i + 1} should return 401"

        ***REMOVED*** Next request should be rate limited
        response = client.post(
            "/api/auth/login/json",
            json={"username": "testuser", "password": "wrongpassword"},
        )

        ***REMOVED*** Check if we got rate limited (429) or still getting auth errors (401)
        ***REMOVED*** This depends on whether Redis is available and rate limiting is enabled
    ...
    def test_register_rate_limit_blocks_after_limit(self, client: TestClient, admin_user):
        ...
        ***REMOVED*** Get auth headers
        ...
        for i in range(max_attempts):
            ...
            ***REMOVED*** Should get 201 (created) or 400 (validation error) not 429
            if response.status_code != 429:
                assert response.status_code in [201, 400], f"Attempt {i + 1} should return 201 or 400"

        ***REMOVED*** Next request might be rate limited
        response = client.post(
            "/api/auth/register",
            json={
        ...
        ***REMOVED*** Could be rate limited or successful depending on Redis availability
    ...
    def test_different_endpoints_have_separate_limits(self, client: TestClient):
        ...
        ***REMOVED*** Register should still work (separate limit)
        ***REMOVED*** Note: This would require admin auth, so this is more of a conceptual test
        ***REMOVED*** In practice, the rate limits are independent per endpoint/IP combination
    ...
    def test_rate_limit_headers_in_response(self, client: TestClient):
        ...
        ***REMOVED*** Make enough requests to trigger rate limit
        for _ in range(settings.RATE_LIMIT_LOGIN_ATTEMPTS + 2):
            ...
            if response.status_code == 429:
                ***REMOVED*** Check for rate limit headers
                assert "X-RateLimit-Limit" in response.headers
                assert "X-RateLimit-Remaining" in response.headers
...
class TestRateLimitingEdgeCases:
    ...
    def test_rate_limiting_with_redis_unavailable(self):
        ...
        with patch("redis.from_url", side_effect=redis.ConnectionError("Connection failed")):
            ...
            ***REMOVED*** Should fail open (allow requests)
            is_limited, info = limiter.is_rate_limited(
                key="test:192.168.1.1",
                max_requests=5,
                window_seconds=60,
    ...
    def test_rate_limiting_with_rate_limit_disabled(self, client: TestClient):
        ...
        with patch.object(settings, "RATE_LIMIT_ENABLED", False):
            ***REMOVED*** Make many requests - none should be rate limited
            for _ in range(20):
                response = client.post(
                    "/api/auth/login/json",
                    json={"username": "testuser", "password": "wrongpassword"},
    ...
    def test_concurrent_requests_from_different_ips(self, real_redis_limiter):
        ...
        ***REMOVED*** Both IPs make requests
        ...
        ***REMOVED*** IP1 should be limited, IP2 should not
