...
class TestScheduleGenerationRoutes:
    ...
    def auth_headers(self, client, test_user):
        ...
        ***REMOVED*** TODO: Login and get token
        ***REMOVED*** response = client.post("/api/auth/login", data={
        ***REMOVED***     "username": test_user.username,
        ***REMOVED***     "password": "test_password"
        ***REMOVED*** })
        ***REMOVED*** token = response.json()["access_token"]
        ***REMOVED*** return {"Authorization": f"Bearer {token}"}
        return {}

    @pytest.fixture
    def sample_rotation_template(self, db):
        """Create a sample rotation template for testing."""
        ***REMOVED*** TODO: Create actual rotation template
        return None
    ...
    def test_post_schedule_generate_success(self, client, auth_headers, sample_rotation_template):
        ...
        response = client.post(
            ...
            json={
                ...
                "end_date": "2025-12-31",
                ***REMOVED*** "rotation_template_id": str(sample_rotation_template.id)
            },
        )
        assert response.status_code in [200, 201, 202]  ***REMOVED*** May be async
    ...
    ***REMOVED*** =========================================================================
    ...
    def test_post_emergency_coverage_requires_scheduler_role(self, client, auth_headers):
        ...
        ***REMOVED*** Use non-scheduler user
        response = client.post(
            "/api/schedule/emergency-coverage",
            headers=auth_headers,
            json={
        ...
        ***REMOVED*** Should be 403 for non-scheduler
...
class TestScheduleModificationRoutes:
    ...
    def test_delete_schedule_requires_scheduler_role(self, client, auth_headers):
        ...
        ***REMOVED*** Should succeed for scheduler, fail for others
