...
***REMOVED*** ============================================================================
***REMOVED*** Tests for OvernightCallCoverageConstraint
***REMOVED*** ============================================================================
...
class TestOvernightCallCoverageConstraint:
    ...
    def test_validate_empty_assignments(self):
        ...
        ***REMOVED*** Should have violations for each Sun-Thurs night (5 nights)
        call_nights = get_call_nights(blocks)
        assert result.satisfied is False
        assert len(result.violations) == len(call_nights)

        ***REMOVED*** Each violation should be CRITICAL
    ...
    def test_validate_exactly_one_call_per_night(self):
        ...
        ***REMOVED*** Create one call assignment per call night
        call_nights = get_call_nights(blocks)
        assignments = [
            MockAssignment(
    ...
    def test_validate_no_call_on_one_night(self):
        ...
        ***REMOVED*** Create call assignments for all nights except one
        call_nights = get_call_nights(blocks)
        assignments = [
            MockAssignment(
                person_id=faculty.id,
            ...
            for block in call_nights[:-1]  ***REMOVED*** Skip last night
        ]
    ...
    def test_validate_multiple_calls_same_night(self):
        ...
        ***REMOVED*** Assign two faculty to same night
        assignments = [
            MockAssignment(
        ...
        ***REMOVED*** Add single coverage for other nights
    ...
    def test_validate_friday_saturday_ignored(self):
        ...
        ***REMOVED*** Only assign call to Sun-Thurs (not Fri-Sat)
        call_nights = get_call_nights(blocks)
        assignments = [
            MockAssignment(
                person_id=faculty.id,
        ...
        ***REMOVED*** Should pass even though Fri-Sat have no coverage
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_non_call_assignments_ignored(self):
        ...
        ***REMOVED*** Create assignments without call_type
        assignments = [
            MockAssignment(
                person_id=faculty.id,
                block_id=block.id,
                call_type=None,  ***REMOVED*** Not a call assignment
            )
            for block in call_nights
        ]
        ...
        ***REMOVED*** Should fail because these don't count as call coverage
        assert result.satisfied is False
        assert len(result.violations) == len(call_nights)
    ...
    def test_validate_with_specific_weekdays(self):
        ...
        ***REMOVED*** Create blocks for specific dates
        ***REMOVED*** 2025-01-06 is Monday
        ...
        ***REMOVED*** Verify OVERNIGHT_CALL_DAYS constant
        assert monday.weekday() in OVERNIGHT_CALL_DAYS  ***REMOVED*** 0
        assert tuesday.weekday() in OVERNIGHT_CALL_DAYS  ***REMOVED*** 1
        assert wednesday.weekday() in OVERNIGHT_CALL_DAYS  ***REMOVED*** 2
        assert thursday.weekday() in OVERNIGHT_CALL_DAYS  ***REMOVED*** 3
        assert friday.weekday() not in OVERNIGHT_CALL_DAYS  ***REMOVED*** 4
        assert saturday.weekday() not in OVERNIGHT_CALL_DAYS  ***REMOVED*** 5
        assert sunday.weekday() in OVERNIGHT_CALL_DAYS  ***REMOVED*** 6
        ...
        ***REMOVED*** Should have exactly 5 violations (Mon-Thu + Sun)
        assert len(result.violations) == 5


***REMOVED*** ============================================================================
***REMOVED*** Tests for AdjunctCallExclusionConstraint
***REMOVED*** ============================================================================
...
class TestAdjunctCallExclusionConstraint:
    ...
    def test_validate_adjunct_assigned_to_call(self):
        ...
        ***REMOVED*** Assign adjunct to call
    ...
    def test_validate_adjunct_uppercase_role(self):
        ...
        adjunct_faculty = MockPerson(
            person_type="faculty",
            name="Dr. Adjunct",
            faculty_role="ADJUNCT",  ***REMOVED*** Uppercase
        )
    ...
    def test_validate_mixed_faculty_roles(self):
        ...
        assignments = [
            ***REMOVED*** Core faculty on call - OK
            MockAssignment(
                person_id=core_faculty.id,
                block_id=call_nights[0].id,
                call_type="overnight",
            ),
            ***REMOVED*** Adjunct on call - Violation
            MockAssignment(
                person_id=adjunct_faculty.id,
    ...
    def test_validate_adjunct_non_call_assignment(self):
        ...
        ***REMOVED*** Adjunct has regular assignment, not call
        assignment = MockAssignment(
            person_id=adjunct_faculty.id,
        ...
        ***REMOVED*** Should pass - constraint only applies to call assignments
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_faculty_without_role_attribute(self):
        ...
        faculty = MockPerson(
            person_type="faculty",
            name="Dr. NoRole",
            ***REMOVED*** No faculty_role attribute
        )
        ***REMOVED*** Remove faculty_role if it exists
        ...
        ***REMOVED*** Should pass - no faculty_role means not adjunct
        assert result.satisfied is True
        assert len(result.violations) == 0


***REMOVED*** ============================================================================
***REMOVED*** Tests for CallAvailabilityConstraint
***REMOVED*** ============================================================================
...
class TestCallAvailabilityConstraint:
    ...
    def test_validate_available_faculty(self):
        ...
        ***REMOVED*** Faculty is available (no availability restrictions)
        context = SchedulingContext(
            residents=[],
            faculty=[faculty],
            blocks=blocks,
            templates=[],
            availability={},  ***REMOVED*** Empty = all available
        )
    ...
    def test_validate_unavailable_faculty_assigned(self):
        ...
        ***REMOVED*** Mark faculty as unavailable for this block
        context = SchedulingContext(
            residents=[],
            faculty=[faculty],
            blocks=blocks,
    ...
    def test_validate_faculty_available_explicit_true(self):
        ...
        ***REMOVED*** Explicitly mark as available
        context = SchedulingContext(
            residents=[],
            faculty=[faculty],
            blocks=blocks,
    ...
    def test_validate_partial_availability(self):
        ...
        ***REMOVED*** night1: available, night2: unavailable, night3: not specified (defaults available)
        context = SchedulingContext(
            residents=[],
            faculty=[faculty],
            blocks=blocks,
    ...
    def test_validate_non_call_assignment_ignored(self):
        ...
        ***REMOVED*** Regular assignment (not call)
        assignment = MockAssignment(
            person_id=faculty.id,
            block_id=blocks[0].id,
            call_type=None,  ***REMOVED*** Not a call assignment
    ...
    def test_validate_multiple_faculty_mixed_availability(self):
        ...
        assignments = [
            ***REMOVED*** Faculty 1 on night 1 - available
            MockAssignment(
                person_id=faculty1.id,
                block_id=night1.id,
                call_type="overnight",
            ),
            ***REMOVED*** Faculty 2 on night 2 - unavailable
            MockAssignment(
                person_id=faculty2.id,
                block_id=night2.id,
                call_type="overnight",
    ...
    def test_validate_missing_block_in_context(self):
        ...
        ***REMOVED*** Assignment references block not in context
        ...
        ***REMOVED*** Should still detect violation based on availability dict
        assert result.satisfied is False
        assert len(result.violations) == 1
    ...
    def test_validate_missing_faculty_in_context(self):
        ...
        ***REMOVED*** Assignment references faculty not in context
        ...
        context = SchedulingContext(
            residents=[],
            faculty=[],  ***REMOVED*** Empty faculty list
            blocks=blocks,
            templates=[],
            availability={},
        )
        ...
        ***REMOVED*** Should pass - if faculty not in context, we can't validate
        ***REMOVED*** (This is a data consistency issue, not a constraint violation)
        assert result.satisfied is True
        assert len(result.violations) == 0
...
class TestCallCoverageIntegration:
    ...
    def test_complete_valid_week(self):
        ...
        ***REMOVED*** Valid assignments: core faculty only, one per night, all available
        assignments = []
        for i, night in enumerate(call_nights):
            ***REMOVED*** Alternate between two core faculty
        ...
        ***REMOVED*** Validate all three constraints
    ...
    def test_complete_invalid_week_multiple_violations(self):
        ...
        assignments = [
            ***REMOVED*** Night 1: Adjunct assigned (violation)
            MockAssignment(
                person_id=adjunct_faculty.id,
                block_id=call_nights[0].id,
                call_type="overnight",
            ),
            ***REMOVED*** Night 2: Core faculty but unavailable (violation)
            MockAssignment(
                person_id=core_faculty.id,
                block_id=call_nights[1].id,
                call_type="overnight",
            ),
            ***REMOVED*** Night 3: Two faculty assigned (violation)
            MockAssignment(
                person_id=core_faculty.id,
            ...
            ***REMOVED*** Nights 4-5: No coverage (violations)
        ]
        ...
        ***REMOVED*** All should have violations
        ...
        ***REMOVED*** Coverage: missing 2 nights + 1 double-booked = 3 violations
        assert len(coverage_result.violations) == 3

        ***REMOVED*** Adjunct: 2 nights with adjunct assigned
        assert len(adjunct_result.violations) == 2

        ***REMOVED*** Availability: 1 unavailable faculty assigned
