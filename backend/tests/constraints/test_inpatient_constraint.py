...
def create_residents_by_pgy(pgy_levels=None):
    ...
    for pgy in pgy_levels:
        ***REMOVED*** Create 2 residents per PGY level
...
***REMOVED*** ============================================================================
***REMOVED*** Tests for ResidentInpatientHeadcountConstraint
***REMOVED*** ============================================================================
...
class TestResidentInpatientHeadcountConstraint:
    ...
    def test_validate_one_fmit_per_pgy_per_block_passes(self):
        ...
        ***REMOVED*** Create one FMIT assignment per PGY level for first block
    ...
    def test_validate_multiple_fmit_same_pgy_same_block_fails(self):
        ...
        ***REMOVED*** Assign TWO PGY-1 residents to FMIT in same block (violation)
        assignments = [
            MockAssignment(
    ...
    def test_validate_different_pgy_levels_same_block_passes(self):
        ...
        ***REMOVED*** One resident from each PGY level - should be OK (3 total FMIT)
        assignments = [
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=first_block.id,
    ...
    def test_validate_one_night_float_per_block_passes(self):
        ...
        ***REMOVED*** Assign one resident to Night Float for each block
        assignments = []
        for i, block in enumerate(blocks[:3]):  ***REMOVED*** Test first 3 blocks
            resident = residents_by_pgy[1][0]  ***REMOVED*** Use same resident for simplicity
            assignments.append(
                MockAssignment(
                    person_id=resident.id,
                    block_id=block.id,
    ...
    def test_validate_multiple_night_float_same_block_fails(self):
        ...
        ***REMOVED*** Assign TWO residents to Night Float in same block (violation)
        assignments = [
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=first_block.id,
    ...
    def test_validate_mixed_fmit_and_nf_violations(self):
        ...
        assignments = [
            ***REMOVED*** Block 1: Two PGY-1 on FMIT (violation)
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=first_block.id,
                rotation_template_id=fmit_templates[0].id,
            ...
            ***REMOVED*** Block 2: Two residents on Night Float (violation)
            MockAssignment(
                person_id=residents_by_pgy[2][0].id,
        ...
        ***REMOVED*** Check violation messages
    ...
    def test_get_fmit_template_ids(self):
        ...
        templates = [
            ...
            MockRotationTemplate(name="Night Float"),  ***REMOVED*** Should be excluded
            MockRotationTemplate(name="Clinic"),  ***REMOVED*** Should be excluded
        ]
        ...
        ***REMOVED*** Should only get the two FMIT templates (not Night Float)
        assert len(fmit_ids) == 2
        assert templates[0].id in fmit_ids
        assert templates[1].id in fmit_ids
        assert templates[2].id not in fmit_ids  ***REMOVED*** Night Float excluded
        assert templates[3].id not in fmit_ids  ***REMOVED*** Clinic excluded
    ...
    def test_get_nf_template_ids(self):
        ...
        templates = [
            ...
            MockRotationTemplate(name="FMIT Inpatient"),  ***REMOVED*** Should be excluded
            MockRotationTemplate(name="Clinic"),  ***REMOVED*** Should be excluded
        ]
        ...
        ***REMOVED*** Should get all three night/NF templates
        assert len(nf_ids) == 3
        assert templates[0].id in nf_ids
        assert templates[1].id in nf_ids
        assert templates[2].id in nf_ids
        assert templates[3].id not in nf_ids  ***REMOVED*** FMIT excluded
        assert templates[4].id not in nf_ids  ***REMOVED*** Clinic excluded
    ...
    def test_validate_no_templates_passes(self):
        ...
        ***REMOVED*** Only clinic templates, no FMIT or NF
        templates = [
        ...
        ***REMOVED*** Even with assignments, should pass since no FMIT/NF templates
        assignments = [
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
    ...
    def test_validate_resident_without_pgy_level(self):
        ...
        ***REMOVED*** Create resident without pgy_level
        ...
        ***REMOVED*** Should not cause error, resident is ignored if no PGY level
        result = constraint.validate(assignments, context)

        ***REMOVED*** Should pass - resident without PGY level is not counted
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_multiple_violations_same_pgy_different_blocks(self):
        ...
        ***REMOVED*** Create violations in multiple blocks for PGY-1
        assignments = [
            ***REMOVED*** Block 0: Two PGY-1 (violation)
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=blocks[0].id,
                rotation_template_id=fmit_templates[0].id,
            ...
            ***REMOVED*** Block 1: Two PGY-1 (violation)
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
        ...
        assert result.satisfied is False
        assert len(result.violations) == 2  ***REMOVED*** One per block

        ***REMOVED*** Both violations should be for PGY-1
...
***REMOVED*** ============================================================================
***REMOVED*** Tests for FMITResidentClinicDayConstraint
***REMOVED*** ============================================================================
...
class TestFMITResidentClinicDayConstraint:
    ...
    def test_validate_empty_assignments(self):
        ...
        ***REMOVED*** Should pass - validation currently handled by pre-loading
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_with_assignments_passes(self):
        ...
        ***REMOVED*** Should pass - constraint is currently a placeholder
        ***REMOVED*** Pre-loading mechanism handles clinic day assignment
        assert result.satisfied is True
        assert len(result.violations) == 0
...
class TestInpatientConstraintsIntegration:
    ...
    def test_complete_valid_week_all_constraints(self):
        ...
        ***REMOVED*** Valid assignments: 1 FMIT per PGY per block, 1 NF per block
        assignments = []

        ***REMOVED*** Day 1: One of each PGY on FMIT
        assignments.extend([
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=blocks[0].id,
        ...
        ***REMOVED*** Day 1 PM: One resident on Night Float
        assignments.append(
            MockAssignment(
                person_id=residents_by_pgy[1][1].id,
                block_id=blocks[1].id,
    ...
    def test_complete_invalid_week_multiple_violations(self):
        ...
        assignments = [
            ***REMOVED*** Block 0: Two PGY-1 on FMIT (violation)
            MockAssignment(
                person_id=residents_by_pgy[1][0].id,
                block_id=blocks[0].id,
                rotation_template_id=fmit_templates[0].id,
            ...
            ***REMOVED*** Block 1: Two residents on Night Float (violation)
            MockAssignment(
                person_id=residents_by_pgy[2][0].id,
                block_id=blocks[1].id,
                rotation_template_id=nf_templates[0].id,
            ...
            ***REMOVED*** Block 2: Three PGY-2 on FMIT (violation)
            MockAssignment(
                person_id=residents_by_pgy[2][0].id,
        ...
        assert result.satisfied is False
        ***REMOVED*** Should have 3 violations: PGY-1 FMIT, Night Float, PGY-2 FMIT
        assert len(result.violations) == 3

        ***REMOVED*** Verify each violation type
        violation_details = [
            (v.details.get("pgy_level"), v.message) for v in result.violations
        ]

        ***REMOVED*** Check for PGY-1 FMIT violation
        assert any(
            pgy == 1 and "FMIT" in msg for pgy, msg in violation_details
        )

        ***REMOVED*** Check for Night Float violation
        assert any(
            "Night Float" in msg for _, msg in violation_details
        )

        ***REMOVED*** Check for PGY-2 FMIT violation
        assert any(
