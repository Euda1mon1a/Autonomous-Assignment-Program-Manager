...
class TestMyScheduleEndpoint:
    ...
    def test_get_my_schedule_unauthorized(self, client: TestClient):
        ...
        ***REMOVED*** Should return 401, but current implementation has a bug where
        ***REMOVED*** it allows None user and then raises AttributeError
    ...
    def test_get_my_schedule_with_conflicts(
        ...
        ***REMOVED*** Check that conflicts are returned
        fmit_weeks = data.get("fmit_weeks", [])
        if fmit_weeks:
            ***REMOVED*** At least one week should have conflict information available
...
class TestMySwapsEndpoint:
    ...
    def test_create_swap_request_notifies_candidates(
        ...
        ***REMOVED*** Should have notified at least one candidate (if any exist with preferences)
...
class TestSwapRespondEndpoint:
    ...
    def test_respond_to_swap_not_found(
        ...
        ***REMOVED*** Should indicate swap not implemented or not found
        assert response.status_code == 200
...
***REMOVED*** Fixtures for faculty authentication
@pytest.fixture
def faculty_user(db, sample_faculty):
    ...
    ***REMOVED*** Create user with same email as faculty
    user = User(
        id=uuid4(),
    ...
    ***REMOVED*** Update faculty email to match if needed
...
def fmit_week_with_conflict(db, sample_faculty):
    ...
    ***REMOVED*** Create a block for next week
    next_week = date.today() + timedelta(days=7)
    next_week_start = next_week - timedelta(days=next_week.weekday())  ***REMOVED*** Get Monday
    ...
    ***REMOVED*** Create a conflict alert
...
def candidate_faculty_with_prefs(db):
    ...
    ***REMOVED*** Create a candidate faculty member
    candidate = Person(
    ...
    ***REMOVED*** Create preferences with notifications enabled
