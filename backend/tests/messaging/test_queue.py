...
class TestRetryPolicy:
    ...
    def test_get_delay_exponential_backoff(self):
        ...
        ***REMOVED*** Attempt 0: 1.0 * (2^0) = 1.0
        delay0 = policy.get_delay(0)
        assert delay0 == 1.0

        ***REMOVED*** Attempt 1: 1.0 * (2^1) = 2.0
        delay1 = policy.get_delay(1)
        assert delay1 == 2.0

        ***REMOVED*** Attempt 2: 1.0 * (2^2) = 4.0
        delay2 = policy.get_delay(2)
        assert delay2 == 4.0

        ***REMOVED*** Attempt 3: 1.0 * (2^3) = 8.0
    ...
    def test_get_delay_max_delay_cap(self):
        ...
        ***REMOVED*** Attempt 10 would be 1024.0, but should be capped at 5.0
    ...
    def test_get_delay_with_jitter(self):
        ...
        ***REMOVED*** With jitter, delay should be in range [base, base * 1.5]
        base_delay = 10.0
        max_jitter = base_delay * 1.5

        delays = [policy.get_delay(0) for _ in range(100)]

        ***REMOVED*** All delays should be >= base_delay and <= max_jitter
        assert all(base_delay <= d <= max_jitter for d in delays)

        ***REMOVED*** Delays should vary (not all the same due to randomness)
...
class TestQueueConfig:
    ...
    def test_from_env_defaults(self, monkeypatch):
        ...
        ***REMOVED*** Clear any existing env vars
...
class TestMessage:
    ...
    def test_default_values(self):
        ...
        assert msg.message_id is not None  ***REMOVED*** Auto-generated
        assert msg.timestamp is not None  ***REMOVED*** Auto-generated
    ...
    def test_message_id_auto_generation(self):
        ...
        assert msg1.message_id != msg2.message_id  ***REMOVED*** Should be unique
...
class TestMessageQueueAdapter:
    ...
    def test_serialize_message(self):
        ...
        config = QueueConfig()
        adapter = RedisQueueAdapter(config)  ***REMOVED*** Use concrete implementation
    ...
    def test_serialize_message_with_datetime(self):
        ...
        ***REMOVED*** Datetime should be converted to string
...
class TestRedisQueueAdapter:
    ...
    async def test_connect_already_connected(self):
        ...
        with patch("redis.asyncio.from_url") as mock_from_url:
            await adapter.connect()
            ***REMOVED*** Should not attempt connection
    ...
    async def test_purge_queue(self):
        ...
        ***REMOVED*** Setup connected state
        mock_redis = AsyncMock()
        mock_redis.llen = AsyncMock(side_effect=[5, 2, 1])  ***REMOVED*** queue, processing, retry
...
class TestIntegrationScenarios:
    ...
    async def test_publish_consume_pattern_redis(self):
        ...
        ***REMOVED*** Mock Redis client
        ...
        async def mock_brpop(key, timeout):
            await asyncio.sleep(0.01)  ***REMOVED*** Simulate blocking
            if messages_queue:
    ...
    async def test_message_routing_patterns(self):
        ...
        ***REMOVED*** Test different routing patterns
        patterns = [
            "schedule.created",
            "schedule.updated",
            "assignment.changed",
        ...
        ***REMOVED*** Verify all messages were published with correct routing keys
