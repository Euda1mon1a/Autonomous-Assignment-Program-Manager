...
async def test_query_cache_get_or_fetch_hit(query_cache):
    ...
    ***REMOVED*** Second call - cache hit
    ...
    assert call_count == 1  ***REMOVED*** Only called once


@pytest.mark.asyncio
async def test_query_cache_invalidate_pattern(query_cache):
    """Test pattern-based cache invalidation."""
    ***REMOVED*** Set multiple keys
    ...
    ***REMOVED*** Invalidate schedule keys
...
async def test_query_cache_metrics(query_cache):
    ...
    ***REMOVED*** Generate some hits and misses
    await query_cache.get_or_fetch("key1", fetch_fn)
    await query_cache.get_or_fetch("key1", fetch_fn)  ***REMOVED*** Hit
...
async def test_cached_query_decorator(query_cache):
    ...
    ***REMOVED*** Second call with same params - cache hit
    result2 = await expensive_query("test", 123)

    ***REMOVED*** Third call with different params - cache miss
    result3 = await expensive_query("test", 456)

    assert result1 == result2 == "test:123"
    assert result3 == "test:456"
    assert call_count == 2  ***REMOVED*** Only 2 unique calls
...
async def test_query_cache_generate_key():
    ...
    ***REMOVED*** Same query + params should generate same key
    assert key1 == key2

    ***REMOVED*** Different params should generate different key
    assert key1 != key3
...
async def test_query_cache_concurrent_access(query_cache):
    ...
    async def fetch_fn():
        nonlocal call_count
        call_count += 1
        await asyncio.sleep(0.1)  ***REMOVED*** Simulate slow operation
        return {"data": "test"}

    ***REMOVED*** Make multiple concurrent requests
    results = await asyncio.gather(*[
        query_cache.get_or_fetch("concurrent_key", fetch_fn)
        for _ in range(5)
    ])

    ***REMOVED*** All results should be the same
    assert all(r == results[0] for r in results)

    ***REMOVED*** Function should only be called once (first miss)
    ***REMOVED*** Note: This might be > 1 due to race conditions, but should be < 5
    assert call_count < 5
