...
def regular_user_token(db):
    ...
    user = User(
        ...
        role="RESIDENT",  ***REMOVED*** Non-admin role
        is_active=True,
    )
    db.add(user)
    db.commit()
...
class TestProfilingEndpointSecurity:
    ...
    ***REMOVED*** All profiling endpoints that must be admin-only
    PROFILING_ENDPOINTS = [
    ...
    def test_profiling_endpoints_allow_admin_access(
        ...
        if method == "GET":
            ...
            ***REMOVED*** Some endpoints need specific payloads
        ...
        ***REMOVED*** Admin should NOT get authentication/authorization errors
    ...
    def test_profiling_data_not_leaked_in_error_messages(self, client):
        ...
        ***REMOVED*** Error message should be generic
...
class TestProfilingRateLimiting:
    ...
    def test_profiling_endpoints_have_rate_limits(self, client, admin_user_token):
        ...
        ***REMOVED*** Make multiple rapid requests
        for _ in range(100):
            response = client.post(
                "/profiling/start", json={"cpu": True}, headers=headers
            )

        ***REMOVED*** Should eventually get rate limited
...
class TestProfilingAuditLogging:
    ...
    def test_profiling_access_is_logged(self, client, admin_user_token, db):
        ...
        ***REMOVED*** Check that access was logged
        ***REMOVED*** (implementation depends on your audit logging system)
        assert response.status_code in [200, 404]
        ***REMOVED*** TODO: Verify audit log entry was created
