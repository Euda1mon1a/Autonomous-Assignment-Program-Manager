...
***REMOVED*** ==================== CONFLICT ANALYSIS TESTS ====================
...
class TestAnalyzeConflict:
    ...
    def test_analyze_conflict_root_cause(
        ...
        ***REMOVED*** Should contain relevant information about the conflict type
    ...
    def test_analyze_conflict_complexity_scoring(
        ...
        ***REMOVED*** Simple conflict - single faculty, single week
        ...
        ***REMOVED*** Critical conflict - higher complexity
        ...
        ***REMOVED*** Critical should have higher complexity
    ...
    def test_analyze_conflict_safety_checks_performed(
        ...
        assert len(analysis.safety_checks) == 5  ***REMOVED*** All 5 safety checks
    ...
    def test_analyze_conflict_recommends_strategies(
        ...
        ***REMOVED*** Should recommend at least one valid strategy
        assert all(
    ...
    def test_analyze_conflict_identifies_constraints(
        ...
        ***REMOVED*** Create conflict with leave constraint
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.LEAVE_FMIT_OVERLAP,
...
class TestSafetyChecks:
    ...
    def test_acgme_compliance_check_resident(
        ...
        ***REMOVED*** Should pass for basic scenario without excessive hours
    ...
    def test_coverage_gap_check_sufficient_coverage(
        ...
        ***REMOVED*** Create blocks and assignments for the week
        ...
        for i in range(7):
            ...
            for time_of_day in ["AM", "PM"]:
                ...
                ***REMOVED*** Add assignment for available faculty
    ...
    def test_coverage_gap_check_insufficient_coverage(
        ...
        assert result.check_type == SafetyCheckType.COVERAGE_GAP
        ***REMOVED*** Should fail when no other faculty assigned
    ...
    def test_faculty_availability_check_no_available_faculty(
        ...
        ***REMOVED*** Create conflict where all faculty are busy
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.LEAVE_FMIT_OVERLAP,
        ...
        ***REMOVED*** May pass or fail depending on test data - just verify structure
        assert result.check_type == SafetyCheckType.FACULTY_AVAILABILITY
    ...
    def test_supervision_ratio_check(
        ...
        ***REMOVED*** Should pass for basic faculty conflict
        assert result.passed
...
***REMOVED*** ==================== RESOLUTION OPTIONS TESTS ====================
...
class TestGenerateResolutionOptions:
    ...
    def test_generate_options_basic(
        ...
        ***REMOVED*** Should always include defer to human as fallback
        assert any(
    ...
    def test_generate_options_sorted_by_score(
        ...
        if len(options) > 1:
            ***REMOVED*** Verify descending order by overall score
    ...
    def test_generate_options_caching(
        ...
        ***REMOVED*** Should have same option IDs
        assert [o.id for o in options1] == [o.id for o in options2]


***REMOVED*** ==================== STRATEGY-SPECIFIC OPTION TESTS ====================
...
class TestAutoResolveIfSafe:
    ...
    def test_auto_resolve_unsafe_conflict(
        ...
        ***REMOVED*** Create a complex, critical conflict
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.CALL_CASCADE,
        ...
        ***REMOVED*** May or may not be safe depending on context
    ...
    def test_auto_resolve_with_specific_strategy(
        ...
        ***REMOVED*** May succeed or fail depending on whether strategy is available
        assert isinstance(result, ResolutionResult)
        if result.success:
    ...
    def test_auto_resolve_successful_resolution(
        ...
        ***REMOVED*** Create a simple, safe-to-resolve conflict
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.LEAVE_FMIT_OVERLAP,
...
***REMOVED*** ==================== BATCH RESOLUTION TESTS ====================
...
class TestBatchAutoResolve:
    ...
    def test_batch_resolve_with_auto_apply_safe(
        ...
        assert report.total_conflicts == 1
        ***REMOVED*** May or may not apply depending on safety checks
        assert (
    ...
    def test_batch_resolve_risk_level_filtering(
        ...
        ***REMOVED*** Try with low risk only
        report = resolver.batch_auto_resolve(
            [alert.id], auto_apply_safe=True, max_risk_level="low"
        )

        ***REMOVED*** High risk conflict should not be auto-applied with low max_risk
    ...
    def test_batch_resolve_handles_errors_gracefully(
        ...
        ***REMOVED*** Include non-existent conflict ID
        fake_ids = [uuid4(), uuid4(), uuid4()]
        report = resolver.batch_auto_resolve(fake_ids)

        assert report.total_conflicts == 3
        ***REMOVED*** Should handle errors without crashing
    ...
    def test_batch_resolve_generates_recommendations(
        ...
        ***REMOVED*** Should generate recommendations if there are deferred/failed resolutions
        if report.resolutions_deferred > 0 or report.resolutions_failed > 0:
            assert len(report.recommendations) > 0


***REMOVED*** ==================== PRIORITY HANDLING TESTS ====================
...
class TestPriorityHandling:
    ...
    def test_critical_conflicts_analyzed_correctly(
        ...
        ***REMOVED*** Critical conflicts should have higher complexity
    ...
    def test_info_level_conflicts_handled(
        ...
        ***REMOVED*** Info conflicts should have lower complexity
    ...
    def test_risk_level_affects_auto_apply(
        ...
        ***REMOVED*** High risk options should not be auto-applicable
...
class TestEdgeCases:
    ...
    def test_conflict_in_past_handled(
        ...
        ***REMOVED*** Should analyze without errors
        analysis = resolver.analyze_conflict(alert.id)
        assert analysis is not None
    ...
    def test_resolution_cache_expiry(
        ...
        ***REMOVED*** Generate options (cached)
        options1 = resolver.generate_resolution_options(sample_conflict_alert.id)

        ***REMOVED*** Mock time passage
        with patch("app.services.conflict_auto_resolver.datetime") as mock_datetime:
            ...
            ***REMOVED*** Should regenerate after cache expiry
            options2 = resolver.generate_resolution_options(sample_conflict_alert.id)
    ...
    def test_empty_available_faculty_list(
        ...
        ***REMOVED*** Create conflict when faculty member is the only one
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.LEAVE_FMIT_OVERLAP,
        ...
        ***REMOVED*** Should identify lack of available faculty as a constraint
        faculty_check = next(
            (
                c
...
class TestResolutionWorkflow:
    ...
    def test_full_resolution_workflow(
        ...
        ***REMOVED*** Step 1: Create conflict
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.LEAVE_FMIT_OVERLAP,
        ...
        ***REMOVED*** Step 2: Analyze conflict
        analysis = resolver.analyze_conflict(alert.id)
        assert analysis.conflict_id == alert.id

        ***REMOVED*** Step 3: Generate options
        options = resolver.generate_resolution_options(alert.id)
        assert len(options) > 0

        ***REMOVED*** Step 4: Attempt auto-resolution
        result = resolver.auto_resolve_if_safe(alert.id, user_id=sample_user.id)

        ***REMOVED*** Verify result structure regardless of success
        assert isinstance(result, ResolutionResult)
        assert result.conflict_id == alert.id

        if result.success:
            ***REMOVED*** Verify conflict was marked as resolved
            db.refresh(alert)
            assert alert.status == ConflictAlertStatus.RESOLVED
    ...
    def test_multi_conflict_scenario(
        ...
        ***REMOVED*** Create overlapping conflicts
        alerts = []
    ...
    def test_resolution_with_cascading_effects(
        ...
        ***REMOVED*** Create a conflict that might have cascading effects
        alert = ConflictAlert(
            id=uuid4(),
            faculty_id=sample_faculty.id,
            conflict_type=ConflictType.CALL_CASCADE,
        ...
        ***REMOVED*** Should identify cascading nature
        assert (
