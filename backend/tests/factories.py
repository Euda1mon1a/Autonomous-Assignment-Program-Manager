...
class RotationTemplateFactory(factory.Factory):
    ...
    leave_eligible = True  ***REMOVED*** Most rotations allow scheduled leave
    max_residents = 4
    supervision_required = True
    max_supervision_ratio = 4

    class Params:
        ***REMOVED*** Trait for coverage/inpatient rotations that don't allow leave
...
class AbsenceFactory(factory.Factory):
    ...
    is_blocking = None  ***REMOVED*** Auto-determined based on type
    return_date_tentative = False
    created_by_id = None  ***REMOVED*** Set when admin enters on behalf of someone
    notes = factory.Faker('sentence')

    class Params:
        ***REMOVED*** Trait for emergency absences (Hawaii default: 10 days, blocking, tentative return)
        ...
        ***REMOVED*** Trait for bereavement (Hawaii default: 10 days for mainland travel)
...
***REMOVED*** Helper functions for complex scenarios
...
def create_residency_cohort(size: int = 10) -> list[Person]:
    ...
    residents = []
    for i in range(size):
        pgy = (i % 3) + 1  ***REMOVED*** Distribute across PGY 1-3
