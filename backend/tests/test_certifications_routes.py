...
def expiring_person_cert(
    ...
    person_cert = PersonCertification(
        id=uuid4(),
        person_id=sample_resident.id,
        certification_type_id=sample_cert_types[1].id,  ***REMOVED*** ACLS
        certification_number="ACLS-789",
        issued_date=date.today() - timedelta(days=700),
        expiration_date=date.today() + timedelta(days=30),  ***REMOVED*** Expires in 30 days
...
def expired_person_cert(
    ...
    person_cert = PersonCertification(
        ...
        certification_type_id=sample_cert_types[2].id,  ***REMOVED*** PALS
...
***REMOVED*** ============================================================================
***REMOVED*** Certification Type Endpoints
***REMOVED*** ============================================================================
...
class TestListCertificationTypesEndpoint:
    ...
    def test_list_certification_types_active_only(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Create active and inactive cert types
        ...
        ***REMOVED*** Default: active_only=True
        response = client.get("/api/v1/certifications/types")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
        ...
        ***REMOVED*** Explicitly set active_only=False
...
class TestCreateCertificationTypeEndpoint:
    ...
    def test_create_certification_type_duplicate_name(
        ...
        cert_data = {
            "name": sample_cert_type.name,  ***REMOVED*** Duplicate
...
***REMOVED*** ============================================================================
***REMOVED*** Expiration & Compliance Endpoints
***REMOVED*** ============================================================================
...
class TestGetExpiringCertificationsEndpoint:
    ...
    def test_get_expiring_certifications_default_days(
        ...
        ***REMOVED*** Should include the expiring cert (30 days)
        assert data["total"] >= 1
    ...
    def test_get_expiring_certifications_custom_days(
        ...
        ***REMOVED*** Should include expiring cert (30 days)
        assert data["total"] >= 1
    ...
    def test_get_expiring_certifications_narrow_window(
        ...
        ***REMOVED*** sample_person_cert expires in 365 days, should not be included
        assert data["total"] == 0
...
***REMOVED*** ============================================================================
***REMOVED*** Person Certification Endpoints
***REMOVED*** ============================================================================
...
class TestListCertificationsForPersonEndpoint:
    ...
    def test_list_certifications_include_expired(
        ...
        ***REMOVED*** Create an expired cert
        ...
        ***REMOVED*** With expired (default)
        response = client.get(f"/api/v1/certifications/by-person/{sample_resident.id}")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] >= 1

        ***REMOVED*** Without expired
        response = client.get(
...
class TestCreatePersonCertificationEndpoint:
    ...
    def test_create_person_certification_missing_required_fields(
        ...
        cert_data = {
            ***REMOVED*** Missing person_id, certification_type_id, dates
...
class TestDeletePersonCertificationEndpoint:
    ...
    def test_delete_person_certification_success(
        ...
        ***REMOVED*** Create a cert to delete
    ...
    def test_trigger_reminders_requires_admin(
        ...
        ***REMOVED*** Create non-admin user
        ...
        user = User(
            ...
            role="coordinator",  ***REMOVED*** Not admin
            is_active=True,
        )
        db.add(user)
        db.commit()

        ***REMOVED*** Login as regular user
        response = client.post(
        ...
        ***REMOVED*** Try to trigger reminders
        response = client.post(
            "/api/v1/certifications/admin/send-reminders",
            headers=headers,
        )
...
class TestCertificationEdgeCases:
    ...
    def test_certification_with_far_future_expiration(
        ...
        cert_data = {
            ...
            "certification_type_id": str(sample_cert_type.id),
            "issued_date": date.today().isoformat(),
            "expiration_date": (date.today() + timedelta(days=3650)).isoformat(),  ***REMOVED*** 10 years
        }
