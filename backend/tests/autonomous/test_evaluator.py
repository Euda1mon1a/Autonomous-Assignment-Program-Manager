...
class TestScoreComponent:
    ...
    def test_weighted_value_calculation(self):
        ...
        assert component.weighted_value == 0.20  ***REMOVED*** 0.25 * 0.80
...
class TestEvaluationResult:
    ...
    def test_is_better_than_valid_vs_invalid(self):
        ...
        invalid = EvaluationResult(
            valid=False,
            score=0.90,  ***REMOVED*** Higher score but invalid
...
class TestScheduleEvaluatorWeights:
    ...
    def test_weights_sum_to_one(self):
        ...
        assert abs(total - 1.0) < 0.001  ***REMOVED*** Allow floating point error
...
class TestEvaluatorCoverageScore:
    ...
    def test_full_coverage_score(self):
        ...
        ***REMOVED*** This would need a database fixture in real tests
        ***REMOVED*** For now, test the math logic
        assigned = 100
        total = 100
    ...
    def test_no_blocks_coverage(self):
        ...
        assert coverage == 1.0  ***REMOVED*** Default to 1.0 if no blocks
...
class TestEvaluatorLoadBalance:
    ...
    def test_perfectly_balanced(self):
        ...
        ***REMOVED*** All people have same number of assignments
        counts = [10, 10, 10, 10]
    ...
    def test_unbalanced_load(self):
        ...
        counts = [1, 10, 10, 10]  ***REMOVED*** One person underloaded
        mean = sum(counts) / len(counts)
        variance = sum((x - mean) ** 2 for x in counts) / len(counts)
        std_dev = variance**0.5
        cv = std_dev / mean if mean > 0 else 0.0
    ...
    def test_severely_unbalanced(self):
        ...
        counts = [1, 100]  ***REMOVED*** Extreme imbalance
        mean = sum(counts) / len(counts)
        variance = sum((x - mean) ** 2 for x in counts) / len(counts)
        std_dev = variance**0.5
        cv = std_dev / mean if mean > 0 else 0.0
        ...
        assert cv > 0.5  ***REMOVED*** High coefficient of variation
        assert score < 0.5  ***REMOVED*** Low score
