...
class TestParameterAdapter:
    ...
    def test_no_change_on_empty_history(self):
        ...
        ***REMOVED*** Should return unchanged or with algorithm switch for ACGME
    ...
    def test_switch_algorithm_on_acgme_violation(self):
        ...
        assert new_params.algorithm == "cp_sat"  ***REMOVED*** Switched from greedy

    def test_algorithm_rotation(self):
        """Test that algorithm switches rotate through all options."""
        adapter = ParameterAdapter()

        ***REMOVED*** Start with greedy
        ...
        assert adapter._next_algorithm("hybrid") == "greedy"  ***REMOVED*** Wraps around
    ...
    def test_diversification_on_stagnation(self):
        ...
        ***REMOVED*** 5 iterations with similar scores = stagnation
    ...
    def test_timeout_increase_on_slow_run(self):
        ...
        ***REMOVED*** History with long duration and low score
        history = [
            IterationRecord(
                iteration=0,
                timestamp=datetime.now(),
                ...
                total_violations=0,
                violation_types=[],
                duration_seconds=58.0,  ***REMOVED*** Close to timeout
            )
        ]
    ...
    def test_narrow_search_near_feasible(self):
        ...
        result = make_result(score=0.85, valid=False)  ***REMOVED*** Close but not valid
        history = []
...
class TestAdaptationRule:
    ...
    def test_custom_rule(self):
        ...
        ***REMOVED*** Custom rule that always triggers
        custom_rule = AdaptationRule(
            name="always_switch",
            condition=lambda e, h: True,
            action=AdaptationAction.SWITCH_ALGORITHM,
            priority=1000,  ***REMOVED*** High priority
        )

        adapter = ParameterAdapter(custom_rules=[custom_rule])
        params = GeneratorParams(algorithm="greedy")
        result = make_result(score=0.9, valid=True)  ***REMOVED*** Good result
        history = []

        new_params = adapter.adapt(params, result, history)

        ***REMOVED*** Custom rule should trigger despite good result
        assert new_params.algorithm == "cp_sat"

    def test_rule_priority_ordering(self):
        """Test that higher priority rules are checked first."""
        ***REMOVED*** Low priority rule that switches algorithm
        ...
        ***REMOVED*** High priority rule that increases timeout
        ...
        ***REMOVED*** High priority should win - timeout should increase
        assert new_params.timeout_seconds == 120.0
        assert new_params.algorithm == "greedy"  ***REMOVED*** Unchanged
...
class TestConditionFunctions:
    ...
    def test_is_stagnating_false_short_history(self):
        ...
        result = make_result(score=0.7)
        history = make_history([0.7, 0.7, 0.7])  ***REMOVED*** Only 3 iterations
