...
def marketplace_flag_no_residents(db: Session) -> FeatureFlag:
    ...
    flag = FeatureFlag(
        ...
        flag_type="boolean",
        enabled=True,
        target_roles=["admin", "coordinator", "faculty"],  ***REMOVED*** Excludes resident
    )
    db.add(flag)
    db.commit()
    db.refresh(flag)
...
***REMOVED*** ============================================================================
***REMOVED*** Tests: Marketplace View Access
***REMOVED*** ============================================================================
...
class TestMarketplaceViewAccess:
    ...
    def test_faculty_can_view_marketplace(
        ...
        ***REMOVED*** Should be 200 (success) or 403 for no faculty profile, not for role restriction
        assert response.status_code in [200, 403]
        if response.status_code == 403:
            ***REMOVED*** Make sure it's not a marketplace access error
    ...
    def test_resident_can_view_marketplace_when_enabled(
        ...
        ***REMOVED*** Should be 200 (success) or 403 for no faculty profile, not for role restriction
        assert response.status_code in [200, 403]
        if response.status_code == 403:
            ***REMOVED*** Error should be about faculty profile, not marketplace access
            assert "marketplace access" not in response.json().get("detail", "").lower()


***REMOVED*** ============================================================================
***REMOVED*** Tests: Marketplace Posting (Swap Creation Without Target)
***REMOVED*** ============================================================================
...
class TestMarketplacePostAccess:
    ...
    def test_resident_cannot_post_to_marketplace(
        ...
        response = client.post(
            ...
            json={
                ...
                ***REMOVED*** No preferred_target_faculty_id = marketplace posting
            },
            headers=resident_auth_headers,
        )
        assert response.status_code == 403
    ...
    def test_resident_can_request_direct_swap(
        ...
        ***REMOVED*** Should not get marketplace access error (might get other errors like no faculty profile)
        if response.status_code == 403:
    ...
    def test_faculty_can_post_to_marketplace(
        ...
        ***REMOVED*** Should not get marketplace access error
        if response.status_code == 403:
            assert "marketplace access" not in response.json().get("detail", "").lower()
    ...
    def test_resident_can_post_when_flag_allows(
        ...
        ***REMOVED*** Should not get marketplace access error (might get other errors)
        if response.status_code == 403:
            assert "marketplace access" not in response.json().get("detail", "").lower()


***REMOVED*** ============================================================================
***REMOVED*** Tests: Feature Flag Edge Cases
***REMOVED*** ============================================================================
...
class TestFeatureFlagEdgeCases:
    ...
    def test_disabled_flag_blocks_all_users(self, db: Session, client: TestClient):
        ...
        ***REMOVED*** Create a disabled flag
        flag = FeatureFlag(
            id=uuid4(),
            key="swap_marketplace_enabled",
            name="Enable Swap Marketplace",
            flag_type="boolean",
            enabled=False,  ***REMOVED*** Globally disabled
        ...
        ***REMOVED*** Try to access marketplace
    ...
    def test_admin_can_always_access_marketplace(
        ...
        ***REMOVED*** Create restrictive flag that doesn't include admin (edge case)
        flag = FeatureFlag(
            id=uuid4(),
            key="swap_marketplace_enabled",
            name="Enable Swap Marketplace",
            flag_type="boolean",
            enabled=True,
            target_roles=["faculty"],  ***REMOVED*** Only faculty
        )
        db.add(flag)
        db.commit()

        ***REMOVED*** Admin should still be blocked by role targeting
        ***REMOVED*** (This verifies the feature flag logic is working correctly)
        response = client.get("/api/portal/marketplace", headers=auth_headers)
        ***REMOVED*** Admin should get 403 because they're not in target_roles
        ***REMOVED*** Unless they have a faculty profile, then they'd get a different error
        assert response.status_code in [200, 403]


***REMOVED*** ============================================================================
***REMOVED*** Additional fixtures from conftest
***REMOVED*** ============================================================================
