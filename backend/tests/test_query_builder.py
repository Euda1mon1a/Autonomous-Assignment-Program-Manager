...
class TestQueryBuilderBasicFilters:
    ...
    def test_filter_gt(self, db: Session, sample_residents: list[Person]):
        ...
        assert all(p.pgy_level > 1 for p in result)
        assert len(result) == 2  ***REMOVED*** PGY 2 and 3
    ...
    def test_filter_lt(self, db: Session, sample_residents: list[Person]):
        ...
        assert len(result) == 2  ***REMOVED*** PGY 1 and 2
    ...
    def test_filter_is_null(self, db: Session, sample_resident: Person, sample_faculty: Person):
        ...
        ***REMOVED*** Faculty should have NULL pgy_level
        assert len(result) >= 1
...
class TestQueryBuilderSorting:
    ...
    def test_order_by_multiple_fields(self, db: Session):
        ...
        ***REMOVED*** Create residents with same PGY but different names
        ...
        ***REMOVED*** Should be sorted by pgy_level first, then name
        assert result[0].name == "Charlie"  ***REMOVED*** PGY 1
        assert result[1].name == "Alice"  ***REMOVED*** PGY 2, alphabetically first
        assert result[2].name == "Bob"  ***REMOVED*** PGY 2, alphabetically second
...
class TestQueryBuilderPagination:
    ...
    def test_paginate_middle_page(self, db: Session):
        ...
        ***REMOVED*** Create 25 residents
        for i in range(25):
            person = Person(
                id=uuid4(),
                name=f"Resident {i:02d}",  ***REMOVED*** Zero-padded for sorting
                type="resident",
                pgy_level=1,
            )
            db.add(person)
        ...
        assert len(result["items"]) == 10
        ***REMOVED*** Should get items 10-19 (0-indexed: items 10-19)
    ...
    def test_paginate_last_page_partial(self, db: Session):
        ...
        ***REMOVED*** Create 25 residents
        ...
        assert len(result["items"]) == 5  ***REMOVED*** Only 5 items on last page
    ...
    def test_limit_offset(self, db: Session, sample_residents: list[Person]):
        ...
        assert len(result) == 2
        ***REMOVED*** Should skip first resident (PGY 1) and get PGY 2 and 3
        pgy_levels = [p.pgy_level for p in result]
...
class TestQueryBuilderJoins:
    ...
    def test_join_related_relationship(
        ...
        ***REMOVED*** Create assignment
        ...
        assert len(result) == 1
        ***REMOVED*** Verify relationships are loaded (no additional queries)
    ...
    def test_filter_by_joined(
        ...
        ***REMOVED*** Create assignments for different residents
...
class TestQueryBuilderAggregations:
    ...
    def test_aggregate_avg(self, db: Session, sample_residents: list[Person]):
        ...
        assert avg == 2.0  ***REMOVED*** (1 + 2 + 3) / 3
    ...
    def test_group_by_with_count(self, db: Session, sample_residents: list[Person]):
        ...
        ***REMOVED*** Each PGY level should have 1 resident
        assert len(result) == 3
...
class TestQueryBuilderSubqueries:
    ...
    def test_filter_in_with_subquery(
        ...
        ***REMOVED*** Create assignments for PGY 2 and 3 only
        for resident in sample_residents:
...
class TestQueryBuilderDateFilters:
    ...
    def test_filter_date_range_both_bounds(self, db: Session):
        ...
        ***REMOVED*** Create blocks
        ...
        assert len(result) == 8  ***REMOVED*** Days 0-7 inclusive
...
class TestQueryBuilderArrayOperations:
    ...
    def test_filter_contains(self, db: Session):
        ...
        ***REMOVED*** Create faculty with different specialties
        fac1 = Person(
            id=uuid4(),
            name="Dr. Sports",
            type="faculty",
...
class TestQueryBuilderBulkOperations:
    ...
    def test_bulk_update(self, db: Session, sample_residents: list[Person]):
        ...
        ***REMOVED*** Verify update
        db.expire_all()  ***REMOVED*** Clear session cache
        updated = db.query(Person).filter(Person.pgy_level == 2).all()
        assert len(updated) == 2  ***REMOVED*** Original PGY 2 + updated from PGY 1
...
class TestQueryBuilderClone:
    ...
    def test_clone_basic(self, db: Session, sample_residents: list[Person]):
        ...
        ***REMOVED*** Clone and add different filters
...
class TestQueryBuilderDistinct:
    ...
    def test_distinct_on_field(self, db: Session):
        ...
        ***REMOVED*** Should get 3 results (one per PGY level)
        pgy_levels = [p.pgy_level for p in result]
...
class TestQueryBuilderChaining:
    ...
    def test_complex_chaining(self, db: Session):
        ...
        ***REMOVED*** Complex query with multiple chained methods
        result = (
...
class TestQueryBuilderSQLInjectionPrevention:
    ...
    def test_filter_by_prevents_injection(self, db: Session, sample_resident: Person):
        ...
        ***REMOVED*** Attempt SQL injection via field value
        malicious_value = "'; DROP TABLE people; --"

        qb = QueryBuilder(Person, db)
        result = qb.filter_by(name=malicious_value).all()

        ***REMOVED*** Should safely return no results, not execute SQL injection
        assert len(result) == 0

        ***REMOVED*** Verify table still exists
    ...
    def test_invalid_column_name_injection(self, db: Session):
        ...
        ***REMOVED*** Attempt injection via column name
        with pytest.raises(ValueError, match="Invalid column"):
