...
class TestAuthController:
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_login_inactive_user(self, db):
        ...
        ***REMOVED*** Create an inactive user
        user = User(
            id=uuid4(),
    ...
    def test_login_lockout_after_failed_attempts(self, db):
        ...
        ***REMOVED*** Attempt multiple failed logins
        for _ in range(5):
            try:
        ...
        ***REMOVED*** Next attempt should trigger lockout (429)
        with pytest.raises(HTTPException) as exc_info:
            controller.login("lockouttest", "WrongPassword!")
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_register_user_success(self, db):
        ...
        ***REMOVED*** Registration without current_user for initial admin setup
        ***REMOVED*** or with admin user for subsequent registrations
        admin_user = User(
            id=uuid4(),
            username="admin",
    ...
    def test_register_user_duplicate_username(self, db):
        ...
        ***REMOVED*** Create admin for registration
        admin_user = User(
            id=uuid4(),
            username="admin",
            email="admin@test.org",
        ...
        user_data = UserCreate(
            username="existinguser",  ***REMOVED*** Duplicate username
            email="new@test.org",
            password="NewPassword123!",
            role="resident",
        )
    ...
    def test_register_user_duplicate_email(self, db):
        ...
        ***REMOVED*** Create admin for registration
        ...
        user_data = UserCreate(
            ...
            email="duplicate@test.org",  ***REMOVED*** Duplicate email
            password="NewPassword123!",
            role="resident",
        )
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_register_then_login_workflow(self, db):
        ...
        ***REMOVED*** Login with newly registered user
    ...
    def test_lockout_clears_on_successful_login(self, db):
        ...
        ***REMOVED*** Make some failed attempts (not enough to lock out)
        for _ in range(2):
            try:
        ...
        ***REMOVED*** Successful login should clear attempts
        token = controller.login("lockoutclear", "CorrectPassword123!")
        assert token.access_token is not None

        ***REMOVED*** Failed attempts should reset, so we shouldn't be locked out
        for _ in range(2):
            try:
                controller.login("lockoutclear", "WrongPassword!")
            except HTTPException as e:
                ***REMOVED*** Should be 401 (invalid credentials), not 429 (locked out)
