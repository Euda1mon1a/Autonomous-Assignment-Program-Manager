...
class TestPersonServiceQueries:
    ...
    def test_filter_residents_by_pgy_level(self, db: Session, sample_residents: list):
        ...
        ***REMOVED*** Query for PGY 2 residents
        pgy2_residents = (
            db.query(Person)
...
class TestAbsenceQueries:
    ...
    def test_find_absences_in_date_range(self, db: Session, sample_resident: Person):
        ...
        ***REMOVED*** Query for absences in range
        absences = (
            db.query(Absence)
            .filter(
    ...
    def test_blocking_absences_flag(self, db: Session, sample_resident: Person):
        ...
        ***REMOVED*** Create blocking and non-blocking absences
        blocking = Absence(
            id=uuid4(),
            person_id=sample_resident.id,
        ...
        ***REMOVED*** Query blocking absences
        blocking_absences = (
            db.query(Absence)
...
class TestBlockQueries:
    ...
    def test_find_blocks_for_date_range(self, db: Session, sample_blocks: list):
        ...
        ***REMOVED*** 4 days * 2 half-days = 8 blocks
        assert len(blocks) == 8
    ...
    def test_filter_weekend_blocks(self, db: Session, sample_blocks: list):
        ...
        for block in weekend_blocks:
            ...
            assert block.date.weekday() >= 5  ***REMOVED*** Saturday or Sunday
    ...
    def test_filter_am_pm_blocks(self, db: Session, sample_blocks: list):
        ...
        assert len(am_blocks) == 7  ***REMOVED*** 7 days
...
class TestPersonValidation:
    ...
    def test_resident_requires_pgy_level(self, db: Session):
        ...
        resident = Person(
            ...
            pgy_level=None,  ***REMOVED*** Missing PGY level
        )

        ***REMOVED*** In a real scenario, this would be caught by schema validation
        ***REMOVED*** Here we just verify the model allows it (validation is at schema level)
        db.add(resident)
        db.commit()
