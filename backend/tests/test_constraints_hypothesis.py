...
***REMOVED*** Strategy to generate a list of daily work hours (0-12 hours per day)
...
def test_80_hour_rule_catches_all_violations(daily_hours):
    ...
    ***REMOVED*** Build blocks and assignments based on hours
    blocks = []
    assignments = []

    for day_idx, hours in enumerate(daily_hours):
        block_date = start_date + timedelta(days=day_idx)
        ***REMOVED*** Create blocks proportional to hours (each block = 6 hours)
        num_blocks = hours // 6
    ...
    ***REMOVED*** Check for actual violations (manual calculation)
    has_violation = False
    ...
    ***REMOVED*** Validate constraint
    result = constraint.validate(assignments, context)

    ***REMOVED*** Property: constraint should find violation if one exists
    if has_violation:
        assert (
...
def test_one_in_seven_rule_property(work_days):
    ...
    """
    ***REMOVED*** Count max consecutive True values
    max_consecutive = 0
    ...
    ***REMOVED*** Build mock schedule
    ...
    if not blocks:
        return  ***REMOVED*** Skip if no work days
