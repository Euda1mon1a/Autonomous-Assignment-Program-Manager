...
class TestListBlocksEndpoint:
    ...
    def test_list_blocks_with_data(self, client: TestClient, sample_blocks):
        ...
        ***REMOVED*** Validate block structure
        block = data["items"][0]
        assert "id" in block
    ...
    def test_list_blocks_filter_by_start_date(self, client: TestClient, sample_blocks):
        ...
        ***REMOVED*** All returned blocks should be on or after the filter date
    ...
    def test_list_blocks_filter_by_end_date(self, client: TestClient, sample_blocks):
        ...
        ***REMOVED*** All returned blocks should be on or before the filter date
        for block in data["items"]:
            block_date = date.fromisoformat(block["date"])
            assert block_date <= date.fromisoformat(filter_date)
    ...
    def test_list_blocks_filter_by_date_range(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Create blocks across a larger range
        start = date.today()
        for i in range(14):
            for tod in ["AM", "PM"]:
                block = Block(
        ...
        ***REMOVED*** Filter for middle week
        ...
        ***REMOVED*** Should get 5 days * 2 blocks = 10 blocks
        assert data["total"] == 10

        ***REMOVED*** All blocks should be in range
        for block in data["items"]:
            block_date = date.fromisoformat(block["date"])
    ...
    def test_list_blocks_filter_by_block_number(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Create blocks with different block numbers
        today = date.today()
        ...
        ***REMOVED*** Should get only blocks with block_number 2
        assert data["total"] == 3
        for block in data["items"]:
    ...
    def test_list_blocks_combined_filters(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Filter by date range AND block number
        ...
        ***REMOVED*** Validate combined filters
        for block in data["items"]:
            block_date = date.fromisoformat(block["date"])
            assert date.fromisoformat(filter_start) <= block_date <= date.fromisoformat(filter_end)
            assert block["block_number"] == 2
    ...
    def test_list_blocks_no_results_in_range(self, client: TestClient, sample_blocks):
        ...
        ***REMOVED*** Query far in the future
...
class TestCreateBlockEndpoint:
    ...
    def test_create_block_invalid_time_of_day(self, client: TestClient):
        ...
        block_data = {
            ...
            "time_of_day": "EVENING",  ***REMOVED*** Invalid, must be AM or PM
            "block_number": 1,
            "is_weekend": False,
            "is_holiday": False,
        }
    ...
    def test_create_block_missing_required_fields(self, client: TestClient):
        ...
        block_data = {
            ...
            ***REMOVED*** Missing time_of_day and block_number
        }
    ...
    def test_create_block_duplicate(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Should fail with 400 or 422 depending on validation
    ...
    def test_create_block_negative_block_number(self, client: TestClient):
        ...
        ***REMOVED*** Depending on validation, might succeed or fail
        ***REMOVED*** If it succeeds, verify the block_number is stored
...
class TestGenerateBlocksEndpoint:
    ...
    def test_generate_blocks_single_day(self, client: TestClient):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 2  ***REMOVED*** AM and PM
        assert len(data["items"]) == 2

        ***REMOVED*** Verify both AM and PM blocks
    ...
    def test_generate_blocks_week(self, client: TestClient):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 14  ***REMOVED*** 7 days * 2 blocks
    ...
    def test_generate_blocks_month(self, client: TestClient):
        ...
        end = start + timedelta(days=29)  ***REMOVED*** 30 days
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 60  ***REMOVED*** 30 days * 2 blocks
    ...
    def test_generate_blocks_invalid_date_range(self, client: TestClient):
        ...
        end = start - timedelta(days=5)  ***REMOVED*** Invalid: end before start
        ...
        ***REMOVED*** Should handle gracefully (either error or return 0 blocks)
    ...
    def test_generate_blocks_default_base_number(self, client: TestClient):
        ...
        response = client.post(
            ...
            params={
                "start_date": test_date.isoformat(),
                "end_date": test_date.isoformat(),
                ***REMOVED*** base_block_number not provided, should default to 1
            }
        )
        ...
        ***REMOVED*** Should use default base number
        for block in data["items"]:
            assert "block_number" in block
...
class TestDeleteBlockEndpoint:
    ...
    def test_delete_block_success(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Create a block to delete
        block = Block(
            id=uuid4(),
        ...
        ***REMOVED*** Verify block is deleted
...
class TestBlockStructureAndValidation:
    ...
    def test_block_response_has_all_fields(self, client: TestClient, sample_block: Block):
        ...
        ***REMOVED*** Optional fields
        assert "holiday_name" in data  ***REMOVED*** Should be present even if null
...
class TestBlockEdgeCases:
    ...
    def test_create_block_far_future_date(self, client: TestClient):
        ...
        far_future = date.today() + timedelta(days=3650)  ***REMOVED*** 10 years
    ...
    def test_create_block_past_date(self, client: TestClient):
        ...
        ***REMOVED*** Should succeed (past dates might be needed for historical data)
    ...
    def test_generate_blocks_zero_days(self, client: TestClient):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 2  ***REMOVED*** Should still create AM and PM for that day
    ...
    def test_list_blocks_boundary_dates(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Query with exact start date
        response = client.get(
            "/api/blocks",
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total"] >= 5  ***REMOVED*** Should include blocks on exact start date
    ...
    def test_holiday_name_without_holiday_flag(self, client: TestClient):
        ...
        ***REMOVED*** Should succeed - validation may or may not enforce consistency
        assert response.status_code in [201, 400, 422]
