...
def sample_schedule_data(db):
    ...
    ***REMOVED*** Create assignments
    assignments = []
    for block in blocks[:10]:  ***REMOVED*** Assign first 5 days
        ***REMOVED*** Resident 1 to clinic
    ...
    ***REMOVED*** Create FMIT swap record
    swap = SwapRecord(
        id=uuid4(),
...
class TestHeatmapService:
    ...
    def test_calculate_date_range_week(self):
        ...
        reference = date(2024, 1, 15)  ***REMOVED*** Monday
        ...
        assert start == date(2024, 1, 15)  ***REMOVED*** Monday
        assert end == date(2024, 1, 21)  ***REMOVED*** Sunday
        assert (end - start).days == 6

    def test_calculate_date_range_week_from_wednesday(self):
        """Test calculating date range for a week starting from Wednesday."""
        service = HeatmapService()
        reference = date(2024, 1, 17)  ***REMOVED*** Wednesday
        time_range = TimeRangeType(
            range_type="week",
            reference_date=reference,
        )
        ...
        assert start == date(2024, 1, 15)  ***REMOVED*** Monday of that week
        assert end == date(2024, 1, 21)  ***REMOVED*** Sunday of that week
    ...
    def test_calculate_date_range_month_february(self):
        ...
        service = HeatmapService()
        reference = date(2024, 2, 15)  ***REMOVED*** 2024 is leap year
        time_range = TimeRangeType(
            range_type="month",
            reference_date=reference,
        )
        ...
        assert end == date(2024, 2, 29)  ***REMOVED*** Leap year

    def test_calculate_date_range_quarter_q1(self):
        """Test calculating date range for Q1."""
        service = HeatmapService()
        reference = date(2024, 2, 15)  ***REMOVED*** In Q1
        time_range = TimeRangeType(
            range_type="quarter",
            reference_date=reference,
        )
    ...
    def test_calculate_date_range_quarter_q4(self):
        ...
        service = HeatmapService()
        reference = date(2024, 11, 15)  ***REMOVED*** In Q4
        time_range = TimeRangeType(
            range_type="quarter",
            reference_date=reference,
        )
    ...
    def test_generate_unified_heatmap_by_person(self, db, sample_schedule_data):
        ...
        assert len(result.data.x_labels) == 7  ***REMOVED*** 7 days
        assert len(result.data.y_labels) == 3  ***REMOVED*** 3 people
        assert len(result.data.z_values) == 3  ***REMOVED*** 3 rows
        assert len(result.data.z_values[0]) == 7  ***REMOVED*** 7 columns
        assert result.metadata["total_assignments"] == 30  ***REMOVED*** 3 people * 10 blocks
    ...
    def test_generate_unified_heatmap_with_person_filter(
        ...
        assert len(result.data.y_labels) == 1  ***REMOVED*** Only 1 person
    ...
    def test_generate_person_workload_heatmap_exclude_weekends(
        ...
        ***REMOVED*** Should have 5 weekdays
    ...
    def test_export_heatmap_image_png(self, db, sample_schedule_data):
        ...
        ***REMOVED*** Note: This test requires kaleido to be installed
        ***REMOVED*** If kaleido is not available, the test will fail with a RuntimeError
        try:
...
class TestVisualizationAPI:
    ...
    def test_get_heatmap_image(self, client, admin_token, sample_schedule_data):
        ...
        try:
            ...
            ***REMOVED*** Kaleido might not be available in test environment
    ...
    def test_unified_heatmap_with_week_range(
        ...
        assert len(data["data"]["x_labels"]) == 7  ***REMOVED*** 7 days in a week
    ...
    def test_unified_heatmap_with_quarter_range(
        ...
        data = response.json()
        assert "data" in data
        ***REMOVED*** Q1 has 31 + 29 (leap year) + 31 = 91 days in 2024
