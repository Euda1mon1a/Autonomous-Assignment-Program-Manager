...
***REMOVED*** ============================================================================
***REMOVED*** Mock Constraints for Testing
***REMOVED*** ============================================================================
...
class ConferenceConstraint(HardConstraint):
    ...
    def validate(self, assignments, context) -> ConstraintResult:
        ...
        ***REMOVED*** Check each conference block
        for block_id in self.conference_blocks:
            ***REMOVED*** Find assignments during conference
            conflicting = [
                a
            ...
            for assignment in conflicting:
                ***REMOVED*** Conference is mandatory, so any other assignment conflicts
...
class PreferenceConstraint(SoftConstraint):
    ...
    def __init__(self, weight: float = 1.0) -> None:
        ...
        self.preferences: dict = {}  ***REMOVED*** {person_id: preferred_session}
    ...
    def validate(self, assignments, context) -> ConstraintResult:
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Get block to check session
...
class TestConstraintInteractions:
    ...
    def test_conflicting_hard_constraints(self, db):
        ...
        ***REMOVED*** Create conference block (Wednesday AM)
        ...
        ***REMOVED*** Create rotation template for call
        ...
        ***REMOVED*** ACTION: Validate constraints
        result = conference_constraint.validate([call_assignment], context)

        ***REMOVED*** ASSERT: Conflict detected
        ...
        ***REMOVED*** Verify assignment exists (conflict should be caught before creation in real system)
    ...
    def test_soft_constraint_zero_weight(self, db):
        ...
        ***REMOVED*** Create afternoon block
        afternoon_date = date(2025, 1, 15)
        afternoon_block = Block(
            id=uuid4(),
            date=afternoon_date,
            session=Session.PM,  ***REMOVED*** Resident prefers AM
            block_number=15,
            academic_year=2025,
        )
        db.add(afternoon_block)

        ***REMOVED*** Create rotation template
        ...
        ***REMOVED*** Create afternoon assignment (violates preference)
        ...
        ***REMOVED*** Create soft constraint with weight=0
        preference_zero = PreferenceConstraint(weight=0.0)
        preference_zero.add_preference(resident.id, Session.AM)  ***REMOVED*** Prefers mornings

        ***REMOVED*** Create soft constraint with non-zero weight for comparison
        ...
        ***REMOVED*** ACTION: Validate with zero weight
        result_zero = preference_zero.validate([afternoon_assignment], context)

        ***REMOVED*** ASSERT: Should be valid despite violating preference
    ...
    def test_credential_expiring_mid_block(self, db):
        ...
        ***REMOVED*** Create 4-week block assignment (Jan 1-28)
        ...
        ***REMOVED*** Create rotation template requiring BLS
        ...
        ***REMOVED*** Create assignments for the 4 weeks
        assignments = []
        for block in blocks[:56]:  ***REMOVED*** 4 weeks * 7 days * 2 sessions
        ...
        ***REMOVED*** Mock credential data (BLS expires Jan 15, mid-block)
        bls_expiration = date(2025, 1, 15)

        ***REMOVED*** Create mock credential validator
        class CredentialExpiringConstraint(SoftConstraint):
            ...
            def __init__(self, credentials: dict) -> None:
                super().__init__(
                    ...
                    constraint_type=ConstraintType.AVAILABILITY,
                    weight=3.0,  ***REMOVED*** Warning level
                    priority=ConstraintPriority.HIGH,
                )
                self.credentials = credentials  ***REMOVED*** {person_id: {credential_type: expiration_date}}
            ...
            def validate(self, assignments, context) -> ConstraintResult:
                ...
                ***REMOVED*** Group assignments by person and date range
                person_assignments = {}
                for a in assignments:
                ...
                for person_id, person_assigns in person_assignments.items():
                    ...
                    ***REMOVED*** Get date range for this person's assignments
                    dates = [
                    ...
                    ***REMOVED*** Check each credential
                    for cred_type, expiration in self.credentials[person_id].items():
                        ***REMOVED*** Credential expires during assignment period
                ...
                return ConstraintResult(
                    satisfied=True,  ***REMOVED*** Still valid but flagged
        ...
        ***REMOVED*** ACTION: Validate credentials
        result = credential_constraint.validate(assignments, context)

        ***REMOVED*** ASSERT: Should be valid but with warnings
        ...
        ***REMOVED*** Verify days until expiration calculated
    ...
    def test_leave_overlapping_call_assignment(self, db):
        ...
        ***REMOVED*** Create call blocks (Jan 15-16, 24-hour shift)
        call_start_date = date(2025, 1, 15)
        call_end_date = date(2025, 1, 16)

        call_blocks = []
        for day_offset in range(2):  ***REMOVED*** 15th and 16th
            for session in [Session.AM, Session.PM]:
                block = Block(
                    id=uuid4(),
        ...
        ***REMOVED*** Create call rotation template
        call_template = RotationTemplate(
            id=uuid4(),
            name="Call Coverage",
            activity_type="call",
        ...
        ***REMOVED*** Create call assignments
        call_assignments = []
        ...
        ***REMOVED*** Create leave request overlapping call (Jan 15-17)
        leave_request = Absence(
            ...
            is_blocking=True,  ***REMOVED*** Blocking absence
            notes="Family vacation planned before call assignment",
        )
        db.add(leave_request)
        db.commit()

        ***REMOVED*** Create constraint to detect leave conflicts
        class LeaveConflictConstraint(HardConstraint):
            ...
            def validate(self, assignments, context) -> ConstraintResult:
                ...
                ***REMOVED*** Query all blocking absences
                ...
                ***REMOVED*** Check each assignment against absences
                for assignment in assignments:
                    ***REMOVED*** Get block date
                    block = next((b for b in context.blocks if b.id == assignment.block_id), None)
                    if not block:
                        continue

                    ***REMOVED*** Check if person has leave on this date
        ...
        ***REMOVED*** ACTION: Validate leave conflicts
        ...
        ***REMOVED*** Check violation details
        ...
        ***REMOVED*** Verify we have violations for the overlapping dates (15th and 16th)
        ...
        ***REMOVED*** Verify leave request exists and is marked as having conflicts
    ...
    def test_multiple_constraints_validation(self, db):
        ...
        """
        ***REMOVED*** SETUP: Create resident and faculty
        resident = Person(
            id=uuid4(),
        ...
        ***REMOVED*** Create multiple constraints
        preference = PreferenceConstraint(weight=5.0)
        preference.add_preference(resident.id, Session.AM)  ***REMOVED*** Prefers AM
        ...
        ***REMOVED*** Create constraint manager and add constraints
        manager = ConstraintManager()
        manager.add(preference)

        ***REMOVED*** ACTION: Validate all constraints
        result = manager.validate_all([resident_am, resident_pm], context)

        ***REMOVED*** ASSERT: Results aggregated correctly
        assert isinstance(result, ConstraintResult)
        ***REMOVED*** Should be satisfied (soft constraints don't block)
        assert result.satisfied is True
        ***REMOVED*** Should have penalty for PM assignment (violates preference)
        assert result.penalty > 0.0
        ***REMOVED*** Should have at least one violation (the PM preference)
        assert len(result.violations) > 0

        ***REMOVED*** Check that penalty is from preference constraint
