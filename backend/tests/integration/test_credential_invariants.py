...
***REMOVED*** ============================================================================
***REMOVED*** Slot-Type Invariant Catalog (Mock)
***REMOVED*** ============================================================================
...
def is_eligible(
    ...
    ***REMOVED*** Hard constraints - must pass all
    for req_cert_name in reqs.get("hard", []):
        ***REMOVED*** Find certification type
        cert_type = (
        ...
        ***REMOVED*** Check if person has this certification
        person_cert = (
    ...
    ***REMOVED*** If any hard constraint fails, not eligible
    ...
    ***REMOVED*** Soft constraints - accumulate penalties
    for soft in reqs.get("soft", []):
        if soft["name"] == "expiring_soon":
            ...
            ***REMOVED*** Check all required hard credentials for expiration
            for req_cert_name in reqs.get("hard", []):
                cert_type = (
                    db.query(CertificationType)
                    .filter(CertificationType.name == req_cert_name)
                ...
                if person_cert and person_cert.expiration_date <= window_end:
                    ...
                    break  ***REMOVED*** Only apply penalty once per slot
...
def predict_next_block_failures(db: Session, current_block_number: int) -> list[dict]:
    ...
    ***REMOVED*** Get all assignments in next block
    ...
    for assignment in next_block_assignments:
        ...
        ***REMOVED*** Get slot type from rotation template
        rotation = (
        ...
        ***REMOVED*** Use activity_type as slot_type (simplified)
        slot_type = rotation.activity_type

        ***REMOVED*** Check eligibility for block start date
        ...
        if not eligible:
            ***REMOVED*** Check if already in predictions
            existing = next(
...
def faculty_no_n95(db: Session, cert_types: dict) -> Person:
    ...
    ***REMOVED*** Add BLS and HIPAA certifications
...
def faculty_with_expiring_bls(db: Session, cert_types: dict) -> Person:
    ...
    ***REMOVED*** Add BLS expiring in 10 days
    ...
    ***REMOVED*** Add HIPAA (not expiring)
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 7.1: Hard Credential Requirement Enforcement
***REMOVED*** ============================================================================
...
class TestHardCredentialEnforcement:
    ...
    def test_hard_credential_requirement_all_present(
        ...
        ***REMOVED*** Add all required certifications for inpatient call
    ...
    def test_hard_credential_expired_blocks_assignment(
        ...
        ***REMOVED*** Add all certifications, but N95 is expired
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 7.2: Soft Credential Penalties
***REMOVED*** ============================================================================
...
class TestSoftCredentialPenalties:
    ...
    def test_soft_credential_no_penalty_outside_window(
        ...
        ***REMOVED*** BLS expires in 30 days (outside 14-day window)
        bls_cert = PersonCertification(
            id=uuid4(),
            person_id=faculty.id,
            certification_type_id=cert_types["BLS"].id,
    ...
    def test_soft_credential_penalty_multiple_slots(
        ...
        ***REMOVED*** BLS expires in 20 days
        ***REMOVED*** Should trigger penalty for procedures_half_day (30-day window, penalty=5)
        bls_cert = PersonCertification(
            id=uuid4(),
            person_id=faculty.id,
            certification_type_id=cert_types["BLS"].id,
        ...
        ***REMOVED*** Add other required certs
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 7.3: Credential Renewal During Assignment
***REMOVED*** ============================================================================
...
class TestCredentialRenewalDuringAssignment:
    ...
    def test_credential_renewal_removes_penalty(
        ...
        ***REMOVED*** ACTION: Faculty completes BLS renewal
        old_bls = (
            db.query(PersonCertification)
            .filter(
        ...
        ***REMOVED*** Update expiration date to 2 years in future
        old_bls.expiration_date = date.today() + timedelta(days=730)
        old_bls.status = "current"
        db.commit()

        ***REMOVED*** ASSERT: Verify penalty removed
    ...
    def test_renewal_before_expiration_proactive(
        ...
        ***REMOVED*** BLS expires in 60 days (proactively renewing)
        bls_cert = PersonCertification(
            id=uuid4(),
            person_id=faculty.id,
            certification_type_id=cert_types["BLS"].id,
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 7.4: Dashboard Hard Failure Prediction
***REMOVED*** ============================================================================
...
class TestDashboardHardFailurePrediction:
    ...
    def test_dashboard_hard_failure_prediction_next_block(
        ...
        """
        ***REMOVED*** SETUP
        ***REMOVED*** Create faculty with BLS expiring between blocks
        faculty = Person(
            id=uuid4(),
        ...
        ***REMOVED*** Current block ends Jan 15, next block starts Jan 22
        ***REMOVED*** BLS expires Jan 20 (between blocks)
        ...
        ***REMOVED*** Create blocks for next period
        block_11 = Block(
            id=uuid4(),
        ...
        ***REMOVED*** Create peds clinic rotation requiring BLS
        ...
        ***REMOVED*** Create assignment in block 11 requiring BLS
    ...
    def test_dashboard_no_failures_when_all_valid(self, db: Session, cert_types: dict):
        ...
        ***REMOVED*** All certifications valid for next block
        next_block_start = date(2025, 1, 22)
        ...
        for cert_name in ["BLS", "HIPAA"]:
            cert = PersonCertification(
                ...
                certification_type_id=cert_types[cert_name].id,
                issued_date=date(2024, 1, 1),
                expiration_date=date(2025, 12, 31),  ***REMOVED*** Valid all year
                status="current",
            )
            db.add(cert)
        db.commit()

        ***REMOVED*** Create block and assignment
        block_11 = Block(
            id=uuid4(),
            date=next_block_start,
            time_of_day="AM",
    ...
    def test_dashboard_multiple_faculty_predictions(
        ...
        ***REMOVED*** Create multiple faculty with expiring credentials
        faculty_list = []
        for i in range(3):
            faculty = Person(
                id=uuid4(),
        ...
        ***REMOVED*** Faculty 0: BLS expires before next block
        bls_cert = PersonCertification(
            id=uuid4(),
        ...
        ***REMOVED*** Faculty 1: HIPAA expires before next block
        hipaa_cert = PersonCertification(
            id=uuid4(),
        ...
        ***REMOVED*** Faculty 2: All valid (control)
        for cert_name in ["BLS", "HIPAA"]:
            cert = PersonCertification(
                id=uuid4(),
        ...
        ***REMOVED*** Create assignments for all faculty
        ...
        ***REMOVED*** Faculty 0 should have BLS failure
        fac0_pred = next(
        ...
        ***REMOVED*** Faculty 1 should have HIPAA failure
        fac1_pred = next(
        ...
        ***REMOVED*** Faculty 2 should not be in predictions
        fac2_pred = next(
