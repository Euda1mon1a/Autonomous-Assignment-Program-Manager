...
***REMOVED*** ============================================================================
***REMOVED*** Test Fixtures for Concurrent Scenarios
***REMOVED*** ============================================================================
...
def concurrent_edit_scenario(integration_db: Session, full_program_setup: dict):
    ...
    ***REMOVED*** Create an assignment that will be edited concurrently
...
def swap_scenario(integration_db: Session, full_program_setup: dict):
    ...
    ***REMOVED*** Create two assignments on different days
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 5.1: Concurrent Assignment Edit Tests
***REMOVED*** ============================================================================
...
class TestConcurrentAssignmentEdit:
    ...
    def test_concurrent_assignment_edit_with_separate_sessions(
        ...
        def edit_with_session_a():
            ...
            db = SessionLocal()
            try:
                ***REMOVED*** Fetch assignment
                assignment = db.query(Assignment).filter_by(id=assignment_id).first()

                ***REMOVED*** Simulate some processing time
                time.sleep(0.1)

                ***REMOVED*** Modify assignment
        ...
        def edit_with_session_b():
            ...
            try:
                ...
                ***REMOVED*** Modify assignment (different field)
        ...
        ***REMOVED*** Execute both edits concurrently
        ...
        ***REMOVED*** Both should succeed with SQLite (no row-level locking)
        ***REMOVED*** In PostgreSQL with proper optimistic locking, one would fail
        ***REMOVED*** This test documents the current behavior
        assert results["session_a_success"] is not None
        assert results["session_b_success"] is not None

        ***REMOVED*** Verify final state - last write wins in SQLite
        db = SessionLocal()
        try:
            assignment = db.query(Assignment).filter_by(id=assignment_id).first()
            ***REMOVED*** One of the changes should be present
    ...
    def test_concurrent_edit_with_explicit_locking(self, concurrent_edit_scenario):
        ...
        def edit_with_lock(user_name: str, delay: float):
            ...
            db = SessionLocal()
            try:
                ***REMOVED*** Acquire row-level lock (PostgreSQL: FOR UPDATE, SQLite: immediate lock)
                assignment = (
                    db.query(Assignment)
        ...
        ***REMOVED*** Execute edits concurrently
        ...
        thread_a.start()
        time.sleep(0.05)  ***REMOVED*** Ensure A starts first
        thread_b.start()

        thread_a.join(timeout=2.0)
        thread_b.join(timeout=2.0)

        ***REMOVED*** Both should complete
        assert len(results["update_order"]) == 2

        ***REMOVED*** Verify sequential execution (User A should finish first due to earlier lock)
        ***REMOVED*** Note: In SQLite this may behave differently than PostgreSQL
        db = SessionLocal()
        try:
            assignment = db.query(Assignment).filter_by(id=assignment_id).first()
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 5.2: Swap During Schedule Generation Tests
***REMOVED*** ============================================================================
...
class TestSwapDuringGeneration:
    ...
    async def test_swap_during_schedule_generation(self, swap_scenario):
        ...
        def simulate_schedule_generation():
            ...
            time.sleep(0.5)  ***REMOVED*** Simulate generation taking 500ms
        ...
        def attempt_swap():
            ...
            ***REMOVED*** Wait for generation to start
            ...
            try:
                ...
                ***REMOVED*** In real system, swap executor would check for generation lock
                ***REMOVED*** For now, we just verify swap can be created
...
***REMOVED*** ============================================================================
***REMOVED*** Frame 5.3: Task Cancellation Tests
***REMOVED*** ============================================================================
...
class TestTaskCancellation:
    ...
    async def test_task_cancellation_mid_execution(self, integration_db):
        ...
        def long_running_task():
            ...
            try:
                ***REMOVED*** Simulate creating assignments in batches
                for i in range(10):
                    if cancellation_flag.is_set():
                        ***REMOVED*** Cancellation requested - rollback and exit
                        db.rollback()
                        task_state["cleanup_done"] = True
                        return

                    ***REMOVED*** Create assignment (simulating work)
                    ***REMOVED*** In real scenario, this would be actual assignment creation
                    task_state["assignments_created"] += 1
                    time.sleep(0.1)  ***REMOVED*** Simulate work

                ***REMOVED*** If we got here, task completed normally
                db.commit()
        ...
        ***REMOVED*** Wait a bit, then cancel
        time.sleep(0.25)  ***REMOVED*** Let it create ~2-3 assignments
        cancellation_flag.set()

        ***REMOVED*** Wait for task to acknowledge cancellation
        task_thread.join(timeout=2.0)

        ***REMOVED*** Verify task stopped early
    ...
    async def test_task_cancellation_with_asyncio(self):
        ...
        ***REMOVED*** Wait for cancellation to propagate
        with pytest.raises(asyncio.CancelledError):
            await task

        ***REMOVED*** Verify task was cancelled mid-execution
        assert len(progress) < 10
        assert progress[-1] == "cancelled"


***REMOVED*** ============================================================================
***REMOVED*** Frame 5.4: Swap Race Condition Tests
***REMOVED*** ============================================================================
...
class TestSwapRaceCondition:
    ...
    def test_swap_race_condition_two_faculty_want_same_shift(
        ...
        faculty_a = setup["faculty"][0]
        faculty_b = setup["faculty"][1]
        faculty_c = setup["residents"][0]  ***REMOVED*** Use resident as third party
        template = setup["templates"][0]

        ***REMOVED*** Create assignments
        ***REMOVED*** Faculty A has shift 1
        ***REMOVED*** Faculty C has shift 1 (same as A - conflict scenario)
        ***REMOVED*** Faculty B has shift 2
        ...
        assignment_c = Assignment(
            ...
            rotation_template_id=template.id,
            role="backup",  ***REMOVED*** Different role to avoid unique constraint
        )
        ...
        def create_swap_a_to_b():
            ...
            try:
                ...
                ***REMOVED*** Simulate swap execution with small delay
                time.sleep(0.05)

                ***REMOVED*** Mark as executed
        ...
        def create_swap_c_to_b():
            ...
            db = SessionLocal()
            try:
                ***REMOVED*** Small delay to ensure A starts first
                time.sleep(0.02)
                ...
                ***REMOVED*** Try to execute
                time.sleep(0.05)

                ***REMOVED*** In real system, this should fail because B's assignment already swapped
                ***REMOVED*** For now, both will succeed in creating requests
                swap.status = SwapStatus.EXECUTED
                swap.executed_at = datetime.utcnow()
                db.commit()
        ...
        ***REMOVED*** Execute both swap requests concurrently
        ...
        ***REMOVED*** Both should create requests (real swap executor would handle conflicts)
        ***REMOVED*** This test documents current behavior and serves as basis for improvements
    ...
    def test_swap_auto_matcher_prevents_double_booking(
        ...
        def create_first_swap():
            ...
            try:
                ...
                ***REMOVED*** Hold lock for a moment
                time.sleep(0.2)
        ...
        def create_second_swap():
            ...
            ***REMOVED*** Wait a bit to ensure first swap starts
            time.sleep(0.05)

            db = SessionLocal()
            try:
                ***REMOVED*** Try to lock the same assignment
                assignment = (
                    db.query(Assignment)
                    .filter_by(id=assignment_a.id)
                    .with_for_update(nowait=False)  ***REMOVED*** Will wait for lock
                    .first()
                )

                ***REMOVED*** If we get here, first swap already committed
                ***REMOVED*** Check if assignment is already involved in a swap
                swap = SwapRecord(
                    id=uuid4(),
                    source_faculty_id=faculty_a.id,
                    source_week=assignment.block.date,
        ...
        ***REMOVED*** Execute both concurrently
        ...
        ***REMOVED*** First should always succeed
        assert results["first_swap"] == "success"

        ***REMOVED*** Second may succeed or fail depending on timing (both are valid behaviors)
        ***REMOVED*** Real swap executor would add additional validation
        assert results["second_swap"] is not None


***REMOVED*** ============================================================================
***REMOVED*** Additional Edge Cases
***REMOVED*** ============================================================================
...
class TestConcurrentEdgeCases:
    ...
    def test_concurrent_read_during_write(self, concurrent_edit_scenario):
        ...
        def write_assignment():
            ...
            try:
                assignment = db.query(Assignment).filter_by(id=assignment_id).first()
                time.sleep(0.1)  ***REMOVED*** Simulate slow update
        ...
        def read_assignment():
            ...
            time.sleep(0.05)  ***REMOVED*** Start read during write
            db = SessionLocal()
            try:
                assignment = db.query(Assignment).filter_by(id=assignment_id).first()
        ...
        ***REMOVED*** Read should see either None (original) or "Updated value" (new)
        ***REMOVED*** Never partial state
    ...
    def test_deadlock_prevention(self, integration_db, full_program_setup):
        ...
        def transaction_2():
            ...
            time.sleep(0.05)  ***REMOVED*** Start slightly after thread 1
            db = SessionLocal()
            try:
                a1 = (
                    db.query(Assignment)
        ...
        ***REMOVED*** Both should complete (one waits for the other)
