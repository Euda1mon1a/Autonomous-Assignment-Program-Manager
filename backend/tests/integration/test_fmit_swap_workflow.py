...
class TestFMITSwapWorkflowBasic:
    ...
    def create_fmit_excel(self, providers_schedule: dict[str, list[int]]) -> bytes:
        ...
        ***REMOVED*** Header row - Provider and date columns
        ws["A1"] = "Provider"
        base_date = date(2025, 3, 3)  ***REMOVED*** Monday
        max_week = (
    ...
    def test_full_workflow_upload_and_find_candidates(
        ...
        ***REMOVED*** Create FMIT schedule with 3 providers
        ***REMOVED*** Dr. Smith: weeks 1, 3, 5 (alternating pattern)
        ***REMOVED*** Dr. Jones: weeks 2, 6 (no pattern)
        ***REMOVED*** Dr. Lee: week 4 only (under target)
        schedule = {
            "Dr. Smith": [1, 3, 5],
            "Dr. Jones": [2, 6],
            "Dr. Lee": [4],
        ...
        ***REMOVED*** Request to find candidates for Dr. Smith's week 1
        request_data = {
        ...
        ***REMOVED*** Should have 2 candidates (Jones and Lee)
    ...
    def test_swap_candidates_include_constraint_flags(
        ...
        ***REMOVED*** Create schedule where taking a week would create back-to-back
        schedule = {
            "Dr. Smith": [1, 3],
            "Dr. Jones": [2],  ***REMOVED*** Week 2 - taking week 1 or 3 would be back-to-back
        }
        excel_bytes = self.create_fmit_excel(schedule)

        request_data = {
            "target_faculty": "Dr. Smith",
            "target_week": "2025-03-03",  ***REMOVED*** Week 1
            "include_absence_conflicts": False,
        }
        ...
        ***REMOVED*** Find Dr. Jones in candidates
        jones = next(
            (c for c in data["candidates"] if c["faculty"] == "Dr. Jones"), None
        )
        assert jones is not None

        ***REMOVED*** Jones has week 2, taking week 1 would be back-to-back
        assert jones["back_to_back_ok"] is False
        ***REMOVED*** Verify reason is properly set
    ...
    def test_alternating_pattern_detection(
        ...
        ***REMOVED*** Create schedule with alternating pattern
        schedule = {
            "Dr. Smith": [1, 3, 5, 7],  ***REMOVED*** Alternating pattern - 3 cycles
            "Dr. Jones": [2, 4],
            "Dr. Lee": [6],
        }
        excel_bytes = self.create_fmit_excel(schedule)
        ...
        ***REMOVED*** Check alternating patterns are reported
        assert "alternating_patterns" in data

        ***REMOVED*** Dr. Smith should be flagged for excessive alternating
...
class TestFMITSwapWorkflowWithConflicts:
    ...
    def test_external_conflict_flagging(
        ...
        ***REMOVED*** Add external conflict for Dr. Jones during week 1
        request_data = {
            "target_faculty": "Dr. Smith",
            "target_week": "2025-03-03",  ***REMOVED*** Week 1
            "include_absence_conflicts": False,
    ...
    def test_absence_integration(
        ...
        ***REMOVED*** Create blocking absence during week 1
        absence = Absence(
            id=uuid4(),
        ...
        ***REMOVED*** Create schedule including this faculty
        schedule = {
            "Dr. Smith": [1],
        ...
        request_data = {
            "target_faculty": "Dr. Smith",
            "target_week": "2025-03-03",
            "include_absence_conflicts": True,  ***REMOVED*** Enable absence integration
        }
        ...
        ***REMOVED*** Dr. Database Faculty should have TDY conflict flagged
        ***REMOVED*** Note: This test depends on database absence integration working correctly
        ***REMOVED*** The absence may not be matched if the faculty name doesn't match exactly
        db_faculty = next(
            (c for c in data["candidates"] if c["faculty"] == "Dr. Database Faculty"),
            None,
        )
        ***REMOVED*** The test creates an absence but integration may require exact name matching
        ***REMOVED*** We just verify the candidate was returned
    ...
    def test_multiple_providers_different_patterns(
        ...
        ***REMOVED*** Create diverse schedule:
        ***REMOVED*** - Provider A: Heavy load (weeks 1,2,3,4,5)
        ***REMOVED*** - Provider B: Alternating (weeks 1,3,5)
        ***REMOVED*** - Provider C: Light load (week 6 only)
        ***REMOVED*** - Provider D: Moderate (weeks 2,4)
        schedule = {
            "Dr. Heavy": [1, 2, 3, 4, 5],
        ...
        ***REMOVED*** Should find Light and Moderate as candidates (Alternating also has week 1)
        assert data["total_candidates"] >= 2

        ***REMOVED*** Light should be viable (no back-to-back issues)
        light = next(
...
class TestFMITSwapWorkflowErrorHandling:
    ...
    def test_missing_file_error(
        ...
        ***REMOVED*** Should error with 422 for missing required field
    ...
    def test_invalid_date_format_error(
        ...
        ***REMOVED*** Should error with validation error
            ...
            "external_conflicts": [
                {
                    ...
                    "end_date": "2025-03-01",  ***REMOVED*** Before start_date!
                    "conflict_type": "leave",
...
class TestFMITSwapWorkflowRanking:
    ...
    def test_candidates_with_faculty_targets(
        ...
        schedule = {
            "Dr. Overload": [1, 2, 3, 4, 5, 6],  ***REMOVED*** 6 weeks
            "Dr. Underload": [1],  ***REMOVED*** 1 week
            "Dr. Target": [2, 3, 4],  ***REMOVED*** 3 weeks
        }
        excel_bytes = self.create_fmit_excel(schedule)

        ***REMOVED*** Set targets: everyone should have 4 weeks
        request_data = {
        ...
        ***REMOVED*** Should have candidates
        assert data["total_candidates"] >= 1

        ***REMOVED*** Underload should be in candidates (ranking based on targets)
        underload = next(
            (c for c in data["candidates"] if c["faculty"] == "Dr. Underload"), None
        )
        ***REMOVED*** Verify underload is a candidate (actual gap calculation may vary by implementation)
    ...
    def test_viable_candidates_count(
        ...
        schedule = {
            "Dr. Smith": [1, 3],
            "Dr. BackToBack": [2],  ***REMOVED*** Would create back-to-back with week 1 or 3
            "Dr. Viable": [5],  ***REMOVED*** No back-to-back issue
        }
        excel_bytes = self.create_fmit_excel(schedule)
        ...
        ***REMOVED*** Should have 2 total candidates
        assert data["total_candidates"] == 2

        ***REMOVED*** But only 1 viable (Dr. Viable)
        ***REMOVED*** Note: This assertion might need adjustment based on actual implementation
        ***REMOVED*** The API might count differently
