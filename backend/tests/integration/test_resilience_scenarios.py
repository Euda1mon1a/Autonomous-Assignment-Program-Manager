...
def pals_certified_faculty(db: Session) -> Person:
    ...
    ***REMOVED*** Add PALS credential (requires certification type first)
...
def peds_clinic_blocks(db: Session) -> list[Block]:
    ...
    for i in range(4):
        block = Block(
            ...
            is_weekend=False,
            is_holiday=False,
            requires_credential="PALS",  ***REMOVED*** Critical: requires PALS
        )
        db.add(block)
        blocks.append(block)
...
def balanced_blocks(db: Session) -> list[Block]:
    ...
    blocks = []
    start_date = date(2025, 1, 13)  ***REMOVED*** Monday
...
def balanced_assignments(
    ...
    ***REMOVED*** Assign first 14 blocks round-robin to 8 faculty
...
***REMOVED*** ============================================================================
***REMOVED*** Test 1: N-1 Single Point of Failure (Frame 4.1)
***REMOVED*** ============================================================================
...
def test_n1_single_point_of_failure(
    ...
    """
    ***REMOVED*** SETUP: Create assignments requiring PALS
    all_faculty = [pals_certified_faculty] + faculty_without_pals
    pals_assignments = []

    for block in peds_clinic_blocks:
        ***REMOVED*** Only PALS-certified faculty can cover these blocks
        assignment = Assignment(
            id=uuid4(),
            block_id=block.id,
    ...
    ***REMOVED*** Coverage requirements: each peds clinic block needs 1 PALS-certified faculty
    coverage_requirements = {block.id: 1 for block in peds_clinic_blocks}

    ***REMOVED*** ACTION: Run N-1 analysis
    ...
    ***REMOVED*** ASSERT: Critical vulnerability detected
    assert len(vulnerabilities) > 0, "Should identify at least one vulnerability"

    ***REMOVED*** Find the PALS expert in vulnerabilities
    ...
    ***REMOVED*** Verify mitigation recommendations
    ...
    ***REMOVED*** Check for cross-training recommendation
...
***REMOVED*** ============================================================================
***REMOVED*** Test 2: N-2 Cascading Failure (Frame 4.2)
***REMOVED*** ============================================================================
...
def test_n2_cascading_failure(
    ...
    """
    ***REMOVED*** SETUP: We have 8 faculty at 87.5% utilization (balanced_assignments fixture)
    ***REMOVED*** Each faculty works ~70 hours/week (87.5% of 80-hour max)

    coverage_requirements = {block.id: 1 for block in balanced_blocks}

    ***REMOVED*** ACTION: Run N-2 analysis
    fatal_pairs = contingency_analyzer.analyze_n2(
        ...
        coverage_requirements=coverage_requirements,
        critical_faculty_only=False,  ***REMOVED*** Check all pairs for comprehensive analysis
    )

    ***REMOVED*** ASSERT: Some fatal pairs should be detected
    ***REMOVED*** With 8 faculty at 87.5% utilization, losing 2 (25% capacity) should cause issues
    assert len(fatal_pairs) > 0, (
        "Should identify fatal pairs in high-utilization scenario"
    )

    ***REMOVED*** ACTION: Simulate cascade failure with loss of 2 faculty
    failed_faculty_ids = [
    ...
    cascade = contingency_analyzer.simulate_cascade_failure(
        ...
        max_utilization=0.80,  ***REMOVED*** 80% threshold
        overload_threshold=1.2,  ***REMOVED*** 120% triggers cascade
    )

    ***REMOVED*** ASSERT: Cascade simulation results
    assert cascade.total_failures >= 2, (
        f"Should have at least 2 failures (initial), got {cascade.total_failures}"
    )

    ***REMOVED*** Loss of 2/8 faculty (25%) at 87.5% utilization means remaining 6 must absorb load
    ***REMOVED*** Original load: 8 faculty × 0.875 = 7 faculty-equivalents of work
    ***REMOVED*** Remaining capacity: 6 faculty
    ***REMOVED*** New utilization: 7/6 = 116.7%, which exceeds 80% safe threshold
    ***REMOVED*** This should trigger cascading failures or critical coverage issues

    ***REMOVED*** Verify final coverage is reduced
    assert cascade.final_coverage < 1.0, (
        "Final coverage should be reduced after cascade"
    )

    ***REMOVED*** Generate full report with phase transition detection
    report = contingency_analyzer.generate_report(
        faculty=balanced_faculty_pool,
        blocks=balanced_blocks,
        assignments=balanced_assignments,
        coverage_requirements=coverage_requirements,
        current_utilization=0.875,  ***REMOVED*** 87.5% baseline
        recent_changes=[],
    )

    ***REMOVED*** ASSERT: Report shows elevated risk
    ...
    ***REMOVED*** Should recommend mitigation strategies
    assert len(report.recommended_actions) > 0, (
        "Should provide mitigation recommendations"
    )

    ***REMOVED*** Check for sacrifice hierarchy or load shedding recommendations
    sacrifice_recommendations = [
        rec
        for rec in report.recommended_actions
        if "sacrifice" in rec.lower() or "reduce load" in rec.lower()
    ]
    ***REMOVED*** Note: May or may not recommend sacrifice depending on specific scenario


***REMOVED*** ============================================================================
***REMOVED*** Test 3: Utilization Threshold Transitions (Frame 4.3)
***REMOVED*** ============================================================================
...
def test_utilization_threshold_transitions(
    ...
    """
    ***REMOVED*** SETUP: Create faculty member (80-hour max per week)
    faculty = Person(
        id=uuid4(),
    ...
    ***REMOVED*** Test scenarios: (hours_worked, utilization_pct, expected_level, description)
    test_scenarios = [
    ...
    for hours_worked, expected_util_pct, expected_level, description in test_scenarios:
        ***REMOVED*** ACTION: Calculate utilization
        ***REMOVED*** Each "block" represents work hours
        ***REMOVED*** If blocks_per_faculty_per_day = 1 and days_in_period = 1,
        ***REMOVED*** then required_blocks directly maps to hours
        metrics = utilization_monitor.calculate_utilization(
            available_faculty=[faculty],
            required_blocks=int(hours_worked),  ***REMOVED*** Work hours needed
            blocks_per_faculty_per_day=80,  ***REMOVED*** Max 80 hours per week
            days_in_period=1,
        )

        ***REMOVED*** ASSERT: Utilization rate matches expected
        ...
        ***REMOVED*** ASSERT: Defense level matches expected
    ...
    ***REMOVED*** VERIFY: 80% threshold is the critical transition point
    ***REMOVED*** Test boundary: 79.999% should be GREEN, 80.001% should be YELLOW
    boundary_test_cases = [
    ...
    for hours, expected_level, desc in boundary_test_cases:
        ...
        ***REMOVED*** 80% threshold should trigger YELLOW or higher
...
***REMOVED*** ============================================================================
***REMOVED*** Test 4: Defense Level Escalation (Frame 4.4)
***REMOVED*** ============================================================================
...
def test_defense_level_escalation(
    ...
    """
    ***REMOVED*** Test escalation scenarios: (coverage_rate, expected_level, description)
    ...
    for coverage_rate, expected_level, description in escalation_scenarios:
        ***REMOVED*** ACTION: Get recommended defense level
        recommended_level = defense_in_depth.get_recommended_level(coverage_rate)

        ***REMOVED*** ASSERT: Recommended level matches expected
        assert recommended_level == expected_level, (
            f"{description}: Expected {expected_level.name}, "
            f"got {recommended_level.name}"
        )

    ***REMOVED*** Test escalation logic
    ***REMOVED*** Start at GREEN (prevention), escalate to RED (containment)
    escalation_sequence = [
    ...
    for coverage, expected, desc in escalation_sequence:
        ...
        ***REMOVED*** Verify escalation (level value increases)
    ...
    ***REMOVED*** Test de-escalation logic
    ...
    for coverage, expected, desc in de_escalation_sequence:
        ...
        ***REMOVED*** Verify de-escalation (level value decreases)
        if previous_level is not None:
            assert current_level.value <= previous_level.value, (
    ...
    ***REMOVED*** Test action activation and tracking
    ***REMOVED*** Activate actions at each defense level
    levels_to_test = [
    ...
    ***REMOVED*** Verify all levels have actions
...
***REMOVED*** ============================================================================
***REMOVED*** Test 5: Edge Cases and Integration
***REMOVED*** ============================================================================
...
def test_utilization_exactly_at_threshold(
    ...
    ***REMOVED*** Exactly 80.0% utilization (64 hours out of 80)
    metrics = utilization_monitor.calculate_utilization(
        available_faculty=[faculty],
        required_blocks=64,
        blocks_per_faculty_per_day=80,
    ...
    ***REMOVED*** At exactly 80%, should be YELLOW (warning threshold)
...
def test_redundancy_check_n_plus_2_rule(
    ...
    """
    ***REMOVED*** Test scenarios: (available, minimum_required, expected_status)
    ...
    for available_count, minimum, expected_status, description in redundancy_scenarios:
        ***REMOVED*** Create mock providers
        providers = [f"Provider_{i}" for i in range(available_count)]

        ***REMOVED*** ACTION: Check redundancy
        ...
        ***REMOVED*** ASSERT: Status matches expected
        assert status.status == expected_status, (
            f"{description}: Expected status '{expected_status}', got '{status.status}'"
        )

        ***REMOVED*** ASSERT: Redundancy level calculated correctly
...
def test_cascade_simulation_with_no_failures(
    ...
    """
    ***REMOVED*** ACTION: Simulate loss of 1 faculty at low utilization
    initial_failure = [balanced_faculty_pool[0].id]

    cascade = contingency_analyzer.simulate_cascade_failure(
        faculty=balanced_faculty_pool,
        blocks=balanced_blocks,
        assignments=balanced_assignments[:8],  ***REMOVED*** Only 8 assignments for low utilization
        initial_failures=initial_failure,
        max_utilization=0.80,
        overload_threshold=1.2,
    )

    ***REMOVED*** ASSERT: Should be stable (no cascading failures)
...
***REMOVED*** ============================================================================
***REMOVED*** Test 6: Comprehensive Vulnerability Report
***REMOVED*** ============================================================================
...
def test_comprehensive_vulnerability_report(
    ...
    """
    ***REMOVED*** SETUP: Create PALS assignments (single point of failure scenario)
    all_faculty = [pals_certified_faculty] + faculty_without_pals
    pals_assignments = []
    ...
    ***REMOVED*** ACTION: Generate comprehensive report
    report = contingency_analyzer.generate_report(
        faculty=all_faculty,
        blocks=peds_clinic_blocks,
        assignments=pals_assignments,
        coverage_requirements=coverage_requirements,
        current_utilization=0.85,  ***REMOVED*** High utilization
        recent_changes=[
    ...
    ***REMOVED*** ASSERT: Most critical faculty identified
