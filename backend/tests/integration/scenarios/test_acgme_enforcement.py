...
class TestACGMEEnforcementScenarios:
    ...
    def test_prevent_80_hour_violation_scenario(
        ...
        ***REMOVED*** Create near-maximum workload
        for i in range(6):
            for tod in ["AM", "PM"]:
                block = Block(
                    id=uuid4(),
        ...
        ***REMOVED*** Try to add assignment that would violate 80-hour rule
    ...
    def test_enforce_mandatory_day_off_scenario(
        ...
        ***REMOVED*** Create 7 consecutive days of assignments
        for i in range(7):
            for tod in ["AM", "PM"]:
                block = Block(
                    id=uuid4(),
        ...
        ***REMOVED*** Try to add 8th day (should violate 1-in-7)
        ...
        ***REMOVED*** Should reject or flag violation
        assert response.status_code in [200, 201, 400, 422]
    ...
    def test_supervision_ratio_enforcement_scenario(
        ...
        ***REMOVED*** Create supervised rotation
        template = RotationTemplate(
            ...
            max_supervision_ratio=2,  ***REMOVED*** 1:2
        )
        db.add(template)
        db.commit()
        ...
        ***REMOVED*** Try to assign 3 residents without faculty
        for i in range(3):
            response = client.post(
                "/api/assignments/",
                json={
        ...
        ***REMOVED*** Should detect violation
