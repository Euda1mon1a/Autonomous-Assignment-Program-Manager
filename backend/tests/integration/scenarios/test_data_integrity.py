...
class TestDataIntegrityScenarios:
    ...
    def test_referential_integrity_scenario(
        ...
        ***REMOVED*** Try to delete person with assignments
        response = client.delete(
            f"/api/people/{sample_assignment.person_id}",
            headers=auth_headers,
        )

        ***REMOVED*** Should prevent or cascade properly
        if response.status_code in [200, 204]:
            ***REMOVED*** Verify assignments handled
            assignment_check = (
                db.query(Assignment)
                .filter(Assignment.id == sample_assignment.id)
                .first()
            )
            ***REMOVED*** Should be deleted or orphaned properly
    ...
    def test_unique_constraint_scenario(
        ...
        ***REMOVED*** Try to create duplicate email
        response2 = client.post(
            "/api/people/",
            json={
                "name": "Dr. Duplicate",
                "type": "resident",
                "email": "unique@test.org",  ***REMOVED*** Same email
                "pgy_level": 1,
            },
            headers=auth_headers,
        )

        ***REMOVED*** Should reject duplicate
    ...
    def test_no_orphaned_records_scenario(
        ...
        ***REMOVED*** Delete block
        ...
        if response.status_code in [200, 204]:
            ***REMOVED*** Verify no orphaned assignments
            orphaned = (
