...
***REMOVED*** Mock SeismicSIRBridge class for testing (will be implemented)
class SeismicSIRBridge:
    ...
    def update_from_alerts(self, alerts: list[SeismicAlert]) -> dict:
        ...
        ***REMOVED*** Apply smoothing (exponential moving average)
        self._beta_smoothed = (
...
def network():
    ...
    ***REMOVED*** Create network with varying connectivity
    ***REMOVED*** Resident 0: hub (degree 5)
    for i in range(1, 6):
        G.add_edge(residents[0], residents[i])

    ***REMOVED*** Resident 1: medium connectivity (degree 3)
    G.add_edge(residents[1], residents[6])
    G.add_edge(residents[1], residents[7])

    ***REMOVED*** Remaining: low connectivity (degree 1-2)
    for i in range(2, 9):
...
class TestSTALTATransformation:
    ...
    def test_transform_below_threshold(self, bridge):
        ...
        ***REMOVED*** Test values below threshold
        ...
        ***REMOVED*** Boundary: exactly at threshold should still trigger
    ...
    def test_transform_moderate_range(self, bridge):
        ...
        ***REMOVED*** f(r) = log₂(r / 2.5)

        ***REMOVED*** At r=5.0: log₂(5.0 / 2.5) = log₂(2) = 1.0
        assert bridge._transform_sta_lta(5.0) == pytest.approx(1.0, rel=0.01)

        ***REMOVED*** At r=10.0: log₂(10.0 / 2.5) = log₂(4) = 2.0
        assert bridge._transform_sta_lta(10.0) == pytest.approx(2.0, rel=0.01)

        ***REMOVED*** At r=7.5: log₂(7.5 / 2.5) = log₂(3) ≈ 1.585
    ...
    def test_transform_critical_range(self, bridge):
        ...
        ***REMOVED*** f(r) = log₂(4) + 0.5 × (r - 10)

        ***REMOVED*** At r=10.0: log₂(4) + 0.5 × 0 = 2.0
        assert bridge._transform_sta_lta(10.0) == pytest.approx(2.0, rel=0.01)

        ***REMOVED*** At r=15.0: log₂(4) + 0.5 × 5 = 2.0 + 2.5 = 4.5
        assert bridge._transform_sta_lta(15.0) == pytest.approx(4.5, rel=0.01)

        ***REMOVED*** At r=20.0: log₂(4) + 0.5 × 10 = 2.0 + 5.0 = 7.0
        assert bridge._transform_sta_lta(20.0) == pytest.approx(7.0, rel=0.01)

        ***REMOVED*** Linear increment: should increase by 0.5 per unit
    ...
    def test_transform_normal_elevation(self, bridge):
        ...
        ***REMOVED*** Typical moderate alert: ratio=3.5
        ***REMOVED*** f(3.5) = log₂(3.5 / 2.5) = log₂(1.4) ≈ 0.485
        result = bridge._transform_sta_lta(3.5)
        expected = np.log2(3.5 / 2.5)
        assert result == pytest.approx(expected, abs=0.01)
        assert 0.4 < result < 0.6  ***REMOVED*** Sanity check

    def test_transform_elevated_precursor(self, bridge):
        """Test transformation for elevated STA/LTA ratio."""
        ***REMOVED*** Strong precursor signal: ratio=8.0
        ***REMOVED*** f(8.0) = log₂(8.0 / 2.5) = log₂(3.2) ≈ 1.678
    ...
    def test_transform_critical_precursor(self, bridge):
        ...
        ***REMOVED*** Critical signal: ratio=18.0
        ***REMOVED*** f(18.0) = log₂(4) + 0.5 × (18 - 10) = 2.0 + 4.0 = 6.0
...
class TestMultiResidentAggregation:
    ...
    def test_single_alert_aggregation(self, bridge, network):
        ...
        alert = SeismicAlert(
            ...
            sta_lta_ratio=5.0,  ***REMOVED*** f(5.0) = 1.0
        ...
        ***REMOVED*** With single alert, weight doesn't matter (normalized to 1)
        ***REMOVED*** delta = f(5.0) = 1.0
    ...
    def test_degree_weighted_averaging(self, bridge, network):
        ...
        ***REMOVED*** Resident 0: hub with degree=5, ratio=8.0 → f(8.0) ≈ 1.678
        ***REMOVED*** Resident 9: low degree=1, ratio=4.0 → f(4.0) ≈ 0.678
        alert_hub = SeismicAlert(
            ...
            resident_id=residents[0],  ***REMOVED*** High degree node
        )
        ...
        alert_low = SeismicAlert(
            ...
            resident_id=residents[9],  ***REMOVED*** Low degree node
        )
        ...
        ***REMOVED*** Weighted average: (w_hub × f_hub + w_low × f_low) / (w_hub + w_low)
        ***REMOVED*** w_hub = degree(0) = 5, w_low = degree(9) = 1
        ...
        ***REMOVED*** Hub's signal should dominate (higher weight)
        assert delta > f_low
        assert delta < f_hub  ***REMOVED*** But not as high as hub alone
    ...
    def test_three_residents_different_degrees(self, bridge, network):
        ...
        ***REMOVED*** Create alerts for residents with different degrees
        alerts = [
            SeismicAlert(
                signal_type=PrecursorSignal.SWAP_REQUESTS,
                sta_lta_ratio=6.0,  ***REMOVED*** f(6.0) = log₂(2.4) ≈ 1.263
                trigger_time=datetime.now(),
                severity="medium",
                predicted_magnitude=4.5,
                resident_id=residents[0],  ***REMOVED*** degree=5
            ),
            SeismicAlert(
                signal_type=PrecursorSignal.SICK_CALLS,
                sta_lta_ratio=5.0,  ***REMOVED*** f(5.0) = 1.0
                ...
                severity="medium",
                predicted_magnitude=4.0,
                resident_id=residents[1],  ***REMOVED*** degree=3
            ),
            SeismicAlert(
                signal_type=PrecursorSignal.RESPONSE_DELAYS,
                sta_lta_ratio=4.0,  ***REMOVED*** f(4.0) ≈ 0.678
                trigger_time=datetime.now(),
                severity="low",
                predicted_magnitude=3.0,
                resident_id=residents[5],  ***REMOVED*** degree=1
            ),
        ]
        ...
        ***REMOVED*** Calculate expected weighted average
...
class TestSmoothingPreventsOscillation:
    ...
    def test_smoothing_gradual_increase(self, bridge, network):
        ...
        ***REMOVED*** First update: moderate alert
        alert1 = SeismicAlert(
            signal_type=PrecursorSignal.SWAP_REQUESTS,
            sta_lta_ratio=5.0,  ***REMOVED*** f(5.0) = 1.0
            trigger_time=datetime.now(),
        ...
        ***REMOVED*** Beta should increase from baseline
        assert beta_after_first > bridge.beta_base

        ***REMOVED*** Second update: higher alert (should increase further but gradually)
        alert2 = SeismicAlert(
            signal_type=PrecursorSignal.SICK_CALLS,
            sta_lta_ratio=10.0,  ***REMOVED*** f(10.0) = 2.0
            trigger_time=datetime.now(),
            severity="high",
        ...
        ***REMOVED*** Beta should increase but not jump to maximum
        assert beta_after_second > beta_after_first

        ***REMOVED*** With smoothing_factor=0.2, change should be gradual
        ***REMOVED*** The increase should be less than if no smoothing applied
    ...
    def test_smoothing_factor_effect(self, seismic_detector, epi_model):
        ...
        ***REMOVED*** Create two bridges with different smoothing factors
        bridge_fast = SeismicSIRBridge(
            seismic_detector=seismic_detector,
            epidemiology_model=epi_model,
            sensitivity=0.3,
            smoothing_factor=0.8,  ***REMOVED*** Fast response (80% weight on new)
            beta_base=0.05,
        )
        ...
        bridge_slow = SeismicSIRBridge(
            ...
            epidemiology_model=epi_model,
            sensitivity=0.3,
            smoothing_factor=0.1,  ***REMOVED*** Slow response (10% weight on new)
            beta_base=0.05,
        )

        ***REMOVED*** Same alert to both
        alert = SeismicAlert(
            signal_type=PrecursorSignal.SWAP_REQUESTS,
            sta_lta_ratio=8.0,
            trigger_time=datetime.now(),
        ...
        ***REMOVED*** Fast bridge should respond more strongly
        assert result_fast["beta_adjusted"] > result_slow["beta_adjusted"]

        ***REMOVED*** Both should be above baseline
...
class TestBetaClamping:
    ...
    def test_beta_clamped_to_maximum(self, bridge, network):
        ...
        ***REMOVED*** Create extreme alerts that would push beta > 0.95
        extreme_alerts = [
            SeismicAlert(
                signal_type=PrecursorSignal.SICK_CALLS,
                sta_lta_ratio=50.0,  ***REMOVED*** Extremely high
                trigger_time=datetime.now(),
                severity="critical",
        ...
        ***REMOVED*** Beta should be clamped to max
    ...
    def test_beta_clamped_to_minimum(self, seismic_detector, epi_model):
        ...
        ***REMOVED*** Create bridge with very low baseline
        bridge = SeismicSIRBridge(
            seismic_detector=seismic_detector,
            epidemiology_model=epi_model,
            sensitivity=0.0,  ***REMOVED*** No sensitivity to alerts
            smoothing_factor=0.2,
            beta_base=0.01,  ***REMOVED*** At minimum
        )

        ***REMOVED*** Even with no alerts, beta should stay at minimum
    ...
    def test_multiple_updates_stay_in_bounds(self, bridge, network):
        ...
        ***REMOVED*** Simulate multiple update cycles with varying intensities
        for i in range(10):
            ratio = 3.0 + i * 2.0  ***REMOVED*** Increasing from 3 to 21
            alert = SeismicAlert(
                signal_type=PrecursorSignal.SWAP_REQUESTS,
                sta_lta_ratio=ratio,
                trigger_time=datetime.now(),
    ...
    def test_beta_calculation_components(self, bridge, network):
        ...
        ***REMOVED*** Verify formula: β_current = β_base × (1 + α × beta_delta)
        ***REMOVED*** beta_delta = f(5.0) = 1.0 (with single alert, no weighting needed)
        ***REMOVED*** β_current = 0.05 × (1 + 0.3 × 1.0) = 0.05 × 1.3 = 0.065

        ***REMOVED*** With smoothing (first update): β_smoothed = γ × β_current + (1-γ) × β_base
        ***REMOVED*** β_smoothed = 0.2 × 0.065 + 0.8 × 0.05 = 0.013 + 0.040 = 0.053
...
class TestBridgeIntegration:
    ...
    def test_end_to_end_beta_adjustment(self, bridge, network):
        ...
        ***REMOVED*** Scenario: Multiple residents showing stress signals
        alerts = [
            SeismicAlert(
                signal_type=PrecursorSignal.SWAP_REQUESTS,
                sta_lta_ratio=6.0,
        ...
        ***REMOVED*** Verify all expected fields present
        ...
        ***REMOVED*** Verify beta increased from baseline
        assert result["beta_adjusted"] > result["beta_base"]

        ***REMOVED*** Verify alerts are tracked
        assert result["num_active_alerts"] == 2

        ***REMOVED*** Verify current beta matches
    ...
    def test_no_alerts_no_adjustment(self, bridge):
        ...
        ***REMOVED*** Beta should remain at baseline
        ...
        assert abs(result["adjustment_magnitude"]) < 1.0  ***REMOVED*** Minimal change

    def test_parameter_validation(self, seismic_detector, epi_model):
        """Test that invalid parameters raise errors."""
        ***REMOVED*** Invalid sensitivity
        with pytest.raises(ValueError, match="sensitivity"):
            SeismicSIRBridge(
                seismic_detector,
                epi_model,
                sensitivity=1.5,  ***REMOVED*** > 1.0
            )

        ***REMOVED*** Invalid smoothing_factor
        with pytest.raises(ValueError, match="smoothing_factor"):
            SeismicSIRBridge(
                seismic_detector,
                epi_model,
                smoothing_factor=-0.1,  ***REMOVED*** < 0.0
            )

        ***REMOVED*** Invalid beta_base
        with pytest.raises(ValueError, match="beta_base"):
            SeismicSIRBridge(
                seismic_detector,
                epi_model,
                beta_base=1.0,  ***REMOVED*** > 0.95
            )
