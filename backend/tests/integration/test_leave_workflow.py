...
class TestLeaveCreation:
    ...
    def test_create_deployment_leave(
        ...
        response = integration_client.post(
            ...
            json={
                ...
                "is_blocking": False,  ***REMOVED*** Should be overridden to True
    ...
    def test_create_leave_invalid_dates(
        ...
        faculty = setup["faculty"][0]
        start = (date.today() + timedelta(days=30)).isoformat()
        end = (date.today() + timedelta(days=20)).isoformat()  ***REMOVED*** Before start
    ...
    def test_create_multiple_leave_types(
        ...
        ***REMOVED*** If auth is working, should have created all
        if created_count > 0:
...
class TestLeaveUpdate:
    ...
    def test_update_leave_type(
        ...
        ***REMOVED*** Update type to conference
        response = integration_client.put(
            f"/api/leave/{absence.id}",
            json={
    ...
    def test_update_leave_blocking_status(
        ...
        ***REMOVED*** Create non-blocking leave
        absence = Absence(
            id=uuid4(),
            person_id=faculty.id,
            start_date=date.today() + timedelta(days=30),
...
class TestLeaveCalendar:
    ...
    def test_get_leave_calendar_with_leave(
        ...
        ***REMOVED*** Create multiple leave records
        ...
        ***REMOVED*** Get calendar
        ...
        if response.status_code == 200:
            ...
            ***REMOVED*** Verify all faculty are represented
    ...
    def test_leave_calendar_date_filtering(
        ...
        ***REMOVED*** Create leave outside the query range
        ...
        ***REMOVED*** Create leave inside the query range
        ...
        ***REMOVED*** Query for future dates only
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Should only include the current absence, not the early one
...
class TestLeaveConflictDetection:
    ...
    def test_leave_with_assignment_conflict(
        ...
        ***REMOVED*** Create an assignment for the faculty
        block = setup["blocks"][10]  ***REMOVED*** Some block in the future
        ...
        ***REMOVED*** Create leave that overlaps with the assignment
        absence = Absence(
            id=uuid4(),
            person_id=faculty.id,
        ...
        ***REMOVED*** Get calendar to check for conflicts
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Calendar endpoint includes conflict detection
            assert "entries" in data
            assert len(data["entries"]) >= 1
    ...
    def test_calendar_conflict_detection_with_fmit(
        ...
        ***REMOVED*** Create blocking leave that overlaps
        absence = Absence(
            id=uuid4(),
            person_id=faculty.id,
            start_date=start,
        ...
        ***REMOVED*** Get calendar
        ...
        if response.status_code == 200:
            data = response.json()
            assert "conflict_count" in data
            ***REMOVED*** Should have detected conflict
    ...
    def test_blocking_vs_nonblocking_leave(
        ...
        ***REMOVED*** Get calendar
        ...
        if response.status_code == 200:
            ...
            ***REMOVED*** Find each entry and check blocking status
    ...
    def test_background_conflict_detection_triggered(
        ...
        ***REMOVED*** Create leave - should trigger background task
        response = integration_client.post(
            "/api/leave/",
            json={
                "faculty_id": str(faculty.id),
        ...
        ***REMOVED*** Should successfully create leave
        assert response.status_code in [201, 401, 403]
        if response.status_code == 201:
            data = response.json()
            assert "id" in data
            ***REMOVED*** Background task will be triggered asynchronously
            ***REMOVED*** In a full integration test with Celery, we would verify the task ran
...
class TestBulkLeaveImport:
    ...
    def test_bulk_import_skip_duplicates(
        ...
        ***REMOVED*** Try to import duplicate + new record
        records = [
            {
                "faculty_id": str(faculty.id),
        ...
        if response.status_code == 200:
            data = response.json()
            assert data["imported_count"] == 1  ***REMOVED*** Only the new one
            assert data["skipped_count"] == 1  ***REMOVED*** The duplicate
    ...
    def test_bulk_import_partial_failure(
        ...
        records = [
            ...
            ***REMOVED*** Invalid: non-existent faculty
            {
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Should have imported the valid one
...
class TestLeaveWebhook:
    ...
    def test_webhook_created_event(
        ...
        ***REMOVED*** Webhook should accept the event
...
class TestLeaveListingAndFiltering:
    ...
    def test_list_leave_filter_by_faculty(
        ...
        ***REMOVED*** Create leave for different faculty
        for fac in faculty:
            absence = Absence(
                id=uuid4(),
                person_id=fac.id,
        ...
        ***REMOVED*** Filter by first faculty
        response = integration_client.get(
        ...
        if response.status_code == 200:
            ...
            ***REMOVED*** All results should be for the specified faculty
    ...
    def test_list_leave_filter_by_date_range(
        ...
        ***REMOVED*** Create leave in different time periods
        early_leave = Absence(
            id=uuid4(),
            person_id=faculty.id,
            start_date=date.today() + timedelta(days=5),
        ...
        ***REMOVED*** Query for early period only
        start = (date.today() + timedelta(days=1)).isoformat()
        end = (date.today() + timedelta(days=20)).isoformat()
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Should only get the early leave, not the late one
            assert len(data["items"]) >= 1
            for item in data["items"]:
                item_start = date.fromisoformat(item["start_date"])
                item_end = date.fromisoformat(item["end_date"])
                ***REMOVED*** Should overlap with query range
