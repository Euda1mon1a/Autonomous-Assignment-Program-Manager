...
class TestAssignmentRouteAuthentication:
    ...
    def test_get_assignment_requires_auth(
        ...
        response = client.get(f"/api/v1/assignments/{test_assignment.id}")
        assert response.status_code == 401
        assert "Not authenticated" in response.json()["detail"]
    ...
    def test_delete_assignment_requires_auth(
        ...
        response = client.delete(f"/api/v1/assignments/{test_assignment.id}")
        assert response.status_code == 401
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_get_assignment_with_invalid_uuid(
        ...
        response = client.get(
            "/api/v1/assignments/not-a-valid-uuid",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 422  # Validation error
