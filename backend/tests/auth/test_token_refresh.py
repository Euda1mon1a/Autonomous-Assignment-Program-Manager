...
class TestRefreshTokenCreation:
    ...
    async def test_create_refresh_token(self, token_service, sample_user):
        ...
        assert len(token_string) > 50  ***REMOVED*** Secure random string
    ...
    async def test_create_with_device_binding(
        ...
        assert len(token_data.device_fingerprint) == 64  ***REMOVED*** SHA-256 hex
    ...
    async def test_concurrent_token_limit(self, token_service, sample_user):
        ...
        ***REMOVED*** Create one more - should revoke oldest
        new_token_data, _ = await token_service.create_refresh_token(request)

        ***REMOVED*** Verify still at max
        user_tokens = token_service.get_user_tokens(sample_user["user_id"])
        active_tokens = [t for t in user_tokens if t.status == RefreshTokenStatus.ACTIVE]
        assert len(active_tokens) == token_service.max_tokens_per_user

        ***REMOVED*** Verify new token is active
    ...
    async def test_token_family_creation(self, token_service, sample_user):
        ...
        ***REMOVED*** Verify family contains both tokens
...
class TestTokenValidationAndRotation:
    ...
    async def test_validate_and_rotate_always(
        ...
        ***REMOVED*** Original token should be marked USED
    ...
    async def test_validate_without_rotation(self, token_service, sample_user):
        ...
        ***REMOVED*** Validate without rotation
        ...
        assert result_string is None  ***REMOVED*** No new token
    ...
    async def test_device_mismatch_rejection(
        ...
        ***REMOVED*** Try to use from different device
        different_device = DeviceFingerprint(
            ip_address="10.0.0.1",  ***REMOVED*** Different IP
...
class TestTokenReuseDetection:
    ...
    async def test_reuse_detection(self, token_service, sample_user):
        ...
        ***REMOVED*** First use - should rotate
        new_token_data, new_token_string, status = (
            await token_service.validate_and_rotate_token(token_string)
        )
        ...
        ***REMOVED*** Try to reuse old token - should detect and revoke family
        ...
        ***REMOVED*** Verify entire family was revoked
        family_tokens = token_service.get_token_family(family_id)
    ...
    async def test_multi_generation_reuse(self, token_service, sample_user):
        ...
        ***REMOVED*** Verify entire family was revoked (3 tokens)
        family_tokens = token_service.get_token_family(family_id)
...
class TestTokenRevocation:
    ...
    async def test_revoke_single_token(self, token_service, sample_user):
        ...
        ***REMOVED*** Try to use revoked token
        result_data, result_string, status = (
            await token_service.validate_and_rotate_token(token_string)
        )

        assert result_data is None
        assert "not found" in status.lower()  ***REMOVED*** Token removed from storage
    ...
    async def test_revoke_token_family(self, token_service, sample_user):
        ...
        ***REMOVED*** Create multiple tokens in same family
        for i in range(3):
            request = RefreshTokenCreate(
                user_id=sample_user["user_id"],
                username=sample_user["username"],
        ...
        ***REMOVED*** Verify all tokens revoked
        family_tokens = token_service.get_token_family(family_id)
        assert len(family_tokens) == 0  ***REMOVED*** All removed from storage

    @pytest.mark.asyncio
    async def test_revoke_user_tokens(self, token_service, sample_user):
        """Test revoking all tokens for a user."""
        ***REMOVED*** Create multiple tokens
        for i in range(3):
            request = RefreshTokenCreate(
                user_id=sample_user["user_id"],
                username=sample_user["username"],
    ...
    async def test_revoke_user_tokens_except_one(self, token_service, sample_user):
        ...
        ***REMOVED*** Keep first token, revoke others
        ...
        ***REMOVED*** Verify kept token is still active
        user_tokens = token_service.get_user_tokens(sample_user["user_id"])
        active_tokens = [t for t in user_tokens if t.status == RefreshTokenStatus.ACTIVE]
...
class TestTokenCleanup:
    ...
    async def test_cleanup_expired_tokens(self, token_service, sample_user):
        ...
        ***REMOVED*** Create tokens with different expiration times
        for days in [1, 2, 3]:
            request = RefreshTokenCreate(
                user_id=sample_user["user_id"],
                username=sample_user["username"],
        ...
        ***REMOVED*** Manually expire first two tokens
        ...
        ***REMOVED*** Verify only 1 token remains
...
class TestTokenRotationStrategies:
    ...
    async def test_threshold_rotation(self, sample_user):
        ...
        service = RefreshTokenService(
            ...
            rotation_threshold=0.5,  ***REMOVED*** Rotate if >50% lifetime used
            default_expiry_days=1,  ***REMOVED*** 1 day for easier testing
        )
        ...
        ***REMOVED*** Simulate time passing (>50% of lifetime)
        token_data.created_at = datetime.utcnow() - timedelta(hours=13)

        ***REMOVED*** Should rotate
        result_data, result_string, status = await service.validate_and_rotate_token(
            token_string
        )
    ...
    async def test_threshold_no_rotation(self, sample_user):
        ...
        ***REMOVED*** Token is fresh (<50% lifetime used)
        result_data, result_string, status = await service.validate_and_rotate_token(
            token_string
        )

        assert status == "valid"
        assert result_string is None  ***REMOVED*** No rotation
        assert result_data.token_id == token_data.token_id
