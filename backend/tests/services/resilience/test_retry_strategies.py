...
***REMOVED*** ============================================================================
***REMOVED*** Backoff Strategy Tests
***REMOVED*** ============================================================================
...
class TestFixedDelayStrategy:
    ...
    def test_calculate_delay_constant(self):
        ...
        ***REMOVED*** Should return same delay regardless of attempt
    ...
    def test_get_delay_with_max(self):
        ...
        ***REMOVED*** Should be capped at max_delay
...
class TestLinearBackoffStrategy:
    ...
    def test_calculate_delay_linear_growth(self):
        ...
        ***REMOVED*** Formula: base_delay + (attempt * increment)
        assert strategy.calculate_delay(0) == 1.0  ***REMOVED*** 1 + (0 * 2)
        assert strategy.calculate_delay(1) == 3.0  ***REMOVED*** 1 + (1 * 2)
        assert strategy.calculate_delay(2) == 5.0  ***REMOVED*** 1 + (2 * 2)
        assert strategy.calculate_delay(3) == 7.0  ***REMOVED*** 1 + (3 * 2)
        assert strategy.calculate_delay(10) == 21.0  ***REMOVED*** 1 + (10 * 2)
    ...
    def test_get_delay_with_max(self):
        ...
        assert strategy.get_delay(0) == 1.0  ***REMOVED*** Below cap
        assert strategy.get_delay(1) == 3.0  ***REMOVED*** Below cap
        assert strategy.get_delay(2) == 5.0  ***REMOVED*** Below cap
        assert strategy.get_delay(3) == 6.0  ***REMOVED*** Capped at 6.0
        assert strategy.get_delay(10) == 6.0  ***REMOVED*** Capped at 6.0
...
class TestExponentialBackoffStrategy:
    ...
    def test_calculate_delay_exponential_growth(self):
        ...
        ***REMOVED*** Formula: base_delay * (multiplier ^ attempt)
        assert strategy.calculate_delay(0) == 1.0  ***REMOVED*** 1 * (2^0) = 1
        assert strategy.calculate_delay(1) == 2.0  ***REMOVED*** 1 * (2^1) = 2
        assert strategy.calculate_delay(2) == 4.0  ***REMOVED*** 1 * (2^2) = 4
        assert strategy.calculate_delay(3) == 8.0  ***REMOVED*** 1 * (2^3) = 8
        assert strategy.calculate_delay(4) == 16.0  ***REMOVED*** 1 * (2^4) = 16
    ...
    def test_calculate_delay_custom_multiplier(self):
        ...
        assert strategy.calculate_delay(0) == 2.0  ***REMOVED*** 2 * (3^0) = 2
        assert strategy.calculate_delay(1) == 6.0  ***REMOVED*** 2 * (3^1) = 6
        assert strategy.calculate_delay(2) == 18.0  ***REMOVED*** 2 * (3^2) = 18
    ...
    def test_get_delay_with_max(self):
        ...
        assert strategy.get_delay(0) == 1.0  ***REMOVED*** Below cap
        assert strategy.get_delay(1) == 2.0  ***REMOVED*** Below cap
        assert strategy.get_delay(2) == 4.0  ***REMOVED*** Below cap
        assert strategy.get_delay(3) == 8.0  ***REMOVED*** Below cap
        assert strategy.get_delay(4) == 10.0  ***REMOVED*** Capped at 10.0
        assert strategy.get_delay(5) == 10.0  ***REMOVED*** Capped at 10.0
...
class TestExponentialBackoffWithCeiling:
    ...
    def test_ceiling_applied_through_get_delay(self):
        ...
        ***REMOVED*** Ceiling is applied via parent class get_delay method
        assert strategy.get_delay(4) == 10.0  ***REMOVED*** 16 capped to 10
        assert strategy.get_delay(5) == 10.0  ***REMOVED*** 32 capped to 10
...
class TestGetRetryStrategy:
    ...
    def test_invalid_strategy_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Unknown backoff strategy"):
            ***REMOVED*** Create an invalid enum value for testing
...
class TestFullJitter:
    ...
    def test_apply_returns_value_in_range(self):
        ...
        ***REMOVED*** Test multiple times due to randomness
        for _ in range(100):
...
class TestEqualJitter:
    ...
    def test_apply_returns_value_in_range(self):
        ...
        ***REMOVED*** Test multiple times due to randomness
        for _ in range(100):
            result = jitter.apply(10.0, attempt=0)
            ***REMOVED*** Should be between 5.0 and 10.0
    ...
    def test_apply_formula(self):
        ...
        for _ in range(50):
            result = jitter.apply(delay, attempt=0)
            ***REMOVED*** Formula: delay/2 + random(0, delay/2)
            ***REMOVED*** Min: 5.0 + 0 = 5.0
            ***REMOVED*** Max: 5.0 + 5.0 = 10.0
            assert 5.0 <= result <= 10.0

    @patch("app.resilience.retry.jitter.random.uniform")
    def test_apply_calculation(self, mock_uniform):
        """Test equal jitter calculation."""
        mock_uniform.return_value = 2.5  ***REMOVED*** Random part
        jitter = EqualJitter()

        result = jitter.apply(10.0, attempt=0)

        ***REMOVED*** Should be: 10/2 + 2.5 = 5.0 + 2.5 = 7.5
...
class TestDecorrelatedJitter:
    ...
    def test_first_attempt_uses_delay(self):
        ...
        ***REMOVED*** First attempt with no previous delay
        for _ in range(50):
            result = jitter.apply(
                delay=10.0,
                attempt=0,
            ...
            ***REMOVED*** Should be between base_delay and delay
    ...
    def test_subsequent_attempts_use_previous_delay(self):
        ...
        ***REMOVED*** Subsequent attempt with previous delay
        for _ in range(50):
            result = jitter.apply(
                delay=10.0,
                attempt=1,
            ...
            ***REMOVED*** Should be between base_delay and previous_delay * 3
            ***REMOVED*** = between 1.0 and 15.0
    ...
    def test_subsequent_attempt_calculation(self, mock_uniform):
        ...
        ***REMOVED*** Should call with base_delay and previous_delay * 3
...
class TestRetryContext:
    ...
    def test_elapsed_time(self, context):
        ...
        assert elapsed >= 0.1
        assert elapsed < 1.0  ***REMOVED*** Should be quick
    ...
    def test_callback_error_handling(self, context):
        ...
        ***REMOVED*** Should not raise, just log
...
class TestRetryExecutor:
    ...
    def config(self):
        ...
        return RetryConfig(
            ...
            base_delay=0.1,  ***REMOVED*** Short delay for fast tests
            jitter=JitterType.NONE,  ***REMOVED*** No jitter for predictability
        )
    ...
    def test_execute_sync_timeout(self):
        ...
        config = RetryConfig(
            max_attempts=10,
            base_delay=0.5,
            timeout=0.3,  ***REMOVED*** Short timeout
...
class TestRetryDecorator:
    ...
    def test_decorator_with_callbacks(self):
        ...
        assert result == "success"
        assert retry_count[0] == 2  ***REMOVED*** Retried twice
        assert success_count[0] == 1  ***REMOVED*** Succeeded once
    ...
    def test_decorator_exception_filtering(self):
        ...
        ***REMOVED*** ValueError is not in retryable_exceptions
...
class TestRetryIntegration:
    ...
    def test_exponential_backoff_delays(self):
        ...
        ***REMOVED*** Should have delays: 0.1, 0.2, 0.4
        ***REMOVED*** Total: ~0.7 seconds
        assert result == "success"
        assert attempts[0] == 4
        assert 0.6 < elapsed < 1.0  ***REMOVED*** Allow some margin
    ...
    def test_linear_backoff_delays(self):
        ...
        ***REMOVED*** Should have delays: 0.1, 0.2
        ***REMOVED*** Total: ~0.3 seconds
        assert result == "success"
        assert attempts[0] == 3
    ...
    def test_jitter_reduces_thundering_herd(self):
        ...
        def record_delays():
            ***REMOVED*** Capture delays through monkey patching
            raise ValueError("Fail")

        ***REMOVED*** Execute multiple times and collect delays
        for _ in range(5):
            attempts = [0]
        ...
        ***REMOVED*** With full jitter, delays should vary
        ***REMOVED*** (This is probabilistic, but very unlikely to all be identical)
    ...
    async def test_async_concurrent_retries(self):
        ...
        async def async_operation(task_id):
            ***REMOVED*** Simulate some async work
            await asyncio.sleep(0.05)
            return f"result_{task_id}"

        ***REMOVED*** Run multiple operations concurrently
    ...
    def test_callback_observability(self):
        ...
        assert result == "success"
        ***REMOVED*** Should have 2 retry events and 1 success event
        assert len(events) == 3
    ...
    def test_max_delay_cap(self):
        ...
        ***REMOVED*** Test delay calculation
        ***REMOVED*** Attempt 0: 1.0
        ***REMOVED*** Attempt 1: 2.0
        ***REMOVED*** Attempt 2: 4.0
        ***REMOVED*** Attempt 3: 8.0 -> capped to 5.0
        ***REMOVED*** Attempt 4: 16.0 -> capped to 5.0

        assert executor.strategy.get_delay(0) == 1.0
        assert executor.strategy.get_delay(1) == 2.0
        assert executor.strategy.get_delay(2) == 4.0
        assert executor.strategy.get_delay(3) == 5.0  ***REMOVED*** Capped
        assert executor.strategy.get_delay(4) == 5.0  ***REMOVED*** Capped
