...
async def blocks(db_session):
    ...
    for day_offset in range(14):  ***REMOVED*** 14 days
    ...
    ***REMOVED*** Filter out weekends for workday-only coverage
...
class TestShapleyValueCalculation:
    ...
    async def test_equal_contribution_equal_shapley(
        ...
        ***REMOVED*** Setup: Each faculty covers equal number of blocks
        ...
        ***REMOVED*** Verify: All Shapley values should be approximately equal (within 5%)
        shapley_values = [r.shapley_value for r in results.values()]
        expected = 1.0 / 3  ***REMOVED*** Three faculty, equal split
        for sv in shapley_values:
            assert abs(sv - expected) < 0.05, (
                f"Shapley value {sv} not close to {expected}"
            )

        ***REMOVED*** Verify: Sum of Shapley values = 1.0 (efficiency property)
    ...
    async def test_unequal_contribution_proportional_shapley(
        ...
        ***REMOVED*** Setup: Faculty 1 covers 50%, Faculty 2 covers 30%, Faculty 3 covers 20%
        total = len(blocks)
        ...
        ***REMOVED*** Test
        ...
        results = await service.calculate_shapley_values(
            ...
            start_date=blocks[0].date,
            end_date=blocks[-1].date,
            num_samples=2000,  ***REMOVED*** More samples for better accuracy
        )

        ***REMOVED*** Verify: Shapley values roughly proportional to contributions
        sorted_results = sorted(
            results.values(), key=lambda x: x.shapley_value, reverse=True
        )

        ***REMOVED*** Faculty 1 should have highest Shapley value (~0.5)
        assert sorted_results[0].shapley_value > 0.4
        ***REMOVED*** Faculty 2 should have medium Shapley value (~0.3)
        assert 0.2 < sorted_results[1].shapley_value < 0.4
        ***REMOVED*** Faculty 3 should have lowest Shapley value (~0.2)
    ...
    async def test_zero_contribution_zero_shapley(
        ...
        ***REMOVED*** Setup: Only first two faculty have assignments, third has none
        ...
        ***REMOVED*** Verify: Third faculty (no assignments) has near-zero Shapley value
...
class TestEquityGapDetection:
    ...
    async def test_equity_gap_calculation(self, db_session, faculty_members, blocks):
        ...
        ***REMOVED*** Setup: Faculty 1 works 60%, Faculty 2 works 30%, Faculty 3 works 10%
        total = len(blocks)
        ...
        ***REMOVED*** Verify: Faculty 1 should be overworked (positive equity gap)
        fac1_result = results[faculty_members[0].id]
        assert fac1_result.equity_gap > 0, "Faculty 1 should be overworked"

        ***REMOVED*** Verify: Faculty 3 should be underworked (negative equity gap)
    ...
    async def test_equity_report_summary(self, db_session, faculty_members, blocks):
        ...
        ***REMOVED*** Setup: Unbalanced workload
        total = len(blocks)
        allocations = [int(total * 0.5), int(total * 0.3), int(total * 0.2)]
...
class TestWorkloadAdjustments:
    ...
    async def test_suggest_adjustments_threshold(
        ...
        ***REMOVED*** Setup: Significant imbalance (70/20/10 split)
        total = len(blocks)
        ...
        ***REMOVED*** Test with high threshold (should get fewer suggestions)
        service = ShapleyValueService(db_session)
        suggestions_high = await service.suggest_workload_adjustments(
            faculty_ids=[f.id for f in faculty_members],
            start_date=blocks[0].date,
            end_date=blocks[-1].date,
            threshold=20.0,  ***REMOVED*** 20 hours
        )

        ***REMOVED*** Test with low threshold (should get more suggestions)
        suggestions_low = await service.suggest_workload_adjustments(
            faculty_ids=[f.id for f in faculty_members],
            start_date=blocks[0].date,
            end_date=blocks[-1].date,
            threshold=5.0,  ***REMOVED*** 5 hours
        )
    ...
    async def test_adjustment_sorting(self, db_session, faculty_members, blocks):
        ...
        ***REMOVED*** Setup: Varying imbalances
        total = len(blocks)
        ...
        ***REMOVED*** Verify: Sorted by magnitude (most urgent first)
...
class TestEdgeCases:
    ...
    async def test_empty_assignments(self, db_session, faculty_members, blocks):
        ...
        ***REMOVED*** No assignments created
        service = ShapleyValueService(db_session)
        results = await service.calculate_shapley_values(
            faculty_ids=[f.id for f in faculty_members],
            start_date=blocks[0].date,
        ...
        ***REMOVED*** Verify: All faculty have zero workload and near-equal Shapley values
        for result in results.values():
            assert result.current_workload == 0.0
            assert result.fair_workload_target == 0.0
            ***REMOVED*** Shapley values should be roughly equal (1/3 each)
    ...
    async def test_monte_carlo_consistency(self, db_session, faculty_members, blocks):
        ...
        ***REMOVED*** Setup: Equal distribution
        blocks_per_faculty = len(blocks) // 3
        for i, faculty in enumerate(faculty_members):
            start_idx = i * blocks_per_faculty
            end_idx = start_idx + blocks_per_faculty
        ...
        ***REMOVED*** Verify: Both should converge to ~0.33, but high samples should be closer
        expected = 1.0 / 3
        variance_low = sum(
        ...
        ***REMOVED*** High sample variance should be lower (more consistent)
        assert variance_high <= variance_low * 1.5  ***REMOVED*** Allow some randomness
