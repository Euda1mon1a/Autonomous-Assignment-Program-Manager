    ...
    def test_get_model_lazy_loads(self):
        ...
        ***REMOVED*** Reset model to None
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** First call should load model
    ...
    def test_get_model_returns_singleton(self):
        ...
        ***REMOVED*** Reset model to None
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Should only be called once (singleton)
...
class TestEmbeddingServiceEmbedText:
    ...
    def test_embed_text_success(self):
        ...
        ***REMOVED*** Mock the model
        mock_model = Mock()
        mock_embedding = np.array([0.1, 0.2, 0.3] * 128)  ***REMOVED*** 384 dimensions
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Verify encode was called with correct parameters
            mock_model.encode.assert_called_once_with(
                "Test text", convert_to_numpy=True
            )

            ***REMOVED*** Verify result is a list of floats
...
class TestEmbeddingServiceEmbedBatch:
    ...
    def test_embed_batch_multiple_texts(self):
        ...
        ***REMOVED*** Reset model
        ...
        mock_model = Mock()
        ***REMOVED*** Mock batch of 3 embeddings
    ...
    def test_embed_batch_mixed_lengths(self):
        ...
        ***REMOVED*** Reset model
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            assert len(result) == 3
            ***REMOVED*** All embeddings should have same dimension regardless of input length
            assert all(len(emb) == 384 for emb in result)
...
class TestEmbeddingServiceHashText:
    ...
    def test_hash_text_basic(self):
        ...
        ***REMOVED*** Should return a hex string
        assert isinstance(result, str)
        ***REMOVED*** SHA256 produces 64 hex characters
        assert len(result) == 64
        ***REMOVED*** Should be valid hex
    ...
    def test_hash_text_empty_string(self):
        ...
        ***REMOVED*** Should still produce a valid hash
        assert isinstance(result, str)
        assert len(result) == 64

        ***REMOVED*** Verify it matches expected SHA256 of empty string
    ...
    def test_hash_text_whitespace_sensitive(self):
        ...
        text1 = "hello world"
        text2 = "hello  world"  ***REMOVED*** Extra space
        text3 = "hello world "  ***REMOVED*** Trailing space

        hash1 = EmbeddingService.hash_text(text1)
        hash2 = EmbeddingService.hash_text(text2)
        hash3 = EmbeddingService.hash_text(text3)

        ***REMOVED*** All should be different
        assert hash1 != hash2
        assert hash1 != hash3
        assert hash2 != hash3
...
class TestGetCachedEmbedding:
    ...
    def test_get_cached_embedding_returns_tuple(self):
        ...
        ***REMOVED*** Reset model and clear cache
        EmbeddingService._model = None
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Should return tuple (for hashability)
    ...
    def test_get_cached_embedding_caches_results(self):
        ...
        ***REMOVED*** Reset model and clear cache
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Second call with same text
            result2 = get_cached_embedding("Test text")

            ***REMOVED*** Should return same result
            assert result1 == result2

            ***REMOVED*** Model encode should only be called once (due to caching)
    ...
    def test_get_cached_embedding_different_texts(self):
        ...
        ***REMOVED*** Reset model and clear cache
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Should be different
            assert result1 != result2

            ***REMOVED*** Both should have been computed
    ...
    def test_get_cached_embedding_respects_cache_size(self):
        ...
        ***REMOVED*** Reset model
        ...
        mock_model = Mock()
        ***REMOVED*** Generate different embeddings for each call
        mock_model.encode.side_effect = [np.array([float(i)] * 384) for i in range(150)]

        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            mock_st.return_value = mock_model

            ***REMOVED*** Generate embeddings for more than cache size (maxsize=100)
            for i in range(150):
                get_cached_embedding(f"Text {i}")

            ***REMOVED*** Cache info should show some evictions
    ...
    def test_get_cached_embedding_cache_clear(self):
        ...
        ***REMOVED*** Reset model
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Generate some cached entries
            ...
            ***REMOVED*** Cache should be empty
            info = get_cached_embedding.cache_info()
            assert info.currsize == 0

            ***REMOVED*** Next call should miss cache
            get_cached_embedding("Text A")
            info = get_cached_embedding.cache_info()
...
class TestEmbeddingServiceIntegration:
    ...
    def test_embed_and_hash_workflow(self):
        ...
        ***REMOVED*** Reset model
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Generate hash for change detection
            ...
            ***REMOVED*** Re-hash should produce same result
    ...
    def test_batch_vs_single_embedding_consistency(self):
        ...
        ***REMOVED*** Setup consistent mock responses
        single_embedding = np.array([0.6] * 384)
        batch_embeddings = np.array([[0.6] * 384])

        ***REMOVED*** Mock encode to return different values based on input type
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Get embedding as single
            single_result = EmbeddingService.embed_text(text)

            ***REMOVED*** Reset model for batch call
            mock_model.encode.side_effect = mock_encode

            ***REMOVED*** Get embedding as batch
            batch_result = EmbeddingService.embed_batch([text])

            ***REMOVED*** Should produce equivalent results
            assert len(single_result) == len(batch_result[0])
            ***REMOVED*** Values should match (approximately)
...
class TestEmbeddingServiceEdgeCases:
    ...
    def test_hash_text_very_long_text(self):
        ...
        long_text = "A" * 1000000  ***REMOVED*** 1 million characters
        hash_result = EmbeddingService.hash_text(long_text)

        ***REMOVED*** Should still produce valid hash
    ...
    def test_embed_batch_preserves_order(self):
        ...
        ***REMOVED*** Reset model
        ...
        mock_model = Mock()
        ***REMOVED*** Different embeddings for each input
        mock_embeddings = np.array(
            [
                [0.1] * 384,
                [0.2] * 384,
        ...
        with patch("app.services.embedding_service.SentenceTransformer") as mock_st:
            ...
            ***REMOVED*** Order should be preserved
            assert len(result) == 5
            ***REMOVED*** First embedding should be all 0.1, second all 0.2, etc.
