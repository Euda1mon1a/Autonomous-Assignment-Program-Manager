...
class TestDatabaseLeaveProvider:
    ...
    def test_is_person_absent_true(self, db, sample_resident):
        ...
        is_absent = provider.is_person_absent(
            ...
            date(2025, 1, 12),  ***REMOVED*** Middle of absence
        )
    ...
    def test_is_person_absent_false(self, db, sample_resident):
        ...
        is_absent = provider.is_person_absent(
            str(sample_resident.id),
            date(2025, 1, 20),  ***REMOVED*** After absence
        )
...
class TestCSVLeaveProvider:
    ...
    def test_load_absences_from_csv(self):
        ...
        ***REMOVED*** Create temporary CSV file
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
            csv_path = Path(f.name)
        ...
        try:
            ...
            assert len(absences) >= 2
            ***REMOVED*** CSV provider returns dicts, not model objects
        finally:
            csv_path.unlink()
    ...
    def test_get_absences_for_person_from_csv(self):
        ...
        try:
            ...
            ***REMOVED*** Should find 2 absences for person-123
            assert len(absences) >= 1
        finally:
            csv_path.unlink()
    ...
    def test_csv_file_not_found(self):
        ...
        with pytest.raises((FileNotFoundError, Exception)):
            provider = CSVLeaveProvider(csv_path)
            ***REMOVED*** Depending on implementation, error might occur at init or first read
    ...
    def test_csv_malformed_data(self):
        ...
        try:
            provider = CSVLeaveProvider(csv_path)
            ***REMOVED*** Provider should handle or raise error for malformed data
            ***REMOVED*** Behavior depends on implementation
        finally:
            csv_path.unlink()
