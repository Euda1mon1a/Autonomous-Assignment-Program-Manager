...
class TestCertificationSchedulerStart:
    ...
    def test_start_when_disabled(self):
        ...
        with patch.dict("os.environ", {"CERT_CHECK_ENABLED": "false"}):
            ...
            ***REMOVED*** When disabled, scheduler should remain None
    ...
    def test_start_success(self, mock_scheduler_class):
        ...
        with patch.dict(
            ...
            ***REMOVED*** Verify scheduler was created and started
            mock_scheduler_class.assert_called_once()
            mock_scheduler.add_job.assert_called_once()
            mock_scheduler.start.assert_called_once()
            ***REMOVED*** Verify scheduler object is set
    ...
    def test_start_adds_cron_job(self, mock_scheduler_class):
        ...
        with patch.dict(
            ...
            ***REMOVED*** Verify job configuration
    ...
    def test_start_without_apscheduler(self, caplog):
        ...
        with (
            ...
            ***REMOVED*** Need to reload module to pick up the missing import
            scheduler = CertificationScheduler()
            scheduler.start()

            ***REMOVED*** Since we can't actually make apscheduler disappear after it's loaded,
            ***REMOVED*** we just verify the scheduler didn't crash
            ***REMOVED*** The actual import error handling is tested by the code structure
    ...
    def test_start_handles_exception(self, mock_scheduler_class):
        ...
        with patch.dict("os.environ", {"CERT_CHECK_ENABLED": "true"}):
            scheduler = CertificationScheduler()
            ***REMOVED*** Should not raise - exception is caught internally
            scheduler.start()

            ***REMOVED*** Scheduler should remain None after failed start
            assert scheduler.scheduler is None
...
class TestCertificationSchedulerRunDailyCheck:
    ...
    def test_run_daily_check_updates_statuses(
        ...
        ***REMOVED*** Run daily check
        scheduler = CertificationScheduler()
        scheduler.run_daily_check()

        ***REMOVED*** Verify status update was called
    ...
    def test_run_daily_check_sends_reminders_for_all_thresholds(
        ...
        ***REMOVED*** Verify all thresholds were checked
    ...
    def test_run_daily_check_sends_admin_summary_when_configured(
        ...
        ***REMOVED*** Run with admin email configured
        with patch.dict("os.environ", {"CERT_ADMIN_EMAIL": "admin@hospital.org"}):
            scheduler = CertificationScheduler()
            scheduler.run_daily_check()

            ***REMOVED*** Verify admin summary was sent
    ...
    def test_run_daily_check_skips_admin_summary_when_not_configured(
        ...
        ***REMOVED*** Run without admin email
        with patch.dict("os.environ", {"CERT_ADMIN_EMAIL": ""}, clear=True):
            scheduler = CertificationScheduler()
            scheduler.run_daily_check()

            ***REMOVED*** Verify admin summary was NOT sent
    ...
    def test_run_daily_check_handles_exception(
        ...
        ***REMOVED*** Setup mocks to raise exception
        mock_db = MagicMock()
        mock_session_local.return_value = mock_db
        ...
        ***REMOVED*** Run daily check - should not raise
        scheduler = CertificationScheduler()
        scheduler.run_daily_check()

        ***REMOVED*** Verify database was closed even after exception
        mock_db.close.assert_called_once()
...
class TestSendRemindersForThreshold:
    ...
    def test_send_reminders_for_threshold_sends_emails(self):
        ...
        ***REMOVED*** Create mock certification with enabled reminder
    ...
    def test_send_reminders_for_threshold_skips_disabled_reminders(self):
        ...
        ***REMOVED*** Create mock certification with DISABLED reminder for 30 days
        mock_cert_type = Mock(spec=CertificationType)
    ...
    def test_send_reminders_for_threshold_handles_email_failure(self):
        ...
        ***REMOVED*** Create mock certification
        mock_cert_type = Mock(spec=CertificationType)
        ...
        assert sent_count == 0
        ***REMOVED*** Should not mark as sent if email failed
    ...
    def test_send_reminders_for_threshold_multiple_certs(self):
        ...
        ***REMOVED*** Create multiple mock certifications
        mock_certs = []
        for i in range(3):
            mock_cert_type = Mock(spec=CertificationType)
...
class TestSendAdminSummary:
    ...
    def test_send_admin_summary_handles_exception(self):
        ...
        ***REMOVED*** Verify email was not sent (exception prevented it)
        mock_email_service.send_compliance_summary.assert_not_called()
...
class TestIntegrationScenarios:
    ...
    def test_complete_daily_check_workflow(
        ...
        ***REMOVED*** Create mock certifications needing reminders
        mock_cert_type = Mock(spec=CertificationType)
        mock_cert_type.reminder_days_30 = True
        mock_cert_type.reminder_days_7 = True
        ...
        ***REMOVED*** Setup certification service
        mock_cert_service = MagicMock()
        ...
        ***REMOVED*** Setup email service
        mock_email_service = MagicMock()
        mock_email_service.send_certification_reminder.return_value = True
        mock_email_service_class.return_value = mock_email_service

        ***REMOVED*** Run daily check with admin email
        with patch.dict("os.environ", {"CERT_ADMIN_EMAIL": "admin@hospital.org"}):
            scheduler = CertificationScheduler()
            scheduler.run_daily_check()
        ...
        ***REMOVED*** Verify workflow
        ...
        mock_email_service.send_compliance_summary.assert_called_once()
        ***REMOVED*** Database should be closed
        mock_db.close.assert_called_once()
