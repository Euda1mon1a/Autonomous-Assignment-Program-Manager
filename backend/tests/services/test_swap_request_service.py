...
class TestIsWeekAssignedToFaculty:
    ...
    def test_returns_true_when_faculty_has_fmit_assignment_for_week(self, db):
        ...
        ***REMOVED*** Create blocks for a week (Monday to Sunday)
        week_start = date(2024, 1, 1)  ***REMOVED*** Monday
        blocks = []
        ...
        ***REMOVED*** Create FMIT assignment for the week
    ...
    def test_returns_false_when_faculty_has_no_fmit_assignment_for_week(self, db):
        ...
        ***REMOVED*** Test with a week that has no assignments
    ...
    def test_returns_false_when_no_fmit_template_exists(self, db):
        ...
        ***REMOVED*** Test without FMIT template
        week_start = date(2024, 1, 1)
        service = SwapRequestService(db)
        result = service._is_week_assigned_to_faculty(faculty.id, week_start)
    ...
    def test_returns_false_when_faculty_has_non_fmit_assignment_for_week(self, db):
        ...
        ***REMOVED*** Create clinic rotation template (not FMIT)
        ...
        ***REMOVED*** Create clinic assignment (not FMIT) for the week
        assignment = Assignment(
            id=uuid4(),
            block_id=blocks[0].id,
            person_id=faculty.id,
            rotation_template_id=clinic_template.id,  ***REMOVED*** Clinic, not FMIT
            role="primary",
        )
        db.add(assignment)
        db.commit()
    ...
    def test_returns_true_with_multiple_fmit_blocks_in_week(self, db):
        ...
        ***REMOVED*** Create multiple FMIT assignments for the week
        for i in range(5):
            assignment = Assignment(
                id=uuid4(),
    ...
    def test_returns_false_for_different_faculty(self, db):
        ...
        ***REMOVED*** Create FMIT assignment for faculty1
        assignment = Assignment(
            id=uuid4(),
            block_id=blocks[0].id,
        ...
        ***REMOVED*** Test with faculty2 (who has no assignments)
        service = SwapRequestService(db)
    ...
    def test_week_boundary_checking(self, db):
        ...
        ***REMOVED*** Create blocks for week 1 (Jan 1-7)
        ...
        ***REMOVED*** Create blocks for week 2 (Jan 8-14)
        ...
        ***REMOVED*** Create FMIT assignment for week 1 only
        assignment = Assignment(
            id=uuid4(),
...
class TestCreateRequest:
    ...
    def test_create_request_fails_when_week_not_assigned(self, db):
        ...
        ***REMOVED*** Create FMIT rotation template (but no assignments)
        fmit_template = RotationTemplate(
            id=uuid4(),
            name="FMIT",
            activity_type="inpatient",
        ...
        ***REMOVED*** Try to create swap request for unassigned week
        service = SwapRequestService(db)
        week_start = date(2024, 1, 1)
