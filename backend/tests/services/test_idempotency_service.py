...
class TestIdempotencyService:
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Compute Body Hash Tests
    ***REMOVED*** ========================================================================
    ...
    def test_compute_body_hash_success(self, db):
        ...
        assert len(result) == 64  ***REMOVED*** SHA-256 produces 64 hex chars
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Get Existing Request Tests
    ***REMOVED*** ========================================================================
    ...
    def test_get_existing_request_found(self, db):
        ...
        ***REMOVED*** Create existing request
    ...
    def test_get_existing_request_expired(self, db):
        ...
        key = "test-key-123"
        body_hash = "abc123def456"
        expires_at = datetime.utcnow() - timedelta(hours=1)  ***REMOVED*** Already expired

        ***REMOVED*** Create expired request
        existing = IdempotencyRequest(
            id=uuid4(),
            idempotency_key=key,
            body_hash=body_hash,
        ...
        assert result is None  ***REMOVED*** Expired should not be returned
    ...
    def test_get_existing_request_wrong_hash(self, db):
        ...
        ***REMOVED*** Create request with one hash
        existing = IdempotencyRequest(
            id=uuid4(),
            idempotency_key=key,
            body_hash=body_hash,
        ...
        ***REMOVED*** Query with different hash
        result = service.get_existing_request(key, "different-hash")

        assert result is None

    ***REMOVED*** ========================================================================
    ***REMOVED*** Check Key Conflict Tests
    ***REMOVED*** ========================================================================
    ...
    def test_check_key_conflict_detected(self, db):
        ...
        ***REMOVED*** Create request with original hash
        existing = IdempotencyRequest(
            id=uuid4(),
            idempotency_key=key,
        ...
        ***REMOVED*** Check for conflict with different hash
    ...
    def test_check_key_conflict_no_conflict(self, db):
        ...
        ***REMOVED*** Check with same hash - no conflict
    ...
    def test_check_key_conflict_expired_ignored(self, db):
        ...
        original_hash = "abc123def456"
        different_hash = "xyz789ghi012"
        expires_at = datetime.utcnow() - timedelta(hours=1)  ***REMOVED*** Expired
        ...
        ***REMOVED*** Check for conflict - expired should be ignored
        result = service.check_key_conflict(key, different_hash)
    ...
    def test_check_key_conflict_different_key_no_conflict(self, db):
        ...
        ***REMOVED*** Create request with key1 and hash1
        existing = IdempotencyRequest(
            id=uuid4(),
        ...
        ***REMOVED*** Check key2 with hash2 - different keys, no conflict
        result = service.check_key_conflict(key2, hash2)

        assert result is None

    ***REMOVED*** ========================================================================
    ***REMOVED*** Create Request Tests
    ***REMOVED*** ========================================================================
    ...
    def test_create_request_default_expiration(self, db):
        ...
        ***REMOVED*** Expiration should be within a few seconds of expected
    ...
    def test_create_request_persists_to_database(self, db):
        ...
        ***REMOVED*** Query directly from database
    ...
    def test_create_request_handles_race_condition(self, db):
        ...
        ***REMOVED*** Create first request
        first = service.create_request(key, body_hash, params)
        db.commit()

        ***REMOVED*** Attempt to create duplicate - should return existing
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_cleanup_expired_keeps_valid_records(self, db):
        ...
        ***REMOVED*** Create valid (non-expired) request
    ...
    def test_cleanup_expired_multiple_records(self, db):
        ...
        ***REMOVED*** Create 3 expired and 2 valid
        for i in range(3):
            expired = IdempotencyRequest(
                id=uuid4(),
        ...
        assert count == 3
        ***REMOVED*** Verify correct records remain
    ...
    def test_cleanup_expired_respects_batch_size(self, db):
        ...
        ***REMOVED*** Create 5 expired records
        for i in range(5):
            expired = IdempotencyRequest(
                id=uuid4(),
                idempotency_key=f"expired-{i}",
        ...
        ***REMOVED*** Cleanup with batch_size=3
        count = service.cleanup_expired(batch_size=3)

        assert count == 3
        ***REMOVED*** Verify 2 records remain
        remaining = db.query(IdempotencyRequest).count()
        assert remaining == 2
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Timeout Stale Pending Tests
    ***REMOVED*** ========================================================================
    ...
    def test_timeout_stale_pending_marks_stale_as_failed(self, db):
        ...
        ***REMOVED*** Create stale pending request (created 30 minutes ago)
    ...
    def test_timeout_stale_pending_keeps_recent_pending(self, db):
        ...
        ***REMOVED*** Create recent pending request (created 5 minutes ago)
    ...
    def test_timeout_stale_pending_ignores_completed(self, db):
        ...
        ***REMOVED*** Create old completed request
    ...
    def test_timeout_stale_pending_ignores_already_failed(self, db):
        ...
        ***REMOVED*** Create old failed request
        ...
        assert count == 0
        ***REMOVED*** Verify error message unchanged
        db.refresh(failed)
    ...
    def test_timeout_stale_pending_respects_batch_size(self, db):
        ...
        ***REMOVED*** Create 5 stale pending requests
        for i in range(5):
            stale = IdempotencyRequest(
                id=uuid4(),
        ...
        ***REMOVED*** Timeout with batch_size=3
        count = service.timeout_stale_pending(timeout_minutes=10, batch_size=3)

        assert count == 3
        ***REMOVED*** Verify 2 requests still pending
    ...
    def test_timeout_stale_pending_custom_timeout(self, db):
        ...
        ***REMOVED*** Create request that's 15 minutes old
        request = IdempotencyRequest(
            id=uuid4(),
            idempotency_key="test-key",
            body_hash="abc123",
        ...
        ***REMOVED*** Should timeout with 10 minute cutoff
        count = service.timeout_stale_pending(timeout_minutes=10)
        assert count == 1

        ***REMOVED*** Create another 15 minute old request
        ...
        ***REMOVED*** Should NOT timeout with 20 minute cutoff
...
***REMOVED*** ============================================================================
***REMOVED*** Standalone Function Tests
***REMOVED*** ============================================================================
...
class TestExtractIdempotencyParams:
    ...
    def test_extract_converts_uuids_to_strings(self):
        ...
        ***REMOVED*** Should be sorted strings
    ...
    def test_extract_ignores_extra_fields(self):
        ...
        ***REMOVED*** Should only contain the 5 relevant fields
        assert len(result) == 5
