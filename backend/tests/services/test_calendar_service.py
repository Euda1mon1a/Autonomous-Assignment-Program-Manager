...
class TestCalendarService:
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_create_timezone(self):
        ...
        ***REMOVED*** Verify timezone has standard component
        ...
        assert standard["tzoffsetfrom"].total_seconds() == -36000  ***REMOVED*** -10 hours
        assert standard["tzoffsetto"].total_seconds() == -36000  ***REMOVED*** -10 hours

    ***REMOVED*** ========================================================================
    ***REMOVED*** Block Time Tests
    ***REMOVED*** ========================================================================
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Generate ICS for Person Tests
    ***REMOVED*** ========================================================================
    ...
    def test_generate_ics_for_person_success(
        ...
        ***REMOVED*** Parse calendar and verify events
    ...
    def test_generate_ics_for_person_with_filters(
        ...
        ***REMOVED*** Create rotation templates with different types
        ...
        ***REMOVED*** Create assignments with different types
        ...
        ***REMOVED*** Generate ICS with filter
        ics_content = CalendarService.generate_ics_for_person(
            db=db,
            person_id=sample_resident.id,
            start_date=sample_blocks[0].date,
        ...
        ***REMOVED*** Parse and verify only outpatient events
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 1
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Generate ICS for Rotation Tests
    ***REMOVED*** ========================================================================
    ...
    def test_generate_ics_for_rotation_success(
        ...
        ***REMOVED*** Create assignments for multiple residents
        for i in range(3):
            assignment = Assignment(
                id=uuid4(),
                block_id=sample_blocks[i].id,
        ...
        ***REMOVED*** Parse and verify events
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 3

        ***REMOVED*** Verify each event contains person name
    ...
    def test_generate_ics_for_rotation_includes_pgy_level(
        ...
        ***REMOVED*** Parse and verify PGY level in description
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 1
    ...
    ***REMOVED*** ========================================================================
    ***REMOVED*** Generate ICS All Tests
    ***REMOVED*** ========================================================================
    ...
    def test_generate_ics_all_filter_by_person_ids(
        ...
        ***REMOVED*** Create assignments for all residents
        for i in range(3):
            assignment = Assignment(
                id=uuid4(),
                block_id=sample_blocks[i].id,
        ...
        ***REMOVED*** Generate ICS for only first two residents
        ics_content = CalendarService.generate_ics_all(
            db=db,
            start_date=sample_blocks[0].date,
            end_date=sample_blocks[6].date,
        ...
        ***REMOVED*** Parse and verify only 2 events
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 2
    ...
    def test_generate_ics_all_filter_by_rotation_ids(
        ...
        ***REMOVED*** Generate ICS for only first rotation
        ics_content = CalendarService.generate_ics_all(
            db=db,
            start_date=sample_blocks[0].date,
            end_date=sample_blocks[6].date,
        ...
        ***REMOVED*** Parse and verify only 1 event
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 1
        assert "Clinic" in str(events[0]["summary"])
    ...
    def test_generate_ics_all_filter_by_activity_types(
        ...
        ***REMOVED*** Generate ICS with activity type filter
        ics_content = CalendarService.generate_ics_all(
            db=db,
            start_date=sample_blocks[0].date,
            end_date=sample_blocks[6].date,
        ...
        ***REMOVED*** Parse and verify only outpatient event
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 1

    ***REMOVED*** ========================================================================
    ***REMOVED*** Calendar Subscription Tests
    ***REMOVED*** ========================================================================
    ...
    def test_validate_subscription_token_success(
        ...
        ***REMOVED*** Verify last_accessed_at was updated
    ...
    def test_validate_subscription_token_expired(
        ...
        ***REMOVED*** Create expired subscription
        subscription = CalendarSubscription.create(
            person_id=sample_resident.id,
            expires_days=1,
        )
        ***REMOVED*** Manually set expiration to past
    ...
    def test_list_subscriptions_no_filters(
        ...
        ***REMOVED*** Create subscriptions for different residents
    ...
    def test_list_subscriptions_filter_by_creator(
        ...
        ***REMOVED*** Create subscriptions with different creators
        CalendarService.create_subscription(
            db=db,
            person_id=sample_resident.id,
            created_by_user_id=admin_user.id,
    ...
    def test_list_subscriptions_active_only(
        ...
        ***REMOVED*** Create active subscription
        active_sub = CalendarService.create_subscription(
            db=db,
            person_id=sample_resident.id,
        )

        ***REMOVED*** Create revoked subscription
    ...
    def test_revoke_subscription_success(
        ...
        ***REMOVED*** Verify subscription is revoked
        db.refresh(subscription)
    ...
    ***REMOVED*** ========================================================================
    ...
    def test_generate_ics_for_person_no_assignments(
        ...
        ***REMOVED*** Should return valid calendar with no events
        assert "BEGIN:VCALENDAR" in ics_content
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 0
    ...
    def test_ics_includes_proper_timestamps(
        ...
        ***REMOVED*** Mock datetime.utcnow() for consistent testing
        ...
        ***REMOVED*** Parse calendar and verify timestamp fields
        cal = Calendar.from_ical(ics_content)
        events = [c for c in cal.walk() if c.name == "VEVENT"]
        assert len(events) == 1
