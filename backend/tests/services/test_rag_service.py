...
class TestRAGServiceChunking:
    ...
    def test_chunk_text_with_overlap(self, db):
        ...
        ***REMOVED*** Verify chunks overlap by checking for common content
        if len(chunks) > 1:
            ***REMOVED*** There should be some overlap between consecutive chunks
            ***REMOVED*** This is a weak test but sufficient for validation
    ...
    def test_chunk_text_small_input(self, db):
        ...
        ***REMOVED*** Small text should result in single chunk
        assert len(chunks) == 1
    ...
    def test_chunk_text_empty_input(self, db):
        ...
        ***REMOVED*** Empty text should result in empty chunks list
        assert len(chunks) == 0
...
class TestRAGServiceIngestion:
    ...
    @pytest.mark.skipif(
        True,  ***REMOVED*** Skip for SQLite tests
    ...
    async def test_ingest_document_success(self, db):
        ...
        """ * 5  ***REMOVED*** Make it long enough to chunk
    ...
    async def test_ingest_document_invalid_overlap(self, db):
        ...
        with pytest.raises(ValueError, match="chunk_overlap must be less than chunk_size"):
            await service.ingest_document(
                ...
                doc_type="acgme_rules",
                chunk_size=100,
                chunk_overlap=150,  ***REMOVED*** Greater than chunk_size
            )
...
class TestRAGServiceRetrieval:
    ...
    async def test_retrieve_documents(self, db):
        ...
        ***REMOVED*** First ingest some documents
        await service.ingest_document(
        ...
        ***REMOVED*** Most similar document should mention 80 hours
    ...
    async def test_retrieve_with_doc_type_filter(self, db):
        ...
        ***REMOVED*** Ingest documents of different types
        await service.ingest_document(
    ...
    async def test_retrieve_invalid_similarity(self, db):
        ...
        with pytest.raises(ValueError, match="min_similarity must be between"):
            await service.retrieve(
                query="test",
                min_similarity=1.5,  ***REMOVED*** Greater than 1.0
            )
...
class TestRAGServiceContextBuilding:
    ...
    async def test_build_context(self, db):
        ...
        ***REMOVED*** Ingest sample documents
        await service.ingest_document(
    ...
    async def test_build_context_respects_token_limit(self, db):
        ...
        ***REMOVED*** Ingest multiple large documents
        for i in range(10):
            await service.ingest_document(
        ...
        ***REMOVED*** Build context with small token limit
        context = await service.build_context(
            query="test",
            max_tokens=500,
        )

        ***REMOVED*** Should not exceed limit
    ...
    async def test_build_context_invalid_max_tokens(self, db):
        ...
        with pytest.raises(ValueError, match="max_tokens must be at least"):
            await service.build_context(
                query="test",
                max_tokens=50,  ***REMOVED*** Too small
            )
...
class TestRAGServiceDeletion:
    ...
    async def test_delete_by_doc_type(self, db):
        ...
        ***REMOVED*** Delete type_a documents
        count = service.delete_by_doc_type("type_a")

        assert count > 0

        ***REMOVED*** Verify type_a documents are gone
        health = await service.get_health()
    ...
    async def test_delete_by_id(self, db):
        ...
        ***REMOVED*** Try to delete again (should return False)
