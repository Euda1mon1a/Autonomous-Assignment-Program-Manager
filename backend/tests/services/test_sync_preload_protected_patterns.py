...
def test_preload_nf_combined_half_block_pattern(db):
    """NF combined (NF-first) uses half-block pattern: NF days 1-14, recovery day 15, specialty days 16+."""
    _create_activity(db, "OFF", "OFF", ActivityCategory.TIME_OFF.value)
    _create_activity(db, "NF", "NF", ActivityCategory.CLINICAL.value)
    _create_activity(db, "CARDS", "CARDS", ActivityCategory.CLINICAL.value)
    _create_activity(db, "W", "W", ActivityCategory.TIME_OFF.value)
    ...
    resident = Person(
        id=uuid4(), name="Resident NF-Combo", type="resident", pgy_level=3
    )
    template = RotationTemplate(
        id=uuid4(),
        name="NF + Cardiology",
    ...
    # First-half weekday (Monday after start) -> OFF/NF
    first_monday = _find_weekday(start, mid_block - timedelta(days=1), 0)
    assert first_monday is not None
    assert _get_assignment_code(db, resident.id, first_monday, "AM") == "OFF"
    assert _get_assignment_code(db, resident.id, first_monday, "PM") == "NF"

    # Mid-block day (day 15) -> recovery
    ...
    # Second-half weekday (Monday after mid-block) -> CARDS/CARDS
