...
class TestScheduleSlot:
    ...
    def test_key_consistency(self):
        ...
        key2 = slot.key
        assert key1 == key2
        assert key1 is not key2  ***REMOVED*** Different tuple instances
...
class TestScheduleConflict:
    ...
    def test_specialty_unavailable_message(self):
        ...
        conflict = ScheduleConflict(
            provider_name="Dr. Faculty-C-Last",
            date=date(2024, 3, 10),
            time_of_day="AM",
        ...
        assert "Dr. Faculty-C-Last" in conflict.message
...
class TestProviderSchedule:
    ...
    def test_get_slot_wrong_time(self):
        ...
        ***REMOVED*** Same date, different time
    ...
    def test_get_fmit_weeks_single_week(self):
        ...
        ***REMOVED*** Monday through Friday of same week
        for day_offset in range(5):
            slot = ScheduleSlot(
                provider_name="Dr. Test",
                date=date(2024, 1, 15) + timedelta(days=day_offset),  ***REMOVED*** Mon-Fri
                time_of_day="AM",
                slot_type=SlotType.FMIT,
                raw_value="FMIT",
            )
        ...
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 1
        ***REMOVED*** Should be Monday-Sunday of that week
        assert weeks[0][0] == date(2024, 1, 15)  ***REMOVED*** Monday
        assert weeks[0][1] == date(2024, 1, 21)  ***REMOVED*** Sunday

    def test_get_fmit_weeks_multiple_weeks(self):
        """Test get_fmit_weeks with slots in different weeks."""
        schedule = ProviderSchedule(name="Dr. Test")

        ***REMOVED*** Week 1: Jan 15-21 (Monday-Sunday)
        slot1 = ScheduleSlot(
            provider_name="Dr. Test",
            date=date(2024, 1, 15),  ***REMOVED*** Monday
            time_of_day="AM",
            slot_type=SlotType.FMIT,
            raw_value="FMIT",
        )
        ...
        ***REMOVED*** Week 2: Jan 29 - Feb 4 (skip one week)
        slot2 = ScheduleSlot(
            provider_name="Dr. Test",
            date=date(2024, 1, 29),  ***REMOVED*** Monday
            time_of_day="AM",
            slot_type=SlotType.FMIT,
            raw_value="FMIT",
        )
    ...
    def test_get_fmit_weeks_ignores_non_fmit(self):
        ...
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 1  ***REMOVED*** Only counts FMIT week

    def test_get_fmit_weeks_mid_week_start(self):
        """Test get_fmit_weeks when FMIT starts mid-week."""
        schedule = ProviderSchedule(name="Dr. Test")

        ***REMOVED*** Wednesday of a week
        slot = ScheduleSlot(
            provider_name="Dr. Test",
            date=date(2024, 1, 17),  ***REMOVED*** Wednesday
            time_of_day="AM",
            slot_type=SlotType.FMIT,
            raw_value="FMIT",
        )
        ...
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 1
        ***REMOVED*** Should still be Monday-Sunday of that week
        assert weeks[0][0] == date(2024, 1, 15)  ***REMOVED*** Monday
        assert weeks[0][1] == date(2024, 1, 21)  ***REMOVED*** Sunday
    ...
    def test_has_alternating_pattern_true(self):
        ...
        ***REMOVED*** Week 1, skip week 2, week 3, skip week 4, week 5
        ***REMOVED*** This creates alternating pattern
        week_dates = [
            date(2024, 1, 15),   ***REMOVED*** Week 1 (Mon)
            date(2024, 1, 29),   ***REMOVED*** Week 3 (skip week 2)
            date(2024, 2, 12),   ***REMOVED*** Week 5 (skip week 4)
        ]
    ...
    def test_has_alternating_pattern_consecutive_false(self):
        ...
        ***REMOVED*** Three consecutive weeks
        ...
        ***REMOVED*** Consecutive is not alternating
        assert schedule.has_alternating_pattern() is False
    ...
    def test_has_alternating_pattern_wide_gaps_false(self):
        ...
        ***REMOVED*** Weeks with large gaps (more than 2 weeks)
        week_dates = [
            date(2024, 1, 15),   ***REMOVED*** Week 1
            date(2024, 2, 5),    ***REMOVED*** Week 4 (3 week gap)
            date(2024, 2, 26),   ***REMOVED*** Week 7 (3 week gap)
        ]
        ...
        ***REMOVED*** Gaps too large for alternating pattern
        assert schedule.has_alternating_pattern() is False
    ...
    def test_has_alternating_pattern_boundary(self):
        ...
        ***REMOVED*** Exactly 7 day gap (boundary case)
        week_dates = [
            date(2024, 1, 15),   ***REMOVED*** Week 1 (Mon)
            date(2024, 1, 29),   ***REMOVED*** Week 3 (14 days later = 7 day gap after week 1 ends)
            date(2024, 2, 12),   ***REMOVED*** Week 5
        ]
...
class TestSlotTypeMapping:
    ...
    def test_compound_codes(self):
        ...
        ***REMOVED*** Should prioritize restrictive types
        assert importer.classify_slot("PC/OFF") == SlotType.OFF
        assert importer.classify_slot("C/CV") == SlotType.CLINIC
        assert importer.classify_slot("FMIT/VAC") == SlotType.FMIT  ***REMOVED*** FMIT is restrictive

    def test_prefix_matching(self):
        """Test prefix matching for longer codes."""
        importer = ClinicScheduleImporter()

        ***REMOVED*** Should match prefixes (2+ chars)
...
class TestEdgeCases:
    ...
    def test_leap_year_date(self):
        ...
        slot = ScheduleSlot(
            provider_name="Dr. Test",
            date=date(2024, 2, 29),  ***REMOVED*** 2024 is leap year
            time_of_day="AM",
            slot_type=SlotType.FMIT,
            raw_value="FMIT",
        )
    ...
    def test_schedule_with_single_slot(self):
        ...
        assert len(schedule.slots) == 1
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 0  ***REMOVED*** Not FMIT
    ...
    def test_schedule_all_same_type(self):
        ...
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 0  ***REMOVED*** No FMIT weeks
...
class TestHelperFunctions:
    ...
    def test_has_back_to_back_conflict_true(self):
        ...
        weeks = [
            date(2024, 1, 15),  ***REMOVED*** Week 1
            date(2024, 1, 22),  ***REMOVED*** Week 2 (7 days later)
        ]
    ...
    def test_count_alternating_cycles_no_alternating(self):
        ...
        ***REMOVED*** All consecutive (7 day gaps), not alternating
        assert count_alternating_cycles(weeks) == 0
    ...
    def test_count_alternating_cycles_alternating(self):
        ...
        weeks = [
            date(2024, 1, 15),   ***REMOVED*** Week 1
            date(2024, 1, 29),   ***REMOVED*** Week 3 (14 days = alternating)
            date(2024, 2, 12),   ***REMOVED*** Week 5 (14 days = alternating)
        ]
    ...
    def test_get_schedule_flexibility_boundary(self):
        ...
        ***REMOVED*** At exactly release date (90 days), condition is days_until < 90, which is False
        ***REMOVED*** So it returns "easy" (the else clause)
        assert flexibility == "easy"
...
class TestWeekCalculationBoundaries:
    ...
    def test_week_starting_sunday(self):
        ...
        ***REMOVED*** Sunday (weekday=6)
        ...
        weeks = schedule.get_fmit_weeks()
        assert len(weeks) == 1
        ***REMOVED*** Should group into Monday-Sunday of that week
        assert weeks[0][0] == date(2024, 1, 15)  ***REMOVED*** Monday
        assert weeks[0][1] == date(2024, 1, 21)  ***REMOVED*** Sunday
    ...
    def test_multiple_slots_same_week(self):
        ...
        ***REMOVED*** Add multiple slots throughout the week
        base_date = date(2024, 1, 15)  ***REMOVED*** Monday
        for i in range(7):  ***REMOVED*** Mon-Sun
            slot = ScheduleSlot(
                provider_name="Dr. Test",
        ...
        weeks = schedule.get_fmit_weeks()
        ***REMOVED*** Should still be one week
        assert len(weeks) == 1
    ...
    def test_sparse_fmit_slots(self):
        ...
        ***REMOVED*** Far apart FMIT slots
        dates = [
            date(2024, 1, 15),   ***REMOVED*** January
            date(2024, 6, 10),   ***REMOVED*** June (5 months later)
            date(2024, 12, 2),   ***REMOVED*** December (6 months later)
        ]
    ...
    def test_year_boundary_weeks(self):
        ...
        ***REMOVED*** Week that spans 2024-2025 boundary
        ***REMOVED*** Dec 30, 2024 is a Monday
        slot = ScheduleSlot(
            provider_name="Dr. Test",
            date=date(2024, 12, 30),  ***REMOVED*** Monday
            time_of_day="AM",
            slot_type=SlotType.FMIT,
            raw_value="FMIT",
        )
        ...
        assert weeks[0][1] == date(2025, 1, 5)  ***REMOVED*** Sunday in new year
