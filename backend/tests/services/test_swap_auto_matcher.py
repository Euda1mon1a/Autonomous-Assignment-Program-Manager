...
class TestSwapAutoMatcher:
    ...
    def test_find_compatible_swaps_single_request(
        ...
        assert len(matches) == 0  ***REMOVED*** No other compatible requests
    ...
    def test_find_compatible_swaps_only_pending(
        ...
        ***REMOVED*** Complete swap_request_2
        ...
        ***REMOVED*** Should not match completed swaps
        assert isinstance(matches, list)
    ...
    def test_find_compatible_swaps_completed_request(
        ...
        ***REMOVED*** Complete the request
        swap_request_1.status = SwapStatus.COMPLETED
        db.commit()

        matcher = SwapAutoMatcher(db)

        ***REMOVED*** Finding matches for completed request should fail or return empty
        try:
            matches = matcher.find_compatible_swaps(swap_request_1.id)
            assert len(matches) == 0
        except ValueError:
            ***REMOVED*** May throw if request status is wrong
            pass
    ...
    def test_score_compatibility_same_dates(
        ...
        ***REMOVED*** Same dates should have higher score
        assert isinstance(score, (int, float))
    ...
    def test_score_compatibility_distant_dates(
        ...
        ***REMOVED*** Distant dates should have lower score
        assert isinstance(score, (int, float))
    ...
    def test_find_compatible_swaps_return_type(
        ...
        for match in matches:
            ***REMOVED*** Each match should be a SwapMatch object
    ...
    def test_matcher_with_three_candidates(
        ...
        ***REMOVED*** Should find multiple matches if available
        assert isinstance(matches, list)
    ...
    def test_matcher_database_session_persistence(self, db: Session):
        ...
        ***REMOVED*** Session should be usable after matcher creation
