...
def create_call_assignments(
    ...
    for day_offset in range(7):
        current_date = week_start + timedelta(days=day_offset)
        ***REMOVED*** Friday (4) and Saturday (5)
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test 1: Create swap request → approval → execution flow
***REMOVED*** ============================================================================
...
class TestSwapRequestApprovalExecutionFlow:
    ...
    def test_one_to_one_swap_full_lifecycle(
        ...
        ***REMOVED*** Setup: Create FMIT assignments for both faculty
        ...
        ***REMOVED*** Step 1: Create swap request
        ...
        ***REMOVED*** Verify swap record created
        ...
        ***REMOVED*** Step 2: Target faculty responds (accepts)
        ...
        ***REMOVED*** Verify swap was executed
        ...
        ***REMOVED*** Week A assignments should now belong to faculty_b
        ...
        ***REMOVED*** Week B assignments should now belong to faculty_a
    ...
    def test_absorb_swap_full_lifecycle(
        ...
        ***REMOVED*** Setup: Create FMIT assignment for faculty_a only
        week_a = date.today() + timedelta(days=14)
        blocks_a = create_blocks_for_week(db, week_a)
        assignments_a = create_fmit_assignments(db, faculty_a, blocks_a, fmit_template)

        ***REMOVED*** Create swap request (absorb type - no desired weeks)
        swap_service = SwapRequestService(db)
        request_result = swap_service.create_request(
            requester_id=faculty_a.id,
            source_week=week_a,
            desired_weeks=None,  ***REMOVED*** No desired weeks = ABSORB
        ...
        ***REMOVED*** Accept the absorb swap
        response_result = swap_service.respond_to_request(
            request_id=request_id,
            faculty_id=faculty_b.id,
            accept=True,
        ...
        ***REMOVED*** Verify assignments transferred to faculty_b
        for assignment in assignments_a:
            db.refresh(assignment)
            assert assignment.person_id == faculty_b.id
    ...
    def test_swap_rejection_flow(
        ...
        ***REMOVED*** Verify swap was rejected
        swap_record = db.query(SwapRecord).filter_by(id=request_id).first()
    ...
    def test_counter_offer_flow(
        ...
        week_a = date.today() + timedelta(days=14)
        week_b = date.today() + timedelta(days=21)
        week_c = date.today() + timedelta(days=28)  ***REMOVED*** Counter-offer week
        ...
        ***REMOVED*** Counter-offer with different week
        response_result = swap_service.respond_to_request(
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test 2: Swap rollback within 24-hour window
***REMOVED*** ============================================================================
...
class TestSwapRollback:
    ...
    def test_rollback_within_24_hour_window(
        ...
        ***REMOVED*** Verify swap status changed
        db.refresh(swap_record)
        ...
        ***REMOVED*** Verify assignments were restored
        for assignment in assignments_a:
            db.refresh(assignment)
            assert assignment.person_id == faculty_a.id
    ...
    def test_rollback_outside_24_hour_window(
        ...
        ***REMOVED*** Manually update executed_at to be >24 hours ago
        swap_record = db.query(SwapRecord).filter_by(id=swap_id).first()
        swap_record.executed_at = datetime.utcnow() - timedelta(hours=25)
        db.commit()

        ***REMOVED*** Check can_rollback returns False
        can_rollback = executor.can_rollback(swap_id)
        assert can_rollback is False

        ***REMOVED*** Attempt rollback should fail
        rollback_result = executor.rollback_swap(
            swap_id=swap_id,
            reason="Too late",
        )
        ...
        ***REMOVED*** Verify status unchanged
        db.refresh(swap_record)
        assert swap_record.status == SwapStatus.EXECUTED
    ...
    def test_rollback_call_cascade(
        ...
        week_a = date.today() + timedelta(days=14)
        ***REMOVED*** Ensure week starts on a Monday for predictable Friday/Saturday
        ...
        ***REMOVED*** Verify call cascade was updated to faculty_b
        ...
        ***REMOVED*** Verify call cascade was restored to faculty_a
        for call_assignment in call_assignments_a:
            db.refresh(call_assignment)
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test 3: Auto-matching swap candidates
***REMOVED*** ============================================================================
...
class TestSwapAutoMatching:
    ...
    def test_auto_match_finds_compatible_swaps(
        ...
        ***REMOVED*** Create mutual swap requests (A wants B's week, B wants A's week)
        swap_a = SwapRecord(
            id=uuid4(),
            source_faculty_id=faculty_a.id,
            source_week=week_a,
        ...
        ***REMOVED*** Verify the match is mutual
    ...
    def test_auto_matcher_suggests_optimal_matches(
        ...
        ***REMOVED*** Create faculty preferences to influence scoring
        ...
        ***REMOVED*** Create pending swap request from faculty_a
        ...
        ***REMOVED*** Use auto-matcher to suggest matches
        matcher = SwapAutoMatcher(db)
        matches = matcher.suggest_optimal_matches(swap_request.id, top_k=5)

        ***REMOVED*** Should have matches since faculty_b prefers week_a
        assert len(matches) >= 0  ***REMOVED*** May or may not find matches depending on data
    ...
    def test_batch_auto_match_pending_requests(
        ...
        ***REMOVED*** Create multiple pending requests
        for i in range(3):
            swap = SwapRecord(
                id=uuid4(),
                source_faculty_id=faculty_a.id,
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test 4: ACGME validation after swap execution
***REMOVED*** ============================================================================
...
class TestACGMEValidationAfterSwap:
    ...
    def test_swap_maintains_acgme_compliance(
        ...
        ***REMOVED*** Create clinic assignments for residents (require supervision)
        for resident in residents[:2]:  ***REMOVED*** 2 residents
            for block in blocks[:4]:  ***REMOVED*** First 2 days
                assignment = Assignment(
                    id=uuid4(),
                    block_id=block.id,
        ...
        ***REMOVED*** Assign faculty_a as supervisor
        ...
        ***REMOVED*** Should be compliant (2 residents, 1 faculty = 2:1 ratio, within 4:1 limit)
        assert result_before.valid is True

        ***REMOVED*** Now swap clinic assignments - this should maintain compliance
        ***REMOVED*** (Implementation would update assignments to different faculty)

        ***REMOVED*** Validate after hypothetical swap
        result_after = validator.validate_all(
            start_date=week_start,
            end_date=week_start + timedelta(days=6),
        )

        ***REMOVED*** Should still be compliant
    ...
    def test_swap_detects_acgme_violation(
        ...
        ***REMOVED*** Create scenario that would violate supervision ratio
        ***REMOVED*** Assign 5 residents to clinic (exceeds 4:1 ratio with 0 faculty)
        ...
        ***REMOVED*** Validate - should detect supervision ratio violation
        validator = ACGMEValidator(db)
        ...
        ***REMOVED*** Should have violations (no supervising faculty)
        assert len(result.violations) > 0


***REMOVED*** ============================================================================
***REMOVED*** E2E Test 5: Notification triggers during swap lifecycle
***REMOVED*** ============================================================================
...
class TestSwapNotifications:
    ...
    def test_notification_on_swap_request_received(
        ...
        ***REMOVED*** Create notification for swap request
        ...
        ***REMOVED*** Verify notification is pending
    ...
    def test_notification_on_swap_executed(
        ...
        ***REMOVED*** Should send notification to both parties
    ...
    def test_send_pending_notifications(
        ...
        ***REMOVED*** Should have 2 pending
        assert notifier.get_pending_count() == 2

        ***REMOVED*** Send all pending
        sent_count = notifier.send_pending_notifications()

        ***REMOVED*** Should attempt to send all (may fail if email service not configured)
        assert sent_count >= 0

        ***REMOVED*** Pending queue should be cleared
    ...
    def test_full_lifecycle_with_notifications(
        ...
        ***REMOVED*** Create request (should trigger notification)
        request_result = swap_service.create_request(
            requester_id=faculty_a.id,
            source_week=week_a,
            desired_weeks=[week_b],
        ...
        ***REMOVED*** Verify notification was triggered
        assert request_result.candidates_notified == 1

        ***REMOVED*** Accept swap (should trigger acceptance + execution notifications)
        response_result = swap_service.respond_to_request(
            request_id=request_id,
            faculty_id=faculty_b.id,
            accept=True,
        ...
        ***REMOVED*** Notifications should have been queued and sent
        ***REMOVED*** (Actual email sending depends on email service configuration)
...
class TestSwapLifecycleIntegration:
    ...
    def test_complete_swap_with_all_features(
        ...
        ***REMOVED*** Ensure week_a starts on Monday for predictable call cascade
        week_a = week_a - timedelta(days=week_a.weekday())
        ...
        ***REMOVED*** Step 1: Validate before swap
        ...
        ***REMOVED*** Step 3: Verify assignments swapped
        for assignment in assignments_a:
            db.refresh(assignment)
            assert assignment.person_id == faculty_b.id
        ...
        ***REMOVED*** Step 4: Verify call cascade swapped
        ...
        ***REMOVED*** Step 5: Verify can rollback
        can_rollback = executor.can_rollback(swap_id)
        assert can_rollback is True

        ***REMOVED*** Step 6: Execute rollback
        rollback_result = executor.rollback_swap(
            swap_id=swap_id,
        ...
        ***REMOVED*** Step 7: Verify everything restored
        for assignment in assignments_a:
            db.refresh(assignment)
            assert assignment.person_id == faculty_a.id
        ...
        ***REMOVED*** Step 8: Verify swap record status
        swap_record = db.query(SwapRecord).filter_by(id=swap_id).first()
        assert swap_record.status == SwapStatus.ROLLED_BACK
