...
def user_admin_setup(db: Session, admin_user: User) -> dict:
    ...
    ***REMOVED*** Create users with different roles
    user_configs = [
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Complete User Administration Workflow
***REMOVED*** ============================================================================
...
class TestUserAdministrationE2E:
    ...
    def test_full_user_lifecycle_workflow(
        ...
        """
        ***REMOVED*** Step 1: Admin creates a new coordinator user
        ...
        ***REMOVED*** Step 2: New user logs in
        ...
        ***REMOVED*** Step 3: Verify user can access their own info
        ...
        ***REMOVED*** Step 4: Verify user cannot access admin-only endpoints
        ...
        ***REMOVED*** Verify user cannot create other users
        ...
        ***REMOVED*** Step 5: User logs out
        logout_response = client.post("/api/auth/logout", headers=user_headers)

        assert logout_response.status_code == 200
        assert logout_response.json()["message"] == "Successfully logged out"

        ***REMOVED*** Step 6: Verify token is invalidated after logout
    ...
    def test_admin_manages_multiple_users(
        ...
        """
        ***REMOVED*** Step 1: Admin creates users with different roles
        roles_to_create = [
        ...
        ***REMOVED*** Step 2: Admin lists all users
    ...
    def test_role_based_access_verification(
        ...
        ***REMOVED*** Step 1: Admin can access all endpoints
        ...
        ***REMOVED*** Step 2-4: Non-admin users cannot access admin endpoints
        non_admin_roles = ["coordinator", "faculty", "resident"]
        for role in non_admin_roles:
            ***REMOVED*** Login as non-admin user
            ...
            ***REMOVED*** Try to access admin endpoints
            ...
            ***REMOVED*** Step 5: All users can access their own information
            me_response = client.get("/api/auth/me", headers=user_headers)
            assert me_response.status_code == 200
            me_data = me_response.json()
    ...
    def test_user_authentication_tokens(
        ...
        ***REMOVED*** Step 1: User receives valid JWT on login
        login_response = client.post(
            "/api/auth/login/json",
        ...
        ***REMOVED*** Step 2: Token contains correct user information
        ...
        ***REMOVED*** Step 3: Token can be used to access protected endpoints
        user_headers = {"Authorization": f"Bearer {token}"}
        me_response = client.get("/api/auth/me", headers=user_headers)
        assert me_response.status_code == 200
        assert me_response.json()["username"] == "token_test_user"

        ***REMOVED*** Step 4: Expired tokens are rejected
        ...
        ***REMOVED*** Step 5: Invalid tokens are rejected
    ...
    def test_first_user_becomes_admin(
        ...
        """
        ***REMOVED*** Step 1: Clear all users
        db.query(User).delete()
        db.commit()

        ***REMOVED*** Step 2: Register first user
        response = client.post(
            "/api/auth/register",
            json={
                "username": "firstuser",
                ...
                "password": "password123",
                "role": "coordinator",  ***REMOVED*** Request coordinator role
            },
        )

        ***REMOVED*** Step 3: Verify user is auto-promoted to admin
        assert response.status_code == 201
        user_data = response.json()
        assert user_data["username"] == "firstuser"
        assert user_data["role"] == "admin"  ***REMOVED*** Auto-promoted
        assert user_data["is_active"] is True

        ***REMOVED*** Step 4: Verify admin can perform admin operations
        ***REMOVED*** Login as first user
        login_response = client.post(
            "/api/auth/login/json",
        ...
        ***REMOVED*** Create another user (admin operation)
        create_response = client.post(
            "/api/auth/register",
            json={
                "username": "seconduser",
        ...
        assert create_response.status_code == 201
        assert create_response.json()["role"] == "coordinator"  ***REMOVED*** Not auto-promoted

        ***REMOVED*** List users (admin operation)
...
class TestUserAdministrationEdgeCases:
    ...
    def test_duplicate_username_rejection(
        ...
        """
        ***REMOVED*** Step 1: Create first user
        ...
        ***REMOVED*** Step 2: Try to create duplicate
        response2 = client.post(
            "/api/auth/register",
            json={
                "username": "duplicate_test",  ***REMOVED*** Same username
                "email": "user2@test.org",  ***REMOVED*** Different email
                "password": "password123",
                "role": "coordinator",
            },
            headers=auth_headers,
        ...
        ***REMOVED*** Step 3: Verify rejection
    ...
    def test_duplicate_email_rejection(
        ...
        ***REMOVED*** Step 2: Try to create duplicate email
        response2 = client.post(
            "/api/auth/register",
            json={
                "username": "user2",  ***REMOVED*** Different username
                "email": "duplicate@test.org",  ***REMOVED*** Same email
                "password": "password123",
                "role": "coordinator",
            },
            headers=auth_headers,
    ...
    def test_invalid_role_rejection(
        ...
        """
        ***REMOVED*** Test 1: Non-existent role
        response1 = client.post(
            "/api/auth/register",
            json={
                ...
                "password": "password123",
                "role": "super_admin",  ***REMOVED*** Invalid role
            },
            headers=auth_headers,
        )
        assert response1.status_code in [400, 422]

        ***REMOVED*** Test 2: Empty role (should use default or fail)
        response2 = client.post(
            "/api/auth/register",
            json={
    ...
    def test_weak_password_handling(
        ...
        """
        ***REMOVED*** Test 1: Very short password
        response1 = client.post(
            "/api/auth/register",
            json={
                "username": "short_pw_user",
                "email": "shortpw@test.org",
                "password": "123",  ***REMOVED*** Very short
                "role": "coordinator",
            },
            headers=auth_headers,
        )
        ***REMOVED*** May be accepted or rejected depending on validation rules
        assert response1.status_code in [201, 400, 422]

        ***REMOVED*** Test 2: Empty password
        response2 = client.post(
            "/api/auth/register",
            json={
        ...
        assert response2.status_code == 422  ***REMOVED*** Validation error

        ***REMOVED*** Test 3: Password is hashed
        ...
        if response3.status_code == 201:
            ***REMOVED*** Verify password is not stored in plain text
    ...
    def test_inactive_user_cannot_login(
        ...
        """
        ***REMOVED*** Step 1: Create active user
        create_response = client.post(
            "/api/auth/register",
            json={
        ...
        ***REMOVED*** Step 2: User logs in successfully
        login_response = client.post(
            "/api/auth/login/json",
        ...
        ***REMOVED*** Step 3: Deactivate user
        user = db.query(User).filter(User.username == "inactive_test").first()
        user.is_active = False
        db.commit()

        ***REMOVED*** Step 4: Verify user cannot log in
        ...
        ***REMOVED*** Step 5: Verify existing token is invalidated
    ...
    def test_concurrent_user_creation(
        ...
        """
        ***REMOVED*** Try to create same user twice
        user_data = {
        ...
        ***REMOVED*** One should succeed, one should fail
        ...
        assert 201 in statuses  ***REMOVED*** One succeeds
        assert 400 in statuses  ***REMOVED*** One fails due to duplicate
    ...
    def test_user_list_pagination_and_ordering(
        ...
        """
        ***REMOVED*** Create multiple users
        for i in range(5):
            client.post(
                "/api/auth/register",
                json={
                    "username": f"list_user_{chr(97 + i)}",  ***REMOVED*** a, b, c, d, e
        ...
        ***REMOVED*** Get user list
        response = client.get("/api/auth/users", headers=auth_headers)
        assert response.status_code == 200
        users = response.json()

        ***REMOVED*** Test 1: Consistent ordering
        usernames = [u["username"] for u in users]
        assert usernames == sorted(usernames)

        ***REMOVED*** Test 2: All fields present
        if users:
            user = users[0]
            assert "id" in user
        ...
        ***REMOVED*** Test 3: Sensitive fields excluded
        for user in users:
    ...
    def test_token_refresh_after_user_creation(
        ...
        """
        ***REMOVED*** Step 1: Create user and log in
        client.post(
            "/api/auth/register",
            json={
        ...
        ***REMOVED*** Step 2: Use refresh token
        ...
        ***REMOVED*** Step 3: Verify new access token works
        me_response = client.get(
        ...
        ***REMOVED*** Step 4: Verify old refresh token is invalidated
