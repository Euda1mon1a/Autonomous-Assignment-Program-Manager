...
def complete_program_setup(db: Session) -> dict:
    ...
    """
    ***REMOVED*** Create residents (2 per PGY level)
    residents = []
    ...
    ***REMOVED*** Create faculty members
    ...
    for i, specs in enumerate(specialties_list, 1):
        fac = Person(
            ...
            performs_procedures=(i % 2 == 1),  ***REMOVED*** Faculty 1 and 3 do procedures
            specialties=specs,
        )
        db.add(fac)
        faculty.append(fac)
    ...
    ***REMOVED*** Create blocks for 2 weeks (14 days)
    blocks = []
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Complete Schedule Generation Workflow
***REMOVED*** ============================================================================
...
class TestScheduleGenerationWorkflow:
    ...
    def test_full_schedule_generation_greedy_algorithm(
        ...
        ***REMOVED*** Step 1: Generate schedule using greedy algorithm
        ...
        ***REMOVED*** Verify generation succeeded
        if generate_response.status_code == 200:
            ...
            ***REMOVED*** Step 2: Verify assignments were created in database
            assignments = (
                db.query(Assignment)
                .join(Block)
                .filter(
            ...
            ***REMOVED*** Step 3: Validate ACGME compliance
            ...
            ***REMOVED*** Note: Greedy algorithm may not always produce compliant schedules
            ***REMOVED*** but should produce some result
            assert validation_result.total_violations >= 0

            ***REMOVED*** Step 4: Export to CSV
            ...
            ***REMOVED*** Should succeed or require additional permissions
            assert csv_response.status_code in [200, 401, 403, 404]

            ***REMOVED*** Step 5: Export to JSON
            ...
            ***REMOVED*** Schedule generation may fail due to auth or missing routes
            ***REMOVED*** This is acceptable in test environment
    ...
    def test_schedule_generation_with_validation(
        ...
        ***REMOVED*** Create scheduling engine
        ...
        ***REMOVED*** Generate schedule
        result = engine.generate(
            algorithm="greedy",
            timeout_seconds=30,
            check_resilience=False,  ***REMOVED*** Skip resilience check for faster test
        )
        ...
        ***REMOVED*** Should have created some assignments
        ***REMOVED*** (exact count depends on algorithm and constraints)
    ...
    def test_schedule_generation_multiple_algorithms(
        ...
        """
        setup = complete_program_setup
        algorithms = ["greedy"]  ***REMOVED*** Start with greedy, add others if available

        results = {}

        for algorithm in algorithms:
            ***REMOVED*** Fresh engine for each algorithm
            engine = SchedulingEngine(
                db=db,
                start_date=setup["start_date"],
                end_date=setup["end_date"],
            ...
            ***REMOVED*** Clear previous assignments to prevent conflicts
            ...
            ***REMOVED*** Generate with this algorithm
            try:
                result = engine.generate(
                    algorithm=algorithm,
                    timeout_seconds=30,
                ...
                ***REMOVED*** Verify basic result structure
                assert result is not None
                assert "status" in result
        ...
        ***REMOVED*** If multiple algorithms ran, verify they all produced results
    ...
    def test_export_formats_consistency(
        ...
        ***REMOVED*** Create some sample assignments manually
        sample_assignments = []
        for i, resident in enumerate(setup["residents"][:3]):
            for j, block in enumerate(setup["blocks"][:6]):
                if j % 2 == i % 2:  ***REMOVED*** Alternate assignments
        ...
        ***REMOVED*** Verify both exports succeeded or failed for same reason
        if csv_response.status_code == 200:
            assert json_response.status_code == 200, "Both exports should succeed"

            ***REMOVED*** Parse CSV (basic validation - count non-header lines)
            csv_content = csv_response.content.decode("utf-8")
            csv_lines = [line for line in csv_content.split("\n") if line.strip()]
            csv_data_rows = len(csv_lines) - 1  ***REMOVED*** Subtract header

            ***REMOVED*** Parse JSON
            json_data = json_response.json()
            json_rows = len(json_data) if isinstance(json_data, list) else 0

            ***REMOVED*** They should have similar counts (allowing for formatting differences)
    ...
    def test_acgme_validation_after_generation(
        ...
        ***REMOVED*** Create schedule with intentional 80-hour violation
        ***REMOVED*** Assign resident to all blocks for a week (should exceed 80 hours)
        violation_blocks = [
            b for b in setup["blocks"][:14]  ***REMOVED*** 7 days * 2 blocks/day
        ]
        ...
        for block in violation_blocks:
            assignment = Assignment(
                id=uuid4(),
                person_id=resident.id,
                rotation_template_id=setup["templates"][1].id,  ***REMOVED*** Inpatient
                block_id=block.id,
                role="primary",
            )
            db.add(assignment)
        ...
        ***REMOVED*** Verify validation detected violations
        ***REMOVED*** (14 blocks * 6 hours/block = 84 hours in one week)
        assert validation_result is not None
        ***REMOVED*** Note: Specific violation detection depends on validator implementation
        ***REMOVED*** This test ensures validator runs and produces a result
    ...
    def test_schedule_generation_error_handling(
        ...
        """
        ***REMOVED*** Test 1: Invalid date range
        response = client.post(
            "/api/schedule/generate",
            json={
                "start_date": "2025-12-31",
                "end_date": "2025-01-01",  ***REMOVED*** End before start
                "algorithm": "greedy",
            },
            headers=auth_headers,
        )
        ...
        ***REMOVED*** Test 2: Invalid algorithm
        response = client.post(
            "/api/schedule/generate",
            json={
        ...
        ***REMOVED*** Should either reject or default to greedy
        assert response.status_code in [200, 400, 422]

        ***REMOVED*** Test 3: Missing required fields
        response = client.post(
            "/api/schedule/generate",
            json={
                "algorithm": "greedy",
                ***REMOVED*** Missing start_date and end_date
            },
            headers=auth_headers,
        )
    ...
    def test_schedule_persistence_across_sessions(
        ...
        ***REMOVED*** Verify initial creation
        assignments_before = (
            db.query(Assignment)
            .join(Block)
            .filter(
        ...
        ***REMOVED*** Commit and close session
        db.commit()
        db.flush()

        ***REMOVED*** Query again (simulating new session)
        assignments_after = (
            db.query(Assignment)
            .join(Block)
            .filter(
    ...
    def test_concurrent_validation_and_export(
        ...
        ***REMOVED*** Simultaneously query for export
        assignments = (
            db.query(Assignment)
            .join(Block)
            .filter(
        ...
        ***REMOVED*** Both operations should complete successfully
        assert validation_result is not None
        assert len(assignments) > 0


***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Edge Cases and Integration Scenarios
***REMOVED*** ============================================================================
...
class TestScheduleGenerationEdgeCases:
    ...
    def test_empty_schedule_generation(
        ...
        """
        ***REMOVED*** Create empty date range
        start_date = date.today()
        ...
        ***REMOVED*** Should handle empty data without crashing
        result = engine.generate(
            algorithm="greedy",
            timeout_seconds=10,
            check_resilience=False,
    ...
    def test_long_schedule_generation(
        ...
        setup = complete_program_setup
        start_date = setup["start_date"]
        end_date = start_date + timedelta(days=28)  ***REMOVED*** 4 weeks

        ***REMOVED*** Need to create additional blocks
        existing_blocks = db.query(Block).filter(
            Block.date >= start_date,
            Block.date <= end_date,
        ).count()

        ***REMOVED*** Create missing blocks
        for i in range(14, 29):  ***REMOVED*** Days 14-28 (we already have 0-13)
            current_date = start_date + timedelta(days=i)
            for time_of_day in ["AM", "PM"]:
                block = Block(
                    id=uuid4(),
        ...
        result = engine.generate(
            algorithm="greedy",
            timeout_seconds=60,  ***REMOVED*** Allow more time for larger schedule
            check_resilience=False,
        )
        ...
        ***REMOVED*** Validate 4-week period (important for rolling window checks)
        validator = ACGMEValidator(db)
        validation_result = validator.validate_all(
            start_date=start_date,
            end_date=end_date,
        ...
        assert validation_result is not None
        ***REMOVED*** Should check rolling 4-week windows
        assert validation_result.total_violations >= 0
