...
def create_clinic_schedule_xlsx(
    providers: list[str],
    dates: list[date],
    schedule_data: dict[str, dict[date, tuple[str, str]]],  ***REMOVED*** provider -> date -> (AM, PM)
) -> bytes:
    """
    ...
    ***REMOVED*** Header row 1: Dates
    header_row = ["Provider"]
    for d in dates:
        header_row.extend([d, d])  ***REMOVED*** Two columns per date (AM/PM)

    ws.append(header_row)

    ***REMOVED*** Header row 2: AM/PM labels
    am_pm_row = [""]
    for _ in dates:
    ...
    ***REMOVED*** Data rows: One row per provider
    for provider in providers:
        ...
        for d in dates:
            if provider in schedule_data and d in schedule_data[provider]:
                ...
                row.extend(["", ""])  ***REMOVED*** Empty slots

        ws.append(row)

    ***REMOVED*** Save to bytes
    buffer = io.BytesIO()
    wb.save(buffer)
    buffer.seek(0)
    return buffer.read()
...
def create_fmit_schedule_xlsx(
    faculty: list[str],
    weeks: list[tuple[date, date]],  ***REMOVED*** List of (week_start, week_end) tuples
    assignments: dict[str, list[int]],  ***REMOVED*** faculty_name -> list of week indices
) -> bytes:
    """
    ...
    ***REMOVED*** Header row: Faculty names
    header_row = ["Week"]
    header_row.extend(faculty)
    ws.append(header_row)

    ***REMOVED*** Data rows: One row per week
    for week_idx, (week_start, week_end) in enumerate(weeks):
        week_label = f"{week_start.strftime('%b %d')} - {week_end.strftime('%b %d')}"
        row = [week_label]

        for fac_name in faculty:
            ***REMOVED*** Check if this faculty is assigned to this week
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Complete XLSX Import Workflow
***REMOVED*** ============================================================================
...
class TestXLSXImportWorkflowE2E:
    ...
    def test_full_clinic_import_workflow(
        ...
        """
        ***REMOVED*** Step 1: Create test clinic schedule
        providers = ["Dr. Alice Smith", "Dr. Bob Johnson"]
        today = date.today()
        dates = [today + timedelta(days=i) for i in range(7)]  ***REMOVED*** One week

        schedule_data = {
            "Dr. Alice Smith": {
                dates[0]: ("C", "C"),  ***REMOVED*** Clinic all day Monday
                dates[1]: ("C", "OFF"),  ***REMOVED*** Clinic AM Tuesday
                dates[2]: ("OFF", "OFF"),  ***REMOVED*** Off Wednesday
            },
            "Dr. Bob Johnson": {
                dates[0]: ("FMIT", "FMIT"),  ***REMOVED*** FMIT all day Monday
                dates[1]: ("C", "C"),  ***REMOVED*** Clinic all day Tuesday
                dates[2]: ("C", "C"),  ***REMOVED*** Clinic all day Wednesday
            },
        }

        xlsx_bytes = create_clinic_schedule_xlsx(providers, dates, schedule_data)

        ***REMOVED*** Step 2: Upload and analyze via API
        ...
        ***REMOVED*** Step 3: Verify response
        assert response.status_code == 200
        result = response.json()

        ***REMOVED*** Basic success check
        assert result["success"] is True

        ***REMOVED*** Step 4: Verify using service directly for detailed validation
        ...
        ***REMOVED*** Step 5: Validate slot classification
        ...
        ***REMOVED*** Step 6: Validate statistics
    ...
    def test_conflict_detection_workflow(
        ...
        - Conflict reporting
        """
        ***REMOVED*** Create schedule with conflicts
        providers = ["Dr. Smith", "Dr. Jones"]
        today = date.today()
        dates = [today + timedelta(days=i) for i in range(3)]

        ***REMOVED*** Create double-booking conflict
        schedule_data = {
            "Dr. Smith": {
                dates[0]: ("C", "C"),  ***REMOVED*** Clinic (would conflict if also FMIT)
                dates[1]: ("FMIT", "FMIT"),  ***REMOVED*** FMIT all day
                dates[2]: ("C", "C"),  ***REMOVED*** Clinic
            },
            "Dr. Jones": {
                dates[0]: ("C", "C"),
                dates[1]: ("C", "C"),
        ...
        ***REMOVED*** Use service directly to detect conflicts
        importer = ClinicScheduleImporter(db)
        result = importer.import_file(file_bytes=xlsx_bytes)

        assert result.success is True

        ***REMOVED*** Verify schedule was parsed
    ...
    def test_fmit_alternating_pattern_detection(
        ...
        """
        ***REMOVED*** Create FMIT schedule with alternating pattern
        faculty = ["Dr. Williams"]
        today = date.today()

        ***REMOVED*** Create alternating weeks (week on, week off, week on, week off)
        weeks = [
            (today, today + timedelta(days=6)),  ***REMOVED*** Week 1
            (today + timedelta(days=14), today + timedelta(days=20)),  ***REMOVED*** Week 3 (skip 2)
            (today + timedelta(days=28), today + timedelta(days=34)),  ***REMOVED*** Week 5 (skip 4)
        ]

        assignments = {"Dr. Williams": [0, 1, 2]}  ***REMOVED*** All weeks assigned

        xlsx_bytes = create_fmit_schedule_xlsx(faculty, weeks, assignments)

        ***REMOVED*** Parse and check for alternating pattern
        importer = ClinicScheduleImporter(db)
        result = importer.import_file(file_bytes=xlsx_bytes)
        ...
        ***REMOVED*** Note: This tests the has_alternating_pattern method
        ***REMOVED*** The actual detection requires at least 3 FMIT weeks
        ***REMOVED*** and gaps of ~7 days between them
        fmit_weeks = williams_schedule.get_fmit_weeks()
        assert len(fmit_weeks) >= 3  ***REMOVED*** Has multiple FMIT weeks
    ...
    def test_invalid_file_handling(
        ...
        ***REMOVED*** Should reject corrupt file
        assert response.status_code in [400, 422]

        ***REMOVED*** Test 2: Use service directly to verify error handling
        importer = ClinicScheduleImporter(db)
    ...
    def test_empty_schedule_handling(
        ...
        - Single day
        """
        ***REMOVED*** Create minimal schedule (1 provider, 1 day)
        providers = ["Dr. Solo"]
        dates = [date.today()]
        schedule_data = {
            "Dr. Solo": {
        ...
        assert result.total_slots == 2  ***REMOVED*** AM and PM
    ...
    def test_transposed_format_import(
        ...
        """
        ***REMOVED*** Create transposed format
        wb = Workbook()
        ws = wb.active
        ws.title = "Schedule"

        ***REMOVED*** Header: providers in columns
        ...
        ***REMOVED*** Import and verify
        importer = ClinicScheduleImporter(db)
        result = importer.import_file(file_bytes=xlsx_bytes)

        ***REMOVED*** Should detect transposed format and parse correctly
        assert result.success is True
        ***REMOVED*** Depending on implementation, this may or may not work
        ***REMOVED*** The test validates the importer handles both formats
    ...
    def test_special_slot_types_classification(
        ...
        schedule_data = {
            "Dr. Test": {
                dates[0]: ("VAC", "VAC"),  ***REMOVED*** Vacation
                dates[1]: ("CONF", "CONF"),  ***REMOVED*** Conference
                dates[2]: ("ADMIN", "ADMIN"),  ***REMOVED*** Admin
                dates[3]: ("C", "C"),  ***REMOVED*** Clinic
                dates[4]: ("FMIT", "FMIT"),  ***REMOVED*** FMIT
                dates[5]: ("XYZ123", "UNKNOWN"),  ***REMOVED*** Unknown codes
            }
        }
        ...
        ***REMOVED*** Verify classifications
    ...
    def test_multi_provider_schedule_import(
        ...
        ***REMOVED*** Verify all providers parsed
        ...
        ***REMOVED*** Verify statistics
        assert result.total_slots == len(provider_names) * len(dates) * 2  ***REMOVED*** AM + PM
        assert result.clinic_slots > 0
        assert result.fmit_slots > 0


***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Service Integration
***REMOVED*** ============================================================================
...
class TestXLSXImportServiceIntegration:
    ...
    def test_format_detection_providers_in_rows(
        ...
        ***REMOVED*** Load workbook to test format detection
        ...
        ***REMOVED*** Should detect providers in rows
    ...
    def test_provider_schedule_methods(
        ...
        ***REMOVED*** Add slots
        dates = [date.today() + timedelta(days=i * 7) for i in range(4)]  ***REMOVED*** 4 weeks
        ...
        ***REMOVED*** Test get_fmit_weeks
        fmit_weeks = schedule.get_fmit_weeks()
        assert len(fmit_weeks) == 4  ***REMOVED*** 4 weeks

        ***REMOVED*** Test has_alternating_pattern (4 weeks with 7-day gaps)
        ***REMOVED*** Gaps between weeks: 0 days (consecutive weeks in calendar)
        ***REMOVED*** This won't be alternating since weeks are consecutive
        ***REMOVED*** For alternating, we'd need: week 1, skip week 2, week 3, skip week 4
        assert schedule.has_alternating_pattern() is False


***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Error Scenarios and Edge Cases
***REMOVED*** ============================================================================
...
class TestXLSXImportEdgeCases:
    ...
    def test_missing_file_parameter(
        ...
        ***REMOVED*** Should require file parameter
    ...
    def test_large_schedule_import(
        ...
        - Correct statistics
        """
        ***REMOVED*** Create large schedule: 20 providers, 90 days
        ...
        for prov in providers:
            ...
            for idx, d in enumerate(dates):
                ***REMOVED*** Alternate between clinic and off
                if idx % 2 == 0:
        ...
        ***REMOVED*** 20 providers × 90 days × 2 (AM/PM) = 3600 total slots
    ...
    def test_schedule_with_merged_cells(
        ...
        """
        ***REMOVED*** Create schedule with merged cells
        wb = Workbook()
        ws = wb.active

        ***REMOVED*** Merge some cells in header
        ...
        ***REMOVED*** Should handle merged cells
        importer = ClinicScheduleImporter(db)
        result = importer.import_file(file_bytes=xlsx_bytes)

        ***REMOVED*** May or may not parse correctly depending on implementation
        ***REMOVED*** The test validates the importer doesn't crash
    ...
    def test_unicode_provider_names(
        ...
        providers = [
            ...
            "Dr. Müller",
            "Dr. O'Brien",
            "Dr. 李明",  ***REMOVED*** Chinese characters
        ]
        dates = [date.today()]
        ...
        ***REMOVED*** Verify all providers parsed correctly
        for prov in providers:
    ...
    def test_duplicate_provider_names(
        ...
        """
        ***REMOVED*** Create schedule with duplicate provider
        providers = ["Dr. Smith", "Dr. Jones", "Dr. Smith"]  ***REMOVED*** Duplicate
        dates = [date.today()]
        schedule_data = {
        ...
        assert result.success is True
        ***REMOVED*** Should have 2 unique providers (duplicate merged or overwritten)
        assert len(result.providers) >= 2
