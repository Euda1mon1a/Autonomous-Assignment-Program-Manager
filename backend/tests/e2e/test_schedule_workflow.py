...
***REMOVED*** ============================================================================
***REMOVED*** Fixtures for E2E Tests
***REMOVED*** ============================================================================
...
def academic_year_setup(db: Session) -> dict:
    ...
    """
    ***REMOVED*** Create residents (3 PGY levels)
    residents = []
    for pgy in range(1, 4):
        for i in range(2):  ***REMOVED*** 2 residents per PGY level
            resident = Person(
                id=uuid4(),
    ...
    ***REMOVED*** Create faculty (5 faculty members)
    faculty = []
    for i in range(5):
        fac = Person(
            id=uuid4(),
            ...
            performs_procedures=(i < 2),  ***REMOVED*** First 2 can do procedures
            specialties=["General Medicine", "Primary Care"],
        )
        db.add(fac)
        faculty.append(fac)

    ***REMOVED*** Create rotation templates (outpatient templates for solver)
    templates = []
    ...
    ***REMOVED*** Calculate date range (2 weeks)
...
def schedule_with_absence(db: Session, academic_year_setup: dict) -> dict:
    ...
    ***REMOVED*** Create blocking absence for first week
    absence = Absence(
        id=uuid4(),
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Full Schedule Generation Flow
***REMOVED*** ============================================================================
...
class TestScheduleGenerationE2E:
    ...
    def test_full_schedule_generation_greedy_algorithm(
        ...
        ***REMOVED*** Generate schedule
        result = engine.generate(
            ...
            check_resilience=False,  ***REMOVED*** Skip resilience checks for speed
        )
        ...
        ***REMOVED*** Verify assignments were created
        assert result["total_assigned"] > 0
        assert result["total_blocks"] > 0

        ***REMOVED*** Verify validation was performed
    ...
    def test_schedule_generation_with_multiple_algorithms(
        ...
        for algorithm in algorithms:
            ***REMOVED*** Create fresh engine for each algorithm
            engine = SchedulingEngine(
                db=db,
                start_date=setup["start_date"],
                end_date=setup["end_date"],
            ...
            ***REMOVED*** Verify successful generation
            assert result["status"] in ["success", "partial"]
            assert result["total_assigned"] > 0
        ...
        ***REMOVED*** Compare results - all algorithms should produce assignments
    ...
    def test_schedule_generation_respects_absences(
        ...
        ***REMOVED*** Query assignments for the resident during absence period
        ...
        ***REMOVED*** Should have no assignments during absence (or only absence assignments)
        for assignment in conflicting_assignments:
            ***REMOVED*** If there are assignments, they should be absence-type rotations
    ...
    def test_schedule_generation_with_pgy_level_filter(
        ...
        ***REMOVED*** Generate schedule for PGY-1 residents only
        engine = SchedulingEngine(
            db=db,
            start_date=setup["start_date"],
            end_date=setup["end_date"],
        ...
        result = engine.generate(
            pgy_levels=[1],  ***REMOVED*** Only PGY-1
            algorithm="greedy",
            timeout_seconds=30.0,
            check_resilience=False,
        )
        ...
        ***REMOVED*** Verify only PGY-1 residents were assigned
        from app.models.assignment import Assignment
    ...
    def test_schedule_generation_creates_faculty_supervision(
        ...
        ***REMOVED*** Query faculty assignments
        from app.models.assignment import Assignment
        ...
        ***REMOVED*** Should have faculty assignments for supervision
        assert len(faculty_assignments) > 0, (
            "No faculty supervision assignments created"
        )

        ***REMOVED*** Verify faculty assignments have correct role
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: ACGME Compliance Validation
***REMOVED*** ============================================================================
...
class TestACGMEValidationE2E:
    ...
    def test_acgme_validation_after_generation(
        ...
        ***REMOVED*** Verify validation result structure
        assert "valid" in validation
        assert "total_violations" in validation
        assert "violations" in validation
        assert "coverage_rate" in validation

        ***REMOVED*** Violations should be a list
        assert isinstance(validation["violations"], list)

        ***REMOVED*** Each violation should have required fields
    ...
    def test_acgme_validator_standalone(self, db: Session, academic_year_setup: dict):
        ...
        ***REMOVED*** First generate a schedule
        engine = SchedulingEngine(
            db=db,
            start_date=setup["start_date"],
            end_date=setup["end_date"],
        ...
        ***REMOVED*** Now validate independently
        ...
        ***REMOVED*** Verify validation result
        ...
        ***REMOVED*** Validation results should match generation results
        assert (
    ...
    def test_validation_detects_supervision_violations(
        ...
        ***REMOVED*** Add more residents to create supervision pressure
        for i in range(10):
            resident = Person(
                id=uuid4(),
        ...
        ***REMOVED*** Generate schedule (may have supervision violations)
        engine = SchedulingEngine(
            db=db,
            start_date=setup["start_date"],
            end_date=setup["end_date"],
        ...
        ***REMOVED*** Check validation results
        validation = result["validation"]

        ***REMOVED*** With many more residents, we might have violations
        ***REMOVED*** (This is a stress test - violations are expected)
        if not validation["valid"]:
            ***REMOVED*** If there are violations, they should be properly formatted
            for violation in validation["violations"]:
...
***REMOVED*** ============================================================================
***REMOVED*** E2E Test: Schedule Export to Different Formats
***REMOVED*** ============================================================================
...
class TestScheduleExportE2E:
    ...
    async def test_export_schedule_to_json(
        ...
        ***REMOVED*** Generate schedule first
        engine = SchedulingEngine(
            db=db,
            start_date=setup["start_date"],
            end_date=setup["end_date"],
        ...
        ***REMOVED*** Export to JSON
        ***REMOVED*** Note: JSONExporter expects AsyncSession, so we'll test the format
        ***REMOVED*** by validating the data structure instead
        from app.models.assignment import Assignment
        ...
        ***REMOVED*** Verify we have assignments to export
        assert len(assignments) > 0

        ***REMOVED*** Verify assignments have required fields for export
        for assignment in assignments:
    ...
    async def test_export_schedule_to_csv(self, db: Session, academic_year_setup: dict):
        ...
        ***REMOVED*** Verify assignments exist for export
        from app.models.assignment import Assignment
        ...
        ***REMOVED*** Verify assignment data completeness
        for assignment in assignments:
            db.refresh(assignment.block)
            db.refresh(assignment.person)

            ***REMOVED*** Verify required fields for CSV export
    ...
    def test_export_includes_all_schedule_components(
        ...
        ***REMOVED*** Query assignments with all relationships
        from app.models.assignment import Assignment
        ...
        ***REMOVED*** Verify each assignment has all related data
        for assignment in assignments:
            ***REMOVED*** Block data
...
class TestScheduleErrorHandlingE2E:
    ...
    def test_invalid_date_range_error(self, db: Session):
        ...
        """
        ***REMOVED*** Create engine with invalid date range (end before start)
        start_date = date.today()
        end_date = start_date - timedelta(days=7)

        with pytest.raises(Exception):
            ***REMOVED*** Engine initialization should succeed
            engine = SchedulingEngine(
                db=db,
                start_date=start_date,
                end_date=end_date,
            )
            ***REMOVED*** But generation with invalid dates should fail during validation
            ***REMOVED*** (The actual validation happens in ScheduleRequest schema)
    ...
    def test_no_residents_error_handling(self, db: Session):
        ...
        """
        ***REMOVED*** Create date range
        start_date = date.today()
        end_date = start_date + timedelta(days=13)

        ***REMOVED*** Create engine (no residents in database)
        engine = SchedulingEngine(
            db=db,
            start_date=start_date,
            end_date=end_date,
        ...
        ***REMOVED*** Should fail gracefully
    ...
    def test_no_templates_error_handling(self, db: Session):
        ...
        """
        ***REMOVED*** Create residents but no templates
        resident = Person(
            id=uuid4(),
        ...
        ***REMOVED*** May succeed with 0 assignments or fail
        ***REMOVED*** Either is acceptable when no templates exist
    ...
    def test_invalid_algorithm_fallback(self, db: Session, academic_year_setup: dict):
        ...
        ***REMOVED*** Try to use invalid algorithm
        result = engine.generate(
        ...
        ***REMOVED*** Should fall back to greedy and still succeed
        assert result["status"] in ["success", "partial"]
        ***REMOVED*** Algorithm in run record should be greedy (fallback)
        run = db.query(ScheduleRun).filter(ScheduleRun.id == result["run_id"]).first()
        assert run.algorithm == "greedy"
    ...
    def test_timeout_handling(self, db: Session, academic_year_setup: dict):
        ...
        ***REMOVED*** Use very short timeout (may cause partial results)
        result = engine.generate(
            algorithm="cp_sat",  ***REMOVED*** CP-SAT more likely to timeout
            timeout_seconds=0.1,  ***REMOVED*** Very short timeout
            check_resilience=False,
        )

        ***REMOVED*** Should complete (may be partial due to timeout)
        assert result["status"] in ["success", "partial", "failed"]

        ***REMOVED*** If failed, should have fallback to greedy
        if result["status"] == "partial" or result["status"] == "failed":
            ***REMOVED*** Verify graceful handling - no exceptions raised
            assert "message" in result
