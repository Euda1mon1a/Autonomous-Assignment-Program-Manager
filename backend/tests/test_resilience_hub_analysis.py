...
def mock_assignments():
    ...
    ***REMOVED*** Alice has 10 assignments, Bob has 5, Carol has 3, Dave has 0
    assignments = []
...
class TestFacultyCentrality:
    ...
    def test_composite_score_default_weights(self):
        ...
        ***REMOVED*** Default weights: degree=0.2, betweenness=0.3, eigenvector=0.2, pagerank=0.15, replacement=0.15
        expected = (0.5 * 0.2) + (0.6 * 0.3) + (0.4 * 0.2) + (0.3 * 0.15) + (0.7 * 0.15)
        ***REMOVED*** = 0.1 + 0.18 + 0.08 + 0.045 + 0.105 = 0.51
    ...
    def test_composite_score_custom_weights(self):
        ...
        ***REMOVED*** = 0.15 + 0.18 + 0.04 + 0.03 + 0.14 = 0.54
        assert abs(centrality.composite_score - expected) < 0.001
...
class TestHubAnalyzer:
    ...
    def test_calculate_centrality_basic(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** Should have 4 faculty members
        assert len(results) == 4

        ***REMOVED*** Results should be sorted by composite score (descending)
        for i in range(len(results) - 1):
            assert results[i].composite_score >= results[i + 1].composite_score

        ***REMOVED*** Alice should have the highest score (most assignments, unique service)
        alice_result = next(r for r in results if r.faculty_name == 'Alice')
        assert alice_result.unique_services == 1  ***REMOVED*** service_1 only Alice
        assert alice_result.services_covered == 4  ***REMOVED*** services 1,2,3,4
        assert alice_result.total_assignments == 10

        ***REMOVED*** Carol should have a unique service too
        carol_result = next(r for r in results if r.faculty_name == 'Carol')
        assert carol_result.unique_services == 1  ***REMOVED*** service_6 only Carol
    ...
    def test_calculate_centrality_replacement_difficulty(
        ...
        ***REMOVED*** Alice covers service_1 (1 provider), service_2 (2 providers),
        ***REMOVED*** service_3 (2 providers), service_4 (3 providers)
        ***REMOVED*** Difficulties: 1.0, 0.5, 0.5, 0.33...
        ***REMOVED*** Average: (1.0 + 0.5 + 0.5 + 0.33) / 4 ≈ 0.58
    ...
    def test_identify_hubs_from_cache(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** First calculate centrality
        ...
        ***REMOVED*** Now identify hubs from cache
        hubs = analyzer.identify_hubs()

        ***REMOVED*** Should have at least 2 hubs (Alice and Carol have unique services)
        assert len(hubs) >= 2

        ***REMOVED*** All hubs should have is_hub == True
        for hub in hubs:
    ...
    def test_get_top_hubs(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** Should be sorted by composite score (descending)
        for i in range(len(top_3) - 1):
            assert top_3[i].composite_score >= top_3[i + 1].composite_score


***REMOVED*** ============================================================================
***REMOVED*** TestHubAnalyzerWithNetworkX
***REMOVED*** ============================================================================
...
class TestHubAnalyzerWithNetworkX:
    ...
    def test_calculate_centrality_with_networkx(
        ...
        ***REMOVED*** Results should be sorted by composite score
        for i in range(len(results) - 1):
            assert results[i].composite_score >= results[i + 1].composite_score

        ***REMOVED*** All centrality metrics should be calculated
    ...
    def test_centrality_scores_differ_from_basic(
        ...
        ***REMOVED*** Find Alice in both result sets
        ...
        ***REMOVED*** NetworkX should provide more sophisticated centrality measures
        ***REMOVED*** The scores should differ (at least for eigenvector centrality)
        assert alice_basic.eigenvector_centrality != alice_nx.eigenvector_centrality


***REMOVED*** ============================================================================
***REMOVED*** TestCrossTrainingRecommendations
***REMOVED*** ============================================================================
...
class TestCrossTrainingRecommendations:
    ...
    def test_generate_recommendations_single_provider(self, mock_services):
        ...
        ***REMOVED*** Should have recommendations for single-provider services
        ***REMOVED*** service_1 (Alice only) and service_6 (Carol only)
        ...
        ***REMOVED*** Check that they have the right reason
    ...
    def test_generate_recommendations_dual_provider(self, mock_services):
        ...
        ***REMOVED*** Should have recommendations for dual coverage
        dual_coverage_recs = [
            r for r in recommendations
        ...
        ***REMOVED*** Check they have the right reason
    ...
    def test_generate_recommendations_well_covered(self, mock_services):
        ...
        ***REMOVED*** service_4 has 3 providers (well covered)
        ***REMOVED*** It should not have HIGH or URGENT recommendations
        well_covered_urgent = [
            r for r in recommendations
    ...
    def test_recommendations_sorted_by_priority(self, mock_services):
        ...
        ***REMOVED*** Check that recommendations are sorted by priority
        priority_order = {
...
class TestHubProtection:
    ...
    def test_create_hub_profile(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Calculate centrality first
        analyzer.calculate_centrality(
            faculty=mock_faculty,
            assignments=mock_assignments,
            services=mock_services,
        ...
        ***REMOVED*** Create profile for Alice (has unique service)
        alice_id = UUID('00000000-0000-0000-0000-000000000001')
    ...
    def test_profile_unique_skills(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Alice should have 'Critical Surgery' as unique skill
    ...
    def test_profile_risk_factors(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Alice has 1 unique service, so should have risk factor
    ...
    def test_profile_mitigation_actions(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Should have mitigation actions
    ...
    def test_create_protection_plan(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** Calculate centrality and create profile
        analyzer.calculate_centrality(
            faculty=mock_faculty,
            assignments=mock_assignments,
            services=mock_services,
        ...
        ***REMOVED*** Create protection plan
        today = date.today()
    ...
    def test_create_protection_plan_non_hub_rejected(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** Dave has no assignments or unique services (not a hub)
        ...
        ***REMOVED*** Should return None for non-hub
        assert plan is None
...
***REMOVED*** ============================================================================
***REMOVED*** TestHubDistributionReport
***REMOVED*** ============================================================================
...
class TestHubDistributionReport:
    ...
    def test_get_distribution_report(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        assert report.total_hubs >= 2  ***REMOVED*** At least Alice and Carol
    ...
    def test_report_counts_by_risk_level(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Sum of all risk categories should equal total hubs
        total_by_risk = (
        ...
        ***REMOVED*** Note: total_hubs includes moderate and low risk hubs too
    ...
    def test_report_single_point_of_failure_detection(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Should detect 2 single-provider services
        ...
        ***REMOVED*** Should have at least 2 single points of failure
    ...
    def test_report_recommendations(self, mock_faculty, mock_assignments, mock_services, mock_service_names):
        ...
        ***REMOVED*** Should include cross-training priorities
        assert len(report.cross_training_priorities) > 0

        ***REMOVED*** Recommendations should mention single points of failure
        assert any('single point' in rec.lower() for rec in report.recommendations)


***REMOVED*** ============================================================================
***REMOVED*** TestHubStatus
***REMOVED*** ============================================================================
...
class TestHubStatus:
    ...
    def test_hub_status_structure(self, mock_faculty, mock_assignments, mock_services):
        ...
        ***REMOVED*** Check hubs_by_risk structure
        ...
        ***REMOVED*** All risk counts should be non-negative integers
        for count in status['hubs_by_risk'].values():
            assert isinstance(count, int)
            assert count >= 0

        ***REMOVED*** Last analysis should be a valid ISO timestamp string
        if status['last_analysis']:
            ***REMOVED*** Should be parseable as ISO format
