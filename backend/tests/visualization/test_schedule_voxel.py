...
class TestVoxelColor:
    ...
    def test_to_hex(self):
        ...
        assert color.to_hex() == "***REMOVED***ff0000"

        color = VoxelColor(0.0, 1.0, 0.0)
        assert color.to_hex() == "***REMOVED***00ff00"

        color = VoxelColor(0.0, 0.0, 1.0)
        assert color.to_hex() == "***REMOVED***0000ff"

    def test_from_hex(self):
        """Test creation from hex string."""
        color = VoxelColor.from_hex("***REMOVED***ff0000")
        assert color.r == 1.0
        assert color.g == 0.0
        assert color.b == 0.0
    ...
    def test_from_hex_with_hash(self):
        ...
        color = VoxelColor.from_hex("***REMOVED***3B82F6")
        assert color.to_hex() == "***REMOVED***3b82f6"
...
class TestScheduleVoxelGrid:
    ...
    def test_get_voxels_at_position(self, sample_dimensions):
        ...
        ***REMOVED*** Add two voxels at same x,y but different z
    ...
    def test_detect_conflicts(self, sample_dimensions):
        ...
        ***REMOVED*** Add two voxels at same position (conflict)
    ...
    def test_no_conflicts_different_positions(self, sample_dimensions):
        ...
        grid.add_voxel(ScheduleVoxel(x=2, y=2, z=0))  ***REMOVED*** Different x
    ...
    def test_get_coverage_gaps(self, sample_dimensions):
        ...
        ***REMOVED*** Add only one voxel for person 0 at time 0
        grid.add_voxel(ScheduleVoxel(x=0, y=0, z=0))

        gaps = grid.get_coverage_gaps()
        ***REMOVED*** Should have gaps for all other time slots
        assert len(gaps) > 0
...
class TestScheduleVoxelTransformer:
    ...
    def test_transform_basic(self, sample_data):
        ...
        assert grid.dimensions.x_size == 3  ***REMOVED*** 3 blocks
        assert grid.dimensions.y_size == 3  ***REMOVED*** 3 persons
        assert grid.dimensions.z_size == 2  ***REMOVED*** 2 activity types (clinic, inpatient)
    ...
    def test_transform_person_ordering(self, sample_data):
        ...
        ***REMOVED*** Faculty should be first (y=0)
    ...
    def test_transform_block_ordering(self, sample_data):
        ...
        ***REMOVED*** Should be ordered: 01-15 AM, 01-15 PM, 01-16 AM
...
class TestVoxelGridNumpyConversion:
    ...
    def test_to_numpy_grid(self):
        ...
        ***REMOVED*** Add violation voxel
        ...
        assert numpy_grid[0, 0, 0] == 1  ***REMOVED*** occupied
        assert numpy_grid[1, 1, 0] == 2  ***REMOVED*** conflict
        assert numpy_grid[2, 2, 1] == 3  ***REMOVED*** violation
