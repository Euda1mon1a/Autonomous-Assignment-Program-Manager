...
def test_constraint_pruner():
    ...
    ***REMOVED*** Should have pruned some assignments
    assert result["pruned_count"] > 0
    assert result["total_evaluated"] > len(result["feasible_assignments"])
    assert result["reduction_ratio"] > 0

    ***REMOVED*** Check pruning reasons
...
def test_constraint_pruner_person_type_restriction():
    ...
    ***REMOVED*** Faculty should be pruned from resident-only rotation
...
async def test_solution_cache_different_problems():
    ...
    ***REMOVED*** Different constraints should generate different hashes
    assert hash1 != hash2
...
async def test_solution_cache_partial():
    ...
    ***REMOVED*** Retrieve partial solution
...
async def test_parallel_solver():
    ...
    assert result.objective_value == 100  ***REMOVED*** Best (lowest) should be from solver 0
...
async def test_parallel_solver_timeout():
    ...
    async def slow_solver(problem_data: dict, solver_id: int) -> dict:
        ...
        import asyncio
        await asyncio.sleep(10)  ***REMOVED*** Will timeout
...
async def test_parallel_solver_with_strategies():
    ...
    assert result.success
    assert result.objective_value == 100  ***REMOVED*** Greedy should win
