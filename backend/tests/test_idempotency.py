...
class TestIdempotencyModel:
    ...
    def test_idempotency_status_transitions(self, db: Session):
        ...
        ***REMOVED*** Start as pending
        assert request.is_pending
        assert not request.is_completed

        ***REMOVED*** Mark completed
...
class TestIdempotencyService:
    ...
    def test_compute_body_hash_deterministic(self, db: Session):
        ...
        assert hash1 == hash2
        assert len(hash1) == 64  ***REMOVED*** SHA-256 produces 64 hex chars
    ...
    def test_get_existing_request(self, db: Session):
        ...
        ***REMOVED*** Create a completed request
        request = IdempotencyRequest(
            id=uuid4(),
    ...
    def test_get_existing_request_ignores_expired(self, db: Session):
        ...
        ***REMOVED*** Create an expired request
        expired = IdempotencyRequest(
            id=uuid4(),
            idempotency_key="expired-key",
            body_hash="test-hash",
    ...
    def test_check_key_conflict(self, db: Session):
        ...
        ***REMOVED*** Create a request with specific hash
        request = IdempotencyRequest(
            id=uuid4(),
...
class TestExtractIdempotencyParams:
    ...
    def test_extracts_core_params(self):
        ...
        full_request = {
            ...
            "timeout_seconds": 60.0,  ***REMOVED*** Should not be included
        }
    ...
    def test_handles_missing_optional_params(self):
        ...
        assert params["start_date"] == "2024-01-01"
        assert params["end_date"] == "2024-01-07"
        assert params["algorithm"] == "greedy"  ***REMOVED*** Default
...
class TestIdempotencyEndpoint:
    ...
    def test_generate_without_idempotency_key(
        ...
        ***REMOVED*** No idempotency header in response
    ...
    def test_generate_with_idempotency_key_first_request(
        ...
        ***REMOVED*** Should have created idempotency record
    ...
    def test_generate_with_idempotency_key_replay(
        ...
        ***REMOVED*** Second request with same key and body
        ...
        ***REMOVED*** Should be replayed (same data, replay header)
        assert response2.headers.get("X-Idempotency-Replayed") == "true"
        assert data1["run_id"] == data2["run_id"]  ***REMOVED*** Same run ID
    ...
    def test_generate_with_idempotency_key_conflict(
        ...
        ***REMOVED*** Second request with same key but different body
        payload2 = {
            "start_date": start_date.isoformat(),
            "end_date": (end_date + timedelta(days=1)).isoformat(),  ***REMOVED*** Different end date
            "algorithm": "greedy",
        }
        response2 = client.post(
            "/api/schedule/generate",
...
class TestRowLevelLocking:
    ...
    def test_delete_existing_assignments_with_locking(
        ...
        ***REMOVED*** Second generation (should delete old and create new)
        response2 = client.post("/api/schedule/generate", json=payload)
        assert response2.status_code in [200, 207]

        ***REMOVED*** Should still have similar count (old deleted, new created)
        count2 = db.query(Assignment).join(Block).filter(
            Block.date >= start_date,
            Block.date <= end_date,
        ).count()

        ***REMOVED*** Note: Count might differ slightly due to different random seed,
        ***REMOVED*** but there shouldn't be duplicates
        assert count2 > 0  ***REMOVED*** Should have some assignments
    ...
    def test_concurrent_generation_protection(
        ...
        ***REMOVED*** Simulate an in-progress generation
        ...
        ***REMOVED*** Try to generate for same range
        payload = {
            "start_date": start_date.isoformat(),
            "end_date": end_date.isoformat(),
            "algorithm": "greedy",
...
class TestOptimisticLocking:
    ...
    def test_update_with_stale_version(
        ...
        ***REMOVED*** Should be rejected with 409 Conflict
        assert response.status_code == 409
