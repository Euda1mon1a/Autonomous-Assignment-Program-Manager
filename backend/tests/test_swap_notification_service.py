...
def service_with_faculty(mock_db, sample_faculty, sample_preferences):
    ...
    ***REMOVED*** Mock the query chain for faculty
    faculty_query = Mock()
    faculty_query.filter.return_value.first.return_value = sample_faculty

    ***REMOVED*** Mock the query chain for preferences
    prefs_query = Mock()
    prefs_query.filter.return_value.first.return_value = sample_preferences

    ***REMOVED*** Set up query to return appropriate mock based on model
...
***REMOVED*** ============================================================================
***REMOVED*** Test SwapNotification Dataclass
***REMOVED*** ============================================================================
...
class TestSwapNotification:
    ...
    def test_notification_auto_timestamp(self):
        ...
        ***REMOVED*** Should be very recent (within last second)
...
***REMOVED*** ============================================================================
***REMOVED*** Test SwapNotificationType Enum
***REMOVED*** ============================================================================
...
***REMOVED*** ============================================================================
***REMOVED*** Test notify_swap_request_received
***REMOVED*** ============================================================================
...
class TestNotifySwapRequestReceived:
    ...
    def test_creates_notification_successfully(
        ...
        ***REMOVED*** Verify it was added to pending notifications
    ...
    def test_notification_without_reason(
        ...
        assert notification is not None
        assert "Dr. John Doe" in notification.body
        ***REMOVED*** Body should still be well-formed without reason
    ...
    def test_respects_notification_preferences_disabled(
        ...
        ***REMOVED*** Create preferences with swap requests disabled
        prefs = Mock(spec=FacultyPreference)
        prefs.notify_swap_requests = False

        ***REMOVED*** Set up query mocks
        faculty_query = Mock()
        faculty_query.filter.return_value.first.return_value = sample_faculty
    ...
    def test_faculty_not_found(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** Mock query to return None for faculty
        faculty_query = Mock()
    ...
    def test_no_preferences_defaults_to_notify(
        ...
        ***REMOVED*** Mock query to return None for preferences
        faculty_query = Mock()
        faculty_query.filter.return_value.first.return_value = sample_faculty
    ...
    def test_faculty_without_email(
        ...
        ***REMOVED*** Faculty without email
        ...
        assert notification is not None
        ***REMOVED*** Should generate email from name (note: "Dr. Jane Smith" -> "dr..jane.smith")
        assert "dr..jane.smith@example.com" == notification.recipient_email


***REMOVED*** ============================================================================
***REMOVED*** Test notify_swap_accepted
***REMOVED*** ============================================================================
...
***REMOVED*** ============================================================================
***REMOVED*** Test notify_swap_rejected
***REMOVED*** ============================================================================
...
class TestNotifySwapRejected:
    ...
    def test_creates_notification_without_reason(
        ...
        assert notification is not None
        assert "Dr. Bob Wilson" in notification.body
        ***REMOVED*** Should still have helpful content without reason
...
***REMOVED*** ============================================================================
***REMOVED*** Test notify_swap_executed
***REMOVED*** ============================================================================
...
class TestNotifySwapExecuted:
    ...
    def test_notifies_all_parties(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** Create preferences - all enabled for schedule changes
        prefs = Mock(spec=FacultyPreference)
        prefs.notify_schedule_changes = True

        call_count = [0]  ***REMOVED*** Use list to make it mutable in closure

        def query_side_effect(model):
            if model == Person:
                query = Mock()
                ***REMOVED*** Return faculty in order based on call count
    ...
    def test_respects_individual_preferences(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** First faculty wants notifications
        ...
        ***REMOVED*** Second faculty doesn't want notifications
        ...
        ***REMOVED*** Track calls separately for each model
        person_calls = [0]
        prefs_calls = [0]
        ...
        def query_side_effect(model):
            if model == Person:
                ...
                def filter_func(*args, **kwargs):
                    result = Mock()
                    ***REMOVED*** Return faculty1 for first call, faculty2 for second
                ...
                def filter_func(*args, **kwargs):
                    result = Mock()
                    ***REMOVED*** Return prefs1 for first call, prefs2 for second
        ...
        ***REMOVED*** Only first faculty should receive notification
    ...
    def test_handles_faculty_not_found(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** Only first faculty exists
        faculty1 = Mock(spec=Person)
        faculty1.id = faculty_ids[0]
        ...
        def query_side_effect(model):
            if model == Person:
                ...
                def filter_func(*args, **kwargs):
                    result = Mock()
                    ***REMOVED*** Return faculty1 for first, None for second
        ...
        ***REMOVED*** Only first faculty should get notification
        assert len(notifications) == 1
        assert notifications[0].recipient_id == faculty_ids[0]
...
***REMOVED*** ============================================================================
***REMOVED*** Test notify_swap_rolled_back
***REMOVED*** ============================================================================
...
class TestNotifySwapRolledBack:
    ...
    def test_does_not_check_preferences(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** Even with all preferences disabled, should still notify
        prefs = Mock(spec=FacultyPreference)
        ...
        ***REMOVED*** Should still notify despite preferences
        assert len(notifications) == 1
    ...
    def test_handles_faculty_not_found(self, mock_db, swap_id, sample_week):
        ...
        ***REMOVED*** Only found faculty should be notified
        assert len(notifications) == 1
        assert notifications[0].recipient_id == faculty_ids[0]


***REMOVED*** ============================================================================
***REMOVED*** Test notify_marketplace_match
***REMOVED*** ============================================================================
...
***REMOVED*** ============================================================================
***REMOVED*** Test send_pending_notifications and get_pending_count
***REMOVED*** ============================================================================
...
class TestPendingNotificationManagement:
    ...
    def test_send_pending_notifications(
        ...
        ***REMOVED*** Add some notifications
        service_with_faculty.notify_swap_request_received(
            recipient_faculty_id=faculty_id,
            requester_name="Dr. Test",
            week_offered=sample_week,
        ...
        ***REMOVED*** Verify print was called (placeholder implementation)
    ...
    def test_send_pending_clears_queue(
        ...
        ***REMOVED*** Sending again should return 0
        count = service_with_faculty.send_pending_notifications()
        assert count == 0
...
class TestHelperMethods:
    ...
    def test_build_swap_request_body_without_reason(self, mock_db, sample_week):
        ...
        assert sample_week.isoformat() in body
        assert "portal" in body.lower()
        ***REMOVED*** Should not have "Reason:" when no reason provided
    ...
    def test_send_notification_placeholder(self, mock_print, mock_db):
        ...
        ***REMOVED*** Verify email and subject were printed
...
class TestIntegrationScenarios:
    ...
    def test_complete_swap_workflow(self, mock_db, sample_week, swap_id):
        ...
        ***REMOVED*** Create faculty mocks
        ...
        def query_side_effect(model):
            if model == Person:
                ...
                def filter_func(*args, **kwargs):
                    result = Mock()
                    ***REMOVED*** Alternate between requester and target
        ...
        ***REMOVED*** Send all notifications
        count = service.send_pending_notifications()
        assert count == 4
        assert service.get_pending_count() == 0
    ...
    def test_swap_rejection_workflow(self, mock_db, sample_week, swap_id):
        ...
        ***REMOVED*** Step 2: Request rejected
    ...
    def test_multiple_notifications_same_recipient(
        ...
        ***REMOVED*** Create multiple notifications for the same person
        service_with_faculty.notify_swap_request_received(
            recipient_faculty_id=faculty_id,
