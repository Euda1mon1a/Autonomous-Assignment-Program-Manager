...
class TestFMITContinuityTurfConstraint:
    ...
    def test_constraint_initialization(self, constraint):
        ...
        assert constraint.priority.value == 75  ***REMOVED*** HIGH priority
    ...
    def test_orange_level_warning(self, constraint, base_context):
        ...
        assert result.satisfied is True  ***REMOVED*** Informational, not a failure
    ...
    def test_missing_load_shedding_defaults_to_normal(self, constraint, base_context):
        ...
        ***REMOVED*** Don't set load_shedding_level
        result = constraint.validate([], base_context)
...
class TestFMITStaffingFloorConstraint:
    ...
    def base_context(self, mock_faculty):
        ...
        faculty = mock_faculty(10)  ***REMOVED*** Default to 10 faculty
        context = SchedulingContext(
            residents=[],
            faculty=faculty,
            blocks=[],
    ...
    def test_constraint_initialization(self, constraint):
        ...
        assert constraint.priority.value == 100  ***REMOVED*** CRITICAL priority
    ...
    def test_below_minimum_faculty_blocks_fmit(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** Create context with only 4 faculty
        context = SchedulingContext(
            residents=[],
        ...
        ***REMOVED*** Create FMIT assignment
    ...
    def test_exactly_minimum_faculty_allows_fmit(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** Create context with exactly 5 faculty
        context = SchedulingContext(
            residents=[],
        ...
        ***REMOVED*** Create 1 FMIT assignment (5 * 0.20 = 1 allowed)
        assignment = self._create_assignment(
            context.faculty[0].id,
            context.blocks[0].id,
            mock_fmit_template.id
    ...
    def test_utilization_cap_with_10_faculty(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** Create 2 concurrent FMIT assignments (10 * 0.20 = 2 allowed)
        assignments = [
    ...
    def test_exceeding_utilization_cap_fails(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** Create 3 concurrent FMIT assignments (10 * 0.20 = 2 allowed, so 3 exceeds)
        assignments = [
            self._create_assignment(
    ...
    def test_utilization_cap_edge_case_5_faculty(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** 5 * 0.20 = 1, should allow 1
        assignment = self._create_assignment(
            context.faculty[0].id,
            context.blocks[0].id,
            mock_fmit_template.id
        ...
        ***REMOVED*** Try 2 concurrent (should fail)
        assignments = [
            self._create_assignment(
                context.faculty[i].id,
                context.blocks[0].id,
    ...
    def test_utilization_cap_rounds_up(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** 6 faculty * 0.20 = 1.2 -> should allow max(1, int(1.2)) = 1
        context = SchedulingContext(
            residents=[],
    ...
    def test_multiple_weeks_tracked_separately(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** 2 FMIT in week 1, 2 FMIT in week 2 (both weeks at cap but valid)
        assignments = [
            ***REMOVED*** Week 1
    ...
    def test_non_fmit_assignments_ignored(self, constraint, mock_faculty, mock_fmit_template, mock_non_fmit_template):
        ...
        ***REMOVED*** 2 FMIT (at cap) + 5 clinic (ignored)
        assignments = [
            self._create_assignment(context.faculty[0].id, context.blocks[0].id, mock_fmit_template.id),
            self._create_assignment(context.faculty[1].id, context.blocks[0].id, mock_fmit_template.id),
    ...
    def test_violation_details_comprehensive(self, constraint, mock_faculty, mock_fmit_template):
        ...
        ***REMOVED*** 2 concurrent FMIT with only 5 faculty (exceeds cap)
        assignments = [
...
class TestFMITCrisisIntegration:
    ...
    def test_staffing_floor_triggers_before_turf_needed(self):
        ...
        ***REMOVED*** With 4 faculty, staffing floor blocks FMIT entirely
        ***REMOVED*** This prevents the need for OB turf in the first place
        floor = FMITStaffingFloorConstraint()
        ...
        context.load_shedding_level = 0  ***REMOVED*** NORMAL - would require FM attendance

        ***REMOVED*** No FMIT assignments to validate, but if there were, floor would block them
        result = floor.validate([], context)
        assert result.satisfied is True  ***REMOVED*** No violations because no assignments
    ...
    def test_pcs_scenario_4_faculty(self):
        ...
        ***REMOVED*** Create minimal context
        context = SchedulingContext(
            residents=[],
        ...
        context.load_shedding_level = 3  ***REMOVED*** RED - system stressed

        ***REMOVED*** Attempt FMIT assignment
        assignment = Mock(
            id=uuid4(),
        ...
        ***REMOVED*** Staffing floor should block it
        ...
        ***REMOVED*** Turf should indicate OB is covering
        turf_result = turf.validate([], context)
        assert turf_result.satisfied is True  ***REMOVED*** Informational
        assert len(turf_result.violations) == 1  ***REMOVED*** Info message about OB coverage
