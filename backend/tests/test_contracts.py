...
class TestContractInteraction:
    ...
    def test_create_interaction(self):
        ...
        assert interaction.id is not None  ***REMOVED*** Auto-generated UUID
...
class TestContract:
    ...
    def test_invalid_version_format(self):
        ...
        with pytest.raises(ValueError, match="Invalid version format"):
            Contract(
                consumer="app1",
                provider="app2",
                version="1.0",  ***REMOVED*** Missing patch version
            )
    ...
    def test_contract_hash(self):
        ...
        ***REMOVED*** Same contract should have same hash
        ...
        ***REMOVED*** Different contract should have different hash
...
class TestContractTester:
    ...
    def test_save_and_load_contract(self):
        ...
        ***REMOVED*** Save to temporary file
...
class TestContractVerifier:
    ...
    async def test_verify_simple_contract(self):
        ...
        ***REMOVED*** Create a simple FastAPI app
        app = FastAPI()
    ...
    async def test_verify_contract_status_mismatch(self):
        ...
        @app.get("/api/test")
        def test_endpoint():
            return {"result": "success"}  ***REMOVED*** Returns 200

        contract = Contract(consumer="app", provider="api", version="1.0.0")
        request = ContractRequest(method="GET", path="/api/test")
        response = ContractResponse(
            status=201, body={"result": "success"}  ***REMOVED*** Expects 201
        )
        interaction = ContractInteraction(
            description="Test", request=request, response=response
        )
...
class TestContractPublisher:
    ...
    async def test_publish_contract_failure(self, mock_client_class):
        ...
        ***REMOVED*** Mock HTTP error response
        mock_response = Mock()
    ...
    async def test_fetch_contract_success(self, mock_client_class):
        ...
        ***REMOVED*** Mock Pact format response
        pact_data = {
            "consumer": {"name": "app1"},
            "provider": {"name": "app2"},
...
class TestContractCompatibilityChecker:
    ...
    def test_removed_endpoint_breaking_change(self):
        ...
        ***REMOVED*** No interactions - endpoint removed
    ...
    def test_removed_field_breaking_change(self):
        ...
        new_response = ContractResponse(
            status=200, body={"id": "123", "name": "John"}  ***REMOVED*** email removed
        )
        new_interaction = ContractInteraction(
    ...
    def test_changed_field_type_breaking_change(self):
        ...
        old_response = ContractResponse(status=200, body={"count": 42})  ***REMOVED*** int
        old_interaction = ContractInteraction(
        ...
        new_contract = Contract(consumer="app", provider="api", version="2.0.0")
        new_response = ContractResponse(status=200, body={"count": "42"})  ***REMOVED*** str
        new_interaction = ContractInteraction(
    ...
    def test_added_field_not_breaking(self):
        ...
        new_response = ContractResponse(
            status=200,
            body={"id": "123", "name": "John", "email": "john@example.com"},  ***REMOVED*** Added field
        )
        new_interaction = ContractInteraction(
            description="Get person", request=request, response=new_response
        )
    ...
    def test_nested_object_field_removal(self):
        ...
        new_response = ContractResponse(
            status=200,
            body={"id": "123", "profile": {"name": "John"}},  ***REMOVED*** email removed
        )
        new_interaction = ContractInteraction(
...
class TestContractIntegration:
    ...
    async def test_complete_consumer_provider_workflow(self):
        ...
        ***REMOVED*** Consumer side: Create contract
        tester = ContractTester(
            consumer="frontend-app",
        ...
        ***REMOVED*** Add interaction: Get person
        request = ContractRequest(
            method="GET",
        ...
        ***REMOVED*** Provider side: Create API
        app = FastAPI()
        ...
        ***REMOVED*** Provider side: Verify contract
        verifier = ContractVerifier(app=app)
        result = await verifier.verify_contract(contract)
    ...
    async def test_version_evolution_workflow(self):
        ...
        ***REMOVED*** Version 2.0.0 - Remove field (breaking change)
        v2_contract = Contract(consumer="app", provider="api", version="2.0.0")
        response_v2 = ContractResponse(status=200, body={"id": "123"})  ***REMOVED*** value removed
        ...
        ***REMOVED*** Version 1.1.0 - Add field (non-breaking)
        ...
        assert is_compatible is True  ***REMOVED*** Adding fields is compatible
        assert len(breaking_changes) == 0
