...
async def test_bulk_upsert(db_session):
    ...
    ***REMOVED*** Upsert with updates and new records
    upsert_persons = [
        {
            "id": f"person_{i}",
            "name": f"Updated Person {i}",  ***REMOVED*** Update
            "type": "resident",
            "email": f"person{i}@example.com",
            "created_at": datetime.utcnow(),
            "updated_at": datetime.utcnow(),
        }
        for i in range(150)  ***REMOVED*** Includes 100 updates + 50 new
    ]
...
async def test_batch_size_handling(db_session):
    ...
    ***REMOVED*** Insert 60 records with batch size of 25
    ***REMOVED*** Should create 3 batches: 25, 25, 10
    persons = [
        {
            "id": f"person_{i}",
            "name": f"Person {i}",
...
async def test_empty_batch_operations(db_session):
    ...
    ***REMOVED*** All operations should return 0 for empty data
