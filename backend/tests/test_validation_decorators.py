...
@validate_body({
    ...
    "pgy_level": []  ***REMOVED*** Conditionally required
})
@validate_conditional_field(
    field="pgy_level",
...
class TestCrossFieldValidation:
    ...
    def test_missing_field_for_cross_validation(self):
        ...
        assert response.status_code == 422
        ***REMOVED*** Should fail on required validation, not cross-field
...
class TestValidationRules:
    ...
    def test_all_of_combinator(self):
        ...
        with validation_scope() as ctx:
            assert not rule("field", "ab", ctx)  ***REMOVED*** Too short
            assert ctx.has_errors()

        with validation_scope() as ctx:
            assert not rule("field", "invalid", ctx)  ***REMOVED*** Not in enum
            assert ctx.has_errors()
