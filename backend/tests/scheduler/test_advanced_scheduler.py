...
class TestPriorityTaskQueue:
    ...
    def test_enqueue_and_dequeue(self):
        ...
        ***REMOVED*** Create test tasks with different priorities
        ...
        ***REMOVED*** Enqueue in non-priority order
        queue.enqueue(task_low)
        queue.enqueue(task_high)
        queue.enqueue(task_critical)

        ***REMOVED*** Dequeue should return in priority order
    ...
    def test_peek(self):
        ...
        assert queue.size() == 1  ***REMOVED*** Still in queue
...
class TestTaskDependencyGraph:
    ...
    def test_add_task_with_dependencies(self):
        ...
        ***REMOVED*** Add task1 with no dependencies
        graph.add_task("task1", [])

        ***REMOVED*** Add task2 that depends on task1
    ...
    def test_circular_dependency_detection(self):
        ...
        ***REMOVED*** Try to add task1 depending on task2 (creates cycle)
        with pytest.raises(ValueError, match="circular dependency"):
            graph.add_task(
                "task3",
                [
                    TaskDependency(task_id="task2"),
                    TaskDependency(task_id="task3"),  ***REMOVED*** Self-cycle
                ],
            )
    ...
    def test_topological_sort(self):
        ...
        ***REMOVED*** Create a simple DAG: task1 -> task2 -> task3
        graph.add_task("task1", [])
        graph.add_task("task2", [TaskDependency(task_id="task1")])
        ...
        ***REMOVED*** task1 should come before task2, task2 before task3
...
class TestTaskRetryManager:
    ...
    def test_calculate_delay_fixed(self):
        ...
        delay = manager.calculate_delay(execution, config)
        assert delay == 60  ***REMOVED*** Fixed delay
    ...
    def test_calculate_delay_exponential(self):
        ...
        ***REMOVED*** First retry: 10 * 2^0 = 10
        delay = manager.calculate_delay(execution, config)
        assert delay == 10

        ***REMOVED*** Second retry: 10 * 2^1 = 20
        execution.retry_count = 1
        delay = manager.calculate_delay(execution, config)
        assert delay == 20

        ***REMOVED*** Third retry: 10 * 2^2 = 40
        execution.retry_count = 2
        delay = manager.calculate_delay(execution, config)
        assert delay == 40
    ...
    def test_calculate_delay_linear(self):
        ...
        ***REMOVED*** First retry: 30 * 1 = 30
        delay = manager.calculate_delay(execution, config)
        assert delay == 30

        ***REMOVED*** Second retry: 30 * 2 = 60
        execution.retry_count = 1
        delay = manager.calculate_delay(execution, config)
        assert delay == 60
...
class TestSchedulerHealthMonitor:
    ...
    def test_get_health_status(self):
        ...
        ***REMOVED*** Record some executions
        for i in range(10):
            execution = TaskExecution(
                execution_id=uuid4(),
                task_id=f"task{i}",
...
class TestAdvancedTaskScheduler:
    ...
    async def test_schedule_cron_task(self, scheduler):
        ...
        ***REMOVED*** Schedule to run every hour
...
class TestAdvancedTaskSchedulerAsync:
    ...
    async def test_task_execution_flow(self, scheduler):
        ...
        ***REMOVED*** Define a simple test task
        def test_task():
            return {"result": "success"}

        ***REMOVED*** Mock the get_job_function to return our test task
        with patch(
            ...
            ***REMOVED*** Wait for task to complete
            await asyncio.sleep(2)

            ***REMOVED*** Stop scheduler
            await scheduler.stop()

            ***REMOVED*** Verify execution
            status = scheduler.get_task_status(execution_id)
            assert status is not None
        ...
        with patch(
            ...
            task_def = TaskDefinition(
                ...
                retry_config=RetryConfig(
                    strategy=RetryStrategy.FIXED,
                    max_attempts=3,
                    initial_delay=1,  ***REMOVED*** Short delay for testing
                ),
            )
            ...
            ***REMOVED*** Verify task was retried
            assert call_count >= 2
