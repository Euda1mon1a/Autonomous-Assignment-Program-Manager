...
class TestBruteForceProtection:
    ...
    def test_multiple_failed_login_attempts(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Make multiple failed login attempts
        failed_attempts = 0
        for i in range(10):
            ...
            if response.status_code == status.HTTP_401_UNAUTHORIZED:
                ...
                ***REMOVED*** Rate limiting kicked in
                assert failed_attempts >= 3  ***REMOVED*** Should trigger after a few attempts
                return

        ***REMOVED*** If we get here, rate limiting may not be enforced
    ...
    def test_account_lockout_after_failures(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** This would test account lockout feature
        ***REMOVED*** May not be implemented yet
...
class TestPasswordComplexity:
    ...
    def test_password_minimum_length(self, client: TestClient, db: Session):
        ...
        response = client.post(
            ...
            json={
                ...
                "password": "short",  ***REMOVED*** Too short
                "role": "resident",
            },
        )

        ***REMOVED*** Should fail validation (or endpoint doesn't exist)
    ...
    def test_password_requires_uppercase(self):
        ...
        ***REMOVED*** This would test uppercase requirement
...
class TestSQLInjectionPrevention:
    ...
    def test_login_sql_injection_attempt(self, client: TestClient):
        ...
        ***REMOVED*** Try SQL injection in username
        response = client.post(
            "/api/auth/login/json",
            json={
    ...
    def test_query_parameter_sql_injection(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Try SQL injection in query parameter
        response = client.get(
            "/api/users?search=' OR '1'='1",
            headers=headers,
        )

        ***REMOVED*** Should not expose all users (endpoint may not exist)
        if response.status_code == status.HTTP_404_NOT_FOUND:
            pytest.skip("User search endpoint not available")

        ***REMOVED*** If endpoint exists, should not succeed with injection
    ...
    def test_orm_prevents_raw_sql_injection(self, db: Session):
        ...
        ***REMOVED*** This tests that we use ORM, not raw SQL
        injection_attempt = "test'; DROP TABLE users; --"

        ***REMOVED*** Try to query with injection attempt
        result = db.query(User).filter(User.username == injection_attempt).first()

        ***REMOVED*** Should return None (no user found), not execute DROP TABLE
        assert result is None

        ***REMOVED*** Verify users table still exists
        try:
            db.execute(text("SELECT COUNT(*) FROM users"))
            ***REMOVED*** Table exists - injection prevented
        except Exception:
...
class TestXSSPrevention:
    ...
    def test_script_tags_in_username_escaped(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Try to create user with XSS in name
        response = client.post(
            "/api/persons",
            headers=headers,
            json={
        ...
        ***REMOVED*** Should either reject or escape the script tags
        if response.status_code == status.HTTP_201_CREATED:
            ***REMOVED*** If created, verify script tags are escaped
            data = response.json()
            assert "<script>" not in data.get("name", "")

    def test_html_in_json_response_escaped(self):
        """HTML in JSON responses is properly escaped."""
        ***REMOVED*** This would test response serialization
...
class TestCSRFProtection:
    ...
    def test_csrf_token_required_for_state_change(self):
        ...
        ***REMOVED*** This would test CSRF middleware
        ***REMOVED*** May use httpOnly cookies + CSRF tokens
...
class TestSessionFixation:
    ...
    def test_session_regenerated_on_login(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Tokens should be different (new session each time)
...
class TestSecureHeaders:
    ...
    def test_strict_transport_security_header(self, client: TestClient):
        ...
        ***REMOVED*** Check for HSTS header (may not be set in test environment)
        ***REMOVED*** HSTS is typically set at reverse proxy level (nginx)
        ***REMOVED*** headers = response.headers
        ***REMOVED*** This test is informational
    ...
    def test_x_frame_options_header(self, client: TestClient):
        ...
        response = client.get("/api/health")
        ***REMOVED*** May not be set in API responses (more relevant for HTML)
...
class TestInputValidation:
    ...
    def test_email_format_validation(self, client: TestClient):
        ...
        response = client.post(
            ...
            json={
                "username": "emailtest",
                "email": "not-an-email",  ***REMOVED*** Invalid email
        ...
        ***REMOVED*** Should fail validation
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    ...
    def test_uuid_format_validation(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Try to access resource with invalid UUID
...
class TestAuditLogging:
    ...
    def test_login_success_logged(self, client: TestClient, db: Session):
        ...
        ***REMOVED*** Check audit log (implementation specific)
        ***REMOVED*** This would query an audit_log table if it exists
...
class TestDataLeakagePrevention:
    ...
    def test_error_messages_dont_leak_info(self, client: TestClient):
        ...
        ***REMOVED*** Try to login with non-existent user
        response = client.post(
            "/api/auth/login/json",
            json={"username": "nonexistent", "password": "password"},
        )

        ***REMOVED*** Error should not reveal whether user exists
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        detail = response.json().get("detail", "")
        ***REMOVED*** Should be generic message, not "user not found"
    ...
    def test_stack_traces_not_exposed(self):
        ...
        ***REMOVED*** This would test global exception handler
...
class TestRateLimitingByRole:
    ...
    def test_rate_limit_per_ip(self, client: TestClient):
        ...
        ***REMOVED*** Make many requests quickly
        responses = []
        for i in range(100):
            response = client.get("/api/health")
            ...
            if response.status_code == status.HTTP_429_TOO_MANY_REQUESTS:
                ***REMOVED*** Rate limit hit
                return

        ***REMOVED*** If we get here, rate limiting may not be strict
...
class TestCacheInvalidation:
    ...
    def test_cache_cleared_on_role_change(self, db: Session):
        ...
        ***REMOVED*** (Implementation specific - may use Redis cache)
...
class TestSecretManagement:
    ...
    def test_secret_key_not_default(self):
        ...
        ***REMOVED*** Application should refuse to start with default key
        ***REMOVED*** This is enforced in config validation
    ...
    def test_webhook_secret_configured(self):
        ...
        settings = get_settings()
        ***REMOVED*** May not be required in all deployments
        ***REMOVED*** This is validated at config level
...
class TestSessionExpiration:
    ...
    def test_access_token_expires(self):
        ...
        ***REMOVED*** Create token with 1 second expiration
        token, _, _ = create_access_token(data, expires_delta=timedelta(seconds=1))

        ***REMOVED*** Immediate decode should work
        payload = jwt.decode(token, settings.SECRET_KEY, algorithms=["HS256"])
        assert payload is not None

        ***REMOVED*** After expiration, should fail
        ***REMOVED*** (Would need to wait 1 second or mock time)
    ...
    def test_refresh_token_longer_lifetime(self):
        ...
        ***REMOVED*** Already tested in token creation tests
        pass
...
class TestCORSConfiguration:
    ...
    def test_cors_headers_present(self, client: TestClient):
        ...
        ***REMOVED*** Check for CORS headers
        ***REMOVED*** May not be set in test client
