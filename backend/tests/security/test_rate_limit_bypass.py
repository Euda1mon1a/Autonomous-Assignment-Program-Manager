...
def redis_client():
    ...
    try:
        client = redis.Redis(
            host="localhost",
            port=6379,
            db=15,  ***REMOVED*** Use separate DB for testing
            decode_responses=True
        )
        client.ping()
        yield client
        ***REMOVED*** Cleanup after tests
        client.flushdb()
...
class TestRateLimitBypassDetector:
    ...
    def test_fingerprint_generation(self, detector, mock_request):
        ...
        assert fingerprint1
        assert len(fingerprint1) == 32  ***REMOVED*** SHA256 hash truncated to 32 chars

        ***REMOVED*** Same request should generate same fingerprint
        fingerprint2 = detector._generate_fingerprint(mock_request)
        assert fingerprint1 == fingerprint2

        ***REMOVED*** Different UA should generate different fingerprint
    ...
    def test_client_ip_extraction(self, detector, mock_request):
        ...
        ***REMOVED*** Test direct IP
        ip = detector._get_client_ip(mock_request)
        assert ip == "192.168.1.100"

        ***REMOVED*** Test X-Forwarded-For
    ...
    def test_ip_rotation_with_session(self, detector):
        ...
        ***REMOVED*** Multiple IPs for same session
        for i in range(1, 5):
        ...
        ***REMOVED*** Should detect on 4th IP
        assert detection is not None
        assert detection.technique == BypassTechnique.IP_ROTATION
    ...
    def test_ip_rotation_time_window(self, detector, redis_client):
        ...
        ***REMOVED*** Add old entries manually (simulate expired window)
        ...
        ***REMOVED*** New IP should not trigger (old ones expired)
        detection = detector._detect_ip_rotation("192.168.1.4", user_id, None)
        assert detection is None
    ...
    def test_user_agent_spoofing_detection(self, detector):
        ...
        ***REMOVED*** Second UA - no detection
        detection = detector._detect_user_agent_spoofing(
            ip_address,
            user_id,
        ...
        ***REMOVED*** Third UA - should trigger detection
        detection = detector._detect_user_agent_spoofing(
            ip_address,
            user_id,
    ...
    def test_distributed_attack_detection(self, detector):
        ...
        ***REMOVED*** Add requests from multiple IPs
        for i in range(1, detector.DISTRIBUTED_ATTACK_THRESHOLD):
            detection = detector._detect_distributed_attack(
                f"192.168.1.{i}",
                endpoint
            )
            assert detection is None  ***REMOVED*** Not enough IPs yet

        ***REMOVED*** Add one more IP to trigger detection
        detection = detector._detect_distributed_attack(
    ...
    def test_behavioral_anomaly_missing_headers(self, detector, mock_request):
        ...
        ***REMOVED*** Remove common headers
        mock_request.headers = {
            "User-Agent": "Mozilla/5.0"
            ***REMOVED*** Missing Accept and Accept-Language
        }
        ...
        ***REMOVED*** First request may not trigger (need to accumulate)
    ...
    def test_behavioral_anomaly_suspicious_proxy(self, detector, mock_request):
        ...
        ***REMOVED*** May need multiple requests to accumulate
        for _ in range(3):
            detection = detector._detect_behavioral_anomaly(
                mock_request,
                "192.168.1.100",
    ...
    def test_fingerprint_mismatch_detection(self, detector):
        ...
        ***REMOVED*** First request - establish fingerprint
        ...
        ***REMOVED*** Same fingerprint - no detection
        detection = detector._detect_fingerprint_mismatch(
            ip_address,
            user_id,
            "fingerprint1"
        ...
        ***REMOVED*** Different fingerprint - should detect
        detection = detector._detect_fingerprint_mismatch(
            ip_address,
            user_id,
            "fingerprint2"
    ...
    def test_blocked_request_returns_detection(self, detector, mock_request):
        ...
        ***REMOVED*** Block the IP
        detector.block_ip(ip_address)

        ***REMOVED*** Request should be immediately detected as blocked
    ...
    async def test_log_and_alert(self, detector, redis_client):
        ...
        ***REMOVED*** Log and alert (without DB for now)
        await detector.log_and_alert(detection, db=None)

        ***REMOVED*** Verify detection was stored in Redis
    ...
    async def test_check_for_bypass_auto_blocks(self, detector, mock_request, redis_client):
        ...
        ***REMOVED*** Trigger IP rotation to cause detection
        for i in range(1, 5):
            detector._detect_ip_rotation(f"192.168.1.{i}", user_id, None)

        mock_request.client.host = "192.168.1.4"

        ***REMOVED*** Check for bypass with auto-block enabled
    ...
    async def test_check_for_bypass_no_auto_block(self, detector, mock_request, redis_client):
        ...
        ***REMOVED*** Check for bypass with auto-block disabled
        with patch("app.security.rate_limit_bypass.get_bypass_detector", return_value=detector):
            detection = await check_for_bypass(
                mock_request,
                user_id=user_id,
        ...
        ***REMOVED*** Even with detection, should not block if auto_block=False
        if detection:
            ***REMOVED*** Cannot assert not blocked because detect_bypass_attempt might block
            ***REMOVED*** Just verify detection was returned
            assert detection is not None
...
class TestBypassDetectorIntegration:
    ...
    def test_full_detection_workflow(self, detector, mock_request):
        ...
        ***REMOVED*** Simulate multiple requests with suspicious patterns
        for i in range(5):
            ...
            ***REMOVED*** Should eventually detect bypass
            if detection:
                assert detection.should_block is True
                break
    ...
    def test_global_detector_instance(self):
        ...
        ***REMOVED*** Should return same instance
...
class TestBypassDetectionEdgeCases:
    ...
    def test_missing_request_headers(self, detector):
        ...
        request.headers = {}  ***REMOVED*** No headers
        request.url = Mock()
        request.url.path = "/api/test"

        ***REMOVED*** Should not crash
    ...
    def test_unknown_client_ip(self, detector):
        ...
        request = Mock(spec=Request)
        request.client = None  ***REMOVED*** No client
        request.headers = {}
        request.url = Mock()
        request.url.path = "/api/test"
