...
class AssignmentFactory:
    ...
    def create_supervised_session(
        ...
        ***REMOVED*** Assign residents as primary
        ...
        ***REMOVED*** Assign faculty as supervising
    ...
    def create_conflicting_assignments(
        ...
        for i, block in enumerate(blocks):
            ***REMOVED*** Assign to different rotations on same blocks
    ...
    def create_double_booked_assignment(
        ...
        for template in rotation_templates:
            ***REMOVED*** This will violate unique_person_per_block constraint
            assignment = Assignment(
                id=uuid4(),
                person_id=person.id,
                block_id=block.id,
        ...
        ***REMOVED*** Don't commit - let test handle the constraint violation
        return assignments
    ...
    def create_acgme_compliant_week(
        ...
        ***REMOVED*** Assumes blocks are sorted by date and time
        ***REMOVED*** Mon-Thu AM: Clinic
        for i in [0, 2, 4, 6]:  ***REMOVED*** AM blocks for Mon-Thu
            assignment = AssignmentFactory.create_assignment(
                db, person=resident, block=blocks[i], rotation_template=clinic_template
            )
            assignments.append(assignment)

        ***REMOVED*** Fri AM: Conference
        assignment = AssignmentFactory.create_assignment(
            db, person=resident, block=blocks[8], rotation_template=conference_template
        )
        assignments.append(assignment)

        ***REMOVED*** PM and weekend blocks are unassigned (time off)
