...
class PersonFactory:
    ...
    def create_batch_residents(
        ...
        if pgy_distribution:
            ...
            ***REMOVED*** Even distribution across PGY levels
            for i in range(count):
                pgy_level = (i % 3) + 1  ***REMOVED*** Cycles 1, 2, 3, 1, 2, 3...
                resident = PersonFactory.create_resident(db, pgy_level=pgy_level)
                residents.append(resident)
    ...
    def create_batch_faculty(
        ...
        if include_leadership and count >= 3:
            ***REMOVED*** Create leadership first
        ...
        ***REMOVED*** Create remaining faculty (mix of core and sports med)
        for i in range(remaining):
            if i % 4 == 0:
                ***REMOVED*** Every 4th is sports medicine
    ...
    def create_chief_resident(
        ...
        chief = Person(
            ...
            + "@chief.mil",
            pgy_level=pgy_level,
            target_clinical_blocks=24,  ***REMOVED*** 6 clinical + 6 admin weeks
        )
        db.add(chief)
        db.commit()
        db.refresh(chief)
    ...
    def create_research_resident(
        ...
        research = Person(
            ...
            + "@research.mil",
            pgy_level=pgy_level,
            target_clinical_blocks=8,  ***REMOVED*** 2 clinical weeks only
        )
        db.add(research)
        db.commit()
        db.refresh(research)
