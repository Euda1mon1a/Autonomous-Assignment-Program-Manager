...
class ComplianceFactory:
    ...
    def create_80_hour_violation(
        ...
        ***REMOVED*** Create one week of blocks (14 blocks = 7 days × AM/PM)
        blocks = BlockFactory.create_week_blocks(db, start_date=start_date)

        ***REMOVED*** Create clinic template (2 hours per half-day = 4 hours per day)
        clinic_template = RotationFactory.create_clinic_template(db)

        ***REMOVED*** Assign to ALL 14 blocks = 14 half-days = 7 full days × 12 hours/day = 84 hours
        ***REMOVED*** This violates the 80-hour rule
        assignments = []
        for block in blocks:
        ...
        return {
            ...
            "estimated_hours": 84,  ***REMOVED*** 14 half-days × 6 hours avg
        }
    ...
    def create_supervision_ratio_violation(
        ...
        ***REMOVED*** Assign minimal faculty (violates ratio)
        for fac in faculty:
    ...
    def create_double_booked_violation(
        ...
        ***REMOVED*** Create single block
        block = BlockFactory.create_block(db)

        ***REMOVED*** Create two different rotation templates
        clinic_template = RotationFactory.create_clinic_template(db)
        procedure_template = RotationFactory.create_procedure_template(db)

        ***REMOVED*** Try to create double-booked assignments
        ***REMOVED*** NOTE: This will violate unique_person_per_block constraint
    ...
    def create_unsupervised_pgy1_violation(
        ...
        Returns:
            dict: Violation scenario data
        """
        ***REMOVED*** Create PGY-1 residents
        ...
        ***REMOVED*** Create clinic template requiring supervision
        clinic_template = RotationFactory.create_clinic_template(db, pgy_level=1)

        ***REMOVED*** Assign residents WITHOUT faculty supervisor
        assignments = []
        for resident in residents:
            assignment = AssignmentFactory.create_primary_assignment(
                db, person=resident, block=block, rotation_template=clinic_template
    ...
    def create_capacity_violation(
        ...
        ***REMOVED*** Create clinic template with capacity limit
        clinic_template = RotationFactory.create_clinic_template(
            db, max_residents=max_residents
        )

        ***REMOVED*** Assign all residents (exceeds capacity)
        assignments = []
        for resident in residents:
            assignment = AssignmentFactory.create_primary_assignment(
                db, person=resident, block=block, rotation_template=clinic_template
    ...
    def create_missing_specialty_violation(
        ...
        ***REMOVED*** Create faculty WITHOUT Sports Medicine specialty
        ...
        ***REMOVED*** Create Sports Medicine template (requires specialty)
        sm_template = RotationFactory.create_sports_medicine_template(db)

        ***REMOVED*** Assign resident and non-SM faculty
    ...
    def create_consecutive_weeks_violation(
        ...
        ***REMOVED*** Create blocks for consecutive weeks
        blocks = BlockFactory.create_block_period(db, days=num_weeks * 7)

        ***REMOVED*** Create clinic template
        clinic_template = RotationFactory.create_clinic_template(db)

        ***REMOVED*** Assign to every weekday (Mon-Fri, skip weekends)
        assignments = []
        for block in blocks:
