...
class TestPreSolverValidator:
    ...
    def basic_context(self):
        ...
        ***REMOVED*** Create 3 residents
        residents = [MockPerson(f"Resident {i}", "resident") for i in range(1, 4)]

        ***REMOVED*** Create 1 week of blocks (Mon-Fri, AM/PM = 10 blocks)
    ...
    def test_validate_insufficient_coverage(self, validator):
        ...
        ***REMOVED*** 1 resident, 10 blocks = can only cover 10 slots, but need 10
        ***REMOVED*** This is exactly at capacity (ratio = 1.0), which is below MIN_PERSONNEL_RATIO
        resident = MockPerson()
        ...
        ***REMOVED*** Should warn about tight coverage (ratio = 1.0 < 1.2)
    ...
    def test_validate_resident_with_no_availability(self, validator):
        ...
        ***REMOVED*** Mark second resident as completely unavailable
        availability = {
            residents[1].id: {
    ...
    def test_validate_block_with_no_available_residents(self, validator):
        ...
        ***REMOVED*** Mark all residents unavailable for the first block
        availability = {
    ...
    def test_complexity_estimation_low(self, validator):
        ...
        ***REMOVED*** 2 residents × 2 blocks × 1 template = 4 variables
        residents = [MockPerson(f"R{i}") for i in range(2)]
    ...
    def test_complexity_estimation_high(self, validator):
        ...
        ***REMOVED*** 20 residents × 100 blocks × 5 templates = 10,000 variables
        ...
        ***REMOVED*** Should be at least MEDIUM complexity
        assert result.complexity_estimate >= validator.COMPLEXITY_MEDIUM
    ...
    def test_existing_assignments_over_constrained(self, validator):
        ...
        ***REMOVED*** Pre-assign 4 out of 5 blocks (80%)
        ...
        ***REMOVED*** Should warn about limited optimization room
    ...
    def test_detect_conflicts_convenience_method(self, validator):
        ...
        ***REMOVED*** Create infeasible context
        context = SchedulingContext(
            residents=[],
            faculty=[],
            blocks=[MockBlock(date(2025, 1, 1))],
    ...
    def test_over_assignment_detection(self, validator):
        ...
        ***REMOVED*** 1 resident needs to cover 20 blocks, but max is 90% = 18 blocks
        resident = MockPerson()
        base_date = date(2025, 1, 1)
        blocks = [
            MockBlock(base_date + timedelta(days=i // 2), "AM" if i % 2 == 0 else "PM")
        ...
        ***REMOVED*** Should detect over-assignment (1 resident × 20 blocks × 0.9 = 18, but need 20)
    ...
    def test_recommendations_provided(self, validator):
        ...
        ***REMOVED*** Create infeasible scenario
        context = SchedulingContext(
            residents=[MockPerson()],
            faculty=[],
        ...
        ***REMOVED*** Should have issues and recommendations
        if result.issues:
            assert len(result.recommendations) > 0
    ...
    def test_availability_with_partial_absences(self, validator):
        ...
        ***REMOVED*** One resident available for only 20% of blocks
        availability = {
            residents[0].id: {
                blocks[i].id: {"available": False}
                for i in range(8)  ***REMOVED*** Unavailable for 8 out of 10
            }
        }
        ...
        ***REMOVED*** Should warn about low availability
    ...
    def test_weekend_blocks_excluded(self, validator):
        ...
        blocks = [
            ...
            MockBlock(base_date + timedelta(days=5), "AM", is_weekend=True),  ***REMOVED*** Saturday
            MockBlock(base_date + timedelta(days=6), "AM", is_weekend=True),  ***REMOVED*** Sunday
        ]
        ...
        ***REMOVED*** Should only count 2 workday blocks
    ...
    def test_hour_balance_warning(self, validator):
        ...
        ***REMOVED*** Resident only available for 30% of blocks (below 50% threshold)
        availability = {
            residents[0].id: {
                blocks[i].id: {"available": False}
                for i in range(7)  ***REMOVED*** Unavailable for 7 out of 10
            }
        }
