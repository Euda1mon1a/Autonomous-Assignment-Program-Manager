...
***REMOVED*** Simple mock objects for testing
class MockPerson:
...
***REMOVED*** Named tuple for mock assignments
...
class TestPrimaryDutyConfig:
    ...
    def test_from_airtable_record_basic(self):
        ...
        assert config.available_days == {0, 1, 3, 4}  ***REMOVED*** Mon, Tue, Thu, Fri
        assert 2 not in config.available_days  ***REMOVED*** Wed not available
    ...
    def test_from_airtable_record_defaults(self):
        ...
        record = {
            ...
            "fields": {
                ...
                ***REMOVED*** No clinic fields, no availability
            },
        }
        ...
        assert config.clinic_min_per_week == 0  ***REMOVED*** Default
        assert config.clinic_max_per_week == 10  ***REMOVED*** Default
        assert config.available_days == {0, 1, 2, 3, 4}  ***REMOVED*** All weekdays
...
class TestLoadPrimaryDutiesConfig:
    ...
    def test_load_from_default_path(self):
        ...
        ***REMOVED*** Should have loaded some records (if file exists)
        if configs:
            assert len(configs) > 0
            ***REMOVED*** Check that we can access by name
...
class TestFacultyPrimaryDutyClinicConstraint:
    ...
    def context(self):
        ...
        ***REMOVED*** Create a week of blocks (Mon-Fri, AM/PM)
        base_date = date(2025, 1, 6)  ***REMOVED*** A Monday
        blocks = []
    ...
    def test_validate_meets_minimum(self, constraint, context):
        ...
        faculty = context.faculty[0]  ***REMOVED*** Faculty Alpha (min 2)
        clinic_template = context.templates[0]

        ***REMOVED*** Assign 3 clinic sessions (meets min of 2)
        assignments = [
    ...
    def test_validate_below_minimum(self, constraint, context):
        ...
        ***REMOVED*** Assign only 1 clinic session (below min of 2)
        assignments = [
            MockAssignment(faculty.id, context.blocks[0].id, clinic_template.id),
        ]
    ...
    def test_validate_exceeds_maximum(self, constraint, context):
        ...
        faculty = context.faculty[0]  ***REMOVED*** Faculty Alpha (max 4)
        clinic_template = context.templates[0]

        ***REMOVED*** Assign 5 clinic sessions (exceeds max of 4)
        assignments = [
    ...
    def test_validate_pd_no_clinic(self, constraint, context):
        ...
        pd_faculty = context.faculty[1]  ***REMOVED*** Program Director (max 0)
        clinic_template = context.templates[0]

        ***REMOVED*** Assign 1 clinic session (exceeds max of 0)
        assignments = [
    ...
    def test_validate_ignores_inpatient(self, constraint, context):
        ...
        ***REMOVED*** Assign only inpatient (should not satisfy clinic minimum)
        assignments = [
        ...
        ***REMOVED*** Should fail because clinic minimum not met
        assert not result.satisfied
...
class TestFacultyDayAvailabilityConstraint:
    ...
    def duty_configs(self):
        ...
        return {
            "Faculty Alpha": PrimaryDutyConfig(
                duty_id="rec1",
                duty_name="Faculty Alpha",
                available_days={0, 1, 3, 4},  ***REMOVED*** Mon, Tue, Thu, Fri (no Wed)
            ),
        }
    ...
    def context(self):
        ...
        ***REMOVED*** Create blocks for Mon-Fri
        base_date = date(2025, 1, 6)  ***REMOVED*** Monday
        blocks = []
    ...
    def test_validate_available_day(self, constraint, context):
        ...
        faculty = context.faculty[0]
        clinic_template = context.templates[0]
        monday_block = context.blocks[0]  ***REMOVED*** Monday is available
    ...
    def test_validate_unavailable_day(self, constraint, context):
        ...
        faculty = context.faculty[0]
        clinic_template = context.templates[0]
        wednesday_block = context.blocks[2]  ***REMOVED*** Wednesday is NOT available
...
class TestFacultyClinicEquitySoftConstraint:
    ...
    def test_soft_constraint_always_satisfied(self, constraint, context):
        ...
        ***REMOVED*** Below target (target is 3)
        assignments = [
            MockAssignment(faculty.id, context.blocks[0].id, clinic_template.id),
        ]
        ...
        ***REMOVED*** Soft constraints return satisfied=True
        assert result.satisfied
        ***REMOVED*** But there should be a penalty
    ...
    def test_at_target_no_penalty(self, constraint, context):
        ...
        ***REMOVED*** At target (3 assignments)
        assignments = [
            MockAssignment(faculty.id, context.blocks[0].id, clinic_template.id),
            MockAssignment(faculty.id, context.blocks[1].id, clinic_template.id),
            MockAssignment(faculty.id, context.blocks[2].id, clinic_template.id),
        ...
        assert result.satisfied
        assert result.penalty == 0  ***REMOVED*** At target = no penalty
