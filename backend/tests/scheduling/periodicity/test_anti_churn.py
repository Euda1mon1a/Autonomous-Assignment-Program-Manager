...
def minimal_churn_schedules(person_ids, block_ids, template_ids):
    ...
    modified = [
        (person_ids[0], block_ids[0], template_ids["clinic"]),
        (person_ids[0], block_ids[1], template_ids["elective"]),  ***REMOVED*** Changed template
        (person_ids[1], block_ids[2], template_ids["clinic"]),
    ]
...
def high_churn_schedules(person_ids, block_ids, template_ids):
    ...
    modified = [
        (person_ids[0], block_ids[4], template_ids["elective"]),  ***REMOVED*** Completely different
        (person_ids[1], block_ids[5], template_ids["procedures"]),
        (person_ids[2], block_ids[6], template_ids["clinic"]),  ***REMOVED*** New person
    ]
...
class TestScheduleSnapshot:
    ...
    def test_immutability(self, person_ids, block_ids, template_ids):
        ...
        ***REMOVED*** frozenset should prevent modification
...
class TestHammingDistance:
    ...
    def test_minimal_change(self, minimal_churn_schedules):
        ...
        ***REMOVED*** One assignment removed from A, one different added to B = 2 differences
        assert distance == 2
    ...
    def test_high_churn(self, high_churn_schedules):
        ...
        ***REMOVED*** Original: 4 assignments, Modified: 3 assignments, 0 overlap = 7 differences
        assert distance == 7
...
class TestHammingDistanceByPerson:
    ...
    def test_minimal_churn_by_person(self, minimal_churn_schedules, person_ids):
        ...
        ***REMOVED*** person_ids[0] had one template change (2 differences: old + new)
        assert churn_by_person[person_ids[0]] == 2

        ***REMOVED*** person_ids[1] had no changes
    ...
    def test_identify_high_churn_person(self, high_churn_schedules, person_ids):
        ...
        ***REMOVED*** person_ids[0] had all assignments change
        assert churn_by_person[person_ids[0]] > 0

        ***REMOVED*** person_ids[2] is new (only in schedule_b)
...
class TestScheduleRigidity:
    ...
    def test_high_rigidity(self, minimal_churn_schedules):
        ...
        ***REMOVED*** 2 differences out of 6 total assignments (3 original + 3 new)
        ***REMOVED*** rigidity = 1 - (2/6) = 0.667
        assert rigidity > 0.6
        assert rigidity < 0.8
    ...
    def test_low_rigidity(self, high_churn_schedules):
        ...
        ***REMOVED*** 7 differences out of 7 total (4 original + 3 new)
        ***REMOVED*** rigidity = 1 - (7/7) = 0.0
        assert rigidity == 0.0
...
class TestTimeCrystalObjective:
    ...
    def test_perfect_schedule(self, identical_schedules):
        ...
        ***REMOVED*** All constraints satisfied
        constraint_results = [
        ...
        ***REMOVED*** Should be close to 1.0 (perfect score)
        assert score >= 0.95
        assert score <= 1.0
    ...
    def test_constraint_violation_penalty(self, identical_schedules):
        ...
        ***REMOVED*** One hard violation
        constraint_results = [
            ConstraintResult(satisfied=False, penalty=5.0),  ***REMOVED*** Hard violation
            ConstraintResult(satisfied=True, penalty=0.0),
        ]
        ...
        ***REMOVED*** Hard violation should reduce constraint score significantly
        ***REMOVED*** Even with perfect rigidity, overall score should be lower
        assert score < 0.8
    ...
    def test_rigidity_penalty(self, high_churn_schedules):
        ...
        ***REMOVED*** Low rigidity (0.0) should reduce score
        ***REMOVED*** score = 0.6*1.0 + 0.3*0.0 + 0.1*? = 0.6 + small_fairness_term
        assert score < 0.8
    ...
    def test_alpha_weight_effect(self, minimal_churn_schedules):
        ...
        ***REMOVED*** Low alpha - prioritize constraints
        score_low_alpha = time_crystal_objective(
            schedule_b, schedule_a, constraint_results, alpha=0.1, beta=0.0
        )

        ***REMOVED*** High alpha - prioritize rigidity
        score_high_alpha = time_crystal_objective(
            schedule_b, schedule_a, constraint_results, alpha=0.5, beta=0.0
        )

        ***REMOVED*** With high churn, higher alpha should give lower score
        ***REMOVED*** (because rigidity is weighted more, and rigidity is low)
