...
class TestQUBOFormulation:
    ...
    def test_variable_indexing(self):
        ...
        ***REMOVED*** Should have 2 residents * 5 blocks * 2 templates = 20 variables
        assert formulation.num_variables == 20

        ***REMOVED*** Check index mapping is bijective
    ...
    def test_coverage_objective(self):
        ...
        ***REMOVED*** Diagonal terms should be negative (encourage x[i] = 1)
        diagonal_sum = sum(coef for (i, j), coef in Q.items() if i == j)
        ***REMOVED*** Some diagonal terms are negative (coverage) but constraint penalties positive
        ***REMOVED*** Just verify Q is not empty
        assert len(Q) > 0
    ...
    def test_solution_decoding(self):
        ...
        ***REMOVED*** Should have one assignment
        assert len(assignments) == 1
        assert len(assignments[0]) == 3  ***REMOVED*** (person_id, block_id, template_id)
...
class TestSimulatedQuantumAnnealingSolver:
    ...
    def test_deterministic_with_seed(self):
        ...
        ***REMOVED*** (exact equality may vary due to implementation details)
    ...
    def test_pure_python_fallback(self):
        ...
        ***REMOVED*** Build QUBO and solve with pure Python
        formulation = QUBOFormulation(context)
        Q = formulation.build()
...
class TestQuantumInspiredSolver:
    ...
    def test_without_quantum_hardware(self):
        ...
        solver = QuantumInspiredSolver(
            use_quantum_hardware=False,  ***REMOVED*** Explicitly disable
            timeout_seconds=10.0,
        )
...
class TestQUBOConstraints:
    ...
    def test_one_per_block_constraint(self):
        ...
        ***REMOVED*** Should have quadratic penalty terms between variables for same block
        ***REMOVED*** These are off-diagonal terms with positive coefficients
    ...
    def test_availability_constraint(self):
        ...
        ***REMOVED*** The unavailable slot should have high penalty
        assert len(Q) > 0  ***REMOVED*** QUBO was built
...
class TestPyQUBOIntegration:
    ...
    def test_pyqubo_solver(self):
        ...
        ***REMOVED*** Should succeed (may use fallback if dwave-samplers not available)
