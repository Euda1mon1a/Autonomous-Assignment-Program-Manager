...
class TestParetoFront:
    ...
    def test_pareto_front_add_non_dominated(self):
        ...
        ***REMOVED*** Add first solution
        ind1 = self._create_individual(0.8, 0.6, 1)
        pf.add(ind1)
        assert len(pf.individuals) == 1

        ***REMOVED*** Add non-dominated solution (different trade-off)
    ...
    def test_pareto_front_rejects_dominated(self):
        ...
        ***REMOVED*** Add dominant solution
        ind1 = self._create_individual(0.9, 0.9, 1)
        pf.add(ind1)

        ***REMOVED*** Try to add dominated solution
        ind2 = self._create_individual(0.7, 0.7, 2)
        pf.add(ind2)

        ***REMOVED*** Should still have only 1
        assert len(pf.individuals) == 1
    ...
    def test_pareto_front_removes_dominated(self):
        ...
        ***REMOVED*** Add weaker solution first
    ...
    def test_get_extreme_solutions(self):
        ...
        ***REMOVED*** Add solutions with different extremes
        pf.add(self._create_individual(0.9, 0.3, 1))  ***REMOVED*** Best coverage
        pf.add(self._create_individual(0.3, 0.9, 2))  ***REMOVED*** Best fairness
        pf.add(self._create_individual(0.6, 0.6, 3))  ***REMOVED*** Balanced
    ...
    def test_get_knee_point(self):
        ...
        ***REMOVED*** Create a Pareto front with clear knee
        pf.add(self._create_individual(0.9, 0.1, 1))
        pf.add(self._create_individual(0.7, 0.5, 2))  ***REMOVED*** Likely knee
        pf.add(self._create_individual(0.5, 0.7, 3))
        pf.add(self._create_individual(0.1, 0.9, 4))

        knee = pf.get_knee_point()
        assert knee is not None
        ***REMOVED*** Knee should be one of the middle solutions
...
class TestCrowdingDistance:
    ...
    def test_boundary_points_infinite_distance(self):
        ...
        individuals = self._create_individuals([
            (0.1, 0.9),  ***REMOVED*** Boundary
            (0.5, 0.5),  ***REMOVED*** Middle
            (0.9, 0.1),  ***REMOVED*** Boundary
        ])

        CrowdingDistance.compute(individuals, ["coverage", "fairness"])

        ***REMOVED*** First and last (by coverage) should have infinite distance
    ...
    def test_crowding_distance_middle_points(self):
        ...
        individuals = self._create_individuals([
            (0.1, 0.9),
            (0.3, 0.7),  ***REMOVED*** Middle
            (0.5, 0.5),  ***REMOVED*** Middle
            (0.7, 0.3),  ***REMOVED*** Middle
            (0.9, 0.1),
        ])

        CrowdingDistance.compute(individuals, ["coverage", "fairness"])

        ***REMOVED*** Middle points should have finite distance > 0
...
class TestNSGA2Config:
    ...
    def test_default_config(self):
        ...
        assert config.tournament_size == 2  ***REMOVED*** Binary for NSGA-II
...
class TestNSGA2Solver:
    ...
    def test_fast_non_dominated_sort_single_front(self):
        ...
        ***REMOVED*** Create population where no solution dominates another
        population = []
        for i in range(5):
            ind = Individual(
                chromosome=Chromosome.create_empty(3, 5),
        ...
        ***REMOVED*** All should be in front 0 (non-dominated)
        assert len(fronts) >= 1
    ...
    def test_fast_non_dominated_sort_multiple_fronts(self):
        ...
        population = [
            Individual(
                chromosome=Chromosome.create_empty(3, 5),
                fitness=FitnessVector(coverage=0.9, fairness=0.9),  ***REMOVED*** Front 0
                id=1,
            ),
            Individual(
                chromosome=Chromosome.create_empty(3, 5),
                fitness=FitnessVector(coverage=0.8, fairness=0.8),  ***REMOVED*** Front 1
                id=2,
            ),
            Individual(
                chromosome=Chromosome.create_empty(3, 5),
                fitness=FitnessVector(coverage=0.5, fairness=0.5),  ***REMOVED*** Front 2
                id=3,
            ),
        ]
