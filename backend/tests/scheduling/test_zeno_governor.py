...
class TestZenoGovernor:
    ...
    async def test_multiple_interventions_increase_risk(self, governor):
        ...
        ***REMOVED*** Log many interventions in short time
        for i in range(15):
    ...
    def test_compute_measurement_frequency_multiple(self, governor):
        ...
        ***REMOVED*** Add 6 interventions in past 12 hours
        for i in range(6):
        ...
        expected = 6 / 12  ***REMOVED*** 0.5 interventions per hour
    ...
    def test_compute_local_optima_risk_low(self, governor):
        ...
        governor.total_assignments = 100
        governor.frozen_assignments = {uuid.uuid4() for _ in range(5)}  ***REMOVED*** 5% frozen

        risk = governor.compute_local_optima_risk()
        assert risk < 0.3  ***REMOVED*** Low risk

    def test_compute_local_optima_risk_high(self, governor):
        """Test local optima risk with problematic state."""
        governor.total_assignments = 100
        governor.frozen_assignments = {uuid.uuid4() for _ in range(80)}  ***REMOVED*** 80% frozen

        ***REMOVED*** Add many interventions
        for i in range(20):
        ...
        risk = governor.compute_local_optima_risk()
        assert risk > 0.6  ***REMOVED*** High risk
    ...
    def test_recommend_policy_critical_risk(self, governor):
        ...
        governor.total_assignments = 100
        governor.frozen_assignments = {uuid.uuid4() for _ in range(60)}  ***REMOVED*** 60% frozen
    ...
    def test_compute_evolution_prevented_with_blocks(self, governor):
        ...
        ***REMOVED*** Log blocked solver attempts
        for _ in range(5):
    ...
    async def test_is_in_freedom_window_true(self, governor):
        ...
        ***REMOVED*** Create window that includes current time
    ...
    def test_is_in_freedom_window_expired(self, governor):
        ...
        ***REMOVED*** Create window in the past
...
class TestZenoDashboard:
    ...
    async def test_get_solver_performance_summary_with_data(self, dashboard, governor):
        ...
        ***REMOVED*** Log solver attempts
        for i in range(10):
            governor.log_solver_attempt(
                proposed_changes={uuid.uuid4()},
                successful=(i % 3 == 0),  ***REMOVED*** 3/10 successful
                blocked=(i % 5 == 0),  ***REMOVED*** 2/10 blocked
            )

        summary = await dashboard.get_solver_performance_summary()

        assert summary["total_attempts"] == 10
        assert summary["successful_attempts"] == 4  ***REMOVED*** indices 0, 3, 6, 9
        assert summary["blocked_attempts"] == 2  ***REMOVED*** indices 0, 5
        assert 0.0 <= summary["success_rate"] <= 1.0

    @pytest.mark.asyncio
    async def test_export_metrics_for_monitoring(self, dashboard, governor):
        """Test exporting metrics for Prometheus."""
        ***REMOVED*** Add some data
        await governor.log_human_intervention(
            checkpoint_time=datetime.now(),
        ...
        ***REMOVED*** Check metric structure
...
class TestIntegrationScenarios:
    ...
    async def test_normal_operations_scenario(self):
        ...
        ***REMOVED*** 3 reviews per day, spaced 8 hours apart - should stay LOW risk
        for day in range(3):
        ...
        ***REMOVED*** Should still be low risk
        assert risk == ZenoRisk.LOW
    ...
    async def test_over_monitoring_scenario(self):
        ...
        ***REMOVED*** Check every hour for a day - should escalate to HIGH/CRITICAL
        for hour in range(24):
            timestamp = datetime.now() - timedelta(hours=23 - hour)
            assignments = {uuid.uuid4() for _ in range(5)}
            locked = {list(assignments)[0]}  ***REMOVED*** Lock one assignment each time
        ...
        ***REMOVED*** Should be high or critical risk
        assert risk in [ZenoRisk.HIGH, ZenoRisk.CRITICAL]
    ...
    async def test_solver_frozen_scenario(self):
        ...
        ***REMOVED*** Lock 60% of assignments
        ...
        ***REMOVED*** Try solver improvements that conflict with frozen assignments
        for _ in range(10):
            ***REMOVED*** Mix of frozen and unfrozen in proposed changes
        ...
        ***REMOVED*** Should detect high frozen ratio and blocked evolution
        assert metrics.frozen_ratio >= 0.6
        assert metrics.evolution_prevented > 0
        assert metrics.local_optima_risk > 0.6

        ***REMOVED*** Should recommend strict policy
        policy = governor.recommend_intervention_policy()
