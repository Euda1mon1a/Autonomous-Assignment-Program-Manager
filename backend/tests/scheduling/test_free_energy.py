...
***REMOVED*** Mock objects for testing
class MockPerson:
...
class MockTemplate:
    ...
    def __init__(self, name: str = "Clinic", activity_type: str = "outpatient"):
        self.id = uuid.uuid4()
        self.name = name
...
class TestDemandForecast:
    ...
    def test_initialization(self):
        ...
        assert forecast.forecast_horizon_days == 28  ***REMOVED*** default
    ...
    def test_to_vector_missing_template(self):
        ...
        coverage = {
            template_ids[0]: 0.7,
            ***REMOVED*** template_ids[1] missing
            template_ids[2]: 0.3,
        }
        ...
        assert vector[0] == 0.7
        assert vector[1] == 0.0  ***REMOVED*** missing -> 0.0
        assert vector[2] == 0.3
    ...
    def test_from_historical_data(self):
        ...
        ***REMOVED*** Create mock assignments
        assignments = []
        for i in range(60):
            template_idx = i % 3  ***REMOVED*** Uniform distribution
            assignments.append(
                MockAssignment(
        ...
        ***REMOVED*** Should have uniform coverage (20 assignments per template)
    ...
    def test_from_historical_data_skewed(self):
        ...
        ***REMOVED*** Skewed distribution: 70% template 0, 20% template 1, 10% template 2
        assignments = []
        for _ in range(70):
            assignments.append(
                MockAssignment(
    ...
    def test_from_uniform_prior(self):
        ...
        ***REMOVED*** Should have uniform coverage
        assert len(forecast.predicted_coverage) == 3
        for coverage in forecast.predicted_coverage.values():
            assert abs(coverage - 0.333) < 0.01

        assert forecast.confidence < 0.5  ***REMOVED*** Low confidence (no data)
        assert forecast.metadata.get("prior_type") == "uniform"


***REMOVED*** ==================== ScheduleOutcome Tests ====================
...
class TestGenerativeModel:
    ...
    def test_update_single_outcome(self):
        ...
        ***REMOVED*** Create outcome with skewed distribution
        coverage = {
            template_ids[0]: 0.6,
            template_ids[1]: 0.3,
            template_ids[2]: 0.1,
        ...
        ***REMOVED*** Weights should have moved from uniform toward actual distribution
    ...
    def test_update_multiple_outcomes(self):
        ...
        ***REMOVED*** Consistently skewed outcomes
        for _ in range(10):
            coverage = {
                template_ids[0]: 0.7,
                template_ids[1]: 0.2,
        ...
        ***REMOVED*** Weights should converge to actual distribution
    ...
    def test_kl_divergence(self):
        ...
        ***REMOVED*** Initially, KL should be ~0 (learned == prior)
        initial_kl = model.compute_kl_divergence()
        assert abs(initial_kl) < 0.01

        ***REMOVED*** After learning, KL should increase
        template_ids = [uuid.uuid4() for _ in range(3)]
        coverage = {
            template_ids[0]: 0.9,
            template_ids[1]: 0.05,
...
***REMOVED*** ==================== FreeEnergyScheduler Tests ====================
...
class TestFreeEnergyScheduler:
    ...
    def test_initialization(self):
        ...
        assert solver.generative_model is None  ***REMOVED*** Not initialized yet
    ...
    def test_compute_prediction_error(self):
        ...
        ***REMOVED*** Create chromosome matching forecast
        chromosome = Chromosome.create_empty(n_residents=2, n_blocks=4)
        ***REMOVED*** Assign to match: 60% template 0, 40% template 1
        chromosome.genes[0, 0] = 1  ***REMOVED*** template 0
        chromosome.genes[0, 1] = 1
        chromosome.genes[0, 2] = 1
        chromosome.genes[1, 0] = 2  ***REMOVED*** template 1
        ...
        assert error >= 0.0
        ***REMOVED*** Error should be small since we matched the forecast
        assert error < 0.5
    ...
    def test_minimize_free_energy(self):
        ...
        ***REMOVED*** Energy should decrease or stay same
    ...
    def test_active_inference_step(self):
        ...
        ***REMOVED*** Model should have been updated
        assert solver.generative_model.outcomes_seen == initial_outcomes + 1

        ***REMOVED*** Schedule and forecast should be returned
    ...
    def test_solve_simple(self):
        ...
        ***REMOVED*** Success depends on constraints, so we don't assert True/False
    ...
    def test_surprise_tracking(self):
        ...
        ***REMOVED*** Surprise history should be populated
...
class TestFreeEnergyIntegration:
    ...
    async def test_forecast_generator_placeholder(self):
        ...
        ***REMOVED*** This would test ForecastGenerator with actual database
        ***REMOVED*** For now, just verify imports work
...
class TestFreeEnergyEdgeCases:
    ...
    def test_zero_templates(self):
        ...
        ***REMOVED*** (actual behavior depends on implementation)
