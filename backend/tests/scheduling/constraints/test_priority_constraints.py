...
***REMOVED*** ============================================================================
***REMOVED*** ClinicCapacityConstraint Tests
***REMOVED*** ============================================================================
...
class TestClinicCapacityConstraint:
    ...
    def test_validate_within_capacity(self):
        ...
        ***REMOVED*** Create mock template with max 4 residents
        template = Mock()
        template.id = uuid4()
        template.name = "Sports Medicine Clinic"
        template.max_residents = 4

        ***REMOVED*** Create mock block
        block = Mock()
        block.id = uuid4()
        block.date = date.today()

        ***REMOVED*** Create 3 assignments (within capacity of 4)
        assignments = []
        for i in range(3):
            assignment = Mock()
    ...
    def test_validate_at_capacity(self):
        ...
        ***REMOVED*** Create exactly 4 assignments (at capacity)
        assignments = []
        for i in range(4):
            assignment = Mock()
            assignment.block_id = block.id
    ...
    def test_validate_over_capacity(self):
        ...
        ***REMOVED*** Create 5 assignments (over capacity of 4)
        assignments = []
        for i in range(5):
            assignment = Mock()
            assignment.block_id = block.id
    ...
    def test_validate_no_capacity_limit(self):
        ...
        ***REMOVED*** Template with no max_residents set
        template = Mock()
        template.id = uuid4()
        ...
        ***REMOVED*** Should be satisfied (no limit enforced)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_zero_capacity(self):
        ...
        ***REMOVED*** max_residents=0 means no limit enforced (per implementation)
        assert result.satisfied is True
    ...
    def test_validate_multiple_blocks_same_template(self):
        ...
        ***REMOVED*** 2 assignments per block (each at capacity)
        assignments = []
        ...
        ***REMOVED*** Should be satisfied (capacity enforced per block)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_multiple_templates_same_block(self):
        ...
        ***REMOVED*** 2 assignments for template1, 3 for template2 (both at capacity)
        assignments = []
        ...
        ***REMOVED*** Should be satisfied (capacity enforced per template)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_assignment_without_template(self):
        ...
        ***REMOVED*** Assignment without rotation_template_id
        assignment = Mock()
        assignment.block_id = block.id
        ...
        ***REMOVED*** Should be satisfied (assignment ignored)
        assert result.satisfied is True
        assert len(result.violations) == 0


***REMOVED*** ============================================================================
***REMOVED*** NightFloatPostCallConstraint Tests
***REMOVED*** ============================================================================
...
class TestNightFloatPostCallConstraint:
    ...
    def test_validate_with_proper_post_call(self):
        ...
        ***REMOVED*** Create NF and PC templates
        nf_template = Mock()
        ...
        ***REMOVED*** Create blocks for block-half transition
        ***REMOVED*** Block 1, half 1, last day (day 14)
        ...
        ***REMOVED*** Block 1, half 2, first day (day 15) - PC day
        ...
        ***REMOVED*** Create assignments: NF on last day of half 1
        ...
        ***REMOVED*** PC assignments on day 15 (both AM and PM)
        ...
        ***REMOVED*** Should be satisfied (PC properly assigned)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_missing_post_call_am(self):
        ...
        ***REMOVED*** Last day of block-half
        last_day = date(2025, 1, 14)
        nf_block = Mock()
        nf_block.id = uuid4()
        ...
        ***REMOVED*** Need to add AM block for pc_day (even though no assignment)
        pc_am_block = Mock()
        pc_am_block.id = uuid4()
        pc_am_block.date = pc_day
        pc_am_block.time_of_day = "AM"
        ...
        ***REMOVED*** Should have violation for missing AM PC
        assert result.satisfied is False
        assert len(result.violations) >= 1
        violation = result.violations[0]
    ...
    def test_validate_missing_post_call_pm(self):
        ...
        ***REMOVED*** NF assignment
        nf_assignment = Mock()
        nf_assignment.block_id = nf_block.id
        nf_assignment.person_id = resident.id
        nf_assignment.rotation_template_id = nf_template.id

        ***REMOVED*** Only AM PC (PM missing)
        pc_assignment_am = Mock()
        pc_assignment_am.block_id = pc_am_block.id
        pc_assignment_am.person_id = resident.id
        pc_assignment_am.rotation_template_id = pc_template.id
        ...
        ***REMOVED*** Should have violation for missing PM PC
        assert result.satisfied is False
        assert len(result.violations) >= 1
        ***REMOVED*** Find the PM violation
    ...
    def test_validate_no_night_float_assignments(self):
        ...
        ***REMOVED*** Regular assignment (not NF)
        assignment = Mock()
        assignment.block_id = block.id
        ...
        ***REMOVED*** Should be satisfied (no NF to enforce PC for)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_no_templates(self):
        ...
        context = SchedulingContext(
            ...
            faculty=[],
            blocks=[block],
            templates=[],  ***REMOVED*** No templates
        )

        result = constraint.validate([assignment], context)

        ***REMOVED*** Should be satisfied (can't enforce without templates)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_multiple_residents(self):
        ...
        ***REMOVED*** Resident 1: NF + proper PC
        r1_nf = Mock()
        ...
        ***REMOVED*** Resident 2: NF + proper PC
        r2_nf = Mock()
        ...
        ***REMOVED*** Both residents have proper PC
        assert result.satisfied is True
        assert len(result.violations) == 0
