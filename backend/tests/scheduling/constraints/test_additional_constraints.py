...
***REMOVED*** ============================================================================
***REMOVED*** EightyHourRuleConstraint Tests
***REMOVED*** ============================================================================
...
class TestEightyHourRuleConstraint:
    ...
    def test_constraint_initialization(self):
        ...
        ***REMOVED*** Max blocks calculation: (80 * 4) / 6 = 53.33 -> 53
    ...
    def test_validate_well_under_limit(self):
        ...
        ***REMOVED*** Assign resident to 40 blocks (40 * 6 = 240 hours / 4 weeks = 60 hours/week)
        assignments = []
        for i in range(40):
            assignment = Mock()
    ...
    def test_validate_exactly_at_limit(self):
        ...
        ***REMOVED*** Assign exactly 53 blocks (53 * 6 = 318 hours / 4 weeks = 79.5 hours/week)
        ***REMOVED*** This is the maximum allowed
        assignments = []
        for i in range(53):
            assignment = Mock()
            assignment.person_id = resident.id
        ...
        ***REMOVED*** Should pass - at or just under the limit
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_over_limit(self):
        ...
        ***REMOVED*** Assign 56 blocks (56 * 6 = 336 hours / 4 weeks = 84 hours/week) - VIOLATION
        assignments = []
        for i in range(56):
            assignment = Mock()
            assignment.person_id = resident.id
    ...
    def test_validate_rolling_window_violation(self):
        ...
        ***REMOVED*** Create 8 weeks of blocks
        start_date = date(2025, 1, 1)
        blocks = []
        for day_offset in range(56):  ***REMOVED*** 8 weeks
            for time_of_day in ["AM", "PM"]:
                block = Mock()
                block.id = uuid4()
                block.date = start_date + timedelta(days=day_offset)
        ...
        ***REMOVED*** Create assignments that violate in the middle 4-week window
        ***REMOVED*** First 2 weeks: light (20 blocks)
        ***REMOVED*** Middle 4 weeks: heavy (60 blocks = 90 hours/week average)
        ***REMOVED*** Last 2 weeks: light (20 blocks)
        assignments = []

        ***REMOVED*** First 2 weeks: 20 blocks
        for i in range(20):
            assignment = Mock()
            assignment.person_id = resident.id
            assignment.block_id = blocks[i].id
        ...
        ***REMOVED*** Middle 4 weeks (days 14-41): 60 blocks
        for i in range(28, 88):  ***REMOVED*** Block indices for middle period
            assignment = Mock()
            assignment.person_id = resident.id
            assignment.block_id = blocks[i].id
            assignments.append(assignment)

        ***REMOVED*** Last 2 weeks: 20 blocks
        ...
        ***REMOVED*** Should detect violation in the middle rolling window
        assert result.satisfied is False
        assert len(result.violations) >= 1
    ...
    def test_validate_multiple_residents(self):
        ...
        ***REMOVED*** Resident 1: 40 blocks (60 hours/week - OK)
        for i in range(40):
            assignment = Mock()
        ...
        ***REMOVED*** Resident 2: 56 blocks (84 hours/week - VIOLATION)
        for i in range(56):
            assignment = Mock()
        ...
        ***REMOVED*** Should have violation for resident2 only
        assert result.satisfied is False
        assert len(result.violations) == 1
...
***REMOVED*** ============================================================================
***REMOVED*** SupervisionRatioConstraint Tests
***REMOVED*** ============================================================================
...
class TestSupervisionRatioConstraint:
    ...
    def test_constraint_initialization(self):
        ...
        ***REMOVED*** Verify ratios
        assert constraint.PGY1_RATIO == 2  ***REMOVED*** 1 faculty per 2 PGY-1
        assert constraint.OTHER_RATIO == 4  ***REMOVED*** 1 faculty per 4 PGY-2/3
    ...
    def test_calculate_required_faculty_pgy23_only(self):
        ...
        ***REMOVED*** 5 PGY-2/3 = 2 faculty (1.25 load)
        assert constraint.calculate_required_faculty(0, 5) == 2

        ***REMOVED*** 8 PGY-2/3 = 2 faculty (2.0 load)
    ...
    def test_calculate_required_faculty_mixed(self):
        ...
        ***REMOVED*** 1 PGY-1 + 1 PGY-2: 0.5 + 0.25 = 0.75 → 1 faculty
        assert constraint.calculate_required_faculty(1, 1) == 1

        ***REMOVED*** 1 PGY-1 + 2 PGY-2: 0.5 + 0.5 = 1.0 → 1 faculty
        assert constraint.calculate_required_faculty(1, 2) == 1

        ***REMOVED*** 1 PGY-1 + 3 PGY-2: 0.5 + 0.75 = 1.25 → 2 faculty
        assert constraint.calculate_required_faculty(1, 3) == 2

        ***REMOVED*** 2 PGY-1 + 4 PGY-2: 1.0 + 1.0 = 2.0 → 2 faculty
        assert constraint.calculate_required_faculty(2, 4) == 2

        ***REMOVED*** 3 PGY-1 + 4 PGY-2: 1.5 + 1.0 = 2.5 → 3 faculty
    ...
    def test_validate_adequate_supervision_pgy1(self):
        ...
        ***REMOVED*** 2 PGY-1 residents + 1 faculty (meets 1:2 ratio)
        assignments = []
    ...
    def test_validate_inadequate_supervision_pgy1(self):
        ...
        ***REMOVED*** 3 PGY-1 residents + 1 faculty (violates 1:2 ratio - need 2 faculty)
        assignments = []
    ...
    def test_validate_adequate_supervision_mixed(self):
        ...
        ***REMOVED*** 1 PGY-1 + 2 PGY-2/3 + 1 faculty
        ***REMOVED*** Load: 0.5 + 0.25 + 0.25 = 1.0 → 1 faculty required ✓
        assignments = []
    ...
    def test_validate_no_faculty_assigned(self):
        ...
        ***REMOVED*** 1 resident, 0 faculty (violation)
        assignment = Mock()
        assignment.person_id = resident.id
        assignment.block_id = block.id
    ...
    def test_validate_multiple_blocks_independently(self):
        ...
        ***REMOVED*** Block 1: Resident + Faculty (OK)
        r1_assignment = Mock()
        ...
        ***REMOVED*** Block 2: Resident only (VIOLATION)
        r2_assignment = Mock()
        ...
        ***REMOVED*** Should have violation for block2
        assert result.satisfied is False
        assert len(result.violations) == 1
...
***REMOVED*** ============================================================================
***REMOVED*** OvernightCallCoverageConstraint Tests
***REMOVED*** ============================================================================
...
class TestOvernightCallCoverageConstraint:
    ...
    def test_validate_proper_coverage_all_nights(self):
        ...
        ***REMOVED*** Sunday through Thursday (5 call nights)
        sunday = date(2025, 1, 5)  ***REMOVED*** Sunday
        blocks = []
        for day_offset in range(5):  ***REMOVED*** Sun-Thu
            block = Mock()
            block.id = uuid4()
        ...
        ***REMOVED*** Create call assignments (one per night)
        assignments = []
    ...
    def test_validate_missing_call_coverage(self):
        ...
        ***REMOVED*** Sunday through Thursday
        ...
        ***REMOVED*** Only assign 4 nights (skip Wednesday)
        assignments = []
        for i, block in enumerate(blocks):
            if i == 3:  ***REMOVED*** Skip Wednesday
                continue
            assignment = Mock()
    ...
    def test_validate_double_coverage(self):
        ...
        ***REMOVED*** Monday night
        monday = date(2025, 1, 6)  ***REMOVED*** Monday
        block = Mock()
        block.id = uuid4()
        block.date = monday
        block.time_of_day = "AM"

        ***REMOVED*** Two faculty assigned to same night (violation)
        assignment1 = Mock()
    ...
    def test_validate_friday_saturday_excluded(self):
        ...
        ***REMOVED*** Friday and Saturday (should be ignored)
        friday = date(2025, 1, 10)  ***REMOVED*** Friday
        saturday = date(2025, 1, 11)  ***REMOVED*** Saturday
        ...
        ***REMOVED*** No call assignments for Fri/Sat
        context = SchedulingContext(
            residents=[],
            faculty=[],
            blocks=blocks,
        ...
        ***REMOVED*** Should be satisfied (Fri/Sat not checked)
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_ignores_non_overnight_assignments(self):
        ...
        ***REMOVED*** Regular assignment (not overnight call)
        assignment = Mock()
        assignment.person_id = faculty.id
        assignment.block_id = block.id
        assignment.call_type = None  ***REMOVED*** Not an overnight call
        ...
        ***REMOVED*** Should have violation (no overnight call found)
        assert result.satisfied is False
        assert len(result.violations) == 1
    ...
    def test_validate_empty_assignments(self):
        ...
        ***REMOVED*** Should have violation (Monday requires call)
        assert result.satisfied is False
        assert len(result.violations) == 1


***REMOVED*** ============================================================================
***REMOVED*** OneInSevenRuleConstraint Tests
***REMOVED*** ============================================================================
...
class TestOneInSevenRuleConstraint:
    ...
    def test_validate_within_limit(self):
        ...
        ***REMOVED*** Create 7 days of blocks
        start_date = date(2025, 1, 1)
        blocks = []
        ...
        ***REMOVED*** Work 5 consecutive days, then 2 days off
        assignments = []
        for i in range(5):  ***REMOVED*** Days 0-4
            assignment = Mock()
            assignment.person_id = resident.id
            assignment.block_id = blocks[i].id
            assignments.append(assignment)
    ...
    def test_validate_exactly_at_limit(self):
        ...
        ***REMOVED*** Work exactly 6 consecutive days
        assignments = []
        for i in range(6):  ***REMOVED*** Days 0-5
            assignment = Mock()
            assignment.person_id = resident.id
            assignment.block_id = blocks[i].id
            assignments.append(assignment)
        ...
        ***REMOVED*** Should pass - 6 consecutive is the maximum allowed
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_over_limit(self):
        ...
        ***REMOVED*** Work 7 consecutive days (violation)
        assignments = []
        for i in range(7):
            assignment = Mock()
            assignment.person_id = resident.id
    ...
    def test_validate_pattern_with_breaks(self):
        ...
        ***REMOVED*** Pattern: 5 days on, 1 day off, 5 days on, 1 day off
        ...
        ***REMOVED*** Should pass - max consecutive is 5 days
        assert result.satisfied is True
        assert len(result.violations) == 0
    ...
    def test_validate_multiple_residents_independent(self):
        ...
        ***REMOVED*** Resident 1: 5 consecutive days (OK)
        for i in range(5):
            assignment = Mock()
            assignment.person_id = resident1.id
            assignment.block_id = blocks[i].id
        ...
        ***REMOVED*** Resident 2: 7 consecutive days (VIOLATION)
        for i in range(7):
            assignment = Mock()
            assignment.person_id = resident2.id
            assignment.block_id = blocks[i].id
