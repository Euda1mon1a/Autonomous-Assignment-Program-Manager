...
def minimal_app():
    ...
    ***REMOVED*** Add exception handler for testing error cases
    @app.exception_handler(Exception)
    async def global_exception_handler(request: Request, exc: Exception):
        """Handle all unhandled exceptions."""
        ***REMOVED*** Get request ID from context and include in response headers
...
***REMOVED*** ============================================================================
***REMOVED*** Unit Tests - Request ID Generation
***REMOVED*** ============================================================================
...
class TestRequestIDGeneration:
    ...
    def test_generates_uuid_when_no_header_provided(self, test_client):
        ...
        ***REMOVED*** Verify it's a valid UUID format
        try:
    ...
    def test_request_id_in_response_body(self, test_client):
        ...
        ***REMOVED*** Request ID in response header should match the one in context
...
***REMOVED*** ============================================================================
***REMOVED*** Integration Tests - Response Headers
***REMOVED*** ============================================================================
...
class TestResponseHeaders:
    ...
    def test_request_id_in_error_responses(self, test_client):
        ...
        ***REMOVED*** Endpoint raises ValueError, should get 500 error
        assert response.status_code == 500
        ***REMOVED*** In production, exception handlers can access request ID via get_request_id()
        ***REMOVED*** and include it in error responses (as demonstrated in main.py)
        ***REMOVED*** This test verifies the endpoint returns an error response
...
***REMOVED*** ============================================================================
***REMOVED*** Integration Tests - Context Variable
***REMOVED*** ============================================================================
...
class TestContextIntegration:
    ...
    def test_request_id_available_in_context(self, test_client):
        ...
        data = response.json()
        ***REMOVED*** The endpoint uses get_request_id() which reads from context
        assert data["request_id"] == custom_id

    def test_context_cleared_after_request(self, test_client):
        """Test that context is cleared after request completes."""
        ***REMOVED*** Make a request
        response = test_client.get("/test")
        assert response.status_code == 200

        ***REMOVED*** After request completes, context should be cleared
        ***REMOVED*** (We can't directly test this in TestClient, but verify no leakage between requests)
        custom_id = str(uuid.uuid4())
        response2 = test_client.get("/test", headers={"X-Request-ID": custom_id})
        data2 = response2.json()

        ***REMOVED*** Second request should have its own ID, not the first one's
        assert data2["request_id"] == custom_id


***REMOVED*** ============================================================================
***REMOVED*** Integration Tests - Logging
***REMOVED*** ============================================================================
...
class TestLoggingIntegration:
    ...
    def test_sets_request_id_in_logging_context(
        ...
        assert response.status_code == 200
        ***REMOVED*** Verify set_logging_request_id was called with the request ID
        mock_set_logging_request_id.assert_called()
        ***REMOVED*** Get the actual call arguments
...
class TestRequestIDSecurity:
    ...
    def test_accepts_various_valid_id_formats(self, test_client):
        ...
        valid_ids = [
            str(uuid.uuid4()),  ***REMOVED*** Standard UUID
            "trace-123-abc",  ***REMOVED*** Custom format
            "12345",  ***REMOVED*** Numeric
            "a" * 64,  ***REMOVED*** Long alphanumeric (64 chars)
        ]
    ...
    def test_handles_empty_request_id_header(self, test_client):
        ...
        assert response.status_code == 200
        ***REMOVED*** Should generate a new UUID when header is empty
        request_id = response.headers["X-Request-ID"]
        assert len(request_id) > 0
        ***REMOVED*** Verify it's a valid UUID
        try:
            uuid.UUID(request_id)
            is_valid_uuid = True
        except ValueError:
    ...
    def test_handles_whitespace_only_request_id(self, test_client):
        ...
        assert response.status_code == 200
        request_id = response.headers["X-Request-ID"]
        ***REMOVED*** Should not be whitespace
    ...
    def test_rejects_excessively_long_request_id(self, test_client):
        ...
        ***REMOVED*** Create a request ID longer than MAX_REQUEST_ID_LENGTH (255)
        long_id = "x" * 300
        ...
        ***REMOVED*** Should generate a new UUID instead of using the long ID
        assert request_id != long_id
        assert len(request_id) <= 255
        ***REMOVED*** Should be a valid UUID
        try:
            uuid.UUID(request_id)
            is_valid_uuid = True
        except ValueError:
    ...
    def test_accepts_request_id_at_max_length(self, test_client):
        ...
        ***REMOVED*** Create a request ID at exactly MAX_REQUEST_ID_LENGTH (255)
        max_length_id = "x" * 255
        response = test_client.get("/test", headers={"X-Request-ID": max_length_id})

        assert response.status_code == 200
        ***REMOVED*** Should accept this ID
...
class TestEdgeCases:
    ...
    def test_multiple_concurrent_requests(self, test_client):
        ...
        ***REMOVED*** Simulate multiple concurrent requests
        responses = []
        for i in range(5):
            response = test_client.get("/test")
        ...
        ***REMOVED*** All should have different request IDs
    ...
    def test_request_id_with_special_characters(self, test_client):
        ...
        ***REMOVED*** Test with various special characters that might appear in trace IDs
        special_ids = [
    ...
    def test_request_id_persists_through_error(self, test_client):
        ...
        ***REMOVED*** Endpoint raises error
        assert response.status_code == 500
        ***REMOVED*** Error response should be returned
        assert response.json()["detail"] == "Internal server error"
...
class TestPerformance:
    ...
    def test_minimal_overhead(self, test_client):
        ...
        ***REMOVED*** Measure time for multiple requests
        start = time.perf_counter()
        for _ in range(100):
            test_client.get("/test")
        elapsed = time.perf_counter() - start

        ***REMOVED*** Should complete 100 requests in reasonable time (< 5 seconds even on slow CI)
        assert elapsed < 5.0, f"100 requests took {elapsed:.2f}s, expected < 5.0s"


***REMOVED*** ============================================================================
***REMOVED*** Functional Tests - get_request_id()
***REMOVED*** ============================================================================
...
class TestGetRequestId:
    ...
    def test_get_request_id_returns_none_outside_request(self):
        ...
        ***REMOVED*** Outside of a request, context should be None or the default value
        request_id = get_request_id()
    ...
    def test_get_request_id_inside_request(self, test_client):
        ...
        data = response.json()
        ***REMOVED*** The endpoint calls get_request_id() and returns it
        assert data["request_id"] == custom_id
