...
class TestDailyManifestEndpoint:
    ...
    def test_daily_manifest_requires_date_param(
        ...
        ***REMOVED*** Should return 422 for missing required param
    ...
    def test_daily_manifest_success(
        ...
        ***REMOVED*** May return 200 or 401 depending on auth fixture
    ...
    def test_daily_manifest_invalid_time_of_day(
        ...
        ***REMOVED*** Should return 400 for invalid time_of_day
    ...
    def test_daily_manifest_response_structure(
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Verify expected fields
            assert "date" in data
    ...
    def test_daily_manifest_future_date(
        ...
        ***REMOVED*** Should work for future dates
        assert response.status_code in [200, 401]
    ...
    def test_daily_manifest_past_date(
        ...
        ***REMOVED*** Should work for past dates
        assert response.status_code in [200, 401]
...
class TestDailyManifestWithData:
    ...
    def test_daily_manifest_with_assignments(
        ...
        ***REMOVED*** Get the date from the sample assignment's block
        ...
        if response.status_code == 200:
            data = response.json()
            assert "locations" in data
            ***REMOVED*** May or may not have locations depending on data
...
class TestDailyManifestEdgeCases:
    ...
    def test_daily_manifest_empty_day(
        ...
        ***REMOVED*** Use a far future date unlikely to have assignments
        ...
        if response.status_code == 200:
            data = response.json()
            ***REMOVED*** Should return empty locations list
            assert "locations" in data
...
class TestDailyManifestIntegration:
    ...
    def test_daily_manifest_performance(
        ...
        ***REMOVED*** Should respond within 2 seconds
        assert elapsed < 2.0
        assert response.status_code in [200, 401]
