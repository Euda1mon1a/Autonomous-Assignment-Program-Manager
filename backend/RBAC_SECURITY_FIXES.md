***REMOVED*** RBAC Security Core Fixes - Session 32
...
***REMOVED******REMOVED******REMOVED*** 1. Role Hierarchy Mismatch (CRITICAL)
...
HIERARCHY: dict[UserRole, set[UserRole]] = {
    ...
    UserRole.FACULTY: {UserRole.ADMIN, UserRole.COORDINATOR},  ***REMOVED*** WRONG!
    ...
}

***REMOVED*** AFTER (CORRECT):
HIERARCHY: dict[UserRole, set[UserRole]] = {
    UserRole.ADMIN: set(),                    ***REMOVED*** Top level
    ...
    UserRole.RESIDENT: set(),                 ***REMOVED*** INDEPENDENT
    UserRole.CLINICAL_STAFF: set(),          ***REMOVED*** INDEPENDENT
    UserRole.RN: {UserRole.CLINICAL_STAFF},  ***REMOVED*** Inherits from Clinical Staff
    UserRole.LPN: {UserRole.CLINICAL_STAFF}, ***REMOVED*** Inherits from Clinical Staff
    UserRole.MSA: {UserRole.CLINICAL_STAFF}, ***REMOVED*** Inherits from Clinical Staff
}
```
...
***REMOVED******REMOVED******REMOVED*** 2. Missing Context Validation for APPROVE/REJECT Actions (CRITICAL)
...
***REMOVED******REMOVED******REMOVED*** 3. Missing Cache Invalidation on Role Changes (CRITICAL)
...
async def invalidate_user_role_cache(
    ...
    Example:
        ***REMOVED*** In your user update service/endpoint:
        user.role = "coordinator"
        await db.commit()

        ***REMOVED*** CRITICAL: Invalidate cache immediately after role change
...
***REMOVED******REMOVED******REMOVED*** 4. Role Comparison Functions Added (ENHANCEMENT)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Role Level Constants
```python
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `get_role_level(role: UserRole) -> int`
Returns numeric privilege level for a role.

***REMOVED******REMOVED******REMOVED******REMOVED*** `is_higher_role(role: UserRole, compared_to: UserRole) -> bool`
Check if one role has higher privilege than another.
- Uses numeric levels instead of inheritance
- More intuitive comparison

***REMOVED******REMOVED******REMOVED******REMOVED*** `is_equal_or_higher_role(role: UserRole, compared_to: UserRole) -> bool`
Check if role has equal or higher privilege.

***REMOVED******REMOVED******REMOVED******REMOVED*** `can_manage_role(manager_role: UserRole, target_role: UserRole) -> bool`
...
***REMOVED******REMOVED******REMOVED*** Core RBAC Files
...
***REMOVED******REMOVED*** Security Impact Analysis

***REMOVED******REMOVED******REMOVED*** Before Fixes (VULNERABLE)
...
***REMOVED******REMOVED******REMOVED*** After Fixes (SECURE)
...
***REMOVED******REMOVED******REMOVED*** Access Matrix Tests (`test_access_matrix.py`)
...
***REMOVED******REMOVED******REMOVED*** Approval Context Tests (`test_approve_reject_context.py`)
...
***REMOVED******REMOVED******REMOVED*** For Endpoints Using APPROVE/REJECT Actions
...
async def approve_leave(
    ...
    ***REMOVED*** Fetch leave request
    leave = await get_leave_request(db, leave_id)

    ***REMOVED*** Check permission WITH context
    context = PermissionContext(
...
***REMOVED******REMOVED******REMOVED*** For Endpoints Changing User Roles
...
async def update_user_role(
    ...
    ***REMOVED*** CRITICAL: Invalidate cache immediately
    await invalidate_user_role_cache(
        user_id=str(user.id),
        old_role=old_role,
        new_role=new_role
...
***REMOVED******REMOVED******REMOVED*** Post-Deployment Verification
...
***REMOVED******REMOVED******REMOVED*** Manual Security Testing

```python
***REMOVED*** Test 1: Faculty should NOT be able to approve leave
...
***REMOVED*** Test 2: Coordinator cannot approve without context
context = PermissionContext(
    user_id=uuid4(),
    user_role=UserRole.COORDINATOR,
    resource_owner_id=None,  ***REMOVED*** Missing!
    resource_metadata={}
)
assert not acm.has_permission(
...
***REMOVED*** Test 3: Faculty cannot approve swaps they're not involved in
faculty_id = uuid4()
context = PermissionContext(
    user_id=faculty_id,
    ...
    resource_metadata={
        "requesting_user_id": str(uuid4()),  ***REMOVED*** Different user
        "target_user_id": str(uuid4()),      ***REMOVED*** Different user
    }
)
assert not acm.has_permission(
    UserRole.FACULTY,
...
***REMOVED******REMOVED******REMOVED*** Risk Level: **CRITICAL** → **LOW**
