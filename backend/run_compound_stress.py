...
def print_single_result(result, config):
    ...
    if result.fmit_survived:
        if result.fmit_weeks_turfed == 0:
            ...
            print(
                f"   🟡 FMIT SURVIVED - {result.fmit_weeks_turfed} weeks turfed to OB"
            )
    ...
    print(
        f"   Days in crisis:      {result.days_in_crisis}/{result.days_survived} ({result.days_in_crisis / max(1, result.days_survived) * 100:.0f}%)"
    )
