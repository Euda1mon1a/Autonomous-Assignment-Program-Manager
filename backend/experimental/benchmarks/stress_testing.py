...
Usage:
    ...
    >>>
    >>> ***REMOVED*** Create stress tester
    >>> tester = StressTester()
    >>>
    >>> ***REMOVED*** Create scheduler
    ...
    ... }
    >>>
    >>> ***REMOVED*** Run stress test at CRITICAL level
    ...
    >>>
    >>> ***REMOVED*** Run full degradation ladder
...
class StressLevel(str, Enum):
    ...
    NORMAL = "normal"  ***REMOVED*** 100% capacity
    ELEVATED = "elevated"  ***REMOVED*** 110% load
    HIGH = "high"  ***REMOVED*** 120% load
    CRITICAL = "critical"  ***REMOVED*** 130% load
    CRISIS = "crisis"  ***REMOVED*** 150% load
...
class StressTester:
    ...
    def run_stress_test(
        ...
        ***REMOVED*** Apply stress conditions to scenario based on stress level
        ...
        ***REMOVED*** Track constraints before scheduling
        ...
        ***REMOVED*** Run scheduler under stress
        try:
            ...
            ***REMOVED*** Catastrophic failure - system did not degrade gracefully
            notes.append(f"CATASTROPHIC FAILURE: {str(e)}")
            return StressResult(
                stress_level=stress_level,
                constraints_satisfied=0,
                constraints_relaxed=0,
                constraints_violated=999,  ***REMOVED*** Maximum violations
        ...
        ***REMOVED*** Track constraint satisfaction after scheduling
    ...
    def _apply_stress_conditions(
        ...
        ***REMOVED*** Map stress level to faculty absence rate
        absence_rates = {
        ...
        ***REMOVED*** Map stress level to solver timeout (less time = more stress)
        timeout_map = {
        ...
        ***REMOVED*** At CRISIS level, also prefer faster solver
        if stress_level == StressLevel.CRISIS:
            stressed["algorithm"] = "greedy"  ***REMOVED*** Fastest solver for emergency
        elif stress_level == StressLevel.CRITICAL:
            stressed["algorithm"] = "cp_sat"  ***REMOVED*** Balance speed and quality
        else:
    ...
    def _calculate_acgme_compliance(
        ...
        ***REMOVED*** Check if validation object has the expected attributes
        ...
        ***REMOVED*** Count ACGME-specific violations (80-hour, 1-in-7, supervision)
        acgme_violations = [
        ...
        ***REMOVED*** Calculate compliance based on violations
        ***REMOVED*** Assumption: For each resident/block, we check ~3 ACGME rules
        ***REMOVED*** Total checks = (assignments / 10) * 3 (rough estimate)
        total_assignments = getattr(validation, "statistics", {}).get("total_assignments", 100)
        estimated_checks = max(1, (total_assignments // 10) * 3)

        ***REMOVED*** Compliance = (checks passed) / (total checks)
    ...
    def _verify_patient_safety(
        ...
        ***REMOVED*** Check for supervision ratio violations
    ...
    def _check_graceful_degradation(
        ...
        """
        ***REMOVED*** Already handled catastrophic failure in main method
        ***REMOVED*** If we got here, system produced a result

        ***REMOVED*** Check 1: Got a result
        if not result:
            notes.append("✗ Graceful degradation FAILED - no result produced")
            return False

        ***REMOVED*** Check 2: Proportional degradation
        ***REMOVED*** Higher stress should result in more constraint relaxation
        ...
        ***REMOVED*** Allow some tolerance (±50%)
        if constraints_relaxed > expected_relaxation * 3:
            ...
            ***REMOVED*** Not a failure, just a warning

        ***REMOVED*** Check 3: Status is reasonable
        status = result.get("status")
        if status == "failed":
            notes.append("✗ Graceful degradation FAILED - scheduler reported failure")
            return False

        ***REMOVED*** Check 4: Validation exists
        if not validation:
            notes.append("⚠ Validation missing - cannot verify degradation")
            ***REMOVED*** Not a hard failure, but concerning

        ***REMOVED*** Check 5: Schedule has some coverage
        coverage_rate = getattr(validation, "coverage_rate", 0.0) if validation else 0.0

        ***REMOVED*** Minimum acceptable coverage by stress level
        min_coverage = {
