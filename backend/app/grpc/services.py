...
class ScheduleServicer:
    ...
    def GenerateSchedule(self, request: dict, context: grpc.ServicerContext) -> dict:
        ...
        try:
            ...
            ***REMOVED*** Get user from context (set by AuthenticationInterceptor)
            user_id = getattr(context, 'user_id', None)
            if not user_id:
                context.abort(grpc.StatusCode.UNAUTHENTICATED, "User not authenticated")

            ***REMOVED*** Import scheduling service
            from app.services.cached_schedule_service import CachedScheduleService

            schedule_service = CachedScheduleService(db)

            ***REMOVED*** Generate schedule (simplified - in production this would be async)
            logger.info(
                f"Generating schedule: {start_date} to {end_date}, "
                f"level={optimization_level}, user={user_id}"
            )

            ***REMOVED*** For demonstration, we return a pending status
            ***REMOVED*** In production, this would trigger a Celery task
            result = {
                "schedule_id": "pending",
                "status": "pending",
    ...
    def StreamAssignments(
        ...
        try:
            ...
            ***REMOVED*** Query assignments in batches
            service = AssignmentService(db)

            ***REMOVED*** For streaming, we query in batches to avoid loading everything into memory
...
class AssignmentServicer:
    ...
    def CreateAssignment(self, request: dict, context: grpc.ServicerContext) -> dict:
        ...
        db = SessionLocal()
        try:
            ***REMOVED*** Get user from context
            user_id = getattr(context, 'user_id', None)
            username = getattr(context, 'username', 'unknown')

            ***REMOVED*** Convert from proto format
            ...
            ***REMOVED*** Return with ACGME warnings
...
class PersonServicer:
    ...
    def ListPersons(self, request: dict, context: grpc.ServicerContext) -> dict:
        ...
        try:
            ...
            ***REMOVED*** Convert to proto format
            return {
...
class HealthServicer:
    ...
    def Check(self, request: dict, context: grpc.ServicerContext) -> dict:
        ...
        ***REMOVED*** Check database connectivity
        db = SessionLocal()
        try:
            ***REMOVED*** Simple query to verify DB is accessible
            db.execute("SELECT 1")
            db_status = "healthy"
        except Exception as e:
    ...
    def Watch(self, request: dict, context: grpc.ServicerContext) -> Iterator[dict]:
        ...
        """
        ***REMOVED*** Initial status
        yield {"status": "SERVING"}

        ***REMOVED*** In production, this would monitor for status changes
        ***REMOVED*** For now, we just keep the stream open
        while context.is_active():
            import time
            time.sleep(30)  ***REMOVED*** Check every 30 seconds

            ***REMOVED*** Send periodic heartbeat
            yield {"status": "SERVING"}
