...
class AuthenticationInterceptor(grpc.ServerInterceptor):
    ...
    def intercept_service(
        ...
        ***REMOVED*** Skip authentication for exempt methods
        if method in self.EXEMPT_METHODS:
            return continuation(handler_call_details)

        ***REMOVED*** Extract authorization metadata
        metadata_dict = dict(handler_call_details.invocation_metadata)
        auth_header = metadata_dict.get("authorization", "")

        if not auth_header.startswith("Bearer "):
            ***REMOVED*** Missing or invalid authorization header
        ...
        ***REMOVED*** Validate token
        ...
        try:
            ...
            ***REMOVED*** Token is valid - store user info in context for handlers to use
            ***REMOVED*** We'll pass it via a custom context object
...
class ErrorHandlingInterceptor(grpc.ServerInterceptor):
    ...
    def _handle_exception(exception: Exception, context: grpc.ServicerContext) -> None:
        ...
        ***REMOVED*** Map exception types to gRPC status codes
        if isinstance(exception, (ValueError, TypeError)):
            ...
            ***REMOVED*** Handle custom exceptions with status_code attribute
            status_code = grpc.StatusCode.INTERNAL
            message = getattr(exception, "message", str(exception))
        else:
            status_code = grpc.StatusCode.INTERNAL
            ***REMOVED*** Don't leak internal details in production
            if settings.DEBUG:
...
class LoggingInterceptor(grpc.ServerInterceptor):
    ...
    def _wrap_unary_unary(
        self, handler: grpc.RpcMethodHandler, method: str, user: str, start_time: float
    ) -> grpc.RpcMethodHandler:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.request_duration = Histogram(
                "grpc_request_duration_seconds", "gRPC request latency", ["method"]
            )
            self.requests_in_flight = Gauge(
                "grpc_requests_in_flight", "Active gRPC requests", ["method"]
            )
