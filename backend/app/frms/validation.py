...
class PredictionAccuracy:
    ...
    ***REMOVED*** Error metrics
    rmse: float = 0.0  ***REMOVED*** Root Mean Squared Error
    mae: float = 0.0  ***REMOVED*** Mean Absolute Error
    correlation: float = 0.0  ***REMOVED*** Pearson correlation

    ***REMOVED*** Classification metrics
    true_positives: int = 0  ***REMOVED*** Predicted high-risk, actual high fatigue
    true_negatives: int = 0  ***REMOVED*** Predicted low-risk, actual low fatigue
    false_positives: int = 0  ***REMOVED*** Predicted high-risk, actual low fatigue
    false_negatives: int = 0  ***REMOVED*** Predicted low-risk, actual high fatigue

    ***REMOVED*** Derived metrics
    sensitivity: float = 0.0  ***REMOVED*** TP / (TP + FN)
    specificity: float = 0.0  ***REMOVED*** TN / (TN + FP)
    ppv: float = 0.0  ***REMOVED*** Positive Predictive Value
    npv: float = 0.0  ***REMOVED*** Negative Predictive Value
    accuracy: float = 0.0  ***REMOVED*** (TP + TN) / total
    auc_roc: float = 0.0  ***REMOVED*** Area Under ROC Curve

    ***REMOVED*** Calibration
    calibration_slope: float = 1.0  ***REMOVED*** Ideal = 1.0
    calibration_intercept: float = 0.0  ***REMOVED*** Ideal = 0.0
...
class ValidationDataPoint:
    ...
    actual_fatigue_score: float | None = None  ***REMOVED*** 1-10 scale self-report
    actual_risk_reported: bool = False  ***REMOVED*** Binary: did they report fatigue?
    pvt_score: float | None = None  ***REMOVED*** Optional objective test score
    error_occurred: bool = False  ***REMOVED*** Did a clinical error occur?
    notes: str = ""
...
class ValidationStudy:
    ...
    Usage:
        ...
        ***REMOVED*** Add data points as they're collected
        study.add_data_point(
            person_id=resident_id,
            timestamp=datetime.now(),
            predicted_effectiveness=75.5,
            predicted_risk_level="caution",
            actual_fatigue_score=7.0,  ***REMOVED*** Self-reported 1-10
            actual_risk_reported=True,
        )

        ***REMOVED*** Calculate accuracy metrics
    ...
    def __init__(
        self,
        study_name: str,
        effectiveness_threshold: float = 77.0,  ***REMOVED*** FAA caution
        fatigue_threshold: float = 6.0,  ***REMOVED*** Self-report threshold (1-10 scale)
    ):
        """
    ...
    def calculate_accuracy(self) -> PredictionAccuracy:
        ...
        ***REMOVED*** Filter to points with actual reports
        with_actual = [
        ...
        ***REMOVED*** Calculate error metrics
    ...
    def _calculate_error_metrics(
        ...
        ***REMOVED*** Convert self-reported fatigue (1-10) to effectiveness scale (0-100)
        ***REMOVED*** 10 = highest fatigue -> 0% effectiveness
        ***REMOVED*** 1 = lowest fatigue -> 100% effectiveness

        squared_errors = []
        absolute_errors = []

        for dp in data_points:
            ***REMOVED*** Convert actual fatigue score to effectiveness
            ***REMOVED*** fatigue 1-10 -> effectiveness 100-0
    ...
    def _calculate_classification_metrics(
        ...
        for dp in data_points:
            ***REMOVED*** FRMS prediction: at-risk if below threshold
            predicted_at_risk = dp.predicted_effectiveness < self.effectiveness_threshold

            ***REMOVED*** Actual: fatigued if self-report above threshold
            ...
            else:  ***REMOVED*** not predicted_at_risk and actual_fatigued
    ...
    def _calculate_auc(
        ...
        ***REMOVED*** Sort by predicted effectiveness (descending = higher risk first)
        sorted_points = sorted(
            data_points,
            key=lambda dp: dp.predicted_effectiveness
        )

        ***REMOVED*** Calculate TPR and FPR at each threshold
        n_positive = sum(
        ...
        roc_points = [(0.0, 0.0)]  ***REMOVED*** (FPR, TPR)
        tp = 0
        fp = 0
        ...
        ***REMOVED*** Calculate AUC using trapezoidal rule
        auc = 0.0
    ...
    def analyze_by_subgroup(
        ...
        for dp in self.data_points:
            ***REMOVED*** For now, just group by risk level as example
        ...
        for group_name, group_points in groups.items():
            ***REMOVED*** Create temporary study for this group
    ...
    def generate_calibration_plot_data(self) -> dict:
        ...
        ***REMOVED*** Sort by predicted effectiveness
        sorted_points = sorted(
            with_actual,
            key=lambda dp: dp.predicted_effectiveness
        )

        ***REMOVED*** Create decile bins
        n = len(sorted_points)
        bin_size = n // 10 or 1
