...
class CircadianPhase(str, Enum):
    ...
    WOCL = "wocl"  ***REMOVED*** Window of Circadian Low (2:00-6:00 AM) - minimum alertness
    MORNING_RISE = "morning_rise"  ***REMOVED*** 6:00-9:00 AM - cortisol rising
    MORNING_PEAK = "morning_peak"  ***REMOVED*** 9:00-12:00 PM - peak alertness
    POST_LUNCH_DIP = "post_lunch_dip"  ***REMOVED*** 12:00-3:00 PM - alertness decline
    AFTERNOON_RISE = "afternoon_rise"  ***REMOVED*** 3:00-6:00 PM - secondary peak
    EVENING = "evening"  ***REMOVED*** 6:00-10:00 PM - gradual decline
    PRE_SLEEP = "pre_sleep"  ***REMOVED*** 10:00 PM-2:00 AM - melatonin rising
...
class SleepInertiaState(str, Enum):
    ...
    SEVERE = "severe"  ***REMOVED*** 0-5 minutes: 20-30% impairment
    MODERATE = "moderate"  ***REMOVED*** 5-15 minutes: 10-20% impairment
    MILD = "mild"  ***REMOVED*** 15-30 minutes: 5-10% impairment
    RESOLVED = "resolved"  ***REMOVED*** >30 minutes: no impairment
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "time_series": [
                ...
                for t, e in self.time_series[-48:]  ***REMOVED*** Last 48 data points
            ],
        }
...
class ThreeProcessModel:
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Model Constants (from SAFTE-FAST literature)
    ***REMOVED*** =========================================================================

    ***REMOVED*** Process S: Homeostatic constants
    TAU_WAKE = 18.2  ***REMOVED*** Time constant for sleep pressure rise (hours)
    TAU_SLEEP = 4.2  ***REMOVED*** Time constant for sleep pressure decay (hours)
    S_MIN = 0.0  ***REMOVED*** Minimum sleep reservoir level
    S_MAX = 100.0  ***REMOVED*** Maximum sleep reservoir level
    DEPLETION_RATE = 5.5  ***REMOVED*** Points lost per TAU_WAKE during waking
    RECOVERY_RATE = 20.0  ***REMOVED*** Points gained per TAU_SLEEP during sleep

    ***REMOVED*** Process C: Circadian constants
    CIRCADIAN_PERIOD = 24.0  ***REMOVED*** Period in hours
    CIRCADIAN_AMPLITUDE = 0.25  ***REMOVED*** Amplitude as fraction of mean (25% swing)
    CIRCADIAN_PHASE_SHIFT = 4.0  ***REMOVED*** Hours after midnight for minimum (4:00 AM)
    WOCL_START = 2.0  ***REMOVED*** Window of Circadian Low start (2:00 AM)
    WOCL_END = 6.0  ***REMOVED*** Window of Circadian Low end (6:00 AM)

    ***REMOVED*** Process W: Sleep inertia constants
    INERTIA_DURATION = 30.0  ***REMOVED*** Minutes for inertia to resolve
    INERTIA_MAX_PENALTY = 25.0  ***REMOVED*** Maximum penalty percentage
    INERTIA_DEBT_MULTIPLIER = 1.5  ***REMOVED*** Extra inertia with sleep debt

    ***REMOVED*** Effectiveness thresholds
    THRESHOLD_OPTIMAL = 95.0  ***REMOVED*** Fully rested optimal performance
    THRESHOLD_ACCEPTABLE = 85.0  ***REMOVED*** Acceptable performance
    THRESHOLD_FAA_CAUTION = 77.0  ***REMOVED*** FAA caution threshold
    THRESHOLD_FRA_HIGH_RISK = 70.0  ***REMOVED*** Federal Rail Administration threshold
    THRESHOLD_UNACCEPTABLE = 60.0  ***REMOVED*** Unacceptable for safety-critical tasks

    ***REMOVED*** Component weights for combined score
    WEIGHT_HOMEOSTATIC = 0.55  ***REMOVED*** 55% weight on sleep debt
    WEIGHT_CIRCADIAN = 0.35  ***REMOVED*** 35% weight on circadian rhythm
    WEIGHT_INERTIA = 0.10  ***REMOVED*** 10% weight on sleep inertia (when applicable)
    ...
    ***REMOVED*** =========================================================================
    ...
    def create_state(
        ...
        ***REMOVED*** Calculate initial effectiveness
    ...
    def update_wakefulness(
        ...
        ***REMOVED*** Process S: Sleep reservoir depletion during waking
        ***REMOVED*** Exponential decay toward minimum
        depletion = self.DEPLETION_RATE * (hours / self.TAU_WAKE)
        new_reservoir = max(self.S_MIN, state.sleep_reservoir - depletion)

        ***REMOVED*** Track cumulative debt if below optimal
        debt_threshold = 85.0
        if new_reservoir < debt_threshold:
            ...
            new_debt = max(0, state.cumulative_debt - hours * 0.5)  ***REMOVED*** Slow recovery

        ***REMOVED*** Update circadian phase
        time_of_day = new_timestamp.hour + new_timestamp.minute / 60.0
        new_phase = self._get_circadian_phase(time_of_day)

        ***REMOVED*** Create updated state
    ...
    def update_sleep(
        ...
        quality = max(0.5, min(1.0, quality))  ***REMOVED*** Clamp to valid range

        ***REMOVED*** Calculate effective sleep (adjusted for quality)
        effective_hours = hours * quality

        ***REMOVED*** Update timestamp to end of sleep
        new_timestamp = state.timestamp + timedelta(hours=hours)

        ***REMOVED*** Process S: Sleep reservoir recovery during sleep
        recovery = self.RECOVERY_RATE * (effective_hours / self.TAU_SLEEP)
        new_reservoir = min(self.S_MAX, state.sleep_reservoir + recovery)

        ***REMOVED*** Reduce cumulative debt (sleep pays off debt)
        debt_recovery = effective_hours * 2.0  ***REMOVED*** Each hour of sleep reduces 2 hours of debt
        ...
        ***REMOVED*** Create updated state
        new_state = AlertnessState(
            ...
            timestamp=new_timestamp,
            sleep_reservoir=new_reservoir,
            hours_awake=0.0,  ***REMOVED*** Reset on wake
        ...
        ***REMOVED*** Calculate new effectiveness (will include sleep inertia)
        new_state.effectiveness = self.calculate_effectiveness(new_state)
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Effectiveness Calculation
    ***REMOVED*** =========================================================================
    ...
    def calculate_effectiveness(
        ...
        ***REMOVED*** Process S: Homeostatic component (normalized 0-100)
        homeostatic = state.sleep_reservoir

        ***REMOVED*** Apply cumulative debt penalty
        if state.cumulative_debt > 0:
            debt_penalty = min(20, state.cumulative_debt * 0.5)
            homeostatic = max(0, homeostatic - debt_penalty)

        ***REMOVED*** Process C: Circadian component (normalized 0-100)
        circadian = self._circadian_component(time_of_day)

        ***REMOVED*** Process W: Sleep inertia penalty
        inertia_penalty = self._sleep_inertia_penalty(state)

        ***REMOVED*** Combined effectiveness (weighted sum minus inertia)
        combined = (
    ...
    def _circadian_component(self, time_of_day: float) -> float:
        ...
        """
        ***REMOVED*** Sinusoidal: minimum at 4:00 AM, maximum at 4:00 PM
        ***REMOVED*** C(t) = mean + amplitude * sin(2π(t - phase) / period)
        mean = 75.0
        ...
        ***REMOVED*** Shift by -π/2 so minimum is at phase_shift
    ...
    def _sleep_inertia_penalty(self, state: AlertnessState) -> float:
        ...
        ***REMOVED*** Linear decay of inertia over INERTIA_DURATION minutes
        inertia_fraction = 1.0 - (minutes_since_wake / self.INERTIA_DURATION)

        ***REMOVED*** Base penalty scaled by inertia fraction
        base_penalty = self.INERTIA_MAX_PENALTY * inertia_fraction

        ***REMOVED*** Additional penalty if sleep deprived (makes inertia worse)
        if state.cumulative_debt > 0:
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** WOCL and Time-of-Day Analysis
    ***REMOVED*** =========================================================================
    ...
    ***REMOVED*** =========================================================================
    ...
    def predict_shift_effectiveness(
        ...
        ***REMOVED*** Advance to shift start if needed
