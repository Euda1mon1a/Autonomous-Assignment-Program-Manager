...
class FatigueQUBOConfig:
    ...
    ***REMOVED*** Penalty weights
    fatigue_penalty_base: float = 100.0  ***REMOVED*** Base penalty for low effectiveness
    wocl_penalty_multiplier: float = 2.0  ***REMOVED*** Extra penalty for WOCL shifts
    consecutive_day_penalty: float = 50.0  ***REMOVED*** Per day over 5 consecutive
    night_shift_cluster_bonus: float = -20.0  ***REMOVED*** Negative = reward clustering

    ***REMOVED*** Thresholds
    effectiveness_threshold: float = 77.0  ***REMOVED*** FAA caution threshold
    critical_threshold: float = 70.0  ***REMOVED*** FRA threshold (higher penalty)
...
class FatigueQUBOIntegration:
    ...
    Usage:
        ...
        ***REMOVED*** Build base QUBO
        Q = formulation.build()

        ***REMOVED*** Add fatigue penalties
        Q = fatigue_integration.add_fatigue_penalties(Q, formulation, context)

        ***REMOVED*** Solve with fatigue-aware objective
    ...
    def __init__(self, config: FatigueQUBOConfig | None = None):
        ...
        ***REMOVED*** Cache for effectiveness predictions
    ...
    def add_fatigue_penalties(
        ...
        formulation: Any,  ***REMOVED*** QUBOFormulation
        context: Any,  ***REMOVED*** SchedulingContext
        ...
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Calculate penalty
                ...
                if penalty > 0:
                    ***REMOVED*** Add penalty to all template variables for this person-block
    ...
    def _add_consecutive_day_penalties(
        ...
        ***REMOVED*** Group blocks by date
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Look at pairs of consecutive days
            for i in range(len(sorted_dates) - 1):
                date1 = sorted_dates[i]
                date2 = sorted_dates[i + 1]

                ***REMOVED*** Only penalize if actually consecutive
                if (date2 - date1).days != 1:
                    continue

                ***REMOVED*** Get variable indices for blocks on these days
                ...
                ***REMOVED*** Add quadratic penalty between all pairs
                penalty = self.config.consecutive_day_penalty

                ***REMOVED*** Increase penalty for 6+ consecutive days
                if i >= 5:
                    penalty *= 2.0
    ...
    def _add_night_clustering_terms(
        ...
        ***REMOVED*** Identify PM (night) blocks
        night_blocks = [
        ...
        ***REMOVED*** Group by date
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Reward consecutive night shifts for same person
            for i in range(len(sorted_dates) - 1):
                date1 = sorted_dates[i]
                date2 = sorted_dates[i + 1]
                ...
                ***REMOVED*** Negative penalty = reward
    ...
    def _add_wocl_penalties(
        ...
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Check if block likely includes WOCL
                tod = getattr(block, "time_of_day", "AM")
                has_wocl = tod == "PM"  ***REMOVED*** Night blocks include WOCL
    ...
    def _add_recovery_penalties(
        ...
        ***REMOVED*** Minimum recovery hours between shifts
        MIN_RECOVERY_HOURS = 10.0

        ***REMOVED*** Group blocks by date and time
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Check pairs of blocks for recovery violations
            for i, block1 in enumerate(blocks_ordered):
                for block2 in blocks_ordered[i + 1 : i + 3]:  ***REMOVED*** Check next 2 blocks
                    ***REMOVED*** Calculate time gap
                    gap_hours = self._calculate_gap_hours(block1, block2)

                    if gap_hours < MIN_RECOVERY_HOURS:
                        ***REMOVED*** Add penalty for both assignments occurring
    ...
    def _predict_effectiveness(
        ...
        ***REMOVED*** Create temporary state and calculate
    ...
    def _calculate_fatigue_penalty(self, effectiveness: float) -> float:
        ...
        ***REMOVED*** Quadratic penalty below critical threshold
        gap = self.config.effectiveness_threshold - effectiveness
    ...
    def _calculate_gap_hours(self, block1: Any, block2: Any) -> float:
        ...
        ***REMOVED*** Assume 6-hour blocks
        ***REMOVED*** AM: 7:00-13:00, PM: 13:00-19:00
...
def create_fatigue_qubo_solver(
    ...
    class FatigueAwareQUBOSolver:
        ...
        def solve(self, context, existing_assignments=None):
            ...
            ***REMOVED*** Solve using base solver
