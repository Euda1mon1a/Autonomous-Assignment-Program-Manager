...
***REMOVED*** =============================================================================
***REMOVED*** Constraint Type Aliases (FRMS types now in base ConstraintType)
***REMOVED*** =============================================================================

***REMOVED*** FRMS constraint types are now defined in ConstraintType enum.
***REMOVED*** These aliases provide backward compatibility.
FRMSConstraintType = ConstraintType  ***REMOVED*** Alias for backward compatibility
...
class FatigueViolation:
    ...
    person_id: UUID
    block_id: UUID
    violation_type: str  ***REMOVED*** "effectiveness_below_threshold", "wocl_exposure", etc.
...
***REMOVED*** =============================================================================
***REMOVED*** Hard Fatigue Constraint
***REMOVED*** =============================================================================
...
class FatigueConstraint(HardConstraint):
    ...
    ***REMOVED*** FAA/FRA thresholds
    FAA_CAUTION_THRESHOLD = 77.0  ***REMOVED*** FAA caution level
    FRA_HIGH_RISK_THRESHOLD = 70.0  ***REMOVED*** FRA high-risk threshold (hard limit)
    CRITICAL_THRESHOLD = 60.0  ***REMOVED*** Never schedule below this
    ...
    def initialize_states(
        ...
        for resident in context.residents:
            self._alertness_states[resident.id] = self.model.create_state(
                ...
                initial_reservoir=100.0,  ***REMOVED*** Assume fully rested at start
    ...
    def predict_effectiveness(
        ...
        if not state:
            return 100.0  ***REMOVED*** Assume optimal if no state

        ***REMOVED*** Calculate time of day as float
        if time_of_day == "AM":
            tod = 8.0  ***REMOVED*** 8:00 AM
        else:
            tod = 14.0  ***REMOVED*** 2:00 PM

        ***REMOVED*** Update state for hours worked today
        ...
        ***REMOVED*** Calculate effectiveness
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Get assignment variables
        x = variables.get("x")  ***REMOVED*** x[r, b, t] binary assignment vars
        if x is None:
            logger.warning(
        ...
        ***REMOVED*** Initialize states if not done
        ...
        ***REMOVED*** For each potential assignment, check if it would violate fatigue limit
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Predict effectiveness for this assignment
                ...
                ***REMOVED*** If below threshold, block assignment completely
                if effectiveness < self.threshold:
                    ***REMOVED*** Block all template assignments for this person-block
                    for template in context.templates:
                        t_idx = context.template_idx.get(template.id)
                        if t_idx is not None and (r_idx, b_idx, t_idx) in x:
                            ***REMOVED*** Force variable to 0
                            model.Add(x[r_idx, b_idx, t_idx] == 0)
                            blocked_count += 1

                ***REMOVED*** If below critical threshold, log warning
    ...
    def add_to_pulp(
        ...
        ***REMOVED*** PuLP implementation follows same logic as CP-SAT
        ***REMOVED*** Force variables to 0 for assignments below threshold
        x = variables.get("x")
        if x is None:
            return
...
***REMOVED*** =============================================================================
***REMOVED*** Soft Fatigue Constraint (Optimization Objective)
***REMOVED*** =============================================================================
...
class FatigueSoftConstraint(SoftConstraint):
    ...
    def calculate_penalty(self, effectiveness: float) -> float:
        ...
        if effectiveness >= self.FRA_HIGH_RISK_THRESHOLD:
            ***REMOVED*** Linear penalty in caution zone
            gap = self.FAA_CAUTION_THRESHOLD - effectiveness
            return self.weight * (gap / 10.0)  ***REMOVED*** 0.7 weight per percent below 77

        ***REMOVED*** Quadratic penalty below FRA threshold
        gap = self.FAA_CAUTION_THRESHOLD - effectiveness
    ...
    def add_to_cpsat(
        ...
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Get or predict effectiveness
                ...
                if penalty > 0:
                    for template in context.templates:
                        t_idx = context.template_idx.get(template.id)
                        if t_idx is not None and (r_idx, b_idx, t_idx) in x:
                            ***REMOVED*** Add penalty term: penalty * x[r,b,t]
    ...
    def add_to_pulp(
        ...
        ***REMOVED*** PuLP implementation similar to CP-SAT
        ***REMOVED*** Add penalty coefficients to objective
        pass  ***REMOVED*** Implementation follows CP-SAT pattern
    ...
    def validate(
        ...
        return ConstraintResult(
            satisfied=True,  ***REMOVED*** Soft constraint is always "satisfied"
            violations=violations,
...
***REMOVED*** =============================================================================
***REMOVED*** Circadian Protection Constraint
***REMOVED*** =============================================================================
...
class CircadianConstraint(SoftConstraint):
    ...
    def calculate_circadian_penalty(
        ...
        ***REMOVED*** Check if shift overlaps WOCL
        ...
        if wocl_overlap:
            ...
            ***REMOVED*** Extra penalty for procedures during WOCL
            if has_procedure:
        ...
        ***REMOVED*** Shift duration penalty
    ...
    def _check_wocl_overlap(self, start_hour: float, duration: float) -> bool:
        ...
        ***REMOVED*** Handle overnight shifts
        if end_hour > 24:
            ***REMOVED*** Shift wraps past midnight
            return (start_hour < self.WOCL_END) or (end_hour % 24 > self.WOCL_START)

        ***REMOVED*** Normal shift
    ...
    def _calculate_wocl_hours(self, start_hour: float, duration: float) -> float:
        ...
        ***REMOVED*** Calculate overlap with WOCL window (2-6 AM)
        if end_hour > 24:
            ***REMOVED*** Overnight shift
            ***REMOVED*** First check 2-6 AM on next day
            wocl_start_next = max(
                0, min(self.WOCL_END, end_hour % 24) - self.WOCL_START
            )
            wocl_hours += max(0, wocl_start_next)

            ***REMOVED*** Check if shift started before WOCL ended
            if start_hour < self.WOCL_END:
                wocl_hours += min(self.WOCL_END, 24) - max(self.WOCL_START, start_hour)
        else:
            ***REMOVED*** Same-day shift
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Similar to FatigueSoftConstraint - add penalty terms to objective
        pass
    ...
    def validate(
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Get shift timing
            tod = getattr(block, "time_of_day", "AM")
            start_hour = 7.0 if tod == "AM" else 13.0
            duration = 6.0  ***REMOVED*** Half-day block

            ***REMOVED*** Check for procedures
            has_procedure = False
            procedure_type = ""
            if rotation:
            ...
            ***REMOVED*** Add violation for significant WOCL exposure
