...
class FatigueTimeSeries:
    ...
    ***REMOVED*** Time series data points
    data_points: list[dict] = field(default_factory=list)
    ***REMOVED*** Each point: {timestamp, effectiveness, risk_level, circadian_phase, ...}
    ...
    ***REMOVED*** Visualization parameters
    color_gradient: list[str] = field(default_factory=list)  ***REMOVED*** Colors for risk levels
...
class TemporalDynamicsData:
    ...
    ***REMOVED*** Circadian pattern (24 hourly bins)
    circadian_pattern: list[dict] = field(default_factory=list)
    ***REMOVED*** Each: {hour, avg_effectiveness, std_dev, sample_count}

    ***REMOVED*** Weekly pattern (7 daily bins)
    weekly_pattern: list[dict] = field(default_factory=list)
    ***REMOVED*** Each: {day_of_week, avg_effectiveness, std_dev}

    ***REMOVED*** Resonance detection (circadian-workload alignment)
    resonance_patterns: list[dict] = field(default_factory=list)

    ***REMOVED*** Aggregate time series
    aggregate_time_series: list[dict] = field(default_factory=list)

    ***REMOVED*** Heatmap data (hour x day matrix)
...
class SpatialFatigueMap:
    ...
    export_time: datetime
    grid_dimensions: dict = field(default_factory=dict)  ***REMOVED*** {x, y, z sizes}
    voxels: list[dict] = field(default_factory=list)

    ***REMOVED*** Axis labels
    x_labels: list[str] = field(default_factory=list)  ***REMOVED*** Dates/blocks
...
class HolographicExporter:
    ...
    Usage:
        ...
        ***REMOVED*** Export individual time series
        ts = exporter.export_time_series(person_id, assignments, hours=168)

        ***REMOVED*** Export temporal dynamics
        dynamics = exporter.export_temporal_dynamics(all_assignments, days=14)

        ***REMOVED*** Export spatial map
        spatial = exporter.export_spatial_map(assignments, persons, blocks)
    """

    ***REMOVED*** Color palette for effectiveness levels
    EFFECTIVENESS_COLORS = {
        "optimal": "***REMOVED***22c55e",  ***REMOVED*** green (95-100%)
        "acceptable": "***REMOVED***84cc16",  ***REMOVED*** lime (85-94%)
        "caution": "***REMOVED***eab308",  ***REMOVED*** yellow (77-84%)
        "high_risk": "***REMOVED***f97316",  ***REMOVED*** orange (70-76%)
        "critical": "***REMOVED***ef4444",  ***REMOVED*** red (<70%)
    }

    ***REMOVED*** Circadian phase colors
    PHASE_COLORS = {
        CircadianPhase.WOCL: "***REMOVED***7c3aed",  ***REMOVED*** violet
        CircadianPhase.MORNING_RISE: "***REMOVED***f59e0b",  ***REMOVED*** amber
        CircadianPhase.MORNING_PEAK: "***REMOVED***22c55e",  ***REMOVED*** green
        CircadianPhase.POST_LUNCH_DIP: "***REMOVED***3b82f6",  ***REMOVED*** blue
        CircadianPhase.AFTERNOON_RISE: "***REMOVED***10b981",  ***REMOVED*** teal
        CircadianPhase.EVENING: "***REMOVED***6366f1",  ***REMOVED*** indigo
        CircadianPhase.PRE_SLEEP: "***REMOVED***8b5cf6",  ***REMOVED*** purple
    }
    ...
    def export_time_series(
        ...
        hours: int = 168,  ***REMOVED*** 1 week default
        sample_interval_minutes: int = 30,
        pgy_level: int | None = None,
    ) -> FatigueTimeSeries:
        """
        ...
        while current_time <= now:
            ...
            ***REMOVED*** Create data point
            data_points.append({
                ...
                "position": {
                    "x": (current_time - start_time).total_seconds() / 3600.0,  ***REMOVED*** hours
                    "y": score.overall / 100.0,  ***REMOVED*** normalized
                    "z": 0,  ***REMOVED*** can be used for 3D trajectory
                },
            })
    ...
    def export_temporal_dynamics(
        ...
        ***REMOVED*** Collect all data points in analysis window
        hourly_bins: dict[int, list[float]] = {h: [] for h in range(24)}
        daily_bins: dict[int, list[float]] = {d: [] for d in range(7)}
        heatmap: dict[tuple[int, int], list[float]] = {}  ***REMOVED*** (hour, day) -> values
        ...
        ***REMOVED*** Calculate circadian pattern
        circadian_pattern = []
        ...
        ***REMOVED*** Calculate weekly pattern
        ...
        ***REMOVED*** Detect resonance patterns
    ...
    def export_spatial_map(
        ...
        ***REMOVED*** Sort persons by PGY level
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Get effectiveness
            if effectiveness_data:
                ...
                effectiveness = 85.0  ***REMOVED*** Default
    ...
    def export_predictive_waves(
        ...
        persons: list[dict],
        assignments: list[dict],
        forecast_hours: int = 168,  ***REMOVED*** 1 week
    ) -> dict:
        """
        ...
        for person in persons:
            ...
            ***REMOVED*** Get person's assignments
            person_assignments = [
            ...
            ***REMOVED*** Generate predictions
            ...
            while current_time <= now + timedelta(hours=forecast_hours):
                ...
                ***REMOVED*** Confidence increases with prediction distance
                hours_ahead = (current_time - now).total_seconds() / 3600.0
                confidence_width = 5.0 + hours_ahead * 0.5  ***REMOVED*** Wider bands further out
    ...
    def _detect_resonance_patterns(
        ...
        ***REMOVED*** Find WOCL hours (2-6 AM)
        ...
        ***REMOVED*** Find post-lunch dip
        ...
        if lunch_hours:
            ...
            ***REMOVED*** Compare to morning peak
    ...
    def _identify_risk_periods(
        ...
        ***REMOVED*** Close any open risk period
