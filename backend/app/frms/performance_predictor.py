...
class ClinicalRiskLevel(str, Enum):
    ...
    MINIMAL = "minimal"  ***REMOVED*** <5% error rate increase
    LOW = "low"  ***REMOVED*** 5-20% error rate increase
    MODERATE = "moderate"  ***REMOVED*** 20-50% error rate increase
    HIGH = "high"  ***REMOVED*** 50-100% error rate increase
    SEVERE = "severe"  ***REMOVED*** >100% error rate increase (2x+ baseline)
...
class PerformancePredictor:
    ...
    ***REMOVED*** Feature importance weights (learned from literature/domain expertise)
    FEATURE_WEIGHTS = {
    ...
    ***REMOVED*** Thresholds for risk classification
    DEGRADATION_THRESHOLDS = {
        "severe": 0.80,  ***REMOVED*** >80% probability = severe
        "high": 0.60,  ***REMOVED*** >60% = high
        "moderate": 0.40,  ***REMOVED*** >40% = moderate
        "low": 0.20,  ***REMOVED*** >20% = low
        "minimal": 0.0,  ***REMOVED*** <20% = minimal
    }

    ***REMOVED*** Error multiplier mapping
    ...
    def extract_features(
        ...
        ***REMOVED*** Sort assignments by date
        ...
        ***REMOVED*** Calculate time-based features
        ...
        ***REMOVED*** Rotation transitions
        features.rotation_transitions_14d = self._count_transitions(
            sorted_assignments, current_date, 14
        )

        ***REMOVED*** Sleep and circadian features
        if sleep_data:
        ...
        if fatigue_effectiveness is not None:
            ***REMOVED*** Map effectiveness to circadian alignment
    ...
    def predict(
        ...
        ***REMOVED*** Calculate weighted risk score
        risk_score = self._calculate_risk_score(features)

        ***REMOVED*** Apply sigmoid to get probability
        degradation_prob = self._sigmoid(risk_score)

        ***REMOVED*** Determine clinical risk level
        clinical_risk = self._determine_risk_level(degradation_prob)

        ***REMOVED*** Get error multiplier
        error_multiplier = self.ERROR_MULTIPLIER_MAP[clinical_risk]

        ***REMOVED*** Calculate contributing factors
        contributing_factors = self._calculate_factor_contributions(features)

        ***REMOVED*** Calculate confidence interval (simple estimate based on data completeness)
        ci_width = 0.15  ***REMOVED*** Base confidence interval width
        confidence_interval = (
    ...
    def _calculate_risk_score(self, features: ScheduleFeatures) -> float:
        ...
        ***REMOVED*** Hours since rest (exponential penalty after 16 hours)
        ...
        ***REMOVED*** Consecutive duty days (strong penalty after 6 days)
        ...
        ***REMOVED*** Sleep deficit (inverse relationship)
        sleep_deficit = max(0, 8 - features.average_sleep_hours)
        score += 0.07 * (sleep_deficit / 4)

        ***REMOVED*** Circadian misalignment (inverse of alignment score)
        ...
        ***REMOVED*** Current night shift bonus
    ...
    def _calculate_factor_contributions(
        ...
        ***REMOVED*** Normalize each factor's contribution
        total = 0.0
        raw_contributions = {
    ...
    def _estimate_recovery_time(
        ...
        base_recovery = 8.0  ***REMOVED*** Minimum full night's sleep

        ***REMOVED*** Additional recovery for each risk factor
        extra_recovery = 0.0
        ...
        ***REMOVED*** Scale by probability
        total_recovery = base_recovery + extra_recovery * probability

        return min(48.0, total_recovery)  ***REMOVED*** Cap at 48 hours
    ...
    def _calculate_optimal_break(
        ...
        if probability < 0.3:
            return None  ***REMOVED*** No break urgently needed

        ***REMOVED*** Suggest break during next circadian peak
        current_hour = current_time.hour

        ***REMOVED*** Morning peak: 9-11 AM, Afternoon peak: 3-5 PM
        if current_hour < 9:
            ***REMOVED*** Before morning peak - can push to 9 AM
            optimal_hour = 9
        elif current_hour < 15:
            ***REMOVED*** Between peaks - suggest afternoon break
            optimal_hour = 15
        else:
            ***REMOVED*** After afternoon peak - suggest next morning
            optimal_hour = 9
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Feature Extraction Helpers
    ***REMOVED*** =========================================================================
    ...
    def _count_hours_in_window(
        ...
        for a in assignments:
            ...
            if a_date and window_start <= a_date <= current_date:
                ***REMOVED*** Assume 6 hours per half-day block
    ...
    def _estimate_wocl_hours(
        ...
        ***REMOVED*** Approximate: night shifts include ~4 hours of WOCL
    ...
    def _count_weekend_days(
        ...
        for a in assignments:
            ...
            if isinstance(a_date, str):
                a_date = date.fromisoformat(a_date)
            if a_date and window_start <= a_date <= current_date:
                if a_date.weekday() >= 5:  ***REMOVED*** Saturday=5, Sunday=6
