...
***REMOVED*** Initialize structured logging
setup_logging(
    ...
    ***REMOVED*** Check WEBHOOK_SECRET
    ...
    if errors:
        ...
        if not settings.DEBUG:
            ***REMOVED*** Production mode: fail fast
            raise ValueError(
                f"{error_msg}\n\n"
...
async def lifespan(app: FastAPI):
    ...
    ***REMOVED*** Validate security configuration at startup
    _validate_security_config()

    ***REMOVED*** Initialize OpenTelemetry tracer if enabled
    ...
    ***REMOVED*** Initialize Prometheus instrumentation
    try:
    ...
    ***REMOVED*** Initialize resilience metrics
    try:
    ...
    ***REMOVED*** Initialize service cache
    try:
    ...
    ***REMOVED*** Log audit system status (without exposing sensitive config values)
    ...
    ***REMOVED*** Shutdown OpenTelemetry tracer if enabled
    if settings.TELEMETRY_ENABLED:
        try:
...
***REMOVED*** Attach rate limiter to app state for slowapi
app.state.limiter = limiter

***REMOVED*** Add rate limit exceeded exception handler
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    ***REMOVED*** Log the full error for debugging
    ***REMOVED*** Use repr(exc) to avoid loguru format string issues with curly braces in exception messages
    logger.error(
    ...
    ***REMOVED*** Return generic error to client
    if settings.DEBUG:
        ***REMOVED*** In debug mode, include more details for development
        return JSONResponse(
            status_code=500, content={"detail": str(exc), "type": type(exc).__name__}
        )
    else:
        ***REMOVED*** In production, hide implementation details
        return JSONResponse(
            status_code=500,
...
***REMOVED*** Security headers middleware - adds OWASP recommended security headers
app.add_middleware(SecurityHeadersMiddleware)
logger.info("Security headers middleware enabled")

***REMOVED*** Slowapi rate limiting middleware - applies global rate limits
...
***REMOVED*** Support both explicit origins list and regex pattern for flexible domain matching
cors_kwargs = {
...
***REMOVED*** Add explicit origins if configured
if settings.CORS_ORIGINS:
    cors_kwargs["allow_origins"] = settings.CORS_ORIGINS

***REMOVED*** Add regex pattern if configured (allows flexible domain matching)
if settings.CORS_ORIGINS_REGEX:
    cors_kwargs["allow_origin_regex"] = settings.CORS_ORIGINS_REGEX

app.add_middleware(CORSMiddleware, **cors_kwargs)

***REMOVED*** Trusted host middleware - prevents host header attacks (production only)
if settings.TRUSTED_HOSTS:
    app.add_middleware(
        TrustedHostMiddleware,
        allowed_hosts=settings.TRUSTED_HOSTS,
    )
    ***REMOVED*** Log count only, not actual values (security: avoid exposing infrastructure)
    logger.info(
        f"Trusted hosts middleware enabled. {len(settings.TRUSTED_HOSTS)} host(s) configured."
    )

***REMOVED*** Audit context middleware - captures user for version history tracking
app.add_middleware(AuditContextMiddleware)

***REMOVED*** Request ID middleware - adds X-Request-ID for distributed tracing
try:
...
***REMOVED*** Internal network IP ranges for metrics endpoint restriction
INTERNAL_NETWORKS = [
...
***REMOVED*** Backwards compatibility redirect - redirect /api/... to /api/v1/...
@app.middleware("http")
...
***REMOVED*** Metrics endpoint protection - restrict to internal IPs in production
@app.middleware("http")
...
***REMOVED*** Include GraphQL endpoint
try:
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "database": "connected",  ***REMOVED*** Would check actual DB connection in production
    }
