...
class QuotaTracker:
    ...
    def _get_ttl_daily(self) -> int:
        ...
        ttl_seconds = int((end_of_day - now).total_seconds()) + 3600  ***REMOVED*** +1 hour buffer
        return ttl_seconds
    ...
    def _get_ttl_monthly(self) -> int:
        ...
        """
        now = datetime.utcnow()
        ***REMOVED*** Get last day of current month
        if now.month == 12:
        ...
        ttl_seconds = int((end_of_month - now).total_seconds()) + 86400  ***REMOVED*** +1 day buffer
        return ttl_seconds
    ...
    def get_remaining(
        ...
        ***REMOVED*** Get limits based on resource type
        ...
        else:  ***REMOVED*** "api" or default
            daily_limit = policy.daily_limit
            monthly_limit = policy.monthly_limit

        ***REMOVED*** Calculate remaining with overage
    ...
    def get_reset_times(self) -> tuple[datetime, datetime]:
        ...
        ***REMOVED*** Daily reset: midnight UTC
        daily_reset = (now + timedelta(days=1)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )

        ***REMOVED*** Monthly reset: first day of next month at midnight UTC
        if now.month == 12:
    ...
    def should_alert(
        ...
        ***REMOVED*** Use the higher percentage
