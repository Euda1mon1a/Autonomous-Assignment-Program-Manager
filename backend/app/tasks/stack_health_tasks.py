...
def check_frontend_typecheck() -> dict[str, Any]:
    ...
    code, stdout, stderr = run_command(
        ["npm", "run", "type-check"], cwd=FRONTEND_DIR, timeout=120
    )
...
def check_frontend_lint() -> dict[str, Any]:
    ...
    code, stdout, stderr = run_command(
        ["npm", "run", "lint"], cwd=FRONTEND_DIR, timeout=120
    )
    ...
    summary = re.search(
        r"(\d+)\s+problems?\s+\((\d+)\s+errors?,\s+(\d+)\s+warnings?\)", output
    )
...
def check_backend_lint() -> dict[str, Any]:
    ...
    code, stdout, stderr = run_command(
        ["ruff", "check", "."], cwd=BACKEND_DIR, timeout=60
    )
...
def check_docker_health() -> dict[str, Any]:
    ...
    code, stdout, stderr = run_command(
        ["docker", "compose", "ps", "--format", "json"], cwd=PROJECT_ROOT, timeout=30
    )
