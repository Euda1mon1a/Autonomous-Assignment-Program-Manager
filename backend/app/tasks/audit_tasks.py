...
Usage:
-----
    ***REMOVED*** Trigger manual archival
    ...
    ***REMOVED*** Trigger compliance report
...
def archive_old_audit_logs(
    ...
    try:
        ...
        ***REMOVED*** Create archiver
        archiver = AuditArchiver(db)

        ***REMOVED*** Perform archival
        import asyncio
        ...
        ***REMOVED*** No logs to archive - this is normal, not an error
...
def cleanup_old_archives(
    ...
    try:
        ...
        ***REMOVED*** Calculate cutoff date
        cutoff_date = datetime.utcnow() - timedelta(days=retention_years * 365)

        ***REMOVED*** Get storage backend
        storage = get_storage_backend()

        ***REMOVED*** List archives
        all_archives = storage.list_archives()

        ***REMOVED*** Filter archives older than cutoff
        old_archives = [
            a
...
def verify_archive_integrity(
    ...
    try:
        ...
        ***REMOVED*** Get archives to verify
        if archive_id:
        ...
        for archive_meta in archives:
            ...
            try:
                ***REMOVED*** Try to load archive
                archive_data = storage.get_archive(aid)

                ***REMOVED*** Verify required fields
...
***REMOVED*** Celery beat schedule for audit tasks
AUDIT_TASKS_BEAT_SCHEDULE = {
    ***REMOVED*** Daily archival at 2 AM
    "audit-archive-daily": {
        "task": "app.tasks.audit_tasks.archive_old_audit_logs",
        "schedule": {"hour": 2, "minute": 0},
        "kwargs": {
            "days": None,  ***REMOVED*** Uses config
    ...
    ***REMOVED*** Weekly compliance report on Monday at 6 AM
    "audit-compliance-report-weekly": {
        "task": "app.tasks.audit_tasks.generate_audit_compliance_report",
        "schedule": {"hour": 6, "minute": 0, "day_of_week": 1},  ***REMOVED*** Monday
        "kwargs": {
            "months_back": 3,
        },
        "options": {
    ...
    ***REMOVED*** Monthly archive cleanup on 1st of month at 3 AM
    ...
    ***REMOVED*** Weekly archive integrity check on Sunday at 4 AM
    "audit-integrity-check-weekly": {
        "task": "app.tasks.audit_tasks.verify_archive_integrity",
        "schedule": {"hour": 4, "minute": 0, "day_of_week": 0},  ***REMOVED*** Sunday
        "options": {
            "queue": "audit",
        },
    },
