...
@shared_task(
    ...
    max_retries=3,
    default_retry_delay=600,  ***REMOVED*** 10 minutes
)
def create_full_backup(
    ...
    ***REMOVED*** Check if backups are enabled
    ...
    try:
        ***REMOVED*** Create backup service
        backup_service = BackupService(
            compression_enabled=compression_enabled,
        )

        ***REMOVED*** Create full backup (async operation wrapped in sync task)
        loop = asyncio.get_event_loop()
        backup_id = loop.run_until_complete(
            backup_service.create_backup(db, strategy="full")
        )

        ***REMOVED*** Get backup info
...
def create_incremental_backup(
    ...
    ***REMOVED*** Check if backups are enabled
    ...
    try:
        ...
        ***REMOVED*** Check if a base backup exists
        ...
        if not backups:
            ...
            ***REMOVED*** Delegate to full backup task
            return create_full_backup.apply().get()

        ***REMOVED*** Create incremental backup
        loop = asyncio.get_event_loop()
        backup_id = loop.run_until_complete(
        ...
        ***REMOVED*** No base backup - this is expected, create a full backup instead
...
def cleanup_old_backups(
    ...
    try:
        ***REMOVED*** Get retention period from config if not specified
    ...
    try:
        ...
        ***REMOVED*** Get all backups
        ...
        return {
            ...
            "errors": errors[:10],  ***REMOVED*** Limit to first 10 errors
            "completed_at": datetime.utcnow().isoformat(),
        }
...
def generate_backup_report(self) -> dict[str, Any]:
    ...
    try:
        ...
        ***REMOVED*** Get backup statistics
        stats = backup_service.get_backup_statistics()

        ***REMOVED*** Add task metadata
...
def verify_backup_chain(
    ...
    try:
        ***REMOVED*** Create restore service
        restore_service = RestoreService()

        ***REMOVED*** Validate backup chain
...
***REMOVED*** Celery beat schedule for backup tasks
BACKUP_TASKS_BEAT_SCHEDULE = {
    ***REMOVED*** Daily full backup at 1 AM
    "backup-full-daily": {
        ...
        "options": {
            "queue": "backup",
            "expires": 7200,  ***REMOVED*** 2 hours
        },
    },
    ***REMOVED*** Hourly incremental backup during business hours (8 AM - 6 PM)
    "backup-incremental-hourly": {
        ...
        "options": {
            "queue": "backup",
            "expires": 3600,  ***REMOVED*** 1 hour
        },
    },
    ***REMOVED*** Weekly backup verification on Sunday at 5 AM
    "backup-verify-weekly": {
        "task": "app.tasks.backup_tasks.verify_all_backups",
        "schedule": {"hour": 5, "minute": 0, "day_of_week": 0},  ***REMOVED*** Sunday
        "kwargs": {
            "limit": 100,
        },
        "options": {
    ...
    ***REMOVED*** Weekly backup report on Monday at 7 AM
    "backup-report-weekly": {
        "task": "app.tasks.backup_tasks.generate_backup_report",
        "schedule": {"hour": 7, "minute": 0, "day_of_week": 1},  ***REMOVED*** Monday
        "options": {
            "queue": "backup",
        },
    },
