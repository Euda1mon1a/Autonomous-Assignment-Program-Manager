...
def compute_schedule_metrics(
    ...
    try:
        ...
        ***REMOVED*** Compute metrics using AnalyticsEngine
        ...
        ***REMOVED*** Compute stability metrics
...
def compute_version_diff(
    ...
    try:
        ...
        ***REMOVED*** Use existing comparison method
        comparison = engine.compare_schedules(run_id_1, run_id_2)

        ***REMOVED*** Compute metrics for each run to get fairness/coverage changes
        ...
        ***REMOVED*** Calculate metric deltas
        fairness_change = (
...
def snapshot_metrics(
    ...
    try:
        ...
        ***REMOVED*** Compute current metrics
        engine = AnalyticsEngine(db)
        schedule_analysis = engine.analyze_schedule(start_date, end_date)
        stability = compute_stability_metrics(db, start_date, end_date)

        ***REMOVED*** Create snapshot record (stored as JSON in existing tables)
        ***REMOVED*** Note: In production, you might want a dedicated MetricsSnapshot table
        snapshot_data = {
            "timestamp": datetime.utcnow().isoformat(),
            "period": {
        ...
        ***REMOVED*** Log snapshot for now (in production, save to dedicated table)
        logger.info(
...
def cleanup_old_snapshots(
    ...
    try:
        ...
        ***REMOVED*** Query old schedule runs
        old_runs = db.query(ScheduleRun).filter(
            ScheduleRun.created_at < cutoff_date
        ).all()

        ***REMOVED*** Count by status before deletion
        status_counts = {}
        for run in old_runs:
            status = run.status
            status_counts[status] = status_counts.get(status, 0) + 1

        ***REMOVED*** Delete old runs
...
def generate_fairness_trend_report(
    ...
    try:
        ...
        ***REMOVED*** Collect weekly fairness metrics
        weekly_metrics = []
        ...
        ***REMOVED*** Simple trend detection (compare first half vs second half)
        ...
        ***REMOVED*** Identify anomalies (fairness < 0.75)
        anomalies = [
