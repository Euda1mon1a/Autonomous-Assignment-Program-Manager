...
def save_report_to_file(report_bytes: bytes, filename: str, report_type: str) -> str:
    ...
    """
    try:
        ***REMOVED*** Get reports directory from environment or use default
        reports_dir = os.getenv("REPORTS_DIR", "/tmp/reports")

        ***REMOVED*** Create subdirectory for report type
...
def get_program_directors_emails(db) -> list[str]:
    ...
    """
    try:
        ***REMOVED*** Query for faculty with Program Director role
...
def get_compliance_stakeholder_emails() -> list[str]:
    ...
    """
    ***REMOVED*** Get from environment or use default
...
@shared_task(
    ...
    bind=True,
    max_retries=3,
    default_retry_delay=300,  ***REMOVED*** 5 minutes
)
def generate_daily_compliance_summary(self, lookback_days: int = 1) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            ***REMOVED*** Extract summary metrics
            summary = {
            ...
            ***REMOVED*** Send email notification if violations found
            if summary["total_violations"] > 0:
                logger.warning(
                    f"Found {summary['total_violations']} violations in daily summary"
                )

                ***REMOVED*** Get stakeholder emails
                stakeholder_emails = get_compliance_stakeholder_emails()

                ***REMOVED*** Send email to each stakeholder
...
def generate_weekly_compliance_report(
    ...
    try:
        db = SessionLocal()
        try:
            ***REMOVED*** Previous week (Monday to Sunday)
            today = date.today()
            ...
            ***REMOVED*** Save report to file system
            try:
            ...
            ***REMOVED*** Send email to stakeholders with attachment
            stakeholder_emails = get_compliance_stakeholder_emails()
            for email in stakeholder_emails:
                try:
                    ***REMOVED*** Note: Celery send_email task doesn't support attachments yet
                    ***REMOVED*** For now, send link to report location or inline summary
                    send_email.delay(
                        to=email,
...
def generate_monthly_executive_summary(self) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            ***REMOVED*** Save reports to file system
            pdf_filepath = None
            excel_filepath = None
            ...
            ***REMOVED*** Send email to executive stakeholders
...
def generate_custom_compliance_report(
    ...
    try:
        ...
        try:
            ...
            ***REMOVED*** Convert resident_ids to UUIDs if provided
            from uuid import UUID
            ...
            ***REMOVED*** Save report to file system and return file path for download
            filepath = None
            try:
                filepath = save_report_to_file(
                    report_bytes=report_bytes,
...
def check_violation_alerts(self, lookback_days: int = 1) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            if violations:
                ***REMOVED*** Critical violations need immediate attention
                critical_violations = [
...
""",
                ...
                ***REMOVED*** Create in-app notifications for program directors
                from app.models.notification import Notification

                ***REMOVED*** Query program directors for in-app notifications
                stmt = select(Person).where(
                    Person.type == "faculty",
                    Person.faculty_role == FacultyRole.PD.value
                )
                        ...
                        ***REMOVED*** Create notification record
                        notification = Notification(
                            ...
                            data={
                                ...
                                "start_date": str(start_date),
                                "end_date": str(end_date),
                                "violations": violations[:5]  ***REMOVED*** Include first 5 for quick view
                            }
                        )
                        db.add(notification)
                ...
                ***REMOVED*** Commit all notifications
                ...
                return {
                    ...
                    "violations": violations[:10],  ***REMOVED*** Include first 10
                }
            else:
