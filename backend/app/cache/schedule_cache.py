...
class ScheduleCache:
    ...
    def __init__(self):
        ...
        self.default_ttl = 3600  ***REMOVED*** 1 hour
    ...
    async def set_rotation_coverage(
        ...
        """
        key = f"rotation_coverage:{rotation_id}:{target_date}"
        await self.cache.set(key, json.dumps(coverage_data), ttl or 1800)  ***REMOVED*** 30 min
    ...
    async def invalidate_schedule(
        ...
        if start_date and end_date:
            ***REMOVED*** Invalidate specific date range
            pattern = f"schedule:*:{start_date}:*:{end_date}:*"
        else:
            ***REMOVED*** Invalidate all schedules
            pattern = "schedule:*"
    ...
    async def warm_person_schedule(
        ...
        ***REMOVED*** Fetch and cache schedule
    ...
    def _generate_schedule_key(
        ...
        Returns:
            Cache key
        """
        ***REMOVED*** Create deterministic key from parameters
