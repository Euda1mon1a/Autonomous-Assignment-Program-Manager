...
Example:
    ...
    ***REMOVED*** Tag-based invalidation
    await service.invalidate_by_tag("user:123")

    ***REMOVED*** Pattern-based invalidation
    await service.invalidate_by_pattern("schedule:*:assignments")

    ***REMOVED*** Dependency-aware invalidation
...
class CacheInvalidationService:
    ...
    Example:
        ...
        ***REMOVED*** Register dependencies
        await service.register_dependency(
            "schedule:main",
            "schedule:user:123"
        )

        ***REMOVED*** Invalidate with dependencies
        await service.invalidate_with_dependencies("schedule:main")

        ***REMOVED*** Subscribe to invalidation events
    ...
    def __init__(
        ...
        ***REMOVED*** Event history (last 1000 events)
    ...
    ***REMOVED*** Tag-based invalidation
    ...
    async def invalidate_by_tag(
        ...
        Example:
            ***REMOVED*** Invalidate all cache entries for user 123
        ...
        try:
            ...
            ***REMOVED*** Get all keys associated with this tag
            ...
            ***REMOVED*** Record metrics
            if self.enable_metrics:
                ...
                self._record_invalidation(
                    InvalidationEventType.TAG_INVALIDATION,
                    total_count,
                    duration_ms
                )
    ...
    async def invalidate_by_tags(
        ...
        Example:
            ***REMOVED*** Invalidate multiple tags
        ...
        ***REMOVED*** Publish single event for batch operation
        if publish_event and self.enable_event_publishing:
            event = InvalidationEvent(
    ...
    ***REMOVED*** Pattern-based invalidation
    ...
    async def invalidate_by_pattern(
        ...
        Example:
            ***REMOVED*** Invalidate all schedule-related cache entries
        ...
        try:
            ...
            while True:
                ...
                if keys:
                    ...
                    ***REMOVED*** Track keys for dependency invalidation
                    if cascade:
            ...
            ***REMOVED*** Record metrics
            if self.enable_metrics:
                duration_ms = (time.time() - start_time) * 1000
                self._record_invalidation(
                    InvalidationEventType.PATTERN_INVALIDATION,
                    total_count,
                    duration_ms
                )
            ...
            ***REMOVED*** Publish event
            ...
            logger.error(
                f"Error invalidating pattern '{pattern}': {e}",
                exc_info=True
            )
            return 0
    ...
    ***REMOVED*** Dependency-aware invalidation
    ...
    def register_dependency(self, key: str, dependent_key: str) -> None:
        ...
        Example:
            ***REMOVED*** When schedule:main is invalidated, also invalidate schedule:user:123
    ...
    def clear_dependencies(self, key: str) -> None:
        ...
        """
        with self._dependency_lock:
            ***REMOVED*** Remove from dependencies
            dependent_keys = self._dependencies.pop(key, set())

            ***REMOVED*** Remove from reverse dependencies
            for dependent_key in dependent_keys:
                self._reverse_dependencies[dependent_key].discard(key)
    ...
    async def invalidate_with_dependencies(
        ...
        Example:
            ***REMOVED*** Invalidate schedule:main and all dependent entries
        ...
        try:
            ...
            ***REMOVED*** Track invalidated keys to avoid cycles
            invalidated = set()
            to_invalidate = [key]
            depth = 0
            ...
            ***REMOVED*** Record metrics
            if self.enable_metrics:
                duration_ms = (time.time() - start_time) * 1000
                self._record_invalidation(
                    InvalidationEventType.DEPENDENCY_INVALIDATION,
                    count,
                    duration_ms
                )
            ...
            ***REMOVED*** Publish event
            ...
            logger.warning(f"Redis connection error during dependency invalidation: {e}")
            return 0
        except Exception as e:
            logger.error(
                f"Error invalidating key with dependencies '{key}': {e}",
                exc_info=True
            )
            return 0
    ...
    async def _invalidate_dependencies(
        ...
        try:
            ...
            if count > 0:
                logger.debug(
                    f"Invalidated {count} dependencies for key '{key}'"
                )
        ...
        try:
            ...
            ***REMOVED*** Process in batches
            for i in range(0, len(keys), batch_size):
                ...
                ***REMOVED*** Cascade invalidation if enabled
                if cascade:
                    for key in batch:
            ...
            ***REMOVED*** Record metrics
            if self.enable_metrics:
                duration_ms = (time.time() - start_time) * 1000
                self._record_invalidation(
                    InvalidationEventType.BATCH_INVALIDATION,
                    total_count,
                    duration_ms
                )
    ...
    ***REMOVED*** Cascading invalidation
    ...
    async def cascade_invalidate(
        ...
        Example:
            ***REMOVED*** Invalidate schedule and all related cached views
        ...
        try:
            ...
            ***REMOVED*** Record metrics
            if self.enable_metrics:
                duration_ms = (time.time() - start_time) * 1000
                self._record_invalidation(
                    InvalidationEventType.CASCADING_INVALIDATION,
                    count,
                    duration_ms
                )
