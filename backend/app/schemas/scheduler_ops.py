...
class FixItMode(str, Enum):
    ...
    GREEDY = "greedy"  ***REMOVED*** Aggressive fixes, may impact quality
    CONSERVATIVE = "conservative"  ***REMOVED*** Careful fixes, prioritize stability
    BALANCED = "balanced"  ***REMOVED*** Balance between speed and quality
...
***REMOVED*** Situation Report Schemas
...
class CoverageMetrics(BaseModel):
    ...
    coverage_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Overall coverage rate"
    )
    blocks_covered: int = Field(
        ..., ge=0, description="Number of blocks with assignments"
    )
    blocks_total: int = Field(..., ge=0, description="Total number of blocks")
    critical_gaps: int = Field(
        default=0, ge=0, description="Number of critical coverage gaps"
    )
    faculty_utilization: float = Field(
        ..., ge=0.0, le=1.0, description="Faculty utilization rate"
    )
...
class SitrepResponse(BaseModel):
    ...
    ***REMOVED*** Recent activity
    recent_tasks: list[RecentTaskInfo] = Field(
        default_factory=list, description="Recent tasks"
    )

    ***REMOVED*** Coverage metrics
    coverage_metrics: CoverageMetrics | None = Field(
        None, description="Schedule coverage metrics"
    )

    ***REMOVED*** Alerts and recommendations
    immediate_actions: list[str] = Field(
        default_factory=list, description="Required immediate actions"
    )
    watch_items: list[str] = Field(default_factory=list, description="Items to monitor")

    ***REMOVED*** System metadata
...
***REMOVED*** Fix-It Mode Schemas


class FixItRequest(BaseModel):
    """Request to initiate fix-it mode."""

    mode: FixItMode = Field(
        default=FixItMode.BALANCED, description="Fix-it execution mode"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum retry attempts per task"
    )
    auto_approve: bool = Field(
        default=False, description="Automatically approve changes"
    )
    initiated_by: str = Field(
        ..., min_length=1, max_length=100, description="User who initiated fix-it"
    )
    target_task_ids: list[str] | None = Field(
        None, description="Specific tasks to fix (None = all failed)"
    )
    dry_run: bool = Field(
...
class FixItResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Execution status: initiated, running, completed, failed"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    mode: FixItMode

    ***REMOVED*** Metrics
    tasks_fixed: int = Field(
        default=0, ge=0, description="Number of tasks successfully fixed"
    )
    ...
    tasks_failed: int = Field(
        default=0, ge=0, description="Number of tasks that failed to fix"
    )

    ***REMOVED*** Details
    affected_tasks: list[AffectedTask] = Field(
        default_factory=list, description="Tasks affected by fix-it"
    )
    ...
    ***REMOVED*** Messages
    ...
    warnings: list[str] = Field(
        default_factory=list, description="Warnings generated during execution"
    )


***REMOVED*** Approval Schemas
...
class ApprovalRequest(BaseModel):
    ...
    task_id: str | None = Field(
        None, description="Specific task ID (None = all pending for token)"
    )
    action: ApprovalAction = Field(
        default=ApprovalAction.APPROVE, description="Approve or deny"
    )
    approved_by: str = Field(
        ..., min_length=1, max_length=100, description="User who approved/denied"
    )
    approved_by_id: str | None = Field(None, description="User ID who approved/denied")
    notes: str | None = Field(
        None, max_length=500, description="Optional approval notes"
    )
...
class ApprovalResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Status: approved, denied, invalid_token, not_found"
    )
    action: ApprovalAction

    ***REMOVED*** Task information
    task_id: str | None = Field(
        None, description="Task ID (or 'multiple' if batch approval)"
    )
    ...
    ***REMOVED*** Details
    task_details: list[ApprovedTaskInfo] = Field(
        default_factory=list, description="Details of approved/denied tasks"
    )
...
***REMOVED*** Solver Control Schemas (Kill-Switch)
