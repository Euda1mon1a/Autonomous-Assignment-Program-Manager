...
class ScheduleReportRequest(ReportRequest):
    ...
    person_ids: list[UUID] | None = Field(
        default=None, description="Filter by specific people (residents/faculty)"
    )
    rotation_template_ids: list[UUID] | None = Field(
        default=None, description="Filter by specific rotation templates"
    )
    include_details: bool = Field(
        default=True, description="Include detailed assignment information"
    )
...
class ComplianceReportRequest(ReportRequest):
    ...
    resident_ids: list[UUID] | None = Field(
        default=None, description="Filter by specific residents"
    )
    pgy_levels: list[int] | None = Field(
        default=None, description="Filter by PGY level (1, 2, 3)"
    )
    include_violations_only: bool = Field(
        default=False, description="Only include residents with violations"
    )
...
class AnalyticsReportRequest(ReportRequest):
    ...
    include_fairness_metrics: bool = Field(
        default=True, description="Include fairness and equity metrics"
    )
...
class FacultySummaryReportRequest(ReportRequest):
    ...
    faculty_ids: list[UUID] | None = Field(
        default=None, description="Filter by specific faculty members"
    )
    include_workload: bool = Field(
        default=True, description="Include workload statistics"
    )
    include_supervision: bool = Field(
        default=True, description="Include supervision metrics"
    )
...
class DetailedComplianceReportRequest(BaseModel):
    ...
    pgy_levels: list[int] | None = Field(
        default=None, description="Filter by PGY levels (e.g., [1, 2, 3])"
    )
    include_violations_only: bool = Field(
        default=False, description="Only include residents with violations"
    )
    include_charts: bool = Field(
        default=True, description="Include trend charts in PDF export"
    )
    include_details: bool = Field(
        default=True, description="Include detailed resident summaries"
    )
...
class ScheduledReportConfig(BaseModel):
    ...
    report_type: str = Field(
        default="compliance", description="Type of report to generate"
    )
    lookback_days: int = Field(
        default=30, description="Number of days to include in report (from today)"
    )
    format: str = Field(default="pdf", description="Export format: 'pdf' or 'excel'")
    recipients: list[str] = Field(
        default_factory=list, description="Email addresses to send report to"
    )
    pgy_levels: list[int] | None = Field(
        default=None, description="Filter by PGY levels"
    )
    include_violations_only: bool = Field(
        default=False, description="Only include residents with violations"
    )
