...
class MatchPriority(str, Enum):
    ...
    CRITICAL = "critical"  ***REMOVED*** Blocked week, urgent need
    HIGH = "high"  ***REMOVED*** Strong preference alignment
    MEDIUM = "medium"  ***REMOVED*** Good compatibility
    LOW = "low"  ***REMOVED*** Acceptable match


class MatchType(str, Enum):
    """Type of swap match."""
    MUTUAL = "mutual"  ***REMOVED*** Both parties want each other's weeks
    ONE_WAY = "one_way"  ***REMOVED*** One party wants, other is available
    ABSORB = "absorb"  ***REMOVED*** One party takes on a week
...
class ScoringBreakdown(BaseModel):
    ...
    date_proximity_score: float = Field(ge=0.0, le=1.0, description="Score based on date closeness")
    preference_alignment_score: float = Field(ge=0.0, le=1.0, description="Score based on preference matching")
    workload_balance_score: float = Field(ge=0.0, le=1.0, description="Score based on workload fairness")
    history_score: float = Field(ge=0.0, le=1.0, description="Score based on past swap patterns")
    availability_score: float = Field(ge=0.0, le=1.0, description="Score based on faculty availability")
    blocking_penalty: float = Field(ge=0.0, le=1.0, description="Penalty if weeks are blocked (1.0 = no penalty)")
    total_score: float = Field(ge=0.0, le=1.0, description="Overall compatibility score")
...
class RankedMatch(BaseModel):
    ...
    compatibility_score: float = Field(ge=0.0, le=1.0, description="Overall compatibility score (0-1)")
    ...
    explanation: str = Field(description="Human-readable explanation of why this is a good match")
    estimated_acceptance_probability: float = Field(
        ge=0.0, le=1.0,
        description="Estimated probability both parties will accept"
    )
...
class AutoMatchResult(BaseModel):
    ...
    target_week: date | None = Field(None, description="Week they want to receive (if specified)")
    matches_found: int = Field(ge=0, description="Number of compatible matches found")
    best_match: RankedMatch | None = Field(None, description="The best match if any found")
    all_matches: list[RankedMatch] = Field(default_factory=list, description="All compatible matches")
...
class BatchAutoMatchResult(BaseModel):
    ...
    total_requests_processed: int = Field(ge=0, description="Number of requests processed")
    ...
    successful_matches: list[AutoMatchResult] = Field(
        ...
        description="Requests with successful matches"
    )
    no_matches: list[UUID] = Field(
        default_factory=list,
        description="Request IDs that had no matches"
    )
    high_priority_matches: list[AutoMatchResult] = Field(
        default_factory=list,
        description="High-priority matches requiring attention"
    )
    execution_time_seconds: float = Field(ge=0.0, description="Time taken to complete matching")
...
class MatchingCriteria(BaseModel):
    ...
    ***REMOVED*** Weight factors (must sum close to 1.0)
    date_proximity_weight: float = Field(
        default=0.20,
        ge=0.0,
        le=1.0,
        description="Weight for date proximity factor"
    )
    ...
    workload_balance_weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Weight for workload balance factor"
    )
    history_weight: float = Field(
        default=0.15,
        ge=0.0,
        le=1.0,
        description="Weight for past swap history factor"
    )
    ...
    ***REMOVED*** Constraints
    ...
    require_mutual_availability: bool = Field(
        default=True,
        description="Whether both parties must be available for the swap"
    )
    exclude_blocked_weeks: bool = Field(
        default=True,
        description="Whether to exclude matches involving blocked weeks"
    )
    ...
    ***REMOVED*** Historical analysis
    ...
    @field_validator('preference_alignment_weight', 'workload_balance_weight',
                     'history_weight', 'availability_weight', 'date_proximity_weight')
    @classmethod
...
class MatchingSuggestion(BaseModel):
    ...
    benefit_score: float = Field(ge=0.0, le=1.0, description="How beneficial this swap would be")
...
class MatchingAnalytics(BaseModel):
    ...
    average_match_score: float = Field(ge=0.0, le=1.0, description="Average compatibility score")
    average_time_to_match_days: float = Field(ge=0.0, description="Average days to find a match")
    ...
    common_blocking_reasons: list[str] = Field(
        default_factory=list,
        description="Most common reasons matches fail"
    )
