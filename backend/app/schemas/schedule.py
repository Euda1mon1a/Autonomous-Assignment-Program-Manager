...
class SchedulingAlgorithm(str, Enum):
    ...
    GREEDY = "greedy"  ***REMOVED*** Fast heuristic, good for initial solutions
    CP_SAT = "cp_sat"  ***REMOVED*** OR-Tools constraint programming, optimal solutions
    PULP = "pulp"  ***REMOVED*** PuLP linear programming, fast for large problems
    HYBRID = "hybrid"  ***REMOVED*** Combines CP-SAT and PuLP for best results
...
class ScheduleRequest(BaseModel):
    ...
    start_date: date
    end_date: date
    pgy_levels: list[int] | None = None  ***REMOVED*** Filter residents by PGY level
    rotation_template_ids: list[UUID] | None = None  ***REMOVED*** Specific templates to use
...
class Violation(BaseModel):
    ...
    type: str  ***REMOVED*** 'SUPERVISION_RATIO', '80_HOUR', '1_IN_7', etc.
...
class ValidationResult(BaseModel):
    ...
    coverage_rate: float  ***REMOVED*** Percentage of blocks covered
...
class SolverStatistics(BaseModel):
    ...
    branches: int | None = None  ***REMOVED*** CP-SAT specific
    conflicts: int | None = None  ***REMOVED*** CP-SAT specific
...
class ScheduleResponse(BaseModel):
    ...
    acgme_override_count: int = 0  ***REMOVED*** Number of acknowledged ACGME overrides
...
***REMOVED*** Import/Conflict Analysis Schemas
...
class ConflictItem(BaseModel):
    ...
    date: str
    time: str
    type: str  ***REMOVED*** 'double_book', 'specialty_unavailable', 'consecutive_weeks'
    severity: str  ***REMOVED*** 'error', 'warning', 'info'
    message: str
...
class ScheduleSummary(BaseModel):
    ...
    providers: list[str]
    date_range: list[str | None]  ***REMOVED*** [start, end] as ISO strings
    total_slots: int
    fmit_slots: int = 0
    clinic_slots: int = 0
...
class Recommendation(BaseModel):
    ...
    type: str  ***REMOVED*** 'consolidate_fmit', 'specialty_coverage', 'resolve_double_booking'
...
***REMOVED*** SwapFinder API Schemas
...
class SwapCandidateResponse(BaseModel):
    ...
    faculty: str
    can_take_week: str  ***REMOVED*** ISO date
    gives_week: str | None = None  ***REMOVED*** ISO date for 1:1 swap
    back_to_back_ok: bool
    external_conflict: str | None = None
    flexibility: str  ***REMOVED*** "easy", "hard", "very_hard", "impossible"
    reason: str = ""
    rank: int = 0  ***REMOVED*** 1 = best candidate
...
class AlternatingPatternInfo(BaseModel):
    ...
    faculty: str
    cycle_count: int
    fmit_weeks: list[str]  ***REMOVED*** ISO dates
    recommendation: str
...
class SwapFinderResponse(BaseModel):
    ...
    viable_candidates: int  ***REMOVED*** Candidates with no back-to-back issues
    alternating_patterns: list[AlternatingPatternInfo] = []
    message: str = ""


***REMOVED*** JSON-based swap candidate schemas for MCP integration
class SwapCandidateJsonRequest(BaseModel):
    """Request schema for JSON-based swap candidate lookup (no file upload)."""

    person_id: str
    assignment_id: str | None = None  ***REMOVED*** Optional: specific assignment
    block_id: str | None = None  ***REMOVED*** Optional: specific block
...
class SwapCandidateJsonItem(BaseModel):
    ...
    candidate_name: str
    candidate_role: str  ***REMOVED*** Role-based identifier (e.g., "Faculty") - no PII
    assignment_id: str | None = None
    block_date: str  ***REMOVED*** ISO date
    block_session: str  ***REMOVED*** AM/PM
    match_score: float = Field(ge=0.0, le=1.0)
    rotation_name: str | None = None
    compatibility_factors: dict = Field(default_factory=dict)
    mutual_benefit: bool = False
    approval_likelihood: str = "medium"  ***REMOVED*** "low", "medium", "high"
