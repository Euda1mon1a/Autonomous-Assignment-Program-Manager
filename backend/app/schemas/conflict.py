...
class ConflictTypeEnum(str, Enum):
    ...
    ***REMOVED*** Cross-system conflicts
    LEAVE_FMIT_OVERLAP = "leave_fmit_overlap"
    RESIDENCY_FMIT_DOUBLE_BOOKING = "residency_fmit_double_booking"

    ***REMOVED*** Schedule pattern conflicts
    BACK_TO_BACK = "back_to_back"
    EXCESSIVE_ALTERNATING = "excessive_alternating"

    ***REMOVED*** ACGME compliance violations
    WORK_HOUR_VIOLATION = "work_hour_violation"  ***REMOVED*** 80-hour weekly limit
    REST_DAY_VIOLATION = "rest_day_violation"  ***REMOVED*** 1-in-7 day off requirement

    ***REMOVED*** Supervision conflicts
    SUPERVISION_RATIO_VIOLATION = "supervision_ratio_violation"
    MISSING_SUPERVISION = "missing_supervision"

    ***REMOVED*** Other conflicts
...
class ConflictGroup(BaseModel):
    ...
    group_by: str  ***REMOVED*** "type", "person", "severity", "date"
    group_key: str  ***REMOVED*** The value being grouped by
    conflict_count: int
...
class ConflictDetectionRequest(BaseModel):
    ...
    group_by: str | None = None  ***REMOVED*** "type", "person", "severity", "date"
...
class ACGMEComplianceCheck(BaseModel):
    ...
    ***REMOVED*** 80-hour work week compliance
    weekly_hours: dict[str, float] = Field(default_factory=dict)  ***REMOVED*** week_start -> hours
    hours_violations: list[str] = Field(default_factory=list)

    ***REMOVED*** 1-in-7 day off compliance
    consecutive_work_days: list[dict] = Field(default_factory=list)  ***REMOVED*** [{start, end, days}]
    rest_day_violations: list[str] = Field(default_factory=list)

    ***REMOVED*** Overall compliance
...
class SupervisionRatioCheck(BaseModel):
    ...
    ***REMOVED*** Ratios (actual vs required)
    pgy1_ratio_actual: float | None = None
    pgy1_ratio_required: float = 2.0  ***REMOVED*** 1:2 for PGY-1
    pgy2_3_ratio_actual: float | None = None
    pgy2_3_ratio_required: float = 4.0  ***REMOVED*** 1:4 for PGY-2/3
