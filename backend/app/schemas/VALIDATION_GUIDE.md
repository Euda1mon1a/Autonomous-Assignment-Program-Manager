***REMOVED*** API Validation Enhancement Guide
...
1. [Validation Principles](***REMOVED***validation-principles)
2. [Field-Level Validation](***REMOVED***field-level-validation)
3. [Cross-Field Validation](***REMOVED***cross-field-validation)
4. [Custom Validators](***REMOVED***custom-validators)
5. [Request/Response Examples](***REMOVED***requestresponse-examples)
6. [Error Messages](***REMOVED***error-messages)
7. [Testing Validation](***REMOVED***testing-validation)
8. [Best Practices](***REMOVED***best-practices)

---

***REMOVED******REMOVED*** Validation Principles

***REMOVED******REMOVED******REMOVED*** 1. **Fail Fast**
Validate at the schema level before data reaches business logic.

***REMOVED******REMOVED******REMOVED*** 2. **Clear Error Messages**
Provide actionable error messages that tell users what's wrong and how to fix it.

***REMOVED******REMOVED******REMOVED*** 3. **Consistent Patterns**
Use the same validation patterns across all schemas for consistency.

***REMOVED******REMOVED******REMOVED*** 4. **Security First**
Validate to prevent injection attacks, XSS, and data leaks.

***REMOVED******REMOVED******REMOVED*** 5. **Document Constraints**
Include field descriptions that explain validation rules.

---

***REMOVED******REMOVED*** Field-Level Validation

***REMOVED******REMOVED******REMOVED*** String Validation
...
class PersonCreate(BaseModel):
    ...
    ***REMOVED*** Basic string with length constraints
    name: str = Field(
        ...,
        min_length=2,
        max_length=100,
    ...
    ***REMOVED*** Email validation (using EmailStr)
    ...
    ***REMOVED*** String with pattern validation (regex)
    ...
    ***REMOVED*** Enum-like string (limited values)
...
***REMOVED******REMOVED******REMOVED*** Numeric Validation
...
class AssignmentCreate(BaseModel):
    ...
    ***REMOVED*** Float with range and precision
...
***REMOVED******REMOVED******REMOVED*** Date and DateTime Validation
...
class DateRangeFilter(BaseModel):
    ...
    def validate_end_date_not_too_far_future(cls, v: date) -> date:
        ...
        max_date = date.today() + timedelta(days=730)  ***REMOVED*** 2 years
        if v > max_date:
            raise ValueError(
...
***REMOVED******REMOVED******REMOVED*** List and Collection Validation
...
***REMOVED******REMOVED*** Cross-Field Validation

***REMOVED******REMOVED******REMOVED*** Model Validator (After)
...
class AbsenceCreate(BaseModel):
    ...
    def validate_date_range(self):
        ...
        ***REMOVED*** Validate date range is reasonable
...
***REMOVED******REMOVED******REMOVED*** Conditional Field Requirements
...
class SwapRequest(BaseModel):
    ...
    ***REMOVED*** Optional fields (required for one_to_one)
...
***REMOVED******REMOVED*** Custom Validators

***REMOVED******REMOVED******REMOVED*** Reusable Validators Module
...
***REMOVED******REMOVED******REMOVED*** Usage in Schemas
...
class ProjectCreate(BaseModel):
    ...
    def validate_project_dates(self):
        ...
        validate_date_range(
            self.start_date,
            self.end_date,
            max_days=730,  ***REMOVED*** 2 years max
            field_name="project"
        )
        return self
```
...
***REMOVED******REMOVED*** Request/Response Examples

***REMOVED******REMOVED******REMOVED*** Adding Examples to Schemas
...
***REMOVED******REMOVED******REMOVED*** Multiple Examples for Different Scenarios
...
***REMOVED******REMOVED*** Error Messages

***REMOVED******REMOVED******REMOVED*** Clear, Actionable Error Messages
...
***REMOVED******REMOVED******REMOVED*** Error Message Best Practices
    ...
    def test_notes_too_long_rejected(self):
        ...
        with pytest.raises(ValueError):
            AssignmentCreate(
                ...
                person_id=uuid4(),
                role="primary",
                notes="x" * 1001  ***REMOVED*** Over 1000 char limit
            )
...
***REMOVED******REMOVED******REMOVED*** Test Cross-Field Validation
...
class TestDateRangeValidation:
    ...
    def test_range_too_large_rejected(self):
        ...
        start = date(2025, 1, 1)
        end = start + timedelta(days=366)  ***REMOVED*** Over 365 day limit
...
***REMOVED******REMOVED******REMOVED*** 1. **Use Field Constraints Over Validators When Possible**
...
***REMOVED******REMOVED******REMOVED*** 2. **Use Enums for Fixed Value Sets**
...
***REMOVED******REMOVED******REMOVED*** 3. **Validate Business Rules in Services, Not Schemas**
...
***REMOVED******REMOVED******REMOVED*** 4. **Document All Constraints**
...
***REMOVED******REMOVED******REMOVED*** 5. **Provide Helpful Examples**
...
***REMOVED******REMOVED******REMOVED*** 6. **Sanitize User Input**
...
def sanitize_notes(cls, v: str | None) -> str | None:
    ...
    if v is not None:
        ***REMOVED*** Remove HTML tags
        v = re.sub(r'<[^>]*>', '', v)
        ***REMOVED*** Strip whitespace
        v = v.strip()
    return v
```

***REMOVED******REMOVED******REMOVED*** 7. **Use Consistent Patterns**
