...
class ResidentRow(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Resident name")
    pgy_level: int = Field(..., ge=1, le=3, description="PGY level (1-3)")
...
class MatrixCell(BaseModel):
    ...
    row_index: int = Field(..., ge=0, description="Row index (resident)")
    column_index: int = Field(..., ge=0, description="Column index (block)")
    rotation: str | None = Field(
        None, max_length=50, description="Rotation name or abbreviation"
    )
    rotation_full_name: str | None = Field(
        None, max_length=200, description="Full rotation name"
    )
    hours: float = Field(..., ge=0, description="Total hours in this block")
...
class BlockSummary(BaseModel):
    ...
    average_hours: float = Field(..., ge=0, description="Average hours per resident")
