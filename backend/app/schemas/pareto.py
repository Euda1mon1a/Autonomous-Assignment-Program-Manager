...
class ParetoObjective(BaseModel):
    ...
    name: ObjectiveName = Field(..., description="Name of the objective to optimize")
    weight: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
    ...
    target_value: float | None = Field(
        default=None, description="Optional target value for the objective"
    )
...
class ParetoSolution(BaseModel):
    ...
    solution_id: int = Field(..., description="Unique identifier for this solution")
    ...
    is_feasible: bool = Field(
        default=True, description="Whether this solution satisfies all constraints"
    )
    constraint_violations: list[str] = Field(
        default=[], description="List of constraint violations if infeasible"
    )
    rank: int | None = Field(
        default=None,
    ...
    crowding_distance: float | None = Field(
        default=None, description="Crowding distance for diversity metric"
    )
...
class ParetoConstraint(BaseModel):
    ...
    parameters: dict[str, Any] = Field(
        default={}, description="Parameters for the constraint"
    )
    is_hard: bool = Field(
        default=True,
...
class ParetoResult(BaseModel):
    ...
    hypervolume: float | None = Field(
        default=None, description="Hypervolume indicator for solution quality"
    )
    total_solutions: int = Field(..., description="Total number of solutions evaluated")
    convergence_metric: float | None = Field(
        default=None, description="Metric indicating algorithm convergence"
    )
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete optimization"
    )
    algorithm: str = Field(
        default="NSGA-II", description="Algorithm used for optimization"
    )
    termination_reason: str | None = Field(
        default=None, description="Reason why optimization terminated"
    )
...
class ParetoOptimizeRequest(BaseModel):
    ...
    constraints: list[ParetoConstraint] = Field(
        default=[], description="List of constraints to satisfy"
    )
    ...
    n_generations: int = Field(
        default=100, ge=10, le=1000, description="Number of generations to evolve"
    )
    ...
    person_ids: list[UUID] | None = Field(
        default=None, description="Filter to specific persons for assignment"
    )
    block_ids: list[UUID] | None = Field(
        default=None, description="Filter to specific blocks for assignment"
    )
    ...
    def validate_weights(self) -> "ParetoOptimizeRequest":
        ...
        if total_weight > 0 and (
            total_weight < 0.1 or total_weight > len(self.objectives)
        ):
            raise ValueError(
...
class ParetoOptimizeResponse(BaseModel):
    ...
    result: ParetoResult | None = Field(
        default=None, description="Optimization results if successful"
    )
    error: str | None = Field(
        default=None, description="Error message if optimization failed"
    )
    recommended_solution_id: int | None = Field(
        default=None, description="ID of the recommended solution based on weights"
    )
...
class SolutionRankRequest(BaseModel):
    ...
    normalization: str = Field(
        default="minmax", description="Normalization method (minmax, zscore, none)"
    )
    ...
    def validate_weights(self) -> "SolutionRankRequest":
        ...
        for obj_name, weight in self.weights.items():
            if weight < 0:
                raise ValueError(
                    f"Weight for {obj_name} must be non-negative, got {weight}"
                )
        return self
