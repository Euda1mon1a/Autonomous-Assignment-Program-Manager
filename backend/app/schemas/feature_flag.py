...
class FeatureFlagBase(BaseModel):
    ...
    key: str = Field(..., min_length=1, max_length=100, description="Unique feature flag key")
    name: str = Field(..., min_length=1, max_length=255, description="Human-readable name")
    description: str | None = Field(None, description="Detailed description of the feature")
    flag_type: str = Field(default='boolean', description="Flag type: boolean, percentage, or variant")
    enabled: bool = Field(default=False, description="Whether the flag is enabled")
    rollout_percentage: float | None = Field(None, ge=0.0, le=1.0, description="Percentage rollout (0.0-1.0)")
    environments: list[str] | None = Field(None, description="Target environments (null = all)")
    target_user_ids: list[str] | None = Field(None, description="Target user IDs (null = all)")
    target_roles: list[str] | None = Field(None, description="Target roles (null = all)")
    variants: dict[str, float] | None = Field(None, description="A/B test variants with weights")
    dependencies: list[str] | None = Field(None, description="Required feature flag keys")
    custom_attributes: dict[str, Any] | None = Field(None, description="Custom targeting attributes")
    ...
    def validate_variants(cls, v: dict[str, float] | None) -> dict[str, float] | None:
        ...
        if v is not None:
            total = sum(v.values())
            if not (0.99 <= total <= 1.01):  ***REMOVED*** Allow small floating point errors
                raise ValueError(f"Variant weights must sum to 1.0, got {total}")
            for variant_name, weight in v.items():
                if not (0.0 <= weight <= 1.0):
                    raise ValueError(f"Variant weight must be between 0.0 and 1.0, got {weight} for {variant_name}")
        return v
        ...
        if v is not None:
            ...
            for env in v:
                if env not in allowed_envs:
                    raise ValueError(f"Invalid environment: {env}. Allowed: {', '.join(allowed_envs)}")
        return v
    ...
    def validate_roles(cls, v: list[str] | None) -> list[str] | None:
        """Validate role names."""
        if v is not None:
            allowed_roles = {'admin', 'coordinator', 'faculty', 'clinical_staff', 'rn', 'lpn', 'msa', 'resident'}
            for role in v:
...
class FeatureFlagEvaluationRequest(BaseModel):
    ...
    environment: str | None = Field(None, description="Environment (dev, staging, production)")
    context: dict[str, Any] | None = Field(None, description="Additional context for evaluation")


class FeatureFlagEvaluationResponse(BaseModel):
    """Schema for feature flag evaluation result."""
    enabled: bool = Field(..., description="Whether the feature is enabled for this context")
...
class FeatureFlagBulkEvaluationRequest(BaseModel):
    ...
    flag_keys: list[str] = Field(..., description="List of feature flag keys to evaluate")
    user_id: str | None = Field(None, description="User ID for targeting")
    user_role: str | None = Field(None, description="User role for targeting")
...
class FeatureFlagBulkEvaluationResponse(BaseModel):
    ...
    flags: dict[str, FeatureFlagEvaluationResponse] = Field(..., description="Map of flag key to evaluation result")
