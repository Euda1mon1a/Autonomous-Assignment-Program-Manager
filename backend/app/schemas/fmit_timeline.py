...
class WorkloadSummary(BaseModel):
    ...
    is_balanced: bool = Field(
        True, description="Whether workload is within acceptable range"
    )
    variance_from_target: float = Field(
        0.0, description="Weeks above/below target (positive = over)"
    )
...
class FacultyTimeline(BaseModel):
    ...
    weeks_assigned: list[WeekAssignment] = Field(
        default_factory=list, description="Weekly assignments"
    )
...
class AggregateMetrics(BaseModel):
    ...
    load_distribution: LoadDistribution = Field(
        ..., description="Statistical distribution"
    )
    total_faculty: int = Field(0, description="Total faculty members")
    total_weeks_scheduled: int = Field(
        0, description="Total weeks scheduled across all faculty"
    )
    coverage_percentage: float = Field(
        0.0, description="Percentage of required coverage achieved"
    )
...
class TimelineResponse(BaseModel):
    ...
    timeline_data: list[FacultyTimeline] = Field(
        default_factory=list, description="Faculty timelines"
    )
...
class GanttTask(BaseModel):
    ...
    styles: dict = Field(
        default_factory=dict, description="Custom styling for the task"
    )
...
class GanttGroup(BaseModel):
    ...
    tasks: list[GanttTask] = Field(
        default_factory=list, description="Tasks in this group"
    )
...
class GanttDataResponse(BaseModel):
    ...
    groups: list[GanttGroup] = Field(
        default_factory=list, description="Task groups (by faculty)"
    )
    all_tasks: list[GanttTask] = Field(
        default_factory=list, description="All tasks flattened"
    )
...
class WeeklyView(BaseModel):
    ...
    faculty_assignments: list[dict] = Field(
        default_factory=list, description="Faculty assignments this week"
    )
...
class WeeklyViewResponse(BaseModel):
    ...
    adjacent_weeks: dict = Field(
        default_factory=dict, description="Previous and next week summaries"
    )
    generated_at: datetime = Field(..., description="Generation timestamp")
