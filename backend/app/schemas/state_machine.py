...
class TransitionDefinition(BaseModel):
    ...
    priority: int = Field(0, description="Priority when multiple transitions match (higher wins)")
...
class StateMachineCreateRequest(BaseModel):
    ...
    entity_type: str | None = Field(None, description="Type of entity this machine manages")
    entity_id: UUID | None = Field(None, description="ID of entity this machine manages")
...
class TransitionRequest(BaseModel):
    ...
    context_updates: dict[str, Any] = Field(
        default_factory=dict,
        description="Updates to merge into context"
    )
