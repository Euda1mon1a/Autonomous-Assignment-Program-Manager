...
API_KEY_LENGTH = 64  ***REMOVED*** Characters in the random part
SIGNATURE_TOLERANCE_SECONDS = 300  ***REMOVED*** 5 minutes for replay attack prevention
...
class APIKeyManager:
    ...
    def generate_api_key(self) -> tuple[str, str, str]:
        ...
        """
        ***REMOVED*** Generate random key
        random_part = secrets.token_urlsafe(API_KEY_LENGTH)
        full_key = f"{API_KEY_PREFIX}{random_part}"

        ***REMOVED*** Hash for storage
        key_hash = hashlib.sha256(full_key.encode()).hexdigest()

        ***REMOVED*** Extract prefix for display
        key_prefix = full_key[:16]  ***REMOVED*** "sk_live_" + first 8 chars
    ...
    async def create_api_key(
        ...
        ***REMOVED*** Return response with full key (only time we show it)
    ...
    async def rotate_api_key(
        ...
        ***REMOVED*** Create new key with same settings
        ...
        ***REMOVED*** Schedule old key expiration
...
class OAuth2Manager:
    ...
    async def issue_access_token(
        ...
        ***REMOVED*** Update usage statistics
...
class IPFilterManager:
    ...
    def _ip_in_range(self, ip: str, range_spec: str) -> bool:
        ...
        try:
            ...
            ***REMOVED*** Check if range_spec is a CIDR range
            if "/" in range_spec:
                network = ipaddress.ip_network(range_spec, strict=False)
                return ip_obj in network
            else:
                ***REMOVED*** Single IP comparison
    ...
    async def is_ip_blacklisted(
        ...
        """
        ***REMOVED*** Get all active blacklist entries
        result = self.db.execute(
    ...
    async def is_ip_whitelisted(
        ...
        """
        ***REMOVED*** Get all active whitelist entries
        result = self.db.execute(
    ...
    async def check_ip_allowed(
        ...
        """
        ***REMOVED*** Check blacklist first (takes precedence)
        ...
        ***REMOVED*** Check allowed IPs list (for API key restrictions)
        if allowed_ips:
            ip_allowed = any(
        ...
        ***REMOVED*** Check whitelist (informational, doesn't block)
...
class RequestSignatureValidator:
    ...
    def _generate_signature(
        ...
        Returns:
            HMAC signature (hex)
        """
        ***REMOVED*** Build signing string: METHOD\nPATH\nTIMESTAMP\nBODY
    ...
    async def verify_signature(
        ...
        try:
            ...
            ***REMOVED*** Check timestamp tolerance (prevent replay attacks)
            ...
            ***REMOVED*** Check for signature replay
            ...
            if existing:
                ***REMOVED*** Log failed verification
            ...
            ***REMOVED*** Generate expected signature
            ***REMOVED*** Note: In production, you'd use the actual API key value, not the hash
            ***REMOVED*** This is a simplified example - actual implementation would need
            ***REMOVED*** to store a signing secret separately or derive it from the key
            expected_signature = self._generate_signature(
                secret=api_key.key_hash,  ***REMOVED*** In reality, use actual key or separate signing secret
            ...
            ***REMOVED*** Log verification attempt
...
class GatewayAuthenticator:
    ...
    async def validate_request(
        ...
        ***REMOVED*** Check IP blacklist first
        (
            is_blacklisted,
            blacklist_entry,
    ...
    async def _validate_api_key(
        ...
        ***REMOVED*** Verify signature if provided
        if signature_data:
    ...
    async def _validate_jwt(
        ...
        ***REMOVED*** Check IP whitelist/blacklist
        is_allowed, reason = await self.ip_filter_manager.check_ip_allowed(
        ...
        ***REMOVED*** Get user
        ...
        return GatewayAuthValidationResponse(
            ...
            auth_type="jwt",
            user_id=user.id,
            scopes=["user"],  ***REMOVED*** Could be expanded based on user roles
        )
    ...
    def _get_client_ip(self, request: Request) -> str:
        ...
        ***REMOVED*** Check X-Forwarded-For header first
...
***REMOVED*** Utility functions for FastAPI dependencies
...
async def require_api_key(request: Request, db: Session) -> APIKey:
    ...
    """
    ***REMOVED*** Extract API key from header
    ...
    ***REMOVED*** Get and return the API key object
