...
class OverageAlert:
    ...
    period: AggregationPeriod
    timestamp: datetime
    severity: str  ***REMOVED*** "warning", "critical", "emergency"
...
class PricingConfig:
    ...
    ***REMOVED*** Base prices per unit (in cents)
    RESOURCE_PRICES = {
        ...
        MeteredResource.STORAGE_UPLOAD: Decimal("0.05"),  ***REMOVED*** per MB
        MeteredResource.STORAGE_DOWNLOAD: Decimal("0.02"),  ***REMOVED*** per MB
    }

    ***REMOVED*** Tiered pricing multipliers
    TIER_MULTIPLIERS = {
        UsageTier.FREE: Decimal("0"),  ***REMOVED*** Free tier
        UsageTier.BASIC: Decimal("1.0"),  ***REMOVED*** Full price
        UsageTier.PROFESSIONAL: Decimal("0.75"),  ***REMOVED*** 25% discount
        UsageTier.ENTERPRISE: Decimal("0.50"),  ***REMOVED*** 50% discount
    }

    ***REMOVED*** Volume discounts (quantity threshold -> discount percentage)
    VOLUME_DISCOUNTS = {
        1000: Decimal("0.05"),  ***REMOVED*** 5% discount above 1k units
        10000: Decimal("0.10"),  ***REMOVED*** 10% discount above 10k units
        100000: Decimal("0.15"),  ***REMOVED*** 15% discount above 100k units
        1000000: Decimal("0.20"),  ***REMOVED*** 20% discount above 1M units
    }
...
class MeteringService:
    ...
    ***REMOVED*** =========================================================================
    ...
    async def record_event(
        ...
        ***REMOVED*** Store in Redis for real-time tracking
        await self._store_event_redis(event)

        ***REMOVED*** Store in database for persistent tracking (would need a model)
        ***REMOVED*** await self._store_event_db(event)
    ...
    async def _store_event_redis(self, event: UsageEvent) -> None:
        ...
        """
        ***REMOVED*** Store in sorted set by timestamp
        ...
        ***REMOVED*** Set expiration (keep events for 90 days in Redis)
        self.redis.expire(key, 90 * 24 * 3600)

        ***REMOVED*** Update real-time counters
    ...
    async def _update_realtime_counters(self, event: UsageEvent) -> None:
        ...
        ***REMOVED*** Hourly counter
        ...
        self.redis.expire(hour_key, 7 * 24 * 3600)  ***REMOVED*** 7 days

        ***REMOVED*** Daily counter
        day_key = f"metering:daily:{event.user_id}:{event.resource.value}:{now.strftime('%Y-%m-%d')}"
        self.redis.incrby(day_key, event.quantity)
        self.redis.expire(day_key, 90 * 24 * 3600)  ***REMOVED*** 90 days

        ***REMOVED*** Monthly counter
        ...
        self.redis.expire(month_key, 365 * 24 * 3600)  ***REMOVED*** 1 year

    ***REMOVED*** =========================================================================
    ***REMOVED*** Usage Aggregation
    ***REMOVED*** =========================================================================
    ...
    async def aggregate_usage(
        ...
        ***REMOVED*** Determine resources to aggregate
    ...
    async def _aggregate_resource_usage(
        ...
        """
        ***REMOVED*** Get events from Redis
        key = f"metering:events:{user_id}:{resource.value}"

        ***REMOVED*** Query by timestamp range
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _get_current_period_usage(
        ...
        ***REMOVED*** Determine key based on period
        if period == AggregationPeriod.HOURLY:
            ...
            ***REMOVED*** For weekly/yearly, calculate from events
            return await self._calculate_period_usage(user_id, resource, period)

        ***REMOVED*** Get counter from Redis
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _store_overage_alert(self, alert: OverageAlert) -> None:
        ...
        self.redis.setex(key, 7 * 24 * 3600, value)  ***REMOVED*** 7 days expiration
        ...
        for res in resources:
            ...
            if alert_data:
                ...
                alert = OverageAlert(
                    ...
                    period=AggregationPeriod.DAILY,  ***REMOVED*** Default
    ...
    ***REMOVED*** =========================================================================
    ...
    async def generate_usage_report(
        ...
        ***REMOVED*** Get overage alerts
        overages = await self.get_overage_alerts(user_id)

        ***REMOVED*** Generate forecast if requested
        forecasted_usage = None
        if include_forecast:
            ***REMOVED*** Forecast for the most used resource
            if by_resource:
                ...
                forecasted_usage = await self.forecast_usage(
                    ...
                    forecast_periods=7,  ***REMOVED*** 7 days ahead
                )

        ***REMOVED*** Aggregate by period (daily breakdown)
    ...
    async def _aggregate_by_daily_periods(
        ...
        while current <= end_date:
            ...
            ***REMOVED*** Aggregate all resources for this day
            day_quantity = 0
            day_cost = Decimal("0")
            day_events = 0
            ...
            if day_events > 0:
                by_period[day_start.strftime("%Y-%m-%d")] = UsageAggregation(
                    user_id=user_id,
                    resource=MeteredResource.API_READ,  ***REMOVED*** Placeholder
                    period=AggregationPeriod.DAILY,
    ...
    ***REMOVED*** =========================================================================
    ...
    async def generate_billing_record(
        ...
        billing_period_start: datetime,
        billing_period_end: datetime,
        tax_rate: Decimal = Decimal("0.08"),  ***REMOVED*** 8% default
    ) -> BillingRecord:
        """
        ...
        ***REMOVED*** Generate line items for each resource
        for resource in MeteredResource:
            agg = await self._aggregate_resource_usage(
                user_id=user_id,
                resource=resource,
                ...
                line_items.append(
                    {
                        ...
                        ),  ***REMOVED*** Average unit price
                        "total": str(agg.total_cost),
                    }
                )
                subtotal += agg.total_cost

        ***REMOVED*** Calculate taxes
    ...
    async def export_billing_data(
        ...
        if format == "stripe":
            ***REMOVED*** Format for Stripe billing API
            return {
                "customer": user_id,
                "currency": billing_record.currency.lower(),
                "amount": int(billing_record.total * 100),  ***REMOVED*** Stripe uses cents
    ...
    ***REMOVED*** =========================================================================
    ...
    async def forecast_usage(
        ...
        """
        ***REMOVED*** Get historical data (last 30 days)
        end_date = datetime.utcnow()
        start_date = end_date - timedelta(days=30)
        ...
        ***REMOVED*** Calculate simple moving average
        if not historical_usage:
            ...
            ***REMOVED*** Calculate standard deviation
            variance = sum((x - predicted_usage) ** 2 for x in historical_usage) / len(
                historical_usage
            )
            std_dev = int(variance**0.5)

        ***REMOVED*** Calculate confidence interval (using normal distribution approximation)
        ***REMOVED*** For 95% confidence, z-score is approximately 1.96
        z_score = 1.96 if confidence_level == 0.95 else 2.576  ***REMOVED*** 99% confidence
        ...
        ***REMOVED*** Estimate model accuracy (R-squared approximation)
        if historical_usage:
    ...
    async def get_usage_trends(
        ...
        ***REMOVED*** Calculate statistics
        ...
        if usage_values:
            ...
            ***REMOVED*** Calculate trend (simple linear regression slope)
            n = len(usage_values)
            sum_x = sum(range(n))
