...
def example_task_dependencies():
    ...
    ***REMOVED*** Task 2: Process data (depends on fetch_data)
    task2 = TaskDefinition(
        ...
        dependencies=[
            TaskDependency(
                task_id="fetch_data",
                dependency_type="success",  ***REMOVED*** Only run if fetch succeeded
            )
        ],
    )
    scheduler.register_task(task2)

    ***REMOVED*** Task 3: Generate report (depends on process_data)
...
***REMOVED*** Example 4: Task with distributed lock
def example_distributed_lock():
    ...
    task_def = TaskDefinition(
        ...
        priority=TaskPriority.HIGH,
        require_lock=True,
        lock_timeout=600,  ***REMOVED*** 10 minutes
    )
...
***REMOVED*** Example 5: Cron-scheduled task
def example_cron_task():
    ...
    task_def = TaskDefinition(
        ...
        args=[90],  ***REMOVED*** retention_days
    )

    scheduler.register_task(task_def)

    ***REMOVED*** Schedule to run every hour at minute 15
    execution_ids = scheduler.schedule_cron_task(
        task_id="hourly_cleanup",
        cron_expression="15 * * * *",  ***REMOVED*** Every hour at :15
    )

    print(f"Scheduled cron task(s): {execution_ids}")


***REMOVED*** Example 6: Multi-priority task queue
def example_multi_priority():
    """Example demonstrating priority-based task execution."""
    scheduler = get_advanced_scheduler()

    ***REMOVED*** Register tasks with different priorities
    priorities = [
...
***REMOVED*** Example 7: Task with timeout
...
***REMOVED*** Example 8: Monitoring and health checks
async def example_health_monitoring():
    ...
    ***REMOVED*** Wait a bit for tasks to execute
    ...
    ***REMOVED*** Stop scheduler
    await scheduler.stop()


***REMOVED*** Example 9: Complex workflow with multiple features
async def example_complex_workflow():
    ...
    ***REMOVED*** Step 1: Data validation (high priority, with lock)
    ...
    ***REMOVED*** Step 2: Data processing (depends on validation, with retry)
    ...
    ***REMOVED*** Step 3: Generate report (depends on processing)
    ...
    ***REMOVED*** Step 4: Notify users (low priority, runs after report)
    notify_task = TaskDefinition(
        ...
        dependencies=[
            TaskDependency(
                task_id="generate_final_report",
                dependency_type="completion",  ***REMOVED*** Run even if report failed
            )
        ],
    )
    scheduler.register_task(notify_task)

    ***REMOVED*** Schedule the workflow
    await scheduler.start()
...
***REMOVED*** Example 10: Scheduled task status tracking
