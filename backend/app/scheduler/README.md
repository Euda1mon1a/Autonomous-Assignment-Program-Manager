***REMOVED*** Advanced Task Scheduler
...
***REMOVED******REMOVED******REMOVED*** Basic Task Scheduling
...
***REMOVED*** Initialize scheduler
scheduler = AdvancedTaskScheduler()

***REMOVED*** Define a task
...
***REMOVED*** BACKGROUND tasks run last
...
***REMOVED*** Task 2: Process data (depends on fetch)
process_task = TaskDefinition(
    ...
    dependencies=[
        TaskDependency(
            task_id="fetch_data",
            dependency_type="success",  ***REMOVED*** Only run if fetch succeeds
        )
    ],
)

***REMOVED*** Task 3: Generate report (depends on process)
...
task_def = TaskDefinition(
    ...
    require_lock=True,
    lock_timeout=600,  ***REMOVED*** Lock expires after 10 minutes
)
```
...
***REMOVED******REMOVED******REMOVED*** Retry with Backoff
...
retry_config = RetryConfig(
    ...
    jitter=True,  ***REMOVED*** Add random jitter
)
...
***REMOVED******REMOVED******REMOVED*** Cron Scheduling

Schedule tasks using cron expressions:

```python
***REMOVED*** Run every hour at minute 15
scheduler.schedule_cron_task(
    task_id="hourly_task",
    cron_expression="15 * * * *",
)

***REMOVED*** Run daily at 2:30 AM
scheduler.schedule_cron_task(
    task_id="daily_task",
    cron_expression="30 2 * * *",
)

***REMOVED*** Run every Monday at 9 AM
scheduler.schedule_cron_task(
    task_id="weekly_task",
...
print(f"Status: {health['status']}")  ***REMOVED*** healthy or degraded
...
***REMOVED******REMOVED******REMOVED*** Task Status Tracking
...
***REMOVED*** Cancel before execution
...
***REMOVED******REMOVED*** Integration with Existing Scheduler
...
```python
scheduler = AdvancedTaskScheduler(
    redis_client=custom_redis_client,  ***REMOVED*** Optional
    max_concurrent_tasks=10,  ***REMOVED*** Maximum parallel tasks
)
```
...
***REMOVED******REMOVED******REMOVED*** Key Methods
...
***REMOVED******REMOVED******REMOVED*** Lock Acquisition Failures
...
***REMOVED******REMOVED******REMOVED*** Dependency Errors
