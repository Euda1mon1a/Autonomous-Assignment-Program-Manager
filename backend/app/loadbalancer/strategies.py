...
class RoundRobinStrategy(LoadBalancingStrategy):
    ...
    def __init__(self):
        ...
        ***REMOVED*** Track current index per service
    ...
    async def select_instance(
        ...
        ***REMOVED*** Get service name from first instance
        service_name = instances[0].service_name

        ***REMOVED*** Select instance at current index
        index = self._current_index[service_name] % len(instances)
        selected = instances[index]

        ***REMOVED*** Increment index for next selection
...
class WeightedRoundRobinStrategy(LoadBalancingStrategy):
    ...
    async def select_instance(
        ...
        ***REMOVED*** Get service name
        service_name = instances[0].service_name

        ***REMOVED*** Calculate max weight
        max_weight = max(i.weight for i in instances)
        total_weight = sum(i.weight for i in instances)

        if total_weight == 0:
            ***REMOVED*** All weights are 0, fall back to simple round-robin
            index = self._current_index[service_name] % len(instances)
            selected = instances[index]
            self._current_index[service_name] = (index + 1) % len(instances)
            self._total_selections += 1
        ...
        ***REMOVED*** Weighted selection using smooth weighted round-robin algorithm
        while True:
            ***REMOVED*** Increment index
            index = self._current_index[service_name] % len(instances)
            instance = instances[index]

            ***REMOVED*** Increment current weight
            self._current_weight[service_name] += max_weight

            ***REMOVED*** Check if current instance should be selected
            ...
            ***REMOVED*** Move to next instance
            self._current_index[service_name] = (index + 1) % len(instances)

            ***REMOVED*** Safety check to prevent infinite loop
            if self._current_index[service_name] == 0:
                break

        ***REMOVED*** Fallback: select first instance
        self._total_selections += 1
        return instances[0]
...
class LeastConnectionsStrategy(LoadBalancingStrategy):
    ...
    def __init__(self):
        ...
        ***REMOVED*** Track active connections per instance
    ...
    async def select_instance(
        ...
        ***REMOVED*** Find instance with minimum connections
        ...
        if selected:
            ***REMOVED*** Increment connection count for selected instance
...
class HealthBasedStrategy(LoadBalancingStrategy):
    ...
    async def select_instance(
        ...
        ***REMOVED*** Filter to healthy instances only
        ...
        ***REMOVED*** Track filtered count
        self._unhealthy_filtered += len(instances) - len(healthy_instances)

        ***REMOVED*** Use underlying strategy to select from healthy instances
...
class IPHashStrategy(LoadBalancingStrategy):
    ...
    def __init__(self):
        ...
        self._total_selections = 0
        self._cache: dict[str, str] = {}  ***REMOVED*** Maps client_ip -> instance_id
    ...
    async def select_instance(
        ...
        if not client_ip:
            ***REMOVED*** No client IP provided, fall back to random
            selected = random.choice(instances)
            self._total_selections += 1
            return selected
        ...
        ***REMOVED*** Calculate hash and select instance
        hash_value = hash(client_ip)
        index = hash_value % len(instances)
        selected = instances[index]

        ***REMOVED*** Cache the selection
