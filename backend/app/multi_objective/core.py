...
class ObjectiveType(Enum):
    ...
    ***REMOVED*** Core scheduling objectives
    COVERAGE = "coverage"
    EQUITY = "equity"
...
class ObjectiveConfig:
    ...
    def normalize(self, value: float) -> float:
        ...
        ***REMOVED*** Normalize to 0 = ideal (reference), 1 = worst (nadir)
...
class DominanceRelation(Enum):
    ...
    DOMINATES = "dominates"  ***REMOVED*** First solution dominates second
    DOMINATED = "dominated"  ***REMOVED*** First solution is dominated by second
    INCOMPARABLE = "incomparable"  ***REMOVED*** Neither dominates the other
    EQUAL = "equal"  ***REMOVED*** Solutions are equal in all objectives
...
def compare_dominance(
    ...
    """
    ***REMOVED*** Feasibility-first comparison
    if consider_constraints:
        ...
        if not sol1.is_feasible and not sol2.is_feasible:
            ***REMOVED*** Compare by constraint violation
            if sol1.total_constraint_violation < sol2.total_constraint_violation:
                return DominanceRelation.DOMINATES
            if sol1.total_constraint_violation > sol2.total_constraint_violation:
                return DominanceRelation.DOMINATED

    ***REMOVED*** Pareto dominance comparison
    better_count = 0
    worse_count = 0
    equal_count = 0
...
class ParetoFrontier:
    ...
    def add(self, solution: Solution) -> bool:
        ...
        for i, existing in enumerate(self.solutions):
            ...
            if relation == DominanceRelation.DOMINATED:
                return False  ***REMOVED*** New solution is dominated
            elif relation == DominanceRelation.DOMINATES:
                dominated_by_new.append(i)
            elif relation == DominanceRelation.EQUAL:
                return False  ***REMOVED*** Duplicate

        ***REMOVED*** Remove dominated solutions
    ...
    def get_knee_solution(self) -> Solution | None:
        ...
        ***REMOVED*** Normalize all solutions
        normalized = []
        ...
        ***REMOVED*** Find solution with maximum distance from the line connecting extremes
        ***REMOVED*** This is a simplified knee detection using the trade-off curve approach
        max_distance = 0
        knee_idx = 0

        ***REMOVED*** Calculate centroid
        centroid = np.mean(normalized, axis=0)

        for i, norm_vec in enumerate(normalized):
            ***REMOVED*** Distance from centroid (approximation of knee)
...
class SolutionArchive:
    ...
    solutions: list[Solution] = field(default_factory=list)
    objectives: list[ObjectiveConfig] = field(default_factory=list)
    pruning_method: str = "crowding"  ***REMOVED*** "crowding", "random", "age"
    ...
    def add(self, solution: Solution) -> bool:
        ...
        """
        ***REMOVED*** Check if dominated by any existing solution
        ...
        ***REMOVED*** Remove solutions dominated by the new one
        self.solutions = [
            s
        ...
        ***REMOVED*** Prune if over capacity
    ...
    def _prune_one(self) -> None:
        ...
        if self.pruning_method == "crowding":
            ...
            ***REMOVED*** Remove solution with minimum crowding distance
            ...
            ***REMOVED*** Remove oldest solution
...
***REMOVED*** Common objective configurations for scheduling
SCHEDULING_OBJECTIVES = [
    ObjectiveConfig(
        ...
        weight=1.0,
        reference_point=1.0,  ***REMOVED*** 100% coverage
        nadir_point=0.0,  ***REMOVED*** 0% coverage
    ),
    ObjectiveConfig(
        name="equity",
        ...
        weight=0.8,
        reference_point=0.0,  ***REMOVED*** Perfect equity
        nadir_point=1.0,  ***REMOVED*** Complete inequity
    ),
    ObjectiveConfig(
        ...
        weight=0.6,
        reference_point=1.0,  ***REMOVED*** All preferences satisfied
        nadir_point=0.0,  ***REMOVED*** No preferences satisfied
    ),
    ObjectiveConfig(
        name="resilience",
        ...
        weight=0.7,
        reference_point=1.0,  ***REMOVED*** Fully resilient
        nadir_point=0.0,  ***REMOVED*** No resilience
    ),
    ObjectiveConfig(
        name="call_equity",
        ...
        weight=0.5,
        reference_point=0.0,  ***REMOVED*** Perfect call equity
        nadir_point=1.0,  ***REMOVED*** Complete inequity
    ),
]
