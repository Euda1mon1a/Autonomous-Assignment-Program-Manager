...
class HypervolumeIndicator(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** Get objective matrix (normalized to minimization)
        points = self._get_normalized_points(front)

        ***REMOVED*** Determine reference point
    ...
    def _get_reference_point(
        ...
        ***REMOVED*** Use nadir point + small offset
    ...
    def _calculate_hv(self, points: np.ndarray, ref_point: np.ndarray) -> float:
        ...
        if n_dim == 1:
            ***REMOVED*** 1D case: simple interval
            return float(ref_point[0] - np.min(points))

        if n_dim == 2:
            ***REMOVED*** 2D case: efficient sweep algorithm
            return self._hv_2d(points, ref_point)

        ***REMOVED*** General case: WFG recursive algorithm
    ...
    def _hv_wfg(self, points: np.ndarray, ref_point: np.ndarray) -> float:
        ...
        ***REMOVED*** Filter points dominated by reference point
        ...
        ***REMOVED*** Recursive calculation
        hv = 0.0
        while len(sorted_points) > 0:
            ***REMOVED*** Get point with best first objective
            p = sorted_points[0]
            sorted_points = sorted_points[1:]

            ***REMOVED*** Volume contributed by this point
            if n_dim == 2:
                contrib = (ref_point[0] - p[0]) * (ref_point[1] - p[1])
            else:
                ***REMOVED*** Recursive for remaining dimensions
            ...
            ***REMOVED*** Update reference for remaining points
...
class InvertedGenerationalDistance(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** Calculate distances from reference to nearest approximation
        distances = []
        for ref in ref_points:
            min_dist = float("inf")
            for point in approx_points:
...
class SpreadIndicator(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** For 2D, use simple consecutive distance spread
        if points.shape[1] == 2:
            return self._spread_2d(points)

        ***REMOVED*** General case: use crowding-based spread
    ...
    def _spread_2d(self, points: np.ndarray) -> float:
        ...
        ***REMOVED*** Calculate consecutive distances
        distances = []
        for i in range(n - 1):
    ...
    def _spread_general(self, points: np.ndarray) -> float:
        ...
        ***REMOVED*** Calculate all pairwise distances
...
class EpsilonIndicator(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** For each reference point, find minimum epsilon
...
class Spacing(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** Calculate minimum distance from each point to others
        d = []
        for i in range(n):
            min_dist = float("inf")
            for j in range(n):
                if i != j:
                    ***REMOVED*** Use L1 (Manhattan) distance
        ...
        ***REMOVED*** Spacing = standard deviation of distances
...
class MaximumSpread(QualityIndicator):
    ...
    def calculate(
        ...
        ***REMOVED*** Calculate extent in each dimension
        extents = []
        for j in range(points.shape[1]):
            extent = np.max(points[:, j]) - np.min(points[:, j])
            extents.append(extent)

        ***REMOVED*** Maximum spread is the Euclidean norm of extents
...
class QualityEvaluator:
    ...
    def evaluate(
        ...
        ***REMOVED*** Set ideal and nadir points
        ...
        ***REMOVED*** Calculate each indicator
        for indicator in self.indicators:
            try:
                ...
                ***REMOVED*** Skip failed indicators
                pass
