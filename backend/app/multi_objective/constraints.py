...
class DynamicPenaltyMethod(PenaltyMethod):
    ...
    def calculate_penalty(
        ...
        ***REMOVED*** Coefficient grows with generation
...
class AdaptivePenaltyMethod(PenaltyMethod):
    ...
    def update_feasibility(self, feasibility_ratio: float) -> None:
        ...
        if feasibility_ratio < self.target_feasibility:
            ***REMOVED*** Too few feasible - reduce penalty to allow exploration
            self.coefficient *= 1 - self.adaptation_rate
        else:
            ***REMOVED*** Too many feasible - increase penalty for convergence
            self.coefficient *= 1 + self.adaptation_rate

        ***REMOVED*** Keep within reasonable bounds
...
class ConstraintDominanceMethod(PenaltyMethod):
    ...
    def calculate_penalty(
        ...
        ***REMOVED*** If any hard constraint is violated, add large penalty
...
class GreedyRepairOperator(RepairOperator):
    ...
    def can_repair(self, solution: Solution, violations: list[ConstraintViolation]) -> bool:
        ...
        for v in violations:
            if v.is_hard and not v.relaxable:
                ***REMOVED*** Check if we have a repair function
    ...
    def repair(
        ...
        for _ in range(self.max_iterations):
            ...
            ***REMOVED*** Pick violation with smallest magnitude (easier to fix)
            remaining_violations.sort(key=lambda v: v.magnitude)
            violation = remaining_violations[0]

            ***REMOVED*** Apply repair function
            ...
            if repair_fn:
                ...
                if success:
                    ...
                    ***REMOVED*** Could not repair this violation
...
class RandomRepairOperator(RepairOperator):
    ...
    def repair(
        ...
        for _ in range(self.max_attempts):
            ***REMOVED*** Create random perturbation
            candidate = solution.copy()

            ***REMOVED*** Apply random changes based on affected entities
            for v in violations:
                for entity_id in v.affected_entities:
                    ***REMOVED*** Randomly reassign entity (implementation depends on problem)
                    pass

            ***REMOVED*** Evaluate candidate
...
class FeasibilityPreserver:
    ...
    def generate_feasible_offspring(
        ...
        for _ in range(self.max_attempts):
            ...
            ***REMOVED*** Check feasibility
            if self.feasibility_check(offspring, context):
                return offspring

        ***REMOVED*** Fallback: return best parent
...
class ConstraintRelaxer:
    ...
    def update(self, is_feasible: bool, feasibility_ratio: float) -> dict[str, float]:
        ...
        if is_feasible and feasibility_ratio > 0.1:
            ...
            ***REMOVED*** Restore constraints if feasible for several generations
            ...
            ***REMOVED*** Relax constraints if infeasible for several generations
    ...
    def _restore_constraints(self) -> None:
        ...
        for level in self.relaxations.values():
            if level.is_relaxed:
                ...
                ***REMOVED*** Check if fully restored
    ...
    def process_solution(
        ...
        ***REMOVED*** Apply handling method
        if self.method == ConstraintHandlingMethod.PENALTY:
            ...
            ***REMOVED*** Try repair first, then apply penalty to remaining violations
            if self.repair_operator and self.repair_operator.can_repair(solution, violations):
                repaired, success = self.repair_operator.repair(solution, violations, context)
                if success:
                    self.repaired_count += 1
                    ...
                    repaired.total_constraint_violation = 0.0
                    return repaired
                ***REMOVED*** Partial repair - recalculate violations
                solution = repaired
...
***REMOVED*** Pre-configured constraint handlers for common scenarios
