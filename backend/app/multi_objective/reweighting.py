...
class FeedbackType(Enum):
    ...
    RATING = "rating"  ***REMOVED*** 1-5 star rating
    COMPARISON = "comparison"  ***REMOVED*** Pairwise comparison
    ADJUSTMENT = "adjustment"  ***REMOVED*** Direct weight adjustment
    SELECTION = "selection"  ***REMOVED*** Solution selection
    REJECTION = "rejection"  ***REMOVED*** Solution rejection
    COMPLAINT = "complaint"  ***REMOVED*** Dissatisfaction with objective
    PRIORITY = "priority"  ***REMOVED*** Explicit priority ordering
...
class FeedbackEvent:
    ...
    confidence: float = 1.0  ***REMOVED*** How confident is this signal
...
class FeedbackProcessor:
    ...
    def __init__(
        ...
        ***REMOVED*** Feedback history
        self.history: list[FeedbackEvent] = []

        ***REMOVED*** Cumulative adjustment signals
    ...
    def process_feedback(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        ***REMOVED*** Update cumulative signals
    ...
    def _process_rating(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        deltas = {}
        normalized_rating = (rating - 3) / 2  ***REMOVED*** Map to [-1, 1]

        ***REMOVED*** High rating increases weight of objectives where solution excels
        for obj in self.active_objectives:
            if obj.name in solution_objectives:
                val = solution_objectives[obj.name]

                ***REMOVED*** Determine if this is a "good" value for this objective
                if obj.reference_point is not None and obj.nadir_point is not None:
                    normalized = obj.normalize(val)
                    ***REMOVED*** If solution is good in this objective and rated highly,
                    ***REMOVED*** increase its weight (user values it)
    ...
    def _process_comparison(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        for obj in self.active_objectives:
            if obj.name in preferred_objectives and obj.name in other_objectives:
                ...
                ***REMOVED*** If preferred is better in this objective, increase weight
    ...
    def _process_selection(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        for obj in self.active_objectives:
            if obj.name in solution_objectives:
                ***REMOVED*** Increase weight for objectives where selected solution is good
    ...
    def _process_rejection(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        ***REMOVED*** If reason mentions specific objective, decrease that weight
        for obj in self.active_objectives:
    ...
    def _process_complaint(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        severity = event.data.get("severity", 1.0)  ***REMOVED*** 0-1
    ...
    def _process_priority(self, event: FeedbackEvent) -> dict[str, float]:
        ...
        for i, obj_name in enumerate(priority_order):
            if obj_name in [o.name for o in self.active_objectives]:
                ***REMOVED*** Higher priority = higher weight adjustment
...
class ObjectiveAdjuster:
    ...
    def apply_deltas(
        ...
        Returns:
            New weight values
        """
        ***REMOVED*** Apply deltas with bounds
        for obj_name, delta in deltas.items():
...
class ContextProfile:
    ...
    context: ContextType
    weight_multipliers: dict[str, float]  ***REMOVED*** Multiply base weights by these
    description: str
...
class ContextualReweighter:
    ...
    def __init__(self, objectives: list[ObjectiveConfig]):
        ...
        ***REMOVED*** Default context profiles
...
class TemporalSchedule:
    ...
    reason: str
    priority: int = 0  ***REMOVED*** Higher priority overrides
...
class TemporalReweighter:
    ...
    def __init__(self, objectives: list[ObjectiveConfig]):
        ...
        ***REMOVED*** Scheduled weight changes
    ...
    def add_schedule(self, schedule: TemporalSchedule) -> None:
        ...
        ***REMOVED*** Sort by priority (higher first)
...
class DynamicReweighter:
    ...
    def __init__(
        ...
        ***REMOVED*** State
        self.auto_adjust = True  ***REMOVED*** Whether to auto-apply feedback
        self.current_context = ContextType.NORMAL
    ...
    def set_context(self, context: ContextType) -> dict[str, float]:
        ...
        ***REMOVED*** Apply to current weights
    ...
    def get_weights(self) -> dict[str, float]:
        ...
        """
        ***REMOVED*** Start with temporal weights
        weights = self.temporal.get_current_weights()

        ***REMOVED*** Apply context
    ...
    def boost_objective(
        ...
        if objective in new_weights:
            ***REMOVED*** Multiply the target objective's weight
            new_weights[objective] *= factor

            ***REMOVED*** Normalize to sum to 1 (bypassing bounds for boost)
            total = sum(new_weights.values())
            if total > 0:
                new_weights = {k: v / total for k, v in new_weights.items()}

            if duration:
                ***REMOVED*** Schedule temporary boost
                ...
                ***REMOVED*** Permanent change - update adjuster directly
