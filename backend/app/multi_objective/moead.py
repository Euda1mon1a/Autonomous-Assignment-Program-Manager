...
class DecompositionType(Enum):
    ...
    PBI = "pbi"  ***REMOVED*** Penalty Boundary Intersection
...
class TchebycheffDecomposition(DecompositionMethod):
    ...
    def scalarize(
        ...
        ***REMOVED*** Avoid zero weights causing division issues
...
class PBIDecomposition(DecompositionMethod):
    ...
    def scalarize(
        ...
        ***REMOVED*** Vector from reference point to solution
        diff = objective_values - reference_point

        ***REMOVED*** Normalize weight vector
        ...
        ***REMOVED*** d1: projection onto weight direction (convergence)
        d1 = float(np.dot(diff, unit_weight))

        ***REMOVED*** d2: perpendicular distance (diversity)
...
def _uniform_weight_design(n_objectives: int, n_vectors: int) -> list[np.ndarray]:
    ...
    if n_objectives == 2:
        ***REMOVED*** Simple case: evenly spaced on line
        weights = []
    ...
    ***REMOVED*** General case: Das and Dennis method
    ***REMOVED*** Approximate divisions per axis
    H = 1
...
class MOEADConfig:
    ...
    delta: float = 0.9  ***REMOVED*** Probability to select from neighborhood
    nr: int = 2  ***REMOVED*** Maximum replacement number
...
class MOEADAlgorithm:
    ...
    Usage:
        ***REMOVED*** Configure objectives
        objectives = [coverage_obj, equity_obj, resilience_obj]

        ***REMOVED*** Create algorithm
    ...
    def _initialize_weights(self) -> None:
        ...
        ***REMOVED*** Generate weight vectors
        ...
        ***REMOVED*** Create WeightVector objects
        self.weight_vectors = [
            WeightVector(id=i, weights=w) for i, w in enumerate(raw_weights)
        ]

        ***REMOVED*** Compute neighborhoods based on Euclidean distance
        ...
        ***REMOVED*** Assign neighbors (T closest vectors)
        T = min(self.config.n_neighbors, n - 1)
        for i in range(n):
            ***REMOVED*** Get indices sorted by distance
            neighbor_indices = np.argsort(distances[i])
            ***REMOVED*** Skip self (distance 0)
            ...
            for wv in self.weight_vectors:
                ***REMOVED*** Select parents
                if np.random.random() < self.config.delta:
                    ...
                    ***REMOVED*** Select from entire population
                    parent_pool = [
                        w.current_solution
    ...
    def _initialize_population(
        ...
        ***REMOVED*** Evaluate initial solutions if needed
        evaluated = []
        ...
        ***REMOVED*** Assign solutions to weight vectors
        ***REMOVED*** Use simple distance-based assignment
        for sol in evaluated:
            ...
            ***REMOVED*** Add to archive
            self.archive.add(sol)

        ***REMOVED*** Fill remaining weight vectors with random copies
        if evaluated:
            for wv in self.weight_vectors:
    ...
    def _get_normalized_objectives(self, solution: Solution) -> np.ndarray:
        ...
        for obj in self.active_objectives:
            ...
            ***REMOVED*** Convert to minimization form
    ...
    def _update_neighbors(self, wv: WeightVector, offspring: Solution) -> None:
        ...
        ***REMOVED*** Count replacements
        replacements = 0

        ***REMOVED*** Try to update neighbors
        neighbor_indices = (
            wv.neighbors
            if np.random.random() < self.config.delta
    ...
    def get_balanced_solutions(self, top_k: int = 10) -> list[Solution]:
        ...
        ***REMOVED*** Calculate normalized scores for each solution
        scored = []
        for sol in self.archive.solutions:
            obj_vec = self._get_normalized_objectives(sol)
            ***REMOVED*** Use distance from ideal as balance metric
...
***REMOVED*** Convenience functions for common decomposition methods
