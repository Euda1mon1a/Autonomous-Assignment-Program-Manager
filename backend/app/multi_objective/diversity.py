...
class DiversityStats:
    ...
    min_distance: float
    max_distance: float
    uniformity: float  ***REMOVED*** 0-1, higher is more uniform
    coverage: float  ***REMOVED*** 0-1, how much of objective space is covered
    cluster_count: int  ***REMOVED*** Number of distinct clusters
...
class CrowdingDistance:
    ...
    def calculate(self, solutions: list[Solution]) -> None:
        ...
        ***REMOVED*** Reset distances
        for sol in solutions:
            sol.crowding_distance = 0.0

        ***REMOVED*** Calculate for each objective
        for obj in self.active_objectives:
            ***REMOVED*** Sort by this objective
            ...
            ***REMOVED*** Calculate objective range
            ...
            ***REMOVED*** Calculate crowding for intermediate solutions
    ...
    def tournament_select(
        ...
        ***REMOVED*** First compare by rank, then by crowding distance
...
class EpsilonDominance:
    ...
    def update_archive(
        ...
        Returns:
            Updated archive
        """
        ***REMOVED*** Check if new solution is epsilon-dominated
        for sol in archive:
            if self.epsilon_dominates(sol, new_solution):
                return archive  ***REMOVED*** Reject new solution

        ***REMOVED*** Remove solutions epsilon-dominated by new solution
        updated = [
            sol for sol in archive if not self.epsilon_dominates(new_solution, sol)
        ]

        ***REMOVED*** Check grid cell conflict
        ...
        if cell_conflict is not None:
            ***REMOVED*** Keep the one closer to grid corner (better)
    ...
    def _is_closer_to_corner(self, sol_a: Solution, sol_b: Solution) -> bool:
        ...
        for obj in self.active_objectives:
            ...
            ***REMOVED*** Distance from grid corner
    ...
    def prune_archive(
        ...
        ***REMOVED*** Increase epsilon until archive fits
        ...
        while len(current_archive) > max_size:
            ...
            ***REMOVED*** Rebuild archive with larger epsilon
    ...
    def _update_with_scaled_epsilon(
        ...
        ***REMOVED*** Temporary epsilon dominance with scaled values
...
class NichingOperator:
    ...
    def shared_fitness(
        ...
        for sol in solutions:
            ...
            niche = max(niche_counts.get(sol_id, 1.0), 1.0)  ***REMOVED*** Avoid division by zero
...
class ReferencePointAssociation:
    ...
    def __init__(
        ...
        ***REMOVED*** Generate reference points
    ...
    def _generate_reference_points(self, n: int) -> np.ndarray:
        ...
        ***REMOVED*** Das and Dennis's systematic approach
    ...
    def associate(
        ...
        for sol in solutions:
            ***REMOVED*** Get normalized objective vector
            obj_vec = self._get_normalized_vector(sol)

            ***REMOVED*** Find closest reference point
            min_dist = float("inf")
            closest_ref = 0
    ...
    def _perpendicular_distance(
        ...
        ***REMOVED*** Reference line goes from origin through reference point
        ...
        ***REMOVED*** Projection of point onto reference line
        projection = np.dot(point, unit_ref)

        ***REMOVED*** Perpendicular distance
    ...
    def niching_selection(
        ...
        ***REMOVED*** Track niche counts
        niche_counts = dict.fromkeys(range(len(self.reference_points)), 0)

        while len(selected) < n_select:
            ***REMOVED*** Find reference point with minimum count
            ...
            ***REMOVED*** Randomly select from candidates
            ...
            ***REMOVED*** Select closest to reference point
            best = min(
                candidates,
...
class DiversityMechanism:
    ...
    def calculate_diversity(self, solutions: list[Solution]) -> None:
        ...
        if self.primary_metric == DiversityMetric.CROWDING_DISTANCE:
            ...
            ***REMOVED*** Set diversity as grid distance to neighbors
            for sol in solutions:
            ...
            for ref_idx, assoc_sols in associations.items():
                for sol in assoc_sols:
                    ***REMOVED*** Diversity based on number sharing reference point
    ...
    def get_diversity_stats(self, solutions: list[Solution]) -> DiversityStats:
        ...
        ***REMOVED*** Calculate uniformity (inverse of distance variance)
        mean_dist = np.mean(distances)
        std_dist = np.std(distances)
        uniformity = 1.0 / (1.0 + std_dist / max(mean_dist, 1e-10))

        ***REMOVED*** Estimate coverage using grid cells
        grid_locations = set(self.epsilon_dom.get_grid_location(s) for s in solutions)
        ***REMOVED*** Estimate total possible cells (crude approximation)
        max_cells = max(1, len(solutions) * 10)
        coverage = len(grid_locations) / max_cells

        ***REMOVED*** Estimate cluster count using grid adjacency
    ...
    def _count_clusters(self, solutions: list[Solution]) -> int:
        ...
        for i in range(len(solutions)):
            ...
            ***REMOVED*** BFS from this solution
            clusters += 1
            queue = [i]
            while queue:
                current = queue.pop(0)
