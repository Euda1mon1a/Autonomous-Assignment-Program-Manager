...
class WeightedSum(PreferenceMethod):
    ...
    def score(
        ...
        for obj in objectives:
            ...
            ***REMOVED*** Normalize to minimization
...
class AchievementScalarizing(PreferenceMethod):
    ...
    def score(
        ...
        for obj in objectives:
            ...
            ***REMOVED*** Normalize to minimization form
            if obj.direction == ObjectiveDirection.MAXIMIZE:
                value = -value
                target = -target
...
class WierzbickiASF(PreferenceMethod):
    ...
    def score(
        ...
        for obj in objectives:
            ...
            ***REMOVED*** Calculate deviation
            deviation = value - aspiration

            ***REMOVED*** If reservation point exists, use relative scaling
            if self.reservation:
...
class LexicographicOrdering(PreferenceMethod):
    ...
    def score(
        ...
        for name in self.priority_order:
            ...
            ***REMOVED*** Normalize to [0, 1] range if possible
            if obj.reference_point is not None and obj.nadir_point is not None:
                value = obj.normalize(value)

            ***REMOVED*** Convert to minimization
            if obj.direction == ObjectiveDirection.MAXIMIZE:
                value = -value

            score += multiplier * value
            multiplier *= 0.001  ***REMOVED*** Decrease weight for lower priority
...
class LightBeamSearch(PreferenceMethod):
    ...
    def score(
        ...
        ***REMOVED*** Vector from ideal to solution
        to_solution = f - z

        ***REMOVED*** Projection onto beam
        ...
        ***REMOVED*** Combine: distance along beam + penalty for being off-beam
...
class InteractivePreferenceElicitor:
    ...
    def start_elicitation(
        ...
        ***REMOVED*** Select diverse representatives
    ...
    def _process_rating_feedback(
        ...
        ratings = feedback.get("ratings", {})  ***REMOVED*** {solution_id: rating}

        if not ratings:
            return self._select_representatives(frontier, self.initial_samples)

        ***REMOVED*** Find best-rated solution
        ...
        if best_solution:
            ...
            ***REMOVED*** Update reference point based on best solution
            ...
            ***REMOVED*** Find solutions near the best one
    ...
    def _process_comparison_feedback(
        ...
        ***REMOVED*** Learn weights from comparisons using simple heuristic
        ***REMOVED*** Each comparison (A > B) suggests objectives where A is better matter more
        ...
        for comp in comparisons:
            ...
            if preferred and other:
                ***REMOVED*** Increase weight for objectives where preferred is better
        ...
        ***REMOVED*** Use weighted sum to find new representatives
    ...
    def _process_reference_feedback(
        ...
        ***REMOVED*** Find solutions closest to reference
    ...
    def _process_bounds_feedback(
        ...
        bounds = feedback.get("bounds", {})  ***REMOVED*** {obj_name: (min, max)}

        ***REMOVED*** Filter solutions within bounds
        valid_solutions = []
        ...
        if not valid_solutions:
            ***REMOVED*** No solutions in bounds - return closest
            return self._select_representatives(frontier, self.initial_samples)

        ***REMOVED*** Return diverse subset of valid solutions
    ...
    def _process_classification_feedback(
        ...
        ***REMOVED*** Build reference based on classifications
        reference_values = {}
        for obj in self.active_objectives:
            ...
            if classification == "improve":
                ***REMOVED*** Target better than current
                if obj.direction == ObjectiveDirection.MAXIMIZE:
                ...
                ***REMOVED*** Allow worse than current
                if obj.direction == ObjectiveDirection.MAXIMIZE:
                    reference_values[obj.name] = current_val * 0.9
                else:
                    reference_values[obj.name] = current_val * 1.1

            else:  ***REMOVED*** accept or free
    ...
    def _find_neighborhood(
        ...
        ***REMOVED*** Return closest n solutions plus some diversity
        close = [s for _, s in distances[: n // 2]]
        remaining = [s for _, s in distances[n // 2 :]]

        ***REMOVED*** Add some diverse ones
        if remaining:
    ...
    def _solution_distance(self, sol1: Solution, sol2: Solution) -> float:
        ...
        for obj in self.active_objectives:
            ...
            ***REMOVED*** Normalize using reference and nadir if available
            if obj.reference_point is not None and obj.nadir_point is not None:
...
class PreferenceArticulator:
    ...
    def select_best(
        ...
        if not self.method:
            ***REMOVED*** Default: return knee and extremes
