...
class CanaryState(str, Enum):
    ...
    """
    PREPARING = "preparing"  ***REMOVED*** Initial setup, pre-flight checks
    RAMPING = "ramping"  ***REMOVED*** Gradually increasing traffic
    MONITORING = "monitoring"  ***REMOVED*** At target traffic, observing metrics
    ROLLING_BACK = "rolling_back"  ***REMOVED*** Reverting to baseline
    ROLLED_BACK = "rolled_back"  ***REMOVED*** Rollback complete
    COMPLETE = "complete"  ***REMOVED*** Successfully deployed
    PAUSED = "paused"  ***REMOVED*** Manually paused for investigation


class CanaryOutcome(str, Enum):
    """Final outcome of a canary release."""
    SUCCESS = "success"  ***REMOVED*** Canary passed all checks
    AUTOMATIC_ROLLBACK = "automatic_rollback"  ***REMOVED*** Auto-rolled back due to metrics
    MANUAL_ROLLBACK = "manual_rollback"  ***REMOVED*** Manually rolled back
    TIMEOUT = "timeout"  ***REMOVED*** Release exceeded max duration
    ERROR = "error"  ***REMOVED*** System error during release
    IN_PROGRESS = "in_progress"  ***REMOVED*** Still ongoing


class RollbackReason(str, Enum):
    """Reason for rolling back a canary release."""
    ERROR_RATE_SPIKE = "error_rate_spike"  ***REMOVED*** Error rate exceeded threshold
    LATENCY_DEGRADATION = "latency_degradation"  ***REMOVED*** Response time increased
    AVAILABILITY_DROP = "availability_drop"  ***REMOVED*** Service availability decreased
    MANUAL = "manual"  ***REMOVED*** Manual intervention
    TIMEOUT = "timeout"  ***REMOVED*** Release took too long
    METRIC_COMPARISON_FAILED = "metric_comparison_failed"  ***REMOVED*** A/B test failed
    HEALTHCHECK_FAILED = "healthcheck_failed"  ***REMOVED*** Health check failed
    EXCEPTION_RATE = "exception_rate"  ***REMOVED*** Too many exceptions
...
class UserSegment:
    ...
    ***REMOVED*** Segment criteria
    user_ids: list[UUID] = field(default_factory=list)  ***REMOVED*** Specific users
    user_roles: list[str] = field(default_factory=list)  ***REMOVED*** By role (e.g., "admin")
    organizations: list[UUID] = field(default_factory=list)  ***REMOVED*** By organization
    percentage: float = 0.0  ***REMOVED*** Random percentage of all users

    ***REMOVED*** Geographic or time-based
    regions: list[str] = field(default_factory=list)  ***REMOVED*** Geographic regions
    time_zones: list[str] = field(default_factory=list)  ***REMOVED*** Time zones

    ***REMOVED*** Feature flags
    required_flags: dict[str, bool] = field(default_factory=dict)  ***REMOVED*** Feature flags
    ...
    ***REMOVED*** Metadata
    ...
    def matches(self, user_id: UUID, user_role: str = None,
        ...
        ***REMOVED*** Check specific user IDs
        ...
        ***REMOVED*** Check organization
        if self.organizations and organization_id in self.organizations:
            return True

        ***REMOVED*** Check percentage-based assignment
...
class TrafficSplit:
    ...
    """
    canary_percentage: float = 0.0  ***REMOVED*** 0.0 to 100.0
    baseline_percentage: float = 100.0  ***REMOVED*** Should be (100 - canary_percentage)

    ***REMOVED*** Ramp-up schedule
    ramp_increment: float = 10.0  ***REMOVED*** Increase by this much each step
    ramp_interval_minutes: int = 15  ***REMOVED*** Wait this long between increases
    target_percentage: float = 100.0  ***REMOVED*** Final target percentage

    ***REMOVED*** Safety limits
    max_ramp_percentage: float = 50.0  ***REMOVED*** Don't go above this during ramp

    def __post_init__(self):
        """Validate percentages sum to 100."""
        total = self.canary_percentage + self.baseline_percentage
        if not (99.9 <= total <= 100.1):  ***REMOVED*** Allow small floating point error
            raise ValueError(
...
class CanaryMetrics:
    ...
    ***REMOVED*** Latency metrics (milliseconds)
    ...
    ***REMOVED*** Availability metrics
    canary_availability: float = 100.0  ***REMOVED*** Percentage
    baseline_availability: float = 100.0

    ***REMOVED*** Exception metrics
...
class MetricComparison:
    ...
    threshold: float  ***REMOVED*** Maximum acceptable difference
...
class CanaryConfig:
    ...
    ***REMOVED*** Release identification
    release_name: str = ""
    canary_version: str = ""
    baseline_version: str = ""

    ***REMOVED*** Traffic control
    initial_traffic_percentage: float = 5.0  ***REMOVED*** Start with 5%
    target_traffic_percentage: float = 100.0
    ramp_increment: float = 10.0  ***REMOVED*** Increase by 10% each step
    ramp_interval_minutes: int = 15  ***REMOVED*** Wait 15 min between increases

    ***REMOVED*** Metric thresholds for automatic rollback
    max_error_rate_increase: float = 2.0  ***REMOVED*** Max 2% increase in error rate
    max_latency_increase_percentage: float = 20.0  ***REMOVED*** Max 20% latency increase
    max_availability_decrease: float = 1.0  ***REMOVED*** Max 1% availability decrease
    max_exception_rate: float = 5.0  ***REMOVED*** Max 5% exception rate

    ***REMOVED*** Timing
    minimum_observation_minutes: int = 30  ***REMOVED*** Observe at each level for 30 min
    maximum_release_duration_hours: int = 24  ***REMOVED*** Auto-rollback after 24 hours

    ***REMOVED*** User segmentation
    target_segments: list[UserSegment] = field(default_factory=list)

    ***REMOVED*** Automatic rollback
...
class CanaryRelease:
    ...
    ***REMOVED*** Event log
    ...
    def can_ramp_up(self) -> bool:
        ...
        ***REMOVED*** Check minimum observation time
...
class CanaryReleaseManager:
    ...
    Usage:
        ...
        ***REMOVED*** Check health and potentially ramp up
    ...
    def record_request(
        ...
        if is_canary:
            ...
            ***REMOVED*** Update latency percentiles (simplified - use proper percentile tracking)
    ...
    def collect_metrics(self, release_id: UUID) -> CanaryMetrics:
        ...
        ***REMOVED*** Call registered metric collectors
        ...
        ***REMOVED*** Save current metrics to history
    ...
    def check_rollback_conditions(self, release_id: UUID) -> tuple[bool, RollbackReason | None]:
        ...
        ***REMOVED*** Check latency degradation
        ...
        ***REMOVED*** Check availability drop
        ...
        ***REMOVED*** Check metric comparisons
    ...
    def rollback(
        ...
        ***REMOVED*** Set traffic to 100% baseline
    ...
    def ramp_up(self, release_id: UUID) -> bool:
        ...
        ***REMOVED*** Check if we've reached target
    ...
    def check_and_ramp(self, release_id: UUID) -> dict[str, Any]:
        ...
        ***REMOVED*** Compare metrics
        comparisons = self.compare_metrics(release_id)

        ***REMOVED*** Check rollback conditions
        ...
        ***REMOVED*** Check if we can ramp up
        if release.can_ramp_up():
            ***REMOVED*** Verify minimum observation time
            ...
            ***REMOVED*** All checks passed, ramp up
        ...
        ***REMOVED*** Check if monitoring is complete
