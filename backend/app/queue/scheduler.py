...
class TaskScheduler:
    ...
    def schedule_task(
        ...
        Example:
            ...
            >>> ***REMOVED*** Execute in 5 minutes
            >>> task_id = scheduler.schedule_task(
            ...     "app.tasks.send_notification",
            ...     args=(user_id,),
            ...     countdown=300,
            ... )
            >>> ***REMOVED*** Execute at specific time
    ...
    def add_periodic_task(
        ...
        Example:
            ...
            ...     schedule_config={
            ...         "interval": {"every": 300}  ***REMOVED*** seconds
            ...     },
            ... )
        """
        kwargs = kwargs or {}
        ...
        ***REMOVED*** Parse schedule configuration
        task_schedule = self._parse_schedule_config(schedule_config)

        ***REMOVED*** Add to beat schedule
    ...
    def reschedule_task(
        ...
        ***REMOVED*** Cancel old task
        self.cancel_scheduled_task(task_id)

        ***REMOVED*** Get task details
        task_name = result.name
        if not task_name:
            raise ValueError(f"Task {task_id} not found or has no name")

        ***REMOVED*** Create new scheduled task
        ***REMOVED*** Note: We lose the original args/kwargs here
        ***REMOVED*** In production, you'd want to store these in a database
        task = self.app.tasks.get(task_name)
        if not task:
            raise ValueError(f"Task {task_name} not found in registry")
    ...
    def enable_periodic_task(self, name: str) -> bool:
        ...
        ***REMOVED*** Remove 'enabled': False if present
