...
class QueueManager:
    ...
    ***REMOVED*** Queue names by priority
    QUEUE_PRIORITY_MAP = {
    ...
    ***REMOVED*** Dead letter queue
    ...
    def submit_task(
        ...
        ***REMOVED*** Determine queue based on priority if not specified
        if queue is None:
            queue = self.QUEUE_PRIORITY_MAP.get(priority, "default")

        ***REMOVED*** Get task from registry
    ...
    def submit_task_with_dependencies(
        ...
        if not dependencies:
            ***REMOVED*** No dependencies, submit normally
            return self.submit_task(task_name, args, kwargs, priority)

        ***REMOVED*** Wait for all dependencies to complete
        ***REMOVED*** Use chord: (group of dependencies) | callback task
        task = self.app.tasks.get(task_name)
        if not task:
            raise ValueError(f"Task {task_name} not found in registry")

        ***REMOVED*** Create dependency group (empty tasks that just return AsyncResult)
        dep_results = [AsyncResult(dep_id, app=self.app) for dep_id in dependencies]

        ***REMOVED*** Create a chord: wait for all dependencies, then execute task
    ...
    def get_task_status(self, task_id: str) -> dict[str, Any]:
        ...
        ***REMOVED*** Add task info if available
    ...
    def send_to_dead_letter_queue(
        ...
        ***REMOVED*** Store in persistent dead letter queue
        self.store_in_dead_letter_queue(
            ...
            error=error,
            retry_count=0,  ***REMOVED*** Tasks reaching DLQ have exhausted retries
        )
    ...
    def store_in_dead_letter_queue(
        ...
        try:
            ...
            ***REMOVED*** Try Redis first (using synchronous client)
            try:
                import redis
            ...
            ***REMOVED*** Fallback to file storage
