...
class ExportSchedulerService:
    ...
    async def create_job(self, job_data: dict[str, Any], created_by: str) -> ExportJob:
        ...
        ***REMOVED*** Calculate next run time if scheduled
    ...
    async def update_job(
        ...
        ***REMOVED*** Recalculate next run time if schedule changed
    ...
    async def execute_job(
        ...
        ***REMOVED*** Create execution record
        ...
        try:
            ...
            ***REMOVED*** Determine content type
            content_types = {
            ...
            ***REMOVED*** Update execution with error
            execution.finished_at = datetime.utcnow()
            execution.runtime_seconds = int(
                (execution.finished_at - execution.started_at).total_seconds()
            )
    ...
    async def _generate_export_data(
        ...
        """
        ***REMOVED*** Get data based on template
        data, headers = await self._get_template_data(template, filters, columns)

        ***REMOVED*** Format data based on format
    ...
    def _calculate_next_run_time(self, cron_expression: str) -> datetime:
        ...
        try:
            ...
            ***REMOVED*** Default to 1 day from now
