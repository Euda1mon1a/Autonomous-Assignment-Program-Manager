...
class BackupScheduler:
    ...
    def get_next_scheduled(self) -> dict[str, Any] | None:
        ...
        ***REMOVED*** Check weekly backup
        if self.config.get("weekly_backup", {}).get("enabled"):
            ...
            ***REMOVED*** Calculate next occurrence of target day
            ...
            ***REMOVED*** If it's today but time has passed, schedule for next week
            if next_weekly <= now:
        ...
        ***REMOVED*** Return the soonest scheduled backup
    ...
    def run_pending_backups(self) -> list[dict[str, Any]]:
        ...
        ***REMOVED*** Check and run daily backup
        ...
        ***REMOVED*** Check and run weekly backup
        ...
        ***REMOVED*** Apply retention policy after backups
        if results:
            ...
            try:
                ...
                ***REMOVED*** Don't fail the whole operation if retention policy fails
    ...
    ***REMOVED*** Private helper methods
    ...
    def _should_run_backup(self, schedule: dict[str, Any], now: datetime) -> bool:
        ...
        if schedule["type"] == "daily":
            ***REMOVED*** Run if last run was on a different day and current time >= scheduled time
            if last_run.date() < now.date() and now.time() >= backup_time:
                return True

        elif schedule["type"] == "weekly":
            ***REMOVED*** Run if it's the scheduled day, time is right, and hasn't run this week
    ...
    def _apply_retention_policy(self) -> dict[str, Any]:
        ...
        ***REMOVED*** Apply time-based retention
        if keep_days:
        ...
        ***REMOVED*** Apply count-based retention
        if keep_count:
            backups = self.backup_service.list_backups()  ***REMOVED*** Refresh after time-based deletion
            if len(backups) > keep_count:
                ***REMOVED*** Keep the most recent backups
