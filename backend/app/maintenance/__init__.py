...
def quick_restore(
    ...
    Example:
        >>> from app.db.session import SessionLocal
        >>> db = SessionLocal()
        >>> ***REMOVED*** Preview restore
        >>> preview = quick_restore(db, backup_id, dry_run=True)
        >>> ***REMOVED*** Actually restore
        >>> result = quick_restore(db, backup_id)
        >>> if result['success']:
...
def setup_daily_backup(
    ...
    Example:
        >>> from app.db.session import SessionLocal
        >>> db = SessionLocal()
        >>> ***REMOVED*** Schedule daily backup at 3 AM
...
def setup_weekly_backup(
    ...
    Example:
        >>> from app.db.session import SessionLocal
        >>> db = SessionLocal()
        >>> ***REMOVED*** Schedule weekly backup on Sunday at 2 AM
