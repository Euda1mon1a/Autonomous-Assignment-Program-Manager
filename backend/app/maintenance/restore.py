...
***REMOVED*** Import custom exceptions
...
class RestoreService:
    ...
    def restore_from_backup(
        ...
        ***REMOVED*** Validate mode
        ...
        try:
            ***REMOVED*** Load backup data
            backup_data = self._load_backup(backup_id)

            ***REMOVED*** Validate before restoring
            ...
            if not validation['valid']:
                ...
                raise RestoreValidationError(
                    f"Backup validation failed: {error_msg}"
                )
            ...
            ***REMOVED*** Create restore point for rollback
            ...
            ***REMOVED*** Save restore metadata
            ...
            raise RestoreError(
                f"Database error during restore: {e}"
            ) from e
    ...
    def restore_schedule_range(
        ...
        ***REMOVED*** Validate date range
        ...
        try:
            ...
            ***REMOVED*** Filter data by date range
            logger.debug(f"Filtering backup data for date range {start_date} to {end_date}")
            filtered_blocks = [
            ...
            ***REMOVED*** Create filtered backup data
            filtered_data = {
                **backup_data,
            ...
            ***REMOVED*** Delete existing data in range
            logger.debug(f"Deleting existing blocks in date range")
            self.db.query(Block).filter(
                and_(Block.start_date >= start_date, Block.start_date <= end_date)
            ).delete(synchronize_session=False)

            ***REMOVED*** Restore filtered data
    ...
    def validate_backup(self, backup_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Validate backup_id matches
            ...
            ***REMOVED*** Validate data structure
            validation_results = {
                "valid": True,
                "backup_id": backup_id,
    ...
    def rollback_restore(self, restore_id: str) -> dict[str, Any]:
        ...
        try:
            ***REMOVED*** Validate restore_id to prevent path traversal
            restore_id = validate_backup_id(restore_id)

            ***REMOVED*** Load restore metadata and rollback data
            restore_file = self.backup_path / f"restore_{restore_id}.json"

            ***REMOVED*** Validate path is within backup directory
            ...
            ***REMOVED*** Restore the rollback data
            ...
            raise RestoreRollbackError(
                f"Database error during rollback: {e}"
            ) from e
        except (OSError, IOError) as e:
            self.db.rollback()
            logger.error(f"File I/O error during rollback of restore {restore_id}: {e}")
            raise RestoreRollbackError(
                f"Failed to read restore metadata: {e}"
            ) from e
        except Exception as e:
            self.db.rollback()
            logger.error(f"Unexpected error during rollback of restore {restore_id}: {e}")
            raise RestoreRollbackError(
                f"Unexpected error during rollback: {e}"
            ) from e
    ...
    ***REMOVED*** Private helper methods
    ...
    def _load_backup(self, backup_id: str) -> dict[str, Any]:
        ...
        Raises:
            ...
            FileSecurityError: If path validation fails
        """
        ***REMOVED*** Validate backup_id to prevent path traversal
        backup_id = validate_backup_id(backup_id)

        metadata_file = self.backup_path / f"metadata_{backup_id}.json"

        ***REMOVED*** Validate metadata path is within backup directory
        ...
        ***REMOVED*** Validate backup file path is within backup directory
    ...
    def _replace_restore(self, backup_data: dict[str, Any]) -> dict[str, int]:
        ...
        """
        try:
            ***REMOVED*** Delete all existing data (in correct order due to FK constraints)
            ...
            ***REMOVED*** Restore all data
            ...
            raise RestoreDataError(
                f"Failed during replace restore: {e}"
            ) from e
    ...
    def _merge_restore(self, backup_data: dict[str, Any]) -> dict[str, int]:
        ...
        Raises:
            RestoreDataError: If data restore fails
        """
        ***REMOVED*** For merge, we restore data without deleting
    ...
    def _restore_data(self, backup_data: dict[str, Any]) -> dict[str, int]:
        ...
        try:
            ...
            ***REMOVED*** Restore in correct order (due to FK constraints)
            ...
            raise RestoreDataError(
                f"Database error during data restore: {e}"
            ) from e
        except Exception as e:
    ...
    def _prepare_model_data(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in data.items():
            if value is None:
                ...
                ***REMOVED*** Convert ISO date strings back to date/datetime objects
                if 'T' in value:
    ...
    def _create_restore_point(self, restore_id: str) -> dict[str, Any]:
        ...
        ***REMOVED*** Create a snapshot of current data
        return {
    ...
    def _save_restore_metadata(
        ...
        try:
            ...
            ***REMOVED*** Don't fail the restore if we can't save metadata
