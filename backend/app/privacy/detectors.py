...
Usage:
    ...
    ***REMOVED*** Returns: [PIIMatch(type=PIIType.EMAIL, value='john@example.com', start=13, end=31), ...]
"""
...
class PIIDetector:
    ...
    ***REMOVED*** Regex patterns for PII detection
    ...
    ***REMOVED*** Credit card pattern (basic Luhn algorithm validation can be added)
    CREDIT_CARD_PATTERN = re.compile(
        r'\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|6(?:011|5[0-9]{2})[0-9]{12}|(?:2131|1800|35\d{3})\d{11})\b'
    )

    ***REMOVED*** Medical record number pattern (MRN) - common formats
    MRN_PATTERN = re.compile(
        r'\b(?:MRN|mrn|medical\s*record)[\s:]*([A-Z0-9]{6,12})\b',
        re.IGNORECASE
    )

    ***REMOVED*** Date of birth patterns
    DOB_PATTERN = re.compile(
        r'\b(?:DOB|dob|date\s*of\s*birth)[\s:]*(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})\b',
        re.IGNORECASE
    )

    ***REMOVED*** Common name prefixes/titles for name detection
    NAME_PREFIXES = {
    ...
    def detect_all(self, text: str) -> list[PIIMatch]:
        ...
        ***REMOVED*** Detect custom patterns
        ...
        ***REMOVED*** Sort by position in text
    ...
    def detect_credit_cards(self, text: str) -> list[PIIMatch]:
        ...
        ***REMOVED*** Validate using Luhn algorithm
        validated_matches = []
    ...
    def detect_names(self, text: str) -> list[PIIMatch]:
        ...
        ***REMOVED*** Look for name prefixes followed by capitalized words
        pattern = re.compile(
        ...
        for match in pattern.finditer(text):
            matches.append(PIIMatch(
                ...
                confidence=0.7,  ***REMOVED*** Lower confidence for heuristic detection
    ...
    def _validate_luhn(self, card_number: str) -> bool:
        ...
        """
        ***REMOVED*** Remove spaces and dashes
        digits = ''.join(c for c in card_number if c.isdigit())

        if not digits:
            return False

        ***REMOVED*** Luhn algorithm
        checksum = 0
