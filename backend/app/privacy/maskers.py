...
Usage:
    ...
    ***REMOVED*** Returns: "j***@example.com" or similar based on strategy
"""
...
class PartialMasker(BaseMasker):
    ...
    def mask(self, value: str) -> str:
        ...
        if len(value) <= (self.show_start + self.show_end):
            ***REMOVED*** Value too short, mask entirely
        ...
        middle = self.mask_char * min(middle_length, 5)  ***REMOVED*** Cap asterisks for readability
...
class TokenMasker(BaseMasker):
    ...
    def __init__(
        ...
        self.prefix = prefix
        self._token_map: dict[str, str] = {}  ***REMOVED*** token -> original
        self._reverse_map: dict[str, str] = {}  ***REMOVED*** original -> token

    def mask(self, value: str) -> str:
        """Generate and return random token."""
        ***REMOVED*** Check if we've already tokenized this value
        if self.reversible and value in self._reverse_map:
            return self._reverse_map[value]

        ***REMOVED*** Generate new token
        ...
        ***REMOVED*** Store mapping if reversible
        if self.reversible:
...
class EmailMasker(BaseMasker):
    ...
    def mask(self, value: str) -> str:
        """Mask email address."""
        if '@' not in value:
            ***REMOVED*** Not a valid email, use basic masking
...
class PhoneMasker(BaseMasker):
    ...
    def mask(self, value: str) -> str:
        ...
        ***REMOVED*** Extract digits
        digits = ''.join(c for c in value if c.isdigit())

        if len(digits) <= self.show_last_digits:
            ***REMOVED*** Too short, mask entirely
            return re.sub(r'\d', '*', value)

        ***REMOVED*** Mask all but last N digits
...
class SSNMasker(BaseMasker):
    ...
    def mask(self, value: str) -> str:
        ...
        ***REMOVED*** Extract digits
        ...
        if len(digits) != 9:
            ***REMOVED*** Invalid SSN, use basic masking
            return "XXX-XX-XXXX"

        last_four = digits[-4:]

        ***REMOVED*** Preserve original format
        if '-' in value:
...
class MaskerFactory:
    ...
    def get_masker(self, pii_type: str, **kwargs) -> BaseMasker:
        ...
        ***REMOVED*** Handle callable factories
