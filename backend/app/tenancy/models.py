...
class TenantStatus(str, Enum):
    ...
    PENDING = "pending"  ***REMOVED*** Tenant created but not yet provisioned
    ACTIVE = "active"  ***REMOVED*** Tenant is active and operational
    SUSPENDED = "suspended"  ***REMOVED*** Tenant temporarily suspended (billing, violation)
    INACTIVE = "inactive"  ***REMOVED*** Tenant deprovisioned (soft delete)
...
class Tenant(Base):
    ...
    ***REMOVED*** Primary identification
    ...
    ***REMOVED*** Isolation configuration
    ...
    ***REMOVED*** Configuration and limits
    settings = Column(
    ...
    ***REMOVED*** Relationships (if we add user-tenant mapping)
    ***REMOVED*** users = relationship("TenantUser", back_populates="tenant")
...
class TenantUser(Base):
    ...
    ***REMOVED*** Role within this tenant (can differ from global role)
    role = Column(
        String(50), nullable=False, default="coordinator",
        doc="User's role within this tenant"
    )

    ***REMOVED*** Primary tenant flag
    is_primary = Column(
    ...
    ***REMOVED*** tenant = relationship("Tenant", back_populates="users")
    ***REMOVED*** user = relationship("User", back_populates="tenants")

    __table_args__ = (
        ***REMOVED*** Ensure user can only be in a tenant once
        CheckConstraint(
            "tenant_id IS NOT NULL AND user_id IS NOT NULL",
            name="check_tenant_user_not_null",
        ),
        ***REMOVED*** Note: We'd add a unique constraint on (tenant_id, user_id)
        ***REMOVED*** but that requires creating the constraint separately
    )
