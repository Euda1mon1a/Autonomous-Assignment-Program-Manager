...
***REMOVED*** Context variables for tenant tracking
***REMOVED*** These are async-safe and isolated per request/task
...
class TenantContext:
    ...
    Example:
        with TenantContext(tenant_id):
            ***REMOVED*** Code here runs with tenant_id set
            person = db.query(Person).first()  ***REMOVED*** Auto-filtered by tenant

        ***REMOVED*** Tenant context is cleared after the block
    """
...
async def require_tenant(db: Session = Depends(get_db)):
    ...
    Example:
        ...
        async def list_people(
            ...
            ***REMOVED*** tenant is guaranteed to be set
...
async def require_active_tenant(db: Session = Depends(get_db)):
    ...
    Example:
        ...
        async def create_schedule(
            ...
            ***REMOVED*** Tenant is guaranteed to be active
...
async def require_admin_for_cross_tenant(
    ...
    Example:
        ...
        async def list_all_schedules(
            ...
            ***REMOVED*** Can now query across all tenants
            set_current_tenant(bypass=True)
            ...
    """
    if not current_user_id or not db:
        ***REMOVED*** If no user ID or DB provided, we can't check permissions
        ***REMOVED*** Log a warning and deny access
...
async def check_user_permission(
    ...
    try:
        ...
        ***REMOVED*** Get user with their role
        ...
        ***REMOVED*** Check role-based permissions
        role_permissions = _get_role_permissions(user_obj.role)

        ***REMOVED*** Check if role has permission for this resource/action
...
def restore_tenant_from_celery_task(task_kwargs: dict) -> None:
    ...
    Example:
        ...
        def generate_schedule_task(**kwargs):
            ...
            ***REMOVED*** Now tenant context is set
            tenant_id = get_current_tenant_id()
            ...
    """
...
def propagate_tenant_to_tracing() -> None:
    ...
    Example:
        ***REMOVED*** In middleware, after setting tenant context
        set_current_tenant(tenant_id=tenant_id)
        propagate_tenant_to_tracing()

        ***REMOVED*** Now the tenant ID is included in all traces
    """
    try:
