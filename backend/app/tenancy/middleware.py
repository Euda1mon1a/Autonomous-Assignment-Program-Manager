...
class TenantIdentificationMethod:
    ...
    HEADER = "header"  ***REMOVED*** X-Tenant-ID or X-Tenant-Slug headers
    SUBDOMAIN = "subdomain"  ***REMOVED*** tenant.example.com
    PATH = "path"  ***REMOVED*** /tenants/{tenant_slug}/...
    QUERY = "query"  ***REMOVED*** ?tenant=tenant_slug
...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        Returns:
            HTTP response
        """
        ***REMOVED*** Skip tenant identification for excluded paths
        if request.url.path in self.excluded_paths:
            return await call_next(request)

        ***REMOVED*** Clear any previous tenant context (shouldn't happen, but defensive)
        clear_current_tenant()

        ***REMOVED*** Identify tenant from request
        tenant = await self._identify_tenant(request)

        if tenant:
            ***REMOVED*** Set tenant context
            set_current_tenant(tenant_id=tenant.id, tenant_slug=tenant.slug)

            ***REMOVED*** Propagate to distributed tracing
            propagate_tenant_to_tracing()

            ***REMOVED*** Add tenant info to request state (for logging, etc.)
            ...
            ***REMOVED*** Tenant required but not found
            logger.warning(f"No tenant identified for request: {request.url.path}")
            ***REMOVED*** Could return 403 here, but let the route handler decide
            ***REMOVED*** return Response(status_code=403, content="Tenant required")
        ...
        try:
            ...
            ***REMOVED*** Clear tenant context after request
            clear_current_tenant()
    ...
    async def _identify_tenant(self, request: Request) -> Optional[Tenant]:
        ...
        ***REMOVED*** Try identification based on method
    ...
    def _extract_from_headers(self, request: Request) -> tuple[Optional[UUID], Optional[str]]:
        ...
        ***REMOVED*** Try X-Tenant-ID header
        ...
        ***REMOVED*** Try X-Tenant-Slug header
    ...
    def _extract_from_subdomain(self, request: Request) -> Optional[str]:
        ...
        ***REMOVED*** Split host into parts
        parts = host.split(".")

        ***REMOVED*** If we have at least 3 parts (subdomain.domain.tld), use first as tenant
        if len(parts) >= 3:
            subdomain = parts[0]
            ***REMOVED*** Skip common subdomains
    ...
    def _extract_from_path(self, request: Request) -> Optional[str]:
        ...
        ***REMOVED*** Look for /tenants/{slug} or /t/{slug} pattern
    ...
    async def _lookup_tenant(
        ...
        ***REMOVED*** Create a database session for tenant lookup
        db = SessionLocal()
        try:
            query = select(Tenant)

            ***REMOVED*** Filter by ID or slug
            if tenant_id:
            ...
            ***REMOVED*** Check if tenant is active
