...
async def validate_rotation_eligibility(
    ...
    ***REMOVED*** PGY-1 orientation requirement
    if person.type == "resident" and person.pgy_level == 1:
        ***REMOVED*** Check if rotation is in first 2 weeks of academic year
        ***REMOVED*** (Simplified: would need academic year start date)
        ***REMOVED*** For now, just check if it's complex rotation
        if rotation_type in ["surgery", "ob_gyn", "emergency"]:
            ***REMOVED*** PGY-1 should complete orientation first
            ***REMOVED*** Would check person's rotation history here
            pass

    ***REMOVED*** Specialty matching
    ...
    ***REMOVED*** Procedure requirements
...
async def validate_workload_distribution(
    ...
    ***REMOVED*** Get average for comparison (would compare with peers here)
    ***REMOVED*** For now, simplified
    average_blocks = max_clinical_blocks * 0.7  ***REMOVED*** Assume 70% utilization is average
...
async def validate_call_equity(
    ...
    ***REMOVED*** Get all residents for comparison
    ...
    ***REMOVED*** Check if person has significantly more calls than average
...
async def validate_preference_satisfaction(
    ...
    ***REMOVED*** Would check against preferences/absences table here
    ***REMOVED*** Simplified for now

    conflicts = []
    warnings = []

    ***REMOVED*** Check for absence requests (would query absences table)
    ***REMOVED*** For now, simplified
...
def validate_business_rule(
    ...
    ***REMOVED*** Route to appropriate validator
    ***REMOVED*** (In production, would have a registry pattern)
