...
def validate_time_window(
    ...
    ***REMOVED*** Calculate duration
    duration_days = (validated_end - validated_start).days + 1  ***REMOVED*** Inclusive
...
def validate_sequence_order(
    ...
    ***REMOVED*** Check for gaps if not allowed
    if not allow_gaps:
        for i in range(len(dates) - 1):
            current = dates[i]
            next_date = dates[i + 1]
...
def validate_temporal_dependency(
    ...
    ***REMOVED*** Check maximum gap if specified
