...
async def validate_acgme_compliance(
    ...
    ***REMOVED*** Check 80-hour weekly limit
    ...
    ***REMOVED*** Check for approaching limits (warnings)
...
async def validate_institutional_policy(
    ...
    """
    ***REMOVED*** Define institutional policies
    policies = {
    ...
    ***REMOVED*** Validate based on policy type
...
async def validate_professional_standards(
    ...
    ***REMOVED*** Check certifications and credentials
    today = date.today()
    ...
    ***REMOVED*** Check for valid credentials
    ...
    ***REMOVED*** Check certifications from Certification model
...
***REMOVED*** =============================================================================
***REMOVED*** ACGME Compliance Helper Functions
***REMOVED*** =============================================================================
...
async def _check_80_hour_rule(
    ...
    """
    violations = []
    hours_per_block = 6  ***REMOVED*** 6 hours per half-day block
    max_weekly_hours = 80
    rolling_weeks = 4

    ***REMOVED*** Check each 4-week rolling window in the period
    current_start = start_date
    while current_start <= end_date - timedelta(weeks=rolling_weeks):
        window_end = current_start + timedelta(weeks=rolling_weeks)

        ***REMOVED*** Count assignments in this window
        result = await db.execute(
...
async def _check_day_off_rule(
    ...
    ***REMOVED*** Get all assignment dates in the period
    result = await db.execute(
        select(Block.date)
        .distinct()
    ...
    ***REMOVED*** Check for 7+ consecutive working days
    consecutive_count = 1
...
async def _check_approaching_hour_limit(
    ...
    max_weekly_hours = 80
    rolling_weeks = 4
    warning_threshold = 0.9  ***REMOVED*** 90%

    ***REMOVED*** Check current 4-week window
...
***REMOVED*** =============================================================================
***REMOVED*** Institutional Policy Helper Functions
***REMOVED*** =============================================================================
...
async def _check_max_consecutive_days(
    ...
    ***REMOVED*** Get all assignment dates
    result = await db.execute(
        select(Block.date)
        .distinct()
        .join(Assignment, Assignment.block_id == Block.id)
    ...
    ***REMOVED*** Check for consecutive days exceeding limit
    consecutive_count = 1
    consecutive_start = work_dates[0] if work_dates else None
...
async def _check_min_shift_gap(
    ...
    ***REMOVED*** For now, check if there are back-to-back shifts
    ***REMOVED*** This would need more detailed shift timing in a real implementation
    day_before = target_date - timedelta(days=1)
    day_after = target_date + timedelta(days=1)

    ***REMOVED*** Check for PM -> AM back-to-back
    result = await db.execute(
        select(func.count(Assignment.id))
        .join(Block, Assignment.block_id == Block.id)
        .where(
    ...
    ***REMOVED*** If multiple assignments on adjacent days, flag potential gap issue
    if adjacent_count >= 4:  ***REMOVED*** Both AM+PM on two consecutive days
        violations.append({
...
async def _check_annual_training(
    ...
    ***REMOVED*** Check certifications for required trainings
    result = await db.execute(
        select(Certification).where(Certification.person_id == person_id)
    )
    certifications = result.scalars().all()

    ***REMOVED*** Map certifications by name
    cert_map = {}
    for cert in certifications:
        name = getattr(cert, "name", None) or getattr(cert, "certification_type", "")
        cert_map[name] = cert

    ***REMOVED*** Check each required training
    for training in required_trainings:
        ...
        if not cert:
            ...
            ***REMOVED*** Check if completed within last year
