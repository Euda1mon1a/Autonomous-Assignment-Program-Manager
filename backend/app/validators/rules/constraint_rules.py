...
async def validate_hard_constraint(
    ...
    ***REMOVED*** Validate constraint based on type
    is_satisfied = True
...
async def validate_soft_constraint(
    ...
    ***REMOVED*** Calculate satisfaction based on constraint type
    satisfaction = 0.8  ***REMOVED*** Default satisfaction if we can't calculate
    ...
    return {
        "is_satisfied": satisfaction > 0.5,  ***REMOVED*** >50% satisfaction
        "constraint_name": constraint_name,
        "constraint_description": constraint["description"],
...
async def validate_constraint_priority(
    ...
    ***REMOVED*** Sort constraints by priority
    ...
    ***REMOVED*** Check for conflicts
    ...
    for i, c1 in enumerate(sorted_constraints):
        for c2 in sorted_constraints[i + 1 :]:
            ***REMOVED*** Would check if constraints conflict
            ***REMOVED*** (Simplified for now)
            pass
...
***REMOVED*** =============================================================================
***REMOVED*** Hard Constraint Helper Functions
***REMOVED*** =============================================================================
...
async def _check_no_double_booking(
    ...
    assignment_id = context.get("assignment_id")  ***REMOVED*** For updates, exclude self

    if not person_id or not block_id:
        return True, None

    ***REMOVED*** Check for existing assignment at this block
    ...
    if assignment_id:
        ***REMOVED*** Exclude the current assignment (for updates)
...
async def _check_required_credentials(
    ...
    if not procedure_id:
        ***REMOVED*** No specific procedure requirement
        return True, None

    ***REMOVED*** Check for valid credential
    today = date.today()
...
async def _check_minimum_supervision(
    ...
    ***REMOVED*** Get assignments for this block
    result = await db.execute(select(Assignment).where(Assignment.block_id == block_id))
    assignments = result.scalars().all()

    ***REMOVED*** Count residents and faculty by role/PGY level
    resident_count = 0
    faculty_count = 0

    for assignment in assignments:
        ***REMOVED*** Would check person.role in real implementation
        ***REMOVED*** For now, assume assignment has a role indicator
    ...
    ***REMOVED*** ACGME supervision ratios
    ***REMOVED*** PGY-1: 1 faculty per 2 residents
    ***REMOVED*** PGY-2+: 1 faculty per 4 residents
    if pgy_level == 1:
        required_ratio = 0.5  ***REMOVED*** 1:2
    else:
        required_ratio = 0.25  ***REMOVED*** 1:4
...
async def _check_acgme_duty_hours(
    ...
    ***REMOVED*** Calculate 4-week window
    window_start = target_date - timedelta(weeks=4)
    window_end = target_date

    ***REMOVED*** Count assignments in window
    result = await db.execute(
    ...
    ***REMOVED*** Calculate hours (6 hours per half-day block)
    hours_per_block = 6
    total_hours = assignment_count * hours_per_block

    ***REMOVED*** 80 hours/week * 4 weeks = 320 max
    max_hours = 80 * 4
...
***REMOVED*** =============================================================================
***REMOVED*** Soft Constraint Satisfaction Calculators
***REMOVED*** =============================================================================
...
async def _calculate_rotation_preference_satisfaction(
    ...
    if not person_id or not rotation_id:
        return 1.0  ***REMOVED*** No preference to check

    ***REMOVED*** Check if this rotation is preferred
    result = await db.execute(
    ...
    if not prefs:
        return 0.8  ***REMOVED*** No preferences set, neutral

    ***REMOVED*** For rotation preferences, we'd check preferred_rotations if that field existed
    ***REMOVED*** Since FacultyPreference uses preferred_weeks, return default
    return 0.8
...
async def _calculate_shift_preference_satisfaction(
    ...
    ***REMOVED*** Get faculty preferences
    ...
    if not prefs:
        return 0.8  ***REMOVED*** No preferences, neutral

    ***REMOVED*** Calculate week string
    week_start = target_date - timedelta(days=target_date.weekday())
    week_str = week_start.isoformat()

    ***REMOVED*** Check if this is a preferred week
    if prefs.preferred_weeks and week_str in prefs.preferred_weeks:
        return 1.0  ***REMOVED*** Preferred week = full satisfaction
    elif prefs.blocked_weeks and week_str in prefs.blocked_weeks:
        return 0.0  ***REMOVED*** Blocked week = no satisfaction
    else:
        return 0.7  ***REMOVED*** Neutral week
...
async def _calculate_workload_balance_satisfaction(
    ...
    ***REMOVED*** Calculate current workload for this person
    ...
    ***REMOVED*** Get average across all persons
    ...
    ***REMOVED*** Perfect balance = 1.0, more deviation = lower score
...
async def _calculate_call_equity_satisfaction(
    ...
    ***REMOVED*** For call equity, we'd typically check the distribution of call shifts
    ***REMOVED*** across all eligible persons over a period of time.
    ***REMOVED*** This requires knowing which assignments are "call" assignments.

    ***REMOVED*** For now, return a moderate satisfaction that indicates
    ***REMOVED*** call equity should be considered but not necessarily violated
    return 0.75
