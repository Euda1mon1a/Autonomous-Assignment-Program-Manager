...
***REMOVED*** Common credential types
REQUIRED_CREDENTIALS = {
    "annual_training": [
        "HIPAA",
        "Cyber_Training",
        "AUP",  ***REMOVED*** Acceptable Use Policy
    ],
    "safety": [
        "N95_Fit",
        "Chaperone",
        "BBP_Module",  ***REMOVED*** Bloodborne Pathogens
        "Sharps_Safety",
    ],
    "immunizations": [
        "Flu_Vax",
    ...
    "procedures": [
        "BLS",  ***REMOVED*** Basic Life Support
        "ACLS",  ***REMOVED*** Advanced Cardiac Life Support
        "PALS",  ***REMOVED*** Pediatric Advanced Life Support
        "NRP",  ***REMOVED*** Neonatal Resuscitation Program
    ],
}

***REMOVED*** Credential validity periods (in months)
CREDENTIAL_VALIDITY = {
    "HIPAA": 12,
    "Cyber_Training": 12,
    "AUP": 12,
    ...
    "Sharps_Safety": 12,
    "Flu_Vax": 12,
    "Tdap": 120,  ***REMOVED*** 10 years
    "Hep_B": None,  ***REMOVED*** Lifetime
    "BLS": 24,
    "ACLS": 24,
    "PALS": 24,
    "NRP": 24,
...
***REMOVED*** Slot type credential requirements
...
def validate_credential_expiration(
    ...
    ***REMOVED*** Check if expiration is unreasonably far in the future
    max_future = date.today() + timedelta(days=365 * 15)  ***REMOVED*** 15 years max
...
def validate_credential_issue_date(
    ...
    ***REMOVED*** Issue date should not be in the future
    today = date.today()
    if validated_date > today:
        raise ValidationError(
    ...
    ***REMOVED*** Issue date should not be too far in the past (> 20 years)
    ...
    ***REMOVED*** If expiration date provided, issue date must be before it
...
async def validate_person_has_credential(
    ...
    ***REMOVED*** Check if credential exists
    if not credential:
        return {
    ...
    ***REMOVED*** Check expiration if requested
...
async def validate_slot_type_credentials(
    ...
    ***REMOVED*** Check hard requirements
    ...
    ***REMOVED*** Check soft requirements (warnings only)
    ...
    ***REMOVED*** Get person
    ...
    return {
        ...
        "penalty_score": len(soft_warnings) * 3,  ***REMOVED*** 3 points per missing soft requirement
    }
...
async def validate_credential_update(
    ...
    ***REMOVED*** Get existing credential
    result = await db.execute(
        select(Certification).where(
            Certification.person_id == person_id,
    ...
    ***REMOVED*** Check if this is an extension of existing credential
    is_renewal = False
    if existing and existing.expires_at:
        ***REMOVED*** Should be close to expiration or within renewal window
        renewal_window_start = existing.expires_at - timedelta(days=90)  ***REMOVED*** 90 days before
        today = date.today()
