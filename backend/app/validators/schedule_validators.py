...
async def validate_no_duplicate_assignment(
    ...
    ***REMOVED*** Exclude specific assignment (for updates)
...
async def validate_supervision_ratio(
    ...
    ***REMOVED*** Count residents and faculty
    pgy1_count = 0
    pgy23_count = 0
    faculty_count = 0
    ...
    ***REMOVED*** If adding new resident, increment counts
    if new_resident_id:
    ...
    ***REMOVED*** PGY-1: need 1 faculty per 2 residents
    required_faculty_pgy1 = (pgy1_count + 1) // 2  ***REMOVED*** Ceiling division

    ***REMOVED*** PGY-2/3: need 1 faculty per 4 residents
    required_faculty_pgy23 = (pgy23_count + 3) // 4  ***REMOVED*** Ceiling division
...
async def validate_weekly_hours_limit(
    ...
    ***REMOVED*** Only check for residents
    if person.type != "resident":
        return

    ***REMOVED*** Check 7 days before and after
    start_date = check_date - timedelta(days=7)
    end_date = check_date + timedelta(days=7)

    ***REMOVED*** Get all assignments in this period
    result = await db.execute(
        select(Assignment)
        .join(Block)
        .where(
    ...
    ***REMOVED*** Calculate hours per week (rough estimate: 12 hours per block)
    ***REMOVED*** In production, this would use actual shift hours from rotation templates
    hours_per_block = 12.0
    total_hours = len(assignments) * hours_per_block + additional_hours

    ***REMOVED*** Check rolling 7-day periods
    weeks_checked = 0
    for i in range(8):  ***REMOVED*** Check 8 possible 7-day windows
        week_start = start_date + timedelta(days=i)
        week_end = week_start + timedelta(days=6)

        ***REMOVED*** Count blocks in this week
        week_blocks = sum(
...
async def validate_one_in_seven_rule(
    ...
    ***REMOVED*** Check 7 days before
    start_date = check_date - timedelta(days=6)
    end_date = check_date

    ***REMOVED*** Get all dates with assignments in this period
    result = await db.execute(
        select(Block.date)
        .join(Assignment)
        .where(
    ...
    ***REMOVED*** Add the new date
    assigned_dates.add(check_date)

    ***REMOVED*** Check if person has all 7 days assigned
...
def validate_assignment_score(score: float | None) -> float | None:
    ...
    ***REMOVED*** Allow negative scores (penalties) but check for unreasonable values
