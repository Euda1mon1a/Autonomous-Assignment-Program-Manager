...
***REMOVED*** Valid person types
VALID_PERSON_TYPES = ["resident", "faculty"]

***REMOVED*** Valid faculty roles
VALID_FACULTY_ROLES = ["pd", "apd", "oic", "dept_chief", "sports_med", "core"]

***REMOVED*** Valid PGY levels
MIN_PGY_LEVEL = 1
MAX_PGY_LEVEL = 3

***REMOVED*** Common medical specialties
COMMON_SPECIALTIES = [
    "Family Medicine",
    "Sports Medicine",
    "Dermatology",
...
def validate_faculty_role(faculty_role: str | None, person_type: str) -> str | None:
    ...
    ***REMOVED*** Residents should not have faculty role
    if person_type_lower == "resident":
        if faculty_role is not None:
            raise ValidationError("Residents cannot have a faculty role")
        return None

    ***REMOVED*** Faculty role is optional for faculty
...
def validate_target_clinical_blocks(
    ...
    ***REMOVED*** Basic range validation
    validated = validate_integer_range(
        blocks,
        min_value=0,
        max_value=365 * 2,  ***REMOVED*** Max 2 blocks per day for whole year
        field_name="Target clinical blocks"
    )

    ***REMOVED*** Warn if outside typical ranges for residents
    if person_type.lower() == "resident" and pgy_level is not None:
        if validated < 4:
            ***REMOVED*** Less than 1 week is suspicious
            raise ValidationError(
        ...
        if validated > 200:
            ***REMOVED*** More than 50 weeks is suspicious
            raise ValidationError(
...
def validate_supervision_requirements(
    ...
    if person_type_lower == "resident":
        ...
        ***REMOVED*** PGY-1 requires closer supervision
...
def validate_call_counts(
    ...
    """
    ***REMOVED*** Default to 0 if not provided
    ...
    ***REMOVED*** FMIT typically capped around 6 per year
    if fmit > 12:
        raise ValidationError(
...
def validate_primary_duty(primary_duty: str | None, person_type: str) -> str | None:
    ...
    ***REMOVED*** Validate length and content
