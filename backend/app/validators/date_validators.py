...
def validate_academic_year_dates(start_date: date, end_date: date) -> tuple[date, date]:
    ...
    ***REMOVED*** Calculate days
    days_diff = (end - start).days + 1  ***REMOVED*** +1 to include both start and end

    ***REMOVED*** Should be 365 or 366 days
...
def validate_acgme_work_period(
    ...
    start, end = validate_date_range(
        ...
        end_date,
        allow_same_day=True,
        max_days=28,  ***REMOVED*** Maximum rolling period
        start_field_name="ACGME period start",
        end_field_name="ACGME period end"
    )

    days = (end - start).days + 1  ***REMOVED*** +1 to include both days

    ***REMOVED*** Common valid periods:
    ***REMOVED*** - 1 day: single day check
    ***REMOVED*** - 7 days: weekly check (1-in-7 rule)
    ***REMOVED*** - 28 days: 4-week rolling average (80-hour rule)

    ***REMOVED*** Allow 1-28 days, but warn if unusual
    if days not in (1, 7, 28):
        ***REMOVED*** This is just informational - still allow the period
        pass
...
def validate_week_number(week: int, year: int | None = None) -> int:
    ...
    ***REMOVED*** If year provided, check it's valid for that year
    if year is not None:
        ***REMOVED*** Some years have 52 weeks, some have 53
        ***REMOVED*** ISO week date system
