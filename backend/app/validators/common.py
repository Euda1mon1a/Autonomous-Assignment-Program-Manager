...
def validate_email_address(email: str) -> str:
    ...
    try:
        ***REMOVED*** Pydantic v2 validation
        validated = validate_email(email)
        ***REMOVED*** Return normalized email (lowercase)
...
def validate_phone_number(phone: str, allow_international: bool = True) -> str:
    ...
    ***REMOVED*** Remove common formatting characters
    digits_only = re.sub(r"[\s\-\(\)\+\.]", "", phone)

    ***REMOVED*** Check if contains only digits (and optionally leading +)
    if not re.match(
        r"^\+?\d+$",
        phone.replace(" ", "")
        .replace("-", "")
    ...
    ***REMOVED*** US phone number: 10 digits
    if len(digits_only) == 10:
        return digits_only

    ***REMOVED*** US with country code: 11 digits (1 + 10)
    if len(digits_only) == 11 and digits_only.startswith("1"):
        return digits_only

    ***REMOVED*** International numbers (allow if enabled)
...
def validate_name(name: str, min_length: int = 1, max_length: int = 255) -> str:
    ...
    ***REMOVED*** Allow letters (including unicode), spaces, hyphens, apostrophes, and periods
    ***REMOVED*** This supports names like "O'Brien", "Jean-Pierre", "Dr. Smith", "José María"
    ...
    ***REMOVED*** Check for suspicious SQL patterns (defense in depth)
    sql_patterns = [
        r"(;|\-\-|\/\*|\*\/)",  ***REMOVED*** SQL comment markers
        r"(union|select|insert|update|delete|drop|create|alter)\s",  ***REMOVED*** SQL keywords
        r"(\bor\b|\band\b)\s*[\'\"0-9]",  ***REMOVED*** OR/AND with quotes or numbers
    ]
...
def validate_military_id(military_id: str) -> str:
    ...
    ***REMOVED*** Remove spaces and hyphens
    clean_id = military_id.replace(" ", "").replace("-", "")

    ***REMOVED*** Must be exactly 10 digits
