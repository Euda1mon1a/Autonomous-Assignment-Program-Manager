...
def sanitize_html(text: str, allow_safe_tags: bool = False) -> str:
    ...
    Examples:
        ...
        '&lt;script&gt;alert(&***REMOVED***x27;xss&***REMOVED***x27;)&lt;/script&gt;'
    ...
    ***REMOVED*** Escape all HTML by default
    sanitized = html.escape(text)

    ***REMOVED*** If allowing safe tags, unescape only those
    if allow_safe_tags:
...
def sanitize_sql_like_pattern(pattern: str) -> str:
    ...
    ***REMOVED*** Escape SQL LIKE wildcards and special characters
    sanitized = pattern.replace("\\", "\\\\")  ***REMOVED*** Escape backslash first
...
def sanitize_path(path_str: str, allow_absolute: bool = False) -> str:
    ...
    ***REMOVED*** Convert to Path for normalization
    try:
        path = Path(sanitized)
    ...
    ***REMOVED*** Check for absolute path
...
def sanitize_filename(filename: str) -> str:
    ...
    ***REMOVED*** Get just the filename (no path components)
    ...
    ***REMOVED*** Remove path separators
    sanitized = sanitized.replace("/", "_").replace("\\", "_")

    ***REMOVED*** Remove leading dots (hidden files)
    while sanitized.startswith("."):
        sanitized = sanitized[1:]

    ***REMOVED*** Remove leading/trailing spaces
    sanitized = sanitized.strip()

    ***REMOVED*** Only allow safe characters: alphanumeric, dots, underscores, hyphens, spaces
    sanitized = re.sub(r"[^a-zA-Z0-9._\-\s]", "_", sanitized)

    ***REMOVED*** Limit length
    if len(sanitized) > 255:
        ***REMOVED*** Keep extension, truncate name
...
def normalize_unicode(text: str, form: str = "NFKC") -> str:
    ...
    Examples:
        >>> normalize_unicode("café")  ***REMOVED*** é as single char vs e + combining accent
        'café'
    """
    if not text:
        return text

    ***REMOVED*** Normalize Unicode
...
def sanitize_email_input(email: str) -> str:
    ...
    ***REMOVED*** Convert to lowercase (email local part can be case-sensitive,
    ***REMOVED*** but for safety and consistency, we normalize)
...
def sanitize_search_query(query: str, max_length: int = 200) -> str:
    ...
    ***REMOVED*** Check for SQL injection patterns
    sql_patterns = [
        ...
        r"(\bDROP\b.*\bTABLE\b)",
        r"(\bINSERT\b.*\bINTO\b)",
        r"(;.*(-{2}|\/\*))",  ***REMOVED*** SQL comments after semicolon
    ]
    ...
    ***REMOVED*** Check for command injection patterns
    cmd_patterns = [
        r"[;&|`$()]",  ***REMOVED*** Shell metacharacters
        r"\$\{",  ***REMOVED*** Variable expansion
        r"\$\(",  ***REMOVED*** Command substitution
    ]
...
def strip_dangerous_characters(text: str) -> str:
    ...
    ***REMOVED*** Remove most control characters (keep \t, \n, \r)
    cleaned = "".join(char for char in cleaned if ord(char) >= 32 or char in "\t\n\r")

    ***REMOVED*** Remove zero-width characters
    zero_width = [
    ...
    ***REMOVED*** Remove RTL/LTR override characters (can be used for spoofing)
    direction_chars = [
        "\u202a",  ***REMOVED*** Left-to-right embedding
        "\u202b",  ***REMOVED*** Right-to-left embedding
        "\u202c",  ***REMOVED*** Pop directional formatting
        "\u202d",  ***REMOVED*** Left-to-right override
        "\u202e",  ***REMOVED*** Right-to-left override
    ]
...
def sanitize_json_input(data: Any) -> Any:
    ...
    if isinstance(data, str):
        ...
        ***REMOVED*** Numbers, booleans, None - return as is
        return data
...
def validate_no_script_tags(text: str) -> str:
    ...
    ***REMOVED*** Check for javascript: protocol
...
def sanitize_identifier(identifier: str, max_length: int = 64) -> str:
    ...
    ***REMOVED*** Must start with letter or underscore
    if not re.match(r"^[a-zA-Z_]", sanitized):
        raise SanitizationError("Identifier must start with letter or underscore")

    ***REMOVED*** Must contain only alphanumeric and underscore
