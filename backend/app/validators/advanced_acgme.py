...
class AdvancedACGMEValidator:
    ...
    ***REMOVED*** Constants
    MAX_DUTY_HOURS = 24
    MAX_HANDOFF_HOURS = 4
    MAX_CONTINUOUS_HOURS = 28  ***REMOVED*** 24 + 4
    MAX_NIGHT_FLOAT_CONSECUTIVE = 6
    HOURS_PER_HALF_DAY = 6
    MAX_MOONLIGHTING_HOURS_PER_WEEK = 80  ***REMOVED*** Total including internal + external

    ***REMOVED*** PGY-specific limits
    PGY1_MAX_SHIFT_LENGTH = 16  ***REMOVED*** First year limit (without supervision)
    PGY2_PLUS_MAX_SHIFT_LENGTH = 24  ***REMOVED*** Upper level limit
    ...
    def validate_24_plus_4_rule(
        ...
        ***REMOVED*** Check for continuous duty periods exceeding 28 hours
        ...
        while i < len(dates):
            ...
            ***REMOVED*** Count continuous hours
            j = i
    ...
    def validate_night_float_limits(
        ...
        ***REMOVED*** Get all assignments for the person
        assignments = (
            self.db.query(Assignment)
            .join(Block)
            .filter(
        ...
        ***REMOVED*** Identify night shifts (PM blocks or explicitly marked)
        night_dates = set()
        for assignment in assignments:
            block = self.db.query(Block).filter(Block.id == assignment.block_id).first()
            ***REMOVED*** Consider PM blocks as potential night coverage
        ...
        ***REMOVED*** Check for consecutive night shifts
    ...
    def validate_moonlighting_hours(
        ...
        ***REMOVED*** Calculate internal duty hours
        assignments = (
            self.db.query(Assignment)
            .join(Block)
            .filter(
        ...
        ***REMOVED*** Check weekly average (simplified: total hours / weeks)
    ...
    def validate_pgy_specific_rules(
        ...
        ***REMOVED*** Check shift lengths based on PGY level
        max_shift_hours = (
        ...
        ***REMOVED*** Group by consecutive dates to find shifts
        hours_by_date = self._assignments_to_hours(assignments)
