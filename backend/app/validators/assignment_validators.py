...
async def validate_role_compatibility(
    ...
    ***REMOVED*** Check role compatibility with person type
    if role == "supervising":
    ...
    ***REMOVED*** Check if rotation template is appropriate for person
    if rotation_template_id is not None:
        ...
        if rotation:
            ***REMOVED*** Could add rotation-specific validation here
            ***REMOVED*** e.g., checking if person has required certifications
            pass
...
async def validate_optimistic_lock(
    ...
    ***REMOVED*** Check if updated_at matches
...
async def validate_rotation_template_compatibility(
    ...
    ***REMOVED*** Check if rotation is appropriate for person type
    ***REMOVED*** For example, some rotations might be resident-only or faculty-only
    ***REMOVED*** This would depend on your specific business rules

    ***REMOVED*** Check specialty matching if needed
...
async def validate_bulk_assignment_create(
    ...
    ***REMOVED*** Track assignments by person/block for conflict detection
    person_blocks = {}
    ...
    for idx, assignment_data in enumerate(assignments):
        ...
        ***REMOVED*** Check for duplicates within batch
        if check_conflicts:
        ...
        ***REMOVED*** Validate individual assignment (would call other validators here)
...
async def validate_assignment_override(
    ...
    if not acgme_violations:
        ***REMOVED*** No violations, no override needed
        return {
            "is_valid": True,
            "requires_override": False,
        }

    ***REMOVED*** Has violations - override reason is required
    ...
    ***REMOVED*** Validate override reason length
...
async def validate_assignment_cascade_delete(
    ...
    ***REMOVED*** Check if this creates a coverage gap
    ***REMOVED*** (Are there other assignments for this block?)
    result = await db.execute(
