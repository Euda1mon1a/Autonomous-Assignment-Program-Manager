...
***REMOVED*** ACGME Constants
MAX_HOURS_PER_WEEK = 80
MAX_CONSECUTIVE_HOURS = 24
MAX_HOURS_AFTER_24_SHIFT = 4  ***REMOVED*** 24+4 rule
MAX_CONSECUTIVE_DAYS = 6  ***REMOVED*** Must have 1 day off in 7
MIN_TIME_OFF_BETWEEN_SHIFTS = 8  ***REMOVED*** Minimum hours between shifts
MIN_TIME_OFF_AFTER_24_SHIFT = 14  ***REMOVED*** Minimum hours off after 24-hour shift
ROLLING_AVERAGE_WEEKS = 4  ***REMOVED*** 80-hour limit averaged over 4 weeks

***REMOVED*** Supervision ratios
PGY1_SUPERVISION_RATIO = 2  ***REMOVED*** 1 faculty per 2 PGY-1 residents
PGY23_SUPERVISION_RATIO = 4  ***REMOVED*** 1 faculty per 4 PGY-2/3 residents
...
async def validate_80_hour_rule(
    ...
    ***REMOVED*** Only validate for residents
    if person.type != "resident":
        return []

    violations = []

    ***REMOVED*** Get all assignments for this person in the period
    ...
    ***REMOVED*** Calculate hours per day (simplified: 12 hours per block/half-day)
    ***REMOVED*** In production, would use actual shift hours from rotation templates
    hours_by_date = {}
    for assignment in assignments:
        block_date = assignment.block.date
        hours_by_date[block_date] = hours_by_date.get(block_date, 0) + 12.0

    ***REMOVED*** Check rolling 4-week periods
    current_date = start_date
    while current_date <= end_date:
        ***REMOVED*** Calculate 4-week period
        period_start = current_date
        period_end = current_date + timedelta(days=27)  ***REMOVED*** 4 weeks = 28 days

        ***REMOVED*** Calculate total hours in this period
        total_hours = 0.0
        for check_date in (period_start + timedelta(days=i) for i in range(28)):
            total_hours += hours_by_date.get(check_date, 0)

        ***REMOVED*** Add additional hours if date falls in period
...
async def validate_one_in_seven_rule(
    ...
    ***REMOVED*** Get all dates with assignments
    result = await db.execute(
        select(Block.date)
        .join(Assignment)
        .where(
    ...
    ***REMOVED*** Check for 7 consecutive days without a day off
    for i in range(len(assigned_dates) - 6):
        consecutive = assigned_dates[i : i + 7]
        ***REMOVED*** Check if these are 7 consecutive days
...
async def validate_24_plus_4_rule(
    ...
    Returns:
        list[dict]: List of violations with details
    """
    ***REMOVED*** This is simplified - in production would track actual shift start/end times
    ***REMOVED*** For now, just check if person has more than 2 blocks (AM + PM) on same day
    violations = []
    ...
    ***REMOVED*** Get all blocks for this person on this date
    result = await db.execute(
        select(Assignment)
        .join(Block)
    ...
    ***REMOVED*** Count blocks (simplified check)
...
async def validate_post_call_restrictions(
    ...
    ***REMOVED*** Check if person has assignments the next day (potential violation)
    ...
    ***REMOVED*** If person has assignments next day after call, might violate rest requirement
...
async def validate_acgme_compliance(
    ...
    ***REMOVED*** Check each date for 24+4 rule
    current_date = start_date
    while current_date <= end_date:
...
async def validate_assignment_acgme_compliance(
    ...
    ***REMOVED*** Check 1-in-7 rule around this date
    ...
    ***REMOVED*** Check 80-hour rule for 4-week period around this date
    ...
    ***REMOVED*** Check supervision ratio if requested
    ...
    return {
        ...
        "warnings": [],  ***REMOVED*** Could add warnings for near-violations
    }
