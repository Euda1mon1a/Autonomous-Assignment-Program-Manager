...
class ScheduleScorer:
    ...
    def score_schedule(
        ...
        ***REMOVED*** Calculate weighted overall score
        overall_score = (
    ...
    def _score_preferences(self, schedule: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Count low-preference assignments (score < 0.4)
    ...
    def _score_workload_balance(self, schedule: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Identify overloaded people (above 85% utilization)
        overloaded = self.workload_optimizer.identify_overloaded(people, threshold=0.85)

        ***REMOVED*** Convert Gini coefficient to balance score (lower Gini = better balance)
        ***REMOVED*** Gini ranges from 0 (perfect equality) to 1 (perfect inequality)
        gini = fairness.get("gini_coefficient", 0.0)
        balance_score = 1.0 - gini

        ***REMOVED*** Penalize if there are overloaded people
        if overloaded:
        ...
        return {
            ...
            "overloaded_people": overloaded[:5],  ***REMOVED*** Top 5 most overloaded
    ...
    def _score_conflict_risk(self, schedule: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Identify high-risk assignments
        high_risk = self.conflict_predictor.identify_high_risk_assignments(
            assignments, threshold=0.7
        )

        ***REMOVED*** Calculate average risk across all assignments
        risks = []
        for assignment in assignments:
        ...
        ***REMOVED*** Safety score is inverse of average risk
        safety_score = 1.0 - avg_risk

        ***REMOVED*** Penalize if there are high-risk assignments
        if high_risk:
            penalty = min(0.3, len(high_risk) * 0.05)
            safety_score = max(0.0, safety_score - penalty)

        ***REMOVED*** Risk distribution
        risk_distribution = {
        ...
        return {
            ...
            "high_risk_assignments": high_risk[:5],  ***REMOVED*** Top 5 highest risk
    ...
    def suggest_improvements(
        ...
        ***REMOVED*** 1. Workload rebalancing suggestions
        people = schedule.get("people", [])
        if people:
            ...
            for suggestion in workload_suggestions[:5]:  ***REMOVED*** Top 5 workload suggestions
                suggestions.append(
                    {
        ...
        ***REMOVED*** 2. High-risk conflict assignments
        assignments = schedule.get("assignments", [])
        if assignments:
            high_risk = self.conflict_predictor.identify_high_risk_assignments(
            ...
            for risk_item in high_risk[:5]:  ***REMOVED*** Top 5 risky assignments
                suggestions.append(
                    {
        ...
        ***REMOVED*** 3. Low-preference assignments
        for assignment in assignments:
            score = self.preference_predictor.predict(
                person_data=assignment.get("person", {}),
                rotation_data=assignment.get("rotation", {}),
            ...
            if score < 0.3:  ***REMOVED*** Very low preference
                suggestions.append(
                    {
                        "type": "preference_improvement",
                        "priority": 1.0 - score,  ***REMOVED*** Lower score = higher priority
                        "impact": "medium",
        ...
        ***REMOVED*** Sort by priority (highest first)
    ...
    def generate_report(self, schedule: dict[str, Any]) -> str:
        ...
        report = []
        report.append("***REMOVED*** Schedule Quality Report\n")

        ***REMOVED*** Overall score
        report.append(f"***REMOVED******REMOVED*** Overall Grade: {score_result['grade']}")
        report.append(f"**Score:** {score_result['overall_score']:.2%}\n")

        ***REMOVED*** Component scores
        report.append("***REMOVED******REMOVED*** Component Scores\n")

        ***REMOVED*** Preference
        pref = score_result["components"]["preference"]
        report.append(f"***REMOVED******REMOVED******REMOVED*** 1. Preference Score: {pref['average_score']:.2%}")
        ...
        ***REMOVED*** Workload
        ...
        report.append(f"***REMOVED******REMOVED******REMOVED*** 2. Workload Balance: {workload['balance_score']:.2%}")
        report.append(
        ...
        ***REMOVED*** Conflict
        ...
        report.append(f"***REMOVED******REMOVED******REMOVED*** 3. Conflict Safety: {conflict['safety_score']:.2%}")
        ...
        ***REMOVED*** Suggestions
        if suggestions:
            report.append("***REMOVED******REMOVED*** Top Improvement Suggestions\n")
