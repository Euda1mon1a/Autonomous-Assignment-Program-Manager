...
***REMOVED*** SwapRecord model doesn't track assignment-level swaps
***REMOVED*** from app.models.swap import SwapRecord
...
class TrainingDataPipeline:
    ...
    async def extract_preference_training_data(
        ...
        ***REMOVED*** Default to last year if not specified
        if end_date is None:
            end_date = datetime.utcnow()
        if start_date is None:
            start_date = end_date - timedelta(days=365)

        ***REMOVED*** Query assignments with related data
        query = (
        ...
        ***REMOVED*** Extract features and labels
        features_list = []
        labels_list = []
        ...
        for assignment, person, block, rotation_template in rows:
            ...
            ***REMOVED*** Calculate historical stats from database
            ...
            ***REMOVED*** Build feature dict (mimicking PreferencePredictor.extract_features)
            features = self._build_preference_features(
                person_data, rotation_data, block_data, historical_stats
            )

            ***REMOVED*** Calculate label (preference score)
            ***REMOVED*** Use assignment score if available, otherwise infer from swaps
            if assignment.score is not None:
                label = assignment.score
            else:
                ***REMOVED*** Check if this assignment was swapped (indicates low preference)
                was_swapped = await self._was_assignment_swapped(assignment.id)
                label = 0.3 if was_swapped else 0.7  ***REMOVED*** Default scores

            features_list.append(features)
            labels_list.append(label)

        ***REMOVED*** Convert to DataFrame and array
    ...
    async def extract_workload_training_data(
        ...
        ***REMOVED*** Get all people
        ...
        for person in people:
            ***REMOVED*** Get assignments for this person in time range
            assignments_query = (
            ...
            ***REMOVED*** Calculate historical workload data from database
            ...
            ***REMOVED*** Calculate optimal workload (label)
            ***REMOVED*** Use 80% utilization as baseline, adjust based on actual performance
            target = person_data["target_clinical_blocks"]
            actual = len(current_assignments)
            utilization = actual / target if target > 0 else 0

            ***REMOVED*** Check if they had conflicts (would indicate overload)
            had_conflicts = await self._person_had_conflicts(
                person.id, start_date, end_date
            )

            if had_conflicts:
                optimal = max(0.0, utilization - 0.1)  ***REMOVED*** Reduce by 10% if had conflicts
            else:
                optimal = min(1.0, utilization)  ***REMOVED*** Current level is good

            ***REMOVED*** Clip to reasonable range
    ...
    async def extract_conflict_training_data(
        ...
        ***REMOVED*** Get all assignments
        ...
        for assignment, person, block in rows:
            ***REMOVED*** Get existing assignments for this person up to this block
            existing_query = (
                select(Assignment, Block)
                .join(Block, Assignment.block_id == Block.id)
                .where(Assignment.person_id == person.id)
            ...
            ***REMOVED*** Proposed assignment (the one we're evaluating)
            proposed_assignment = {
                "date": block.date,
                "is_weekend": block.is_weekend,
                "is_holiday": block.is_holiday,
            ...
            ***REMOVED*** Calculate context data from database
            ...
            ***REMOVED*** Check if this assignment had a conflict
    ...
    def _build_workload_features(
        ...
        ***REMOVED*** Person characteristics
        features["is_faculty"] = 1 if person_data.get("type") == "faculty" else 0
        features["is_resident"] = 1 if person_data.get("type") == "resident" else 0
        features["pgy_level"] = person_data.get("pgy_level", 0) or 0

        ***REMOVED*** Faculty role
        faculty_role = person_data.get("faculty_role", "")
        for role in ["pd", "apd", "oic", "dept_chief", "sports_med", "core"]:
            features[f"role_{role}"] = 1 if faculty_role == role else 0
        ...
        ***REMOVED*** Workload concentration
    ...
    def _build_conflict_features(
        ...
        ***REMOVED*** Proposed assignment
        ...
        features["proposed_day_of_week"] = 0  ***REMOVED*** Simplified

        ***REMOVED*** Rotation type
    ...
    async def _was_assignment_swapped(self, assignment_id: str) -> bool:
        ...
        """
        ***REMOVED*** SwapRecord doesn't track assignment-level swaps
        ***REMOVED*** Return False as we can't determine this from current data
        return False
    ...
    async def _assignment_had_conflict(self, assignment_id: str) -> bool:
        ...
        ***REMOVED*** Simplified: check if there are any conflict alerts
        query = select(func.count(ConflictAlert.id)).where(
            ConflictAlert.severity.in_(["warning", "error", "critical"])
        )
        result = await self._execute(query)
        count = result.scalar()
        ***REMOVED*** Randomly assign some as conflicts for training (simplified)
        return (count % 5) == 0  ***REMOVED*** Simplified heuristic
    ...
    async def _calculate_preference_historical_stats(
        ...
        """
        ***REMOVED*** Calculate date range for historical lookup (last 90 days)
        end_date = target_date
        start_date = target_date - timedelta(days=90)

        ***REMOVED*** Count similar past assignments
        similar_query = (
        ...
        ***REMOVED*** Calculate current workload (assignments in last 4 weeks)
        ...
        ***REMOVED*** Calculate workload utilization (assume 56 blocks per 4 weeks max)
        max_blocks_per_4_weeks = 56
        current_workload = current_assignments / max_blocks_per_4_weeks

        ***REMOVED*** Calculate average preference score from past assignments
        ***REMOVED*** (Use score field if available, otherwise default)
        score_query = (
        ...
        return {
            ...
            "swap_rate": 0.0,  ***REMOVED*** Would need swap tracking per assignment
    ...
    async def _calculate_workload_historical_data(
        ...
        """
        ***REMOVED*** Calculate workload per week over the period
        weeks = max(1, (end_date - start_date).days // 7)

        ***REMOVED*** Count total assignments
        total_query = (
            select(func.count(Assignment.id))
            .join(Block, Assignment.block_id == Block.id)
            .where(Assignment.person_id == person_id)
        ...
        ***REMOVED*** Average workload per week (assume 14 blocks per week max)
        max_per_week = 14
        avg_per_week = total_assignments / weeks if weeks > 0 else 0
        avg_workload = min(1.0, avg_per_week / max_per_week)

        ***REMOVED*** Estimate max workload (assume busiest week was 20% more than average)
        max_workload = min(1.0, avg_workload * 1.2)

        ***REMOVED*** Count conflicts for this person
        conflict_query = (
            select(func.count(ConflictAlert.id))
            .where(ConflictAlert.created_at >= start_date)
            .where(ConflictAlert.created_at <= end_date)
        ...
        ***REMOVED*** Conflict rate = conflicts per total assignments
        conflict_rate = conflict_count / max(total_assignments, 1)

        return {
            "avg_workload": avg_workload,
            "max_workload": max_workload,
            "swap_rate": 0.0,  ***REMOVED*** Would need swap tracking
    ...
    async def _calculate_conflict_context_data(
        ...
        """
        ***REMOVED*** Count faculty and residents assigned on the target date
        faculty_query = (
        ...
        faculty_count = result.scalar() or 1  ***REMOVED*** Default to 1 to avoid division by zero
        ...
        ***REMOVED*** Historical conflict rate
        ...
        ***REMOVED*** Coverage level (ratio of actual to expected staffing)
        ***REMOVED*** Assume expected is 2:1 resident to faculty ratio
        ...
        return {
            ...
            "recent_swap_count": 0,  ***REMOVED*** Would need swap tracking
            "coverage_level": min(2.0, coverage_level),  ***REMOVED*** Cap at 200%
        }
