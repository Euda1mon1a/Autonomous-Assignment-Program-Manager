...
***REMOVED*** Thresholds for sync lag warnings
SYNC_LAG_WARNING_SECONDS = 60  ***REMOVED*** 1 minute
SYNC_LAG_CRITICAL_SECONDS = 300  ***REMOVED*** 5 minutes
...
class SyncMetrics:
    ...
    def update_lag(self, event_timestamp: datetime) -> None:
        ...
        if event_timestamp:
            ...
            ***REMOVED*** Update status based on lag
    ...
    def record_success(self, processing_time_ms: float) -> None:
        ...
        ***REMOVED*** Update average processing time (exponential moving average)
        alpha = 0.1  ***REMOVED*** Smoothing factor
...
***REMOVED*** =============================================================================
***REMOVED*** Read Model Sync Service
***REMOVED*** =============================================================================
...
class ReadModelSyncService:
    ...
    def __init__(
        ...
        ***REMOVED*** Subscription tracking
        self._subscription_ids: dict[str, str] = {}

        ***REMOVED*** Sync control
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Projector Registration
    ***REMOVED*** =========================================================================
    ...
    async def register_projector(
        ...
        ***REMOVED*** Subscribe to real-time events
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _subscribe_to_events(self, projector_name: str) -> None:
        ...
        async def event_handler(event: DomainEvent) -> None:
            ...
            if self.enable_batch_processing:
                ...
                ***REMOVED*** Process immediately
                await self._process_single_event(projector_name, event)

        ***REMOVED*** Subscribe to all events (projector will filter)
        subscription_id = self.event_bus.subscribe(
            event_type="*",  ***REMOVED*** Subscribe to all event types
    ...
    async def start_realtime_sync(self) -> None:
        ...
        ***REMOVED*** Start batch processing task if enabled
        if self.enable_batch_processing:
    ...
    async def stop_realtime_sync(self) -> None:
        ...
        ***REMOVED*** Cancel batch processing task
        if self._batch_task:
        ...
        ***REMOVED*** Process any remaining events in batch
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Batch Event Processing
    ***REMOVED*** =========================================================================
    ...
    async def _add_to_batch(self, projector_name: str, event: DomainEvent) -> None:
        ...
        async with self._batch_lock:
            ...
            ***REMOVED*** Flush if batch is full
    ...
    async def _flush_batch(self) -> None:
        ...
        async with self._batch_lock:
            ...
            ***REMOVED*** Group events by projector
            ...
            ***REMOVED*** Process each projector's events
    ...
    async def process_event_batch(
        ...
        Example:
            ***REMOVED*** Catch up on last 500 events
        ...
        for projector_name, projector in self._projectors.items():
            ***REMOVED*** Skip paused projectors
            if projector_name in self._paused_projectors:
                continue

            ***REMOVED*** Get starting sequence
            ...
            ***REMOVED*** Fetch events from store
            try:
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Single Event Processing
    ***REMOVED*** =========================================================================
    ...
    async def _process_single_event(
        ...
        try:
            ***REMOVED*** Update lag before processing
            metrics.update_lag(event.metadata.timestamp)
            metrics.status = SyncStatus.SYNCING

            ***REMOVED*** Project the event
            await projector.project(event)

            ***REMOVED*** Commit read database changes
            ...
            ***REMOVED*** Rollback read database changes
            ...
            ***REMOVED*** Re-raise for retry logic
            raise

    ***REMOVED*** =========================================================================
    ***REMOVED*** Consistency Verification
    ***REMOVED*** =========================================================================
    ...
    async def _check_consistency(self, projector_name: str, event: DomainEvent) -> None:
        ...
        """
        try:
            ***REMOVED*** Get the aggregate ID from the event
            aggregate_id = getattr(event, "aggregate_id", None)
            if not aggregate_id:
                ***REMOVED*** Event doesn't have aggregate ID, skip consistency check
                return

            ***REMOVED*** Verify consistency for this aggregate
            ...
            if not is_consistent and conflict:
                ...
                ***REMOVED*** Handle conflict based on resolution strategy
                if self.conflict_resolution_strategy != ConflictResolutionStrategy.MANUAL:
                    await self.resolve_conflict(conflict)

        except Exception as e:
            ***REMOVED*** Don't fail event processing due to consistency check failures
            logger.error(
    ...
    async def verify_consistency(
        ...
        """
        try:
            ***REMOVED*** Get the projector
            projector = self.projectors.get(projector_name)
            if not projector:
                logger.warning(f"Projector {projector_name} not found for verification")
                return True, None

            ***REMOVED*** Get write model state (source of truth)
            write_state = await self._get_write_model_state(
                projector_name, aggregate_id
            )

            if write_state is None:
                ***REMOVED*** Aggregate doesn't exist in write model
                ***REMOVED*** Check if it exists in read model (orphaned read)
                read_state = await self._get_read_model_state(
                    projector_name, aggregate_id
                )

                if read_state is not None:
                    ***REMOVED*** Read model has data that doesn't exist in write model
                ...
                ***REMOVED*** Neither exists, that's consistent (both empty)
                return True, None

            ***REMOVED*** Get read model state
            read_state = await self._get_read_model_state(projector_name, aggregate_id)

            if read_state is None:
                ***REMOVED*** Write model has data but read model doesn't (stale read)
                conflict = SyncConflict(
                    read_model_name=projector_name,
                    aggregate_id=aggregate_id,
                    event_id="unknown",
            ...
            ***REMOVED*** States match, consistent
            return True, None
            ...
            ***REMOVED*** On error, assume consistent to avoid false alarms
            return True, None
    ...
    async def _get_write_model_state(
        ...
        """
        ***REMOVED*** This is projector-specific - would need to be implemented
        ***REMOVED*** based on the specific read model type
        ***REMOVED*** For now, return None to indicate no verification possible
        logger.debug(
    ...
    def _compare_states(
        ...
        ***REMOVED*** Get all keys from both states
        all_keys = set(write_state.keys()) | set(read_state.keys())

        ***REMOVED*** Ignore audit fields that are expected to differ
        ignore_fields = {
            "created_at",
            "updated_at",
            "last_modified",
        ...
        for key in all_keys:
            ...
            ***REMOVED*** Normalize values for comparison
            if write_val != read_val:
                ***REMOVED*** Handle type conversions and edge cases
    ...
    def _values_equal(self, val1: Any, val2: Any) -> bool:
        ...
        """
        ***REMOVED*** Handle None equality
        if val1 is None and val2 is None:
            return True
        if val1 is None or val2 is None:
            return False

        ***REMOVED*** Handle string/UUID conversions
        if isinstance(val1, str) and hasattr(val2, "__str__"):
            return str(val1) == str(val2)
        if isinstance(val2, str) and hasattr(val1, "__str__"):
            return str(val1) == str(val2)

        ***REMOVED*** Handle datetime comparisons (truncate microseconds)
        if isinstance(val1, datetime) and isinstance(val2, datetime):
            ***REMOVED*** Compare with 1-second precision to handle serialization differences
            return abs((val1 - val2).total_seconds()) < 1.0

        ***REMOVED*** Handle numeric comparisons with tolerance
        if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
            return abs(val1 - val2) < 1e-9

        ***REMOVED*** Default equality check
        return val1 == val2
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _apply_write_value(self, conflict: SyncConflict) -> None:
        ...
        ***REMOVED*** Implementation would update read model with write DB value
    ...
    async def _merge_values(self, conflict: SyncConflict) -> None:
        ...
        ***REMOVED*** Implementation would merge the values intelligently
        logger.debug(f"Merged values for conflict: {conflict.conflict_id}")

    ***REMOVED*** =========================================================================
    ***REMOVED*** Checkpointing and Resume
    ***REMOVED*** =========================================================================
    ...
    async def load_checkpoint(self, projector_name: str) -> SyncCheckpoint | None:
        ...
        """
        ***REMOVED*** Implementation would load from database
    ...
    ***REMOVED*** =========================================================================
    ...
    async def check_health(self) -> dict[str, Any]:
        ...
        ***REMOVED*** Check for failed projectors
        for name, metrics in self._metrics.items():
            ...
            ***REMOVED*** Check for critical lag
        ...
        ***REMOVED*** Check for unresolved conflicts
        unresolved = [c for c in self._conflicts if not c.resolved]
        if len(unresolved) > 10:  ***REMOVED*** Threshold
            health["healthy"] = False
            health["issues"].append(
                {
