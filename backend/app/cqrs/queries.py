...
::
    ...
    ***REMOVED*** 1. Define a read model (denormalized for fast queries)
    @dataclass
    class AssignmentSummary(ReadModel):
        assignment_id: UUID
        person_name: str        ***REMOVED*** Denormalized from Person table
        block_date: date        ***REMOVED*** Denormalized from Block table
        rotation_name: str      ***REMOVED*** Denormalized from Rotation table
        location: str
        total_hours: float      ***REMOVED*** Pre-computed aggregate

    ***REMOVED*** 2. Define a query (immutable, question-based naming)
    ...
    ***REMOVED*** 3. Define a handler
    class GetPersonAssignmentsHandler(QueryHandler[GetPersonAssignmentsQuery]):
        async def handle(self, query: GetPersonAssignmentsQuery) -> QueryResult:
            ***REMOVED*** Query the read model table (not the normalized write tables!)
    ...
    ***REMOVED*** 4. Execute with caching enabled
    ...
    class AssignmentSummaryProjector(ReadModelProjector):
        ...
        async def project(self, event: DomainEvent) -> None:
            if isinstance(event, AssignmentCreatedEvent):
                ...
                await self._on_person_updated(event)  ***REMOVED*** Update denormalized name

        async def _on_assignment_created(self, event):
            ***REMOVED*** Fetch related data from write DB to build denormalized read model
            ...
            ***REMOVED*** Create denormalized read model
            summary = AssignmentSummary(
                assignment_id=assignment.id,
                person_name=person.name,  ***REMOVED*** Denormalized!
                block_date=assignment.block.date,
                rotation_name=rotation.name,  ***REMOVED*** Denormalized!
        ...
        async def rebuild(self) -> None:
            ...
            ***REMOVED*** Clear existing read models
            await self.db_read.execute("DELETE FROM assignment_summaries")

            ***REMOVED*** Rebuild from all assignments
...
***REMOVED*** Type variables for generic query handling
    ...
    Example:
        class AssignmentReadModelProjector(ReadModelProjector):
            ...
            async def _on_assignment_created(self, event: AssignmentCreatedEvent):
                ***REMOVED*** Fetch data from write DB
                assignment = self.db_write.get(Assignment, event.aggregate_id)

                ***REMOVED*** Create read model in read DB
...
class QueryBus:
    ...
    def _get_cached_result(self, query: Query) -> QueryResult | None:
        ...
        if cache_key in self._cache:
            ...
            ***REMOVED*** Simple TTL check (could be made configurable)
            age = (datetime.utcnow() - cached_at).total_seconds()
            if age < 300:  ***REMOVED*** 5 minutes TTL
                return cached_result
            else:
                ***REMOVED*** Expired, remove from cache
    ...
    def _make_cache_key(self, query: Query) -> str:
        ...
        Returns:
            Cache key string
        """
        ***REMOVED*** Simple implementation - could be enhanced with better hashing
...
***REMOVED*** Example query implementations for reference
...
***REMOVED*** Example read model for reference
@dataclass
class ExampleReadModel(ReadModel):
    ...
    email: str
    created_at: datetime
    ***REMOVED*** Denormalized fields for fast querying
