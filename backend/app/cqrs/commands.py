...
::
    ...
    ***REMOVED*** 1. Define a command (immutable, intent-based)
    ...
    ***REMOVED*** 2. Define a handler
    class CreateAssignmentHandler(CommandHandler[CreateAssignmentCommand]):
        async def handle(self, command: CreateAssignmentCommand) -> CommandResult:
            ...
            ***REMOVED*** Execute business logic
    ...
    ***REMOVED*** 3. Register and execute
    bus = CommandBus(db)
...
***REMOVED*** Type variables for generic command handling
...
class CommandValidationMiddleware:
    ...
    Example:
        ...
        ***REMOVED*** Validation happens automatically in CommandBus
    """
    ...
    async def validate(self, command: Command) -> None:
        ...
        if command_type not in self._schemas:
            ***REMOVED*** No schema registered, skip validation
            return

        schema = self._schemas[command_type]

        ***REMOVED*** Extract command data (excluding base Command fields)
        command_data = {
            k: v
        ...
        try:
            ***REMOVED*** Validate using Pydantic
            schema(**command_data)
...
class CommandBus:
    ...
    async def execute(self, command: TCommand) -> CommandResult:
        ...
        ***REMOVED*** Run validation middleware
    ...
    async def _publish_events(self, events: list[DomainEvent]) -> None:
        ...
        for event in events:
            ...
            for handler in self._event_handlers:
                try:
                    if callable(handler):
                        ***REMOVED*** Support both sync and async handlers
                    ...
                    ***REMOVED*** Don't let event handler failures fail the command


***REMOVED*** Example command implementations for reference
***REMOVED*** Note: When creating commands that inherit from Command, all custom fields
***REMOVED*** must have defaults since Command has default fields. Alternatively, use
***REMOVED*** field(default=...) or don't use frozen=True and set in __post_init__.
...
class ExampleCreateCommand(Command):
    ...
    def __init__(
        ...
        ***REMOVED*** Use object.__setattr__ since dataclass is frozen
        object.__setattr__(self, "name", name)
        object.__setattr__(self, "email", email)
        ***REMOVED*** Initialize base Command fields
