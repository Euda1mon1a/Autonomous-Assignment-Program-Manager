...
class FullBackupStrategy(BackupStrategy):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            ***REMOVED*** Backup each table
            for table in tables:
                logger.debug(f"Backing up table: {table}")

                try:
                    ***REMOVED*** Get table data
    ...
    def _backup_table_full(self, db: Session, table: str) -> dict[str, Any]:
        ...
        """
        ***REMOVED*** Get all rows
        query = text(f"SELECT * FROM {table}")
        result = db.execute(query)

        ***REMOVED*** Convert rows to dictionaries
        rows = []
        for row in result:
            row_dict = dict(row._mapping)
            ***REMOVED*** Convert non-JSON-serializable types
...
class IncrementalBackupStrategy(BackupStrategy):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ***REMOVED*** Convert timestamp to datetime
            ...
            ***REMOVED*** Backup changed data from each table
            for table in tables:
                logger.debug(f"Checking for changes in table: {table}")

                try:
                    ***REMOVED*** Get changed rows since last backup
    ...
    def _backup_table_incremental(
        ...
        """
        ***REMOVED*** Check if table has updated_at column
        column_query = text(f"""
            SELECT column_name
        ...
        if timestamp_column:
            ***REMOVED*** Query rows changed since last backup
            query = text(f"""
                SELECT *
                FROM {table}
            ...
            ***REMOVED*** No timestamp column - include all rows as a safety measure
...
class DifferentialBackupStrategy(BackupStrategy):
    ...
    async def execute(
        ...
        ***REMOVED*** Differential backup is similar to incremental but references
        ***REMOVED*** the last full backup instead of the last backup
        ...
        ***REMOVED*** Update metadata to reflect differential strategy
