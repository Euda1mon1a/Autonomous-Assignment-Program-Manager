...
class RestoreService:
    ...
    async def restore_from_backup(
        ...
        try:
            ...
            ***REMOVED*** Build restoration chain
            ...
            ***REMOVED*** Apply each backup in chain
    ...
    def _build_restore_chain(
        ...
        ***REMOVED*** Full backup - just return itself
        if backup_type == "full":
            return [
                {
                    "backup_id": backup_id,
        ...
        ***REMOVED*** Incremental or differential - need to find base and build chain
        ...
        ***REMOVED*** Get all backups
        all_backups = self.storage.list_backups(limit=1000)

        ***REMOVED*** Find base full backup
        base_backup = None
        ...
        ***REMOVED*** For differential, just add the differential backup
        ...
        ***REMOVED*** For incremental, add all incrementals between base and target
        incrementals = []
        for backup in all_backups:
        ...
        ***REMOVED*** Sort incrementals by created_at
    ...
    async def _restore_backup_data(
        ...
        for table_name, table_data in backup_tables.items():
            ***REMOVED*** Skip if not in requested tables
            if tables and table_name not in tables:
                continue

            ***REMOVED*** Skip if table has errors
            ...
            try:
                ...
                ***REMOVED*** Continue with other tables
                continue
    ...
    async def _restore_table(
        ...
        ***REMOVED*** Build UPSERT query
        ***REMOVED*** Note: This assumes tables have a primary key named 'id'
        ***REMOVED*** For tables with different primary keys, we'd need schema introspection

        rows_restored = 0

        for row in rows:
            if dry_run:
                ***REMOVED*** Just validate the row structure
                rows_restored += 1
                continue

            try:
                ***REMOVED*** Build INSERT ... ON CONFLICT DO UPDATE
                column_names = ", ".join(columns)
                placeholders = ", ".join([f":{col}" for col in columns])

                ***REMOVED*** Build update clause (all columns except id)
                ...
                ***REMOVED*** Continue with other rows
                continue
    ...
    async def restore_to_point_in_time(
        ...
        """
        ***REMOVED*** Convert string to datetime if needed
        ...
        try:
            ...
            ***REMOVED*** Find most recent backup before target time
            suitable_backups = [
                b
            ...
            ***REMOVED*** Use the most recent suitable backup
            ...
            ***REMOVED*** Restore from this backup
    ...
    def validate_backup_chain(self, backup_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Build restore chain
            restore_chain = self._build_restore_chain(backup_id, backup_data)

            ***REMOVED*** Verify each backup in chain
            validation_results = []
            ...
            ***REMOVED*** Check if all backups in chain are valid
