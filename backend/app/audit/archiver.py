...
class AuditArchiver:
    ...
    async def archive_old_logs(
        ...
        """
        ***REMOVED*** Determine retention policy
        if policy is None:
        ...
        ***REMOVED*** Extract logs to archive
        ...
        ***REMOVED*** Optionally purge from database
        purged = False
    ...
    async def archive_by_date_range(
        ...
        ***REMOVED*** Extract logs in date range
    ...
    async def _extract_logs_for_archive(
        ...
        """
        ***REMOVED*** Version tables to query
        version_tables = [
            "assignment_version",
            "absence_version",
        ...
        ***REMOVED*** Filter by entity types if specified
        if entity_types:
            entity_table_map = {
        ...
        for table_name in version_tables:
            try:
                ***REMOVED*** Query version table with transaction join
                query = text(f"""
                    SELECT
                        v.id as entity_id,
                        v.transaction_id,
                ...
                for row in result:
                    ***REMOVED*** Extract entity type from table name
    ...
    def _build_archive_data(
        ...
        ***REMOVED*** Count by operation type
        operation_counts = {}
    ...
    def _generate_archive_id(
        ...
        """
        if end_date:
            ***REMOVED*** Range-based archive
            return (
                f"archive-{cutoff_date.strftime('%Y-%m-%d')}-"
                f"to-{end_date.strftime('%Y-%m-%d')}"
            )
        else:
            ***REMOVED*** Date-based archive
