...
class LocalArchiveStorage(ArchiveStorageBackend):
    ...
    def list_archives(
        ...
        try:
            ***REMOVED*** Search all archive files
    ...
    def _find_archive_path(self, archive_id: str) -> Path | None:
        ...
        """
        ***REMOVED*** Search for both compressed and uncompressed versions
...
class S3ArchiveStorage(ArchiveStorageBackend):
    ...
    def list_archives(
        ...
        try:
            ***REMOVED*** List all objects with prefix
            ...
            for page in pages:
                for obj in page.get("Contents", []):
                    ...
                    ***REMOVED*** Extract archive_id from key
                    filename = Path(s3_key).name
                    archive_id = filename.replace(".json.gz", "").replace(".json", "")

                    ***REMOVED*** Get object metadata
    ...
    def _find_s3_key(self, archive_id: str) -> str | None:
        ...
        """
        try:
            ***REMOVED*** List objects with prefix
            paginator = self.s3_client.get_paginator("list_objects_v2")
            pages = paginator.paginate(Bucket=self.bucket_name, Prefix=self.prefix)
