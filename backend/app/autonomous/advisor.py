...
class SuggestionSchema:
    ...
    ***REMOVED*** Valid algorithms the LLM can suggest
    VALID_ALGORITHMS = {"greedy", "cp_sat", "pulp", "hybrid"}

    ***REMOVED*** Valid parameter ranges
    TIMEOUT_RANGE = (10.0, 600.0)  ***REMOVED*** 10 seconds to 10 minutes
    WEIGHT_RANGE = (0.0, 2.0)  ***REMOVED*** Constraint weights
    DIVERSIFICATION_RANGE = (0.0, 1.0)

    ***REMOVED*** Valid constraint names for weight adjustments
    VALID_CONSTRAINTS = {
        "acgme_compliance",
        "coverage_rate",
        "resilience",
    ...
    def validate_params(cls, params: dict[str, Any]) -> tuple[bool, str]:
        ...
        ***REMOVED*** Validate diversification
        ...
        ***REMOVED*** Validate constraint weights
...
class LLMAdvisor:
    ...
    async def suggest(
        ...
        try:
            ...
            ***REMOVED*** Log but don't crash - the loop can continue without LLM
    ...
    def validate_suggestion(self, suggestion: Suggestion) -> bool:
        ...
        """
        ***REMOVED*** Basic validation
        if suggestion.confidence < 0.3:
            logger.debug("Suggestion rejected: confidence too low")
            return False  ***REMOVED*** Low confidence suggestions are ignored

        if not suggestion.reasoning:
            logger.debug("Suggestion rejected: no reasoning provided")
            return False  ***REMOVED*** Require reasoning

        ***REMOVED*** Validate parameters if present
    ...
    def _build_prompt(
        ...
        if last_evaluation:
            ...
            if last_evaluation.violations:
                ...
                for v in last_evaluation.violations[:5]:  ***REMOVED*** Limit to 5
    ...
    async def _query_llm(self, prompt: str) -> str:
        ...
        ***REMOVED*** Build LLM request
        request = LLMRequest(
            prompt=prompt,
            system=SCHEDULING_ASSISTANT_SYSTEM_PROMPT,
            model=self.model,
            provider="ollama",  ***REMOVED*** Use local Ollama for airgap compatibility
    ...
    def _parse_response(self, response: str) -> Suggestion:
        ...
        ***REMOVED*** Extract JSON from response
        ...
        ***REMOVED*** Parse params if present
        params = None
...
class MockLLMAdvisor(LLMAdvisor):
    ...
    def __init__(self):
        ...
        ***REMOVED*** Don't call super().__init__ to avoid creating router
    ...
    async def suggest(
        ...
        ***REMOVED*** Simple heuristic-based suggestions
