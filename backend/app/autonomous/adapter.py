...
***REMOVED*** Check for scipy availability
try:
...
class ParameterAdapter:
    ...
    ***REMOVED*** Algorithm rotation order
    ...
    def adapt(
        ...
        """
        ***REMOVED*** Check each rule in priority order
        for rule in self.rules:
            try:
                if rule.condition(evaluation, history):
                    return self._apply_rule(rule, current_params, evaluation, history)
            except Exception:
                ***REMOVED*** Rule evaluation failed, skip it
                continue

        ***REMOVED*** No rule matched, return unchanged
    ...
    def _build_default_rules(self) -> list[AdaptationRule]:
        ...
        ***REMOVED*** Rule 1: If score stagnating, increase diversification
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 2: If solver times out, increase timeout
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 3: If too many ACGME violations, switch algorithm
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 4: If N-1 vulnerable, increase resilience weight
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 5: If close to feasible (score > 0.8), narrow search
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 6: If utilization too high, adjust coverage weight
        rules.append(
            AdaptationRule(
        ...
        ***REMOVED*** Rule 7: Random restart every N iterations without improvement
        rules.append(
            AdaptationRule(
    ...
    ***REMOVED*** Condition functions
    ...
    def _is_stagnating(
        ...
        ***REMOVED*** Check if last 5 iterations have similar scores
        recent_scores = [r.score for r in history[-5:]]
        score_range = max(recent_scores) - min(recent_scores)
        return score_range < 0.01  ***REMOVED*** Less than 1% variation
    ...
    def _is_timing_out(
        ...
        last = history[-1]
        ***REMOVED*** Heuristic: if duration is close to timeout and score is low
    ...
    def _has_acgme_violations(
        ...
        ***REMOVED*** Check for 80_HOUR or SUPERVISION violations
        acgme_types = {
            "80_HOUR_VIOLATION",
            "1_IN_7_VIOLATION",
    ...
    def _needs_restart(
        ...
        ***REMOVED*** Check if last 10 iterations had no improvement
        if history:
...
class BayesianAdapter(ParameterAdapter):
    ...
    def adapt(
        ...
        ***REMOVED*** Need at least 10 observations for Bayesian optimization
        if len(self._observations) < 10:
            return super().adapt(current_params, evaluation, history)

        ***REMOVED*** Try Bayesian optimization
        try:
            return self._bayesian_suggest(current_params)
        except Exception:
            ***REMOVED*** Fall back to deterministic
            return super().adapt(current_params, evaluation, history)
    ...
    def _bayesian_suggest(self, current: GeneratorParams) -> GeneratorParams:
        ...
        if not HAS_SCIPY or len(self._observations) < 10:
            ***REMOVED*** Fall back to random perturbation
            return self._random_perturbation(current)

        try:
            ***REMOVED*** Extract observations into arrays
            X = []  ***REMOVED*** Parameter vectors
            y = []  ***REMOVED*** Scores

            for params_dict, score in self._observations:
                ***REMOVED*** Extract continuous parameters
                param_vec = [
                    params_dict.get("timeout_seconds", 60.0) / 120.0,  ***REMOVED*** Normalize
            ...
            ***REMOVED*** Find best observed
            best_idx = np.argmax(y)
            best_score = y[best_idx]
            best_x = X[best_idx]

            ***REMOVED*** Simple acquisition: sample around best with decay based on observations
            ***REMOVED*** More observations = tighter sampling (exploitation)
            exploration_scale = max(0.05, 1.0 / len(self._observations))

            ***REMOVED*** Sample new point using Expected Improvement heuristic
            ***REMOVED*** Move toward best observation with some randomness
            new_x = best_x + np.random.normal(0, exploration_scale, size=best_x.shape)

            ***REMOVED*** Clip to valid bounds
            new_x = np.clip(new_x, 0.0, 1.0)

            ***REMOVED*** Convert back to parameters
            new_timeout = new_x[0] * 120.0  ***REMOVED*** Denormalize
