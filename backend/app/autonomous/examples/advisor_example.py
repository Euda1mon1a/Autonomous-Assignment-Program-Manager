...
async def example_with_real_llm():
    ...
    ***REMOVED*** Create LLM advisor with Ollama
    advisor = LLMAdvisor(
    ...
    ***REMOVED*** Create mock evaluation with some violations
    ...
    ***REMOVED*** Create some iteration history
    history = [
        IterationRecord(
            iteration=i,
...
def example_with_mock_advisor():
    ...
    ***REMOVED*** Create mock advisor
    advisor = MockLLMAdvisor()

    ***REMOVED*** Create state with critical violations
    state = RunState(
        run_id="mock-001",
        scenario="baseline",
        start_date=date(2025, 1, 1),
    ...
    ***REMOVED*** Get suggestion synchronously (mock doesn't need async)
...
def main():
    ...
    ***REMOVED*** Example 1: Mock advisor (always works)
    example_with_mock_advisor()

    ***REMOVED*** Example 2: Router health check
    try:
        asyncio.run(example_router_health_check())
    except Exception as e:
        logger.warning(f"Router health check failed: {e}")

    ***REMOVED*** Example 3: Real LLM advisor (requires Ollama)
    try:
        ...
        logger.info("  ***REMOVED*** Wait for Ollama to pull llama3.2 model")
