...
class ViolationSeverity(str, Enum):
    ...
    CRITICAL = "critical"  ***REMOVED*** ACGME violations, must fix
    HIGH = "high"  ***REMOVED*** Serious issues, should fix
    MEDIUM = "medium"  ***REMOVED*** Minor issues, good to address
    LOW = "low"  ***REMOVED*** Preferences not met
...
class ScheduleEvaluator:
    ...
    ***REMOVED*** Weight distribution for score components
    WEIGHTS = {
    ...
    ***REMOVED*** Penalty multipliers by severity
    SEVERITY_PENALTIES = {
        ViolationSeverity.CRITICAL: 0.20,  ***REMOVED*** Each critical violation costs 20% of component
    ...
    def evaluate(
        ...
        ***REMOVED*** Load data if not provided
        if faculty is None:
        ...
        ***REMOVED*** 1. ACGME Compliance (hard constraints)
        ...
        ***REMOVED*** Check hard constraint pass (no CRITICAL violations)
        critical_count = sum(
            1 for v in violations if v.severity == ViolationSeverity.CRITICAL
        )
        hard_pass = critical_count == 0

        ***REMOVED*** If hard constraints fail, score is 0
        if not hard_pass:
            total_score = 0.0

        ***REMOVED*** Soft score (for comparing invalid schedules)
        soft_score = sum(
            c.weighted_value for c in components if c.name != "acgme_compliance"
        )

        ***REMOVED*** Coverage rate as percentage
    ...
    def _evaluate_acgme(
        ...
        ***REMOVED*** Convert violations to our format
        for v in result.violations:
        ...
        ***REMOVED*** Calculate score: start at 1.0, subtract penalties
        raw_score = 1.0
        for v in violations:
            raw_score -= v.penalty
    ...
    def _evaluate_coverage(
        ...
        ***REMOVED*** Count unique blocks with assignments
        assigned_blocks = {a.block_id for a in assignments}

        ***REMOVED*** Only count weekday blocks
    ...
    def _evaluate_resilience(
        ...
        try:
            ...
            ***REMOVED*** N-1 compliance (single faculty loss)
            ...
            ***REMOVED*** N-2 compliance (pair faculty loss)
            ...
            ***REMOVED*** If resilience check fails, give partial credit
            raw_score = 0.5
    ...
    def _evaluate_load_balance(
        ...
        ***REMOVED*** Convert CV to score: CV of 0 = perfect (1.0), CV of 1 = poor (0.0)
        ***REMOVED*** CV > 1 is very unbalanced
    ...
    def _evaluate_preferences(
        ...
        ***REMOVED*** Build a map of assignments per faculty with their blocks
        ...
        ***REMOVED*** Score calculation
        ...
        for faculty_id, assignments_blocks in faculty_assignments.items():
            ...
            if not pref:
                ***REMOVED*** No preferences recorded, count as satisfied
                total_checks += 1
                satisfied_checks += 1
                continue
        ...
        ***REMOVED*** Calculate raw score
        if total_checks == 0:
            ...
            ***REMOVED*** Start with satisfaction ratio
            satisfaction_ratio = satisfied_checks / total_checks

            ***REMOVED*** Penalize blocked week violations heavily
            violation_penalty = blocked_week_violations * 0.15

            ***REMOVED*** Bonus for preferred week matches (small bonus)
