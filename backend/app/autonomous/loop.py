...
class AutonomousLoop:
    ...
    def from_config(
        ...
        ***REMOVED*** Create store
        store = StateStore(base_path=runs_path)

        ***REMOVED*** Create or resume state
        if resume_run_id:
        ...
        ***REMOVED*** Create evaluator
        evaluator = ScheduleEvaluator(
            db=db,
            resilience_config=resilience_config,
        )

        ***REMOVED*** Create adapter (lazy import to avoid circular dependency)
        from app.autonomous.adapter import ParameterAdapter
        from app.autonomous.advisor import LLMAdvisor

        adapter = ParameterAdapter()

        ***REMOVED*** Create LLM advisor (optional, airgap-compatible)
        ***REMOVED*** Only create if Ollama is available, otherwise loop runs without advisor
        try:
            advisor = LLMAdvisor(
                llm_router=None,  ***REMOVED*** Will create default Ollama router
                model="llama3.2",
                airgap_mode=True,  ***REMOVED*** Local only by default
            )
        ...
        return cls(
            ...
            config=config,
            adapter=adapter,
            advisor=advisor,  ***REMOVED*** LLM advisor is optional but enabled by default
        )
    ...
    def _run_iteration(self) -> None:
        ...
        ***REMOVED*** 1. Get parameters for this iteration
        params = self._get_current_params()

        ***REMOVED*** 2. Generate candidates
        ...
        ***REMOVED*** 3. Evaluate each candidate
        ...
        for candidate in candidates:
            ...
            ***REMOVED*** Track best this iteration
        ...
        ***REMOVED*** 4. Update state
        self.state.update_with_result(evaluation, params)

        ***REMOVED*** 5. Update global best if improved
        ...
        ***REMOVED*** 6. Record iteration in history
        ...
        ***REMOVED*** 7. Adapt parameters for next iteration
        if self.adapter:
            next_params = self.adapter.adapt(
                current_params=params,
                evaluation=evaluation,
                history=self.store.load_history(self.state.run_id)[-10:],  ***REMOVED*** Last 10
            )
...
class AutonomousLoopWithAdvisor(AutonomousLoop):
    ...
    async def _run_iteration_async(self) -> None:
        ...
        ***REMOVED*** First, try to get LLM suggestion if advisor is configured
        llm_suggestion = None
        if self.advisor:
            try:
        ...
        ***REMOVED*** If LLM suggested parameters, use them
        ...
        ***REMOVED*** Run standard iteration
        super()._run_iteration()

        ***REMOVED*** Log LLM usage if any
        if llm_suggestion:
            self.store.log(
                self.state,
    ...
    def _run_iteration(self) -> None:
        ...
        ***REMOVED*** Check if we're already in an event loop
        try:
            loop = asyncio.get_running_loop()
            ***REMOVED*** If we're already in a loop, run the async version directly
            ***REMOVED*** This requires the caller to use asyncio.run() or similar
            logger.warning(
                "Running async iteration in existing event loop - "
                "consider making the entire loop async"
            )
            ***REMOVED*** Create a task and wait for it
            task = loop.create_task(self._run_iteration_async())
            ***REMOVED*** This is a bit hacky but works for now
            loop.run_until_complete(task)
        except RuntimeError:
            ***REMOVED*** No event loop running, create one
