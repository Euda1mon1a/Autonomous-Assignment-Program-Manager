...
class CandidateGenerator:
    ...
    def generate_candidates(
        ...
        if k == 1:
            ***REMOVED*** Single candidate with specified algorithm
            candidate = self._generate_single(params)
            if candidate:
                candidates.append(candidate)
        else:
            ***REMOVED*** Multiple candidates with different algorithms
    ...
    def _generate_single(
        ...
        ***REMOVED*** Create engine
        ...
        try:
            ***REMOVED*** Note: SchedulingEngine.generate() commits to database
            ***REMOVED*** For candidate generation, we need a different approach
            ***REMOVED*** We'll use the engine to generate but track the assignments before commit

            ***REMOVED*** First, ensure blocks exist
            blocks = engine._ensure_blocks_exist(commit=False)

            ***REMOVED*** Build availability matrix
            ...
            ***REMOVED*** Convert solver results to assignments (without saving)
            assignments: list[Assignment] = []
            ...
            ***REMOVED*** Add faculty assignments
            engine.assignments = assignments
            engine._assign_faculty(faculty, blocks)
            assignments = engine.assignments

            ***REMOVED*** Rollback any database changes (we don't want to persist yet)
            self.db.rollback()
            ...
            self.db.rollback()
            ***REMOVED*** Log error but don't crash - return None to indicate failure
            import logging
...
class GeneratorWithVariation(CandidateGenerator):
    ...
    def generate_with_perturbation(
        ...
        ***REMOVED*** Get available persons and blocks
        residents = self._get_residents()
        ...
        ***REMOVED*** Randomly modify some assignments
        for _ in range(num_to_perturb):
            if not assignments:
                break

            ***REMOVED*** Pick random assignment to modify
            idx = random.randint(0, len(assignments) - 1)
            old_assignment = assignments[idx]

            ***REMOVED*** Swap person or block
        ...
        return ScheduleCandidate(
            ...
            generation_time=0.0,  ***REMOVED*** Perturbation is fast
            feasible=True,  ***REMOVED*** May not actually be feasible
