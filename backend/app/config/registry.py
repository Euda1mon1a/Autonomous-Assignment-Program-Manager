...
class ConfigRegistry:
    ...
    def __init__(
        ...
        ***REMOVED*** Current configuration
        self._current_config: Optional[Settings] = None

        ***REMOVED*** Configuration history for rollback
        self._config_history: List[Settings] = []
        self._max_history = 10

        ***REMOVED*** Change notification callbacks
        self._change_callbacks: List[Callable[[ConfigChangeEvent], None]] = []

        ***REMOVED*** Lock for thread-safe operations
        self._lock = asyncio.Lock()

        ***REMOVED*** Track if registry is initialized
        self._initialized = False

        ***REMOVED*** Register file watcher callback
    ...
    async def initialize(self) -> None:
        ...
        try:
            ***REMOVED*** Load initial configuration
            await self.reload()

            ***REMOVED*** Add .env file to watcher
            env_file = self.loader.get_env_file_path()
            if env_file.exists():
                self.watcher.add_file(env_file)

            ***REMOVED*** Start file watcher
            if self.enable_auto_reload:
    ...
    async def reload(self, validate: bool = True) -> Settings:
        ...
        async with self._lock:
            try:
                ...
                ***REMOVED*** Save current config for rollback
                old_config = self._current_config

                ***REMOVED*** Load new configuration
            ...
            try:
                ...
                ***REMOVED*** Validate partial update
                if validate:
                    ...
                    ***REMOVED*** Merge with current config without validation
    ...
    def _update_config(
        ...
        """
        ***REMOVED*** Add old config to history
    ...
    def _on_file_change(self, file_path: Path) -> None:
        ...
        ***REMOVED*** Reload configuration (run in event loop)
        try:
            ***REMOVED*** Create task to reload
            loop = asyncio.get_event_loop()
            loop.create_task(self._safe_reload())
        except RuntimeError:
            ***REMOVED*** No event loop, reload synchronously
