...
class ConfigLoader:
    ...
    def _find_env_file(self) -> Path:
        ...
        """
        ***REMOVED*** Start from current file and go up to find project root
        ...
        ***REMOVED*** Default to backend/.env
    ...
    def load(self) -> Settings:
        ...
        """
        try:
            ***REMOVED*** Load from .env file
            env_values = self._load_env_file()

            ***REMOVED*** Merge with environment variables (env vars take precedence)
            merged_config = self._merge_with_environment(env_values)

            ***REMOVED*** Create Settings instance
            settings = Settings(**merged_config)

            ***REMOVED*** Track last modified time
    ...
    def _load_env_file(self) -> dict[str, Any]:
        ...
        try:
            ***REMOVED*** Use dotenv_values to parse .env file
            env_values = dotenv_values(
                self.env_file,
                encoding=self.env_file_encoding,
            )

            ***REMOVED*** Convert to proper types
    ...
    def _merge_with_environment(
        ...
        ***REMOVED*** Get all Settings field names
        settings_fields = Settings.model_fields.keys()

        ***REMOVED*** Override with environment variables
    ...
    def _convert_value(self, key: str, value: str) -> Any:
        ...
        ***REMOVED*** List values (comma-separated)
        if key in [
            "CORS_ORIGINS",
            "TRUSTED_HOSTS",
