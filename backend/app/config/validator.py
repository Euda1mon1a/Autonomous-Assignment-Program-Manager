...
class ConfigValidator:
    ...
    ***REMOVED*** Fields that cannot be changed at runtime (require restart)
    ...
    ***REMOVED*** Fields that require special validation
    ...
    def validate(
        ...
        try:
            ***REMOVED*** Validate against Pydantic schema
            self._validate_schema(new_config)

            ***REMOVED*** Validate immutable fields haven't changed
            if current_config:
                self._validate_immutable_fields(new_config, current_config)

            ***REMOVED*** Validate critical fields
            self._validate_critical_fields(new_config)

            ***REMOVED*** Validate resilience thresholds
            self._validate_resilience_thresholds(new_config)

            ***REMOVED*** Validate rate limiting settings
            self._validate_rate_limiting(new_config)

            ***REMOVED*** Validate file upload settings
    ...
    def _validate_schema(self, config: Dict[str, Any]) -> None:
        ...
        """
        try:
            ***REMOVED*** Create Settings instance to validate schema
            Settings(**config)
        except ValidationError as e:
    ...
    def _validate_resilience_thresholds(self, config: Dict[str, Any]) -> None:
        ...
        ***REMOVED*** Validate ranges (0.0 to 1.0)
        thresholds = {
        ...
        ***REMOVED*** Warn about dangerous values
        if max_util > 0.85:
            logger.warning(
    ...
    def _validate_rate_limiting(self, config: Dict[str, Any]) -> None:
        ...
        ***REMOVED*** Warn about overly permissive settings
    ...
    def _validate_file_upload_settings(self, config: Dict[str, Any]) -> None:
        ...
        ***REMOVED*** Validate S3 settings if S3 backend is configured
        if backend == "s3":
