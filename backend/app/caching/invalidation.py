...
class InvalidationReason(str, Enum):
    ...
    MANUAL = "manual"  ***REMOVED*** Manual invalidation (admin action)
    UPDATE = "update"  ***REMOVED*** Resource was updated
    DELETE = "delete"  ***REMOVED*** Resource was deleted
    CREATE = "create"  ***REMOVED*** Related resource was created
    EXPIRED = "expired"  ***REMOVED*** TTL expired
    POLICY = "policy"  ***REMOVED*** Policy-based invalidation
    CASCADE = "cascade"  ***REMOVED*** Cascading invalidation from related resource
    ERROR = "error"  ***REMOVED*** Error recovery
...
class InvalidationEvent:
    ...
    resource: str  ***REMOVED*** Resource type (e.g., "users", "schedules")
    resource_id: Optional[str] = None  ***REMOVED*** Specific resource ID
    reason: InvalidationReason = InvalidationReason.MANUAL
    patterns: list[str] = field(default_factory=list)  ***REMOVED*** Cache key patterns to invalidate
    tags: list[str] = field(default_factory=list)  ***REMOVED*** Cache tags to invalidate
    cascade: bool = False  ***REMOVED*** Whether to cascade to related resources
    metadata: dict[str, Any] = field(default_factory=dict)  ***REMOVED*** Additional context
    timestamp: datetime = field(default_factory=datetime.utcnow)

    def __post_init__(self):
        """Generate patterns if not provided."""
        if not self.patterns and self.resource:
            ***REMOVED*** Generate default patterns
    ...
    def _generate_default_patterns(self) -> list[str]:
        ...
        ***REMOVED*** Pattern for resource collection
        patterns.append(f"/api/{self.resource}*")

        ***REMOVED*** Pattern for specific resource
        if self.resource_id:
...
class CacheInvalidator:
    ...
    Example:
        ...
        ***REMOVED*** Invalidate single resource
        await invalidator.invalidate_resource("users", "123")

        ***REMOVED*** Invalidate with event
        event = InvalidationEvent(
            resource="schedules",
            reason=InvalidationReason.UPDATE,
            cascade=True
        ...
        ***REMOVED*** Register invalidation hook
        async def on_user_update(event: InvalidationEvent):
            ***REMOVED*** Custom invalidation logic
            pass
    ...
    async def invalidate(self, event: InvalidationEvent) -> int:
        ...
        ***REMOVED*** Invalidate by patterns
    ...
    async def invalidate_resource(
        ...
        Example:
            ***REMOVED*** Invalidate all users
            await invalidator.invalidate_resource("users")

            ***REMOVED*** Invalidate specific user
            await invalidator.invalidate_resource("users", "123")

            ***REMOVED*** Invalidate with cascade
        ...
        ***REMOVED*** Handle cascade invalidation
        if cascade:
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        Example:
            ***REMOVED*** Invalidate all user endpoints
            await invalidator.invalidate_pattern("/api/users/*")

            ***REMOVED*** Invalidate specific version
    ...
    def register_hook(
        ...
        Example:
            async def on_user_invalidate(event: InvalidationEvent):
                ***REMOVED*** Invalidate user's related caches
    ...
    async def _run_hooks(self, event: InvalidationEvent) -> None:
        ...
        for hook in hooks:
            try:
                ***REMOVED*** Call hook (may be sync or async)
                result = hook(event)

                ***REMOVED*** Await if coroutine
    ...
    async def _cascade_invalidate(self, event: InvalidationEvent) -> int:
        ...
        ***REMOVED*** Define cascade relationships
        ***REMOVED*** Format: {resource: [related_resources]}
        cascade_map = {
        ...
        for related_resource in related_resources:
            ***REMOVED*** Generate patterns for related resource
            patterns = []

            if event.resource_id:
                ***REMOVED*** Specific resource - invalidate related queries
            ...
            ***REMOVED*** Invalidate collection (might be affected)
...
***REMOVED*** Global invalidator instance
