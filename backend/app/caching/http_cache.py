...
class CacheDirective:
    ...
    Example:
        ***REMOVED*** Public cache with 5-minute expiration
    ...
    ***REMOVED*** Response directives
    public: bool = False  ***REMOVED*** Can be cached by any cache
    private: bool = False  ***REMOVED*** Can only be cached by client
    no_cache: bool = False  ***REMOVED*** Must revalidate before use
    no_store: bool = False  ***REMOVED*** Do not cache at all
    no_transform: bool = False  ***REMOVED*** Do not transform content
    must_revalidate: bool = False  ***REMOVED*** Must revalidate when stale
    proxy_revalidate: bool = False  ***REMOVED*** Proxy must revalidate
    max_age: Optional[int] = None  ***REMOVED*** Max age in seconds
    s_maxage: Optional[int] = None  ***REMOVED*** Shared cache max age
    immutable: bool = False  ***REMOVED*** Never changes (non-standard but widely supported)

    ***REMOVED*** Custom directives
    stale_while_revalidate: Optional[int] = None  ***REMOVED*** Serve stale while revalidating
    stale_if_error: Optional[int] = None  ***REMOVED*** Serve stale if error
    ...
    def to_header(self) -> str:
        ...
        Example:
            ...
            ***REMOVED*** Returns: "public, max-age=300"
        """
        directives = []
    ...
    def is_cacheable(self) -> bool:
        ...
        """
        ***REMOVED*** no-store means never cache
        if self.no_store:
            return False

        ***REMOVED*** Must have public or max_age to be cacheable
...
class HTTPCacheConfig:
    ...
    enabled: bool = True
    default_max_age: int = 300  ***REMOVED*** 5 minutes
    max_cache_size_mb: int = 100  ***REMOVED*** Maximum cache size
    respect_cache_control: bool = True  ***REMOVED*** Honor Cache-Control headers
    default_public: bool = False  ***REMOVED*** Default to public caching
    enable_etag: bool = True  ***REMOVED*** Generate ETags
    enable_last_modified: bool = True  ***REMOVED*** Track Last-Modified
...
class HTTPCache:
    ...
    Example:
        ...
        ***REMOVED*** Retrieve response
        cached = await cache.get("/api/users")

        ***REMOVED*** Check if modified
    ...
    def _make_cache_key(self, key: str, vary_values: Optional[dict[str, str]] = None) -> str:
        ...
        ***REMOVED*** Add vary values to key if present
        if vary_values:
    ...
    async def is_modified(
        ...
        Example:
            ...
            if not is_modified:
                ***REMOVED*** Return 304 Not Modified
                return Response(status_code=304)
        """
        cached = await self.get(key, vary_values)

        if cached is None:
            ***REMOVED*** No cached version, assume modified
            return True

        ***REMOVED*** Check ETag (If-None-Match)
        if etag and cached.etag:
            from app.caching.etag import ETagGenerator

            ***REMOVED*** Parse If-None-Match (can be comma-separated list)
            etag_list = ETagGenerator.parse_if_none_match(etag)

            ***REMOVED*** If any ETag matches, not modified
            if ETagGenerator.matches_any(cached.etag, etag_list):
                return False

        ***REMOVED*** Check Last-Modified (If-Modified-Since)
        if if_modified_since and cached.last_modified:
            ***REMOVED*** Round to seconds (HTTP dates don't include milliseconds)
        ...
        ***REMOVED*** If we have either condition but reached here, it's modified
        if etag or if_modified_since:
            return True

        ***REMOVED*** No conditions provided, assume modified
        return True
