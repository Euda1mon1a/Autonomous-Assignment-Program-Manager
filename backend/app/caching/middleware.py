...
class HTTPCacheMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        ***REMOVED*** Methods that trigger cache invalidation
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        """
        ***REMOVED*** Skip if caching disabled
    ...
    async def _handle_cacheable_request(self, request: Request, call_next):
        ...
        ***REMOVED*** Check for conditional request headers
        ...
        ***REMOVED*** Check if resource has been modified
        ...
        ***REMOVED*** Cache miss - get fresh response
        response = await call_next(request)

        ***REMOVED*** Cache successful responses
    ...
    async def _handle_mutation_request(self, request: Request, call_next):
        ...
        ***REMOVED*** Invalidate cache on successful mutations
    ...
    async def _cache_response(
        ...
        """
        ***REMOVED*** Only cache if response has body
        if not hasattr(response, "body") or not response.body:
            return

        ***REMOVED*** Check if response is cacheable
        ...
        ***REMOVED*** Generate ETag if enabled
        etag = None
        if self.enable_etag:
            etag = self.etag_generator.generate(response.body)
            response.headers["ETag"] = etag

        ***REMOVED*** Add Last-Modified if enabled
        last_modified = None
        ...
        ***REMOVED*** Add Cache-Control if not present
        ...
        ***REMOVED*** Create cached response
    ...
    def _create_response_from_cache(self, cached: CachedResponse) -> Response:
        ...
        Returns:
            Response object
        """
        ***REMOVED*** Create response with cached data
        response = Response(
            content=cached.body,
        ...
        ***REMOVED*** Add Age header (RFC 7234)
        response.headers["Age"] = str(cached.age)

        ***REMOVED*** Add X-Cache header for debugging
    ...
    async def _invalidate_for_mutation(self, request: Request) -> None:
        ...
        """
        ***REMOVED*** Extract resource from path
        ...
        ***REMOVED*** Determine invalidation reason
        reason_map = {
        ...
        ***REMOVED*** Extract resource ID if present
        ...
        ***REMOVED*** Invalidate
        await self.invalidator.invalidate_resource(
            ...
            reason=reason,
            cascade=True,  ***REMOVED*** Cascade to related resources
        )
    ...
    def _extract_resource(self, path: str) -> str | None:
        ...
        """
        ***REMOVED*** Remove leading slash and split
        parts = path.lstrip("/").split("/")

        ***REMOVED*** Expect format: /api/{resource}/{id}
    ...
    def _extract_resource_id(self, path: str, resource: str) -> str | None:
        ...
        ***REMOVED*** Find resource index
        try:
            resource_index = parts.index(resource)

            ***REMOVED*** ID should be next part
