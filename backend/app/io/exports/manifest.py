...
***REMOVED*** Supported export types
...
def canonicalize_row(row: dict) -> str:
    ...
    Example:
        ...
        '{"id":123,"name":"John"}'  ***REMOVED*** Same output despite different order
    """
...
def generate_manifest(
    ...
    ***REMOVED*** Compute hash
    sha256_rows = _compute_rows_hash(rows)

    ***REMOVED*** Generate UTC timestamp
    generated_at = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")

    ***REMOVED*** Create and validate manifest
    manifest = ExportManifest(
...
def write_manifest(manifest: dict, path: Path) -> None:
    ...
    """
    ***REMOVED*** Validate manifest structure
...
def verify_manifest(rows: list[dict], manifest_path: Path) -> tuple[bool, str]:
    ...
    """
    ***REMOVED*** Check manifest file exists
    if not manifest_path.exists():
        return False, f"Manifest file not found: {manifest_path}"

    ***REMOVED*** Load and validate manifest
    try:
