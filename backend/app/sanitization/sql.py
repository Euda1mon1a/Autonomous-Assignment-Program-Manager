...
***REMOVED*** Common SQL injection patterns to detect
SQL_INJECTION_PATTERNS = [
    ***REMOVED*** Comment-based injection
    r"--",
    r"/\*",
    r"\*/",
    r"***REMOVED***",
    ***REMOVED*** Union-based injection
    r"\bunion\b.*\bselect\b",
    r"\bor\b.*\b=\b.*\b(or|and)\b",
    ***REMOVED*** Boolean-based injection
    r"'\s*(or|and)\s*'?\w*'?\s*=\s*'?\w*'?",
    r'"\s*(or|and)\s*"?\w*"?\s*=\s*"?\w*"?',
    r"1\s*=\s*1",
    r"'\s*or\s*'1'\s*=\s*'1",
    r'"\s*or\s*"1"\s*=\s*"1',
    ***REMOVED*** Time-based injection
    r"\bsleep\s*\(",
    r"\bwaitfor\s+delay\b",
    r"\bbenchmark\s*\(",
    ***REMOVED*** Stacked queries
    r";\s*(drop|delete|update|insert|create|alter)\s+",
    ***REMOVED*** System commands
    r"\bexec\s*\(",
    r"\bexecute\s*\(",
    r"\bxp_cmdshell\b",
    ***REMOVED*** Information schema access
    r"\binformation_schema\b",
    r"\bsys\.\b",
    r"\bsystem\.\b",
    ***REMOVED*** Database-specific functions
    r"\bload_file\s*\(",
    r"\boutfile\s+",
    r"\bdumpfile\s+",
    ***REMOVED*** Subqueries
...
***REMOVED*** Keywords that should not appear in user input
...
***REMOVED*** Valid identifier pattern (alphanumeric + underscore, must start with letter)
...
def detect_sql_injection(input_string: str, strict: bool = True) -> bool:
    ...
    ***REMOVED*** In strict mode, also check for dangerous keywords
    if strict:
    ...
    ***REMOVED*** Check for multiple SQL statements (semicolon followed by keyword)
    if ";" in input_string:
...
def sanitize_sql_input(
    ...
    ***REMOVED*** Remove SQL wildcards unless explicitly allowed
...
def validate_identifier(identifier: str, max_length: int = 64) -> str:
    ...
    ***REMOVED*** Check against dangerous keywords
...
def validate_like_pattern(pattern: str, max_length: int = 100) -> str:
    ...
    ***REMOVED*** Remove wildcards temporarily for injection check
    temp_pattern = pattern.replace("%", "").replace("_", "")

    ***REMOVED*** Check for SQL injection in non-wildcard parts
...
def escape_like_wildcards(text: str) -> str:
    ...
    ***REMOVED*** Escape backslash first, then wildcards
    text = text.replace("\\", "\\\\")
    text = text.replace("%", "\\%")
...
def is_safe_order_by(column: str, allowed_columns: set[str]) -> bool:
    ...
    ***REMOVED*** Remove DESC/ASC suffix if present
    column_clean = column.upper().replace(" DESC", "").replace(" ASC", "").strip()

    ***REMOVED*** Validate identifier format
    try:
        validate_identifier(column_clean)
    except SQLInjectionError:
        return False

    ***REMOVED*** Check against whitelist
...
def sanitize_numeric_id(value: str) -> int | None:
    ...
    ***REMOVED*** Remove whitespace
    value = value.strip()

    ***REMOVED*** Check if it's a valid integer
    try:
        numeric_id = int(value)
    except ValueError:
        return None

    ***REMOVED*** Check for reasonable range (prevent integer overflow attacks)
    if numeric_id < 0 or numeric_id > 2147483647:  ***REMOVED*** Max 32-bit signed int
        return None
...
def check_query_complexity(query: str, max_length: int = 1000) -> bool:
    ...
    ***REMOVED*** Check for excessive nesting (subqueries)
    if query.count("(") > 10 or query.count(")") > 10:
        return False

    ***REMOVED*** Check for excessive UNION statements
