...
def parse_block(
    ...
    ***REMOVED*** Parse FMIT if requested
    ...
    if output == "json":
        ...
        ***REMOVED*** Rich table output
        _render_block_summary(result, fmit_weeks)

    ***REMOVED*** Generate markdown if requested
    if markdown_file:
    ...
    for f in fmit_weeks:
        if f.block_number != current_block:
            if current_block is not None:
                table.add_row("", "", "", "")  ***REMOVED*** Separator
...
def _render_block_summary(result, fmit_weeks=None):
    ...
    ***REMOVED*** Warnings
    if result.warnings:
        ...
        for w in result.warnings[:5]:  ***REMOVED*** Show first 5
            console.print(f"  ⚠ {w}")
