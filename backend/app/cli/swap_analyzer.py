...
Examples:
    ***REMOVED*** Find swap candidates for a specific faculty and week
    python -m app.cli.swap_analyzer -f fmit_schedule.xlsx -F "Johnson" -w 2025-03-10

    ***REMOVED*** Analyze all faculty with alternating patterns
...
***REMOVED*** Add parent to path for imports when running as script
...
def print_full_analysis(swap_finder: SwapFinder, verbose: bool = False) -> None:
    ...
    ***REMOVED*** Alternating patterns
    print_alternating_patterns(swap_finder)

    ***REMOVED*** External conflicts summary
...
def main() -> int:
    ...
    ***REMOVED*** Validate file exists
    file_path = Path(args.file)
    if not file_path.exists():
        print(f"Error: File not found: {args.file}", file=sys.stderr)
        return 1

    ***REMOVED*** Load external conflicts if provided
    external_conflicts = []
    ...
    ***REMOVED*** Create SwapFinder
    try:
        swap_finder = SwapFinder.from_fmit_file(
            ...
            include_absence_conflicts=False,  ***REMOVED*** No DB in CLI mode
        )
    except Exception as e:
    ...
    ***REMOVED*** Dispatch to appropriate action
    if args.analyze_all:
