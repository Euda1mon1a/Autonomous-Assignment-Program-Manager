...
def backup(output: Optional[str], compress: bool) -> None:
    ...
    try:
        ...
        ***REMOVED*** Parse database URL
        import subprocess
        from urllib.parse import urlparse

        parsed = urlparse(settings.DATABASE_URL)

        ***REMOVED*** Build pg_dump command
        cmd = [
            "pg_dump",
            "-h", parsed.hostname or "localhost",
            "-p", str(parsed.port or 5432),
            "-U", parsed.username or "postgres",
            "-F", "c",  ***REMOVED*** Custom format
            "-f", str(output_path),
        ]

        if compress:
            cmd.extend(["-Z", "9"])  ***REMOVED*** Maximum compression

        cmd.append(parsed.path.lstrip("/"))  ***REMOVED*** Database name

        ***REMOVED*** Set password environment variable
...
def restore(file: str, confirm: bool) -> None:
    ...
    try:
        ...
        ***REMOVED*** Build pg_restore command
        cmd = [
            "pg_restore",
            "-h", parsed.hostname or "localhost",
            "-p", str(parsed.port or 5432),
            ...
            "--clean",  ***REMOVED*** Drop existing objects
            "--if-exists",  ***REMOVED*** Ignore errors if objects don't exist
            str(file_path),
        ]
        ...
        ***REMOVED*** Run pg_restore
        ...
        if result.returncode != 0:
            ***REMOVED*** pg_restore may have warnings, check stderr
...
def cleanup(days: int, confirm: bool) -> None:
    ...
    try:
        ...
        ***REMOVED*** Count records to delete
        ...
        ***REMOVED*** Delete old assignments
...
def vacuum() -> None:
    ...
    try:
        ...
        ***REMOVED*** VACUUM must be run outside a transaction
