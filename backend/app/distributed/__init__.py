...
Example:
    ...
    ***REMOVED*** Basic lock usage
    async with DistributedLock("my-resource", timeout=10) as lock:
        ***REMOVED*** Critical section - only one process can execute this
        await perform_critical_operation()

    ***REMOVED*** Reentrant lock usage
    lock = ReentrantLock("user:123:update", owner_id="worker-1")
    async with lock:
        ***REMOVED*** Can acquire same lock multiple times from same owner
        async with lock:
            await nested_operation()

    ***REMOVED*** Fair lock usage (FIFO queue)
    async with FairLock("schedule-generation", timeout=30) as lock:
        ***REMOVED*** Locks acquired in order they were requested
