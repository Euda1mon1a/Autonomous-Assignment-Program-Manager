...
***REMOVED*** Prometheus metrics (if available)
try:
...
class ChaosExperimentStatus(str, enum.Enum):
    ...
    PENDING = "pending"  ***REMOVED*** Scheduled but not started
    RUNNING = "running"  ***REMOVED*** Currently executing
    COMPLETED = "completed"  ***REMOVED*** Finished successfully
    ROLLED_BACK = "rolled_back"  ***REMOVED*** Stopped due to SLO breach
    FAILED = "failed"  ***REMOVED*** Technical failure
    CANCELLED = "cancelled"  ***REMOVED*** Manually stopped


class BlastRadiusScope(str, enum.Enum):
    """Scope of blast radius for chaos experiments."""

    SINGLE_REQUEST = "single_request"  ***REMOVED*** Single request only
    PERCENTAGE = "percentage"  ***REMOVED*** % of requests/operations
    SPECIFIC_USER = "specific_user"  ***REMOVED*** Specific user ID
    SPECIFIC_SERVICE = "specific_service"  ***REMOVED*** Specific service/component
    ZONE_ISOLATED = "zone_isolated"  ***REMOVED*** Within scheduling zone
    GLOBAL = "global"  ***REMOVED*** Entire system (use with caution!)
...
class SLOThreshold:
    ...
    comparison: str = "less_than"  ***REMOVED*** less_than, greater_than, equals
    breach_count_limit: int = 3  ***REMOVED*** Consecutive breaches before rollback
    description: str = ""
...
class ChaosExperimentConfig:
    ...
    blast_radius: float = 0.1  ***REMOVED*** 0.0 to 1.0 (10% by default)
    ...
    target_component: Optional[str] = None  ***REMOVED*** Specific component to target
    target_zone_id: Optional[uuid.UUID] = None  ***REMOVED*** Specific zone (for zone isolation)
    target_user_id: Optional[uuid.UUID] = None  ***REMOVED*** Specific user (for user isolation)
...
class ChaosExperimentRecord(Base):
    ...
    ***REMOVED*** Blast radius configuration
    ...
    ***REMOVED*** Map Python attr 'experiment_metadata' to DB column 'metadata' (reserved name workaround)
    ...
    def check_metric(self, metric_name: str, current_value: float) -> bool:
        ...
        for threshold in self.thresholds:
            ...
            if threshold.is_breached(current_value):
                ...
                ***REMOVED*** Reset breach count if metric is healthy
...
class MemoryStressor:
    ...
    def allocate(self) -> None:
        ...
        try:
            ***REMOVED*** Allocate memory in chunks
            chunk_size = 1024 * 1024  ***REMOVED*** 1MB chunks
            for _ in range(self.size_mb):
                buffer = bytearray(chunk_size)
                ***REMOVED*** Touch the memory to ensure it's actually allocated
                buffer[0] = 1
                buffer[-1] = 1
...
class ServiceFailureSimulator:
    ...
    async def simulate(self) -> None:
        ...
        if self.failure_mode == "unavailable":
            ...
            await asyncio.sleep(30)  ***REMOVED*** Long delay to simulate timeout
...
***REMOVED*** =============================================================================
***REMOVED*** Chaos Experiment Orchestration
***REMOVED*** =============================================================================
...
class ChaosExperiment:
    ...
    def should_apply_to_request(
        ...
        ***REMOVED*** Scope-based filtering
        if scope == BlastRadiusScope.SPECIFIC_USER:
            ...
            ***REMOVED*** Random sampling based on blast radius percentage
    ...
    def start(self) -> None:
        ...
        ***REMOVED*** Persist to database if session provided
        if self.db:
            self._save_to_db()
...
class ChaosExperimentScheduler:
    ...
    def schedule_experiment(
        ...
        ***REMOVED*** Save to database with scheduled status
