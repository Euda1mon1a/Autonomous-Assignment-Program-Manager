...
class TimeSeriesAnalyzer:
    ...
    async def _get_assignment_count_series(
        ...
        ***REMOVED*** Resample based on granularity
        if granularity == "week":
    ...
    async def _get_coverage_rate_series(
        ...
        ***REMOVED*** Get total capacity
        person_result = await self.db.execute(select(func.count(Person.id)))
        total_capacity = person_result.scalar() or 1

        ***REMOVED*** Get assignment counts
        assignment_series = await self._get_assignment_count_series(
            start_date, end_date, granularity
        )

        ***REMOVED*** Calculate coverage rate
        ***REMOVED*** For daily: coverage = assignments / capacity
        ***REMOVED*** For weekly/monthly: coverage = avg assignments per day / capacity
        if granularity == "day":
            coverage_series = assignment_series / total_capacity
        else:
            ***REMOVED*** For aggregated periods, divide by number of days in period
            coverage_series = assignment_series / (total_capacity * 7)  ***REMOVED*** Assume 7 days per week
    ...
    async def _get_workload_series(
        ...
        ***REMOVED*** Convert to hours (4 hours per half-day block)
    ...
    async def _get_utilization_series(
        ...
        ***REMOVED*** Utilization = assignments / total capacity
    ...
    def calculate_trend(
        ...
        ***REMOVED*** Calculate linear regression
    ...
    def detect_seasonality(
        ...
        ***REMOVED*** Simple autocorrelation at lag=period
