...
class DataAggregator:
    ...
    async def get_workload_distribution(
        ...
        ***REMOVED*** Calculate statistics
        ...
        for row in workload_data:
            estimated_hours = row.assignment_count * 4  ***REMOVED*** 4 hours per half-day
            distribution.append({
    ...
    async def aggregate_by_rotation(
        ...
        for row in rotation_data:
            aggregated[row.name] = {
                "count": row.count,
                "percentage": 0,  ***REMOVED*** Will calculate after getting total
            }
    ...
    async def get_person_rotation_summary(
        ...
        for row in rotation_data:
            rotations.append({
                "rotation": row.name,
                "blocks": row.count,
                "weeks": round(row.count / 4, 2),  ***REMOVED*** 4 blocks per week (AM/PM * 2.5 days)
            })
