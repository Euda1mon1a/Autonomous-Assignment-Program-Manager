...
class MetricCalculator:
    ...
    async def calculate_schedule_metrics(
        ...
        """
        ***REMOVED*** Build query for assignments in date range
        query = (
            select(Assignment)
            .join(Block)
            .where(
        ...
        ***REMOVED*** Get total blocks in period
        ...
        ***REMOVED*** Calculate workload (estimate 4 hours per half-day block)
        total_hours = total_assignments * 4
        days = (end_date - start_date).days + 1
        weeks = days / 7
        avg_weekly_hours = total_hours / weeks if weeks > 0 else 0

        ***REMOVED*** Rotation distribution
    ...
    async def calculate_compliance_metrics(
        ...
        """
        ***REMOVED*** Get all residents (or specific person)
        ...
        for person in persons:
            ***REMOVED*** Get assignments for this person in date range
            query = (
                select(Assignment)
                .join(Block)
                .where(
            ...
            ***REMOVED*** Calculate weekly hours
        ...
        ***REMOVED*** Calculate compliance score (100 = perfect, 0 = many violations)
    ...
    async def calculate_resilience_metrics(
        ...
        ***REMOVED*** Get assignments per day
        query = (
        ...
        for day_date, assignment_count in daily_assignments:
            if total_capacity > 0:
                ...
                ***REMOVED*** N-1 vulnerable if losing 1 person would exceed capacity
                if utilization > 0.9:
                    n1_vulnerable_count += 1

        ***REMOVED*** Get swap count
    ...
    def _check_80_hour_rule(
        ...
        ***REMOVED*** Check each 4-week rolling window
        ...
        for i in range(len(week_nums) - 3):
            ...
            ***REMOVED*** Estimate hours (4 per half-day block)
            total_hours = len(window_assignments) * 4

            if total_hours > 80 * 4:  ***REMOVED*** 80 hours/week * 4 weeks
                violations.append(
                    {
                        "type": "80_hour_rule",
    ...
    def _check_1_in_7_rule(self, assignments: list[Assignment]) -> list[dict[str, Any]]:
        ...
        ***REMOVED*** Check for 7 consecutive days with assignments
        consecutive_days = 0
        last_date = None
    ...
    def _calculate_weekly_hours(self, assignments: list[Assignment]) -> list[float]:
        ...
        ***REMOVED*** Convert block counts to hours (4 hours per half-day)
