...
------
    ...
    ***REMOVED*** Quick forecast
    result = forecast_workload(
        workload_history=[8.0, 10.0, 9.5, 11.0, 10.5, ...],
        periods=7
    )

    ***REMOVED*** Advanced usage with custom parameters
...
***REMOVED*** Try to import statsmodels ARIMA
try:
    ...
    ARIMA = None  ***REMOVED*** type: ignore
    adfuller = None  ***REMOVED*** type: ignore
...
class ARIMAOrder(TypedDict):
    ...
    p: int  ***REMOVED*** AR order (autoregressive)
    d: int  ***REMOVED*** Differencing order
    q: int  ***REMOVED*** MA order (moving average)
...
class ARIMAConfig:
    ...
    max_p: int = 5  ***REMOVED*** Maximum AR order to search
    max_d: int = 2  ***REMOVED*** Maximum differencing order
    max_q: int = 5  ***REMOVED*** Maximum MA order to search
    criterion: str = "aic"  ***REMOVED*** Selection criterion: "aic" or "bic"
    significance_level: float = 0.05  ***REMOVED*** For stationarity test
    confidence_level: float = 0.95  ***REMOVED*** For prediction intervals
    seasonal: bool = False  ***REMOVED*** Seasonal ARIMA (SARIMA) - not implemented yet


***REMOVED*** =============================================================================
***REMOVED*** Core ARIMA Forecaster Class
***REMOVED*** =============================================================================
...
class ARIMAForecaster:
    ...
    def fit(
        ...
        """
        ***REMOVED*** Convert input to WorkloadSeries
        ...
        if not HAS_STATSMODELS:
            ***REMOVED*** Fallback: use simple moving average model
            self._fit_fallback()
            return self

        ***REMOVED*** Determine differencing order (d) if auto
        if self.auto_order:
    ...
    def _determine_differencing_order(self) -> int:
        ...
        for d in range(self.config.max_d + 1):
            ...
            ***REMOVED*** ADF test for stationarity
            try:
        ...
        return 1  ***REMOVED*** Default if no stationarity found
        ...
        for p in range(self.config.max_p + 1):
            for q in range(self.config.max_q + 1):
                ...
                try:
                    ...
                    continue  ***REMOVED*** Skip failed models
    ...
    def forecast(
        ...
        if self.model_ is not None and HAS_STATSMODELS:
            ***REMOVED*** Use statsmodels forecast
            try:
                ...
                ***REMOVED*** Convert to numpy arrays for consistent indexing
            ...
            ***REMOVED*** Fallback forecast
            forecasts = self._forecast_fallback(periods, alpha)

        ***REMOVED*** Build diagnostics
    ...
    def _get_diagnostics(self) -> ModelDiagnostics:
        ...
        if self.model_ is not None and HAS_STATSMODELS:
            ...
            ***REMOVED*** Fallback diagnostics
            n = len(self._series.values)
            rss = (
                float(np.sum(self.residuals_**2)) if self.residuals_ is not None else 0
            )
            k = sum(self.order_) + 1  ***REMOVED*** Number of parameters
            aic = n * np.log(rss / n) + 2 * k
            bic = n * np.log(rss / n) + k * np.log(n)

        ***REMOVED*** Check stationarity
        is_stationary = True
...
def auto_select_arima_order(
    ...
    if forecaster.order_ is None:
        return {"p": 1, "d": 1, "q": 1}  ***REMOVED*** Default
