...
class ReportGenerator:
    ...
    def generate_monthly_report(self, year: int, month: int) -> MonthlyReport:
        ...
        ***REMOVED*** Get blocks and assignments
        blocks = (
            self.db.query(Block)
    ...
    def generate_resident_report(
        ...
        ***REMOVED*** Build assignment dicts with dates
        assignment_dicts = [
            {
                "id": str(a.id),
    ...
    def generate_compliance_report(
        ...
        """
        ***REMOVED*** Get schedule runs in period
        schedule_runs = (
            self.db.query(ScheduleRun)
            .filter(
                and_(
        ...
        ***REMOVED*** Get blocks for compliance rate calculation
        blocks = (
            self.db.query(Block)
            .filter(and_(Block.date >= start_date, Block.date <= end_date))
            .all()
        ...
        ***REMOVED*** Get all assignments to check supervision
        assignments = (
            self.db.query(Assignment)
            .join(Block)
            .join(Person)
        ...
        ***REMOVED*** Recommendations
        ...
        return {
            ...
            "details": {
                ...
                "supervision_issues": supervision_issues[:10],  ***REMOVED*** Top 10
            },
            "charts": {
                "violations_by_run": [
                    {
