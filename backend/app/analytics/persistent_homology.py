...
***REMOVED*** Try to import TDA libraries
try:
...
***REMOVED*** Try to import dimensionality reduction for embeddings
try:
...
class TopologicalFeature:
    ...
    def is_significant(self) -> bool:
        ...
        """
        if self.birth == 0:
            return self.persistence > 0.1  ***REMOVED*** Absolute threshold for birth at 0
        return (self.persistence / self.birth) > 0.5  ***REMOVED*** Relative threshold
...
class PersistentScheduleAnalyzer:
        ...
        ***REMOVED*** Standardize features
        if HAS_SKLEARN:
            ...
            ***REMOVED*** Manual standardization
            ...
            std = np.std(features, axis=0) + 1e-8  ***REMOVED*** Avoid division by zero
            features_scaled = (features - mean) / std

        ***REMOVED*** Apply dimensionality reduction
        if method == "pca" and HAS_SKLEARN:
            ...
            ***REMOVED*** t-SNE works better with more initial dimensions
            ...
            ***REMOVED*** Simple manual embedding: take first n_components features
            point_cloud = features_scaled[:, :n_components]
        else:
            ***REMOVED*** Fallback: use first n_components dimensions
    ...
    def _extract_assignment_features(self, assignments: list[Any]) -> np.ndarray:
        ...
        ***REMOVED*** Build lookup maps for categorical encoding
        ...
        ***REMOVED*** Role encoding
        ...
        for assignment in assignments:
            ***REMOVED*** Get block for temporal features
            block = self.db.query(Block).filter(Block.id == assignment.block_id).first()

            if not block or not block.date:
                ***REMOVED*** Skip assignments without valid blocks
                continue

            ***REMOVED*** Feature vector for this assignment
            feature_vec = [
                ***REMOVED*** Categorical encodings
                ...
                ***REMOVED*** Temporal features
                block.date.toordinal(),  ***REMOVED*** Absolute day number
                block.date.weekday(),  ***REMOVED*** Day of week (0-6)
                block.date.isocalendar()[1],  ***REMOVED*** Week of year (1-53)
                1 if block.session == "AM" else 0,  ***REMOVED*** Session (AM=1, PM=0)
                ***REMOVED*** Workload proxy (could be computed from role)
    ...
    def compute_persistence_diagram(
        ...
        ***REMOVED*** Compute persistent homology using ripser
        ...
        try:
            ...
            ***REMOVED*** Parse results into TopologicalFeature objects
            h0_features = []
            h1_features = []
            h2_features = []

            ***REMOVED*** H0 (connected components)
            if len(dgms) > 0:
                for birth, death in dgms[0]:
                    if not np.isinf(death):  ***REMOVED*** Filter out infinite persistence
                        h0_features.append(
                            TopologicalFeature(
                                dimension=0,
                                birth=float(birth),
            ...
            ***REMOVED*** Return empty diagram on error
    ...
    def _mock_persistence_diagram(self, point_cloud: np.ndarray) -> PersistenceDiagram:
        ...
        """
        ***REMOVED*** Compute pairwise distances
        from scipy.spatial.distance import pdist, squareform

        distances = squareform(pdist(point_cloud))

        ***REMOVED*** Mock H0: connected components based on distance threshold
        h0_features = []
        ...
        return PersistenceDiagram(
            h0_features=h0_features,
            h1_features=[],  ***REMOVED*** No mock H1
            h2_features=[],  ***REMOVED*** No mock H2
            max_dimension=0,
        )
    ...
    def extract_schedule_voids(
        ...
        ***REMOVED*** Get significant H2 features
        ...
        ***REMOVED*** Get date range from assignments
        from app.models.block import Block
        ...
        ***REMOVED*** Map each H2 feature to a coverage void
        for idx, feature in enumerate(significant_h2):
            ***REMOVED*** Estimate affected date range based on persistence
            ***REMOVED*** Higher persistence = larger void = longer time span
            void_span_days = int(feature.persistence * date_range_days / 10)
            void_span_days = max(1, min(void_span_days, date_range_days))

            ***REMOVED*** Estimate start date (proportional to birth time)
            offset_days = int(
                feature.birth
                * date_range_days
            ...
            ***REMOVED*** Severity based on persistence and birth time
            ...
            voids.append(
                CoverageVoid(
                    ...
                    affected_rotations=[],  ***REMOVED*** Would need clustering to determine
                    severity=severity,
    ...
    def detect_cyclic_patterns(
        ...
        ***REMOVED*** Get significant H1 features (loops)
        ...
        ***REMOVED*** Common cycle lengths in medical residency (days)
        common_cycles = [7, 14, 21, 28]  ***REMOVED*** Weekly, biweekly, 3-week, 4-week

        for idx, feature in enumerate(significant_h1):
            ***REMOVED*** Estimate cycle length from persistence
            ***REMOVED*** Higher persistence = more stable cycle
            ***REMOVED*** Birth time relates to cycle period
            estimated_cycle = feature.birth * 30  ***REMOVED*** Scale to days

            ***REMOVED*** Snap to nearest common cycle length
            cycle_length = min(common_cycles, key=lambda x: abs(x - estimated_cycle))

            ***REMOVED*** Strength based on persistence (how consistent the pattern is)
            ...
            patterns.append(
                CyclicPattern(
                    ...
                    residents_involved=[],  ***REMOVED*** Would need clustering to determine
                    rotations_involved=[],  ***REMOVED*** Would need clustering to determine
                    strength=strength,
                    persistence=feature.persistence,
                )
            )
    ...
    def compute_structural_anomaly_score(self, diagram: PersistenceDiagram) -> float:
        ...
        """
        ***REMOVED*** Baseline expectations for a "normal" schedule
        EXPECTED_H0_COUNT = 5  ***REMOVED*** Some clustering is normal
        EXPECTED_H1_COUNT = 3  ***REMOVED*** A few cyclic patterns expected
        EXPECTED_H2_COUNT = 0  ***REMOVED*** Voids are bad (should be 0)

        ***REMOVED*** Count significant features
        ...
        ***REMOVED*** Deviation from expectations
        h0_dev = abs(sig_h0 - EXPECTED_H0_COUNT) / max(EXPECTED_H0_COUNT, 1)
        h1_dev = abs(sig_h1 - EXPECTED_H1_COUNT) / max(EXPECTED_H1_COUNT, 1)
        h2_excess = sig_h2  ***REMOVED*** Any H2 is anomalous

        ***REMOVED*** Weighted anomaly score
        anomaly_score = (
            0.3 * min(1.0, h0_dev)  ***REMOVED*** Too many/few clusters
            + 0.3 * min(1.0, h1_dev)  ***REMOVED*** Unusual cyclic patterns
            + 0.4 * min(1.0, h2_excess / 3)  ***REMOVED*** Voids are the worst
        )
    ...
    def compare_schedules_topologically(
        ...
        ***REMOVED*** Compute bottleneck distance for each dimension
        distances = []
        ...
        try:
            ...
            ***REMOVED*** Combined distance (weighted average across dimensions)
            if distances:
                total_distance = float(np.mean(distances))
            else:
                ***REMOVED*** No features to compare
                total_distance = 0.0
    ...
    def analyze_schedule(
        ...
        ***REMOVED*** Get assignments in date range
