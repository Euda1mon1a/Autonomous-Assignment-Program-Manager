...
def calculate_fairness_index(assignments: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ***REMOVED*** Convert to fairness index (1 - Gini, so higher is better)
    fairness = 1 - abs(gini)
...
def calculate_preference_satisfaction(
    ...
    ***REMOVED*** Build preference map: person_id -> set of preferred rotation_template_ids
    pref_map = defaultdict(set)
    for pref in preferences:
        pref_map[pref["person_id"]].add(pref["rotation_template_id"])

    ***REMOVED*** Check how many assignments match preferences
    matched = 0
    total_with_prefs = 0
...
def calculate_consecutive_duty_stats(
    ...
    """
    ***REMOVED*** Filter assignments for this person and sort by date
    person_assignments = [
    ...
    ***REMOVED*** Track consecutive days
    duty_dates = set()
    ...
    ***REMOVED*** Calculate consecutive streaks
    max_consecutive = 1
    ...
    for i in range(1, len(duty_dates_sorted)):
        ...
        ***REMOVED*** Check if dates are consecutive
    ...
    ***REMOVED*** Determine status based on ACGME guidelines (max 6 consecutive days)
