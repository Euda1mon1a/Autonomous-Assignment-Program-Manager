...
class APIAnalyticsService:
    ...
    def __init__(self, redis_client: redis.Redis | None = None):
        ...
        self._retention_days = 30  ***REMOVED*** Keep metrics for 30 days

        ***REMOVED*** Key prefixes for different metric types
    ...
    ***REMOVED*** =========================================================================
    ...
    async def record_request(
        ...
        try:
            ...
            ***REMOVED*** Build pipeline for atomic operations
            pipe = redis_client.pipeline()

            ***REMOVED*** 1. Record request count
            pipe.incr(f"{self._prefix_requests}:{day_key}:total")

            ***REMOVED*** 2. Record endpoint stats
            ...
            ***REMOVED*** 3. Record latency in sorted set for percentile calculations
            ...
            ***REMOVED*** 4. Record user activity
            if user_id:
                ...
                ***REMOVED*** Track unique users globally
                pipe.pfadd(f"{self._prefix_requests}:{day_key}:unique_users", user_id)

                ***REMOVED*** Track endpoint-specific unique users
                pipe.pfadd(f"{endpoint_key}:unique_users", user_id)

            ***REMOVED*** 5. Record API version usage
            if api_version:
            ...
            ***REMOVED*** 6. Record geographic distribution (from IP address)
            if ip_address:
                ***REMOVED*** Simple country/region extraction (in production, use GeoIP database)
            ...
            ***REMOVED*** 7. Record custom dimensions
            ...
            ***REMOVED*** Execute pipeline
            await pipe.execute()

        except Exception as e:
            logger.error(f"Error recording API request metrics: {e}", exc_info=True)
            ***REMOVED*** Don't raise - metrics collection should not break the API

    ***REMOVED*** =========================================================================
    ***REMOVED*** Metrics Retrieval
    ***REMOVED*** =========================================================================
    ...
    async def get_endpoint_stats(
        ...
        try:
            ...
            ***REMOVED*** Aggregate stats across date range
            ...
            while current <= end_date:
                ...
                ***REMOVED*** Get all endpoint keys for this day
                pattern = f"{self._prefix_endpoints}:{day_key}:*"
                async for key in redis_client.scan_iter(match=pattern, count=100):
                    ***REMOVED*** Parse key: api:metrics:endpoints:YYYY-MM-DD:METHOD:ENDPOINT
                    parts = key.split(":", 5)
                    if len(parts) != 6:
                        continue
            ...
            ***REMOVED*** Sort by request count and limit
    ...
    async def get_user_activity(
        ...
        try:
            ...
            while current <= end_date:
                ...
                ***REMOVED*** Get all user keys for this day
                pattern = f"{self._prefix_users}:{day_key}:*"
                async for key in redis_client.scan_iter(match=pattern, count=100):
                    ***REMOVED*** Parse key: api:metrics:users:YYYY-MM-DD:USER_ID
                    parts = key.split(":", 4)
                    if len(parts) != 5:
                        continue
                    ...
                    ***REMOVED*** Skip endpoint sets
                    if key.endswith(":endpoints"):
                        continue

                    ***REMOVED*** Get user stats
                    data = await redis_client.hgetall(key)
                    if data:
    ...
    async def get_geographic_distribution(
        ...
        try:
            ...
            while current <= end_date:
                ...
                async for key in redis_client.scan_iter(match=pattern, count=100):
                    ***REMOVED*** Skip latency and user keys
            ...
            for country, data in geo_data.items():
                ...
                results.append(
                    GeographicDistribution(
                        country=country,
                        region=None,  ***REMOVED*** Would need GeoIP for region
    ...
    async def get_api_version_stats(
        ...
        try:
            ...
            while current <= end_date:
                ...
                ***REMOVED*** Get total requests for adoption rate
                ...
                async for key in redis_client.scan_iter(match=pattern, count=100):
                    ***REMOVED*** Skip endpoint and user keys
            ...
            for version, data in version_data.items():
                ...
                results.append(
                    APIVersionStats(
                        ...
                        deprecated=False,  ***REMOVED*** Would need to maintain this separately
                    )
                )
    ...
    async def get_comprehensive_metrics(
        ...
        try:
            ...
            while current <= end_date:
                ...
                ***REMOVED*** Get total requests
                total_key = f"{self._prefix_requests}:{day_key}:total"
                day_total = await redis_client.get(total_key)
                if day_total:
                    total_requests += int(day_total)

                ***REMOVED*** Collect unique users key for pfcount
                unique_users_sets.append(
                    f"{self._prefix_requests}:{day_key}:unique_users"
                )

                ***REMOVED*** Get error data
                ...
                async for key in redis_client.scan_iter(match=error_pattern, count=100):
                    ...
                    try:
                        ...
                        ***REMOVED*** Get endpoints for this error
                        endpoints = await redis_client.smembers(f"{key}:endpoints")
                        if endpoints:
            ...
            ***REMOVED*** Calculate latency percentiles
    ...
    ***REMOVED*** =========================================================================
    ...
    def _extract_geo_info(ip_address: str) -> tuple[str, str | None]:
        ...
        ***REMOVED*** In production, use: import geoip2.database
