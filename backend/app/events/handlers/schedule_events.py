...
async def on_schedule_created(event: ScheduleCreatedEvent):
    ...
    ***REMOVED*** Send notification to coordinators
    ...
    ***REMOVED*** Initialize cache for schedule
...
async def on_schedule_updated(event: ScheduleUpdatedEvent):
    ...
    ***REMOVED*** Invalidate schedule cache
    await invalidate_cache(f"schedule:{event.schedule_id}")
    await invalidate_cache(f"schedule:{event.schedule_id}:assignments")

    ***REMOVED*** Notify affected faculty/residents
    await send_notification(
...
async def on_schedule_published(event: SchedulePublishedEvent):
    ...
    ***REMOVED*** Send notifications to all assigned faculty/residents
    await send_notification(
    ...
    ***REMOVED*** Generate PDF/Excel exports
    logger.info(f"[EXPORT] Generating exports for schedule {event.schedule_id}")

    ***REMOVED*** Update calendar integrations
...
async def on_assignment_created(event: AssignmentCreatedEvent):
    ...
    ***REMOVED*** Invalidate person's schedule cache
...
async def on_assignment_updated(event: AssignmentUpdatedEvent):
    ...
    ***REMOVED*** Re-validate ACGME compliance if schedule changed
    try:
        from app.scheduling.acgme_validator import ACGMEValidator
        validator = ACGMEValidator()
...
async def on_assignment_deleted(event: AssignmentDeletedEvent):
    ...
    ***REMOVED*** Notify coordinators of gap
    await send_notification(
        user_id="coordinators",
...
async def on_swap_requested(event: SwapRequestedEvent):
    ...
    ***REMOVED*** Notify target person or find matches
    ...
    ***REMOVED*** Check ACGME pre-validation
    try:
        from app.scheduling.acgme_validator import ACGMEValidator
        validator = ACGMEValidator()
...
async def on_swap_approved(event: SwapApprovedEvent):
    ...
    ***REMOVED*** Schedule automatic execution
...
async def on_swap_executed(event: SwapExecutedEvent):
    ...
    ***REMOVED*** Send confirmation emails
    await send_notification(
    ...
    ***REMOVED*** Invalidate affected caches
...
async def on_acgme_violation_detected(event: ACGMEViolationDetectedEvent):
    ...
    ***REMOVED*** Send immediate alert to program director
    await send_notification(
...
async def on_acgme_override_applied(event: ACGMEOverrideAppliedEvent):
    ...
    ***REMOVED*** Track for accreditation reporting
...
async def on_any_event(event):
    ...
    ***REMOVED*** Feed to WebSocket for real-time updates
...
async def update_metrics(metric_name: str, value: float):
    ...
    try:
        ...
        ***REMOVED*** Increment counter or update gauge based on metric name
