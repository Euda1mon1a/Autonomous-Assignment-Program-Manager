...
***REMOVED*** =============================================================================
***REMOVED*** Snapshot Metadata and Validation
***REMOVED*** =============================================================================
...
class SnapshotMetadata(BaseModel):
    ...
    ***REMOVED*** Compression info
    is_compressed: bool = False
    original_size_bytes: int = 0
    compressed_size_bytes: int = 0
    compression_ratio: float = 1.0

    ***REMOVED*** Validation info
    checksum: str
...
***REMOVED*** =============================================================================
***REMOVED*** Snapshot Store Service
***REMOVED*** =============================================================================
...
class SnapshotStore:
    ...
    async def create_snapshot(
        ...
        ***REMOVED*** Check if snapshot is needed (unless forced)
        ...
        ***REMOVED*** Store metadata separately for quick access
        metadata = {
        ...
        ***REMOVED*** Update snapshot with metadata (stored in snapshot_data)
        ...
        ***REMOVED*** Cleanup old snapshots if enabled
    ...
    async def get_latest_snapshot(
        ...
        ***REMOVED*** Extract and decompress data
        snapshot_data = snapshot.snapshot_data
        metadata = snapshot_data.get("_metadata", {})

        ***REMOVED*** Handle compressed data
        if snapshot_data.get("_compressed", False):
            ...
            ***REMOVED*** Remove metadata keys from actual data
            actual_data = {
    ...
    async def restore_from_snapshot(
        ...
        Example:
            ***REMOVED*** Without snapshot: Replay 10,000 events (slow)
            ***REMOVED*** With snapshot: Restore snapshot + replay 100 events (fast)
        """
        ***REMOVED*** Get latest snapshot
        ...
        ***REMOVED*** If requested, replay events after snapshot
        if include_events_after_snapshot:
            ...
            if events_after_snapshot:
                ...
                ***REMOVED*** Note: Actual event replay logic would be implemented
                ***REMOVED*** by the specific aggregate type. We just return the data
                ***REMOVED*** and event count for the caller to handle replay.
    ...
    async def validate_snapshot(
        ...
        ***REMOVED*** Validate schema version compatibility
        ...
        ***REMOVED*** Validate checksum if available
        stored_checksum = metadata.get("checksum")
        if stored_checksum:
            try:
                ***REMOVED*** Decompress if needed
                if snapshot_data.get("_compressed", False):
                    compressed_hex = snapshot_data["_data"]
                    compressed_bytes = bytes.fromhex(compressed_hex)
                    decompressed = self._decompress_data(compressed_bytes)
                    ...
                    ***REMOVED*** Remove metadata for checksum calculation
                    actual_data = {
                        k: v for k, v in snapshot_data.items() if not k.startswith("_")
                    }
        ...
        ***REMOVED*** Validate data can be deserialized
        try:
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _cleanup_old_snapshots(
        ...
        ***REMOVED*** Count-based cleanup: Keep only N most recent snapshots
        all_snapshots = (
            self.db.query(EventSnapshot)
            .filter(EventSnapshot.aggregate_id == aggregate_id)
            .order_by(desc(EventSnapshot.aggregate_version))
        ...
        ***REMOVED*** Time-based cleanup: Delete snapshots older than retention period
        ...
        for snapshot in old_snapshots:
            ***REMOVED*** Don't delete if it's one of the most recent N snapshots
...
async def auto_snapshot_aggregate(
    ...
    ***REMOVED*** This will only create snapshot if frequency rules are met
    try:
        snapshot_id = await snapshot_store.create_snapshot(
            aggregate_id=aggregate_id,
            aggregate_type=aggregate_type,
            ...
            created_by=created_by,
            force=False,  ***REMOVED*** Respect frequency rules
        )
        return snapshot_id
    except Exception as e:
...
async def cleanup_all_old_snapshots(
    ...
    ***REMOVED*** Get all unique aggregate IDs
