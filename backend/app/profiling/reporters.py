...
class PerformanceReporter:
    ...
    def _build_summary(
        ...
        ***REMOVED*** Profile results summary
        if profile_results:
...
<head>
    ...
    <style>
        ...
        h1 {{ color: ***REMOVED***333; margin: 0 0 10px 0; }}
        h2 {{ color: ***REMOVED***555; border-bottom: 2px solid ***REMOVED***007bff; padding-bottom: 10px; }}
        .metric {{
        ...
        .bottleneck.critical {{ border-color: ***REMOVED***dc3545; background: ***REMOVED***fff5f5; }}
        .bottleneck.high {{ border-color: ***REMOVED***fd7e14; background: ***REMOVED***fff8f5; }}
        .bottleneck.medium {{ border-color: ***REMOVED***ffc107; background: ***REMOVED***fffbf0; }}
        .bottleneck.low {{ border-color: ***REMOVED***28a745; background: ***REMOVED***f0fff4; }}
        .insight {{
            border-left: 4px solid ***REMOVED***007bff;
            padding: 10px;
            margin: 10px 0;
            background: ***REMOVED***f0f8ff;
        }}
        .recommendation {{
            padding: 5px 10px;
            background: ***REMOVED***e9ecef;
            margin: 5px 0;
            border-radius: 4px;
        }}
    </style>
...
class FlameGraphGenerator:
    ...
    def generate_from_profile(self, result: ProfileResult) -> dict[str, Any]:
        ...
        ***REMOVED*** Extract call stack from pstats
        flame_data = {
        ...
        ***REMOVED*** Note: Full flame graph generation would require parsing pstats
    ...
    def generate_from_traces(self, traces: list[Trace]) -> dict[str, Any]:
        ...
        ***REMOVED*** Group traces by trace_id
        ...
        ***REMOVED*** Build tree structure for the first trace
        first_trace_id = next(iter(trace_groups))
        spans = trace_groups[first_trace_id]

        ***REMOVED*** Find root span (no parent)
    ...
    def export_to_speedscope(self, result: ProfileResult) -> dict[str, Any]:
        ...
        """
        ***REMOVED*** Speedscope format: https://www.speedscope.app/
        speedscope_data = {
    ...
    def export_to_d3_hierarchy(self, flame_data: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        ***REMOVED*** D3 hierarchy format is the same as our internal format
        return flame_data
