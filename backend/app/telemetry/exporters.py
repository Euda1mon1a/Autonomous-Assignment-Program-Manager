...
class ExporterFactory:
    ...
    def _create_jaeger_exporter(config: ExporterConfig) -> SpanExporter:
        ...
        try:
            ***REMOVED*** Try OTLP exporter first (preferred for Jaeger 1.35+)
            ...
            ***REMOVED*** Fall back to legacy Jaeger exporter
            try:
                from opentelemetry.exporter.jaeger.thrift import JaegerExporter

                ***REMOVED*** Legacy Jaeger uses different endpoint format
                jaeger_endpoint = config.endpoint or "localhost"
                if "://" in jaeger_endpoint:
                    ***REMOVED*** Extract host from URL
                    jaeger_endpoint = jaeger_endpoint.split("://")[1].split(":")[0]

                exporter = JaegerExporter(
                    agent_host_name=jaeger_endpoint,
                    agent_port=6831,  ***REMOVED*** Default Jaeger agent port
                )
...
***REMOVED*** Convenience functions for common configurations
