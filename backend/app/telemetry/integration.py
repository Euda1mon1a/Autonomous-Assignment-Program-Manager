...
def setup_telemetry(app: FastAPI) -> bool:
    ...
    ***REMOVED*** Skip if telemetry is disabled
    if not settings.TELEMETRY_ENABLED:
        logger.info("OpenTelemetry tracing is disabled")
        return False

    try:
        ***REMOVED*** Initialize tracer with exporter configuration
        config = TracerConfig(
        ...
        ***REMOVED*** Note: Exporter is now configured directly in TracerConfig and initialized
        ***REMOVED*** during initialize_tracer(). The code below provides an alternative approach
        ***REMOVED*** using the ExporterFactory if additional exporters are needed.

        ***REMOVED*** Add additional exporter if needed (for multi-exporter setup)
        ***REMOVED*** exporter_config = ExporterConfig(
        ***REMOVED***     exporter_type=ExporterType(settings.TELEMETRY_EXPORTER_TYPE),
        ***REMOVED***     endpoint=settings.TELEMETRY_EXPORTER_ENDPOINT,
        ***REMOVED***     service_name=settings.TELEMETRY_SERVICE_NAME,
        ***REMOVED***     headers=settings.TELEMETRY_EXPORTER_HEADERS,
        ***REMOVED***     insecure=settings.TELEMETRY_EXPORTER_INSECURE,
        ***REMOVED*** )
        ***REMOVED***
        ***REMOVED*** processor = create_span_processor(exporter_config)
        ***REMOVED*** get_tracer_manager().add_span_processor(processor)
        ...
        ***REMOVED*** Add tracing middleware
        app.add_middleware(
            TracingMiddleware,
...
***REMOVED*** Example usage patterns
...
def example_fastapi_integration():
    ...
    ***REMOVED*** Setup telemetry during application startup
    ...
    ***REMOVED*** Now all requests will be automatically traced
    @app.get("/users/{user_id}")
    async def get_user(user_id: int):
        ***REMOVED*** This request is automatically traced by the middleware
...
def example_manual_tracing():
    ...
    ***REMOVED*** Basic tracing
    @traced(name="process_data", capture_args=True)
    async def process_data(data: dict):
        ***REMOVED*** Function execution is automatically traced
        return {"processed": True}

    ***REMOVED*** Service method tracing
    class MyService:
        @trace_service_method()
        async def create_item(self, item_data: dict):
            ***REMOVED*** Service method is traced with service-specific attributes
            return {"id": 1}
...
def example_baggage_propagation():
    ...
    async def handle_request(user_id: str, tenant_id: str):
        ***REMOVED*** Set baggage that will be propagated to downstream services
        set_baggage("user.id", user_id)
        set_baggage("tenant.id", tenant_id)

        ***REMOVED*** Call other services - baggage is automatically propagated
        await call_external_service()

    async def downstream_handler():
        ***REMOVED*** Retrieve baggage from upstream service
        user_id = get_baggage("user.id")
        tenant_id = get_baggage("tenant.id")

        ***REMOVED*** Use the context in this service
