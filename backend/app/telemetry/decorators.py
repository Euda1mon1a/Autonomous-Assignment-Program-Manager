...
def traced(
    ...
    def decorator(func: Callable) -> Callable:
        ***REMOVED*** Get tracer for this function's module
        ...
        ***REMOVED*** Determine span name
        span_name = name or f"{func.__module__}.{func.__name__}"

        ***REMOVED*** Handle both sync and async functions
        ...
        if is_coroutine:
            ...
            async def async_wrapper(*args, **kwargs):
                with tracer.start_as_current_span(
                    ...
                    ***REMOVED*** Set static attributes
                    if attributes:
                        for key, value in attributes.items():
                            span.set_attribute(key, value)

                    ***REMOVED*** Capture arguments if enabled
                    if capture_args:
                    ...
                    try:
                        ...
                        ***REMOVED*** Capture result if enabled
                        if capture_result:
...
def trace_class(
    ...
    def decorator(cls):
        ***REMOVED*** Get all public methods
        for attr_name in dir(cls):
            ***REMOVED*** Skip private/magic methods
            if attr_name.startswith("_"):
                continue

            attr = getattr(cls, attr_name)

            ***REMOVED*** Only trace callable methods
            if callable(attr):
                ***REMOVED*** Apply traced decorator
            ...
            with tracer.start_as_current_span(span_name) as span:
                ***REMOVED*** Set static attributes
                ...
                try:
                    ...
                    ***REMOVED*** Record total items yielded
...
def _capture_function_args(
    ...
    """
    ***REMOVED*** Get function signature
    sig = inspect.signature(func)
    bound_args = sig.bind_partial(*args, **kwargs)

    ***REMOVED*** Set arguments as attributes (with sanitization)
    for param_name, param_value in bound_args.arguments.items():
        ***REMOVED*** Skip 'self' and 'cls' parameters
        if param_name in ("self", "cls"):
            continue

        ***REMOVED*** Sanitize sensitive parameters
        if param_name.lower() in ("password", "secret", "token", "api_key"):
            param_value = "***REDACTED***"
        else:
            ***REMOVED*** Convert to string and limit length
...
def _capture_result(span: Span, result: Any):
    ...
    """
    ***REMOVED*** Don't capture None results
    if result is None:
        return

    ***REMOVED*** Handle different result types
    if isinstance(result, (str, int, float, bool)):
        ...
        ***REMOVED*** For other types, just record the type
        span.set_attribute("function.result.type", type(result).__name__)


***REMOVED*** Specialized decorators for common use cases
...
def trace_controller_method(
    ...
    return traced(
        ...
        capture_args=False,  ***REMOVED*** Don't capture args (may contain sensitive data)
        span_kind=trace.SpanKind.INTERNAL,
    )
