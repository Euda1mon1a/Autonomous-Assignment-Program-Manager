...
class TracingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        """
        ***REMOVED*** Skip tracing for excluded paths
        if request.url.path in self.excluded_paths:
            return await call_next(request)

        ***REMOVED*** Extract trace context from headers (for distributed tracing)
        ***REMOVED*** Context propagation is handled automatically by OpenTelemetry

        ***REMOVED*** Create span for this request
        ...
        with self.tracer.start_as_current_span(
            ...
            ***REMOVED*** Process request
            ...
            try:
                ...
                ***REMOVED*** Set response attributes
                span.set_attribute(SpanAttributes.HTTP_STATUS_CODE, response.status_code)

                ***REMOVED*** Set span status based on HTTP status code
                ...
                ***REMOVED*** Record exception in span
                span.record_exception(exc)
                span.set_status(Status(StatusCode.ERROR, str(exc)))

                ***REMOVED*** Add error attributes
    ...
    def _set_header_attributes(self, span: Span, request: Request):
        ...
        for header_name in self.header_allowlist:
            ...
            if header_value:
                ***REMOVED*** Use lowercase header name for consistency
    ...
    def _set_query_attributes(self, span: Span, request: Request):
        ...
        if request.query_params:
            ***REMOVED*** Convert query params to string representation
            query_string = str(request.query_params)
            span.set_attribute(SpanAttributes.HTTP_FLAVOR, query_string)

            ***REMOVED*** Set individual query parameters (limit to avoid span bloat)
...
class DatabaseTracingMiddleware:
    ...
    def add_query_attributes(
        ...
        span.set_attribute("db.statement", query[:1000])  ***REMOVED*** Limit query length

        if table_name:
            span.set_attribute("db.sql.table", table_name)

        if params:
            ***REMOVED*** Sanitize parameters (don't include sensitive data)
            sanitized_params = {
...
class ExternalServiceTracingHelper:
    ...
    def trace_http_call(
        ...
        ***REMOVED*** Set HTTP client attributes
