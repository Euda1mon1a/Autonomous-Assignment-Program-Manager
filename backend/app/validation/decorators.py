...
def validate_request(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ***REMOVED*** Create validation context
            ctx = create_validation_context(locale)

            try:
                ***REMOVED*** Extract request object
                request = None
                for arg in args:
                ...
                ***REMOVED*** Call original function
        ...
        def sync_wrapper(*args, **kwargs):
            ***REMOVED*** Create validation context
            ...
            try:
                ...
                ***REMOVED*** Note: Sync functions can't validate body easily
                ***REMOVED*** For body validation, use async functions
...
def _validate_fields(
    ...
    """
    ***REMOVED*** Store data in context for conditional rules
    ctx.set_data("request_data", data)

    ***REMOVED*** Apply rules to each field
    for field, field_rules in rules.items():
        value = data.get(field)

        ***REMOVED*** Apply each rule
        for rule in field_rules:
            rule(field, value, ctx)


***REMOVED*** Convenience decorators for common validation patterns
...
def validate_date_range_params(
    ...
    def cross_validator(data: dict[str, Any], ctx: ValidationContext) -> None:
        ...
        if start and end:
            ***REMOVED*** Convert to dates if strings
            if isinstance(start, str):
                try:
                    start = datetime.fromisoformat(start).date()
                except ValueError:
                    return  ***REMOVED*** Let other validators handle format errors
