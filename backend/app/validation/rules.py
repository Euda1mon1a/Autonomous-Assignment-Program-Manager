...
***REMOVED*** Type alias for validation rule functions
...
def string_length(
    ...
    def validate(field: str, value: Any, ctx: ValidationContext) -> bool:
        if value is None:
            return True  ***REMOVED*** Use 'required' rule for None checks
...
def email_format() -> ValidationRule:
    ...
    Returns:
        ValidationRule: Validation rule function
    """
    ***REMOVED*** Simple email regex (basic validation)
...
def date_range(
    ...
    def validate(field: str, value: Any, ctx: ValidationContext) -> bool:
        ...
        ***REMOVED*** Convert to date if datetime
...
def conditional(
    ...
    def validate(field: str, value: Any, ctx: ValidationContext) -> bool:
        ***REMOVED*** Get request data from context
...
def any_of(*rules: ValidationRule) -> ValidationRule:
    ...
    def validate(field: str, value: Any, ctx: ValidationContext) -> bool:
        ***REMOVED*** Try each rule with a temporary context
        for rule in rules:
            temp_ctx = ValidationContext(ctx.locale)
            if rule(field, value, temp_ctx):
                return True

        ***REMOVED*** If none passed, add error
...
***REMOVED*** Predefined common rules
...
def phone_number_rule() -> ValidationRule:
    ...
    ***REMOVED*** Matches: (123) 456-7890, 123-456-7890, 1234567890
