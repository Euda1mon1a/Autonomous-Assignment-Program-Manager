...
***REMOVED*** Default expiration time for idempotency keys (24 hours)
...
class IdempotencyService:
    ...
    def compute_body_hash(self, request_params: dict) -> str:
        ...
        """
        ***REMOVED*** Sort keys for consistent hashing
    ...
    def create_request(
        ...
        self.db.add(request)
        try:
            self.db.flush()  ***REMOVED*** Flush to catch integrity errors early
        except IntegrityError:
            self.db.rollback()
            ***REMOVED*** Another request with same key+hash was just created
            ***REMOVED*** Return the existing one
