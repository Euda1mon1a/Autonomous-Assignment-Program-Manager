...
Usage:
    ...
    result = executor.execute_swap(
        ...
        target_week=None,  ***REMOVED*** Absorb swap (one-way)
        swap_type="absorb",
...
class SwapExecutor:
    ...
    Example:
        >>> executor = SwapExecutor(db)
        >>> ***REMOVED*** Execute an absorb swap (one-way transfer)
        >>> result = executor.execute_swap(
        ...     source_faculty_id=faculty_a.id,
        ...     source_week=date(2025, 1, 6),
        ...     target_faculty_id=faculty_b.id,
        ...     target_week=None,  ***REMOVED*** Absorb (no reciprocal)
        ...     swap_type="absorb",
        ... )
        >>> ***REMOVED*** Execute a one-to-one swap (bidirectional)
        >>> result = executor.execute_swap(
        ...     source_faculty_id=faculty_a.id,
        ...     source_week=date(2025, 1, 6),
        ...     target_faculty_id=faculty_b.id,
        ...     target_week=date(2025, 2, 3),  ***REMOVED*** Reciprocal week
    ...
    def rollback_swap(
        ...
        Example:
            >>> ***REMOVED*** Rollback a swap that was just executed
        ...
        try:
            with transactional_with_retry(self.db, max_retries=3, timeout_seconds=30.0):
                ***REMOVED*** Retrieve swap record with row-level lock to prevent concurrent rollbacks
                swap_record = (
                ...
                ***REMOVED*** Check if swap is in a state that can be rolled back
                ...
                ***REMOVED*** Check if rollback is within the allowed time window
                ...
                ***REMOVED*** Reverse the schedule assignments
                self._update_schedule_assignments(
                ...
                ***REMOVED*** Reverse the call cascade
                ...
                ***REMOVED*** Update swap record status
                ...
                ***REMOVED*** Transaction auto-commits
    ...
    def can_rollback(self, swap_id: UUID) -> bool:
        ...
        ***REMOVED*** Check if within rollback window
        time_since_execution = datetime.utcnow() - swap_record.executed_at
        rollback_window = timedelta(hours=self.ROLLBACK_WINDOW_HOURS)
    ...
    def _update_schedule_assignments(
        ...
        """
        ***REMOVED*** Calculate week end date (assuming week starts on Monday)
        source_week_end = source_week + timedelta(days=6)

        ***REMOVED*** Get all blocks in the source week with eager-loaded assignments
        ***REMOVED*** Row-level lock to prevent concurrent modifications
        ***REMOVED*** N+1 Optimization: Load blocks with their assignments in a single batch query
        source_blocks = (
            self.db.query(Block)
        ...
        ***REMOVED*** Update assignments for source faculty in source week
        ***REMOVED*** N+1 Optimization: Assignments are already loaded, no additional queries needed
        for block in source_blocks:
            for assignment in block.assignments:
                if assignment.person_id == source_faculty_id:
                    ***REMOVED*** Transfer assignment to target faculty
                    assignment.person_id = target_faculty_id
                    assignment.notes = (
                        f"Swapped from faculty {source_faculty_id} via swap execution"
                    )

        ***REMOVED*** If this is a one-to-one swap, update target week assignments
        if target_week:
            target_week_end = target_week + timedelta(days=6)

            ***REMOVED*** N+1 Optimization: Eager load assignments for target week blocks
            ***REMOVED*** Row-level lock to prevent concurrent modifications
            target_blocks = (
                self.db.query(Block)
                .options(selectinload(Block.assignments))
            ...
            ***REMOVED*** N+1 Optimization: Assignments are already loaded
            for block in target_blocks:
                for assignment in block.assignments:
                    if assignment.person_id == target_faculty_id:
                        ***REMOVED*** Transfer assignment to source faculty
    ...
    def _update_call_cascade(self, week: date, new_faculty_id: UUID) -> None:
        ...
        """
        ***REMOVED*** Calculate week end date
        week_end = week + timedelta(days=6)

        ***REMOVED*** Find Friday and Saturday in the week
        current_date = week
        while current_date <= week_end:
            ***REMOVED*** Check if this is Friday (weekday 4) or Saturday (weekday 5)
            if current_date.weekday() in [4, 5]:
                ***REMOVED*** Update call assignments for this date
                ***REMOVED*** N+1 Optimization: Eager load person relationship
                call_assignments = (
