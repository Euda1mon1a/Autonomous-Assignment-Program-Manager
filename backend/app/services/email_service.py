...
class EmailService:
    ...
    def send_email(
        ...
        try:
            ...
            ***REMOVED*** Attach text and HTML versions
            if body_text:
                msg.attach(MIMEText(body_text, "plain"))
            msg.attach(MIMEText(body_html, "html"))

            ***REMOVED*** Connect and send
    ...
    def send_certification_reminder(
        ...
        ***REMOVED*** Determine urgency level for styling
        if days_until_expiration <= 7:
            urgency = "URGENT"
            urgency_color = "***REMOVED***dc3545"  ***REMOVED*** Red
        elif days_until_expiration <= 30:
            urgency = "ACTION REQUIRED"
            urgency_color = "***REMOVED***fd7e14"  ***REMOVED*** Orange
        elif days_until_expiration <= 90:
            urgency = "REMINDER"
            urgency_color = "***REMOVED***ffc107"  ***REMOVED*** Yellow
        else:
            urgency = "NOTICE"
            urgency_color = "***REMOVED***17a2b8"  ***REMOVED*** Blue
        ...
        <head>
            <style>
                body {{ font-family: Arial, sans-serif; line-height: 1.6; color: ***REMOVED***333; }}
                .container {{ max-width: 600px; margin: 0 auto; padding: 20px; }}
                .header {{ background-color: {urgency_color}; color: white; padding: 20px; text-align: center; }}
                .content {{ padding: 20px; background-color: ***REMOVED***f9f9f9; }}
                .details {{ background-color: white; padding: 15px; margin: 15px 0; border-left: 4px solid {urgency_color}; }}
                .footer {{ padding: 15px; font-size: 12px; color: ***REMOVED***666; text-align: center; }}
...
This is an automated message from the Residency Scheduling System.
    ...
    def send_compliance_summary(
        ...
        ***REMOVED*** Build expiring list HTML
        expiring_rows = ""
        ...
        ***REMOVED*** Build expired list HTML
        expired_rows = ""
        for cert in expired_certs:
            expired_rows += f"""
            <tr style="background-color: ***REMOVED***ffe6e6;">
                <td>{cert.person.name}</td>
                <td>{cert.certification_type.name}</td>
                <td>{cert.expiration_date.strftime('%Y-%m-%d')}</td>
        ...
        <head>
            <style>
                ...
                .header {{ background-color: ***REMOVED***343a40; color: white; padding: 20px; text-align: center; }}
                .content {{ padding: 20px; }}
                table {{ width: 100%; border-collapse: collapse; margin: 15px 0; }}
                th, td {{ border: 1px solid ***REMOVED***ddd; padding: 10px; text-align: left; }}
                th {{ background-color: ***REMOVED***f8f9fa; }}
                .section {{ margin: 20px 0; }}
                .section h2 {{ color: ***REMOVED***343a40; border-bottom: 2px solid ***REMOVED***343a40; padding-bottom: 10px; }}
                .alert {{ padding: 15px; margin: 15px 0; border-radius: 5px; }}
                .alert-danger {{ background-color: ***REMOVED***f8d7da; border: 1px solid ***REMOVED***f5c6cb; color: ***REMOVED***721c24; }}
                .alert-warning {{ background-color: ***REMOVED***fff3cd; border: 1px solid ***REMOVED***ffeeba; color: ***REMOVED***856404; }}
            </style>
        </head>
        <body>
            <div class="container">
