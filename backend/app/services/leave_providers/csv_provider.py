...
***REMOVED*** Default allowed directory for CSV files
***REMOVED*** Can be configured via environment or application settings
...
class CSVLeaveProvider(LeaveProvider):
    ...
    def __init__(self, file_path: Path, allowed_base_dir: Path | None = None):
        ...
        """
        ***REMOVED*** Use provided base directory or default
        base_dir = allowed_base_dir or ALLOWED_CSV_DIR

        ***REMOVED*** Ensure the allowed base directory exists
        base_dir.mkdir(parents=True, exist_ok=True)

        ***REMOVED*** Validate that file_path is within the allowed directory
        try:
            validated_path = validate_file_path(file_path, base_dir)
            self.file_path = validated_path
        except FileSecurityError as exc:
            ***REMOVED*** If validation fails, raise with more context
