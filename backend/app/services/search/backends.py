...
class PostgreSQLSearchBackend(SearchBackend):
    ...
    async def search(
        ...
        ***REMOVED*** Process query with analyzer
        ...
        ***REMOVED*** Sort by relevance score (if available)
    ...
    async def _search_people(
        ...
        ***REMOVED*** Full-text search on name and email
        if query:
            ***REMOVED*** Use PostgreSQL ILIKE for simple pattern matching
            ***REMOVED*** In production, consider using tsvector for better performance
        ...
        ***REMOVED*** Apply ordering (by relevance)
        ***REMOVED*** Prioritize exact name matches
        if query:
            stmt = stmt.order_by(
                case(
                    (Person.name.ilike(f'{query}%'), 1),  ***REMOVED*** Starts with query
                    (Person.name.ilike(f'%{query}%'), 2),  ***REMOVED*** Contains query
                    else_=3
                ),
                Person.name
            )
    ...
    async def _search_swaps(
        ...
        ***REMOVED*** Apply ordering (most recent first)
        stmt = stmt.order_by(SwapRecord.created_at.desc())
        stmt = stmt.limit(limit).offset(offset)
    ...
    async def _calculate_people_facets(
        ...
        ***REMOVED*** Type facets (resident vs faculty)
        type_stmt = select(
            Person.type,
        ...
        ***REMOVED*** PGY level facets (for residents)
        pgy_stmt = select(
            Person.pgy_level,
            func.count(Person.id).label('count')
    ...
    async def suggest(
        ...
        if entity_type == 'person':
            ***REMOVED*** Get person name suggestions
            ...
            ***REMOVED*** Get rotation name suggestions
    ...
    def _calculate_relevance_score(self, text: str, query: str) -> float:
        ...
        ***REMOVED*** Fuzzy match (simple word overlap)
    ...
    def _highlight_text(self, text: str, query: str) -> Dict[str, List[str]]:
        ...
        ***REMOVED*** Find query position in text (case-insensitive)
        text_lower = text.lower()
        query_lower = query.lower()
