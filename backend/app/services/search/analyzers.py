...
class SearchAnalyzer(ABC):
    ...
    def _normalize(self, text: str) -> str:
        ...
        ***REMOVED*** Convert to lowercase
        text = text.lower()
        ***REMOVED*** Remove extra whitespace
    ...
    def _tokenize(self, text: str) -> list[str]:
        ...
        Returns:
            List of word tokens
        """
        ***REMOVED*** Split on word boundaries, keeping alphanumeric and hyphens
...
class StandardAnalyzer(SearchAnalyzer):
    ...
    ***REMOVED*** Common English stopwords
    ...
    def analyze(self, text: str) -> list[str]:
        ...
        """
        ***REMOVED*** Normalize and tokenize
        normalized = self._normalize(text)
        tokens = self._tokenize(normalized)

        ***REMOVED*** Filter by length
        tokens = [t for t in tokens if len(t) >= self.min_token_length]

        ***REMOVED*** Remove stopwords if enabled
...
class PersonNameAnalyzer(SearchAnalyzer):
    ...
    ***REMOVED*** Common name prefixes to preserve
    PREFIXES: set[str] = {
        "dr",
        "md",
        "do",
    ...
    def analyze(self, text: str) -> list[str]:
        ...
        ***REMOVED*** Add full tokens (preserve all parts of name)
        result.extend(tokens)

        ***REMOVED*** Add n-grams for fuzzy matching
        if self.generate_ngrams:
            for token in tokens:
                ***REMOVED*** Skip very short tokens and prefixes
                if len(token) >= 3 and token not in self.PREFIXES:
                    ***REMOVED*** Generate 3-grams for fuzzy matching
...
class MedicalTermAnalyzer(SearchAnalyzer):
    ...
    ***REMOVED*** Medical and scheduling abbreviations to preserve
    ...
    ***REMOVED*** Acronym expansions
    ...
    def analyze(self, text: str) -> list[str]:
        ...
        ***REMOVED*** Normalize and tokenize
        ...
        for token in tokens:
            ***REMOVED*** Always include the original token
            result.append(token)

            ***REMOVED*** Expand acronyms if enabled and found
            ...
            ***REMOVED*** Handle PGY levels specially
...
class FuzzyAnalyzer(SearchAnalyzer):
    ...
    def analyze(self, text: str) -> list[str]:
        ...
        ***REMOVED*** Normalize and tokenize
        ...
        for token in tokens:
            ***REMOVED*** Include original token
            result.append(token)

            ***REMOVED*** Add soundex encoding for phonetic matching
            if len(token) >= 3:
    ...
    def _soundex(self, word: str) -> str:
        ...
        ***REMOVED*** Convert remaining letters
        for char in word[1:]:
            code = soundex_map.get(char, "0")
            if code != "0" and code != soundex[-1]:
                soundex += code

        ***REMOVED*** Pad with zeros to length 4
