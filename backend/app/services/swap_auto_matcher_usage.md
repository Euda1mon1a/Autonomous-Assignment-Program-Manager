***REMOVED*** Enhanced Swap Auto-Matcher Usage Guide
...
***REMOVED*** Initialize with default criteria
matcher = SwapAutoMatcher(db=session)

***REMOVED*** Or with custom criteria
...
***REMOVED******REMOVED*** Core Methods

***REMOVED******REMOVED******REMOVED*** 1. Find Compatible Swaps
...
***REMOVED******REMOVED******REMOVED*** 2. Score Swap Compatibility
...
print(f"Compatibility Score: {score:.2f}")  ***REMOVED*** 0.0 to 1.0
```

***REMOVED******REMOVED******REMOVED*** 3. Suggest Optimal Matches

Get ranked matches with detailed explanations:

```python
***REMOVED*** Get top 5 matches
...
for ranked_match in ranked_matches:
    ...
    ***REMOVED*** Detailed scoring breakdown
...
***REMOVED******REMOVED******REMOVED*** 4. Auto-Match Pending Requests

Process all pending swap requests at once:

```python
***REMOVED*** Run batch auto-matching
...
***REMOVED*** Review high-priority matches
...
***REMOVED*** Review successful matches
...
***REMOVED*** Review requests with no matches
if result.no_matches:
    print(f"\nRequests with no matches: {len(result.no_matches)}")
```

***REMOVED******REMOVED******REMOVED*** 5. Proactive Swap Suggestions
...
***REMOVED******REMOVED*** Scoring Factors

The compatibility score (0.0 to 1.0) is calculated using these factors:

***REMOVED******REMOVED******REMOVED*** 1. Date Proximity (Default Weight: 20%)
- **1.0**: Dates within 2 weeks
- **0.5-1.0**: Dates within max separation
- **0.0**: Dates too far apart

***REMOVED******REMOVED******REMOVED*** 2. Preference Alignment (Default Weight: 35%)
...
***REMOVED******REMOVED******REMOVED*** 3. Workload Balance (Default Weight: 15%)
...
***REMOVED******REMOVED******REMOVED*** 4. Past Swap History (Default Weight: 15%)
...
***REMOVED******REMOVED******REMOVED*** 5. Faculty Availability (Default Weight: 15%)
- **1.0**: Both available and prefer the weeks
- **0.5**: One prefers, one is available
- **0.0**: One or both have blocked weeks

***REMOVED******REMOVED******REMOVED*** Blocking Penalty
- **1.0**: No blocked weeks (no penalty)
- **0.0**: One or both weeks are blocked (complete penalty)

***REMOVED******REMOVED*** Customizing Matching Criteria

```python
criteria = MatchingCriteria(
    ***REMOVED*** Adjust scoring weights (must sum close to 1.0)
    date_proximity_weight=0.25,        ***REMOVED*** Emphasize date closeness
    preference_alignment_weight=0.40,   ***REMOVED*** Most important factor
    workload_balance_weight=0.15,
    history_weight=0.10,
    availability_weight=0.10,

    ***REMOVED*** Set thresholds
    minimum_score_threshold=0.5,       ***REMOVED*** Only show matches >= 0.5
    high_priority_threshold=0.80,      ***REMOVED*** High priority at >= 0.8

    ***REMOVED*** Configure constraints
    max_matches_per_request=10,        ***REMOVED*** Return up to 10 matches
    max_date_separation_days=60,       ***REMOVED*** Only match dates within 60 days
    require_mutual_availability=True,  ***REMOVED*** Both must be available
    exclude_blocked_weeks=True,        ***REMOVED*** Skip blocked weeks
    consider_past_rejections=True,     ***REMOVED*** Avoid past rejections

    ***REMOVED*** Historical analysis
    history_lookback_days=365,         ***REMOVED*** Look back 1 year
)
...
***REMOVED******REMOVED******REMOVED*** API Endpoint Example
...
***REMOVED******REMOVED******REMOVED*** Background Task Example
...
def run_daily_auto_matching():
    ...
    try:
        ...
        ***REMOVED*** Notify high-priority matches
        for auto_match in result.high_priority_matches:
