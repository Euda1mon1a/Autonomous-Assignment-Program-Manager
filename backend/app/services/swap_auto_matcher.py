...
class SwapAutoMatcher:
    ...
    def find_compatible_swaps(self, request_id: UUID) -> list[SwapMatch]:
        ...
        """
        ***REMOVED*** Get the source request
        ...
        if request.status != SwapStatus.PENDING:
            return []  ***REMOVED*** Only match pending requests

        ***REMOVED*** Find all other pending requests
        ...
        for candidate in candidates:
            ***REMOVED*** Check if this is a compatible match
    ...
    def score_swap_compatibility(
        ...
        """
        ***REMOVED*** Calculate individual factor scores
        ...
        ***REMOVED*** Normalize by total weight
    ...
    def suggest_optimal_matches(
        ...
        ***REMOVED*** Find all compatible matches
        compatible_matches = self.find_compatible_swaps(request_id)

        ***REMOVED*** Score and rank each match
        ranked_matches = []

        for match in compatible_matches:
            ***REMOVED*** Get the candidate request
            ...
            ***REMOVED*** Calculate detailed scoring
            date_score = self._score_date_proximity(request, candidate)
            preference_score = self._score_preference_alignment(request, candidate)
            workload_score = self._score_workload_balance(request, candidate)
            history_score = self._score_swap_history(request, candidate)
        ...
        ***REMOVED*** Sort by compatibility score (highest first)
        ranked_matches.sort(
            key=lambda x: x.compatibility_score,
            reverse=True
        )

        ***REMOVED*** Return top K matches
    ...
    def auto_match_pending_requests(self) -> BatchAutoMatchResult:
        ...
        ***REMOVED*** Get all pending requests
        ...
        for request in pending_requests:
            try:
                ***REMOVED*** Find matches for this request
                ...
                if matches_found > 0:
                    ...
                    ***REMOVED*** Check if any are high priority
                ...
                ***REMOVED*** Log error but continue processing
                no_matches.append(request.id)
                continue
    ...
    def suggest_proactive_swaps(
        ...
        """
        ***REMOVED*** Get faculty's preferences
        preferences = self.preference_service.get_or_create_preferences(faculty_id)

        ***REMOVED*** Get faculty's current future assignments
        ...
        for assignment, block in current_assignments:
            ...
            ***REMOVED*** Check if this week is blocked by the faculty
            is_blocked = self.preference_service.is_week_blocked(
                faculty_id, current_week
            )

            if is_blocked:
                ***REMOVED*** Find faculty who prefer this week
                ...
                for partner_id in interested_faculty:
                    ...
                    ***REMOVED*** Find their assignments on weeks this faculty prefers
                    ...
                    for partner_assignment, partner_block in partner_assignments:
                        ...
                        ***REMOVED*** Check if we prefer their week
    ...
    ***REMOVED*** ===== PRIVATE HELPER METHODS =====
    ...
    def _is_compatible(self, request_a: SwapRecord, request_b: SwapRecord) -> bool:
        ...
        """
        ***REMOVED*** Don't match with self
        if request_a.source_faculty_id == request_b.source_faculty_id:
            return False

        ***REMOVED*** Check date separation constraint
        ...
        ***REMOVED*** Check if weeks are blocked
        ...
        ***REMOVED*** Check mutual availability if required
        if self.criteria.require_mutual_availability:
            ***REMOVED*** Validate both directions
    ...
    def _create_swap_match(
        ...
        ***REMOVED*** Check if it's a mutual match
        is_mutual = (
    ...
    def _score_date_proximity(
        ...
        ***REMOVED*** If dates are very close (within 2 weeks), score high
        if days_apart <= 14:
            return 1.0
        ***REMOVED*** If dates are within max separation, score decreases linearly
        elif days_apart <= self.criteria.max_date_separation_days:
            ratio = days_apart / self.criteria.max_date_separation_days
            return 1.0 - (ratio * 0.5)  ***REMOVED*** Score from 1.0 to 0.5
        else:
            return 0.0
    ...
    def _score_preference_alignment(
        ...
        Returns:
            Score between 0.0 and 1.0
        """
        ***REMOVED*** Use the existing preference service logic
    ...
    def _score_swap_history(
        ...
        if self.criteria.consider_past_rejections:
            ***REMOVED*** Check for past rejections between these two faculty
            ...
            if past_rejections > 0:
                ***REMOVED*** Penalize repeated rejections
                penalty = min(past_rejections * 0.2, 0.6)
                base_score = 1.0 - penalty
            else:
                base_score = 1.0

            ***REMOVED*** Check for successful past swaps (bonus)
            ...
            ***REMOVED*** Use overall acceptance rates
    ...
    def _score_availability(
        ...
        ***REMOVED*** Check if faculty A is available for faculty B's week
        a_available_for_b = not self.preference_service.is_week_blocked(
            request_a.source_faculty_id,
            request_b.source_week
        )

        ***REMOVED*** Check if faculty B is available for faculty A's week
        ...
        ***REMOVED*** Bonus if weeks are preferred
    ...
    def _check_blocking_constraints(
        ...
        """
        ***REMOVED*** Check if either party has blocked the week they would receive
        a_blocks_b = self.preference_service.is_week_blocked(
            request_a.source_faculty_id,
            request_b.source_week
        )
        ...
        if a_blocks_b or b_blocks_a:
            return 0.0  ***REMOVED*** Complete penalty
        else:
            return 1.0  ***REMOVED*** No penalty
    ...
    def _determine_priority(
        ...
        ***REMOVED*** Check for urgent situations (blocked weeks)
    ...
    def _estimate_acceptance_probability(
        ...
        """
        ***REMOVED*** Start with compatibility score as base
        base_prob = compatibility_score

        ***REMOVED*** Adjust based on historical acceptance rates
        ...
        ***REMOVED*** Combined probability (both must accept)
        historical_factor = (a_rate + b_rate) / 2.0

        ***REMOVED*** Weight base score more heavily, but consider history
    ...
    def _check_match_warnings(
        ...
        ***REMOVED*** Check if weeks are very soon
    ...
    def _calculate_suggestion_benefit(
        ...
        ***REMOVED*** Check date proximity (prefer closer dates)
