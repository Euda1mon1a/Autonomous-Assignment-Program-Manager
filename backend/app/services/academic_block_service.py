...
class AcademicBlockService:
    ...
    ***REMOVED*** Constants
    ...
    BLOCK_DURATION_DAYS = 28  ***REMOVED*** 4 weeks
    HOURS_PER_HALF_DAY = 6
    MAX_WEEKLY_HOURS_ACGME = 80  ***REMOVED*** ACGME limit
    EXPECTED_BLOCKS_PER_YEAR = 13  ***REMOVED*** 52 weeks / 4 weeks per block
    ...
    def get_block_matrix(
        ...
        """
        ***REMOVED*** Parse academic year and generate blocks
        ...
        ***REMOVED*** Calculate summary statistics
    ...
    def _parse_academic_year(self, academic_year: str) -> tuple[date, date]:
        ...
        try:
            ...
            ***REMOVED*** Academic year: July 1 of start year to June 30 of end year
    ...
    def _generate_academic_blocks(
        ...
        while current_date <= end_date:
            ***REMOVED*** Calculate block end date (4 weeks from start)
            block_end = min(
    ...
    def _get_residents(self, pgy_level: int | None = None) -> list[ResidentRow]:
        ...
        ***REMOVED*** Convert to ResidentRow schema
        return [
            ResidentRow(
    ...
    def _build_matrix_cells(
        ...
        ***REMOVED*** Index assignments by person and date for quick lookup
        assignments_by_person = defaultdict(list)
        for assignment in assignments:
            assignments_by_person[assignment.person_id].append(assignment)

        ***REMOVED*** Build cells for each resident × block combination
    ...
    def _build_matrix_cell(
        ...
        """
        ***REMOVED*** Filter assignments to this block period
        block_assignments = [
            a
        ...
        ***REMOVED*** Calculate hours
        total_hours = len(block_assignments) * self.HOURS_PER_HALF_DAY

        ***REMOVED*** Determine primary rotation (most common rotation in this block)
    ...
    def _get_primary_rotation(
        ...
        ***REMOVED*** Get most common rotation
        if rotation_counts:
    ...
    def _check_acgme_compliance(
        ...
        ***REMOVED*** Run advanced ACGME validations
        try:
            ***REMOVED*** 24+4 hour rule violations
            ...
            ***REMOVED*** PGY-specific violations
            ...
            ***REMOVED*** Log error but don't fail the entire request
            warnings.append(f"Error checking ACGME compliance: {str(e)}")

        ***REMOVED*** Warnings for approaching limits
    ...
    def _calculate_block_summary(
        ...
        """
        ***REMOVED*** Filter assignments to this block
        block_assignments = [
            a
            for a in assignments
            if a.block and block.start_date <= a.block.date <= block.end_date
        ...
        ***REMOVED*** Calculate compliance
        ...
        for resident_id in resident_ids:
            ...
            ***REMOVED*** Simple compliance check (weekly hours)
            block_days = (block.end_date - block.start_date).days + 1
            weeks = max(1, block_days / 7)
            avg_weekly_hours = hours / weeks
