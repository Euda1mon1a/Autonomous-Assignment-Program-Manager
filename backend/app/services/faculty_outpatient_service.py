...
class FacultyOutpatientAssignmentService:
    ...
    def generate_faculty_outpatient_assignments(
        ...
        ***REMOVED*** Step 1: Get block date range
        ...
        ***REMOVED*** Step 2: Get blocks for this period
        ...
        ***REMOVED*** Step 3: Clear existing faculty outpatient assignments if regenerating
        cleared_count = 0
        if regenerate:
        ...
        ***REMOVED*** Step 4: Get available faculty (not on FMIT, leave, etc.)
        ...
        ***REMOVED*** Step 5: Get outpatient templates
        ...
        ***REMOVED*** Step 6: Generate clinic assignments
        clinic_assignments = []
        ...
        ***REMOVED*** Step 7: Generate supervision assignments
        ...
        ***REMOVED*** Step 8: Commit all assignments
        all_assignments = clinic_assignments + supervision_assignments
        for assignment in all_assignments:
            self.db.add(assignment)
        self.db.commit()

        ***REMOVED*** Step 9: Build faculty summaries
    ...
    def _get_block_dates(self, block_number: int) -> tuple[date | None, date | None]:
        ...
        """
        ***REMOVED*** Query the database for blocks with this block_number
        block = (
            self.db.query(Block)
        ...
        ***REMOVED*** Find last block with this block_number
        last_block = (
            self.db.query(Block)
            .filter(Block.block_number == block_number)
    ...
    def _clear_existing_faculty_outpatient(
        ...
        ***REMOVED*** Get block IDs in range
        block_ids = [
            b.id
        ...
        ***REMOVED*** Delete matching assignments
        deleted = (
    ...
    def _get_available_faculty(
        ...
        ***REMOVED*** Get faculty with FMIT during this period
        fmit_templates = (
        ...
        ***REMOVED*** Get faculty with blocking absences
        ...
        absences = (
            ...
            .filter(
                ...
                Absence.is_blocking == True,  ***REMOVED*** noqa: E712
            )
            .all()
        )
        ...
        ***REMOVED*** Exclude faculty on FMIT/absences AND adjunct faculty (not auto-scheduled)
    ...
    def _generate_clinic_assignments(
        ...
        ***REMOVED*** Filter to workday blocks only (no weekends)
        workday_blocks = [b for b in blocks if not b.is_weekend]

        ***REMOVED*** Get FM Clinic template (primary clinic assignment target)
        ...
        ***REMOVED*** Sort blocks by date/time
        ...
        for block in workday_blocks:
            ...
            ***REMOVED*** Find faculty who can work this block
            candidates = []
            for f in faculty:
                weekly_limit = getattr(f, "weekly_clinic_limit", 4)
                current_count = faculty_weekly_counts[f.id][week_start]

                ***REMOVED*** Skip if at weekly limit
                if current_count >= weekly_limit:
                    continue

                ***REMOVED*** Skip Sports Med faculty for regular clinic
                if (
                ...
                ***REMOVED*** Score by load (fewer = higher priority)
            ...
            ***REMOVED*** Sort by load (least-loaded first)
            candidates.sort(key=lambda x: x[1])

            ***REMOVED*** Assign one faculty to this block
    ...
    def _generate_supervision_assignments(
        ...
        ***REMOVED*** Build set of (block_id, person_id) from new clinic assignments
        ***REMOVED*** These faculty are already assigned to these blocks
        ...
        ***REMOVED*** Get all resident assignments in this period
        block_ids = [b.id for b in blocks]

        if not block_ids:
            return []

        ***REMOVED*** Get outpatient templates (what residents are assigned to)
        outpatient_templates = self._get_outpatient_templates()
        ...
        ***REMOVED*** Get person lookup for PGY levels
        ...
        ***REMOVED*** For each block with residents, assign supervising faculty
        for block_id, block_assignments in assignments_by_block.items():
            ***REMOVED*** Count PGY levels
            pgy1_count = sum(
                1
            ...
            ***REMOVED*** Calculate required faculty using fractional supervision load
            ***REMOVED*** PGY-1: 0.5 supervision load each (1:2 ratio)
            ***REMOVED*** PGY-2/3: 0.25 supervision load each (1:4 ratio)
            ***REMOVED*** Sum loads THEN ceiling to avoid overcounting in mixed PGY scenarios
            ...
            ***REMOVED*** Get a rotation template from the resident assignments
            template_id = block_assignments[0].rotation_template_id

            ***REMOVED*** Find available faculty (not already assigned to this block)
            ...
            ***REMOVED*** Also exclude faculty assigned via new clinic assignments (not yet committed)
            ...
            ***REMOVED*** Sort by load
            available.sort(key=lambda f: faculty_load.get(f.id, 0))

            ***REMOVED*** Assign required number of faculty
    ...
    def _build_faculty_summaries(
        ...
        ***REMOVED*** Sort by total sessions descending
