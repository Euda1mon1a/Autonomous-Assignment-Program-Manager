...
class HeatmapService:
    ...
    def calculate_date_range(time_range: TimeRangeType) -> tuple[date, date]:
        ...
        if range_type == "custom":
            ...
            ***REMOVED*** Start from Monday of the week containing reference_date
            start_of_week = reference - timedelta(days=reference.weekday())
            end_of_week = start_of_week + timedelta(days=6)
            return start_of_week, end_of_week

        elif range_type == "month":
            ***REMOVED*** Start from first day of the month
            start_of_month = reference.replace(day=1)
            ***REMOVED*** Calculate last day of month
            ...
            ***REMOVED*** Determine which quarter the reference date is in
            quarter_month = ((reference.month - 1) // 3) * 3 + 1
            start_of_quarter = reference.replace(month=quarter_month, day=1)

            ***REMOVED*** Calculate end of quarter (3 months later, minus 1 day)
            if quarter_month >= 10:
    ...
    def _generate_daily_heatmap(
        ...
        ***REMOVED*** Count assignments per day
        ...
        ***REMOVED*** Create single row with daily counts
    ...
    def _generate_weekly_heatmap(
        ...
        ***REMOVED*** Count assignments per week
        weekly_counts = dict.fromkeys(weeks, 0)
        for assignment in assignments:
            block_date = assignment.block.date
            ***REMOVED*** Find the week this assignment belongs to
            week_start = block_date - timedelta(days=block_date.weekday())
            if week_start in weekly_counts:
                weekly_counts[week_start] += 1

        ***REMOVED*** Create single row with weekly counts
    ...
    def generate_unified_heatmap(
        ...
        """
        ***REMOVED*** Get assignments
        assignments = self._get_assignments_in_range(
            db, start_date, end_date, person_ids, rotation_ids
        )

        ***REMOVED*** Handle daily and weekly grouping (group by date instead of entity)
        if group_by == "daily":
        ...
        ***REMOVED*** Original person/rotation grouping logic
        dates = self._get_date_range(start_date, end_date)
        x_labels = [d.strftime("%Y-%m-%d") for d in dates]

        ***REMOVED*** Build assignment map: (person_id or rotation_id, date) -> count
        ...
        ***REMOVED*** Get unique entities (people or rotations)
        if group_by == "person":
            ...
            if rotation_ids:
                ...
                ***REMOVED*** Get all rotation templates with assignments in range
                entity_ids = {
        ...
        ***REMOVED*** Include FMIT swap data if requested
        metadata: dict[str, Any] = {
            "total_assignments": len(assignments),
            "date_range_days": len(dates),
    ...
    def generate_coverage_heatmap(
        ...
        ***REMOVED*** Build z_values matrix (coverage level: 0 = no coverage, 1+ = covered)
        ***REMOVED*** Normalize to 0-1 scale for visualization
        z_values = []
        ...
        for rotation_id, rotation in zip(rotation_ids, rotations, strict=False):
            ...
            for d in dates:
                ...
                ***REMOVED*** Determine expected coverage (simplified - could be more sophisticated)
                expected = 1  ***REMOVED*** Assume 1 person minimum per day

                ***REMOVED*** Calculate coverage ratio
        ...
        ***REMOVED*** Calculate overall coverage percentage
        ...
        heatmap_data = HeatmapData(
            ...
            y_labels=y_labels,
            z_values=z_values,
            color_scale="RdYlGn",  ***REMOVED*** Red-Yellow-Green for coverage
            annotations=None,
        )
    ...
    def generate_person_workload_heatmap(
        ...
        if not include_weekends:
            ***REMOVED*** Filter out weekends (Saturday=5, Sunday=6)
        ...
        ***REMOVED*** Get assignments for these people
        assignments = self._get_assignments_in_range(
            db, start_date, end_date, person_ids=person_ids
        )

        ***REMOVED*** Build workload map: (person_id, date) -> block count
        ...
        ***REMOVED*** Calculate workload statistics
