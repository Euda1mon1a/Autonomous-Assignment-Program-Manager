...
class ResponseResult:
    ...
    success: bool
    message: str = ""
    new_request_id: UUID | None = None  ***REMOVED*** For counter-offers
    executed: bool = False
    errors: list[ValidationError] = field(default_factory=list)
...
class SwapRequestService:
    ...
    def create_request(
        ...
        """
        ***REMOVED*** Verify requester is faculty
        ...
        ***REMOVED*** Verify source week is assigned to requester
        ...
        ***REMOVED*** Check if there's already a pending request for this week
        ...
        ***REMOVED*** If target specified, validate the swap
        ...
        ***REMOVED*** Create the swap request
        swap_type = SwapType.ONE_TO_ONE if desired_weeks else SwapType.ABSORB

        ***REMOVED*** If no target specified, this becomes a marketplace posting
        if not target_faculty_id:
            ***REMOVED*** Find potential candidates
            ...
            if not candidates:
                ***REMOVED*** Create marketplace request without target
                request_id = uuid4()
                swap_record = SwapRecord(
                    id=request_id,
                    source_faculty_id=requester_id,
                    source_week=source_week,
                    target_faculty_id=requester_id,  ***REMOVED*** Placeholder
                    target_week=desired_weeks[0] if desired_weeks else None,
                    swap_type=swap_type,
            ...
            ***REMOVED*** Use first candidate as target
            target_faculty_id = candidates[0]

        ***REMOVED*** Create swap record with specific target
        request_id = uuid4()
        swap_record = SwapRecord(
            id=request_id,
            source_faculty_id=requester_id,
        ...
        ***REMOVED*** Send notification to target
    ...
    def get_requests_for_me(self, faculty_id: UUID) -> list[RequestDetail]:
        ...
        swaps = (
            ...
            .filter(
                ...
                SwapRecord.source_faculty_id != faculty_id,  ***REMOVED*** Exclude self-requests
    ...
    def respond_to_request(
        ...
        ***REMOVED*** Verify faculty is the target
        ...
        ***REMOVED*** Verify request is still pending
        ...
        ***REMOVED*** Handle counter-offer
        if counter_week and not accept:
            ***REMOVED*** Create a new swap request as counter-offer
            ...
            ***REMOVED*** Reject original request
            swap.status = SwapStatus.REJECTED
            swap.notes = f"Counter-offered with week {counter_week}"
            self.db.commit()

            ***REMOVED*** Notify original requester
            if source_faculty:
        ...
        ***REMOVED*** Handle acceptance
        ***REMOVED*** Re-validate the swap before executing
        validation = self.validator.validate_swap(
            source_faculty_id=swap.source_faculty_id,
            source_week=swap.source_week,
            target_faculty_id=swap.target_faculty_id,
        ...
        ***REMOVED*** Mark as approved
        swap.status = SwapStatus.APPROVED
        swap.approved_at = datetime.utcnow()
        self.db.commit()

        ***REMOVED*** Execute the swap
        ...
        if execution.success:
            ...
            ***REMOVED*** Execution failed, revert to pending
    ...
    def cancel_request(self, request_id: UUID, requester_id: UUID) -> CancelResult:
        ...
        ***REMOVED*** Verify faculty is the requester
    ...
    def auto_match_requests(self) -> MatchResult:
        ...
        for i, swap_a in enumerate(pending_swaps):
            for swap_b in pending_swaps[i + 1:]:
                ***REMOVED*** Check for mutual compatibility
    ...
    ***REMOVED*** Helper methods
    ...
    def _is_week_assigned_to_faculty(self, faculty_id: UUID, week_start: date) -> bool:
        ...
        ***REMOVED*** Find FMIT rotation template
        fmit_template = self.db.query(RotationTemplate).filter(
            RotationTemplate.name == "FMIT"
        ).first()

        if not fmit_template:
            ***REMOVED*** No FMIT template configured - cannot verify
            return False

        ***REMOVED*** Query for FMIT assignments in the specified week
        ***REMOVED*** N+1 Optimization: Eager load person and block in case assignment is returned
        assignment = (
    ...
    def _find_compatible_candidates(
        ...
        ***REMOVED*** Find faculty without blocks for this week
        available_candidates = self.preference_service.get_faculty_without_blocks_for_week(
            week_date=source_week,
            exclude_faculty_ids=[source_faculty_id]
        )

        ***REMOVED*** Prioritize faculty who prefer the week
        candidates.extend(preferred_candidates)

        ***REMOVED*** Add other available candidates
        for candidate_id in available_candidates:
            if candidate_id not in candidates:
                ***REMOVED*** Validate the swap would work
                validation = self.validator.validate_swap(
        ...
        return candidates[:5]  ***REMOVED*** Limit to top 5 candidates
