...
class UploadProgress:
    ...
    def __init__(self, upload_id: str, total_size: int):
        ...
        self.status = "in_progress"  ***REMOVED*** in_progress, completed, failed
...
class UploadService:
    ...
    def upload_file(
        ...
        try:
            ***REMOVED*** Convert file-like object to bytes if needed
            ...
            ***REMOVED*** Validate file
            ...
            progress.update(len(file_content) // 4)  ***REMOVED*** 25% - validation complete

            ***REMOVED*** Prepare upload metadata
            upload_metadata = {
            ...
            ***REMOVED*** Merge user metadata
            if metadata:
                upload_metadata.update(metadata)

            ***REMOVED*** Process images if enabled
            ...
            if process_images and validation_result["mime_type"].startswith("image/"):
                ...
                progress.update(len(file_content) // 4)  ***REMOVED*** 50% - processing complete

            ***REMOVED*** Store original file
            ...
            progress.update(len(file_content) // 4)  ***REMOVED*** 75% - storage complete

            ***REMOVED*** Store processed versions (if any)
            version_results = {}
            if processed_versions:
                for version_name, version_data in processed_versions[
                    "versions"
                ].items():
                    if version_name != "original":  ***REMOVED*** Original is already stored
                        version_filename = (
            ...
            progress.update(len(file_content) // 4)  ***REMOVED*** 100% - complete
            progress.complete()
            ...
            ***REMOVED*** Add processed versions
            if version_results:
                result["versions"] = version_results

            ***REMOVED*** Add image-specific data
            if processed_versions:
                result["image_data"] = {
...
***REMOVED*** Import Path for filename operations
