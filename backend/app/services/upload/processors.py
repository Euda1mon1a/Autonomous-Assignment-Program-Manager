    ...
    ***REMOVED*** Standard image sizes
    ...
    def process_image(
        ...
        try:
            ...
            ***REMOVED*** Extract EXIF data
            exif_data = self._extract_exif(image)

            ***REMOVED*** Auto-rotate based on EXIF orientation
            image = self.ImageOps.exif_transpose(image)

            ***REMOVED*** Get original dimensions
            original_width, original_height = image.size

            ***REMOVED*** Convert RGBA to RGB if saving as JPEG
            if format == "JPEG" or (
                format is None and image.format in ["JPEG", "JPG"]
            ):
                if image.mode in ("RGBA", "LA", "P"):
                    ***REMOVED*** Create white background
            ...
            ***REMOVED*** Generate resized versions
            if sizes:
                for size_name in sizes:
                    ...
                    ***REMOVED*** Only resize if image is larger
                    if original_width > target_width or original_height > target_height:
                        ...
                        ***REMOVED*** Image is already smaller, use original
    ...
    def generate_thumbnail(
        ...
        try:
            ...
            ***REMOVED*** Auto-rotate based on EXIF
            image = self.ImageOps.exif_transpose(image)

            ***REMOVED*** Convert to RGB if needed
            if image.mode in ("RGBA", "LA", "P"):
                background = self.Image.new("RGB", image.size, (255, 255, 255))
                if image.mode == "P":
                    image = image.convert("RGBA")
    ...
    def _resize_image(
        ...
        """
        ***REMOVED*** Calculate new dimensions maintaining aspect ratio
        width, height = image.size
        aspect = width / height

        if width > max_width or height > max_height:
            if width / max_width > height / max_height:
                ***REMOVED*** Width is the limiting factor
                new_width = max_width
                new_height = int(max_width / aspect)
            else:
                ***REMOVED*** Height is the limiting factor
