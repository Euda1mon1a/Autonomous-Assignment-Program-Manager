...
class PreferenceScorer:
    ...
    async def score_match(
        ...
        """
        ***REMOVED*** Score individual components
    ...
    async def learn_from_history(
        ...
        ***REMOVED*** Preferred days of week
        ...
        ***REMOVED*** Average time to accept
        accept_times = []
    ...
    ***REMOVED*** ===== Private Scoring Methods =====
    ...
    async def _score_explicit_preferences(
        ...
        ***REMOVED*** For now, basic scoring

        ***REMOVED*** Check if weeks are explicitly requested
        if request_a.target_week == request_b.source_week:
            score = 1.0  ***REMOVED*** Perfect match
        elif request_a.target_faculty_id == request_b.source_faculty_id:
            score = 0.9  ***REMOVED*** Targeted match
        else:
            score = 0.5  ***REMOVED*** General availability
    ...
    async def _score_historical_patterns(
        ...
        """
        ***REMOVED*** Check for past swaps between these faculty
        result = await self.db.execute(
            select(SwapRecord).where(
                and_(
        ...
        ***REMOVED*** Check reverse direction
        result = await self.db.execute(
            select(SwapRecord).where(
                and_(
        ...
        if total_past >= 3:
            return 1.0  ***REMOVED*** Proven successful partnership
        elif total_past == 2:
            return 0.9
        elif total_past == 1:
            return 0.8
        else:
            return 0.6  ***REMOVED*** No history (neutral)
    ...
    def _score_temporal_alignment(
        ...
        ***REMOVED*** Same season/month bonus
    ...
    async def _score_workload_fairness(
        ...
        """
        ***REMOVED*** Get total swaps for each faculty
        ...
        ***REMOVED*** Score based on balance
