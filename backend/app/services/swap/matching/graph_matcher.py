...
class GraphMatcher:
    ...
    async def find_optimal_matching(
        ...
        ***REMOVED*** Add edges with weights
        for i, request_a in enumerate(requests):
            for request_b in requests[i + 1 :]:
                ***REMOVED*** Calculate compatibility score
                ...
                if score > 0.3:  ***REMOVED*** Only add edge if reasonably compatible
                    graph.add_edge(
                        str(request_a.id),
                        str(request_b.id),
                        weight=score,
        ...
        ***REMOVED*** Find maximum weight matching
        matching = nx.max_weight_matching(graph, maxcardinality=False)

        ***REMOVED*** Convert matching to result format
        matched_pairs = []
        total_weight = 0.0
        ...
        for edge in matching:
            ...
            ***REMOVED*** Get edge weight
            weight = graph[edge[0]][edge[1]]["weight"]
            total_weight += weight

        ***REMOVED*** Find unmatched requests
    ...
    async def find_stable_matching(
        ...
        ***REMOVED*** Implement Gale-Shapley algorithm
        ***REMOVED*** (Simplified version for demonstration)

        matched_pairs = []
        matched = set()

        ***REMOVED*** Greedy matching based on preferences
        for request in requests:
    ...
    def _calculate_basic_compatibility(
        ...
        ***REMOVED*** Check temporal proximity
        ...
        ***REMOVED*** Check if specific target matches
    ...
    async def _build_preference_lists(
        ...
        for request in requests:
            ***REMOVED*** Build ordered list of other requests by compatibility
            ...
            ***REMOVED*** Sort by score (highest first)
