...
class CompatibilityScore:
    ...
    overall_score: float  ***REMOVED*** 0.0 to 1.0
...
class CompatibilityChecker:
    ...
    async def check_compatibility(
        ...
        ***REMOVED*** Check basic eligibility
        ...
        ***REMOVED*** If we have blocking issues, return early
        if blocking_issues:
        ...
        ***REMOVED*** Calculate individual scores
        ...
        ***REMOVED*** Generate warnings based on scores
    ...
    ***REMOVED*** ===== Private Scoring Methods =====
    ...
    async def _score_schedule_compatibility(
        ...
        ***REMOVED*** Get assignments for both weeks
        ...
        ***REMOVED*** Check for rotation type compatibility
        ...
        ***REMOVED*** If both faculty can do each other's rotations, full score
        ***REMOVED*** Otherwise, reduce score based on incompatibility
        if a_rotation_types and b_rotation_types:
            ***REMOVED*** Check if rotations are swappable
            ***REMOVED*** For now, simple heuristic: similar rotation types score higher
            if a_rotation_types == b_rotation_types:
                score = 1.0
            elif a_rotation_types & b_rotation_types:  ***REMOVED*** Some overlap
                score = 0.8
            else:
                score = 0.6  ***REMOVED*** Different rotations
    ...
    async def _score_preference_alignment(
        ...
        Returns:
            Score from 0.0 to 1.0
        """
        ***REMOVED*** This would integrate with faculty preference service
        ***REMOVED*** For now, simple heuristic based on swap type

        if request_a.swap_type == SwapType.ONE_TO_ONE and request_b.swap_type == SwapType.ONE_TO_ONE:
            ***REMOVED*** Both want to swap - higher alignment
            return 0.9

        if request_a.swap_type == SwapType.ABSORB or request_b.swap_type == SwapType.ABSORB:
            ***REMOVED*** One party just giving away - medium alignment
            return 0.7
    ...
    async def _score_workload_balance(
        ...
        Returns:
            Score from 0.0 to 1.0
        """
        ***REMOVED*** Get total assignments for both faculty
        ...
        ***REMOVED*** Calculate current difference
        current_diff = abs(count_a - count_b)

        ***REMOVED*** Score based on current balance
        ***REMOVED*** Closer to equal = higher score
        if current_diff == 0:
            return 1.0
        elif current_diff <= 2:
            return 0.9
    ...
    async def _score_credential_compatibility(
        ...
        Returns:
            Score from 0.0 to 1.0
        """
        ***REMOVED*** This would check:
        ***REMOVED*** - BLS/ACLS certifications
        ***REMOVED*** - Procedure credentials
        ***REMOVED*** - Specialty qualifications

        ***REMOVED*** For now, assume compatible
        return 0.9
