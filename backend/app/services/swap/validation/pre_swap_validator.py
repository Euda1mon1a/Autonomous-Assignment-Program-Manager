...
class PreSwapValidator:
    ...
    async def validate(self, swap: SwapRecord) -> ValidationResult:
        ...
        ***REMOVED*** Check 1: Faculty existence
        ...
        ***REMOVED*** Check 3: Freeze horizon
        ...
        ***REMOVED*** Check 4: Assignment existence
        ...
        ***REMOVED*** Check 5: No conflicting swaps
        ...
        ***REMOVED*** Check 6: Faculty availability
        checks_performed.append("faculty_availability")
        ***REMOVED*** Check if target faculty has leave/TDY during source week
        if target_faculty:
        ...
        ***REMOVED*** Check 7: Reciprocal availability (for one-to-one swaps)
        if swap.target_week:
    ...
    async def quick_validate(self, swap: SwapRecord) -> bool:
        ...
        Returns:
            True if valid
        """
        ***REMOVED*** Check faculty existence
        ...
        ***REMOVED*** Check week not in past
        if swap.source_week < datetime.utcnow().date():
            return False
    ...
    ***REMOVED*** ===== Private Helper Methods =====
    ...
    async def _check_availability(
        ...
        ***REMOVED*** Query for blocking absences that overlap with the week
        result = await self.db.execute(
            select(Absence).where(
                and_(
                    Absence.person_id == faculty_id,
                    Absence.is_blocking == True,
                    ***REMOVED*** Absence overlaps with week if:
                    ***REMOVED*** absence.start_date <= week_end AND absence.end_date >= week_start
        ...
        if blocking_absences:
            ***REMOVED*** Log which absences are blocking
