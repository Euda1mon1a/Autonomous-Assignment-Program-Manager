...
class SwapChain:
    ...
    chain_type: str  ***REMOVED*** "cycle", "linear", "tree"
    total_swaps: int
    is_valid: bool
...
class ChainSwapCoordinator:
    ...
    async def discover_chains(
        ...
        """
        ***REMOVED*** Get all pending swap requests
        ...
        ***REMOVED*** Build directed graph of swap preferences
        graph = nx.DiGraph()

        for request in pending_requests:
            ***REMOVED*** Add edge: source_faculty -> target_faculty for their desired swap
        ...
        ***REMOVED*** Find cycles (most valuable chain type)
        chains = []

        ***REMOVED*** Find all simple cycles
        try:
        ...
        ***REMOVED*** Generate execution plan
    ...
    async def execute_chain(
        ...
        try:
            ***REMOVED*** Execute all swaps in the chain atomically
            executed_swaps = []

            for i, node in enumerate(chain.nodes):
                next_node = chain.nodes[(i + 1) % len(chain.nodes)]

                ***REMOVED*** Create or update swap record
                if node.swap_request_id:
                    ***REMOVED*** Update existing request
                    result = await self.db.execute(
    ...
    async def validate_chain(self, chain: SwapChain) -> tuple[bool, list[str]]:
        ...
        ***REMOVED*** Check minimum participants
        if len(chain.nodes) < 2:
            errors.append("Chain must have at least 2 participants")

        ***REMOVED*** Verify chain closure (for cycles)
        ...
        ***REMOVED*** Check for conflicting weeks
        weeks_in_use = set()
        ...
        ***REMOVED*** Validate each faculty member can do the swap
        for node in chain.nodes:
    ...
    ***REMOVED*** ===== Private Helper Methods =====
    ...
    async def _build_chain_from_cycle(
        ...
        for i, faculty_id_str in enumerate(cycle):
            ...
            ***REMOVED*** Find the swap request edge
    ...
    def _find_linear_chains(
        ...
        ***REMOVED*** Find all simple paths up to max_length
        for source in graph.nodes():
            for target in graph.nodes():
                ...
                try:
                    ...
                    for path in paths:
                        if len(path) >= 2:  ***REMOVED*** At least 2 participants
                            chains.append(path)
    ...
    def _create_execution_plan(
        ...
        ***REMOVED*** Sort chains by total swaps (larger chains first)
