...
class ClaudeService:
    ...
    def __init__(self):
        ...
        self.model = "claude-3-5-sonnet-20241022"  ***REMOVED*** Latest Claude model
    ...
    async def stream_task(
        ...
        """
        try:
            ***REMOVED*** Build system prompt based on action
            system_prompt = self._build_system_prompt(request.action)

            ***REMOVED*** Build user message with context
            user_message = self._build_user_message(request)

            ***REMOVED*** Stream from Claude
            with self.client.messages.stream(
                ...
                for text in stream.text_stream:
                    ...
                    ***REMOVED*** Yield text updates
                    yield StreamUpdate(
                        type="text",
                        content=text,
                    )

                    ***REMOVED*** Check for code blocks (simplified detection)
                    if "```" in text:
                        ***REMOVED*** More sophisticated code block parsing would go here
                        pass

                    ***REMOVED*** Check for artifact markers
                    if "[ARTIFACT]" in text:
                        yield StreamUpdate(
                            type="artifact",
                ...
                ***REMOVED*** Final completion update
                yield StreamUpdate(
                    type="status",
