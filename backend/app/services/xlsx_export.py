...
***REMOVED*** Color definitions (ARGB format for openpyxl)
COLORS = {
    ***REMOVED*** Rotation type colors (for column A labels)
    "military": PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid"),  ***REMOVED*** Yellow
    "night_float": PatternFill(start_color="FF0000", end_color="FF0000", fill_type="solid"),  ***REMOVED*** Red
    "nicu": PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid"),  ***REMOVED*** Yellow
    "fmit": PatternFill(start_color="00B0F0", end_color="00B0F0", fill_type="solid"),  ***REMOVED*** Blue
    "default": PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid"),  ***REMOVED*** White

    ***REMOVED*** Special cell colors
    ...
    "holiday": PatternFill(start_color="92D050", end_color="92D050", fill_type="solid"),  ***REMOVED*** Green
...
class LegacyXlsxExporter:
    ...
    def generate_block_schedule(
        ...
        ***REMOVED*** Create sheet
        sheet_name = f"Block {block_number}"
        ws = self.wb.create_sheet(title=sheet_name)

        ***REMOVED*** Get all people grouped by type and PGY level
        residents = (
        ...
        ***REMOVED*** Get assignments for this date range
        assignments = (
        ...
        ***REMOVED*** Build assignment lookup: {(person_id, date, time_of_day): assignment}
        assignment_lookup = {}
        ...
        ***REMOVED*** Get absences for this date range
        absences = (
        ...
        ***REMOVED*** Build absence lookup: {(person_id, date): absence_type}
        absence_lookup = {}
        ...
        ***REMOVED*** Calculate number of days
        num_days = (end_date - start_date).days + 1

        ***REMOVED*** Write header section
        self._write_header(ws, block_number, start_date, num_days, federal_holidays)

        ***REMOVED*** Write resident data (starting at row 9)
        current_row = 9
        current_pgy = None

        for resident in residents:
            ***REMOVED*** Add gap row between PGY levels
        ...
        ***REMOVED*** Add gap before faculty section
        current_row += 2

        ***REMOVED*** Write faculty data
        for fac in faculty:
            self._write_person_row(
    ...
    def _write_header(
        ...
        ***REMOVED*** Block number in merged cell C1:D3
        ws.merge_cells('C1:D3')
        ws['C1'] = block_number
        ws['C1'].font = FONTS["block_number"]
        ws['C1'].alignment = Alignment(horizontal='center', vertical='center')

        ***REMOVED*** Date range label
        ...
        ***REMOVED*** Day headers and dates
        col = 6  ***REMOVED*** Start at column F
        for day_offset in range(num_days):
            current_date = start_date + timedelta(days=day_offset)

            ***REMOVED*** Each day has 2 columns (AM/PM)
            am_col = col
            pm_col = col + 1
            am_letter = get_column_letter(am_col)
            pm_letter = get_column_letter(pm_col)

            ***REMOVED*** Row 1-2: Day of week (merged)
            ...
            ***REMOVED*** Row 2: AM/PM labels
            ws[f'{am_letter}2'] = "am"
            ws[f'{pm_letter}2'] = "pm"

            ***REMOVED*** Row 3: Date (merged)
            ...
            ***REMOVED*** Highlight weekends and holidays
            if current_date.weekday() >= 5:  ***REMOVED*** Weekend
    ...
    def _write_person_row(
        ...
        ***REMOVED*** Column A: Rotation label (with color) - use their current rotation if any
        ***REMOVED*** For simplicity, we'll determine this from their first assignment
        ...
        ***REMOVED*** Data columns (F onwards)
        ...
        for day_offset in range(num_days):
            ...
            for time_of_day in ["AM", "PM"]:
                ...
                ***REMOVED*** Set default font first (avoids StyleProxy hashing issue)
                cell.font = FONTS["data"]

                ***REMOVED*** Check for absence first
                ...
                if absence_key in absence_lookup:
                    ...
                    ***REMOVED*** Check for assignment
                    ...
                    if assignment_key in assignment_lookup:
                        ...
                        ***REMOVED*** Color code certain assignments
                        ...
                        ***REMOVED*** Default to W (working) or empty
                        cell.value = "W"
    ...
    def _adjust_column_widths(self, ws, num_days: int) -> None:
        ...
        ***REMOVED*** Data columns (narrower for AM/PM)
        col = 6
...
def generate_legacy_xlsx(
    ...
    ***REMOVED*** Calculate block number if not provided (assume 28-day blocks starting July 1)
    if block_number is None:
        ***REMOVED*** Rough calculation - days since July 1 divided by 28
