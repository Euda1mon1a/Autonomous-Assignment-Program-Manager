...
class ScheduleConflict:
    ...
    date: date
    time_of_day: str
    conflict_type: str  ***REMOVED*** "double_book", "fmit_clinic_overlap", "specialty_unavailable"
    fmit_assignment: str | None = None
    clinic_assignment: str | None = None
    severity: str = "warning"  ***REMOVED*** "error", "warning", "info"
    message: str = ""
...
class ProviderSchedule:
    ...
    def get_fmit_weeks(self) -> list[tuple[date, date]]:
        ...
        for d in fmit_dates:
            ***REMOVED*** Get Monday of this week
    ...
    def has_alternating_pattern(self) -> bool:
        ...
        ***REMOVED*** Check for alternating pattern (gap of exactly 1 week between FMIT weeks)
        gaps = []
        ...
        ***REMOVED*** Alternating = gaps of ~7 days (1 week off between FMIT weeks)
        alternating_count = sum(1 for g in gaps if 6 <= g <= 8)
        return alternating_count >= 2  ***REMOVED*** At least 2 alternating gaps
...
class ClinicScheduleImporter:
    ...
    ***REMOVED*** Common abbreviations for slot types
    ***REMOVED*** -------------------------------------------------------------------------
    ***REMOVED*** SLOT TYPE MAPPING
    ***REMOVED*** Maps cell values to schedule slot types. Direct lookups are checked
    ***REMOVED*** first, then fuzzy matching (number stripping, prefix matching).
    ***REMOVED*** -------------------------------------------------------------------------
    SLOT_TYPE_MAPPING = {
        ***REMOVED*** --- CLINIC (Schedulable patient care) ---
        "c": SlotType.CLINIC,
        "cc": SlotType.CLINIC,  ***REMOVED*** Continuity Clinic
        "clc": SlotType.CLINIC,
        ...
        "sm": SlotType.CLINIC,  ***REMOVED*** Sports Medicine
        "asm": SlotType.CLINIC,  ***REMOVED*** Advanced Sports Medicine
        "sports": SlotType.CLINIC,
        "pr": SlotType.CLINIC,  ***REMOVED*** Procedures
        "vas": SlotType.CLINIC,  ***REMOVED*** Vasectomy Clinic
        "pedc": SlotType.CLINIC,  ***REMOVED*** Peds Clinic
        "pedsp": SlotType.CLINIC,  ***REMOVED*** Peds Specialty
        "hv": SlotType.CLINIC,  ***REMOVED*** Home Visit
        "hc": SlotType.CLINIC,  ***REMOVED*** Home Care
        "c-i": SlotType.CLINIC,  ***REMOVED*** Clinic Internship
        "rcc": SlotType.CLINIC,  ***REMOVED*** Resident Continuity Clinic
        ***REMOVED*** --- FMIT / INPATIENT (Blocks clinic availability) ---
        "fmit": SlotType.FMIT,
        "nf": SlotType.FMIT,  ***REMOVED*** Night Float
        "nicu": SlotType.FMIT,  ***REMOVED*** NICU rotation
        "pedw": SlotType.FMIT,  ***REMOVED*** Peds Ward
        "imw": SlotType.FMIT,  ***REMOVED*** IM Ward
        "inpt": SlotType.FMIT,
        "inpatient": SlotType.FMIT,
        "ward": SlotType.FMIT,
        "wards": SlotType.FMIT,
        "er": SlotType.FMIT,  ***REMOVED*** ER shifts block clinic
        "kap": SlotType.FMIT,  ***REMOVED*** Kapiolani (offsite hospital)
        "straub": SlotType.FMIT,  ***REMOVED*** Straub (offsite hospital)
        "oic": SlotType.FMIT,  ***REMOVED*** Officer In Charge (on-call)
        ***REMOVED*** --- OFF / UNAVAILABLE (Do not schedule) ---
        "off": SlotType.OFF,
        "pc": SlotType.OFF,  ***REMOVED*** Post Call
        "do": SlotType.OFF,  ***REMOVED*** Day Off
        "w": SlotType.OFF,  ***REMOVED*** Weekend (context: Sat/Sun cells)
        "x": SlotType.OFF,
        "-": SlotType.OFF,
        "": SlotType.OFF,
        "fed": SlotType.OFF,  ***REMOVED*** Federal Holiday
        "hol": SlotType.OFF,  ***REMOVED*** Holiday
        ***REMOVED*** --- VACATION / LEAVE ---
        "vac": SlotType.VACATION,
        "vacation": SlotType.VACATION,
        "lv": SlotType.VACATION,
        "leave": SlotType.VACATION,
        "al": SlotType.VACATION,  ***REMOVED*** Annual Leave
        "dep": SlotType.VACATION,  ***REMOVED*** Deployment
        "tdy": SlotType.VACATION,  ***REMOVED*** TDY (temporary duty)
        ***REMOVED*** --- CONFERENCE / ACADEMIC (Protected time) ---
        ...
        ***REMOVED*** --- ADMIN (Administrative time) ---
        ...
        "rsh": SlotType.ADMIN,  ***REMOVED*** Research
    ...
    def _load_known_providers(self) -> None:
        ...
        for person in people:
            ***REMOVED*** Store by lowercase name for fuzzy matching
    ...
    def classify_slot(self, value: str) -> SlotType:
        ...
        ***REMOVED*** Normalize: lowercase, strip whitespace
        clean = str(value).strip().lower()

        if not clean:
            return SlotType.OFF

        ***REMOVED*** 1. Handle compound codes like "PC/OFF" or "C/CV"
        if "/" in clean:
            parts = clean.split("/")
            ***REMOVED*** Prioritize restrictive types (safety first)
            for part in parts:
                part = part.strip()
                if part in self.SLOT_TYPE_MAPPING:
                    ...
                    if slot_type in (SlotType.FMIT, SlotType.OFF, SlotType.VACATION):
                        return slot_type
            ***REMOVED*** Otherwise classify first part
            first_part = parts[0].strip()
            if first_part:
                return self.classify_slot(first_part)

        ***REMOVED*** 2. Direct lookup
        if clean in self.SLOT_TYPE_MAPPING:
            return self.SLOT_TYPE_MAPPING[clean]

        ***REMOVED*** 3. Strip trailing numbers (e.g., "C30" -> "C", "FMIT2" -> "FMIT")
        ...
        ***REMOVED*** 4. Prefix matching - only for keys with 2+ chars to avoid false positives
        for key, slot_type in self.SLOT_TYPE_MAPPING.items():
            if len(key) >= 2 and clean.startswith(key):
                return slot_type

        ***REMOVED*** 5. Fallback heuristics for common terms
        if "call" in clean:
    ...
    def parse_date_from_header(self, value) -> date | None:
        ...
        ***REMOVED*** Already a date/datetime
        if isinstance(value, datetime):
            return value.date()
        if isinstance(value, date):
            return value

        ***REMOVED*** String formats
        text = str(value).strip()

        ***REMOVED*** Try common formats
        formats = [
            "%Y-%m-%d",
            "%m/%d/%Y",
            "%m/%d/%y",
        ...
        for fmt in formats:
            try:
                ...
                ***REMOVED*** If year missing, assume current academic year
                if parsed.year == 1900:
                    today = date.today()
                    if parsed.month >= 7:
    ...
    def detect_format(self, ws: Worksheet) -> dict:
        ...
        """
        ***REMOVED*** Sample first 10 rows/cols to detect structure
        max_sample = 15

        ***REMOVED*** Look for dates in row 1-5
        date_row = None
        date_cols = []
        ...
        for row_idx in range(1, min(6, ws.max_row + 1)):
            ...
            if len(found_dates) >= 3:  ***REMOVED*** At least 3 dates = likely header row
                date_row = row_idx
                date_cols = found_dates
                break

        if not date_row:
            ***REMOVED*** Try transposed format (dates in column A)
            found_dates = []
        ...
        ***REMOVED*** Find provider column (usually leftmost non-date column)
        provider_col = 1
        for col_idx in range(1, min(6, ws.max_column + 1)):
            ...
            if (
                ...
                ***REMOVED*** Check if it looks like a name
                if len(str(cell.value)) > 2:
                    provider_col = col_idx
                    break

        ***REMOVED*** Detect AM/PM columns (dates might have 2 columns each)
        expanded_date_cols = []
        for i, (col_idx, d) in enumerate(date_cols):
            ***REMOVED*** Check if next column is also for same date (AM/PM split)
            ...
            ***REMOVED*** Check row above or below for AM/PM labels
            ...
            if time_label and str(time_label).lower().strip() in ["am", "pm"]:
                ...
                ***REMOVED*** Single column per date - assume full day
                expanded_date_cols.append((col_idx, d, "AM"))
    ...
    def import_worksheet(
        ...
        ***REMOVED*** Detect or use provided format
    ...
    def _import_providers_in_rows(
        ...
        for row_idx in range(data_start_row, ws.max_row + 1):
            ...
            ***REMOVED*** Get or create provider schedule
            ...
            ***REMOVED*** Process each date column
    ...
    def _import_providers_in_columns(
        ...
        ***REMOVED*** Get provider names from header row
        providers = []
        ...
        for row_idx, slot_date in date_rows:
            for col_idx, provider_name in providers:
                ...
                slot = ScheduleSlot(
                    provider_name=provider_name,
                    date=slot_date,
                    time_of_day="AM",  ***REMOVED*** Assume full day
                    slot_type=slot_type,
                    raw_value=raw_value,
                )
...
class SwapCandidate:
    ...
    faculty: str
    can_take_week: date
    gives_week: date | None = None  ***REMOVED*** None if absorbing, date if 1:1 swap
...
class ConflictDetector:
    ...
    def _detect_double_bookings(self) -> None:
        ...
        ***REMOVED*** Get all providers in both schedules
...
class SwapFinder:
    ...
    def from_fmit_file(
        ...
        ***REMOVED*** Build external conflicts list
        all_conflicts = list(external_conflicts or [])

        ***REMOVED*** Add absence-based conflicts if requested
...
def analyze_schedule_conflicts(
    ...
    ***REMOVED*** Import clinic schedule if provided
    clinic_result = None
    ...
    ***REMOVED*** Detect conflicts
    ...
    if clinic_result:
        ...
        ***REMOVED*** Just check FMIT schedule for alternating patterns
...
def load_external_conflicts_from_absences(
    ...
    ***REMOVED*** Query blocking absences within the date range
    absences = (
    ...
    for absence in absences:
        ***REMOVED*** Only include blocking absences
        if not absence.should_block_assignment:
            continue

        ***REMOVED*** Map absence_type to conflict_type
        conflict_type_map = {
            "vacation": "leave",
...
***REMOVED*** =============================================================================
***REMOVED*** BLOCK SCHEDULE PARSER - Fuzzy-tolerant parsing for rotation spreadsheets
***REMOVED*** =============================================================================
...
class ParsedBlockAssignment:
    ...
    person_name: str
    date: date
    template: str  ***REMOVED*** R1, R2, R3, C19, etc.
    role: str  ***REMOVED*** PGY 1, PGY 2, PGY 3, FAC
    slot_am: Optional[str] = None  ***REMOVED*** Value in AM column
    slot_pm: Optional[str] = None  ***REMOVED*** Value in PM column
    row_idx: int = 0
    confidence: float = 1.0  ***REMOVED*** Lower if fuzzy matched
...
class BlockParseResult:
    ...
    residents: list[dict] = field(default_factory=list)  ***REMOVED*** {name, template, role}
...
class BlockScheduleParser:
    ...
    ***REMOVED*** Patterns for finding column headers
    HEADER_PATTERNS = {
    ...
    ***REMOVED*** Name pattern: "Last, First" or "Last, First Middle"
    NAME_PATTERN = re.compile(r"^([A-Z][a-z-]+),\s+([A-Z][a-z-]+)", re.I)

    ***REMOVED*** Known rotation templates
    VALID_TEMPLATES = {
        "R1", "R2", "R3",  ***REMOVED*** Resident rotations
        "C19", "Ca9", "CP",  ***REMOVED*** Clinic/faculty
        "TY", "PSYCH", "IPAP",  ***REMOVED*** Transitional/specialty
        "NF",  ***REMOVED*** Night float
    }
    ...
    ***REMOVED*** Known roles
    ...
    def __init__(self, known_people: Optional[list[str]] = None):
        ...
        self._name_cache: dict[str, tuple[str, float]] = {}  ***REMOVED*** raw -> (matched, confidence)
    ...
    def parse_block_sheet(
        ...
        ***REMOVED*** Extract block number from sheet name or content
    ...
    def _find_anchors(self, ws: Worksheet) -> dict:
        ...
        ***REMOVED*** Scan first 10 rows for headers
    ...
    def _find_date_columns(self, ws: Worksheet) -> dict[int, date]:
        ...
        ***REMOVED*** Scan first 5 rows, columns 1-100
        for row_idx in range(1, 6):
            for col_idx in range(1, 100):
                try:
                    cell = ws.cell(row=row_idx, column=col_idx)

                    ***REMOVED*** Skip merged cells
    ...
    def _find_person_rows(self, ws: Worksheet, anchors: dict) -> list[dict]:
        ...
        ***REMOVED*** Use discovered anchors or sensible defaults
        ...
        ***REMOVED*** Scan rows after header
        for row_idx in range(header_row + 1, min(ws.max_row + 1, 200)):
            ...
            ***REMOVED*** Check if it looks like a name (Last, First pattern)
            ...
            ***REMOVED*** Log unknown templates for debugging
    ...
    def _match_name(self, raw_name: str) -> tuple[str, float]:
        ...
        ***REMOVED*** Normalize for comparison
        ...
        for known in self.known_people:
            ...
            ***REMOVED*** Fuzzy match using SequenceMatcher
        ...
        ***REMOVED*** No good match - return original
    ...
    def _get_cell_value(self, ws: Worksheet, row: int, col: int) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(cell, MergedCell):
                ***REMOVED*** Find the parent cell of the merge range
    ...
    def _extract_block_number(self, sheet_name: str, ws: Worksheet) -> int:
        ...
        ***REMOVED*** Try sheet name first: "Block 10", "Block10", "10"
        match = re.search(r"block\s*(\d+)", sheet_name, re.I)
        if match:
            return int(match.group(1))

        ***REMOVED*** Just a number
        match = re.search(r"^(\d+)", sheet_name.strip())
        if match:
            return int(match.group(1))

        ***REMOVED*** Try cells A1, B1, B2 for block number
...
def parse_block_schedule(
    ...
    ***REMOVED*** Try common sheet name patterns
    sheet_names = [
    ...
    ***REMOVED*** Fuzzy match sheet name
...
def parse_fmit_attending(
    ...
    if sheet_name not in wb.sheetnames:
        ***REMOVED*** Try to find it by partial match
    ...
    ***REMOVED*** Scan for block rows
    for row_idx in range(1, ws.max_row + 1):
        col0_val = ws.cell(row=row_idx, column=1).value

        ***REMOVED*** Check if this is a block header row (has block number)
        if col0_val and str(col0_val).strip().isdigit():
            block_num = int(str(col0_val).strip())

            ***REMOVED*** Faculty names are in the next row, columns 3-6
