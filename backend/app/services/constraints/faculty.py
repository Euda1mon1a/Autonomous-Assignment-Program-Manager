...
class FacultyPreferenceCache:
    ...
    DEFAULT_TTL = 900  ***REMOVED*** 15 minutes
    ...
    def __init__(self, ttl_seconds: int = DEFAULT_TTL):
        ...
        ***REMOVED*** Local LRU cache fallback when Redis is unavailable
    ...
    def set_preferences(self, faculty_id: UUID, preferences: FacultyPreference) -> None:
        ...
        """
        key = f"{self.KEY_PREFIX}{faculty_id}"
        ***REMOVED*** Cache a dictionary representation, not the ORM object
        pref_data = {
    ...
    def invalidate_faculty(self, faculty_id: UUID) -> int:
        ...
        ***REMOVED*** Also clear from local cache
        with self._lock:
    ...
    def _get(self, key: str) -> Any | None:
        ...
        ***REMOVED*** Fallback to local cache
        with self._lock:
    ...
    def _put(self, key: str, value: Any) -> None:
        ...
        ***REMOVED*** Fallback to local cache
        with self._lock:
            ...
            ***REMOVED*** Limit local cache size
            if len(self._local_cache) > 1000:
                ***REMOVED*** Remove oldest 20%
                sorted_keys = sorted(
    ...
    def get_preferences(self, faculty_id: UUID) -> FacultyPreference | None:
        ...
        ***REMOVED*** Check cache first
        ...
        if cached is not None:
            ***REMOVED*** Reconstruct a FacultyPreference-like object from cached data
...
class FacultyConstraintService:
    ...
    def validate_faculty_access(self, user: User, faculty_id: UUID) -> bool:
        ...
        """
        ***REMOVED*** Admin users can access any faculty
        if user.role == "admin":
            return True

        ***REMOVED*** Non-admin users can only access their own data
    ...
    def get_preferences_for_constraint(
        ...
        """
        ***REMOVED*** Validate access if user provided
        if user is not None:
