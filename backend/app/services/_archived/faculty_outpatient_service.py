...
class FacultyOutpatientAssignmentService:
    ...
    def generate_faculty_outpatient_assignments(
        ...
        # Step 3: Get outpatient templates
        ...
        # Step 4: Clear existing faculty outpatient assignments if regenerating
        cleared_count = 0
        if regenerate:
            cleared_count = self._clear_existing_faculty_outpatient(
                block_start, block_end, outpatient_templates
            )

        # Step 5: Get available faculty (not on FMIT, leave, etc.)
    ...
    def _clear_existing_faculty_outpatient(
        ...
        """
        Clear existing faculty outpatient half-day assignments for the block.

        Only clears:
        - Faculty assignments (not resident)
        - Activities tied to outpatient templates or AT supervision
        - Non-override manual rows generated by this service
        """
        # Get faculty IDs
        faculty_ids = [
            f.id
        ...
        # Resolve activity IDs for outpatient templates
        activity_ids: set[UUID] = set()
        for template in templates:
            activity = self._get_or_create_activity_for_template(template)
            if activity:
                activity_ids.add(activity.id)

        # Include AT supervision activity if present
        ...
        # Delete outpatient activity rows only (MANUAL/SOLVER/TEMPLATE)
        # Do NOT delete OFF slots - they're needed to maintain 56 slots per faculty
        # The generation functions use update-in-place to overwrite OFF when needed
        deleted = (
    ...
    def _get_available_faculty(
        ...
        # Get faculty with FMIT during this period (preload-based)
        fmit_preloads = (
            self.db.query(InpatientPreload)
            .filter(
                InpatientPreload.rotation_type == InpatientRotationType.FMIT,
                InpatientPreload.start_date <= block_end,
                InpatientPreload.end_date >= block_start,
            )
            .all()
        )
        fmit_faculty_ids = {p.person_id for p in fmit_preloads}
    ...
    def _get_outpatient_templates(self) -> list[RotationTemplate]:
        """Get rotation templates for outpatient activities."""
        return (
            self.db.query(RotationTemplate)
            .filter(RotationTemplate.rotation_type == "outpatient")
            .all()
        )

    def _get_or_create_activity_for_template(
        self, template: RotationTemplate
    ) -> Activity | None:
        """Resolve or create the Activity for an outpatient rotation template."""
        cache = getattr(self, "_activity_cache", {})
        if template.id in cache:
        ...
        activity = (
            ...
            .filter(
                or_(
                    Activity.display_abbreviation == display_abbrev,
        ...
        cache[template.id] = activity
        self._activity_cache = cache
        return activity

    def _get_supervision_activity(self) -> Activity | None:
        """Resolve or create the AT supervision activity."""
        cached = getattr(self, "_supervision_activity", None)
        if cached:
            return cached
        ...
        activity = (
            ...
            .filter(
                or_(
                    func.lower(Activity.code) == "at",
                    func.upper(Activity.display_abbreviation) == "AT",
                )
            )
            .first()
        )
        ...
        self._supervision_activity = activity
        return activity
    ...
    def _generate_clinic_assignments(
        ...
        clinic_activity = self._get_or_create_activity_for_template(fm_clinic_template)
        ...
        for block in workday_blocks:
            ...
            counts_toward = activity_counts_toward_fmc_capacity_for_template(
    ...
    def _generate_supervision_assignments(
        ...
        faculty: list[Person],
        blocks: list[Block],
        clinic_assignments: list[HalfDayAssignment] | None = None,
    ) -> list[HalfDayAssignment]:
        """
        ...
        # Build set of (date, time_of_day, person_id) from new clinic assignments
        # These faculty are already assigned to these slots
        new_assignments_set: set[tuple[date, str, UUID]] = set()
        if clinic_assignments:
            for ca in clinic_assignments:
                new_assignments_set.add((ca.date, ca.time_of_day, ca.person_id))
        ...
        for block_id, block_assignments in assignments_by_block.items():
            ...
            for slot_date, slot_time, fid in new_assignments_set:
                if slot_date == block.date and slot_time == block.time_of_day:
    ...
    def _build_faculty_summaries(
        self,
        faculty: list[Person],
        clinic_assignments: list[HalfDayAssignment],
        supervision_assignments: list[HalfDayAssignment],
