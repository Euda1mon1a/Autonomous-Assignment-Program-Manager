...
***REMOVED*** Optional dependency - game theory features require axelrod
try:
    ...
    if not TYPE_CHECKING:
        ***REMOVED*** Create dummy module for runtime
        class DummyAxelrod:
            Player = Any
            Action = Any
            Game = Any
        ...
        axl = DummyAxelrod()  ***REMOVED*** type: ignore
...
***REMOVED*** Only define ResilienceConfigPlayer if axelrod is available
if AXELROD_AVAILABLE:

    class ResilienceConfigPlayer(axl.Player):  ***REMOVED*** type: ignore
        """
        ...
        def strategy(self, opponent: axl.Player) -> axl.Action:
            ...
            ***REMOVED*** Strategy-specific behavior
            if self.strategy_type == "cooperative":
                ...
                ***REMOVED*** Win-stay, lose-shift
                if len(self.history) == 0:
                    return axl.Action.C
                last_payoff = self._calculate_last_payoff(opponent)
                if last_payoff >= 3:  ***REMOVED*** Good outcome (CC or DC)
                ...
                ***REMOVED*** Defect first, then TFT
                if len(self.history) == 0:
                    return axl.Action.D
                return opponent.history[-1]

            elif self.strategy_type == "forgiving_tft":
                ***REMOVED*** TFT but only retaliate if opponent defected multiple times
    ...
    ***REMOVED*** Create a stub class when axelrod is not available
    ResilienceConfigPlayer = None  ***REMOVED*** type: ignore
...
class GameTheoryService:
    ...
    def run_tournament(self, tournament_id: UUID) -> dict:
        ...
        ***REMOVED*** Update status
        ...
        try:
            ...
            ***REMOVED*** Convert to Axelrod players
            players = [self._strategy_to_player(s) for s in strategies]

            ***REMOVED*** Create custom game with specified payoffs
            game = axl.Game(
                r=tournament.payoff_cc,  ***REMOVED*** Reward for mutual cooperation
                s=tournament.payoff_cd,  ***REMOVED*** Sucker's payoff
                t=tournament.payoff_dc,  ***REMOVED*** Temptation to defect
                p=tournament.payoff_dd,  ***REMOVED*** Punishment for mutual defection
            )
            ...
            ***REMOVED*** Store matches
            for interaction in results.interactions:
                ***REMOVED*** Simplified - store summary matches
                pass

            ***REMOVED*** Update tournament
    ...
    ***REMOVED*** =========================================================================
    ...
    def run_evolution(self, evolution_id: UUID) -> dict:
        ...
        ***REMOVED*** Update status
        ...
        try:
            ***REMOVED*** Build initial population
            players = []
            ...
            ***REMOVED*** Track population history
            population_history = []
            ...
            try:
                ...
                for gen, pop in enumerate(mp):
                    ...
                    ***REMOVED*** Check if fixation occurred
                    if mp.fixated:
                        break
            ...
            ***REMOVED*** Update evolution record
            ...
            ***REMOVED*** Find winner strategy ID
            if winner_name:
                winner = (
                    self.db.query(ConfigStrategy)
    ...
    def validate_strategy(
        ...
        ***REMOVED*** Run matches
        ...
        for _ in range(repetitions):
            ...
            ***REMOVED*** Reset players for next match
            config_player.reset()
            tft_player.reset()
    ...
    ***REMOVED*** =========================================================================
    ...
    def analyze_current_config(
        ...
        """
        ***REMOVED*** Determine strategy type based on config
