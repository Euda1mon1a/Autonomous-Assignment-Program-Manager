...
class ShapleyValueService:
    ...
    async def calculate_shapley_values(
        ...
        Example:
            ...
            ...     num_samples=2000
            ... )
            >>> results[fac1_id].shapley_value  ***REMOVED*** 0.35 (35% contribution)
            >>> results[fac1_id].fair_workload_target  ***REMOVED*** 280 hours (35% of total)
        """
        ***REMOVED*** Load faculty data
        ...
        ***REMOVED*** Build results
        ...
        for fac_id in faculty_ids:
            ...
            ***REMOVED*** Normalize Shapley value to proportion (0-1)
            shapley_proportion = shapley_value / total_value if total_value > 0 else 0.0

            ***REMOVED*** Fair workload target based on Shapley proportion
            fair_workload_target = shapley_proportion * total_hours

            ***REMOVED*** Equity gap (positive = overworked, negative = underworked)
            equity_gap = current_workload - fair_workload_target

            ***REMOVED*** Marginal contribution (value added by this faculty)
    ...
    async def get_equity_report(
        ...
        Example:
            ...
            ... )
            >>> metrics.equity_gap_std_dev  ***REMOVED*** 12.5 hours (low = fair distribution)
            >>> metrics.overworked_count  ***REMOVED*** 2 faculty over their fair share
        """
        ...
        ***REMOVED*** Count faculty over/under fair share
        overworked_count = sum(1 for gap in equity_gaps if gap > 0)
        underworked_count = sum(1 for gap in equity_gaps if gap < 0)

        ***REMOVED*** Calculate standard deviation of equity gaps (lower = more equitable)
        import statistics
        ...
        ***REMOVED*** Identify most/least loaded faculty
    ...
    ***REMOVED*** =========================================================================
    ...
    async def _calculate_total_coverage(
        ...
        stmt = select(func.count(Block.id)).where(
            ...
            Block.is_holiday == False,  ***REMOVED*** noqa: E712
        )
        result = await self.db.execute(stmt)
    ...
    async def _calculate_workloads(
        ...
        for fac_id in faculty_ids:
            ...
            ***REMOVED*** Each assignment is a half-day block = 4 hours
    ...
    async def _monte_carlo_shapley(
        ...
        for _ in range(num_samples):
            ***REMOVED*** Random permutation (order of joining coalition)
            order = random.sample(faculty_ids, len(faculty_ids))

            coalition = set()
            for fac_id in order:
                ***REMOVED*** Marginal contribution: v(S ∪ {i}) - v(S)
    ...
    ***REMOVED*** =========================================================================
    ...
    async def suggest_workload_adjustments(
        ...
        ***REMOVED*** Sort by magnitude of gap (most urgent first)
