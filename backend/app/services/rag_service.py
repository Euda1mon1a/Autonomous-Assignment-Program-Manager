...
class RAGService:
    ...
    ***REMOVED*** Default configuration
    DEFAULT_CHUNK_SIZE = 500  ***REMOVED*** tokens
    DEFAULT_CHUNK_OVERLAP = 50  ***REMOVED*** tokens
    DEFAULT_TOP_K = 5
    DEFAULT_MIN_SIMILARITY = 0.5
    APPROX_CHARS_PER_TOKEN = 4  ***REMOVED*** Rough approximation for English text
    ...
    async def ingest_document(
        ...
        try:
            ***REMOVED*** Chunk the document
            chunks = self._chunk_text(content, chunk_size, chunk_overlap)
            logger.info(
                f"Chunked document into {len(chunks)} chunks (type: {doc_type})"
            )

            ***REMOVED*** Generate embeddings for all chunks (batch for efficiency)
            embeddings = self.embedding_service.embed_batch(chunks)

            ***REMOVED*** Create RAGDocument records
            chunk_ids = []
            for chunk_text, embedding in zip(chunks, embeddings):
                ...
                self.db.add(rag_doc)
                self.db.flush()  ***REMOVED*** Get ID without committing
                chunk_ids.append(rag_doc.id)

            ***REMOVED*** Commit all chunks
            self.db.commit()
    ...
    async def retrieve(
        ...
        try:
            ...
            ***REMOVED*** Build query with vector similarity search
            ***REMOVED*** Using cosine distance: 1 - cosine_similarity
            ***REMOVED*** So similarity = 1 - distance
            stmt = select(
                RAGDocument,
            ...
            ***REMOVED*** Apply metadata filters
            if metadata_filters:
                for key, value in metadata_filters.items():
                    ***REMOVED*** Use PostgreSQL JSON operators for metadata filtering
                    stmt = stmt.where(
                        RAGDocument.metadata_[key].astext == str(value)
                    )

            ***REMOVED*** Filter by minimum similarity
            ***REMOVED*** Note: This is done in SQL for efficiency
            stmt = stmt.where(
                (1 - RAGDocument.embedding.cosine_distance(query_embedding))
                >= min_similarity
            )

            ***REMOVED*** Order by similarity (highest first) and limit
            stmt = stmt.order_by(text("similarity DESC")).limit(top_k)

            ***REMOVED*** Execute query
            results = self.db.execute(stmt).all()

            ***REMOVED*** Convert to RetrievedDocument schema
            documents = [
                RetrievedDocument(
    ...
    async def build_context(
        ...
        ***REMOVED*** Retrieve relevant documents
        retrieval_response = await self.retrieve(
            query=query,
            top_k=20,  ***REMOVED*** Get more results, we'll filter by token count
            doc_type=doc_type,
            min_similarity=0.5,
        )

        ***REMOVED*** Build context string with token budget
        context_parts = []
        sources = []
        current_tokens = 0
        max_content_tokens = max_tokens - 200  ***REMOVED*** Reserve tokens for formatting

        for doc in retrieval_response.documents:
            ***REMOVED*** Estimate tokens for this document
            doc_text = doc.content
            if include_metadata and doc.metadata:
                ***REMOVED*** Add metadata as context
                metadata_str = ", ".join(
            ...
            ***REMOVED*** Check if adding this document would exceed budget
            if current_tokens + doc_tokens > max_content_tokens:
                ***REMOVED*** If we haven't added any documents yet, truncate this one
        ...
        ***REMOVED*** Format final context
        context = "\n\n---\n\n".join(context_parts)

        ***REMOVED*** Add header
        header = f"***REMOVED*** Relevant Context for Query: {query}\n\n"
        context = header + context

        ***REMOVED*** Calculate final token count
    ...
    async def get_health(self) -> RAGHealthResponse:
        ...
        try:
            ...
            ***REMOVED*** Check if pgvector extension is enabled
            vector_enabled = self.db.execute(
                text("SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'vector')")
            ).scalar()

            ***REMOVED*** Check if indexes exist
    ...
    def _chunk_text(
        ...
        ***REMOVED*** Split into sentences (simple approach)
        ...
        for sentence in sentences:
            ...
            ***REMOVED*** If adding this sentence would exceed chunk size
            if current_length + sentence_length > chunk_chars and current_chunk:
                ***REMOVED*** Save current chunk
                chunks.append(" ".join(current_chunk))

                ***REMOVED*** Start new chunk with overlap
                ***REMOVED*** Keep last few sentences for overlap
                overlap_text = " ".join(current_chunk)
                if len(overlap_text) > overlap_chars:
                    ***REMOVED*** Trim to overlap size
                    overlap_text = overlap_text[-overlap_chars:]
                    ***REMOVED*** Try to start at sentence boundary
                ...
                current_length += sentence_length + 1  ***REMOVED*** +1 for space

        ***REMOVED*** Add final chunk if any
        if current_chunk:
            chunks.append(" ".join(current_chunk))

        ***REMOVED*** Filter out very small chunks (< 10% of target size)
    ...
    def _split_into_sentences(self, text: str) -> list[str]:
        ...
        Returns:
            List of sentences
        """
        ***REMOVED*** Simple sentence splitting regex
        ***REMOVED*** Matches periods, exclamation marks, question marks followed by space/newline
        sentence_endings = re.compile(r"(?<=[.!?])\s+")
        sentences = sentence_endings.split(text)

        ***REMOVED*** Clean up sentences
