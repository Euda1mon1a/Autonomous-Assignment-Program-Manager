...
Usage:
    ...
    ***REMOVED*** Create service instance
...
class AuditService:
    ...
    def query_audit_logs(
        ...
        ***REMOVED*** Convert to Pydantic models
    ...
    def get_entity_history(
        ...
        """
        ***REMOVED*** Get history from repository
        history = self.repository.get_entity_history(entity_type, entity_id)

        ***REMOVED*** Also try to get detailed version info from the model
        try:
        ...
        ***REMOVED*** Fall back to repository data
    ...
    def export_audit_logs(
        ...
        ***REMOVED*** Get all matching entries
        entries, _ = self.repository.get_audit_entries(
            filters=filters,
            page=1,
            page_size=10000,  ***REMOVED*** Max export size
            sort_by="changed_at",
            sort_direction="desc",
        )
    ...
    ***REMOVED*** Legacy static methods for backward compatibility
    @staticmethod
    def get_assignment_history(
        ...
        try:
            ...
            ***REMOVED*** Get the assignment with its version class
            ...
            for i, version in enumerate(versions):
                ...
                ***REMOVED*** Calculate changes from previous version
                if i > 0:
                    prev = versions[i - 1]
    ...
    def get_recent_changes(
        ...
        try:
            ...
            ***REMOVED*** Sort by changed_at descending
            results.sort(key=lambda x: x["changed_at"] or datetime.min, reverse=True)

            return results[:100]  ***REMOVED*** Limit total results
    ...
    ***REMOVED*** Private helper methods
    ...
    def _export_to_csv(
        ...
        for entry in entries:
            ***REMOVED*** Only include requested columns
            row = {col: entry.get(col, "") for col in columns}
            ***REMOVED*** Format datetime
...
def _diff_versions(old_version, new_version) -> dict:
    ...
    ***REMOVED*** Get columns to compare (exclude version-specific columns)
