...
class CalendarService:
    ...
    def _create_timezone() -> Timezone:
        ...
        ***REMOVED*** Hawaii Standard Time (HST) - UTC-10, no daylight saving time
    ...
    def generate_ics_for_person(
        ...
        ***REMOVED*** Query assignments with blocks and rotation templates
        query = (
            db.query(Assignment)
            .options(
        ...
        ***REMOVED*** Add proper VTIMEZONE component
        cal.add_component(CalendarService._create_timezone())

        ***REMOVED*** Add events for each assignment
        for assignment in assignments:
            event = Event()

            ***REMOVED*** Get activity name
            ...
            ***REMOVED*** Add location if available
            ...
            ***REMOVED*** Add last modified timestamp
    ...
    def generate_ics_for_rotation(
        ...
        Returns:
            ICS file content as string
        """
        ***REMOVED*** Query assignments for this rotation
        assignments = (
            db.query(Assignment)
            .options(
                joinedload(Assignment.block),
        ...
        ***REMOVED*** Get rotation template name
        ...
        ***REMOVED*** Add events for each assignment
        for assignment in assignments:
            ...
            ***REMOVED*** Summary includes person name and role
    ...
    def generate_ics_all(
        ...
        Returns:
            ICS file content as string
        """
        ***REMOVED*** Query assignments with blocks, rotation templates, and persons
        query = (
            db.query(Assignment)
            .options(
                joinedload(Assignment.block),
        ...
        ***REMOVED*** Filter by person IDs if specified
        if person_ids:
            query = query.filter(Assignment.person_id.in_(person_ids))

        ***REMOVED*** Filter by rotation IDs if specified
        if rotation_ids:
        ...
        ***REMOVED*** Add events for each assignment
        for assignment in assignments:
            ...
            ***REMOVED*** Get activity name with role
            activity_name = assignment.activity_name
    ...
    def create_subscription(
        ...
        """
        ***REMOVED*** Verify person exists
        person = db.query(Person).filter(Person.id == person_id).first()
        if not person:
            raise ValueError(f"Person not found: {person_id}")

        ***REMOVED*** Create subscription
    ...
    def validate_subscription_token(db: Session, token: str) -> UUID | None:
        ...
        ***REMOVED*** Update last accessed timestamp
        subscription.touch()
        db.commit()
