...
class BlockSchedulerService:
    ...
    def get_block_dates(
        ...
        """
        ***REMOVED*** Academic year starts July 1
        year_start = date(academic_year, 7, 1)

        if block_number == 0:
            ***REMOVED*** Block 0 is orientation, typically last week of June
            start = year_start - timedelta(days=7)
            end = year_start - timedelta(days=1)
        else:
            ***REMOVED*** Regular blocks are 28 days each
    ...
    def get_residents_with_leave_in_block(
        ...
        ***REMOVED*** Get all residents
        ...
        for resident in residents:
            ***REMOVED*** Get absences that overlap with the block
            absences = self.absence_repo.get_by_person_and_date_range(
                resident.id, start_date, end_date
            )

            if absences:
                ***REMOVED*** Calculate total leave days in block
                leave_days = 0
                leave_types = set()

                for absence in absences:
                    ***REMOVED*** Calculate overlap with block
    ...
    def get_rotation_capacities(
        ...
        ***REMOVED*** Get all rotation templates
        ...
        for template in templates:
            ***REMOVED*** Count current assignments
    ...
    def find_best_leave_eligible_rotation(
        ...
        """
        candidates: list[tuple[RotationTemplate, int]] = []  ***REMOVED*** (template, score)
        ...
        for rotation_id, slot in capacities.items():
            ...
            ***REMOVED*** Skip if at capacity
            if slot.is_full:
                continue

            ***REMOVED*** Calculate match score
            score = 0

            ***REMOVED*** Prefer rotations with available capacity (not overcrowded)
            if slot.available is not None:
                score += slot.available * 10

            ***REMOVED*** Specialty match bonus
            if resident.specialties and slot.template.requires_specialty:
                if slot.template.requires_specialty in resident.specialties:
                    score += 50

            ***REMOVED*** PGY level consideration (some rotations may be better for certain levels)
            ***REMOVED*** For now, all PGY levels are treated equally
        ...
        ***REMOVED*** Sort by score descending, return best match
    ...
    def find_best_coverage_rotation(
        ...
        for rotation_id, slot in capacities.items():
            ...
            ***REMOVED*** High priority for non-leave-eligible (coverage critical)
            if not slot.template.leave_eligible:
                score += 100

            ***REMOVED*** Prefer under-staffed rotations
    ...
    def schedule_block(
        ...
        ***REMOVED*** Step 1: Assign residents with leave to leave-eligible rotations
        for leave_data in residents_with_leave:
            ...
            if rotation:
                ...
                ***REMOVED*** No leave-eligible rotation available - conflict
        ...
        ***REMOVED*** Step 2: Assign remaining residents (if requested)
        ...
        ***REMOVED*** Step 3: Identify coverage gaps
        for rotation_id, slot in capacities.items():
            if slot.max_capacity and slot.current_count < slot.max_capacity:
        ...
        ***REMOVED*** Step 4: Save if not dry run
        if not dry_run:
            ***REMOVED*** Clear existing assignments for this block
            self.assignment_repo.delete_block_assignments(block_number, academic_year)
            self.db.flush()

            ***REMOVED*** Create new assignments
            self.assignment_repo.bulk_create(assignment_records)
            self.db.commit()

        ***REMOVED*** Build rotation capacity response
        rotation_capacities = [
            RotationCapacity(
    ...
    def update_assignment(
        ...
        ***REMOVED*** Mark as manual if rotation is changed
