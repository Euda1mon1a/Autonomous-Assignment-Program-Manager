...
class SchedulingProblem(Problem):
    ...
    def __init__(
        ...
        ***REMOVED*** Decision variables: assignment matrix (n_blocks x n_persons)
        ***REMOVED*** Each variable is binary (0 or 1) indicating assignment
        n_vars = n_persons * n_blocks

        ***REMOVED*** Number of objectives
        n_obj = len(objectives)

        ***REMOVED*** Number of constraints
        ...
        super().__init__(
            ...
            xl=0.0,  ***REMOVED*** Lower bound
            xu=1.0,  ***REMOVED*** Upper bound
        )
    ...
    def _decode_solution(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        ***REMOVED*** Reshape into assignment matrix
        assignment_matrix = x.reshape(self.n_blocks, self.n_persons)

        ***REMOVED*** Apply threshold to create binary assignments
    ...
    def _calculate_fairness(self, assignment_matrix: np.ndarray) -> float:
        ...
        ***REMOVED*** Calculate coefficient of variation
    ...
    def _calculate_preference_satisfaction(self, assignment_matrix: np.ndarray) -> float:
        ...
        for block_idx in range(self.n_blocks):
            for person_idx in range(self.n_persons):
                if assignment_matrix[block_idx, person_idx] == 1:
                    total_assignments += 1
                    ***REMOVED*** Check if this person has preference for this block type
                    ***REMOVED*** (simplified - would use actual preference data)
                    person = self.person_data[person_idx]
                    block = self.block_data[block_idx]

                    ***REMOVED*** Example preference matching logic
        ...
        ***REMOVED*** Normalize by total assignments
        if total_assignments > 0:
            satisfaction_score /= total_assignments

        ***REMOVED*** Return negative for minimization
    ...
    def _calculate_consecutive_days(self, assignment_matrix: np.ndarray) -> float:
        ...
        ***REMOVED*** Check each person's assignments chronologically
        for person_idx in range(self.n_persons):
            ...
            for block_idx in range(self.n_blocks):
                if person_assignments[block_idx] == 1:
                    consecutive_count += 1
                    if consecutive_count > 5:  ***REMOVED*** Penalize more than 5 consecutive
    ...
    def _evaluate(self, x: np.ndarray, out: dict, *args, **kwargs):
        ...
        ***REMOVED*** Calculate objectives
        ...
        for obj in self.objectives_config:
            ...
            ***REMOVED*** Apply direction (pymoo minimizes by default)
        ...
        ***REMOVED*** Calculate constraint violations
...
class ParetoOptimizationService:
    ...
    def optimize_schedule_pareto(
        ...
        ***REMOVED*** Fetch persons and blocks
        ...
        ***REMOVED*** Configure NSGA-II algorithm
        algorithm = NSGA2(
        ...
        ***REMOVED*** Set up termination criteria
        ...
        ***REMOVED*** Find Pareto frontier indices
        frontier_indices = self._find_pareto_frontier(solutions)

        ***REMOVED*** Calculate hypervolume
    ...
    def rank_solutions(
        ...
        ***REMOVED*** Extract objective values
        objective_names = list(solutions[0].objective_values.keys())
        n_solutions = len(solutions)

        ***REMOVED*** Create matrix of objective values
        ...
        for i, sol in enumerate(solutions):
            ...
            ranked_solutions.append(
                RankedSolution(
                    ...
                    rank=0,  ***REMOVED*** Will be set after sorting
                    score=total_score,
    ...
    def _get_persons(self, person_ids: list[UUID] | None) -> list[Person]:
        ...
        return self.person_repo.list_all()[:50]  ***REMOVED*** Limit for performance
    ...
    def _get_blocks(self, block_ids: list[UUID] | None) -> list[Block]:
        ...
        return self.block_repo.list_all()[:100]  ***REMOVED*** Limit for performance
    ...
    def _extract_solutions(
        ...
        ***REMOVED*** Handle both single and multiple solutions
        ...
        for sol_idx in range(len(X)):
            ...
            ***REMOVED*** Decode assignment matrix
            assignment_matrix = (x.reshape(len(blocks), len(persons)) > 0.5).astype(int)

            ***REMOVED*** Create decision variables dict
            decision_vars = {}
            ...
            ***REMOVED*** Create objective values dict
            objective_values = {}
            for obj_idx, obj in enumerate(objectives):
                value = f[obj_idx]
                ***REMOVED*** Reverse negation for maximization objectives
                if obj.direction == ObjectiveDirection.MAXIMIZE:
                    value = -value
    ...
    def _find_pareto_frontier(self, solutions: list[ParetoSolution]) -> list[int]:
        ...
        ***REMOVED*** Check dominance
        for i in range(n_solutions):
            for j in range(n_solutions):
                if i == j:
                    continue

                ***REMOVED*** Check if j dominates i (all objectives better or equal, at least one strictly better)
                dominates = True
                strictly_better = False

                for k in range(len(objective_names)):
                    if obj_matrix[j, k] < obj_matrix[i, k]:  ***REMOVED*** Assuming minimization
        ...
        ***REMOVED*** Return non-dominated indices
    ...
    def _calculate_hypervolume(
        ...
        try:
            ***REMOVED*** Extract frontier solutions
            objective_names = list(solutions[0].objective_values.keys())
            frontier_points = []
            ...
            ***REMOVED*** Calculate hypervolume with reference point
            F = np.array(frontier_points)
            ref_point = np.max(F, axis=0) + 1.0  ***REMOVED*** Reference point slightly worse than worst
