...
class ConflictInfo:
    ...
    faculty_id: UUID
    faculty_name: str
    conflict_type: str  ***REMOVED*** leave_fmit_overlap, back_to_back, excessive_alternating
    fmit_week: date
    leave_id: Optional[UUID] = None
    severity: str = "warning"  ***REMOVED*** critical, warning, info
    description: str = ""
...
class ConflictAutoDetector:
    ...
    def create_conflict_alerts(
        ...
        """
        ***REMOVED*** TODO: Implement when ConflictAlert model is wired to __init__.py
        ***REMOVED*** This will create database records for each conflict
        alert_ids = []

        for conflict in conflicts:
            ***REMOVED*** Placeholder - actual implementation creates ConflictAlert records
            alert_id = uuid4()
    ...
    def _find_fmit_overlaps(
        ...
        """
        ***REMOVED*** This would query the schedule/assignment data to find FMIT weeks
        ***REMOVED*** For now, return empty list - actual implementation needs schedule source
        ***REMOVED*** TODO: Query Assignment records where rotation is FMIT type
        return []
