...
class ConflictInfo:
    ...
    faculty_id: UUID
    faculty_name: str
    conflict_type: str  ***REMOVED*** leave_fmit_overlap, back_to_back, double_booking, etc.
    fmit_week: date | None = None
    leave_id: UUID | None = None
    assignment_id: UUID | None = None
    residency_block_id: UUID | None = None
    severity: str = "medium"  ***REMOVED*** critical, high, medium, low
    description: str = ""
...
class ConflictAutoDetector:
    ...
    def detect_conflicts_for_absence(
        ...
        ***REMOVED*** OPTIMIZATION: Use joinedload to eagerly load person relationship
        absence = (
        ...
        ***REMOVED*** Check for FMIT overlap if absence is blocking
    ...
    def detect_all_conflicts(
        ...
        ***REMOVED*** 1. Check absence-based conflicts
        ***REMOVED*** OPTIMIZATION: Eagerly load person relationship to avoid N+1 queries
        query = (
            self.db.query(Absence)
            .options(joinedload(Absence.person))
            .filter(
        ...
        ***REMOVED*** Process absences inline to avoid additional queries
        ...
        ***REMOVED*** 2. Check for double-booking across systems
        conflicts.extend(
            self._detect_cross_system_double_booking(faculty_id, start_date, end_date)
        )

        ***REMOVED*** 3. Check ACGME compliance violations
        conflicts.extend(
            self._detect_acgme_violations(faculty_id, start_date, end_date)
        )

        ***REMOVED*** 4. Check supervision ratio violations
        conflicts.extend(
    ...
    def create_conflict_alerts(
        ...
        ***REMOVED*** Map new conflict types to existing enum values (for backward compatibility)
        conflict_type_map = {
            "leave_fmit_overlap": "LEAVE_FMIT_OVERLAP",
            "residency_fmit_double_booking": "LEAVE_FMIT_OVERLAP",  ***REMOVED*** Map to closest existing
            "back_to_back": "BACK_TO_BACK",
            "excessive_alternating": "EXCESSIVE_ALTERNATING",
            "work_hour_violation": "EXTERNAL_COMMITMENT",  ***REMOVED*** Map to generic type
            "rest_day_violation": "EXTERNAL_COMMITMENT",  ***REMOVED*** Map to generic type
            "supervision_ratio_violation": "EXTERNAL_COMMITMENT",  ***REMOVED*** Map to generic type
            "missing_supervision": "EXTERNAL_COMMITMENT",  ***REMOVED*** Map to generic type
            "call_cascade": "CALL_CASCADE",
            "external_commitment": "EXTERNAL_COMMITMENT",
        }

        ***REMOVED*** Map new severity levels to existing enum values
        severity_map = {
            "critical": "CRITICAL",
            "high": "WARNING",
            "medium": "WARNING",
            "low": "INFO",
            "warning": "WARNING",  ***REMOVED*** Backward compatibility
            "info": "INFO",  ***REMOVED*** Backward compatibility
        }

        for conflict in conflicts:
            ***REMOVED*** Map conflict type to existing enum
            conflict_type_key = conflict_type_map.get(conflict.conflict_type.lower())
            if not conflict_type_key:
                ***REMOVED*** Skip if type not mappable
                continue

            try:
                conflict_type_enum = ConflictType[conflict_type_key]
            except KeyError:
                ***REMOVED*** Skip if enum value doesn't exist
                continue

            ***REMOVED*** Map severity to existing enum
            severity_key = severity_map.get(conflict.severity.lower(), "WARNING")
            severity_enum = ConflictSeverity[severity_key]

            ***REMOVED*** Check if a similar alert already exists
            ...
            if existing:
                ***REMOVED*** Update existing alert if severity increased
    ...
    def _find_fmit_overlaps(
        ...
        ***REMOVED*** Query for FMIT assignments in the date range
        fmit_weeks = []

        ***REMOVED*** OPTIMIZATION: Use joinedload to eagerly load block relationship
        assignments = (
        ...
        ***REMOVED*** Group by week start dates
        seen_weeks = set()
        for assignment in assignments:
            block_date = assignment.block.date
            ***REMOVED*** Get week start (Monday)
    ...
    def _check_back_to_back(
        ...
        if has_back_to_back_conflict(sorted(fmit_weeks)):
            ***REMOVED*** Find which weeks are back-to-back
            conflicts = []
            sorted_weeks = sorted(fmit_weeks)
            for i in range(len(sorted_weeks) - 1):
                gap = (sorted_weeks[i+1] - sorted_weeks[i]).days
                if gap <= 14:  ***REMOVED*** Less than 2 week gap
                    conflicts.append(ConflictInfo(
                        faculty_id=faculty_id,
                        faculty_name="",  ***REMOVED*** Would be populated from query
    ...
    def _detect_cross_system_double_booking(
        ...
        ***REMOVED*** Query for people with assignments in date range
        query = (
        ...
        ***REMOVED*** Group assignments by person and block
        ...
        ***REMOVED*** Check for double-booking (person assigned multiple times to same block)
    ...
    def _detect_acgme_violations(
        ...
        ***REMOVED*** Query all residents (ACGME rules apply to residents)
        ***REMOVED*** OPTIMIZATION: Eagerly load assignments to reduce queries in violation checks
        query = (
    ...
    def _check_80_hour_violations(
        ...
        ***REMOVED*** Iterate through weeks in the date range
        current_date = start_date
        while current_date <= end_date:
            week_start = current_date - timedelta(days=current_date.weekday())
            week_end = week_start + timedelta(days=6)

            ***REMOVED*** Count work blocks in this week (each block = 4 hours)
            work_blocks = (
                self.db.query(Assignment)
                .join(Block)
                .filter(
            ...
            ***REMOVED*** Each block is 4 hours (half-day)
    ...
    def _check_1_in_7_violations(
        ...
        ***REMOVED*** Get all work days in the date range
        work_days_query = (
        ...
        ***REMOVED*** Check for consecutive work days > 6
        consecutive_days = 1
    ...
    def _detect_supervision_violations(
        ...
        ***REMOVED*** OPTIMIZATION: Load blocks with their assignments and related data in one query
        blocks_query = (
            self.db.query(Block)
            .options(
        ...
        for block in blocks:
            ***REMOVED*** Get all assignments for this block (already loaded)
            assignments = block.assignments

            if not assignments:
                continue

            ***REMOVED*** Count residents by PGY level and faculty
            pgy1_count = 0
            pgy2_3_count = 0
            faculty_count = 0
            rotation_name = None
            ...
            ***REMOVED*** Check ratios
            if pgy1_count > 0 or pgy2_3_count > 0:
                ***REMOVED*** Check if supervision required (skip if no residents or already has faculty)
                if faculty_count == 0:
                    conflicts.append(ConflictInfo(
    ...
    def _severity_priority(severity: str) -> int:
        ...
        priorities = {
            ...
            "high": 3,
            "critical": 4,
            "warning": 2,  ***REMOVED*** Backward compatibility
            "info": 1,  ***REMOVED*** Backward compatibility
        }
