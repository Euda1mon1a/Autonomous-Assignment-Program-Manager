...
class AgentMatcher:
    ...
    Example:
        ...
        best_agent = matches[0]  ***REMOVED*** Highest similarity match
    """

    ***REMOVED*** Default model tiers based on archetype complexity
    MODEL_TIERS = {
        "Researcher": "sonnet",
        "Validator": "haiku",
        "Generator": "sonnet",
    ...
    def _find_agents_dir(self) -> Path:
        ...
        ***REMOVED*** Try common locations
        ...
        for candidate in candidates:
            if candidate.exists():
                return candidate
        return candidates[0]  ***REMOVED*** Default to first option
    ...
    def load_agents(self) -> None:
        ...
        for agent_file in agent_files:
            if agent_file.name in ("AGENT_FACTORY.md", "README.md"):
                continue  ***REMOVED*** Skip meta-documents
    ...
    def _parse_agent_spec(self, file_path: Path) -> AgentSpec | None:
        ...
        name = file_path.stem  ***REMOVED*** Filename without extension

        ***REMOVED*** Extract archetype from header
        archetype = "Generator"  ***REMOVED*** Default
        for line in content.split("\n"):
            if "Archetype:" in line:
                archetype = line.split(":")[-1].strip().strip("*")
                break

        ***REMOVED*** Extract authority level
        authority_level = "Propose-Only"  ***REMOVED*** Default
        for line in content.split("\n"):
            if "Authority Level:" in line:
                authority_level = line.split(":")[-1].strip().strip("*")
                break

        ***REMOVED*** Extract capabilities from charter and responsibilities sections
        capabilities_parts = []
        ...
        ***REMOVED*** Get primary responsibilities
        ...
        ***REMOVED*** Generate embedding for capabilities
    ...
    def match_task(
        ...
        ***REMOVED*** Get task embedding (cached for repeated queries)
        task_embedding = np.array(
            get_cached_embedding(task_description), dtype=np.float32
        )

        ***REMOVED*** Calculate cosine similarity with all agents
        matches = []
        ...
        ***REMOVED*** Sort by similarity (descending)
...
***REMOVED*** Module-level singleton for convenience
