...
class UnifiedHeatmapService:
    ...
    def generate_coverage_heatmap(
        ...
        ***REMOVED*** Filter rotations based on inclusion flags
        ...
        ***REMOVED*** Build coverage map: (rotation_id, date) -> count of people assigned
        coverage_map: dict[tuple[UUID, date], int] = defaultdict(int)

        ***REMOVED*** Get all assignments in range
        assignments = (
            db.query(Assignment)
        ...
        ***REMOVED*** Count assignments per rotation per date
        for assignment in assignments:
            if assignment.rotation_template_id:
                key = (assignment.rotation_template_id, assignment.block.date)
                coverage_map[key] += 1

        ***REMOVED*** Build z_values matrix (coverage count per day per rotation)
        z_values = []
        total_assignments = 0
        max_coverage = 0
        ...
        ***REMOVED*** Include FMIT swap metadata if requested
        ...
        if include_fmit:
            ***REMOVED*** Get swap records in range
            swaps = (
                db.query(SwapRecord)
                .filter(
    ...
    def generate_person_coverage_heatmap(
        ...
        ***REMOVED*** Build assignment map: (person_id, date) -> count
        assignment_map: dict[tuple[UUID, date], int] = defaultdict(int)

        ***REMOVED*** Get regular assignments
        assignments = (
            db.query(Assignment)
            .join(Block, Assignment.block_id == Block.id)
            .filter(
        ...
        ***REMOVED*** Add call assignments if requested
        if include_call:
            call_assignments = (
            ...
            for call in call_assignments:
                ...
                ***REMOVED*** Add extra weight for call assignments
    ...
    def generate_weekly_fmit_heatmap(
        ...
        ***REMOVED*** Build assignment map: (faculty_id, week_start) -> has_assignment (1 or 0)
        assignment_map: dict[tuple[UUID, date], int] = defaultdict(int)

        ***REMOVED*** Get all FMIT assignments in range
        assignments = (
            db.query(Assignment)
            .join(Block, Assignment.block_id == Block.id)
            .filter(
        ...
        ***REMOVED*** Group by faculty and week
        for assignment in assignments:
            week_start = self._get_week_start(assignment.block.date)
            key = (assignment.person_id, week_start)
            assignment_map[key] = 1  ***REMOVED*** Binary: has assignment or not
