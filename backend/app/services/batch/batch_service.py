...
***REMOVED*** In-memory storage for batch operation status (in production, use Redis or database)
...
class BatchService:
    ...
    async def create_batch(
        ...
        ***REMOVED*** Store initial status
        ...
        try:
            ...
            ***REMOVED*** If dry run, return validation results without processing
            if request.dry_run:
                _batch_operations[operation_id][
                    "status"
            ...
            ***REMOVED*** Calculate success/failure counts
    ...
    def get_batch_status(self, operation_id: UUID) -> BatchStatusResponse | None:
        ...
        ***REMOVED*** Calculate progress percentage
        total = operation["total"]
        completed = operation["succeeded"] + operation["failed"]
        progress_percentage = (completed / total * 100) if total > 0 else 0

        ***REMOVED*** Estimate completion time for in-progress operations
