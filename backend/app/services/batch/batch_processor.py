...
class BatchProcessor:
    ...
    async def process_create_batch(
        ...
        try:
            ***REMOVED*** Performance: Pre-build all assignment dictionaries before database operations
            assignment_dicts = []
            ...
            ***REMOVED*** Process assignments with batched operations
            for idx, assignment_dict in enumerate(assignment_dicts):
                try:
                    ...
                    if rollback_on_error:
                        ***REMOVED*** Rollback and stop processing
                        self.db.rollback()
                        ***REMOVED*** Mark remaining as failed
            ...
            ***REMOVED*** Performance: Single commit for all successful operations
            if all(r.success for r in results) or not rollback_on_error:
                self.db.commit()
                ***REMOVED*** Performance: Batch refresh using db.expire_all() instead of individual refreshes
                if created_assignments:
                    self.db.expire_all()
                    ***REMOVED*** Only refresh if we actually need the data
    ...
    async def process_update_batch(
        ...
        try:
            for idx, assignment_data in enumerate(assignments):
                try:
                    ***REMOVED*** Get existing assignment
                    ...
                    ***REMOVED*** Check optimistic locking
            ...
            ***REMOVED*** Commit if all succeeded or if not rolling back on error
            if all(r.success for r in results) or not rollback_on_error:
                self.db.commit()
                ***REMOVED*** Refresh all updated assignments
                ...
                try:
                    ...
                    ***REMOVED*** Delete assignment
                    if assignment_data.soft_delete:
                        ***REMOVED*** Soft delete: mark as deleted without removing
                        ***REMOVED*** For now, we'll add a note indicating soft delete
                        existing.notes = (existing.notes or "") + "\n[SOFT DELETED]"
                        self.db.commit()
                    else:
                        ***REMOVED*** Hard delete: remove from database
