...
class FacultyPreferenceService:
    ...
    def _get_cache(self):
        ...
        if self._cache is None:
            try:
                ...
                self._cache = False  ***REMOVED*** Mark as unavailable
    ...
    def update_preferences(
        ...
        ***REMOVED*** Invalidate cache after update
    ...
    def add_preferred_week(self, faculty_id: UUID, week_date: date) -> FacultyPreference:
        ...
        if week_str not in preferences.preferred_weeks:
            ***REMOVED*** Remove from blocked if present
    ...
    def add_blocked_week(self, faculty_id: UUID, week_date: date) -> FacultyPreference:
        ...
        if week_str not in preferences.blocked_weeks:
            ***REMOVED*** Remove from preferred if present
    ...
    ***REMOVED*** ===== SWAP AUTO-MATCHING LOGIC =====
    ...
    def find_best_matches(
        ...
        """
        ***REMOVED*** Get the original swap request
        ...
        ***REMOVED*** Find potential matching swaps (pending swaps where someone wants the source week)
        potential_matches = self.db.query(SwapRecord).filter(
            and_(
                SwapRecord.id != swap_request_id,
                SwapRecord.status == SwapStatus.PENDING,
                or_(
                    ***REMOVED*** They want our week as their target
                    SwapRecord.target_week == swap_request.source_week,
                    ***REMOVED*** Or we want their source week as our target
        ...
        for candidate in potential_matches:
            ...
            if score > 0:  ***REMOVED*** Only include viable matches
    ...
    def auto_suggest_swaps(
        ...
        ***REMOVED*** Get person's current assignments
        ...
        for assignment, block in current_assignments:
            ...
            ***REMOVED*** Check if this week is blocked by the person
            if self.is_week_blocked(person_id, current_week):
                ***REMOVED*** Find someone who wants this week
                ...
                for partner_id in interested_faculty:
                    ***REMOVED*** Find their assignments on preferred weeks
                    ...
                    for partner_assignment, partner_block in partner_assignments:
                        ...
                        ***REMOVED*** Check if we prefer their week and they don't block it
    ...
    def learn_from_swap_history(
        ...
        ***REMOVED*** Get swap history where this person was involved
        ...
        if not swaps:
            return {
                "acceptance_rate": 0.5,  ***REMOVED*** Neutral default
        ...
        ***REMOVED*** Analyze week patterns
        ...
        for swap in swaps:
            if swap.source_faculty_id == faculty_id:
                ***REMOVED*** They initiated - they want target_week, avoid source_week
                if swap.target_week:
                ...
                ***REMOVED*** They were target - they want source_week, would give up target_week
        ...
        ***REMOVED*** Sort and get top items
    ...
    ***REMOVED*** ===== PRIVATE HELPER METHODS =====
    ...
    def _score_preference_alignment(
        ...
        ***REMOVED*** Check if A wants what B has, and B wants what A has
        a_wants_b_source = (
        ...
        ***REMOVED*** Perfect mutual alignment
        if a_wants_b_source and b_wants_a_source:
            score = 1.0
        ***REMOVED*** One-way alignment
        elif a_wants_b_source or b_wants_a_source:
            score = 0.6
        else:
            ***REMOVED*** Check preference lists
    ...
    def _score_blocking_constraints(
        ...
        ***REMOVED*** Check if either party has blocked the week they would receive
        ...
        if a_blocks_b_week or b_blocks_a_week:
            return 0.0  ***REMOVED*** Incompatible

        return 1.0  ***REMOVED*** No blocking issues
    ...
    def _score_historical_acceptance(
        ...
        ***REMOVED*** Average of both acceptance rates
    ...
    def _score_workload_balance(
        ...
        """
        ***REMOVED*** Get preferences for both parties
        a_prefs = self.get_or_create_preferences(request_a.source_faculty_id)
        b_prefs = self.get_or_create_preferences(request_b.source_faculty_id)

        ***REMOVED*** Count current assignments for each person
        ...
        ***REMOVED*** Get target weeks per year
        a_target = a_prefs.target_weeks_per_year or 6
        b_target = b_prefs.target_weeks_per_year or 6

        ***REMOVED*** Calculate how far each is from their target
        a_delta = abs(a_count - a_target)
        b_delta = abs(b_count - b_target)

        ***REMOVED*** Swap is better if it helps both get closer to their targets
        ***REMOVED*** For simplicity, if both are close to their targets, score higher
        max_delta = max(a_delta, b_delta, 1)  ***REMOVED*** Avoid division by zero

        return 1.0 - (max_delta / 52.0)  ***REMOVED*** Normalize assuming max 52 weeks
    ...
    def _generate_match_reason(
        ...
        ***REMOVED*** Check mutual preference
        ...
        ***REMOVED*** Check historical compatibility
        a_history = self.learn_from_swap_history(request_a.source_faculty_id)
    ...
    def _calculate_swap_benefit(
        ...
        ***REMOVED*** Check if current week is blocked
        if self.is_week_blocked(person_id, current_week):
            score += 0.4

        ***REMOVED*** Check if proposed week is preferred
        if self.is_week_preferred(person_id, proposed_week):
            score += 0.4

        ***REMOVED*** Check historical patterns
