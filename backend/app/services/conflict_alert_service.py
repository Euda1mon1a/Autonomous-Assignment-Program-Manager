...
class ConflictAlertService:
    ...
    ***REMOVED*** ==================== PRIVATE HELPER METHODS ====================
    ...
    def _find_available_faculty_for_week(
        ...
        """
        ***REMOVED*** Get all faculty members
        all_faculty = self.db.query(Person).filter(
            Person.type == "faculty",
            Person.id != exclude_faculty_id,
        ).all()

        ***REMOVED*** Filter out those with existing conflicts or assignments that week
        available = []
        week_end = fmit_week + timedelta(days=6)

        for faculty in all_faculty:
            ***REMOVED*** Check for existing alerts in that week
            ...
            if not has_conflicts:
                ***REMOVED*** Check for existing FMIT assignments that week
