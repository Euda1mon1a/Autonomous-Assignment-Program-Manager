...
class BatchProcessor:
    ...
    async def process(
        ...
        ***REMOVED*** Create batches
        batches = [
            items[i : i + self.batch_size]
            for i in range(0, total_items, self.batch_size)
        ]

        ***REMOVED*** Process batches with concurrency limit
...
class StreamingBatchProcessor:
    ...
    async def stop(self):
        ...
        ***REMOVED*** Flush remaining items
        if self.current_batch:
            await self._flush_batch()

        ***REMOVED*** Wait for worker to finish
    ...
    async def _worker(self):
        ...
        while self._running:
            try:
                ...
                ***REMOVED*** Check if batch is full or flush interval reached
                now = datetime.utcnow()
