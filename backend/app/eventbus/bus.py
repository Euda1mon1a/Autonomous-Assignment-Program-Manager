...
***REMOVED*** =============================================================================
***REMOVED*** Event Bus Configuration
***REMOVED*** =============================================================================
...
class EventBusMode(str, Enum):
    ...
    IN_MEMORY = "in_memory"  ***REMOVED*** Local events only (single process)
    DISTRIBUTED = "distributed"  ***REMOVED*** Redis pub/sub (multi-process)
    HYBRID = "hybrid"  ***REMOVED*** Both in-memory and distributed
...
***REMOVED*** =============================================================================
***REMOVED*** Event Filtering and Transformation
***REMOVED*** =============================================================================
...
class EventSubscription(BaseModel):
    ...
    topic_pattern: str
    handler: Any  ***REMOVED*** EventHandler (can't serialize Callable)
    ...
    def matches_topic(self, topic: str) -> bool:
        ...
        ***REMOVED*** Convert glob pattern to regex
        if "*" in self.topic_pattern:
            ***REMOVED*** Replace * with appropriate regex
            pattern = self.topic_pattern.replace(".", r"\.")
            pattern = pattern.replace("**", ".*")  ***REMOVED*** Multi-level wildcard
            pattern = pattern.replace("*", r"[^.]+")  ***REMOVED*** Single-level wildcard
            pattern = f"^{pattern}$"
            return bool(re.match(pattern, topic))

        ***REMOVED*** Try as direct regex
        try:
...
class DeadLetterQueue:
    ...
    async def get_all(self) -> list[DeadLetterEvent]:
        ...
        ***REMOVED*** Get from Redis if available
        if self._redis_client:
            try:
    ...
    async def remove(self, event_id: str) -> bool:
        ...
        ***REMOVED*** Remove from Redis if available
        if self._redis_client and removed:
            try:
                ***REMOVED*** This is inefficient but Redis lists don't support removal by value easily
...
class EventStore:
    ...
    async def store(self, event: Event) -> bool:
        ...
        """
        try:
            ***REMOVED*** Store event data
            event_key = f"{self._store_key_prefix}:{event.metadata.event_id}"
            event_data = event.to_json()
            await self._redis.setex(event_key, 30 * 24 * 3600, event_data)  ***REMOVED*** 30 days TTL

            ***REMOVED*** Add to topic index
            ...
            ***REMOVED*** Add to global time index
...
class EventBus:
    ...
    Example:
        ***REMOVED*** Create distributed event bus
        bus = EventBus(mode=EventBusMode.DISTRIBUTED, enable_persistence=True)
        await bus.start()

        ***REMOVED*** Subscribe to events with wildcard
    ...
    async def start(self) -> None:
        ...
        ***REMOVED*** Initialize Redis for distributed or hybrid mode
        if self._mode in (EventBusMode.DISTRIBUTED, EventBusMode.HYBRID):
            await self._init_redis()

        ***REMOVED*** Initialize event store if persistence is enabled
        ...
        ***REMOVED*** Initialize dead letter queue
        ...
        ***REMOVED*** Start pub/sub listener for distributed mode
        if self._mode in (EventBusMode.DISTRIBUTED, EventBusMode.HYBRID):
    ...
    async def stop(self) -> None:
        ...
        ***REMOVED*** Close Redis connections
    ...
    async def subscribe(
        ...
        Example:
            ...
            ***REMOVED*** Wildcard match
            await bus.subscribe("user.*", handler)

            ***REMOVED*** Multi-level wildcard
            await bus.subscribe("user.**", handler)
        ...
        async with self._subscription_lock:
            ...
            ***REMOVED*** Add to subscriptions
            self._subscriptions[topic_pattern].append(subscription)

            ***REMOVED*** Subscribe to Redis channel if distributed
            if self._mode in (EventBusMode.DISTRIBUTED, EventBusMode.HYBRID):
                if self._redis_pubsub:
                    ***REMOVED*** Subscribe to actual topics (we'll filter with pattern matching)
            ...
            for topic_pattern, subscriptions in self._subscriptions.items():
                for sub in subscriptions:
                    if sub.subscription_id == subscription_id:
                        ...
                        ***REMOVED*** Clean up empty pattern lists
    ...
    async def publish(
        ...
        try:
            ***REMOVED*** Persist event if enabled
            ...
            ***REMOVED*** Publish to in-memory subscribers (in-memory and hybrid modes)
            if self._mode in (EventBusMode.IN_MEMORY, EventBusMode.HYBRID):
                await self._publish_in_memory(event)

            ***REMOVED*** Publish to Redis (distributed and hybrid modes)
            if self._mode in (EventBusMode.DISTRIBUTED, EventBusMode.HYBRID):
    ...
    async def replay(
        ...
        Example:
            ***REMOVED*** Replay all events from last hour
            one_hour_ago = datetime.utcnow() - timedelta(hours=1)
            events = await bus.replay(from_timestamp=one_hour_ago)

            ***REMOVED*** Replay specific topic
    ...
    async def replay_and_publish(
        ...
        published_count = 0
        for event in events:
            ***REMOVED*** Don't persist replayed events again
        ...
        for dle in dead_letters:
            if dle.event_id == event_id:
                ...
                ***REMOVED*** Republish
                ...
                if success:
                    ***REMOVED*** Remove from dead letter queue
    ...
    ***REMOVED*** Private methods
        ...
        try:
            async for message in self._redis_pubsub.listen():
                if message["type"] in ("message", "pmessage"):
                    try:
                        ***REMOVED*** Decode message data
                        data = message["data"]
                        if isinstance(data, bytes):
                            data = data.decode()

                        ***REMOVED*** Deserialize event
                        event = Event.from_json(data)

                        ***REMOVED*** Process in-memory
                        await self._publish_in_memory(event)
    ...
    async def _publish_in_memory(self, event: Event) -> None:
        ...
        ***REMOVED*** Process all matching subscriptions concurrently
        tasks = [
    ...
    async def _handle_subscription(
        ...
        """
        ***REMOVED*** Apply filter if present
        ...
        ***REMOVED*** All retries failed - add to dead letter queue
        logger.error(
...
***REMOVED*** =============================================================================
***REMOVED*** Global Event Bus Instance
***REMOVED*** =============================================================================
