...
***REMOVED******REMOVED******REMOVED*** Token Family Concept
...
***REMOVED******REMOVED******REMOVED*** Creating a Refresh Token
...
***REMOVED*** Get service instance
service = get_refresh_token_service(db)

***REMOVED*** Create device fingerprint from request
...
***REMOVED*** Return token_string to client
return {"refresh_token": token_string}
```

***REMOVED******REMOVED******REMOVED*** Validating and Rotating a Token

```python
***REMOVED*** Validate and rotate token
device_fp = DeviceFingerprint(
    ip_address=request.client.host,
    user_agent=request.headers.get("user-agent"),
)
...
if new_token_string:
    ***REMOVED*** Token was rotated - return new token
    return {
        "access_token": create_access_token(new_token_data),
        "refresh_token": new_token_string,
    }
elif new_token_data:
    ***REMOVED*** Token is valid but not rotated
    return {"access_token": create_access_token(new_token_data)}
else:
    ***REMOVED*** Token is invalid
...
***REMOVED*** Revoke all user tokens except current (logout other devices)
await service.revoke_user_tokens(user_id, except_token_id=current_token_id)

***REMOVED*** Revoke token family (reuse detected)
...
service = RefreshTokenService(
    db=db,
    max_tokens_per_user=5,              ***REMOVED*** Max concurrent tokens
    default_expiry_days=30,             ***REMOVED*** Default token lifetime
    rotation_strategy=TokenRotationStrategy.ALWAYS,  ***REMOVED*** Rotation strategy
    rotation_threshold=0.5,             ***REMOVED*** For THRESHOLD strategy
    enable_device_binding=True,         ***REMOVED*** Enable device fingerprinting
    enable_reuse_detection=True,        ***REMOVED*** Enable reuse detection
)
```

***REMOVED******REMOVED******REMOVED*** Rotation Strategies
...
***REMOVED******REMOVED******REMOVED*** Token Reuse Attack
...
***REMOVED******REMOVED******REMOVED*** Device Binding
...
***REMOVED******REMOVED******REMOVED*** Storage Backend
...
***REMOVED******REMOVED*** Integration with Access Tokens
...
2. **Configure secure defaults**:
   ```python
   max_tokens_per_user=3  ***REMOVED*** Limit concurrent sessions
   default_expiry_days=30  ***REMOVED*** Balance security vs UX
   rotation_strategy=TokenRotationStrategy.ALWAYS  ***REMOVED*** Maximum security
   enable_device_binding=True  ***REMOVED*** Prevent token theft
   enable_reuse_detection=True  ***REMOVED*** Detect attacks
   ```
...
4. **Periodic cleanup**:
   ```python
   ***REMOVED*** Run as Celery task every hour
   @celery_app.task
