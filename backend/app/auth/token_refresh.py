...
***REMOVED*** Import observability metrics (graceful degradation)
try:
...
class RefreshTokenStatus(str, Enum):
    ...
    ACTIVE = "active"
    USED = "used"  ***REMOVED*** Token has been refreshed (child token created)
    REVOKED = "revoked"  ***REMOVED*** Manually revoked
    EXPIRED = "expired"  ***REMOVED*** Time-based expiration
    FAMILY_REVOKED = "family_revoked"  ***REMOVED*** Entire family revoked due to reuse


class TokenRotationStrategy(str, Enum):
    """Token rotation strategy enumeration."""

    ALWAYS = "always"  ***REMOVED*** Always issue new refresh token (most secure)
    THRESHOLD = "threshold"  ***REMOVED*** Issue new token if > X% of lifetime used
    NEVER = "never"  ***REMOVED*** Reuse same refresh token (least secure)
...
class DeviceFingerprint(BaseModel):
    ...
    device_id: str | None = None  ***REMOVED*** Optional client-provided device ID
...
class RefreshTokenData(BaseModel):
    ...
    ***REMOVED*** Token lifecycle
    created_at: datetime = Field(default_factory=datetime.utcnow)
    expires_at: datetime
    last_used_at: datetime | None = None
    status: RefreshTokenStatus = RefreshTokenStatus.ACTIVE

    ***REMOVED*** Token family tree (for reuse detection)
    parent_token_id: str | None = None  ***REMOVED*** Previous token in family
    child_token_id: str | None = None  ***REMOVED*** Next token in family (after refresh)

    ***REMOVED*** Device binding
    ...
    ***REMOVED*** Metadata
    session_id: str | None = None  ***REMOVED*** Link to session if using session management
...
class RefreshTokenCreate(BaseModel):
    ...
    user_id: str
    username: str
    family_id: str | None = None  ***REMOVED*** Reuse family ID for rotation
...
class RefreshTokenResponse(BaseModel):
    ...
    refresh_token: str  ***REMOVED*** Encrypted token string
    token_id: str
    expires_at: datetime
...
class RefreshTokenService:
    ...
    def __init__(
        ...
        rotation_threshold: float = 0.5,  ***REMOVED*** Rotate if >50% of lifetime used
        ...
        ***REMOVED*** In-memory storage for development (replace with Redis in production)
        ***REMOVED*** Format: {token_id: RefreshTokenData}
        self._token_storage: dict[str, RefreshTokenData] = {}

        ***REMOVED*** Token family index for fast lookup
        ***REMOVED*** Format: {family_id: [token_id1, token_id2, ...]}
        self._family_index: dict[str, list[str]] = {}

        ***REMOVED*** User token index
        ***REMOVED*** Format: {user_id: [token_id1, token_id2, ...]}
    ...
    def _unindex_token(self, token_data: RefreshTokenData) -> None:
        ...
        """
        ***REMOVED*** Remove from family index
        ...
        ***REMOVED*** Remove from user index
    ...
    async def create_refresh_token(
        ...
        """
        ***REMOVED*** Check concurrent token limit
        ...
        if len(active_tokens) >= self.max_tokens_per_user:
            ***REMOVED*** Revoke oldest token to make room
        ...
        ***REMOVED*** Generate token identifiers
        ...
        ***REMOVED*** Generate device fingerprint hash
        device_fp_hash = None
        device_info = {}
        ...
        ***REMOVED*** Store token (map token_string -> token_data)
        ***REMOVED*** In production, use Redis with token_string as key
        self._token_storage[token_string] = token_data
        self._index_token(token_data)

        ***REMOVED*** Update parent token to link child
    ...
    async def validate_and_rotate_token(
        ...
        ***REMOVED*** Determine if rotation is needed
        ...
        if should_rotate:
            ***REMOVED*** Mark current token as USED
            token_data.status = RefreshTokenStatus.USED

            ***REMOVED*** Create new token in same family (rotation)
            new_request = RefreshTokenCreate(
                user_id=token_data.user_id,
                username=token_data.username,
                family_id=token_data.family_id,  ***REMOVED*** Same family
                parent_token_id=token_data.token_id,  ***REMOVED*** Link to parent
        ...
        ***REMOVED*** No rotation - return existing token
    ...
    async def revoke_token(
        ...
        ***REMOVED*** Add to database blacklist
...
***REMOVED*** Global service instance (singleton)
...
def get_refresh_token_service(db: Session | None = None) -> RefreshTokenService:
    ...
    if _refresh_token_service is None:
        _refresh_token_service = RefreshTokenService(
            db=db,
            max_tokens_per_user=settings.RATE_LIMIT_LOGIN_ATTEMPTS,  ***REMOVED*** Reuse config
