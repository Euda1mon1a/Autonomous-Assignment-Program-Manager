"""
DEPRECATED: archived permission system (PermissionResolver).

Use `app.auth` (AccessControlMatrix) for active authorization.
...
from app.auth._archived_permissions.cache import (
    PermissionCache,
    close_permission_cache,
    get_permission_cache,
)
from app.auth._archived_permissions.decorators import (
    PermissionDenied,
    RequireAnalytics,
...
from app.auth._archived_permissions.resolver import (
    PermissionResolver,
    get_permission_resolver,
)

warnings.warn(
    "app.auth._archived_permissions is deprecated; use app.auth instead.",
    DeprecationWarning,
    stacklevel=2,
)
