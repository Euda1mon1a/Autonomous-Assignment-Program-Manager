...
class SessionMiddleware(BaseHTTPMiddleware):
    ...
    def _get_session_id(self, request: Request) -> str | None:
        ...
        """
        ***REMOVED*** Check cookie first
        session_id = request.cookies.get(self.session_cookie_name)
        if session_id:
            return session_id

        ***REMOVED*** Check header
        session_id = request.headers.get(self.session_header_name)
        if session_id:
            return session_id

        ***REMOVED*** Check query param (for websockets)
    ...
    def _get_client_ip(self, request: Request) -> str | None:
        ...
        Returns:
            Client IP address
        """
        ***REMOVED*** Check proxy headers
        forwarded_for = request.headers.get("x-forwarded-for")
        if forwarded_for:
            ***REMOVED*** Take first IP (client)
        ...
        ***REMOVED*** Fall back to direct connection
        if request.client:
    ...
    async def dispatch(
        ...
        """
        ***REMOVED*** Skip exempt paths
        if self._is_path_exempt(request.url.path):
            return await call_next(request)

        ***REMOVED*** Get session ID
        session_id = self._get_session_id(request)

        ***REMOVED*** Validate and update session
        ...
        if session_id:
            ...
            if session:
                ***REMOVED*** Attach session to request state for route handlers
        ...
        ***REMOVED*** Optionally set session cookie if not present
        ***REMOVED*** (This would be done by login endpoint, but can refresh here)
        if session and session_id:
            ***REMOVED*** Refresh cookie expiration
            response.set_cookie(
