...
class RedisSessionStorage:
    ...
    async def create(
        ...
        try:
            ...
            ***REMOVED*** Calculate TTL
            if ttl_seconds is None and session.expires_at:
                ttl_seconds = int(
                    (session.expires_at - datetime.utcnow()).total_seconds()
                )
            ttl_seconds = ttl_seconds or 86400  ***REMOVED*** Default 24 hours

            ***REMOVED*** Serialize session data
            session_json = session.model_dump_json()

            ***REMOVED*** Use pipeline for atomic operations
            async with redis_client.pipeline() as pipe:
                ***REMOVED*** Store session data
                await pipe.setex(session_key, ttl_seconds, session_json)

                ***REMOVED*** Add to user's session index
                await pipe.sadd(user_index_key, session.session_id)

                ***REMOVED*** Set TTL on user index (slightly longer than max session TTL)
    ...
    async def get(self, session_id: str) -> SessionData | None:
        ...
        try:
            ...
            ***REMOVED*** Deserialize session data
    ...
    async def update(
        ...
        try:
            ...
            ***REMOVED*** Check if session exists
    ...
    async def delete(self, session_id: str) -> bool:
        ...
        try:
            ...
            ***REMOVED*** Get session to find user_id for index cleanup
            ...
            if session:
                ...
                ***REMOVED*** Session not found, just try to delete the key
    ...
    async def get_user_sessions(self, user_id: str) -> list[SessionData]:
        ...
        try:
            ...
            ***REMOVED*** Get all session IDs for this user
    ...
    async def cleanup_expired(self) -> int:
        ...
        Returns:
            Number of sessions removed
        """
        ***REMOVED*** Redis handles TTL automatically, so we don't need to do much here
        ***REMOVED*** This is mainly for sessions that were manually marked as expired
    ...
    async def get_session_count(self) -> int:
        ...
        try:
            ...
            ***REMOVED*** Use SCAN for better performance with large datasets
            cursor = 0
            while True:
...
***REMOVED*** Global storage instance
