...
class PermissionResolver:
    ...
    async def get_role_permissions(
        ...
        ***REMOVED*** Compute permissions with hierarchy
        all_permissions = set()

        ***REMOVED*** Get all roles in hierarchy
        hierarchy = self._resolve_role_hierarchy(role)

        ***REMOVED*** Accumulate permissions from all roles
    ...
    async def get_user_permissions(
        ...
        ***REMOVED*** Get permissions from role
        try:
    ...
    async def _check_resource_level_permission(
        ...
        """
        ***REMOVED*** Admin always has access
        if user.is_admin:
            return True

        ***REMOVED*** Resource-specific checks
        if resource_type == ResourceType.USER.value:
            ***REMOVED*** Users can only modify their own account unless they're admin
        ...
        if resource_type == ResourceType.SWAP.value:
            ***REMOVED*** Users can create swaps for their own assignments
            ***REMOVED*** Coordinators can approve any swap
            if action == "request_swap":
                return True  ***REMOVED*** Can request swaps for own assignments
            if action == "approve_swap":
                return user.can_manage_schedules

        if resource_type == ResourceType.ABSENCE.value:
            ***REMOVED*** Users can request leave for themselves
            ***REMOVED*** Coordinators can approve leave requests
            if action == "request_leave":
                return True
            if action == "approve_leave":
                return user.can_manage_schedules

        ***REMOVED*** Default: allow if base permission exists
        return True
    ...
    async def get_permissions_summary(self, user: User) -> dict[str, Any]:
        ...
        ***REMOVED*** Group permissions by resource type
...
***REMOVED*** Global resolver instance (singleton pattern)
