...
class PermissionCache:
    ...
    ***REMOVED*** Cache key prefixes
    ROLE_PERMISSIONS_PREFIX = "perm:role"
    USER_PERMISSIONS_PREFIX = "perm:user"
    RESOURCE_PERMISSIONS_PREFIX = "perm:resource"

    ***REMOVED*** Cache TTL (in seconds)
    DEFAULT_TTL = 3600  ***REMOVED*** 1 hour
    ROLE_TTL = 86400  ***REMOVED*** 24 hours (roles change infrequently)
    USER_TTL = 3600  ***REMOVED*** 1 hour (user roles can change)
    ...
    async def invalidate_all_permissions(self) -> bool:
        ...
        try:
            ***REMOVED*** Delete all keys matching permission patterns
            patterns = [
...
***REMOVED*** Global cache instance (singleton pattern)
...
async def get_permission_cache() -> PermissionCache:
    ...
    if _cache_instance is None:
        ...
        ***REMOVED*** Ensure Redis connection is established
