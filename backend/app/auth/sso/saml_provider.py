...
class SAMLProvider:
    ...
    ***REMOVED*** SAML namespaces
    SAML_NS = "urn:oasis:names:tc:SAML:2.0:assertion"
    SAMLP_NS = "urn:oasis:names:tc:SAML:2.0:protocol"
    DS_NS = "http://www.w3.org/2000/09/xmldsig***REMOVED***"
    ...
    def generate_authn_request(self) -> tuple[str, str]:
        ...
        ***REMOVED*** Build AuthnRequest XML
...
<samlp:AuthnRequest
    ...
    <samlp:NameIDPolicy
        ...
        ***REMOVED*** Encode request for HTTP-Redirect binding
        encoded_request = self._encode_saml_request(authn_request)

        ***REMOVED*** Build redirect URL
    ...
    def parse_saml_response(
        ...
        ***REMOVED*** Validate response status
        self._validate_response_status(root)

        ***REMOVED*** Extract assertion
        assertion = self._extract_assertion(root)

        ***REMOVED*** Validate signature if required
        ...
        ***REMOVED*** Validate conditions (NotBefore, NotOnOrAfter, AudienceRestriction)
        self._validate_conditions(assertion)

        ***REMOVED*** Extract and map attributes
        attributes = self._extract_attributes(assertion)

        ***REMOVED*** Extract NameID
        name_id = self._extract_name_id(assertion)

        ***REMOVED*** Extract session info
...
<md:EntityDescriptor
    ...
    def parse_idp_metadata(self, metadata_xml: str) -> dict[str, str]:
        ...
        ***REMOVED*** Define namespaces
        ns = {
            "md": "urn:oasis:names:tc:SAML:2.0:metadata",
            "ds": self.DS_NS,
        }

        ***REMOVED*** Extract entity ID
        entity_id = root.get("entityID")
        if not entity_id:
            raise ValueError("Missing entityID in IdP metadata")

        ***REMOVED*** Extract SSO URL
        ...
        ***REMOVED*** Extract X.509 certificate
    ...
    def _validate_signature(self, response: ET.Element, assertion: ET.Element) -> None:
        ...
        """
        ***REMOVED*** Check if signature exists on response or assertion
        ns = {"ds": self.DS_NS}
        ...
        ***REMOVED*** Note: Actual signature verification requires cryptographic validation
        ***REMOVED*** This would typically be done using xmlsec library or similar
        ***REMOVED*** For production use, integrate python3-saml or pysaml2
    ...
    def _validate_conditions(self, assertion: ET.Element) -> None:
        ...
        if conditions is None:
            return  ***REMOVED*** Conditions are optional

        now = datetime.utcnow()

        ***REMOVED*** Check NotBefore
        ...
        ***REMOVED*** Check AudienceRestriction
    ...
    def _extract_attributes(self, assertion: ET.Element) -> dict[str, str]:
        ...
        for attr in attribute_statement.findall(".//saml:Attribute", ns):
            ...
            if value_elem is not None and name:
                ...
                ***REMOVED*** Map attribute using configured mapping
