...
***REMOVED*** PKCE Constants (RFC 7636)
...
def generate_code_verifier() -> str:
    ...
    """
    ***REMOVED*** Generate 32 random bytes, encode as base64url (43 chars)
...
def generate_code_challenge(
    ...
    if method == CODE_CHALLENGE_METHOD_S256:
        ***REMOVED*** SHA256 hash of the verifier, then base64url encode
        digest = hashlib.sha256(code_verifier.encode("utf-8")).digest()
        challenge = base64.urlsafe_b64encode(digest).decode("utf-8").rstrip("=")
        return challenge
    elif method == CODE_CHALLENGE_METHOD_PLAIN:
        ***REMOVED*** Plain method: challenge = verifier (not recommended)
        return code_verifier
    else:
...
def generate_authorization_code() -> str:
    ...
    """
    ***REMOVED*** Generate 24 random bytes, encode as base64url (32 chars)
...
async def validate_client(db: Session, client_id: str) -> PKCEClient:
    ...
    client = (
        ...
        .filter(
            ...
            PKCEClient.is_active == True,  ***REMOVED*** noqa: E712
        )
        .first()
    )
...
async def create_authorization_code(
    ...
    ***REMOVED*** Generate authorization code
    code = generate_authorization_code()
    expires_at = datetime.utcnow() + timedelta(
        minutes=AUTHORIZATION_CODE_EXPIRE_MINUTES
    )

    ***REMOVED*** Store authorization code in database
...
async def exchange_code_for_token(
    ...
    ***REMOVED*** Retrieve authorization code
    auth_code = (
    ...
    ***REMOVED*** Check if code has already been used (prevent replay attacks)
    ...
    ***REMOVED*** Check if code has expired
    ...
    ***REMOVED*** Validate redirect URI matches
    ...
    ***REMOVED*** Verify PKCE code challenge
    ...
    ***REMOVED*** Mark authorization code as used
    auth_code.mark_as_used()
    db.commit()

    ***REMOVED*** Retrieve user
...
async def introspect_token(
    ...
    """
    ***REMOVED*** Verify and decode token
    token_data = verify_token(token, db)

    if token_data is None or token_data.user_id is None:
        ***REMOVED*** Token is invalid or expired
        return TokenIntrospectionResponse(active=False)

    ***REMOVED*** Retrieve user to get additional metadata
    user = db.query(User).filter(User.id == UUID(token_data.user_id)).first()

    if not user or not user.is_active:
        return TokenIntrospectionResponse(active=False)

    ***REMOVED*** Decode token to get claims (already verified above)
...
async def validate_state_parameter(
    ...
    if expected_state is None and received_state is None:
        ***REMOVED*** Both are None, which is acceptable but not recommended
        return
...
async def register_client(
    ...
    """
    ***REMOVED*** Generate unique client_id
