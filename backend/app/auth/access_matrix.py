...
class ResourceType(str, Enum):
    ...
    ***REMOVED*** Core scheduling resources
    SCHEDULE = "schedule"
    ...
    ***REMOVED*** Procedures and credentials
    PROCEDURE = "procedure"
    CREDENTIAL = "credential"
    CERTIFICATION = "certification"

    ***REMOVED*** Conflict management
    CONFLICT = "conflict"
...
class PermissionAction(str, Enum):
    ...
    EXPORT = "export"
    IMPORT = "import"
    MANAGE = "manage"  ***REMOVED*** All CRUD operations
...
class PermissionAuditEntry(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    action: str  ***REMOVED*** "granted", "revoked", "checked", "denied"
    role: UserRole
...
class RoleHierarchy:
    ...
    ***REMOVED*** Role level constants for comparison
    ...
    ***REMOVED*** Role hierarchy map (role -> parent roles)
    ***REMOVED*** SECURITY FIX: Faculty does NOT inherit from Coordinator
    HIERARCHY: dict[UserRole, set[UserRole]] = {
        UserRole.ADMIN: set(),  ***REMOVED*** Top level, no parents
        UserRole.COORDINATOR: {UserRole.ADMIN},  ***REMOVED*** Inherits from admin only
        UserRole.FACULTY: set(),  ***REMOVED*** INDEPENDENT - no inheritance
        UserRole.CLINICAL_STAFF: set(),  ***REMOVED*** INDEPENDENT - no inheritance
        UserRole.RN: {UserRole.CLINICAL_STAFF},  ***REMOVED*** Inherits from clinical_staff
        UserRole.LPN: {UserRole.CLINICAL_STAFF},  ***REMOVED*** Inherits from clinical_staff
        UserRole.MSA: {UserRole.CLINICAL_STAFF},  ***REMOVED*** Inherits from clinical_staff
        UserRole.RESIDENT: set(),  ***REMOVED*** INDEPENDENT - no inheritance
    }
    ...
    def can_manage_role(cls, manager_role: UserRole, target_role: UserRole) -> bool:
        ...
        ***REMOVED*** Coordinator can manage all except Admin and Coordinator
        if manager_role == UserRole.COORDINATOR:
            return target_role not in {UserRole.ADMIN, UserRole.COORDINATOR}

        ***REMOVED*** All other roles cannot manage others
        return False


***REMOVED*** ============================================================================
***REMOVED*** Access Control Matrix
***REMOVED*** ============================================================================
...
class AccessControlMatrix:
    ...
    ***REMOVED*** Core permission matrix: {role: {resource: {actions}}}
    PERMISSION_MATRIX: dict[UserRole, dict[ResourceType, set[PermissionAction]]] = {
        ***REMOVED*** ADMIN: Full access to everything
        UserRole.ADMIN: {
            resource: {action for action in PermissionAction}
            for resource in ResourceType
        },
        ***REMOVED*** COORDINATOR: Schedule management and oversight
        ...
        ***REMOVED*** FACULTY: View schedules, manage own availability and swaps
        UserRole.FACULTY: {
        ...
        ***REMOVED*** CLINICAL_STAFF: View access to schedules and manifests
        ...
        ***REMOVED*** RN, LPN, MSA: Same as CLINICAL_STAFF (inherit via hierarchy)
        UserRole.RN: {
            ResourceType.SCHEDULE: {PermissionAction.READ, PermissionAction.LIST},
            ResourceType.ASSIGNMENT: {PermissionAction.READ, PermissionAction.LIST},
            ResourceType.BLOCK: {PermissionAction.READ, PermissionAction.LIST},
        ...
        ***REMOVED*** RESIDENT: View own schedule, manage own swaps
        UserRole.RESIDENT: {
            ResourceType.SCHEDULE: {PermissionAction.READ, PermissionAction.LIST},
            ResourceType.ASSIGNMENT: {PermissionAction.READ, PermissionAction.LIST},
            ResourceType.BLOCK: {PermissionAction.READ, PermissionAction.LIST},
    ...
    def _register_default_context_evaluators(self) -> None:
        ...
        def is_supervisor(context: PermissionContext) -> bool:
            ...
            ***REMOVED*** This would typically check database relationships
            ***REMOVED*** For now, coordinators and admins are supervisors
    ...
    def has_permission(
        ...
        """
        ***REMOVED*** Convert strings to enums
        ...
        ***REMOVED*** Check static permissions in matrix
        has_static_permission = self._check_static_permission(role, resource, action)

        ***REMOVED*** Check inherited permissions from role hierarchy
        ...
        ***REMOVED*** Apply context-aware checks if context is provided
        ...
        ***REMOVED*** Audit the permission check
        self._audit_permission_check(
            role,
            resource,
            action,
    ...
    def _check_static_permission(
        ...
        ***REMOVED*** MANAGE action includes all CRUD operations
        if (
            action
            in {
                PermissionAction.CREATE,
    ...
    def _check_context_permission(
        ...
        """
        ***REMOVED*** SECURITY: APPROVE/REJECT actions require context validation
        ...
        ***REMOVED*** Own resource rule: Users with limited permissions can modify their own resources
        if role in {UserRole.RESIDENT, UserRole.FACULTY}:
            if action in {PermissionAction.UPDATE, PermissionAction.DELETE}:
                if resource in {
                    ...
                    ***REMOVED*** Must be their own resource
                    return self._context_evaluators["is_own_resource"](context)

        return True  ***REMOVED*** No additional context restrictions
    ...
    def _validate_approval_context(
        ...
        """
        ***REMOVED*** Admin can approve anything
        if role == UserRole.ADMIN:
            return True

        ***REMOVED*** Validate resource owner is set for approvals
        ...
        ***REMOVED*** Validate resource metadata exists
        ...
        ***REMOVED*** Resource-specific validation
        if resource in {ResourceType.ABSENCE, ResourceType.LEAVE}:
            ***REMOVED*** Must specify department/unit for leave approvals
        ...
        if resource in {ResourceType.SWAP, ResourceType.SWAP_REQUEST}:
            ***REMOVED*** Must specify both parties involved in swap
            ...
            ***REMOVED*** Coordinator role check for swap approvals
            if role != UserRole.COORDINATOR:
                ***REMOVED*** Faculty can only approve swaps they're involved in
        ...
        ***REMOVED*** Check supervisor relationship for non-admin approvals
        if role == UserRole.COORDINATOR:
            ***REMOVED*** Coordinators must be supervisor of the resource owner
    ...
    def get_role_permissions(
        ...
        ***REMOVED*** Start with direct permissions
        permissions = {}
        ...
        ***REMOVED*** Add inherited permissions
        inherited_roles = RoleHierarchy.get_inherited_roles(role)
        for inherited_role in inherited_roles:
    ...
    def _export_markdown(self) -> str:
        ...
        lines = [
            "***REMOVED*** Access Control Matrix",
            "",
            "***REMOVED******REMOVED*** Role Permissions",
            "",
        ]

        for role in UserRole:
            lines.append(f"***REMOVED******REMOVED******REMOVED*** {role.value.upper()}")
            lines.append("")
...
***REMOVED*** ============================================================================
***REMOVED*** Global Instance and Helpers
***REMOVED*** ============================================================================


***REMOVED*** Global ACM instance
...
def require_permission(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ***REMOVED*** Extract user from kwargs (assuming it's a dependency)
            ...
            ***REMOVED*** Build context if builder provided
            context = None
            if context_builder:
                context = context_builder(*args, **kwargs)

            ***REMOVED*** Check permission
