...
class CallScheduleModel:
    ...
    def _create_variables(self) -> None:
        ...
        ***REMOVED*** Binary assignment variables: is_assigned[day][faculty] = 1 if faculty on call that day
        ...
        ***REMOVED*** Total calls assigned to each faculty member
        ...
        ***REMOVED*** Deviation from target (can be positive or negative)
        for fac in self.input.faculty:
            target = self.input.target_calls.get(fac, 0)
            ***REMOVED*** Allow deviations from -target to +max_possible_calls
            var_name = f"deviation_{fac}"
            self.deviation[fac] = self.model.NewIntVar(
                -target, max_possible_calls, var_name
            )

        ***REMOVED*** Absolute deviation (always non-negative)
        for fac in self.input.faculty:
        ...
        ***REMOVED*** Maximum deviation across all faculty (for min-max fairness)
    ...
    def _add_coverage_constraints(self) -> None:
        ...
        for day in self.input.days:
            ***REMOVED*** Exactly one faculty member per day
            self.model.Add(
        ...
        for day in self.input.days:
            ...
            for fac in self.input.faculty:
                if fac not in eligible_faculty:
                    ***REMOVED*** Faculty not eligible for this day - force assignment to 0
    ...
    def _add_call_count_constraints(self) -> None:
        ...
        for fac in self.input.faculty:
            ***REMOVED*** Sum of assignments across all days equals total calls for this faculty
    ...
    def _add_deviation_constraints(self) -> None:
        ...
        for fac in self.input.faculty:
            ...
            ***REMOVED*** deviation = actual - target
            self.model.Add(self.deviation[fac] == self.calls_assigned[fac] - target)

            ***REMOVED*** Absolute value linearization:
            ***REMOVED*** abs_deviation >= deviation
            self.model.Add(self.abs_deviation[fac] >= self.deviation[fac])
            ***REMOVED*** abs_deviation >= -deviation
            self.model.Add(self.abs_deviation[fac] >= -self.deviation[fac])

            ***REMOVED*** Max deviation tracking
    ...
    def minimize_sum_deviations(
        ...
        """
        ***REMOVED*** Fix max_deviation to the value found in phase 1
        self.model.Add(self.max_deviation == fixed_max_dev)

        ***REMOVED*** New objective: minimize sum of absolute deviations
...
def optimize_call_schedule(
    ...
    Example:
        >>> input_data = CallScheduleInput(
        ...     days=[date(2025, 1, 3), date(2025, 1, 10)],  ***REMOVED*** Fridays
    ...
    ***REMOVED*** Extract the optimal max_deviation value from phase 1
    ***REMOVED*** Need to solve again to get solver instance with solution
    ...
    ***REMOVED*** Phase 2: Minimize sum of deviations with max_deviation fixed
    ...
    if phase2_status not in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ***REMOVED*** Phase 2 failed - return phase 1 solution
    ...
    ***REMOVED*** Extract final solution from phase 2
    ...
    ***REMOVED*** Determine overall status (optimal only if both phases optimal)
...
def _validate_input(input_data: CallScheduleInput) -> None:
    ...
    ***REMOVED*** Check that all faculty have target_calls defined
    ...
    ***REMOVED*** Check that all days have eligibility defined
    ...
    ***REMOVED*** Check that eligibility only references valid faculty
