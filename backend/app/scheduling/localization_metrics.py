...
class LocalizationQuality(str, Enum):
    ...
    EXCELLENT = "excellent"  ***REMOVED*** < 10% scope, strong barriers
    GOOD = "good"  ***REMOVED*** 10-20% scope, moderate barriers
    FAIR = "fair"  ***REMOVED*** 20-40% scope, weak barriers
    POOR = "poor"  ***REMOVED*** > 40% scope, cascade risk
...
class LocalizationMetrics:
    ...
    total_events: int = 0
    localized_count: int = 0  ***REMOVED*** Successfully localized (< 20% scope)
    extended_count: int = 0  ***REMOVED*** Extended regions (20-60% scope)
    global_count: int = 0  ***REMOVED*** Global cascades (> 60% scope)

    ***REMOVED*** Scope metrics
    ...
    ***REMOVED*** Quality distribution
    quality_distribution: dict[LocalizationQuality, int] = field(
        default_factory=lambda: defaultdict(int)
    )

    ***REMOVED*** Per-disruption-type metrics
...
class LocalizationMetricsTracker:
    ...
    def record_event(
        ...
        ***REMOVED*** Add to event history
        self.events.append(event)

        ***REMOVED*** Trim to window size
    ...
    def _classify_quality(self, region: LocalizationRegion) -> LocalizationQuality:
        ...
        - Escape probability
        """
        ***REMOVED*** Estimate total schedule size (rough heuristic)
        ***REMOVED*** In real implementation, would get from context
        estimated_total_size = 1000  ***REMOVED*** assignments

        scope_fraction = region.region_size / estimated_total_size

        ***REMOVED*** Excellent: Small scope + strong barriers
        if (
            scope_fraction < 0.1
        ...
        ***REMOVED*** Good: Moderate scope, decent barriers
        if (
            scope_fraction < 0.2
        ...
        ***REMOVED*** Fair: Larger scope or weaker barriers
        if scope_fraction < 0.4 and region.escape_probability < 0.6:
            return LocalizationQuality.FAIR

        ***REMOVED*** Poor: Large scope or high cascade risk
    ...
    def _update_metrics(self) -> None:
        ...
        ***REMOVED*** Count by region type
        for event in self.events:
            ...
            ***REMOVED*** Update quality distribution
            self.metrics.quality_distribution[event.quality] += 1

        ***REMOVED*** Compute averages
        ...
        ***REMOVED*** Compute percentiles for computation time
...
***REMOVED*** Pydantic schemas for API responses
