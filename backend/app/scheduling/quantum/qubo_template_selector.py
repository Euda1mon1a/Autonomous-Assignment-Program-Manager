...
**Objective Weights (for weighted-sum scalarization):**
    ...
    coverage_weight: 1.0     ***REMOVED*** Maximize filled slots
    fairness_weight: 1.0     ***REMOVED*** Equal distribution of desirable rotations
    preference_weight: 0.5   ***REMOVED*** Honor individual preferences
    learning_goal_weight: 0.3 ***REMOVED*** Encourage rotation variety
...
===============================================================================
    ...
    ***REMOVED*** Configure for fairness-focused optimization
    config = TemplateSelectionConfig(
        coverage_weight=1.0,
        fairness_weight=2.0,  ***REMOVED*** Prioritize fair distribution
    ...
    ***REMOVED*** Create solver
    solver = QUBOTemplateSolver(config=config)

    ***REMOVED*** Get full result with Pareto front
...
***REMOVED*** ============================================================================
***REMOVED*** CONSTANTS AND CONFIGURATION
***REMOVED*** ============================================================================
...
class TemplateDesirability(str, Enum):
    ...
    HIGHLY_DESIRABLE = "highly_desirable"  ***REMOVED*** e.g., Sports Medicine, Electives
    NEUTRAL = "neutral"  ***REMOVED*** e.g., Standard Clinic
    UNDESIRABLE = "undesirable"  ***REMOVED*** e.g., Night Float, Weekend Call


***REMOVED*** Default desirability mappings (can be overridden)
...
class TemplateSelectionConfig:
    ...
    ***REMOVED*** Objective weights (for weighted-sum scalarization)
    coverage_weight: float = 1.0
    fairness_weight: float = 1.0
    preference_weight: float = 0.5
    learning_goal_weight: float = 0.3

    ***REMOVED*** Annealing parameters
    num_reads: int = 100
    num_sweeps: int = 1000
    beta_start: float = 0.1
    beta_end: float = 4.2

    ***REMOVED*** Adaptive temperature parameters
    use_adaptive_temperature: bool = True
    reheat_threshold: float = 0.001  ***REMOVED*** Reheat if improvement < threshold
    reheat_factor: float = 0.5  ***REMOVED*** Multiply beta by this on reheat

    ***REMOVED*** Pareto parameters
    pareto_population: int = 50
    pareto_generations: int = 100

    ***REMOVED*** Seed for reproducibility
    seed: int | None = None
    ...
    def to_json(self) -> dict:
        ...
        return {
            ...
            "energy_landscape": [
                ...
                for point in self.energy_landscape[:100]  ***REMOVED*** Limit for export
            ],
...
***REMOVED*** ============================================================================
***REMOVED*** QUBO TEMPLATE FORMULATION
***REMOVED*** ============================================================================
...
class QUBOTemplateFormulation:
    ...
    def __init__(
        ...
        ***REMOVED*** Aggregate blocks into rotation periods (2-week chunks)
        self.rotation_periods = self._build_rotation_periods()

        ***REMOVED*** Variable indexing: x[r, p, t] -> flat index
        self.var_index: dict[tuple[int, int, int], int] = {}
        self.index_to_var: dict[int, tuple[int, int, int]] = {}
        self._build_variable_index()

        ***REMOVED*** QUBO matrix (sparse representation)
        self.Q: dict[tuple[int, int], float] = {}

        ***REMOVED*** Objective components for Pareto analysis
    ...
    def _build_rotation_periods(self) -> list[list[Any]]:
        ...
        ***REMOVED*** Group into 2-week (10 workday) periods
        periods = []
        current_period = []

        for block in workday_blocks:
            current_period.append(block)
            if len(current_period) >= 10:  ***REMOVED*** ~2 weeks of workdays
    ...
    def build(
        ...
        ***REMOVED*** Build individual objective matrices
        ...
        ***REMOVED*** Combine into final Q using weights
    ...
    def _add_fairness_objective(self) -> None:
        ...
        ***REMOVED*** Group templates by desirability
        ...
        ***REMOVED*** For each desirability category, add equity constraints
        for desirability, template_indices in desirability_templates.items():
            if not template_indices:
                continue

            ***REMOVED*** Expected count per resident for this category
            n_periods = len(self.rotation_periods)
            expected_per_resident = len(template_indices) * n_periods / n_residents

            ***REMOVED*** Penalty weight inversely proportional to expected count
            penalty = self.config.fairness_penalty / max(expected_per_resident, 1)

            ***REMOVED*** Penalize pairs of same-category assignments to same resident
            ***REMOVED*** This encourages spreading desirable templates across residents
            for r_i in range(n_residents):
                ...
                ***REMOVED*** Small penalty for each pair (discourages hoarding)
    ...
    def _add_preference_objective(self) -> None:
        ...
        ***REMOVED*** Check if context has explicit preference data
        has_preferences = (
        ...
        for (r_i, p_i, t_i), idx in self.var_index.items():
            ...
            if template and resident:
                ***REMOVED*** Use explicit preferences if available
                if has_preferences and resident.id in self.context.preferences:
                    ...
                    ***REMOVED*** Apply preference weight (positive = preferred)
                    key = (idx, idx)
                    matrix[key] = matrix.get(key, 0.0) - template_pref
                else:
                    ***REMOVED*** Fall back to desirability as proxy
                    desirability = self.desirability_map.get(
                        template.name, TemplateDesirability.NEUTRAL
                    )

                    ***REMOVED*** Bonus for desirable templates
    ...
    def _add_learning_goal_objective(self) -> None:
        ...
        for r_i in range(len(self.context.residents)):
            ***REMOVED*** For each template, if assigned to multiple periods, add small penalty
            ***REMOVED*** This encourages variety
            for t_i in range(len(valid_templates)):
                ...
                ***REMOVED*** Small penalty for consecutive same-template assignments
    ...
    def _add_one_template_per_period_constraint(self) -> None:
        ...
        for r_i in range(len(self.context.residents)):
            for p_i in range(n_periods):
                ***REMOVED*** Get all variables for this (resident, period)
                period_vars = []
                for t_i in range(len(valid_templates)):
                    if (r_i, p_i, t_i) in self.var_index:
                        period_vars.append(self.var_index[(r_i, p_i, t_i)])
                ...
                ***REMOVED*** (sum - 1)^2 = sum^2 - 2*sum + 1
                ***REMOVED*** = sum_i sum_j x_i * x_j - 2*sum_i x_i + 1
                ***REMOVED*** = sum_i x_i^2 + 2*sum_{i<j} x_i*x_j - 2*sum_i x_i + 1
                ***REMOVED*** = sum_i (x_i - 2*x_i) + 2*sum_{i<j} x_i*x_j + 1
                ***REMOVED*** = -sum_i x_i + 2*sum_{i<j} x_i*x_j + 1

                penalty = self.config.hard_constraint_penalty

                ***REMOVED*** Linear terms (encourage at least one selection)
                for idx in period_vars:
                    key = (idx, idx)
                    matrix[key] = matrix.get(key, 0.0) - penalty

                ***REMOVED*** Quadratic terms (penalize multiple selections)
    ...
    def _add_template_capacity_constraints(self) -> None:
        ...
        for t_i, template in enumerate(valid_templates):
            ...
            ***REMOVED*** For each period, penalize if more than max_capacity assigned
            for p_i in range(n_periods):
                period_vars = []
                for r_i in range(n_residents):
                    if (r_i, p_i, t_i) in self.var_index:
                ...
                ***REMOVED*** Soft penalty for exceeding capacity
                ***REMOVED*** Use quadratic penalty scaled by over-capacity
    ...
    def _add_availability_constraints(self) -> None:
        ...
        for r_i, resident in enumerate(self.context.residents):
            ...
            for p_i, period_blocks in enumerate(self.rotation_periods):
                ***REMOVED*** Check if any block in period is unavailable
                ...
                if unavailable_in_period:
                    ***REMOVED*** Penalize all template assignments for this period
                    for t_i in range(len(valid_templates)):
                        if (r_i, p_i, t_i) in self.var_index:
        ...
        for idx, value in sample.items():
            if value == 1 and idx in self.index_to_var:
                ...
                if resident and template and period_blocks:
                    ***REMOVED*** Create assignment for each block in the period
...
***REMOVED*** ============================================================================
***REMOVED*** ADAPTIVE TEMPERATURE SIMULATED ANNEALING
***REMOVED*** ============================================================================
...
class AdaptiveTemperatureSchedule:
    ...
    def get_beta(self, sweep: int, current_energy: float) -> float:
        ...
        ***REMOVED*** Check for plateau (no improvement for 10% of sweeps)
        ...
        if (sweep - self.last_improvement_sweep) > plateau_threshold:
            ...
            ***REMOVED*** Normal geometric cooling
    ...
    def get_tunneling_probability(
        ...
        ***REMOVED*** Quantum tunneling term
...
***REMOVED*** ============================================================================
***REMOVED*** ENERGY LANDSCAPE EXPLORER
***REMOVED*** ============================================================================
...
class EnergyLandscapeExplorer:
    ...
    def explore(self) -> list[EnergyLandscapePoint]:
        ...
        ***REMOVED*** Identify local minima by local search
        self._identify_local_minima(Q)

        ***REMOVED*** Compute tunneling probabilities between minima
        self._compute_tunneling_probabilities()

        ***REMOVED*** Estimate basin sizes
    ...
    def _identify_local_minima(self, Q: dict) -> None:
        ...
        for point in self.points:
            ...
            ***REMOVED*** Check all single-bit flips
            for flip_idx in range(min(n, 50)):  ***REMOVED*** Limit for efficiency
                ***REMOVED*** Compute energy after flip
                delta = self._compute_delta_energy(point.state, Q, flip_idx)
                if delta < -1e-6:  ***REMOVED*** Found improvement
                    is_minimum = False
                    break
    ...
    def _estimate_basin_sizes(self, Q: dict) -> None:
        ...
        for point in self.points:
            ***REMOVED*** Count how many non-minima points would descend to this one
            if point.is_local_minimum:
                basin = sum(
                    1
                    for p in self.points
                    if not p.is_local_minimum
                    and abs(p.energy - point.energy) < 100  ***REMOVED*** Rough proximity
                )
...
class ParetoFrontExplorer:
    ...
    def explore(self) -> list[ParetoSolution]:
        ...
        ***REMOVED*** Generate diverse weight vectors
        ...
        for gen in range(self.generations):
            ...
            ***REMOVED*** Prune dominated solutions periodically
            if gen % 10 == 0:
                self._compute_pareto_ranks()
                ***REMOVED*** Keep only non-dominated and slightly dominated
                self.solutions = [s for s in self.solutions if s.rank <= 2]

        ***REMOVED*** Final ranking
        self._compute_pareto_ranks()
        self._compute_crowding_distances()

        ***REMOVED*** Extract frontier
    ...
    def _generate_weight_vectors(self, n: int) -> list[list[float]]:
        ...
        ***REMOVED*** Uniform grid on simplex
        for i in range(n):
        ...
        ***REMOVED*** Add extreme weights (single objective)
    ...
    def _compute_pareto_ranks(self) -> None:
        ...
        while remaining:
            ...
            if rank > 100:  ***REMOVED*** Safety limit
                break
    ...
    def _estimate_hypervolume(self) -> float:
        ...
        ***REMOVED*** Simple 2D hypervolume for first two objectives
        ...
        ***REMOVED*** Sort by first objective
        points.sort()

        ***REMOVED*** Reference point (nadir)
        ref = (
            max(p[0] for p in points) + 1,
            max(p[1] for p in points) + 1,
        )

        ***REMOVED*** Compute hypervolume
        hv = 0.0
        prev_y = ref[1]
...
***REMOVED*** ============================================================================
***REMOVED*** HYBRID CLASSICAL-QUANTUM PIPELINE
***REMOVED*** ============================================================================
...
class HybridTemplatePipeline:
    ...
    def __init__(
        ...
        ***REMOVED*** Pipeline stage results
    ...
    def run(self) -> TemplateSelectionResult:
        ...
        ***REMOVED*** Stage 1: QUBO coarse optimization
        ...
        ***REMOVED*** Run quantum-inspired annealing with adaptive temperature
        ...
        ***REMOVED*** Stage 2: Classical refinement (gradient-like descent)
        ...
        ***REMOVED*** Stage 3: Constraint repair
        ...
        ***REMOVED*** Explore energy landscape
        ...
        ***REMOVED*** Find recommended solution from Pareto front
        recommended_id = None
        if pareto_frontier:
            ***REMOVED*** Recommend solution with best coverage
    ...
    def _classical_refinement(
        ...
        while improved and iteration < max_iterations:
            ...
            ***REMOVED*** Try each variable flip
            for i in range(n):
                delta = self._compute_delta_energy(current, Q, i)

                if delta < -1e-6:  ***REMOVED*** Improvement found
    ...
    def _constraint_repair(
        ...
        ***REMOVED*** Ensure exactly one template per resident per period
        for r_i in range(len(self.context.residents)):
            for p_i in range(n_periods):
                period_vars = []
                for t_i in range(len(valid_templates)):
                ...
                ***REMOVED*** Count assigned
                assigned = [
                    idx for idx, t_i in period_vars if repaired.get(idx, 0) == 1
                ]

                if len(assigned) == 0:
                    ***REMOVED*** Assign first available template
                    if period_vars:
                        repaired[period_vars[0][0]] = 1
                elif len(assigned) > 1:
                    ***REMOVED*** Keep first, unassign rest
...
class TemplateBenchmark:
    ...
    def run_benchmark(self) -> dict:
        ...
        ***REMOVED*** Classical greedy approach
        ...
        ***REMOVED*** Compute improvement metrics
