...
**Penalty Weight Hierarchy:**
    ...
    HARD_CONSTRAINT_PENALTY = 50000  ***REMOVED*** Must satisfy (coverage)
    ACGME_PENALTY = 25000            ***REMOVED*** Critical safety (no consecutive)
    EQUITY_PENALTY = 500             ***REMOVED*** Quality (fair distribution)
    PREFERENCE_PENALTY = 50          ***REMOVED*** Nice-to-have (individual preferences)
...
**Example 1: Basic Call Assignment**
    ...
    ***REMOVED*** Create call nights for January
    call_nights = create_call_nights_from_dates(
        start_date=date(2025, 1, 1),
        end_date=date(2025, 1, 31),
        call_type=CallType.OVERNIGHT,
        exclude_weekends=True,  ***REMOVED*** FMIT handles weekends
    )
    ...
    ***REMOVED*** Create residents
    candidates = [
        CallCandidate(
            ...
            avoid_days={6},  ***REMOVED*** Avoid Sunday
        ),
        CallCandidate(
            person_id=uuid4(),
            name="Dr. Jones",
            pgy_level=3,
            max_calls_per_week=2,
            preference_bonus={0: 0.5},  ***REMOVED*** Prefers Monday
        ),
    ]
...
**Example 2: Manual QUBO Construction**
    ...
    ***REMOVED*** Solve with custom parameters
    ...
    ***REMOVED*** Analyze constraint breakdown
...
***REMOVED*** =============================================================================
***REMOVED*** ENUMERATIONS AND DATA CLASSES
***REMOVED*** =============================================================================
...
class CallType(str, Enum):
    ...
    OVERNIGHT = "overnight"  ***REMOVED*** Sun-Thu nights
    BACKUP = "backup"  ***REMOVED*** Backup coverage
    WEEKEND = "weekend"  ***REMOVED*** Fri/Sat (typically FMIT)


class ConstraintHardness(str, Enum):
    """Constraint classification for penalty weighting."""

    HARD = "hard"  ***REMOVED*** Must satisfy (large penalty)
    SOFT = "soft"  ***REMOVED*** Prefer to satisfy (small penalty)
...
class CallCandidate:
    ...
    avoid_days: set[int] = field(default_factory=set)  ***REMOVED*** weekdays to avoid
    preference_bonus: dict[int, float] = field(default_factory=dict)  ***REMOVED*** weekday -> bonus
...
class QUBOSolution:
    ...
    sample: dict[int, int]  ***REMOVED*** variable index -> 0/1
    energy: float
...
***REMOVED*** =============================================================================
***REMOVED*** QUBO FORMULATION FOR CALL ASSIGNMENT
***REMOVED*** =============================================================================
...
class CallAssignmentQUBO:
    ...
    ***REMOVED*** Penalty weights for constraint violations
    HARD_CONSTRAINT_PENALTY = 50000.0  ***REMOVED*** Coverage, ACGME (must satisfy)
    ACGME_PENALTY = 25000.0  ***REMOVED*** Post-call rules
    EQUITY_PENALTY = 500.0  ***REMOVED*** Fair distribution
    PREFERENCE_PENALTY = 50.0  ***REMOVED*** Individual preferences
    ...
    def __init__(
        ...
        ***REMOVED*** Landscape tracking for visualization
    ...
    def build(self) -> dict[tuple[int, int], float]:
        ...
        ***REMOVED*** Objective: Encourage assignment (negative diagonal)
        self._add_assignment_incentive()

        ***REMOVED*** Hard: Exactly one resident per call night
        self._add_coverage_constraint()

        ***REMOVED*** Hard: At most one call per resident per night (already enforced by encoding)
        ***REMOVED*** Hard: ACGME consecutive call limit
        self._add_consecutive_call_constraint()

        ***REMOVED*** Soft: Call equity
        self._add_equity_constraint()

        ***REMOVED*** Soft: Sunday call equity (tracked separately)
        self._add_sunday_equity_constraint()

        ***REMOVED*** Soft: Weekday preferences
        self._add_preference_constraints()

        ***REMOVED*** Soft: Call spacing (avoid back-to-back)
    ...
    def _add_assignment_incentive(self) -> None:
        ...
        ***REMOVED*** Each assignment contributes -1 to objective (we minimize)
    ...
    def _add_coverage_constraint(self) -> None:
        ...
        """
        for n_i in range(self.num_nights):
            ***REMOVED*** Get all variables for this night
            night_vars = [
                self.var_index[(r_i, n_i)] for r_i in range(self.num_candidates)
            ]

            ***REMOVED*** Linear part: -2P * Σ x[r,n] (combined with assignment incentive)
            for idx in night_vars:
                self._add_linear(idx, -2 * self.HARD_CONSTRAINT_PENALTY + 1)

            ***REMOVED*** Quadratic part: P * Σ_r Σ_s x[r,n]x[s,n] for r≠s
    ...
    def _add_consecutive_call_constraint(self) -> None:
        ...
        for r_i, candidate in enumerate(self.candidates):
            ...
            ***REMOVED*** For standard case (max_consecutive=1), penalize any consecutive
            if max_consecutive <= 1:
                for n_i in range(self.num_nights - 1):
                    ***REMOVED*** Check if nights are actually consecutive dates
                    if (
    ...
    def _add_equity_constraint(self) -> None:
        ...
        for r_i in range(self.num_candidates):
            ...
            ***REMOVED*** Linear: -2*mean*x (encourages filling to mean)
            for idx in resident_vars:
                self._add_linear(idx, -2 * mean_calls * self.EQUITY_PENALTY)

            ***REMOVED*** Quadratic: x[r,n]*x[r,m] (penalizes exceeding mean)
            ***REMOVED*** Scale penalty by 1/num_nights to avoid overwhelming other terms
    ...
    def _add_sunday_equity_constraint(self) -> None:
        ...
        sunday_penalty = self.EQUITY_PENALTY * 2  ***REMOVED*** Higher weight for Sunday
        ...
        for r_i in range(self.num_candidates):
            ...
            ***REMOVED*** Similar to general equity but for Sunday subset
        ...
        for r_i in range(self.num_candidates):
            for n_i in range(self.num_nights):
                for n_j in range(n_i + 1, min(n_i + 4, self.num_nights)):
                    ***REMOVED*** Check actual day spacing
                    days_apart = (
                        self.call_nights[n_j].date - self.call_nights[n_i].date
                    ).days

                    if 2 <= days_apart <= 3:  ***REMOVED*** Close but not consecutive
                        idx1 = self.var_index[(r_i, n_i)]
                        idx2 = self.var_index[(r_i, n_j)]
                        ***REMOVED*** Smaller penalty for 3 days apart than 2
    ...
    def get_constraint_breakdown(self, sample: dict[int, int]) -> dict[str, float]:
        ...
        """
        ***REMOVED*** This is a simplified analysis - for full breakdown would need
        ***REMOVED*** to rebuild Q with tracking
        breakdown = {
            "coverage": 0.0,
            "consecutive": 0.0,
            "equity": 0.0,
        ...
        ***REMOVED*** Count assignments per resident and night for analysis
...
***REMOVED*** =============================================================================
***REMOVED*** QUANTUM-INSPIRED SIMULATED ANNEALING SOLVER
***REMOVED*** =============================================================================
...
class QuantumTunnelingAnnealingSolver:
    ...
    def __init__(
        ...
        ***REMOVED*** Landscape data for export
        self.landscape_history: list[LandscapePoint] = []
    ...
    def solve(self, formulation: CallAssignmentQUBO) -> QUBOSolution:
        ...
        for read in range(self.num_reads):
            ***REMOVED*** Initialize with random state
            ...
            for sweep in range(self.num_sweeps):
                ***REMOVED*** Path-integral inspired temperature schedule
                ***REMOVED*** Higher power maintains exploration longer
                t = sweep / self.num_sweeps
                alpha = 2.0  ***REMOVED*** Schedule exponent
                beta = beta_start + (beta_end - beta_start) * (t**alpha)

                ***REMOVED*** Shuffle variable order for better mixing
                var_order = list(range(n))
                random.shuffle(var_order)

                for i in var_order:
                    ***REMOVED*** Compute energy change for flipping variable i
                    delta_e = self._compute_delta_energy(sample, Q, i)

                    ***REMOVED*** Acceptance probability with quantum tunneling
                    accept = False
                    if delta_e <= 0:
                        accept = True
                    else:
                        ***REMOVED*** Classical Metropolis probability
                        classical_prob = math.exp(-beta * delta_e)

                        ***REMOVED*** Quantum tunneling probability
                        ***REMOVED*** P_tunnel = exp(-κ * √|ΔE|)
                        tunneling_prob = math.exp(
                            -self.barrier_coefficient * math.sqrt(abs(delta_e))
                        )

                        ***REMOVED*** Combined probability
                ...
                ***REMOVED*** Track landscape periodically
    ...
    def _compute_delta_energy(
        ...
        delta = new_val - current_val  ***REMOVED*** +1 or -1
    ...
    def _validate_solution(
        ...
        ***REMOVED*** Check coverage: exactly one resident per night
        ...
        ***REMOVED*** Check consecutive call constraint
...
class CallAssignmentValidator:
    ...
    def validate(
        ...
        ***REMOVED*** 4. Specialty validation
    ...
    def _validate_acgme(
        ...
        for person_id, calls in by_resident.items():
            ...
            ***REMOVED*** Sort calls by date
            sorted_calls = sorted(calls, key=lambda x: x[0])

            ***REMOVED*** Check consecutive calls
            ...
            ***REMOVED*** Check weekly limit
            ***REMOVED*** Group by ISO week
    ...
    def _validate_equity(
        ...
        ***REMOVED*** Check Sunday equity separately
        sunday_counts = {}
...
***REMOVED*** =============================================================================
***REMOVED*** BENCHMARKING AGAINST OR-TOOLS
***REMOVED*** =============================================================================
...
class CallAssignmentBenchmark:
    ...
    def run_ortools_solver(self, timeout_seconds: float = 60.0) -> dict[str, Any]:
        ...
        ***REMOVED*** Variables: x[r, n] = 1 if resident r on call night n
        x = {}
        for r_i in range(n_candidates):
            for n_i in range(n_nights):
                x[r_i, n_i] = model.NewBoolVar(f"x_{r_i}_{n_i}")

        ***REMOVED*** Constraint: Exactly one resident per night
        for n_i in range(n_nights):
            model.AddExactlyOne([x[r_i, n_i] for r_i in range(n_candidates)])

        ***REMOVED*** Constraint: Max calls per week
        for r_i, candidate in enumerate(self.candidates):
            ***REMOVED*** Group nights by week
        ...
        ***REMOVED*** Constraint: No consecutive calls
        for r_i, candidate in enumerate(self.candidates):
            if candidate.max_consecutive_call_days <= 1:
        ...
        ***REMOVED*** Objective: Minimize max calls (equity)
    ...
    def compare(
        ...
        ***REMOVED*** Compare if both succeeded
...
def solve_call_assignment(
    ...
    ***REMOVED*** Export landscape if requested
...
def create_call_nights_from_dates(
    ...
    while current <= end_date:
        ...
        ***REMOVED*** Skip Friday (4) and Saturday (5) if excluding weekends
