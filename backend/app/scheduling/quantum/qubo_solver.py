...
class QUBOFormulation:
    ...
    ***REMOVED*** Penalty weights for constraint violations
    HARD_CONSTRAINT_PENALTY = 10000.0  ***REMOVED*** Must be satisfied
    ACGME_PENALTY = 5000.0  ***REMOVED*** ACGME compliance
    SOFT_CONSTRAINT_PENALTY = 100.0  ***REMOVED*** Optimization targets
    ...
    def __init__(self, context: SchedulingContext) -> None:
        ...
        ***REMOVED*** Variable indexing: x[r, b, t] -> flat index
        ...
        for resident in self.context.residents:
            ...
            for block in self.workday_blocks:
                ...
                for template in self.context.templates:
                    ***REMOVED*** Skip templates requiring credentials
    ...
    def build(self) -> dict[tuple[int, int], float]:
        ...
        ***REMOVED*** Hard: One assignment per person per block
        self._add_one_per_block_constraint()

        ***REMOVED*** Hard: Availability constraints
        self._add_availability_constraints()

        ***REMOVED*** Hard: ACGME 80-hour rule (simplified)
        self._add_80_hour_constraint()

        ***REMOVED*** Soft: Equity (work distribution)
    ...
    def _add_coverage_objective(self) -> None:
        ...
        for idx in range(self.num_variables):
            ***REMOVED*** Negative weight encourages x[i] = 1
    ...
    def _add_one_per_block_constraint(self) -> None:
        ...
        ***REMOVED*** Group variables by block
        ...
        for b_i, indices in block_vars.items():
            ...
            ***REMOVED*** Penalty for multiple assignments: sum over pairs
    ...
    def _add_availability_constraints(self) -> None:
        ...
        for resident in self.context.residents:
            ...
            for block in self.workday_blocks:
                b_i = self.context.block_idx[block.id]
                if b_i in unavail:
                    ***REMOVED*** Penalize any assignment to this block
    ...
    def _add_80_hour_constraint(self) -> None:
        ...
        ***REMOVED*** Group blocks by week
        ...
        for block in self.workday_blocks:
            b_i = self.context.block_idx[block.id]
            ***REMOVED*** Simple week calculation (0-indexed)
            week = b_i // 10  ***REMOVED*** Assuming ~10 blocks per week
            if week not in week_blocks:
                week_blocks[week] = []
            week_blocks[week].append(b_i)

        ***REMOVED*** For each resident, penalize exceeding max per week
        for resident in self.context.residents:
            r_i = self.context.resident_idx[resident.id]

            for week, block_indices in week_blocks.items():
                ***REMOVED*** Get all variable indices for this resident in this week
                week_vars = []
                ...
                ***REMOVED*** Penalty for exceeding threshold: (sum - max)^2 if sum > max
                ***REMOVED*** Approximated as quadratic penalty on pairs beyond threshold
    ...
    def _add_equity_objective(self) -> None:
        ...
        ***REMOVED*** Penalize pairs within same resident (encourages spreading)
        for r_i, indices in resident_vars.items():
            ***REMOVED*** Small penalty for having many assignments to same resident
...
class SimulatedQuantumAnnealingSolver(BaseSolver):
    ...
    def solve(
        ...
        ***REMOVED*** Build QUBO formulation
        ...
        ***REMOVED*** Try D-Wave samplers first, fall back to pure Python
        ...
        ***REMOVED*** Decode solution
        ...
        return SolverResult(
            ...
            status="feasible",
            objective_value=-energy,  ***REMOVED*** Negate since we minimized
    ...
    def _solve_pure_python(
        ...
        for read in range(self.num_reads):
            ***REMOVED*** Initialize random state
            ...
            for sweep in range(self.num_sweeps):
                ***REMOVED*** Linear beta schedule
                t = sweep / self.num_sweeps
                beta = beta_start + t * (beta_end - beta_start)

                ***REMOVED*** Sweep through all variables
                for i in range(n):
                    ***REMOVED*** Compute energy change if we flip variable i
                    delta_e = self._compute_delta_energy(sample, Q, i)

                    ***REMOVED*** Quantum-inspired acceptance probability
                    ***REMOVED*** Includes tunneling term for barrier penetration
                    if delta_e <= 0:
                        accept = True
                    else:
                        ***REMOVED*** Classical Metropolis
                        classical_prob = math.exp(-beta * delta_e)

                        ***REMOVED*** Quantum tunneling contribution
                        ***REMOVED*** Simplified model: probability to tunnel through barrier
                        barrier_width = 1.0  ***REMOVED*** Normalized
...
class QUBOSolver(BaseSolver):
    ...
    def solve(
        ...
        ***REMOVED*** Compile to BQM with placeholder values
        ***REMOVED*** lambda_block is the penalty weight for the one-per-block constraint
        ...
        ***REMOVED*** Solve with appropriate sampler
        if DWAVE_SAMPLERS_AVAILABLE:
            if self.use_tabu:
            ...
            ***REMOVED*** Fallback to pure Python
            formulation = QUBOFormulation(context)
            Q = formulation.build()
        ...
        ***REMOVED*** Extract assignments from decoded solution
    ...
    def _build_pyqubo_model(self, context: SchedulingContext) -> tuple[Any, dict]:
        ...
        ***REMOVED*** Create binary variables: x[resident, block, template]
        x = Array.create(
            "x", shape=(n_residents, n_blocks, n_templates), vartype="BINARY"
        )

        ***REMOVED*** Build index mappings
        var_map = {
        ...
        ***REMOVED*** Objective: Maximize coverage
        H_coverage = -sum(
            x[r, b, t]
        ...
        ***REMOVED*** Constraint: At most one assignment per resident per block
        H_one_per_block = sum(
            Constraint(
        ...
        ***REMOVED*** Constraint: At most one resident per block (simplified)
        H_one_resident = sum(
            Constraint(
                (
                    sum(
...
class QuantumInspiredSolver(BaseSolver):
    ...
    def __init__(
        ...
        ***REMOVED*** Validate quantum hardware availability
        ...
        if use_quantum_hardware:
            if not DWAVE_SYSTEM_AVAILABLE:
                ...
                ***REMOVED*** All requirements met - enable quantum hardware
    ...
    def solve(
        ...
        ***REMOVED*** Select solver based on problem size
        if n_vars == 0:
            return SolverResult(
                success=False,
                assignments=[],
        ...
        ***REMOVED*** Use simulated annealing for all sizes
    ...
    def _solve_with_dwave(
        ...
        ***REMOVED*** Try D-Wave quantum hardware
        try:
            from dwave.system import DWaveSampler, EmbeddingComposite
            ...
            response = sampler.sample_qubo(
                Q,
                num_reads=100,
                annealing_time=20,  ***REMOVED*** microseconds
            )
            best = response.first
            ...
            ***REMOVED*** Graceful fallback to classical SA
...
def get_quantum_solver_config() -> dict[str, Any]:
    ...
    ***REMOVED*** Only use quantum hardware if:
    ***REMOVED*** 1. Quantum solver is enabled
    ***REMOVED*** 2. Backend is set to "quantum"
    ***REMOVED*** 3. D-Wave token is provided
    ***REMOVED*** 4. D-Wave libraries are available
...
def create_quantum_solver_from_env(
    ...
    Example:
        ...
        ... else:
        ...     ***REMOVED*** Use classical solver instead
