...
class SolverControl:
    ...
    ***REMOVED*** TTLs for cleanup
    ABORT_TTL_SECONDS = 3600  ***REMOVED*** 1 hour
    PROGRESS_TTL_SECONDS = 7200  ***REMOVED*** 2 hours
    RESULT_TTL_SECONDS = 86400  ***REMOVED*** 24 hours
    ...
    def update_progress(
        ...
        try:
            ...
            ***REMOVED*** Don't fail solver on progress update errors
    ...
    def get_active_runs(cls) -> list[dict]:
        ...
        try:
            ...
            ***REMOVED*** Scan for progress keys
            active_runs = []
    ...
    def cleanup_stale(cls) -> int:
        ...
        try:
            ...
            for key in redis_client.scan_iter(f"{cls.PROGRESS_KEY_PREFIX}*", count=100):
                ***REMOVED*** Redis TTL handles expiration, but we can clean up
                ***REMOVED*** entries with status != "running" that are old
                ttl = redis_client.ttl(key)
                if ttl == -1:  ***REMOVED*** No TTL set
                    redis_client.expire(key, cls.PROGRESS_TTL_SECONDS)
                    cleaned += 1
