...
class SchedulingEngine:
    ...
    ***REMOVED*** Available algorithms
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize resilience service for health monitoring
    ...
    def generate(
        ...
        ***REMOVED*** Validate algorithm
        if algorithm not in self.ALGORITHMS:
            logger.warning(f"Unknown algorithm '{algorithm}', using 'greedy'")
            algorithm = "greedy"

        ***REMOVED*** Issue ***REMOVED***3: Transaction boundaries - wrap everything in a single transaction
        ***REMOVED*** Create an "in_progress" run record first
        run = self._create_initial_run(algorithm)

        ***REMOVED*** Pre-generation resilience check
        ***REMOVED*** Store as instance attribute for _populate_resilience_data to access
        self._pre_health_report = None
        if check_resilience:
            self._pre_health_report = self._check_pre_generation_resilience()

        try:
            ***REMOVED*** Step 1: Ensure blocks exist (but don't commit yet)
            blocks = self._ensure_blocks_exist(commit=False)

            ***REMOVED*** Step 1.5a: Load FMIT faculty assignments if preserving them
            fmit_assignments = []
            ...
            ***REMOVED*** Step 1.5b: Load resident inpatient assignments if preserving them
            ...
            ***REMOVED*** Step 1.5c: Load absence assignments if preserving them
            absence_assignments = []
            if preserve_absence:
            ...
            ***REMOVED*** Step 1.5d: Load off-site assignments (Hilo, Kapiolani, Okinawa)
            ...
            ***REMOVED*** Step 1.5e: Load recovery assignments (Post-Call)
            ***REMOVED*** Note: PC is also enforced by NightFloatPostCallConstraint
            ...
            ***REMOVED*** Step 1.5f: Load education assignments (FMO, GME, Lectures)
            ...
            ***REMOVED*** NOTE: Deletion deferred until after successful solve (see Step 5.5)
            ***REMOVED*** This prevents data loss if the solver fails

            ***REMOVED*** Step 2: Load absences and build availability matrix
            self._build_availability_matrix()

            ***REMOVED*** Step 3: Get residents, faculty, and templates
            ...
            ***REMOVED*** Step 4: Create scheduling context (with resilience data if available)
            ***REMOVED*** Combine all preserved assignments to pass to context as immutable
            ...
            ***REMOVED*** Step 4.5: Pre-solver validation
            ***REMOVED*** Check constraint saturation before invoking expensive solver
            ...
            if not validation_result.feasible:
                ***REMOVED*** Problem is obviously infeasible - fail fast
                logger.error(
            ...
            ***REMOVED*** Log pre-solver validation results
            logger.info(
            ...
            ***REMOVED*** Step 5: Run solver
            ...
            if not solver_result.success:
                ...
                ***REMOVED*** Fallback to greedy if advanced solver fails
                if algorithm != "greedy":
                    logger.info("Falling back to greedy solver")
                    solver_result = self._run_solver("greedy", context, timeout_seconds)

            ***REMOVED*** Step 5.5: Delete existing assignments (except preserved ones)
            ***REMOVED*** This happens AFTER successful solve to prevent data loss on solver failure
            self._delete_existing_assignments(preserve_ids)

            ***REMOVED*** Step 6: Convert solver results to assignments
            ***REMOVED*** Pass preserved_assignments to filter out conflicts with immutable slots
            self._create_assignments_from_result(
                solver_result, residents, templates, run.id, preserved_assignments
            )

            ***REMOVED*** Step 6.5: Create call assignments from solver results
            ***REMOVED*** Creates CallAssignment records for overnight call (Sun-Thurs)
            call_assignments = self._create_call_assignments_from_result(
                solver_result, context
            )

            ***REMOVED*** Step 7: Assign faculty supervision
            ***REMOVED*** Pass preserved_assignments so faculty with FMIT/absences are excluded
            self._assign_faculty(faculty, blocks, run.id, preserved_assignments)

            ***REMOVED*** Step 8: Add assignments to session (but don't commit yet)
            for assignment in self.assignments:
                self.db.add(assignment)

            ***REMOVED*** Step 9: Validate
            validation = self.validator.validate_all(self.start_date, self.end_date)

            ***REMOVED*** Step 9.5: Run NF -> PC audit and surface violations
            ...
            ***REMOVED*** Step 10: Update run record with results
            runtime = time.time() - start_time
            self._update_run_with_results(
                run, algorithm, validation, runtime, solver_result
            )

            ***REMOVED*** Issue ***REMOVED***3: Single atomic commit - all or nothing
            self.db.commit()
            self.db.refresh(run)

            ***REMOVED*** Post-generation resilience check
            ...
            return {
                ...
                "resilience": {
                    ...
                    ***REMOVED*** New: Include resilience constraint activity
            ...
            ***REMOVED*** Issue ***REMOVED***3: Rollback on any failure to prevent partial persistence
            logger.error(f"Schedule generation failed: {e}")
            self.db.rollback()

            ***REMOVED*** Try to update run status to failed (in a new transaction)
            try:
                self.db.refresh(run)
                self._update_run_status(run, "failed", 0, 0, time.time() - start_time)
                self.db.commit()
    ...
    def _build_context(
        ...
        """
        ***REMOVED*** Get call-eligible faculty (excludes adjuncts)
        call_eligible = self._get_call_eligible_faculty(faculty)

        ***REMOVED*** Build base context
        ...
        ***REMOVED*** Populate resilience data if available and requested
    ...
    def _populate_resilience_data(
        ...
        try:
            ***REMOVED*** =================================================================
            ***REMOVED*** TIER 1: Critical resilience data
            ***REMOVED*** =================================================================

            ***REMOVED*** Get hub scores
            ...
            if hub_scores:
                ...
                ***REMOVED*** Enable hub protection constraint if we have data
                if self.constraint_manager:
                    self.constraint_manager.enable("HubProtection")

            ***REMOVED*** Get current utilization from pre-generation check
            if hasattr(self, "_pre_health_report") and self._pre_health_report:
                ...
                ***REMOVED*** Enable utilization buffer constraint
                if self.constraint_manager:
                    self.constraint_manager.enable("UtilizationBuffer")

            ***REMOVED*** =================================================================
            ***REMOVED*** TIER 2: Strategic resilience data
            ***REMOVED*** =================================================================

            ***REMOVED*** Get N-1 vulnerable faculty from contingency analysis
            ...
            ***REMOVED*** Always enable N1 constraint - it can work without pre-identified faculty
            ***REMOVED*** by analyzing the solution for single points of failure
            if self.constraint_manager:
                self.constraint_manager.enable("N1Vulnerability")

            ***REMOVED*** Get preference trails from stigmergy
            ...
            if preference_trails:
                ...
                ***REMOVED*** Enable preference trail constraint
                if self.constraint_manager:
                    self.constraint_manager.enable("PreferenceTrail")

            ***REMOVED*** Get zone assignments from blast radius isolation
            ...
            if zone_data:
                ...
                ***REMOVED*** Enable zone boundary constraint
                if self.constraint_manager:
                    self.constraint_manager.enable("ZoneBoundary")

        except Exception as e:
            logger.warning(f"Failed to populate resilience data: {e}")
            ***REMOVED*** Continue without resilience data - constraints will be no-ops
    ...
    def _get_hub_scores(self, faculty: list[Person]) -> dict[UUID, float]:
        ...
        try:
            ***REMOVED*** Check if we have cached hub analysis results
            if hasattr(self.resilience, "hub_analyzer"):
                ***REMOVED*** Get latest centrality data
                for fac in faculty:
    ...
    def _get_n1_vulnerable_faculty(
        ...
        try:
            ***REMOVED*** Use contingency analyzer if available
            if hasattr(self.resilience, "contingency"):
                ***REMOVED*** This would need existing assignments to analyze
                ***REMOVED*** For now, return empty - will be populated after solving
                pass
        except Exception as e:
    ...
    def _get_zone_assignments(
        ...
        try:
            if hasattr(self.resilience, "blast_radius"):
                ...
                ***REMOVED*** Get faculty zone assignments
                for fac in faculty:
                    zone = blast_radius.get_faculty_zone(fac.id)
                    if zone:
                        zone_data["faculty_zones"][fac.id] = zone.id

                ***REMOVED*** Get block zone assignments (from rotation template or service type)
                for block in blocks:
                    ***REMOVED*** Blocks may be associated with zones through their rotation template
                    ***REMOVED*** or through explicit zone assignment
        ...
        ***REMOVED*** Only return if we have meaningful data
    ...
    def _run_solver(
        ...
        try:
            ...
            ***REMOVED*** Pass existing_assignments from context as immutable constraints
    ...
    def _create_assignments_from_result(
        ...
        """
        ***REMOVED*** Build set of (person_id, block_id) pairs that are already assigned
        ...
        for person_id, block_id, template_id in result.assignments:
            ***REMOVED*** Skip if this person+block is already occupied by an immutable assignment
    ...
    def _create_call_assignments_from_result(
        ...
        ***REMOVED*** Build block lookup for date extraction
        ...
        for person_id, block_id, call_type in result.call_assignments:
            ...
            call_assignment = CallAssignment(
                ...
                person_id=person_id,
                call_type=call_type,
                is_weekend=(block.date.weekday() == 6),  ***REMOVED*** Sunday
                is_holiday=False,  ***REMOVED*** Could be enhanced to check holiday calendar
            )
    ...
    def _ensure_blocks_exist(self, commit: bool = True) -> list[Block]:
        ...
        if commit:
            ...
            self.db.flush()  ***REMOVED*** Make blocks available in the session without committing
    ...
    def _build_availability_matrix(self):
        ...
        Example:
            >>> ***REMOVED*** After building availability matrix:
            >>> is_available = self.availability_matrix[resident_id][block_id]["available"]
            >>> if is_available:
            ...     ***REMOVED*** Can assign resident to this block
        """
        ***REMOVED*** Get all people (no eager loading needed - only accessing person.id)
                ...
                ***REMOVED*** Check absences
                for absence in absences:
                    if (
                        ...
                        ***REMOVED*** Use the should_block_assignment property to determine
                        if absence.should_block_assignment:
                            ***REMOVED*** Blocking absence - person cannot be assigned
                            is_available = False
                            replacement_activity = absence.replacement_activity
                            break
                        else:
                            ***REMOVED*** Partial absence - person can be assigned but we track it
                            has_partial_absence = True
                            replacement_activity = absence.replacement_activity
                            ***REMOVED*** Don't break - keep checking for blocking absences
    ...
    def _assign_faculty(
        ...
        """
        ***REMOVED*** Build set of (faculty_id, block_id) pairs that are already assigned
        ***REMOVED*** Faculty with FMIT/absences OR solver-assigned cannot supervise clinic
        faculty_occupied_slots: set[tuple[UUID, UUID]] = set()
        faculty_ids = {f.id for f in faculty}

        ***REMOVED*** Check preserved assignments (FMIT, absences, etc.)
        ...
        ***REMOVED*** Also check assignments created by solver in this session
        ...
        ***REMOVED*** Pre-fetch all residents who have assignments (N+1 fix)
        ***REMOVED*** This replaces the per-block query inside the loop
        ...
        ***REMOVED*** Pre-fetch all rotation templates used in assignments (N+1 fix)
        ***REMOVED*** This is used by _get_primary_template_for_block to break ties
        all_template_ids = {
        ...
        ***REMOVED*** Assign faculty to each block
        ...
        for block_id, block_assignments in assignments_by_block.items():
            ***REMOVED*** Get resident details for this block from pre-fetched data
            ...
            ***REMOVED*** Calculate supervision using fractional load approach
            ***REMOVED*** PGY-1: 2 units (0.5 load = 1:2 ratio), PGY-2/3: 1 unit (0.25 load = 1:4 ratio)
            ***REMOVED*** Sum loads THEN ceiling (4 units = 1 faculty)
            ...
            ***REMOVED*** Find available faculty (not on leave AND not already assigned to this block)
            available = [
                f
                for f in faculty
            ...
            ***REMOVED*** Determine primary rotation template from resident assignments in this block
            ***REMOVED*** Faculty should be assigned the same rotation as the residents they supervise
    ...
    def _get_primary_template_for_block(
        ...
        ***REMOVED*** Count template occurrences
        ...
        ***REMOVED*** Find most common template
        ***REMOVED*** If there's a tie, we'll prefer clinical templates (inpatient, etc.)
        ...
        ***REMOVED*** Break ties by preferring inpatient templates
        ***REMOVED*** Use cache if available to avoid N+1 queries
        for tid in candidates:
            if templates_cache is not None:
                template = templates_cache.get(tid)
            else:
                ***REMOVED*** Fallback to database query (backward compatibility)
                template = (
                    self.db.query(RotationTemplate)
        ...
        ***REMOVED*** Default to first candidate
        return candidates[0]
    ...
    def _delete_existing_assignments(self, preserve_ids: set[UUID] | None = None):
        ...
        ***REMOVED*** Get all block IDs in the date range with row-level lock
        ***REMOVED*** The FOR UPDATE lock ensures exclusive access to these blocks
        ***REMOVED*** during the transaction, preventing concurrent generations from
        ***REMOVED*** creating conflicting assignments
        blocks = (
            self.db.query(Block)
            .filter(
                Block.date >= self.start_date,
            ...
            .with_for_update(nowait=False)  ***REMOVED*** Wait for lock rather than fail immediately
            .all()
        )
        block_ids = [block.id for block in blocks]

        if block_ids:
            ***REMOVED*** Lock and delete existing assignments for these blocks
            ***REMOVED*** We select first to acquire locks, then delete
            ...
            for assignment in existing_assignments:
                if assignment.id in preserve_ids:
                    preserved_count += 1
                    continue  ***REMOVED*** Skip preserved assignments (e.g., FMIT)
    ...
    def _audit_nf_pc_allocations(self) -> dict:
        ...
        Blindspot ***REMOVED***6: Post-generation audit for NF->PC allocations.
        ...
        ***REMOVED*** Get all assignments grouped by person and block
        ...
        ***REMOVED*** Get NF and PC template IDs
        nf_template = (
            self.db.query(RotationTemplate)
        ...
        ***REMOVED*** Pre-fetch all blocks in the date range (N+1 fix)
        ***REMOVED*** Build lookups by ID and by date for efficient access
        all_blocks = (
            self.db.query(Block)
            .filter(
                Block.date >= self.start_date,
        ...
        ***REMOVED*** Pre-fetch all persons who have assignments (N+1 fix)
        ...
        ***REMOVED*** Check each person's NF assignments
        for person_id, person_assignments in assignments_by_person.items():
            ***REMOVED*** Find NF assignments
            nf_assignments = [
                a
            ...
            ***REMOVED*** Group NF assignments by date to find transition days
            ***REMOVED*** Use pre-fetched blocks instead of querying per assignment
            ...
            ***REMOVED*** For each NF date, check if next day has PC
            for nf_date in sorted(nf_by_date.keys()):
                pc_required_date = nf_date + timedelta(days=1)

                ***REMOVED*** Skip if outside schedule range
                if pc_required_date > self.end_date:
                    continue

                total_transitions += 1

                ***REMOVED*** Check for PC assignments on the required date
                ***REMOVED*** Use pre-fetched blocks instead of querying per date
                ...
                for block in pc_blocks:
                    ***REMOVED*** Check if person has PC assignment for this block
                    pc_assignment = next(
                        (
                            a
                            for a in person_assignments
                ...
                ***REMOVED*** Record violations
                if not has_am_pc or not has_pm_pc:
                    ***REMOVED*** Use pre-fetched person instead of querying per violation
    ...
    def _check_pre_generation_resilience(self):
        ...
        try:
            ...
            ***REMOVED*** Log warning if system is stressed
    ...
    def _check_post_generation_resilience(
        ...
        try:
            ...
            ***REMOVED*** Log if schedule creates concerning conditions
            if report.overall_status in ("critical", "emergency"):
                logger.warning(
    ...
    def _get_resilience_warnings(self, health_report) -> list[str]:
        ...
        ***REMOVED*** Phase transition warning
