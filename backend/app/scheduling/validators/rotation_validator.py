...
***REMOVED*** Rotation Constants
MIN_ROTATION_DAYS = 7  ***REMOVED*** Minimum ~1 week

***REMOVED*** PGY-Level Requirements (annual targets)
...
***REMOVED*** Continuity clinic (weekly ongoing patient relationships)
MIN_CONTINUITY_CLINIC_FREQUENCY = 2  ***REMOVED*** Blocks per month

***REMOVED*** Procedure volume targets per year
MIN_PROCEDURES_PER_YEAR = 50
MIN_PROCEDURES_PGY1 = 20
MIN_PROCEDURES_PGY2 = 30

***REMOVED*** Conference attendance
MIN_CONFERENCE_ATTENDANCE = 0.95  ***REMOVED*** 95%
...
class RotationWarning:
    ...
    person_id: UUID
    warning_type: str  ***REMOVED*** "approaching_end", "low_volume"
    message: str
    rotation_name: str
...
class RotationValidator:
    ...
    def validate_pgy_level_clinic_requirements(
        ...
        if pgy_level == 1:
            ...
            return None  ***REMOVED*** Unknown PGY level
    ...
    def validate_specialty_rotation_completion(
        ...
        Returns:
            RotationViolation if insufficient, None if adequate
        """
        ***REMOVED*** PGY-1 not typically required specialty blocks
        if pgy_level < 2:
            return None
    ...
    def validate_procedure_volume(
        ...
        """
        ***REMOVED*** Determine target based on PGY level
        ...
        ***REMOVED*** Critical threshold: less than 60% of target
        ...
        ***REMOVED*** Warning threshold: less than 85% of target
    ...
    def validate_rotation_sequence(
        ...
        ***REMOVED*** PGY-1 specific rules
        if pgy_level == 1:
            ***REMOVED*** FMIT should be early (first 6 months ideally)
            rotation_names = [r.get('rotation_name', '').upper()
                             for r in completed_rotations]

            ***REMOVED*** Check if FMIT appears late
            if 'FMIT' in rotation_names:
                fmit_index = rotation_names.index('FMIT')
                if fmit_index > 5:  ***REMOVED*** After 5 other rotations
                    violations.append(
                        RotationViolation(
                            person_id=person_id,
                            violation_type="missing_required",
        ...
        ***REMOVED*** PGY-2/3 rules
        if pgy_level >= 2:
            ***REMOVED*** Specialty rotations should be distributed (not clustered)
            rotation_names = [r.get('rotation_name', '').upper()
                             for r in completed_rotations]
            specialty_indices = [
            ...
            ***REMOVED*** Check for clustering (more than 2 consecutive specialty rotations)
    ...
    def get_annual_rotation_summary(
        ...
        return {
            ...
            'diversity_score': len(rotation_by_type),  ***REMOVED*** Number of distinct rotation types
        }
