...
***REMOVED*** ACGME Call Constants
MAX_CALL_FREQUENCY_DAYS = 3  ***REMOVED*** Every third night rule
ROLLING_WINDOW_WEEKS = 4  ***REMOVED*** 4-week averaging period
ROLLING_WINDOW_DAYS = 28
MAX_CONSECUTIVE_NIGHTS = 2  ***REMOVED*** Clinical guideline (stricter than 1-in-7)
MIN_CALL_SPACING_DAYS = 2  ***REMOVED*** Minimum days between calls
POST_CALL_MANDATORY_HOURS = 10  ***REMOVED*** Minimum rest after overnight call
...
class CallViolation:
    ...
    violation_count: int  ***REMOVED*** How many calls violate
...
class CallWarning:
    ...
    person_id: UUID
    warning_type: str  ***REMOVED*** "approaching_limit", "imbalance"
    message: str
    call_count: int
    equity_metric: float  ***REMOVED*** Standard deviation from mean
...
class CallValidator:
        ...
        ***REMOVED*** Check every possible 28-day rolling window
        for i, start_date in enumerate(sorted_dates):
            ...
            ***REMOVED*** Check for violation (>10 calls in 28 days)
            ...
            elif call_count > self.max_calls_per_window * 0.9:  ***REMOVED*** Approaching limit
                warnings.append(
                    CallWarning(
                        person_id=person_id,
    ...
    def validate_consecutive_nights(
        ...
        ***REMOVED*** Find consecutive sequences
        ...
        for i in range(1, len(sorted_dates)):
            ...
            ***REMOVED*** Check if consecutive
            if (current_date - previous_date).days == 1:
                consecutive_sequence.append(current_date)
            else:
                ***REMOVED*** End of sequence, check length
    ...
    def validate_call_equity(
        ...
        imbalance_ratio = max(counts) / (min(counts) + 0.1)  ***REMOVED*** Avoid division by zero

        ***REMOVED*** Identify over/under-assigned
        over_assigned = [
        ...
        ***REMOVED*** Create warnings for imbalance
        if imbalance_ratio > 1.5:  ***REMOVED*** More than 50% difference
            warnings.append(
                CallWarning(
