...
CONDITIONAL_BLOCKING_LEAVE_TYPES = {
    'medical': 7,  ***REMOVED*** Block if > 7 days
    'sick': 3,     ***REMOVED*** Block if > 3 days
}
...
class LeaveWarning:
    ...
    person_id: UUID
    warning_type: str  ***REMOVED*** "tentative_return", "approaching_end"
    message: str
    absence_id: UUID
...
class LeaveValidator:
    ...
    def should_block_assignment(
        ...
        ***REMOVED*** Non-blocking types
        if absence_type in self.non_blocking_types:
            return False

        ***REMOVED*** Default: block unknown types (conservative)
        return True
    ...
    def validate_no_assignment_during_block(
        ...
        """
        ***REMOVED*** Determine if should block
        if not self.should_block_assignment(
            absence_type, start_date, end_date, is_blocking
        ):
            return None  ***REMOVED*** Non-blocking, no violation possible

        ***REMOVED*** Check for assignments during absence
        conflict_dates = [
        ...
        if not conflict_dates:
            return None  ***REMOVED*** No conflicts

        ***REMOVED*** Create violation
        message = (
    ...
    def validate_sick_leave_compliance(
        ...
        ***REMOVED*** Short sick leave (<= 3 days) should not block
        if duration <= 3:
            if days_blocked:
                return LeaveViolation(
                    person_id=person_id,
        ...
        ***REMOVED*** Extended sick leave (>3 days) should block
        ***REMOVED*** No violation possible here (blocking is expected)
        return None
    ...
    def validate_medical_leave_compliance(
        ...
        """
        ***REMOVED*** If medically cleared before end_date, should reduce absence duration
    ...
    def validate_tdy_deployment_compliance(
        ...
        """
        ***REMOVED*** TDY requires destination
        ...
        ***REMOVED*** Deployment should have orders
    ...
    def validate_tentative_return_date(
        ...
        if not return_date_tentative:
            return None  ***REMOVED*** Return date confirmed

        days_until_return = (return_date - today).days

        if days_until_return <= 7:  ***REMOVED*** Return approaching
            return LeaveWarning(
                person_id=person_id,
    ...
    def validate_convalescent_leave_recovery(
        ...
        ***REMOVED*** Common minimum recovery times (in days)
        minimum_recovery = {
            'appendectomy': 14,
            'acl_repair': 42,
    ...
    def get_leave_impact_summary(
        ...
        work_capacity_hours = (schedule_period_days - blocking_days) * 12  ***REMOVED*** 12h/day
        work_hours_limit = 80 * 4  ***REMOVED*** 80h/week × 4 weeks
