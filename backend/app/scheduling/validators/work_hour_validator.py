...
***REMOVED*** ACGME Work Hour Constants
MAX_WEEKLY_HOURS = 80
ROLLING_WEEKS = 4
ROLLING_DAYS = 28  ***REMOVED*** Exact 28-day window, not 4 calendar weeks
HOURS_PER_BLOCK = 6  ***REMOVED*** Standard block duration (AM/PM sessions)
...
MOONLIGHTING_HOURS_PER_WEEK_WARNING = 20  ***REMOVED*** Alert if exceeding this
...
class WorkHourViolation:
    ...
    hours: float
    limit: float
    violation_percentage: float = 0.0  ***REMOVED*** (hours - limit) / limit * 100
...
class WorkHourValidator:
    ...
    def validate_80_hour_rolling_average(
        ...
        ***REMOVED*** Integrate moonlighting hours
        ...
        ***REMOVED*** Check every possible 28-day rolling window
        for start_date in sorted_dates:
            window_end = start_date + timedelta(days=self.rolling_days - 1)

            ***REMOVED*** Calculate total hours in window
            total_hours = sum(
                hours
            ...
            ***REMOVED*** Calculate average weekly hours
            ...
            ***REMOVED*** Check for violation
            ...
            elif average_weekly > self.max_weekly_hours * 0.95:  ***REMOVED*** 76+ hours
                warnings.append(
                    WorkHourWarning(
                        person_id=person_id,
    ...
    def validate_24_plus_4_rule(
        ...
        ***REMOVED*** Group shifts by approximate continuous period
        if not shift_data:
            return violations, warnings
        ...
        for i, shift in enumerate(sorted_shifts):
            ...
            if duration > MAX_CONSECUTIVE_HOURS:
                ***REMOVED*** Check if 24+4 exception applies
                if duration <= MAX_TOTAL_SHIFT_HOURS:
                    ***REMOVED*** 24+4 rule allows up to 28 hours
                    if duration > 26:  ***REMOVED*** Close to limit
                        warnings.append(
                            WorkHourWarning(
                                person_id=person_id,
    ...
    def validate_rest_period(
        ...
        for i in range(len(sorted_shifts) - 1):
            ...
            if current_duration < MAX_CONSECUTIVE_HOURS:
                continue  ***REMOVED*** No rest requirement after short shifts

            ***REMOVED*** Calculate rest period
            ***REMOVED*** Simplified: assumes shifts on consecutive days
            rest_hours = 10  ***REMOVED*** Placeholder for actual calculation
    ...
    def validate_moonlighting_integration(
        ...
        ***REMOVED*** Calculate total moonlighting per week
        weeks_data = {}
        for work_date, hours in moonlighting_hours.items():
            ***REMOVED*** Calculate week starting Monday
        ...
        ***REMOVED*** Check weekly moonlighting
    ...
    def create_violation_notification_level(self, hours: float) -> str | None:
        ...
        elif hours >= self.max_weekly_hours * 0.975:  ***REMOVED*** 78 hours
            return "orange"  ***REMOVED*** Critical warning
        elif hours >= self.max_weekly_hours * 0.9375:  ***REMOVED*** 75 hours
    ...
    def check_exemption_eligibility(self, person_id: UUID, violation_type: str) -> bool:
        ...
        """
        ***REMOVED*** Exemptions are rare and require special approval
        ***REMOVED*** This is a placeholder for institutional policy
        if violation_type == "24_plus_4":
            ***REMOVED*** Continuity of care exception possible with documentation
            return True
        return False
...
class BlockBasedWorkHourCalculator:
    ...
    HOURS_PER_BLOCK = 6  ***REMOVED*** Standard assumption
    HOURS_PER_INTENSIVE_BLOCK = 12  ***REMOVED*** FMIT, Night Float, inpatient
    ...
    def calculate_hours_from_assignments(
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Determine hours based on rotation intensity
