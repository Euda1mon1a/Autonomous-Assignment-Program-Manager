...
class TensegritySolver:
    ...
    Example:
        ...
        >>> print(solution)  ***REMOVED*** {'shift_start': 8.0, 'task_a': 8.5, ...}
    """
    ...
    def build_force_density_matrix(self) -> sparse.csr_matrix:
        ...
        Example:
            ...
            >>> print(F.shape)  ***REMOVED*** (n_nodes, n_nodes)
        """
        n = len(self.nodes)
        node_ids = list(self.nodes.keys())
        node_index = {node_id: i for i, node_id in enumerate(node_ids)}

        ***REMOVED*** Build sparse matrix using LIL format for efficient construction
        ...
        for node1, node2, q, edge_type in self.edges:
            ...
            ***REMOVED*** For compression elements, we negate the force density
            ***REMOVED*** This creates repulsive forces that push nodes apart
            q_effective = -q if edge_type == "compression" else q

            ***REMOVED*** Off-diagonal: -q_ij
            F[i, j] -= q_effective
            F[j, i] -= q_effective

            ***REMOVED*** Diagonal: sum of incident force densities
            F[i, i] += q_effective
            F[j, j] += q_effective

        ***REMOVED*** Convert to CSR format for efficient solving
        return F.tocsr()
    ...
    def solve(self) -> dict[str, float]:
        ...
        Example:
            ...
            >>> solution = solver.solve()
            >>> print(solution['task'])  ***REMOVED*** Should be between 8.0 and 17.0
        """
        if not self.nodes:
            raise ValueError("No nodes in structure")

        ***REMOVED*** Build full force density matrix
        F = self.build_force_density_matrix()

        ***REMOVED*** Separate free and anchor nodes
        node_ids = list(self.nodes.keys())
        free_indices = [
        ...
        ***REMOVED*** If no free nodes, return anchor positions
        if not free_indices:
            return {nid: self.nodes[nid]["position"] for nid in node_ids}

        ***REMOVED*** Extract submatrices
        ***REMOVED*** F_ff: free-to-free coupling
        F_ff = F[np.ix_(free_indices, free_indices)]

        ***REMOVED*** Build right-hand side
        if anchor_indices:
            ***REMOVED*** F_fa: free-to-anchor coupling
            F_fa = F[np.ix_(free_indices, anchor_indices)]

            ***REMOVED*** x_a: anchor positions
            x_a = np.array(
                [self.nodes[node_ids[i]]["position"] for i in anchor_indices]
            )

            ***REMOVED*** p_f = -F_fa · x_a
            p_f = -F_fa.dot(x_a)
        else:
            ***REMOVED*** No anchors, use initial positions as weak constraints
            p_f = np.zeros(len(free_indices))

        ***REMOVED*** Solve F_ff · x_f = p_f
        try:
            ***REMOVED*** Use sparse solver for efficiency
            x_f = sp_linalg.spsolve(F_ff, p_f)
        except RuntimeError as e:
            raise RuntimeError(f"Failed to solve linear system: {e}") from e

        ***REMOVED*** Combine free and anchor solutions
        solution = {}
    ...
    def is_stable(self, solution: dict[str, float] | None = None) -> bool:
        ...
        ***REMOVED*** Check for NaN or infinite values
        for pos in solution.values():
            if not np.isfinite(pos):
                return False

        ***REMOVED*** Check for negative times (invalid)
        if any(pos < 0 for pos in solution.values()):
            return False

        ***REMOVED*** Check causality for tension elements (dependencies)
        ***REMOVED*** Tension should maintain order constraints
        for node1, node2, q, edge_type in self.edges:
            if edge_type == "tension" and q > 0:
                ***REMOVED*** For high tension, expect nodes to be close
                ***REMOVED*** This is a soft check - just verify positions are reasonable
                pos1 = solution[node1]
                pos2 = solution[node2]
                ***REMOVED*** Allow reasonable spread (e.g., within 24 hours for scheduling)
    ...
    def get_internal_forces(
        ...
        for node1, node2, q, edge_type in self.edges:
            ...
            ***REMOVED*** F = q · (x2 - x1)
            ***REMOVED*** For compression elements, q was already negated in the matrix
            q_effective = -q if edge_type == "compression" else q
    ...
    def visualize(self, solution: dict[str, float]) -> str:
        ...
        ***REMOVED*** Sort nodes by position
        ...
        ***REMOVED*** Timeline visualization (simplified)
