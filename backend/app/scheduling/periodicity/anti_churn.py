...
Usage:
    ***REMOVED*** Compare current and proposed schedules
    rigidity = calculate_schedule_rigidity(new_schedule, current_schedule)
    ***REMOVED*** rigidity = 0.92 means 92% of assignments unchanged

    ***REMOVED*** Use as optimization objective
    score = time_crystal_objective(
        new_schedule,
        current_schedule,
        constraints,
        alpha=0.3  ***REMOVED*** 30% weight on stability, 70% on constraints
    )
...
def hamming_distance(
    ...
    Example:
        ...
        ...     (person1, block1, template_clinic),      ***REMOVED*** Same
        ...     (person2, block2, template_elective)     ***REMOVED*** Changed template
        ... ])
        >>> hamming_distance(current, proposed)
        1  ***REMOVED*** Only one assignment changed
    """
    ***REMOVED*** Convert to sets for symmetric difference
    set_a = schedule_a.assignments
    set_b = schedule_b.assignments

    ***REMOVED*** Symmetric difference gives all positions that differ
    ***REMOVED*** (assignments in A but not B) ∪ (assignments in B but not A)
...
def hamming_distance_by_person(
    ...
    Example:
        ...
        >>> ***REMOVED*** {person1: 0, person2: 3, person3: 1}
        >>> ***REMOVED*** person2 had 3 assignment changes - highest churn
    """
    ...
    ***REMOVED*** Calculate per-person differences
...
def calculate_schedule_rigidity(
    ...
    Example:
        ...
        >>> ***REMOVED*** "Schedule changed significantly (rigidity=0.65)"
    """
    ***REMOVED*** Calculate Hamming distance
    distance = hamming_distance(new_schedule, current_schedule)

    ***REMOVED*** Normalize by maximum possible distance
    ***REMOVED*** Maximum distance occurs when schedules share no assignments
    max_distance = len(new_schedule.assignments) + len(current_schedule.assignments)

    if max_distance == 0:
        ***REMOVED*** Both schedules empty - perfect rigidity
        return 1.0

    ***REMOVED*** Rigidity = 1 - normalized_distance
...
def time_crystal_objective(
    ...
    Example:
        ...
        ...     ConstraintResult(satisfied=True, penalty=0.1),  ***REMOVED*** Soft violation
        ...     ConstraintResult(satisfied=False, penalty=10.0) ***REMOVED*** Hard violation
        ... ]
        >>> score = time_crystal_objective(new, current, constraint_results, alpha=0.3)
        >>> ***REMOVED*** Lower score due to hard constraint violation and churn
    """
    if alpha + beta > 1.0:
        raise ValueError(
    ...
    ***REMOVED*** 1. Constraint satisfaction score
    ***REMOVED*** Count hard constraint violations (must be zero for valid schedule)
    hard_violations = sum(1 for r in constraint_results if not r.satisfied)

    ***REMOVED*** Sum soft constraint penalties
    total_penalty = sum(r.penalty for r in constraint_results)

    ***REMOVED*** Normalize constraint score
    ***REMOVED*** Perfect score = 1.0 (no violations, no penalties)
    ***REMOVED*** Each hard violation subtracts 0.5
    ***REMOVED*** Penalties normalized by reasonable maximum (10.0)
    max_penalty = 10.0
    constraint_score = max(
        0.0, 1.0 - (hard_violations * 0.5) - min(total_penalty / max_penalty, 0.5)
    )

    ***REMOVED*** 2. Rigidity score (anti-churn)
    rigidity_score = calculate_schedule_rigidity(new_schedule, current_schedule)

    ***REMOVED*** 3. Fairness score (churn distribution)
    ***REMOVED*** Churn should be evenly distributed, not concentrated on a few people
    ...
    if len(churn_by_person) > 0:
        ...
        ***REMOVED*** Fairness = 1 - (coefficient_of_variation)
        ***REMOVED*** CV = std / mean, measures relative variability
        ***REMOVED*** Low CV = fair distribution, High CV = concentrated churn
        if mean_churn > 0:
            cv = std_churn / mean_churn
            fairness_score = max(0.0, 1.0 - min(cv, 1.0))
        else:
            ***REMOVED*** No churn = perfect fairness
            fairness_score = 1.0
    else:
        fairness_score = 1.0

    ***REMOVED*** 4. Combined objective
    ***REMOVED*** Weights: (1-α-β) for constraints, α for rigidity, β for fairness
    objective = (
...
def estimate_churn_impact(
    ...
    Example:
        ...
        >>> ***REMOVED*** Impact: moderate
        >>> ***REMOVED*** 8 residents affected
    """
...
def detect_periodic_patterns(
    ...
    Example:
        ...
        >>> ***REMOVED*** [7, 14, 28]  ***REMOVED*** Found weekly, bi-weekly, and 4-week cycles
        >>> logger.info(f"Detected natural cycles: {periods} days")
    """
    if not assignments:
        return []

    ***REMOVED*** Build time series of assignment counts per day
    ...
    ***REMOVED*** Count assignments for each day
    for assignment in assignments:
        if assignment.block:
            assignment_counts_by_day[assignment.block.date] += 1

    ***REMOVED*** Create time series array
    ...
    if len(time_series) < base_period_days * 2:
        ***REMOVED*** Not enough data for meaningful autocorrelation
        return []
    ...
    ***REMOVED*** Find peaks that are multiples of base period
    detected_periods = []

    ***REMOVED*** Check multiples of base period (1x, 2x, 3x, 4x)
    for multiplier in range(1, 5):
        period = base_period_days * multiplier
        if period < len(autocorr):
            ***REMOVED*** Check autocorrelation at this lag
