***REMOVED*** Time Crystal Anti-Churn Integration Guide
...
***REMOVED*** Get detailed impact analysis
...
***REMOVED******REMOVED******REMOVED*** 2. Integration with Existing Solvers

***REMOVED******REMOVED******REMOVED******REMOVED*** Option A: Post-Solver Evaluation (Simplest)

Add rigidity checking after solver runs but before committing to database:

```python
***REMOVED*** In engine.py, after solver.solve()
...
def generate(self, algorithm="cp_sat", max_churn_severity="moderate", **kwargs):
    ...
    ***REMOVED*** Run solver
    ...
    if result.success:
        ***REMOVED*** NEW: Check churn impact before accepting
        ...
        ***REMOVED*** Reject if churn is too high
    ...
    ***REMOVED*** ... continue with existing code ...
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Option B: Integrate into Solver Objective (Advanced)

Modify solvers to optimize for both constraints AND rigidity:

```python
***REMOVED*** In solvers.py - CP-SAT example

def solve(self, context: SchedulingContext, existing_assignments: list[Assignment] = None) -> SolverResult:
    ***REMOVED*** ... existing setup code ...

    ***REMOVED*** NEW: Build current assignment tracking
    ...
    ***REMOVED*** ... existing variable creation ...

    ***REMOVED*** OBJECTIVE FUNCTION (Modified)
    ***REMOVED*** Add anti-churn penalty term

    churn_penalty_vars = []
    for person in context.residents:
        for block in context.blocks:
            for template in context.templates:
                ***REMOVED*** Check if this differs from current assignment
                current_template = current_assignment_map.get(
                    (person.id, block.id), None
                )

                if current_template is not None:
                    ***REMOVED*** This block was already assigned
                    if current_template != template.id:
                        ***REMOVED*** Different template = churn
                        churn_indicator = model.NewBoolVar(
                            f"churn_{person.id}_{block.id}_{template.id}"
                        )
                        ***REMOVED*** churn_indicator = 1 if this assignment differs from current
                        model.Add(churn_indicator == x[(person.id, block.id, template.id)])
                        churn_penalty_vars.append(churn_indicator)

    ***REMOVED*** Original objective terms
    coverage_penalty = ... ***REMOVED*** existing
    equity_penalty = ... ***REMOVED*** existing
    template_balance_penalty = ... ***REMOVED*** existing

    ***REMOVED*** NEW: Churn penalty
    churn_penalty = sum(churn_penalty_vars)

    ***REMOVED*** Weighted objective
    ***REMOVED*** alpha = 0.3 means 30% weight on minimizing churn
    alpha = 0.3  ***REMOVED*** Make this configurable
    model.Minimize(
        int((1 - alpha) * (coverage_penalty + equity_penalty + template_balance_penalty))
        + int(alpha * 100 * churn_penalty)  ***REMOVED*** Scale churn penalty
    )

    ***REMOVED*** ... rest of solver ...
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Option C: Constraint-Based Churn Limit (Recommended)

Add a soft constraint that penalizes excessive churn:

```python
***REMOVED*** In constraints/base.py or constraints/stability.py (new file)
...
class AntiChurnConstraint(SoftConstraint):
    ...
    def __init__(
        ...
        ***REMOVED*** Build lookup: (person_id, block_id) -> template_id
        self.current_map = {
    ...
    def add_to_cpsat(self, model, x, context: SchedulingContext) -> None:
                ...
                if current_template is not None:
                    for template in context.templates:
                        if template.id != current_template:
                            ***REMOVED*** This would be a change
                            var_name = f"{person.id}_{block.id}_{template.id}"
                            if var_name in x:
                                churn_indicators.append(x[var_name])

            ***REMOVED*** churn_count = sum of all changes for this person
            model.Add(churn_count == sum(churn_indicators))
            churn_by_person[person.id] = churn_count

            ***REMOVED*** Soft penalty: prefer to stay under max_churn_per_person
            ***REMOVED*** This will be added to the objective function
        ...
        return churn_by_person  ***REMOVED*** Return for use in objective
    ...
    def validate(
        ...
        ***REMOVED*** Calculate penalty
        ...
        return ConstraintResult(
            satisfied=True,  ***REMOVED*** Soft constraint, never fails
...
```python
***REMOVED*** In constraints/manager.py
...
def create_anti_churn_manager(
    ...
    ***REMOVED*** Add anti-churn constraint
...
```bash
***REMOVED*** Time Crystal Anti-Churn Settings
SCHEDULE_ANTI_CHURN_ENABLED=true
SCHEDULE_ANTI_CHURN_ALPHA=0.3        ***REMOVED*** Rigidity weight (0.0-1.0)
SCHEDULE_ANTI_CHURN_MAX_PER_PERSON=5 ***REMOVED*** Max changes per person
SCHEDULE_MAX_CHURN_SEVERITY=moderate ***REMOVED*** minimal|low|moderate|high|critical
```

***REMOVED******REMOVED******REMOVED*** Engine Configuration

```python
***REMOVED*** In engine.py
...
def generate(
    ...
    **kwargs
    ...
    """
    ***REMOVED*** ... implementation using options above ...
```
...
```python
***REMOVED*** In analytics or monitoring service
...
def track_schedule_generation(
    ...
    ***REMOVED*** Store in schedule_run metadata
    ...
    ***REMOVED*** Emit metrics for Prometheus/Grafana
...
```python
***REMOVED*** tests/integration/test_anti_churn_integration.py
...
async def test_schedule_regeneration_with_anti_churn(db, sample_schedule):
    ...
    ***REMOVED*** Regenerate with anti-churn enabled
    engine = SchedulingEngine(db, start_date, end_date)
    result = await engine.generate(
        algorithm="cp_sat",
        enable_anti_churn=True,
        anti_churn_alpha=0.5,  ***REMOVED*** High stability preference
    )

    ***REMOVED*** Check rigidity
...
```python
***REMOVED*** load-tests/scenarios/anti-churn-regeneration.js
...
***REMOVED******REMOVED******REMOVED*** Issue: Rigidity too high (solver can't improve schedule)

**Solution**: Lower alpha value or disable anti-churn temporarily

```python
engine.generate(algorithm="cp_sat", anti_churn_alpha=0.1)  ***REMOVED*** Lower weight
```

***REMOVED******REMOVED******REMOVED*** Issue: Rigidity too low (too much churn)
...
engine.generate(
    algorithm="cp_sat",
    anti_churn_alpha=0.5,  ***REMOVED*** Higher weight
    max_churn_per_person=3,  ***REMOVED*** Stricter limit
)
```

***REMOVED******REMOVED******REMOVED*** Issue: Solver timeout with anti-churn enabled

**Solution**: Anti-churn adds variables - increase timeout or use hybrid solver

```python
engine.generate(
    algorithm="hybrid",  ***REMOVED*** Use hybrid for complex problems
    timeout_seconds=120,  ***REMOVED*** Increase timeout
)
```
...
- [SYNERGY_ANALYSIS.md Section 11](../../../docs/SYNERGY_ANALYSIS.md***REMOVED***11-time-crystal-dynamics-for-schedule-stability)
