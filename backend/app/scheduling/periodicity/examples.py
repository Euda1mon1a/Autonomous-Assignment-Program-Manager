...
def example_basic_rigidity_check():
    ...
    """
    ***REMOVED*** Current schedule (from database)
    current_assignments = [
        Assignment(
            person_id=uuid4(),
            block_id=uuid4(),
    ...
    ***REMOVED*** Proposed schedule (from solver)
    proposed_tuples = [
        (uuid4(), uuid4(), uuid4()),  ***REMOVED*** (person_id, block_id, template_id)
        ***REMOVED*** ... more assignments
    ]
...
def example_per_person_churn():
    ...
    ***REMOVED*** Identify high-churn residents
...
def example_tuning_alpha():
    ...
    ***REMOVED*** Constraints: one soft violation
    constraints = [
        ...
        ConstraintResult(satisfied=True, penalty=1.5),  ***REMOVED*** Soft penalty
    ]
...
def example_solver_integration(solver_result, current_assignments):
    ...
    ***REMOVED*** (constraint results would come from validator)
    constraint_results = [
        ConstraintResult(satisfied=True, penalty=0.0),
        ***REMOVED*** ... from ACGME validator, etc.
    ]
    ...
    score = time_crystal_objective(
        ...
        current_snapshot,
        constraint_results,
        alpha=0.3,  ***REMOVED*** Configurable via environment
    )

    ***REMOVED*** Check impact
...
def example_monitoring_setup():
    ...
    """
    ***REMOVED*** Pseudo-code for monitoring setup
    print("=== Monitoring Setup ===")
    print("""
    ***REMOVED*** Prometheus metrics to add:
