...
class NightFloatPostCallConstraint(HardConstraint):
    ...
    ***REMOVED*** Activity type identifiers
    NF_ABBREVIATION = "NF"  ***REMOVED*** Night Float abbreviation
    PC_ABBREVIATION = "PC"  ***REMOVED*** Post-Call abbreviation
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find NF and PC template IDs
        ...
        ***REMOVED*** Get block-half transition days
        ...
        ***REMOVED*** For each resident
        for resident in context.residents:
            ...
            ***REMOVED*** For each block-half transition day
            for transition_date, prev_half_blocks in transition_days.items():
                ***REMOVED*** Check if any NF assignment exists on the last day of previous half
                ...
                for block in prev_half_blocks:
                    ...
                    if (r_i, b_i, nf_t_i) in template_vars:
                        ...
                        ***REMOVED*** Find PC day blocks (transition_date is the PC day)
                        am_blocks = blocks_by_date_time.get((transition_date, "AM"), [])
                        pm_blocks = blocks_by_date_time.get((transition_date, "PM"), [])

                        ***REMOVED*** If NF on last day => PC on transition day (both AM and PM)
        ...
        for resident in context.residents:
            ...
            for transition_date, prev_half_blocks in transition_days.items():
                for block in prev_half_blocks:
                    ...
                    ***REMOVED*** Implication as linear constraint: pc >= nf
                    for am_block in am_blocks:
                        am_b_i = context.block_idx.get(am_block.id)
                        if (
                            am_b_i is not None
    ...
    def validate(
        ...
        ***REMOVED*** Find NF and PC template IDs
        ...
        if not nf_template_id or not pc_template_id:
            ***REMOVED*** No templates, can't validate
        ...
        ***REMOVED*** Group assignments by person, date, time_of_day
        ...
        ***REMOVED*** Find NF assignments on last day of each block-half
        transition_days = self._get_block_half_transition_days(context)
        ...
        for a in assignments:
            ...
            ***REMOVED*** Check if this block is on the last day of a block-half
            for transition_date, prev_half_blocks in transition_days.items():
                if any(b.id == block.id for b in prev_half_blocks):
                    ***REMOVED*** This NF assignment is on last day before transition
                    ***REMOVED*** Check for PC on transition_date
    ...
    def _get_block_half_transition_days(
        ...
        ***REMOVED*** Group blocks by block_number and block_half
