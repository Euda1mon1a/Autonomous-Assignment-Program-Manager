...
class SMResidentFacultyAlignmentConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find SM rotation residents
        sm_residents = self._get_sm_rotation_residents(context)
        if not sm_residents:
            return

        ***REMOVED*** For each block and SM template, if any SM resident is assigned,
        ***REMOVED*** at least one SM faculty must be assigned
        for block in context.blocks:
            ...
            for template_id in sm_template_ids:
                ...
                ***REMOVED*** Collect resident assignment variables for this block/template
                resident_vars = []
    ...
    def add_to_pulp(
        ...
        for block in context.blocks:
            ...
            for template_id in sm_template_ids:
                ...
                ***REMOVED*** Linear approximation: sum(faculty) >= sum(residents) / max_residents
                ***REMOVED*** This ensures faculty present if any residents
                if resident_vars and faculty_vars:
                    ***REMOVED*** Simpler: if sum(residents) >= 1, sum(faculty) >= 1
                    ***REMOVED*** Using big-M: sum(faculty) >= sum(residents) - M*(1-y)
                    ***REMOVED*** where y=1 if residents present
                    ***REMOVED*** Simplified: sum(faculty) >= (1/N) * sum(residents) where N = max residents
    ...
    def validate(
        ...
        ***REMOVED*** Group assignments by block and template
        ...
        ***REMOVED*** Check each block/template combination
