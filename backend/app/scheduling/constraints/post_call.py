...
class PostCallAutoAssignmentConstraint(HardConstraint):
    ...
    ***REMOVED*** Activity type identifiers
    PCAT_ACTIVITY = "PCAT"  ***REMOVED*** Post-Call Attending
    DO_ACTIVITY = "DO"       ***REMOVED*** Direct Observation
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find PCAT and DO templates
        ...
        ***REMOVED*** For each faculty and Sun-Thurs date
        for faculty in context.faculty:
            ...
            for block in context.blocks:
                ***REMOVED*** Only Sun-Thurs for overnight call
                if not is_sun_thurs(block.date):
                    continue

                b_i = context.block_idx[block.id]

                ***REMOVED*** Check if overnight call variable exists
    ...
    def add_to_pulp(
        ...
        for faculty in context.faculty:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Implication as linear constraint: pcat >= call
                for am_block in am_blocks:
                    am_b_i = context.block_idx[am_block.id]
                    if (f_i, am_b_i, pcat_t_i) in template_vars:
                        model += (
    ...
    def validate(
        ...
        ***REMOVED*** Find overnight call assignments
        ...
        ***REMOVED*** Find PCAT and DO template IDs
        pcat_template_id = self._find_template_id(context, self.PCAT_ACTIVITY)
        do_template_id = self._find_template_id(context, self.DO_ACTIVITY)

        ***REMOVED*** Group regular assignments by person, date, time_of_day
        ...
        ***REMOVED*** Check each Sun-Thurs call assignment
    ...
    def _extract_call_assignments(self, assignments: list[Any], context: SchedulingContext) -> list[Any]:
        ...
        for a in assignments:
            ***REMOVED*** Check if this is a call assignment type
            if hasattr(a, 'call_type') and a.call_type == 'overnight':
                call_assignments.append(a)
            ***REMOVED*** Check existing_assignments in context
        ***REMOVED*** Also check context.existing_assignments for call data
        ***REMOVED*** This depends on how call assignments are stored
