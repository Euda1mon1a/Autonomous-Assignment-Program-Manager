...
class SundayCallEquityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find Sunday blocks
        sunday_blocks = [b for b in context.blocks if b.date.weekday() == 6]  ***REMOVED*** Sunday
        ...
        ***REMOVED*** Create max count variable and minimize it
        ...
        ***REMOVED*** Add to objective (minimize max)
    ...
    def add_to_pulp(
        ...
        ***REMOVED*** Track Sunday calls per faculty and add variance penalty
        faculty_counts = []
        for faculty in context.faculty:
            f_i = context.resident_idx.get(faculty.id)
            if f_i is None:
        ...
        ***REMOVED*** Minimize variance through auxiliary constraints
        ***REMOVED*** (PuLP doesn't handle quadratic well, so we minimize max)
        if faculty_counts:
    ...
    def validate(
        ...
        ***REMOVED*** Count Sunday calls per faculty
        ...
        for a in assignments:
            ...
            ***REMOVED*** Check if this is an overnight call assignment
        ...
        ***REMOVED*** Calculate penalty based on variance
        ...
        if max_count - min_count > 2:  ***REMOVED*** More than 2 difference is concerning
            violations.append(
                ConstraintViolation(
        ...
        return ConstraintResult(
            satisfied=True,  ***REMOVED*** Soft constraint, always "satisfied"
...
class WeekdayCallEquityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find Mon-Thurs blocks (weekday 0-3)
        weekday_blocks = [
            b for b in context.blocks if b.date.weekday() in (0, 1, 2, 3)  ***REMOVED*** Mon-Thurs
        ]
        ...
        ***REMOVED*** Minimize max weekday calls
...
class TuesdayCallPreferenceConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find Tuesday blocks (weekday 1)
        tuesday_blocks = [b for b in context.blocks if b.date.weekday() == 1]  ***REMOVED*** Tuesday

        if not tuesday_blocks:
            return

        ***REMOVED*** Find PD and APD faculty
        penalty_vars = []
        for faculty in context.faculty:
            if (
...
class CallSpacingConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Group blocks by week (using ISO week number)
        ...
        ***REMOVED*** For each faculty, penalize consecutive week calls
        penalty_vars = []
        for faculty in context.faculty:
            f_i = context.resident_idx.get(faculty.id)
            if f_i is None:
            ...
            ***REMOVED*** Check each pair of consecutive weeks
            for i in range(len(sorted_weeks) - 1):
                week1 = sorted_weeks[i]
                week2 = sorted_weeks[i + 1]

                ***REMOVED*** Get call vars for this faculty in each week
                week1_calls = []
    ...
    def add_to_pulp(
        ...
        for faculty in context.faculty:
            ...
            for i in range(len(sorted_weeks) - 1):
                ...
                if week1_calls and week2_calls:
                    ***REMOVED*** Create indicator variables
                    ...
                    ***REMOVED*** has_week = 1 if any call in that week
                    ***REMOVED*** sum(calls) >= 1 implies has_week = 1
                    M = len(week1_calls)  ***REMOVED*** Big-M for week1
                    ...
                    ***REMOVED*** back_to_back penalty linearization
                    ***REMOVED*** We want to penalize when both = 1
                    ***REMOVED*** Add (has_week1 + has_week2 - 1) capped at 0
                    ***REMOVED*** Or simply: penalty = has_week1 + has_week2 - 1 when both are 1
                    ***REMOVED*** Use auxiliary variable
                    btb = pulp.LpVariable(
                        f"btb_{f_i}_{constraint_count}", cat="Binary"
                    )
                    ***REMOVED*** btb = 1 iff has_week1 = 1 AND has_week2 = 1
                    model += btb <= has_week1
                    model += btb <= has_week2
        ...
        ***REMOVED*** Group call assignments by faculty and week
        ...
        for faculty_id, week_calls in calls_by_faculty_week.items():
            ...
            for i in range(len(sorted_weeks) - 1):
                ...
                ***REMOVED*** Check if weeks are consecutive
                ***REMOVED*** (same year and adjacent week, or year rollover)
...
class DeptChiefWednesdayPreferenceConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find Wednesday blocks (weekday 2)
        wednesday_blocks = [
            b for b in context.blocks if b.date.weekday() == 2  ***REMOVED*** Wednesday
        ]

        if not wednesday_blocks:
            return

        ***REMOVED*** Find Dept Chief
        bonus_vars = []
        for faculty in context.faculty:
            if (
        ...
        if bonus_vars:
            ***REMOVED*** Negative weight = bonus (minimize objective, so negative helps)
            objective_vars = variables.get("objective_terms", [])
    ...
    def validate(
        ...
        ***REMOVED*** Return negative penalty (bonus)
        return ConstraintResult(
            satisfied=True,
            violations=[],
            penalty=-bonus,
