...
class ResidentInpatientHeadcountConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Get residents grouped by PGY level
        ...
        ***REMOVED*** NF headcount per half-day (across all residents)
        for block in context.blocks:
            b_i = context.block_idx.get(block.id)
            if b_i is None:
                continue
    ...
    def add_to_pulp(
        ...
        ***REMOVED*** NF headcount per half-day
        for block in context.blocks:
            b_i = context.block_idx.get(block.id)
            if b_i is None:
                continue
    ...
    def validate(
        ...
        ***REMOVED*** Build person lookup
        person_by_id = {p.id: p for p in context.residents}

        ***REMOVED*** Count FMIT per block per PGY level
...
class FMITResidentClinicDayConstraint(HardConstraint):
    ...
    ***REMOVED*** Clinic days by PGY level (weekday: 0=Mon, 1=Tue, 2=Wed, etc.)
    FMIT_CLINIC_DAYS = {
    ...
    def add_to_cpsat(
        ...
        than generates.
        """
        ***REMOVED*** Implementation note: The pre-loading mechanism handles the assignment
        ***REMOVED*** of FMIT residents. This constraint would be used for validation
        ***REMOVED*** or for solvers that generate FMIT assignments.
        pass
    ...
    def add_to_pulp(
        ...
        ***REMOVED*** Same as above - pre-loading handles this
        pass
    ...
    def validate(
        ...
        ***REMOVED*** This validation would check:
        ***REMOVED*** 1. Find all residents on FMIT in the date range
        ***REMOVED*** 2. For each, verify they have clinic on their designated day
        ***REMOVED*** 3. Skip validation for holidays/closures

        ***REMOVED*** For now, return satisfied as pre-loading handles the rules
        return ConstraintResult(
            satisfied=True,
            violations=violations,
        )
