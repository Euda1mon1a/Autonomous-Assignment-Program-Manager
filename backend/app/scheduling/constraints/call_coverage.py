...
***REMOVED*** Weekdays for overnight call (Mon=0, Tue=1, Wed=2, Thu=3, Sun=6)
***REMOVED*** Note: The call is FOR that night, meaning the faculty covers overnight INTO next day
OVERNIGHT_CALL_DAYS = {0, 1, 2, 3, 6}  ***REMOVED*** Mon-Thu + Sunday
...
class OvernightCallCoverageConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Group blocks by date (we only need one assignment per date)
        dates_covered = set()
        for block in context.blocks:
            if block.date.weekday() not in OVERNIGHT_CALL_DAYS:
                continue  ***REMOVED*** Skip Fri-Sat
            if block.date in dates_covered:
                continue  ***REMOVED*** Only process each date once
            dates_covered.add(block.date)

            b_i = context.block_idx[block.id]

            ***REMOVED*** Collect all call variables for this date
            date_call_vars = []
            ...
            if date_call_vars:
                ***REMOVED*** Exactly one faculty on call this night
    ...
    def validate(
        ...
        ***REMOVED*** Count call assignments per date
        ...
        ***REMOVED*** Check each expected date
        violations = []
        dates_checked = set()
        ...
        for faculty in call_eligible_faculty:
            ***REMOVED*** Check if adjunct
            ...
            if faculty_role and faculty_role.upper() == "ADJUNCT":
                ...
                ***REMOVED*** Block all call assignments for this adjunct
                for block in context.blocks:
                    b_i = context.block_idx[block.id]
                    if (f_i, b_i, "overnight") in call_vars:
...
class CallAvailabilityConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        for faculty in call_eligible_faculty:
            ...
            ***REMOVED*** Check availability for each block
