...
def get_fmit_week_dates(any_date: date) -> tuple[date, date]:
    ...
    ***REMOVED*** Friday = weekday 4
    ...
    if day_of_week >= 4:  ***REMOVED*** Fri(4), Sat(5), Sun(6)
        days_since_friday = day_of_week - 4
    else:  ***REMOVED*** Mon(0), Tue(1), Wed(2), Thu(3)
...
def is_sun_thurs(any_date: date) -> bool:
    ...
    return any_date.weekday() in (6, 0, 1, 2, 3)  ***REMOVED*** Sun=6, Mon-Thu=0-3
...
class FMITWeekBlockingConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Find FMIT assignments in existing assignments
        ...
        ***REMOVED*** Identify clinic templates
        ...
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
            ...
            for friday_start, thursday_end in fmit_weeks:
                ***REMOVED*** Block clinic for entire FMIT week
                for block in context.blocks:
                    ...
                    ***REMOVED*** Block clinic templates
                    if template_vars:
    ...
    def validate(
        ...
        for a in assignments:
            ...
            ***REMOVED*** Check if this faculty is on FMIT for this date
            fmit_weeks = fmit_weeks_by_faculty.get(a.person_id, [])
            for friday_start, thursday_end in fmit_weeks:
                if not (friday_start <= block.date <= thursday_end):
                    continue

                ***REMOVED*** Check if assigned to clinic
    ...
    def _identify_fmit_weeks(
        ...
        ***REMOVED*** Look for FMIT assignments in existing assignments
        for a in context.existing_assignments:
            ***REMOVED*** Check if this is an FMIT assignment
            ***REMOVED*** FMIT templates have activity_type='inpatient' and name contains 'FMIT'
            template = None
            for t in context.templates:
            ...
            ***REMOVED*** Calculate FMIT week for this date
...
class FMITMandatoryCallConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
            ...
            for friday_start, thursday_end in fmit_weeks:
                ***REMOVED*** Find Friday and Saturday blocks
                for block in context.blocks:
                    if block.date == friday_start:  ***REMOVED*** Friday
                        b_i = context.block_idx[block.id]
                        if (f_i, b_i, "overnight") in call_vars:
    ...
    def validate(
        ...
        ***REMOVED*** Check call assignments for FMIT faculty
        ***REMOVED*** This would require call assignment data in context
        ***REMOVED*** For now, just return satisfied if we can't validate
        return ConstraintResult(satisfied=True, violations=[])
...
class PostFMITRecoveryConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
            ...
            for friday_start, thursday_end in fmit_weeks:
                ***REMOVED*** Recovery Friday is the day after FMIT Thursday
                recovery_friday = thursday_end + timedelta(days=1)

                ***REMOVED*** Block all assignments on recovery Friday
                for block in context.blocks:
                    ...
                    ***REMOVED*** Block all template assignments
                    if template_vars:
                    ...
                    ***REMOVED*** Block call assignments
                    if call_vars:
    ...
    def validate(
        ...
        ***REMOVED*** Calculate recovery Fridays for each faculty
        recovery_fridays = {}  ***REMOVED*** {faculty_id: set of recovery friday dates}
        for faculty_id, fmit_weeks in fmit_weeks_by_faculty.items():
        ...
        for a in assignments:
            ...
            ***REMOVED*** Check if this is a recovery Friday
...
class PostFMITSundayBlockingConstraint(HardConstraint):
    ...
    def validate(
        ...
        ***REMOVED*** Build lookup of blocked Sundays per faculty
...
class FMITContinuityTurfConstraint(HardConstraint):
    ...
    def add_to_cpsat(
        ...
        """
        ***REMOVED*** This constraint is primarily for validation and reporting
        ***REMOVED*** It doesn't add solver constraints since turf decisions are made dynamically
        pass
    ...
    def validate(
        ...
        ***REMOVED*** Get current load shedding level from context (default to NORMAL if not set)
        load_shedding_level = getattr(context, "load_shedding_level", 0)  ***REMOVED*** 0 = NORMAL

        ***REMOVED*** For now, this is a reporting constraint
        ***REMOVED*** In a full implementation, continuity deliveries would be tracked in context
        ***REMOVED*** and we would validate FM attendance based on load shedding level

        if load_shedding_level >= 2:  ***REMOVED*** ORANGE or higher
            violations.append(
                ConstraintViolation(
                    constraint_name=self.name,
                    constraint_type=self.constraint_type,
        ...
        return ConstraintResult(
            satisfied=True,  ***REMOVED*** This is informational, not a hard failure
            violations=violations,
        )
...
class FMITStaffingFloorConstraint(HardConstraint):
    ...
    FMIT_UTILIZATION_CAP = 0.20  ***REMOVED*** Max 20% on FMIT at once
    ...
    def add_to_cpsat(
        ...
        ***REMOVED*** Count active faculty (not residents)
        ...
        ***REMOVED*** Calculate maximum concurrent FMIT
        max_concurrent_fmit = max(1, int(total_faculty * self.FMIT_UTILIZATION_CAP))

        ***REMOVED*** For CP-SAT, we would need to track concurrent FMIT weeks
        ***REMOVED*** This requires week-level grouping which is complex in CP-SAT
        ***REMOVED*** For now, we rely on validation to catch violations
        ***REMOVED*** Full CP-SAT implementation would require week variables
    ...
    def validate(
        ...
        ***REMOVED*** Count active faculty
        active_faculty = [f for f in context.faculty if self._is_faculty_active(f)]
        total_faculty = len(active_faculty)

        ***REMOVED*** Identify FMIT assignments
        fmit_assignments = []
        template_by_id = {t.id: t for t in context.templates}
        ...
        ***REMOVED*** Check minimum faculty threshold
        if total_faculty < self.MINIMUM_FACULTY_FOR_FMIT:
        ...
        ***REMOVED*** Check utilization cap
        if total_faculty >= self.MINIMUM_FACULTY_FOR_FMIT:
            ...
            ***REMOVED*** Group FMIT assignments by week
    ...
    def _is_faculty_active(self, person: Any) -> bool:
        ...
        ***REMOVED*** In a full implementation, this would check person status
        ***REMOVED*** For now, assume all faculty in context.faculty are active
        return True
    ...
    def _block_all_fmit_assignments(
        ...
        ***REMOVED*** Identify FMIT templates
        fmit_template_ids = {
        ...
        ***REMOVED*** Block all FMIT assignments
        for (f_i, b_i, t_i), var in template_vars.items():
            ...
            if t_id in fmit_template_ids:
                try:
                    ***REMOVED*** Try CP-SAT style
                    model.Add(var == 0)
                except AttributeError:
                    ***REMOVED*** Try PuLP style
                    model += var == 0
    ...
    def _group_fmit_by_week(
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Get FMIT week for this date
