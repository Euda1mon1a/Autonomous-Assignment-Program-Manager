...
class CoverageConstraintTemplate(SoftConstraint):
    ...
    def __init__(self, weight: float = 1.5):
        ...
        self.required_slots = None  ***REMOVED*** List of (block_id, rotation_id) tuples
        self.min_coverage = 1  ***REMOVED*** Minimum per slot (usually 1)
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** For each required slot, ensure minimum coverage
        uncovered_penalties = []
        ...
        for slot in self.required_slots or []:
            ...
            ***REMOVED*** Collect variables for this slot
            slot_vars = []

            ***REMOVED*** Check regular assignments
            ...
            ***REMOVED*** Check template-specific assignments
            if template_vars:
                for person in context.residents + context.faculty:
                    p_i = context.resident_idx.get(person.id)
                    if p_i is not None:
            ...
            if slot_vars:
                ***REMOVED*** Create penalty variable for uncovered slots
                ...
                ***REMOVED*** Penalty for not being covered
                uncovered_penalties.append(is_covered.Not())

        ***REMOVED*** Add penalties to objective
    ...
    def add_to_pulp(self, model, variables, context):
        ...
        ***REMOVED*** For each required slot, add coverage penalty
        total_penalty = 0
        ...
        for slot in self.required_slots or []:
            ...
            ***REMOVED*** Check template-specific assignments
            ...
            if slot_vars:
                ***REMOVED*** Create binary variable for coverage violation
                ...
                ***REMOVED*** If sum < min_coverage, is_uncovered can be 1 (penalized)
                ***REMOVED*** This is approximated by: sum + is_uncovered * M >= min_coverage
                ***REMOVED*** Where M is a large number
        ...
        ***REMOVED*** Add total penalty to objective
        if total_penalty:
