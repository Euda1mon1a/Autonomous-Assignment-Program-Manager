...
class PreferenceConstraintTemplate(SoftConstraint):
    ...
    def __init__(self, weight: float = 1.0):
        ...
        self.preference_type = None  ***REMOVED*** 'time_of_day', 'clinic', 'day_of_week'
        self.preferred_value = None
        self.penalty_for_violation = 1  ***REMOVED*** Violations count multiplier
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** Identify blocks that violate the preference
        violation_vars = []
        ...
        for block in context.blocks:
            ...
            if self.preference_type == "time_of_day":
                ***REMOVED*** Check AM/PM preference
                ...
                ***REMOVED*** Check weekday preference (0=Monday, 6=Sunday)
                if hasattr(block, "date") and self.preferred_value is not None:
                    violates = block.date.weekday() != self.preferred_value

            if violates:
                ***REMOVED*** All assignments to this block violate preference
                for key in x:
                    if key[1] == b_i:
                        violation_vars.append(x[key])

        ***REMOVED*** Add weighted violations to soft objectives
        if violation_vars:
