...
class SequenceConstraintTemplate(HardConstraint):
    ...
    def __init__(self):
        ...
        self.prerequisite_rotation = None  ***REMOVED*** Must come after this
        self.required_rotation = None  ***REMOVED*** Then must have this
        self.max_gap_days = 1  ***REMOVED*** Maximum days between sequence
    ...
    def validate(
        ...
        ***REMOVED*** Group by person and sort by date
        person_assignments = {}
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** For each person, check sequence requirements
        for person in context.residents + context.faculty:
            p_i = context.resident_idx.get(person.id)
            if p_i is None:
                continue

            ***REMOVED*** Find prerequisite blocks
            for i, block in enumerate(context.blocks):
                b_i = context.block_idx[block.id]

                ***REMOVED*** Check if this block has prerequisite rotation
                ...
                if has_prerequisite is None and (p_i, b_i) in x:
                    ***REMOVED*** Check rotation type from block (if available)
                    if (
                ...
                ***REMOVED*** If prerequisite found, ensure required rotation follows
                if has_prerequisite is not None:
                    ***REMOVED*** Find blocks within max_gap_days
                    following_blocks = []
                    for j in range(
                        ...
                        ***REMOVED*** Check for required rotation in following blocks
                        if template_vars:
                            for template in context.templates:
                                if (
                    ...
                    ***REMOVED*** Enforce: if has_prerequisite, then at least one following block
                    if following_blocks:
            ...
            for i, block in enumerate(context.blocks):
                ...
                ***REMOVED*** If prerequisite found, ensure required rotation follows
                for prereq_var in prereq_vars:
                    ...
                    ***REMOVED*** Enforce: if prereq_var = 1, then sum(following) >= 1
                    ***REMOVED*** Linearized: sum(following) >= prereq_var
                    if following_blocks:
                        model += (
