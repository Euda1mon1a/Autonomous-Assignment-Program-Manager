...
class ExclusionConstraintTemplate(HardConstraint):
    ...
    def __init__(self):
        ...
        self.excluded_person_type = None  ***REMOVED*** 'adjunct', 'new_faculty', etc.
        self.excluded_rotation_types = []  ***REMOVED*** Rotations excluded for type
        self.excluded_combinations = []  ***REMOVED*** (person_type, rotation) tuples
    ...
    def validate(
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Check person type exclusions
    ...
    def _should_be_excluded(self, person, rotation):
        ...
        ***REMOVED*** Check person type
        if self._matches_person_type(person):
            ***REMOVED*** Check if rotation is excluded
            if rotation.type in self.excluded_rotation_types:
                return True

        ***REMOVED*** Check specific combinations
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** For each person, check if they match excluded type
        for person in context.residents + context.faculty:
            ...
            ***REMOVED*** Exclude from specific rotation types
            for block in context.blocks:
                b_i = context.block_idx[block.id]

                ***REMOVED*** Check template-specific exclusions
                if template_vars:
                    for template in context.templates:
                        if template.type in self.excluded_rotation_types:
                            ***REMOVED*** Prevent assignment to this template
                            if (p_i, b_i, template.id) in template_vars:
                                model.Add(template_vars[p_i, b_i, template.id] == 0)

                ***REMOVED*** Check general exclusions
                if (p_i, b_i) in x:
                    ***REMOVED*** Check if block has excluded rotation type
                    if (
            ...
            ***REMOVED*** Check specific exclusion combinations
            for excluded_type, excluded_rotation in self.excluded_combinations:
                if self._matches_person_type(person, excluded_type):
                    for block in context.blocks:
                        b_i = context.block_idx[block.id]
