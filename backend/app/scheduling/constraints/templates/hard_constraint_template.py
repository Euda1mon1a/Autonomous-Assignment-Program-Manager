...
class MyConstraint(HardConstraint):
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            name="MyConstraint",
            constraint_type=ConstraintType.AVAILABILITY,  ***REMOVED*** Change to appropriate type
            priority=ConstraintPriority.HIGH,  ***REMOVED*** Adjust priority as needed
        )
    ...
    def add_to_cpsat(
        ...
        Example:
            ...
            ...         ***REMOVED*** Add constraint: x[r_i, b_i] <= 1
            ...         model.Add(x[r_i, b_i] <= 1)
        """
        ***REMOVED*** Get decision variables
        x = variables.get("assignments", {})

        ***REMOVED*** Iterate over relevant entities
        for resident in context.residents:
            r_i = context.resident_idx[resident.id]
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Build constraint logic here
                ***REMOVED*** Example: model.Add(x[r_i, b_i] == 0)  ***REMOVED*** Prevent assignment

        ***REMOVED*** Add model.Add() calls for each constraint expression
        ...
        ***REMOVED*** Iterate over relevant entities
        for resident in context.residents:
            ...
            for block in context.blocks:
                ...
                ***REMOVED*** Build constraint logic here
                ***REMOVED*** Example: model += x[r_i, b_i] == 0, f"const_{r_i}_{b_i}"

        ***REMOVED*** Add model += (constraint, label) calls for each constraint
    ...
    def validate(
        ...
        ***REMOVED*** Check constraint logic for each assignment
        for assignment in assignments:
            ***REMOVED*** Example: Check if person is available
            person_id = assignment.person_id
            block_id = assignment.block_id

            ***REMOVED*** Validate constraint conditions
            if self._violates_constraint(assignment, context):
                violations.append(
                    ConstraintViolation(
                        constraint_name=self.name,
                        constraint_type=self.constraint_type,
                        severity="CRITICAL",  ***REMOVED*** or HIGH, MEDIUM, LOW
...
***REMOVED*** Alternative constraint types can be created by changing the parent class:
***REMOVED***
***REMOVED*** class MyHardConstraint(HardConstraint):
***REMOVED***     """Hard constraint - must be satisfied."""
***REMOVED***     ...
***REMOVED***
***REMOVED*** class MySoftConstraint(SoftConstraint):
***REMOVED***     """Soft constraint - optimization objective."""
***REMOVED***     def __init__(self, weight: float = 1.0):
***REMOVED***         super().__init__(
***REMOVED***             name="MySoftConstraint",
***REMOVED***             constraint_type=ConstraintType.PREFERENCE,
***REMOVED***             weight=weight,  ***REMOVED*** Soft constraints have weight
***REMOVED***         )
