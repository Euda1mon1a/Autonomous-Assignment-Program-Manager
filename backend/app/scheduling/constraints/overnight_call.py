...
def is_overnight_call_night(block_date: date) -> bool:
    ...
    """
    return block_date.weekday() in (6, 0, 1, 2, 3)  ***REMOVED*** Sun=6, Mon-Thu=0-3
...
class OvernightCallGenerationConstraint(HardConstraint):
    ...
    def _get_eligible_faculty(
        ...
        for faculty in context.faculty:
            ***REMOVED*** Rule 1: Exclude ADJUNCT faculty
            ...
            ***REMOVED*** Rule 2: Check FMIT blocking
            is_blocked = False
            ...
            ***REMOVED*** Rule 3: Check post-FMIT Sunday blocking
            if target_date.weekday() == 6:  ***REMOVED*** Sunday
                if faculty.id in fmit_weeks_by_faculty:
                    for friday_start, thursday_end in fmit_weeks_by_faculty[faculty.id]:
            ...
            ***REMOVED*** Rule 4: Check availability (absences)
    ...
    def add_to_cpsat(
        ...
        """
        ***REMOVED*** Initialize call_assignments dict if not present
        ...
        ***REMOVED*** Get unique dates from blocks
        dates = sorted(set(b.date for b in context.blocks))

        ***REMOVED*** Get overnight call nights
        ...
        for target_date in call_nights:
            ***REMOVED*** Get eligible faculty for this night
            ...
            ***REMOVED*** Find block index for this date (use AM block as reference)
            b_i = None
            for block in context.blocks:
            ...
            if b_i is None:
                ***REMOVED*** Try any block on this date if no AM block found
                for block in context.blocks:
                    if block.date == target_date:
                        b_i = context.block_idx.get(block.id)
                        break
            ...
            ***REMOVED*** Create decision variables for each eligible faculty
            night_vars = []
            ...
            ***REMOVED*** Constraint: Exactly one faculty takes call each night
            if night_vars:
    ...
    def validate(
        ...
        ***REMOVED*** Get unique dates requiring call
        dates = sorted(set(b.date for b in context.blocks))
        call_nights = [d for d in dates if is_overnight_call_night(d)]

        ***REMOVED*** Build map of call assignments by date
        ...
        ***REMOVED*** Check each night has exactly one call
        for night in call_nights:
            ...
            if len(calls) == 0:
                ...
                ***REMOVED*** Validate the assigned faculty is eligible
                call = calls[0]
                faculty = faculty_by_id.get(call.person_id)

                if faculty:
                    ***REMOVED*** Check ADJUNCT exclusion
                    is_adjunct = False
                    if hasattr(faculty, "role_enum") and faculty.role_enum == FacultyRole.ADJUNCT:
                        is_adjunct = True
                    if hasattr(faculty, "faculty_role") and faculty.faculty_role == "adjunct":
