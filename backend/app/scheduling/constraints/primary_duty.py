...
class PrimaryDutyConfig:
    ...
    clinic_max_per_week: int = 10  ***REMOVED*** Default high max
    ...
    ***REMOVED*** Additional constraints from Airtable (for future use)
    ...
    def from_airtable_record(cls, record: dict[str, Any]) -> "PrimaryDutyConfig":
        ...
        ***REMOVED*** Parse day availability
        ...
        ***REMOVED*** If no days specified, default to all weekdays
...
def load_primary_duties_config(
    ...
    if json_path is None:
        ***REMOVED*** Default to sanitized file in docs/schedules
        json_path = (
    ...
    try:
        ...
        for record in data.get("records", []):
            ...
            ***REMOVED*** Also index by duty_id for lookup
...
class FacultyPrimaryDutyClinicConstraint(HardConstraint):
        ...
        for faculty in context.faculty:
            ...
            ***REMOVED*** Group blocks by week
            ...
            for week_start, week_blocks in weeks.items():
                ...
                ***REMOVED*** Minimum constraint (coverage requirement)
                if config.clinic_min_per_week > 0:
                    model.Add(week_sum >= config.clinic_min_per_week)
                    constraints_added += 1

                ***REMOVED*** Maximum constraint
                if config.clinic_max_per_week < 10:  ***REMOVED*** Only if explicitly limited
    ...
    def validate(
        ...
        ***REMOVED*** Count clinic assignments by faculty and week
        ...
        ***REMOVED*** Check constraints
        for faculty in context.faculty:
            ...
            ***REMOVED*** Get all weeks in the scheduling period
            weeks = self._group_blocks_by_week(context.blocks)

            for week_start in weeks:
                count = weekly_counts.get(week_start, 0)

                ***REMOVED*** Check minimum
    ...
    def add_to_cpsat(
        ...
        for faculty in context.faculty:
            ...
            for block in context.blocks:
                ***REMOVED*** Check if block's day is available
                day_of_week = block.date.weekday()
                if day_of_week in config.available_days:
                    continue  ***REMOVED*** Available, no constraint needed

                ***REMOVED*** Block assignments on unavailable days
                b_i = context.block_idx[block.id]
                for template in context.templates:
                    if template.id not in clinic_template_ids:
                        continue
...
class FacultyClinicEquitySoftConstraint(SoftConstraint):
    ...
    def add_to_cpsat(
        ...
        for faculty in context.faculty:
            ...
            ***REMOVED*** Calculate target as midpoint of min/max
            ...
            for week_start, week_blocks in weeks.items():
                ...
                ***REMOVED*** Create deviation variable
    ...
    def validate(
        ...
        ***REMOVED*** Calculate total deviation
        total_penalty = 0.0
