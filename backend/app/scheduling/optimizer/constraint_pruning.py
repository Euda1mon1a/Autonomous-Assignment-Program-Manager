...
Example:
    ...
    >>> ***REMOVED*** Pass only feasible assignments to solver
...
class ConstraintPruner:
    ...
    Example:
        ...
        >>> ***REMOVED*** Use result["feasible_assignments"] for solver
    """
    ...
    def prune_assignments(
        ...
        Example:
            ...
            ...     rotations=[{"id": "r1", "min_pgy_level": 3}],  ***REMOVED*** Requires PGY-3+
            ...     blocks=[{"id": "b1", "date": date(2024, 1, 1)}]
            ... )
            >>> ***REMOVED*** p1 (PGY-2) cannot be assigned to r1 (requires PGY-3+)
        ...
        ***REMOVED*** Build existing assignment index
        ...
        ***REMOVED*** Evaluate all combinations
        for person in persons:
            for block in blocks:
                self.total_evaluated += 1

                ***REMOVED*** Check if already assigned
                if (person["id"], block["id"]) in existing_by_person_block:
                    continue

                ***REMOVED*** Get feasible rotations for this person-block
    ...
    def _check_feasibility(
        ...
        ***REMOVED*** Check specialty restrictions
        ...
        ***REMOVED*** Check availability (e.g., leave, deployments)
        ...
        ***REMOVED*** Check rotation capacity
        if rotation.get("max_people"):
            ***REMOVED*** This would need to check current assignments
            ***REMOVED*** For now, we just mark it as potentially feasible
            pass
    ...
    def estimate_search_space_reduction(
        ...
        ***REMOVED*** Estimate exponential search space reduction
        ***REMOVED*** If we prune 50% of assignments, we reduce search space by 2^n
...
def prune_infeasible_assignments(
    ...
    Example:
        ...
        >>> feasible = result['feasible_assignments']
        >>> ***REMOVED*** Pass feasible to solver
    """
    pruner = ConstraintPruner()
