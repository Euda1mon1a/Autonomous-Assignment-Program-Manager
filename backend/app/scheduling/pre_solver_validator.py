...
class PreSolverValidator:
    ...
    ***REMOVED*** Complexity thresholds (based on typical solver performance)
    COMPLEXITY_LOW = 10_000  ***REMOVED*** < 10K: trivial, solver finishes in seconds
    COMPLEXITY_MEDIUM = 100_000  ***REMOVED*** 10K-100K: moderate, solver may take minutes
    COMPLEXITY_HIGH = 1_000_000  ***REMOVED*** 100K-1M: high, solver may timeout
    COMPLEXITY_EXTREME = 10_000_000  ***REMOVED*** > 1M: extreme, likely infeasible

    ***REMOVED*** Personnel thresholds
    MIN_PERSONNEL_RATIO = 1.2  ***REMOVED*** Need at least 20% more personnel than coverage slots
    WORKLOAD_MAX_RATIO = 0.9  ***REMOVED*** Personnel shouldn't be assigned >90% of blocks
    ...
    def validate_saturation(
        ...
        ***REMOVED*** Add statistics
        result.statistics = self._gather_statistics(context)

        ***REMOVED*** If any issues found, mark as infeasible
        if result.issues:
    ...
    def _check_hour_balance(
        ...
        ***REMOVED*** Calculate minimum required slots from templates
        ***REMOVED*** For now, we assume each resident needs approximately equal distribution
        ***REMOVED*** across templates, but this could be refined based on template requirements
        min_required_slots = len(workday_blocks) * len(
            context.residents
        )  ***REMOVED*** At minimum, fill all slots

        ***REMOVED*** Check if we have enough slots
        ...
        ***REMOVED*** Check for oversubscription (residents with too few available blocks)
        for resident in context.residents:
            ...
            min_needed = len(workday_blocks) // 2  ***REMOVED*** Should have at least 50% availability
    ...
    def _check_coverage_ratio(
        ...
        ***REMOVED*** Calculate required coverage
        total_coverage_needed = len(workday_blocks) * min_coverage_per_block

        ***REMOVED*** Calculate available personnel-blocks (accounting for absences)
        total_available = 0
        for resident in context.residents:
            for block in workday_blocks:
                if self._is_person_available(resident.id, block.id, context):
        ...
        ***REMOVED*** Check if we have enough coverage
        coverage_ratio = (
        ...
        ***REMOVED*** Check for over-assignment risk
    ...
    def _check_availability_conflicts(
        ...
        ***REMOVED*** Check for completely unavailable residents
        for resident in context.residents:
            available_count = sum(
                1
                for block in workday_blocks
        ...
        ***REMOVED*** Check for blocks with insufficient available residents
        for block in workday_blocks:
    ...
    def _check_existing_assignment_conflicts(
        ...
        ***REMOVED*** Build set of (person_id, block_id) pairs that are occupied
        ...
        ***REMOVED*** Check if any resident has too many pre-assignments
        workday_blocks = [b for b in context.blocks if not b.is_weekend]
        for resident in context.residents:
            ...
            ***REMOVED*** If more than 80% of blocks are pre-assigned, warn
        ...
        ***REMOVED*** Check if pre-assignments reduce available capacity too much
    ...
    def estimate_complexity(self, num_vars: int, num_constraints: int) -> int:
        ...
        """
        ***REMOVED*** Simple heuristic: complexity ≈ variables × constraints
        ***REMOVED*** Real complexity depends on constraint structure, but this gives a rough estimate
    ...
    def _estimate_complexity(
        ...
        ***REMOVED*** Estimate decision variables
        ***REMOVED*** Each (resident, block, template) triple is a binary variable
        ...
        ***REMOVED*** Estimate constraints
        ***REMOVED*** This is approximate - actual count depends on constraint manager
        ***REMOVED*** Typical constraints: availability, capacity, equity, etc.
        num_constraints = (
            num_residents * num_blocks  ***REMOVED*** Availability constraints
            + num_blocks * num_templates  ***REMOVED*** Capacity constraints
            + num_residents  ***REMOVED*** Equity constraints
            + num_blocks  ***REMOVED*** Coverage constraints
        )

        complexity = self.estimate_complexity(num_vars, num_constraints)
        result.complexity_estimate = complexity

        ***REMOVED*** Categorize complexity
    ...
    def _gather_statistics(self, context: SchedulingContext) -> dict[str, Any]:
        ...
        ***REMOVED*** Calculate availability statistics
    ...
    def _is_person_available(
        ...
        if person_id not in context.availability:
            return True  ***REMOVED*** No absence data = available

        if block_id not in context.availability[person_id]:
            return True  ***REMOVED*** No absence for this block = available
