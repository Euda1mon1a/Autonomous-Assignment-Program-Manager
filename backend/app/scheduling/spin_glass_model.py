...
class SpinGlassScheduler:
    ...
    def __init__(
        ...
        ***REMOVED*** Cache constraint interaction matrix for efficiency
    ...
    def compute_frustration_index(self, constraints: list[Constraint] | None = None) -> float:
        ...
        ***REMOVED*** Build interaction matrix if not cached
        if self._interaction_matrix is None:
            self._build_interaction_matrix(constraints)

        ***REMOVED*** Compute frustration as fraction of negative (conflicting) interactions
        total_pairs = 0
        frustrated_pairs = 0

        for (i, j), coupling in self._interaction_matrix.items():
            total_pairs += 1
            ***REMOVED*** Negative coupling indicates antiferromagnetic (conflicting) interaction
            if coupling < 0:
                frustrated_pairs += 1
    ...
    def generate_replica_schedules(
        ...
        for replica_idx in range(n_replicas):
            ***REMOVED*** Generate a perturbed schedule via simulated annealing
            assignments, energy = self._thermal_sample_schedule(
                base_solution=base_solver_result,
                replica_id=replica_idx,
            )

            ***REMOVED*** Compute magnetization (alignment with soft constraints)
            magnetization = self._compute_magnetization(assignments)

            ***REMOVED*** Break down energy by constraint type
    ...
    def find_glass_transition_threshold(
        ...
        for density in densities:
            ***REMOVED*** Generate replicas at this constraint density
            ***REMOVED*** (in practice, we vary temperature instead of adding/removing constraints)
            temp = 1.0 / density  ***REMOVED*** Higher density = lower temperature
            self.temperature = temp

            replicas = self.generate_replica_schedules(n_replicas=5)

            ***REMOVED*** Compute mean overlap between replica pairs
            if len(replicas) >= 2:
        ...
        ***REMOVED*** Find transition point as maximum derivative of overlap
    ...
    def analyze_energy_landscape(
        ...
        ***REMOVED*** Cluster schedules into basins using energy and overlap
        basins = self._identify_solution_basins(schedules)

        ***REMOVED*** Extract local minima (lowest energy in each basin)
        local_minima = [
            min(schedules[idx].energy for idx in basin_indices)
            for basin_indices in basins.values()
        ]

        ***REMOVED*** Estimate energy barriers between basins
        barriers = self._estimate_energy_barriers(schedules, basins)

        ***REMOVED*** Identify frustration clusters
        frustration_clusters = self._identify_frustration_clusters()

        ***REMOVED*** Estimate glass transition temperature
    ...
    def compute_replica_symmetry_analysis(
        ...
        ***REMOVED*** Build Parisi overlap matrix
        ...
        ***REMOVED*** Compute RSB order parameter (variance of overlap distribution)
        off_diagonal = overlap_matrix[np.triu_indices(n, k=1)]
        mean_overlap = np.mean(off_diagonal)
        overlap_variance = np.var(off_diagonal)

        ***REMOVED*** RSB order parameter: higher variance = more symmetry breaking
        rsb_parameter = min(1.0, overlap_variance * 4.0)  ***REMOVED*** Normalize to [0, 1]

        ***REMOVED*** Diversity score: inverse of mean overlap
        diversity_score = 1.0 - mean_overlap

        ***REMOVED*** Compute overlap distribution histogram
        ...
        ***REMOVED*** Compute ultrametric distances (hierarchical clustering metric)
    ...
    ***REMOVED*** -------------------------------------------------------------------------
    ...
    def _build_interaction_matrix(self, constraints: list[Constraint]) -> None:
        ...
        ***REMOVED*** For each pair of constraints, estimate coupling strength
        for i, c1 in enumerate(constraints):
            for j, c2 in enumerate(constraints):
                if i >= j:
                    continue

                ***REMOVED*** Estimate interaction by checking if constraints conflict
                ***REMOVED*** Positive = ferromagnetic (aligned), Negative = antiferromagnetic (conflicting)
    ...
    def _estimate_constraint_coupling(
        ...
        """
        ***REMOVED*** Heuristic: Constraints of different types often conflict
        type_map = {
            "equity": 0,
            "preference": 1,
            "capacity": 2,
        ...
        ***REMOVED*** Equity vs preference typically conflicts
        if (t1 == 0 and t2 == 1) or (t1 == 1 and t2 == 0):
            return -0.5

        ***REMOVED*** Same type usually aligns
        if t1 == t2:
            return 0.5

        ***REMOVED*** Default: weak alignment
        return 0.1
    ...
    def _thermal_sample_schedule(
        ...
        """
        ***REMOVED*** Start from base solution if provided, otherwise random
        ...
        ***REMOVED*** Simulated annealing with thermal perturbations
        n_steps = 100
        for step in range(n_steps):
            ***REMOVED*** Propose a random move (swap, reassign, etc.)
    ...
    def _random_feasible_schedule(self) -> list[tuple[UUID, UUID, UUID | None]]:
        ...
        for block in self.context.blocks:
            ***REMOVED*** Randomly assign a resident to this block
    ...
    def _propose_move(
        ...
        ***REMOVED*** Random move type
        ...
        if move_type == "swap" and len(new_assignments) >= 2:
            ...
            ***REMOVED*** Reassign a random block to a different resident
            ...
            ***REMOVED*** Change template for a random assignment
            idx = self.rng.integers(0, len(new_assignments))
    ...
    def _compute_energy(
        ...
        ***REMOVED*** Validate against all constraints
        for constraint in self.constraints:
            result = constraint.validate(assignments, self.context)

            if not result.satisfied:
                ***REMOVED*** Hard constraint violation = high energy penalty
                if hasattr(constraint, "get_penalty"):
                    penalty = constraint.get_penalty()
                    if penalty == float("inf"):
                        penalty = 1000.0  ***REMOVED*** Cap for numerical stability
                    total_energy += penalty
                else:
                    total_energy += 100.0  ***REMOVED*** Default penalty

            ***REMOVED*** Add soft constraint penalties
    ...
    def _compute_magnetization(
        ...
        for constraint in self.constraints:
            ***REMOVED*** Only consider soft constraints
            if hasattr(constraint, "weight"):
                result = constraint.validate(assignments, self.context)

                ***REMOVED*** Normalize penalty to [-1, 1] range
    ...
    def _identify_solution_basins(
        ...
        ***REMOVED*** Simple clustering: schedules with high overlap are in same basin
        visited = set()
        basin_id = 0
        ...
        for i in range(n):
            ...
            ***REMOVED*** Start new basin
            basin_members = [i]
            visited.add(i)

            ***REMOVED*** Find all schedules with high overlap to schedule i
            for j in range(n):
                if j in visited:
                    continue

                overlap = self.compute_parisi_overlap(schedules[i], schedules[j])
                if overlap > 0.7:  ***REMOVED*** Threshold for same basin
    ...
    def _estimate_energy_barriers(
        ...
        for i, basin_a_id in enumerate(basin_ids):
            for basin_b_id in basin_ids[i + 1 :]:
                ***REMOVED*** Find minimum energy in each basin
                ...
                ***REMOVED*** Barrier height approximation
    ...
    def _identify_frustration_clusters(self) -> list[FrustrationCluster]:
        ...
        ***REMOVED*** Find strongly negative couplings (high frustration)
        conflicting_pairs = [
            (i, j, coupling)
            for (i, j), coupling in self._interaction_matrix.items()
            if coupling < -0.3
        ...
        ***REMOVED*** Group into clusters (simplified: one cluster per conflicting pair)
        for i, j, coupling in conflicting_pairs[:5]:  ***REMOVED*** Limit to top 5
    ...
    def _estimate_glass_temperature(self, schedules: list[ReplicaSchedule]) -> float:
        ...
        ***REMOVED*** Estimate from energy distribution
        energies = np.array([s.energy for s in schedules])
        energy_std = np.std(energies)

        ***REMOVED*** Tg ~ width of energy distribution
    ...
    def _compute_ultrametric_distances(
        ...
        ***REMOVED*** Convert overlap to distance: d_ij = 1 - q_ij
        for i in range(n):
