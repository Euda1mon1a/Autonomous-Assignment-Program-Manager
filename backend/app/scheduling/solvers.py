...
COVERAGE_WEIGHT = 1000  ***REMOVED*** Primary objective: maximize schedule coverage
EQUITY_PENALTY_WEIGHT = 10  ***REMOVED*** Secondary: balance workload across residents
TEMPLATE_BALANCE_WEIGHT = 5  ***REMOVED*** Tertiary: distribute assignments across rotation types

***REMOVED*** Redis TTL for solver progress tracking
SOLVER_PROGRESS_TTL_SECONDS = 300  ***REMOVED*** 5 minutes
...
class SolverResult:
    ...
    def __init__(
        ...
        ],  ***REMOVED*** (person_id, block_id, template_id)
        status: str,
        objective_value: float = 0.0,
        runtime_seconds: float = 0.0,
        solver_status: str = "",
        statistics: dict = None,
        explanations: dict = None,  ***REMOVED*** person_id, block_id -> DecisionExplanation
        random_seed: int = None,
        call_assignments: list[
            tuple[UUID, UUID, str]
        ] = None,  ***REMOVED*** (person_id, block_id, call_type)
    ):
        ...
        self.explanations = (
            explanations or {}
        )  ***REMOVED*** (person_id, block_id) -> explanation dict
        self.random_seed = random_seed
        self.call_assignments = (
            call_assignments or []
        )  ***REMOVED*** (person_id, block_id, call_type)
...
class BaseSolver(ABC):
    ...
    def _select_template(
        ...
        for template in templates:
            ***REMOVED*** Skip templates requiring procedure credentials
...
class PuLPSolver(BaseSolver):
    ...
    def solve(
        ...
        ***REMOVED*** Filter to workday blocks
        ...
        ***REMOVED*** Create the LP problem
        prob = pulp.LpProblem("ResidencySchedule", pulp.LpMaximize)

        ***REMOVED*** ==================================================
        ***REMOVED*** DECISION VARIABLES
        ***REMOVED*** x[r_i, b_i, t_i] = 1 if resident r assigned to rotation t during block b
        ***REMOVED*** ==================================================
        x = {}
        ...
        for resident in context.residents:
            ...
            for block in workday_blocks:
                ...
                for template in context.templates:
                    ***REMOVED*** Skip templates requiring procedure credentials if resident doesn't have them
                    if template.requires_procedure_credential:
                        continue
        ...
        ***REMOVED*** Store both 2D (for legacy constraints) and 3D variables
        ***REMOVED*** 2D view: x_2d[r_i, b_i] = sum over all rotations
        x_2d = {}
        for resident in context.residents:
            r_i = context.resident_idx[resident.id]
            for block in workday_blocks:
        ...
        ***REMOVED*** ==================================================
        ***REMOVED*** OVERNIGHT CALL DECISION VARIABLES
        ***REMOVED*** call[f_i, b_i, "overnight"] = 1 if faculty f on call for block b's date
        ***REMOVED*** Only Sun-Thurs nights (weekday 0,1,2,3,6)
        ***REMOVED*** ==================================================
        call = {}
        ...
        if call_eligible:
            ...
            for block in workday_blocks:
                ***REMOVED*** Only Sun-Thurs nights (Sun=6, Mon=0, Tue=1, Wed=2, Thu=3)
                if block.date.weekday() not in (0, 1, 2, 3, 6):
                    continue
                ***REMOVED*** Only one variable per date (not per block/session)
        ...
        variables = {
            ...
            "call_assignments": call,  ***REMOVED*** Overnight call assignments
        }
        ...
        ***REMOVED*** ==================================================
        ***REMOVED*** APPLY CONSTRAINTS FROM MANAGER
        ***REMOVED*** ==================================================
        self.constraint_manager.apply_to_pulp(prob, variables, context)

        ***REMOVED*** ==================================================
        ***REMOVED*** PRESERVE EXISTING ASSIGNMENTS
        ***REMOVED*** ==================================================
        ...
        ***REMOVED*** ==================================================
        ***REMOVED*** OBJECTIVE FUNCTION
        ***REMOVED*** Maximize: coverage * 1000 - equity_penalty - template_balance_penalty
        ***REMOVED***
        ***REMOVED*** Template balance ensures assignments are distributed across rotation
        ***REMOVED*** types, not concentrated in a single rotation (e.g., all Night Float).
        ***REMOVED*** ==================================================
        coverage = pulp.lpSum(x.values())

        ***REMOVED*** Calculate template assignment counts for balance penalty
        template_counts = {}
        ...
        ***REMOVED*** Template balance penalty: penalize max template count to encourage distribution
        ***REMOVED*** This prevents all assignments going to one rotation type
        ...
        ***REMOVED*** Build objective with all penalties
        ...
        ***REMOVED*** ==================================================
        ***REMOVED*** EXTRACT SOLUTION - Overnight Call Assignments
        ***REMOVED*** ==================================================
        call_assignments_result = []
        if call_eligible and call:
            for (f_i, b_i, call_type), var in call.items():
                if pulp.value(var) == 1:
                    ***REMOVED*** Find the faculty and block for this variable
                    faculty_id = None
                    block_id = None
                    for fac in call_eligible:
...
class SolverProgressCallback:
    ...
    def __init__(self, task_id: str, redis_client):
        ...
        try:
            ...
            ***REMOVED*** Create a dynamic class that inherits from CpSolverSolutionCallback
            class _ProgressCallback(cp_model.CpSolverSolutionCallback):
                ...
                def on_solution_callback(self):
                    ...
                    ***REMOVED*** Get current objective value and best bound
                    current_obj = self.ObjectiveValue()
                    best_bound = self.BestObjectiveBound()

                    ***REMOVED*** Calculate optimality gap (0-100%)
                    ***REMOVED*** Gap = |best_bound - current_obj| / |best_bound| * 100
                    gap = 0.0
                    if best_bound != 0:
                        gap = abs(best_bound - current_obj) / abs(best_bound) * 100

                    ***REMOVED*** Estimate progress (inverse of gap, capped at 99% until optimal)
                    ***REMOVED*** Progress = 100 - gap, but cap at 99% for non-optimal solutions
                    ...
                    try:
                        ***REMOVED*** Store in Redis with 5 minute expiry
                        self.redis.setex(
            ...
            ***REMOVED*** Store the callback instance
...
class CPSATSolver(BaseSolver):
    ...
    def solve(
        ...
        ***REMOVED*** Store both 2D (for legacy constraints) and 3D variables
        ***REMOVED*** 2D view: x_2d[r_i, b_i] = OR of all rotations
        x_2d = {}
        for resident in context.residents:
            r_i = context.resident_idx[resident.id]
            for block in workday_blocks:
                ...
                if rotation_vars:
                    ***REMOVED*** Create a 2D indicator: 1 if assigned to any rotation in this block
                    x_2d[r_i, b_i] = model.NewBoolVar(f"x_2d_{r_i}_{b_i}")
                    ***REMOVED*** x_2d = 1 iff at least one rotation is selected
        ...
        ***REMOVED*** ==================================================
        ...
        if call_eligible:
            ***REMOVED*** Track dates already processed (one call per date, not per block)
            call_dates_processed = set()
            call_blocks = [
                block
            ...
            for block in call_blocks:
                ***REMOVED*** Only Sun-Thurs nights (Mon=0, Tue=1, Wed=2, Thu=3, Sun=6)
                if block.date in call_dates_processed:
                    continue
                call_dates_processed.add(block.date)
        ...
        ***REMOVED*** Create progress callback if Redis client is available
        callback = None
        ...
        ***REMOVED*** Solve with or without callback
        if callback:
        ...
        ***REMOVED*** Store final status in Redis if callback was used
        if self.task_id and self.redis_client:
            try:
        ...
        Example response:
            {
                ...
                "status": "solving",  ***REMOVED*** or "completed" / "failed"
                "solver_status": "OPTIMAL",  ***REMOVED*** OR-Tools status name
...
class HybridSolver(BaseSolver):
    ...
    def solve(
        ...
        ***REMOVED*** Try CP-SAT first (usually gives better results)
...
class GreedySolver(BaseSolver):
    ...
    def solve(
        ...
        Algorithm:
            1. Calculate difficulty score for each block (***REMOVED*** of eligible residents)
        ...
        ***REMOVED*** Initialize explainability service
        explainability = None
        ...
        ***REMOVED*** Calculate difficulty (eligible resident count) for each block
        ...
        ***REMOVED*** Sort blocks by difficulty (hardest first)
        sorted_blocks = sorted(
            [b for b in workday_blocks if b.id not in assigned_blocks],
            key=count_eligible,
        )

        ***REMOVED*** Assign residents with explanations
        assignments = []
        explanations = {}

        for block in sorted_blocks:
            ***REMOVED*** Find eligible residents
            eligible = [
                r
                for r in context.residents
                if self._is_available(r.id, block.id, context)
            ...
            ***REMOVED*** Score each candidate (lower is better for greedy equity)
            ***REMOVED*** Score = current assignments (we want to minimize this)
            candidate_scores = {}
            for r in eligible:
                ***REMOVED*** Invert so higher score = better candidate
                ***REMOVED*** Max assignments across all residents - this resident's count
                max_assigns = (
                    max(assignment_counts.values()) if assignment_counts else 0
                )
                candidate_scores[r.id] = (max_assigns - assignment_counts[r.id]) * 100

            ***REMOVED*** Select resident with fewest assignments (highest inverted score)
            selected = max(eligible, key=lambda r: candidate_scores[r.id])

            ***REMOVED*** Select best available template with rotation distribution
            ***REMOVED*** Instead of picking first valid template, distribute across templates
            template = None
            valid_templates = []
            for t in context.templates:
                ***REMOVED*** Skip if requires procedure credential
                ...
                ***REMOVED*** This template is valid - add to candidates
                valid_templates.append(t)

            if valid_templates:
                ***REMOVED*** Select template with fewest total assignments for even distribution
                ***REMOVED*** This prevents all assignments going to the first template
                template = min(
                    valid_templates,
            ...
            if not template:
                ***REMOVED*** No valid template available, skip this block
                logger.debug(
                    f"No valid rotation template for resident {selected.name} in block {block.id}"
                )
                continue

            ***REMOVED*** Generate explanation for this decision
            if explainability:
                explanation = explainability.explain_assignment(
                    ...
                    score_breakdown={
                        ...
                        "coverage": COVERAGE_WEIGHT,  ***REMOVED*** Base coverage value
                    },
                )
        ...
        ***REMOVED*** Calculate confidence distribution
        high_conf = sum(
...
class SolverFactory:
    ...
    Example:
        >>> ***REMOVED*** Create a CP-SAT solver with custom timeout
        ...
        >>> ***REMOVED*** Create a greedy solver with explanations
        >>> solver = SolverFactory.create(
        ...     "greedy",
    ...
    _solvers = {
        ...
        "cp_sat": CPSATSolver,
        "hybrid": HybridSolver,
        "pyomo": None,  ***REMOVED*** Lazy-loaded to avoid import if not used
        "quantum": None,  ***REMOVED*** Lazy-loaded quantum-inspired solver
        "qubo": None,  ***REMOVED*** Lazy-loaded PyQUBO-based solver
        "quantum_sa": None,  ***REMOVED*** Lazy-loaded simulated quantum annealing
    }
    ...
    def create(
        ...
        **kwargs,
        ...
        Example:
            >>> ***REMOVED*** Create CP-SAT solver with long timeout
            >>> solver = SolverFactory.create(
            ...     "cp_sat",
            ...
            >>> ***REMOVED*** Create custom constraint manager
        ...
        ***REMOVED*** Handle lazy-loaded solvers
        if name == "pyomo":
