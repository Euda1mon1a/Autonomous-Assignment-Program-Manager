...
***REMOVED*** Penrose Process Constants
PENROSE_EFFICIENCY_LIMIT = 0.29  ***REMOVED*** Theoretical maximum extraction from rotating black holes (29%)
PM_HOUR_OFFSET = 12  ***REMOVED*** Hour offset for PM blocks (noon)
AM_HOUR_OFFSET = 8  ***REMOVED*** Hour offset for AM blocks (8 AM)
HALF_DAY_BLOCK_HOURS = 4  ***REMOVED*** Duration of a half-day block
MIN_REST_HOURS = 8  ***REMOVED*** Minimum rest period between shifts
PRE_TRANSITION_HOURS = 12  ***REMOVED*** Hours before transition for pre-phase
POST_TRANSITION_HOURS = 12  ***REMOVED*** Hours after transition for post-phase

***REMOVED*** Rotation Velocity Calculation Constants
BASE_VELOCITY_SWAPS_PER_DAY = 1.5  ***REMOVED*** Base average swaps per day
WEEKEND_FACTOR = 2.0  ***REMOVED*** Multiplier for weekend swap activity
BLOCK_TRANSITION_FACTOR = 1.5  ***REMOVED*** Multiplier near block boundaries
HOLIDAY_FACTOR = 1.3  ***REMOVED*** Multiplier during holiday periods
ROTATION_VELOCITY_MULTIPLIER = 0.1  ***REMOVED*** Multiplier for extraction potential from velocity
BLOCK_TRANSITION_VELOCITY_MULTIPLIER = 0.15  ***REMOVED*** Higher multiplier for block transitions
HOURS_PER_DAY = 24  ***REMOVED*** Hours in a day
...
class ErgospherePeriod:
    ...
    start_time: datetime
    end_time: datetime
    rotation_velocity: float  ***REMOVED*** Swaps per day
    extraction_potential: float  ***REMOVED*** 0-1 scale, max ~0.29 (Penrose theoretical limit)
    boundary_type: str  ***REMOVED*** "week_end", "block_transition", "rotation_handoff"
    ...
    def duration_hours(self) -> float:
        ...
        return (self.end_time - self.start_time).total_seconds() / (60 * 60)  ***REMOVED*** Convert seconds to hours
...
class PhaseComponent:
    ...
    assignment_id: UUID
    phase_type: str
    energy_state: str  ***REMOVED*** "positive", "negative", "zero"
    phase_start: datetime
    phase_end: datetime
    conflict_score: int
    flexibility_score: float  ***REMOVED*** 0-1, higher = more swap options
...
class PenroseSwap:
    ...
    assignment_a: UUID
    assignment_b: UUID
    local_cost: float  ***REMOVED*** Can be negative (local improvement)
    global_benefit: float  ***REMOVED*** Should be positive
    ...
    def extraction_ratio(self) -> float:
        ...
        if abs(self.local_cost) < 0.001:  ***REMOVED*** Avoid division by zero
...
class RotationEnergyTracker:
    ...
    def __init__(self, initial_rotation_energy: float):
        ...
        self.max_extraction_fraction = PENROSE_EFFICIENCY_LIMIT  ***REMOVED*** Penrose theoretical limit
...
class PenroseEfficiencyExtractor:
    ...
    async def identify_ergosphere_periods(
        ...
        ***REMOVED*** Load all blocks in the date range
    ...
    async def _calculate_rotation_velocity(
        ...
        """
        ***REMOVED*** Calculate period duration
        days = (end_date - start_date).days
        if days == 0:
            return 0.0

        ***REMOVED*** Base velocity on average
        base_velocity = BASE_VELOCITY_SWAPS_PER_DAY

        ***REMOVED*** Weekend factor: More swaps happen around weekends
        weekend_factor = WEEKEND_FACTOR if start_date.weekday() >= 4 else 1.0

        ***REMOVED*** Block transition factor: Check if this period is near a block boundary
        ***REMOVED*** Blocks typically start on the 1st and 15th of months
        transition_factor = 1.0
        if start_date.day in range(1, 8) or start_date.day in range(15, 22):
            transition_factor = BLOCK_TRANSITION_FACTOR

        ***REMOVED*** Time of year factor: More swaps during holiday periods
        month = start_date.month
        holiday_multiplier = 1.0
        if month in [11, 12, 6, 7]:  ***REMOVED*** November, December, June, July (holidays)
        ...
        ***REMOVED*** Find transitions between consecutive blocks
        ...
        for i in range(len(sorted_block_nums) - 1):
            ...
            if next_num == current_num + 1:
                ***REMOVED*** Find last block of current and first block of next
                ...
                if current_blocks and next_blocks:
                    ...
                    ***REMOVED*** Block transitions have higher extraction potential
    ...
    async def _count_conflicts_in_period(
        ...
        ***REMOVED*** Get all assignments for this person in the time range
        result = await self.db.execute(
            select(Assignment)
            .join(Block)
            .where(
                Assignment.person_id == assignment.person_id,
                Assignment.id != assignment.id,  ***REMOVED*** Exclude current assignment
            )
        )
        ...
        for other in other_assignments:
            ...
            ***REMOVED*** Convert other assignment to datetime
            ...
            ***REMOVED*** Check for overlap with the period
            if start_time <= other_dt < end_time:
                conflict_count += 1

            ***REMOVED*** Check for back-to-back shifts (less than minimum rest hours apart)
            if assignment.block:
                ...
                time_between = abs((other_dt - assignment_dt).total_seconds() / (60 * 60))  ***REMOVED*** Convert to hours
    ...
    def _calculate_flexibility_score(
        ...
        if not all_assignments:
            return 0.5  ***REMOVED*** Default neutral flexibility

        ***REMOVED*** Count potential swap candidates
        swap_candidates = 0
        ...
        for other in all_assignments:
            ...
            ***REMOVED*** Same rotation type = potential swap candidate
            if other.rotation_template_id == assignment.rotation_template_id:
                swap_candidates += 1

            ***REMOVED*** Same role = potential swap candidate
            if other.role == assignment.role:
                swap_candidates += 0.5

        if total_other_assignments == 0:
            return 0.3  ***REMOVED*** Low flexibility if no other assignments

        ***REMOVED*** Flexibility is proportion of potential swap candidates
        flexibility = swap_candidates / total_other_assignments
        return min(1.0, flexibility)  ***REMOVED*** Cap at 1.0
    ...
    def _calculate_confidence_score(
        ...
        """
        ***REMOVED*** Base confidence from data quality
        ***REMOVED*** Confidence increases logarithmically with data size
        if historical_data_size == 0:
            data_confidence = 0.2  ***REMOVED*** Minimum baseline
        else:
            ***REMOVED*** Scale from 0.2 to 1.0 based on data size
            ***REMOVED*** 10 data points = ~0.5, 100 = ~0.8, 1000 = ~1.0
            data_confidence = min(1.0, 0.2 + (0.8 * (historical_data_size / 1000)))

        ***REMOVED*** Similarity bonus: same rotation type increases confidence
        ...
        ***REMOVED*** Role match bonus
        if assignment_a.role == assignment_b.role:
            similarity_bonus += 0.05

        ***REMOVED*** Temporal proximity: same week increases confidence
        temporal_bonus = 0.0
    ...
    async def decompose_into_phases(
        ...
        ***REMOVED*** Determine energy states based on conflict patterns
        ***REMOVED*** Negative energy = transition reduces conflicts
        ...
        phases.extend(
            [
                ...
                PhaseComponent(
                    ...
                    flexibility_score=flexibility * 0.7,  ***REMOVED*** Slightly lower during transition
                ),
                PhaseComponent(
                    assignment_id=assignment.id,
    ...
    async def find_negative_energy_swaps(
        ...
        ***REMOVED*** Get assignments in this ergosphere period
        result = await self.db.execute(
            select(Assignment)
            .join(Block)
            .where(Assignment.id.in_(period.affected_assignments))
            .options()  ***REMOVED*** Could add selectinload for relations
        )
        assignments = result.scalars().all()

        ***REMOVED*** Find potential swap pairs
        for i, assign_a in enumerate(assignments):
            for assign_b in assignments[i + 1 :]:
                ***REMOVED*** Calculate local and global impacts
                ...
                ***REMOVED*** Only include if globally beneficial
                if global_benefit > abs(local_cost):
                    ***REMOVED*** Calculate confidence based on historical data and assignment similarity
        ...
        ***REMOVED*** Sort by net extraction (highest first)
    ...
    async def _calculate_local_cost(
        ...
        """
        ***REMOVED*** Cost factors weighted by impact
        cost = 0.0

        ***REMOVED*** 1. Skill/qualification mismatch (highest cost)
        if assign_a.rotation_template_id != assign_b.rotation_template_id:
            cost += 2.0  ***REMOVED*** Different rotation types = higher cost

        ***REMOVED*** 2. Role mismatch (medium cost)
        if assign_a.role != assign_b.role:
            cost += 1.5

        ***REMOVED*** 3. Person preference mismatch (estimated from historical patterns)
        ***REMOVED*** Higher cost if persons have consistently preferred different rotations
    ...
    async def _calculate_global_benefit(
        ...
        ***REMOVED*** 1. Workload balance improvement
        ***REMOVED*** Count assignments per person before and after swap
        ...
        ***REMOVED*** Calculate workload variance before swap
        workload_before_variance = abs(person_a_count - person_b_count)

        ***REMOVED*** After swap, counts would be swapped for these specific assignments
        workload_after_variance = abs((person_a_count - 1) - (person_b_count + 1))

        if workload_after_variance < workload_before_variance:
            benefit += 3.0  ***REMOVED*** Improved balance

        ***REMOVED*** 2. ACGME compliance improvement
        ***REMOVED*** Check if swap reduces consecutive shifts
        if assign_a.block and assign_b.block:
            date_diff = abs((assign_a.block.date - assign_b.block.date).days)

            ***REMOVED*** If assignments are close together (< 2 days), swapping might reduce fatigue
            if date_diff <= 2:
                benefit += 2.5

            ***REMOVED*** If different weeks, swapping helps distribute work
            if date_diff >= 7:
                benefit += 1.5

        ***REMOVED*** 3. Coverage distribution improvement
        ***REMOVED*** If swapping provides better specialty coverage (different rotations)
        if (
            ...
            benefit += 2.0  ***REMOVED*** Cross-training benefit

        ***REMOVED*** 4. Rotation variety improvement
        ***REMOVED*** Count how many times each person has done their assigned rotation
        ...
        ***REMOVED*** If person has done this rotation many times, swapping provides variety
    ...
    def compute_extraction_efficiency(self, swaps_executed: list[PenroseSwap]) -> float:
        ...
        ***REMOVED*** Normalize by initial rotation energy if tracker is initialized
        if self.energy_tracker:
            efficiency = total_extraction / self.energy_tracker.initial_rotation_energy
            efficiency = min(efficiency, PENROSE_EFFICIENCY_LIMIT)  ***REMOVED*** Cap at Penrose limit
        else:
            ***REMOVED*** Without tracker, estimate based on number of swaps
    ...
    async def execute_penrose_cascade(
        ...
        """
        ***REMOVED*** Calculate initial schedule energy (flexibility/swap potential)
        ***REMOVED*** Get all assignments to assess schedule flexibility
        result = await self.db.execute(select(Assignment))
        all_assignments = result.scalars().all()

        ***REMOVED*** Initial energy based on:
        ***REMOVED*** 1. Number of assignments (more = more swap potential)
        ***REMOVED*** 2. Variety of rotations (diverse = more flexibility)
        ***REMOVED*** 3. Person distribution (balanced = easier to swap)
        ...
        ***REMOVED*** Energy formula: base energy scales with assignment count and diversity
        initial_energy = (
            num_assignments * 0.5  ***REMOVED*** Base from assignment count
            + unique_rotations * 5.0  ***REMOVED*** Bonus for rotation variety
            + unique_persons * 3.0  ***REMOVED*** Bonus for person diversity
        )
        initial_energy = max(100.0, initial_energy)  ***REMOVED*** Minimum baseline energy
        ...
        while iteration < max_iterations and not self.energy_tracker.is_exhausted:
            ...
            ***REMOVED*** Get actual schedule date range from assignments
            if all_assignments:
            ...
            ***REMOVED*** Find swaps in high-potential ergospheres
        ...
        ***REMOVED*** Calculate final conflict count across all assignments
        final_conflicts = 0
        for assignment in all_assignments:
            if assignment.block:
                ***REMOVED*** Count conflicts for this assignment
                assignment_dt = datetime.combine(
                    assignment.block.date, datetime.min.time()
                ) + timedelta(hours=PM_HOUR_OFFSET if assignment.block.time_of_day == "PM" else 0)
        ...
        result = {
            ...
            "improvement": efficiency * 100,  ***REMOVED*** Percentage improvement
    ...
    ***REMOVED*** Helper methods for conflict and flexibility estimation
    ...
    async def _estimate_conflicts_in_period(
        ...
        """
        ***REMOVED*** Simplified heuristic: conflicts are higher during transitions
        duration_hours = (end - start).total_seconds() / 3600
        if duration_hours <= 4:
            ***REMOVED*** Transition periods have fewer conflicts
            return 0
        elif duration_hours <= 12:
            ***REMOVED*** Standard half-day blocks
            return 1
        else:
            ***REMOVED*** Longer periods may have multiple conflicts
    ...
    def _calculate_flexibility_score(
        ...
        """
        ***REMOVED*** Flexibility based on phase type
        if phase_type == "transition":
            ***REMOVED*** Transition periods are less flexible
            return 0.5
        elif phase_type == "pre_transition":
            ***REMOVED*** Pre-transition has moderate flexibility
            return 0.7
        else:  ***REMOVED*** post_transition
            ***REMOVED*** Post-transition has moderate flexibility
            return 0.7
    ...
    def _calculate_swap_confidence(
        ...
        """
        ***REMOVED*** Base confidence
        confidence = 0.7

        ***REMOVED*** Increase confidence with more context
        if total_assignments > 50:
            confidence += 0.1
        if total_assignments > 100:
            confidence += 0.1

        ***REMOVED*** Decrease confidence if assignments lack data
    ...
    async def _estimate_preference_mismatch(
        ...
        """
        ***REMOVED*** Heuristic: assume 0.3 base mismatch cost
        return 0.3
    ...
    def _estimate_workload_balance_improvement(
        ...
        """
        ***REMOVED*** Simplified: assume moderate improvement
        return 0.6
    ...
    async def _estimate_acgme_improvement(
        ...
        """
        ***REMOVED*** Heuristic: swaps during off-peak times improve compliance
        return 0.4
    ...
    def _estimate_coverage_improvement(
        ...
        """
        ***REMOVED*** Different rotations indicate coverage rebalancing
        if assign_a.rotation_template_id != assign_b.rotation_template_id:
            return 0.7
        return 0.2
    ...
    async def _estimate_preference_improvement(
        ...
        """
        ***REMOVED*** Heuristic: assume moderate preference improvement
        return 0.5
    ...
    async def _calculate_initial_rotation_energy(self, schedule_id: UUID) -> float:
        ...
        """
        ***REMOVED*** Query schedule complexity metrics
        ***REMOVED*** Base energy on number of potential swap opportunities
        result = await self.db.execute(
            select(Assignment).where(Assignment.id == schedule_id).limit(100)
        )
        assignments = result.scalars().all()

        ***REMOVED*** Energy scales with number of assignments and diversity
        base_energy = len(assignments) * 0.5  ***REMOVED*** 0.5 energy per assignment
        diversity_factor = 1.2  ***REMOVED*** Assume 20% diversity bonus
    ...
    async def _get_schedule_date_range(self, schedule_id: UUID) -> dict[str, date]:
        ...
        """
        ***REMOVED*** Query min and max dates from assignments
        result = await self.db.execute(
            select(Assignment)
            .join(Block)
        ...
        if not first_assignment or not first_assignment.block:
            ***REMOVED*** Default to current date + 28 days
            return {"start": date.today(), "end": date.today() + timedelta(days=28)}

        ***REMOVED*** Use block date as reference
        start_date = first_assignment.block.date
        end_date = start_date + timedelta(days=28)  ***REMOVED*** One block period
    ...
    async def _estimate_total_conflicts(self, schedule_id: UUID) -> int:
        ...
        """
        ***REMOVED*** Query assignments and estimate conflicts
        result = await self.db.execute(
            select(Assignment).where(Assignment.id == schedule_id).limit(100)
        )
        assignments = result.scalars().all()

        ***REMOVED*** Heuristic: ~5% of assignments have conflicts
