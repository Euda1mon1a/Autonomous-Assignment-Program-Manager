...
class ConstraintRigidityAnalyzer:
    ...
    def __init__(self) -> None:
        ...
        self.pebbles: dict[str, list[str]] = {}  ***REMOVED*** node -> list of pebble ids
        self.edge_status: dict[tuple[str, str], str] = {}  ***REMOVED*** edge -> "independent" or "redundant"
        self.parent_map: dict[str, Optional[str]] = {}  ***REMOVED*** for pebble search
    ...
    def build_constraint_graph(
        ...
        ***REMOVED*** Add nodes for each task
        for task in tasks:
            task_id = task['id']
            graph.add_node(task_id, **{k: v for k, v in task.items() if k != 'id'})

        ***REMOVED*** Add edges for each constraint
        for i, constraint in enumerate(constraints):
            constraint_tasks = constraint.get('tasks', [])
            constraint_type = constraint.get('type', 'unknown')

            ***REMOVED*** Most constraints are binary (between two tasks)
            if len(constraint_tasks) >= 2:
                ***REMOVED*** For multi-task constraints, create edges between all pairs
                for j in range(len(constraint_tasks)):
                    for k in range(j + 1, len(constraint_tasks)):
                        ...
                        if task_a in graph and task_b in graph:
                            ***REMOVED*** Add edge with constraint metadata
                            graph.add_edge(
                                task_a,
                                task_b,
                                constraint_id=i,
    ...
    def run_pebble_game(self, graph: nx.Graph) -> dict[str, Any]:
        ...
        ***REMOVED*** Initialize: 2 pebbles per node
        self.pebbles = {}
        ...
        ***REMOVED*** Process each edge
        for u, v in self.graph.edges():
            ***REMOVED*** Check (2,3)-sparsity condition before trying to add the edge
            ***REMOVED*** For any subgraph, |E| <= 2|V| - 3
            ***REMOVED*** Check if adding this edge would violate sparsity in any subgraph
            if self._would_violate_sparsity(u, v):
                self.edge_status[(u, v)] = "redundant"
                redundant_count += 1
            else:
                ***REMOVED*** Try to find a pebble to cover this edge
        ...
        ***REMOVED*** Count remaining pebbles (degrees of freedom)
    ...
    def _would_violate_sparsity(self, u: str, v: str) -> bool:
        ...
        """
        ***REMOVED*** Find the subgraph of nodes connected by independent edges
        ***REMOVED*** that includes u and v
        connected_nodes = set()
        to_explore = {u, v}
        ...
        while to_explore:
            ...
            ***REMOVED*** Explore neighbors through independent edges
        ...
        ***REMOVED*** Count independent edges in this subgraph
        ...
        ***REMOVED*** Check if adding one more edge would violate sparsity
        ***REMOVED*** |E| + 1 <= 2|V| - 3
    ...
    def _find_and_place_pebble(self, u: str, v: str) -> bool:
        ...
        """
        ***REMOVED*** Quick check: do either endpoint have free pebbles?
        ...
        ***REMOVED*** Need to search for a pebble through the graph
        ***REMOVED*** Try searching from u
        if self._search_for_pebble(u, set()):
            return True

        ***REMOVED*** Try searching from v
    ...
    def _search_for_pebble(self, start: str, visited: set[str]) -> bool:
        ...
        while queue:
            ...
            ***REMOVED*** Check if this node has a free pebble
            if len(self.pebbles.get(node, [])) > 0:
                self.pebbles[node].pop()
                return True

            ***REMOVED*** Explore neighbors through independent edges only
            for neighbor in self.graph.neighbors(node):
                if neighbor not in visited:
                    ***REMOVED*** Create normalized edge key (always smaller node first)
                    edge_key = (min(node, neighbor), max(node, neighbor))

                    ***REMOVED*** Can only traverse edges that are already covered (independent)
                    if edge_key in self.edge_status and self.edge_status[edge_key] == "independent":
    ...
    def rearrange_pebbles(self, node: str, target: str) -> bool:
        ...
        """
        ***REMOVED*** Use BFS to find a pebble
        queue = deque([node])
        visited = {node}
        ...
        while queue:
            ...
            ***REMOVED*** Found a node with a free pebble
            if len(self.pebbles.get(current, [])) > 0:
                ***REMOVED*** Remove the pebble from current
                self.pebbles[current].pop()
                return True

            ***REMOVED*** Explore neighbors
    ...
    def identify_flexible_regions(self) -> list[set[str]]:
        ...
        for component in nx.connected_components(self.graph):
            ...
            ***REMOVED*** Count pebbles remaining in this component
            pebbles_in_component = sum(
                len(self.pebbles.get(node, []))
                for node in component
            )

            ***REMOVED*** Count independent edges
            independent_edges = sum(
                1 for edge in subgraph.edges()
                if self.edge_status.get((min(edge), max(edge)), "") == "independent"
            )

            ***REMOVED*** Flexible if has degrees of freedom remaining
            expected_for_rigid = 2 * num_nodes - 3
    ...
    def identify_stressed_regions(self) -> list[set[str]]:
        ...
        for component in nx.connected_components(self.graph):
            ...
            ***REMOVED*** Check if this component has any redundant edges
            has_redundant = any(
    ...
    def recommend_constraint_changes(self) -> list[dict[str, Any]]:
        ...
        ***REMOVED*** Analyze flexible regions (under-constrained)
        flexible_regions = self.identify_flexible_regions()
        for region in flexible_regions:
            num_nodes = len(region)
            subgraph = self.graph.subgraph(region)

            ***REMOVED*** Count current independent edges
            independent_edges = sum(
                1 for edge in subgraph.edges()
                if self.edge_status.get((min(edge), max(edge)), "") == "independent"
            )

            ***REMOVED*** Calculate how many more edges needed for rigidity
            expected_for_rigid = 2 * num_nodes - 3
