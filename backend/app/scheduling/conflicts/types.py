...
class ConflictCategory(str, Enum):
    ...
    TIME_OVERLAP = "time_overlap"  ***REMOVED*** Multiple assignments at same time
    RESOURCE_CONTENTION = "resource_contention"  ***REMOVED*** Insufficient resources
    ACGME_VIOLATION = "acgme_violation"  ***REMOVED*** ACGME compliance violation
    SUPERVISION_ISSUE = "supervision_issue"  ***REMOVED*** Inadequate supervision
    AVAILABILITY_CONFLICT = "availability_conflict"  ***REMOVED*** Assignment during absence
    WORKLOAD_IMBALANCE = "workload_imbalance"  ***REMOVED*** Unfair distribution
    PATTERN_VIOLATION = "pattern_violation"  ***REMOVED*** Undesirable patterns
...
class ConflictType(str, Enum):
    ...
    ***REMOVED*** Time overlap conflicts
    DOUBLE_BOOKING = "double_booking"  ***REMOVED*** Person scheduled twice at same time
    OVERLAPPING_SHIFTS = "overlapping_shifts"  ***REMOVED*** Shifts overlap in time

    ***REMOVED*** Resource contention
    INSUFFICIENT_COVERAGE = "insufficient_coverage"  ***REMOVED*** Not enough people
    SUPERVISION_RATIO_VIOLATION = "supervision_ratio_violation"  ***REMOVED*** Too few faculty
    FACILITY_OVERBOOKED = "facility_overbooked"  ***REMOVED*** Too many in one location

    ***REMOVED*** ACGME violations
    EIGHTY_HOUR_VIOLATION = "eighty_hour_violation"  ***REMOVED*** Exceeds 80 hours/week
    ONE_IN_SEVEN_VIOLATION = "one_in_seven_violation"  ***REMOVED*** No day off in 7 days
    CONTINUOUS_DUTY_VIOLATION = "continuous_duty_violation"  ***REMOVED*** Exceeds 24+4 hours
    NIGHT_FLOAT_VIOLATION = "night_float_violation"  ***REMOVED*** Exceeds 6 consecutive nights
    PGY_SHIFT_LENGTH_VIOLATION = "pgy_shift_length_violation"  ***REMOVED*** Exceeds PGY limits

    ***REMOVED*** Supervision issues
    UNSUPERVISED_RESIDENT = "unsupervised_resident"  ***REMOVED*** Resident without faculty
    INADEQUATE_SUPERVISION = "inadequate_supervision"  ***REMOVED*** Wrong supervision ratio

    ***REMOVED*** Availability conflicts
    ASSIGNED_DURING_ABSENCE = "assigned_during_absence"  ***REMOVED*** Scheduled during leave
    ASSIGNED_DURING_DEPLOYMENT = "assigned_during_deployment"  ***REMOVED*** Military duty
    ASSIGNED_DURING_TDY = "assigned_during_tdy"  ***REMOVED*** Temporary duty

    ***REMOVED*** Workload imbalance
    EXCESSIVE_WORKLOAD = "excessive_workload"  ***REMOVED*** Too many assignments
    INSUFFICIENT_WORKLOAD = "insufficient_workload"  ***REMOVED*** Too few assignments
    UNFAIR_CALL_DISTRIBUTION = "unfair_call_distribution"  ***REMOVED*** Unequal call burden

    ***REMOVED*** Pattern violations
    EXCESSIVE_CONSECUTIVE_DAYS = "excessive_consecutive_days"  ***REMOVED*** Too many days in a row
    EXCESSIVE_BACK_TO_BACK = "excessive_back_to_back"  ***REMOVED*** Too many back-to-back shifts
    POOR_ROTATION_SEQUENCING = "poor_rotation_sequencing"  ***REMOVED*** Suboptimal rotation order
...
class Conflict(BaseModel):
    ...
    ***REMOVED*** Temporal information
    start_date: date
    end_date: date
    detected_at: datetime = Field(default_factory=datetime.utcnow)

    ***REMOVED*** Affected entities
    ...
    ***REMOVED*** Severity scoring (0.0 - 1.0)
    ...
    ***REMOVED*** Resolution metadata
    is_auto_resolvable: bool = Field(default=False)
    resolution_difficulty: str = Field(default="medium")  ***REMOVED*** easy, medium, hard
...
class TimeOverlapConflict(Conflict):
    ...
    ***REMOVED*** Specific fields for time overlap
...
class ACGMEViolationConflict(Conflict):
    ...
    severity: ConflictSeverity = ConflictSeverity.CRITICAL  ***REMOVED*** Always critical

    ***REMOVED*** ACGME-specific information
    acgme_rule: str = Field(description="Which ACGME rule is violated")
    person_id: UUID
    person_name: str
    pgy_level: int | None = None

    ***REMOVED*** Violation details
    ...
    ***REMOVED*** Duty hour details (if applicable)
