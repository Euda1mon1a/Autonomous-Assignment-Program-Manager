...
class ConflictVisualizer:
    ...
    ***REMOVED*** Severity score mapping
    SEVERITY_SCORES = {
    ...
    async def generate_timeline(
        ...
        ***REMOVED*** Initialize timeline
        timeline = ConflictTimeline(
            start_date=start_date,
            end_date=end_date,
        )

        ***REMOVED*** Group conflicts by date
        conflicts_by_date: dict[date, list[Conflict]] = defaultdict(list)

        for conflict in conflicts:
            ***REMOVED*** Add conflict to all dates in its range
        ...
        ***REMOVED*** Build timeline entries
        current_date = start_date
        while current_date <= end_date:
            date_conflicts = conflicts_by_date.get(current_date, [])

            ***REMOVED*** Calculate severity score for this date
            severity_score = 0.0
            if date_conflicts:
                severity_score = max(
        ...
        ***REMOVED*** Build person-specific timelines
        for conflict in conflicts:
            for person_id in conflict.affected_people:
                ...
                ***REMOVED*** Add all dates in conflict range
                current_date = conflict.start_date
                while current_date <= conflict.end_date and current_date <= end_date:
                    if current_date >= start_date:
        ...
        ***REMOVED*** Generate category timeline (weekly aggregation)
    ...
    async def generate_heatmap_data(
        ...
        ***REMOVED*** Build date -> severity mapping
        ...
        for conflict in conflicts:
            ...
            while current_date <= conflict.end_date:
                date_str = current_date.isoformat()
                ***REMOVED*** Use max severity if multiple conflicts on same date
        ...
        ***REMOVED*** Convert to heatmap data points
        heatmap_data = [
            {
                "date": date_str,
                "value": severity,
        ...
        ***REMOVED*** Sort by date
        ...
        return {
            ...
            "color_scale": "red",
            "levels": 4,  ***REMOVED*** LOW, MEDIUM, HIGH, CRITICAL
        }
    ...
    async def generate_gantt_data(
        ...
        ***REMOVED*** Sort by start date then severity
        gantt_data.sort(
            key=lambda x: (
                x["start"],
    ...
    def _get_severity_color(self, severity: ConflictSeverity) -> str:
        ...
        Returns:
            Hex color code string:
                - CRITICAL: ***REMOVED***DC2626 (red-600)
                - HIGH: ***REMOVED***EA580C (orange-600)
                - MEDIUM: ***REMOVED***F59E0B (amber-500)
                - LOW: ***REMOVED***FCD34D (amber-300)
                - Default: ***REMOVED***9CA3AF (gray-400)
        """
        colors = {
            ConflictSeverity.CRITICAL: "***REMOVED***DC2626",  ***REMOVED*** red-600
            ConflictSeverity.HIGH: "***REMOVED***EA580C",  ***REMOVED*** orange-600
            ConflictSeverity.MEDIUM: "***REMOVED***F59E0B",  ***REMOVED*** amber-500
            ConflictSeverity.LOW: "***REMOVED***FCD34D",  ***REMOVED*** amber-300
        }
        return colors.get(severity, "***REMOVED***9CA3AF")  ***REMOVED*** gray-400 default
    ...
    async def generate_person_impact_chart(
        ...
        """
        ***REMOVED*** Count conflicts per person
        ...
        ***REMOVED*** Sort by conflict count (descending)
