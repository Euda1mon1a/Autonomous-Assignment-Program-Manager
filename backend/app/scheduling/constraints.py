...
class ConstraintPriority(Enum):
    ...
    CRITICAL = 100  ***REMOVED*** ACGME compliance, must satisfy
    HIGH = 75       ***REMOVED*** Important operational constraints
    MEDIUM = 50     ***REMOVED*** Preferences and soft requirements
    LOW = 25        ***REMOVED*** Nice-to-have optimizations
...
class ConstraintType(Enum):
    ...
    CALL = "call"
    SPECIALTY = "specialty"
    ***REMOVED*** Resilience-aware constraint types (Tier 1)
    RESILIENCE = "resilience"
    HUB_PROTECTION = "hub_protection"
    UTILIZATION_BUFFER = "utilization_buffer"
    ***REMOVED*** Resilience-aware constraint types (Tier 2)
...
class ConstraintViolation:
    ...
    severity: str  ***REMOVED*** CRITICAL, HIGH, MEDIUM, LOW
    message: str
...
class ConstraintResult:
    ...
    penalty: float = 0.0  ***REMOVED*** For soft constraints
...
class SchedulingContext:
    ...
    """
    residents: list  ***REMOVED*** List of Person objects
    faculty: list    ***REMOVED*** List of Person objects
    blocks: list     ***REMOVED*** List of Block objects
    templates: list  ***REMOVED*** List of RotationTemplate objects

    ***REMOVED*** Lookup dictionaries for fast access
    ...
    ***REMOVED*** Availability matrix: {person_id: {block_id: {'available': bool, 'replacement': str}}}
    availability: dict[UUID, dict[UUID, dict]] = field(default_factory=dict)

    ***REMOVED*** Existing assignments (for incremental scheduling)
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Resilience Data (populated by ResilienceService)
    ***REMOVED*** =========================================================================

    ***REMOVED*** Hub vulnerability scores: {faculty_id: composite_score (0.0-1.0)}
    ***REMOVED*** Higher scores = more critical = should be protected from over-assignment
    hub_scores: dict[UUID, float] = field(default_factory=dict)

    ***REMOVED*** Current system utilization rate (0.0-1.0)
    ***REMOVED*** Target: <0.80 to maintain 20% buffer per queuing theory
    current_utilization: float = 0.0

    ***REMOVED*** Faculty whose loss creates N-1 vulnerability (single point of failure)
    n1_vulnerable_faculty: set[UUID] = field(default_factory=set)

    ***REMOVED*** Preference trails from stigmergy: {faculty_id: {slot_type: strength}}
    ***REMOVED*** Used to weight assignments based on learned preferences
    preference_trails: dict[UUID, dict[str, float]] = field(default_factory=dict)

    ***REMOVED*** Zone assignments: {faculty_id: zone_id}
    ***REMOVED*** For blast radius isolation - faculty should primarily work in their zone
    zone_assignments: dict[UUID, UUID] = field(default_factory=dict)

    ***REMOVED*** Block zone assignments: {block_id: zone_id}
    ***REMOVED*** Maps blocks to zones for cross-zone penalty calculation
    block_zones: dict[UUID, UUID] = field(default_factory=dict)

    ***REMOVED*** Target utilization for buffer constraint (default 80%)
...
***REMOVED*** =============================================================================
***REMOVED*** HARD CONSTRAINTS - ACGME Compliance
***REMOVED*** =============================================================================
...
class EightyHourRuleConstraint(HardConstraint):
    ...
    def __init__(self):
        ...
        ***REMOVED*** Max blocks per 4-week window: (80 * 4) / 6 = 53.33 -> 53
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** For each possible 28-day window starting point
        for window_start in dates:
            ...
            ***REMOVED*** For each resident, sum of blocks in window <= max
            for resident in context.residents:
                r_i = context.resident_idx[resident.id]
                window_vars = [
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Group assignments by resident
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Check each 28-day window
            for start_date in sorted_dates:
                ...
                if avg_weekly > self.MAX_WEEKLY_HOURS:
                    ...
                    break  ***REMOVED*** One violation per resident is enough
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Check each possible 7-day window
            for start_idx in range(len(dates)):
                ...
                ***REMOVED*** Get 7 consecutive calendar days
                consecutive_dates = []
                ...
                ***REMOVED*** Create indicator variables for each day
                day_worked_vars = []
                ...
                ***REMOVED*** At most 6 days worked in any 7-day window
    ...
    def add_to_pulp(self, model, variables: dict, context: SchedulingContext):
        ...
        for resident in context.residents:
            ...
            for start_idx in range(len(dates)):
                ...
                ***REMOVED*** Sum of all blocks across 7 days <= 6 * 2 (max 2 blocks per day)
                ***REMOVED*** This is a relaxation, but works for most cases
                all_vars = []
                for d in consecutive_dates[:self.MAX_CONSECUTIVE_DAYS + 1]:
...
class SupervisionRatioConstraint(HardConstraint):
    ...
    PGY1_RATIO = 2  ***REMOVED*** 1 faculty per 2 PGY-1
    OTHER_RATIO = 4  ***REMOVED*** 1 faculty per 4 PGY-2/3
    ...
    def calculate_required_faculty(self, pgy1_count: int, other_count: int) -> int:
        ...
        Example:
            ...
            3  ***REMOVED*** 2 for PGY-1 (⌈3/2⌉=2), 2 for others (⌈5/4⌉=2), but max(1,2+2)=3
        """
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** This constraint is usually enforced during faculty assignment phase
        ***REMOVED*** The CP-SAT model focuses on resident assignment
        pass
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Create person type lookup
        person_types = {}
        pgy_levels = {}
        for r in context.residents:
...
class ClinicCapacityConstraint(HardConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        template_vars = variables.get("template_assignments", {})  ***REMOVED*** (r, b, t) -> var

        if not template_vars:
            return  ***REMOVED*** No template-specific variables

        ***REMOVED*** Group by (block, template)
        for block in context.blocks:
            b_i = context.block_idx[block.id]
...
class MaxPhysiciansInClinicConstraint(HardConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Identify clinic templates
        clinic_template_ids = {
        ...
        if not clinic_template_ids:
            return  ***REMOVED*** No clinic templates defined

        ***REMOVED*** For each block, sum all clinic assignments (residents + faculty)
        for block in context.blocks:
            b_i = context.block_idx[block.id]
            clinic_vars = []

            ***REMOVED*** Collect resident assignments to clinic templates
            if template_vars:
                for template in context.templates:
            ...
            ***REMOVED*** Faculty assignments are typically handled post-hoc,
            ***REMOVED*** but if faculty variables exist, include them
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Count all persons (faculty + residents) per clinic block
...
class WednesdayAMInternOnlyConstraint(HardConstraint):
    ...
    WEDNESDAY = 2  ***REMOVED*** Python weekday: Monday=0, Tuesday=1, Wednesday=2
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Get PGY levels for residents
        pgy_levels = {r.id: r.pgy_level for r in context.residents}

        ***REMOVED*** For Wednesday AM blocks, prevent non-PGY-1 clinic assignments
        for block in context.blocks:
            ...
            for template in context.templates:
                ...
                for r in context.residents:
                    ***REMOVED*** Skip PGY-1 (they are allowed)
                    if pgy_levels.get(r.id) == 1:
                        continue

                    r_i = context.resident_idx[r.id]
                    if (r_i, b_i, t_i) in template_vars:
                        ***REMOVED*** Force non-intern to 0 on Wednesday AM clinic
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Build lookup tables
        ...
        for a in assignments:
            ***REMOVED*** Only check clinic assignments
            if a.rotation_template_id not in clinic_template_ids:
                continue

            ***REMOVED*** Only check resident assignments (not faculty)
            ...
            ***REMOVED*** Check if resident is NOT PGY-1
...
class EquityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Check if residents have individual targets
        ...
        if has_individual_targets:
            ***REMOVED*** Use individual targets - penalize deviation from each resident's target
            total_deviation = 0
            for resident in context.residents:
                r_i = context.resident_idx[resident.id]
                resident_total = sum(
                ...
                if hasattr(resident, 'target_clinical_blocks') and resident.target_clinical_blocks:
                    ...
                    ***REMOVED*** Create deviation variable (absolute value approximation)
            ...
            ***REMOVED*** Fall back to original equity logic (minimize max assignments)
            ...
            ***REMOVED*** Use individual targets - penalize deviation from each resident's target
            ...
            for resident in context.residents:
                ...
                if resident_vars and hasattr(resident, 'target_clinical_blocks') and resident.target_clinical_blocks:
                    ...
                    ***REMOVED*** Create deviation variables (absolute value via two inequalities)
                    ...
                    ***REMOVED*** resident_total - target = deviation_pos - deviation_neg
            ...
            if total_deviation:
                ...
                ***REMOVED*** No targets set, use original logic
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Penalty based on spread
...
class ContinuityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** This would require tracking template assignments across consecutive blocks
        ***REMOVED*** For simplicity, we handle this in post-processing or validation
        pass
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Group by resident, sorted by date
        by_resident = defaultdict(list)
        for a in assignments:
            for b in context.blocks:
                if b.id == a.block_id:
        ...
        for _person_id, date_templates in by_resident.items():
            ...
            for i in range(1, len(sorted_dt)):
                if sorted_dt[i][1] != sorted_dt[i-1][1]:
                    ***REMOVED*** Different template on consecutive assignment
...
class PreferenceConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Would require template-specific assignment variables
        pass
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        for a in assignments:
            ...
            if a.rotation_template_id:
                pref_score = person_prefs.get(a.rotation_template_id, 0.5)  ***REMOVED*** Neutral default
...
***REMOVED*** =============================================================================
***REMOVED*** RESILIENCE-AWARE SOFT CONSTRAINTS
***REMOVED*** =============================================================================
...
class HubProtectionConstraint(SoftConstraint):
    ...
    ***REMOVED*** Hub score thresholds
    HIGH_HUB_THRESHOLD = 0.4      ***REMOVED*** Above this = significant hub
    CRITICAL_HUB_THRESHOLD = 0.6  ***REMOVED*** Above this = critical hub (2x penalty)
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        if not x or not context.hub_scores:
            return  ***REMOVED*** No resilience data available
        ...
        for faculty in context.faculty:
            ...
            if hub_score < self.HIGH_HUB_THRESHOLD:
                continue  ***REMOVED*** Not a hub, no penalty

            ***REMOVED*** Count assignments to this faculty
            faculty_vars = [
            ...
            ***REMOVED*** Penalty multiplier based on hub criticality
            multiplier = 2.0 if hub_score >= self.CRITICAL_HUB_THRESHOLD else 1.0

            ***REMOVED*** Create penalty variable
            faculty_total = sum(faculty_vars)
            ***REMOVED*** Scale penalty by hub_score and multiplier
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Calculate average assignments
        if faculty_counts:
        ...
        for faculty in context.faculty:
            ...
            ***REMOVED*** Calculate penalty
            penalty = count * hub_score * multiplier * self.weight
            total_penalty += penalty

            ***REMOVED*** Report if hub is over-assigned (> average)
            if count > avg_assignments * 1.2:  ***REMOVED*** 20% above average
...
class UtilizationBufferConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Calculate maximum safe assignments
        ***REMOVED*** Max capacity = faculty * blocks_per_faculty
        ***REMOVED*** Safe capacity = max_capacity * target_utilization
        ...
        ***REMOVED*** Sum all assignments
        total_assignments = sum(x.values())

        ***REMOVED*** Create over-utilization variable
        ...
        ***REMOVED*** Quadratic-ish penalty: over_util * over_util is complex in CP-SAT
        ***REMOVED*** Use linear approximation with higher weight for large overages
    ...
    def add_to_pulp(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** Over-utilization slack variable
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** This is a simplified calculation - actual would consider
        ***REMOVED*** faculty-specific availability
        ...
        ***REMOVED*** Capacity = faculty who can work * average available blocks
        ...
        ***REMOVED*** Simplified: each faculty can cover 1 block per day on average
...
***REMOVED*** =============================================================================
***REMOVED*** TIER 2: STRATEGIC RESILIENCE CONSTRAINTS
***REMOVED*** =============================================================================
...
class ZoneBoundaryConstraint(SoftConstraint):
    ...
    ***REMOVED*** Zone type priority multipliers
    ZONE_PRIORITY = {
        "inpatient": 2.0,      ***REMOVED*** Critical - highest isolation
        "outpatient": 1.5,     ***REMOVED*** Important
        "on_call": 1.5,        ***REMOVED*** Important
        "education": 1.0,      ***REMOVED*** Standard
        "research": 0.8,       ***REMOVED*** Flexible
        "admin": 0.5,          ***REMOVED*** Most flexible
    }
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        if not x or not context.zone_assignments or not context.block_zones:
            return  ***REMOVED*** No zone data available
        ...
        for faculty in context.faculty:
            ...
            if not faculty_zone:
                continue  ***REMOVED*** Faculty not assigned to a zone
            ...
            for block in context.blocks:
                ...
                if not block_zone:
                    continue  ***REMOVED*** Block not in a zone

                ***REMOVED*** Penalty if zones don't match
                if faculty_zone != block_zone:
                    penalty_factor = 10  ***REMOVED*** Base penalty for cross-zone
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        if cross_zone_count > 0:
            ***REMOVED*** Determine severity based on percentage
...
class PreferenceTrailConstraint(SoftConstraint):
    ...
    ***REMOVED*** Trail strength thresholds
    STRONG_TRAIL_THRESHOLD = 0.6   ***REMOVED*** Above this = strong signal
    WEAK_TRAIL_THRESHOLD = 0.3     ***REMOVED*** Below this = ignore
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        if not x or not context.preference_trails:
            return  ***REMOVED*** No preference data available
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        for assignment in assignments:
            ...
            ***REMOVED*** Get block for slot type
            block = None
            for b in context.blocks:
        ...
        if total_checked > 0:
            ...
            ***REMOVED*** Report if significant misalignment
...
class N1VulnerabilityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables: dict, context: SchedulingContext):
        ...
        ***REMOVED*** For each block, count how many faculty could cover it
        ***REMOVED*** If only 1 faculty assigned and no alternatives, that's N-1 vulnerable
        total_n1_penalty = 0

        for block in context.blocks:
            b_i = context.block_idx[block.id]

            ***REMOVED*** Count available faculty for this block
            available_for_block = []
            for faculty in context.faculty:
                f_i = context.resident_idx.get(faculty.id)
                if f_i is None:
            ...
            ***REMOVED*** If only 1-2 faculty available, add penalty for assignments
            if len(available_for_block) <= 2:
                for f_i, _ in available_for_block:
                    ***REMOVED*** Penalty scaled by scarcity: 1 available = high, 2 = medium
                    scarcity_factor = 3 - len(available_for_block)  ***REMOVED*** 2 or 1
    ...
    def validate(self, assignments: list, context: SchedulingContext) -> ConstraintResult:
        ...
        ***REMOVED*** Analyze each block for redundancy
        ...
        ***REMOVED*** Also check faculty in n1_vulnerable_faculty set
        for faculty_id in context.n1_vulnerable_faculty:
            if faculty_id in sole_providers:
                ***REMOVED*** Already counted in sole_providers
                continue
            ***REMOVED*** Check how many assignments this faculty has
            faculty_assignments = [a for a in assignments if a.person_id == faculty_id]
            if faculty_assignments:
                ***REMOVED*** Add extra penalty for known N-1 vulnerable faculty
                total_penalty += len(faculty_assignments) * self.weight * 0.5

        ***REMOVED*** Report violations
        ...
        ***REMOVED*** Report sole providers
        for faculty_id, sole_count in sole_providers.items():
            ...
            if sole_count >= 3:  ***REMOVED*** Report faculty who are sole provider for 3+ blocks
                violations.append(ConstraintViolation(
                    constraint_name=self.name,
                    constraint_type=self.constraint_type,
...
class ConstraintManager:
    ...
    def enable(self, name: str) -> "ConstraintManager":
        ...
        Example:
            >>> ***REMOVED*** Enable resilience constraints when data is available
            >>> if context.has_resilience_data():
    ...
    def create_default(cls) -> "ConstraintManager":
        ...
        ***REMOVED*** Tier 1: Resilience-aware soft constraints (disabled by default for backward compatibility)
        manager.add(HubProtectionConstraint(weight=15.0))
        manager.add(UtilizationBufferConstraint(weight=20.0))
        ***REMOVED*** Tier 2: Strategic resilience constraints (disabled by default)
        ...
        ***REMOVED*** Disabled by default - enabled when resilience data is provided
    ...
    def create_resilience_aware(
        ...
        ***REMOVED*** Tier 1: Core resilience constraints (ENABLED)
        manager.add(HubProtectionConstraint(weight=15.0))
        ...
        ***REMOVED*** Only enable Tier 2 if requested
        if tier < 2:
            manager.disable("ZoneBoundary")
            manager.disable("PreferenceTrail")
            manager.disable("N1Vulnerability")
    ...
    def create_strict(cls) -> "ConstraintManager":
        ...
        ***REMOVED*** Increase soft constraint weights
