...
***REMOVED*** Optional visualization dependencies
try:
...
def plot_energy_landscape(
    ...
    ***REMOVED*** Extract energies
    indices = [r.replica_index for r in replicas]
    energies = [r.energy for r in replicas]

    ***REMOVED*** Color by basin (if basin info available)
    if landscape.basin_sizes:
        ***REMOVED*** Assign colors to basins
...
def plot_parisi_overlap_matrix(
    ...
    ***REMOVED*** Use seaborn if available for nicer heatmap
...
def plot_overlap_distribution(
    ...
    ***REMOVED*** Extract bin centers and counts
...
def plot_frustration_network(
    ...
    ***REMOVED*** Extract constraint names from clusters
    all_constraints = set()
    edges = []
    ...
    for cluster in frustration_clusters:
        ...
        ***REMOVED*** Create edges between conflicting constraints
        for i, c1 in enumerate(constraints):
            for c2 in constraints[i + 1 :]:
                edges.append((c1, c2, cluster.frustration_index))

    ***REMOVED*** Simple circular layout
    ...
    ***REMOVED*** Draw edges (frustration bonds)
    for c1, c2, frustration in edges:
        x1, y1 = positions[c1]
        x2, y2 = positions[c2]

        ***REMOVED*** Line width proportional to frustration
...
def plot_solution_basins(
    ...
    ***REMOVED*** Assign colors by basin
    basin_assignments = _assign_replicas_to_basins(replicas, landscape)
...
def _assign_replicas_to_basins(
    ...
    for basin_id, replica_indices in landscape.basin_sizes.items():
        ***REMOVED*** Note: basin_sizes stores (basin_id -> count), not (basin_id -> indices)
        ***REMOVED*** For proper implementation, would need to store basin membership in LandscapeAnalysis
        ***REMOVED*** For now, use simple heuristic based on energy proximity
        pass

    ***REMOVED*** Fallback: assign all to basin 0
    for i in range(len(replicas)):
