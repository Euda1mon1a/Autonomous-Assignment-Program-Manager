...
class CrossoverMethod(Enum):
    ...
    TWO_POINT = "two_point"
    ROW_BASED = "row_based"  ***REMOVED*** Swap entire resident schedules
    BLOCK_BASED = "block_based"  ***REMOVED*** Swap entire block assignments


class MutationMethod(Enum):
    """Mutation methods for exploration."""

    SWAP = "swap"  ***REMOVED*** Swap two assignments
    INSERT = "insert"  ***REMOVED*** Insert new assignment
    DELETE = "delete"  ***REMOVED*** Remove assignment
    FLIP = "flip"  ***REMOVED*** Change template for assignment
    SCRAMBLE = "scramble"  ***REMOVED*** Shuffle segment
...
class SelectionOperator:
    ...
    def _tournament_select(
        ...
        for _ in range(n_parents):
            ***REMOVED*** Select random tournament participants
            tournament = random.sample(
                population, min(self.tournament_size, len(population))
            )
            ***REMOVED*** Winner is the one with best fitness
            winner = max(
                tournament,
    ...
    def _rank_select(
        ...
        ***REMOVED*** Assign ranks (1 = best)
        n = len(sorted_pop)
        probabilities = []
        for rank in range(1, n + 1):
            ***REMOVED*** Linear ranking with pressure
            prob = (
...
class CrossoverOperator:
    ...
    def crossover(
        ...
        if random.random() > self.crossover_rate:
            ***REMOVED*** No crossover, return copies
    ...
    def _row_based_crossover(
        ...
        ***REMOVED*** Randomly select rows to swap
    ...
    def _block_based_crossover(
        ...
        ***REMOVED*** Randomly select columns to swap
...
class MutationOperator:
    ...
    def _apply_mutation(
        ...
        if self.method == MutationMethod.FLIP:
            ***REMOVED*** Change to different template or unassign
            current = chromosome.genes[row, col]
            if current == 0:
                chromosome.genes[row, col] = random.randint(1, n_templates)
            else:
                ***REMOVED*** 20% chance to unassign, 80% to change template
            ...
            ***REMOVED*** Swap with another position in same row
            ...
            ***REMOVED*** Insert random assignment
            chromosome.genes[row, col] = random.randint(1, n_templates)
    ...
    def adapt_rate(
        ...
        ***REMOVED*** Calculate improvement rate over last 10 generations
        ...
        ***REMOVED*** Adapt rate
        if self._stagnation_counter > 10:
            ***REMOVED*** Strong stagnation: increase rate significantly
            ...
            ***REMOVED*** Good progress: decrease rate to exploit
            self.current_rate = max(self.min_rate, self.current_rate * 0.95)
        elif improvement_rate < 0.2:
            ***REMOVED*** Poor progress: increase rate to explore
            self.current_rate = min(self.max_rate, self.current_rate * 1.1)

        ***REMOVED*** Consider diversity
        if diversity < 0.1:
            ***REMOVED*** Very low diversity: increase rate
            self.current_rate = min(self.max_rate, self.current_rate * 1.2)
        elif diversity > 0.5:
            ***REMOVED*** High diversity: slight decrease
...
class GeneticAlgorithmSolver(BioInspiredSolver):
    ...
    def _evolve(self, context: SchedulingContext) -> Individual:
        ...
        ***REMOVED*** Evolution loop
        for generation in range(self.max_generations):
            ...
            ***REMOVED*** Evolve one generation
            improved = self._evolve_generation(context, generation)

            ***REMOVED*** Adaptive mutation
    ...
    def _evolve_generation(
        ...
        ***REMOVED*** Create new population
        new_population = [ind.copy() for ind in elite]

        ***REMOVED*** Fill rest of population with offspring
        ...
        ***REMOVED*** Apply niching if enabled
    ...
    def _apply_niching(self, population: list[Individual]) -> list[Individual]:
        ...
        for ind in population:
            ...
            ***REMOVED*** Count neighbors within niche radius
            niche_count = 0
            ...
            ***REMOVED*** Share fitness with niche members
            if niche_count > 0:
                share_factor = 1.0 / (niche_count + 1)
                ***REMOVED*** Apply to weighted sum (stored separately for display)
                ind.rank = int(niche_count)  ***REMOVED*** Abuse rank field for niche count
