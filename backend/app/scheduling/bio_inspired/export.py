...
.. code-block:: python
    ...
    ***REMOVED*** Export to dictionary
    data = exporter.export(
        pareto_front,
        objectives=["coverage", "fairness", "acgme_compliance"],
        include_chromosomes=False,  ***REMOVED*** Omit large chromosome data
    )

    ***REMOVED*** Export to file
...
.. code-block:: python
    ...
    data = exporter.export(
        ...
        include_full_pareto=True,  ***REMOVED*** Include detailed Pareto data
    )
...
.. code-block:: python
    ...
    data = exporter.export_complete(
        ...
        pareto_front=pareto,
        population_snapshots=snapshots,  ***REMOVED*** For animation
...
class ParetoExporter:
    ...
    def export(
        ...
        for ind in pareto_front:
            ...
            ***REMOVED*** Optionally include chromosome
        ...
        ***REMOVED*** Sort by first objective for easier visualization
        solutions.sort(key=lambda s: s.get("objectives", {}).get(objectives[0], 0))

        ***REMOVED*** Compute front statistics
    ...
    def _compute_pareto_stats(
        ...
        ***REMOVED*** Hypervolume (2D approximation)
        if len(objectives) >= 2:
...
class EvolutionExporter:
    ...
    def export(
        ...
        ***REMOVED*** Pareto front evolution (summarized)
        pareto_data = None
        if pareto_history:
            pareto_data = [
                {
            ...
            if include_full_pareto and len(pareto_history) > 0:
                ***REMOVED*** Include final Pareto front details
    ...
    def _compute_summary(
        ...
        ***REMOVED*** Find convergence point (90% of final improvement)
        convergence_gen = self._find_convergence_generation(best_fitness_series)

        ***REMOVED*** Compute improvement rate
    ...
    def _find_first_best(self, fitness_series: list[float]) -> int:
        ...
        for i, f in enumerate(fitness_series):
            if f >= best * 0.999:  ***REMOVED*** Within 0.1% of best
                return i
...
class HolographicExporter:
    ...
    def export_complete(
        ...
        ***REMOVED*** Build holographic-specific data
    ...
    def _build_holographic_data(
        ...
        ***REMOVED*** 3D node positions (coverage, fairness, ACGME as axes)
        nodes = []
        for ind in pareto_front:
            if ind.fitness:
                nodes.append(
                    {
                        ...
                        "size": ind.fitness.weighted_sum() * 10,  ***REMOVED*** Scale for visibility
                        "color": self._fitness_to_color(ind.fitness.weighted_sum()),
                        "label": f"Solution {ind.id}",
                    }
                )

        ***REMOVED*** Animation frames from population snapshots
        frames = []
        for gen, pop in enumerate(population_snapshots[:50]):  ***REMOVED*** Limit frames
            frame_nodes = []
            for ind in pop[:50]:  ***REMOVED*** Limit nodes per frame
                if ind.fitness:
                    frame_nodes.append(
                        {
                            "id": ind.id,
        ...
        ***REMOVED*** Pareto surface (3D mesh)
        surface = self._build_pareto_surface(pareto_front)

        return {
            "nodes": nodes,
            "edges": [],  ***REMOVED*** Could add parent-child relationships
            "frames": frames,
            "surface": surface,
            "camera": {
                "initial_position": {"x": 1.5, "y": 1.5, "z": 1.5},
    ...
    def _fitness_to_color(self, fitness: float) -> str:
        ...
        ***REMOVED*** Normalize fitness to 0-1 range
        normalized = min(1.0, max(0.0, fitness))

        ***REMOVED*** Green to red gradient
        r = int(255 * (1 - normalized))
        g = int(255 * normalized)
        b = 50

        return f"***REMOVED***{r:02x}{g:02x}{b:02x}"
    ...
    def _build_pareto_surface(
        ...
        ***REMOVED*** Extract 3D coordinates
        vertices = []
        for ind in pareto_front:
            if ind.fitness:
                vertices.append(
        ...
        ***REMOVED*** Simple triangulation (would use scipy.spatial.Delaunay for real impl)
        faces = []
