...
**Chromosome**
    ...
    .. code-block:: python
        ...
        ***REMOVED*** Convert to assignment list
        assignments = chrom.to_assignment_list(context)  ***REMOVED*** [(person_id, block_id, template_id), ...]
...
**FitnessVector**
    ...
    .. code-block:: python
        ...
        ***REMOVED*** Check Pareto dominance
        if fitness.dominates(other_fitness):
            print("This solution is strictly better")

        ***REMOVED*** Compute weighted sum for single-objective optimization
        score = fitness.weighted_sum()  ***REMOVED*** Uses default weights
        score = fitness.weighted_sum({"coverage": 0.4, "fairness": 0.6})  ***REMOVED*** Custom
...
.. code-block:: python
    ...
    class MyCustomSolver(BioInspiredSolver):
        ...
        def _evolve(self, context: SchedulingContext) -> Individual:
            ...
            ***REMOVED*** Your custom evolution logic
...
.. code-block:: python
    ...
    ***REMOVED*** Fairness-focused weights
    weights = {
        "coverage": 0.20,
        "fairness": 0.35,
...
class FitnessVector:
    ...
    ***REMOVED*** Objective directions (all maximized after normalization)
    _objectives: dict = field(
        default_factory=lambda: {
            "coverage": ObjectiveType.MAXIMIZE,
            "fairness": ObjectiveType.MAXIMIZE,  ***REMOVED*** Higher = more fair
...
class Chromosome:
    ...
    genes: np.ndarray  ***REMOVED*** Shape: (n_residents, n_blocks)
    ...
    def create_random(
        ...
        ***REMOVED*** Generate random assignments
        genes = np.zeros((n_residents, n_blocks), dtype=np.int32)

        for r in range(n_residents):
            for b in range(n_blocks):
                if np.random.random() < density:
                    ***REMOVED*** Assign a random template (1-indexed, 0 = unassigned)
    ...
    def to_assignment_list(
        ...
        for r_idx in range(n_residents):
            for b_idx in range(n_blocks):
                ...
                if t_idx > 0:  ***REMOVED*** 0 = unassigned
                    person_id = idx_to_resident.get(r_idx)
                    block_id = idx_to_block.get(b_idx)
                    ***REMOVED*** Template index is 1-based in chromosome
                    template_id = (
...
class Individual:
    ...
    rank: int = 0  ***REMOVED*** Pareto rank (0 = non-dominated)
...
class PopulationStats:
    ...
    mean_fitness: float
    std_fitness: float
    diversity: float  ***REMOVED*** Average pairwise distance
    pareto_front_size: int
    hypervolume: float  ***REMOVED*** Volume of dominated objective space
    convergence: float  ***REMOVED*** Distance to reference Pareto front

    ***REMOVED*** Objective-specific statistics
    best_coverage: float = 0.0
    best_fairness: float = 0.0
    best_acgme: float = 0.0

    ***REMOVED*** Evolution dynamics
    selection_pressure: float = 0.0  ***REMOVED*** Ratio of best to mean fitness
...
class BioInspiredSolver(BaseSolver, ABC):
    ...
    def evaluate_fitness(
        ...
        ***REMOVED*** Coverage: Proportion of possible assignments made
        ...
        ***REMOVED*** Fairness: 1 - normalized variance in resident workload
        ...
        if len(resident_counts) > 1 and np.mean(resident_counts) > 0:
            ...
            fairness = 1.0 / (1.0 + cv)  ***REMOVED*** Higher = more fair
        else:
            fairness = 1.0

        ***REMOVED*** ACGME Compliance: Check 80-hour rule approximation
        ***REMOVED*** Simplified: penalize if any resident exceeds threshold
        acgme_violations = 0

        for r in range(n_residents):
            assignments = chromosome.get_resident_assignments(r)
            ***REMOVED*** Check weekly chunks
        ...
        ***REMOVED*** Continuity: Reward consecutive assignments to same template
        continuity_score = 0
        total_transitions = 0
        ...
        ***REMOVED*** Preferences: Placeholder (would need preference data)
        preferences = 0.5  ***REMOVED*** Neutral

        ***REMOVED*** Learning Goals: Placeholder (would need curriculum data)
        learning_goals = 0.5  ***REMOVED*** Neutral
    ...
    def initialize_population(
        ...
        for i in range(self.population_size):
            ***REMOVED*** Vary density for diversity
            ind_density = density * (
    ...
    def update_pareto_front(self, population: list[Individual]) -> None:
        ...
        ***REMOVED*** Find non-dominated solutions
        non_dominated = []
        for ind in candidates:
            ...
            if not dominated:
                ***REMOVED*** Check if already in non_dominated (by fitness)
                is_duplicate = any(
    ...
    def compute_population_stats(
        ...
        ***REMOVED*** Diversity: average pairwise similarity
        if len(population) > 1:
        ...
        ***REMOVED*** Hypervolume (simplified 2D projection)
        ***REMOVED*** Use coverage and fairness for visualization
        if self.pareto_front:
            ***REMOVED*** Reference point: worst possible
        ...
        return PopulationStats(
            ...
            convergence=1.0 - diversity,  ***REMOVED*** Inverse of diversity
