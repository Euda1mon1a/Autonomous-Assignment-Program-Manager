...
class AntPath:
    ...
    resident_idx: int
    path: list[int]  ***REMOVED*** Sequence of template indices for each block
    fitness: float = 0.0
...
class PheromoneMatrix:
    ...
    def __init__(
        ...
        self.n_templates = n_templates + 1  ***REMOVED*** Include 0 for unassigned
        self.evaporation_rate = evaporation_rate
        self.min_pheromone = min_pheromone
        self.max_pheromone = max_pheromone

        ***REMOVED*** Assignment pheromone: [resident, block, template]
        self.assignment = np.full(
            (n_residents, n_blocks, self.n_templates), initial_value, dtype=np.float64
        )

        ***REMOVED*** Transition pheromone: [from_template, to_template]
    ...
    def get_assignment_probability(
        ...
        ***REMOVED*** Combine pheromone and heuristic
...
class ACOConfig:
    ...
    colony_size: int = ACO_DEFAULT_COLONY_SIZE  ***REMOVED*** Number of ants
    max_iterations: int = ACO_DEFAULT_MAX_ITERATIONS

    ***REMOVED*** ACO parameters
    alpha: float = ACO_DEFAULT_ALPHA  ***REMOVED*** Pheromone importance
    beta: float = ACO_DEFAULT_BETA  ***REMOVED*** Heuristic importance
    evaporation_rate: float = ACO_DEFAULT_EVAPORATION_RATE
...
class AntColonySolver(BioInspiredSolver):
    ...
    def _evolve(self, context: SchedulingContext) -> Individual:
        ...
        ***REMOVED*** Initialize pheromone matrix
        ...
        ***REMOVED*** Build heuristic matrix
        self._build_heuristic(context, n_residents, n_blocks)

        ***REMOVED*** Initialize best solution
        ...
        ***REMOVED*** Main ACO loop
        for iteration in range(self.config.max_iterations):
            ...
            ***REMOVED*** Construct solutions with all ants
            ant_solutions = []
            ...
            ***REMOVED*** Apply local search to best solutions
            ...
            ***REMOVED*** Evaporate pheromone
            self.pheromone.evaporate()

            ***REMOVED*** Deposit pheromone
            self._deposit_pheromone(ant_solutions)

            ***REMOVED*** Elite ant deposition
        ...
        ***REMOVED*** Create best individual
    ...
    def _build_heuristic(
        ...
        ***REMOVED*** Penalize unavailable slots
        for resident in context.residents:
            ...
            for b_idx in unavail:
                if 0 <= b_idx < n_blocks:
                    ***REMOVED*** Set all templates to low probability
                    self.heuristic_matrix[r_idx, b_idx, :] = ACO_UNAVAILABLE_SLOT_HEURISTIC

        ***REMOVED*** Boost unassigned to prevent over-scheduling
    ...
    def _construct_solution(
        ...
        for r_idx in range(n_residents):
            prev_template = 0  ***REMOVED*** Start unassigned

            for b_idx in range(n_blocks):
                ***REMOVED*** Get probability distribution
    ...
    def _local_search(
        ...
        for _ in range(self.config.local_search_iterations):
            ***REMOVED*** Random swap within a resident's schedule
    ...
    def _deposit_pheromone(
        ...
        ***REMOVED*** Deposit for top solutions
        for rank, (chromosome, fitness, weighted) in enumerate(
            ...
            ***REMOVED*** Pheromone amount based on fitness and rank
    ...
    def get_pheromone_analysis(self) -> dict:
        ...
        ***REMOVED*** Find hotspots (high pheromone)
        ...
        ***REMOVED*** Find preferred transitions
        top_transitions = []
