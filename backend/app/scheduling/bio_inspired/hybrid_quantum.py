...
.. code-block:: python
    ...
    config = HybridConfig(
        ...
        ***REMOVED*** GA parameters
        crossover_rate=0.85,
        mutation_rate=0.12,
        elite_size=4,

        ***REMOVED*** QUBO parameters
        qubo_num_reads=75,
        qubo_num_sweeps=750,
        qubo_timeout=8.0,
...
.. code-block:: python
    ...
    config = HybridConfig(
        ...
        n_islands=1,  ***REMOVED*** Single island, one strategy
    )
...
.. code-block:: python
    ...
    ***REMOVED*** Island performance comparison
...
.. code-block:: python
    ...
    ***REMOVED*** Group residents by PGY level
    def create_pgy_decomposition(context):
        ...
        for i, resident in enumerate(context.residents):
            pgy_level = resident.pgy_level  ***REMOVED*** 1, 2, or 3
            r_idx = context.resident_idx[resident.id]
            partition[r_idx, :] = pgy_level - 1  ***REMOVED*** 0, 1, or 2
...
class DecompositionStrategy(Enum):
    ...
    BY_RESIDENT = "by_resident"  ***REMOVED*** Each resident as separate QUBO
    BY_BLOCK_WEEK = "by_block_week"  ***REMOVED*** Weekly blocks as QUBO
    BY_TEMPLATE = "by_template"  ***REMOVED*** Templates as separate problems
    ADAPTIVE = "adaptive"  ***REMOVED*** Evolved decomposition boundaries
...
class ProblemDecomposition:
    ...
    ***REMOVED*** Decomposition matrix: [n_residents, n_blocks] -> sub-problem index
    partition: np.ndarray
    n_subproblems: int
    strategy: DecompositionStrategy

    ***REMOVED*** Sub-problem solutions
...
class HybridConfig:
    ...
    ***REMOVED*** Decomposition
    ...
    n_subproblems: int = 10
    min_subproblem_size: int = 5  ***REMOVED*** Minimum variables per QUBO
    ...
    ***REMOVED*** QUBO parameters
    ...
    qubo_timeout: float = 5.0  ***REMOVED*** Per sub-problem

    ***REMOVED*** Migration (for island model)
...
class HybridGAQUBOSolver(BioInspiredSolver):
    ...
    def __init__(
        ...
        ***REMOVED*** Islands for parallel populations
    ...
    def _evolve(self, context: SchedulingContext) -> Individual:
        ...
        ***REMOVED*** Evolution loop
        for generation in range(self.config.max_generations):
            ...
            ***REMOVED*** Update best individual
            self._update_best()
    ...
    def _initialize_islands(
        ...
        strategies = [
            ...
            DecompositionStrategy.ADAPTIVE,  ***REMOVED*** Duplicate adaptive for diversity
        ]
    ...
    def _evolve_island(
        ...
        ***REMOVED*** Create offspring
        while len(new_island) < len(island):
            ***REMOVED*** Select parents
            parents = self.selection.select(island, 2)

            ***REMOVED*** Crossover decomposition partitions
            ***REMOVED*** Treat partition as chromosome for crossover
            ...
            ***REMOVED*** Crossover on partition genes
            ...
            ***REMOVED*** Create decompositions and solve
        ...
        ***REMOVED*** Replace island population
        island.clear()
    ...
    def _solve_decomposition(
        ...
        for subproblem_idx in range(decomp.n_subproblems):
            ...
            if n_vars < self.config.min_subproblem_size:
                ***REMOVED*** Too small for QUBO, use random assignment
            ...
            ***REMOVED*** Solve sub-problem with QUBO
            try:
    ...
    def _solve_subproblem_qubo(
        ...
        """
        ***REMOVED*** Build mini-QUBO for this sub-problem
        ***REMOVED*** Map positions to flat indices
        ...
        ***REMOVED*** Create QUBO matrix for sub-problem
        ***REMOVED*** Each position can be assigned one of n_templates
        total_vars = n_vars * self._n_templates
        Q = {}

        ***REMOVED*** Coverage objective (negative = want x=1)
        for i in range(total_vars):
            Q[(i, i)] = -1.0

        ***REMOVED*** One template per position constraint
        for pos_idx in range(n_vars):
            ...
            for t1 in range(self._n_templates):
                for t2 in range(t1 + 1, self._n_templates):
                    i1 = start + t1
                    i2 = start + t2
                    Q[(i1, i2)] = 100.0  ***REMOVED*** Penalty for multiple templates
        ...
        for pos_idx, (r_idx, b_idx) in enumerate(positions):
            ...
            for t_idx in range(self._n_templates):
                if sample.get(start + t_idx, 0) == 1:
                    solution[r_idx, b_idx] = t_idx + 1  ***REMOVED*** 1-indexed
    ...
    def _simulated_annealing(
        ...
        ***REMOVED*** Simple SA implementation
    ...
    def _chromosome_to_decomposition(
        ...
        ***REMOVED*** Use template values modulo n_subproblems as partition
    ...
    def _mutate_partition(
        ...
        ***REMOVED*** Random reassignment for mutated positions
    ...
    def _migrate_between_islands(self) -> None:
        ...
        ***REMOVED*** Ring topology: island i sends to island (i+1) % n
        migrants = []
        ...
        ***REMOVED*** Receive migrants from previous island
        for i, island in enumerate(self.islands):
            from_island = (i - 1) % len(self.islands)
            ***REMOVED*** Replace worst individuals with migrants
            island.sort(
                key=lambda ind: ind.fitness.weighted_sum() if ind.fitness else 0
            )
    ...
    def _track_generation(self, generation: int) -> None:
        ...
        stats = PopulationStats(
            ...
            diversity=0.0,  ***REMOVED*** Would need chromosome comparison
            pareto_front_size=1,
