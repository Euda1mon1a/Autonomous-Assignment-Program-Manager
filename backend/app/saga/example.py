...
***REMOVED*** Example: Scheduling Assignment Transaction
***REMOVED*** ===========================================
***REMOVED*** This saga handles creating a schedule assignment with multiple steps:
***REMOVED*** 1. Validate ACGME compliance
***REMOVED*** 2. Reserve the time slot
***REMOVED*** 3. Create assignment record
***REMOVED*** 4. Send notifications
***REMOVED***
***REMOVED*** If any step fails, previous steps are compensated (rolled back).
...
async def validate_acgme_compliance(data: dict[str, Any]) -> dict[str, Any]:
    ...
    ***REMOVED*** Simulate validation logic
    ***REMOVED*** In real implementation, this would check:
    ***REMOVED*** - 80-hour rule
    ***REMOVED*** - 1-in-7 rule
    ***REMOVED*** - Supervision ratios
    await asyncio.sleep(0.1)  ***REMOVED*** Simulate DB query
...
async def reserve_time_slot(data: dict[str, Any]) -> dict[str, Any]:
    ...
    ***REMOVED*** Simulate reservation logic
...
async def compensate_time_slot_reservation(data: dict[str, Any]) -> None:
    ...
    ***REMOVED*** Simulate releasing reservation
...
async def send_notifications(data: dict[str, Any]) -> dict[str, Any]:
    ...
    ***REMOVED*** Simulate sending emails/push notifications
    await asyncio.sleep(0.1)
...
async def compensate_notifications(data: dict[str, Any]) -> None:
    ...
    ***REMOVED*** Simulate sending cancellation emails
    await asyncio.sleep(0.05)


***REMOVED*** Define the saga
ASSIGNMENT_CREATION_SAGA = SagaDefinition(
    ...
    steps=[
        ...
        SagaStepDefinition(
            ...
            timeout_seconds=15,
            retry_attempts=2,
            idempotent=False,  ***REMOVED*** Not idempotent - creates new record
        ),
        SagaStepDefinition(
    ...
    timeout_seconds=300,  ***REMOVED*** 5 minutes total
)


***REMOVED*** Example: Parallel Service Updates
***REMOVED*** ==================================
***REMOVED*** This saga demonstrates parallel execution of independent steps.
...
PARALLEL_UPDATE_SAGA = SagaDefinition(
    ...
    version="1.0",
    steps=[
        ***REMOVED*** These three steps execute in parallel
        SagaStepDefinition(
        ...
        ***REMOVED*** This step executes after parallel group completes
        SagaStepDefinition(
            name="finalize_updates",
            action=send_notifications,  ***REMOVED*** Reuse from above
            compensation=compensate_notifications,
            timeout_seconds=30,
        ),
    ],
...
async def create_assignment_with_saga(
    ...
    ***REMOVED*** Register saga definition
    orchestrator.register_saga(ASSIGNMENT_CREATION_SAGA)

    ***REMOVED*** Execute saga
...
async def recover_interrupted_sagas(db: Session) -> None:
    ...
    ***REMOVED*** Register all saga definitions that might need recovery
    orchestrator.register_saga(ASSIGNMENT_CREATION_SAGA)
    orchestrator.register_saga(PARALLEL_UPDATE_SAGA)

    ***REMOVED*** Recover pending sagas
