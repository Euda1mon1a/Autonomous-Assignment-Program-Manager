...
class SagaStatus(str, Enum):
    ...
    PENDING = "pending"  ***REMOVED*** Saga created but not started
    RUNNING = "running"  ***REMOVED*** Saga currently executing
    COMPENSATING = "compensating"  ***REMOVED*** Rolling back completed steps
    COMPLETED = "completed"  ***REMOVED*** All steps completed successfully
    FAILED = "failed"  ***REMOVED*** Saga failed and compensations completed
    TIMEOUT = "timeout"  ***REMOVED*** Saga timed out
    CANCELLED = "cancelled"  ***REMOVED*** Saga manually cancelled


class StepStatus(str, Enum):
    """Status of a saga step execution."""
    PENDING = "pending"  ***REMOVED*** Step not yet started
    RUNNING = "running"  ***REMOVED*** Step currently executing
    COMPLETED = "completed"  ***REMOVED*** Step completed successfully
    FAILED = "failed"  ***REMOVED*** Step failed
    COMPENSATING = "compensating"  ***REMOVED*** Compensation in progress
    COMPENSATED = "compensated"  ***REMOVED*** Compensation completed


class StepType(str, Enum):
    """Type of saga step execution."""
    SEQUENTIAL = "sequential"  ***REMOVED*** Execute one at a time
    PARALLEL = "parallel"  ***REMOVED*** Execute concurrently


***REMOVED*** Type aliases for step functions
...
class SagaStepDefinition:
    ...
    timeout_seconds: int = 300  ***REMOVED*** 5 minutes default
...
class SagaDefinition:
    ...
    timeout_seconds: int = 3600  ***REMOVED*** 1 hour default
    description: str = ""
    version: str = "1.0"
    ...
    def __post_init__(self):
        ...
        ***REMOVED*** Check for duplicate step names
        step_names = [step.name for step in self.steps]
        if len(step_names) != len(set(step_names)):
            raise ValueError("Step names must be unique")

        ***REMOVED*** Validate parallel groups
        ...
        ***REMOVED*** All steps in a parallel group should be consecutive
