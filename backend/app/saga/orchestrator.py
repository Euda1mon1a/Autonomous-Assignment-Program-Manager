...
class SagaOrchestrator:
    ...
    async def execute_saga(
        ...
        ***REMOVED*** Create or recover saga execution
        if saga_id:
        ...
        try:
            ...
            ***REMOVED*** Trigger compensation for completed steps
    ...
    def _group_steps_for_execution(
        ...
        for step in steps:
            if step.parallel_group:
                if step.parallel_group != current_parallel_group:
                    ***REMOVED*** Start new parallel group
                    if current_group:
                        groups.append(current_group)
                    current_group = [step]
                    current_parallel_group = step.parallel_group
                else:
                    ***REMOVED*** Continue current parallel group
                    current_group.append(step)
            else:
                ***REMOVED*** Sequential step
                if current_group:
                    groups.append(current_group)
                groups.append([step])
                current_group = []
        ...
        ***REMOVED*** Add final group if exists
        if current_group:
            groups.append(current_group)
    ...
    async def _execute_step(
        ...
        """
        ***REMOVED*** Get or create step execution record
        ...
        ***REMOVED*** Prepare input data (merge context)
        step_input = {
            **context.input_data,
        ...
        while attempt < max_attempts:
            try:
                ...
                ***REMOVED*** Merge output into context
                ...
                if attempt < max_attempts - 1 and step_def.idempotent:
                    ***REMOVED*** Retry for idempotent steps
                    attempt += 1
                    await asyncio.sleep(step_def.retry_delay_seconds)
                else:
                    ***REMOVED*** Final failure
    ...
    async def _execute_parallel_steps(
        ...
        ***REMOVED*** Execute all steps concurrently
        tasks = [self._execute_step(step, context, saga_exec) for step in steps]

        results = await asyncio.gather(*tasks, return_exceptions=True)

        ***REMOVED*** Convert exceptions to failed results
    ...
    async def _compensate_saga(
        ...
        ***REMOVED*** Get completed steps in reverse order
        completed_steps = [
    ...
    async def _get_or_create_step_execution(
        ...
        """
        ***REMOVED*** Check if step already exists (for recovery)
        existing = next(
        ...
        ***REMOVED*** Create new step execution
    ...
    async def recover_pending_sagas(self) -> list[UUID]:
        ...
        Call this method on service startup:
            ...
            orchestrator.register_saga(MY_SAGA_DEF)  ***REMOVED*** Must register first
        ...
        for saga_exec in pending_sagas:
            try:
                ...
                ***REMOVED*** Mark as failed for manual review
                ***REMOVED*** In production, you might want to attempt recovery instead
                saga_exec.status = SagaStatus.FAILED.value
                saga_exec.completed_at = datetime.utcnow()
