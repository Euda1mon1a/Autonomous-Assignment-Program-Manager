...
***REMOVED*** Try to import NetworkX for graph analysis
try:
...
class KeystoneRiskLevel(str, Enum):
    ...
    LOW = "low"  ***REMOVED*** Minimal impact, easy replacement
    MODERATE = "moderate"  ***REMOVED*** Noticeable impact, replacement available
    HIGH = "high"  ***REMOVED*** Significant disruption, limited replacements
    CRITICAL = "critical"  ***REMOVED*** Major cascade, difficult to replace
    CATASTROPHIC = "catastrophic"  ***REMOVED*** System collapse likely


class SuccessionStatus(str, Enum):
    """Status of succession planning for a keystone."""

    NONE = "none"  ***REMOVED*** No succession plan
    PLANNED = "planned"  ***REMOVED*** Plan exists but not started
    IN_PROGRESS = "in_progress"  ***REMOVED*** Actively training backup
    COMPLETED = "completed"  ***REMOVED*** Backup fully trained
    VERIFIED = "verified"  ***REMOVED*** Backup proven capable
...
class KeystoneResource:
    ...
    ***REMOVED*** Core metrics
    keystoneness_score: float  ***REMOVED*** 0.0-1.0, high = disproportionate impact
    abundance: float  ***REMOVED*** 0.0-1.0, relative presence in system
    impact_if_removed: float  ***REMOVED*** 0.0-1.0, cascade damage potential
    replacement_difficulty: float  ***REMOVED*** 0.0-1.0, how hard to find substitute

    ***REMOVED*** Dependency analysis
    direct_dependents: int  ***REMOVED*** Entities that directly depend on this
    indirect_dependents: int  ***REMOVED*** Entities affected via cascade
    cascade_depth: int  ***REMOVED*** How many levels cascade propagates
    bottleneck_score: float  ***REMOVED*** How much of a bottleneck (0.0-1.0)

    ***REMOVED*** Functional redundancy
    functional_redundancy: float  ***REMOVED*** 0.0-1.0, high = many substitutes
...
class CascadeAnalysis:
    ...
    ***REMOVED*** Impact assessment
    affected_assignments: int = 0  ***REMOVED*** Assignments that cannot be filled
    coverage_loss: float = 0.0  ***REMOVED*** Percentage of coverage lost (0.0-1.0)
    recovery_time_days: int = 0  ***REMOVED*** Estimated days to recover
    is_catastrophic: bool = False  ***REMOVED*** Coverage loss > 50%

    ***REMOVED*** Cascade characteristics
    cascade_velocity: float = 0.0  ***REMOVED*** How quickly cascade spreads
    amplification_factor: float = 1.0  ***REMOVED*** Initial impact / final impact
...
class SuccessionPlan:
    ...
    ***REMOVED*** Backup candidates (ranked by suitability)
    ...
    ***REMOVED*** Implementation
    timeline: dict[str, date] = field(default_factory=dict)  ***REMOVED*** milestone -> date
...
class KeystoneAnalyzer:
    ...
    def build_dependency_graph(
        ...
        residents: list,
        assignments: list,
        services: dict[UUID, list[UUID]],  ***REMOVED*** service_id -> capable faculty/residents
        rotations: dict[UUID, dict],  ***REMOVED*** rotation_id -> {name, required_skills, etc}
    ) -> Optional["nx.DiGraph"]:
        """
        ...
        ***REMOVED*** Add dependency edges
        ***REMOVED*** Faculty/Residents -> Services they enable
        for service_id, capable_ids in services.items():
            for provider_id in capable_ids:
                G.add_edge(
                    ...
                    weight=1.0 / len(capable_ids),  ***REMOVED*** Weight by replaceability
                )

        ***REMOVED*** Services -> Rotations that require them
        for rot_id, rot_data in rotations.items():
        ...
        ***REMOVED*** Assignments create dependencies: Rotation -> Person (person is assigned)
    ...
    def compute_keystoneness_score(
        ...
        ***REMOVED*** Calculate impact metrics
        out_degree = graph.out_degree(entity_id)  ***REMOVED*** What depends on this
        in_degree = graph.in_degree(entity_id)  ***REMOVED*** What this depends on

        ***REMOVED*** Betweenness centrality: how often on critical paths
        try:
            betweenness = nx.betweenness_centrality(graph).get(entity_id, 0.0)
        except Exception:
            betweenness = 0.0

        ***REMOVED*** Abundance: normalized presence
        total_nodes = graph.number_of_nodes()
        total_edges = graph.number_of_edges()

        ***REMOVED*** Node abundance (how connected)
        node_abundance = (in_degree + out_degree) / (2 * max(total_nodes, 1))

        ***REMOVED*** Edge weight sum (how utilized)
        edge_weights = sum(
        ...
        ***REMOVED*** Combined abundance (normalized 0-1)
        abundance = (node_abundance + edge_abundance) / 2

        ***REMOVED*** Impact: combination of betweenness and out-degree
        impact = (betweenness + (out_degree / max(total_nodes, 1))) / 2

        ***REMOVED*** Keystoneness = impact / abundance (avoid division by zero)
        if abundance < 0.01:
            ***REMOVED*** Very low abundance - if has any impact, it's a keystone
    ...
    def compute_functional_redundancy(
        ...
        """
        ***REMOVED*** Extract UUID from entity_id (might be prefixed)
        try:
            if ":" in entity_id:
    ...
    def identify_keystone_resources(
        ...
        ***REMOVED*** Build dependency graph
        ...
        for entity in all_entities:
            ...
            ***REMOVED*** Skip if below threshold
            ...
            ***REMOVED*** Create keystone resource
        ...
        ***REMOVED*** Sort by keystoneness score
    ...
    def simulate_removal_cascade(
        ...
        ***REMOVED*** Level 1: Services lost
        ...
        for service_id, providers in services.items():
            if entity_id in providers:
                ...
                if len(remaining) == 0:
                    ***REMOVED*** Service completely lost
        ...
        ***REMOVED*** Level 2: Rotations affected
        affected_rotations = []
        for rot_id, rot_data in rotations.items():
            required_services = rot_data.get("required_services", [])
            ***REMOVED*** Check if any required service is lost
        ...
        ***REMOVED*** Level 3: Assignments lost
        ...
        for a in assignments:
            ...
            ***REMOVED*** Assignment affected if:
            ***REMOVED*** 1. Assigned to removed entity
            ***REMOVED*** 2. Rotation is affected
        ...
        ***REMOVED*** Estimate recovery time (heuristic)
        if cascade.is_catastrophic:
            cascade.recovery_time_days = 90  ***REMOVED*** Major restructure needed
        elif cascade.coverage_loss > 0.3:
            cascade.recovery_time_days = 30  ***REMOVED*** Significant adjustments
        elif cascade.coverage_loss > 0.1:
            cascade.recovery_time_days = 7  ***REMOVED*** Minor adjustments
        else:
            cascade.recovery_time_days = 1  ***REMOVED*** Quick fix

        ***REMOVED*** Amplification factor
        initial_impact = 1  ***REMOVED*** One entity removed
        final_impact = cascade.total_affected
        cascade.amplification_factor = final_impact / max(initial_impact, 1)

        ***REMOVED*** Cache simulation
    ...
    def recommend_succession_plan(
        ...
        """
        ***REMOVED*** Find what services this keystone provides
        keystone_services = [
            sid
        ...
        ***REMOVED*** Find potential backup candidates
        backup_candidates = []
        ...
        for entity in all_entities:
            ...
            ***REMOVED*** Don't suggest self as backup
            if entity_id == keystone.entity_id:
                continue

            ***REMOVED*** Calculate suitability score
            ***REMOVED*** How many of keystone's services can they already do?
            entity_services = [
                sid for sid, providers in services.items() if entity_id in providers
            ]
            ...
            ***REMOVED*** Bonus for already providing related services
            if overlap > 0:
                suitability += 0.2
        ...
        ***REMOVED*** Sort by suitability
        backup_candidates.sort(key=lambda x: -x[2])

        ***REMOVED*** Identify training needs (services keystone provides but candidate doesn't)
        training_needed = []
        ...
        ***REMOVED*** Estimate training hours (rough heuristic)
        estimated_hours = len(training_needed) * 40  ***REMOVED*** 40 hours per skill

        ***REMOVED*** Determine priority based on risk level
        priority_map = {
        ...
        ***REMOVED*** Create timeline
        ...
        if priority == "urgent":
            ...
            ***REMOVED*** More relaxed timeline for lower priority
        ...
        ***REMOVED*** Risk reduction estimate
        if backup_candidates:
        ...
        plan = SuccessionPlan(
            ...
            backup_candidates=backup_candidates[:5],  ***REMOVED*** Top 5
