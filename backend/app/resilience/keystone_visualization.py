...
class KeystoneVisualizer:
    ...
    def visualize_dependency_graph(
        ...
        ***REMOVED*** Create figure
        fig, ax = plt.subplots(figsize=(16, 12))

        ***REMOVED*** Build keystone ID set for coloring
        keystone_ids = {str(k.entity_id) for k in keystones}

        ***REMOVED*** Node colors based on type and keystone status
        node_colors = []
        node_sizes = []
        ...
        for node in graph.nodes():
            ...
            if node in keystone_ids:
                ***REMOVED*** Keystone - red
                node_colors.append("***REMOVED***FF4444")
                node_sizes.append(800)
            elif entity_type == "faculty":
                ***REMOVED*** Faculty - blue
                node_colors.append("***REMOVED***4444FF")
                node_sizes.append(500)
            elif entity_type == "resident":
                ***REMOVED*** Resident - green
                node_colors.append("***REMOVED***44FF44")
                node_sizes.append(500)
            elif entity_type == "service":
                ***REMOVED*** Service - orange
                node_colors.append("***REMOVED***FF8844")
                node_sizes.append(300)
            elif entity_type == "rotation":
                ***REMOVED*** Rotation - purple
                node_colors.append("***REMOVED***8844FF")
                node_sizes.append(300)
            else:
                ***REMOVED*** Unknown - gray
                node_colors.append("***REMOVED***888888")
                node_sizes.append(200)

        ***REMOVED*** Highlight specific entity if requested
        if highlight_entity:
            highlight_str = str(highlight_entity)
            if highlight_str in graph:
                idx = list(graph.nodes()).index(highlight_str)
                node_colors[idx] = "***REMOVED***FFFF00"  ***REMOVED*** Yellow
                node_sizes[idx] = 1000
        ...
        ***REMOVED*** Draw labels (only for keystones and highlighted entity)
        ...
        for node in label_nodes:
            if node in graph:
                node_data = graph.nodes[node]
                labels[node] = node_data.get("name", node)[:15]  ***REMOVED*** Truncate long names
        ...
        ***REMOVED*** Legend
        legend_elements = [
            mpatches.Patch(color="***REMOVED***FF4444", label="Keystone Resource"),
            mpatches.Patch(color="***REMOVED***4444FF", label="Faculty"),
            mpatches.Patch(color="***REMOVED***44FF44", label="Resident"),
            mpatches.Patch(color="***REMOVED***FF8844", label="Service"),
            mpatches.Patch(color="***REMOVED***8844FF", label="Rotation"),
        ]

        if highlight_entity:
            legend_elements.append(
                mpatches.Patch(color="***REMOVED***FFFF00", label="Cascade Origin")
            )
    ...
    def visualize_cascade(
        ...
        ***REMOVED*** Extract cascade levels from steps
        levels = []
        max_level = cascade_analysis.cascade_depth

        ***REMOVED*** Level 0: Origin
        levels.append(
            {
                "level": 0,
        ...
        ***REMOVED*** Subsequent levels from cascade steps
            ...
            levels.append(
                {
                    ...
                    "color": f"***REMOVED***{255 - level_num * 40:02x}4444",
                }
            )

        ***REMOVED*** Draw cascade levels
        ...
        for i, level_data in enumerate(levels):
            ...
            ***REMOVED*** Draw arrows to next level
            if i < len(levels) - 1:
    ...
    def visualize_redundancy_matrix(
        ...
        for i, keystone in enumerate(keystones):
            for j, service_id in enumerate(service_list):
                ...
                if keystone.entity_id in providers:
                    ***REMOVED*** Count alternatives (exclude keystone itself)
                    alternatives = len(
                        [p for p in providers if p != keystone.entity_id]
                    )
                    matrix[i, j] = alternatives

        ***REMOVED*** Create heatmap
        ...
        ***REMOVED*** Add text annotations for zero redundancy (critical)
    ...
    def visualize_succession_timeline(
        ...
        ***REMOVED*** Draw timeline bars
        for i, plan in enumerate(succession_plans):
            ...
            ***REMOVED*** Calculate bar position
            start_offset = (start_date - min_date).days
            duration = (end_date - start_date).days

            ***REMOVED*** Color by priority
            color_map = {
                "urgent": "***REMOVED***FF4444",
                "high": "***REMOVED***FF8844",
                "medium": "***REMOVED***FFBB44",
                "low": "***REMOVED***88FF88",
            }
            color = color_map.get(plan.training_priority, "***REMOVED***CCCCCC")
        ...
        ***REMOVED*** Legend
        legend_elements = [
            mpatches.Patch(color="***REMOVED***FF4444", label="Urgent Priority"),
            mpatches.Patch(color="***REMOVED***FF8844", label="High Priority"),
            mpatches.Patch(color="***REMOVED***FFBB44", label="Medium Priority"),
            mpatches.Patch(color="***REMOVED***88FF88", label="Low Priority"),
        ]
    ...
    def export_to_graphviz(
        ...
        try:
            ...
            ***REMOVED*** Add visual attributes to graph
            keystone_ids = {str(k.entity_id) for k in keystones}
