...
***REMOVED*** Level descriptions for UI display
LEVEL_DESCRIPTIONS = {
...
***REMOVED*** Clinical duty safety thresholds
DUTY_THRESHOLDS = {
    "critical_care": SamnPerelliLevel.A_LITTLE_TIRED,  ***REMOVED*** Max level 4 for ICU/ER
    "procedures": SamnPerelliLevel.OKAY,  ***REMOVED*** Max level 3 for procedures
    "inpatient": SamnPerelliLevel.MODERATELY_TIRED,  ***REMOVED*** Max level 5 for wards
    "outpatient": SamnPerelliLevel.MODERATELY_TIRED,  ***REMOVED*** Max level 5 for clinic
    "education": SamnPerelliLevel.EXTREMELY_TIRED,  ***REMOVED*** Max level 6 for conferences
    "administrative": SamnPerelliLevel.EXTREMELY_TIRED,  ***REMOVED*** Max level 6 for admin
}
...
def assess_fatigue_level(
    ...
    Example:
        ...
        ... )
        >>> print(assessment.safe_for_duty)  ***REMOVED*** True but with restrictions
        >>> print(assessment.duty_restrictions)  ***REMOVED*** ["critical_care", "procedures"]
    """
    ***REMOVED*** Validate level
...
def is_safe_for_duty(
    ...
    Example:
        ...
        >>> print(safe)  ***REMOVED*** False
        >>> print(reason)  ***REMOVED*** "Level 5 exceeds threshold of 3 for procedures"
    """
...
def estimate_level_from_factors(
    ...
    Example:
        ...
        ...     time_of_day_hour=4,  ***REMOVED*** Early morning
        ...     prior_sleep_hours=5.0
        ... )
        >>> print(level)  ***REMOVED*** Likely EXTREMELY_TIRED or EXHAUSTED
    """
    ***REMOVED*** Base score starts at 1 (fully alert)
    score = 1.0

    ***REMOVED*** Hours awake impact (major factor)
    ***REMOVED*** Awake > 16 hours significantly impairs performance
    if hours_awake > 24:
        score += 3.0
    elif hours_awake > 20:
        score += 2.5
    ...
    ***REMOVED*** Consecutive night shifts (circadian disruption)
    score += min(consecutive_night_shifts * 0.5, 1.5)

    ***REMOVED*** Time of day (circadian low point: 2-6 AM)
    if 2 <= time_of_day_hour <= 6:
        score += 1.5
    elif 14 <= time_of_day_hour <= 16:
        ***REMOVED*** Post-lunch dip
        score += 0.3

    ***REMOVED*** Prior sleep deficit
