...
class ShiftType(str, Enum):
    ...
    DAY = "day"  ***REMOVED*** Standard daytime (7 AM - 5 PM)
    EVENING = "evening"  ***REMOVED*** Evening shift (3 PM - 11 PM)
    NIGHT = "night"  ***REMOVED*** Night shift (7 PM - 7 AM)
    CALL_24 = "call_24"  ***REMOVED*** 24-hour in-house call
    CALL_28 = "call_28"  ***REMOVED*** 28-hour extended call
    POST_CALL = "post_call"  ***REMOVED*** Day after call (typically off by 12 PM)
    OFF = "off"  ***REMOVED*** Day off
    HALF_DAY = "half_day"  ***REMOVED*** Half-day clinic or education


***REMOVED*** Shift characteristics for fatigue modeling
SHIFT_CHARACTERISTICS = {
    ...
    ShiftType.POST_CALL: {
        ...
        "duration_hours": 5,
        "circadian_aligned": True,
        "fatigue_factor": 1.5,  ***REMOVED*** Still fatigued from call
    },
    ShiftType.OFF: {
...
class AlertnessPredictor:
    ...
    ***REMOVED*** Alertness thresholds
    HIGH_ALERT_THRESHOLD = 0.8  ***REMOVED*** Above this = good performance
    MODERATE_ALERT_THRESHOLD = 0.6  ***REMOVED*** Below this = concerning
    LOW_ALERT_THRESHOLD = 0.4  ***REMOVED*** Below this = high risk

    ***REMOVED*** Sleep inertia parameters
    ...
    def predict_alertness(
        ...
        Example:
            ...
            ...     []
            ... )
            >>> print(prediction.risk_level)  ***REMOVED*** "high" or "moderate"
        """
        ***REMOVED*** Calculate component scores
        ...
        ***REMOVED*** Estimate sleep debt if not provided
        ...
        ***REMOVED*** Combine into overall alertness score
        ***REMOVED*** Base alertness starts at 1.0 and is reduced by various factors
        base_alertness = 1.0

        ***REMOVED*** Apply circadian modulation
        alertness = base_alertness * circadian_score

        ***REMOVED*** Apply hours awake penalty (exponential decay after 16 hours)
        awake_penalty = self._hours_awake_penalty(hours_awake)
        alertness *= (1.0 - awake_penalty)

        ***REMOVED*** Apply sleep debt penalty
        alertness *= (1.0 - debt_impact)

        ***REMOVED*** Apply workload impact
        alertness *= (1.0 - workload_impact)

        ***REMOVED*** Apply sleep inertia if recently woke
        alertness *= (1.0 - sleep_inertia)

        ***REMOVED*** Clamp to valid range
        alertness = max(0.1, min(1.0, alertness))

        ***REMOVED*** Derive other metrics
    ...
    def predict_shift_trajectory(
        ...
        for i, shift in enumerate(upcoming_shifts):
            ***REMOVED*** Create sleep opportunity before this shift
            ...
            ***REMOVED*** Predict at shift midpoint (when fatigue often peaks)
            ...
            ***REMOVED*** Update running debt
            if shift.shift_type == ShiftType.OFF:
                ***REMOVED*** Recovery on off day
                ...
                ***REMOVED*** Accumulate debt from inadequate sleep
    ...
    def _estimate_hours_awake(
        ...
        if not sleep_history:
            ***REMOVED*** Default assumption: 8 hours if no history
            return 8.0

        ***REMOVED*** Find most recent sleep end time
        most_recent = max(sleep_history, key=lambda s: s.end_time)
        delta = target_time - most_recent.end_time

        ***REMOVED*** If sleep end is in the future, assume just woke up
    ...
    def _calculate_sleep_inertia(
        ...
        ***REMOVED*** Linear decay of inertia
    ...
    def _calculate_workload_impact(
        ...
        ***REMOVED*** Look at shifts in last 7 days
        ...
        ***REMOVED*** Sum fatigue factors weighted by recency
        total_impact = 0.0
        for shift in recent:
        ...
        ***REMOVED*** Cap at 0.4 (40% reduction)
    ...
    def _calculate_debt_impact(self, sleep_debt: float) -> float:
        ...
        ***REMOVED*** Linear relationship with saturation
        ***REMOVED*** 10 hours debt ≈ 20% reduction
    ...
    def _alertness_to_samn_perelli(
        ...
        ***REMOVED*** Use estimate function from samn_perelli module
        ***REMOVED*** Map alertness to hours worked approximation
        ...
        return estimate_level_from_factors(
            ...
            time_of_day_hour=12,  ***REMOVED*** Neutral
            prior_sleep_hours=7.0 * alertness,  ***REMOVED*** Inverse relationship
        )
    ...
    def _classify_risk(
        ...
        ***REMOVED*** High risk if any critical threshold exceeded
    ...
    def _generate_recommendations(
        ...
        ***REMOVED*** Circadian-specific advice
