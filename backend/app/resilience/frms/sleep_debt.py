...
class CircadianPhase(str, Enum):
    ...
    NADIR = "nadir"  ***REMOVED*** 2-6 AM: Lowest alertness, highest fatigue
...
***REMOVED*** Circadian alertness multipliers (1.0 = baseline)
***REMOVED*** Values < 1.0 indicate reduced alertness
CIRCADIAN_MULTIPLIERS = {
    CircadianPhase.NADIR: 0.6,  ***REMOVED*** Significant impairment
...
class SleepOpportunity:
    ...
    def effective_sleep_hours(self) -> float:
        ...
        ***REMOVED*** Quality adjustment
        base *= self.quality_factor

        ***REMOVED*** Circadian misalignment reduces recovery value by 20%
        if not self.circadian_aligned:
            base *= 0.8

        ***REMOVED*** Each interruption reduces value by 5%
        interruption_penalty = 1 - (self.interruptions * 0.05)
        base *= max(0.5, interruption_penalty)

        ***REMOVED*** Environment adjustment
...
class SleepDebtModel:
    ...
    BASELINE_SLEEP_NEED: float = 7.5  ***REMOVED*** Average adult requirement
    MAX_DEBT_HOURS: float = 40.0  ***REMOVED*** Beyond this, cumulative tracking less reliable
    DEBT_RECOVERY_RATIO: float = 1.5  ***REMOVED*** 1.5 hours extra per hour of debt
    ...
    def __init__(
        ...
        self._current_debt: dict[UUID, float] = {}  ***REMOVED*** In-memory cache
    ...
    def calculate_daily_debt(
        ...
        Example:
            ...
            ...     end_time=datetime(2024, 1, 2, 4, 30),  ***REMOVED*** Only 5.5 hours
            ...     quality_factor=0.9
            ... )
            >>> debt = model.calculate_daily_debt([sleep])
            >>> print(debt)  ***REMOVED*** ~2.5 hours (7.5 - 5.5 * 0.9)
        """
        total_effective_sleep = sum(
            s.effective_sleep_hours for s in sleep_opportunities
        )

        ***REMOVED*** Debt is the difference from baseline
    ...
    def update_cumulative_debt(
        ...
        ***REMOVED*** Track consecutive deficit days
        if daily_debt_change > 0:
            deficit_days += 1
        else:
            ***REMOVED*** If sleeping adequately, count toward recovery
            if natural_recovery and daily_debt_change < -0.5:
                ***REMOVED*** Reduce accumulated debt with recovery ratio
    ...
    def predict_debt_trajectory(
        ...
        Example:
            ...
            ...     start_debt=10.0
            ... )
            >>> ***REMOVED*** Shows debt increasing then recovering
        """
        ...
        for day, sleep_hours in enumerate(planned_sleep_hours):
            ***REMOVED*** Calculate daily change
            daily_change = self.baseline_sleep_need - sleep_hours

            ***REMOVED*** Update cumulative debt
            if daily_change > 0:
                current_debt += daily_change
                deficit_days += 1
            else:
                ***REMOVED*** Recovery during adequate sleep
    ...
    def estimate_recovery_time(
        ...
        Example:
            ...
            >>> print(nights)  ***REMOVED*** ~15 nights
        """
        if current_debt <= 0:
            return 0

        ***REMOVED*** Extra sleep per night above baseline
        extra_per_night = recovery_sleep_per_night - self.baseline_sleep_need

        if extra_per_night <= 0:
            return -1  ***REMOVED*** Cannot recover without extra sleep

        ***REMOVED*** Recovery per night (accounting for efficiency)
    ...
    def _calculate_impairment_equivalent(self, debt_hours: float) -> float:
        ...
        """
        ***REMOVED*** Linear approximation: 0.005 BAC per hour of debt
        bac = debt_hours * 0.005

        ***REMOVED*** Cap at 0.15 (severe impairment)
        return min(bac, 0.15)
