...
***REMOVED*** Hazard level thresholds
THRESHOLDS = {
    HazardLevel.GREEN: {
        "alertness_min": 0.7,
        "sleep_debt_max": 5.0,
    ...
    HazardLevel.BLACK: {
        "alertness_min": 0.0,  ***REMOVED*** Below RED triggers BLACK
        "sleep_debt_max": 40.0,
...
***REMOVED*** Required mitigations by hazard level
LEVEL_MITIGATIONS = {
    HazardLevel.GREEN: [],
    HazardLevel.YELLOW: [
...
class HazardThresholdEngine:
    ...
    Usage:
        ...
        if hazard.is_critical:
            ***REMOVED*** Trigger immediate intervention
            ...
    """
    ...
    def evaluate_hazard(
        ...
        """
        ***REMOVED*** Extract from prediction if provided
        if prediction:
        ...
        ***REMOVED*** Collect triggers and find highest hazard level
        triggers = []
        highest_level = HazardLevel.GREEN

        ***REMOVED*** Check each metric against thresholds
        ...
        ***REMOVED*** Get required mitigations for this level
        required_mits = LEVEL_MITIGATIONS.get(highest_level, [])

        ***REMOVED*** Calculate escalation time
    ...
    def _check_alertness(self, alertness: float) -> HazardLevel:
        ...
        for level in [HazardLevel.BLACK, HazardLevel.RED, HazardLevel.ORANGE,
            ...
            if alertness < THRESHOLDS[level]["alertness_min"]:
                ***REMOVED*** Below this level's minimum, so return the higher level
    ...
    def _estimate_escalation_time(
        ...
        if current_level == HazardLevel.BLACK:
            return None  ***REMOVED*** Already at highest

        now = datetime.utcnow()

        ***REMOVED*** Estimate based on hours awake progression
        if hours_awake is not None:
            current_threshold = THRESHOLDS[current_level]["hours_awake_max"]

            ***REMOVED*** Find next level's threshold
    ...
    def _get_recommended_mitigations(
        ...
        ***REMOVED*** Recommend mitigations from adjacent levels
        recommended = []
