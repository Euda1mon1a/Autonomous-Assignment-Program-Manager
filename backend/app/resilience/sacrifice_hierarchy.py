...
class LoadSheddingLevel(IntEnum):
    ...
    NORMAL = 0  ***REMOVED*** No shedding - all activities
    YELLOW = 1  ***REMOVED*** Suspend optional education
    ...
    CRITICAL = 5  ***REMOVED*** Patient safety only
...
class Activity:
    ...
    min_frequency: str | None = None  ***REMOVED*** e.g., "weekly", "monthly"
...
class SacrificeHierarchy:
    ...
    ***REMOVED*** Default sacrifice order (index = cut order, 0 = cut first)
    SACRIFICE_ORDER = [
        ...
        ***REMOVED*** PATIENT_SAFETY is never in sacrifice order
    ]
    ...
    def calculate_required_level(
        ...
        ***REMOVED*** Calculate capacity at each level
    ...
    def shed_load(
        ...
        """
        ***REMOVED*** Sort by sacrifice order (first to sacrifice = highest index in category)
        sorted_activities = sorted(
            current_demand,
            key=lambda a: (
                a.category.value,  ***REMOVED*** Higher category value = less protected
                not a.is_required,  ***REMOVED*** Required activities last within category
                a.faculty_hours,  ***REMOVED*** Smaller activities first
            ),
        ...
        ***REMOVED*** Build from most protected up
    ...
    def activate_level(
        ...
        Example:
            >>> hierarchy = SacrificeHierarchy()
            >>> ***REMOVED*** Escalate to ORANGE: suspend admin, research, optional education
        ...
        decision = SacrificeDecision(
            ...
            reason=reason,
            coverage_before=0.0,  ***REMOVED*** Would need context to calculate
            coverage_after=0.0,
            approved_by=approved_by,
    ...
    def deactivate_level(self, to_level: LoadSheddingLevel = LoadSheddingLevel.NORMAL):
        ...
        ***REMOVED*** Restore activities not in new sacrifice list
        restored = []
    ...
    def get_status(self) -> LoadSheddingStatus:
        ...
        return LoadSheddingStatus(
            ...
            decisions_log=self.decisions_log[-10:],  ***REMOVED*** Last 10 decisions
        )
    ...
    def get_recovery_plan(self) -> list[dict]:
        ...
        ***REMOVED*** Sort by category (most important first = restore first)
