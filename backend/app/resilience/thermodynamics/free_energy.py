...
def _calculate_internal_energy(assignments: list[Any]) -> float:
    ...
    ***REMOVED*** Count assignments per person (penalize overload)
    ...
    ***REMOVED*** Energy contribution from workload imbalance
    if person_counts:
        counts = list(person_counts.values())
        mean_count = sum(counts) / len(counts)
        variance = sum((c - mean_count) ** 2 for c in counts) / len(counts)
        energy += variance * 0.1  ***REMOVED*** Workload imbalance penalty

    ***REMOVED*** Energy contribution from constraint violations
    for assignment in assignments:
        ***REMOVED*** Check for score field (lower score = higher energy)
        score = getattr(assignment, "score", None)
        if score is not None:
            energy += (1.0 - score) * 0.5  ***REMOVED*** Preference mismatch penalty

        ***REMOVED*** Check for violation flags
        has_violation = getattr(assignment, "has_violation", False)
        if has_violation:
            energy += 1.0  ***REMOVED*** Hard constraint violation
...
def _calculate_configuration_entropy(assignments: list[Any]) -> float:
    ...
    ***REMOVED*** Count unique configurations (person-rotation pairs)
...
def calculate_free_energy(
    ...
    ***REMOVED*** Helmholtz free energy
    free_energy = internal_energy - entropy_term

    ***REMOVED*** Count violations for reporting
...
class EnergyLandscapeAnalyzer:
    ...
    def analyze_landscape(
        ...
        ***REMOVED*** Check if current is local minimum (all gradients >= 0)
        is_local_minimum = all(g >= -0.01 for g in gradients)

        ***REMOVED*** Estimate ruggedness (standard deviation of energies)
        ruggedness = float(np.std(energy_array)) if len(energy_array) > 1 else 0.0

        ***REMOVED*** Find barrier heights (energy increases from current)
        barriers = [g for g in gradients if g > 0]

        ***REMOVED*** Estimate basin depth (max energy decrease possible)
    ...
    def _perturb_assignments(self, assignments: list[Any]) -> list[Any]:
        ...
        ***REMOVED*** Create a shallow copy and randomly swap a few elements
...
def adaptive_temperature(
    ...
    if schedule == "exponential":
        ***REMOVED*** Geometric cooling: T(t) = T0 * rate^t
        temp = initial_temp * (cooling_rate**iteration)

    elif schedule == "linear":
        ***REMOVED*** Linear cooling: T(t) = max(T0 - rate * t, min_temp)
        temp = initial_temp - cooling_rate * iteration

    elif schedule == "logarithmic":
        ***REMOVED*** Logarithmic cooling: T(t) = T0 / log(t + 2)
        ***REMOVED*** This is proven to be asymptotically optimal for SA
        temp = initial_temp / math.log(iteration + 2)

    elif schedule == "cauchy":
        ***REMOVED*** Cauchy cooling: T(t) = T0 / (1 + t)
        ***REMOVED*** Faster than logarithmic, still good convergence
        temp = initial_temp / (1 + iteration)

    elif schedule == "boltzmann":
        ***REMOVED*** Boltzmann cooling: T(t) = T0 / (1 + log(1 + t))
        ***REMOVED*** Compromise between logarithmic and cauchy
...
def boltzmann_probability(energy_delta: float, temperature: float) -> float:
    ...
    if temperature <= 0:
        return 0.0  ***REMOVED*** Never accept worsening at T=0

    ***REMOVED*** Boltzmann factor
    exponent = -energy_delta / temperature
    if exponent < -700:  ***REMOVED*** Avoid underflow
        return 0.0
