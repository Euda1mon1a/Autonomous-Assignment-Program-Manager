...
class FreeEnergyMetrics:
    ...
    def stability_score(self) -> float:
        ...
        """
        ***REMOVED*** Convert free energy to 0-1 scale using sigmoid
...
def _calculate_internal_energy(assignments: list[Any]) -> float:
    ...
    ***REMOVED*** Extract person IDs and rotation types for analysis
    person_counts: Counter = Counter()
    rotation_counts: Counter = Counter()

    for assignment in assignments:
        ***REMOVED*** Handle both dict and object assignments
    ...
    ***REMOVED*** Workload imbalance energy (variance in assignments per person)
    if person_counts:
        counts = list(person_counts.values())
        mean_count = sum(counts) / len(counts)
        variance = sum((c - mean_count) ** 2 for c in counts) / len(counts)
        ***REMOVED*** Normalize variance contribution
        energy += math.sqrt(variance) * 0.5

    ***REMOVED*** Rotation diversity penalty (too many same-type assignments = higher energy)
    if rotation_counts:
        total = sum(rotation_counts.values())
        ***REMOVED*** Calculate Gini coefficient for rotation distribution
        sorted_counts = sorted(rotation_counts.values())
        n = len(sorted_counts)
        if n > 1 and total > 0:
            gini = sum((2 * i - n - 1) * c for i, c in enumerate(sorted_counts, 1))
            gini = gini / (n * total)
            ***REMOVED*** Lower Gini = more equal distribution = lower energy
...
def _calculate_entropy(assignments: list[Any]) -> float:
    ...
    ***REMOVED*** Count unique configurations
    config_counts: Counter = Counter()

    for assignment in assignments:
        ***REMOVED*** Create configuration key
        if isinstance(assignment, dict):
            rotation = assignment.get("rotation_type", assignment.get("rotation", "?"))
        else:
            rotation = getattr(
    ...
    ***REMOVED*** Calculate Shannon entropy
...
class LandscapeAnalysis:
    ...
    num_basins: int
    roughness: float  ***REMOVED*** Measure of landscape ruggedness
    barrier_heights: list[float]  ***REMOVED*** Heights of barriers between minima
    funnel_score: float  ***REMOVED*** 0-1, higher = more funnel-like (easier optimization)
...
class EnergyLandscapeAnalyzer:
    ...
    def analyze_landscape(self, assignments: list[Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Analyze collected samples
        ...
        ***REMOVED*** Count local minima (approximation based on gradient changes)
        local_minima_count = self._estimate_local_minima(energy_gradients)

        ***REMOVED*** Calculate roughness (normalized variance of gradients)
        if energy_gradients:
        ...
        ***REMOVED*** Funnel score: inverse of roughness, adjusted for energy concentration
        ...
        ***REMOVED*** Estimate barrier heights
    ...
    def _random_perturbation(self) -> float:
        ...
        ***REMOVED*** Use normal distribution centered at 0
    ...
    def _estimate_local_minima(self, gradients: list[float]) -> int:
        ...
        ***REMOVED*** Approximate local minima count
    ...
    def _estimate_barriers(self) -> float:
        ...
        ***REMOVED*** Barrier estimate: difference between highest and median
    ...
    def _estimate_basin_sizes(self) -> list[float]:
        ...
        ***REMOVED*** Simple binning approach
...
class AdaptiveTemperatureScheduler:
    ...
    Example:
        ...
        for iteration in range(1000):
            ...
            ***REMOVED*** Use temp for acceptance probability
            if accepted:
    ...
    def get_temperature(self, iteration: int) -> float:
        ...
        ***REMOVED*** Check for reheat
        ...
        if self.schedule_type == "exponential":
            ...
            ***REMOVED*** Default to exponential
            temp = self.initial_temp * (self.cooling_rate**iteration)

        ***REMOVED*** Clamp to final temperature
    ...
    def record_acceptance(self, accepted: bool) -> None:
        ...
        ***REMOVED*** Keep only recent history
...
def boltzmann_probability(energy_delta: float, temperature: float) -> float:
    ...
    """
    if energy_delta <= 0:
        return 1.0  ***REMOVED*** Always accept improvements

    if temperature <= 0:
        return 0.0  ***REMOVED*** Never accept worse at T=0

    exponent = -energy_delta / temperature
    if exponent < -700:  ***REMOVED*** Prevent underflow
        return 0.0
