...
def calculate_shannon_entropy(distribution: list[Any]) -> float:
    ...
    Example:
        >>> calculate_shannon_entropy([1, 1, 1, 1])  ***REMOVED*** Uniform
        2.0
        >>> calculate_shannon_entropy([1, 1, 1, 2])  ***REMOVED*** Skewed
        0.811
        >>> calculate_shannon_entropy([1, 1, 1, 1, 2, 2, 2, 2])  ***REMOVED*** Balanced
        1.0
    """
    if not distribution:
        return 0.0
...
def calculate_schedule_entropy(assignments: list[Any]) -> EntropyMetrics:
    ...
    ***REMOVED*** Calculate individual entropies
    ...
    ***REMOVED*** Calculate joint entropy (person, rotation)
    if person_dist and rotation_dist:
        ...
        ***REMOVED*** Mutual information: how much knowing person tells about rotation
        MI = H_person + H_rotation - H_joint
    else:
        H_joint = 0.0
        MI = 0.0

    ***REMOVED*** Normalized entropy (relative to maximum possible)
    ***REMOVED*** Maximum entropy occurs with uniform distribution
...
def mutual_information(dist_X: list[Any], dist_Y: list[Any]) -> float:
    ...
    Example:
        >>> ***REMOVED*** Faculty and rotation strongly coupled
        >>> faculty = [1, 1, 2, 2, 3, 3]
        >>> rotations = ['A', 'A', 'B', 'B', 'C', 'C']
        >>> mutual_information(faculty, rotations)
        1.585  ***REMOVED*** High MI: perfect correlation
    """
    ...
    ***REMOVED*** Mutual information
    ...
    return max(0.0, MI)  ***REMOVED*** Ensure non-negative (numerical errors)
...
def conditional_entropy(dist_X: list[Any], dist_Y: list[Any]) -> float:
    ...
    ***REMOVED*** Joint and individual entropies
    joint_dist = list(zip(dist_X, dist_Y))
    H_joint = calculate_shannon_entropy(joint_dist)
    H_Y = calculate_shannon_entropy(dist_Y)
...
def entropy_production_rate(
    ...
    ***REMOVED*** Entropy change
    dS = S_new - S_old

    ***REMOVED*** Production rate (only count increases, not decreases)
    ***REMOVED*** Decreases are compensated by entropy export to environment
...
class ScheduleEntropyMonitor:
    ...
    def update(self, assignments: list[Any]):
        ...
        ***REMOVED*** Calculate production rate if we have previous state
    ...
    def detect_critical_slowing(self) -> bool:
        ...
        ***REMOVED*** Critical slowing: high autocorrelation + low rate of change
