...
class PhaseTransitionDetector:
    ...
    def detect_critical_phenomena(self) -> PhaseTransitionRisk:
        ...
        for metric_name, values in self.metric_history.items():
            ...
            ***REMOVED*** 4. Skewness change
            signal = self._detect_skewness(metric_name, values)
            if signal:
                all_signals.append(signal)

        ***REMOVED*** Assess overall severity
    ...
    def _detect_variance_trend(
        ...
        ***REMOVED*** Split into early and recent halves
        mid = len(values) // 2
        ...
        ***REMOVED*** Thresholds
        if variance_increase > 0.5:  ***REMOVED*** 50% increase
    ...
    def _detect_autocorrelation(
        ...
        ***REMOVED*** High autocorrelation indicates critical slowing
        if autocorr > 0.7:
    ...
    def _detect_flickering(
        ...
        ***REMOVED*** Calculate flicker rate
        flicker_rate = self._calculate_flicker_rate(values)

        if flicker_rate > 0.3:  ***REMOVED*** 30% of samples show flickering
    ...
    def _calculate_flicker_rate(self, values: list[float]) -> float:
        ...
        ***REMOVED*** Threshold for significant change
    ...
    def _estimate_time_to_transition(self) -> Optional[float]:
        ...
        ***REMOVED*** Distance to critical point ∝ (1 - autocorr)
        ...
        if distance < 0.05:
            return 0.0  ***REMOVED*** Imminent

        ***REMOVED*** Rough estimate: time constant ∝ 1 / (1 - r)
        ***REMOVED*** This is a simplified model
        time_constant = 1.0 / distance

        ***REMOVED*** Convert to hours (calibration factor)
        estimated_hours = time_constant * 10  ***REMOVED*** Calibrate based on empirical data
    ...
    def _generate_recommendations(
        ...
        ***REMOVED*** Signal-specific recommendations
        ...
        return list(set(recommendations))  ***REMOVED*** Remove duplicates
    ...
    def _calculate_confidence(self, signals: list[CriticalSignal]) -> float:
        ...
        ***REMOVED*** Count distinct metric names (independent evidence)
        unique_metrics = len(set(s.metric_name for s in signals))

        ***REMOVED*** Count distinct signal types
        unique_types = len(set(s.signal_type for s in signals))

        ***REMOVED*** Confidence increases with multiple independent lines of evidence
...
class CriticalPhenomenaMonitor:
    ...
    async def update_and_assess(self, metrics: dict[str, float]) -> PhaseTransitionRisk:
        ...
        ***REMOVED*** Trigger alerts if critical
