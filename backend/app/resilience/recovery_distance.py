...
class RecoveryDistanceCalculator:
    ...
    def calculate_for_event(
        ...
        ***REMOVED*** Extract schedule components
        assignments = schedule.get("assignments", [])
        blocks = schedule.get("blocks", [])
        people = schedule.get("people", [])

        ***REMOVED*** Step 1: Apply event (remove resource)
        post_event_assignments = self._apply_event(assignments, event)

        ***REMOVED*** Step 2: Check if still feasible (RD=0)
        ...
        ***REMOVED*** Step 3: Bounded DFS for minimum edits
        for depth in range(1, self.max_depth + 1):
            ...
            ***REMOVED*** Try all edit combinations at this depth
        ...
        ***REMOVED*** No solution found within depth/time limits
        result = RecoveryResult(
            event=event,
    ...
    def calculate_aggregate(
        ...
        ***REMOVED*** 95th percentile (for worst-case planning)
        if distances:
        ...
        ***REMOVED*** Breakdown by event type
    ...
    def generate_test_events(
        ...
        ***REMOVED*** Build assignment index
        ...
        ***REMOVED*** Generate faculty absence events
        ...
        ***REMOVED*** Generate resident sick day events
        residents = [p for p in people if getattr(p, "role", None) == "RESIDENT"]
        for resident in residents:
            affected = assignments_by_person.get(resident.id, [])
            if affected:
                ***REMOVED*** For residents, test single-day absence (2 blocks: AM + PM)
                if blocks:
                    ***REMOVED*** Pick a representative day
    ...
    ***REMOVED*** =========================================================================
    ...
    def _is_feasible(
        ...
        """
        ***REMOVED*** Build coverage map
        coverage: dict[UUID, int] = {}
        for assignment in assignments:
            block_id = assignment.block_id
            coverage[block_id] = coverage.get(block_id, 0) + 1

        ***REMOVED*** Check minimum coverage (at least 1 per block for now)
        ***REMOVED*** In production, use schedule.get("coverage_requirements", {})
        for block in blocks:
            if coverage.get(block.id, 0) < 1:
                return False

        ***REMOVED*** Check for double-booking
    ...
    def _search_at_depth(
        ...
        """
        ***REMOVED*** For depth 1, try simple edits
        if depth == 1:
            ***REMOVED*** Try reassigning each uncovered block to available person
            ...
            for block in uncovered_blocks:
                for person in people:
                    ***REMOVED*** Try assigning this person to this block
        ...
        ***REMOVED*** For deeper searches, use recursive DFS (simplified for now)
        ***REMOVED*** In production, implement full combinatorial search with pruning
    ...
    def _create_mock_assignment(self, person_id: UUID, block_id: UUID):
        ...
        ***REMOVED*** Create a simple mock object with required attributes
