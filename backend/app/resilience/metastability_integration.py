...
Example:
    ...
    ***REMOVED*** Solve with metastability detection
    result = wrapper.solve(context, existing_assignments)

    ***REMOVED*** Access metastability analysis
...
class MetastabilitySolutionCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        ...
        ***REMOVED*** Get solver statistics
        objective_value = self.ObjectiveValue()

        ***REMOVED*** Note: CP-SAT doesn't provide direct access to constraint violations
        ***REMOVED*** in callback, so we assume solutions are always feasible
        state = SolverState(
            iteration=self.iteration,
            objective_value=objective_value,
            constraint_violations=0,  ***REMOVED*** CP-SAT only returns feasible solutions
            num_assignments=0,  ***REMOVED*** Could track if model provides access
            temperature=1.0,  ***REMOVED*** CP-SAT doesn't use explicit temperature
            metadata={
...
class MetastabilitySolverWrapper:
    ...
    Example:
        ...
        ***REMOVED*** Check for metastability
        if result.metastability_analysis.is_metastable:
    ...
    def solve(self, context, existing_assignments=None, run_id: str | None = None):
        ...
        ***REMOVED*** Create trajectory logger (would be attached to actual solver)
        ***REMOVED*** NOTE: This is a simplified example. In practice, you'd integrate
        ***REMOVED*** with the specific solver's callback mechanism
        trajectory: list[SolverState] = []

        ***REMOVED*** Solve with base solver
        ***REMOVED*** In a real implementation, you'd attach callbacks here
        start_time = time.time()
        result = self.base_solver.solve(context, existing_assignments)
        elapsed = time.time() - start_time

        ***REMOVED*** Simulate trajectory for demonstration
        ***REMOVED*** In reality, this would come from solver callbacks
        trajectory = self._simulate_trajectory_from_result(result)

        ***REMOVED*** Perform metastability analysis
        analysis = self.detector.analyze_solver_trajectory(trajectory)

        ***REMOVED*** Apply strategy if auto-apply enabled and metastable
        ...
        if self.auto_apply_strategy and analysis.is_metastable:
            ...
            ***REMOVED*** Apply strategy (simplified - in reality would modify solver params)
            if analysis.recommended_strategy == EscapeStrategy.RESTART_NEW_SEED:
                logger.info("Strategy would restart solver with new seed")
                ***REMOVED*** In practice: re-run solver with different seed
                ...
                ***REMOVED*** In practice: modify solver parameters
            elif analysis.recommended_strategy == EscapeStrategy.BASIN_HOPPING:
                logger.info("Strategy would apply basin hopping")
                ***REMOVED*** In practice: perturb solution and restart

        ***REMOVED*** Create enriched result
    ...
    def _simulate_trajectory_from_result(
        ...
        """
        ***REMOVED*** Create a simple trajectory based on result
        ***REMOVED*** In reality, this would come from actual solver callbacks
        trajectory = []

        ***REMOVED*** Simulate improving trajectory
        if result.success:
            ***REMOVED*** Start from worse objective, improve to final
            initial_obj = result.objective_value * 2.0
            final_obj = result.objective_value

            num_steps = 100
            for i in range(num_steps):
                ***REMOVED*** Exponential decay toward final objective
...
def apply_escape_strategy(
    ...
    if strategy == EscapeStrategy.INCREASE_TEMPERATURE:
        ***REMOVED*** Increase randomness in search
        ...
        ***REMOVED*** Completely new random seed
        import random
        ...
        ***REMOVED*** Increase search randomness significantly
...
def check_metastability_during_solve(
    ...
    """
    ***REMOVED*** Only check at intervals to avoid overhead
    if len(trajectory) % check_interval != 0:
        return False, None

    ***REMOVED*** Analyze trajectory
    analysis = detector.analyze_solver_trajectory(trajectory)

    ***REMOVED*** Decide if intervention needed
    should_intervene = (
