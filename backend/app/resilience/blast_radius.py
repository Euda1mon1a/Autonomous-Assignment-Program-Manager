...
class ZoneStatus(str, Enum):
    ...
    GREEN = "green"         ***REMOVED*** Fully operational, can support others
    YELLOW = "yellow"       ***REMOVED*** Operational but at minimum capacity
    ORANGE = "orange"       ***REMOVED*** Degraded, using backup faculty
...
class ZoneType(str, Enum):
    ...
    INPATIENT = "inpatient"      ***REMOVED*** ICU, wards, procedures
    OUTPATIENT = "outpatient"    ***REMOVED*** Clinics, consults
    EDUCATION = "education"      ***REMOVED*** Didactics, simulation
    RESEARCH = "research"        ***REMOVED*** Research activities
    ADMINISTRATIVE = "admin"     ***REMOVED*** Meetings, committees
    ON_CALL = "on_call"          ***REMOVED*** Call coverage


class BorrowingPriority(str, Enum):
    """Priority levels for cross-zone borrowing."""
    CRITICAL = "critical"    ***REMOVED*** Life safety, must fulfill
    HIGH = "high"            ***REMOVED*** Important, should fulfill if possible
    MEDIUM = "medium"        ***REMOVED*** Normal priority
    LOW = "low"              ***REMOVED*** Optional, decline if any strain


class ContainmentLevel(str, Enum):
    """Level of failure containment."""
    NONE = "none"               ***REMOVED*** No containment active
    SOFT = "soft"               ***REMOVED*** Advisory, log borrowing
    MODERATE = "moderate"       ***REMOVED*** Require approval for borrowing
...
class ZoneFacultyAssignment:
    ...
    faculty_id: UUID
    faculty_name: str
    role: str  ***REMOVED*** "primary", "secondary", "backup"
    available: bool = True
...
class BorrowingRequest:
    ...
    ***REMOVED*** Approval tracking
    status: str = "pending"  ***REMOVED*** pending, approved, denied, completed
...
class ZoneIncident:
    ...
    id: UUID
    zone_id: UUID
    incident_type: str  ***REMOVED*** "faculty_loss", "demand_surge", "quality_issue", "external"
    description: str
    started_at: datetime
    severity: str  ***REMOVED*** "minor", "moderate", "severe", "critical"
...
class SchedulingZone:
    ...
    ***REMOVED*** Services in this zone
    services: list[str] = field(default_factory=list)

    ***REMOVED*** Faculty assignments
...
class ZoneHealthReport:
    ...
    ***REMOVED*** Capacity
    ...
    capacity_ratio: float  ***REMOVED*** available / minimum
...
class BlastRadiusManager:
    ...
    def create_default_zones(self) -> list[SchedulingZone]:
        ...
        zones = [
            self.create_zone(
                ...
                minimum_coverage=3,
                optimal_coverage=5,
                priority=10,  ***REMOVED*** Highest priority
            ),
            self.create_zone(
            ...
            self.create_zone(
                ...
                minimum_coverage=0,
                optimal_coverage=1,
                priority=1,  ***REMOVED*** Lowest priority
            ),
        ]

        ***REMOVED*** Set up borrowing relationships (higher can borrow from lower)
        for _i, zone in enumerate(zones):
            ***REMOVED*** Can borrow from lower priority zones
            zone.can_borrow_from = [z.id for z in zones if z.priority < zone.priority]
            ***REMOVED*** Can lend to higher priority zones
    ...
    def request_borrowing(
        ...
        Example:
            ...
            >>> ***REMOVED*** Request emergency borrowing from outpatient to cover inpatient
        ...
        ***REMOVED*** Check containment level
        ...
        ***REMOVED*** Check zone-level containment
        ...
        ***REMOVED*** Check if lending zone is in can_borrow_from list
        ...
        ***REMOVED*** Check lending zone capacity
    ...
    def record_incident(
        ...
        ***REMOVED*** Calculate capacity lost
        if faculty_affected:
            total_faculty = (
        ...
        ***REMOVED*** Update zone status
        self._update_zone_status(zone)

        ***REMOVED*** Trigger containment if severe
    ...
    def check_zone_health(self, zone_id: UUID) -> ZoneHealthReport | None:
        ...
        ***REMOVED*** Check active incidents
    ...
    def check_all_zones(self) -> BlastRadiusReport:
        ...
        ***REMOVED*** Count borrowing requests
        ...
        for zone in self.zones.values():
            for incident in zone.incidents:
                if incident.id == incident_id:
                    ...
                    ***REMOVED*** Check if containment can be relaxed
                    active_incidents = [i for i in zone.incidents if not i.resolved_at]
