...
class Vulnerability:
    ...
    is_unique_provider: bool = False  ***REMOVED*** Only person who can cover something
    details: str = ""
...
class FatalPair:
    ...
    affected_services: list[str] = field(default_factory=list)
    probability_estimate: str = "unknown"  ***REMOVED*** Could be "low", "medium", "high"
...
class VulnerabilityReport:
    ...
    ***REMOVED*** N-1 Analysis
    ...
    n1_pass: bool = True  ***REMOVED*** True if system survives any single loss

    ***REMOVED*** N-2 Analysis
    n2_fatal_pairs: list[FatalPair] = field(default_factory=list)
    n2_pass: bool = True  ***REMOVED*** True if system survives any pair loss

    ***REMOVED*** Summary
    most_critical_faculty: list[UUID] = field(default_factory=list)
    recommended_actions: list[str] = field(default_factory=list)

    ***REMOVED*** Phase transition indicators
    phase_transition_risk: str = "low"  ***REMOVED*** "low", "medium", "high", "critical"
...
class CentralityScore:
    ...
    faculty_id: UUID
    faculty_name: str
    score: float  ***REMOVED*** 0.0 to 1.0, higher = more critical

    ***REMOVED*** Component scores
    services_covered: int  ***REMOVED*** Unique services they can cover
    unique_coverage_slots: int  ***REMOVED*** Slots only they can fill
    replacement_difficulty: float  ***REMOVED*** 0.0 to 1.0, higher = harder to replace
    workload_share: float  ***REMOVED*** What fraction of total work they do

    ***REMOVED*** NetworkX-based centrality metrics (optional, set when using graph analysis)
    betweenness: float = 0.0  ***REMOVED*** How often on shortest paths
    degree: float = 0.0  ***REMOVED*** Number of connections
    eigenvector: float = 0.0  ***REMOVED*** Importance from important connections
    pagerank: float = 0.0  ***REMOVED*** PageRank score


@dataclass
class CascadeSimulation:
    """Results of simulating a cascade failure."""
    initial_failures: list[UUID]  ***REMOVED*** Faculty who initially failed
    cascade_steps: list[dict]  ***REMOVED*** Each step: {step, failed_faculty, reason, remaining}
    total_failures: int
    final_coverage: float
    cascade_length: int  ***REMOVED*** How many steps before stabilization
    is_catastrophic: bool  ***REMOVED*** Did system collapse completely?
...
class ContingencyAnalyzer:
    ...
    def analyze_n1(
        ...
        ***REMOVED*** Build assignment lookup
        ...
        ***REMOVED*** For each faculty, simulate their absence
        for fac in faculty:
            ...
            for assignment in fac_assignments:
                ...
                ***REMOVED*** Check if this faculty is the only one covering this block
                other_coverage = [
        ...
        ***REMOVED*** Sort by severity and affected blocks
    ...
    def analyze_n2(
        ...
        ***REMOVED*** Build assignment lookups
        assignments_by_faculty = {}
        for assignment in current_assignments:
            if assignment.person_id not in assignments_by_faculty:
                assignments_by_faculty[assignment.person_id] = []
        ...
        ***REMOVED*** Determine which faculty to analyze
        if critical_faculty_only:
            ***REMOVED*** First run N-1 to find critical faculty
            ...
            ***REMOVED*** If no critical faculty, analyze top 5 by assignment count
        ...
        ***REMOVED*** Check all pairs
        for fac1, fac2 in combinations(analysis_faculty, 2):
            ***REMOVED*** Calculate combined absence impact
            ...
            ***REMOVED*** Count uncoverable blocks
            uncoverable = 0
    ...
    def calculate_centrality(
        ...
        faculty: list,
        assignments: list,
        services: dict[UUID, list[UUID]],  ***REMOVED*** service_id -> list of credentialed faculty
        ...
        Example:
            >>> analyzer = ContingencyAnalyzer()
            >>> services = {svc_uuid: [fac1.id, fac2.id]}  ***REMOVED*** Both can do this service
        ...
        ***REMOVED*** Build assignment count by faculty
        assignment_counts = {}
        for a in assignments:
            assignment_counts[a.person_id] = assignment_counts.get(a.person_id, 0) + 1

        for fac in faculty:
            ***REMOVED*** Services covered
            services_covered = sum(
                1 for svc_faculty in services.values()
                if fac.id in svc_faculty
            )

            ***REMOVED*** Unique coverage (services where they're the only option)
            unique_coverage = sum(
                1 for svc_faculty in services.values()
                if svc_faculty == [fac.id]
            )

            ***REMOVED*** Replacement difficulty (inverse of how many others can do their services)
            ...
            ***REMOVED*** Combined score (weighted)
            score = (
    ...
    def build_scheduling_graph(
        ...
        ***REMOVED*** Add assignment edges (faculty -> block)
        ...
        ***REMOVED*** Add credential edges (faculty -> service)
    ...
    def calculate_centrality_networkx(
        ...
        ***REMOVED*** Calculate NetworkX centrality metrics
        try:
            betweenness = nx.betweenness_centrality(G)
            degree = nx.degree_centrality(G)
            pagerank = nx.pagerank(G, alpha=0.85)

            ***REMOVED*** Eigenvector can fail on disconnected graphs
            try:
        ...
        ***REMOVED*** Build scores
        ...
        for fac in faculty:
            ...
            ***REMOVED*** Calculate basic metrics
            services_covered = sum(
                1 for svc_faculty in services.values()
                if fac.id in svc_faculty
            )
            ...
            ***REMOVED*** Combined score using NetworkX metrics
            ***REMOVED*** Weight betweenness and pagerank heavily - they capture "hub" importance
            score = (
                0.25 * nx_betweenness +
                0.25 * nx_pagerank +
                0.15 * nx_degree +
    ...
    def simulate_cascade_failure(
        ...
        Example:
            >>> analyzer = ContingencyAnalyzer()
            >>> ***REMOVED*** Simulate what happens if Dr. Smith becomes unavailable
        ...
        """
        ***REMOVED*** Build faculty workload map
        assignment_counts = {}
        for a in assignments:
            assignment_counts[a.person_id] = assignment_counts.get(a.person_id, 0) + 1
        ...
        ***REMOVED*** Remove initial failures
        for fid in initial_failures:
            if fid in active_faculty:
                del active_faculty[fid]

        ***REMOVED*** Calculate initial load redistribution
        ...
        while remaining_capacity > 0:
            ...
            ***REMOVED*** Calculate load per remaining faculty
            total_load / remaining_capacity if remaining_capacity > 0 else float('inf')

            ***REMOVED*** Find faculty who would be overloaded
            new_failures = []
            for fac_id in list(active_faculty.keys()):
                base_load = assignment_counts.get(fac_id, 0)
                ***REMOVED*** Rough model: each faculty takes equal share of redistributed load
                effective_load = base_load + (total_load / remaining_capacity)

                ***REMOVED*** Normal capacity is their current load / max_utilization
            ...
            if not new_failures:
                break  ***REMOVED*** Stable

            ***REMOVED*** Record cascade step
    ...
    def find_critical_failure_points(
        ...
        ***REMOVED*** For top centrality faculty, simulate cascade
        for score in centrality[:10]:  ***REMOVED*** Top 10
    ...
    def detect_phase_transition_risk(
        ...
        ***REMOVED*** Check change frequency
        if recent_changes:
            ***REMOVED*** Count changes in last 7 days
        ...
        ***REMOVED*** Check for patterns (simplified)
        if len(indicators) >= 3:
            risk = "critical"
    ...
    def generate_report(
        ...
        ***REMOVED*** Identify most critical faculty
        critical_ids = [
            v.faculty_id for v in n1_vulns
            if v.severity in ("critical", "high")
        ][:5]

        ***REMOVED*** Detect phase transition risk
