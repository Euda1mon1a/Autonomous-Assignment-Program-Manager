...
***REMOVED*** Try to import NetworkX for advanced analysis
try:
...
class HubRiskLevel(str, Enum):
    ...
    LOW = "low"            ***REMOVED*** Easy to cover, multiple backups
    MODERATE = "moderate"  ***REMOVED*** Some impact, backups available
    HIGH = "high"          ***REMOVED*** Significant impact, limited backups
    CRITICAL = "critical"  ***REMOVED*** Major disruption, no viable backups
    CATASTROPHIC = "catastrophic"  ***REMOVED*** System failure possible


class HubProtectionStatus(str, Enum):
    """Current protection status of a hub."""
    UNPROTECTED = "unprotected"       ***REMOVED*** No special measures
    MONITORED = "monitored"           ***REMOVED*** Extra attention
    PROTECTED = "protected"           ***REMOVED*** Active measures in place
    REDUNDANT = "redundant"           ***REMOVED*** Backup hub trained
...
class FacultyCentrality:
    ...
    ***REMOVED*** Centrality scores (0.0 - 1.0 normalized)
    degree_centrality: float = 0.0
    betweenness_centrality: float = 0.0
    eigenvector_centrality: float = 0.0
    pagerank: float = 0.0

    ***REMOVED*** Composite score
    composite_score: float = 0.0

    ***REMOVED*** Coverage metrics
    services_covered: int = 0
    unique_services: int = 0  ***REMOVED*** Services only this person can cover
    total_assignments: int = 0

    ***REMOVED*** Replacement difficulty (0.0 easy - 1.0 impossible)
...
class HubProfile:
    ...
    ***REMOVED*** Cross-training needs
    cross_training_candidates: list[tuple[UUID, str]]  ***REMOVED*** (faculty_id, skill)
...
class CrossTrainingRecommendation:
    ...
    risk_reduction: float  ***REMOVED*** How much hub risk would decrease

    ***REMOVED*** Status
    created_at: datetime
    status: str = "pending"  ***REMOVED*** pending, approved, in_progress, completed
...
class HubDistributionReport:
    ...
    ***REMOVED*** Distribution metrics
    hub_concentration: float  ***REMOVED*** 0.0 spread out - 1.0 concentrated
...
class HubProtectionPlan:
    ...
    ***REMOVED*** Protection measures
    workload_reduction: float  ***REMOVED*** 0.0 none - 1.0 complete relief
    backup_assigned: bool
    backup_faculty_ids: list[UUID]
    critical_only: bool  ***REMOVED*** Limit to critical services

    ***REMOVED*** Status
    status: str = "planned"  ***REMOVED*** planned, active, completed
...
class HubAnalyzer:
    ...
    def calculate_centrality(
        ...
        faculty: list,
        assignments: list,
        services: dict[UUID, list[UUID]],  ***REMOVED*** service_id -> faculty_ids who can cover
    ...
    def _calculate_with_networkx(
        ...
        ***REMOVED*** Build centrality objects
        results = []
        ...
        for f in faculty:
            ...
            ***REMOVED*** Count services this faculty can cover
            covered_services = [
                sid for sid, facs in services.items()
                if fac_id in facs
            ]

            ***REMOVED*** Count unique services (only this person)
            unique_services = [
                sid for sid, facs in services.items()
                if len(facs) == 1 and fac_id in facs
            ]

            ***REMOVED*** Calculate replacement difficulty
            if len(covered_services) == 0:
                replacement_diff = 0.0
            else:
                ***REMOVED*** Average of 1/num_capable for each service
                difficulties = []
    ...
    def _calculate_basic(
        ...
        ***REMOVED*** Count assignments per faculty
        ...
        for f in faculty:
            ...
            ***REMOVED*** Basic degree centrality: services / max_services
            degree = len(covered_services) / max_services

            ***REMOVED*** Basic betweenness: unique services add betweenness
            betweenness = len(unique_services) * 0.3

            ***REMOVED*** Replacement difficulty
            if len(covered_services) == 0:
                replacement_diff = 0.0
            else:
                difficulties = []
            ...
            ***REMOVED*** Assignment-based influence
            ...
            centrality = FacultyCentrality(
                ...
                eigenvector_centrality=degree * 0.5,  ***REMOVED*** Simplified
                pagerank=pagerank,
                services_covered=len(covered_services),
                unique_services=len(unique_services),
    ...
    def create_hub_profile(
        ...
        ***REMOVED*** Identify backup faculty
        backup_faculty = []
        for sid, capable in services.items():
            if faculty_id in capable:
        ...
        ***REMOVED*** Cross-training candidates
        cross_training = []
        for skill in unique_skills:
            ***REMOVED*** Find faculty who could learn this skill (simplified)
        ...
        profile = HubProfile(
            ...
            bottleneck_periods=[],  ***REMOVED*** Would need schedule analysis
    ...
    def generate_cross_training_recommendations(
        ...
        for sid, capable in services.items():
            ...
            if len(capable) == 0:
                ***REMOVED*** No one can cover - urgent
    ...
    def get_distribution_report(
        ...
        ***REMOVED*** Hub concentration (Gini-like coefficient)
        ...
        ***REMOVED*** Service categorization
        single_provider = []
        dual_coverage = []
        well_covered = []
        ...
        ***REMOVED*** Generate cross-training priorities
