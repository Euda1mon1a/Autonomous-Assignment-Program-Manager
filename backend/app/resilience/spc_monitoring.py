...
class WorkloadControlChart:
        ...
        ***REMOVED*** Weekly hours for a resident over 8 weeks
        ...
        for alert in alerts:
            if alert.severity == "CRITICAL":
                ***REMOVED*** Immediate intervention required
    ...
    def __init__(
        ...
        ***REMOVED*** Pre-calculate control limits
        self.ucl_3sigma = target_hours + 3 * sigma  ***REMOVED*** Upper Control Limit (3σ)
        self.lcl_3sigma = target_hours - 3 * sigma  ***REMOVED*** Lower Control Limit (3σ)
        self.ucl_2sigma = target_hours + 2 * sigma  ***REMOVED*** Upper Warning Limit (2σ)
        self.lcl_2sigma = target_hours - 2 * sigma  ***REMOVED*** Lower Warning Limit (2σ)
        self.ucl_1sigma = target_hours + 1 * sigma  ***REMOVED*** Upper Zone A/B boundary
        self.lcl_1sigma = target_hours - 1 * sigma  ***REMOVED*** Lower Zone A/B boundary
    ...
    def detect_western_electric_violations(
        ...
        ***REMOVED*** Check each rule in order
    ...
    def _check_rule_2(
        ...
        if len(weekly_hours) < 3:
            return alerts  ***REMOVED*** Need at least 3 points
    ...
    def _check_rule_3(
        ...
        if len(weekly_hours) < 5:
            return alerts  ***REMOVED*** Need at least 5 points
    ...
    def _check_rule_4(
        ...
        if len(weekly_hours) < 8:
            return alerts  ***REMOVED*** Need at least 8 points

        for i in range(len(weekly_hours) - 7):
            window = weekly_hours[i:i+8]

            ***REMOVED*** Check if all points are above centerline
            ...
            ***REMOVED*** Check if all points are below centerline
...
def calculate_control_limits(data: list[float]) -> dict:
    ...
    sigma = statistics.stdev(data)  ***REMOVED*** Sample standard deviation (n-1)
...
def calculate_process_capability(
    ...
    if sigma == 0:
        ***REMOVED*** Perfect process with no variation
        if lsl <= mean <= usl:
            ***REMOVED*** Within spec
            return {
                "cp": float('inf'),
    ...
    ***REMOVED*** Cp: Potential capability (if perfectly centered)
    cp = (usl - lsl) / (6 * sigma)

    ***REMOVED*** Cpk: Actual capability (accounts for centering)
    cpu = (usl - mean) / (3 * sigma)  ***REMOVED*** Upper capability
    cpl = (mean - lsl) / (3 * sigma)  ***REMOVED*** Lower capability
    cpk = min(cpu, cpl)

    ***REMOVED*** Pp and Ppk: For this implementation, same as Cp/Cpk
    ***REMOVED*** (In practice, Pp/Ppk would use overall σ vs within-subgroup σ)
    pp = cp
    ppk = cpk
