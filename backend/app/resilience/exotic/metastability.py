...
class MetastableState:
    ...
    energy: float  ***REMOVED*** Current energy (lower = more stable)
    barrier_height: float  ***REMOVED*** Energy barrier to nearest stable state
    escape_rate: float  ***REMOVED*** Probability of spontaneous escape per time unit
    lifetime: float  ***REMOVED*** Expected lifetime in current state
    is_metastable: bool  ***REMOVED*** True if trapped in local minimum
    stability_score: float  ***REMOVED*** 0-1, higher = more stable
    nearest_stable_state: Optional[float] = None  ***REMOVED*** Energy of nearest true minimum
...
class MetastabilityDetector:
    ...
    def __init__(self, temperature: float = 1.0):
        ...
        self.boltzmann_constant = 1.0  ***REMOVED*** Normalized units

    def analyze_state(
        self,
        current_energy: float,
        energy_landscape: list[float],  ***REMOVED*** Energy at nearby states
        barrier_samples: list[float],  ***REMOVED*** Energy barriers to nearby states
    ) -> MetastableState:
        """
        ...
        ***REMOVED*** Find nearest lower-energy state
        lower_states = [e for e in energy_landscape if e < current_energy]

        if not lower_states:
            ***REMOVED*** Current state is global minimum (not metastable)
            return MetastableState(
                energy=current_energy,
        ...
        ***REMOVED*** Current state has lower-energy states available
        global_min = min(lower_states)
        energy_difference = current_energy - global_min

        ***REMOVED*** Find minimum barrier height to lower-energy state
        min_barrier = min(barrier_samples)

        ***REMOVED*** Calculate escape rate using Kramers theory
        ***REMOVED*** k_escape ≈ ω_0 * exp(-ΔE / kT)
        ***REMOVED*** where ΔE = barrier height, ω_0 = attempt frequency
        attempt_frequency = 1.0  ***REMOVED*** Normalized
        escape_rate = attempt_frequency * np.exp(
            -min_barrier / (self.boltzmann_constant * self.temperature)
        )

        ***REMOVED*** Expected lifetime: τ = 1 / k_escape
        lifetime = 1.0 / escape_rate if escape_rate > 0 else float("inf")

        ***REMOVED*** Determine if metastable
        ***REMOVED*** Criteria: barrier exists AND significantly lower energy state exists
        is_metastable = min_barrier > 0.1 and energy_difference > 0.05

        ***REMOVED*** Stability score: higher barrier = more stable metastable state
        stability_score = min(1.0, min_barrier / 10.0)  ***REMOVED*** Normalize to [0,1]
    ...
    def calculate_transition_probability(
        ...
        ***REMOVED*** Probability: P = 1 - exp(-k*t)
    ...
    def find_escape_paths(
        ...
        ***REMOVED*** Sample random perturbations
        for _ in range(num_paths * 10):  ***REMOVED*** Oversample then select best
            ...
            ***REMOVED*** Calculate barrier (maximum energy along linear path)
            ***REMOVED*** Simplified - linear interpolation
            barrier = max(
                energy_function(
        ...
        ***REMOVED*** Sort by energy change (prioritize downhill paths)
    ...
    def predict_reorganization_risk(
        ...
        """
        ***REMOVED*** Effective barrier reduced by perturbation
