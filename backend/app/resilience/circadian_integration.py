...
Usage in Solver:
    ...
    ***REMOVED*** During schedule optimization
...
***REMOVED*** Use try/except to handle both package and standalone imports
try:
    ...
    ***REMOVED*** Standalone import (for testing)
...
class CircadianObjective:
    ...
    def compute_penalty(self, schedule: list[dict]) -> float:
        ...
        Example:
            ...
            >>> ***REMOVED*** Solver minimizes: total_penalty = coverage_penalty + 0.2 * circadian_penalty
        """
        ***REMOVED*** Get quality score (0-1, higher = better)
        quality_score = self.analyzer.compute_circadian_quality_score(schedule)

        ***REMOVED*** Convert to penalty (0-1, lower = better)
    ...
    def evaluate_constraints(
        self,
        schedule: list[dict],
        quality_threshold: float = 0.55,  ***REMOVED*** FAIR level
        ...
        ***REMOVED*** Penalty for violations
        if satisfied:
            penalty = 0.0
        else:
            penalty = len(violations) * 10.0  ***REMOVED*** 10 penalty points per violation
...
***REMOVED*** =============================================================================
***REMOVED*** Schedule Analysis Helpers
***REMOVED*** =============================================================================
...
class CircadianScheduleOptimizer:
    ...
    def pre_solver_analysis(
        ...
        Example:
            ...
            >>> ***REMOVED*** Use report to set solver constraints
        """
        analysis = {
    ...
    def post_solver_validation(
        ...
        ***REMOVED*** Add recommendations for failed validation
        if not result.satisfied:
    ...
    def optimize_shift_timing(
        ...
        Example:
            ...
            >>> best_shift = ranked[0]  ***REMOVED*** Minimal circadian impact
        """
        if not candidate_shifts:
            return []

        ***REMOVED*** Score each candidate
        scored_shifts = []
        for shift in candidate_shifts:
            ***REMOVED*** Create mini-schedule with just this shift
            mini_schedule = [shift]
        ...
        ***REMOVED*** Sort by quality (descending)
        scored_shifts.sort(key=lambda x: x["quality_score"], reverse=True)

        ***REMOVED*** Return ranked shifts
...
def classify_shift_type(shift_start: datetime, duration: float) -> CircadianShiftType:
    ...
    Example:
        ...
        >>> print(shift_type)  ***REMOVED*** CircadianShiftType.DAY
    """
...
def compute_schedule_regularity(schedule: list[dict]) -> float:
    ...
    Example:
        ...
        >>> print(regularity)  ***REMOVED*** 0.85
    """
    if len(schedule) <= 1:
        return 1.0

    ***REMOVED*** Check shift type consistency
    shift_types = [
    ...
    ***REMOVED*** Regularity decreases with transitions
...
def get_circadian_recommendations(impact: CircadianImpact) -> list[str]:
    ...
    ***REMOVED*** Phase drift recommendations
    ...
    ***REMOVED*** Amplitude recommendations
    ...
    ***REMOVED*** Quality level recommendations
    if impact.quality_level == CircadianQualityLevel.CRITICAL:
        recommendations.append(
    ...
    ***REMOVED*** Misalignment recommendations
