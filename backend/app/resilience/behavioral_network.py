...
class BehavioralRole(str, Enum):
    ...
    NEUTRAL = "neutral"  ***REMOVED*** Normal swap behavior
    POWER_BROKER = "power_broker"  ***REMOVED*** Swaps always approved, others seek them out
    MARTYR = "martyr"  ***REMOVED*** Always absorbs bad shifts, rarely swaps away
    EVADER = "evader"  ***REMOVED*** Systematically avoids difficult shifts
    ISOLATE = "isolate"  ***REMOVED*** Never participates in swap network
    STABILIZER = "stabilizer"  ***REMOVED*** Flexible, helps balance the system


class BurdenCategory(str, Enum):
    """Categories of shift burden."""

    MINIMAL = "minimal"  ***REMOVED*** Admin, easy clinics
    LOW = "low"  ***REMOVED*** Standard daytime
    MODERATE = "moderate"  ***REMOVED*** Busy daytime, early AM
    HIGH = "high"  ***REMOVED*** Evening, weekend day
    SEVERE = "severe"  ***REMOVED*** Night, weekend night
    EXTREME = "extreme"  ***REMOVED*** Holiday, on-call, ICU night


class EquityStatus(str, Enum):
    """Status of burden equity for a person."""

    BALANCED = "balanced"  ***REMOVED*** Within 1 std dev of mean
    LIGHT = "light"  ***REMOVED*** >1 std dev below mean (carrying less)
    HEAVY = "heavy"  ***REMOVED*** >1 std dev above mean (carrying more)
    VERY_LIGHT = "very_light"  ***REMOVED*** >2 std dev below mean (possible evasion)
    CRUSHING = "crushing"  ***REMOVED*** >2 std dev above mean (burnout risk)


class ProtectionLevel(str, Enum):
    """Protection level for martyrs."""

    NONE = "none"  ***REMOVED*** No protection needed
    MONITORING = "monitoring"  ***REMOVED*** Watch for over-volunteering
    SOFT_LIMIT = "soft_limit"  ***REMOVED*** Warn when taking extra burden
    HARD_LIMIT = "hard_limit"  ***REMOVED*** Block additional burden absorption


***REMOVED*** =============================================================================
***REMOVED*** Burden Weighting
***REMOVED*** =============================================================================


***REMOVED*** Default burden weights (can be customized per institution)
DEFAULT_BURDEN_WEIGHTS = {
    ...
    ***REMOVED*** Shift type
    ...
    "conference": 0.4,
    "teaching": 0.6,
    ***REMOVED*** Combiners (multiplicative)
    "weekend_night": 3.5,
    "holiday_night": 4.0,
    "icu_night": 4.0,
}
...
class ShiftBurden:
    ...
    raw_hours: float
    burden_weight: float
    weighted_burden: float  ***REMOVED*** raw_hours * burden_weight
    category: BurdenCategory
    factors: list[str] = field(default_factory=list)  ***REMOVED*** What contributed to weight
...
class FacultyBurdenProfile:
    ...
    ***REMOVED*** Raw metrics
    ...
    total_shifts: int = 0
    shift_breakdown: dict[str, int] = field(default_factory=dict)  ***REMOVED*** type -> count

    ***REMOVED*** Burden metrics
    total_burden: float = 0.0
    burden_per_hour: float = 0.0  ***REMOVED*** efficiency of burden carrying
    high_burden_shifts: int = 0  ***REMOVED*** severe/extreme shifts

    ***REMOVED*** Equity assessment
    equity_status: EquityStatus = EquityStatus.BALANCED
    std_devs_from_mean: float = 0.0

    ***REMOVED*** Behavioral flags
    behavioral_role: BehavioralRole = BehavioralRole.NEUTRAL
    protection_level: ProtectionLevel = ProtectionLevel.NONE


***REMOVED*** =============================================================================
***REMOVED*** Swap Network Analysis
***REMOVED*** =============================================================================
...
class SwapEdge:
    ...
    target_id: UUID
    swap_count: int = 0
    source_initiated: int = 0  ***REMOVED*** Source asked target
    target_initiated: int = 0  ***REMOVED*** Target asked source
    burden_flow: float = 0.0  ***REMOVED*** Net burden transferred (+ = source gave to target)
    success_rate: float = 1.0  ***REMOVED*** What % of attempted swaps succeeded
...
class SwapNetworkNode:
    ...
    ***REMOVED*** Network metrics
    degree: int = 0  ***REMOVED*** Number of unique swap partners
    in_degree: int = 0  ***REMOVED*** Times asked to take swaps
    out_degree: int = 0  ***REMOVED*** Times initiated swap requests
    swap_count: int = 0  ***REMOVED*** Total swaps involved in

    ***REMOVED*** Burden flow (positive = net giver, negative = net receiver)
    net_burden_flow: float = 0.0
    burden_absorbed: float = 0.0  ***REMOVED*** Total burden taken from others
    burden_offloaded: float = 0.0  ***REMOVED*** Total burden given to others
    ...
    ***REMOVED*** Derived metrics
    approval_rate: float = 0.0  ***REMOVED*** How often their requests are granted
    acceptance_rate: float = 0.0  ***REMOVED*** How often they accept others' requests

    ***REMOVED*** Behavioral classification
    behavioral_role: BehavioralRole = BehavioralRole.NEUTRAL
...
class SwapNetworkAnalysis:
    ...
    ***REMOVED*** Network statistics
    network_density: float = 0.0  ***REMOVED*** Actual edges / possible edges
    average_degree: float = 0.0
    clustering_coefficient: float = 0.0

    ***REMOVED*** Role distributions
...
***REMOVED*** =============================================================================
***REMOVED*** Behavioral Network Analyzer
***REMOVED*** =============================================================================
...
class BehavioralNetworkAnalyzer:
    ...
    def __init__(
        ...
        martyr_threshold: float = 1.5,  ***REMOVED*** std devs above mean burden
        evader_threshold: float = -1.5,  ***REMOVED*** std devs below mean burden
    ...
    ***REMOVED*** -------------------------------------------------------------------------
    ...
    def calculate_shift_burden(
        ...
        ***REMOVED*** Base shift type weight
        ...
        ***REMOVED*** Calculate weighted burden
    ...
    def calculate_faculty_burden_profile(
        ...
        ***REMOVED*** Burden per hour (efficiency metric)
        if profile.total_hours > 0:
            profile.burden_per_hour = profile.total_burden / profile.total_hours

        ***REMOVED*** Equity assessment (if group data provided)
    ...
    ***REMOVED*** -------------------------------------------------------------------------
    ...
    def record_swap(
        ...
        ***REMOVED*** Calculate burden flow (positive = source gave more burden to target)
    ...
    def analyze_network(
        ...
        ***REMOVED*** Calculate network statistics
        n = len(self.nodes)
        ...
        ***REMOVED*** Classify behavioral roles
        self._classify_roles()

        ***REMOVED*** Populate analysis with classified roles
        for node in self.nodes.values():
        ...
        ***REMOVED*** Identify martyrs at burnout risk (high burden absorption + high swap count)
        for node in self.nodes.values():
        ...
        ***REMOVED*** Generate equity concerns
    ...
    def _classify_roles(self):
        ...
        ***REMOVED*** Calculate burden flow statistics
        burden_flows = [
        ...
        for node in self.nodes.values():
            ...
            ***REMOVED*** Identify martyrs based on heavy burden absorption even in small samples
            if burden_ratio >= 2:
                node.behavioral_role = BehavioralRole.MARTYR
                node.role_confidence = min(1.0, burden_ratio / 5)
                continue

            ***REMOVED*** Calculate z-score for burden flow
            z_score = (
                (node.net_burden_flow - mean_flow) / std_flow if std_flow > 0 else 0
            )

            ***REMOVED*** Classify based on burden flow and approval patterns
            if z_score >= self.martyr_threshold:
                ***REMOVED*** Absorbs significantly more burden than average
                node.behavioral_role = BehavioralRole.MARTYR
                node.role_confidence = min(1.0, z_score / 3)
            elif z_score <= self.evader_threshold:
                ***REMOVED*** Offloads significantly more burden than average
                ...
                ***REMOVED*** High approval rate + others seek them out
                ...
                ***REMOVED*** Balanced burden flow + helpful
    ...
    ***REMOVED*** -------------------------------------------------------------------------
    ...
    def get_martyr_protection_level(
        ...
        if node.behavioral_role != BehavioralRole.MARTYR:
            ***REMOVED*** Even non-martyrs might need protection at high stress
        ...
        ***REMOVED*** Martyr protection levels based on burden absorbed + stress
    ...
    ***REMOVED*** -------------------------------------------------------------------------
    ***REMOVED*** Burden Equity Analysis
    ***REMOVED*** -------------------------------------------------------------------------
    ...
    def analyze_burden_equity(
        ...
        ***REMOVED*** Gini coefficient (inequality measure, 0 = perfect equality, 1 = perfect inequality)
        ...
        ***REMOVED*** Check for hours vs burden discrepancy (the "forensic accounting" insight)
...
***REMOVED*** =============================================================================
***REMOVED*** Shadow Org Chart Service
***REMOVED*** =============================================================================
...
class ShadowOrgChartService:
    ...
    def generate_report(
        ...
        ***REMOVED*** Generate actionable recommendations
        recommendations = []
