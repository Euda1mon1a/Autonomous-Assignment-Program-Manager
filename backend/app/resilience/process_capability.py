...
class ScheduleCapabilityAnalyzer:
    ...
    Example:
        ...
        ***REMOVED*** Analyze weekly hours for ACGME compliance
    ...
    def calculate_cpk(
        ...
        if std_dev == 0:
            ...
            ***REMOVED*** If process is perfectly centered at target, it's infinitely capable
            ***REMOVED*** If outside specs, it's completely incapable
            if lsl <= mean <= usl:
                return float("inf")
            else:
                return 0.0
    ...
    def calculate_pp(
        ...
        Raises:
            ValueError: If data is empty or has insufficient variation
        """
        ***REMOVED*** For our use case (single time series), Pp = Cp
        ***REMOVED*** In manufacturing, Pp would use long-term σ vs short-term σ
    ...
    def calculate_ppk(
        ...
        Raises:
            ValueError: If data is empty or has insufficient variation
        """
        ***REMOVED*** For our use case (single time series), Ppk = Cpk
    ...
    def calculate_cpm(
        ...
        ***REMOVED*** Taguchi's loss function: penalize deviation from target
    ...
    def get_sigma_level(self, cpk: float) -> float:
        ...
        ***REMOVED*** Linear approximation: sigma_level = 3 * Cpk
        ***REMOVED*** This is accurate enough for practical purposes
        sigma_level = 3.0 * cpk
    ...
    def analyze_workload_capability(
        ...
        ***REMOVED*** Default target to midpoint if not specified
        if target_hours is None:
            target_hours = (min_hours + max_hours) / 2

        ***REMOVED*** Calculate all capability indices
    ...
    def get_capability_summary(self, report: ProcessCapabilityReport) -> dict:
        ...
        """
        ***REMOVED*** Calculate defect rate estimate (simplified)
        ***REMOVED*** For Cpk=1.0: ~2700 PPM (0.27%)
        ***REMOVED*** For Cpk=1.33: ~66 PPM (0.0066%)
        ***REMOVED*** For Cpk=2.0: ~0.002 PPM (near zero)
        if report.cpk > 0:
            ***REMOVED*** Rough approximation: defect_ppm ≈ 10^(6 - 2*sigma_level)
            estimated_defect_ppm = 10 ** (6 - 2 * report.sigma_level)
            estimated_defect_ppm = max(0.001, min(1_000_000, estimated_defect_ppm))
        else:
            estimated_defect_ppm = 1_000_000  ***REMOVED*** 100% if completely incapable
    ...
    def _get_capability_recommendations(
        ...
        ***REMOVED*** Centering recommendations
        if report.cp > 0:
            centering_ratio = report.cpk / report.cp
        ...
        ***REMOVED*** Taguchi recommendations
