...
class N2ScenarioConfig:
    ...
    zone_count: int = 6
    faculty_per_zone: int = 3  ***REMOVED*** With overlap, faculty can cover multiple zones
    minimum_zone_coverage: int = 2  ***REMOVED*** Minimum faculty needed per zone
...
class N2IterationResult:
    ...
    iteration: int
    faculty_pair_lost: tuple[int, int]  ***REMOVED*** Faculty indices
    passed: bool
    zones_failed: int
    zones_degraded: int  ***REMOVED*** Below minimum but not failed
    borrowing_attempts: int
    borrowing_successes: int
    recovery_time: Optional[float]
    failure_mode: Optional[str]  ***REMOVED*** "skill_gap", "capacity", "cascade", None if passed
...
class N2ScenarioResult:
    ...
    failures: int
    pass_rate: float
    failure_modes: dict[str, int] = field(default_factory=dict)  ***REMOVED*** mode -> count
    most_vulnerable_pairs: list[tuple[int, int]] = field(default_factory=list)  ***REMOVED*** Top 5 pairs
    average_recovery_time: float = 0.0
    cascade_rate: float = 0.0  ***REMOVED*** % of failures that cascaded
    zones_most_affected: dict[int, int] = field(default_factory=dict)  ***REMOVED*** zone_idx -> failure_count
...
class N2ContingencyScenario:
    ...
    def __init__(self, config: N2ScenarioConfig):
        self.config = config
        self._faculty: list[dict] = []  ***REMOVED*** Each: {id, zones, skills}
        self._zones: list[dict] = []  ***REMOVED*** Each: {id, faculty, min_coverage, required_skills}
    ...
    def setup(self) -> None:
        ...
        ***REMOVED*** Initialize faculty with zone assignments
        for faculty_id in range(self.config.faculty_count):
            ***REMOVED*** Each faculty member is assigned to 2-4 zones (with some randomness)
            ...
            ***REMOVED*** Update zone assignments
            for zone_id in assigned_zones:
                self._zones[zone_id]['faculty'].add(faculty_id)

        ***REMOVED*** Ensure each zone has at least minimum_zone_coverage + 1 faculty
        ***REMOVED*** (otherwise any 2 losses could trivially fail it)
        self._ensure_minimum_redundancy()

    def _generate_zone_skills(self, zone_id: int) -> set[str]:
        """Generate required skills for a zone."""
        ***REMOVED*** Different zones require different skill combinations
        skill_sets = [
            {'surgery', 'emergency'},
            {'pediatrics', 'general'},
            {'surgery', 'ob_gyn'},
    ...
    def _generate_faculty_skills(self, faculty_id: int) -> set[str]:
        ...
        ***REMOVED*** Each faculty has 2-3 skills
    ...
    def _ensure_minimum_redundancy(self) -> None:
        ...
        for zone in self._zones:
            while len(zone['faculty']) < target_per_zone:
                ***REMOVED*** Find faculty with fewest zone assignments
    ...
    def _check_zone_sufficiency(
        ...
        for zone in self._zones:
            ***REMOVED*** Count available faculty for this zone
            ...
            if available_count >= self.config.minimum_zone_coverage:
                ***REMOVED*** Check if skills are covered
                ...
                if zone['required_skills'].issubset(available_skills):
                    ...
                    ***REMOVED*** Enough people but missing critical skills
    ...
    def _attempt_borrowing(
        ...
        for zone_id in failed_zones:
            ...
            ***REMOVED*** Try to find faculty from other zones who have the needed skills
            needed_skills = zone['required_skills']

            for _ in range(shortfall):
                attempts += 1

                ***REMOVED*** Find faculty not assigned to this zone but with relevant skills
                candidates = []
    ...
    def _run_iteration(self, iteration: int) -> N2IterationResult:
        ...
        ***REMOVED*** Create available faculty set (all except the lost pair)
        available_faculty = set(range(self.config.faculty_count)) - set(faculty_pair)

        ***REMOVED*** Check zone sufficiency
        zone_status = self._check_zone_sufficiency(available_faculty)

        ***REMOVED*** Count failures and degradations
        ...
        ***REMOVED*** Attempt borrowing if there are failures
        borrowing_attempts = 0
        borrowing_successes = 0

        if failed_zones or degraded_zones:
            ***REMOVED*** Make a copy of available faculty for borrowing simulation
        ...
        ***REMOVED*** Determine if this iteration passed
        ***REMOVED*** After borrowing, re-check only if we had successes
        if borrowing_successes > 0:
            ***REMOVED*** Simple heuristic: each success covers one shortfall
            remaining_failures = max(0, len(failed_zones) - borrowing_successes)
            passed = remaining_failures == 0
        else:
            passed = len(failed_zones) == 0

        ***REMOVED*** Determine failure mode
        failure_mode = None
        recovery_time = None

        if not passed:
            ***REMOVED*** Analyze failure mode
            ...
            ***REMOVED*** System passed - estimate recovery time
    ...
    def run(self) -> N2ScenarioResult:
        ...
        """
        ***REMOVED*** Setup if not already done
        if not self._faculty:
            self.setup()

        ***REMOVED*** Run iterations
        self._results = []
        for i in range(self.config.iterations):
            result = self._run_iteration(i)
            self._results.append(result)

        ***REMOVED*** Aggregate and return results
    ...
    def _aggregate_results(self) -> N2ScenarioResult:
        ...
        ***REMOVED*** Collect failure modes
        failure_modes = Counter(
            r.failure_mode for r in self._results
            if r.failure_mode is not None
        )

        ***REMOVED*** Find most vulnerable pairs (pairs that caused failures)
        vulnerable_pairs = [
        ...
        ***REMOVED*** Calculate average recovery time for successful cases
        recovery_times = [
    ...
    def _generate_recommendations(
        ...
        ***REMOVED*** Overall pass rate recommendation
        passes = sum(1 for r in self._results if r.passed)
        pass_rate = passes / len(self._results) if self._results else 0.0
        ...
        ***REMOVED*** Skill gap recommendations
        ...
        ***REMOVED*** Cascade failure recommendations
        ...
        ***REMOVED*** Capacity recommendations
        ...
        ***REMOVED*** Vulnerable pair recommendations
        if vulnerable_pairs:
            recommendations.append(
        ...
        ***REMOVED*** Zone-specific recommendations
        if zones_affected:
        ...
        ***REMOVED*** Borrowing effectiveness
