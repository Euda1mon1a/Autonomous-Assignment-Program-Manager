...
class CircadianShiftType(str, Enum):
    ...
    DAY = "day"  ***REMOVED*** 7 AM - 3 PM: Morning light exposure (phase advance)
    EVENING = "evening"  ***REMOVED*** 3 PM - 11 PM: Evening light exposure (phase delay)
    NIGHT = "night"  ***REMOVED*** 11 PM - 7 AM: Night light exposure (strong phase delay)
    LONG_DAY = "long_day"  ***REMOVED*** 7 AM - 7 PM: Extended day exposure
    SPLIT = "split"  ***REMOVED*** Interrupted: High circadian disruption


class CircadianQualityLevel(str, Enum):
    """Circadian alignment quality categories."""

    EXCELLENT = "excellent"  ***REMOVED*** 0.85-1.0: Strong, well-aligned rhythm
    GOOD = "good"  ***REMOVED*** 0.70-0.84: Adequate alignment
    FAIR = "fair"  ***REMOVED*** 0.55-0.69: Some misalignment
    POOR = "poor"  ***REMOVED*** 0.40-0.54: Significant misalignment
    CRITICAL = "critical"  ***REMOVED*** 0.0-0.39: Severe disruption


***REMOVED*** Circadian period constants
NATURAL_PERIOD_HOURS = 24.2  ***REMOVED*** Free-running period without zeitgebers
ENTRAINED_PERIOD_HOURS = 24.0  ***REMOVED*** Period when entrained to light/dark cycle
MIN_AMPLITUDE = 0.0  ***REMOVED*** Complete circadian rhythm collapse
MAX_AMPLITUDE = 1.0  ***REMOVED*** Perfect circadian rhythm strength

***REMOVED*** Phase Response Curve parameters (in hours of phase shift per hour of stimulus)
PRC_ADVANCE_MAX = 1.5  ***REMOVED*** Maximum advance from morning light (hours)
PRC_DELAY_MAX = -2.5  ***REMOVED*** Maximum delay from evening/night light (hours)
PRC_DEAD_ZONE_START = 12  ***REMOVED*** Hour when PRC effect minimal (noon)
PRC_DEAD_ZONE_END = 16  ***REMOVED*** Hour when PRC effect resumes (4 PM)

***REMOVED*** Amplitude decay/recovery rates
AMPLITUDE_DECAY_RATE = 0.05  ***REMOVED*** Per irregular shift (5% amplitude loss)
AMPLITUDE_RECOVERY_RATE = 0.02  ***REMOVED*** Per regular shift (2% amplitude gain)
AMPLITUDE_RECOVERY_DAYS = 14  ***REMOVED*** Days of regular schedule for full recovery
...
class CircadianOscillator:
    ...
    resident_id: UUID
    phase: float = 0.0  ***REMOVED*** Hours (0-24)
    amplitude: float = 1.0  ***REMOVED*** 0-1
    period: float = NATURAL_PERIOD_HOURS
    last_updated: datetime = field(default_factory=datetime.now)
    chronotype_offset: float = 0.0  ***REMOVED*** Hours (-2 to +2)
    entrainment_strength: float = 1.0  ***REMOVED*** 0-1

    def __post_init__(self):
        """Validate oscillator parameters."""
        self.phase = self.phase % 24.0  ***REMOVED*** Keep phase in [0, 24)
    ...
    def update_phase(self, time_elapsed: timedelta, phase_shift: float = 0.0):
        ...
        ***REMOVED*** Natural progression at intrinsic period
        natural_advance = hours_elapsed * (24.0 / self.period)

        ***REMOVED*** Apply external phase shift (from PRC)
        total_shift = natural_advance + phase_shift

        ***REMOVED*** Update phase
    ...
    def update_amplitude(self, schedule_regularity: float):
        ...
        Example:
            ...
            >>> osc.update_amplitude(0.3)  ***REMOVED*** Irregular schedule
            >>> print(osc.amplitude)  ***REMOVED*** Decreased (e.g., 0.76)
        """
        if schedule_regularity >= 0.8:
            ***REMOVED*** Regular schedule: amplitude recovery
            delta = AMPLITUDE_RECOVERY_RATE * (schedule_regularity - 0.8) / 0.2
            self.amplitude = min(MAX_AMPLITUDE, self.amplitude + delta)
        else:
            ***REMOVED*** Irregular schedule: amplitude decay
    ...
    def get_current_alertness(self, current_time: datetime) -> float:
        ...
        Example:
            ...
            ...     datetime(2024, 1, 1, 10, 0)  ***REMOVED*** 10 AM (4h after wake at 6 AM)
            ... )
            >>> print(alertness)  ***REMOVED*** High (e.g., 0.85)
        """
        ***REMOVED*** Current clock time
        current_hour = current_time.hour + current_time.minute / 60

        ***REMOVED*** Phase represents wake time (e.g., phase=6.0 means wake at 6 AM)
        ***REMOVED*** Peak alertness occurs ~4 hours after wake
        ***REMOVED*** Trough occurs ~22 hours after wake (2 hours before next wake)
        peak_time = (self.phase + 4.0) % 24.0

        ***REMOVED*** Calculate hour offset from peak
        hour_offset = current_hour - peak_time

        ***REMOVED*** Normalize to [-12, 12] range
        while hour_offset > 12:
            hour_offset -= 24
        while hour_offset < -12:
            hour_offset += 24

        ***REMOVED*** Cosine curve: peak at 0, trough at ±12
        ***REMOVED*** cos(0) = 1 (peak), cos(π) = -1 (trough)
        base_alertness = 0.5 + 0.5 * math.cos(2 * math.pi * hour_offset / 24.0)

        ***REMOVED*** Apply amplitude modulation
        ***REMOVED*** Low amplitude = flattened curve (less variation)
        ***REMOVED*** High amplitude = full variation
    ...
    def _get_prc_value(self, hour: float) -> float:
        ...
        """
        ***REMOVED*** Normalize to [0, 24)
        hour = hour % 24.0

        ***REMOVED*** Dead zone: minimal PRC effect
        if PRC_DEAD_ZONE_START <= hour < PRC_DEAD_ZONE_END:
            return 0.0

        ***REMOVED*** Morning advance (6-10 AM): positive PRC
        if 6 <= hour < 10:
            ***REMOVED*** Sinusoidal rise to peak
            t = (hour - 6) / 4  ***REMOVED*** Normalize to [0, 1]
            return PRC_ADVANCE_MAX * math.sin(math.pi * t)

        ***REMOVED*** Evening/night delay (8 PM - 2 AM): negative PRC
        if hour >= 20 or hour < 2:
            ***REMOVED*** Peak delay at midnight
            if hour >= 20:
                t = (hour - 20) / 4  ***REMOVED*** 8 PM to midnight
            else:
                t = (hour + 4) / 4  ***REMOVED*** Midnight to 2 AM (continues curve)
            return PRC_DELAY_MAX * math.sin(math.pi * t)

        ***REMOVED*** Transition zones: linear interpolation
        if 2 <= hour < 6:
            ***REMOVED*** Delay → advance transition
            t = (hour - 2) / 4
            return PRC_DELAY_MAX * (1 - t)  ***REMOVED*** Decay to zero
        elif 10 <= hour < 12:
            ***REMOVED*** Advance → dead zone transition
            t = (hour - 10) / 2
            return PRC_ADVANCE_MAX * (1 - t)
        elif 16 <= hour < 20:
            ***REMOVED*** Dead zone → delay transition
            t = (hour - 16) / 4
            return PRC_DELAY_MAX * t  ***REMOVED*** Rise from zero
...
class CircadianImpact:
    ...
    resident_id: UUID
    phase_drift: float  ***REMOVED*** Hours
    amplitude_change: float  ***REMOVED*** -1 to +1
    quality_score: float  ***REMOVED*** 0-1
    misalignment_hours: float  ***REMOVED*** Hours per week
...
***REMOVED*** =============================================================================
***REMOVED*** Circadian Schedule Analyzer
***REMOVED*** =============================================================================
...
class CircadianScheduleAnalyzer:
    ...
    def __init__(self):
        ...
        ***REMOVED*** Cache of oscillator states per resident
    ...
    def get_or_create_oscillator(
        self,
        resident_id: UUID,
        initial_phase: float = 6.0,  ***REMOVED*** Default: wake at 6 AM
    ...
    def analyze_schedule_impact(
        self,
        resident_id: UUID,
        schedule: list[dict],  ***REMOVED*** List of shifts with start time, duration, type
    ) -> CircadianImpact:
        """
        ...
        ***REMOVED*** Simulate schedule
        ...
        for i, shift in enumerate(schedule):
            ...
            ***REMOVED*** Calculate phase shift from this shift
            phase_shift = osc.compute_phase_shift(start_time, duration, shift_type)

            ***REMOVED*** Update oscillator phase
            if i > 0:
            ...
            ***REMOVED*** Calculate misalignment for this shift
            optimal_phase = 10.0  ***REMOVED*** Optimal wake time: 10 AM circadian phase
            misalignment = abs((osc.phase - optimal_phase + 12) % 24 - 12)
            total_misalignment += misalignment

            ***REMOVED*** Track shift regularity (for amplitude calculation)
            if i > 0:
    ...
    def compute_circadian_quality_score(self, schedule: list[dict]) -> float:
        ...
        Example:
            ...
            >>> print(score)  ***REMOVED*** 0.82
        """
        if not schedule:
            return 1.0

        ***REMOVED*** Group shifts by resident
        ...
        ***REMOVED*** Analyze each resident's schedule
        quality_scores = []
        ...
        ***REMOVED*** Average across all residents
    ...
    def predict_burnout_risk_from_circadian(self, amplitude: float) -> float:
        ...
        Example:
            ...
            >>> print(risk)  ***REMOVED*** 0.6 (elevated risk)
        """
        ***REMOVED*** Inverse relationship: lower amplitude = higher risk
        ***REMOVED*** Sigmoid curve for smooth transition
        risk = 1.0 - amplitude

        ***REMOVED*** Apply sigmoid for more realistic risk curve
        ***REMOVED*** Risk accelerates below amplitude 0.5
        if amplitude < 0.5:
    ...
    def _compute_quality_score(
        ...
        Returns:
            Quality score (0-1)
        """
        ***REMOVED*** Phase drift penalty (0-1, 0 = large drift)
        phase_score = max(0, 1.0 - abs(phase_drift) / 6.0)  ***REMOVED*** 6h drift = score 0

        ***REMOVED*** Amplitude penalty (0-1, 0 = large degradation)
        amp_score = 1.0 if amplitude_change >= 0 else max(0, 1.0 + amplitude_change)

        ***REMOVED*** Misalignment penalty (0-1, 0 = large misalignment)
        misalign_score = max(0, 1.0 - avg_misalignment / 6.0)  ***REMOVED*** 6h misalign = score 0
    ...
    def _estimate_recovery_days(
        ...
        """
        ***REMOVED*** Phase recovery: ~1 day per hour of drift
        phase_recovery_days = abs_phase_drift

        ***REMOVED*** Amplitude recovery: up to 14 days for full recovery
        if amplitude_change < 0:
            amp_recovery_days = abs(amplitude_change) * AMPLITUDE_RECOVERY_DAYS
        else:
            amp_recovery_days = 0

        ***REMOVED*** Take maximum (recovery processes can overlap partially)
