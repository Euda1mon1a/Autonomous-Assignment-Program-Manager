...
Example:
    ...
    ***REMOVED*** During solver run:
    ...
    if detector.detect_plateau(trajectory):
        ...
        ***REMOVED*** Apply strategy: restart with new seed, increase temperature, etc.
"""
...
class EscapeStrategy(str, Enum):
    ...
    CONTINUE_SEARCH = "continue_search"  ***REMOVED*** Not metastable, keep going
    INCREASE_TEMPERATURE = "increase_temperature"  ***REMOVED*** More randomness
    BASIN_HOPPING = "basin_hopping"  ***REMOVED*** Jump to random state, local search
    RESTART_NEW_SEED = "restart_new_seed"  ***REMOVED*** Complete restart with new seed
    ACCEPT_LOCAL_OPTIMUM = "accept_local_optimum"  ***REMOVED*** Barrier too high, accept current
...
class MetastabilityDetector:
    ...
    Example:
        ...
        ***REMOVED*** Track solver progress
        ...
        for iteration, state in enumerate(solver.iterate()):
            ...
            ***REMOVED*** Check for metastability every 50 iterations
            if iteration % 50 == 0:
    ...
    def detect_plateau(
        ...
        Example:
            ...
            ***REMOVED*** True - last 5 values vary < 1% around 85
        """
        if window is None:
        ...
        ***REMOVED*** Get recent window
        recent = solver_trajectory[-window:]

        ***REMOVED*** Check if all values are identical (perfect plateau)
        if len(set(recent)) == 1:
    ...
    def estimate_barrier_height(
        ...
        """
        ***REMOVED*** Heuristic estimation based on state properties
        ***REMOVED*** In reality, this would sample neighbor solutions

        ***REMOVED*** If infeasible, barrier is lower (many directions to explore)
        if current_state.constraint_violations > 0:
            barrier = 1.0 + (current_state.constraint_violations * 0.1)
        else:
            ***REMOVED*** Feasible solution - assume moderate barrier
            ***REMOVED*** In practice, would sample nearby solutions
            barrier = 5.0

        ***REMOVED*** Normalize by objective scale
    ...
    def compute_escape_probability(
        ...
        Example:
            ***REMOVED*** High barrier, low temperature = low escape probability
            prob = detector.compute_escape_probability(barrier=10.0, temperature=1.0)
            ***REMOVED*** prob ≈ 0.000045 (very unlikely to escape)

            ***REMOVED*** High barrier, high temperature = higher escape probability
            prob = detector.compute_escape_probability(barrier=10.0, temperature=5.0)
            ***REMOVED*** prob ≈ 0.135 (more likely to escape)
        """
        ...
        ***REMOVED*** Boltzmann factor: exp(-E/kT)
        ***REMOVED*** Using k=1 for simplicity (dimensionless units)
        try:
            probability = math.exp(-barrier_height / temperature)
        except OverflowError:
            ***REMOVED*** Barrier too high
            probability = 0.0
    ...
    def recommend_escape_strategy(
        ...
        ***REMOVED*** If infeasible, never accept - must keep searching
        if violations > 0:
            if barrier < 5.0:
            ...
            ***REMOVED*** Feasible solution
            if escape_prob > 0.2:
                ***REMOVED*** High escape probability - continue current search
                strategy = EscapeStrategy.CONTINUE_SEARCH
            elif barrier < 5.0:
                ***REMOVED*** Low-medium barrier - increase randomness
                strategy = EscapeStrategy.INCREASE_TEMPERATURE
            elif barrier < 15.0:
                ***REMOVED*** Medium-high barrier - try basin hopping
                strategy = EscapeStrategy.BASIN_HOPPING
            elif barrier < 25.0:
                ***REMOVED*** High barrier - restart needed
                strategy = EscapeStrategy.RESTART_NEW_SEED
            else:
                ***REMOVED*** Extreme barrier - may be global optimum
    ...
    def analyze_solver_trajectory(
        ...
        if len(trajectory) < self.plateau_window:
            ***REMOVED*** Not enough data for analysis
        ...
        ***REMOVED*** Find best solution
        best_idx = np.argmin(objectives)
        best_objective = objectives[best_idx]

        ***REMOVED*** Check for plateau
        plateau_detected = self.detect_plateau(objectives)

        ***REMOVED*** Calculate stagnation duration (iterations since best)
        stagnation = len(trajectory) - best_idx - 1

        ***REMOVED*** Get current state
        current_state = trajectory[-1]

        ***REMOVED*** Estimate barrier height
        barrier_height = self.estimate_barrier_height(current_state)

        ***REMOVED*** Compute escape probability
        escape_probability = self.compute_escape_probability(
            barrier_height, current_state.temperature
        )

        ***REMOVED*** Estimate basin size (number of solutions with similar objective)
        ***REMOVED*** Count how many recent solutions are within 1% of current
        ...
        ***REMOVED*** Determine if metastable
        is_metastable = (
            plateau_detected
            and stagnation >= self.min_stagnation
            and current_state.constraint_violations == 0  ***REMOVED*** Only feasible states
        )

        ***REMOVED*** Create metastable state
        metastable_state = None
