***REMOVED*** Statistical Process Control (SPC) Workload Monitoring
...
***REMOVED******REMOVED*** Western Electric Rules
...
***REMOVED******REMOVED******REMOVED*** Rule 2: Two of Three Consecutive Points Beyond 2σ (WARNING)
...
***REMOVED******REMOVED******REMOVED*** Rule 3: Four of Five Consecutive Points Beyond 1σ (WARNING)
...
***REMOVED******REMOVED******REMOVED*** Rule 4: Eight Consecutive Points on Same Side of Centerline (INFO)
...
***REMOVED*** Create control chart with target=60h, sigma=5h
...
***REMOVED******REMOVED******REMOVED*** Calculate Control Limits from Historical Data

```python
from app.resilience.spc_monitoring import calculate_control_limits

***REMOVED*** Historical weekly hours data
historical_data = [58, 62, 59, 61, 63, 60, 58, 62, 64, 57]

***REMOVED*** Calculate empirical control limits
...
***REMOVED*** Use empirical limits for monitoring
...
***REMOVED******REMOVED******REMOVED*** Calculate Process Capability

```python
from app.resilience.spc_monitoring import calculate_process_capability

***REMOVED*** Weekly hours data
data = [58, 62, 59, 61, 63, 60, 58, 62]

***REMOVED*** ACGME limits: 0-80 hours/week
capability = calculate_process_capability(
    data=data,
    lsl=0,   ***REMOVED*** Lower spec limit
    usl=80,  ***REMOVED*** Upper spec limit (ACGME)
)

print(f"Cpk: {capability['cpk']:.2f}")
print(f"Interpretation: {capability['interpretation']}")

***REMOVED*** Cpk >= 1.33 indicates capable process (4-sigma quality)
if capability['cpk'] < 1.0:
    print("WARNING: Process not capable - violations likely")
```

***REMOVED******REMOVED*** Integration with Existing Process Capability Module
...
***REMOVED*** Real-time monitoring with Western Electric Rules
...
***REMOVED*** Overall process quality assessment with Six Sigma
...
***REMOVED******REMOVED*** Alert Structure
...
class SPCAlert:
    ...
    resident_id: Optional[UUID]    ***REMOVED*** Affected resident (or None for system-wide)
...
***REMOVED******REMOVED*** Control Chart Parameters

***REMOVED******REMOVED******REMOVED*** Default Parameters (Recommended)

```python
WorkloadControlChart(
    target_hours=60.0,  ***REMOVED*** Target weekly hours
...
***REMOVED******REMOVED******REMOVED*** Custom Parameters

Adjust based on your program's specific characteristics:

```python
***REMOVED*** More aggressive monitoring (tighter tolerances)
strict_chart = WorkloadControlChart(target_hours=55, sigma=4)

***REMOVED*** More lenient (wider tolerances)
lenient_chart = WorkloadControlChart(target_hours=65, sigma=6)

***REMOVED*** Empirical (learn from historical data)
...
***REMOVED******REMOVED*** Typical Scenarios

***REMOVED******REMOVED******REMOVED*** Scenario 1: Normal Rotation Variation
...
***REMOVED*** Expected: No CRITICAL or WARNING alerts (may have INFO if trending)
```

***REMOVED******REMOVED******REMOVED*** Scenario 2: ACGME Violation Pattern

```python
***REMOVED*** Gradual increase leading to violation
weekly_hours = [60, 65, 68, 72, 75, 78, 80, 82]
alerts = chart.detect_western_electric_violations(resident_id, weekly_hours)

***REMOVED*** Expected:
***REMOVED*** - Multiple Rule 1 CRITICAL alerts (78, 80, 82 > 75)
***REMOVED*** - Rule 2 WARNING alert (shift detected)
***REMOVED*** - Rule 3 WARNING alert (trend detected)
```

***REMOVED******REMOVED******REMOVED*** Scenario 3: Burnout Risk Pattern

```python
***REMOVED*** Sustained high hours (not violating, but concerning)
weekly_hours = [66, 67, 68, 67, 66, 68, 67, 66]
alerts = chart.detect_western_electric_violations(resident_id, weekly_hours)

***REMOVED*** Expected:
***REMOVED*** - Rule 4 INFO alert (8 consecutive above centerline)
***REMOVED*** - Action: Investigate why consistently high, consider rotation adjustment
```

***REMOVED******REMOVED******REMOVED*** Scenario 4: Scheduling Gap

```python
***REMOVED*** Sudden drop suggesting gap in coverage
weekly_hours = [60, 62, 59, 20, 61]
alerts = chart.detect_western_electric_violations(resident_id, weekly_hours)

***REMOVED*** Expected:
***REMOVED*** - Rule 1 CRITICAL alert (20 < 45 LCL)
***REMOVED*** - Action: Investigate scheduling gap or data error
```

***REMOVED******REMOVED*** Statistical Background

***REMOVED******REMOVED******REMOVED*** Control Chart Theory
...
***REMOVED******REMOVED******REMOVED*** Process Capability Indices
