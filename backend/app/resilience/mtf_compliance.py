...
class DRRSTranslator:
    ...
    ***REMOVED*** Map EquilibriumState to Mission Capability
    ...
    ***REMOVED*** Military Occupational Specialty codes for medical personnel
    MOS_DESCRIPTIONS = {
    ...
    def translate_personnel_strength(
        ...
        ***REMOVED*** Effective availability reduces by overloaded count (they're not really available)
    ...
    ***REMOVED*** Operations always allowed (patient safety)
    ...
    def check_and_trip(
        ...
        if self.state == CircuitBreakerState.OPEN and not self._override_active():
            ***REMOVED*** Already tripped and no override
            return False, self.trigger, self.trigger_details

        trigger = None
        details = None

        ***REMOVED*** Check each threshold
        if not n1_pass:
        ...
        ***REMOVED*** Check if we can close from half-open
