...
class WarningLevel(str, Enum):
    ...
    GREEN = "green"  ***REMOVED*** Healthy - no warning signals
    YELLOW = "yellow"  ***REMOVED*** Single warning signal detected
    ORANGE = "orange"  ***REMOVED*** Two warning signals detected
    RED = "red"  ***REMOVED*** All three signals detected - critical
    UNKNOWN = "unknown"  ***REMOVED*** Insufficient data
...
class CriticalSlowingDownResult:
    ...
    ***REMOVED*** Analysis metadata
    id: UUID
    calculated_at: datetime
    days_analyzed: int
    data_quality: str  ***REMOVED*** "excellent", "good", "fair", "poor"

    ***REMOVED*** Warning status
    is_critical: bool  ***REMOVED*** Approaching critical point?
    warning_level: WarningLevel
    confidence: float  ***REMOVED*** 0.0 to 1.0

    ***REMOVED*** Early warning signals
    relaxation_time_hours: float | None  ***REMOVED*** Current τ (target: < 48)
    relaxation_time_baseline: float | None  ***REMOVED*** Historical baseline
    relaxation_time_increasing: bool

    variance_current: float | None  ***REMOVED*** Current variance
    variance_baseline: float | None  ***REMOVED*** Historical baseline
    variance_slope: float | None  ***REMOVED*** Trend slope (target: < 0.1)
    variance_increasing: bool

    autocorrelation_ac1: float | None  ***REMOVED*** Lag-1 autocorrelation (target: < 0.7)
    autocorrelation_baseline: float | None
    autocorrelation_increasing: bool

    ***REMOVED*** Risk assessment
    signals_triggered: int  ***REMOVED*** Count of warning signals (0-3)
    estimated_days_to_critical: int | None  ***REMOVED*** Projected time to critical point
    avalanche_risk_score: float  ***REMOVED*** 0.0 to 1.0 composite risk

    ***REMOVED*** Actionable insights
...
class SOCAvalanchePredictor:
    ...
    async def detect_critical_slowing_down(
        ...
        Example:
            ...
            >>> utilization = [0.75, 0.76, 0.78, ...]  ***REMOVED*** 60 days
        ...
        ***REMOVED*** Limit to lookback window
        data = utilization_history[-days_lookback:]
        data_points = len(data)

        ***REMOVED*** Assess data quality
        ...
        ***REMOVED*** Determine warning level
        if signals_triggered == 0:
            ...
            confidence = 0.85
        else:  ***REMOVED*** 3 signals
            warning_level = WarningLevel.RED
            confidence = 0.95

        ***REMOVED*** Calculate composite risk score
        ...
        ***REMOVED*** Estimate days to critical
    ...
    def _calculate_relaxation_time(
        ...
        try:
            ***REMOVED*** Detect perturbations (large deviations from rolling mean)
            ...
            ***REMOVED*** Find perturbation events
            perturbations = []
            for i in range(len(deviations) - 10):  ***REMOVED*** Need at least 10 points to recover
                if deviations[i] > threshold:
                    ***REMOVED*** Measure recovery time
                    recovery_time = 0
                    for j in range(i + 1, min(i + 20, len(deviations))):
                        if deviations[j] < threshold * 0.5:  ***REMOVED*** Recovered
                            recovery_time = j - i
                            break
                    if recovery_time > 0:
            ...
            ***REMOVED*** Recent vs baseline relaxation time
            split = len(perturbations) // 2
            baseline_tau = statistics.mean(perturbations[:split]) * 24  ***REMOVED*** Convert to hours
            recent_tau = statistics.mean(perturbations[split:]) * 24

            is_increasing = recent_tau > baseline_tau * 1.2  ***REMOVED*** 20% increase
    ...
    def _calculate_autocorrelation(
        ...
        try:
            ***REMOVED*** Split into baseline and recent
            split = len(data) // 2
            baseline_data = data[:split]
            recent_data = data[split:]

            ***REMOVED*** Calculate AC1 for baseline
            baseline_ac1 = np.corrcoef(baseline_data[:-1], baseline_data[1:])[0, 1]

            ***REMOVED*** Calculate AC1 for recent
            current_ac1 = np.corrcoef(recent_data[:-1], recent_data[1:])[0, 1]

            is_increasing = current_ac1 > baseline_ac1 * 1.1  ***REMOVED*** 10% increase
    ...
    def _calculate_avalanche_risk(
        ...
        ***REMOVED*** AC1 risk
        if ac1 is not None:
            ac1_risk = min(1.0, ac1 / 1.0)  ***REMOVED*** AC1 maxes at 1.0
        ...
        ***REMOVED*** Amplify if multiple signals triggered
