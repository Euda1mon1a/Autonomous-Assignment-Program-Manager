...
class DangerClass(str, Enum):
    ...
    LOW = "low"  ***REMOVED*** FWI < 20: Normal operations
    MODERATE = "moderate"  ***REMOVED*** FWI 20-40: Monitor closely
    HIGH = "high"  ***REMOVED*** FWI 40-60: Reduce workload
    VERY_HIGH = "very_high"  ***REMOVED*** FWI 60-80: Significant restrictions
    EXTREME = "extreme"  ***REMOVED*** FWI 80+: Emergency measures
...
class BurnoutCodeReport:
    ...
    ffmc: float  ***REMOVED*** Fine Fuel Moisture Code (0-100): recent hours
    dmc: float  ***REMOVED*** Duff Moisture Code (0-100): monthly load
    dc: float  ***REMOVED*** Drought Code (0-100): yearly satisfaction
    isi: float  ***REMOVED*** Initial Spread Index (0-100+): spread rate
    bui: float  ***REMOVED*** Buildup Index (0-100+): combined burden
    fwi: float  ***REMOVED*** Fire Weather Index (0-100+): final danger
...
class BurnoutDangerRating:
    ...
    Usage:
        ...
        report = rating.calculate_burnout_danger(
            resident_id=uuid4(),
            recent_hours=65.0,      ***REMOVED*** Last 2 weeks
            monthly_load=250.0,      ***REMOVED*** Last 3 months avg
            yearly_satisfaction=0.6, ***REMOVED*** Job satisfaction (0-1)
            workload_velocity=5.0,   ***REMOVED*** Hours/week increase
        )
    ...
    def __init__(
        self,
        ffmc_target: float = 60.0,  ***REMOVED*** Target 60 hours/2 weeks
        dmc_target: float = 240.0,  ***REMOVED*** Target 240 hours/3 months
        dc_baseline: float = 1.0,  ***REMOVED*** Full satisfaction baseline
    ):
        """
    ...
    def calculate_fine_fuel_moisture_code(
        ...
        ***REMOVED*** Calculate excess workload as proportion of target
        excess = max(0, (recent_hours - target) / target)

        ***REMOVED*** Exponential scaling (FFMC rises quickly with excess hours)
        ***REMOVED*** k = 3.5 tuned so 75 hours (25% over 60h target) gives FFMC ~70
        ***REMOVED*** This makes the system more sensitive to recent overwork
        k = 3.5
    ...
    def calculate_duff_moisture_code(
        ...
        ***REMOVED*** Calculate sustained excess
        excess = max(0, (monthly_load - target) / target)

        ***REMOVED*** Lower exponential constant (duff dries slower than fine fuels)
        ***REMOVED*** k = 2.5 tuned so 270h monthly (12.5% over 240h) gives DMC ~50
        ***REMOVED*** More sensitive to sustained overwork
        k = 2.5
    ...
    def calculate_drought_code(
        ...
        ***REMOVED*** Invert satisfaction (1.0 satisfaction = 0 drought)
        dissatisfaction = 1.0 - yearly_satisfaction

        ***REMOVED*** Exponential to emphasize dissatisfaction
        ***REMOVED*** 0.7 satisfaction → 0.3 dissatisfaction → ~40 DC
        ***REMOVED*** 0.5 satisfaction → 0.5 dissatisfaction → ~60 DC
        ***REMOVED*** 0.0 satisfaction → 1.0 dissatisfaction → 100 DC
        ***REMOVED*** Use exponential curve for more sensitivity
    ...
    def calculate_initial_spread_index(
        ...
        """
        ***REMOVED*** Velocity effect (negative velocity reduces spread)
        velocity_factor = max(0, 1.0 + (workload_velocity / 10.0))

        ***REMOVED*** Van Wagner's ISI formula (adapted)
        ***REMOVED*** Original: ISI = 0.208 * f(W) * f(FFMC)
        ***REMOVED*** We combine FFMC and velocity directly
        ***REMOVED*** Increased coefficient for more sensitivity
    ...
    def calculate_buildup_index(
        ...
        ***REMOVED*** Van Wagner's BUI formula (adapted)
        ***REMOVED*** Adjusted coefficients for burnout context
    ...
    def calculate_fire_weather_index(
        ...
        Returns:
            ...
            - 80+: EXTREME danger
        """
        ***REMOVED*** Calculate fuel effect (fD) based on BUI
        if bui <= 80:
            ***REMOVED*** Low to moderate BUI: power law
            fD = (0.626 * (bui**0.809)) + 2.0
        else:
            ***REMOVED*** High BUI: exponential saturation
            fD = 1000.0 / (25.0 + 108.64 * (2.71828 ** (-0.023 * bui)))

        ***REMOVED*** Final FWI calculation (calibrated for burnout context)
        ***REMOVED*** Scale factor to keep FWI in 0-100+ range with reasonable distribution
        fwi = 0.12 * isi * fD
    ...
    def calculate_burnout_danger(
        ...
        Example:
            ...
            ***REMOVED*** Resident working 75h recently, 260h/month average,
            ***REMOVED*** satisfaction at 0.4, and workload increasing 8h/week
            report = rating.calculate_burnout_danger(
                resident_id=uuid4(),
                recent_hours=75.0,
                monthly_load=260.0,
            ...
            ***REMOVED*** Expected: EXTREME danger
            ***REMOVED*** - FFMC high (recent overwork)
            ***REMOVED*** - DMC high (sustained overwork)
            ***REMOVED*** - DC high (low satisfaction)
            ***REMOVED*** - ISI very high (accelerating)
            ***REMOVED*** - BUI very high (accumulated burden)
            ***REMOVED*** - FWI 80+ (EXTREME)
        """
        ***REMOVED*** Calculate all components
    ...
    def calculate_batch_danger(
        ...
        ***REMOVED*** Sort by FWI (highest risk first)
