...
class DefenseMetrics:
    ...
    utilization: float  ***REMOVED*** Current utilization ratio (0.0-1.0+)
    n1_failures: int  ***REMOVED*** Number of N-1 failure scenarios
    n2_failures: int  ***REMOVED*** Number of N-2 failure scenarios
    coverage_gaps: int  ***REMOVED*** Number of coverage gaps
    burnout_cases: int  ***REMOVED*** Number of residents in burnout state
    cascade_risk: float  ***REMOVED*** Cascade failure risk (0.0-1.0)
    recovery_time: float  ***REMOVED*** Estimated recovery time (hours)
...
class DefenseLevelCalculator:
    ...
    ***REMOVED*** Utilization thresholds from queuing theory (M/M/c queue)
    UTILIZATION_GREEN = 0.80  ***REMOVED*** Below this = green
    UTILIZATION_YELLOW = 0.90  ***REMOVED*** 80-90% = yellow
    UTILIZATION_ORANGE = 0.95  ***REMOVED*** 90-95% = orange
    UTILIZATION_RED = 0.98  ***REMOVED*** 95-98% = red
    ***REMOVED*** Above 98% = black (queue explosion)

    ***REMOVED*** N-1/N-2 thresholds
    N1_YELLOW_THRESHOLD = 3  ***REMOVED*** > 3 N-1 failures = yellow
    N1_ORANGE_THRESHOLD = 10  ***REMOVED*** > 10 N-1 failures = orange
    N1_RED_THRESHOLD = 25  ***REMOVED*** > 25 N-1 failures = red

    N2_ORANGE_THRESHOLD = 5  ***REMOVED*** Any N-2 failures = orange+
    N2_RED_THRESHOLD = 15  ***REMOVED*** > 15 N-2 failures = red

    ***REMOVED*** Cascade risk thresholds
    CASCADE_YELLOW = 0.10  ***REMOVED*** 10% cascade risk
    CASCADE_ORANGE = 0.25  ***REMOVED*** 25% cascade risk
    CASCADE_RED = 0.50  ***REMOVED*** 50% cascade risk
    ...
    def calculate(
        ...
        ***REMOVED*** Calculate scores for each dimension
        ...
        ***REMOVED*** Weighted combination (utilization and N-2 are most critical)
        weights = {
            "utilization": 0.30,
            "n1": 0.15,
            "n2": 0.25,
        ...
        ***REMOVED*** Map combined score to defense level
        level = self._score_to_level(combined_score)

        ***REMOVED*** Generate rationale and recommendations
    ...
    def _score_utilization(self, utilization: float) -> float:
        ...
        if utilization < self.UTILIZATION_GREEN:
            ...
            ***REMOVED*** Linear interpolation GREEN -> YELLOW
            ...
            ***REMOVED*** Exponential growth beyond RED threshold
            excess = utilization - self.UTILIZATION_RED
            return 3.0 + min(1.0, excess / 0.10)  ***REMOVED*** Cap at 4.0
    ...
    def _score_n2_failures(self, n2_failures: int) -> float:
        ...
        if n2_failures == 0:
            ...
            ***REMOVED*** Any N-2 failure is at least YELLOW
    ...
    def _score_coverage_gaps(self, gaps: int) -> float:
        ...
        ***REMOVED*** Coverage gaps are critical
        if gaps == 0:
            return 0.0
        elif gaps <= 2:
            return 1.0 + 0.5 * gaps  ***REMOVED*** 1-2 gaps = YELLOW
        elif gaps <= 5:
            return 2.0 + 0.33 * (gaps - 2)  ***REMOVED*** 3-5 gaps = ORANGE
        elif gaps <= 10:
            return 3.0 + 0.20 * (gaps - 5)  ***REMOVED*** 6-10 gaps = RED
        else:
            return 4.0  ***REMOVED*** > 10 gaps = BLACK
    ...
    def _score_burnout(self, burnout_cases: int) -> float:
        ...
        if burnout_cases == 0:
            ...
            return 1.0 * burnout_cases / 2  ***REMOVED*** 1-2 cases = GREEN-YELLOW
        elif burnout_cases <= 5:
            return 1.0 + 1.0 * (burnout_cases - 2) / 3  ***REMOVED*** 3-5 = YELLOW-ORANGE
        elif burnout_cases <= 10:
            return 2.0 + 1.0 * (burnout_cases - 5) / 5  ***REMOVED*** 6-10 = ORANGE-RED
        else:
