...
class UtilizationSnapshot:
    ...
    period_start: date
    period_end: date
    total_capacity: float  ***REMOVED*** Total available hours
    utilized_capacity: float  ***REMOVED*** Scheduled hours
    utilization_ratio: float  ***REMOVED*** utilized / total (0.0-1.0+)
    num_servers: int  ***REMOVED*** Number of residents/faculty (c in M/M/c)
    arrival_rate: float  ***REMOVED*** λ - requests per hour
    service_rate: float  ***REMOVED*** μ - service per hour per server
    traffic_intensity: float  ***REMOVED*** ρ = λ/(c*μ)
    queue_length: Optional[float] = None  ***REMOVED*** Expected queue length (from Erlang C)
    wait_time: Optional[float] = None  ***REMOVED*** Expected wait time (hours)
...
class UtilizationTrend:
    ...
    trend_slope: float  ***REMOVED*** Linear trend coefficient
    is_increasing: bool
...
class UtilizationMonitor:
    ...
    ***REMOVED*** Critical thresholds from queuing theory
    SAFE_UTILIZATION = 0.80  ***REMOVED*** Queue stable, manageable wait times
    WARNING_UTILIZATION = 0.90  ***REMOVED*** Queue growing, wait times increasing
    DANGER_UTILIZATION = 0.95  ***REMOVED*** Queue exploding, system unstable
    CRITICAL_UTILIZATION = 0.98  ***REMOVED*** System collapse imminent
    ...
    def calculate_snapshot(
        ...
        ***REMOVED*** Calculate traffic intensity if rates provided
        ...
        if arrival_rate is not None and service_rate is not None and service_rate > 0:
            ...
            ***REMOVED*** Calculate Erlang C (probability of queuing)
            if traffic_intensity < 1.0:  ***REMOVED*** System stable
                erlang_c = self._erlang_c(arrival_rate, service_rate, num_servers)
                ***REMOVED*** Expected queue length: L_q = Erlang_C * ρ/(1-ρ)
                queue_length = erlang_c * traffic_intensity / (1.0 - traffic_intensity)
                ***REMOVED*** Expected wait time: W_q = L_q / λ (Little's Law)
    ...
    def _erlang_c(self, arrival_rate: float, service_rate: float, num_servers: int) -> float:
        ...
        """
        A = arrival_rate / service_rate  ***REMOVED*** Total offered load

        if A >= num_servers:
            return 1.0  ***REMOVED*** System unstable, queue infinite

        ***REMOVED*** Calculate numerator: A^c / c! * c/(c-A)
        numerator = (A**num_servers / factorial(num_servers)) * (
            num_servers / (num_servers - A)
        )

        ***REMOVED*** Calculate denominator: sum + numerator
    ...
    def analyze_trend(
        ...
        ***REMOVED*** Linear trend (slope of least squares fit)
        if len(utilizations) > 1:
            x = np.arange(len(utilizations))
            trend_slope = float(np.polyfit(x, utilizations, 1)[0])
            is_increasing = trend_slope > 0.001  ***REMOVED*** > 0.1% per day
        else:
            trend_slope = 0.0
            is_increasing = False

        ***REMOVED*** Count days above thresholds
    ...
    def predict_queue_explosion(
        ...
        ***REMOVED*** Project utilization forward
    ...
    def recommend_capacity_adjustment(
        ...
        ***REMOVED*** Calculate required capacity increase
        ***REMOVED*** If util = used/capacity, and target_util = used/new_capacity
        ***REMOVED*** Then: new_capacity = used/target_util = capacity * (util/target_util)
