...
class AssignmentPriority(int, Enum):
    ...
    CRITICAL = 1  ***REMOVED*** Cannot shed (patient safety, required coverage)
    HIGH = 2  ***REMOVED*** Should avoid shedding (core clinical duties)
    MEDIUM = 3  ***REMOVED*** Can shed if needed (educational activities)
    LOW = 4  ***REMOVED*** Shed first (administrative, optional)
    DISCRETIONARY = 5  ***REMOVED*** Always sheddable (research time, electives)
...
class SacrificeHierarchy:
    ...
    def calculate_shedding_plan(
        ...
        ***REMOVED*** Sort assignments by priority (lowest first)
        sorted_assignments = sorted(
            assignments, key=lambda a: (a.priority.value, -a.load_hours)
        )

        ***REMOVED*** Greedily shed assignments
        to_shed = []
        shed_so_far = 0.0
        ...
        for assignment in sorted_assignments:
            ...
            ***REMOVED*** Don't shed CRITICAL assignments
