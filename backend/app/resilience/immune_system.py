...
class Detector:
    ...
    """
    id: UUID
    center: np.ndarray  ***REMOVED*** Center point in feature space
    radius: float  ***REMOVED*** Detection radius
    created_at: datetime
    matches_count: int = 0  ***REMOVED*** How many anomalies this detector has caught
...
class Antibody:
    ...
    affinity_center: np.ndarray  ***REMOVED*** Feature pattern this antibody targets
    affinity_radius: float  ***REMOVED*** How broadly applicable
    ...
    def get_affinity(self, feature_vector: np.ndarray) -> float:
        ...
        ***REMOVED*** Normalize by radius: distance 0 = affinity 1.0, distance >= radius = affinity 0
...
class AnomalyReport:
    ...
    anomaly_score: float  ***REMOVED*** Distance to nearest detector (higher = more anomalous)
    matching_detectors: list[UUID]  ***REMOVED*** Detectors that triggered
...
class ScheduleImmuneSystem:
    ...
    def __init__(
        ...
        ***REMOVED*** Immune components
        self.detectors: list[Detector] = []
        self.antibodies: dict[str, Antibody] = {}

        ***REMOVED*** Training data (for reference)
    ...
    def extract_features(self, schedule_state: dict) -> np.ndarray:
        ...
        ***REMOVED*** Coverage metrics (4 features)
        ...
        ***REMOVED*** Coverage by type (normalized)
        ...
        ***REMOVED*** ACGME compliance metrics (3 features)
        ...
        ***REMOVED*** Normalize by number of people (avoid unbounded growth)
        people_count = (
        ...
        ***REMOVED*** Hours compliance (0-1, where 1 = compliant)
        avg_hours = schedule_state.get("avg_hours_per_week", 0)
        hours_compliance = 1.0 - min(1.0, max(0, avg_hours - 80) / 20)  ***REMOVED*** Penalty above 80
        features.append(hours_compliance)

        ***REMOVED*** Supervision ratio (2 features)
        supervision_ratio = schedule_state.get("supervision_ratio", 1.0)
        ***REMOVED*** Normalize: 1:1 = 1.0, 1:2 = 0.5, etc.
        ...
        ***REMOVED*** Workload balance (1 feature)
        workload_std = schedule_state.get("workload_std_dev", 0.0)
        ***REMOVED*** Normalize: 0 std dev = perfect (1.0), high std dev = poor (0.0)
        balance_score = 1.0 / (1.0 + workload_std)
        features.append(balance_score)

        ***REMOVED*** Schedule stability (1 feature)
        ...
        ***REMOVED*** Convert to numpy array and pad/truncate to match feature_dims
        features_array = np.array(features, dtype=np.float32)

        if len(features_array) < self.feature_dims:
            ***REMOVED*** Pad with zeros
    ...
    def train(self, valid_schedules: list[dict], max_attempts: int = 1000):
        ...
        ***REMOVED*** Extract features from valid schedules
        ...
        ***REMOVED*** Generate detectors using negative selection
        ...
        while len(self.detectors) < self.detector_count and attempts < max_attempts:
            ...
            ***REMOVED*** Generate random detector center within feature space bounds
            center = np.random.uniform(min_vals, max_vals).astype(np.float32)

            ***REMOVED*** Check if this detector matches any valid schedule (self)
            matches_self = False
            ...
            ***REMOVED*** Negative selection: only keep detectors that DON'T match self
            if not matches_self:
                detector = Detector(
                    id=uuid4(),
                    center=center,
    ...
    def is_anomaly(self, schedule_state: dict) -> bool:
        ...
        ***REMOVED*** Check if any detector matches
    ...
    def get_anomaly_score(self, schedule_state: dict) -> float:
        ...
        ***REMOVED*** Find minimum distance to any detector center
        min_distance = float('inf')
        for detector in self.detectors:
            distance = detector.get_distance(features)
            min_distance = min(min_distance, distance)

        ***REMOVED*** Invert: closer to detector = higher anomaly score
        ***REMOVED*** Normalize by detection radius
        if min_distance < self.detection_radius:
            ***REMOVED*** Inside detection radius: high anomaly score
            score = 2.0 * (1.0 - min_distance / self.detection_radius)
        else:
            ***REMOVED*** Outside all detectors: low anomaly score
    ...
    def detect_anomaly(self, schedule_state: dict) -> AnomalyReport | None:
        ...
        ***REMOVED*** Generate description
        ...
        return AnomalyReport(
            ...
            severity="",  ***REMOVED*** Will be set in __post_init__
    ...
    def register_antibody(
        ...
        """
        ***REMOVED*** If pattern provided, use it as affinity center
        if affinity_pattern:
            affinity_center = self.extract_features(affinity_pattern)
        else:
            ***REMOVED*** Random affinity center
        ...
        try:
            ...
            ***REMOVED*** Update statistics
            ...
            return RepairResult(
                ...
                applied_at=datetime.now(),
                original_state=schedule_state,
                repaired_state=schedule_state,  ***REMOVED*** No change
                successful=False,
                anomaly_before=anomaly_before,
