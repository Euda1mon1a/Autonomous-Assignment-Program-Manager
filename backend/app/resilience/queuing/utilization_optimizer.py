...
class OptimizationResult:
    ...
    service_level: float
    cost_savings_pct: float  ***REMOVED*** vs. current
    rationale: str
...
class UtilizationOptimizer:
    ...
    def optimize_utilization(
        ...
        ***REMOVED*** Search for optimal servers meeting all constraints
        ...
        for num_servers in range(
            ...
            ***REMOVED*** Check constraints
            if result.avg_wait_time > max_wait_time:
                continue  ***REMOVED*** Violates wait time constraint
            if result.service_level < min_service_level:
                continue  ***REMOVED*** Violates service level

            ***REMOVED*** Score: balance high utilization with low wait time
            ***REMOVED*** Score = utilization × (1 - normalized_wait_time)
        ...
        if optimal_servers is None:
            ***REMOVED*** No solution found, use minimum stable
    ...
    def optimize_for_time_varying_demand(
        self,
        hourly_arrival_rates: list[float],  ***REMOVED*** 24 hours of arrival rates
        service_rate: float,
        current_servers: int,
    ) -> dict:
        """
    ...
    def calculate_utilization_risk(self, utilization: float) -> dict:
        ...
        ***REMOVED*** Estimate queue growth factor
        ***REMOVED*** At ρ=0.9, queue is ~9x longer than at ρ=0.5
        if utilization < 1.0:
    ...
    def simulate_capacity_reduction(
        ...
        reduced_servers = max(1, reduced_servers)  ***REMOVED*** At least 1 server
