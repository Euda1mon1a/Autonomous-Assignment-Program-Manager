...
class ErlangCResult:
    ...
    arrival_rate: float  ***REMOVED*** λ - arrivals per hour
    service_rate: float  ***REMOVED*** μ - service per hour per server
    num_servers: int  ***REMOVED*** c - number of servers
    traffic_intensity: float  ***REMOVED*** ρ = λ/(c*μ)
    utilization: float  ***REMOVED*** Same as traffic intensity
    prob_wait: float  ***REMOVED*** Erlang C - probability of waiting
    avg_queue_length: float  ***REMOVED*** L_q - average queue length
    avg_wait_time: float  ***REMOVED*** W_q - average wait time (hours)
    avg_system_time: float  ***REMOVED*** W - average time in system
    service_level: float  ***REMOVED*** Probability served within target time
...
class ErlangC:
    ...
    def calculate(
        ...
        service_rate: float,
        num_servers: int,
        target_wait_time: float = 0.25,  ***REMOVED*** Target wait time for service level (hours)
    ) -> ErlangCResult:
        """
        ...
        ***REMOVED*** Traffic intensity (utilization)
        rho = arrival_rate / (num_servers * service_rate)

        if rho >= 1.0:
            ***REMOVED*** System unstable - queue infinite
        ...
        ***REMOVED*** Average queue length: L_q = Erlang_C * ρ/(1-ρ)
        avg_queue_length = erlang_c * rho / (1.0 - rho)

        ***REMOVED*** Average wait time: W_q = L_q / λ (Little's Law)
        avg_wait_time = avg_queue_length / arrival_rate if arrival_rate > 0 else 0.0

        ***REMOVED*** Average system time: W = W_q + 1/μ
        avg_system_time = avg_wait_time + (1.0 / service_rate)

        ***REMOVED*** Service level: probability wait time < target
        ***REMOVED*** P(W < t) = 1 - Erlang_C * exp(-c*μ*(1-ρ)*t)
    ...
    def _erlang_c_formula(
        ...
        if num_servers <= A:
            return 1.0  ***REMOVED*** System overloaded

        ***REMOVED*** Calculate numerator: A^c / c! * c/(c-A)
        try:
            ***REMOVED*** Use log to avoid overflow
        ...
        ***REMOVED*** Calculate denominator: sum + numerator
        ...
        return min(1.0, max(0.0, erlang_c))  ***REMOVED*** Clamp to [0,1]
    ...
    def calculate_required_servers(
        ...
        target_service_level: float = 0.80,  ***REMOVED*** 80% served immediately
    ) -> int:
        """
        ...
        """
        ***REMOVED*** Minimum servers to meet utilization constraint
        min_servers_util = int(
            np.ceil(arrival_rate / (service_rate * target_utilization))
        )

        ***REMOVED*** Search for servers that meet service level
        ...
        ***REMOVED*** If can't meet service level, return minimum for stability
