...
def gini_coefficient(values: list[float], weights: list[float] | None = None) -> float:
    ...
    Examples:
        >>> gini_coefficient([10, 10, 10, 10])  ***REMOVED*** Perfect equality
        0.0
        >>> gini_coefficient([0, 0, 0, 100])     ***REMOVED*** Maximum inequality
        0.75
        >>> gini_coefficient([20, 30, 40], weights=[1.0, 1.5, 2.0])
        ***REMOVED*** Weighted calculation
    """
    if not values:
    ...
    ***REMOVED*** Handle weights
    if weights is not None:
        ...
        ***REMOVED*** Apply weights to values
        weighted_values = [v * w for v, w in zip(values, weights)]
        arr = np.array(weighted_values, dtype=np.float64)
    else:
        arr = np.array(values, dtype=np.float64)

    ***REMOVED*** Handle edge cases
    if len(arr) == 1:
        return 0.0

    ***REMOVED*** All values are the same (perfect equality)
    if np.allclose(arr, arr[0]):
        return 0.0

    ***REMOVED*** All values are zero
    if np.allclose(arr, 0.0):
        return 0.0

    ***REMOVED*** Calculate Gini coefficient using the standard formula
    ***REMOVED*** G = (Σᵢ Σⱼ |xᵢ - xⱼ|) / (2n² * μ)
    n = len(arr)
    mean_value = np.mean(arr)

    ***REMOVED*** Calculate sum of absolute differences
    ***REMOVED*** Using broadcasting for efficient computation
...
def lorenz_curve(values: list[float]) -> tuple[np.ndarray, np.ndarray]:
    ...
    Examples:
        ...
        >>> ***REMOVED*** x = [0.00, 0.25, 0.50, 0.75, 1.00]
        >>> ***REMOVED*** y = [0.00, 0.10, 0.30, 0.60, 1.00]
    """
    if not values:
        raise ValueError("values list cannot be empty")
...
def equity_report(
    ...
    ***REMOVED*** Validate intensity_weights if provided
    ...
    ***REMOVED*** Extract values and apply weights
    ...
    ***REMOVED*** Calculate statistics (on raw hours, not intensity-adjusted)
    ...
    ***REMOVED*** Find most overloaded and underloaded providers using the same values
    ***REMOVED*** used for delta calculations to keep recommendations consistent
    if weights is not None:
        ***REMOVED*** Use intensity-adjusted hours for identification
        adjusted_hours = {
    ...
    ***REMOVED*** Target Gini coefficient for medical scheduling (based on research)
    ***REMOVED*** Values below 0.15 indicate equitable distribution
    target_gini = 0.15
    ...
    ***REMOVED*** Generate recommendations
    ...
    if not is_equitable:
        ...
        ***REMOVED*** Recommend specific actions
        if overload_delta > mean_adjusted_hours * 0.2:  ***REMOVED*** 20% above mean
            recommendations.append(
        ...
        if underload_delta > mean_adjusted_hours * 0.2:  ***REMOVED*** 20% below mean
            recommendations.append(
        ...
        if std_adjusted_hours > mean_adjusted_hours * 0.25:  ***REMOVED*** High variation
            recommendations.append(
        ...
        ***REMOVED*** Suggest rebalancing between extremes
        ...
        ***REMOVED*** Even if equitable, flag significant outliers
    ...
    ***REMOVED*** Add intensity-specific recommendations if weights were provided
    if weights is not None:
