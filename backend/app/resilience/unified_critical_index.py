...
class CriticalityDomain(str, Enum):
    ...
    EPIDEMIOLOGY = "epidemiology"  ***REMOVED*** Burnout super-spreader potential
...
class RiskPattern(str, Enum):
    ...
    ***REMOVED*** All three domains high - maximum risk
    UNIVERSAL_CRITICAL = "universal_critical"

    ***REMOVED*** Two domains high
    STRUCTURAL_BURNOUT = "structural_burnout"  ***REMOVED*** Contingency + Epidemiology
    INFLUENTIAL_HUB = "influential_hub"  ***REMOVED*** Contingency + Hub
    SOCIAL_CONNECTOR = "social_connector"  ***REMOVED*** Epidemiology + Hub

    ***REMOVED*** Single domain high
    ISOLATED_WORKHORSE = "isolated_workhorse"  ***REMOVED*** Contingency only
    BURNOUT_VECTOR = "burnout_vector"  ***REMOVED*** Epidemiology only
    NETWORK_ANCHOR = "network_anchor"  ***REMOVED*** Hub only

    ***REMOVED*** No domains high
    LOW_RISK = "low_risk"
...
class InterventionType(str, Enum):
    ...
    IMMEDIATE_PROTECTION = "immediate_protection"  ***REMOVED*** Universal critical
    CROSS_TRAINING = "cross_training"  ***REMOVED*** Structural/Hub risks
    WORKLOAD_REDUCTION = "workload_reduction"  ***REMOVED*** Burnout-related patterns
    NETWORK_DIVERSIFICATION = "network_diversification"  ***REMOVED*** Hub-related patterns
    MONITORING = "monitoring"  ***REMOVED*** Low risk, continue observation
    WELLNESS_SUPPORT = "wellness_support"  ***REMOVED*** Epidemiology-related patterns
...
class DomainScore:
    ...
    raw_score: float  ***REMOVED*** 0.0 to 1.0
    normalized_score: float = 0.0  ***REMOVED*** After population normalization
    percentile: float = 0.0  ***REMOVED*** Where this score falls in population
    is_critical: bool = False  ***REMOVED*** Above critical threshold
...
class UnifiedCriticalIndex:
    ...
    ***REMOVED*** Domain-specific scores
    contingency_score: DomainScore
    epidemiology_score: DomainScore
    hub_score: DomainScore

    ***REMOVED*** Unified metrics
    composite_index: float = 0.0  ***REMOVED*** 0.0 to 1.0, weighted combination
    risk_pattern: RiskPattern = RiskPattern.LOW_RISK
    confidence: float = 0.0  ***REMOVED*** 0.0 to 1.0, based on data quality

    ***REMOVED*** Cross-domain insights
    domain_agreement: float = (
        0.0  ***REMOVED*** How much domains agree (0 = conflict, 1 = consensus)
    )
    leading_domain: CriticalityDomain | None = None  ***REMOVED*** Which domain signals first
    conflict_details: list[str] = field(default_factory=list)

    ***REMOVED*** Actionable outputs
    recommended_interventions: list[InterventionType] = field(default_factory=list)
    priority_rank: int = 0  ***REMOVED*** 1 = highest priority across population
    ...
    def _identify_pattern(self):
        ...
        ***REMOVED*** Map combination to pattern
        pattern_map = {
        ...
        ***REMOVED*** Identify leading domain (highest normalized score)
        scores = [
    ...
    def _calculate_agreement(self):
        ...
        ***REMOVED*** Agreement = 1 - coefficient of variation
        ...
        ***REMOVED*** Detect conflicts (disagreement cases worth investigating)
        self.conflict_details = []

        ***REMOVED*** Case 1: High hub but low epidemiology - isolated workaholic
        if (
        ...
        ***REMOVED*** Case 2: High epidemiology but low contingency - social but not critical
        if (
        ...
        ***REMOVED*** Case 3: All scores similar - true consensus (not a conflict)
...
class PopulationAnalysis:
    ...
    ***REMOVED*** Population-level metrics
    risk_concentration: float = 0.0  ***REMOVED*** Gini coefficient of risk
    critical_count: int = 0
    universal_critical_count: int = 0

    ***REMOVED*** Distribution by pattern
    ...
    ***REMOVED*** Top risks
    ...
    def _calculate_concentration(self):
        ...
        ***REMOVED*** Gini coefficient calculation
        cumulative = 0.0
    ...
    def _rank_priorities(self):
        ...
        ***REMOVED*** Sort by composite index descending
        ...
        ***REMOVED*** Top priority = top 20% or universal critical
...
class UnifiedCriticalIndexAnalyzer:
    ...
    def __init__(
        ...
        ***REMOVED*** Validate weights sum to 1
    ...
    def build_network(
        ...
        ***REMOVED*** Add all faculty as nodes
        for fac in faculty:
        ...
        ***REMOVED*** Build edge weights from shared assignments
        ...
        ***REMOVED*** Count shared shifts between pairs
        ...
        ***REMOVED*** Add edges above threshold
        ...
        ***REMOVED*** Pre-compute centrality metrics (expensive, do once)
    ...
    def _compute_centrality_metrics(self):
        ...
        ***REMOVED*** Compute various centrality measures
        try:
            degree = nx.degree_centrality(self._network)
            betweenness = nx.betweenness_centrality(self._network)
            ***REMOVED*** Eigenvector may fail for disconnected graphs
            try:
    ...
    def compute_contingency_score(
        ...
        """
        ***REMOVED*** Get assignments for this faculty member
        faculty_assignments = [a for a in assignments if a.person_id == faculty_id]

        ***REMOVED*** Count blocks where this faculty is the sole provider
        ...
        ***REMOVED*** Group assignments by block
        ...
        for assignment in faculty_assignments:
            ...
            ***REMOVED*** Sole provider: only this faculty on the block
            if len(faculty_on_block) == 1:
                sole_provider_blocks += 1

            ***REMOVED*** Critical: removing this faculty drops below requirement
            if len(faculty_on_block) <= required:
                critical_blocks += 1

        ***REMOVED*** Calculate raw score
        total_blocks = len(set(a.block_id for a in faculty_assignments))
        if total_blocks == 0:
            raw_score = 0.0
        else:
            ***REMOVED*** Weight sole provider heavily, critical blocks moderately
            raw_score = min(
                1.0,
        ...
        return DomainScore(
            ...
            raw_score=raw_score,
            normalized_score=raw_score,  ***REMOVED*** Will be normalized at population level
    ...
    def compute_epidemiology_score(
        ...
        ***REMOVED*** Factors that increase transmission potential:
        ***REMOVED*** 1. Degree (number of connections) - more contacts = more transmission
        ***REMOVED*** 2. Betweenness (bridge between groups) - can spread across clusters
        ***REMOVED*** 3. Current burnout state (if provided) - already infected
        ...
        ***REMOVED*** Check current burnout state
        is_burned_out = False
        if burnout_states:
        ...
        ***REMOVED*** Raw score combines network position with current state
        ...
        if is_burned_out:
            raw_score = min(1.0, raw_score * 1.5)  ***REMOVED*** Amplify if already burned out

        ***REMOVED*** Critical if high degree and currently burning out
    ...
    def compute_hub_score(self, faculty_id: UUID) -> DomainScore:
        ...
        ***REMOVED*** Composite hub score from multiple centrality measures
        raw_score = (
    ...
    def normalize_scores(self, scores: list[DomainScore]) -> list[DomainScore]:
        ...
        for score in scores:
            ***REMOVED*** Min-max normalization
            if range_val > 0:
                score.normalized_score = (score.raw_score - min_val) / range_val
            else:
                score.normalized_score = 0.5  ***REMOVED*** All same, put in middle
    ...
    def _calculate_confidence(
        ...
        ***REMOVED*** Contingency: based on number of blocks analyzed
        ...
        ***REMOVED*** Hub: based on network connectivity
        if self._network and hub.details:
            confidence += 0.35

        ***REMOVED*** Epidemiology: based on burnout state data
    ...
    def analyze_population(
        ...
        ***REMOVED*** Compute raw scores for all faculty
        indices = []
        contingency_scores = []
        epidemiology_scores = []
        hub_scores = []
...
def quick_analysis(
    ...
    if coverage_requirements is None:
        ***REMOVED*** Default: 1 person required per block
