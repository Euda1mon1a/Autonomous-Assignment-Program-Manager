...
class PrecursorSignal(str, Enum):
    ...
    """
    SWAP_REQUESTS = "swap_requests"  ***REMOVED*** Frequency of shift swap requests
    SICK_CALLS = "sick_calls"  ***REMOVED*** Unplanned absences
    PREFERENCE_DECLINE = "preference_decline"  ***REMOVED*** Declining preferred shifts
    RESPONSE_DELAYS = "response_delays"  ***REMOVED*** Slower response times (hours)
    VOLUNTARY_COVERAGE_DECLINE = "voluntary_coverage_decline"  ***REMOVED*** Refusing extra shifts
...
class BurnoutEarlyWarning:
    ...
    Example:
        ...
        ***REMOVED*** Time series: daily swap request counts
    ...
    def classic_sta_lta(
        ...
        ***REMOVED*** Ensure float dtype for division
        data = np.asarray(data, dtype=float)

        ***REMOVED*** Initialize output
        sta_lta = np.zeros(len(data), dtype=float)

        ***REMOVED*** Compute squared data for energy calculation
        data_sq = data ** 2

        ***REMOVED*** Compute STA/LTA for each valid position
        for i in range(nlta, len(data)):
            ***REMOVED*** Short-term average (recent window)
            sta_start = max(0, i - nsta + 1)
            sta = np.mean(data_sq[sta_start:i + 1])

            ***REMOVED*** Long-term average (historical window)
            lta_start = max(0, i - nlta + 1)
            lta = np.mean(data_sq[lta_start:i + 1])

            ***REMOVED*** Compute ratio (avoid division by zero)
            if lta > 1e-10:
                sta_lta[i] = sta / lta
            else:
                sta_lta[i] = 0.0
    ...
    def recursive_sta_lta(
        ...
        ***REMOVED*** Decay factors for exponential moving average
        ***REMOVED*** alpha = 2 / (N + 1) is standard EMA formula
        ...
        ***REMOVED*** Compute recursive averages
        for i in range(len(data)):
            value_sq = data[i] ** 2

            ***REMOVED*** Update exponential moving averages
            sta = alpha_sta * value_sq + (1 - alpha_sta) * sta
            lta = alpha_lta * value_sq + (1 - alpha_lta) * lta

            ***REMOVED*** Compute ratio
            if lta > 1e-10:
                sta_lta[i] = sta / lta
            else:
                sta_lta[i] = 0.0
    ...
    def trigger_onset(
        ...
        for i in range(len(sta_lta)):
            if not is_triggered:
                ***REMOVED*** Check for trigger activation
                if sta_lta[i] >= on_threshold:
                    is_triggered = True
                    trigger_start = i
            else:
                ***REMOVED*** Check for trigger deactivation
                if sta_lta[i] < off_threshold:
                    is_triggered = False
                    triggers.append((trigger_start, i))

        ***REMOVED*** Handle case where trigger is still active at end
        if is_triggered:
    ...
    def detect_precursors(
        ...
        Example:
            ***REMOVED*** Daily swap request counts for last 60 days
        ...
        ***REMOVED*** Convert to numpy array
        data = np.array(time_series, dtype=float)

        ***REMOVED*** Compute STA/LTA using recursive method (more efficient)
        sta_lta = self.recursive_sta_lta(data, self.short_window, self.long_window)

        ***REMOVED*** Detect trigger points
        ...
        ***REMOVED*** Generate alerts for each trigger
        alerts = []
        for start_idx, end_idx in triggers:
            ***REMOVED*** Maximum STA/LTA ratio during this trigger
            max_ratio = float(np.max(sta_lta[start_idx:end_idx + 1]))

            ***REMOVED*** Determine severity based on ratio magnitude
            if max_ratio >= 10.0:
                severity = "critical"
            elif max_ratio >= 5.0:
                severity = "high"
            ...
            ***REMOVED*** Estimate signal growth rate (for time-to-event prediction)
            if end_idx > start_idx:
                growth_rate = (sta_lta[end_idx] - sta_lta[start_idx]) / (end_idx - start_idx)
            else:
                growth_rate = 0.0

            ***REMOVED*** Predict burnout magnitude
            magnitude = self._estimate_magnitude(max_ratio, signal_type)

            ***REMOVED*** Estimate time to burnout event
            time_to_event = None
            if growth_rate > 0.01:  ***REMOVED*** Significant growth
    ...
    def predict_burnout_magnitude(
        ...
        ***REMOVED*** Analyze each signal
        ...
        for signal_type, time_series in precursor_signals.items():
            ...
            ***REMOVED*** Compute STA/LTA
            data = np.array(time_series, dtype=float)
            sta_lta = self.recursive_sta_lta(data, self.short_window, self.long_window)

            ***REMOVED*** Get maximum ratio
            max_ratio = float(np.max(sta_lta))

            ***REMOVED*** Convert to magnitude estimate
            magnitude = self._estimate_magnitude(max_ratio, signal_type)
        ...
        ***REMOVED*** Combine using weighted average and maximum
        ***REMOVED*** (Multiple signals increase confidence)
        avg_magnitude = np.mean(signal_magnitudes)
        max_magnitude = np.max(signal_magnitudes)

        ***REMOVED*** Weight toward maximum but consider average
        ***REMOVED*** (Similar to seismology using multiple stations)
        combined_magnitude = 0.7 * max_magnitude + 0.3 * avg_magnitude

        ***REMOVED*** Apply bonus for multiple simultaneous signals
        if len(signal_magnitudes) >= 3:
            combined_magnitude *= 1.2  ***REMOVED*** 20% increase for multi-signal confirmation

        ***REMOVED*** Clamp to 1-10 range
    ...
    def estimate_time_to_event(
        ...
        """
        ***REMOVED*** Critical threshold for burnout event
        critical_threshold = 10.0

        if signal_growth_rate <= 0.0:
            ***REMOVED*** Not growing - return large value
            return timedelta(days=365)

        if sta_lta_ratio >= critical_threshold:
            ***REMOVED*** Already at critical threshold
            return timedelta(days=1)

        ***REMOVED*** Linear extrapolation (conservative)
        days_to_critical = (critical_threshold - sta_lta_ratio) / signal_growth_rate

        ***REMOVED*** Clamp to reasonable range (1-365 days)
    ...
    def _estimate_magnitude(sta_lta_ratio: float, signal_type: PrecursorSignal) -> float:
        ...
        """
        ***REMOVED*** Base magnitude from logarithmic scale (like Richter)
        ***REMOVED*** log2(ratio) maps: 2->1, 4->2, 8->3, 16->4, etc.
        base_magnitude = np.log2(max(1.0, sta_lta_ratio))

        ***REMOVED*** Apply signal-type specific weights
        ***REMOVED*** (Some signals are stronger predictors than others)
        weights = {
            PrecursorSignal.SWAP_REQUESTS: 1.0,  ***REMOVED*** Moderate indicator
            PrecursorSignal.SICK_CALLS: 1.3,  ***REMOVED*** Strong indicator
            PrecursorSignal.PREFERENCE_DECLINE: 1.2,  ***REMOVED*** Strong indicator
            PrecursorSignal.RESPONSE_DELAYS: 0.9,  ***REMOVED*** Moderate indicator
            PrecursorSignal.VOLUNTARY_COVERAGE_DECLINE: 1.1,  ***REMOVED*** Moderate-strong
        }
