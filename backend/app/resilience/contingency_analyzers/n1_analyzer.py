...
***REMOVED*** N-1 Criticality Thresholds
...
***REMOVED*** Recovery Time Constants (hours)
...
class N1FailureScenario:
    ...
    component_id: str  ***REMOVED*** Person ID or rotation ID
    component_type: str  ***REMOVED*** "person", "rotation", "specialty"
    failure_date: date
    affected_slots: int  ***REMOVED*** Number of slots that become uncovered
    cascade_potential: float  ***REMOVED*** Probability this triggers cascading failures (0-1)
    recovery_time_hours: float  ***REMOVED*** Estimated hours to recover
    criticality_score: float  ***REMOVED*** Overall criticality (0-1)
    backup_available: bool
    backup_ids: list[str]  ***REMOVED*** Available backup resources
...
class N1Analyzer:
    ...
    def analyze_person_failure(
        self,
        person_id: str,
        assigned_slots: list[tuple[date, str]],  ***REMOVED*** (date, rotation_id)
        available_backups: list[str],
        backup_capacity: dict[str, int],  ***REMOVED*** backup_id -> available slots
    ) -> N1FailureScenario:
        """
        ...
        ***REMOVED*** Calculate criticality
        if num_affected == 0:
            criticality = 0.0
        elif has_backup:
            ***REMOVED*** Low criticality if backups available
            criticality = min(CRITICALITY_LOW_WITH_BACKUP, num_affected / ASSIGNMENTS_PER_CRITICALITY_UNIT)
        else:
            ***REMOVED*** High criticality if no backups
        ...
        ***REMOVED*** Estimate recovery time
        if has_backup:
            recovery_hours = RECOVERY_TIME_WITH_BACKUP  ***REMOVED*** Quick swap
        else:
            recovery_hours = num_affected * RECOVERY_TIME_PER_SLOT_NO_BACKUP  ***REMOVED*** Need to find coverage

        ***REMOVED*** Cascade potential - higher if no backups
        cascade_potential = CASCADE_POTENTIAL_WITH_BACKUP if has_backup else min(0.8, num_affected / 15.0)

        ***REMOVED*** Determine mitigation strategy
        if has_backup:
        ...
        ***REMOVED*** Use first slot date as failure date
    ...
    def analyze_specialty_failure(
        ...
        cross_trained: list[str],  ***REMOVED*** Personnel cross-trained in this specialty
    ) -> N1FailureScenario:
        """
        ...
        """
        ***REMOVED*** If only one specialist, losing them is critical
        has_backup = len(available_specialists) > 1 or len(cross_trained) > 0

        if len(available_specialists) == 1:
            criticality = CRITICALITY_SPECIALTY_SINGLE  ***REMOVED*** Single point of failure
            cascade_potential = CASCADE_SPECIALTY_SINGLE
        elif len(available_specialists) == 2:
            criticality = CRITICALITY_SPECIALTY_DUAL  ***REMOVED*** Limited redundancy
            cascade_potential = CASCADE_SPECIALTY_DUAL
        else:
            criticality = CRITICALITY_SPECIALTY_MULTIPLE  ***REMOVED*** Multiple specialists
    ...
    def build_dependency_graph(
        self,
        assignments: list[tuple[str, date, str]],  ***REMOVED*** (person_id, date, rotation_id)
    ) -> nx.DiGraph:
        """
        ...
        for person_id, assignment_date, rotation_id in assignments:
            ...
            ***REMOVED*** Add edges: person -> date -> rotation
    ...
    def identify_critical_paths(self, graph: nx.DiGraph) -> list[list[str]]:
        ...
        ***REMOVED*** Find bridges (edges whose removal disconnects graph)
        bridges = list(nx.bridges(graph.to_undirected()))

        for bridge in bridges:
            ***REMOVED*** Find all paths containing this bridge
            source, target = bridge
            try:
                paths = nx.all_simple_paths(graph, source, target, cutoff=10)
                for path in paths:
                    if len(path) > 2:  ***REMOVED*** Non-trivial path
    ...
    def calculate_redundancy_score(
        ...
        if num_assignments == 0:
            return 1.0  ***REMOVED*** Perfect redundancy (not needed)

        ***REMOVED*** Estimate required backups (1 per 10 shifts)
