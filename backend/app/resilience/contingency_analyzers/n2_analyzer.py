...
***REMOVED*** N-2 Criticality Thresholds
...
***REMOVED*** N-2 Recovery Time Factors
...
class N2FailureScenario:
    ...
    component1_id: str
    component2_id: str
    component_type: str  ***REMOVED*** "person", "specialty", "rotation"
    failure_date: date
    affected_slots: int
    cascade_probability: float  ***REMOVED*** Probability of total cascade (0-1)
    recovery_time_hours: float
    criticality_score: float  ***REMOVED*** 0-1, higher = more critical
    backup_available: bool
    backup_ids: list[str]
    mitigation_strategy: str
    interdependency_type: str  ***REMOVED*** "independent", "correlated", "coupled"
...
class N2Analyzer:
    ...
    def analyze_dual_person_failure(
        ...
        correlation: float = 0.0,  ***REMOVED*** 0-1, correlation between failures
    ) -> N2FailureScenario:
        """
        ...
        ***REMOVED*** Check for overlapping assignments (increases severity)
        ...
        if overlap:
            ...
            total_affected += len(overlap) * 2  ***REMOVED*** Double penalty for overlap

        ***REMOVED*** Determine interdependency type
        if correlation > CORRELATION_COUPLED_THRESHOLD:
            interdependency = "coupled"  ***REMOVED*** Failures likely to occur together
        elif correlation > CORRELATION_CORRELATED_THRESHOLD:
            interdependency = "correlated"  ***REMOVED*** Some connection
        else:
        ...
        ***REMOVED*** Calculate criticality (N-2 is inherently more critical)
        base_criticality = N2_BASE_CRITICALITY  ***REMOVED*** N-2 starts higher than N-1
        ...
        ***REMOVED*** Recovery time
        if has_backup:
            recovery_hours = total_affected * RECOVERY_WITH_BACKUP_MULTIPLIER  ***REMOVED*** Parallel recovery
        else:
            recovery_hours = total_affected * RECOVERY_WITHOUT_BACKUP_MULTIPLIER  ***REMOVED*** Serial recovery
    ...
    def analyze_all_n2_combinations(
        ...
        ***REMOVED*** Generate all 2-combinations
        for person1, person2 in combinations(personnel, 2):
            ...
            ***REMOVED*** Skip if both have no assignments
            if not person1_slots and not person2_slots:
                continue

            ***REMOVED*** Estimate correlation based on assignment overlap
    ...
    def _estimate_correlation(
        ...
        ***REMOVED*** Calculate Jaccard similarity of rotations
        ...
        ***REMOVED*** Correlation is similarity (0-1)
    ...
    def calculate_n2_resilience_score(self) -> float:
        ...
        if not self.scenarios:
            return 1.0  ***REMOVED*** Perfect score if no scenarios analyzed

        ***REMOVED*** Weight by cascade probability
        weighted_sum = sum(
    ...
    def identify_common_cause_failures(
        ...
        for scenario in self.scenarios:
            if scenario.interdependency_type == "coupled":
                ***REMOVED*** Estimate correlation from cascade probability
