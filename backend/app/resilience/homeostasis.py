...
class FeedbackType(str, Enum):
    ...
    NEGATIVE = "negative"  ***REMOVED*** Stabilizing
    POSITIVE = "positive"  ***REMOVED*** Destabilizing


class DeviationSeverity(str, Enum):
    """Severity of deviation from setpoint."""

    NONE = "none"  ***REMOVED*** Within tolerance
    MINOR = "minor"  ***REMOVED*** Slight deviation
    MODERATE = "moderate"  ***REMOVED*** Noticeable deviation
    MAJOR = "major"  ***REMOVED*** Significant deviation
    CRITICAL = "critical"  ***REMOVED*** System stability threatened


class VolatilityLevel(str, Enum):
    """Level of volatility in a metric's values."""

    STABLE = "stable"  ***REMOVED*** Low variance, predictable
    NORMAL = "normal"  ***REMOVED*** Expected fluctuation
    ELEVATED = "elevated"  ***REMOVED*** Higher than normal variance
    HIGH = "high"  ***REMOVED*** Concerning instability
    CRITICAL = "critical"  ***REMOVED*** Approaching phase transition


class CorrectiveActionType(str, Enum):
    """Types of corrective actions."""

    REDISTRIBUTE = "redistribute"  ***REMOVED*** Redistribute workload
    RECRUIT_BACKUP = "recruit_backup"  ***REMOVED*** Bring in backup coverage
    DEFER_ACTIVITY = "defer_activity"  ***REMOVED*** Postpone non-critical work
    PROTECT_RESOURCE = "protect_resource"  ***REMOVED*** Shield overloaded faculty
    REDUCE_SCOPE = "reduce_scope"  ***REMOVED*** Reduce service scope
    ALERT_ONLY = "alert_only"  ***REMOVED*** Just notify, no automatic action
...
class Setpoint:
    ...
    description: str
    target_value: float
    tolerance: float  ***REMOVED*** Acceptable deviation (0.05 = 5%)
    unit: str = ""
    minimum: float = 0.0
    maximum: float = 1.0
...
class VolatilityMetrics:
    ...
    volatility: float  ***REMOVED*** Coefficient of variation (std_dev / mean)
    jitter: float  ***REMOVED*** Oscillation frequency (direction changes / observations)
    momentum: float  ***REMOVED*** Rate of change (slope of recent values)
    distance_to_critical: float  ***REMOVED*** Distance to critical threshold (0-1 normalized)
    level: VolatilityLevel  ***REMOVED*** Categorical classification
...
class FeedbackLoop:
    ...
    ***REMOVED*** History for trend analysis
    ...
    def record_value(self, value: float, timestamp: datetime | None = None):
        ...
        ***REMOVED*** Trim history if needed
    ...
    def is_improving(self) -> bool:
        ...
        ***REMOVED*** Calculate average distance from target
    ...
    def get_jitter(self, window_size: int = 20) -> float:
        ...
        ***REMOVED*** Count direction changes
        ...
        for i in range(2, len(recent_values)):
            ...
            ***REMOVED*** Direction change if sign flips
    ...
    def get_momentum(self, window_size: int = 10) -> float:
        ...
        ***REMOVED*** Simple linear regression slope
        n = len(recent)
        x_vals = list(range(n))
        ...
        ***REMOVED*** Normalize by tolerance threshold
    ...
    def get_distance_to_criticality(self) -> float:
        ...
        )  ***REMOVED*** 5x tolerance = CRITICAL

        deviation = abs(current_value - target)
        if critical_threshold == 0:
            return 1.0

        ***REMOVED*** Normalize: 0 = at critical threshold, 1 = at target
    ...
    def get_volatility_metrics(self) -> VolatilityMetrics:
        ...
        ***REMOVED*** Determine volatility level based on multiple factors
        ***REMOVED*** Higher volatility + high jitter + low distance = more critical
        risk_score = (
            volatility * 2.0  ***REMOVED*** Variance is primary indicator
            + jitter * 1.5  ***REMOVED*** Oscillation amplifies risk
...
class AllostasisMetrics:
    ...
    id: UUID
    entity_id: UUID  ***REMOVED*** Faculty ID or system UUID
    entity_type: str  ***REMOVED*** "faculty" or "system"
    calculated_at: datetime

    ***REMOVED*** Stress factors (weighted)
    ...
    ***REMOVED*** Calculated scores
    acute_stress_score: float = 0.0  ***REMOVED*** Recent/immediate stress
    chronic_stress_score: float = 0.0  ***REMOVED*** Accumulated over time
    ...
    ***REMOVED*** Thresholds
    ...
    def calculate(self):
        ...
        ***REMOVED*** Acute stress (recent, higher weight)
        ...
        ***REMOVED*** Chronic stress (accumulated)
        ...
        ***REMOVED*** Total with chronic stress having more weight long-term
...
class PositiveFeedbackRisk:
    ...
    ***REMOVED*** The loop pattern
    trigger: str  ***REMOVED*** What starts the loop
    amplification: str  ***REMOVED*** How it gets worse
    consequence: str  ***REMOVED*** End result if unchecked

    ***REMOVED*** Detection metrics
    evidence: list[str]
    ...
    ***REMOVED*** Recommended intervention
    intervention: str
    urgency: str  ***REMOVED*** "immediate", "soon", "monitor"
...
class VolatilityAlert:
    ...
    ***REMOVED*** Alert details
    description: str
    evidence: list[str]
    severity: DeviationSeverity

    ***REMOVED*** Recommended action
    intervention: str
    urgency: str  ***REMOVED*** "immediate", "soon", "monitor"
...
class HomeostasisStatus:
    ...
    feedback_loops_volatile: int  ***REMOVED*** NEW: loops with high volatility
    active_corrections: int
    positive_feedback_risks: int
    volatility_alerts: int  ***REMOVED*** NEW: count of volatility alerts
...
class HomeostasisMonitor:
    ...
    def _initialize_default_setpoints(self):
        ...
        default_setpoints = [
            ...
            Setpoint(
                ...
                target_value=0.75,  ***REMOVED*** Below 80% threshold
                tolerance=0.10,
                unit="ratio",
                minimum=0.0,
                maximum=1.0,
            ...
            Setpoint(
                ...
                target_value=0.15,  ***REMOVED*** Low variance is good
                tolerance=0.05,
                unit="std_dev",
                minimum=0.0,
                maximum=1.0,
            ...
            Setpoint(
                ...
                target_value=0.95,  ***REMOVED*** 95% of schedule unchanged
                tolerance=0.05,
                unit="ratio",
                minimum=0.0,
                maximum=1.0,
        ...
        for sp in default_setpoints:
            ...
            ***REMOVED*** Create feedback loop for each setpoint
            loop = FeedbackLoop(
                id=uuid4(),
    ...
    def register_feedback_loop(
        ...
        """
        ***REMOVED*** Register the setpoint if not already registered
        if setpoint.id not in self.setpoints:
            self.setpoints[setpoint.id] = setpoint

        ***REMOVED*** Create the feedback loop
        loop = FeedbackLoop(
            id=uuid4(),
            name=name,
    ...
    def check_feedback_loop(
        ...
        ***REMOVED*** Determine if correction needed
        ...
        ***REMOVED*** For minor/moderate, check for persistent deviation
    ...
    def _trigger_correction(
        ...
        ***REMOVED*** Determine action type based on setpoint and severity
        ...
        ***REMOVED*** Execute if handler registered
    ...
    def detect_positive_feedback_risks(
        ...
        ***REMOVED*** Check for burnout cascade
        high_load_count = sum(
            1
        ...
        if high_load_count > 0:
            ...
            if high_load_ratio > 0.3:  ***REMOVED*** >30% faculty with high allostatic load
                risks.append(
        ...
        ***REMOVED*** Check for coverage gap spiral
        ...
        ***REMOVED*** Check for attrition cascade
        avg_load = (
        ...
        if avg_load > 60:  ***REMOVED*** High system-wide stress
            risks.append(
                PositiveFeedbackRisk(
                    id=uuid4(),
    ...
    def detect_volatility_risks(self) -> list[VolatilityAlert]:
        ...
        for loop in self.feedback_loops.values():
            ...
            ***REMOVED*** Generate alert if volatility is concerning
            if metrics.is_warning:
                ...
                ***REMOVED*** Determine severity and urgency
    ...
    def get_status(
        ...
        ***REMOVED*** Determine overall state (volatility can escalate state)
        if avg_load > 80:
        ...
        ***REMOVED*** Add volatility-based recommendations
