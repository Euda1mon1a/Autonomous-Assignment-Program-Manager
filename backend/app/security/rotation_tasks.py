            ...
            try:
                ***REMOVED*** Check if rotation is due
                ...
                if status.get("due") and config.auto_rotate:
                    ...
                    ***REMOVED*** Send reminder notification
                    logger.info(
...
def rotate_secret(
    ...
    try:
        ...
        ***REMOVED*** Convert string to enum
        secret_type_enum = SecretType(secret_type)

        ***REMOVED*** Convert initiated_by to UUID if provided
        initiated_by_uuid = UUID(initiated_by) if initiated_by else None

        ***REMOVED*** Perform rotation (Celery tasks are synchronous, use asyncio.run)
        import asyncio
...
def complete_grace_periods(self) -> dict:
    ...
    db = get_db_session()
    try:
        ***REMOVED*** Find all grace periods that have expired
...
def send_rotation_reminder(
    ...
    try:
        ***REMOVED*** In production, this would integrate with notification service
        ***REMOVED*** For now, just log the reminder
...
def monitor_rotation_health(self) -> dict:
    ...
    db = get_db_session()
    try:
        ***REMOVED*** Check for failed rotations in last 24 hours
        failed_rotations = (
            db.query(SecretRotationHistory)
            .filter(
        ...
        ***REMOVED*** Check for active grace periods
        ...
        ***REMOVED*** Check for grace periods expiring soon (< 6 hours)
        expiring_soon = (
            db.query(SecretRotationHistory)
            .filter(
                SecretRotationHistory.status == RotationStatus.GRACE_PERIOD,
        ...
        ***REMOVED*** Check for overdue rotations
        service = SecretRotationService(db)
        import asyncio
        ...
        ***REMOVED*** Calculate health status
        ...
        ***REMOVED*** Send alert if unhealthy
...
def emergency_rotate_all(
    ...
    try:
        ...
        ***REMOVED*** Rotate all auto-rotatable secrets
        for secret_type, config in service.DEFAULT_CONFIGS.items():
            if not config.auto_rotate:
                ***REMOVED*** Skip secrets that can't be auto-rotated (e.g., database passwords)
                logger.info(
...
def cleanup_old_rotation_history(
    self,
    retention_days: int = 730,  ***REMOVED*** 2 years
) -> dict:
    """
    ...
    try:
        ...
        ***REMOVED*** Delete old rotation records
        deleted_count = (
            db.query(SecretRotationHistory)
