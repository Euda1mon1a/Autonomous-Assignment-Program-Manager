...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        ***REMOVED*** Pre-compute static headers for performance
        self._static_headers = SecurityHeaders.get_headers(debug=self.debug)

        ***REMOVED*** Log configuration at startup
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        """
        ***REMOVED*** Process request through the application
        response = await call_next(request)

        ***REMOVED*** Add static security headers
        for header_name, header_value in self._static_headers.items():
            response.headers[header_name] = header_value

        ***REMOVED*** Add Content-Security-Policy if enabled
        if self.enable_csp:
...
def create_security_headers_middleware(
    ...
    ***REMOVED*** Create a configured middleware class
