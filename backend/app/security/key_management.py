...
class CryptographicKey(Base):
    ...
    ***REMOVED*** Encrypted key material (base64 encoded)
    ...
    encrypted_private_key = Column(Text, nullable=True)  ***REMOVED*** For asymmetric keys
    public_key = Column(Text, nullable=True)  ***REMOVED*** For asymmetric keys (can be unencrypted)

    ***REMOVED*** Encryption metadata
    encryption_salt = Column(String(64), nullable=False)  ***REMOVED*** Salt for key derivation
    encryption_nonce = Column(String(32), nullable=False)  ***REMOVED*** Nonce for AES-GCM
    encryption_tag = Column(String(32), nullable=False)  ***REMOVED*** Authentication tag

    ***REMOVED*** Access control
    access_policy = Column(String(50), nullable=False, default=AccessPolicy.ADMIN_ONLY.value)
    allowed_users = Column(JSONType, nullable=False, default=list)  ***REMOVED*** List of user IDs
    ...
    ***REMOVED*** Rotation and versioning
    auto_rotate = Column(Boolean, nullable=False, default=False)
    rotation_interval_days = Column(Integer, nullable=True)
    last_rotated_at = Column(DateTime, nullable=True)
    previous_version_id = Column(GUID(), nullable=True)  ***REMOVED*** Link to previous version
    next_version_id = Column(GUID(), nullable=True)  ***REMOVED*** Link to next version
...
***REMOVED*** =============================================================================
***REMOVED*** Key Encryption/Decryption Functions
***REMOVED*** =============================================================================
...
def _encrypt_key_material(key_material: bytes) -> tuple[bytes, bytes, bytes, bytes]:
    ...
    """
    ***REMOVED*** Generate random salt and nonce
    salt = secrets.token_bytes(32)
    nonce = secrets.token_bytes(12)  ***REMOVED*** 96 bits for GCM

    ***REMOVED*** Derive encryption key from master secret
    encryption_key = _derive_encryption_key(salt)

    ***REMOVED*** Encrypt using AES-256-GCM
    cipher = Cipher(
...
def _decrypt_key_material(
    ...
    ***REMOVED*** Decrypt using AES-256-GCM
    cipher = Cipher(
        algorithms.AES(encryption_key),
        modes.GCM(nonce, tag),
        backend=default_backend()
...
***REMOVED*** =============================================================================
***REMOVED*** Key Management Service
***REMOVED*** =============================================================================
...
class KeyManagementService:
    ...
    ***REMOVED*** =========================================================================
    ...
    async def generate_key(
        ...
        ***REMOVED*** Encrypt key material
        encrypted_key, salt, nonce, tag = _encrypt_key_material(key_material)

        ***REMOVED*** Encrypt private key if asymmetric
        ...
        ***REMOVED*** Calculate expiration date
        expires_at = None
        if request.expires_in_days:
            expires_at = datetime.utcnow() + timedelta(days=request.expires_in_days)

        ***REMOVED*** Create database record
        ...
        ***REMOVED*** Store encrypted private key metadata if asymmetric
        if private_key_material:
    ...
    ***REMOVED*** =========================================================================
    ...
    async def list_keys(
        ...
        ***REMOVED*** Filter by access policy
        accessible_keys = [
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Key Lifecycle Management
    ***REMOVED*** =========================================================================
    ...
    async def rotate_key(
        ...
        ***REMOVED*** Delete old key to avoid name conflict
        await db.delete(old_key)
        await db.commit()

        new_key_metadata = await self.generate_key(db, new_request, user_id)

        ***REMOVED*** Re-fetch new key and update version info
        result = await db.execute(
    ...
    ***REMOVED*** =========================================================================
    ...
    def _check_access(self, key: CryptographicKey, user_id: str) -> bool:
        ...
        if policy == AccessPolicy.PUBLIC_READ:
            ...
            ***REMOVED*** In production, check if user_id is an admin
            ***REMOVED*** For now, allow access
            return True
        elif policy in (AccessPolicy.SERVICE_ACCOUNT, AccessPolicy.APPLICATION):
            ***REMOVED*** In production, check service account credentials
            return True
    ...
    async def _record_usage(
        ...
        """
        ***REMOVED*** Update key usage statistics
        key.usage_count += 1
        if success:
            key.last_used_at = datetime.utcnow()

        ***REMOVED*** Create usage log
        log = KeyUsageLog(
            key_id=key.id,
            key_name=key.name,
            used_by=user_id,
...
***REMOVED*** =============================================================================
***REMOVED*** Synchronous Wrapper for Backward Compatibility
***REMOVED*** =============================================================================
...
class SyncKeyManagementService:
    ...
    def generate_key_sync(
        ...
        ***REMOVED*** In production, use asyncio.run() or sync session
