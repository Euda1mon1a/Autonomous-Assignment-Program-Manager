...
class DatabaseHealthCheck:
    ...
    async def check(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Determine status based on response time
            if response_time_ms > 1000:  ***REMOVED*** > 1 second is degraded
    ...
    async def _perform_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** 1. Test basic connectivity with simple query
            result = db.execute(text("SELECT 1"))
            result.scalar()

            ***REMOVED*** 2. Get database version
            version_result = db.execute(text("SELECT version()"))
            db_version = version_result.scalar()

            ***REMOVED*** 3. Get connection pool statistics
            pool_status = self._get_pool_status()

            ***REMOVED*** 4. Test table accessibility (check if core tables exist)
    ...
    def _check_core_tables(self, db: Session) -> dict[str, Any]:
        ...
        for table_name in core_tables:
            try:
                ***REMOVED*** Try to query table (with limit to avoid loading data)
                query = text(f"SELECT 1 FROM {table_name} LIMIT 1")
                db.execute(query)
                accessible.append(table_name)

            except SQLAlchemyError:
                ***REMOVED*** Table not accessible
    ...
    async def check_read_write(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Create a temporary table for health checks
            db.execute(
                text("""
            ...
            ***REMOVED*** Cleanup old records (keep only last 10)
            db.execute(
                text("""
