...
class RedisHealthCheck:
    ...
    async def check(self) -> dict[str, Any]:
        ...
        try:
            ***REMOVED*** Run check with timeout
            ...
            ***REMOVED*** Determine status based on response time and memory
            if response_time_ms > 500:  ***REMOVED*** > 500ms is degraded
                result["status"] = "degraded"
                result["warning"] = "Redis response time is slow"
            elif result.get("memory_used_mb", 0) > 1024:  ***REMOVED*** > 1GB is warning
            ...
            ***REMOVED*** Clean up Redis connection
    ...
    async def _perform_check(self) -> dict[str, Any]:
        ...
        """
        try:
            ***REMOVED*** Create Redis client
            redis_url = settings.redis_url_with_password
            self._redis_client = redis.from_url(redis_url, decode_responses=True)

            ***REMOVED*** 1. Test basic connectivity with PING
            ...
            ***REMOVED*** 2. Get server info
            info = await self._redis_client.info()

            ***REMOVED*** 3. Get memory statistics
            memory_used_bytes = info.get("used_memory", 0)
            memory_used_mb = memory_used_bytes / (1024 * 1024)

            ***REMOVED*** 4. Get connection statistics
            connected_clients = info.get("connected_clients", 0)

            ***REMOVED*** 5. Get key statistics
            keys_total = 0
            try:
                ***REMOVED*** Get dbsize (total keys across all databases)
                keys_total = await self._redis_client.dbsize()
            except Exception as e:
                logger.warning(f"Could not get key count: {e}")

            ***REMOVED*** 6. Get Redis version
    ...
    async def check_read_write(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Write test key
            test_key = "health_check:test"
            test_value = f"test_{int(time.time())}"

            await client.set(test_key, test_value, ex=60)  ***REMOVED*** Expire in 60 seconds
