...
class CircuitBreakerHealthCheck:
    ...
    async def _perform_check(self) -> dict[str, Any]:
        ...
        """
        try:
            ***REMOVED*** Get the global circuit breaker registry
            registry = get_registry()

            ***REMOVED*** Get health check data from registry
            health_data = registry.health_check()

            ***REMOVED*** Determine overall health status
            status = self._determine_status(health_data)

            ***REMOVED*** Build detailed breaker information
            ...
            ***REMOVED*** Add warning if applicable
            if status == "degraded":
    ...
    def _determine_status(self, health_data: dict[str, Any]) -> str:
        ...
        ***REMOVED*** If no breakers registered, that's healthy (no problems)
        if total_breakers == 0:
            return "healthy"

        ***REMOVED*** Check for critical breakers being open
        critical_breakers = {"database", "redis", "external_api"}
        open_critical = set(open_breaker_names) & critical_breakers

        ***REMOVED*** If all breakers are open, system is unhealthy
        if open_breakers == total_breakers:
            return "unhealthy"

        ***REMOVED*** If any critical breaker is open, system is unhealthy
        if open_critical:
            return "unhealthy"

        ***REMOVED*** If some breakers are open or half-open, system is degraded
        if open_breakers > 0 or half_open_breakers > 0:
            return "degraded"

        ***REMOVED*** All breakers are closed
        return "healthy"
    ...
    def _get_breaker_details(self, registry) -> dict[str, Any]:
        ...
        ***REMOVED*** Simplify the breaker details for health check response
        breaker_details = {}
