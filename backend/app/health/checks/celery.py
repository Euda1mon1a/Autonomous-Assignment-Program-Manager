...
class CeleryHealthCheck:
    ...
    async def check(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Determine status based on worker availability
            workers_online = result.get("workers_online", 0)
            if workers_online == 0:
                result["status"] = "unhealthy"
                result["error"] = "No Celery workers are online"
            elif response_time_ms > 2000:  ***REMOVED*** > 2 seconds is degraded
    ...
    def _get_queue_stats(self) -> dict[str, Any]:
        ...
        """
        try:
            ***REMOVED*** Get queue names from celery app config
            queues = ["default", "resilience", "notifications", "metrics"]

            queue_info = {}
            for queue_name in queues:
                queue_info[queue_name] = {
                    "exists": True,  ***REMOVED*** Assume exists if configured
                }
    ...
    async def check_scheduled_tasks(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Get registered periodic tasks from beat schedule
    ...
    async def check_queue_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            ***REMOVED*** Get active queues from workers
