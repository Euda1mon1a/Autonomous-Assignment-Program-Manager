...
***REMOVED*** Feature flag key for swap marketplace access
...
def _check_marketplace_access(db: Session, user: User) -> bool:
    ...
    """
    ***REMOVED*** Query the feature flag
    flag = (
    ...
    ***REMOVED*** If flag doesn't exist, default to allowing access for non-residents
    ***REMOVED*** This maintains backward compatibility while still blocking residents by default
    if flag is None:
        ***REMOVED*** Default behavior: residents are blocked, others allowed
        return user.role != "resident"

    ***REMOVED*** Convert flag to dict for evaluator
    ***REMOVED*** IMPORTANT: Preserve empty lists vs None semantics:
    ***REMOVED*** - None means "no targeting" (allow everyone)
    ***REMOVED*** - [] means "target nobody" (allow no one)
    flag_data = {
        "key": flag.key,
    ...
    ***REMOVED*** Evaluate flag for this user, respecting environment targeting
...
async def get_my_schedule(
    ...
    """
    ***REMOVED*** Get faculty profile linked to current user
    faculty = _get_faculty_for_user(db, current_user)

    ***REMOVED*** Query actual FMIT weeks from schedule
    fmit_template = (
    ...
    ***REMOVED*** Get target weeks from preferences or use default
    preferences = (
...
async def get_my_swaps(
    ...
    ***REMOVED*** Recent completed swaps: executed, approved, or rejected in the last 30 days
    ...
    ***REMOVED*** Convert to SwapRequestSummary
    ...
    ***REMOVED*** For recent swaps, determine if incoming or outgoing
...
async def create_swap_request(
    ...
    """
    ***REMOVED*** Check marketplace access if posting to marketplace (no target specified)
    ...
    ***REMOVED*** Implement swap request creation
    ***REMOVED*** 1. Verify week is assigned to this faculty
    fmit_template = (
        (await db.execute(select(RotationTemplate).where(RotationTemplate.name == "FMIT"))).scalar_one_or_none()
    )
    ...
    ***REMOVED*** Get the week start (Monday)
    week_start = _get_week_start(request.week_to_offload)
    week_end = week_start + timedelta(days=6)

    ***REMOVED*** Verify this faculty has FMIT assignments for this week
    assignments = (
        db.query(Assignment)
        .join(Block, Assignment.block_id == Block.id)
        .options(
    ...
    ***REMOVED*** Check if there's already a pending swap for this week
    existing_swap = (
        db.query(SwapRecord)
        .options(
            joinedload(SwapRecord.source_faculty), joinedload(SwapRecord.target_faculty)
    ...
    ***REMOVED*** 2. Create SwapRecord with PENDING status
    ***REMOVED*** Determine target faculty and swap type
    if request.preferred_target_faculty_id:
        target_faculty_id = request.preferred_target_faculty_id
        swap_type = SwapType.ONE_TO_ONE
    else:
        ***REMOVED*** For marketplace/open requests, use a placeholder or the requesting faculty
        ***REMOVED*** The actual target will be determined when someone accepts the request
    ...
    swap_record = SwapRecord(
        ...
        target_week=None,  ***REMOVED*** Will be filled in when accepted
        swap_type=swap_type,
    ...
    ***REMOVED*** 3. If auto_find_candidates, find and notify potential swap partners
    candidates_notified = 0
    if request.auto_find_candidates and not request.preferred_target_faculty_id:
        ***REMOVED*** Find faculty who:
        ***REMOVED*** - Are not this faculty
        ***REMOVED*** - Have FMIT as their rotation type
        ***REMOVED*** - Have notification preferences enabled
        preferences = (
            db.query(FacultyPreference)
        ...
        ***REMOVED*** Create notifications for candidates
        ...
        ***REMOVED*** Send all pending notifications
...
async def respond_to_swap(
    ...
    ***REMOVED*** Implement swap response
    ***REMOVED*** Query the swap record
    swap = (
        db.query(SwapRecord)
        .options(
            joinedload(SwapRecord.source_faculty), joinedload(SwapRecord.target_faculty)
    ...
    ***REMOVED*** Verify this faculty is the target of the swap
    ...
    ***REMOVED*** Verify swap is still pending
    ...
    if request.accept:
        ...
        ***REMOVED*** Reject the swap
        swap.status = SwapStatus.REJECTED
        swap.approved_at = datetime.utcnow()  ***REMOVED*** Track when rejected
        swap.approved_by_id = current_user.id
...
async def get_my_preferences(
    ...
    ***REMOVED*** Query FacultyPreference for this faculty
    preferences = (
        db.query(FacultyPreference)
        .options(joinedload(FacultyPreference.faculty))
        .filter(FacultyPreference.faculty_id == faculty.id)
    ...
    ***REMOVED*** Return defaults if no preferences exist
    if not preferences:
    ...
    ***REMOVED*** Convert JSON date strings to date objects
    preferred_weeks = []
...
async def update_my_preferences(
    ...
    ***REMOVED*** Update FacultyPreference
    ***REMOVED*** Query existing preferences or create new
    preferences = (
        db.query(FacultyPreference)
        .options(joinedload(FacultyPreference.faculty))
        .filter(FacultyPreference.faculty_id == faculty.id)
    ...
    if not preferences:
        ***REMOVED*** Create new preferences record
    ...
    ***REMOVED*** Update fields from request
    if request.preferred_weeks is not None:
        ***REMOVED*** Convert date objects to ISO format strings
    ...
    ***REMOVED*** Convert back for response
    preferred_weeks = []
    if preferences.preferred_weeks:
        for week_str in preferences.preferred_weeks:
            try:
...
async def get_my_dashboard(
    ...
    ***REMOVED*** Initialize counters
    ...
    if fmit_template:
        ...
        ***REMOVED*** Count completed vs remaining weeks
        ...
        ***REMOVED*** Get upcoming weeks (next 8 weeks)
        eight_weeks_out = today + timedelta(weeks=8)
        for week_start, _week_assignments in sorted(week_map.items()):
            week_end = week_start + timedelta(days=6)

            ***REMOVED*** Only include weeks starting from today up to 8 weeks out
            if week_start >= today and week_start <= eight_weeks_out:
                ...
                ***REMOVED*** Check for pending swaps
                pending_swap = (
                    db.query(SwapRecord)
                    .filter(
                        SwapRecord.source_faculty_id == faculty.id,
                ...
                upcoming_weeks.append(
                    FMITWeekInfo(
                        week_start=week_start,
                        week_end=week_end,
                        is_past=False,  ***REMOVED*** All upcoming weeks are not past
                        has_conflict=has_conflict,
                        conflict_description=conflict_description,
    ...
    ***REMOVED*** Get target weeks from preferences
    preferences = (
        db.query(FacultyPreference)
        .filter(FacultyPreference.faculty_id == faculty.id)
        .first()
    ...
    ***REMOVED*** Count pending swap requests (incoming)
    ...
    ***REMOVED*** Count unread alerts (NEW status only)
    unread_alerts_count = (
        db.query(ConflictAlert)
        .filter(
            ConflictAlert.faculty_id == faculty.id,
    ...
    ***REMOVED*** Get recent conflict alerts (last 30 days, NEW or ACKNOWLEDGED)
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)
    recent_alerts_query = (
        db.query(ConflictAlert)
        .filter(
    ...
    ***REMOVED*** Convert to DashboardAlert
    recent_alerts = []
    for alert in recent_alerts_query:
        ***REMOVED*** Determine severity based on alert type
        severity = "warning"  ***REMOVED*** Default
        if hasattr(alert, "severity") and alert.severity:
            severity = alert.severity
        elif "critical" in alert.description.lower() if alert.description else False:
            severity = "critical"

        ***REMOVED*** Determine action URL - link to schedule view with week filter
        action_url = None
        if alert.fmit_week:
    ...
    ***REMOVED*** Get pending swap decisions (incoming swaps requiring response)
    incoming_swaps = (
        db.query(SwapRecord)
        .options(joinedload(SwapRecord.source_faculty))
        .filter(
    ...
    for swap in incoming_swaps:
        pending_swap_decisions.append(
            SwapRequestSummary(
                ...
                week_to_give=swap.target_week,  ***REMOVED*** What they want from us
                week_to_receive=swap.source_week,  ***REMOVED*** What we would get
                status=swap.status.value,
                created_at=swap.requested_at,
                is_incoming=True,
            )
...
async def get_swap_marketplace(
    ...
    Business Logic:
        ...
        - Marketplace access controlled by 'swap_marketplace_enabled' feature flag
          (disabled for residents by default to prevent gamification)
    """
    ***REMOVED*** Check marketplace access before proceeding
    if not _check_marketplace_access(db, current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    ***REMOVED*** Eager load source_faculty to avoid N+1 when building entries
    open_swaps = (
        ...
        .filter(
            ...
            != faculty.id,  ***REMOVED*** Exclude current user's own requests
        )
        .order_by(SwapRecord.requested_at.desc())
        .all()
    )

    ***REMOVED*** Count user's own postings
    my_postings = (
        db.query(SwapRecord)
        .filter(
            SwapRecord.source_faculty_id == faculty.id,
    ...
    ***REMOVED*** Get faculty's current FMIT schedule to check compatibility
    fmit_template = (
        (await db.execute(select(RotationTemplate).where(RotationTemplate.name == "FMIT"))).scalar_one_or_none()
    )
    ...
    if fmit_template:
        ...
        ***REMOVED*** Get all weeks this faculty is already scheduled
        for assignment in assignments:
            week_start = _get_week_start(assignment.block.date)
            faculty_scheduled_weeks.add(week_start)

    ***REMOVED*** Convert to MarketplaceEntry
    entries = []
    for swap in open_swaps:
        ***REMOVED*** Check if compatible (viewer is not already scheduled that week)
        is_compatible = swap.source_week not in faculty_scheduled_weeks

        ***REMOVED*** Calculate expiration (e.g., 30 days from posting)
...
def _get_faculty_for_user(db: Session, user: User) -> Person:
    ...
    """
    ***REMOVED*** First try to find by email match
    faculty = (
        db.query(Person)
        .filter(
...
def _get_week_start(any_date):
    ...
    ***REMOVED*** Ensure we have a date object
    if isinstance(any_date, datetime):
        any_date = any_date.date()

    ***REMOVED*** Monday is 0, Sunday is 6
