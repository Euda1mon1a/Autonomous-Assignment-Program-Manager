...
***REMOVED*** Initialize changelog generator
***REMOVED*** In production, configure storage path via environment variable
...
async def generate_changelog(request: ChangelogRequest) -> ChangelogResponse:
    ...
    Example Response:
        {
            "changelog": "***REMOVED*** API Changelog...",
            "format": "markdown",
            "old_version": "1.0.0",
            "new_version": "1.1.0",
    ...
    """
    ***REMOVED*** Validate output format
    try:
...
async def generate_migration_guide(
    ...
    Example Response:
        {
            "migration_guide": "***REMOVED*** Migration Guide\\n\\n...",
            "old_version": "1.0.0",
            "new_version": "2.0.0",
    ...
    ***REMOVED*** Count breaking changes
    diff = changelog_generator.get_diff(
        old_data["schema"],
        new_data["schema"],
    )
...
async def suggest_version(
    ...
    ***REMOVED*** Get detailed change counts
    diff = changelog_generator.get_diff(
        old_data["schema"],
        new_data["schema"],
    )
...
async def get_current_schema(request: Request) -> dict[str, Any]:
    ...
    """
    ***REMOVED*** Get current OpenAPI schema from the FastAPI app
    app = request.app
    return app.openapi()
