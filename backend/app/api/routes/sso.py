...
def validate_redirect_url(url: str | None) -> str:
    ...
    ***REMOVED*** Allow relative URLs (safe - same-origin)
    ***REMOVED*** Block protocol-relative URLs (//) which could redirect to external sites
    if url.startswith("/") and not url.startswith("//"):
        return url

    ***REMOVED*** Validate absolute URLs against whitelist
    try:
        parsed = urllib.parse.urlparse(url)
        if parsed.netloc:
            ***REMOVED*** Has a hostname - must be in whitelist
            if parsed.netloc not in ALLOWED_REDIRECT_HOSTS:
                logger.warning(
                    f"SECURITY: Blocked open redirect attempt to: {parsed.netloc}"
                )
                return "/dashboard"
        ***REMOVED*** No netloc but has scheme (e.g., javascript:) - block
        elif parsed.scheme:
...
***REMOVED*** In-memory session store (use Redis in production)
...
def get_or_create_user(db: Session, attributes: dict[str, str], provider: str) -> User:
    ...
    ***REMOVED*** Use email as username if username not provided
    if not username:
        username = email.split("@")[0]

    ***REMOVED*** Check if user exists by email
    user = db.query(User).filter(User.email == email).first()

    if user:
        ***REMOVED*** Update last login
    ...
    ***REMOVED*** Auto-provision user if enabled
    ...
    ***REMOVED*** Check if username is already taken
    existing_username = db.query(User).filter(User.username == username).first()
    if existing_username:
        ***REMOVED*** Append random suffix to username
        import uuid

        username = f"{username}_{uuid.uuid4().hex[:8]}"

    ***REMOVED*** Extract role from attributes or use default
    role = attributes.get("role", sso_config.default_role)

    ***REMOVED*** Validate role
    valid_roles = {
        "admin",
        "coordinator",
        "faculty",
...
def create_session(response: Response, user: User) -> dict:
    ...
    ***REMOVED*** Set httpOnly cookie for XSS protection
    response.set_cookie(
        key="access_token",
...
async def saml_acs(
    ...
    ***REMOVED*** Get SAML response from form data
    ...
    ***REMOVED*** Parse and validate SAML response
    provider = SAMLProvider(sso_config.saml)
    ...
    ***REMOVED*** Extract user attributes
    attributes = saml_data["attributes"]

    ***REMOVED*** Get or create user
...
async def oauth2_callback(
    ...
    ***REMOVED*** Validate state parameter (CSRF protection)
    ...
    ***REMOVED*** Exchange code for tokens
    try:
    ...
    ***REMOVED*** Validate ID token if present (OIDC)
    ...
    ***REMOVED*** Clean up session state
