...
@router.get(
    "/unpaired-half-blocks",
    summary="Detect unpaired half-block assignments",
    description="""
    Find block assignments that use a half-block rotation template
    (is_block_half_rotation=True) but have no secondary_rotation_template_id set.

    These are incomplete split rotations — the resident has a rotation for only
    half the block. Common example: NF-AM assigned but no pairing for weeks 3-4.

    **Parameters:**
    - academic_year: Required
    - block_number: Optional — omit to scan all blocks
    """,
)
async def get_unpaired_half_blocks(
    academic_year: int,
    db: Annotated[AsyncSession, Depends(get_async_db)],
    current_user: Annotated[User, Depends(get_admin_user)],
    block_number: int | None = None,
) -> list[dict]:
    """Return unpaired half-block assignments."""
    unpaired = await detect_unpaired_half_blocks(db, academic_year, block_number)

    return [
        {
            "assignment_id": str(u.assignment_id),
            "block_number": u.block_number,
            "academic_year": u.academic_year,
            "resident_id": str(u.resident_id),
            "resident_name": u.resident_name,
            "rotation_template_id": str(u.rotation_template_id)
            if u.rotation_template_id
            else None,
            "rotation_abbreviation": u.rotation_abbreviation,
            "rotation_name": u.rotation_name,
        }
        for u in unpaired
    ]
...
@router.post(
    "/resolve-splits",
    summary="Resolve unpaired half-block split assignments",
    description="""
    Fix unpaired half-block assignments by pairing them with a second rotation.

    Each override maps an assignment_id to a target rotation template abbreviation:
    - If the target is a combined template (full-block), it replaces the primary rotation
    - If the target is another half-block template, it sets the secondary

    **dry_run=true** (default): Returns what WOULD change without modifying DB.
    Set dry_run=false to apply changes.
    """,
)
async def resolve_splits(
    data: dict,
    db: Annotated[AsyncSession, Depends(get_async_db)],
    current_user: Annotated[User, Depends(get_admin_user)],
) -> dict:
    """Resolve split assignments with user-provided overrides."""
    academic_year = data.get("academic_year")
    block_number = data.get("block_number")
    overrides = data.get("overrides", {})
    dry_run = data.get("dry_run", True)

    if not academic_year or not block_number:
        raise HTTPException(
            status_code=400,
            detail="academic_year and block_number are required",
        )
        ...
        "actions": [
            {
                ...
                "new_primary_template": a.new_primary_template,
                "new_secondary_template": a.new_secondary_template,
                "source": a.source,
            }
            for a in actions
        ],
        "resolved_count": sum(1 for a in actions if a.source == "override"),
        "skipped_count": sum(1 for a in actions if a.source == "skipped"),
    }
