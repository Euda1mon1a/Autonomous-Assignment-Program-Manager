...
async def parse_xlsx_file(
    ...
    ***REMOVED*** Validate file extension
    ...
    ***REMOVED*** Check file size (10MB limit)
    max_size = 10 * 1024 * 1024  ***REMOVED*** 10MB
    ...
    ***REMOVED*** Validate file security
    try:
    ...
    ***REMOVED*** Extract headers from the specified row
    headers: list[str] = []
    header_row_idx = header_row  ***REMOVED*** 1-indexed in openpyxl

    try:
        for cell in ws[header_row_idx]:
            if isinstance(cell, MergedCell):
                ***REMOVED*** For merged cells, we'll use empty string (merged headers are unusual)
                headers.append("")
                warnings.append(
    ...
    ***REMOVED*** Remove trailing empty columns
    ...
    ***REMOVED*** Check for duplicate headers
    seen_headers: set[str] = set()
    for i, h in enumerate(headers):
        if h in seen_headers:
            ***REMOVED*** Make unique by appending index
    ...
    try:
        for row_idx, row in enumerate(
            ...
            for col_idx, cell in enumerate(row):
                ...
                if isinstance(cell, MergedCell):
                    ***REMOVED*** Skip merged cells in data (they're usually formatting)
                    value = None
                else:
                    value = cell.value

                ***REMOVED*** Convert value to appropriate type
                if value is not None:
                    is_empty = False

                    ***REMOVED*** Handle dates
                    if hasattr(value, "isoformat"):
                        value = value.isoformat()
                    ***REMOVED*** Handle booleans
                    elif isinstance(value, bool):
                        value = value
                    ***REMOVED*** Handle numbers
                    elif isinstance(value, (int, float)):
                        ***REMOVED*** Keep numeric types
                        value = value
                    ***REMOVED*** Handle strings
                    else:
                        value = str(value).strip()
                        ***REMOVED*** Check for boolean-like strings
            ...
            ***REMOVED*** Skip empty rows if configured
            if is_empty:
                empty_row_count += 1
                if skip_empty_rows:
                    continue
