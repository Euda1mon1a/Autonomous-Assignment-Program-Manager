...
def get_daily_manifest(
    ...
    """
    ***REMOVED*** Validate time_of_day if provided
    ...
    ***REMOVED*** Query assignments for the given date
    query = (
        db.query(Assignment)
    ...
    ***REMOVED*** Filter by time_of_day if specified
    if time_of_day:
        query = query.filter(Block.time_of_day == time_of_day)

    ***REMOVED*** Order by time of day and person name for consistent display
    query = query.order_by(Block.time_of_day, Person.name)

    assignments = query.all()

    ***REMOVED*** Group assignments by location and time slot
    location_data = defaultdict(lambda: {"AM": [], "PM": []})

    for assignment in assignments:
        ***REMOVED*** Get clinic location from rotation template
        ...
        ***REMOVED*** Get time slot from block
        time_slot = assignment.block.time_of_day

        ***REMOVED*** Create person summary
        ...
        ***REMOVED*** Create assignment summary
        ...
        ***REMOVED*** Add to location data
        location_data[clinic_location][time_slot].append(assignment_summary)

    ***REMOVED*** Build location manifests with staffing summaries
    locations = []
    for clinic_location, time_slots in location_data.items():
        ***REMOVED*** Calculate staffing summary across all time slots
        all_assignments = []
    ...
    ***REMOVED*** Sort locations by name for consistent display (Unassigned last)
    locations.sort(
        key=lambda loc: (
