...
def export_calendar_ics(
    ...
    **Response:**
    ...
    """
    try:
        ***REMOVED*** Generate ICS content using the calendar service
        ics_content = CalendarService.generate_ics_for_person(
            db=db,
            person_id=person_id,
            start_date=start_date,
            end_date=end_date,
            include_types=None,  ***REMOVED*** Include all activity types
        )

        ***REMOVED*** Return ICS file as download with proper headers
        return Response(
            content=ics_content,
        ...
        ***REMOVED*** Person not found or invalid data
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        ***REMOVED*** Unexpected error during calendar generation
        raise HTTPException(
            status_code=500,
