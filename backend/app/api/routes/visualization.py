...
async def get_unified_heatmap(
    ...
    ***REMOVED*** Use cached service for better performance
...
async def get_unified_heatmap_with_time_range(
    ...
    ***REMOVED*** Calculate date range from time_range specification
    try:
    ...
    ***REMOVED*** Generate unified heatmap (cached)
    ...
    ***REMOVED*** Add calculated date range to metadata
...
async def get_heatmap_image(
    ...
    ***REMOVED*** Generate heatmap data (cached)
    ...
    ***REMOVED*** Return as streaming response
...
async def export_heatmap(
    ...
    ***REMOVED*** Generate appropriate heatmap based on type (cached)
    try:
...
***REMOVED*** =============================================================================
***REMOVED*** 3D Voxel Visualization Endpoints (Novel)
***REMOVED*** =============================================================================
...
async def get_3d_voxel_grid(
    ...
    ***REMOVED*** Fetch blocks in date range
    blocks_query = (
        select(Block)
        .where(
    ...
    ***REMOVED*** Fetch persons (optionally filtered)
    ...
    ***REMOVED*** Fetch assignments with related data
    assignments_query = (
        select(Assignment)
    ...
    ***REMOVED*** Convert to dict format for transformer
    assignments_data = []
    for a in assignments:
        ...
        if a.rotation_template:
            ...
            ***REMOVED*** Try to infer activity type from override name
        ...
        ***REMOVED*** Filter by activity type if specified
        ...
        ***REMOVED*** Add ACGME warnings if requested
        if include_violations:
            ***REMOVED*** For now, we don't have warnings stored directly
            ***REMOVED*** This would be populated from validation service
            assignment_dict["acgme_warnings"] = []

        assignments_data.append(assignment_dict)

    ***REMOVED*** Transform to voxel grid
...
async def get_3d_conflicts(
    ...
    """
    ***REMOVED*** Get full voxel grid first
...
async def get_3d_coverage_gaps(
    ...
    ***REMOVED*** Build occupancy map
    occupied = set()
    for voxel in grid_response.get("voxels", []):
        pos = voxel.get("position", {})
        occupied.add((pos.get("x"), pos.get("y")))

    ***REMOVED*** Find gaps (time slots where no one is assigned)
    gaps = []
    x_labels = dimensions.get("x_labels", [])
    y_labels = dimensions.get("y_labels", [])

    for x in range(x_size):
        ***REMOVED*** Check if any person is assigned at this time
            ...
            gaps.append(
                {
                    ...
                    "coverage_count": 0,
                    "severity": "critical",  ***REMOVED*** No one assigned
                }
            )
        elif coverage_at_time < y_size * 0.2:  ***REMOVED*** Less than 20% coverage
            gaps.append(
                {
                    "x": x,
                    "time_label": x_labels[x] if x < len(x_labels) else f"Block {x}",
