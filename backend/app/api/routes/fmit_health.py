...
class FMITHealthStatus(BaseModel):
    ...
    timestamp: datetime
    status: str  ***REMOVED*** "healthy", "degraded", "critical"
...
class CoverageGapsResponse(BaseModel):
    ...
    gaps_by_period: dict[str, int]  ***REMOVED*** "daily", "weekly", "monthly"
...
class CoverageSuggestion(BaseModel):
    ...
    gap_id: str
    suggestion_type: str  ***REMOVED*** "assign_available", "swap_recommended", "overtime"
    priority: int  ***REMOVED*** 1-5, 1 being highest
...
class CoverageForecast(BaseModel):
    ...
    predicted_gaps: int
    confidence_level: float  ***REMOVED*** 0-1
...
def calculate_coverage_percentage(db: Session, start_date: date, end_date: date) -> float:
    ...
    ***REMOVED*** Get all FMIT blocks in the date range
    total_blocks = db.query(Block).filter(
        and_(
            Block.date >= start_date,
            Block.date <= end_date,
            Block.service_type == "FMIT"  ***REMOVED*** Assuming FMIT blocks have this service_type
        )
    ).count()
    ...
    ***REMOVED*** Get covered blocks (those with assignments)
...
def detect_coverage_gaps(db: Session, start_date: date, end_date: date) -> list[CoverageGap]:
    ...
    ***REMOVED*** Get all FMIT blocks in range
    ***REMOVED*** Eager load assignments and rotation_template to avoid N+1 queries
    ...
    ***REMOVED*** Build a map of block_id -> assignments to avoid N+1
    ...
    for block in blocks:
        ***REMOVED*** Get assignments for this block from our map
        assignments = assignments_by_block.get(block.id, [])

        current_count = len(assignments)
        required_count = 1  ***REMOVED*** Minimum required for FMIT coverage
        ...
        if current_count < required_count:
            ...
            ***REMOVED*** Determine affected area based on rotation template
            affected_area = "FMIT"
            department = None
...
def find_available_faculty(db: Session, target_date: date, time_of_day: str) -> list[str]:
    ...
    ***REMOVED*** Get all faculty (people with faculty role)
    ...
    ***REMOVED*** Find block for target date/time
    ...
    for faculty in all_faculty:
        ***REMOVED*** Check if faculty already has assignment on this block
        ...
        if not existing:
            ***REMOVED*** Check for conflicts (assignments on other blocks at same time)
...
def calculate_conflict_score(db: Session, faculty_id: str, target_date: date) -> float:
    ...
    ***REMOVED*** Check assignments in surrounding dates (workload balance)
    ...
    ***REMOVED*** More assignments = higher score
    score += min(week_assignments / 10.0, 0.5)

    ***REMOVED*** Check for adjacent day assignments (prefer spacing)
    ...
    ***REMOVED*** Check for active alerts
...
def analyze_coverage_trend(db: Session, weeks_back: int = 12) -> str:
    ...
    ***REMOVED*** Calculate trend using simple linear regression
...
async def get_fmit_health(
    ...
    ***REMOVED*** Calculate current month range
    today = date.today()
    ...
    ***REMOVED*** Coverage for next 30 days
    coverage = calculate_coverage_percentage(db, today, today + timedelta(days=30))

    ***REMOVED*** Determine overall status
    status = get_health_status(db)

    ***REMOVED*** Build issues and recommendations
    issues = []
    recommendations = []
...
async def get_fmit_detailed_status(
    ...
    ***REMOVED*** Recent swap activity (last 10)
...
async def get_fmit_metrics(
    ...
    ***REMOVED*** Average processing time (for executed swaps)
    ...
    ***REMOVED*** Active alerts
    active_alerts = db.query(ConflictAlert).filter(
        ConflictAlert.status.in_([ConflictAlertStatus.NEW, ConflictAlertStatus.ACKNOWLEDGED])
    ).count()

    ***REMOVED*** Alert resolution rate
...
async def get_coverage_report(
    ...
    - Faculty assigned
    """
    ***REMOVED*** Default date range
    if start_date is None:
        start_date = date.today()
    if end_date is None:
        end_date = start_date + timedelta(days=30)

    ***REMOVED*** Calculate overall coverage
    overall_coverage = calculate_coverage_percentage(db, start_date, end_date)

    ***REMOVED*** Build report based on period
    weeks = []
    current = start_date

    ***REMOVED*** Determine period increment
    if period == "daily":
        increment = 1
    elif period == "monthly":
        increment = 30
    else:  ***REMOVED*** weekly (default)
        increment = 7

    while current <= end_date:
        if period == "monthly":
            ***REMOVED*** For monthly, use calendar month boundaries
            if current.month == 12:
        ...
        ***REMOVED*** Get FMIT blocks for this period
        ...
        ***REMOVED*** Eager load assignments and person to avoid N+1 queries
        covered_count = 0
        faculty_names = set()

        if week_blocks:
            block_ids = [b.id for b in week_blocks]
            ***REMOVED*** Load all assignments with person eager-loaded for this period's blocks
            week_assignments = (
                db.query(Assignment)
            ...
            ***REMOVED*** Build map of block_id -> assignments
            ...
            ***REMOVED*** Count coverage and collect faculty names
        ...
        if period == "monthly":
            ***REMOVED*** Move to first day of next month
            if current.month == 12:
    ...
    for gap in gaps[:max_suggestions]:
        ***REMOVED*** Find available faculty for this slot
        ...
        if not available_faculty:
            ...
            ***REMOVED*** Calculate conflict scores for available faculty
            faculty_scores = []
            for faculty_name in available_faculty[:5]:  ***REMOVED*** Top 5 candidates
                ***REMOVED*** Get faculty person object
            ...
            ***REMOVED*** Sort by score (lower is better)
...
async def get_coverage_forecast(
    ...
    ***REMOVED*** Limit weeks ahead to reasonable range
    weeks_ahead = min(max(weeks_ahead, 1), 52)

    ***REMOVED*** Analyze historical trend
    overall_trend = analyze_coverage_trend(db, weeks_back=12)

    ***REMOVED*** Calculate baseline coverage from recent weeks
    recent_coverage = []
    for i in range(4):
    ...
    ***REMOVED*** Generate forecasts
    ...
    for week_num in range(weeks_ahead):
        ...
        ***REMOVED*** Adjust prediction based on trend
        ...
        ***REMOVED*** Confidence decreases with time
        confidence = max(0.9 - (week_num * 0.02), 0.5)

        ***REMOVED*** Calculate predicted gaps based on expected blocks
        expected_blocks_per_week = 10  ***REMOVED*** Assumption
        predicted_gaps = int(expected_blocks_per_week * (100 - predicted_coverage) / 100)

        ***REMOVED*** Identify risk factors
        risk_factors = []

        if predicted_coverage < 85:
            risk_factors.append("Below target coverage threshold (85%)")

        ***REMOVED*** Check for upcoming holidays or known conflicts
        ...
        ***REMOVED*** Check pending swaps affecting this period
        ...
        ***REMOVED*** Determine period trend
        if week_num == 0:
...
async def get_alert_summary(
    ...
    ***REMOVED*** Counts by severity (active only)
    ...
    ***REMOVED*** Counts by type (active only)
    by_type = {}
    ...
    ***REMOVED*** Average resolution time (for resolved alerts in last 30 days)
