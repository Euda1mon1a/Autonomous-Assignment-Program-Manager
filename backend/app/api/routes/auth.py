...
***REMOVED*** Rate limiting dependencies
...
async def login(
    ...
    ***REMOVED*** Set access token as httpOnly cookie for XSS protection
    response.set_cookie(
        key="access_token",
        value=f"Bearer {token_response.access_token}",
        httponly=True,
        secure=not settings.DEBUG,  ***REMOVED*** Secure in production
        samesite="lax",
        max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60,
        path="/",
    )

    ***REMOVED*** Decode access token to get user info for refresh token
    payload = jwt.decode(
    ...
    ***REMOVED*** Generate refresh token with user data
...
async def logout(
    ...
    if token:
        try:
            ***REMOVED*** Decode token to get jti and expiration
            payload = jwt.decode(token, settings.SECRET_KEY, algorithms=[ALGORITHM])
            jti = payload.get("jti")
            exp = payload.get("exp")

            if jti and exp:
                ***REMOVED*** Convert exp to datetime
                expires_at = datetime.utcfromtimestamp(exp)

                ***REMOVED*** Add to blacklist
                blacklist_token(
                    db=db,
                    jti=jti,
            ...
            pass  ***REMOVED*** Token was invalid anyway, no need to blacklist

    ***REMOVED*** Delete the httpOnly cookie
...
async def refresh_token(
    ...
    ***REMOVED*** Verify refresh token and optionally blacklist it if rotation is enabled
    ***REMOVED*** CRITICAL: blacklist_on_use=True ensures the old token cannot be reused
    ...
    ***REMOVED*** Verify user still exists and is active
    ...
    ***REMOVED*** Create new access token
    ...
    ***REMOVED*** Set new access token as httpOnly cookie
    response.set_cookie(
        key="access_token",
    ...
    ***REMOVED*** Create new refresh token if rotation is enabled
    ***REMOVED*** The old token was already blacklisted in verify_refresh_token
    if settings.REFRESH_TOKEN_ROTATE:
        new_refresh_token, _, _ = create_refresh_token(
            data={"sub": str(user.id), "username": user.username}
        )
    else:
        ***REMOVED*** Return the same refresh token if rotation is disabled
