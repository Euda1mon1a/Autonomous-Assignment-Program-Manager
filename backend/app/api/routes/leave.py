...
async def verify_webhook_signature(
    ...
    ***REMOVED*** Check required headers
    ...
    ***REMOVED*** Validate timestamp to prevent replay attacks
    try:
    ...
    ***REMOVED*** Read the raw body for signature verification
    body = await request.body()

    ***REMOVED*** Compute expected HMAC signature
    ***REMOVED*** Format: HMAC-SHA256(webhook_secret, timestamp + "." + body)
    ...
    ***REMOVED*** Compare signatures using constant-time comparison to prevent timing attacks
...
def create_leave(
    ...
    ***REMOVED*** Verify faculty exists
    person = db.query(Person).filter(Person.id == request.faculty_id).first()
    if not person:
        raise HTTPException(
...
def update_leave(
    ...
    absence = db.query(Absence).filter(Absence.id == leave_id).first()
    if not absence:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def leave_webhook(
    ...
    if payload.event_type == "created":
        ***REMOVED*** Handle new leave
        pass
    elif payload.event_type == "updated":
        ***REMOVED*** Handle leave update
        pass
    elif payload.event_type == "deleted":
        ***REMOVED*** Handle leave deletion
        pass
...
def bulk_import_leave(
    ...
    for record in request.records:
        try:
            ***REMOVED*** Check for duplicates if skip_duplicates is enabled
