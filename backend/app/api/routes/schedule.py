...
async def generate_schedule(
    ...
    ***REMOVED*** Initialize idempotency service
    idempotency_service = IdempotencyService(db)
    idempotency_request = None

    ***REMOVED*** Extract request parameters for hashing
    request_params = {
    ...
    ***REMOVED*** If idempotency key provided, check for existing request
    if idempotency_key:
        body_hash = idempotency_service.compute_body_hash(
            extract_idempotency_params(request_params)
        )

        ***REMOVED*** Check for key conflict (same key, different body)
        ...
        ***REMOVED*** Check for existing request with same key and body
        existing = idempotency_service.get_existing_request(idempotency_key, body_hash)
        if existing:
            if existing.is_pending:
                ***REMOVED*** Request is still being processed
                if obs_metrics:
                ...
                ***REMOVED*** Return cached response
                if obs_metrics:
                ...
                ***REMOVED*** Return cached error response
                if obs_metrics:
                    obs_metrics.record_idempotency_hit()
                logger.info(
        ...
        ***REMOVED*** Create new idempotency request record (cache miss)
        if obs_metrics:
        ...
        try:
            ...
            ***REMOVED*** Continue without idempotency tracking

    ***REMOVED*** Issue ***REMOVED***1: Double-submit / Re-entrancy protection
    ***REMOVED*** Check for in-progress generations for overlapping date ranges
    ...
    in_progress_run = (
        ...
        .filter(
            ...
            ***REMOVED*** Check for overlapping date ranges
    ...
    try:
        ...
        ***REMOVED*** Generate schedule with selected algorithm (timed for metrics)
        if obs_metrics:
        ...
        ***REMOVED*** Build solver statistics if available
        solver_stats = None
        ...
        ***REMOVED*** Issue ***REMOVED***5: Partial success semantics - use proper HTTP status codes
        ***REMOVED*** Return 207 Multi-Status for partial success (some assignments created but with violations)
        ***REMOVED*** Return 422 for validation errors or complete failure
        if result["status"] == "failed":
            ...
            ***REMOVED*** Use 207 Multi-Status for partial success
            if obs_metrics:
        ...
        ***REMOVED*** Success - record metrics and cache the response
        if obs_metrics:
            obs_metrics.record_schedule_success(
                algorithm, result.get("total_assigned", 0)
            )
        ...
        ***REMOVED*** Re-raise HTTP exceptions
        raise
    except Exception as e:
        if obs_metrics:
            obs_metrics.record_schedule_failure(algorithm)
...
async def get_schedule(start_date: str, end_date: str, db: AsyncSession = Depends(get_async_db)):
    ...
    ***REMOVED*** Group by date for calendar view
    schedule_by_date = {}
...
async def analyze_imported_schedules(
    ...
    ***REMOVED*** Parse specialty providers if provided
    specialty_map = None
    ...
    ***REMOVED*** Read file contents
    try:
        fmit_bytes = fmit_file.file.read()
    except Exception:
        raise HTTPException(status_code=400, detail="Failed to read uploaded file")

    ***REMOVED*** Validate FMIT file
    try:
...
async def analyze_single_file(
    ...
    ***REMOVED*** Check for alternating patterns
    ...
    ***REMOVED*** Build provider summary
    provider_summary = []
    for provider_name, schedule in result.providers.items():
        fmit_slots = sum(
        ...
        ***REMOVED*** Check if specialty provider
        specialties = []
        if specialty_map:
...
async def parse_block_schedule_endpoint(
    ...
    ***REMOVED*** Read and validate file
    try:
        file_bytes = file.file.read()
    except Exception:
        raise HTTPException(status_code=400, detail="Failed to read uploaded file")
    ...
    ***REMOVED*** Parse known people if provided
    people_list = None
    if known_people:
        try:
    ...
    ***REMOVED*** Save to temp file (openpyxl needs file path)
    ...
    ***REMOVED*** Parse FMIT if requested
    fmit_schedule = []
    if include_fmit:
        try:
            with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
            ...
            pass  ***REMOVED*** FMIT sheet not found, that's OK

    ***REMOVED*** Convert to response schema
...
async def find_swap_candidates(
    ...
    ***REMOVED*** Build faculty targets dict
    faculty_targets = {
    ...
    ***REMOVED*** Add absence-based conflicts if requested
    if request.include_absence_conflicts:
        try:
            ...
            ***REMOVED*** Log but don't fail - absence integration is optional
            logger.warning(f"Failed to load absence conflicts: {e}")

    try:
        ***REMOVED*** Create SwapFinder from file
        swap_finder = SwapFinder.from_fmit_file(
            ...
            include_absence_conflicts=False,  ***REMOVED*** Already loaded above
    ...
    ***REMOVED*** Validate target faculty exists in schedule
    ...
    ***REMOVED*** Find alternating patterns
...
async def find_swap_candidates_json(
    ...
    ***REMOVED*** Validate person exists
    try:
    ...
    ***REMOVED*** Get the target assignment if specified
    ...
    if request.assignment_id:
        ...
        ***REMOVED*** Find the requester's assignment for this block
        target_assignment = (
            db.query(Assignment)
            .filter(
    ...
    ***REMOVED*** Get future assignments for the requester if no specific target
    if not target_block:
        future_assignments = (
        ...
        ***REMOVED*** Use the first future assignment as target
        target_assignment, target_block = future_assignments[0]

    ***REMOVED*** Find potential swap candidates
    ***REMOVED*** Look for other people with assignments on nearby blocks
    candidates = []

    ***REMOVED*** Get other people's assignments on the same block or nearby
    other_assignments = (
        ...
        .filter(
            Assignment.person_id != person_uuid,
            Person.type == requester.type,  ***REMOVED*** Same type (faculty/resident)
            Block.start_date >= datetime.utcnow().date(),
        )
        .order_by(Block.start_date)
        .limit(100)  ***REMOVED*** Get a pool of candidates
        .all()
    )

    for assignment, block, person, rotation in other_assignments:
        ***REMOVED*** Calculate a simple match score based on various factors
        score = 0.5  ***REMOVED*** Base score

        ***REMOVED*** Boost if same block (direct swap possible)
        if target_block and block.id == target_block.id:
            score += 0.3

        ***REMOVED*** Boost if same rotation type
        if (
            target_assignment
        ...
        ***REMOVED*** Penalize if dates are far apart
        if target_block:
            days_apart = abs((block.start_date - target_block.start_date).days)
            if days_apart <= 7:
                score += 0.1
            elif days_apart <= 28:
                pass  ***REMOVED*** neutral
            else:
                score -= 0.2

        score = max(0.0, min(1.0, score))

        ***REMOVED*** Determine approval likelihood based on score
        if score >= 0.8:
            likelihood = "high"
        elif score >= 0.5:
            likelihood = "medium"
    ...
    ***REMOVED*** Limit to max_candidates
...
***REMOVED*** ============================================================================
***REMOVED*** Faculty Outpatient Assignment Generation
***REMOVED*** ============================================================================
...
async def generate_faculty_outpatient(
    ...
    try:
        ...
        ***REMOVED*** Use repr to avoid format string issues with loguru
