...
# =============================================================================
# Faculty Validation Endpoints
# =============================================================================


@router.get("/faculty/adjunct-gaps", response_model=AdjunctGapsResponse)
def get_adjunct_faculty_gaps(
    block_number: int = Query(..., ge=1, le=13, description="Block number (1-13)"),
    academic_year: int = Query(
        ..., ge=2020, le=2030, description="Academic year (e.g., 2025)"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """
    DEPRECATED: Faculty expansion service was archived.

    Adjunct gap detection moved to half_day_assignments + outpatient workflows.
    This endpoint remains to avoid hard breaks, but will be removed in a follow-up PR.
    """
    raise HTTPException(
        status_code=410,
        detail=(
            "Deprecated: faculty expansion service archived. "
            "Use half-day assignment coverage endpoints instead."
        ),
    )


@router.get("/validation/faculty-coverage", response_model=FacultyCoverageResponse)
def get_faculty_coverage_validation(
    block_number: int = Query(..., ge=1, le=13, description="Block number (1-13)"),
    academic_year: int = Query(
        ..., ge=2020, le=2030, description="Academic year (e.g., 2025)"
    ),
    ...
    Coverage validation should be driven by half_day_assignments after
    outpatient + solver pipelines complete. This endpoint will be replaced.
    """
    raise HTTPException(
        status_code=410,
        detail=(
            "Deprecated: faculty expansion validation archived. "
            "Use half-day assignment coverage metrics instead."
        ),
    )
