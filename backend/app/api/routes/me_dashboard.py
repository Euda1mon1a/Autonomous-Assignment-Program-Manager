...
def _get_base_url(request: Request) -> str:
    ...
    ***REMOVED*** Use X-Forwarded-Proto and X-Forwarded-Host if behind a proxy
...
def _get_person_for_user(db: Session, current_user: User) -> Person | None:
    ...
    """
    ***REMOVED*** Try to find person by matching email
...
async def get_my_dashboard(
    ...
    """
    ***REMOVED*** Get the person associated with this user
    ...
    ***REMOVED*** --- Get upcoming schedule ---
    ...
    for assignment in assignments:
        ...
        ***REMOVED*** Determine if can trade (primary role assignments can typically be traded)
    ...
    ***REMOVED*** --- Get pending swaps ---
    ***REMOVED*** Get swaps where this person is either source or target and status is pending
    pending_swaps_query = (
        db.query(SwapRecord)
        .options(
    ...
    for swap in pending_swaps_query:
        ***REMOVED*** Determine the "other party" (not the current user)
    ...
    ***REMOVED*** --- Get absences ---
    absences_query = (
        db.query(Absence)
        .filter(
            Absence.person_id == person.id,
            Absence.end_date >= today,  ***REMOVED*** Only show current and future absences
        )
    ...
    ***REMOVED*** --- Generate calendar sync URL ---
    ***REMOVED*** Try to get or create a subscription for this person
    calendar_sync_url = None
    try:
        ***REMOVED*** Check if there's already an active subscription
        ...
        if existing_subscriptions:
            ***REMOVED*** Use the first active subscription
            subscription = existing_subscriptions[0]
        else:
            ***REMOVED*** Create a new subscription
            subscription = CalendarService.create_subscription(
                db=db,
                person_id=person.id,
                created_by_user_id=current_user.id,
                label="My Dashboard Calendar",
                expires_days=365,  ***REMOVED*** 1 year expiration
            )
        ...
        ***REMOVED*** If calendar sync fails, continue without it
        calendar_sync_url = None

    ***REMOVED*** --- Calculate summary statistics ---
    ***REMOVED*** Next assignment date
    next_assignment = upcoming_schedule[0].date if upcoming_schedule else None

    ***REMOVED*** Workload in next 4 weeks (count blocks)
