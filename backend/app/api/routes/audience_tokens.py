...
***REMOVED*** =============================================================================
***REMOVED*** Role Hierarchy and Audience Permission Mapping
***REMOVED*** =============================================================================
***REMOVED*** Role levels (lowest to highest privilege)
...
***REMOVED*** Minimum role level required for each audience
***REMOVED*** Audiences not listed here are available to all authenticated users (level 0)
AUDIENCE_REQUIREMENTS: dict[str, int] = {
    ***REMOVED*** Admin-only audiences (level 4)
    "admin.impersonate": 4,
    "database.backup": 4,
    "database.restore": 4,
    ***REMOVED*** Coordinator-level audiences (level 3)
    "schedule.delete": 3,
    "resilience.override": 3,
    "audit.export": 3,
    "jobs.kill": 3,
    "solver.abort": 3,
    ***REMOVED*** Faculty-level audiences (level 2)
    "swap.execute": 2,
    "swap.rollback": 2,
}
...
def check_audience_permission(user_role: str, audience: str) -> tuple[bool, str | None]:
    ...
    ***REMOVED*** Construct informative error message (without revealing exact levels)
    required_role = "admin"
    if required_level == 3:
...
async def request_audience_token(
    ...
    **Role Requirements**:
    ...
    """
    ***REMOVED*** Security: Verify user role has permission for this audience
    ...
    try:
        ...
        ***REMOVED*** Invalid audience or TTL
        raise HTTPException(
...
async def revoke_token(
    ...
    **Ownership Verification**:
    ...
    """
    try:
        ***REMOVED*** Security: Verify token belongs to current user (or user is admin)
        token_owner_id: str | None = None
        expires_at = datetime.utcnow()  ***REMOVED*** Default expiration for cleanup

        ***REMOVED*** Method 1: If token is provided, decode it to get owner and expiration
        if request.token:
            try:
                ***REMOVED*** Decode token without verification (may be expired, that's OK)
                ***REMOVED*** We just need to extract the user_id and expiration
                payload = jwt.decode(
                    request.token,
                    settings.SECRET_KEY,
                    algorithms=[ALGORITHM],
                    options={"verify_exp": False},  ***REMOVED*** Allow expired tokens
                )

                ***REMOVED*** Verify the JTI matches
                ...
                ***REMOVED*** Get actual expiration from token if available
                exp = payload.get("exp")
                if exp:
                ...
                ***REMOVED*** Continue without token owner info - will check blacklist next

        ***REMOVED*** Method 2: Check if token is already in blacklist (has user_id recorded)
        if not token_owner_id:
            token_owner_id = get_token_owner_id(db, request.jti)

        ***REMOVED*** Verify ownership: user must own the token OR be an admin
        if token_owner_id and token_owner_id != str(current_user.id):
            ***REMOVED*** Token belongs to someone else - check if current user is admin
            if not current_user.is_admin:
                ...
                ***REMOVED*** Admin revoking another user's token - log this elevated action
                logger.info(
        ...
        ***REMOVED*** If we couldn't determine ownership and user is not admin, require token
        if not token_owner_id and not current_user.is_admin:
            ***REMOVED*** We can't verify ownership - for security, require the token to be provided
            ***REMOVED*** This prevents users from revoking arbitrary JTIs by guessing
        ...
        ***REMOVED*** Revoke the token
        revoke_audience_token(
            db=db,
            jti=request.jti,
            expires_at=expires_at,
            user_id=str(current_user.id),  ***REMOVED*** Record who revoked it
        ...
        ***REMOVED*** Re-raise HTTP exceptions (403, 400) as-is
        raise
    except Exception as e:
        logger.error(
...
***REMOVED*** Example of using audience authentication in a protected endpoint
***REMOVED*** This is just a demonstration - real implementation would be in appropriate modules
...
async def abort_job_example(
    ...
    **Security**:
    ...
    """
    ***REMOVED*** Verify the audience token belongs to the authenticated user
    ...
    ***REMOVED*** Log the operation with full context
    logger.info(
    ...
    ***REMOVED*** Execute the privileged operation
    ***REMOVED*** ... actual job abort logic here ...
