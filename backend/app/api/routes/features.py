...
async def get_feature_flag_stats(
    ...
    ***REMOVED*** Get flags by environment breakdown
...
async def update_feature_flag(
    ...
    ***REMOVED*** Convert Pydantic model to dict, excluding None values
...
async def evaluate_feature_flag(
    ...
    ***REMOVED*** Use current user's ID and role if not explicitly provided
    ...
    ***REMOVED*** Get flag to determine type
...
async def evaluate_feature_flags_bulk(
    ...
    ***REMOVED*** Use current user's ID and role if not explicitly provided
    ...
    results = await service.evaluate_flags_bulk(
        ...
        user_role=user_role,
        context=evaluation_request.context,
        track_evaluation=False  ***REMOVED*** Don't track bulk evaluations to reduce DB load
    )

    ***REMOVED*** Convert results to response format
    flag_responses = {}
...
async def _get_flags_by_environment(db: AsyncSession) -> dict[str, Any]:
    ...
    for env in environments:
        ***REMOVED*** Get all flags
        all_flags_result = await db.execute(select(FeatureFlag))
        all_flags = all_flags_result.scalars().all()

        ***REMOVED*** Filter flags for this environment
        env_flags = []
        for flag in all_flags:
            ***REMOVED*** If flag.environments is None, it applies to all environments
            ***REMOVED*** If flag.environments is a list, check if env is in it
            if flag.environments is None or env in flag.environments:
                env_flags.append(flag)

        ***REMOVED*** Categorize flags
