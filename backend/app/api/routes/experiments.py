...
async def get_experiment_stats(
    ...
    try:
        ...
        ***REMOVED*** Count total users and metrics across all experiments
        total_users = 0
        total_metrics = 0
        for exp in experiments:
            if exp.status in [ExperimentStatus.RUNNING, ExperimentStatus.COMPLETED]:
                try:
                    ...
                    pass  ***REMOVED*** Skip experiments with missing data
...
async def update_experiment(
    ...
    try:
        ***REMOVED*** Convert to dict, excluding None values
...
async def delete_experiment(
    ...
    try:
        ...
        if experiment.status == ExperimentStatus.RUNNING:
            ***REMOVED*** Cancel running experiment
            await service.pause_experiment(key, paused_by=str(current_user.id))

        ***REMOVED*** Note: The service doesn't have a delete method, so we'll just mark as cancelled
        ***REMOVED*** In production, you might want to add a soft-delete mechanism
    finally:
        await service.close()
