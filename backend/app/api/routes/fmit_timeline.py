...
def get_academic_year_dates() -> tuple[date, date]:
    ...
    today = date.today()
    if today.month >= 7:
        ***REMOVED*** July-December: current year is start year
        start_date = date(today.year, 7, 1)
        end_date = date(today.year + 1, 6, 30)
    else:
        ***REMOVED*** January-June: previous year is start year
...
def get_week_bounds(target_date: date) -> tuple[date, date]:
    ...
    ***REMOVED*** Python weekday: Monday=0, Sunday=6
...
def get_faculty_weekly_assignments(
    ...
    """
    ***REMOVED*** Get all assignments for this faculty in date range
    assignments = (
        db.query(Assignment)
        .join(Block)
        .options(
    ...
    ***REMOVED*** Convert to list of WeekAssignment objects
    today = date.today()
    week_assignments = []

    for week_data in weeks_map.values():
        ***REMOVED*** Determine status based on date
    ...
    ***REMOVED*** Sort by week start date
...
def calculate_workload_summary(
    ...
    if target_weeks == 0:
        ...
        ***REMOVED*** Consider balanced if within 15% of target
...
def get_all_faculty_timelines(
    ...
    ***REMOVED*** Get all faculty with FMIT assignments in the date range
    query = (
        db.query(Person)
        .join(Assignment)
        .join(Block)
    ...
    for faculty in faculty_list:
        ...
        ***REMOVED*** Get department/specialty from person model
...
def calculate_aggregate_metrics(
    ...
    ***REMOVED*** Collect workload values
    ...
    ***REMOVED*** Calculate distribution statistics
    ...
    ***REMOVED*** Calculate coverage percentage
    ***REMOVED*** Count total FMIT blocks that need coverage
    total_blocks = (
        db.query(Block)
    ...
    ***REMOVED*** Count blocks with assignments
    covered_blocks = (
        db.query(Block)
        .join(Assignment)
        .filter(and_(Block.date >= start_date, Block.date <= end_date))
...
async def get_academic_year_timeline(
    ...
    ***REMOVED*** Get all faculty timelines
...
async def get_faculty_timeline(
    ...
    ***REMOVED*** Calculate metrics (even for single faculty)
    aggregate_metrics = calculate_aggregate_metrics(timelines, start_date, end_date, db)
...
async def get_weekly_view(
    ...
    Requires authentication.
    """
    ***REMOVED*** Normalize to Monday
    week_start, week_end = get_week_bounds(week_start)

    ***REMOVED*** Get all assignments for this week
    assignments = (
        db.query(Assignment)
        .join(Block)
        .join(Person)
    ...
    ***REMOVED*** Group assignments by faculty
    faculty_assignments = {}
    for assignment in assignments:
    ...
    ***REMOVED*** Get adjacent week summaries
...
async def get_gantt_data(
    ...
    ***REMOVED*** Get faculty timelines
    ...
    for faculty in faculty_list:
        ...
        ***REMOVED*** Create tasks for each continuous period
        tasks = []
        for week in weekly_assignments:
            task_counter += 1
            task_id = f"task_{faculty.id}_{task_counter}"

            ***REMOVED*** Determine progress based on status
            progress = (
                100.0
            ...
            ***REMOVED*** Custom styling based on workload
            styles = {}
            if week.total_blocks > 8:  ***REMOVED*** Heavy week
                styles = {"backgroundColor": "***REMOVED***ff6b6b", "color": "***REMOVED***ffffff"}
            elif week.total_blocks > 4:
                styles = {"backgroundColor": "***REMOVED***ffd93d", "color": "***REMOVED***000000"}
            else:
                styles = {"backgroundColor": "***REMOVED***6bcf7f", "color": "***REMOVED***ffffff"}
        ...
        ***REMOVED*** Create group for this faculty
