...
***REMOVED*** In-memory storage for approval tokens and fix-it executions
***REMOVED*** In production, this should be in Redis or database
...
def _calculate_task_metrics(db: Session) -> TaskMetrics:
    ...
    try:
        ...
        ***REMOVED*** Query stats for completed/failed tasks
        stats_dict = inspect.stats() or {}

        ***REMOVED*** Aggregate task counts from worker stats
        total_completed = 0
        total_failed = 0

        for worker_name, worker_stats in stats_dict.items():
            ***REMOVED*** Worker stats may contain task counters
            if isinstance(worker_stats, dict):
                total_completed += worker_stats.get("total", {}).get("completed", 0)

        ***REMOVED*** For recent failures, check registered tasks
        registered_dict = inspect.registered() or {}

        ***REMOVED*** Get recent task results from result backend
        ***REMOVED*** Query last 100 task IDs to get completion statistics
        try:
            ***REMOVED*** Use Redis to query recent task keys
            ...
            ***REMOVED*** Celery stores results with pattern: celery-task-meta-*
            task_keys = redis_client.keys("celery-task-meta-*")

            completed_count = 0
            failed_count = 0

            ***REMOVED*** Sample up to 100 most recent tasks
            ...
            ***REMOVED*** Use sampled data if available
            ...
            ***REMOVED*** Fall back to worker stats if Redis query fails
            pass

        ***REMOVED*** Calculate total and success rate
        total_tasks = active_count + pending_count + total_completed + total_failed

        ***REMOVED*** Ensure we have at least some tasks to report
        if total_tasks == 0:
            ***REMOVED*** If no tasks found, return minimal metrics
            return TaskMetrics(
                total_tasks=0,
                active_tasks=0,
                completed_tasks=0,
...
def _get_recent_tasks(db: Session, limit: int = 10) -> list[RecentTaskInfo]:
    ...
    try:
        ...
        ***REMOVED*** Get all task result keys from Redis
        task_keys = redis_client.keys("celery-task-meta-*")

        ***REMOVED*** Parse and collect task information
        task_data_list = []
        ...
        for key in task_keys:
            try:
                ...
                ***REMOVED*** Extract task ID from key (format: celery-task-meta-<uuid>)
                task_id = key.decode("utf-8").replace("celery-task-meta-", "")

                ***REMOVED*** Parse task information
                ...
                ***REMOVED*** Map Celery status to our TaskStatus enum
                status_mapping = {
                ...
                ***REMOVED*** Parse timestamps
                ...
                if date_done:
                    try:
                        ***REMOVED*** Celery date_done is in ISO format
                ...
                ***REMOVED*** Extract error message if failed
                error_message = None
                if status_str == "FAILURE":
                    ...
                    if isinstance(error_result, dict):
                        ...
                        error_message = error_result[:200]  ***REMOVED*** Truncate long errors

                    ***REMOVED*** Use traceback if no error message
        ...
        ***REMOVED*** Sort by completion time (most recent first)
        task_data_list.sort(
            key=lambda x: x["completed_at"] if x["completed_at"] else datetime.min,
            reverse=True,
        )

        ***REMOVED*** Get task result keys from Redis using non-blocking SCAN and limit the sample size
        ...
        ***REMOVED*** Get active tasks to supplement recent tasks
        inspect = celery_app.control.inspect()
        active_tasks_dict = inspect.active() or {}

        ***REMOVED*** Add active tasks to the list
        for worker_name, tasks_list in active_tasks_dict.items():
            for task_info in tasks_list:
                task_id = task_info.get("id", "unknown")
                task_name = task_info.get("name", "Unknown Task")

                ***REMOVED*** Get start time
                started_at = None
                ...
                ***REMOVED*** Parse args/kwargs for description
        ...
        ***REMOVED*** Take the most recent tasks up to limit
        for task_data in task_data_list[:limit]:
            ***REMOVED*** Calculate duration if both timestamps available
            duration_seconds = None
            if task_data.get("started_at") and task_data.get("completed_at"):
                delta = task_data["completed_at"] - task_data["started_at"]
                duration_seconds = delta.total_seconds()

            ***REMOVED*** Generate human-readable task name from Celery task path
            task_name = task_data["name"]
            if "." in task_name:
                ***REMOVED*** Extract last part for readability (e.g., app.resilience.tasks.periodic_health_check -> periodic_health_check)
...
def _calculate_coverage_metrics(db: Session) -> CoverageMetrics:
    ...
    try:
        ***REMOVED*** Get blocks and assignments for next 30 days
        ...
        ***REMOVED*** Get faculty count and calculate utilization
        faculty_count = (
            db.query(func.count(Person.id)).filter(Person.type == "faculty").scalar()
        )

        ***REMOVED*** Estimate utilization (assignments per faculty per day)
        ...
        )  ***REMOVED*** *2 for AM/PM blocks
        faculty_utilization = min(faculty_utilization, 1.0)

        ***REMOVED*** Identify critical gaps (blocks with no assignments)
...
async def get_situation_report(
    ...
    try:
        ***REMOVED*** Get resilience service for health data
        service = _get_resilience_service(db)

        ***REMOVED*** Get system health
        try:
...
async def initiate_fix_it_mode(
    ...
    try:
        ***REMOVED*** Generate execution ID
        execution_id = str(uuid.uuid4())

        ***REMOVED*** Get current failed tasks
        task_metrics = _calculate_task_metrics(db)
        failed_count = task_metrics.failed_tasks

        ***REMOVED*** Simulate fix-it logic
        ***REMOVED*** In production, this would:
        ***REMOVED*** 1. Query actual failed tasks from Celery or task queue
        ***REMOVED*** 2. Apply mode-specific recovery strategies
        ***REMOVED*** 3. Track progress and results
        ***REMOVED*** 4. Return actual affected tasks

        if request.dry_run:
            ***REMOVED*** Dry run - just preview what would happen
            tasks_fixed = 0
            ...
            ***REMOVED*** Actual execution - simulate recovery
            if request.mode == FixItMode.GREEDY:
                ***REMOVED*** Greedy mode: try to fix most tasks aggressively
                success_rate = 0.8
            elif request.mode == FixItMode.CONSERVATIVE:
                ***REMOVED*** Conservative mode: only fix safe tasks
                success_rate = 0.6
            else:  ***REMOVED*** BALANCED
                ***REMOVED*** Balanced mode: moderate success rate
                success_rate = 0.7
            ...
            ***REMOVED*** Store execution for later queries
        ...
        ***REMOVED*** Build affected tasks list (placeholder)
        affected_tasks = []
        for i in range(min(tasks_retried, 10)):  ***REMOVED*** Show up to 10 tasks
            affected_tasks.append(
                AffectedTask(
                    task_id=f"task-{i + 1}",
                    task_name=f"Scheduled task ***REMOVED***{i + 1}",
        ...
        ***REMOVED*** Determine estimated completion
        estimated_completion = None
        if not request.dry_run and tasks_retried > 0:
            ***REMOVED*** Estimate 30 seconds per task
...
async def approve_task(
    ...
    try:
        ...
        ***REMOVED*** Get tasks associated with token
        task_ids = token_data.get("task_ids", [])
        if request.task_id:
            ***REMOVED*** Specific task approval
        ...
        ***REMOVED*** Process approvals
        ...
        for task_id in task_ids:
            ***REMOVED*** In production, this would:
            ***REMOVED*** 1. Update task status in database
            ***REMOVED*** 2. Trigger task execution if approved
            ***REMOVED*** 3. Cancel or rollback if denied
        ...
        ***REMOVED*** Build response message
        if request.action == ApprovalAction.APPROVE:
            status_str = "approved"
        ...
        ***REMOVED*** Determine task_id for response
...
async def generate_approval_token(
    ...
    try:
        ***REMOVED*** Generate secure token
        token = secrets.token_urlsafe(32)

        ***REMOVED*** Store token data
...
***REMOVED*** ============================================================================
***REMOVED*** Solver Control Endpoints (Kill-Switch)
***REMOVED*** ============================================================================
...
async def abort_solver_run(
    ...
    try:
        ***REMOVED*** Check if run exists and is active
        progress = SolverControl.get_progress(run_id)

        if progress and progress.get("status") == "running":
            ***REMOVED*** Request abort
            ...
            ***REMOVED*** Run exists but not running
            return SolverAbortResponse(
            ...
            ***REMOVED*** No progress found - might still be starting or doesn't exist
            ***REMOVED*** Send abort anyway in case it's just starting
