...
Message Protocol (inspired by dzhng/claude-agent-server):
    Client -> Server:
        ...
        - {"type": "interrupt"}  ***REMOVED*** Stop current generation

    Server -> Client:
        - {"type": "connected", "session_id": "..."}
        - {"type": "token", "content": "..."}  ***REMOVED*** Streaming text
...
class ChatMessage(BaseModel):
    ...
    role: str  ***REMOVED*** "user" or "assistant"
    content: str
    timestamp: datetime
...
***REMOVED*** =============================================================================
***REMOVED*** Session Management (In-memory for now, could be Redis)
***REMOVED*** =============================================================================

***REMOVED*** Active sessions: session_id -> ChatSession
_sessions: dict[str, ChatSession] = {}

***REMOVED*** Active WebSocket connections: session_id -> WebSocket
_connections: dict[str, WebSocket] = {}

***REMOVED*** Active streaming tasks: session_id -> asyncio.Event (set when interrupted)
_interrupt_flags: dict[str, asyncio.Event] = {}

***REMOVED*** Active stream tasks: session_id -> asyncio.Task
...
***REMOVED*** =============================================================================
***REMOVED*** MCP Tool Definitions (matches your MCP server tools)
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Tool Execution (calls your existing services)
***REMOVED*** =============================================================================
...
async def execute_tool(tool_name: str, tool_input: dict, db) -> dict[str, Any]:
    ...
    try:
        if tool_name == "validate_schedule":
            ...
            ***REMOVED*** Parse dates if provided, otherwise use defaults
            ...
            if schedule_id:
                ...
                try:
                    ...
                    return {
                        ...
                        "issues": [
                            ...
                            for issue in result.issues[:10]  ***REMOVED*** Limit to 10 issues
                        ],
                ...
                ***REMOVED*** No schedule_id - return general status
                return {
                    "status": "ok",
            ...
            try:
                ...
                ***REMOVED*** Get proactive suggestions for the faculty member
            ...
            ***REMOVED*** Get faculty and assignments for the period
            faculty = (
            ...
            ***REMOVED*** Build coverage requirements (1 faculty per block minimum)
            ...
            ***REMOVED*** Run N-1 analysis
            ...
            return {
                ...
                "n1_vulnerabilities": [
                    ...
                    for v in vulnerabilities[:10]  ***REMOVED*** Limit output
                ],
            ...
            ***REMOVED*** ConflictAnalyzer uses async session
            ...
            ***REMOVED*** Generate summary
            ...
            return {
                ...
                "conflicts": [
                    ...
                    for c in conflicts[:10]  ***REMOVED*** Limit output
                ],
                "summary": {
        ...
        ***REMOVED*** Return sanitized error (no stack traces or sensitive info)
        return {"error": f"Tool execution failed: {type(e).__name__}"}


***REMOVED*** =============================================================================
***REMOVED*** Claude API Integration
***REMOVED*** =============================================================================
...
async def stream_claude_response(
    ...
    try:
        ***REMOVED*** Lazy import to avoid startup dependency
        ...
        ***REMOVED*** Build messages from session history
        messages = []
        for msg in session.messages:
            messages.append({"role": msg.role, "content": msg.content})

        ***REMOVED*** Add current user message
        messages.append({"role": "user", "content": user_message})

        ***REMOVED*** System prompt with scheduler context
...
Be concise but thorough. If there are issues, suggest solutions."""
        ...
        async with client.messages.stream(
            ...
            async for event in stream:
                ***REMOVED*** Check for interruption
                    ...
                    if event.type == "content_block_start":
                        ...
                        if hasattr(event, "delta"):
                            if hasattr(event.delta, "text"):
                                ...
                                ***REMOVED*** Tool input streaming (optional to send)
                                pass

                    elif event.type == "message_stop":
                        break

            ***REMOVED*** Only get final message if not interrupted
            if not interrupted:
                final_message = await stream.get_final_message()

                ***REMOVED*** Process any tool calls
                for block in final_message.content:
                    ***REMOVED*** Check for interruption before each tool call
                    if interrupt_event and interrupt_event.is_set():
                        interrupted = True
                        break
                    ...
                    if block.type == "tool_use":
                        ...
                        ***REMOVED*** Notify frontend of tool execution
                        await websocket.send_json(
                            {
                                "type": "tool_call",
                                "name": tool_name,
                        ...
                        ***REMOVED*** Execute the tool
                        result = await execute_tool(tool_name, tool_input, db)

                        ***REMOVED*** Send result to frontend
                        await websocket.send_json(
        ...
        ***REMOVED*** Store in session history (even partial responses if interrupted)
        ...
        ***REMOVED*** Send appropriate completion message
        if interrupted:
            await websocket.send_json(
                {
                    "type": "interrupted",
        ...
        ***REMOVED*** Task was cancelled (another form of interruption)
        ...
        raise  ***REMOVED*** Re-raise to properly handle cancellation
...
async def claude_chat_websocket(
    ...
    ***REMOVED*** Send connected acknowledgment
    ...
    try:
        while True:
            ***REMOVED*** Receive message from client
            ...
            if msg_type == "user_message":
                ...
                if content.strip():
                    ***REMOVED*** Clear any previous interrupt flag
                    interrupt_event.clear()

                    ***REMOVED*** Create streaming task
                    ...
                    try:
                        ***REMOVED*** Wait for stream to complete
                        await stream_task
                    except asyncio.CancelledError:
                        logger.info(f"Stream task cancelled: {session.session_id}")
                    finally:
                        ***REMOVED*** Clean up task reference
                        if session.session_id in _active_streams:
                            del _active_streams[session.session_id]

            elif msg_type == "interrupt":
                ***REMOVED*** Set the interrupt flag
                interrupt_event.set()

                ***REMOVED*** Also cancel the task if it's running
                if session.session_id in _active_streams:
                    ...
                    ***REMOVED*** No active stream, just acknowledge
                    await websocket.send_json(
                        {"type": "interrupted", "message": "No active generation"}
                    )

            elif msg_type == "get_history":
                ***REMOVED*** Return session history
        ...
        try:
            ...
            pass  ***REMOVED*** Connection may be closed
    finally:
        ***REMOVED*** Cleanup interrupt flag
        if session.session_id in _interrupt_flags:
            del _interrupt_flags[session.session_id]

        ***REMOVED*** Cancel any active stream
        if session.session_id in _active_streams:
            task = _active_streams[session.session_id]
            if not task.done():
                task.cancel()
...
***REMOVED*** =============================================================================
***REMOVED*** REST Endpoints (for non-streaming use cases)
***REMOVED*** =============================================================================
