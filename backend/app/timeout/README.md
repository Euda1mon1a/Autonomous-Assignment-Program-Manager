***REMOVED*** Request Timeout Handling
...
***REMOVED*** Add timeout middleware with 30s default timeout
app.add_middleware(
    TimeoutMiddleware,
...
***REMOVED******REMOVED******REMOVED*** 1. Global Request Timeout (Middleware)
...
@app.get("/api/data")
async def get_data():
    ***REMOVED*** This endpoint inherits the 30s global timeout
    data = await fetch_data()
    return data
```

***REMOVED******REMOVED******REMOVED*** 2. Per-Endpoint Timeout (Decorator)
...
@with_timeout(120.0)  ***REMOVED*** 2 minute timeout for this endpoint
...
***REMOVED******REMOVED******REMOVED*** 3. Database Query Timeout
...
@db_timeout(10.0)  ***REMOVED*** 10s timeout for database queries
async def complex_query(db: Session):
    ***REMOVED*** All database queries in this function have 10s timeout
    result = db.query(Model).filter(...).all()
    return result
```

***REMOVED******REMOVED******REMOVED*** 4. Manual Timeout Handling
...
async def multi_step_operation():
    async with TimeoutHandler(60.0) as handler:
        ...
        ***REMOVED*** Check remaining time
        remaining = handler.get_remaining_time()
        if remaining < 10:
            ***REMOVED*** Not enough time for step 2
...
***REMOVED******REMOVED******REMOVED*** 5. Check Remaining Time
...
async def adaptive_operation():
    ...
    if remaining and remaining < 5:
        ***REMOVED*** Use fast algorithm
        return await quick_algorithm()
    else:
        ***REMOVED*** Use thorough algorithm
        return await thorough_algorithm()
```

***REMOVED******REMOVED******REMOVED*** 6. Timeout Remaining Guard
...
async def expensive_operation():
    ***REMOVED*** Only executes if there's time remaining
    return await do_expensive_work()
```

***REMOVED******REMOVED*** Configuration

***REMOVED******REMOVED******REMOVED*** Middleware Options

```python
app.add_middleware(
    TimeoutMiddleware,
    default_timeout=30.0,        ***REMOVED*** Default timeout in seconds
    timeout_header=True,          ***REMOVED*** Include timeout headers in response
    exclude_paths=["/health"]     ***REMOVED*** Paths to exclude from timeout
)
```
...
```bash
***REMOVED*** Default request timeout (seconds)
REQUEST_TIMEOUT=30

***REMOVED*** Database query timeout (seconds)
DB_QUERY_TIMEOUT=10
```
...
***REMOVED******REMOVED******REMOVED*** 1. Choose Appropriate Timeouts
...
***REMOVED******REMOVED******REMOVED*** 2. Use Database Timeouts

Always use `@db_timeout` for endpoints with complex queries:

```python
@app.get("/api/analytics")
@db_timeout(15.0)  ***REMOVED*** Prevent slow queries
async def analytics(db: Session):
    return await complex_query(db)
```

***REMOVED******REMOVED******REMOVED*** 3. Nested Timeout Handling

Timeouts automatically use the minimum value:

```python
***REMOVED*** Global: 30s
@with_timeout(10.0)  ***REMOVED*** Endpoint: 10s
async def operation():
    async with TimeoutHandler(5.0):  ***REMOVED*** Function: 5s
        ***REMOVED*** Effective timeout: 5s (minimum)
        pass
```

***REMOVED******REMOVED******REMOVED*** 4. Graceful Degradation
...
async def search(query: str):
    ...
    if remaining and remaining < 10:
        ...
        ***REMOVED*** Thorough search
        return await deep_search(query)
```

***REMOVED******REMOVED******REMOVED*** 5. Exclude Health Checks
...
***REMOVED******REMOVED******REMOVED*** Custom Timeout Logic
...
async def batch_process(items: list):
    async with TimeoutHandler(60.0) as handler:
        ...
        for item in items:
            ***REMOVED*** Check if we have time
            if handler.get_remaining_time() < 5:
                ***REMOVED*** Save partial results
...
***REMOVED******REMOVED******REMOVED*** Adaptive Algorithms
...
async def optimize(problem: dict):
    ...
    if remaining > 60:
        ***REMOVED*** Use optimal but slow algorithm
        return await optimal_solve(problem)
    elif remaining > 10:
        ***REMOVED*** Use good heuristic
        return await heuristic_solve(problem)
    else:
        ***REMOVED*** Use fast approximation
        return await quick_solve(problem)
```

***REMOVED******REMOVED*** Troubleshooting

***REMOVED******REMOVED******REMOVED*** Timeout Not Applied
...
- [asyncio Timeouts](https://docs.python.org/3/library/asyncio-task.html***REMOVED***timeouts)
- [SQLAlchemy Query Timeout](https://docs.sqlalchemy.org/en/14/core/connections.html***REMOVED***setting-alternate-search-paths-on-connect)
