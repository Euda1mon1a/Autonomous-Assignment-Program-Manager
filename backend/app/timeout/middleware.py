...
***REMOVED*** Try to import Prometheus metrics
try:
    from prometheus_client import Counter, Histogram

    PROMETHEUS_AVAILABLE = True

    ***REMOVED*** Timeout metrics
...
class TimeoutMiddleware(BaseHTTPMiddleware):
    ...
    Configuration:
        app.add_middleware(
            TimeoutMiddleware,
            default_timeout=30.0,  ***REMOVED*** 30 second default
            timeout_header=True,   ***REMOVED*** Include timeout headers
        )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        """
        ***REMOVED*** Skip timeout for excluded paths
        if request.url.path in self.exclude_paths:
            return await call_next(request)

        ***REMOVED*** Get endpoint-specific timeout if configured
        ...
        try:
            ***REMOVED*** Execute request with timeout
            response = await asyncio.wait_for(call_next(request), timeout=timeout)

            ***REMOVED*** Add timeout headers if enabled
            ...
            ***REMOVED*** Return timeout error response
            ...
            ***REMOVED*** Handle TimeoutError from handler
            elapsed = time.monotonic() - start_time
            ...
            ***REMOVED*** Reset context variables
    ...
    def _get_endpoint_timeout(self, request: Request) -> float | None:
        ...
        """
        ***REMOVED*** Check if route has timeout metadata
