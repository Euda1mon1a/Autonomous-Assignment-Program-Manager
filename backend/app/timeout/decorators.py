...
def with_timeout(timeout: float, error_message: str | None = None):
    ...
    Usage:
        ...
        async def slow_endpoint():
            ***REMOVED*** This endpoint has a 60 second timeout
            await some_long_operation()

        ***REMOVED*** With custom error message
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ***REMOVED*** Check if there's already a timeout from middleware
            existing_timeout = get_timeout_remaining()
            if existing_timeout is not None:
                ***REMOVED*** Use the minimum of existing and decorator timeout
...
def db_timeout(timeout: float):
    ...
    Usage:
        ...
        async def get_complex_data(db: Session):
            ***REMOVED*** All queries here have a 5 second timeout
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ***REMOVED*** Find the Session argument
            db_session = None
            for arg in args:
            ...
            ***REMOVED*** Set statement timeout (PostgreSQL)
            timeout_ms = int(timeout * 1000)
            try:
                ***REMOVED*** Execute SET statement_timeout
                db_session.execute(f"SET statement_timeout = {timeout_ms}")
                logger.debug(f"Set database timeout to {timeout}s for {func.__name__}")

                ***REMOVED*** Execute the function
                result = await func(*args, **kwargs)

                ***REMOVED*** Reset timeout
                db_session.execute("RESET statement_timeout")
                return result

            except Exception as e:
                ***REMOVED*** Reset timeout on error
                try:
                    db_session.execute("RESET statement_timeout")
                except Exception:
                    pass  ***REMOVED*** Ignore errors during cleanup

                ***REMOVED*** Check if it's a timeout error
        ...
        ***REMOVED*** Return appropriate wrapper based on whether function is async
