...
class PoolManager:
    ...
    def initialize(self) -> Engine:
        ...
        ***REMOVED*** Create engine with pool configuration
    ...
    def _setup_pool_events(self):
        ...
        def receive_connect(dbapi_conn, connection_record):
            ...
            ***REMOVED*** Store connect time in connection record
    ...
    def _prewarm_connections(self):
        ...
        try:
            ***REMOVED*** Check out connections to force creation
            ...
            ***REMOVED*** Return all connections to pool
    ...
    def _health_check_loop(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ***REMOVED*** Perform health check
                ...
                if result.is_healthy:
                    ...
                    ***REMOVED*** Attempt recovery if configured
                ...
                ***REMOVED*** Check if dynamic sizing is needed
            ...
            ***REMOVED*** Wait for next check interval
    ...
    def _adjust_pool_size(self):
        ...
        ***REMOVED*** Scale up if utilization is high
        ...
        ***REMOVED*** Scale down if utilization is low
    ...
    def _update_pool_size(self, new_size: int):
        ...
        try:
            ...
            ***REMOVED*** Dispose old engine and pool
            if self._engine:
                logger.debug("Disposing old engine and pool")
                self._engine.dispose()

            ***REMOVED*** Create new engine with updated pool size
            self._engine = create_engine(
                self._database_url,
                poolclass=QueuePool,
                pool_size=self._config.pool_size,
            ...
            ***REMOVED*** Get new pool reference
            self._pool = self._engine.pool

            ***REMOVED*** Update session factory
            self._session_factory = sessionmaker(
                autocommit=False, autoflush=False, bind=self._engine
            )

            ***REMOVED*** Re-setup pool events for monitoring
            if self._config.enable_monitoring and self._monitor:
                self._setup_pool_events()

            ***REMOVED*** Update monitor reference
            if self._monitor:
                self._monitor._pool = self._pool

            ***REMOVED*** Update auto-recovery reference
            if self._auto_recovery:
            ...
            ***REMOVED*** Attempt to restore old size
    ...
    def shutdown(self, timeout: int = 30):
        ...
        ***REMOVED*** Stop health check thread
...
***REMOVED*** Global pool manager instance
