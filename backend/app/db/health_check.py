...
class ConnectionPoolMonitor:
    ...
    def get_current_metrics(self) -> PoolMetrics:
        ...
        ***REMOVED*** Keep only last 100 metrics
    ...
    def record_query(self, query: str, duration_ms: float) -> None:
        ...
        if duration_ms > self.slow_query_threshold_ms:
            self.slow_queries.append(
                {
                    "query": query[:200],  ***REMOVED*** Truncate long queries
                    "duration_ms": duration_ms,
                    "timestamp": datetime.utcnow(),
                }
            )

        ***REMOVED*** Keep only last 1000 queries
        if len(self.query_times) > 1000:
            self.query_times = self.query_times[-1000:]

        ***REMOVED*** Keep only last 100 slow queries
...
class DeadlockDetector:
    ...
    def check_for_deadlocks(self) -> tuple[bool, list[dict[str, Any]]]:
        ...
        """
        try:
            ***REMOVED*** PostgreSQL: Check for deadlock errors
            ...
            ***REMOVED*** Keep only recent deadlocks
        ...
        try:
            ...
            for row in result:
                metric = TableMetrics(
                    ...
                    row_count=row[1],
                    size_mb=row[2],
                    index_count=0,  ***REMOVED*** Could be fetched separately
                    timestamp=datetime.utcnow(),
                )
            ...
            ***REMOVED*** Keep only recent metrics
