...
def example_basic_filtering(db: Session):
    ...
    ***REMOVED*** Get PGY-2 residents
    pgy2_residents = (
        QueryBuilder(Person, db).filter_by(type="resident", pgy_level=2).all()
    )

    ***REMOVED*** Get faculty who perform procedures
    procedure_faculty = (
...
def example_comparison_filters(db: Session):
    ...
    ***REMOVED*** Get senior residents (PGY 2 or higher)
    senior_residents = (
        QueryBuilder(Person, db).filter_by(type="resident").filter_gte("pgy_level", 2).all()
    )

    ***REMOVED*** Get residents who are NOT PGY-1
...
def example_pattern_matching(db: Session):
    ...
    ***REMOVED*** Find people with "Smith" in their name (case-insensitive)
    smiths = QueryBuilder(Person, db).filter_like("name", "%Smith%", case_sensitive=False).all()

    ***REMOVED*** Find emails from specific domain
    hospital_emails = (
...
def example_in_filters(db: Session):
    ...
    ***REMOVED*** Get residents at specific PGY levels
    pgy_1_or_3 = (
        QueryBuilder(Person, db).filter_by(type="resident").filter_in("pgy_level", [1, 3]).all()
    )

    ***REMOVED*** Exclude specific PGY levels
    not_pgy_1 = (
        QueryBuilder(Person, db)
...
def example_null_checks(db: Session):
    ...
    ***REMOVED*** Find faculty (they have NULL pgy_level)
    faculty_via_null = QueryBuilder(Person, db).filter_is_null("pgy_level").all()

    ***REMOVED*** Find people with email set
...
def example_date_ranges(db: Session):
    ...
    ***REMOVED*** Get blocks for next week
    next_week_blocks = (
...
def example_sorting(db: Session):
    ...
    ***REMOVED*** Sort residents by PGY level (ascending)
    sorted_asc = (
        QueryBuilder(Person, db)
        .filter_by(type="resident")
    ...
    ***REMOVED*** Sort by name descending
...
def example_pagination(db: Session):
    ...
    ***REMOVED*** Get page 1 (first 10 residents)
    page1 = (
        QueryBuilder(Person, db)
        .filter_by(type="resident")
        .order_by("name")
    ...
    ***REMOVED*** Access pagination metadata
...
def example_limit_offset(db: Session):
    ...
    ***REMOVED*** Get 5 residents, skipping the first 2
    results = (
        QueryBuilder(Person, db)
        .filter_by(type="resident")
        .order_by("pgy_level")
...
def example_joins(db: Session):
    ...
    ***REMOVED*** Get assignments with eagerly loaded person and block
    ...
    ***REMOVED*** No N+1 queries - all relationships are loaded
    for assignment in assignments_with_relations:
        person_name = assignment.person.name  ***REMOVED*** No additional query
        block_date = assignment.block.date  ***REMOVED*** No additional query
...
def example_filter_by_joined(db: Session):
    ...
    ***REMOVED*** Get assignments for a specific person by name
    ...
    ***REMOVED*** Get assignments for residents only
...
def example_aggregations(db: Session):
    ...
    ***REMOVED*** Min and max PGY levels
...
def example_exists_check(db: Session):
    ...
    ***REMOVED*** Check if any residents exist
    has_residents = QueryBuilder(Person, db).filter_by(type="resident").exists()

    ***REMOVED*** Check if specific person exists
    person_exists = (
...
def example_subqueries(db: Session):
    ...
    ***REMOVED*** Find people who have assignments
    subquery = (
...
def example_or_conditions(db: Session):
    ...
    ***REMOVED*** Get residents at PGY 1 OR PGY 3
    pgy_1_or_3 = (
        QueryBuilder(Person, db)
        .filter_by(type="resident")
...
def example_array_contains(db: Session):
    ...
    ***REMOVED*** Find faculty with Sports Medicine specialty
    sports_med_faculty = (
        QueryBuilder(Person, db)
...
def example_complex_query(db: Session):
    ...
    ***REMOVED*** Find assignments for senior residents (PGY >= 2) in the next week
    ***REMOVED*** Eagerly load relationships to avoid N+1 queries
    results = (
        QueryBuilder(Assignment, db)
        .join_related("person")
        .join_related("block")
        .filter_by_joined("person", type="resident")
        .filter(Person.pgy_level >= 2)  ***REMOVED*** Raw filter for joined table
        .filter_date_range("date", start_date=today, end_date=next_week)  ***REMOVED*** Won't work - need block.date
        .order_by("block_id")
...
def example_bulk_operations(db: Session):
    ...
    """
    ***REMOVED*** Bulk update: Promote all PGY-1 residents to PGY-2
    ***REMOVED*** NOTE: In production, this should be in a transaction with proper validation
    updated_count = (
        QueryBuilder(Person, db)
    ...
    ***REMOVED*** Bulk delete: Remove test accounts
    ***REMOVED*** NOTE: Use with extreme caution
    deleted_count = (
...
def example_clone_for_variations(db: Session):
    ...
    ***REMOVED*** Base query: all residents
    base_query = QueryBuilder(Person, db).filter_by(type="resident").order_by("name")

    ***REMOVED*** Create variations without repeating the base filters
...
def example_factory_function(db: Session):
    ...
    ***REMOVED*** Alternative way to create query builder
...
def example_terminal_methods(db: Session):
    ...
    ***REMOVED*** Get one or none (raises error only if multiple)
    one_or_none = (
        QueryBuilder(Person, db)
        .filter_by(type="resident", pgy_level=2)
        .one_or_none()
    ...
    ***REMOVED*** Just count without fetching
    count = QueryBuilder(Person, db).filter_by(type="resident").count()

    ***REMOVED*** Check existence
...
def example_in_service_layer(db: Session, person_id: str):
    ...
    ***REMOVED*** Simple query with type safety
    person = (
        QueryBuilder(Person, db)
        .filter_by(id=UUID(person_id))
        .join_related("assignments")  ***REMOVED*** Eager load to prevent N+1
        .first()
    )
    ...
    ***REMOVED*** Access relationships without additional queries
...
def example_replacing_repository_pattern(db: Session):
    ...
    """
    ***REMOVED*** Instead of: person_repo.list_residents(pgy_level=2)
    pgy2_residents = (
        QueryBuilder(Person, db)
        .filter_by(type="resident", pgy_level=2)
        .order_by("name")
    ...
    ***REMOVED*** Instead of: person_repo.get_available_for_block(...)
    ***REMOVED*** More complex logic can be composed fluently
    available_faculty = (
        QueryBuilder(Person, db)
        .filter_by(type="faculty")
        .filter_is_not_null("email")
        .filter_not_in("id", [])  ***REMOVED*** Would pass list of unavailable IDs
        .order_by("name")
        .all()
    )
