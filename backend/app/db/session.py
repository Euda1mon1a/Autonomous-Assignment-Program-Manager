...
***REMOVED*** Synchronous engine (legacy support)
engine = create_engine(
    settings.DATABASE_URL,
    ***REMOVED*** Connection pool settings
    pool_pre_ping=settings.DB_POOL_PRE_PING,  ***REMOVED*** Verify connection validity
    pool_size=settings.DB_POOL_SIZE,  ***REMOVED*** Base pool size
    max_overflow=settings.DB_POOL_MAX_OVERFLOW,  ***REMOVED*** Allow burst connections
    pool_timeout=settings.DB_POOL_TIMEOUT,  ***REMOVED*** Wait for connection
    pool_recycle=settings.DB_POOL_RECYCLE,  ***REMOVED*** Prevent stale connections
)

SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

***REMOVED*** Async engine (preferred for all new code)
...
def get_db() -> Generator[Session, None, None]:
    ...
    try:
        ...
        db.rollback()  ***REMOVED*** Clean up dirty state on exception
        raise
    finally:
        db.close()
    ...
    Usage:
        ...
        def my_task():
            with task_session_scope() as session:
                ***REMOVED*** Do database work
                session.query(...)
                ***REMOVED*** Auto-commits on success, rollback on error
