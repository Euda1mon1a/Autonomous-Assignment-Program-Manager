...
class BatchOperations:
    ...
    async def bulk_upsert(
        ...
        ***REMOVED*** Process in batches
        for i in range(0, len(records), self.batch_size):
            ...
            ***REMOVED*** Build update dict for conflict resolution
            update_dict = {
    ...
    async def bulk_update(
        ...
        ***REMOVED*** Process in batches
        for i in range(0, len(updates), self.batch_size):
            ...
            ***REMOVED*** Group updates by unique values for efficiency
            for record in batch:
    ...
    async def copy_from_csv(
        ...
        """
        ***REMOVED*** Get raw connection for COPY command
        connection = await self.session.connection()
        raw_connection = await connection.get_raw_connection()

        cursor = raw_connection.cursor()

        ***REMOVED*** Execute COPY command
