...
***REMOVED*** Context variable to track queries within a request
...
class QueryAnalyzer:
    ...
    def attach_to_engine(self, engine: Engine) -> None:
        ...
        def after_cursor_execute(
            ...
            ***REMOVED*** Get stack trace (limited depth to avoid noise)
            import traceback

            stack_trace = "".join(traceback.format_stack()[-8:-2])

            ***REMOVED*** Create query info
            ...
            ***REMOVED*** Capture EXPLAIN plan if enabled and query is slow
            if (
                self.enable_explain
    ...
    def get_request_stats(self, request_id: str) -> dict[str, Any]:
        ...
        ***REMOVED*** Group by query type
        query_types = defaultdict(int)
        for query in context.queries:
            query_type = self._get_query_type(query.sql)
            query_types[query_type] += 1

        ***REMOVED*** Find slowest queries
    ...
    def _normalize_sql(self, sql: str) -> str:
        ...
        Returns:
            Normalized SQL
        """
        ***REMOVED*** Remove values in WHERE clauses
        normalized = re.sub(r"= \?|\= '[^']*'|= \d+", "= ?", sql)
        ***REMOVED*** Remove IN clause values
    ...
    def _sanitize_sql(self, sql: str) -> str:
        ...
        Returns:
            Sanitized SQL
        """
        ***REMOVED*** This is a simple sanitization - in production, you'd want more robust handling
    ...
    def _are_queries_similar(self, queries: list[QueryInfo]) -> bool:
        ...
        ***REMOVED*** Check if all queries have the same normalized SQL
...
def get_query_analyzer(
    ...
    return QueryAnalyzer(
        ...
        enable_explain=False,  ***REMOVED*** Disable by default due to performance impact
    )
