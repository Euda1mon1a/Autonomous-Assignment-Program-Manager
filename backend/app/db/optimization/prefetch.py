...
***REMOVED*** Note: Certification, LeaveRequest, ProcedureLog, SwapRequest models don't exist
***REMOVED*** Using Absence instead of LeaveRequest, SwapRecord instead of SwapRequest
...
def prefetch_assignments_with_relations(
    ...
    ***REMOVED*** Apply filters
    ...
    if start_date or end_date:
        ...
        ***REMOVED*** Use contains_eager since we're joining
...
def prefetch_persons_with_assignments(
    ...
    ***REMOVED*** Eager load assignments with blocks and rotations
    options = [
    ...
    ***REMOVED*** Optional relations (using actual model relationships)
    if include_leave:
        options.append(selectinload(Person.absences))

    ***REMOVED*** Note: certifications and procedures relationships may not exist on Person model
    ***REMOVED*** if include_certifications:
    ***REMOVED***     options.append(selectinload(Person.certifications))
    ***REMOVED*** if include_procedures:
    ***REMOVED***     options.append(selectinload(Person.procedure_logs))
    ...
    ***REMOVED*** Filter assignments by date if needed
    if start_date or end_date:
...
def prefetch_blocks_with_assignments(
    ...
    ***REMOVED*** Eager load assignments
...
def prefetch_schedule_data(
    ...
    ***REMOVED*** Load assignments with relations
    assignments_query = (
        select(Assignment)
        .join(Block)
        .options(
    ...
    ***REMOVED*** Extract unique person IDs from assignments
    unique_person_ids = list(
        set(str(a.person_id) for a in assignments if a.person_id)
    )

    ***REMOVED*** Load persons with their leave requests
    ...
    ***REMOVED*** Load rotation templates used in this period
    rotation_ids = list(
        set(
    ...
    ***REMOVED*** Load active absences (replaces leave_requests)
    absences_query = (
        select(Absence)
    ...
    return {
        ...
        "persons": persons,
        "rotations": rotations,
        "absences": absences,  ***REMOVED*** renamed from leave_requests
    }
...
def prefetch_person_workload_data(
    ...
    """
    ***REMOVED*** Load person with available relations
    person_query = (
        select(Person)
    ...
    ***REMOVED*** Filter assignments by date
    ...
    ***REMOVED*** Filter absences by date
    filtered_absences = [
        ab
...
def prefetch_rotation_coverage_data(
    ...
    ***REMOVED*** Load assignments for this rotation
    assignments_query = (
        select(Assignment)
        .join(Block)
        .options(
    ...
    ***REMOVED*** Extract unique persons
...
def prefetch_with_caching(
    ...
    **kwargs,
    ...
    """
    ***REMOVED*** Try to get from Redis cache
    try:
        ...
        if cached_data:
            ...
            ***REMOVED*** Deserialize and return
            try:
                ...
                ***REMOVED*** Fall through to fetch fresh data

        ***REMOVED*** Cache miss - fetch fresh data
        logger.debug(f"Cache miss for prefetch: {cache_key}")
        data = fetch_function(db, **kwargs)

        ***REMOVED*** Try to cache the result
        try:
            ***REMOVED*** Serialize data - handle SQLAlchemy models
...
def _serialize_for_cache(data: Any) -> Any:
    ...
    ***REMOVED*** Handle lists
    if isinstance(data, list):
        return [_serialize_for_cache(item) for item in data]

    ***REMOVED*** Handle dictionaries
    if isinstance(data, dict):
        return {key: _serialize_for_cache(value) for key, value in data.items()}

    ***REMOVED*** Handle SQLAlchemy model instances
    if hasattr(data, "__table__"):
        ***REMOVED*** Convert to dict using column names
        result = {}
        for column in data.__table__.columns:
            value = getattr(data, column.name, None)
            ***REMOVED*** Skip relationships and other non-serializable attributes
            if value is not None and not hasattr(value, "__table__"):
                result[column.name] = value
        return result

    ***REMOVED*** Return as-is for primitives
    return data
