...
***REMOVED*** Note: Using Absence instead of LeaveRequest, SwapRecord instead of SwapRequest
...
class OptimizedQueryBuilder:
    ...
    def get_persons_with_assignments(
        ...
        if include_assignments:
            ***REMOVED*** Use selectinload to avoid N+1
            query = query.options(
    ...
    def get_active_swap_requests(
        ...
        Args:
            person_id: Filter by requester or target person ID
    ...
    def get_absences_in_range(
        ...
        """
        ***REMOVED*** Overlap condition: absence.start <= range.end AND absence.end >= range.start
        query = (
            select(Absence)
    ...
    def get_uncovered_blocks(
        ...
        """
        ***REMOVED*** Using LEFT JOIN to find blocks with no assignments
        query = (
            select(Block)
    ...
    def batch_create_assignments(
        ...
        try:
            ***REMOVED*** Use bulk_insert_mappings for better performance
            self.db.bulk_insert_mappings(Assignment, assignments_data)
            self.db.commit()

            ***REMOVED*** Fetch the created assignments
    ...
    def batch_update_assignments(
        ...
        try:
            ***REMOVED*** Use bulk_update_mappings for better performance
    ...
    def batch_delete_assignments(self, assignment_ids: list[str]) -> int:
        ...
        try:
            ***REMOVED*** Use bulk delete for better performance
            count = (
