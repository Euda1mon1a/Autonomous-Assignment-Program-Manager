...
class ShardMigration:
        ...
        try:
            for key in keys:
                try:
                    ...
                    ***REMOVED*** Insert into target
                    await insert_func(target_session, record)
                    await target_session.commit()

                    ***REMOVED*** Delete from source if requested
                    if delete_source:
                        ***REMOVED*** Note: Delete logic depends on your ORM setup
                        ***REMOVED*** This is a placeholder - implement based on your models
                        pass
    ...
    async def rollback(
        ...
        ***REMOVED*** Create reverse migration plan
        rollback_plan = MigrationPlan(
            migration_id=f"{plan.migration_id}_rollback",
            table_name=plan.table_name,
            source_shard_id=plan.target_shard_id,  ***REMOVED*** Reverse
            target_shard_id=plan.source_shard_id,  ***REMOVED*** Reverse
...
class RebalancingStrategy(str, Enum):
    ...
    UNIFORM = "uniform"  ***REMOVED*** Distribute evenly across all shards
    WEIGHTED = "weighted"  ***REMOVED*** Distribute based on shard weights
    MINIMAL_MOVEMENT = "minimal_movement"  ***REMOVED*** Minimize data movement
...
class ShardRebalancer:
    ...
    def _create_uniform_rebalance_plan(
        ...
        """
        ***REMOVED*** Calculate target count per shard
        all_keys = []
                ...
                ***REMOVED*** Find target shards with space
                for target_shard_id in shard_ids:
                    ...
                    if target_count < target_per_shard:
                        ***REMOVED*** Calculate how many to move to this shard
                        ...
                        if to_move > 0:
                            ...
                            ***REMOVED*** Update distribution for next iteration
    ...
    def _create_weighted_rebalance_plan(
        ...
        Returns:
            List of migration plans
        """
        ***REMOVED*** Get shard weights from strategy
        ...
        ***REMOVED*** Calculate target counts based on weights
        all_keys = []
        for keys in current_distribution.values():
            all_keys.extend(keys)
        ...
        ***REMOVED*** Create migration plans (similar to uniform, but with weighted targets)
        plans: list[MigrationPlan] = []
        ***REMOVED*** Implementation similar to uniform but using target_counts
        ***REMOVED*** (Simplified for brevity - full implementation would follow similar logic)
    ...
    def _create_minimal_movement_plan(
        ...
        Returns:
            List of migration plans
        """
        ***REMOVED*** Calculate balance score
        ...
        ***REMOVED*** Only rebalance if severely imbalanced (score > 0.3)
        ...
        ***REMOVED*** Use uniform strategy but only move minimum necessary
        return self._create_uniform_rebalance_plan(table_name, current_distribution)
    ...
    async def add_new_shard(
        ...
        """
        ***REMOVED*** Add shard to strategy
        if table_name not in self.manager.strategies:
            raise ValueError(f"No strategy found for table {table_name}")

        strategy = self.manager.strategies[table_name]

        ***REMOVED*** For hash-based strategies, adding a shard requires rebalancing
        ***REMOVED*** For directory-based, we can explicitly assign some keys to new shard
        if isinstance(strategy, DirectoryShardingStrategy):
            ***REMOVED*** Migrate some keys to new shard for balance
            num_existing_shards = len(strategy.shards)
            keys_per_shard = len(current_keys) // (num_existing_shards + 1)

            ***REMOVED*** Take keys from each existing shard
            plans: list[MigrationPlan] = []
            keys_to_migrate = current_keys[:keys_per_shard]

            if keys_to_migrate:
                ***REMOVED*** Create plan to move keys to new shard
                ***REMOVED*** (Source shard would need to be determined based on current mapping)
                pass

            return plans
        else:
            ***REMOVED*** For other strategies, recalculate distribution
            return []
    ...
    async def remove_shard(
        ...
        ***REMOVED*** Distribute keys to remaining shards
