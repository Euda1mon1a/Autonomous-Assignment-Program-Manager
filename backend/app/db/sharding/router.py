...
class ShardKeyExtractor:
    ...
    def extract(self, obj: Any) -> Any:
        ...
        """
        ***REMOVED*** Use custom extractor if provided
        ...
        ***REMOVED*** Direct value (string, int, etc.)
        if isinstance(obj, (str, int, float)):
            return obj

        ***REMOVED*** Dictionary-like access
        ...
        ***REMOVED*** SQLAlchemy model or object with attributes
...
class CrossShardQuery:
    ...
    async def execute_on_all_shards(
        ...
        ***REMOVED*** Execute queries in parallel
    ...
    async def aggregate_results(
        ...
        Example:
            ***REMOVED*** Count total users across all shards
        ...
        ***REMOVED*** Filter out None results (failed queries)
    ...
    async def merge_sorted_results(
        ...
        ***REMOVED*** Flatten all results
        all_results: List[T] = []
        for results in shard_results.values():
            if results:
                all_results.extend(results)

        ***REMOVED*** Sort merged results
...
class ShardDistribution:
    ...
    def calculate_balance_score(self, distribution: Dict[int, int]) -> float:
        ...
        ***REMOVED*** Normalize to 0-1 scale (CV of 1.0 = perfectly imbalanced)
        return min(cv, 1.0)
