...
***REMOVED*** Type variable for decorator
...
def transactional(
    ...
    Usage:
        ...
        ***REMOVED*** Nested transaction with savepoint:
        with transactional(session) as outer:
            session.add(obj1)
            try:
                with transactional(session, use_savepoint=True) as inner:
                    session.add(obj2)
                    raise ValueError()  ***REMOVED*** Inner rollback only
            except ValueError:
                pass
            ***REMOVED*** obj1 still committed, obj2 rolled back
    ...
    try:
        if use_savepoint:
            ***REMOVED*** Create savepoint for nested transaction
            savepoint = session.begin_nested()
            logger.debug("Started nested transaction with savepoint")
        else:
            ***REMOVED*** Check if we're already in a transaction
        ...
        ***REMOVED*** Check timeout before commit
        if timeout_seconds:
        ...
        ***REMOVED*** Rollback on any exception
        if savepoint:
            savepoint.rollback()
...
def transactional_with_retry(
    ...
    Usage:
        with transactional_with_retry(session, max_retries=3) as txn:
            ***REMOVED*** Operations that might deadlock
    ...
    while retries <= max_retries:
        try:
            with transactional(session, timeout_seconds=timeout_seconds):
                yield session
                return  ***REMOVED*** Success, exit retry loop

        except (sa_exc.OperationalError, sa_exc.DBAPIError) as e:
            ***REMOVED*** Check if it's a retryable error (deadlock, serialization failure)
...
def transactional_decorator(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ***REMOVED*** Extract session from args or kwargs
            session = None
            if "session" in kwargs:
                ...
                ***REMOVED*** Try to find session in args
                for arg in args:
            ...
            ***REMOVED*** Execute with appropriate transaction wrapper
            if max_retries > 0:
...
def optimistic_lock_retry(
    ...
    Usage:
        with optimistic_lock_retry(session) as txn:
            ...
            obj.value += 1  ***REMOVED*** Increments version_id automatically
            session.flush()
    """
    retries = 0
    backoff = initial_backoff
...
def lock_ordering_key(model_class: type, entity_id: Any) -> tuple[str, Any]:
    ...
    Usage:
        ***REMOVED*** Acquire locks in consistent order
        locks_needed = [
            (Assignment, id1),
            (SwapRecord, id2),
            (Assignment, id3),
