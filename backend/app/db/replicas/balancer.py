...
class LoadBalancer:
    ...
    def __init__(
        ...
        ***REMOVED*** Round-robin state
        self._current_index = 0
        self._lock = threading.Lock()

        ***REMOVED*** Replica list for round-robin (maintains insertion order)
    ...
    def select_replica(
        ...
        ***REMOVED*** Try each replica in round-robin order
        ...
        while attempts < num_replicas:
            ...
            ***REMOVED*** Check health if enabled
            if self.enable_health_checks:
                ...
                ***REMOVED*** Health checks disabled, use replica anyway
    ...
    def _get_next_replica(self) -> tuple[str, Engine]:
        ...
        with self._lock:
            ...
            ***REMOVED*** Advance to next replica
    ...
    def remove_replica(self, name: str) -> None:
        ...
        with self._lock:
            ...
            ***REMOVED*** Reset index if it's now out of bounds
...
class StickySessionBalancer(LoadBalancer):
    ...
    def select_replica(
        ...
        """
        if not session_id:
            ***REMOVED*** No session affinity requested, use normal round-robin
            return super().select_replica(session_id)

        ***REMOVED*** Check if session already has a replica assignment
        with self._session_lock:
            assigned_replica = self._session_assignments.get(session_id)

        if assigned_replica:
            ***REMOVED*** Verify assigned replica is still healthy
        ...
        ***REMOVED*** No assignment or assigned replica is unhealthy
        ***REMOVED*** Select a new replica
        selection = super().select_replica(session_id)

        if selection:
            replica_name, engine = selection

            ***REMOVED*** Store assignment for future requests
            with self._session_lock:
