...
class HealthChecker:
    ...
    def check_replica_health(self, engine: Engine, replica_name: str) -> ReplicaHealth:
        ...
        try:
            ***REMOVED*** Test connection with simple query
            with engine.connect() as conn:
                ***REMOVED*** Set statement timeout for this connection
                conn.execute(
                    text(
                ...
                ***REMOVED*** Verify connection is alive
                conn.execute(text("SELECT 1"))

                ***REMOVED*** Measure replication lag
                lag_seconds = self._measure_replication_lag(conn)

                ***REMOVED*** Check if lag is acceptable
            ...
            ***REMOVED*** Connection or query error
    ...
    def _measure_replication_lag(self, connection) -> float | None:
        ...
        """
        try:
            ***REMOVED*** Check if this is a standby (replica)
            result = connection.execute(text("SELECT pg_is_in_recovery()")).scalar()

            if not result:
                ***REMOVED*** This is the primary, not a replica
                return None

            ***REMOVED*** Get replay lag (time between last WAL received and applied)
            ...
            if lag_result and lag_result[1] is not None:
                ***REMOVED*** Return time-based lag (more accurate)
                return float(lag_result[1])

            ***REMOVED*** Fallback: estimate lag from LSN difference
            ***REMOVED*** This is less accurate but better than nothing
            if lag_result and lag_result[0] is not None:
                ***REMOVED*** Rough estimate: assume 16MB/s replication speed
