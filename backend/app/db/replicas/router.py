...
class QueryType(Enum):
    ...
    READ = "read"
    WRITE = "write"
    DDL = "ddl"  ***REMOVED*** Data Definition Language (CREATE, ALTER, DROP)
    TRANSACTION = "transaction"  ***REMOVED*** BEGIN, COMMIT, ROLLBACK
    UNKNOWN = "unknown"
...
class QueryRouter:
    ...
    ***REMOVED*** SQL statement patterns for query classification
    READ_PATTERNS = [
        re.compile(r"^\s*SELECT\s+", re.IGNORECASE),
        re.compile(r"^\s*WITH\s+.*\s+SELECT\s+", re.IGNORECASE),  ***REMOVED*** CTE with SELECT
    ...
    def _route_read_query(self, session_id: Optional[str] = None) -> Engine:
        ...
        ***REMOVED*** Try to select a healthy replica
        ...
        ***REMOVED*** No healthy replicas available
        if self.fallback_to_primary:
            ...
            ***REMOVED*** Return primary anyway to avoid application errors
            ***REMOVED*** Alternative: could raise an exception here
            return self.primary_engine
    ...
    def classify_query(cls, query: str) -> QueryType:
        ...
        ***REMOVED*** Check patterns in order of specificity
        ...
        ***REMOVED*** Default to unknown (will route to primary)
...
class RoutingPolicy:
    ...
    def should_force_primary(
        ...
        """
        ***REMOVED*** Always route writes to primary
        if query_type in (QueryType.WRITE, QueryType.DDL, QueryType.TRANSACTION):
            return True

        ***REMOVED*** Force primary in transactions for consistency
        if in_transaction and self.force_primary_in_transaction:
            return True

        ***REMOVED*** Force primary after recent write for read-your-writes consistency
