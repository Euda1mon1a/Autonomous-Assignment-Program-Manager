...
class ReplicaAwareSessionFactory:
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize health checker and load balancer
        ...
        ***REMOVED*** Session makers for primary and replicas
    ...
    def create_session(
        ...
        ***REMOVED*** Create base session bound to primary
        base_session = self.primary_session_maker()

        ***REMOVED*** Wrap in replica-aware session
    ...
    def session_scope(
        ...
        Usage:
            with session_factory.session_scope() as session:
                ***REMOVED*** Reads automatically route to replicas
                users = session.query(User).all()

                ***REMOVED*** Writes automatically route to primary
...
class ReplicaAwareSession:
    ...
    def __init__(
        ...
        ***REMOVED*** Track session state for routing decisions
        self._in_transaction = False
        self._recent_write = False

        ***REMOVED*** Set up event listeners for transaction tracking
        self._setup_event_listeners()

    def _setup_event_listeners(self) -> None:
        """Set up SQLAlchemy event listeners for tracking state."""
        ***REMOVED*** Track transaction begin/end
        @event.listens_for(self._session, "after_transaction_create")
        def track_transaction_start(session, transaction):
            if transaction.parent is None:  ***REMOVED*** Only track top-level transactions
    ...
    def execute(self, statement, params=None, **kwargs):
        ...
        """
        ***REMOVED*** Determine query type and routing
        ...
        ***REMOVED*** Execute on appropriate engine
        ***REMOVED*** Note: For proper replica routing, we'd need to switch session binding
        ***REMOVED*** For now, we'll use the session's existing connection
        ***REMOVED*** Full implementation would require more sophisticated connection management
