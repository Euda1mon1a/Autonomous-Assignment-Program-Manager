...
***REMOVED*** OAuth2 scheme for token extraction
...
def create_access_token(
    ...
    ***REMOVED*** Generate unique token identifier for blacklist tracking
    jti = str(uuid.uuid4())
...
def create_refresh_token(
    ...
    ***REMOVED*** Generate unique token identifier for blacklist tracking
    ...
    to_encode.update({
        ...
        "jti": jti,
        "iat": datetime.utcnow(),
        "type": "refresh",  ***REMOVED*** Distinguish from access tokens
    })
...
def verify_refresh_token(
    ...
    try:
        ...
        ***REMOVED*** This is critical for secure refresh token rotation
...
def verify_token(token: str, db: Session | None = None) -> TokenData | None:
    ...
    try:
        ...
        ***REMOVED*** Refresh tokens have type="refresh", access tokens have no type field
        if payload.get("type") == "refresh":
            if obs_metrics:
...
async def get_current_user(
    ...
    """
    ***REMOVED*** Priority 1: Check httpOnly cookie (secure against XSS)
    cookie_token = request.cookies.get("access_token")
    if cookie_token:
        ***REMOVED*** PGY2-01ie format is "Bearer <token>", extract the token
        if cookie_token.startswith("Bearer "):
            token = cookie_token[7:]
        else:
            token = cookie_token
    ***REMOVED*** Priority 2: Fall back to Authorization header (for API clients)
    elif not token:
        return None
