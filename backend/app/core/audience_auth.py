...
Usage:
    ***REMOVED*** Generate a token for aborting jobs
    ...
    ***REMOVED*** Use as FastAPI dependency
...
***REMOVED*** JWT algorithm (consistent with main auth)
ALGORITHM = "HS256"

***REMOVED*** Clock skew tolerance (seconds) - allows for minor time differences
...
class AudienceTokenPayload(BaseModel):
    ...
    aud: str  ***REMOVED*** Audience (operation scope)
    exp: datetime  ***REMOVED*** Expiration time
    iat: datetime  ***REMOVED*** Issued at time
    jti: str  ***REMOVED*** Unique token ID for revocation
...
***REMOVED*** Defined audiences for agent operations
***REMOVED*** Add new audiences here as needed
VALID_AUDIENCES = {
...
def create_audience_token(
    user_id: str,
    audience: str,
    ttl_seconds: int = 120,  ***REMOVED*** 2 minutes default
    ...
    """
    ***REMOVED*** Validate audience
    validate_audience(audience)

    ***REMOVED*** Enforce maximum TTL (10 minutes)
    if ttl_seconds > 600:
        raise ValueError("TTL cannot exceed 600 seconds (10 minutes)")

    ***REMOVED*** Enforce minimum TTL (30 seconds)
    if ttl_seconds < 30:
        raise ValueError("TTL must be at least 30 seconds")

    ***REMOVED*** Generate token metadata
    now = datetime.utcnow()
    ...
    ***REMOVED*** Create payload
    payload = {
        ...
        "iat": now,
        "jti": jti,
        "type": "audience",  ***REMOVED*** Distinguish from access/refresh tokens
    }

    ***REMOVED*** Encode token
    token = jwt.encode(payload, settings.SECRET_KEY, algorithm=ALGORITHM)

    ***REMOVED*** Log token creation (audit trail)
    logger.info(
...
def verify_audience_token(
    ...
    """
    try:
        ***REMOVED*** Decode and verify signature
        payload = jwt.decode(token, settings.SECRET_KEY, algorithms=[ALGORITHM])

        ***REMOVED*** Verify this is an audience token (not access/refresh)
        ...
        ***REMOVED*** Validate required claims
        ...
        ***REMOVED*** Convert timestamps
        expires_at = datetime.utcfromtimestamp(exp)
        issued_at = datetime.utcfromtimestamp(iat)
        now = datetime.utcnow()

        ***REMOVED*** Check expiration (with clock skew tolerance)
        ...
        ***REMOVED*** Check for future-dated tokens (clock skew tolerance)
        ...
        ***REMOVED*** Log clock skew if present (for monitoring)
        if now < expires_at - timedelta(seconds=CLOCK_SKEW_TOLERANCE):
        ...
        ***REMOVED*** Verify audience matches
        ...
        ***REMOVED*** Log successful validation
        logger.info(
        ...
        ***REMOVED*** Re-raise HTTPExceptions as-is
        raise
...
def require_audience(audience: str) -> Callable:
    ...
    Example:
        ...
        ... ):
        ...     ***REMOVED*** Access user_id from token_payload.sub
    ...
    async def audience_dependency(
        ...
        """
        ***REMOVED*** Extract token from header
        ...
        ***REMOVED*** Parse "Bearer <token>" format
...
***REMOVED*** Convenience function for manual header extraction (non-dependency use)
...
***REMOVED*** Export all public functions and classes
__all__ = [
    "AudienceTokenPayload",
    "AudienceTokenResponse",
    "VALID_AUDIENCES",
