...
***REMOVED*** Allowed file extensions for Excel uploads
ALLOWED_EXCEL_EXTENSIONS = {'.xlsx', '.xls'}

***REMOVED*** MIME types for Excel files
ALLOWED_EXCEL_MIMETYPES = {
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',  ***REMOVED*** .xlsx
    'application/vnd.ms-excel'  ***REMOVED*** .xls
}

***REMOVED*** Extension to expected MIME type mapping
EXTENSION_TO_MIMETYPE = {
    '.xlsx': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    '.xls': 'application/vnd.ms-excel',
}

***REMOVED*** Maximum upload size in MB
...
def _detect_mime_type(file_content: bytes) -> Optional[str]:
    ...
    try:
        ...
        ***REMOVED*** If magic detection fails, return None
        return None
...
def validate_excel_upload(
    ...
    ***REMOVED*** Check minimum size (empty files are suspicious)
    ...
    ***REMOVED*** Get expected MIME type based on extension
    expected_mime = EXTENSION_TO_MIMETYPE.get(ext)

    ***REMOVED*** Validate Content-Type header if provided
    if content_type:
        ***REMOVED*** Normalize content type (remove parameters like charset)
        normalized_content_type = content_type.split(';')[0].strip().lower()

        ***REMOVED*** Check if Content-Type matches expected MIME type
        ...
        ***REMOVED*** Verify Content-Type matches the file extension
    ...
    ***REMOVED*** Detect MIME type from magic bytes
    detected_mime = _detect_mime_type(file_content)
    if detected_mime:
        ***REMOVED*** Normalize detected MIME type
        detected_mime = detected_mime.lower().strip()

        ***REMOVED*** For XLSX files, python-magic may return 'application/zip'
        ***REMOVED*** since XLSX is a ZIP archive format
        if ext == '.xlsx' and detected_mime == 'application/zip':
            ***REMOVED*** This is acceptable - XLSX files are ZIP archives
            ***REMOVED*** We'll verify the PK signature below
            pass
        ...
        ***REMOVED*** Verify detected MIME type matches extension
        if expected_mime and detected_mime != expected_mime:
            ***REMOVED*** Special case: XLSX as ZIP is acceptable
    ...
    ***REMOVED*** Check magic bytes for XLSX (ZIP archive with PK header)
    ***REMOVED*** XLSX files are essentially ZIP archives, so they start with PK\x03\x04
    if ext == '.xlsx':
    ...
    ***REMOVED*** Check magic bytes for XLS (OLE2/CFB format)
    ***REMOVED*** XLS files start with D0 CF 11 E0 A1 B1 1A E1
    elif ext == '.xls':
...
def sanitize_upload_filename(filename: str) -> str:
    ...
    """
    ***REMOVED*** Get just the filename without any path components
    safe_name = Path(filename).name

    ***REMOVED*** Remove dangerous characters - only allow alphanumeric, dots, underscores, and hyphens
    safe_name = re.sub(r'[^a-zA-Z0-9._-]', '_', safe_name)

    ***REMOVED*** Prevent hidden files (files starting with .)
    if safe_name.startswith('.'):
        safe_name = '_' + safe_name[1:]

    ***REMOVED*** Ensure we still have a filename after sanitization
    if not safe_name or safe_name in {'.', '..'}:
        raise FileValidationError("Invalid filename after sanitization")

    ***REMOVED*** Limit filename length to prevent issues
    if len(safe_name) > 255:
        ***REMOVED*** Keep extension but truncate the name part
...
def validate_and_sanitize_upload(
    ...
    """
    ***REMOVED*** Validate the file content
    validate_excel_upload(file_content, filename, content_type)

    ***REMOVED*** Sanitize the filename
    safe_filename = sanitize_upload_filename(filename)

    return file_content, safe_filename


***REMOVED*** Path Traversal Prevention Functions
...
def validate_backup_id(backup_id: str) -> str:
    ...
    ***REMOVED*** Only allow alphanumeric, hyphens, and underscores
    ...
    ***REMOVED*** Additional check for path traversal patterns
...
def validate_file_path(file_path: Path, allowed_base: Path) -> Path:
    ...
    """
    try:
        ***REMOVED*** Resolve both paths to absolute paths
        resolved_base = allowed_base.resolve()
        resolved_path = file_path.resolve()

        ***REMOVED*** Check if the resolved path is within the base directory
        try:
...
def sanitize_filename(filename: str) -> str:
    ...
    ***REMOVED*** Remove any path separators (both Unix and Windows)
    sanitized = filename.replace('/', '').replace('\\', '')

    ***REMOVED*** Remove path traversal patterns
