...
class RequestMetricsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        ...
        ***REMOVED*** Track active connections
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        ***REMOVED*** Extract route information
        method = request.method
        path = request.url.path

        ***REMOVED*** Normalize endpoint path (remove IDs for better aggregation)
        endpoint = self._normalize_endpoint(path)

        ***REMOVED*** Exclude metrics endpoint from being tracked (avoid recursion)
        if path == "/metrics" or path == "/health":
            return await call_next(request)

        ***REMOVED*** Track active connections and in-progress requests
        ...
        ***REMOVED*** Time the request
        ...
        try:
            ...
            ***REMOVED*** Record exception and re-raise
            ...
            ***REMOVED*** Decrement in-progress counters
            self.metrics.http_requests_in_progress.labels(
                method=method,
                endpoint=endpoint,
            ).dec()
        ...
        ***REMOVED*** Measure response size
        if not exception_raised:
            ...
            ***REMOVED*** Record error metrics for 4xx and 5xx responses
            if status_code >= 400:
    ...
    def _normalize_endpoint(self, path: str) -> str:
        ...
        ***REMOVED*** Replace date patterns (YYYY-MM-DD)
        path = re.sub(r"/\d{4}-\d{2}-\d{2}", "/{date}", path)

        ***REMOVED*** Limit to first 100 chars to prevent extremely long paths
        if len(path) > 100:
            path = path[:100] + "..."
    ...
    def _get_response_size(self, response: Response) -> int:
        ...
        try:
            ...
            ***REMOVED*** Try to estimate from body if available
...
class DatabaseMetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        Returns:
            Response: HTTP response
        """
        ***REMOVED*** Note: Actual query tracking would be done via SQLAlchemy events
        ***REMOVED*** This middleware primarily exists as a hook point for future enhancements
...
class CacheMetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        Returns:
            Response: HTTP response
        """
        ***REMOVED*** Cache metrics are typically tracked in the cache service layer
        ***REMOVED*** This middleware exists as a placeholder for future enhancements
