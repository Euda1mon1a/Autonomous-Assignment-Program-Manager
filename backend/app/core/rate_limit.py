...
class RateLimiter:
    ...
    def __init__(self, redis_client: redis.Redis | None = None):
        ...
        if redis_client is None:
            try:
                ***REMOVED*** Use authenticated Redis URL if password is configured
    ...
    def is_rate_limited(
        ...
        """
        ***REMOVED*** If Redis is not available or rate limiting is disabled, allow the request
        ...
        try:
            ...
            ***REMOVED*** Remove old entries outside the window
            pipe.zremrangebyscore(key, 0, window_start)

            ***REMOVED*** Count current requests in the window
            pipe.zcard(key)

            ***REMOVED*** Add current request with timestamp as score
            pipe.zadd(key, {str(current_time): current_time})

            ***REMOVED*** Set expiration on the key (cleanup)
            pipe.expire(key, window_seconds + 10)

            ***REMOVED*** Execute pipeline
            results = pipe.execute()

            ***REMOVED*** Get count before adding current request
            current_count = results[1]

            ***REMOVED*** Check if limit exceeded (count before current request)
            ...
            ***REMOVED*** Fail open: allow request if Redis fails
            return False, {
                "remaining": max_requests,
                "limit": max_requests,
                "reset_at": int(current_time + window_seconds),
    ...
    def get_remaining(self, key: str, max_requests: int, window_seconds: int) -> int:
        ...
        try:
            ...
            ***REMOVED*** Remove old entries and count current
            pipe = self.redis.pipeline()
            pipe.zremrangebyscore(key, 0, window_start)
            pipe.zcard(key)
            results = pipe.execute()
...
def _is_trusted_proxy(ip: str) -> bool:
    ...
    try:
        ...
        for proxy in trusted_proxies:
            try:
                ***REMOVED*** Check if it's a network (CIDR notation) or single IP
                if "/" in proxy:
...
def get_client_ip(request: Request) -> str:
    ...
    """
    ***REMOVED*** Get direct client IP first
    direct_ip = request.client.host if request.client else None

    ***REMOVED*** Only trust X-Forwarded-For if request comes from a trusted proxy
    if direct_ip and _is_trusted_proxy(direct_ip):
        forwarded_for = request.headers.get("X-Forwarded-For")
        if forwarded_for:
            ***REMOVED*** Take the first IP in the chain (original client IP)
            return forwarded_for.split(",")[0].strip()

    ***REMOVED*** Use direct client IP (no trusted proxy or no X-Forwarded-For)
    if direct_ip:
        return direct_ip
...
def create_rate_limit_dependency(
    ...
    async def rate_limit_dependency(request: Request):
        ...
        if is_limited:
            ***REMOVED*** Return 429 Too Many Requests
            raise HTTPException(
        ...
        ***REMOVED*** Add rate limit headers to response (even when not limited)
        ***REMOVED*** Note: These will be visible in middleware/response processing
...
class AccountLockout:
    ...
    ***REMOVED*** Lockout configuration
    MAX_FAILED_ATTEMPTS: int = 5  ***REMOVED*** Lock after 5 failed attempts
    BASE_LOCKOUT_SECONDS: int = 60  ***REMOVED*** Initial lockout: 1 minute
    MAX_LOCKOUT_SECONDS: int = 3600  ***REMOVED*** Maximum lockout: 1 hour
    BACKOFF_MULTIPLIER: float = 2.0  ***REMOVED*** Double lockout time each failure
    ...
    def record_failed_attempt(self, username: str) -> tuple[bool, int, int]:
        ...
        try:
            ...
            ***REMOVED*** Check if already locked out
            lockout_ttl = self.redis.ttl(lockout_key)
            if lockout_ttl > 0:
                return True, 0, lockout_ttl

            ***REMOVED*** Increment failed attempts
            pipe = self.redis.pipeline()
            pipe.incr(attempts_key)
            pipe.expire(attempts_key, 3600)  ***REMOVED*** Attempts expire after 1 hour
            results = pipe.execute()
            attempts = results[0]

            if attempts >= self.MAX_FAILED_ATTEMPTS:
                ***REMOVED*** Calculate exponential backoff lockout duration
...
***REMOVED*** Global account lockout instance
