...
class CacheTTL:
    ...
    SHORT = 300  ***REMOVED*** 5 minutes - for rapidly changing data
    MEDIUM = 1800  ***REMOVED*** 30 minutes - for moderately stable data
    LONG = 3600  ***REMOVED*** 1 hour - for stable schedule data
    EXTENDED = 14400  ***REMOVED*** 4 hours - for rarely changing data
    DAY = 86400  ***REMOVED*** 24 hours - for static reference data
    WEEK = 604800  ***REMOVED*** 7 days - for very stable configuration
...
class ServiceCache:
    ...
    def __init__(
        ...
        ***REMOVED*** Statistics (per-worker, not shared across processes)
        self._hits = 0
        self._misses = 0
        self._errors = 0
        ...
        ***REMOVED*** Initialize Redis connection
        ...
        try:
            ...
            ***REMOVED*** Test connection
            ...
            self._redis = None  ***REMOVED*** type: ignore
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            ***REMOVED*** Get approximate cache size from Redis
            cache_size = 0
    ...
    def cached(
        ...
        def decorator(func: Callable[P, R]) -> Callable[P, R]:
            ...
            def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
                ...
                ***REMOVED*** Execute function and cache result
            ...
            ***REMOVED*** Add cache management methods to the wrapper
            wrapper.cache_invalidate = lambda: self.invalidate_pattern(  ***REMOVED*** type: ignore
    ...
    def _build_cache_key(
        ...
        ***REMOVED*** Process positional arguments (skip 'self' and db session)
        for i, arg in enumerate(args):
            ***REMOVED*** Skip SQLAlchemy Session and 'self' reference
            if hasattr(arg, "execute") or hasattr(arg, "__self__"):
                continue
            key_parts.append(self._serialize_arg(arg))

        ***REMOVED*** Process keyword arguments (sorted for consistency)
        for k in sorted(kwargs.keys()):
            v = kwargs[k]
            ***REMOVED*** Skip db session
            if k == "db" or hasattr(v, "execute"):
                continue
            key_parts.append(f"{k}={self._serialize_arg(v)}")

        ***REMOVED*** Join and hash if too long
        key = ":".join(key_parts)
        if len(key) > 200:
            ***REMOVED*** Hash long keys to keep them manageable
    ...
    def _serialize_arg(arg: Any) -> str:
        ...
        if arg is None:
            ...
            ***REMOVED*** Sort UUIDs for consistent ordering
            items = sorted(str(item) for item in arg)
            return f"[{','.join(items[:10])}]"  ***REMOVED*** Limit to first 10 items
        elif isinstance(arg, dict):
            ***REMOVED*** Create deterministic string representation
            ...
            return arg[:50]  ***REMOVED*** Truncate long strings
        elif isinstance(arg, Enum):
            return arg.value
        else:
            ***REMOVED*** For other types, use hash of string representation
            return hashlib.md5(str(arg).encode()).hexdigest()[:8]


***REMOVED*** Global cache instance
