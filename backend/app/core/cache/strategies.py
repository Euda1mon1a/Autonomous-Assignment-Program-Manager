...
class InvalidationTrigger(str, Enum):
    ...
    TTL_EXPIRED = "ttl_expired"  ***REMOVED*** Time-to-live expired
    WRITE_OPERATION = "write_operation"  ***REMOVED*** Data was written
    MANUAL = "manual"  ***REMOVED*** Manual invalidation
    TAG_INVALIDATION = "tag_invalidation"  ***REMOVED*** Tag-based group invalidation
    PATTERN_MATCH = "pattern_match"  ***REMOVED*** Pattern-based invalidation
    CACHE_FULL = "cache_full"  ***REMOVED*** Cache eviction due to size
...
class TTLStrategy(InvalidationStrategy):
    ...
    async def should_invalidate(
        ...
        ***REMOVED*** Check remaining TTL if provided
    ...
    async def invalidate(
        ...
        ***REMOVED*** For pattern-based TTL invalidation, we'd need to scan
        ***REMOVED*** but Redis handles TTL automatically, so this is rarely needed
        return 0
...
class TagBasedStrategy(InvalidationStrategy):
    ...
    Example:
        ...
        ***REMOVED*** When data changes:
        await strategy.invalidate_by_tag(redis_client, "user:123")
        ***REMOVED*** Invalidates all entries tagged with "user:123"
    """
    ...
    async def invalidate(
        ...
        if tag:
            ...
            ***REMOVED*** Remove key from all tag sets and delete the key
            deleted = await cache_client.delete(key)
            self._record_invalidation(deleted)
            return deleted
