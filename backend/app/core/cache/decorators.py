...
def cached(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        ***REMOVED*** Determine if function is async
        ...
        def sync_wrapper(*args, **kwargs):
            ***REMOVED*** For sync functions, we need to run async cache operations in event loop
            ***REMOVED*** This is a simplified version - in production, consider using a sync cache
            ***REMOVED*** or running in a thread pool
...
def cache_invalidate(
    ...
    Example:
        ...
        ***REMOVED*** Invalidate multiple patterns
        @cache_invalidate(
            namespace="user",
    ...
    def decorator(func: Callable) -> Callable:
        ...
        @functools.wraps(func)
        async def async_wrapper(*args, **kwargs):
            ***REMOVED*** Invalidate before function execution (for write-through)
            ...
            ***REMOVED*** Could also invalidate after if needed
            ***REMOVED*** await _invalidate_cache(...)

            return result

        @functools.wraps(func)
        def sync_wrapper(*args, **kwargs):
            ***REMOVED*** Execute function (simplified for sync)
            result = func(*args, **kwargs)
            return result
...
def cache_warm(
    ...
    Example:
        ...
        ***REMOVED*** Or use function's return value
        @cache_warm(namespace="reference", ttl=7200)
        async def load_reference_data():
            ***REMOVED*** Return dict of {key: value} to cache
            return {
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            ***REMOVED*** Determine entries to cache
            if entries_func:
...
def invalidate_on_write(
    ...
    Example:
        ...
        ***REMOVED*** Invalidate multiple keys
        @invalidate_on_write(
            namespace="user",
    ...
    def decorator(func: Callable) -> Callable:
        ...
        @functools.wraps(func)
        async def async_wrapper(*args, **kwargs):
            ***REMOVED*** Build cache keys to invalidate
            if key_builder:
            ...
            ***REMOVED*** Invalidate before write
            ...
            ***REMOVED*** Invalidate after write
...
def _build_cache_key(func_name: str, args: tuple, kwargs: dict) -> str:
    ...
    Returns:
        Cache key string
    """
    ***REMOVED*** Combine args and kwargs into a consistent key
    key_parts = [func_name]
...
def _resolve_tags(
    ...
    if callable(tags):
        ***REMOVED*** Get function signature to map args to parameters
...
***REMOVED*** Utility decorators for specific patterns
