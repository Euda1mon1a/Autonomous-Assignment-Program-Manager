...
class MultiLevelCache:
    ...
    def __init__(
        ...
        ***REMOVED*** L1 cache (LRU OrderedDict)
        self._l1_cache: OrderedDict[str, CacheEntry] = OrderedDict()
        self._l1_lock = RLock()

        ***REMOVED*** L2 cache (Redis)
    ...
    async def get(self, key: str, use_l1: bool = True, use_l2: bool = True) -> Any | None:
        ...
        try:
            ...
            ***REMOVED*** Try L2 cache (Redis)
            if self.l2_enabled and use_l2:
                value = await self._get_from_l2(key)
                if value is not None:
                    self._record_l2_hit()

                    ***REMOVED*** Promote to L1
                    if self.l1_enabled and use_l1:
    ...
    async def get_or_fetch(
        ...
        ***REMOVED*** Cache miss - fetch value
        if asyncio.iscoroutinefunction(fetch_func):
            value = await fetch_func(*args, **kwargs)
        else:
            value = fetch_func(*args, **kwargs)

        ***REMOVED*** Cache the fetched value
        if value is not None:
    ...
    async def delete(self, key: str) -> bool:
        ...
        ***REMOVED*** Delete from L1
        if self.l1_enabled:
            deleted = self._delete_from_l1(key) or deleted

        ***REMOVED*** Delete from L2
        if self.l2_enabled:
    ...
    async def invalidate_by_tag(self, tag: str) -> int:
        ...
        ***REMOVED*** Also clear L1 (since we don't have perfect tag tracking in L1)
        self._clear_l1()
    ...
    async def invalidate_by_pattern(self, pattern: str) -> int:
        ...
        Example:
            ***REMOVED*** Invalidate all user profiles
        ...
        ***REMOVED*** Build full pattern with namespace
    ...
    async def warm(self, entries: dict[str, Any], ttl: int | None = None) -> int:
        ...
        Example:
            ***REMOVED*** Pre-warm cache on startup
            await cache.warm({
    ...
    async def persist_stats(self) -> bool:
        ...
        try:
            ...
            await redis_client.set(stats_key, stats_data, ex=86400)  ***REMOVED*** 24 hours
            return True
    ...
    ***REMOVED*** L1 cache operations
    ...
    def _get_from_l1(self, key: str) -> Any | None:
        ...
        with self._l1_lock:
            ...
            ***REMOVED*** Touch for LRU
            entry.touch()

            ***REMOVED*** Move to end for LRU
    ...
    def _set_in_l1(self, key: str, value: Any) -> None:
        ...
        """
        with self._l1_lock:
            ***REMOVED*** Convert to CacheEntry if needed
            if not isinstance(value, CacheEntry):
                value = CacheEntry(value=value)

            ***REMOVED*** Check size limit and evict oldest if needed
...
***REMOVED*** Global cache instances by namespace
...
def get_cache(
    ...
    **kwargs
    ...
    Example:
        ***REMOVED*** Get cache for schedule namespace
        schedule_cache = get_cache("schedule", l1_max_size=2000)

        ***REMOVED*** Use the cache
