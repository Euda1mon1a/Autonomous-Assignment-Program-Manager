...
Usage:
    ...
    ***REMOVED*** At application startup (in main.py)
    setup_logging()

    ***REMOVED*** In any module
...
***REMOVED*** Context variable for request correlation ID (integrates with observability.py)
...
class InterceptHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        ***REMOVED*** Get corresponding loguru level
        try:
            level = logger.level(record.levelname).name
        except ValueError:
            level = record.levelno

        ***REMOVED*** Find caller from where originated the logging call
...
def _json_serializer(record: dict) -> str:
    ...
    ***REMOVED*** Add extra fields from the log call
    if record.get("extra"):
        for key, value in record["extra"].items():
            if key not in ("request_id",):  ***REMOVED*** Avoid duplicates
                subset[key] = value
...
def _text_format(record: dict) -> str:
    ...
    ***REMOVED*** Format extra fields if any
    extra_str = ""
    if record.get("extra"):
        extra_items = [
...
def setup_logging(
    ...
    """
    ***REMOVED*** Remove default handler
    logger.remove()

    ***REMOVED*** Add stderr handler based on format type
    if format_type == "json":
        logger.add(
            _json_sink,
            level=level,
            format="{message}",  ***REMOVED*** Format is handled by _json_sink
            backtrace=True,
            diagnose=False,  ***REMOVED*** Don't include variable values in production
        )
    else:
        logger.add(
            sys.stderr,
    ...
    ***REMOVED*** Add file handler if configured
    if log_file:
        logger.add(
            log_file,
            level=level,
    ...
    ***REMOVED*** Intercept standard library logging
    logging.basicConfig(handlers=[InterceptHandler()], level=0, force=True)

    ***REMOVED*** Set log levels for noisy third-party libraries
...
***REMOVED*** Export the main logger for direct use
__all__ = [
    "logger",
    "get_logger",
    "setup_logging",
