...
class RateLimitConfig:
    ...
    burst_size: int
    burst_refill_rate: float  ***REMOVED*** Tokens per second
...
***REMOVED*** Tier configurations
TIER_CONFIGS = {
    RateLimitTier.FREE: RateLimitConfig(
        requests_per_minute=10,
        requests_per_hour=100,
        burst_size=5,
        burst_refill_rate=0.16,  ***REMOVED*** ~10/min = 0.16/sec
    ),
    RateLimitTier.STANDARD: RateLimitConfig(
        requests_per_minute=60,
        requests_per_hour=1000,
        burst_size=20,
        burst_refill_rate=1.0,  ***REMOVED*** 60/min = 1/sec
    ),
    RateLimitTier.PREMIUM: RateLimitConfig(
        requests_per_minute=120,
        requests_per_hour=5000,
        burst_size=50,
        burst_refill_rate=2.0,  ***REMOVED*** 120/min = 2/sec
    ),
    RateLimitTier.ADMIN: RateLimitConfig(
        requests_per_minute=300,
        requests_per_hour=10000,
        burst_size=100,
        burst_refill_rate=5.0,  ***REMOVED*** 300/min = 5/sec
    ),
    RateLimitTier.INTERNAL: RateLimitConfig(
        requests_per_minute=999999,  ***REMOVED*** Effectively unlimited
...
***REMOVED*** Per-endpoint limits (apply to all tiers unless overridden)
ENDPOINT_LIMITS = {
    ***REMOVED*** Expensive operations get tighter limits
    ...
    ***REMOVED*** Authentication endpoints (already have rate limiting, but adding here for completeness)
...
def get_endpoint_limit(endpoint: str) -> Optional[EndpointLimit]:
    ...
    ***REMOVED*** Pattern matching (e.g., "/api/schedule/*")
...
class TokenBucket:
    ...
    def consume(self, tokens: int = 1) -> tuple[bool, dict]:
        ...
        ***REMOVED*** Lua script for atomic token bucket operations
        lua_script = """
        local key = KEYS[1]
...
class SlidingWindowCounter:
    ...
    def increment(self) -> tuple[bool, dict]:
        ...
        try:
            ...
            ***REMOVED*** Count current requests
            pipe.zcard(self.key)
...
def set_custom_limit(
    ...
    ttl_seconds: int = 86400,  ***REMOVED*** 24 hours
) -> bool:
    """
