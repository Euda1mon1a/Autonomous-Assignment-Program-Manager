...
def get_client_identifier(request: Request) -> str:
    ...
    """
    ***REMOVED*** Check for authenticated user first
    if hasattr(request.state, "user") and request.state.user:
        ***REMOVED*** Use user ID for authenticated requests (more accurate limiting)
        return f"user:{request.state.user.id}"

    ***REMOVED*** Check X-Forwarded-For for proxied requests
    forwarded_for = request.headers.get("X-Forwarded-For")
    if forwarded_for:
        ***REMOVED*** Get the first IP in the chain (original client)
...
***REMOVED*** Uses Redis if available, falls back to in-memory storage
limiter = Limiter(
    key_func=get_client_identifier,
    storage_uri=get_redis_url(),
    default_limits=["200/minute", "1000/hour"],
    enabled=settings.RATE_LIMIT_ENABLED,
    headers_enabled=True,  ***REMOVED*** Add X-RateLimit-* headers to responses
    strategy="fixed-window",  ***REMOVED*** Valid strategies: fixed-window, moving-window
)
...
def rate_limit_exceeded_handler(request: Request, exc: RateLimitExceeded) -> Response:
    ...
    """
    ***REMOVED*** Extract retry-after from the exception
    ...
    ***REMOVED*** Add standard rate limit headers
...
***REMOVED*** Rate limit decorators for different endpoint types
***REMOVED*** These can be used on individual routes for custom limits
