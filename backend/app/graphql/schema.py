...
***REMOVED*** Subscription type for real-time updates
@strawberry.type
class Subscription:
    """
    ...
    async def assignment_updates(
        ...
        ***REMOVED*** In production, this would integrate with Redis pub/sub or similar
        ***REMOVED*** to broadcast real-time updates when assignments change
    ...
    async def schedule_updates(self, info) -> AsyncGenerator[str, None]:
        ...
        """
        ***REMOVED*** Placeholder for real-time schedule updates
        yield "Schedule subscription placeholder"


***REMOVED*** Build the schema
...
async def get_context(
    ...
    ***REMOVED*** Extract and verify JWT token if present
    ***REMOVED*** Check Authorization header
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        ***REMOVED*** Try to get from query params (for subscriptions)
        token = request.query_params.get("token")

    ***REMOVED*** Verify token and add user to context
    if token:
...
***REMOVED*** Create GraphQL router for FastAPI integration
graphql_router = GraphQLRouter(
    schema,
    context_getter=get_context,
    graphiql=True,  ***REMOVED*** Enable GraphQL Playground in development
)
