...
def with_correlation(func: Callable) -> Callable:
    ...
    Usage:
        ...
        async def process_data():
            ***REMOVED*** This function will have correlation context
            logger.info("Processing...")  ***REMOVED*** Will include correlation IDs

        @with_correlation
        def sync_task():
            ***REMOVED*** Works with sync functions too
    ...
    async def async_wrapper(*args, **kwargs):
        ***REMOVED*** Get current context
        context = get_context()

        if context:
            ***REMOVED*** Context already exists, just call the function
            return await func(*args, **kwargs)

        ***REMOVED*** No context, check if we have correlation data in kwargs
        ...
        ***REMOVED*** Initialize context if we have data
        ...
        try:
            ...
            ***REMOVED*** Don't clear context if it was already set
            pass
...
def inject_correlation_context(**extra_kwargs) -> dict[str, Any]:
    ...
    Usage:
        ***REMOVED*** Pass to Celery task
        task.delay(**inject_correlation_context())

        ***REMOVED*** Pass to function
...
class CorrelatedHTTPXClient:
    ...
    Usage:
        async with CorrelatedHTTPXClient() as client:
            ...
            ***REMOVED*** Request automatically includes correlation headers
    """
    ...
    async def request(self, method: str, url: str, **kwargs):
        ...
        ***REMOVED*** Merge correlation headers
...
class CorrelationLogAdapter(logging.LoggerAdapter):
    ...
    Usage:
        ...
        logger.info("Processing data")  ***REMOVED*** Automatically includes correlation IDs
    """
...
def create_child_context() -> CorrelationContext:
    ...
    Example:
        ***REMOVED*** In a request handler
        parent_context = get_context()

        ***REMOVED*** Create child context for background task
        child_context = create_child_context()

        ***REMOVED*** Pass to task
        task.delay(
    ...
    if not parent_context:
        ***REMOVED*** No parent context, create root context
        return initialize_context()

    ***REMOVED*** Create child context with same correlation ID but new request ID
    return initialize_context(
        correlation_id=parent_context.correlation_id,
        parent_id=parent_context.request_id,  ***REMOVED*** Parent's request ID becomes our parent ID
        user_id=parent_context.user_id,
    )


***REMOVED*** Import asyncio for decorator
try:
    import asyncio
except ImportError:
    asyncio = None
