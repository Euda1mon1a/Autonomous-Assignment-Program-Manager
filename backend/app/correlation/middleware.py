...
class CorrelationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        ***REMOVED*** Extract correlation headers
        correlation_id = request.headers.get("X-Correlation-ID")
        request_id = request.headers.get("X-Request-ID")
        parent_id = request.headers.get("X-Parent-ID")

        ***REMOVED*** Generate IDs if not provided
        ...
        ***REMOVED*** Initialize correlation context
        ...
        ***REMOVED*** Store context in request state for access in route handlers
        request.state.correlation_context = context

        ***REMOVED*** Log incoming request
        logger.info(
        ...
        try:
            ...
            ***REMOVED*** Add correlation headers to response
            ...
            ***REMOVED*** Calculate duration even on error
            duration = time.perf_counter() - start_time

            ***REMOVED*** Log error with correlation IDs
            logger.error(
            ...
            ***REMOVED*** Re-raise to let error handlers deal with it
            raise

        finally:
            ***REMOVED*** Clear context after request completes
            clear_context()
...
class UserContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        ***REMOVED*** Check if user is authenticated
        ...
        if user:
            ***REMOVED*** Extract user ID (handle different user object structures)
            user_id = None
...
def get_correlation_context(request: Request) -> CorrelationContext | None:
    ...
    """
    ***REMOVED*** Try to get from request state first (set by middleware)
    if hasattr(request.state, "correlation_context"):
        return request.state.correlation_context

    ***REMOVED*** Fall back to context variables
    return get_context()
