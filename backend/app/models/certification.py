...
class CertificationType(Base):
    ...
    name = Column(String(100), nullable=False, unique=True)  ***REMOVED*** e.g., "BLS", "ACLS"
    full_name = Column(String(255))  ***REMOVED*** e.g., "Basic Life Support"
    description = Column(Text)

    ***REMOVED*** Renewal configuration
    renewal_period_months = Column(Integer, default=24)  ***REMOVED*** Default 2 years
...
class PersonCertification(Base):
    ...
    ***REMOVED*** Verification
    ...
    document_url = Column(String(500))  ***REMOVED*** Link to uploaded certificate image

    ***REMOVED*** Reminder tracking (to avoid duplicate emails)
    ...
    ***REMOVED*** Unique constraint: one certification type per person (latest one)
    ...
    def needs_reminder(self, days: int) -> bool:
        ...
        ***REMOVED*** Check if reminder already sent
