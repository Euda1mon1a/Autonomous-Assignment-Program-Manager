...
class FeatureFlag(Base):
    ...
    ***REMOVED*** Flag type: 'boolean', 'percentage', 'variant'
    flag_type = Column(String(20), nullable=False, default='boolean')

    ***REMOVED*** Enabled status
    enabled = Column(Boolean, nullable=False, default=False)

    ***REMOVED*** Percentage rollout (0.0 - 1.0)
    rollout_percentage = Column(Float, nullable=True)

    ***REMOVED*** Environment targeting (null = all environments)
    ***REMOVED*** JSON array: ["development", "staging", "production"]
    environments = Column(JSON, nullable=True)

    ***REMOVED*** User targeting (null = all users)
    ***REMOVED*** JSON array of user IDs: ["user-uuid-1", "user-uuid-2"]
    target_user_ids = Column(JSON, nullable=True)

    ***REMOVED*** Role targeting (null = all roles)
    ***REMOVED*** JSON array: ["admin", "coordinator"]
    target_roles = Column(JSON, nullable=True)

    ***REMOVED*** A/B testing variants
    ***REMOVED*** JSON object: {"control": 0.5, "variant_a": 0.3, "variant_b": 0.2}
    variants = Column(JSON, nullable=True)

    ***REMOVED*** Dependencies: other flags that must be enabled
    ***REMOVED*** JSON array of flag keys: ["parent_feature", "prerequisite_feature"]
    dependencies = Column(JSON, nullable=True)

    ***REMOVED*** Custom attributes for advanced targeting
    ***REMOVED*** JSON object for arbitrary targeting rules
...
class FeatureFlagEvaluation(Base):
    ...
    ***REMOVED*** Evaluation result
    enabled = Column(Boolean, nullable=False)
    variant = Column(String(50), nullable=True)  ***REMOVED*** For A/B testing

    ***REMOVED*** Context at evaluation time
    environment = Column(String(50), nullable=True)
    rollout_percentage = Column(Float, nullable=True)

    ***REMOVED*** Additional context (for debugging)
...
class FeatureFlagAudit(Base):
    ...
    ***REMOVED*** Who made the change
    user_id = Column(GUID(), nullable=True, index=True)
    username = Column(String(100), nullable=True)

    ***REMOVED*** What changed
    action = Column(String(50), nullable=False)  ***REMOVED*** 'created', 'updated', 'deleted', 'enabled', 'disabled'
    changes = Column(JSON, nullable=True)  ***REMOVED*** Before/after values

    ***REMOVED*** Why (optional)
