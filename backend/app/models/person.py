...
class Person(Base):
    ...
    ***REMOVED*** Capacity/workload fields
    target_clinical_blocks = Column(
        Integer
    )  ***REMOVED*** Expected number of clinical blocks per scheduling period
    ***REMOVED*** Examples:
    ...
    ***REMOVED*** Faculty-specific fields
    ...
    faculty_role = Column(
        String(50)
    )  ***REMOVED*** FacultyRole enum value (pd, apd, oic, dept_chief, sports_med, core)

    ***REMOVED*** Screener-specific fields
    screener_role = Column(
        String(50)
    )  ***REMOVED*** ScreenerRole enum value (dedicated, rn, emt, resident)
    can_screen = Column(
        Boolean, default=False
    )  ***REMOVED*** Whether this person can serve as screener
    screening_efficiency = Column(
        Integer, default=100
    )  ***REMOVED*** Efficiency percentage (70-100)
    ...
    ***REMOVED*** These track cumulative counts for fair distribution
    sunday_call_count = Column(
        Integer, default=0
    )  ***REMOVED*** Sunday calls (tracked separately - worst day)
    weekday_call_count = Column(
        Integer, default=0
    )  ***REMOVED*** Mon-Thurs calls (combined equity pool)
    fmit_weeks_count = Column(
        Integer, default=0
    )  ***REMOVED*** FMIT weeks this year (target ~6 max)
