...
class CalendarSubscription(Base):
    ...
    ***REMOVED*** Secure token for URL (32 bytes = 43 chars base64)
    token = Column(String(64), primary_key=True, index=True)

    ***REMOVED*** Person whose calendar is being subscribed to
    person_id = Column(
        PGUUID(as_uuid=True),
    ...
    ***REMOVED*** User who created the subscription (for audit)
    ...
    ***REMOVED*** Optional label for the subscription
    ...
    ***REMOVED*** Soft delete / revocation
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    revoked_at = Column(DateTime, nullable=True)

    ***REMOVED*** Optional: limit scope to specific date range or rotation types
    ***REMOVED*** NULL means "all future assignments"
    scope_start_date = Column(DateTime, nullable=True)
    scope_end_date = Column(DateTime, nullable=True)
    scope_rotation_types = Column(Text, nullable=True)  ***REMOVED*** JSON array of types
