...
class RotationHalfDayRequirement(Base):
    ...
    ***REMOVED*** Enforce one-to-one relationship at DB level
    __table_args__ = (
        UniqueConstraint(
    ...
    ***REMOVED*** Activity breakdown per block (13 blocks/year, typically 10 half-days each)
    fm_clinic_halfdays = Column(Integer, default=4, nullable=False)
    specialty_halfdays = Column(Integer, default=5, nullable=False)  ***REMOVED*** 4+5+1=10
    specialty_name = Column(String(255))  ***REMOVED*** e.g., "Neurology", "Dermatology"
    academics_halfdays = Column(Integer, default=1, nullable=False)  ***REMOVED*** Wednesday AM
    elective_halfdays = Column(Integer, default=0)  ***REMOVED*** Flexible/buffer time

    ***REMOVED*** Constraints and preferences
    min_consecutive_specialty = Column(Integer, default=1)  ***REMOVED*** Batch specialty days?
    prefer_combined_clinic_days = Column(
        Boolean, default=True
    )  ***REMOVED*** FM + specialty same day
