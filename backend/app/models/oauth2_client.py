...
class PKCEClient(Base):
    ...
    ***REMOVED*** Client identifier (public)
    client_id = Column(String(255), unique=True, nullable=False, index=True)

    ***REMOVED*** Client name and description
    client_name = Column(String(255), nullable=False)
    client_uri = Column(String(512), nullable=True)

    ***REMOVED*** Allowed redirect URIs (whitelist)
    redirect_uris = Column(StringArrayType, nullable=False, default=list)

    ***REMOVED*** Grant types allowed for this client
    grant_types = Column(
        StringArrayType, nullable=False, default=["authorization_code"]
    )

    ***REMOVED*** Response types (code, token, etc.)
    response_types = Column(StringArrayType, nullable=False, default=["code"])

    ***REMOVED*** Scope (space-separated list of allowed scopes)
    scope = Column(Text, nullable=True)

    ***REMOVED*** Client type: public (no secret) or confidential (has secret)
    ***REMOVED*** PKCE clients are always public
    is_public = Column(Boolean, default=True, nullable=False)

    ***REMOVED*** Whether this client is active
