...
class OAuth2AuthorizationCode(Base):
    ...
    ***REMOVED*** The authorization code itself (unique, short-lived)
    code = Column(String(255), unique=True, nullable=False, index=True)

    ***REMOVED*** Client that requested this code
    client_id = Column(String(255), nullable=False, index=True)

    ***REMOVED*** User who authorized this code
    user_id = Column(GUID(), nullable=False, index=True)

    ***REMOVED*** Redirect URI used in the authorization request
    redirect_uri = Column(String(512), nullable=False)

    ***REMOVED*** Scope granted (space-separated)
    scope = Column(Text, nullable=True)

    ***REMOVED*** PKCE code challenge (SHA256 hash of code verifier)
    code_challenge = Column(String(255), nullable=False)

    ***REMOVED*** PKCE code challenge method (S256 or plain)
    code_challenge_method = Column(String(10), nullable=False, default="S256")

    ***REMOVED*** State parameter for CSRF protection
    state = Column(String(255), nullable=True)

    ***REMOVED*** Nonce for OpenID Connect (optional)
    nonce = Column(String(255), nullable=True)

    ***REMOVED*** Whether this code has been used (codes are single-use)
    is_used = Column(String(10), default="false", nullable=False)

    ***REMOVED*** When this code was created
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)

    ***REMOVED*** When this code expires (typically 10 minutes)
    expires_at = Column(DateTime, nullable=False, index=True)

    ***REMOVED*** When this code was used (if applicable)
