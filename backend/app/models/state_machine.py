...
class StateMachineTransition(Base):
    ...
    ***REMOVED*** Reference to state machine instance
    instance_id = Column(
        GUID(),
    ...
    ***REMOVED*** Context snapshot at time of transition
    context_before = Column(JSONType(), nullable=True)
    context_after = Column(JSONType(), nullable=True)

    ***REMOVED*** User who triggered the transition
    triggered_by_id = Column(
        PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )

    ***REMOVED*** Error information if transition failed
    error_message = Column(Text, nullable=True)
