...
class RAGDocument(Base):
    ...
    ***REMOVED*** Create indexes for efficient retrieval
    __table_args__ = (
        ***REMOVED*** GIN index for metadata JSON queries
        Index("ix_rag_documents_metadata", "metadata_", postgresql_using="gin"),
        ***REMOVED*** HNSW index for vector similarity search (optimal for RAG queries)
        ***REMOVED*** m=16: connections per layer, ef_construction=64: search candidates during build
        Index(
        ...
        ***REMOVED*** IVFFlat index as alternative (faster build, slightly slower search)
        ***REMOVED*** lists=100: number of clusters for inverted file
        Index(
