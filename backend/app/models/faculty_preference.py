...
class FacultyPreference(Base):
    ...
    ***REMOVED*** Link to faculty member (one-to-one)
    faculty_id = Column(PGUUID(as_uuid=True), ForeignKey("people.id"), unique=True, nullable=False)

    ***REMOVED*** Week preferences (JSON arrays of ISO date strings)
    preferred_weeks = Column(JSON, default=list)  ***REMOVED*** Weeks they prefer to work
    blocked_weeks = Column(JSON, default=list)    ***REMOVED*** Weeks they cannot work

    ***REMOVED*** Scheduling limits
    max_weeks_per_month = Column(Integer, default=2)
    max_consecutive_weeks = Column(Integer, default=1)
    min_gap_between_weeks = Column(Integer, default=2)  ***REMOVED*** Minimum weeks between FMIT
    target_weeks_per_year = Column(Integer, default=6)  ***REMOVED*** Target annual FMIT count
    ...
    ***REMOVED*** Notification preferences
    ...
    notify_reminder_days = Column(Integer, default=7)  ***REMOVED*** Days before FMIT to remind

    ***REMOVED*** Contact preferences
    preferred_contact_method = Column(Text, default="email")  ***REMOVED*** email, sms, both
