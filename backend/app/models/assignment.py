...
class Assignment(Base):
    ...
    block_id = Column(
        GUID(), ForeignKey("blocks.id", ondelete="CASCADE"), nullable=False, index=True  ***REMOVED*** Performance: Indexed for frequent foreign key lookups
    )
    person_id = Column(
        GUID(), ForeignKey("people.id", ondelete="CASCADE"), nullable=False, index=True  ***REMOVED*** Performance: Indexed for frequent foreign key lookups
    )
    rotation_template_id = Column(GUID(), ForeignKey("rotation_templates.id"), index=True)  ***REMOVED*** Performance: Indexed for template-based queries

    role = Column(String(50), nullable=False)  ***REMOVED*** 'primary', 'supervising', 'backup'
    ...
    ***REMOVED*** Override fields (for manual adjustments)
    ...
    override_acknowledged_at = Column(
        DateTime
    )  ***REMOVED*** When user acknowledged ACGME violation

    ***REMOVED*** Explainability fields - transparency into scheduling decisions
    explain_json = Column(JSONType())  ***REMOVED*** Full DecisionExplanation as JSON
    confidence = Column(Float)  ***REMOVED*** Confidence score 0-1
    score = Column(Float)  ***REMOVED*** Objective score for this assignment
    alternatives_json = Column(
        JSONType()
    )  ***REMOVED*** Top alternatives considered (AlternativeCandidate[])
    audit_hash = Column(
        String(64)
    )  ***REMOVED*** SHA-256 of inputs+outputs for integrity verification
    ...
    ***REMOVED*** Provenance - link to the schedule run that created this assignment
