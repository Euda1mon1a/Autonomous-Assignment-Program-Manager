...
class TokenBlacklist(Base):
    ...
    ***REMOVED*** The JWT ID (jti claim) - unique identifier for each token
    jti = Column(String(36), unique=True, nullable=False, index=True)

    ***REMOVED*** Token type (access, refresh)
    token_type = Column(String(20), default="access")

    ***REMOVED*** User who owned this token
    user_id = Column(GUID(), nullable=True)

    ***REMOVED*** When the token was blacklisted
    blacklisted_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    ***REMOVED*** When the token expires (for cleanup)
    expires_at = Column(DateTime, nullable=False, index=True)

    ***REMOVED*** Reason for blacklisting
