...
class Notification(Base):
    ...
    ***REMOVED*** Notification content
    notification_type = Column(String(50), nullable=False, index=True)
    subject = Column(String(500), nullable=False)
    body = Column(Text, nullable=False)
    data = Column(JSONType, default=dict)

    ***REMOVED*** Priority and channels
    priority = Column(String(20), default="normal")
    channels_delivered = Column(String(200))  ***REMOVED*** Comma-separated list of channels
...
class NotificationPreferenceRecord(Base):
    ...
    ***REMOVED*** User (unique - one preference record per user)
    user_id = Column(
        GUID(),
    ...
    ***REMOVED*** Channel preferences (comma-separated list)
    enabled_channels = Column(String(200), default="in_app,email")

    ***REMOVED*** Notification type preferences (JSON map of type -> enabled)
    notification_types = Column(JSONType, default=dict)

    ***REMOVED*** Quiet hours (no notifications during this window)
    quiet_hours_start = Column(Integer, nullable=True)  ***REMOVED*** Hour (0-23)
    quiet_hours_end = Column(Integer, nullable=True)  ***REMOVED*** Hour (0-23)

    ***REMOVED*** Digest preferences
    email_digest_enabled = Column(Boolean, default=False)
    email_digest_frequency = Column(String(20), default="daily")  ***REMOVED*** daily, weekly
    ...
    ***REMOVED*** Timestamps
    ...
    def is_type_enabled(self, notification_type: str) -> bool:
        ...
        if not self.notification_types:
            return True  ***REMOVED*** Default to enabled
