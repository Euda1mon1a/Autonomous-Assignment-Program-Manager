...
class AgentEmbedding(Base):
    ...
    )  ***REMOVED*** sentence-transformers all-MiniLM-L6-v2
    spec_hash: Mapped[str] = mapped_column(
        String, nullable=False
    )  ***REMOVED*** SHA256 of spec content
    capabilities: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )  ***REMOVED*** Extracted capabilities text
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=func.now(), onupdate=func.now()
    )
...
class TaskHistory(Base):
    ...
    embedding = mapped_column(Vector(384), nullable=True)  ***REMOVED*** Embedded task description
