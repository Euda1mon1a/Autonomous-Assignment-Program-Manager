...
def async_cache(ttl: int):
    ...
    def decorator(
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            ***REMOVED*** Create cache key from args and kwargs
            cache_key = str((args, tuple(sorted(kwargs.items()))))

            ***REMOVED*** Check if cached value exists and is still valid
            if cache_key in cache:
                result, timestamp = cache[cache_key]
                if time.time() - timestamp < ttl:
                    return result

            ***REMOVED*** Call function and cache result
            result = await func(*args, **kwargs)
            cache[cache_key] = (result, time.time())

            return result

        ***REMOVED*** Add cache clearing function
        wrapper.clear_cache = lambda: cache.clear()  ***REMOVED*** type: ignore
