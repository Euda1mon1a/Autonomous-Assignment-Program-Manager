...
class TimezoneConverter:
    ...
    ***REMOVED*** Default timezone for the application
    ...
    def utc_to_local(
        ...
        Example:
            >>> utc_time = datetime(2024, 1, 15, 20, 0)  ***REMOVED*** 8 PM UTC
            >>> local = TimezoneConverter.utc_to_local(utc_time)
            >>> local.hour
            10  ***REMOVED*** 10 AM HST (UTC-10)
        """
        tz_name = tz_name or cls.DEFAULT_TIMEZONE
        target_tz = cls.get_timezone(tz_name)

        ***REMOVED*** Ensure datetime is UTC-aware
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        elif dt.tzinfo != timezone.utc:
            ***REMOVED*** Convert to UTC first if in different timezone
            dt = dt.astimezone(timezone.utc)

        ***REMOVED*** Convert to target timezone
    ...
    def local_to_utc(
        ...
        Example:
            >>> local_time = datetime(2024, 1, 15, 10, 0)  ***REMOVED*** 10 AM HST
            >>> utc = TimezoneConverter.local_to_utc(local_time)
            >>> utc.hour
            20  ***REMOVED*** 8 PM UTC (HST+10)
        """
        tz_name = tz_name or cls.DEFAULT_TIMEZONE
        source_tz = cls.get_timezone(tz_name)

        ***REMOVED*** If naive, assume it's in the source timezone
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=source_tz)
        elif dt.tzinfo != source_tz:
            ***REMOVED*** If already aware but in different timezone, convert to source first
            dt = dt.astimezone(source_tz)

        ***REMOVED*** Convert to UTC
    ...
    def make_aware(
        ...
        ***REMOVED*** For timezones with DST, handle ambiguous times
        ***REMOVED*** ZoneInfo handles this automatically with fold parameter
    ...
    def get_utc_offset(cls, tz_name: str, dt: Optional[datetime] = None) -> timedelta:
        ...
        Example:
            ...
            -10.0  ***REMOVED*** HST is UTC-10
        """
        tz = cls.get_timezone(tz_name)
        dt = dt or datetime.now(tz)

        ***REMOVED*** Ensure datetime is aware in the target timezone
        if dt.tzinfo is None:
    ...
    def is_dst(cls, dt: datetime, tz_name: Optional[str] = None) -> bool:
        ...
        Example:
            ...
            True  ***REMOVED*** EDT is active in summer
            >>> TimezoneConverter.is_dst(summer, "Pacific/Honolulu")
            False  ***REMOVED*** Hawaii has no DST
        """
        tz_name = tz_name or cls.DEFAULT_TIMEZONE
        tz = cls.get_timezone(tz_name)

        ***REMOVED*** Make aware if naive
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=tz)
        else:
            dt = dt.astimezone(tz)

        ***REMOVED*** Check if DST offset is non-zero
        dst = dt.dst()
    ...
    def normalize_datetime_range(
        ...
        ***REMOVED*** Convert start to UTC
        if start.tzinfo is None:
            start = cls.local_to_utc(start, tz_name)
        else:
            start = start.astimezone(timezone.utc)

        ***REMOVED*** Convert end to UTC
    ...
    def today_start_utc(cls, tz_name: Optional[str] = None) -> datetime:
        ...
        Example:
            >>> ***REMOVED*** If today is Jan 15, 2024 in HST
            >>> start = TimezoneConverter.today_start_utc()
            >>> ***REMOVED*** Returns Jan 15, 2024 00:00:00 HST converted to UTC
            >>> ***REMOVED*** which is Jan 15, 2024 10:00:00 UTC
        """
        tz_name = tz_name or cls.DEFAULT_TIMEZONE
        tz = cls.get_timezone(tz_name)

        ***REMOVED*** Get today in local timezone
    ...
    def today_end_utc(cls, tz_name: Optional[str] = None) -> datetime:
        ...
        Example:
            ...
            >>> ***REMOVED*** Returns Jan 15, 2024 23:59:59.999999 HST converted to UTC
        """
        tz_name = tz_name or cls.DEFAULT_TIMEZONE
        tz = cls.get_timezone(tz_name)
