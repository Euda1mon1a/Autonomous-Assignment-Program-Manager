...
class TimezoneDetector:
    ...
    ***REMOVED*** Common timezone aliases for US military installations
    MILITARY_TIMEZONES = {
        "HST": "Pacific/Honolulu",  ***REMOVED*** Hawaii Standard Time
        "AKST": "America/Anchorage",  ***REMOVED*** Alaska Standard Time
        "PST": "America/Los_Angeles",  ***REMOVED*** Pacific Standard Time
        "MST": "America/Denver",  ***REMOVED*** Mountain Standard Time
        "CST": "America/Chicago",  ***REMOVED*** Central Standard Time
        "EST": "America/New_York",  ***REMOVED*** Eastern Standard Time
    }

    ***REMOVED*** Default application timezone
    ...
    def is_valid_timezone(cls, tz_name: str) -> bool:
        ...
        """
        try:
            ***REMOVED*** Check if it's an available timezone
            if tz_name in available_timezones():
                return True

            ***REMOVED*** Check if it's a military timezone alias
            if tz_name.upper() in cls.MILITARY_TIMEZONES:
                return True

            ***REMOVED*** Try to create ZoneInfo object
            ZoneInfo(tz_name)
            return True
        except Exception:
            return False
    ...
    def get_timezone_info(cls, tz_name: str) -> dict:
        ...
        ***REMOVED*** Get abbreviation (if available)
        abbr = now.strftime("%Z")
    ...
    def get_common_timezones(cls) -> list[dict]:
        ...
        for tz_name, display_name in common:
            try:
                ...
                ***REMOVED*** Skip invalid timezones
                continue

        ***REMOVED*** Sort by offset
    ...
    def get_user_timezone(
        ...
        Example:
            >>> ***REMOVED*** User with saved preference
    ...
    def guess_timezone_from_offset(cls, offset_hours: float) -> list[str]:
        ...
        ***REMOVED*** Check common timezones first
        for tz_info in cls.get_common_timezones():
            offset = tz_info["offset_hours"] * 3600
            if abs(offset - target_offset_seconds) < 1:  ***REMOVED*** Within 1 second
    ...
    def validate_user_timezone_preference(cls, tz_name: str) -> str:
        ...
        try:
            ...
            ***REMOVED*** Provide helpful error message
            raise ValueError(
    ...
    def get_timezone_for_location(cls, location: str) -> str | None:
        ...
        Example:
            ...
            'Pacific/Honolulu'
        """
        ***REMOVED*** Simple location-to-timezone mapping for common military bases
        location_map = {
