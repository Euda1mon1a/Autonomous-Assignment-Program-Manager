...
def normalize_phone(phone: str, default_country_code: str = "1") -> str:
    ...
    ***REMOVED*** Remove all non-digit characters
    digits = re.sub(r"\D", "", phone)

    ***REMOVED*** Add country code if not present (US: 11 digits)
    if len(digits) == 10:
...
def normalize_name(name: str) -> str:
    ...
    ***REMOVED*** Normalize Unicode characters
    name = unicodedata.normalize("NFKC", name)

    ***REMOVED*** Remove extra whitespace
    name = " ".join(name.split())

    ***REMOVED*** Title case (preserving special names like McDonald, O'Brien)
    parts = []
    for part in name.split():
        ***REMOVED*** Handle hyphenated names
        if "-" in part:
            hyphen_parts = [p.capitalize() for p in part.split("-")]
            parts.append("-".join(hyphen_parts))
        ***REMOVED*** Handle apostrophes (O'Brien, D'Angelo)
        elif "'" in part:
...
def normalize_text(
    ...
    ***REMOVED*** Remove extra whitespace if requested
    if remove_extra_whitespace:
        text = " ".join(text.split())

    ***REMOVED*** Lowercase if requested
    if lowercase:
        text = text.lower()
...
def normalize_specialty(specialty: str) -> str:
    ...
    """
    ***REMOVED*** Common specialty abbreviations to full names
    specialty_map = {
        "fm": "Family Medicine",
    ...
    ***REMOVED*** Return title case version
...
def normalize_rotation_name(rotation: str) -> str:
    ...
    """
    ***REMOVED*** Common rotation abbreviations
    rotation_map = {
        "fmit": "FMIT",
        "clinic": "Clinic",
        "inpt": "Inpatient",
