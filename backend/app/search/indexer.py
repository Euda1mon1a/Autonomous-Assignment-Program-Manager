...
Example:
    ...
    ***REMOVED*** Define document mapping
...
class FieldType(str, Enum):
    ...
    TEXT = "text"  ***REMOVED*** Full-text search with tokenization
    KEYWORD = "keyword"  ***REMOVED*** Exact match, no tokenization
    INTEGER = "integer"  ***REMOVED*** Numeric field
    FLOAT = "float"  ***REMOVED*** Floating point numeric
    BOOLEAN = "boolean"  ***REMOVED*** Boolean field
    DATE = "date"  ***REMOVED*** Date/datetime field
    GEO = "geo"  ***REMOVED*** Geographic coordinates
    JSON = "json"  ***REMOVED*** Nested JSON object
...
class FieldAnalyzer(str, Enum):
    ...
    STANDARD = "standard"  ***REMOVED*** Standard tokenization (words, lowercase)
    KEYWORD = "keyword"  ***REMOVED*** No tokenization, exact match
    SIMPLE = "simple"  ***REMOVED*** Lowercase, split on non-letters
    WHITESPACE = "whitespace"  ***REMOVED*** Split on whitespace only
    ENGLISH = "english"  ***REMOVED*** English language analyzer with stemming
    LOWERCASE = "lowercase"  ***REMOVED*** Lowercase normalization only
    NGRAM = "ngram"  ***REMOVED*** N-gram tokenization for partial matching
    EDGE_NGRAM = "edge_ngram"  ***REMOVED*** Edge n-gram for autocomplete


class IndexStatus(str, Enum):
    """Index status states."""

    CREATING = "creating"  ***REMOVED*** Index being created
    ACTIVE = "active"  ***REMOVED*** Index is active and searchable
    INDEXING = "indexing"  ***REMOVED*** Bulk indexing in progress
    REINDEXING = "reindexing"  ***REMOVED*** Full reindex in progress
    MIGRATING = "migrating"  ***REMOVED*** Schema migration in progress
    READONLY = "readonly"  ***REMOVED*** Read-only mode
    FAILED = "failed"  ***REMOVED*** Index creation/operation failed
    DELETED = "deleted"  ***REMOVED*** Index has been deleted
...
class SearchIndexer:
    ...
    Example:
        ...
        ***REMOVED*** Create index with mapping
        mapping = DocumentMapping(
            index_name="person",
    ...
    def __init__(
        ...
        ***REMOVED*** Index metadata storage
        ...
        ***REMOVED*** Document count tracking per index
    ...
    async def create_index(
        ...
        try:
            ...
            ***REMOVED*** Store version metadata
        ...
        try:
            ...
            ***REMOVED*** Update health
            if actual_index in self._health:
                ...
                ***REMOVED*** Update rolling average
        ...
        try:
            ...
            ***REMOVED*** Process in batches
            for i in range(0, len(documents), batch_size):
                ...
                try:
                    ***REMOVED*** Use pipeline for efficiency
                    pipe = redis_client.pipeline()

                    for doc_id, document in batch:
                        ***REMOVED*** Validate document
                        if mapping:
                            try:
                                self._validate_document(document, mapping)
                    ...
                    ***REMOVED*** Index fields (separate pass for simplicity)
                    for doc_id, document in batch:
                        if mapping:
                            try:
                                ...
                                logger.warning(
                                    f"Error indexing fields for doc '{doc_id}': {e}"
                                )
                                error_count += 1
            ...
            ***REMOVED*** Update health
            ...
            logger.error(f"Redis connection error during bulk index: {e}")
            raise
        except Exception as e:
    ...
    async def reindex_all(
        ...
        try:
            ***REMOVED*** Create new version if mapping provided
            if new_mapping:
            ...
            ***REMOVED*** Index remaining documents
            if batch:
                result = await self.bulk_index(
                    target_index,
                    batch,
            ...
            ***REMOVED*** Switch alias if new version was created
            if new_mapping:
    ...
    async def search(
        ...
        try:
            ...
            ***REMOVED*** Tokenize query
            tokens = self._tokenize(query)

            ***REMOVED*** Get mapping to determine searchable fields
            mapping = await self._get_mapping(actual_index)
            if not mapping:
                return {
                    "total": 0,
            ...
            ***REMOVED*** Determine which fields to search
            if fields is None:
                ***REMOVED*** Search all text fields
                search_fields = [
                    field
            ...
            ***REMOVED*** Find matching documents
            doc_scores: dict[str, float] = defaultdict(float)

            for token in tokens:
                for field in search_fields:
                    ***REMOVED*** Get documents containing this token in this field
                    field_key = self._get_field_index_key(actual_index, field, token)
                    doc_ids = await redis_client.smembers(field_key)

                    ***REMOVED*** Score documents (simple TF-based scoring)
            ...
            ***REMOVED*** Update health
            ...
            logger.warning(f"Redis connection error during search: {e}")
            return {
                "total": 0,
                "hits": [],
                "query": query,
    ...
    async def delete_document(
        ...
        try:
            ...
            ***REMOVED*** Delete document
            doc_key = self._get_index_key(actual_index, doc_id)
            deleted = await redis_client.delete(doc_key)

            ***REMOVED*** Remove from document set
            docs_set_key = self._get_index_docs_set_key(actual_index)
            ...
            ***REMOVED*** Update metrics
            ...
            logger.warning(f"Redis connection error deleting document: {e}")
            return False
        except Exception as e:
        ...
        try:
            ***REMOVED*** Resolve alias
            ...
            if actual_index not in self._health:
                ***REMOVED*** Try to load from Redis
                redis_client = await self._get_redis()
                metadata_key = self._get_index_metadata_key(actual_index)
            ...
            ***REMOVED*** Calculate health score
            health.health_score = self._calculate_health_score(health)

            return health

        except Exception as e:
            logger.error(f"Error getting index health: {e}", exc_info=True)
            return None
    ...
    async def get_all_indices(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            ***REMOVED*** Find all index metadata keys
            ...
            while True:
                ...
                for key in keys:
                    ...
                    if metadata:
                        ***REMOVED*** Extract index name from key
            ...
            logger.error(f"Error getting all indices: {e}", exc_info=True)
            return []
    ...
    ***REMOVED*** Internal helper methods
    ...
    async def _get_index_versions(self, index_name: str) -> list[IndexVersion]:
        ...
        try:
            ...
            ***REMOVED*** Find all versioned indices
            ...
            logger.error(f"Error getting index versions: {e}", exc_info=True)
            return []
    ...
    async def _update_alias(self, alias_name: str, target_index: str) -> None:
        ...
        try:
            ...
            ***REMOVED*** Store in memory
            ...
            logger.error(f"Error updating alias: {e}", exc_info=True)
            raise
    ...
    async def _resolve_alias(self, index_name: str) -> str:
        ...
        try:
            ...
            ***REMOVED*** No alias, assume it's a direct index name
            return index_name

        except Exception as e:
            logger.error(f"Error resolving alias: {e}", exc_info=True)
            return index_name
    ...
    async def _get_mapping(self, index_name: str) -> DocumentMapping | None:
        ...
        try:
            ...
            ***REMOVED*** Reconstruct DocumentMapping
            mapping = DocumentMapping(
    ...
    def _validate_document(
        ...
        ***REMOVED*** Check field types (basic validation)
        for field, value in document.items():
            if field in mapping.fields:
                field_type = mapping.fields[field]
                ***REMOVED*** Basic type checking
    ...
    async def _index_fields(
        ...
        try:
            ...
            logger.error(f"Error indexing fields: {e}", exc_info=True)
            raise
    ...
    def _tokenize(
        ...
        ***REMOVED*** Standard tokenization: lowercase, split on non-alphanumeric
        text = text.lower()

        if analyzer == FieldAnalyzer.WHITESPACE:
            return text.split()

        ***REMOVED*** Standard: split on non-alphanumeric
        import re

        tokens = re.findall(r"\w+", text)

        ***REMOVED*** Filter out very short tokens
    ...
    async def _increment_doc_count(self, index_name: str) -> None:
        ...
        try:
            ...
            logger.error(f"Error incrementing doc count: {e}")
    ...
    async def _decrement_doc_count(self, index_name: str) -> None:
        ...
        try:
            ...
            logger.error(f"Error decrementing doc count: {e}")
