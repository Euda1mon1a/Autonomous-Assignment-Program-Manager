...
class FuzzyMatcher:
    ...
    def __init__(self, config: Optional[FuzzyMatchConfig] = None):
        ...
        ***REMOVED*** Common nickname mappings for name matching
        self.nickname_map = {
    ...
    def _validate_config(self) -> None:
        ...
        if not (0.99 <= total_weight <= 1.01):  ***REMOVED*** Allow small floating point error
            raise ValueError(
    ...
    def match(self, request: FuzzyMatchRequest) -> FuzzyMatchResponse:
        ...
        ***REMOVED*** Add rank
        ...
        execution_time = (time.time() - start_time) * 1000  ***REMOVED*** Convert to ms
    ...
    def match_names(
        ...
        ***REMOVED*** Normalize query
        query = self._normalize(request.query)
        query_tokens = query.split()

        ***REMOVED*** Build database query
        db_query = db.query(Person)

        ***REMOVED*** Filter by entity type
        ...
        ***REMOVED*** Get all persons
        ...
        for person in persons:
            ...
            ***REMOVED*** Try different matching strategies
            match_type = None
            score = 0.0
            algorithm_scores = {}

            ***REMOVED*** 1. Exact match
            if query == name:
                match_type = 'exact'
                score = 1.0
                algorithm_scores = {'exact': 1.0}

            ***REMOVED*** 2. Fuzzy match on full name
            elif not match_type:
                score, algorithm_scores = self._compute_similarity(
                    query,
                    name,
            ...
            ***REMOVED*** 3. Phonetic match (if enabled)
            ...
            ***REMOVED*** 4. Nickname match (if enabled)
            ...
            ***REMOVED*** 5. Partial name match (first or last name)
            if score < request.threshold and len(query_tokens) == 1:
                ***REMOVED*** Check if query matches any part of the name
                for token in name_tokens:
                    ...
                    if token_score >= request.threshold:
                        match_type = 'partial'
                        score = token_score * 0.9  ***REMOVED*** Slight penalty for partial match
                        algorithm_scores = token_algo_scores
                        break

            ***REMOVED*** Add match if score meets threshold
    ...
    def deduplicate(self, request: DeduplicationRequest) -> DeduplicationResponse:
        ...
        ***REMOVED*** Normalize items
        items = [self._normalize(item) for item in request.items]

        ***REMOVED*** Track unique items and duplicate groups
        unique_items = []
        duplicate_groups = []
        ...
        for i, item in enumerate(items):
            ...
            ***REMOVED*** This item becomes a canonical representative
            duplicates = []
            similarity_scores = {}

            ***REMOVED*** Check remaining items for duplicates
            ...
            ***REMOVED*** Add to unique items
            unique_items.append(item)
            processed_indices.add(i)

            ***REMOVED*** Record duplicate group if duplicates were found
            if duplicates:
    ...
    def batch_match(
        ...
        for query in request.queries:
            ***REMOVED*** Create individual match request
            ...
            ***REMOVED*** Perform match
            match_response = self.match(match_request)

            ***REMOVED*** Determine best match
    ...
    ***REMOVED*** ===== PRIVATE HELPER METHODS =====
    ...
    def _levenshtein_similarity(self, str1: str, str2: str) -> float:
        ...
        ***REMOVED*** Compute Levenshtein distance
        ...
        for i in range(1, len1 + 1):
            for j in range(1, len2 + 1):
                if str1[i - 1] == str2[j - 1]:
                    ...
                    dp[i][j] = 1 + min(
                        dp[i - 1][j],      ***REMOVED*** Deletion
                        dp[i][j - 1],      ***REMOVED*** Insertion
                        dp[i - 1][j - 1]   ***REMOVED*** Substitution
                    )

        distance = dp[len1][len2]
        max_len = max(len1, len2)

        ***REMOVED*** Convert distance to similarity
    ...
    def _soundex(self, text: str) -> str:
        ...
        ***REMOVED*** Remove non-alphabetic characters
        text = re.sub(r'[^A-Z]', '', text)

        if not text:
            return ""

        ***REMOVED*** Soundex digit mapping
        soundex_map = {
            'B': '1', 'F': '1', 'P': '1', 'V': '1',
        ...
        ***REMOVED*** Keep first letter
        soundex = text[0]

        ***REMOVED*** Process remaining letters
        prev_code = soundex_map.get(text[0], '0')

        for char in text[1:]:
            code = soundex_map.get(char, '0')

            ***REMOVED*** Skip vowels and duplicate codes
        ...
        ***REMOVED*** Pad with zeros if needed
    ...
    def _soundex_similarity(self, str1: str, str2: str) -> float:
        ...
        """
        ***REMOVED*** Only use soundex for strings of sufficient length
        ...
        ***REMOVED*** Partial match on soundex codes
    ...
    def _metaphone(self, text: str) -> str:
        ...
        ***REMOVED*** Simplified Metaphone implementation
        ***REMOVED*** (Full implementation is complex; this is a basic version)
        metaphone = ""

        ***REMOVED*** Handle leading characters
        ...
        for i, char in enumerate(text):
            ...
            ***REMOVED*** Skip duplicate letters (except C)
    ...
    def _metaphone_similarity(self, str1: str, str2: str) -> float:
        ...
        Returns:
            Similarity score (0.0-1.0)
        """
        ***REMOVED*** Only use metaphone for strings of sufficient length
        if len(str1) < self.config.min_length_for_phonetic or \
           len(str2) < self.config.min_length_for_phonetic:
            return 0.0
        ...
        ***REMOVED*** Partial match on metaphone codes
    ...
    def _generate_ngrams(self, text: str, n: int) -> Set[str]:
        ...
        Returns:
            Set of n-grams
        """
        ***REMOVED*** Pad text for edge n-grams
        padded = ' ' * (n - 1) + text + ' ' * (n - 1)
    ...
    def _nickname_similarity(
        ...
        for query_token in query_tokens:
            ...
            ***REMOVED*** Check if query is a nickname of any name token
            for name_token in name_tokens:
                ...
                ***REMOVED*** Check nickname map
                if name_lower in self.nickname_map:
                    if query_lower in self.nickname_map[name_lower]:
                        return 0.95  ***REMOVED*** High score but not perfect

                ***REMOVED*** Reverse check (query is full name, name is nickname)
    ...
    def _compute_confidence(
        ...
        ***REMOVED*** Ensure confidence is in valid range
        return max(0.0, min(1.0, confidence))


***REMOVED*** Singleton instance for dependency injection
