...
class FacetType(str, Enum):
    ...
    TERM = "term"              ***REMOVED*** Discrete values (e.g., person type, status)
    RANGE = "range"            ***REMOVED*** Numeric ranges (e.g., age, count)
    DATE_RANGE = "date_range"  ***REMOVED*** Date ranges (e.g., this week, this month)
    HIERARCHICAL = "hierarchical"  ***REMOVED*** Nested categories (e.g., specialty > subspecialty)
...
class FacetOrder(str, Enum):
    ...
    COUNT_DESC = "count_desc"    ***REMOVED*** By count, descending (most popular first)
    COUNT_ASC = "count_asc"      ***REMOVED*** By count, ascending
    VALUE_ASC = "value_asc"      ***REMOVED*** Alphabetically by value
    VALUE_DESC = "value_desc"    ***REMOVED*** Reverse alphabetically
    CUSTOM = "custom"            ***REMOVED*** Custom ordering defined by application
...
class FacetedSearchService:
    ...
    def _get_cache_key(
        ...
        Returns:
            Cache key string
        """
        ***REMOVED*** Create deterministic hash of search parameters
        params = {
            "query": query,
    ...
    async def search_with_facets(
        ...
        ***REMOVED*** Check cache if enabled
        ...
        ***REMOVED*** Execute search with filters
        ...
        ***REMOVED*** Generate facets from results
    ...
    async def _execute_search(
        ...
        ***REMOVED*** Search each entity type
        ...
        ***REMOVED*** Apply facet filters
        for selection in facet_selections:
            if selection.facet_name == "person_type" and selection.values:
                ...
                else:  ***REMOVED*** AND - only makes sense for multi-field searches
    ...
    async def _search_assignments(
        ...
        ***REMOVED*** Apply facet filters
        for selection in facet_selections:
            ...
            ***REMOVED*** Date range filter for assignments
            if selection.facet_name == "date_range":
                if selection.date_start and selection.date_end:
                    ***REMOVED*** This would require joining with Block table
                    pass  ***REMOVED*** Simplified for now
    ...
    async def _generate_facets(
        ...
        ***REMOVED*** Generate date range facets
        ...
        ***REMOVED*** Generate hierarchical facets
    ...
    def _generate_person_type_facet(
        ...
        ***REMOVED*** Sort by count descending
        facet_values.sort(key=lambda x: x.count, reverse=True)

        ***REMOVED*** Limit values
    ...
    def _generate_pgy_level_facet(
        ...
        ***REMOVED*** Sort by value (PGY-1, PGY-2, PGY-3)
    ...
    def _generate_date_range_facet(
        ...
        ***REMOVED*** Count results in each range
        ***REMOVED*** Note: This is simplified - in production, you'd query actual date fields
        for range_value in ranges:
            range_value.count = len(results)  ***REMOVED*** Placeholder
    ...
    def _generate_specialty_hierarchical_facet(
        ...
        """
        ***REMOVED*** Hierarchical structure: Department > Specialty > Subspecialty
        ...
        for result in results:
            if result.type == "person" and result.entity:
                ...
                for specialty in specialties:
                    ***REMOVED*** Split specialty into hierarchy levels
                    ***REMOVED*** Example: "Sports Medicine > Orthopedics" or just "Cardiology"
                    if ">" in specialty:
        ...
        ***REMOVED*** Build hierarchical facet values
        facet_values = []
    ...
    def _apply_dynamic_facet_ordering(self, facets: list[Facet]) -> list[Facet]:
        ...
        Returns:
            Reordered facets
        """
        ***REMOVED*** Sort by analytics data (if available)
    ...
    async def _track_facet_usage(
        ...
        for selection in selections:
            ...
            ***REMOVED*** Calculate result reduction
            if total_before > 0:
                reduction = ((total_before - total_after) / total_before) * 100
                ***REMOVED*** Update moving average
