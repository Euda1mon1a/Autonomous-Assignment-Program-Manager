...
class TemplateEngine:
    ...
    def __init__(
        ...
        ***REMOVED*** Create Jinja2 environment
        if enable_sandbox:
            ***REMOVED*** Use sandboxed environment for security
        ...
        ***REMOVED*** Register custom filters and functions
    ...
    def render_string(
        ...
        """
        try:
            ***REMOVED*** Create template from string
            if self.enable_sandbox:
                ***REMOVED*** Create temporary environment for this template
            ...
            ***REMOVED*** Add locale to context
            context_with_locale = {"locale": self.locale, **context}

            ***REMOVED*** Render template
    ...
    def validate_syntax(self, template_string: str) -> bool:
        ...
        """
        try:
            ***REMOVED*** Parse template to check syntax
            if self.enable_sandbox:
    ...
    def _register_custom_functions(self) -> None:
        ...
        ***REMOVED*** Register global functions
    ...
    def get_translated_text(self, key: str, locale: str = "en", **kwargs) -> str:
        ...
        """
        ***REMOVED*** Load translations (with caching)
        translations = self._load_translations(locale)

        ***REMOVED*** Navigate nested keys (e.g., "notification.swap.approved")
        parts = key.split(".")
        ...
        try:
            ...
            ***REMOVED*** Substitute variables in the translated text
            result = current
            for k, v in kwargs.items():
                result = result.replace(f"{{{k}}}", str(v))
            return result
        except (KeyError, TypeError):
            ***REMOVED*** Fallback to English if not found
            if locale != "en":
                return self.get_translated_text(key, "en", **kwargs)

            logger.warning(f"Translation not found: {key}")
            ***REMOVED*** Return key with variables substituted as fallback
            result = key
            for k, v in kwargs.items():
                result = result.replace(f"{{{k}}}", str(v))
            return result
...
***REMOVED*** Default global instance
