...
class TemplateValidator:
    ...
    ***REMOVED*** Maximum lengths for template components
    MAX_SUBJECT_LENGTH = 500
    MAX_TEMPLATE_LENGTH = 50000  ***REMOVED*** ~50KB

    ***REMOVED*** Forbidden Jinja2 operations for security
    FORBIDDEN_FILTERS = [
        "exec",
        "eval",
        "compile",
    ...
    def validate_subject_template(self, subject_template: str) -> bool:
        ...
        ***REMOVED*** Check for newlines (subjects should be single line)
        if "\n" in subject_template:
            raise ValidationError(
    ...
    def validate_body_template(
        ...
        ***REMOVED*** Validate HTML if needed
    ...
    def validate_required_variables(
        ...
        """
        ***REMOVED*** Parse template to extract variables
        ast = self.env.parse(template_string)
        template_vars = meta.find_undeclared_variables(ast)

        ***REMOVED*** Check for missing required variables
    ...
    def validate_no_forbidden_operations(self, template_string: str) -> bool:
        ...
        """
        ***REMOVED*** Check for forbidden filters
        ...
        ***REMOVED*** Check for forbidden tags
    ...
    def _validate_html_safety(self, html_template: str) -> bool:
        ...
        """
        ***REMOVED*** Check for script tags (basic XSS prevention)
        ***REMOVED*** Note: Jinja2 auto-escaping provides the main protection
        if re.search(r"<script[^>]*>", html_template, re.IGNORECASE):
            ***REMOVED*** Allow script tags in style blocks only
            if not re.search(
        ...
        ***REMOVED*** Check for dangerous attributes
    ...
    def validate_complete_template(
        ...
        ***REMOVED*** Validate required variables if provided
        if required_variables:
            ***REMOVED*** Variables should be present in at least one of the templates
