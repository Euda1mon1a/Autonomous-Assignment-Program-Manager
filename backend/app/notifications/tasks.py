...
::
    ...
    ***REMOVED*** Queue email for delivery
    send_email.delay(
    ...
    ***REMOVED*** Queue webhook for delivery
    send_webhook.delay(
        url="https://hooks.slack.com/services/xxx",
        payload={
            "text": "New schedule published",
            "channel": "***REMOVED***scheduling",
        },
    )

    ***REMOVED*** Trigger conflict detection after leave approval
...
***REMOVED*** Lazy-loaded email service instance
...
@shared_task(
    bind=True,  ***REMOVED*** Required for self.retry()
    name="app.notifications.tasks.send_email",
    max_retries=3,
    default_retry_delay=60,
    autoretry_for=(Exception,),
    retry_backoff=True,  ***REMOVED*** Exponential backoff
    retry_backoff_max=300,  ***REMOVED*** Max 5 minutes between retries
)
def send_email(
    self,
    to: str,
    ...
    try:
        ...
        ***REMOVED*** Use html if provided, otherwise wrap plain text in basic HTML
        if html:
            body_html = html
            body_text = body
        else:
        ...
        ***REMOVED*** Let autoretry_for handle the retry, or raise to trigger it
        raise
