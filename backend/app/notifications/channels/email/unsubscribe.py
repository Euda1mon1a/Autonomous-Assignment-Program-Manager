...
class UnsubscribeRecord(BaseModel):
    ...
    email: str
    notification_types: list[str] = Field(default_factory=list)  ***REMOVED*** Empty = all types
...
class UnsubscribeManager:
    ...
    def is_unsubscribed(self, email: str, notification_type: str | None = None) -> bool:
        ...
        ***REMOVED*** Empty list means unsubscribed from all
        if not record.notification_types:
            return True

        ***REMOVED*** Check specific type
        if notification_type:
