...
class RetryAttempt(BaseModel):
    ...
    status: str = "pending"  ***REMOVED*** pending, retrying, failed, succeeded
...
class RetryHandler:
    ...
    ***REMOVED*** Retry delays (in minutes) by attempt number
    RETRY_DELAYS = {
        1: 1,
        2: 5,
        3: 15,
    ...
    ***REMOVED*** Errors that should not be retried
    ...
    def __init__(self):
        ...
        ***REMOVED*** Pending retries: retry_id -> RetryAttempt
        self._retries: dict[UUID, RetryAttempt] = {}

        ***REMOVED*** Dead letter queue: permanently failed notifications
    ...
    async def schedule_retry(
        ...
        ***REMOVED*** Check if error is retryable
        ...
        async with self._lock:
            ***REMOVED*** Calculate next retry time
        ...
        ***REMOVED*** Get due retries
        ...
        for retry in due_retries:
            try:
                ...
                ***REMOVED*** Check if successful
                if results and results[0].success:
                    ***REMOVED*** Success! Remove from retries
    ...
    async def _handle_retry_failure(self, retry: RetryAttempt, error: str) -> None:
        ...
        async with self._lock:
            ...
            if retry.attempt_count >= retry.max_attempts:
                ***REMOVED*** Move to dead letter queue
                retry.status = "failed"
    ...
    def _is_retryable(self, error: str) -> bool:
        ...
        ***REMOVED*** Check for non-retryable patterns
        ...
        async with self._lock:
            for retry in self._dead_letter:
                if retry.id == retry_id:
                    ***REMOVED*** Reset and move back to retry queue
