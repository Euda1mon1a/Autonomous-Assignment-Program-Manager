...
class BatchingEngine:
    ...
    ***REMOVED*** Notification types that support batching
    BATCHABLE_TYPES = {
    ...
    ***REMOVED*** Default batch windows (in minutes) by type
    DEFAULT_WINDOWS = {
        NotificationType.ASSIGNMENT_CHANGED: 15,  ***REMOVED*** 15 minutes
        NotificationType.SHIFT_REMINDER_24H: 60,  ***REMOVED*** 1 hour
        NotificationType.ABSENCE_APPROVED: 30,  ***REMOVED*** 30 minutes
        NotificationType.ABSENCE_REJECTED: 30,  ***REMOVED*** 30 minutes
    }

    def __init__(self):
        """Initialize the batching engine."""
        ***REMOVED*** Active batches: batch_key -> NotificationBatch
    ...
    async def aggregate_batch(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Different aggregation strategies by type
        if notification_type == NotificationType.ASSIGNMENT_CHANGED:
            ...
            ***REMOVED*** Default: Just combine bodies
            return {
                "count": len(payloads),
