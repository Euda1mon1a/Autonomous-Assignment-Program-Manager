...
class RateLimitBucket:
    ...
    def refill(self) -> None:
        ...
        now = datetime.utcnow()
        elapsed = (now - self.last_refill).total_seconds() / 60  ***REMOVED*** minutes
...
class NotificationRateLimiter:
    ...
    ***REMOVED*** Global rate limits (per user)
    GLOBAL_LIMIT_PER_HOUR = 100

    ***REMOVED*** Per-type rate limits (per user per hour)
    ...
    ***REMOVED*** Per-channel rate limits (per user per hour)
    ...
    def __init__(self):
        ...
        ***REMOVED*** User-level buckets: user_id -> RateLimitBucket
        self._user_buckets: dict[UUID, RateLimitBucket] = {}

        ***REMOVED*** Type-level buckets: (user_id, type) -> RateLimitBucket
        self._type_buckets: dict[tuple[UUID, str], RateLimitBucket] = {}

        ***REMOVED*** Channel-level buckets: (user_id, channel) -> RateLimitBucket
    ...
    async def check_rate_limit(
        ...
        async with self._lock:
            ***REMOVED*** Check global user limit
            ...
            ***REMOVED*** Check per-channel limit
    ...
    def _check_user_limit(self, user_id: UUID) -> bool:
        ...
        if user_id not in self._user_buckets:
            ***REMOVED*** Create new bucket: 100 notifications/hour = 100 capacity, 100/60 refill rate
