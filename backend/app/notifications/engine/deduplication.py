...
class DeduplicationEngine:
    ...
    ***REMOVED*** Default deduplication windows (in minutes) by notification type
    DEFAULT_WINDOWS = {
        NotificationType.ACGME_WARNING: 240,  ***REMOVED*** 4 hours
        NotificationType.SCHEDULE_PUBLISHED: 1440,  ***REMOVED*** 24 hours
        NotificationType.ASSIGNMENT_CHANGED: 60,  ***REMOVED*** 1 hour
        NotificationType.SHIFT_REMINDER_24H: 0,  ***REMOVED*** No dedup
        NotificationType.SHIFT_REMINDER_1H: 0,  ***REMOVED*** No dedup
        NotificationType.ABSENCE_APPROVED: 60,  ***REMOVED*** 1 hour
        NotificationType.ABSENCE_REJECTED: 60,  ***REMOVED*** 1 hour
    }

    ***REMOVED*** Fields to include in fingerprint by notification type
    FINGERPRINT_FIELDS = {
    ...
    def __init__(self):
        ...
        ***REMOVED*** In-memory cache: fingerprint -> last_sent_timestamp
    ...
    def is_duplicate(
        ...
        """
        ***REMOVED*** Get deduplication window
        window = window_minutes or self.DEFAULT_WINDOWS.get(notification_type, 60)

        ***REMOVED*** No deduplication if window is 0
        if window == 0:
            return False

        ***REMOVED*** Generate fingerprint
        fingerprint = self._generate_fingerprint(recipient_id, notification_type, data)

        ***REMOVED*** Check if fingerprint exists and is within window
    ...
    def _generate_fingerprint(
        ...
        """
        ***REMOVED*** Get fields to include in fingerprint
        fields = self.FINGERPRINT_FIELDS.get(notification_type, [])

        ***REMOVED*** Extract relevant data
        fingerprint_data = {
    ...
    def clear_expired(self) -> int:
        ...
        """
        now = datetime.utcnow()
        max_age = timedelta(days=7)  ***REMOVED*** Keep fingerprints for 7 days max
