...
class PreferenceManager:
    ...
    ***REMOVED*** Default preferences for new users
    ...
    async def should_send(
        ...
        Returns:
            True if should send
        """
        ***REMOVED*** Check if type is enabled
        ...
        ***REMOVED*** Check quiet hours
        if await self._is_in_quiet_hours(preferences):
            ***REMOVED*** Allow critical notifications during quiet hours
    ...
    async def _is_in_quiet_hours(self, preferences: UserPreferences) -> bool:
        ...
        ***REMOVED*** Get current hour in user's timezone
        ***REMOVED*** NOTE: For simplicity, using UTC. In production, use user's timezone
        current_hour = datetime.utcnow().hour

        start = preferences.quiet_hours_start
        end = preferences.quiet_hours_end

        ***REMOVED*** Handle overnight quiet hours (e.g., 22:00 - 06:00)
        if start > end:
    ...
    async def update_preferences(
        ...
        """
        ***REMOVED*** Load existing or create new
        result = await self.db.execute(
            select(NotificationPreferenceRecord).where(
                NotificationPreferenceRecord.user_id == user_id
            )
        ...
        if record:
            ...
            ***REMOVED*** Set digest fields if they exist on model
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        """
        ***REMOVED*** Count users by channel preferences
