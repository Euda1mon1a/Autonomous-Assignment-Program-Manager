...
class NotificationPreferences(BaseModel):
    ...
    quiet_hours_start: Optional[int] = None  ***REMOVED*** Hour (0-23)
    quiet_hours_end: Optional[int] = None  ***REMOVED*** Hour (0-23)
...
class NotificationService:
    ...
    async def send_notification(
        ...
        """
        ***REMOVED*** Render the notification template
        ...
        ***REMOVED*** Determine which channels to use
        target_channels = channels or rendered.get("channels", ["in_app"])

        ***REMOVED*** NOTE: User preference filtering disabled until NotificationPreference model exists
        ***REMOVED*** Uncomment when ready:
        ***REMOVED*** preferences = self._get_user_preferences(recipient_id)
        ***REMOVED*** if not self._should_send_notification(preferences, notification_type):
        ***REMOVED***     return []

        ***REMOVED*** Create notification payload
        ...
        ***REMOVED*** Deliver through each channel
        results = []
    ...
    def schedule_notification(
        ...
        ***REMOVED*** NOTE: Using in-memory queue. For production, persist to database
        ***REMOVED*** using ScheduledNotification SQLAlchemy model (not yet created)
    ...
    async def process_scheduled_notifications(self) -> int:
        ...
        ***REMOVED*** NOTE: Using in-memory queue. Replace with database query when model exists
        due_notifications = [
        ...
        for notification in due_notifications:
            try:
                ...
                ***REMOVED*** Update status based on results
    ...
    def get_pending_notifications(
        ...
        """
        ***REMOVED*** TODO: Query database when notification model exists
        ***REMOVED*** For now, return empty list
        ***REMOVED*** notifications = (
        ***REMOVED***     self.db.query(Notification)
        ***REMOVED***     .filter(Notification.recipient_id == user_id)
        ***REMOVED*** )
        ***REMOVED***
        ***REMOVED*** if unread_only:
        ***REMOVED***     notifications = notifications.filter(Notification.is_read == False)
        ***REMOVED***
        ***REMOVED*** notifications = notifications.order_by(
        ***REMOVED***     Notification.created_at.desc()
        ***REMOVED*** ).limit(limit).all()
    ...
    def mark_as_read(self, notification_ids: List[UUID]) -> int:
        ...
        """
        ***REMOVED*** TODO: Update database when notification model exists
        ***REMOVED*** count = (
        ***REMOVED***     self.db.query(Notification)
        ***REMOVED***     .filter(Notification.id.in_(notification_ids))
        ***REMOVED***     .update({"is_read": True, "read_at": datetime.utcnow()})
        ***REMOVED*** )
    ...
    def _get_user_preferences(self, user_id: UUID) -> NotificationPreferences:
        ...
        """
        ***REMOVED*** TODO: Load from database when preference model exists
        ***REMOVED*** For now, return default preferences
    ...
    def _should_send_notification(
        ...
        """
        ***REMOVED*** Check if notification type is enabled
        if not preferences.notification_types.get(notification_type.value, True):
            return False

        ***REMOVED*** Check quiet hours
        if preferences.quiet_hours_start and preferences.quiet_hours_end:
            ...
            if preferences.quiet_hours_start <= current_hour < preferences.quiet_hours_end:
                ***REMOVED*** Don't send during quiet hours unless it's high priority
                return False

        return True


***REMOVED*** Convenience functions for common notification scenarios
