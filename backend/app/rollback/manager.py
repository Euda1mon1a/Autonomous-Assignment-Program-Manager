...
Usage:
    ...
    ***REMOVED*** Create rollback manager
    manager = RollbackManager(db)

    ***REMOVED*** Create rollback point
...
***REMOVED*** Optional import - sqlalchemy_continuum may not be installed
try:
...
class RollbackScope(str, Enum):
    ...
    FULL = "full"  ***REMOVED*** Rollback entire snapshot
    SELECTIVE = "selective"  ***REMOVED*** Rollback specific entities
    CASCADING = "cascading"  ***REMOVED*** Rollback with dependencies
...
class RollbackManager:
    ...
    ***REMOVED*** Default rollback point expiration (days)
    DEFAULT_EXPIRATION_DAYS = 30

    ***REMOVED*** Maximum number of entities per rollback point (prevent memory issues)
    MAX_ENTITIES_PER_ROLLBACK = 10000

    ***REMOVED*** Entity types supported for rollback
    ...
    ***REMOVED*** Roles authorized to create/execute rollbacks
    AUTHORIZED_ROLES = {"admin", "coordinator"}

    ***REMOVED*** Storage directory for rollback points
    ...
    def create_rollback_point(
        ...
        ***REMOVED*** Default to all entity types if none specified
        ...
        ***REMOVED*** Capture entity snapshots
        ...
        ***REMOVED*** Check entity count limit
        ...
        ***REMOVED*** Create rollback point object
        ...
        ***REMOVED*** Persist rollback point (stored as JSON in metadata table or file system)
    ...
    def execute_rollback(
        ...
        ***REMOVED*** Determine scope
        effective_scope = scope or rollback_point.scope

        ***REMOVED*** Determine entities to restore
        ...
        try:
            ***REMOVED*** Update rollback point status
            if not dry_run:
                rollback_point.status = RollbackStatus.IN_PROGRESS
                self._persist_rollback_point(rollback_point)

            ***REMOVED*** Restore each entity
            ...
            ***REMOVED*** Commit transaction if not dry run
            ...
            ***REMOVED*** Verify rollback if requested
            ...
            ***REMOVED*** Log rollback execution
    ...
    def verify_rollback(
        ...
        ***REMOVED*** Determine which snapshots to verify
        ...
        ***REMOVED*** Verify each entity
        for snapshot in snapshots_to_verify:
            try:
                ...
                ***REMOVED*** Compare states (ignore timestamps and audit fields)
        ...
        ***REMOVED*** Determine verification result
    ...
    def list_rollback_points(
        ...
        """
        ***REMOVED*** In production, this would query a database table or file system
    ...
    def get_rollback_history(
        ...
        ***REMOVED*** Query all rollback points that include this entity
        rollback_points = self._list_persisted_rollback_points(limit=1000)

        history = []
        for point in rollback_points:
            ***REMOVED*** Check if entity is in this rollback point
        ...
        ***REMOVED*** Sort by timestamp descending
    ...
    ***REMOVED*** =========================================================================
    ...
    def _capture_entity_snapshots(
        ...
        if entity_ids:
            ***REMOVED*** Snapshot specific entities
            ...
            ***REMOVED*** Snapshot all entities of specified types
    ...
    def _create_entity_snapshot(
        ...
        try:
            ...
            ***REMOVED*** Get current version ID if versioned
            version_id = None
            if HAS_CONTINUUM and version_class:
                try:
                    ...
                    pass  ***REMOVED*** Entity may not be versioned

            ***REMOVED*** Serialize entity state
            state = self._serialize_entity(entity)

            ***REMOVED*** Detect dependencies
    ...
    def _snapshot_all_entities_of_type(
        ...
        snapshots = []
        try:
            ***REMOVED*** Get all entities (with limit to prevent memory issues)
            entities = (
    ...
    def _serialize_entity(self, entity: Any) -> dict[str, Any]:
        ...
        for column in entity.__table__.columns:
            ...
            ***REMOVED*** Convert to JSON-serializable format
    ...
    def _restore_entity(self, snapshot: EntitySnapshot) -> None:
        ...
        ***REMOVED*** Get current entity
        ...
        if not entity:
            ***REMOVED*** Entity was deleted, recreate it
            entity = model_class(id=snapshot.entity_id)
            self.db.add(entity)

        ***REMOVED*** Restore state
        for key, value in snapshot.state.items():
            if key == "id":
                continue  ***REMOVED*** Don't overwrite ID

            ***REMOVED*** Convert string back to proper type if needed
            column = getattr(model_class, key, None)
            if column is not None:
                ***REMOVED*** Handle datetime conversion
                ...
                ***REMOVED*** Handle UUID conversion
        ...
        ***REMOVED*** Mark entity as modified
        self.db.flush()
    ...
    def _resolve_dependencies(
        ...
        ***REMOVED*** BFS to find all dependencies
    ...
    ***REMOVED*** =========================================================================
    ***REMOVED*** Persistence Methods (JSON File-Based Storage)
    ***REMOVED*** =========================================================================
    ...
    def _persist_rollback_point(self, rollback_point: RollbackPoint) -> None:
        ...
        """
        try:
            ***REMOVED*** Serialize rollback point to dict
    ...
    def _list_persisted_rollback_points(self, limit: int = 100) -> list[RollbackPoint]:
        ...
        try:
            ...
            ***REMOVED*** Load rollback points (up to limit)
            rollback_points = []
    ...
    def _serialize_rollback_point(self, rollback_point: RollbackPoint) -> dict[str, Any]:
        ...
        ***REMOVED*** Convert UUIDs, datetimes, enums to strings
        data["id"] = str(data["id"])
        ...
        ***REMOVED*** Serialize entity snapshots
        for snapshot in data["entity_snapshots"]:
            ...
            ***REMOVED*** Convert dependency tuples (type, UUID) to serializable format
    ...
    def _deserialize_rollback_point(self, data: dict[str, Any]) -> RollbackPoint:
        ...
        """
        ***REMOVED*** Convert string UUIDs back to UUID objects
        data["id"] = UUID(data["id"])
        data["created_by"] = UUID(data["created_by"])

        ***REMOVED*** Convert ISO strings back to datetime
        ...
        ***REMOVED*** Convert enum strings back to enums
        data["status"] = RollbackStatus(data["status"])
        data["scope"] = RollbackScope(data["scope"])

        ***REMOVED*** Deserialize entity snapshots
        snapshots = []
        for snapshot_data in data["entity_snapshots"]:
            ...
            ***REMOVED*** Convert dependency lists back to tuples
    ...
    def _update_rollback_index(self, rollback_point: RollbackPoint) -> None:
        ...
        try:
            ...
            ***REMOVED*** Remove existing entry if present
            index["rollback_points"] = [
                entry
