...
class ErrorHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def handle_error(self, request: Request, exc: Exception) -> JSONResponse:
        ...
        """
        ***REMOVED*** Get exception mapping
        mapping = get_exception_mapping(exc)

        ***REMOVED*** Override status code for HTTPException
        ...
        ***REMOVED*** Create error context
        context = ErrorContext(request, exc)

        ***REMOVED*** Generate error fingerprint
        ...
        ***REMOVED*** Get request ID if available
        request_id = request.headers.get("X-Request-ID")

        ***REMOVED*** Determine if we should include details
        include_details = mapping.include_details or settings.DEBUG

        ***REMOVED*** Determine if we should include stack trace
        include_stack_trace = settings.DEBUG

        ***REMOVED*** Format error response
    ...
    async def _report_error(
        ...
        try:
            ...
            ***REMOVED*** Check rate limiting
            ...
            if should_report:
                ***REMOVED*** Report to all configured reporters
                ...
                ***REMOVED*** Log that we're rate limiting
            ...
            ***REMOVED*** Never let error reporting crash the request
...
def create_error_handler(exc_type: type) -> Callable:
    ...
    async def handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        ***REMOVED*** Create error handler middleware instance
        middleware = ErrorHandlerMiddleware(
            app=request.app,
            enable_reporting=True,
        )

        ***REMOVED*** Handle the error
...
def install_error_handlers(app: FastAPI, enable_middleware: bool = True) -> None:
    ...
    if enable_middleware:
        ...
        ***REMOVED*** Register handlers for specific exception types
        exception_types = [
            AppException,
            NotFoundError,
            ValidationError,
...
***REMOVED*** Convenience function for backward compatibility
