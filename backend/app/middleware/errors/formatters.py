...
class ProblemDetail(BaseModel):
    ...
    ***REMOVED*** Extension members (RFC 7807 allows additional fields)
    ...
    ***REMOVED*** Optional fields for additional context
...
class ErrorFormatter:
    ...
    def format_error(
        ...
        """
        ***REMOVED*** Generate unique error ID
        error_id = str(uuid4())

        ***REMOVED*** Build type URI
        type_uri = (
            f"{self.base_url}/errors/{error_code.value.lower().replace('_', '-')}"
        )

        ***REMOVED*** Get error detail message
        detail = self._get_error_detail(exc, include_details)

        ***REMOVED*** Build base problem detail
        ...
        ***REMOVED*** Add validation errors if Pydantic ValidationError
        if isinstance(exc, PydanticValidationError):
            problem.errors = self._format_validation_errors(exc)

        ***REMOVED*** Add stack trace in development mode
    ...
    def _get_error_detail(self, exc: Exception, include_details: bool) -> str:
        ...
        Returns:
            Error detail string
        """
        ***REMOVED*** For AppException, use the safe message attribute
        if hasattr(exc, "message"):
            return exc.message

        ***REMOVED*** For other exceptions, include details only if allowed
        if include_details:
            return str(exc) if str(exc) else "An error occurred"

        ***REMOVED*** Generic message for exceptions that shouldn't expose details
    ...
    def _format_validation_errors(
        ...
        for error in exc.errors():
            ...
            ***REMOVED*** Add input value context if available (sanitized)
            if "input" in error and error["input"] is not None:
                ***REMOVED*** Don't include sensitive values (passwords, tokens, etc.)
    ...
    def _format_stack_trace(self, exc: Exception) -> list[str]:
        ...
        ***REMOVED*** Format traceback
        tb_lines = traceback.format_exception(type(exc), exc, exc.__traceback__)

        ***REMOVED*** Clean up and return as list
...
***REMOVED*** Global formatter instance
