...
class ErrorContext:
    ...
    def get_request_info(self, include_headers: bool = False) -> Dict[str, Any]:
        ...
        ***REMOVED*** Add client information
        ...
        ***REMOVED*** Add headers if requested (sanitized)
        if include_headers:
            info["headers"] = self._sanitize_headers(self.request.headers)

        ***REMOVED*** Add request ID if available
        request_id = self.request.headers.get("X-Request-ID")
        if request_id:
            info["request_id"] = request_id

        ***REMOVED*** Add user information if authenticated
    ...
    def _sanitize_headers(self, headers: Headers) -> Dict[str, str]:
        ...
        """
        ***REMOVED*** List of headers to exclude (case-insensitive)
        sensitive_headers = {
            "authorization",
            "cookie",
            "x-api-key",
    ...
    def get_exception_info(self, include_args: bool = True) -> Dict[str, Any]:
        ...
        ***REMOVED*** Add exception arguments if requested
        if include_args and hasattr(self.exc, "args"):
            ***REMOVED*** Filter out potentially sensitive args
            info["args"] = [
                self._sanitize_arg(arg) for arg in self.exc.args
            ]

        ***REMOVED*** Add custom exception attributes
    ...
    def _sanitize_arg(self, arg: Any) -> Any:
        ...
        ***REMOVED*** Check for sensitive patterns
        sensitive_patterns = [
            "password",
            "secret",
            "token",
        ...
        ***REMOVED*** Truncate long arguments
        if len(arg_str) > 200:
            return arg_str[:200] + "..."
...
class ErrorFingerprinter:
    ...
    def generate_fingerprint(
        ...
        """
        ***REMOVED*** Build fingerprint components
        components = [
            type(exc).__name__,
            ErrorFingerprinter._normalize_path(request_path),
        ]

        ***REMOVED*** Optionally include message for more specific grouping
        if include_message:
            ***REMOVED*** Normalize message to group similar errors
    ...
    def _normalize_path(path: str) -> str:
        ...
        ***REMOVED*** Replace UUIDs with placeholder
        path = re.sub(
        ...
        ***REMOVED*** Replace numeric IDs with placeholder
    ...
    def _normalize_message(message: str) -> str:
        ...
        ***REMOVED*** Remove numeric IDs
        message = re.sub(r"\b\d{4,}\b", "{id}", message)

        ***REMOVED*** Remove timestamps (various formats)
        message = re.sub(
...
class ErrorRateLimiter:
    ...
    def should_report(self, fingerprint: str) -> bool:
        ...
        ***REMOVED*** Initialize or get existing timestamps for this fingerprint
        if fingerprint not in self._error_counts:
            self._error_counts[fingerprint] = []

        timestamps = self._error_counts[fingerprint]

        ***REMOVED*** Remove timestamps outside the window
        cutoff = now.timestamp() - self.window_seconds
        timestamps[:] = [ts for ts in timestamps if ts > cutoff]

        ***REMOVED*** Check if we're under the limit
    ...
    def get_error_count(self, fingerprint: str) -> int:
        ...
        ***REMOVED*** Clean up old timestamps
        timestamps = self._error_counts[fingerprint]
        timestamps[:] = [ts for ts in timestamps if ts > cutoff]
