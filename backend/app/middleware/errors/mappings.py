...
***REMOVED*** Exception type to mapping configuration
EXCEPTION_MAPPINGS: dict[type[Exception], ExceptionMapping] = {
    ***REMOVED*** Application exceptions (custom hierarchy)
    ...
    ***REMOVED*** Generic AppException (fallback)
    ...
    ***REMOVED*** FastAPI HTTPException
    HTTPException: ExceptionMapping(
        status_code=500,  ***REMOVED*** Will be overridden by exception's status_code
    ...
    ***REMOVED*** Python built-in exceptions
    ...
    ***REMOVED*** Catch-all for unexpected exceptions
...
def get_exception_mapping(exc: Exception) -> ExceptionMapping:
    ...
    ***REMOVED*** Walk the MRO to find closest parent class with mapping
    for base_class in exc_type.__mro__:
        if base_class in EXCEPTION_MAPPINGS:
            return EXCEPTION_MAPPINGS[base_class]

    ***REMOVED*** Fallback to generic Exception mapping
