...
class RequestStatus(str, Enum):
    ...
    PROCESSING = "processing"  ***REMOVED*** Request currently being processed
    COMPLETED = "completed"  ***REMOVED*** Request completed successfully
    FAILED = "failed"  ***REMOVED*** Request failed with error
...
class RequestRecord:
    ...
    idempotency_key: str  ***REMOVED*** Unique idempotency key
    status: RequestStatus  ***REMOVED*** Current processing status
    created_at: float  ***REMOVED*** Timestamp when request started
    completed_at: Optional[float] = None  ***REMOVED*** Timestamp when request completed
    ttl: int = 300  ***REMOVED*** Time-to-live in seconds
    response_status: Optional[int] = None  ***REMOVED*** HTTP status code
    response_headers: Optional[dict[str, str]] = None  ***REMOVED*** Response headers
    response_body: Optional[bytes] = None  ***REMOVED*** Cached response body
    error_message: Optional[str] = None  ***REMOVED*** Error message if failed
...
class DeduplicationStorage:
    ...
    ***REMOVED*** Default timeouts
    DEFAULT_TTL = 300  ***REMOVED*** 5 minutes
    LOCK_TIMEOUT = 60  ***REMOVED*** 60 seconds for lock
    LOCK_POLLING_INTERVAL = 0.1  ***REMOVED*** 100ms between lock checks
    ...
    async def acquire_lock(
        ...
        """
        if not self.redis:
            ***REMOVED*** Fallback to local lock if Redis unavailable
        ...
        try:
            ***REMOVED*** Try to acquire lock with expiry
    ...
    async def release_lock(self, idempotency_key: str, lock_id: Optional[str]) -> bool:
        ...
        if not self.redis or not lock_id:
            ***REMOVED*** Release local lock if using fallback
        ...
        try:
            ***REMOVED*** Only release if we own the lock (atomic check-and-delete)
            lua_script = """
    ...
    async def wait_for_completion(
        ...
        while time.time() - start_time < timeout:
            ...
            ***REMOVED*** Wait before polling again
    ...
    async def save_record(self, record: RequestRecord) -> bool:
        ...
        try:
            ***REMOVED*** Serialize record
            data = pickle.dumps(record)

            ***REMOVED*** Save with TTL
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            ...
            while True:
                ...
                for key in keys:
                    ***REMOVED*** Check if expired and delete
                    data = await self.redis.get(key)
                    if data:
                        record = pickle.loads(data)
                        if record.is_expired():
