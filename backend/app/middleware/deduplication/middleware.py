...
class DeduplicationMiddleware(BaseHTTPMiddleware):
    ...
    Usage:
        ...
        ***REMOVED*** Client sends request with idempotency key
        POST /api/v1/schedules
        Headers:
            Idempotency-Key: unique-key-123

        ***REMOVED*** Duplicate request returns cached response
        POST /api/v1/schedules
        Headers:
            Idempotency-Key: unique-key-123
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize storage and service
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        """
        ***REMOVED*** Skip if middleware disabled
        if not self.enabled:
            return await call_next(request)

        ***REMOVED*** Read body once and store in request state
        ***REMOVED*** (so it can be consumed by both middleware and endpoint)
        await self._store_body(request)

        ***REMOVED*** Extract idempotency key
        idempotency_key = self.service.extract_idempotency_key(request)

        ***REMOVED*** If no idempotency key, process normally
        if not idempotency_key:
            return await call_next(request)

        ***REMOVED*** Check for duplicate request
        is_duplicate, existing_record = await self.service.check_duplicate(
            idempotency_key
        )

        if is_duplicate and existing_record:
            ***REMOVED*** Handle duplicate request
        ...
        ***REMOVED*** Start processing new request
        started, lock_id = await self.service.start_processing(idempotency_key)

        if not started:
            ***REMOVED*** Failed to acquire lock - another request is processing
            ***REMOVED*** Wait for it to complete
            logger.info(
        ...
        ***REMOVED*** Process request
        try:
            ***REMOVED*** Add idempotency info to request state
            request.state.idempotency_key = idempotency_key
            request.state.dedup_lock_id = lock_id

            ***REMOVED*** Call next middleware/endpoint
            response = await call_next(request)

            ***REMOVED*** Cache response if successful
            if 200 <= response.status_code < 300:
                ...
                ***REMOVED*** Mark as failed for non-2xx responses
            ...
            ***REMOVED*** Add deduplication headers
            ...
            ***REMOVED*** Mark as failed on exception
            await self.service.fail_processing(
                idempotency_key,
                lock_id,
                str(e),
    ...
    async def _handle_duplicate(
        ...
        ***REMOVED*** If request still processing, wait for completion
        ...
        ***REMOVED*** If request failed, return error
        if record.is_failed():
            return JSONResponse(
        ...
        ***REMOVED*** Return cached response
        if record.response_body is not None:
            ***REMOVED*** Build response headers
        ...
        ***REMOVED*** No cached response (shouldn't happen)
    ...
    async def _cache_response(
        ...
        """
        try:
            ***REMOVED*** Read response body
            response_body = b""
            if hasattr(response, "body"):
                response_body = response.body
            elif hasattr(response, "body_iterator"):
                ***REMOVED*** For streaming responses, collect chunks
                chunks = []
                async for chunk in response.body_iterator:
                    chunks.append(chunk)
                response_body = b"".join(chunks)

                ***REMOVED*** Replace iterator with single chunk
                async def body_iterator():
                    yield response_body

                response.body_iterator = body_iterator()

            ***REMOVED*** Filter headers to cache (exclude hop-by-hop headers)
            headers_to_cache = {}
            skip_headers = {
                "connection",
                "keep-alive",
            ...
            ***REMOVED*** Complete processing with cached response
            ...
            ***REMOVED*** Don't fail the request, just log the error
            await self.service.fail_processing(
                idempotency_key,
                lock_id,
