...
class DeduplicationConfig:
    ...
    ***REMOVED*** HTTP methods that support idempotency
    IDEMPOTENT_METHODS = {"POST", "PUT", "PATCH", "DELETE"}

    ***REMOVED*** Header name for idempotency key
    IDEMPOTENCY_HEADER = "Idempotency-Key"

    ***REMOVED*** Endpoints that support deduplication (regex patterns)
    DEDUP_ENDPOINTS = [
    ...
    ***REMOVED*** Endpoints that are excluded from deduplication
    EXCLUDE_ENDPOINTS = [
        r"^/health$",
        r"^/metrics$",
        r"^/docs",
    ...
    ***REMOVED*** Default TTL for cached responses (5 minutes)
    DEFAULT_TTL = 300

    ***REMOVED*** Maximum wait time for concurrent requests (30 seconds)
    MAX_WAIT_TIME = 30.0
    ...
    def is_endpoint_supported(cls, path: str, method: str) -> bool:
        ...
        ***REMOVED*** Check exclusions first
        for pattern in cls.EXCLUDE_ENDPOINTS:
            if re.match(pattern, path):
                return False

        ***REMOVED*** Check if endpoint is in supported list
...
class DeduplicationService:
    ...
    def extract_idempotency_key(self, request: Request) -> Optional[str]:
        ...
        """
        ***REMOVED*** Check if endpoint supports deduplication
        if not self.config.is_endpoint_supported(
            request.url.path, request.method
        ):
            return None

        ***REMOVED*** 1. Try to get from header
        header_key = request.headers.get(self.config.IDEMPOTENCY_HEADER)
        if header_key:
            logger.debug(f"Idempotency key from header: {header_key}")
            return header_key

        ***REMOVED*** 2. Try to get from request body (if available in state)
        ***REMOVED*** Note: Body is consumed by middleware, stored in request.state
        ...
        ***REMOVED*** 3. Auto-generate from request fingerprint
        ***REMOVED*** (only for specific endpoints where it's safe)
    ...
    def _should_auto_generate(self, request: Request) -> bool:
        ...
        """
        ***REMOVED*** Only auto-generate for specific safe endpoints
        auto_generate_patterns = [
            r"^/api/v1/schedules/generate$",  ***REMOVED*** Safe: schedule params are unique
        ]
    ...
    def _generate_request_fingerprint(self, request: Request) -> str:
        ...
        """
        ***REMOVED*** Collect request components
        components = [
            request.method,
            request.url.path,
    ...
    async def check_duplicate(
        ...
        ***REMOVED*** Check if record is still valid
    ...
    async def handle_duplicate(
        ...
        """
        ***REMOVED*** If request is still processing, wait for completion
        ...
        ***REMOVED*** Request already completed, return cached record
        return record
    ...
    async def start_processing(
        ...
        ***REMOVED*** Create processing record
        try:
            ...
            ***REMOVED*** Release lock if record creation fails
    ...
    async def complete_processing(
        ...
        """
        try:
            ***REMOVED*** Update record with response
    ...
    async def fail_processing(
        ...
        """
        try:
            ***REMOVED*** Update record with error
            updated = await self.storage.update_record(
                idempotency_key=idempotency_key,
