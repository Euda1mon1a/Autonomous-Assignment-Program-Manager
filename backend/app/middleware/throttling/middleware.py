...
class ThrottlingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize Redis storage
        ...
        ***REMOVED*** Initialize strategy
        self.strategy = self._create_strategy(strategy)

        ***REMOVED*** Track request IDs for cleanup
    ...
    def _should_skip_throttling(self, request: Request) -> bool:
        ...
        Returns:
            True if should skip
        """
        ***REMOVED*** Skip certain endpoints
        skip_paths = [
            "/health",
            "/metrics",
            "/docs",
    ...
    async def _wait_for_slot(
        ...
        while time.time() - start_time < timeout:
            ***REMOVED*** Try to acquire slot
            acquired, _ = await self.storage.acquire_slot(
                request_id=request_id,
                user_id=None,  ***REMOVED*** Will be set by caller
                endpoint="",  ***REMOVED*** Will be set by caller
                priority="normal",
                max_concurrent=100,  ***REMOVED*** Default
                timeout=timeout,
            )
            ...
            ***REMOVED*** Wait a bit before retrying
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        """
        ***REMOVED*** Skip if should not throttle
        ...
        ***REMOVED*** Get endpoint and priority
        endpoint = request.url.path
        priority = get_priority_for_endpoint(endpoint)

        ***REMOVED*** Get throttling configuration
        ...
        ***REMOVED*** Try to acquire slot (for ALLOW action or after queue)
        ...
        ***REMOVED*** Process request
        ...
        try:
            ...
            ***REMOVED*** Add throttling info to request state
            ...
            ***REMOVED*** Add throttling headers
