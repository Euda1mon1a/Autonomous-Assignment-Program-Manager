...
class RequestSlot:
    ...
    request_id: str  ***REMOVED*** Unique request identifier
    user_id: Optional[str]  ***REMOVED*** User making the request
    endpoint: str  ***REMOVED*** Endpoint being accessed
    priority: str  ***REMOVED*** Request priority level
    started_at: float  ***REMOVED*** Timestamp when request started
    timeout_at: float  ***REMOVED*** Timestamp when request times out
...
class QueuedRequest:
    ...
    endpoint: str  ***REMOVED*** Endpoint being accessed
    priority: str  ***REMOVED*** Request priority level
    queued_at: float  ***REMOVED*** Timestamp when request was queued
    timeout_at: float  ***REMOVED*** Timestamp when request times out
...
class ThrottleMetrics:
    ...
    active_requests: int  ***REMOVED*** Current active requests
    queued_requests: int  ***REMOVED*** Current queued requests
    total_capacity: int  ***REMOVED*** Total capacity
    utilization: float  ***REMOVED*** Current utilization (0.0 to 1.0)
    queue_utilization: float  ***REMOVED*** Queue utilization (0.0 to 1.0)
...
class ThrottleStorage:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None):
        ...
        self._local_locks: dict[str, asyncio.Lock] = {}  ***REMOVED*** Fallback local locks
    ...
    async def acquire_slot(
        ...
        """
        if not self.redis:
            ***REMOVED*** Fallback: always allow if Redis unavailable
            return True, None
        ...
        try:
            ***REMOVED*** Create request slot data
            slot = RequestSlot(
            ...
            ***REMOVED*** Use Lua script for atomic check-and-set
            lua_script = """
            local key = KEYS[1]
    ...
    async def release_slot(self, request_id: str) -> bool:
        ...
        try:
            ***REMOVED*** Remove all entries with this request_id prefix
            ***REMOVED*** (format is request_id:slot_data)
            pattern = f"{request_id}:*"

            ***REMOVED*** Get all members
            members = await self.redis.zrange(key, 0, -1)

            ***REMOVED*** Find and remove matching members
    ...
    async def enqueue_request(
        ...
        try:
            ***REMOVED*** Create queued request data
            ...
            ***REMOVED*** Priority score: combine priority level with timestamp
            ***REMOVED*** Lower score = higher priority
            priority_scores = {
                "critical": 1000,
                "high": 2000,
                "normal": 3000,
            ...
            score = priority_score + now  ***REMOVED*** Add timestamp for FIFO within priority

            ***REMOVED*** Check queue size and add
            lua_script = """
            local key = KEYS[1]
    ...
    async def dequeue_request(self, request_id: str) -> bool:
        ...
        try:
            ***REMOVED*** Find and remove the request
            members = await self.redis.zrange(key, 0, -1)
    ...
    async def get_metrics(
        ...
        try:
            ...
            ***REMOVED*** Clean up expired entries
