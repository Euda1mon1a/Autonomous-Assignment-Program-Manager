...
class ThrottleAction(str, Enum):
    ...
    ALLOW = "allow"  ***REMOVED*** Allow request immediately
    QUEUE = "queue"  ***REMOVED*** Queue request for later processing
    REJECT = "reject"  ***REMOVED*** Reject request immediately
    DEGRADE = "degrade"  ***REMOVED*** Allow but mark as degraded
...
class QueuedThrottleStrategy(ThrottleStrategy):
    ...
    async def decide(
        ...
        ***REMOVED*** Allow if capacity available
        if metrics.utilization < 1.0:
            return ThrottleDecision(
                action=ThrottleAction.ALLOW,
                reason="Capacity available",
        ...
        ***REMOVED*** Queue if space available
        if metrics.queue_utilization < 1.0:
            ***REMOVED*** Estimate wait time based on queue position
            wait_time = metrics.queued_requests * 0.5  ***REMOVED*** Assume 0.5s per request
            return ThrottleDecision(
...
class PriorityThrottleStrategy(ThrottleStrategy):
    ...
    async def decide(
        ...
        ***REMOVED*** Critical priority - always try to allow
        if priority == ThrottlePriority.CRITICAL:
            if utilization < 0.98:  ***REMOVED*** Allow up to 98% for critical
                return ThrottleDecision(
                    action=ThrottleAction.ALLOW,
        ...
        ***REMOVED*** High priority - generous limits
        ...
        ***REMOVED*** Normal priority - standard throttling
        ...
        ***REMOVED*** Low priority - conservative limits
        ...
        ***REMOVED*** Background priority - strictest limits
        if utilization < 0.60:
            return ThrottleDecision(
                action=ThrottleAction.ALLOW,
...
class AdaptiveThrottleStrategy(ThrottleStrategy):
    ...
    async def decide(
        ...
        ***REMOVED*** Normal operation (< 70% utilization)
        ...
        ***REMOVED*** Warning level (70-80% utilization)
        elif utilization < DEGRADATION_THRESHOLDS["throttle"]:
            ***REMOVED*** Allow critical and high
        ...
        ***REMOVED*** Throttle level (80-90% utilization)
        elif utilization < DEGRADATION_THRESHOLDS["reject"]:
            ***REMOVED*** Critical: Allow or queue
            if priority == ThrottlePriority.CRITICAL:
                if utilization < 0.95:
                    return ThrottleDecision(
                        action=ThrottleAction.ALLOW,
            ...
            ***REMOVED*** High: Allow with limits
            ...
            ***REMOVED*** Reject normal and lower
            return ThrottleDecision(
                action=ThrottleAction.REJECT,
        ...
        ***REMOVED*** Reject level (90-95% utilization)
        elif utilization < DEGRADATION_THRESHOLDS["critical"]:
            ***REMOVED*** Only critical and high priority
            if priority == ThrottlePriority.CRITICAL:
                if utilization < 0.98:
                    return ThrottleDecision(
                        action=ThrottleAction.ALLOW,
        ...
        ***REMOVED*** Critical/Emergency level (> 95% utilization)
        else:
            ***REMOVED*** Only critical priority allowed
