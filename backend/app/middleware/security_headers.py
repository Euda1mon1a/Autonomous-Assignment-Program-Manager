...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self,
        app: ASGIApp,
        hsts_max_age: int = 31536000,  ***REMOVED*** 1 year in seconds
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        ***REMOVED*** Prevent MIME-type sniffing
        response.headers["X-Content-Type-Options"] = "nosniff"

        ***REMOVED*** Prevent clickjacking (use DENY for APIs, SAMEORIGIN for web apps)
        response.headers["X-Frame-Options"] = "DENY"

        ***REMOVED*** Legacy XSS protection for older browsers
        ***REMOVED*** Modern browsers use CSP instead, but this helps older clients
        response.headers["X-XSS-Protection"] = "1; mode=block"

        ***REMOVED*** Control referrer information
        ***REMOVED*** strict-origin-when-cross-origin: sends full URL to same origin,
        ***REMOVED*** only origin to cross-origin, nothing to less secure destination
        response.headers["Referrer-Policy"] = "strict-origin-when-cross-origin"

        ***REMOVED*** HTTP Strict Transport Security (HSTS)
        ***REMOVED*** Only add in production (non-DEBUG mode) to avoid issues in development
        ...
        ***REMOVED*** Default is restrictive for API-only applications
        if self.content_security_policy:
            csp = self.content_security_policy
        else:
            ***REMOVED*** Default CSP for API responses
            csp = (
                "default-src 'none'; "
        ...
        ***REMOVED*** Disable sensitive browser features that APIs don't need
        ...
        ***REMOVED*** Cache-Control for sensitive API responses
        ***REMOVED*** Prevent caching of authenticated responses
        ***REMOVED*** Only set if not already set by the route handler
