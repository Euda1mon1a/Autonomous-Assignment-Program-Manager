...
class AuditContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ***REMOVED*** Set user context for this request
            set_current_user_id(user_id)

            ***REMOVED*** Log audit context for significant operations
            ...
            ***REMOVED*** Always clear context after request
    ...
    async def _extract_user_id(self, request: Request) -> str | None:
        ...
        """
        ***REMOVED*** Check if user was set by auth dependency
        if hasattr(request.state, "user") and request.state.user:
            user = request.state.user
            ***REMOVED*** Handle both User object and dict
            if hasattr(user, "id"):
                return str(user.id)
            if isinstance(user, dict) and "id" in user:
                return str(user["id"])

        ***REMOVED*** Could extend to decode JWT here if needed
        ***REMOVED*** For now, return None for anonymous requests
        return None
