...
class UserContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        """
        ***REMOVED*** Extract user from token
        user_context = await self._extract_user_context(request)

        ***REMOVED*** Store in request state for downstream use
        ...
        ***REMOVED*** Inject into logging context
    ...
    async def _extract_user_context(self, request: Request) -> dict[str, Any]:
        ...
        ***REMOVED*** Try to get user from JWT token
        auth_header = request.headers.get("Authorization")
        if auth_header and auth_header.startswith("Bearer "):
            token = auth_header[7:]

            try:
                ***REMOVED*** Decode token (assuming you have this function)
        ...
        ***REMOVED*** Alternative: Get from session cookie
        if not context["user_id"]:
            session_id = request.cookies.get("session_id")
            if session_id:
                context["session_id"] = session_id
                ***REMOVED*** In production, would fetch user from session store
...
class RoleEnrichmentMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        Returns:
            Response: HTTP response
        """
        ***REMOVED*** Get user role from request state (set by UserContextMiddleware)
        user_role = getattr(request.state, "user_role", None)

        if user_role:
            ***REMOVED*** Add role-specific permissions and context
    ...
    def _get_role_permissions(self, role: str) -> set[str]:
        ...
        """
        ***REMOVED*** Define role-based permissions
        permissions_map = {
            "ADMIN": {
                "read:all",
                "write:all",
