...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        """
        ***REMOVED*** Check if compression is globally disabled
        if not self.config.enabled:
            return await call_next(request)

        ***REMOVED*** Check if path is excluded
        if should_exclude_path(request.url.path, self.config):
            if self.stats:
                self.stats.record_skip()
            return await call_next(request)

        ***REMOVED*** Get client's accepted encodings
        accept_encoding = request.headers.get("Accept-Encoding", "")

        ***REMOVED*** Get encoder for this request
        encoder = self.encoder_factory.get_encoder(accept_encoding)

        if not encoder:
            ***REMOVED*** Client doesn't accept compression
            if self.stats:
                self.stats.record_skip()
            return await call_next(request)
        ...
        ***REMOVED*** Check if response should be compressed
        if not self._should_compress_response(response):
            if self.stats:
                self.stats.record_skip()
            return response

        ***REMOVED*** Compress response
        try:
            ...
            if self.stats:
                self.stats.record_compression(
                    ...
                    compression_time=0.0,  ***REMOVED*** Updated in _compress_response
                )
            return compressed_response
        except Exception as e:
            ***REMOVED*** Log error but return uncompressed response
    ...
    def _should_compress_response(self, response: Response) -> bool:
        ...
        """
        ***REMOVED*** Check if response already has Content-Encoding
        if "content-encoding" in response.headers:
            return False

        ***REMOVED*** Check if response has body
        if not hasattr(response, "body") or not response.body:
            return False

        ***REMOVED*** Check minimum size
    ...
    async def _compress_response(
        ...
        ***REMOVED*** Compress response body
        ***REMOVED*** Use streaming compression for large responses (>100KB)
        ...
        ***REMOVED*** Add Vary header to indicate response varies by encoding
        ...
        ***REMOVED*** Log compression results
        ...
        ***REMOVED*** Update stats with actual compression time
        if self.stats:
