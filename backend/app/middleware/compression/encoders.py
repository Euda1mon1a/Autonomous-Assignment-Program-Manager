...
class GzipEncoder(CompressionEncoder):
    ...
    def compress_stream(self, data: bytes, chunk_size: int = 8192) -> bytes:
        ...
        with gzip.GzipFile(fileobj=output, mode="wb", compresslevel=self.level) as gz:
            ***REMOVED*** Process data in chunks
            offset = 0
...
class BrotliEncoder(CompressionEncoder):
    ...
    def encoding(self) -> str:
        """Get encoding name for Content-Encoding header."""
        return "br"  ***REMOVED*** 'br' is the standard encoding name for brotli
    ...
    def compress_stream(self, data: bytes, chunk_size: int = 8192) -> bytes:
        ...
        ***REMOVED*** Flush any remaining data
...
class EncoderFactory:
    ...
    def get_encoder(self, accept_encoding: str) -> CompressionEncoder | None:
        ...
        ***REMOVED*** Parse Accept-Encoding header
        ***REMOVED*** Format: "encoding1;q=0.9, encoding2, encoding3;q=0.5"
        encodings = self._parse_accept_encoding(accept_encoding)

        ***REMOVED*** Try encoders in order of preference
        ***REMOVED*** Brotli first (better compression)
    ...
    def _parse_accept_encoding(self, header: str) -> set:
        ...
        for part in header.lower().split(","):
            ***REMOVED*** Remove quality value if present (e.g., "gzip;q=0.9" -> "gzip")
