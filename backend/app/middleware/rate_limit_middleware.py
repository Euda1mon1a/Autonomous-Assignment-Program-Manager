...
class RateLimitMiddleware(BaseHTTPMiddleware):
        ...
        if redis_client is None:
            try:
                ...
                self.redis = redis.from_url(
                    redis_url,
                    decode_responses=False,  ***REMOVED*** Keep binary for hash operations
    ...
    def _extract_user_info(self, request: Request) -> tuple[Optional[str], Optional[str]]:
        ...
        """
        ***REMOVED*** Check for Authorization header
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            ***REMOVED*** Check for token in cookie
    ...
    def _get_client_identifier(self, request: Request, user_id: Optional[str]) -> str:
        ...
        ***REMOVED*** Fall back to IP address
    ...
    def _is_internal_service(self, request: Request) -> bool:
        ...
        """
        ***REMOVED*** Check for internal service header
        internal_key = request.headers.get("X-Internal-Service-Key")
        if internal_key:
            ***REMOVED*** In production, validate against stored key
            ***REMOVED*** For now, just check if header exists
            expected_key = settings.SECRET_KEY[:32]  ***REMOVED*** Use part of secret as internal key
            return internal_key == expected_key

        ***REMOVED*** Check if request is from localhost/internal network
        if request.client:
            internal_ips = ["127.0.0.1", "localhost", "::1"]
            if request.client.host in internal_ips:
                ***REMOVED*** Only bypass for specific paths (e.g., health checks, metrics)
    ...
    def _should_skip_rate_limit(self, request: Request) -> bool:
        ...
        Returns:
            True if should skip
        """
        ***REMOVED*** Skip if rate limiting is disabled
        if not settings.RATE_LIMIT_ENABLED:
            return True

        ***REMOVED*** Skip internal services
        if self._is_internal_service(request):
            return True

        ***REMOVED*** Skip health check endpoints
    ...
    def _get_effective_limits(
        ...
        """
        ***REMOVED*** Get base tier config
        tier_config = get_tier_config(tier)

        ***REMOVED*** Check for endpoint-specific limits
        endpoint_limit = get_endpoint_limit(endpoint)

        if endpoint_limit:
            ***REMOVED*** Use endpoint limits if specified, otherwise tier defaults
            ...
            refill = rpm / 60.0  ***REMOVED*** Convert to per-second rate
        else:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        ***REMOVED*** Determine tier
        tier = get_tier_for_role(role)

        ***REMOVED*** Check for custom user limits
        ...
        ***REMOVED*** Apply rate limiting
        ...
        if self.redis and settings.RATE_LIMIT_ENABLED:
            ***REMOVED*** Token bucket check (for bursts)
            ...
            ***REMOVED*** Sliding window check (per minute)
            ...
            ***REMOVED*** Sliding window check (per hour)
    ...
    def _rate_limit_response(self, info: dict, message: str) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            headers={
                ...
                "Retry-After": "60",  ***REMOVED*** Suggest retry after 1 minute
            },
        )
