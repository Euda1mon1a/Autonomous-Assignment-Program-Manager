...
class AcceptHeader:
    ...
    def _parse(self) -> list[tuple[str, float]]:
        ...
        ***REMOVED*** Split by comma to get individual media types
        for part in self.header_value.split(","):
            part = part.strip()
            if not part:
                continue

            ***REMOVED*** Split by semicolon to separate media type from parameters
            components = [c.strip() for c in part.split(";")]
            media_type = components[0].lower()

            ***REMOVED*** Extract quality value (default is 1.0)
            quality = 1.0
            for component in components[1:]:
                if component.startswith("q="):
                    try:
                        quality = float(component[2:])
                        ***REMOVED*** Clamp quality to valid range [0, 1]
        ...
        ***REMOVED*** Sort by quality (highest first), then by specificity
    ...
    def _specificity(self, media_type: str) -> int:
        ...
        ***REMOVED*** */* is least specific
        if type_part == "*" and subtype_part == "*":
            return 0

        ***REMOVED*** type/* is more specific
        if subtype_part == "*":
            return 1

        ***REMOVED*** type/subtype is most specific
        return 2
    ...
    def accepts(self, media_type: str) -> bool:
        ...
        for accepted_type, quality in self.media_types:
            ***REMOVED*** Skip if quality is 0 (explicitly not acceptable)
            if quality == 0:
                continue
    ...
    def get_best_match(self, available_types: list[str]) -> Optional[str]:
        ...
        """
        ***REMOVED*** Try each accepted type in order of preference
        for accepted_type, quality in self.media_types:
            ***REMOVED*** Skip if quality is 0
            if quality == 0:
                continue

            ***REMOVED*** Check for exact match
            if accepted_type in available_types:
                return accepted_type

            ***REMOVED*** Check for wildcard match
            if "/" in accepted_type:
                type_part, subtype_part = accepted_type.split("/", 1)
                ...
                for available_type in available_types:
                    if subtype_part == "*":
                        ...
                        ***REMOVED*** Match */* - return first available
                        return available_type
...
class ContentNegotiationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        """
        ***REMOVED*** Parse request body if present
        ...
        ***REMOVED*** Negotiate response content type
    ...
    async def _parse_request_body(self, request: Request):
        ...
        ***REMOVED*** Get parser for content type
        ...
        try:
            ...
            ***REMOVED*** Store in request state
            ...
            ***REMOVED*** Don't raise - let route handler deal with invalid body
    ...
    async def _negotiate_response(
        ...
        """
        ***REMOVED*** Skip negotiation if response already has content encoding
        if "content-encoding" in response.headers:
            return response

        ***REMOVED*** Skip negotiation if response has no body
        if not hasattr(response, "body") or not response.body:
            return response

        ***REMOVED*** Skip negotiation for non-serializable responses
        current_content_type = response.headers.get("content-type", "")
        if not self._should_negotiate(current_content_type):
            return response

        ***REMOVED*** Parse Accept header
        accept_header = request.headers.get("accept", "")
        accept = AcceptHeader(accept_header)

        ***REMOVED*** Get available serializers
        available_types = self.serializer_registry.get_available_types()

        ***REMOVED*** Find best match
        best_match = accept.get_best_match(available_types)

        ***REMOVED*** Fall back to default if no match
        if not best_match:
            ***REMOVED*** Check if Accept header is present and restrictive
            if accept_header and not accept.accepts("*/*"):
                ***REMOVED*** Client specified Accept header but we can't satisfy it
                logger.debug(
                    f"No acceptable content type found. "
                    f"Accept: {accept_header}, Available: {available_types}"
                )
                ***REMOVED*** Fall back to default anyway (better than 406)
            best_match = self.default_content_type

        ***REMOVED*** Get serializer
        serializer = self.serializer_registry.get_serializer(best_match)

        if not serializer:
            ***REMOVED*** Should not happen, but handle gracefully
            logger.error(f"Serializer not found for type: {best_match}")
            return response

        ***REMOVED*** Skip if already in correct format
        if current_content_type.startswith(best_match):
            return response

        try:
            ***REMOVED*** Parse current response body (assume JSON)
            import json
            data = json.loads(response.body.decode("utf-8"))

            ***REMOVED*** Serialize to negotiated format
            serialized_body = serializer.serialize(data)

            ***REMOVED*** Update response
            ...
            ***REMOVED*** Return original response
        except Exception as e:
    ...
    def _should_negotiate(self, content_type: str) -> bool:
        ...
        ***REMOVED*** Only negotiate for JSON responses
        negotiable_types = [
            "application/json",
            "text/json",
        ]
