...
class XMLParser(Parser):
    ...
    def __init__(self):
        ...
        ***REMOVED*** Try to use lxml (more powerful)
        try:
    ...
    def _element_to_dict(self, element) -> dict:
        ...
        ***REMOVED*** Add element text if present
        if element.text and element.text.strip():
            if len(element) > 0:
                ***REMOVED*** Has children, use special key
                result["_text"] = element.text.strip()
            else:
                ***REMOVED*** No children, text is the value
                return element.text.strip()

        ***REMOVED*** Process children
        children = {}
        for child in element:
            child_data = self._element_to_dict(child)
            tag = child.tag

            ***REMOVED*** Handle multiple children with same tag
            if tag in children:
                ***REMOVED*** Convert to list if not already
...
class FormDataParser(Parser):
    ...
    def parse(self, data: bytes) -> dict:
        ...
        """
        try:
            ***REMOVED*** Decode and parse query string
            query_string = data.decode("utf-8")
            parsed = parse_qs(query_string, keep_blank_values=True)

            ***REMOVED*** Convert single-value lists to strings
            result = {}
...
class ParserRegistry:
    ...
    def get_parser(self, content_type: str) -> Parser | None:
        ...
        """
        ***REMOVED*** Strip parameters from content type (e.g., "application/json; charset=utf-8")
