...
Usage:
    ...
    ***REMOVED*** Basic usage with defaults (JSON)
    app.add_middleware(ContentNegotiationMiddleware)

    ***REMOVED*** Custom default content type
    app.add_middleware(
        ContentNegotiationMiddleware,
        default_content_type="application/xml"
    )

    ***REMOVED*** Register custom serializer
...
Examples:
    ***REMOVED*** Client requests JSON
    GET /api/users
    Accept: application/json

    ***REMOVED*** Client requests XML
    GET /api/users
    Accept: application/xml

    ***REMOVED*** Client requests with quality values
    GET /api/users
    Accept: application/json;q=0.9, application/xml;q=0.8

    ***REMOVED*** POST with JSON
    POST /api/users
    Content-Type: application/json
    {"name": "John"}

    ***REMOVED*** POST with XML
    POST /api/users
