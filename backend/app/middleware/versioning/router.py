...
class VersionedRoute(APIRoute):
    ...
    def get_route_handler(self) -> Callable:
        ...
        async def versioned_handler(request: Request) -> Response:
            ...
            """
            ***REMOVED*** Get current API version
            current_version = get_api_version()

            ***REMOVED*** Check minimum version requirement
            ...
            ***REMOVED*** Check maximum version (sunset)
            if self.max_version and current_version > self.max_version:
                raise HTTPException(
                    status_code=410,  ***REMOVED*** Gone
                    detail=(
            ...
            ***REMOVED*** Check for retired endpoints
            ...
            ***REMOVED*** Add deprecation headers if applicable
...
class VersionedAPIRouter(APIRouter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        ***REMOVED*** Use custom route class
    ...
    def add_api_route(
        ...
        **kwargs,
        ...
        """
        ***REMOVED*** Use router's version requirements if not specified
        if min_version is None:
            min_version = self.min_version
        if max_version is None:
            max_version = self.max_version

        ***REMOVED*** Pass version requirements to route
    ...
    def version_endpoint(
        ...
        **route_kwargs,
        ...
        Usage:
            ...
            def get_users_v1():
                ***REMOVED*** V1 implementation
                pass

            @router.version_endpoint([APIVersion.V2], "/users")
            def get_users_v2():
                ***REMOVED*** V2 implementation with different response format
                pass
        """

        def decorator(func: Callable):
            ***REMOVED*** Determine min/max versions from list
            min_ver = min(versions)
            max_ver = max(versions)

            ***REMOVED*** Add route with version constraints
            self.add_api_route(
                path,
                func,
                min_version=min_ver,
...
***REMOVED*** Utility functions for version-aware routing
...
def version_dispatch(handlers: dict[APIVersion, Callable]):
    ...
    async def dispatcher(*args, **kwargs):
        ...
        ***REMOVED*** Find exact match first
        if current_version in handlers:
            handler = handlers[current_version]
            return await handler(*args, **kwargs)

        ***REMOVED*** Fall back to highest version <= current
        ...
        ***REMOVED*** No compatible handler found
        raise HTTPException(
            status_code=400,
...
***REMOVED*** Deprecation route utilities
