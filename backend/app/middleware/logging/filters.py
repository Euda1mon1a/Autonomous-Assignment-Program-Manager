...
class SensitiveDataFilter:
    ...
    ***REMOVED*** Default sensitive field names (case-insensitive)
    ...
    ***REMOVED*** Patterns for sensitive data (compiled regex)
    SENSITIVE_PATTERNS: List[tuple[re.Pattern, str]] = [
        ***REMOVED*** Credit card numbers (basic pattern)
        (re.compile(r"\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}\b"), "[REDACTED-CARD]"),
        ***REMOVED*** SSN format (XXX-XX-XXXX)
        (re.compile(r"\b\d{3}-\d{2}-\d{4}\b"), "[REDACTED-SSN]"),
        ***REMOVED*** Email addresses
        (re.compile(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b"), "[REDACTED-EMAIL]"),
        ***REMOVED*** Bearer tokens (Authorization header pattern)
        (re.compile(r"Bearer\s+[A-Za-z0-9\-._~+/]+=*", re.IGNORECASE), "Bearer [REDACTED]"),
        ***REMOVED*** API keys (common formats: 32+ hex/alphanumeric chars)
    ...
    def __init__(
        ...
        ***REMOVED*** Compile custom patterns
    ...
    def mask_value(self, value: str, field_name: Optional[str] = None) -> str:
        ...
        ***REMOVED*** For known sensitive fields, use configured masking strategy
        if field_name and field_name.lower() in self.sensitive_fields:
            return self._partial_mask(value)

        ***REMOVED*** Apply pattern-based masking
        masked = value
        for pattern, replacement in self.SENSITIVE_PATTERNS:
            masked = pattern.sub(replacement, masked)

        ***REMOVED*** Apply custom patterns
    ...
    def filter_headers(self, headers: Dict[str, str]) -> Dict[str, str]:
        ...
        for key, value in headers.items():
            ...
            ***REMOVED*** Check if header name contains sensitive keywords
            is_sensitive = any(
            ...
            if is_sensitive:
                ...
                ***REMOVED*** Still apply pattern-based masking to catch tokens in custom headers
                filtered[key] = self.mask_value(value, field_name=key)
    ...
    def should_log_body(self, path: str, method: str) -> bool:
        ...
        """
        ***REMOVED*** Don't log bodies for auth endpoints (even masked, avoid logging)
        sensitive_paths = [
            "/api/v1/auth/login",
...
***REMOVED*** Global filter instance with sensible defaults
default_filter = SensitiveDataFilter(
    show_prefix_chars=0,  ***REMOVED*** Full masking by default
    show_suffix_chars=0,
)
