...
class FileLogStorage(LogStorage):
    ...
    def __init__(
        ...
        self.logger.propagate = False  ***REMOVED*** Don't propagate to root logger

        ***REMOVED*** Remove existing handlers
        self.logger.handlers.clear()

        ***REMOVED*** Create appropriate handler
    ...
    def store(self, log_entry: dict[str, Any]) -> None:
        ...
        ***REMOVED*** Create LogRecord with extra fields
    ...
    def retrieve(
        ...
        try:
            with open(log_path) as f:
                ***REMOVED*** Read last N lines (inefficient for large files)
                lines = f.readlines()
                for line in lines[-limit * 2 :]:  ***REMOVED*** Read extra to account for filtering
                    try:
    ...
    def _matches_filters(self, log: dict[str, Any], filters: dict[str, Any]) -> bool:
        ...
        for key, value in filters.items():
            ***REMOVED*** Support nested keys (e.g., "http.status_code")
            if "." in key:
                parts = key.split(".")
                current = log
                for part in parts:
...
class DatabaseLogStorage(LogStorage):
    ...
    def store(self, log_entry: dict[str, Any]) -> None:
        ...
        """
        ***REMOVED*** Implementation would require creating RequestLog model
        ***REMOVED*** and handling async database operations
        pass
    ...
    def retrieve(
        ...
        ***REMOVED*** Implementation would query RequestLog model
        pass

    def clear(self) -> None:
        """Clear old log entries (with retention policy)."""
        ***REMOVED*** Implementation would delete old records based on retention policy
        pass
...
def get_storage_backend(
    ...
    **kwargs,
    ...
    if storage_type == "memory":
        ...
        ***REMOVED*** Default to memory storage
