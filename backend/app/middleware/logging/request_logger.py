...
class RequestLoggingConfig:
    ...
    def __init__(
        ...
        log_body: bool = True,
        max_body_size: int = 10 * 1024,  ***REMOVED*** 10 KB
        log_response: bool = True,
        max_response_size: int = 10 * 1024,  ***REMOVED*** 10 KB
        excluded_paths: Optional[Set[str]] = None,
        sample_rate: float = 1.0,  ***REMOVED*** 1.0 = log all, 0.5 = log 50%
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        ***REMOVED*** Skip if logging disabled
        ...
        ***REMOVED*** Get request ID (from RequestIDMiddleware or generate new)
        request_id = request.headers.get("X-Request-ID", "unknown")

        ***REMOVED*** Extract user ID if available
    ...
    async def _log_request(
        ...
        ***REMOVED*** Log body (filtered and size-limited)
        body_data = None
        ...
        try:
            ...
            ***REMOVED*** Parse JSON if content-type is application/json
            ...
            if "application/json" in content_type:
                ...
                ***REMOVED*** For non-JSON, just indicate presence
    ...
    async def _log_response(
        ...
        ***REMOVED*** Log response headers (filtered)
        if self.config.log_headers:
            headers_dict = dict(response.headers)
            filtered_headers = self.config.filter.filter_headers(headers_dict)
            log_entry["response_headers"] = filtered_headers

        ***REMOVED*** Determine log level based on status code
    ...
    def _get_log_level(self, path: str) -> str:
        ...
        ***REMOVED*** Check custom log levels
        for prefix, level in self.config.log_levels.items():
            if path.startswith(prefix):
                return level

        ***REMOVED*** Default to INFO
        return "INFO"


***REMOVED*** Default middleware instance
