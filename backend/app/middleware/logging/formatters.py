...
class JSONFormatter(logging.Formatter):
    ...
    def __init__(
        ...
        ***REMOVED*** Reserved field names (don't copy from record.__dict__)
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        ***REMOVED*** Request ID (if available from context)
        if hasattr(record, "request_id"):
            log_data["request_id"] = record.request_id

        ***REMOVED*** Source location (useful for debugging)
        log_data["source"] = {
        ...
        ***REMOVED*** Extra fields (passed via logger.info("msg", extra={...}))
    ...
    def _extract_extra_fields(self, record: logging.LogRecord) -> dict[str, Any]:
        ...
        ***REMOVED*** Copy all non-reserved fields from record
        for key, value in record.__dict__.items():
            if key not in self.reserved_fields and not key.startswith("_"):
                extra[key] = value

        ***REMOVED*** Always include specified extra fields (even if None)
...
class RequestResponseFormatter(JSONFormatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        ***REMOVED*** Request/response data (if present)
        ...
        ***REMOVED*** User info (if available)
        if hasattr(record, "user_id"):
            log_data["user_id"] = record.user_id

        ***REMOVED*** Extra fields
        extra = self._extract_extra_fields(record)
        ***REMOVED*** Remove fields already included in http_data
        for field in [
            "method",
            "path",
            "status_code",
        ...
        ***REMOVED*** Remove fields at top level
        for field in [
            "request_headers",
            "request_body",
            "response_headers",
...
class CompactJSONFormatter(JSONFormatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        ***REMOVED*** Always include request_id if available
        if hasattr(record, "request_id"):
            log_data["request_id"] = record.request_id

        ***REMOVED*** Include only critical HTTP fields
        if hasattr(record, "method"):
