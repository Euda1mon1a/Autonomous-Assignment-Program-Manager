...
class BarrierWeights:
    ...
    kinetic: float = 0.3  ***REMOVED*** Time-based barriers
    thermodynamic: float = 0.2  ***REMOVED*** Equilibrium barriers
    steric: float = 0.25  ***REMOVED*** Credential barriers
    electronic: float = 0.15  ***REMOVED*** Authorization barriers
    regulatory: float = 0.4  ***REMOVED*** Compliance barriers (highest)
...
class BarrierDetector:
    ...
    async def detect_all_barriers(
        ...
        ***REMOVED*** Detect barriers in parallel categories
    ...
    async def _detect_kinetic_barriers(
        ...
        ***REMOVED*** Freeze horizon barrier
        ***REMOVED*** Default freeze is 14 days, but severity scales
        ...
        if days_until <= freeze_horizon_days:
            ...
            self._barriers.append(
                EnergyBarrier(
                    ...
                    is_absolute=days_until <= 0,  ***REMOVED*** Same-day = absolute barrier
        ...
        ***REMOVED*** Short notice barrier (24-48 hour changes)
        if 0 < days_until <= 2:
            self._barriers.append(
                EnergyBarrier(
                    barrier_type=BarrierType.KINETIC,
    ...
    async def _detect_steric_barriers(
        ...
        ***REMOVED*** Check credential requirements
        ***REMOVED*** In a real implementation, query the database for credentials
        ...
        if not has_required_credentials:
            self._barriers.append(
                EnergyBarrier(
                    ...
                    is_absolute=True,  ***REMOVED*** Cannot bypass credential requirements
        ...
        ***REMOVED*** Check competency level
        competency_level = await self._get_competency_level(new_person_id, rotation_id)
        if competency_level and competency_level < 2:  ***REMOVED*** Below "qualified"
            self._barriers.append(
                EnergyBarrier(
                    barrier_type=BarrierType.STERIC,
                    ...
                    is_absolute=False,  ***REMOVED*** Can be supervised
    ...
    async def _detect_electronic_barriers(
        ...
        ***REMOVED*** Role-based authorization barrier
        ...
        ***REMOVED*** Consent barrier for swaps
        if change_type == "swap":
            ...
            if not other_person_consented:
                self._barriers.append(
                    EnergyBarrier(
                        ...
                        is_absolute=True,  ***REMOVED*** Must have consent
    ...
    async def _detect_regulatory_barriers(
        ...
        ***REMOVED*** Check ACGME duty hours
        ...
        if would_violate_hours:
            self._barriers.append(
                EnergyBarrier(
                    ...
                    is_absolute=True,  ***REMOVED*** ACGME violations are absolute barriers
                    source="acgme_hours",
                    metadata={"rule": "80_hour_week"},
                )
            )

        ***REMOVED*** Check 1-in-7 day off rule
    ...
    async def _detect_thermodynamic_barriers(
        ...
        ***REMOVED*** Check workload balance
        current_workload = await self._get_workload(new_person_id)
        if current_workload and current_workload > 0.8:  ***REMOVED*** Over 80% utilized
        ...
        ***REMOVED*** Check preference conflicts
    ...
    ***REMOVED*** Helper methods - integrate with actual services
    async def _check_credentials(self, person_id: UUID, rotation_id: UUID) -> bool:
        ...
        """
        ***REMOVED*** Query for active credentials for this person and rotation's procedures
        ***REMOVED*** First, get the rotation to find its required procedures
        ...
        if not rotation:
            ***REMOVED*** Rotation not found - treat as no credential requirements
            return True

        ***REMOVED*** Get all credentials for this person
        ...
        ***REMOVED*** Check if any credentials are expired
        today = date.today()
        valid_credentials = [
            cred
        ...
        ***REMOVED*** For now, if person has any valid credentials, consider them credentialed
        ***REMOVED*** In a full implementation, would match rotation.required_procedures
    ...
    async def _get_competency_level(
        ...
        """
        ***REMOVED*** Map competency level strings to integers
        level_map = {"trainee": 1, "qualified": 2, "expert": 3, "master": 4}

        ***REMOVED*** Get credentials for this person
        creds_result = await self.db.execute(
            select(ProcedureCredential).where(
                ProcedureCredential.person_id == person_id,
                ProcedureCredential.status == "active",
        ...
        ***REMOVED*** Return the highest competency level found
        max_level = 0
        for cred in credentials:
    ...
    async def _check_acgme_hours(self, person_id: UUID, target_date: Any) -> bool:
        ...
        ***REMOVED*** Calculate the 4-week window around the target date
        window_start = target_date - timedelta(weeks=2)
        window_end = target_date + timedelta(weeks=2)

        ***REMOVED*** Query assignments in the window
        ...
        ***REMOVED*** Calculate hours (assume 6 hours per half-day block)
        hours_per_block = 6
        total_hours = len(assignments) * hours_per_block

        ***REMOVED*** Check if adding one more would violate
        ***REMOVED*** 80 hours/week * 4 weeks = 320 hours max
        max_hours = 80 * 4
    ...
    async def _check_day_off_rule(self, person_id: UUID, target_date: Any) -> bool:
        ...
        ***REMOVED*** Check 7 days before and after target date
        check_start = target_date - timedelta(days=6)
        check_end = target_date + timedelta(days=6)

        ***REMOVED*** Get all assignment dates in this window
        result = await self.db.execute(
            select(Block.date)
            .distinct()
        ...
        ***REMOVED*** Add the proposed date
        work_dates.add(target_date)

        ***REMOVED*** Check for 7+ consecutive days of work
    ...
    async def _get_workload(self, person_id: UUID) -> float | None:
        ...
        ***REMOVED*** Calculate available blocks (2 blocks per day * 28 days)
        ***REMOVED*** Subtract weekends for realistic utilization (10 workdays * 2)
        available_blocks = 28 * 2  ***REMOVED*** 56 blocks in 4 weeks
    ...
    async def _check_preferences(
        ...
        ***REMOVED*** Get the ISO week string for this date
        week_start = target_date - timedelta(days=target_date.weekday())
        week_str = week_start.isoformat()

        ***REMOVED*** Query faculty preferences
        result = await self.db.execute(
        ...
        ***REMOVED*** Check if week is blocked
        if prefs.blocked_weeks and week_str in prefs.blocked_weeks:
            return "blocked_week"

        ***REMOVED*** Check if already at max consecutive weeks
        ***REMOVED*** This would require more context about surrounding assignments
        ***REMOVED*** For now, just flag if the week isn't preferred but isn't blocked
        if prefs.preferred_weeks and week_str not in prefs.preferred_weeks:
            ***REMOVED*** Not in preferred weeks - soft preference violation
    ...
    def calculate_activation_energy(self) -> ActivationEnergy:
        ...
        ***REMOVED*** Normalize to 0-1 range
...
class BarrierClassifier:
    ...
    def classify_actionability(barrier: EnergyBarrier) -> str:
        ...
        ***REMOVED*** Regulatory barriers typically require escalation
        if barrier.barrier_type == BarrierType.REGULATORY:
            return "requires_escalation"

        ***REMOVED*** Electronic barriers can be catalyzed by authorized personnel
        if barrier.barrier_type == BarrierType.ELECTRONIC:
            return "catalyzable"

        ***REMOVED*** Time-based barriers may auto-resolve
        if barrier.barrier_type == BarrierType.KINETIC:
            if barrier.source == "freeze_horizon":
                return "catalyzable"  ***REMOVED*** Can be overridden
            return "auto_resolvable"  ***REMOVED*** Will resolve with time
    ...
    def summarize_barriers(barriers: list[EnergyBarrier]) -> dict[str, Any]:
        ...
        ***REMOVED*** Determine highest severity
