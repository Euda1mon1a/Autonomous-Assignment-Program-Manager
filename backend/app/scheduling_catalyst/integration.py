...
***REMOVED*** Defense level to catalyst mapping
...
class DefenseIntegration:
    ...
    def calculate_system_activation_energy(
        ...
        """
        ***REMOVED*** Higher coverage = lower barrier to changes
        base_energy = 1.0 - coverage_rate

        ***REMOVED*** Apply defense level adjustments
...
class HubIntegration:
    ...
    def hub_to_catalyst(
        ...
        """
        ***REMOVED*** Calculate catalyst score from hub metrics
        ***REMOVED*** Higher centrality = better catalyst
        catalyst_score = min(1.0, (
            hub_metrics.composite_score * 0.5 +
            hub_metrics.degree_centrality * 0.3 +
            (1 - hub_metrics.betweenness_centrality) * 0.2  ***REMOVED*** Lower bottleneck = better
        ))

        ***REMOVED*** Determine barriers addressed based on unique services
        barriers = [BarrierType.THERMODYNAMIC]  ***REMOVED*** All hubs help with workload
        if hub_metrics.unique_services >= 2:
            barriers.append(BarrierType.STERIC)  ***REMOVED*** Can cover multiple services

        ***REMOVED*** Calculate reduction factors
        base_reduction = hub_metrics.composite_score * 0.8
        reduction_factors = {bt: base_reduction for bt in barriers}

        ***REMOVED*** Capacity based on current load (inverse of centrality)
    ...
    def identify_catalyst_hubs(
        ...
        ***REMOVED*** Sort by catalyst score (highest first)
    ...
    def calculate_network_catalyst_capacity(
        ...
        ***REMOVED*** Bottleneck risk based on unique service concentration
        unique_services = [m.unique_services for m in hub_metrics_list]
        max_unique = max(unique_services) if unique_services else 0
        bottleneck_risk = min(1.0, max_unique / 5)  ***REMOVED*** Risk increases with concentration
...
class SacrificeIntegration:
    ...
    ***REMOVED*** Activity priority order (from sacrifice_hierarchy.py)
    ACTIVITY_PRIORITY = [
        "PATIENT_SAFETY",
        "ACGME_REQUIREMENTS",
        "CONTINUITY_OF_CARE",
    ...
    def get_sacrifice_catalyst(
        ...
        """
        ***REMOVED*** Higher utilization = more aggressive sacrifice
        aggressiveness = max(0.0, (utilization_rate - 0.8) / 0.2)

        ***REMOVED*** Determine which activities can be sacrificed
        sacrificeable = self.ACTIVITY_PRIORITY[-(int(aggressiveness * 4) + 1):]

        reduction = 0.5 + aggressiveness * 0.4  ***REMOVED*** 0.5 to 0.9
    ...
    def calculate_sacrifice_barrier_reduction(
        ...
        ***REMOVED*** Lower priority = higher reduction
...
class HomeostasisIntegration:
    ...
    def get_feedback_catalysts(
        ...
        for metric_name, current_value in current_metrics.items():
            ...
            if deviation > 0.1:  ***REMOVED*** Significant deviation
