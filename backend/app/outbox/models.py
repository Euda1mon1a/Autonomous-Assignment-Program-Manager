...
class OutboxStatus(str, enum.Enum):
    ...
    PENDING = "pending"        ***REMOVED*** Waiting to be published
    PROCESSING = "processing"  ***REMOVED*** Currently being published
    PUBLISHED = "published"    ***REMOVED*** Successfully published to broker
    FAILED = "failed"          ***REMOVED*** Publishing failed (will retry)
    ARCHIVED = "archived"      ***REMOVED*** Moved to archive (no longer active)
...
class OutboxMessage(Base):
    ...
    ***REMOVED*** Timestamps
    ...
    __table_args__ = (
        ***REMOVED*** Ensure valid status values
        CheckConstraint(
            "status IN ('pending', 'processing', 'published', 'failed', 'archived')",
            name="check_outbox_status"
        ),
        ***REMOVED*** Composite index for efficient polling of pending messages
        Index(
            "idx_outbox_pending_messages",
            "status",
            "created_at",
        ),
        ***REMOVED*** Index for retry processing
        Index(
            "idx_outbox_retry",
            "status",
            "next_retry_at",
        ),
        ***REMOVED*** Index for ordering within aggregate
        Index(
        ...
        ***REMOVED*** Index for cleanup of old messages
        Index(
...
class OutboxArchive(Base):
    ...
    ***REMOVED*** Same fields as OutboxMessage
    aggregate_type = Column(String(100), nullable=False, index=True)
