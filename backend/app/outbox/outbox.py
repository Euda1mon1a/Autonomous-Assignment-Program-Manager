...
Writing messages within a transaction::
    ...
    def create_assignment(db: Session, assignment_data: dict):
        ***REMOVED*** Business logic - create domain entity
        ...
        ***REMOVED*** Atomic commit - both succeed or both fail
        db.commit()
...
class OutboxWriter:
    ...
    Example:
        ::
            ...
            ***REMOVED*** Correct usage - same transaction
            def transfer_money(db, from_account, to_account, amount):
                ...
                db.commit()  ***REMOVED*** Single atomic commit
    ...
    Warning:
        ...
        ::
            ...
            ***REMOVED*** WRONG - defeats the purpose of transactional outbox
            db.add(assignment)
            db.commit()  ***REMOVED*** DON'T DO THIS
            writer.write_message(...)  ***REMOVED*** Message not atomic with assignment
    """
    ...
    def write_message(
        ...
        """
        ***REMOVED*** Validate payload is JSON-serializable
        try:
            json.dumps(payload)
        except (TypeError, ValueError) as e:
            raise ValueError(f"Payload must be JSON-serializable: {e}")

        ***REMOVED*** Get next sequence number for this aggregate
        sequence = self._get_next_sequence(aggregate_type, aggregate_id)

        ***REMOVED*** Create outbox message
        ...
        self.db.add(message)
        ***REMOVED*** Don't flush here - let the transaction handle it
        ***REMOVED*** This ensures atomicity with business data changes
...
class OutboxRelay:
    ...
    def __init__(
        ...
        ***REMOVED*** Import Celery app if not provided (avoids circular imports)
    ...
    def publish_pending_messages(
        ...
        ***REMOVED*** First, timeout any stuck processing messages
        if timeout_stuck_processing:
            self._timeout_stuck_processing()

        ***REMOVED*** Fetch pending messages (ordered by creation time for FIFO)
        ...
        for message in messages:
            try:
                ***REMOVED*** Mark as processing
                self._mark_processing(message)
                self.db.commit()

                ***REMOVED*** Publish to broker
                ...
                ***REMOVED*** Rollback to clean state
                self.db.rollback()

                ***REMOVED*** Mark as failed and schedule retry
                try:
    ...
    def _fetch_pending_messages(self, batch_size: int) -> list[OutboxMessage]:
        ...
        ***REMOVED*** Query for pending and retryable failed messages
        query = (
            select(OutboxMessage)
            .where(
                or_(
                    ***REMOVED*** Pending messages
                    OutboxMessage.status == OutboxStatus.PENDING.value,
                    ***REMOVED*** Failed messages ready for retry
                    and_(
    ...
    def _mark_failed(self, message: OutboxMessage, error_message: str) -> None:
        ...
        ***REMOVED*** Calculate next retry time using exponential backoff
    ...
    def _calculate_retry_delay(self, retry_count: int) -> int:
        ...
        """
        ***REMOVED*** Exponential backoff: 2^retry_count * base_delay
        ***REMOVED*** Retry 1: 10s, Retry 2: 20s, Retry 3: 40s
        base_delay = 10
        max_delay = 300  ***REMOVED*** Cap at 5 minutes
    ...
    def _publish_message(self, message: OutboxMessage) -> None:
        ...
        """
        ***REMOVED*** Build Celery task arguments
        task_name = self._get_task_name_for_event(message.event_type)

        ***REMOVED*** Send task to Celery
        self.celery_app.send_task(
            ...
            task_id=str(message.id),  ***REMOVED*** Use message ID as task ID for idempotency
            headers=message.headers,
        )
    ...
    def _get_task_name_for_event(self, event_type: str) -> str:
        ...
        """
        ***REMOVED*** Default handler for all outbox events
...
class OutboxCleaner:
    ...
    Example:
        ::
            ...
            ***REMOVED*** In a daily Celery beat task
            @celery_app.task
    ...
    def archive_published_messages(self, batch_size: int = 1000) -> int:
        ...
        ***REMOVED*** Find published messages to archive
        query = (
            select(OutboxMessage)
            .where(
                and_(
    ...
    def delete_old_archive(self, batch_size: int = 1000) -> int:
        ...
        ***REMOVED*** Delete old archive records
        deleted = (
    ...
    def cleanup_failed_messages(
        ...
        ***REMOVED*** Delete old failed messages
        deleted = (
