...
def handle_outbox_event(
    ...
    try:
        ***REMOVED*** Route to specific handlers based on event type
...
def _handle_assignment_event(
    ...
    ***REMOVED*** Extract common fields from payload
    assignment_id = payload.get("assignment_id")
    person_id = payload.get("person_id")
    block_id = payload.get("block_id")
    rotation_id = payload.get("rotation_id")

    ***REMOVED*** Send notifications for assignment changes
    if event_type == "assignment.created":
        ***REMOVED*** Trigger notification to affected person
        logger.info(
            f"Assignment created: person={person_id}, block={block_id}, "
            f"rotation={rotation_id}"
        )
        ***REMOVED*** TODO: Integrate with notification service when available
        ***REMOVED*** Example: send_notification(person_id, "New assignment created", payload)

    elif event_type == "assignment.updated":
        ***REMOVED*** Trigger change notification
        ...
        ***REMOVED*** TODO: Integrate with notification service
        ***REMOVED*** Example: send_notification(person_id, "Assignment updated", payload)

    elif event_type == "assignment.deleted":
        ***REMOVED*** Trigger deletion notification
        logger.info(
            f"Assignment deleted: assignment={assignment_id}, person={person_id}"
        )
        ***REMOVED*** TODO: Integrate with notification service
        ***REMOVED*** Example: send_notification(person_id, "Assignment removed", payload)
...
def _handle_swap_event(
    ...
    ***REMOVED*** Send notifications for swap events
    if event_type == "swap.executed":
        ***REMOVED*** Trigger notifications to both parties
        logger.info(
            f"Swap executed: swap_id={swap_id}, type={swap_type}, "
            f"requester={requester_id}, target={target_id}"
        )
        ***REMOVED*** TODO: Integrate with notification service
        ***REMOVED*** Example: send_notification(requester_id, "Swap completed", payload)
        ***REMOVED*** Example: send_notification(target_id, "Swap completed", payload)

    elif event_type == "swap.requested":
        ***REMOVED*** Trigger request notification to target
        logger.info(
            f"Swap requested: swap_id={swap_id}, type={swap_type}, "
            f"from={requester_id}, to={target_id}"
        )
        ***REMOVED*** TODO: Integrate with notification service
        ***REMOVED*** Example: send_notification(target_id, "New swap request", payload)

    elif event_type == "swap.approved":
        ***REMOVED*** Notify requester of approval
        logger.info(f"Swap approved: swap_id={swap_id}, requester={requester_id}")
        ***REMOVED*** TODO: send_notification(requester_id, "Swap request approved", payload)

    elif event_type == "swap.rejected":
        ***REMOVED*** Notify requester of rejection
        reason = payload.get("rejection_reason", "No reason provided")
        logger.info(f"Swap rejected: swap_id={swap_id}, reason={reason}")
        ***REMOVED*** TODO: send_notification(requester_id, f"Swap rejected: {reason}", payload)
...
def _handle_conflict_event(
    ...
    ***REMOVED*** Send alerts for conflicts
    if event_type == "conflict.detected":
        ***REMOVED*** Trigger alert notification to affected persons and coordinators
        logger.warning(
            f"Conflict detected: id={conflict_id}, type={conflict_type}, "
            f"severity={severity}, affected_persons={affected_persons}"
        )
        ***REMOVED*** TODO: Integrate with notification service
        ***REMOVED*** Example: For high-severity conflicts, notify coordinators immediately
        ***REMOVED*** if severity == "high":
        ***REMOVED***     send_notification("coordinator", "High-severity conflict detected", payload)
        ***REMOVED***
        ***REMOVED*** Example: Notify affected persons
        ***REMOVED*** for person_id in affected_persons:
        ***REMOVED***     send_notification(person_id, f"Schedule conflict: {conflict_type}", payload)

    elif event_type == "conflict.resolved":
        ***REMOVED*** Notify affected persons that conflict has been resolved
        ...
        ***REMOVED*** TODO: send notifications about resolution
        ***REMOVED*** for person_id in affected_persons:
        ***REMOVED***     send_notification(person_id, "Conflict resolved", payload)

    elif event_type == "conflict.escalated":
        ***REMOVED*** Escalate to higher authority (e.g., program director)
        ...
        ***REMOVED*** TODO: Send escalation notifications
        ***REMOVED*** send_notification("program_director", "Conflict requires attention", payload)
