...
down_revision: str | None = "20251226_block0"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(),
            nullable=False,
            comment="Model tier: haiku, sonnet, opus",
        ),
        ...
        sa.Column(
            ...
            sa.Text(),
            nullable=True,
            comment="Optional notes about tier selection",
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            Vector(384),
            nullable=False,
            comment="384-dim sentence-transformers embedding",
        ),
        sa.Column(
            "spec_hash",
            sa.String(),
            nullable=False,
            comment="Hash of agent specification for versioning",
        ),
        sa.Column(
            "capabilities",
            sa.Text(),
            nullable=True,
            comment="Comma-separated list of agent capabilities",
        ),
        sa.Column("updated_at", sa.DateTime(), server_default=sa.func.now()),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            Vector(384),
            nullable=True,
            comment="Embedding of task description",
        ),
        sa.Column(
            "agent_used",
            sa.String(),
            nullable=False,
            comment="Which agent handled this task",
        ),
        sa.Column(
            "model_used",
            sa.String(),
            nullable=False,
            comment="Which model was used (haiku, sonnet, opus)",
        ),
        sa.Column(
            "success",
            sa.Boolean(),
        ...
        sa.Column(
            ...
            sa.Integer(),
            nullable=True,
            comment="Task execution duration in milliseconds",
        ),
        sa.Column(
            "session_id",
            sa.String(),
            nullable=True,
            comment="Claude Code session identifier",
        ),
    ...
    op.execute(
        "CREATE INDEX task_history_embedding_idx ON task_history USING hnsw (embedding vector_cosine_ops)"
    )

    # Create index for agent name lookups in task history
    op.execute(
        "CREATE INDEX task_history_agent_idx ON task_history (agent_used, created_at DESC)"
    )
