...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "deviation_severity", sa.String(20)
    ...
    op.create_index(
        "idx_feedback_loop_timestamp", "feedback_loop_states", ["timestamp"]
    )
    op.create_index("idx_feedback_loop_name", "feedback_loop_states", ["loop_name"])
    op.create_index(
        "idx_feedback_loop_severity", "feedback_loop_states", ["deviation_severity"]
    )
    ...
    op.create_table(
        ...
        # Constraints
        sa.CheckConstraint(
            "entity_type IN ('faculty', 'system')", name="check_entity_type"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "idx_allostasis_entity", "allostasis_records", ["entity_id", "entity_type"]
    )
    op.create_index(
        "idx_allostasis_calculated", "allostasis_records", ["calculated_at"]
    )
    ...
    op.create_index(
        "idx_positive_feedback_detected", "positive_feedback_alerts", ["detected_at"]
    )
    op.create_index(
        "idx_positive_feedback_urgency", "positive_feedback_alerts", ["urgency"]
    )
    op.create_index(
        "idx_positive_feedback_resolved", "positive_feedback_alerts", ["resolved_at"]
    )
    ...
    op.create_table(
        ...
        # Constraints
        sa.CheckConstraint(
            "role IN ('primary', 'secondary', 'backup')", name="check_faculty_role"
        ),
    )
    op.create_index("idx_zone_faculty_zone", "zone_faculty_assignments", ["zone_id"])
    op.create_index(
        "idx_zone_faculty_faculty", "zone_faculty_assignments", ["faculty_id"]
    )
    op.create_index(
        "idx_zone_faculty_available", "zone_faculty_assignments", ["is_available"]
    )
    ...
    op.create_index(
        "idx_borrowing_requesting", "zone_borrowing_records", ["requesting_zone_id"]
    )
    op.create_index(
        "idx_borrowing_lending", "zone_borrowing_records", ["lending_zone_id"]
    )
    op.create_index("idx_borrowing_status", "zone_borrowing_records", ["status"])
    op.create_index(
        "idx_borrowing_requested_at", "zone_borrowing_records", ["requested_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column("description", sa.String(500)),
        sa.Column(
            "applied_at", sa.DateTime(), nullable=False, server_default=sa.func.now()
        ),
        # Quantification
    ...
    op.create_table(
        ...
        sa.Column("description", sa.String(500)),
        sa.Column(
            "initiated_at", sa.DateTime(), nullable=False, server_default=sa.func.now()
        ),
        # Quantification
    ...
    op.create_index(
        "idx_compensation_initiated", "compensation_records", ["initiated_at"]
    )
    ...
    op.create_index(
        "idx_equilibrium_calculated", "equilibrium_shifts", ["calculated_at"]
    )
    op.create_index(
        "idx_equilibrium_state", "equilibrium_shifts", ["equilibrium_state"]
    )
    op.create_index(
        "idx_equilibrium_sustainable", "equilibrium_shifts", ["is_sustainable"]
    )
