...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        ***REMOVED*** Idempotency key provided by client
        sa.Column('idempotency_key', sa.String(255), nullable=False),

        ***REMOVED*** SHA-256 hash of request body
        sa.Column('body_hash', sa.String(64), nullable=False),

        ***REMOVED*** Request parameters for audit
        sa.Column('request_params', postgresql.JSONB(), nullable=True),

        ***REMOVED*** Processing status: pending, completed, failed
        sa.Column('status', sa.String(20), nullable=False, server_default='pending'),

        ***REMOVED*** Reference to result (schedule_run_id)
        sa.Column('result_ref', postgresql.UUID(as_uuid=True), nullable=True),

        ***REMOVED*** Error message if failed
        sa.Column('error_message', sa.Text(), nullable=True),

        ***REMOVED*** Cached response for replay
    ...
    ***REMOVED*** Composite unique index for idempotency lookup
    op.create_index(
