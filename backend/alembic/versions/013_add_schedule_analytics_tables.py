...
def upgrade() -> None:
    ...
    ***REMOVED*** Table 1: schedule_versions - tracks all versions of a schedule
    op.create_table(
        'schedule_versions',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),

        ***REMOVED*** Link to schedule run
        sa.Column(
            'schedule_run_id',
    ...
    ***REMOVED*** Table 2: metric_snapshots - stores calculated metrics per version
    op.create_table(
        'metric_snapshots',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),

        ***REMOVED*** Link to schedule version
        sa.Column(
            'schedule_version_id',
            postgresql.UUID(as_uuid=True),
    ...
    ***REMOVED*** Table 3: schedule_diffs - tracks changes between versions
    op.create_table(
        'schedule_diffs',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),

        ***REMOVED*** Version comparison
        sa.Column(
            'from_version_id',
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey('schedule_versions.id', ondelete='CASCADE'),
        ...
        ***REMOVED*** Diff computation
        sa.Column('computed_at', sa.DateTime(), server_default=sa.func.now()),

        ***REMOVED*** Change details (JSONB for flexibility)
