...
def upgrade() -> None:
    ...
    ***REMOVED*** Note: pgvector extension already enabled in 20251227_pgvector migration

    ***REMOVED*** Create rag_documents table
    op.create_table(
        'rag_documents',
        sa.Column(
            'id',
    ...
    ***REMOVED*** Create B-tree index on doc_type for filtering
    op.create_index(
    ...
    ***REMOVED*** Create GIN index for metadata JSON queries
    ***REMOVED*** Allows efficient queries like: WHERE metadata_->>'source_file' = 'acgme_2024.pdf'
    op.execute(
        """
    ...
    ***REMOVED*** Create HNSW index for vector similarity search
    ***REMOVED*** HNSW (Hierarchical Navigable Small World) is optimal for high-recall queries
    ***REMOVED*** m=16: number of bi-directional links per node (higher = better recall, more memory)
    ***REMOVED*** ef_construction=64: size of dynamic candidate list during index build
    ***REMOVED*** vector_cosine_ops: use cosine distance (best for normalized embeddings)
    op.execute(
        """
    ...
    ***REMOVED*** Create IVFFlat index as alternative
    ***REMOVED*** IVFFlat (Inverted File with Flat compression) builds faster but queries slower
    ***REMOVED*** lists=100: number of clusters (rule of thumb: sqrt(num_rows), minimum 100)
    ***REMOVED*** Useful during development or for smaller datasets
    op.execute(
        """
    ...
    ***REMOVED*** Create updated_at trigger
    op.execute(
        """
...
def downgrade() -> None:
    ...
    ***REMOVED*** Drop indexes (some may be dropped with table, but be explicit)
