...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "phase_transition_risk", sa.String(20)
    ...
    op.create_index(
        "idx_health_checks_timestamp", "resilience_health_checks", ["timestamp"]
    )
    op.create_index(
        "idx_health_checks_status", "resilience_health_checks", ["overall_status"]
    )
    ...
    op.create_table(
        ...
        # Decision details
        sa.Column(
            "from_level", sa.String(20), nullable=False
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "activated_at", sa.DateTime(), nullable=False, server_default=sa.func.now()
        ),
        # Fallback details
    ...
    op.create_index(
        "idx_fallback_activated_at", "fallback_activations", ["activated_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "analyzed_at", sa.DateTime(), nullable=False, server_default=sa.func.now()
        ),
        # Analysis scope
    ...
    op.create_index(
        "idx_vulnerability_analyzed_at", "vulnerability_records", ["analyzed_at"]
    )
