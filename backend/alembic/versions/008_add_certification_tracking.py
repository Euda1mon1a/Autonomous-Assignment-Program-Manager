...
def upgrade() -> None:
    ...
    ***REMOVED*** Table 1: certification_types
    ***REMOVED*** Defines the types of certifications (BLS, ACLS, PALS, etc.)
    op.create_table(
        'certification_types',
    ...
    ***REMOVED*** Table 2: person_certifications
    ***REMOVED*** Tracks individual certifications for each person
    op.create_table(
        ...
        ***REMOVED*** Reminder tracking (timestamps when sent)
        ...
        ***REMOVED*** Constraints
        sa.UniqueConstraint('person_id', 'certification_type_id', name='uq_person_certification_type'),
        sa.CheckConstraint(
    ...
    op.create_index('idx_person_certs_type', 'person_certifications', ['certification_type_id'])
    op.create_index('idx_person_certs_expiration', 'person_certifications', ['expiration_date'])
    op.create_index('idx_person_certs_status', 'person_certifications', ['status'])

    ***REMOVED*** Insert default certification types
    op.execute("""
