...
def upgrade() -> None:
    ...
    overlap_type = sa.Enum(
        "none", "partial", "exact", "contained", "contains", name="overlaptype"
    )
    ...
    op.create_table(
        ...
        sa.Column("overlap_type", overlap_type, nullable=False, server_default="none"),
        sa.Column("overlapping_absence_ids", JSONB, nullable=True),  # List of UUIDs
        sa.Column(
            "overlap_details", JSONB, nullable=True
        ),  # Details about each overlap
        # Status
        sa.Column(
            "status", staged_absence_status, nullable=False, server_default="pending"
        ),
        # Validation
    ...
    op.create_index(
        "ix_import_staged_absences_batch_id", "import_staged_absences", ["batch_id"]
    )
    op.create_index(
        "ix_import_staged_absences_status", "import_staged_absences", ["status"]
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_import_staged_absences_version_end_txn",
        table_name="import_staged_absences_version",
    )
    op.drop_index(
        "ix_import_staged_absences_version_txn",
        table_name="import_staged_absences_version",
    ...
    op.drop_index(
        "ix_import_staged_absences_batch_status", table_name="import_staged_absences"
    )
    op.drop_index(
        "ix_import_staged_absences_overlap_type", table_name="import_staged_absences"
    )
    op.drop_index(
        "ix_import_staged_absences_date_range", table_name="import_staged_absences"
    )
    op.drop_index(
        "ix_import_staged_absences_matched_person_id",
    ...
    op.drop_index(
        "ix_import_staged_absences_status", table_name="import_staged_absences"
    )
    op.drop_index(
        "ix_import_staged_absences_batch_id", table_name="import_staged_absences"
    )
