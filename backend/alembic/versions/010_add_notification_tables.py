...
def upgrade() -> None:
    ...
    ***REMOVED*** Table 1: notifications - stores delivered notifications
    op.create_table(
        'notifications',
    ...
    ***REMOVED*** Table 2: scheduled_notifications - queue for future delivery
    op.create_table(
    ...
    op.create_index('idx_scheduled_recipient', 'scheduled_notifications', ['recipient_id'])
    ...
    ***REMOVED*** Table 3: notification_preferences - user settings
    op.create_table(
        'notification_preferences',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),

        ***REMOVED*** User (unique per user)
        sa.Column(
            'user_id',
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey('people.id', ondelete='CASCADE'),
