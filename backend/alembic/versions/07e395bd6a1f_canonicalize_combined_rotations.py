...
def upgrade() -> None:
    ...
    for canonical_abbrev, info in canonical_mapping.items():
        ...
        # Find duplicates by abbreviation or name aliases
        for alias in aliases:
            dups = conn.execute(
                text("""
                SELECT id FROM rotation_templates
                WHERE (abbreviation ILIKE :alias OR name ILIKE :alias)
                  AND id != :canonical_id
            """),
                {"alias": f"%{alias}%", "canonical_id": canonical_id},
            ).fetchall()
    ...
    for primary_names, secondary_names, canonical_abbrev in split_mappings:
        ...
        for p in primary_names:
            rows = conn.execute(
                text(
                    "SELECT id FROM rotation_templates WHERE abbreviation ILIKE :p OR name ILIKE :p"
                ),
                {"p": f"%{p}%"},
            ).fetchall()
        ...
        for s in secondary_names:
            rows = conn.execute(
                text(
                    "SELECT id FROM rotation_templates WHERE abbreviation ILIKE :s OR name ILIKE :s"
                ),
                {"s": f"%{s}%"},
            ).fetchall()
