...
def upgrade() -> None:
    ***REMOVED*** Insert PC rotation template (idempotent - skip if already exists)
    ***REMOVED*** Note: Using WHERE NOT EXISTS instead of ON CONFLICT because
    ***REMOVED*** abbreviation column lacks a unique constraint
    op.execute(
        sa.text("""
...
def downgrade() -> None:
    ***REMOVED*** Remove PC rotation template
    op.execute(
