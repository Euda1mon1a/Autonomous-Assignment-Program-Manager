...
def upgrade() -> None:
    ***REMOVED*** People table (residents and faculty)
    op.create_table(
        'people',
        ...
        sa.CheckConstraint('pgy_level IS NULL OR pgy_level BETWEEN 1 AND 3', name='check_pgy_level'),
    )
    op.create_index('idx_people_type', 'people', ['type'])

    ***REMOVED*** Blocks table (half-day scheduling blocks)
    op.create_table(
        'blocks',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    ***REMOVED*** Rotation templates table
    op.create_table(
        'rotation_templates',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('name', sa.String(255), nullable=False),
    ...
    ***REMOVED*** Assignments table (the schedule)
    op.create_table(
        'assignments',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        ...
        sa.Column('rotation_template_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('rotation_templates.id')),
    ...
    ***REMOVED*** Call assignments table
    op.create_table(
        'call_assignments',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('date', sa.Date(), nullable=False),
        ...
        sa.Column('is_holiday', sa.Boolean(), default=False),
        sa.Column('created_at', sa.DateTime(), server_default=sa.func.now()),
        sa.UniqueConstraint('date', 'person_id', 'call_type', name='unique_call_per_day'),
        sa.CheckConstraint("call_type IN ('overnight', 'weekend', 'backup')", name='check_call_type'),
    )
    op.create_index('idx_call_date', 'call_assignments', ['date'])
    op.create_index('idx_call_person', 'call_assignments', ['person_id'])

    ***REMOVED*** Schedule runs table (audit trail)
    op.create_table(
        'schedule_runs',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('start_date', sa.Date(), nullable=False),
