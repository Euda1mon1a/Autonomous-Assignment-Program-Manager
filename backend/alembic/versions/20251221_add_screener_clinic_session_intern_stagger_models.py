...
down_revision: str | None = "20251220_add_chaos_experiments"
...
def upgrade() -> None:
    ...
    # ============================================
    op.add_column("people", sa.Column("screener_role", sa.String(50), nullable=True))
    op.add_column(
        "people",
    ...
    op.add_column(
        "people",
        sa.Column(
            "screening_efficiency", sa.Integer(), server_default="100", nullable=True
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "rn_fallback_used", sa.Boolean(), server_default="false", nullable=True
        ),
        sa.Column(
            "created_at",
            sa.DateTime(),
    ...
    op.create_check_constraint(
        "check_session_type", "clinic_sessions", "session_type IN ('am', 'pm')"
    )
    op.create_check_constraint(
        "check_clinic_type",
        "clinic_sessions",
    ...
    op.create_check_constraint(
        "check_physician_count", "clinic_sessions", "physician_count >= 0"
    )
    op.create_check_constraint(
        "check_screener_count", "clinic_sessions", "screener_count >= 0"
    )
    op.create_check_constraint(
    ...
    op.create_table(
        ...
        sa.Column(
            "overlap_efficiency", sa.Integer(), nullable=False, server_default="85"
        ),
        sa.Column(
            "min_intern_a_experience_weeks",
            sa.Integer(),
            nullable=False,
            server_default="2",
        ),
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "check_overlap_efficiency", "intern_stagger_patterns", type_="check"
    )
    op.drop_constraint(
        "check_overlap_duration", "intern_stagger_patterns", type_="check"
    )
