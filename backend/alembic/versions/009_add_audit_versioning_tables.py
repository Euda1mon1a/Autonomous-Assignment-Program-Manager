...
def upgrade() -> None:
    ...
    ***REMOVED*** Transaction table - tracks all versioned transactions
    op.create_table(
        'transaction',
        sa.Column('id', sa.BigInteger(), primary_key=True, autoincrement=True),
        sa.Column('issued_at', sa.DateTime(), server_default=sa.func.now()),
        sa.Column('user_id', sa.String(255)),  ***REMOVED*** Who made the change
        sa.Column('remote_addr', sa.String(50)),  ***REMOVED*** IP address if available
    )
    ...
    ***REMOVED*** Assignment version table - mirrors assignments table structure
    op.create_table(
        'assignment_version',
        ...
        ***REMOVED*** Version tracking columns
        sa.Column('transaction_id', sa.BigInteger(), sa.ForeignKey('transaction.id'), nullable=False),
        sa.Column('operation_type', sa.SmallInteger(), nullable=False),  ***REMOVED*** 0=insert, 1=update, 2=delete
    ...
    ***REMOVED*** Absence version table - mirrors absences table structure
    op.create_table(
        'absence_version',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('person_id', postgresql.UUID(as_uuid=True)),
    ...
    ***REMOVED*** Schedule run version table - mirrors schedule_runs table structure
    op.create_table(
