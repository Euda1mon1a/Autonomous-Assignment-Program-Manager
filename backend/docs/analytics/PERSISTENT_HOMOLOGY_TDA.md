***REMOVED*** Persistent Homology (TDA) for Schedule Analysis
...
***REMOVED******REMOVED*** Mathematical Background

***REMOVED******REMOVED******REMOVED*** What is Persistent Homology?
...
***REMOVED******REMOVED******REMOVED*** Homology Dimensions
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `TopologicalFeature`
...
feature = TopologicalFeature(
    dimension=1,        ***REMOVED*** H1 = loop
    birth=0.5,          ***REMOVED*** Appears at filtration parameter 0.5
    death=2.0,          ***REMOVED*** Disappears at 2.0
    persistence=1.5,    ***REMOVED*** Lifetime = 1.5
    interpretation="Weekly rotation cycle"
)

***REMOVED*** Check if feature is structurally significant
...
diagram = PersistenceDiagram(
    h0_features=[...],  ***REMOVED*** Connected components
    h1_features=[...],  ***REMOVED*** Loops
    h2_features=[...],  ***REMOVED*** Voids
    max_dimension=2
)
...
def analyze_schedule_topology(db: Session):
    ...
    ***REMOVED*** Comprehensive analysis
...
***REMOVED******REMOVED******REMOVED*** Analysis Pipeline

The full TDA pipeline consists of 5 steps:

```python
***REMOVED*** Step 1: Embed assignments as point cloud
point_cloud = analyzer.embed_assignments(
    assignments,
    method='pca',       ***REMOVED*** or 'mds', 'tsne', 'manual'
    n_components=3      ***REMOVED*** 2D or 3D embedding
)

***REMOVED*** Step 2: Compute persistent homology
diagram = analyzer.compute_persistence_diagram(point_cloud)

***REMOVED*** Step 3: Extract coverage voids (from H2 features)
voids = analyzer.extract_schedule_voids(diagram, assignments)

***REMOVED*** Step 4: Detect cyclic patterns (from H1 features)
cycles = analyzer.detect_cyclic_patterns(diagram, assignments)

***REMOVED*** Step 5: Compute structural anomaly score
anomaly_score = analyzer.compute_structural_anomaly_score(diagram)
```

***REMOVED******REMOVED******REMOVED*** Embedding Methods
...
***REMOVED******REMOVED******REMOVED*** Persistence Diagram
...
***REMOVED*** Create persistence diagram (birth-death plot)
fig = viz.create_persistence_diagram(
    diagram,
    title="Schedule Topology - Persistence Diagram"
)

***REMOVED*** Features far from diagonal are significant
***REMOVED*** Diagonal represents birth = death (no persistence)
```

***REMOVED******REMOVED******REMOVED*** Persistence Barcode

```python
***REMOVED*** Create barcode plot (feature lifetimes as horizontal bars)
fig = viz.create_barcode(
    diagram,
    title="Schedule Topology - Persistence Barcode"
)

***REMOVED*** Longer bars = more persistent (significant) features
```

***REMOVED******REMOVED******REMOVED*** Point Cloud Visualization

```python
from app.analytics import visualize_point_cloud

***REMOVED*** Visualize embedded assignments
...
***REMOVED******REMOVED******REMOVED*** Combined View

```python
***REMOVED*** Both persistence diagram and barcode side-by-side
fig = viz.create_combined_view(diagram)
```

***REMOVED******REMOVED*** Applications

***REMOVED******REMOVED******REMOVED*** 1. Coverage Void Detection
...
***REMOVED******REMOVED******REMOVED*** 2. Cyclic Pattern Detection
...
***REMOVED******REMOVED******REMOVED*** 3. Structural Anomaly Detection
...
***REMOVED******REMOVED******REMOVED*** 4. Schedule Comparison

Compare two schedules topologically:

```python
***REMOVED*** Compare current schedule vs proposed schedule
distance = analyzer.compare_schedules_topologically(
    current_assignments,
    proposed_assignments,
    method='bottleneck'  ***REMOVED*** or 'wasserstein'
)
...
***REMOVED******REMOVED******REMOVED*** Example: Enhanced N-1 Analysis

```python
from app.analytics import PersistentScheduleAnalyzer
from app.resilience import compute_n1_vulnerability

***REMOVED*** Traditional N-1 analysis
n1_score = compute_n1_vulnerability(db, assignments)

***REMOVED*** TDA enhancement: detect isolated groups
analyzer = PersistentScheduleAnalyzer(db)
point_cloud = analyzer.embed_assignments(assignments)
diagram = analyzer.compute_persistence_diagram(point_cloud)

***REMOVED*** H0 features reveal connected components (groups that work together)
...
```python
***REMOVED*** 1. Sample if needed (for very large schedules)
...
***REMOVED*** 2. Use PCA for dimensionality reduction
point_cloud = analyzer.embed_assignments(
    assignments_sample,
    method='pca',
    n_components=3  ***REMOVED*** Keep low for faster computation
)

***REMOVED*** 3. Compute persistence (max_dimension=1 is faster than 2)
...
```txt
***REMOVED*** Topological Data Analysis (TDA)
ripser>=0.6.0    ***REMOVED*** Persistent homology computation
persim>=0.3.0    ***REMOVED*** Persistence diagram utilities
```
...
***REMOVED******REMOVED*** API Usage Example

***REMOVED******REMOVED******REMOVED*** REST Endpoint (Future Enhancement)

```python
***REMOVED*** backend/app/api/routes/analytics.py
...
async def visualize_topology(
    ...
    ***REMOVED*** Get assignments and compute persistence
    ***REMOVED*** ... (query assignments)

    point_cloud = analyzer.embed_assignments(assignments)
    diagram = analyzer.compute_persistence_diagram(point_cloud)

    ***REMOVED*** Create visualizations
    viz = BarcodeVisualization()
...
***REMOVED******REMOVED******REMOVED*** Papers & Books
...
***REMOVED******REMOVED******REMOVED*** Applications to Scheduling
...
***REMOVED******REMOVED******REMOVED*** Issue: "ripser not installed" warning
...
***REMOVED******REMOVED******REMOVED*** Issue: "Memory error" during persistence computation

**Solution:** Reduce point cloud size:
```python
***REMOVED*** Use fewer components or sample assignments
point_cloud = analyzer.embed_assignments(
    assignments[:1000],  ***REMOVED*** Sample first 1000
    method='pca',
    n_components=2  ***REMOVED*** Use 2D instead of 3D
)
```

***REMOVED******REMOVED******REMOVED*** Issue: "All features have low persistence"
