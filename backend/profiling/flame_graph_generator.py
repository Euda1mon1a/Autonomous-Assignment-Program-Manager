...
Requirements:
    pip install py-spy  ***REMOVED*** Requires root/sudo on Linux

Usage:
    ***REMOVED*** Using py-spy (recommended for production-like profiling)
    sudo python -m profiling.flame_graph_generator --target schedule --method pyspy

    ***REMOVED*** Using cProfile data
...
def generate_from_pyspy(
    ...
    try:
        ***REMOVED*** Check if py-spy is installed
    ...
    cmd = [
        ...
        output_file,
        "--format",
        "speedscope",  ***REMOVED*** or "flamegraph"
        "--rate",
        str(rate),
        "--duration",
        str(duration),
...
def convert_pstats_to_flamegraph(pstats_file: Path, output_file: str = "flamegraph.svg"):
    ...
    try:
        ...
        ***REMOVED*** For now, just print top functions as a guide
        ...
        print("   apt-get install graphviz  ***REMOVED*** or brew install graphviz")
        print()
...
def profile_and_generate(
    ...
    """
    ***REMOVED*** Map targets to profiling scripts
    target_scripts = {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  ***REMOVED*** Profile schedule generation with py-spy (requires sudo)
  sudo python -m profiling.flame_graph_generator --target schedule --method pyspy

  ***REMOVED*** Convert existing pstats file to flame graph
  python -m profiling.flame_graph_generator --pstats results/schedule_generation.pstats

  ***REMOVED*** Profile running server (get PID with ps aux | grep uvicorn)
  sudo py-spy record -o server_flame.svg --pid <PID> --duration 60
    ...
    if args.pstats:
        ***REMOVED*** Convert pstats file
        convert_pstats_to_flamegraph(args.pstats)
    elif args.target:
        ***REMOVED*** Profile and generate
