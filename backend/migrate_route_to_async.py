...
def migrate_imports(content: str) -> str:
    ...
    ***REMOVED*** Replace Session with AsyncSession
    content = re.sub(
    ...
    ***REMOVED*** Add select import if not present
    if 'from sqlalchemy import' in content and 'select' not in content:
        ...
        ***REMOVED*** Add select import after other sqlalchemy imports
        content = re.sub(
    ...
    ***REMOVED*** Replace get_db with get_async_db
    content = re.sub(
...
def migrate_route_handler(content: str) -> str:
    ...
    ***REMOVED*** Pattern: @router.METHOD(...)\ndef function_name(
    ***REMOVED*** Replace: def -> async def
    content = re.sub(
...
def migrate_db_dependency(content: str) -> str:
    ...
    ***REMOVED*** Replace: db: Session = Depends(get_db)
    ***REMOVED*** With: db: AsyncSession = Depends(get_async_db)
    content = re.sub(
...
def add_await_to_service_calls(content: str) -> str:
    ...
    ***REMOVED*** List of common service methods that should be awaited
    service_methods = [
        'execute_swap',
        'validate_swap',
        'rollback_swap',
    ...
    for method in service_methods:
        ***REMOVED*** Add await if not already present
        content = re.sub(
            rf'(?<!await\s)(\w+)\.{method}\(',
            rf'await \1.{method}(',
            content
...
def migrate_file(file_path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        ***REMOVED*** Apply migrations in order
        ...
        ***REMOVED*** Check if any changes were made
