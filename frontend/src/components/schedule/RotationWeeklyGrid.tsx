...
/**
 * RotationWeeklyGrid - 4-week activity grid editor for rotation templates.
 *
 * Combines:
 * - FacultyWeeklyEditor's paint-to-assign UX (activity palette, click-to-assign, shift+click lock)
 * - RotationEditor's 4-week tabs (All Weeks + Week 1-4 + "Same all weeks" checkbox)
 * - Activity requirements sidebar
 *
 * Behavior by rotation type:
 * - Inpatient: Grid preloaded with fixed pattern, cells shown as locked/amber
 * - Outpatient: Grid shows solver input defaults, cells editable
 */
...
import { useWeeklyPattern, useUpdateWeeklyPattern } from '@/hooks/useWeeklyPattern';
import { useActivities, useActivityRequirements, useReplaceActivityRequirements } from '@/hooks/useActivities';
import type { Activity, ActivityRequirement, ActivityRequirementCreateRequest } from '@/types/activity';
import { getPriorityLabel, getPriorityColor, formatApplicableWeeks } from '@/types/activity';
import type {
  WeeklyPatternGrid,
  WeeklyPatternSlot,
  DayOfWeek,
...
import { DAY_ABBREVIATIONS, createEmptyPattern, hasWeekSpecificPatterns } from '@/types/weekly-pattern';

// ============================================================================
// Types
// ============================================================================

interface RotationWeeklyGridProps {
  templateId: string;
  rotationType: string;
  canEdit: boolean;
  onClose?: () => void;
...
function getSlotFromPattern(
  ...
  return pattern.slots.find(
    (s) =>
      s.dayOfWeek === day &&
      s.timeOfDay === time &&
      (s.weekNumber === null || s.weekNumber === undefined)
  );
}
...
/** Week tabs with "Same all weeks" toggle */
function WeekTabs({
  selectedWeek,
  onWeekChange,
  samePatternAllWeeks,
...
/** Activity palette for click-to-paint */
  ...
  return (
    <div
      className={`
        ...
        ${isInpatient && isProtected ? 'opacity-80' : ''}
            ...
            <tr key={time}>
              ...
              {DISPLAY_ORDER.map((day) => {
                ...
                if (!slot) {
                  return (
                    <td key={day} className="p-0.5">
                      <div className="h-12 border border-gray-200 rounded bg-gray-50" />
                    </td>
                  );
                }
                ...
                const activity = activityId
                  ...
                  : slot.activityType
                    ? activities.find((a) => a.code === slot.activityType) ?? null
                    : null;
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                min={0}
                max={14}
                value={requirement.minHalfdays ?? 0}
            ...
            <div>
              ...
              <input
                ...
                min={0}
                max={14}
                value={requirement.maxHalfdays ?? 14}
...
export function RotationWeeklyGrid({
  templateId,
  rotationType,
  canEdit,
  onClose,
}: RotationWeeklyGridProps) {
  const isInpatient = rotationType === 'inpatient';
  ...
  // Data fetching
  const { data: patternData, isLoading: patternLoading } = useWeeklyPattern(templateId);
  const { data: activitiesData, isLoading: activitiesLoading } = useActivities();
  const { data: requirementsData, isLoading: requirementsLoading } = useActivityRequirements(templateId);

  // Mutations
  const updatePattern = useUpdateWeeklyPattern();
  const replaceRequirements = useReplaceActivityRequirements();

  // Local pattern state for editing
  const [localPattern, setLocalPattern] = useState<WeeklyPatternGrid | null>(null);
  const pattern = localPattern ?? patternData?.pattern ?? createEmptyPattern();
  ...
  const [localRequirements, setLocalRequirements] = useState<
    (ActivityRequirementCreateRequest & { id?: string })[] | null
  >(null);

  const requirements = localRequirements ?? (requirementsData ?? []).map((r: ActivityRequirement) => ({
    id: r.id,
  ...
  const isLoading = patternLoading || activitiesLoading || requirementsLoading;
  ...
  const handleToggleSamePattern = useCallback(() => {
    setSamePatternAllWeeks((prev) => {
      ...
      const updated = { ...pattern, samePatternAllWeeks: newValue };
    ...
    (day: DayOfWeek, time: WeeklyPatternTimeOfDay) => {
      ...
      const newSlots = pattern.slots.map((slot) => {
        if (slot.dayOfWeek === day && slot.timeOfDay === time) {
          return {
            ...slot,
            activityType: activity?.code ?? null,
          };
        }
        return slot;
      });

      setLocalPattern({ ...pattern, slots: newSlots });
      setHasUnsavedChanges(true);
    },
    [pattern, selectedActivityId, activities, readOnly]
  );
  ...
  const handleUpdateRequirement = useCallback(
    (index: number, updates: Partial<ActivityRequirementCreateRequest>) => {
      setLocalRequirements((prev) => {
        ...
        return current.map((req, i) => (i === index ? { ...req, ...updates } : req));
      });
      setHasUnsavedChanges(true);
    },
    [requirements]
  ...
  }, [templateId, localPattern, localRequirements, updatePattern, replaceRequirements]);
    ...
    <div className="space-y-4">
      ...
      {isInpatient && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
          ...
          Inpatient rotation: pattern is preloaded. The solver will not modify locked slots.
        </div>
      )}
      ...
      {!readOnly && (
        <div className="flex justify-end gap-3 pt-4 border-t">
          <button
            ...
            disabled={updatePattern.isPending || replaceRequirements.isPending}
          ...
          <button
            type="button"
            onClick={handleSave}
            disabled={!hasUnsavedChanges || updatePattern.isPending || replaceRequirements.isPending}
          ...
          >
            {(updatePattern.isPending || replaceRequirements.isPending) ? (
...
export default RotationWeeklyGrid;
