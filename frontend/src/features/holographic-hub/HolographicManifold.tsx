...
function rgbToHex(r: number, g: number, b: number): string {
  const toHex = (n: number) =>
    Math.round(Math.min(255, Math.max(0, n * 255)))
      .toString(16)
      .padStart(2, "0");
  return `***REMOVED***${toHex(r)}${toHex(g)}${toHex(b)}`;
}

function hexToRgb(hex: string): [number, number, number] {
  const result = /^***REMOVED***?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
...
function drawPoint(
  ...
  if (isSelected) {
    ctx.strokeStyle = "***REMOVED***ffffff";
    ctx.lineWidth = 2;
    ctx.beginPath();
...
function drawAxes(
  ...
  if (origin.depth > 0 && xEnd.depth > 0) {
    ctx.strokeStyle = "***REMOVED***ff4444";
    ctx.lineWidth = 2;
    ctx.beginPath();
  ...
  if (origin.depth > 0 && yEnd.depth > 0) {
    ctx.strokeStyle = "***REMOVED***44ff44";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(origin.x, origin.y);
  ...
  if (origin.depth > 0 && zEnd.depth > 0) {
    ctx.strokeStyle = "***REMOVED***4444ff";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(origin.x, origin.y);
