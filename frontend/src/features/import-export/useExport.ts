...
export const PEOPLE_EXPORT_COLUMNS: ExportColumn[] = [
  ...
  { key: 'performs_procedures', header: 'Performs Procedures', format: (v) => v ? 'Yes' : 'No' },
  { key: 'specialties', header: 'Specialties', format: (v) => Array.isArray(v) ? v.join(', ') : String(v || '') },
  { key: 'primary_duty', header: 'Primary Duty' },
];

export const ASSIGNMENT_EXPORT_COLUMNS: ExportColumn[] = [
  { key: 'block_id', header: 'Block ID' },
  { key: 'person_id', header: 'Person ID' },
  { key: 'role', header: 'Role' },
  { key: 'rotation_template_id', header: 'Rotation Template ID' },
  { key: 'activity_override', header: 'Activity Override' },
...
export const ABSENCE_EXPORT_COLUMNS: ExportColumn[] = [
  ...
  { key: 'absence_type', header: 'Absence Type' },
  { key: 'deployment_orders', header: 'Deployment Orders', format: (v) => v ? 'Yes' : 'No' },
  { key: 'tdy_location', header: 'TDY Location' },
  { key: 'replacement_activity', header: 'Replacement Activity' },
  { key: 'notes', header: 'Notes' },
];

export const SCHEDULE_EXPORT_COLUMNS: ExportColumn[] = [
  { key: 'date', header: 'Date' },
  { key: 'time_of_day', header: 'Time of Day' },
...
export function useExport(hookOptions: UseExportOptions = {}) {
  ...
  const generatePDFHTML = (
    ...
    const headerRow = options.columns
      .map(col => `<th style="border: 1px solid ***REMOVED***ddd; padding: 8px; background: ***REMOVED***f5f5f5; text-align: left;">${col.header}</th>`)
      .join('');
      ...
      .map(row => {
        const cells = options.columns
          .map(col => {
            ...
            return `<td style="border: 1px solid ***REMOVED***ddd; padding: 8px;">${escapeHTML(formatted)}</td>`;
          })
          .join('');
        return `<tr>${cells}</tr>`;
      })
...
<body>
  ...
  <p style="margin-top: 20px; color: ***REMOVED***666; font-size: 10px;">
