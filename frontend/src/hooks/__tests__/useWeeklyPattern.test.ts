...
const server = setupServer(
  ...
  http.put(`${API_BASE}/rotation-templates/:id/patterns`, async ({ request }) => {
    const body = await request.json() as { patterns: unknown[] };
    const now = new Date().toISOString();
    const created = body.patterns.map((p: Record<string, unknown>, i: number) => ({
      id: `pattern-new-${i}`,
      rotation_template_id: 'template-1',
      ...p,
      created_at: now,
