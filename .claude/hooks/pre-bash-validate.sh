...
***REMOVED*** Note: Patterns use extended regex (-E flag in grep)
WARNING_PATTERNS=(
    "^sudo "
    "chmod 777"
    "curl .* \| .*sh"
...
***REMOVED*** Special check: git push without remote tracking
if echo "$COMMAND" | grep -qE "^git push|git push "; then
    ...
    if [ -n "$CURRENT_BRANCH" ]; then
        ...
        if [ -z "$UPSTREAM" ]; then
            ***REMOVED*** Check if -u flag is present (setting up tracking)
...
***REMOVED*** Special check: git commit on main/master
