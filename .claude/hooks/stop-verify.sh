***REMOVED***!/bin/bash
***REMOVED*** Stop Verification Hook - Session End Checks
***REMOVED*** Runs when Claude Code session ends
...
if [ -d "$PROJECT_ROOT/.git" ]; then
    ...
    if [ "$DIRTY" -gt 0 ]; then
        ...
        ***REMOVED*** List the changed files
...
***REMOVED*** Check if tests were run (look for recent pytest cache)
