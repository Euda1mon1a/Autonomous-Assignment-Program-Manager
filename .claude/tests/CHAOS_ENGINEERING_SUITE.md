***REMOVED*** CHAOS ENGINEERING SUITE - SEARCH_PARTY Testing
...
***REMOVED******REMOVED*** Test 1: Timeout Cascade (All 10 Probes Timeout Simultaneously)
...
```bash
***REMOVED*** Configuration for timeout cascade
PROBE_TIMEOUT_MS=100
PROBE_COUNT=10
TARGET_LOC=15000
EXPECTED_TIMEOUTS=10
CASCADE_THRESHOLD=8  ***REMOVED*** If >8 fail, orchestrator should abort gracefully
```
...
***REMOVED******REMOVED******REMOVED*** Failure Conditions
...
***REMOVED******REMOVED*** Test 2: API Rate Limit Stress (Start at 85% Usage)
...
HOURLY_LIMIT=60000           ***REMOVED*** Anthropic API limit
INITIAL_USAGE_PERCENT=85     ***REMOVED*** Start at 51000/60000
PROBES=10
CALLS_PER_PROBE=15
...
1. **Pre-populate API Usage**
   ```python
   ***REMOVED*** Simulate prior usage
   rate_limiter.consume(51000)  ***REMOVED*** 85% of limit
   ```
...
***REMOVED******REMOVED*** Test 3: Context Pollution (Back-to-Back Runs)
...
```bash
***REMOVED*** Context isolation test
RUNS=5
...
***REMOVED******REMOVED*** Test 4: Target Size Explosion (100K+ LOC)
...
```bash
***REMOVED*** Large target configuration
TARGET_SIZES=[100000, 250000, 500000]  ***REMOVED*** LOC
PROBES=10
MEMORY_LIMIT_GB=8
TIMEOUT_BUFFER=2x  ***REMOVED*** Double normal timeouts
...
***REMOVED******REMOVED*** Test 5: Orchestrator Crash (Kill Mid-Deployment)
...
```bash
***REMOVED*** Orchestrator crash test
PROBES=10
KILL_AT_PERCENT=50
SIGNAL=SIGKILL    ***REMOVED*** Force kill, not graceful
VERIFY_CLEANUP=true
```
...
***REMOVED******REMOVED*** Test 6: Memory Exhaustion Recovery
...
```bash
***REMOVED*** Memory exhaustion test
MEMORY_LIMIT_MB=512
PROBES=10
TARGET_LOC=50000
...
1. **Set Memory Limit**
   ```bash
   ulimit -v 524288  ***REMOVED*** 512MB virtual memory
   ```
...
***REMOVED******REMOVED*** Running All Chaos Tests

***REMOVED******REMOVED******REMOVED*** Quick Run (All Tests, Defaults)
...
***REMOVED******REMOVED******REMOVED*** Individual Test Run
...
***REMOVED******REMOVED*** Metrics & Thresholds
...
***REMOVED******REMOVED*** Debugging Failed Tests

***REMOVED******REMOVED******REMOVED*** Test 1 Fails (Timeout Cascade)
- Check probe timeout implementation
- Verify no timeout synchronization
- Review orchestrator timeout handler

***REMOVED******REMOVED******REMOVED*** Test 2 Fails (Rate Limit)
- Check rate limit middleware
- Verify backoff algorithm
- Review API call logging

***REMOVED******REMOVED******REMOVED*** Test 3 Fails (Context Pollution)
- Check credential storage
- Verify cache key format
- Review session cleanup

***REMOVED******REMOVED******REMOVED*** Test 4 Fails (Target Size)
- Profile memory allocation
- Check for O(n²) algorithms
- Implement streaming analysis

***REMOVED******REMOVED******REMOVED*** Test 5 Fails (Orchestrator Crash)
- Review process cleanup
- Check database transaction handling
- Add startup recovery log

***REMOVED******REMOVED******REMOVED*** Test 6 Fails (Memory Exhaustion)
