***REMOVED*** INTEGRATION TESTS - Multi-Protocol Coordination
...
***REMOVED******REMOVED*** Test 1: G2_RECON → ORCHESTRATOR Handoff
...
***REMOVED******REMOVED******REMOVED*** Test Workflow
...
5. **Phase 4: Validate Output**
   ```bash
   ***REMOVED*** Verify all probe findings in final report
   grep -c "probe_" final_report.json  ***REMOVED*** Should show 10 probes mentioned

   ***REMOVED*** Verify coverage
   jq '.coverage' final_report.json    ***REMOVED*** Should match max of probe coverages

   ***REMOVED*** Verify confidence
   jq '.confidence' final_report.json  ***REMOVED*** Should be aggregated score
   ```
...
3. **Validate Protocol Boundaries**
   ```python
   ***REMOVED*** Each protocol should return results in its own format
...
***REMOVED******REMOVED*** Test 3: Cross-Session Synthesis
...
1. **Load Session A Results**
   ...
   ***REMOVED*** 150 findings from 10 probes
   ```

2. **Load Session B Results**
   ```bash
   session_b=$(load_results("session_b_2025-12-31.json"))
   ***REMOVED*** 180 findings from 8 probes
   ```

3. **Normalize Finding Identifiers**
   ```python
   ***REMOVED*** Convert to canonical form: {file}:{line}:{type}:{severity}
...
4. **Perform Cross-Session Analysis**
   ...
   ***REMOVED*** Track severity changes
...
***REMOVED******REMOVED*** Integration Test Execution
...
***REMOVED******REMOVED******REMOVED*** Full End-to-End Test

```bash
***REMOVED*** Run realistic workflow: Deploy → Execute → Synthesize → Report
...
***REMOVED******REMOVED*** Debugging Integration Issues

***REMOVED******REMOVED******REMOVED*** Test 1 Fails (G2_RECON → ORCHESTRATOR)

**Common Issue:** Probe timeout during handoff
**Debug Steps:**
```bash
***REMOVED*** Check message queue
docker-compose exec mcp-server redis-cli LLEN probe_results_queue

***REMOVED*** Monitor orchestrator logs
