***REMOVED*** QUALITY GATES TEST SUITE - Validation & Confidence
...
***REMOVED******REMOVED******REMOVED*** Three-Tier Quality Framework
...
***REMOVED******REMOVED*** Test 1: Probe Output Validation
...
***REMOVED******REMOVED******REMOVED*** Test Scenario
...
***REMOVED******REMOVED******REMOVED*** Execution Steps
...
2. **Create Test Probes**
   ...
   ***REMOVED*** Corrupted JSON
   ./test_generators/probe_output.sh --corrupt-json

   ***REMOVED*** Semantically bad
...
***REMOVED******REMOVED******REMOVED*** Test Cases & Expected Results

***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 1: Valid Output
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 2: Missing Required Field
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 3: Wrong Data Type
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 4: Out of Range
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 5: Corrupted JSON
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Test Case 6: Semantic Inconsistency
...
***REMOVED******REMOVED*** Test 2: Discrepancy Detection
...
2. **Deploy with One Outlier**
   ```bash
   ***REMOVED*** Probe 1-9: Normal analysis
   ***REMOVED*** Probe 10: Gives wildly different results
   ./test_runners/quality_test.sh \
     --test=discrepancy_detection \
     --probes=10 \
     --target=consistent_target \
...
3. **Deploy with Conflicts**
   ```bash
   ***REMOVED*** Probes 1-5: Finding A is critical
   ***REMOVED*** Probes 6-10: Finding A is low
   ./test_runners/quality_test.sh \
     --test=discrepancy_detection \
     --probes=10 \
     --target=consistent_target \
...
***REMOVED******REMOVED******REMOVED*** Metric Definitions

***REMOVED******REMOVED******REMOVED******REMOVED*** Finding Agreement
...
def calculate_finding_agreement(probe_findings: list[dict]) -> float:
    ...
    all_findings = {}  ***REMOVED*** finding_id -> list[probe_ids]
    ...
    ***REMOVED*** Calculate agreement rate
    agreement_rate = sum(
        1 for f_ids in all_findings.values()
        if len(f_ids) >= len(probes) * 0.8  ***REMOVED*** >80% agreement
    ) / len(all_findings)

    return agreement_rate  ***REMOVED*** 0.0-1.0
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Confidence Variance
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Coverage Agreement
...
def calculate_coverage_agreement(probe_coverages: list[float]) -> tuple[float, float]:
    ...
    ***REMOVED*** Acceptable if no probe deviates >10% from mean
    acceptable = max_deviation <= 10.0  ***REMOVED*** percentage points
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 1: All Consistent
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 2: One Outlier
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario 3: Conflicting Groups
...
***REMOVED******REMOVED*** Test 3: Confidence Aggregation
...
1. **Deploy Probes with Varied Quality**
   ```bash
   ***REMOVED*** Probe 1-3: High quality (95% confidence)
   ***REMOVED*** Probe 4-6: Medium quality (85% confidence)
   ***REMOVED*** Probe 7-9: Low quality (70% confidence)
   ***REMOVED*** Probe 10: Outlier (50% confidence)
...
2. **Calculate Aggregate Confidence**
   ...
   ***REMOVED*** Strategy 1: Simple Average
   simple_avg = np.mean(probe_confidences)  ***REMOVED*** 0.80

   ***REMOVED*** Strategy 2: Weighted (by coverage * speed)
   weights = [1.0, 1.0, 0.95, 0.90, 0.85, 0.88, 0.70, 0.65, 0.60, 0.30]
   weighted_avg = np.average(probe_confidences, weights=weights)  ***REMOVED*** 0.82

   ***REMOVED*** Strategy 3: Conservative (minimum)
   conservative = min(probe_confidences)  ***REMOVED*** 0.50

   ***REMOVED*** Strategy 4: Robust (median)
   robust = np.median(probe_confidences)  ***REMOVED*** 0.80

   ***REMOVED*** Strategy 5: Consensus (>80% agreement)
   agreements = [c for c in probe_confidences if abs(c - np.median(probe_confidences)) < 0.15]
   consensus = len(agreements) / len(probe_confidences)  ***REMOVED*** 0.8 (80%)
   ```

3. **Compare Strategies**

***REMOVED******REMOVED******REMOVED*** Strategy Comparison
...
***REMOVED******REMOVED*** Running Quality Gate Tests

***REMOVED******REMOVED******REMOVED*** Run All Quality Tests

```bash
./test_runners/quality_suite.sh --all
```

***REMOVED******REMOVED******REMOVED*** Run Individual Test
...
***REMOVED******REMOVED******REMOVED*** Integration with SEARCH_PARTY

```python
***REMOVED*** In orchestrator/synthesizer
...
for probe_result in probe_results:
    ...
    ***REMOVED*** All gates passed, proceed to synthesis
    return synthesize(filtered_results)
```

---

***REMOVED******REMOVED*** Monitoring Quality Metrics
