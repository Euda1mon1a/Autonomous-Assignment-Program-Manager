***REMOVED*** AGENT INPUT VALIDATION
...
***REMOVED******REMOVED*** 1. MISSION CONTEXT VALIDATION
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Task Description Validation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scope Validation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Constraint Validation
...
***REMOVED******REMOVED*** 2. FILE PATH VALIDATION

***REMOVED******REMOVED******REMOVED*** Path Sanitization
...
def validate_file_path(path: str, allowed_paths: list[str]) -> bool:
    ...
    """
    ***REMOVED*** Rule 1: No path traversal
    if ".." in path:
        return False  ***REMOVED*** ✗ Path traversal attempt

    ***REMOVED*** Rule 2: No absolute escape
    if path.startswith("/"):
        return False  ***REMOVED*** ✗ Absolute path outside project

    ***REMOVED*** Rule 3: Check against whitelist
    if not any(path.startswith(allowed) for allowed in allowed_paths):
        return False  ***REMOVED*** ✗ Path not in allowed list

    ***REMOVED*** Rule 4: No null bytes
    if "\0" in path:
        return False  ***REMOVED*** ✗ Null byte injection

    ***REMOVED*** Rule 5: Resolve and verify
    resolved = Path(path).resolve()
    project_root = Path("/home/user/Autonomous-Assignment-Program-Manager")

    if not str(resolved).startswith(str(project_root)):
        return False  ***REMOVED*** ✗ Path outside project

    return True
```

***REMOVED******REMOVED******REMOVED*** Common Injection Patterns (Blocked)
...
```python
***REMOVED*** Check for symlinks pointing outside project
if path.is_symlink():
    target = path.resolve()
    if not str(target).startswith(str(project_root)):
        return False  ***REMOVED*** ✗ Symlink escape
```

***REMOVED******REMOVED******REMOVED*** Directory Restrictions
...
***REMOVED******REMOVED*** 3. MISSION CONTEXT SANITIZATION

***REMOVED******REMOVED******REMOVED*** Input Sanitization Rules

***REMOVED******REMOVED******REMOVED******REMOVED*** Task Description Sanitization
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scope Sanitization
...
def normalize_path(path: str) -> str:
    ...
    ***REMOVED*** Remove trailing slashes
    path = path.rstrip("/")

    ***REMOVED*** Remove duplicate slashes
    while "//" in path:
        path = path.replace("//", "/")

    ***REMOVED*** Resolve . and .. (but keep relative)
    parts = path.split("/")
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Success Criteria Sanitization
...
***REMOVED******REMOVED******REMOVED*** Constraint Sanitization
...
def sanitize_timeout(minutes: int) -> int:
    ...
    MIN_TIMEOUT = 5      ***REMOVED*** Too fast for any useful work
    MAX_TIMEOUT = 480    ***REMOVED*** 8 hours max
    DEFAULT = 60
...
***REMOVED******REMOVED*** 4. PROMPT INJECTION PREVENTION

***REMOVED******REMOVED******REMOVED*** Prompt Injection Patterns
...
```python
***REMOVED*** Strictly validate types
...
```python
***REMOVED*** Prevent buffer overflow / DoS
MAX_FIELD_LENGTHS = {
    "agent_id": 32,
...
def validate_semantic_consistency(mission: dict) -> bool:
    ...
    ***REMOVED*** Can't write if permission tier is READ_ONLY
    ...
    ***REMOVED*** Timeout must be reasonable for task type
...
***REMOVED******REMOVED*** 5. VALIDATION CHECKLIST
...
***REMOVED******REMOVED******REMOVED*** Scope Validation
...
***REMOVED******REMOVED******REMOVED*** Security Validation
...
***REMOVED******REMOVED******REMOVED*** Consistency Validation
...
***REMOVED******REMOVED*** 6. VALIDATION FAILURE RESPONSES

***REMOVED******REMOVED******REMOVED*** Input Rejected (Clear Error)
...
***REMOVED******REMOVED******REMOVED*** Input Sanitized and Proceeding
...
***REMOVED******REMOVED******REMOVED*** Input Blocked (Security Violation)
...
***REMOVED******REMOVED*** 7. VALIDATION IMPLEMENTATION CHECKLIST
