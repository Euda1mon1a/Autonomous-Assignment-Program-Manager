...
***REMOVED******REMOVED*** Mission Command Model
...
***REMOVED******REMOVED******REMOVED*** Plan, See, Do Operating Model
...
***REMOVED******REMOVED*** I. TASK DECOMPOSITION RULES

***REMOVED******REMOVED******REMOVED*** A. Complexity Assessment Framework
...
***REMOVED******REMOVED******REMOVED*** B. Complexity Scoring Rubric
...
***REMOVED******REMOVED******REMOVED*** C. Domain Boundaries
...
***REMOVED******REMOVED*** II. DELEGATION TEMPLATES

***REMOVED******REMOVED******REMOVED*** A. Standard Agent Briefing Format

When delegating to a subagent, use this template:

```markdown
***REMOVED******REMOVED*** Agent Assignment: [AGENT_NAME]
...
***REMOVED******REMOVED******REMOVED*** Inputs
...
***REMOVED******REMOVED******REMOVED*** Handoff Protocol
**Upon completion**:
...
```markdown
***REMOVED******REMOVED*** Agent Assignment: SCHEDULER
...
***REMOVED******REMOVED******REMOVED*** B. Context Handoff Protocol

When transferring work between agents, preserve state:

```markdown
***REMOVED******REMOVED*** Handoff: [FROM_AGENT] → [TO_AGENT]
...
***REMOVED******REMOVED******REMOVED*** Questions/Blockers
- **Question**: Should auto-cancellation trigger N-1 reanalysis? (Assuming YES for now)
- **Blocker**: None

***REMOVED******REMOVED******REMOVED*** Relevant References
...
***REMOVED******REMOVED******REMOVED*** C. Timeout and Fallback Rules
...
***REMOVED******REMOVED*** III. AGENT SPAWNING GUIDELINES

***REMOVED******REMOVED******REMOVED*** A. When to Spawn vs. Handle Directly
...
***REMOVED******REMOVED******REMOVED*** B. Maximum Concurrent Agents
...
def can_spawn_agent(current_active_agents, proposed_agent):
    ...
    ***REMOVED*** Check system-wide limit
...
***REMOVED******REMOVED******REMOVED*** C. Agent Capacity Management
...
class AgentCapacityTracker:
    ...
    def __init__(self):
        self.active_tasks = {}  ***REMOVED*** agent_id → list of task_ids
        self.task_complexity = {}  ***REMOVED*** task_id → complexity_score
    ...
    def assign_task(self, agent_id, task):
        ...
        ***REMOVED*** Each agent can handle up to 20 complexity points
...
***REMOVED******REMOVED******REMOVED*** D. Resource Allocation by Task Type
    ...
    for task in tasks:
        ...
        ***REMOVED*** Check if we have resources
        ...
        if can_allocate:
            ...
            ***REMOVED*** Update usage
            for resource in required:
                current_usage[resource] += required[resource]
        else:
            ***REMOVED*** Queue for later
            queue_task(task)

    return assignments
```

***REMOVED******REMOVED******REMOVED*** III.E Auto-Tier Selection Algorithm
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Selection Logic
...
def select_model_tier(task_complexity: int, agent_spec: dict) -> str:
    ...
    """
    ***REMOVED*** Respect agent's declared preference
    if agent_spec.get("model_tier"):
        return agent_spec["model_tier"]

    ***REMOVED*** Auto-select based on complexity
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Tier Cost Weights

For capacity management, apply these weights:

```python
TIER_WEIGHTS = {
    'haiku': 0.5,   ***REMOVED*** 2x more haikus before capacity hit
    'sonnet': 1.0,  ***REMOVED*** Baseline
    'opus': 1.5     ***REMOVED*** Premium resource, reduced capacity
}
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Override Guidance
...
***REMOVED******REMOVED*** IV. SYNTHESIS PATTERNS

***REMOVED******REMOVED******REMOVED*** A. Result Aggregation Strategies

When collecting results from multiple agents, use appropriate aggregation:

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. All-or-Nothing (AND Logic)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Best-Effort (OR Logic)
...
def synthesize_best_effort(results):
    ...
    if successes:
        return {
            "success": True,
            "result": successes[0].data,  ***REMOVED*** First success
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Weighted Aggregation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Merge-and-Deduplicate
...
***REMOVED******REMOVED******REMOVED*** B. Conflict Resolution Between Agents

When agents disagree, use structured conflict resolution:

***REMOVED******REMOVED******REMOVED******REMOVED*** Resolution Framework
...
class ConflictResolver:
    ...
    def resolve(self, conflicting_results):
        ...
        ***REMOVED*** Stage 1: Categorize conflict
        ...
        ***REMOVED*** Stage 2: Domain expertise hierarchy
        if conflict_type == "EXPERTISE":
            return self.defer_to_expert(conflicting_results)

        ***REMOVED*** Stage 3: Voting (peer agents)
        if conflict_type == "OPINION":
            return self.majority_vote(conflicting_results)

        ***REMOVED*** Stage 4: Escalate
        return self.escalate_to_human(conflicting_results)

    def categorize_conflict(self, results):
        """Categorize type of disagreement."""

        ***REMOVED*** Reconcilable: Different perspectives on same truth
        ***REMOVED*** Example: Agent A found 10 conflicts, Agent B found 12
        ***REMOVED*** (Both correct, B found 2 more)

        ***REMOVED*** Expertise: One agent has domain authority
        ***REMOVED*** Example: ARCHITECT says "use PostgreSQL", SCHEDULER says "use SQLite"
        ***REMOVED*** (ARCHITECT has authority on database decisions)

        ***REMOVED*** Opinion: Subjective judgment call
        ***REMOVED*** Example: Agent A prefers algorithm X, Agent B prefers algorithm Y
        ***REMOVED*** (Both valid, use voting or benchmarks)

        ***REMOVED*** Fundamental: Irreconcilable contradiction
        ***REMOVED*** Example: Agent A says "schedule valid", Agent B says "schedule invalid"
        ***REMOVED*** (Cannot both be true, need investigation)
...
***REMOVED******REMOVED******REMOVED*** C. Quality Gates Before Synthesis
...
***REMOVED******REMOVED******REMOVED*** D. Escalation Paths for Disagreements
...
***REMOVED******REMOVED*** V. HANDOFF PROTOCOLS

***REMOVED******REMOVED******REMOVED*** A. State Preservation Requirements
...
class HandoffState:
    ...
    ***REMOVED*** Current state
    working_branch: str
    database_state: dict  ***REMOVED*** migrations applied, seed data, etc.
    environment_state: dict  ***REMOVED*** running services, env vars, etc.

    ***REMOVED*** Remaining work
...
```markdown
***REMOVED******REMOVED*** Handoff Checklist

***REMOVED******REMOVED******REMOVED*** Before Handoff (Sending Agent)
...
***REMOVED******REMOVED******REMOVED*** During Handoff (ORCHESTRATOR)
...
***REMOVED******REMOVED******REMOVED*** After Handoff (Receiving Agent)
...
***REMOVED******REMOVED******REMOVED*** B. Checkpoint Creation
...
class Checkpoint:
    ...
    def restore(self):
        ...
        ***REMOVED*** Database restore
        restore_database_dump(self.database_state["dump_path"])

        ***REMOVED*** Environment restore
...
***REMOVED******REMOVED******REMOVED*** C. Recovery Procedures
...
class HandoffRecovery:
    ...
    def retry_or_reassign(self, handoff):
        ...
        ***REMOVED*** Try 3 times with exponential backoff
        ...
        ***REMOVED*** If still unavailable, reassign
        ...
        ***REMOVED*** No alternatives, escalate
    ...
    def restore_checkpoint(self, handoff):
        ...
        ***REMOVED*** Find most recent checkpoint before handoff
        checkpoint = find_checkpoint_before(handoff.timestamp)

        ***REMOVED*** Restore
        checkpoint.restore()

        ***REMOVED*** Log for investigation
        log_handoff_failure(handoff, checkpoint)

        ***REMOVED*** Notify ORCHESTRATOR
...
***REMOVED******REMOVED*** VI. COORDINATION PATTERNS

***REMOVED******REMOVED******REMOVED*** Pattern 1: Fan-Out / Fan-In
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Pipeline (Sequential Chain)
...
***REMOVED******REMOVED******REMOVED*** Pattern 3: Scatter-Gather (Parallel Fan-Out)
...
***REMOVED******REMOVED******REMOVED*** Pattern 4: Broadcast (Same Question, Multiple Perspectives)
...
***REMOVED******REMOVED******REMOVED*** Pattern 5: Competitive (Multiple Approaches, Best Wins)
...
***REMOVED******REMOVED*** VII. KEY WORKFLOWS

***REMOVED******REMOVED******REMOVED*** Workflow 1: Coordinate Feature Implementation
...
***REMOVED******REMOVED******REMOVED*** Workflow 2: Incident Response
...
***REMOVED******REMOVED******REMOVED*** Workflow 3: Large-Scale Refactoring
...
***REMOVED******REMOVED*** VIII. ANTI-PATTERNS TO AVOID

***REMOVED******REMOVED******REMOVED*** 1. Over-Coordination
...
***REMOVED******REMOVED******REMOVED*** 2. Ambiguous Boundaries
...
***REMOVED******REMOVED******REMOVED*** 3. Sequential When Parallel Possible
...
***REMOVED******REMOVED******REMOVED*** 4. Ignoring Dependencies
...
***REMOVED******REMOVED******REMOVED*** 5. Hiding Conflicts
...
***REMOVED******REMOVED******REMOVED*** When to Escalate to ARCHITECT
...
***REMOVED******REMOVED******REMOVED*** When to Escalate to Faculty (Human)
...
***REMOVED******REMOVED******REMOVED*** When to Consult META_UPDATER
...
```markdown
***REMOVED******REMOVED*** Orchestration Escalation: [Title]
...
***REMOVED******REMOVED******REMOVED*** Agent Perspectives
...
***REMOVED******REMOVED******REMOVED*** Options for Decision-Maker
...
***REMOVED******REMOVED******REMOVED*** Impact if Unresolved
[What happens if decision delayed?]
[Timeline implications]
[Blocked work]

***REMOVED******REMOVED******REMOVED*** Decision Needed
...
***REMOVED******REMOVED******REMOVED*** Coordination Quality
...
***REMOVED******REMOVED******REMOVED*** Agent Satisfaction (Qualitative)
...
***REMOVED******REMOVED******REMOVED*** Read Access (Understand Agent Capabilities)
...
***REMOVED******REMOVED*** IX. COORDINATOR TIER ARCHITECTURE

***REMOVED******REMOVED******REMOVED*** A. Hierarchical Delegation Model
...
***REMOVED******REMOVED******REMOVED*** B. Coordinator Responsibilities
...
***REMOVED******REMOVED******REMOVED*** B.1. ORCHESTRATOR Direct Staff
...
***REMOVED******REMOVED******REMOVED*** C. Delegation Patterns

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 1: Direct Delegation (Simple Tasks)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 2: Broadcast Delegation (Multi-Domain)
...
async def broadcast_to_coordinators(task):
    ...
    return await asyncio.gather(
        ...
        return_exceptions=True  ***REMOVED*** One failure doesn't block others
    )
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 3: Cascade Amplification
...
***REMOVED******REMOVED******REMOVED*** E. Quality Gates Between Tiers
...
***REMOVED******REMOVED******REMOVED*** F. Temporal Layering Integration
...
***REMOVED******REMOVED******REMOVED*** G. Updated Capacity Limits
...
***REMOVED******REMOVED******REMOVED*** H. Coordinator Delegation Template

When delegating to a coordinator:

```markdown
***REMOVED******REMOVED*** Coordinator Assignment: [COORD_ENGINE/COORD_QUALITY/COORD_OPS]

***REMOVED******REMOVED******REMOVED*** Signal
...
***REMOVED******REMOVED******REMOVED*** Expected Agents
[Which agents the coordinator should spawn]

***REMOVED******REMOVED******REMOVED*** Quality Gate
**Threshold**: [Success percentage required]
**Failure Action**: [What to do if gate fails]

***REMOVED******REMOVED******REMOVED*** Downstream
[What happens after coordinator completes]
```

***REMOVED******REMOVED******REMOVED*** I. Coordinator Quick Reference Card
...
***REMOVED******REMOVED*** X. BIOLOGICAL SCALING PATTERNS

***REMOVED******REMOVED******REMOVED*** A. Signal Transduction Principles
...
***REMOVED******REMOVED******REMOVED*** B. Refractory Periods

Prevent feedback loops from creating resonance:

```python
COOLDOWNS = {
    "fast_tools": 100,      ***REMOVED*** 100ms - rapid repeats OK
    "medium_tools": 1000,   ***REMOVED*** 1s - moderate restraint
    "slow_tools": 60000,    ***REMOVED*** 60s - rarely repeat
    "coordinator": 5000,    ***REMOVED*** 5s - don't re-delegate immediately
}
```

***REMOVED******REMOVED******REMOVED*** C. Quorum Sensing
...
***REMOVED******REMOVED*** XI. G-STAFF STRUCTURE (Army Doctrine)

***REMOVED******REMOVED******REMOVED*** A. Organizational Hierarchy
...
***REMOVED******REMOVED******REMOVED*** B. G-Staff Responsibilities
...
***REMOVED******REMOVED******REMOVED*** C. Special Staff Agents
...
***REMOVED******REMOVED******REMOVED*** D. XO Pattern
...
***REMOVED******REMOVED******REMOVED*** E. Session Lifecycle with G-Staff
...
***REMOVED******REMOVED******REMOVED*** F. G-Staff Quick Reference
...
***REMOVED******REMOVED*** Appendix A: Quick Reference Card

> **Purpose:** Condensed reference for local Claude sessions with limited context
> **Full Spec:** See sections above for complete documentation
...
***REMOVED******REMOVED******REMOVED*** Complexity Scoring
...
***REMOVED******REMOVED******REMOVED*** Coordinator Hierarchy
...
***REMOVED******REMOVED******REMOVED*** Agent Routing Table
...
***REMOVED******REMOVED******REMOVED*** Signal Routing
...
SIGNAL_ROUTES = {
    ...
    ***REMOVED*** Multi-domain (broadcast to all)
...
***REMOVED******REMOVED******REMOVED*** Task Tool Mapping
...
Task(
    ...
    prompt="""
    ***REMOVED******REMOVED*** Agent: SCHEDULER
...
***REMOVED******REMOVED******REMOVED*** Synthesis Patterns
...
***REMOVED******REMOVED******REMOVED*** Capacity Limits
...
***REMOVED******REMOVED******REMOVED*** Session Startup (Local Context)

For limited context windows, use this minimal startup:

```bash
***REMOVED*** 1. Check git state
git branch --show-current && git status --porcelain

***REMOVED*** 2. Read minimal context
Read CLAUDE.md          ***REMOVED*** Project rules
Read HUMAN_TODO.md      ***REMOVED*** Current priorities
Read this file          ***REMOVED*** ORCHESTRATOR.md (includes Quick Reference)

***REMOVED*** 3. Load full specs only when needed
***REMOVED*** Read .claude/Agents/COORD_*.md       ***REMOVED*** Only when delegating to coordinators
```

---

***REMOVED******REMOVED******REMOVED*** Cross-Session Memory
