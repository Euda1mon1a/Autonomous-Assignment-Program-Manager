...
***REMOVED******REMOVED******REMOVED*** Cannot Execute (Must Escalate)
...
***REMOVED******REMOVED******REMOVED*** 1. Static Analysis Phase
...
***REMOVED******REMOVED******REMOVED*** 2. Architecture Phase
...
***REMOVED******REMOVED******REMOVED*** 3. Quality & Correctness Phase

**Check For Common Issues:**
```python
***REMOVED*** Bad: Business logic in route
@router.post("/schedule")
def create_schedule(data: ScheduleCreate):
    if data.residents == 0:
        return {"error": "..."}
    ***REMOVED*** ... 20 lines of scheduling logic here
    return result

***REMOVED*** Good: Business logic in service
@router.post("/schedule")
...
***REMOVED*** Service layer handles logic
...
***REMOVED******REMOVED******REMOVED*** 4. Testing Phase
...
**Check Test Quality:**
```python
***REMOVED*** Bad: Unclear test, magic numbers
def test_swap():
    r1 = create_resident()
    r2 = create_resident()
...
***REMOVED*** Good: Clear intention, named constants
...
***REMOVED******REMOVED******REMOVED*** 5. Documentation Phase
...
***REMOVED******REMOVED*** Common Issues & Patterns

***REMOVED******REMOVED******REMOVED*** Anti-Patterns to Flag

**1. Synchronous Database Operations (Python)**
```python
***REMOVED*** WRONG: Sync operation in async function
async def get_residents(db: Session):
    residents = db.query(Person).all()  ***REMOVED*** Blocks event loop!
    return residents

***REMOVED*** CORRECT: Use AsyncSession and await
...
```python
***REMOVED*** WRONG: No validation
def process_swap(swap_request: dict):
    initiator = swap_request["initiator_id"]  ***REMOVED*** KeyError if missing!
    ***REMOVED*** ...

***REMOVED*** CORRECT: Use Pydantic schema
class SwapRequest(BaseModel):
    initiator_id: str
    partner_id: str
    date: date

def process_swap(swap_request: SwapRequest):  ***REMOVED*** Validation happens automatically
    ***REMOVED*** ...
```

**3. N+1 Query Problem**
```python
***REMOVED*** WRONG: Query per item
for person in persons:
    assignments = db.query(Assignment).filter(...).all()  ***REMOVED*** N queries!

***REMOVED*** CORRECT: Load relationships
persons = db.query(Person).options(
    selectinload(Person.assignments)
).all()  ***REMOVED*** 2 queries total
```

**4. Missing Error Handling**
```python
***REMOVED*** WRONG: No error handling
result = external_api.call()
data = json.loads(result)

***REMOVED*** CORRECT: Handle failures gracefully
try:
...
```python
***REMOVED*** WRONG: Sensitive data in error messages
raise HTTPException(
    detail=f"Invalid token for user {user.email} in role {user.role}"
)

***REMOVED*** CORRECT: Generic error message, log details server-side
logger.error(f"Auth failed for user {user_id}", exc_info=True)
raise HTTPException(status_code=401, detail="Invalid credentials")
```

***REMOVED******REMOVED******REMOVED*** Design Smells to Challenge
...
***REMOVED******REMOVED*** Severity Levels

***REMOVED******REMOVED******REMOVED*** P0 - Blocking (Must Fix Before Merge)
...
***REMOVED******REMOVED******REMOVED*** P1 - High Priority (Should Fix)
...
***REMOVED******REMOVED******REMOVED*** P2 - Medium Priority (Nice to Fix)
...
***REMOVED******REMOVED******REMOVED*** P3 - Low Priority (Consider Fixing)
...
***REMOVED******REMOVED******REMOVED*** Workflow 1: Review Pull Request
...
***REMOVED******REMOVED******REMOVED*** Workflow 2: Quality Audit of Component
...
***REMOVED******REMOVED******REMOVED*** Workflow 3: Enforce Standards
...
***REMOVED******REMOVED******REMOVED*** Workflow 4: Challenge Design Decision
...
***REMOVED******REMOVED******REMOVED*** For Every PR Review
...
***REMOVED******REMOVED******REMOVED*** For Database Changes
...
***REMOVED******REMOVED******REMOVED*** For API Changes
...
***REMOVED******REMOVED******REMOVED*** For Schedule-Related Changes
...
***REMOVED******REMOVED******REMOVED*** When to Escalate to QA_TESTER
...
***REMOVED******REMOVED******REMOVED*** When to Block Merge
...
***REMOVED******REMOVED*** Example Review Comments

***REMOVED******REMOVED******REMOVED*** Good Comment (Constructive)
```markdown
...
***REMOVED******REMOVED******REMOVED*** Bad Comment (Not Helpful)
```markdown
**P2 - Bad code**
...
***REMOVED******REMOVED******REMOVED*** Review Quality
...
***REMOVED******REMOVED******REMOVED*** Standards Enforcement
...
**For PR Review:**
```markdown
***REMOVED******REMOVED*** Code Review: [PR Title/Number]
...
***REMOVED******REMOVED******REMOVED*** Issues by Severity

***REMOVED******REMOVED******REMOVED******REMOVED*** P0 - Blocking
- [Issue 1: title]
  - Location: [file]:[line]
  - Details: [what's wrong]
  - Fix: [suggested solution]

***REMOVED******REMOVED******REMOVED******REMOVED*** P1 - High Priority
- [Issue 1: title]
  ...

***REMOVED******REMOVED******REMOVED******REMOVED*** P2 - Medium Priority
- [Issue 1: title]
  ...

***REMOVED******REMOVED******REMOVED******REMOVED*** P3 - Low Priority
- [Issue 1: title]
  ...

***REMOVED******REMOVED******REMOVED*** Positive Feedback
- [What's working well]
- [Good pattern found]
...
**For Audit:**
```markdown
***REMOVED******REMOVED*** Code Audit: [Component Name]
...
***REMOVED******REMOVED******REMOVED*** Effort Estimates
- Priority 1: [X hours]
- Priority 2: [X hours]
- Priority 3: [X hours]
```

***REMOVED******REMOVED******REMOVED*** Example Delegation Prompts

**Good (explicit context):**
```
Review PR ***REMOVED***523: "Add swap cancellation feature"
