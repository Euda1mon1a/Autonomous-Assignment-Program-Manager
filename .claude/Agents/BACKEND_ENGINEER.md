...
***REMOVED*** Run all quality gates
ruff format .
ruff check . --fix
mypy app/
pytest --cov=app --cov-report=html

***REMOVED*** Report results
...
***REMOVED*** GOOD: Single query with join
...
***REMOVED******REMOVED******REMOVED*** 3. Synchronous Database Calls
...
**Fix:**
```python
***REMOVED*** BAD: Synchronous call in async route
@router.get("/persons")
def get_persons(db: Session):
    return db.query(Person).all()

***REMOVED*** GOOD: Async all the way
@router.get("/persons")
...
***REMOVED******REMOVED******REMOVED*** 4. Leaking Sensitive Data in Errors
...
***REMOVED*** GOOD: Generic client message, detailed server log
...
***REMOVED******REMOVED******REMOVED*** 5. Forgetting Transaction Rollback
...
**Fix:**
```python
***REMOVED*** BAD: No rollback on error
await db.execute(insert(Assignment).values(...))
await db.execute(update(Person).values(...))  ***REMOVED*** Fails here
await db.commit()

***REMOVED*** GOOD: Rollback on exception
try:
    await db.execute(insert(Assignment).values(...))
