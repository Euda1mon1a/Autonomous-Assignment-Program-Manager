...
***REMOVED*** Test migration cycle
alembic upgrade head
alembic downgrade -1
alembic upgrade head

***REMOVED*** Verify models match schema
alembic history
alembic current
...
***REMOVED******REMOVED******REMOVED*** 1. Edited Applied Migration
...
**Fix:**
```bash
***REMOVED*** NEVER do this - create a NEW migration instead
***REMOVED*** git checkout backend/alembic/versions/abc123_old_migration.py

***REMOVED*** CORRECT approach:
alembic revision -m "Fix issue from abc123"
***REMOVED*** Edit new migration to correct the schema
```
...
***REMOVED******REMOVED******REMOVED*** 2. Missing Downgrade Logic
...
def downgrade() -> None:
    ***REMOVED*** Add the reverse operation
...
***REMOVED******REMOVED******REMOVED*** 3. Autogenerate Misses Custom Changes
...
**Fix:**
```python
***REMOVED*** After autogenerate, manually add missing operations
def upgrade() -> None:
    ***REMOVED*** Autogenerated
    op.add_column('persons', ...)

    ***REMOVED*** Manually added
...
***REMOVED******REMOVED******REMOVED*** 4. Large Data Migration in Schema Migration
...
**Fix:**
```python
***REMOVED*** SPLIT into two migrations:

***REMOVED*** Migration 1: Schema only (fast)
def upgrade() -> None:
    op.add_column('persons', sa.Column('status', sa.String(20), nullable=True))

***REMOVED*** Migration 2: Data migration (can be run with batching)
def upgrade() -> None:
    op.execute(
...
***REMOVED******REMOVED******REMOVED*** 5. Foreign Key Constraint Violation
...
**Fix:**
```python
***REMOVED*** Ensure referenced table/column exists FIRST
def upgrade() -> None:
    ***REMOVED*** Create referenced table first
    op.create_table('rotations', ...)

    ***REMOVED*** Then create table with foreign key
    op.create_table(
        'assignments',
        sa.Column('rotation_id', sa.String, sa.ForeignKey('rotations.id'))
    )

***REMOVED*** Or add foreign key after both tables exist
def upgrade() -> None:
