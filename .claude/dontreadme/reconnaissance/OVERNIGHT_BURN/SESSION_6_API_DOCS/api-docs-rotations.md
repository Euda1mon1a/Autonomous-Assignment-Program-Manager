***REMOVED*** Rotation API Documentation
...
2. [Core Endpoints](***REMOVED***core-endpoints)
3. [Data Models](***REMOVED***data-models)
4. [Scheduling Integration](***REMOVED***scheduling-integration)
5. [Constraint Management](***REMOVED***constraint-management)
6. [Leave Eligibility](***REMOVED***leave-eligibility)
7. [Template Management Patterns](***REMOVED***template-management-patterns)
...
***REMOVED******REMOVED******REMOVED*** Get Rotation Template by ID
...
***REMOVED******REMOVED******REMOVED*** RotationTemplate (ORM Model)
...
class RotationTemplate(Base):
    ...
    ***REMOVED*** Leave eligibility - core concept
    ...
    ***REMOVED*** Relationships
    ...
    halfday_requirements: RotationHalfDayRequirement  ***REMOVED*** One-to-one
...
***REMOVED******REMOVED******REMOVED*** RotationHalfDayRequirement (ORM Model)
...
class RotationHalfDayRequirement(Base):
    ...
    ***REMOVED*** Activity breakdown per block (sums to 10)
    fm_clinic_halfdays: int = Column(Integer, default=4)
    specialty_halfdays: int = Column(Integer, default=5)
    specialty_name: str | None = Column(String(255))  ***REMOVED*** "Neurology", "Dermatology"
    academics_halfdays: int = Column(Integer, default=1)  ***REMOVED*** Wed AM
    elective_halfdays: int = Column(Integer, default=0)  ***REMOVED*** Buffer/flex time

    ***REMOVED*** Solver constraints
...
***REMOVED******REMOVED******REMOVED*** RotationPreference (ORM Model)
...
class RotationPreference(Base):
    ...
    preference_type: str  ***REMOVED*** full_day_grouping, consecutive_specialty, etc.
    weight: str = "medium"  ***REMOVED*** low, medium, high, required
    config_json: dict  ***REMOVED*** Type-specific configuration
...
***REMOVED******REMOVED******REMOVED*** WeeklyPattern (ORM Model)
...
class WeeklyPattern(Base):
    ...
    day_of_week: int  ***REMOVED*** 0=Sunday, 6=Saturday
    time_of_day: str  ***REMOVED*** "AM" or "PM"
    activity_type: str  ***REMOVED*** fm_clinic, specialty, elective, conference, inpatient, call, procedure, off
    linked_template_id: UUID | None  ***REMOVED*** For split rotations (reference to other template)
    is_protected: bool = False  ***REMOVED*** Wednesday academics protected time
    notes: str | None
...
***REMOVED******REMOVED*** Scheduling Integration

***REMOVED******REMOVED******REMOVED*** Leave-Eligible Rotation Matching Algorithm
...
***REMOVED******REMOVED******REMOVED*** Capacity Constraints
...
***REMOVED******REMOVED******REMOVED*** Supervision Ratio Enforcement
...
***REMOVED******REMOVED*** Constraint Management

***REMOVED******REMOVED******REMOVED*** Leave Eligibility Rules
...
def find_best_leave_eligible_rotation(
    ...
    for rotation_id, slot in capacities.items():
        ***REMOVED*** Skip non-leave-eligible rotations
        if not slot.template.leave_eligible:
            continue

        ***REMOVED*** Skip if already assigned to this rotation
        if rotation_id in already_assigned:
            continue

        ***REMOVED*** Skip if rotation is full
        if slot.is_full:
            continue

        ***REMOVED*** Prefer rotations with available capacity
        return rotation_id
```

***REMOVED******REMOVED******REMOVED*** Absence Conflict Detection
...
***REMOVED******REMOVED*** Template Management Patterns

***REMOVED******REMOVED******REMOVED*** Activity Type Patterns
...
***REMOVED******REMOVED******REMOVED*** Split and Mirrored Rotations
...
***REMOVED******REMOVED******REMOVED*** Example 1: Create a Specialty Clinic Template
...
***REMOVED******REMOVED******REMOVED*** Example 2: Update Template to Reduce Capacity
...
***REMOVED******REMOVED******REMOVED*** Example 3: Filter Rotations by Activity Type
...
***REMOVED******REMOVED******REMOVED*** Example 4: Delete Template
...
***REMOVED******REMOVED******REMOVED*** Example 5: Leave Eligibility Conflict Scenario
...
```python
***REMOVED*** Step 1: Identify residents with leave in block
residents_with_leave = get_residents_with_leave_in_block(block=1)
***REMOVED*** Result: [PGY-1-03 with 7 days leave]

***REMOVED*** Step 2: Find leave-eligible rotations for this resident
leave_eligible_rotations = [
  "Continuity Clinic",  ***REMOVED*** leave_eligible=true
  "Elective",           ***REMOVED*** leave_eligible=true
  "Grand Rounds"        ***REMOVED*** leave_eligible=true
]

***REMOVED*** Step 3: Assign resident to best available rotation
assignment = find_best_leave_eligible_rotation(
  resident=PGY-1-03,
  capacities=rotation_capacities,
  already_assigned=set()
)
***REMOVED*** Result: Assign to "Continuity Clinic" (has capacity)

***REMOVED*** Step 4: Remaining residents → fill coverage needs
remaining_residents = all_residents - residents_with_leave
for resident in remaining_residents:
    ***REMOVED*** Try to assign to coverage-critical rotations (FMIT, Night Float)
    ***REMOVED*** Then balance across other rotations
```
...
***REMOVED******REMOVED******REMOVED*** Example 6: Half-Day Requirements Integration
...
***REMOVED******REMOVED*** Advanced: Rotation Preferences (Soft Constraints)
