***REMOVED*** Security Encryption Audit - Session 4
***REMOVED******REMOVED*** SEARCH_PARTY Reconnaissance Report
...
***REMOVED******REMOVED*** 1. PERCEPTION - Current Encryption Usage Inventory

***REMOVED******REMOVED******REMOVED*** A. Password Hashing
**Status:** IMPLEMENTED & STRONG

**Technology Stack:**
```python
***REMOVED*** Location: backend/app/core/security.py
...
***REMOVED******REMOVED******REMOVED*** B. JSON Web Tokens (JWT)
**Status:** IMPLEMENTED & HARDENED

**Token Encryption:**
```python
***REMOVED*** Location: backend/app/core/security.py
ALGORITHM = "HS256"  ***REMOVED*** HMAC with SHA-256

def create_access_token(data: dict, expires_delta: timedelta | None = None):
    to_encode = data.copy()
    ***REMOVED*** ... expiration logic ...
...
```python
***REMOVED*** Location: backend/app/api/routes/auth.py
response.set_cookie(
    key="access_token",
    value=f"Bearer {token_response.access_token}",
    httponly=True,  ***REMOVED*** Prevents JavaScript access
    secure=not settings.DEBUG,  ***REMOVED*** HTTPS only in production
...
***REMOVED******REMOVED******REMOVED*** C. Cryptographic Key Management
...
class KeyType(str, Enum):
    SYMMETRIC = "symmetric"      ***REMOVED*** AES-256
    RSA_2048 = "rsa_2048"       ***REMOVED*** RSA 2048-bit
    RSA_4096 = "rsa_4096"       ***REMOVED*** RSA 4096-bit
    EC_P256 = "ec_p256"         ***REMOVED*** Elliptic curve P-256
    EC_P384 = "ec_p384"         ***REMOVED*** Elliptic curve P-384
```
...
```python
***REMOVED*** Location: backend/app/security/key_management.py:279-296
...
```python
***REMOVED*** Location: backend/app/security/key_management.py:299-323
...
**Database Storage:**
```python
***REMOVED*** Location: backend/app/security/key_management.py:167-240
class CryptographicKey(Base):
    """Database model for storing encrypted cryptographic keys."""

    ***REMOVED*** Encrypted key material (base64 encoded)
    encrypted_key_material = Column(Text, nullable=False)
    encrypted_private_key = Column(Text, nullable=True)
    public_key = Column(Text, nullable=True)  ***REMOVED*** Can be unencrypted

    ***REMOVED*** Encryption metadata
...
***REMOVED******REMOVED******REMOVED*** D. Tenant Data Encryption
...
class TenantEncryptionService:
    ...
    async def encrypt_data(self, tenant_id: UUID, plaintext: bytes) -> str:
        ***REMOVED*** 1. Derive tenant-specific key from master key
        key = self._derive_tenant_key(tenant_id)

        ***REMOVED*** 2. Generate random nonce
        nonce = secrets.token_bytes(12)

        ***REMOVED*** 3. Encrypt with AES-256-GCM
        aesgcm = AESGCM(key)
        ciphertext = aesgcm.encrypt(nonce, plaintext, None)

        ***REMOVED*** 4. Return base64(nonce + ciphertext + tag)
        encrypted = nonce + ciphertext
        return base64.b64encode(encrypted).decode("utf-8")

    async def decrypt_data(self, tenant_id: UUID, ciphertext_b64: str) -> bytes:
        ***REMOVED*** 1. Derive tenant-specific key
        key = self._derive_tenant_key(tenant_id)

        ***REMOVED*** 2. Extract nonce from ciphertext
        encrypted = base64.b64decode(ciphertext_b64)
        nonce = encrypted[:12]
        ciphertext = encrypted[12:]

        ***REMOVED*** 3. Decrypt with AES-256-GCM
        aesgcm = AESGCM(key)
...
***REMOVED******REMOVED******REMOVED*** E. Secret Rotation Framework
...
class SecretType(str, Enum):
    JWT_SIGNING_KEY = "jwt_signing_key"        ***REMOVED*** 90-day rotation
    DATABASE_PASSWORD = "database_password"    ***REMOVED*** 180-day (manual)
    API_KEY = "api_key"                       ***REMOVED*** 90-day
    ENCRYPTION_KEY = "encryption_key"         ***REMOVED*** 365-day (requires re-encryption)
    REDIS_PASSWORD = "redis_password"         ***REMOVED*** 90-day
    WEBHOOK_SECRET = "webhook_secret"         ***REMOVED*** 180-day
    S3_ACCESS_KEY = "s3_access_key"          ***REMOVED*** 90-day
    S3_SECRET_KEY = "s3_secret_key"          ***REMOVED*** 90-day
```
...
DEFAULT_CONFIGS = {
    SecretType.JWT_SIGNING_KEY: RotationConfig(
        ...
        grace_period_hours=24,      ***REMOVED*** 24-hour validation window
        auto_rotate=True,
        notify_before_hours=72,     ***REMOVED*** 3-day advance notice
        rollback_on_failure=True,
    ),
    SecretType.ENCRYPTION_KEY: RotationConfig(
        rotation_interval_days=365,
        grace_period_hours=None,    ***REMOVED*** Requires manual re-encryption
        auto_rotate=False,
        rollback_on_failure=True,
    ),
    ***REMOVED*** ... additional configurations ...
}
```
...
class SecretRotationHistory(Base):
    ...
    ***REMOVED*** Hash of secrets (not the actual secrets)
...
***REMOVED******REMOVED*** 2. INVESTIGATION - Key Management Analysis

***REMOVED******REMOVED******REMOVED*** A. Key Lifecycle Management

**Key States:**
```python
class KeyStatus(str, Enum):
    ACTIVE = "active"                 ***REMOVED*** Currently in use
    INACTIVE = "inactive"             ***REMOVED*** Not being used
    ROTATING = "rotating"             ***REMOVED*** In transition
    REVOKED = "revoked"               ***REMOVED*** Compromised/no longer trusted
    EXPIRED = "expired"               ***REMOVED*** Past expiration date
    PENDING_DELETION = "pending_deletion"  ***REMOVED*** Scheduled for removal
```
...
***REMOVED******REMOVED******REMOVED*** B. Access Control Policies

**Policy Types:**
```python
class AccessPolicy(str, Enum):
    ADMIN_ONLY = "admin_only"          ***REMOVED*** Only admin users
    SERVICE_ACCOUNT = "service_account"***REMOVED*** Service-to-service access
    APPLICATION = "application"        ***REMOVED*** Application-wide access
    USER_SPECIFIC = "user_specific"    ***REMOVED*** Specific user list
    PUBLIC_READ = "public_read"        ***REMOVED*** Public key material only
```
...
def _check_access(self, key: CryptographicKey, user_id: str) -> bool:
    ...
    if policy == AccessPolicy.PUBLIC_READ:
        return True  ***REMOVED*** Public keys accessible to all
    elif policy == AccessPolicy.USER_SPECIFIC:
        return user_id in key.allowed_users  ***REMOVED*** User whitelist check
    elif policy == AccessPolicy.ADMIN_ONLY:
        return is_admin(user_id)  ***REMOVED*** Admin-only check
    elif policy in (AccessPolicy.SERVICE_ACCOUNT, AccessPolicy.APPLICATION):
        return verify_service_credentials(user_id)  ***REMOVED*** Service auth
...
***REMOVED******REMOVED******REMOVED*** C. HSM Integration Hooks
...
class HSMConfig(BaseModel):
    ...
    enabled: bool = False
    provider: str = "pkcs11"  ***REMOVED*** pkcs11, aws_kms, azure_keyvault, google_kms
...
**Storage Model:**
```python
***REMOVED*** HSM integration metadata in CryptographicKey model
...
***REMOVED******REMOVED*** 3. ARCANA - Cryptographic Algorithm Assessment

***REMOVED******REMOVED******REMOVED*** A. Symmetric Encryption: AES-256-GCM
...
***REMOVED******REMOVED******REMOVED*** B. Key Derivation: PBKDF2

**Specification:**
```python
kdf = PBKDF2(
    algorithm=hashes.SHA256(),    ***REMOVED*** SHA-256 hash function
    length=32,                    ***REMOVED*** 256-bit output for AES-256
    salt=salt,                    ***REMOVED*** Random salt per derivation
    iterations=100000,            ***REMOVED*** Number of hash rounds
    backend=default_backend(),
)
return kdf.derive(settings.SECRET_KEY.encode())
```
...
***REMOVED******REMOVED******REMOVED*** C. Password Hashing: bcrypt
...
***REMOVED******REMOVED******REMOVED*** D. JWT Signature Algorithm: HS256
...
def verify_token(token: str, db: Session | None = None) -> TokenData | None:
    try:
        ...
        ***REMOVED*** Security: Reject refresh tokens used as access tokens
        if payload.get("type") == "refresh":
            return None

        ***REMOVED*** Check blacklist if db available
...
***REMOVED******REMOVED*** 4. HISTORY - Encryption Evolution & Configuration

***REMOVED******REMOVED******REMOVED*** A. Secret Key Management
...
def validate_secrets(cls, v: str, info) -> str:
    ...
    ***REMOVED*** Minimum length: 32 characters
    if len(v) < 32:
...
def validate_secrets_on_startup():
    ...
    ***REMOVED*** Similar checks for WEBHOOK_SECRET
...
***REMOVED******REMOVED******REMOVED*** B. S3 Upload Encryption
...
response = s3_client.put_object(
    ...
    Key=key,
    Body=body,
    ServerSideEncryption="AES256",  ***REMOVED*** S3 server-side encryption enabled
)
```
...
***REMOVED******REMOVED******REMOVED*** C. Redis Password Configuration
...
def validate_redis_password(cls, v: str, info) -> str:
    ...
    ***REMOVED*** Allow empty in development only
    if not v:
        if debug_mode:
    ...
    ***REMOVED*** Check against weak passwords
    if v.lower() in WEAK_PASSWORDS:
        ***REMOVED*** Log warning in debug mode, raise error in production
        ...

    ***REMOVED*** Minimum length: 16 characters
    if len(v) < 16:
...
***REMOVED******REMOVED*** 5. INSIGHT - Data Classification & Encryption Mapping

***REMOVED******REMOVED******REMOVED*** A. Data Classification Levels
...
***REMOVED******REMOVED******REMOVED*** B. Encryption Mapping by Data Type
...
***REMOVED******REMOVED*** 6. RELIGION - Personally Identifiable Information (PII) Encryption Status

***REMOVED******REMOVED******REMOVED*** A. PII Data Types in System

**Resident/Faculty PII:**
```python
***REMOVED*** From backend/app/models/person.py (implicit)
***REMOVED*** Typically includes:
- Full name
- Email address
- Phone number
- License numbers
...
```python
***REMOVED*** From backend/app/models/assignment.py
***REMOVED*** Includes:
...
```python
***REMOVED*** From backend/app/models/absence.py
***REMOVED*** Includes:
- Person ID
- Absence reason
- Dates absent
- Approval status
***REMOVED*** Reveals military movements and operational status
```

***REMOVED******REMOVED******REMOVED*** B. Encryption Status: PII at Rest
...
***REMOVED******REMOVED******REMOVED*** C. Encryption Status: PII in Transit
...
***REMOVED******REMOVED******REMOVED*** D. Recommendation: PII Encryption Strategy

**Option 1: Field-Level Encryption (Recommended)**
```python
class Person(Base):
    ***REMOVED*** Encrypted fields
    name_encrypted = Column(Text, nullable=True)
    email_encrypted = Column(Text, nullable=True)

    ***REMOVED*** Index on hash of encrypted value (for searching)
...
***REMOVED******REMOVED*** 7. NATURE - Encryption Overhead & Performance

***REMOVED******REMOVED******REMOVED*** A. Symmetric Encryption Performance
...
***REMOVED******REMOVED******REMOVED*** B. Encryption Operations in Critical Path
...
***REMOVED******REMOVED******REMOVED*** C. Caching Strategy

**Current Implementation:**
```python
CACHE_ENABLED: bool = True  ***REMOVED*** Service-level caching
CACHE_DEFAULT_TTL: int = 3600  ***REMOVED*** 1 hour
CACHE_SCHEDULE_TTL: int = 1800  ***REMOVED*** 30 minutes
```
...
***REMOVED******REMOVED*** 8. MEDICINE - Encryption Health Checks & Monitoring

***REMOVED******REMOVED******REMOVED*** A. Secret Validation on Startup

**Configuration Validation:**
```python
***REMOVED*** backend/app/main.py:41-77
def validate_secrets_on_startup():
    """Check that SECRET_KEY and WEBHOOK_SECRET are properly set."""

    errors = []

    ***REMOVED*** Validate SECRET_KEY
    if settings.SECRET_KEY in insecure_defaults:
        errors.append("SECRET_KEY is not set or uses an insecure default value")
    elif len(settings.SECRET_KEY) < 32:
        errors.append(f"SECRET_KEY is too short ({len(settings.SECRET_KEY)} chars, minimum 32)")

    ***REMOVED*** Validate WEBHOOK_SECRET
...
***REMOVED******REMOVED******REMOVED*** B. Key Usage Audit Logging
...
class KeyUsageLog(Base):
    ...
    operation = Column(String(100), nullable=False)  ***REMOVED*** encrypt, decrypt, sign, verify
...
***REMOVED******REMOVED******REMOVED*** C. Configuration Tests
...
***REMOVED******REMOVED*** 9. SURVIVAL - Key Rotation & Backup Procedures

***REMOVED******REMOVED******REMOVED*** A. Automatic Key Rotation
...
**Grace Period Logic:**
```python
***REMOVED*** During grace period, both old and new secrets are valid
***REMOVED*** Clients transition to new secret at their own pace
***REMOVED*** Old secret becomes invalid after grace period ends
...
***REMOVED*** Token validation allows both keys during grace period
valid_secrets = [old_secret, new_secret]  ***REMOVED*** Both valid until grace_period_ends
```

***REMOVED******REMOVED******REMOVED*** B. Key Backup Procedures
...
***REMOVED******REMOVED******REMOVED*** C. Key Recovery Procedures
...
***REMOVED******REMOVED*** 10. STEALTH - Plaintext Storage & Data Leakage Analysis

***REMOVED******REMOVED******REMOVED*** A. Plaintext Storage Inventory

**✓ CORRECTLY ENCRYPTED:**
```python
***REMOVED*** Password hashes (bcrypt)
User.hashed_password

***REMOVED*** Cryptographic keys at rest
CryptographicKey.encrypted_key_material
CryptographicKey.encrypted_private_key

***REMOVED*** Tenant-sensitive data
TenantEncryptionService encrypted data

***REMOVED*** Secret rotation history (hashes only, not secrets)
...
User.username          ***REMOVED*** Required for queries
User.email             ***REMOVED*** Required for queries
User.is_active         ***REMOVED*** Non-sensitive flag
User.role              ***REMOVED*** Non-sensitive role identifier
User.created_at        ***REMOVED*** Timestamp

***REMOVED*** Schedule metadata
Assignment.*           ***REMOVED*** Schedule assignments (OPSEC concern)
Block.*                ***REMOVED*** Block definitions
Rotation.*             ***REMOVED*** Rotation templates

***REMOVED*** Tenant isolation
TenantContext.tenant_id  ***REMOVED*** Required for multi-tenancy
```

**✓ NEVER STORED:**
```
JWT tokens              ***REMOVED*** Ephemeral, signed (not secret storage)
Plain passwords         ***REMOVED*** Only bcrypt hashes stored
API keys from clients   ***REMOVED*** Not stored
Webhook secrets         ***REMOVED*** Not stored (only rotated)
```

***REMOVED******REMOVED******REMOVED*** B. Potential Data Leakage Paths
...
***REMOVED******REMOVED******REMOVED*** C. Data Sanitization

**Logging Sanitization (backend/app/core/logging/sanitizers.py):**
```python
***REMOVED*** Removes sensitive fields from logs
sensitive_fields = {
...
```python
***REMOVED*** Generic errors returned to client
raise HTTPException(
    status_code=400,
    detail="Invalid credentials"  ***REMOVED*** No data leakage
)

***REMOVED*** Detailed errors logged server-side only
...
***REMOVED******REMOVED******REMOVED*** STRENGTHS (5)
...
***REMOVED******REMOVED******REMOVED*** WEAKNESSES (3)
...
***REMOVED******REMOVED******REMOVED*** RECOMMENDATIONS (5)
...
***REMOVED******REMOVED******REMOVED*** Standards Coverage

| Standard | Coverage | Status |
|---|---|---|
| **OWASP Top 10** | Cryptography (***REMOVED***2) | PASS |
...
***REMOVED******REMOVED******REMOVED*** Gaps for Healthcare (HIPAA)
