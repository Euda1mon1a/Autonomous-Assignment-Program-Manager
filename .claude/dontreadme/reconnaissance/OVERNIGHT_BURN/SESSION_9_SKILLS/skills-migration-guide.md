***REMOVED*** Skills Migration Guide
...
1. [Version Overview](***REMOVED***version-overview)
2. [Current Skill Versions](***REMOVED***current-skill-versions)
3. [Breaking Changes Inventory](***REMOVED***breaking-changes-inventory)
4. [Migration Procedures](***REMOVED***migration-procedures)
5. [Rollback Procedures](***REMOVED***rollback-procedures)
6. [Backwards Compatibility Matrix](***REMOVED***backwards-compatibility-matrix)
7. [Hidden Breaking Changes](***REMOVED***hidden-breaking-changes)
8. [Skill Evolution Patterns](***REMOVED***skill-evolution-patterns)
9. [Migration Checklist](***REMOVED***migration-checklist)

---

***REMOVED******REMOVED*** Version Overview

***REMOVED******REMOVED******REMOVED*** Versioning Strategy
...
***REMOVED******REMOVED******REMOVED*** Current PAI Version Metadata (Session 025)
...
***REMOVED******REMOVED******REMOVED*** Key Version Features (NEW)

Two new metadata fields introduced in Session 025:

```yaml
model_tier: opus|haiku  ***REMOVED*** Recommended Claude model for skill
parallel_hints:          ***REMOVED*** Parallelization constraints
  can_parallel_with: [skill1, skill2]        ***REMOVED*** Can run simultaneously
  must_serialize_with: [skill3, skill4]      ***REMOVED*** Must run sequentially
  preferred_batch_size: N                    ***REMOVED*** Optimal batch count
```

---

***REMOVED******REMOVED*** Current Skill Versions

***REMOVED******REMOVED******REMOVED*** Kai Pattern Skills (Advanced)
...
***REMOVED******REMOVED******REMOVED*** Legacy Pattern Skills (Standard)
...
***REMOVED******REMOVED*** Breaking Changes Inventory

***REMOVED******REMOVED******REMOVED*** Session 025 Additions

***REMOVED******REMOVED******REMOVED******REMOVED*** Feature: Parallelization Hints (NEW)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Feature: Model Tier Assignment (NEW)
...
***REMOVED******REMOVED******REMOVED*** Historical Breaking Changes (None Recorded)
...
***REMOVED******REMOVED*** Migration Procedures

***REMOVED******REMOVED******REMOVED*** Procedure 1: Adding Parallelization Hints to Existing Skill
...
1. **Identify Safe Parallels**
   ```bash
   ***REMOVED*** List current skill dependencies
   grep -r "dependencies:" .claude/skills/YOUR_SKILL/

   ***REMOVED*** Identify skills that don't share state
   ***REMOVED*** Example: lint-monorepo and test-writer are independent
   ```
...
2. **Update SKILL.md Frontmatter**
   ...
   name: your-skill
   description: ...
   model_tier: opus|haiku          ***REMOVED*** If applicable
   parallel_hints:
...
3. **Document Rationale**
   ```markdown
   ***REMOVED******REMOVED*** Parallelization Strategy
...
4. **Test Parallelization**
   ```bash
   ***REMOVED*** Invoke in parallel context
   ***REMOVED*** Run same operation 3x concurrently
   ***REMOVED*** Verify all instances complete successfully
   ```

5. **Update SKILL_INDEX.md**
   ```bash
   grep -n "your-skill" .claude/SKILL_INDEX.md
   ***REMOVED*** Update routing rules if parallelization changes optimal invocation pattern
   ```

---

***REMOVED******REMOVED******REMOVED*** Procedure 2: Creating Model Tier Assignment
...
***REMOVED******REMOVED******REMOVED*** Procedure 3: Migrating from Legacy to Kai Pattern
...
2. **Create Kai Structure**
   ...
   ├── Workflows/
   │   ├── workflow1.md            ***REMOVED*** Documented workflows
   │   ├── workflow2.md
   │   └── workflow3.md
   └── Reference/
       ├── reference1.md           ***REMOVED*** Reference material
       ├── reference2.md
       └── glossary.md
   ```
...
6. **Update SKILL_INDEX.md**
   ```yaml
   ***REMOVED*** Change pattern from legacy to kai
   pattern: kai
   ```

7. **Version Bump**
   ```yaml
   version: 2.0.0  ***REMOVED*** MAJOR bump (pattern change)
   ```
...
***REMOVED******REMOVED******REMOVED*** Procedure 4: Breaking Change - Adding Required Parameter
...
1. **Assess Impact**
   ```bash
   ***REMOVED*** Find all invocations of this skill
...
2. **Create Migration Guide**
   ```markdown
   ***REMOVED*** Migration: your-skill v1.x → v2.0.0

   ***REMOVED******REMOVED*** Breaking Change
   ...
   ***REMOVED******REMOVED******REMOVED*** How to Update
...
3. **Create Fallback Support (if critical)**
   ```python
   ***REMOVED*** Pseudo-code for backwards-compatible wrapper
   def invoke_skill(skill_name, params):
       if skill_name == "your-skill" and "schedule_id" not in params:
           ***REMOVED*** Try to infer schedule_id from context
...
4. **Deprecation Timeline**
   ```markdown
   ***REMOVED******REMOVED*** Deprecation Timeline
...
5. **Update Version**
   ```yaml
   version: 2.0.0  ***REMOVED*** MAJOR bump
   deprecated: false
   sunset_date: null
   ```
...
***REMOVED******REMOVED******REMOVED*** Scenario 1: Parallel Hints Causing Issues
...
1. **Identify Constraint Issue**
   ```bash
   ***REMOVED*** Check error logs for deadlock or state corruption
   grep -r "parallel" error.log | head -10
   ```

2. **Disable Parallelization (Immediate)**
   ```yaml
   ***REMOVED*** Edit SKILL.md
   parallel_hints:
     can_parallel_with: []           ***REMOVED*** Clear all parallel permissions
     must_serialize_with: [ALL]      ***REMOVED*** Force serialization
     preferred_batch_size: 1
   ```

3. **Test Serial Mode**
   ```bash
   ***REMOVED*** Invoke skill 3x sequentially
   ***REMOVED*** Verify all pass
   ```
...
5. **Restore Parallelization Safely**
   ...
   parallel_hints:
     ...
     must_serialize_with: [database-migration]
     preferred_batch_size: 2  ***REMOVED*** Conservative
   ```

---

***REMOVED******REMOVED******REMOVED*** Scenario 2: Model Tier Causing Issues
...
1. **Identify Model Issue**
   ```bash
   ***REMOVED*** Check if skill output quality degraded
   ***REMOVED*** Compare with previous model tier
   ```

2. **Switch Model Tier (Immediate)**
   ```yaml
   ***REMOVED*** Edit SKILL.md
   model_tier: opus  ***REMOVED*** If was haiku, switch to opus
   ```

3. **Re-run and Compare**
   ```bash
   ***REMOVED*** Run same test case with new model tier
   ***REMOVED*** Compare output quality
   ```
...
5. **Document Decision**
   ```markdown
   ***REMOVED******REMOVED*** Model Tier Rationale
...
***REMOVED******REMOVED******REMOVED*** Scenario 3: Legacy → Kai Migration Issues
...
4. **Re-migrate Carefully**
   ...
   ***REMOVED*** Review each change carefully
   ```

---

***REMOVED******REMOVED******REMOVED*** Scenario 4: Breaking Change Rollback
...
2. **Assess Damage**
   ```bash
   ***REMOVED*** Count failed invocations
...
3. **Two Recovery Paths**
   ...
   **Option B: Deploy Compatibility Layer (Gradual)**
   ```python
   ***REMOVED*** Add fallback logic to handle old API
...
***REMOVED******REMOVED*** Backwards Compatibility Matrix

***REMOVED******REMOVED******REMOVED*** Session 025 Compatibility
...
***REMOVED******REMOVED******REMOVED*** Cross-Version Compatibility
...
***REMOVED******REMOVED******REMOVED*** Skill-Level Compatibility
...
***REMOVED******REMOVED*** Hidden Breaking Changes

***REMOVED******REMOVED******REMOVED*** Watch Out For

***REMOVED******REMOVED******REMOVED******REMOVED*** Change 1: Skill Output Format Evolution

**What:** Skill might return different JSON structure

**How to Detect:**
```python
***REMOVED*** Check output schema version
result = invoke_skill("your-skill", params)
if "schema_version" not in result:
    ***REMOVED*** Likely v1.x output
...
```yaml
result:
  schema_version: "2.0.0"  ***REMOVED*** Add this!
  data: {...}
```

---

***REMOVED******REMOVED******REMOVED******REMOVED*** Change 2: Dependency Version Mismatch

**What:** Skill depends on another skill that changed

**How to Detect:**
```bash
***REMOVED*** Check dependency versions
grep "dependencies:" .claude/skills/YOUR_SKILL/SKILL.md

***REMOVED*** Verify each dependency exists and versions match
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Change 4: Parameter Validation Strictness

**What:** Skill starts rejecting previously-accepted parameters

**How to Detect:**
```bash
***REMOVED*** Look for validation logic changes
git log -p .claude/skills/YOUR_SKILL/ | grep -A10 "validate\|required"
```

**Example:**
```python
***REMOVED*** v1.x - lenient
def invoke_skill(params):
    schedule_id = params.get("schedule_id", None)  ***REMOVED*** Accepts None

***REMOVED*** v2.x - strict
def invoke_skill(params):
    schedule_id = params["schedule_id"]  ***REMOVED*** Requires non-None
    if not schedule_id:
...
***REMOVED******REMOVED*** Skill Evolution Patterns

***REMOVED******REMOVED******REMOVED*** Pattern 1: Legacy → Kai Refactoring (No Breaking Change)
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Feature Addition Cycle
...
***REMOVED******REMOVED******REMOVED*** Pattern 3: Deprecation Cycle
...
***REMOVED******REMOVED*** Quick Reference: Version Bump Decision Tree
...
***REMOVED******REMOVED******REMOVED*** Example 1: Adding Model Tier to automated-code-fixer
...
name: automated-code-fixer
description: Automated IT helper for detecting and fixing code issues
model_tier: opus  ***REMOVED*** NEW
parallel_hints:   ***REMOVED*** NEW
  can_parallel_with: [lint-monorepo, test-writer]
  must_serialize_with: [database-migration]
  preferred_batch_size: 3
---
...
***REMOVED******REMOVED******REMOVED*** Example 2: Adding Required Parameter (Breaking)
...
invoke_skill("swap-management", {
    "requester_id": "faculty_001",
    "schedule_block": "block_10_2025"  ***REMOVED*** NEW REQUIRED
})
```

**Migration Guide:**
```markdown
***REMOVED*** Migration: swap-management v1.x → v2.0.0

***REMOVED******REMOVED*** What's Changing
...
***REMOVED******REMOVED******REMOVED*** Before (v1.x)
```yaml
invoke_skill("swap-management", {
    "requester_id": "faculty_001"
})
...
***REMOVED******REMOVED******REMOVED*** After (v2.0.0)
```yaml
invoke_skill("swap-management", {
    "requester_id": "faculty_001",
    "schedule_block": "block_10_2025"  ***REMOVED*** Add this
})
```
...
***REMOVED******REMOVED*** Appendix: Version History

***REMOVED******REMOVED******REMOVED*** PAI Version History
