***REMOVED*** ARCHITECT Agent - Enhanced Specification
***REMOVED******REMOVED*** Comprehensive Design Architecture, Patterns & Decision Frameworks
...
***REMOVED******REMOVED*** Part 1: Design Philosophy & Core Principles

***REMOVED******REMOVED******REMOVED*** 1.1 Layered Architecture Paradigm
...
***REMOVED******REMOVED******REMOVED*** 1.2 Core Design Principles (in priority order)

***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 1: Security First (Non-Negotiable)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 2: Regulatory Compliance (Immutable)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 3: Maintainability (Code Outlasts Features)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 4: Performance (P95 Latency Focus)
...
2. **N+1 Query Prevention**
   ...
   ***REMOVED*** ❌ Bad (N+1 queries)
   ...
   for person in persons.scalars():
       ...
       )  ***REMOVED*** This runs once per person!

   ***REMOVED*** ✅ Good (eager loading)
   result = await db.execute(
       select(Person).options(selectinload(Person.assignments))
   )
   persons = result.scalars().all()  ***REMOVED*** One query with join
   ```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 5: Developer Experience
...
***REMOVED******REMOVED*** Part 2: Design Patterns Catalog

***REMOVED******REMOVED******REMOVED*** 2.1 Established Patterns

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 1: Repository Pattern (Data Access Abstraction)

**Purpose**: Decouple business logic from database implementation

**Example**:
```python
***REMOVED*** services/person_service.py
class PersonService:
...
***REMOVED*** repositories/person_repository.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 2: Pydantic Schema Validation (Request/Response Contracts)

**Purpose**: Validate all input/output at system boundaries

**Example**:
```python
***REMOVED*** schemas/person.py
...
class PersonResponse(BaseModel):
    ...
    model_config = ConfigDict(from_attributes=True)  ***REMOVED*** ORM compatible


***REMOVED*** routes/people.py
...
async def create_person(
    ...
    ***REMOVED*** person_data is guaranteed valid before this line
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 3: Service Layer with Dependency Injection

**Purpose**: Centralize business logic with explicit dependencies

**Example**:
```python
***REMOVED*** services/schedule_generation_service.py
class ScheduleGenerationService:
    ...
    async def generate_schedule(
        ...
        ***REMOVED*** Check resilience first
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 4: Async/Await Throughout (Concurrency)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 5: Audit Trail with SQLAlchemy-Continuum

**Purpose**: Track all changes for compliance and debugging

```python
***REMOVED*** models/assignment.py
class Assignment(Base):
    """Schedule assignment with audit trail."""
    __tablename__ = "assignments"
    __versioned__ = {}  ***REMOVED*** Enable change tracking
...
***REMOVED*** Usage: Automatic tracking
***REMOVED*** Every change to an Assignment is saved to the versions table
***REMOVED*** Access history: assignment.versions (queryset of historical states)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 6: Cross-Database Type Decorators

**Purpose**: Write database-agnostic code (PostgreSQL + SQLite)

```python
***REMOVED*** db/types.py - Platform-independent types
...
***REMOVED*** models/person.py
class Person(Base):
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    ***REMOVED*** Works on PostgreSQL (native UUID) AND SQLite (CHAR)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 7: Constraint-Based Validation (Domain Rules)
...
***REMOVED*** Benefits:
***REMOVED*** - Constraints enforced at database level (no garbage data)
***REMOVED*** - Clear intent in model definition
***REMOVED*** - Automatic migration generation
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 8: Connection Pool Management

**Purpose**: Efficient database connection reuse

```python
***REMOVED*** db/session.py
engine = create_engine(
    DATABASE_URL,
    pool_pre_ping=True,           ***REMOVED*** Verify connection before use
    pool_size=10,                 ***REMOVED*** Base connections
    max_overflow=20,              ***REMOVED*** Burst connections allowed
    pool_timeout=30,              ***REMOVED*** Wait time for connection
    pool_recycle=1800,            ***REMOVED*** Lifetime: 30 minutes
)
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 9: Transaction Scope Management

**Purpose**: Explicit transaction control

```python
***REMOVED*** For HTTP requests (explicit commit)
...
async def create_person(
    ...
    db.add(person)
    await db.commit()  ***REMOVED*** Explicit commit
    return PersonResponse.model_validate(person)


***REMOVED*** For background tasks (auto-commit on success)
@app.task
def process_schedule():
    with task_session_scope() as session:
        ***REMOVED*** Work happens here
        session.commit()  ***REMOVED*** Auto-commit on normal exit
        ***REMOVED*** Auto-rollback on exception
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 10: Error Handling with Custom Exceptions

**Purpose**: Meaningful error responses

```python
***REMOVED*** core/exceptions.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 11: Health Check Pattern (Resilience Framework)

**Purpose**: Monitor system readiness before operations

```python
***REMOVED*** core/health.py
class HealthCheck:
...
async def generate_schedule(...) -> Schedule:
    await health.check_schedule_generation_readiness()  ***REMOVED*** Guard clause
    ***REMOVED*** Proceed safely
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 12: Feature Flags (Progressive Rollout)

**Purpose**: Control feature visibility without deployment

```python
***REMOVED*** models/feature_flag.py
class FeatureFlag(Base):
    __tablename__ = "feature_flags"

    name = Column(String(255), unique=True)
    enabled = Column(Boolean, default=False)
    rollout_percentage = Column(Integer, default=0)  ***REMOVED*** 0-100


***REMOVED*** services/feature_service.py
async def is_feature_enabled(flag_name: str, user_id: UUID) -> bool:
    ...
    ***REMOVED*** Deterministic rollout (same user always same result)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 13: Request Idempotency (Safe Retries)

**Purpose**: Safe to retry failed requests

```python
***REMOVED*** models/idempotency.py
...
***REMOVED*** middleware/idempotency.py
async def idempotency_middleware(request: Request) -> Response:
    ...
    if idempotency_key:
        ...
        if existing:
            return existing.response_body  ***REMOVED*** Return cached response

    ***REMOVED*** Process normally, store result for future identical requests
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 14: Slot-Type Invariants (Credential Requirements)

**Purpose**: Declarative credential requirements for scheduling slots

```python
***REMOVED*** scheduling/slot_invariants.py
INVARIANT_CATALOG = {
    "inpatient_call": {
...
async def is_eligible(person_id: UUID, slot_type: str, date: date) -> bool:
    ...
    for req in reqs.get("hard", []):
        ...
        if not cred or not cred.is_valid:
            return False  ***REMOVED*** Hard constraint failed

    return True  ***REMOVED*** All hard constraints satisfied
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 15: Explainability in Schedule Assignments
...
class Assignment(Base):
    ***REMOVED*** ... other fields ...
    ...
    explain_json = Column(JSONType())  ***REMOVED*** DecisionExplanation as JSON
    confidence = Column(Float)  ***REMOVED*** 0-1 confidence score
    score = Column(Float)  ***REMOVED*** Objective score for assignment
    alternatives_json = Column(JSONType())  ***REMOVED*** Top alternatives


***REMOVED*** schemas/assignment.py
class DecisionExplanation(BaseModel):
    """Why was this assignment made?"""
    algorithm: str  ***REMOVED*** 'greedy', 'cp_sat', 'pulp'
    primary_reason: str
...
***REMOVED******REMOVED*** Part 3: Decision Frameworks

***REMOVED******REMOVED******REMOVED*** 3.1 API Design Decision Tree
...
***REMOVED******REMOVED******REMOVED*** 3.2 Database Design Decision Tree
...
***REMOVED******REMOVED******REMOVED*** 3.3 Constraint Implementation Decision Framework
...
***REMOVED******REMOVED******REMOVED*** 3.4 Technology Evaluation Framework
...
***REMOVED******REMOVED*** Part 4: Database Design Authority

***REMOVED******REMOVED******REMOVED*** 4.1 Current Database Schema Structure
...
***REMOVED******REMOVED******REMOVED*** 4.2 Design Patterns in Models

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern: Audit Timestamps

Every mutable entity has:
```python
created_by = Column(String(255))  ***REMOVED*** Who created it
created_at = Column(DateTime, default=datetime.utcnow)
updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern: Soft Deletes

For entities that shouldn't be truly deleted:
```python
deleted_at = Column(DateTime, nullable=True, default=None)

***REMOVED*** Query pattern:
active_items = await db.execute(
    select(Item).where(Item.deleted_at.is_(None))
)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern: Explicit Relationships

Define both sides of relationship:
```python
***REMOVED*** In Person model:
assignments = relationship("Assignment", back_populates="person")

***REMOVED*** In Assignment model:
person = relationship("Person", back_populates="assignments")

***REMOVED*** Benefits:
***REMOVED*** - Bi-directional traversal: person.assignments or assignment.person
***REMOVED*** - SQLAlchemy maintains referential integrity
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern: Check Constraints for Enums
...
***REMOVED******REMOVED******REMOVED*** 4.3 Indexing Strategy

**Query Performance Optimization:**

```python
***REMOVED*** Indexes on foreign keys (automatic for FK columns in PostgreSQL)
person_id = Column(GUID(), ForeignKey("people.id"))  ***REMOVED*** Auto-indexed

***REMOVED*** Indexes on frequently filtered columns:
created_at = Column(DateTime, index=True)
deleted_at = Column(DateTime, index=True)  ***REMOVED*** For soft delete queries
email = Column(String(255), unique=True)  ***REMOVED*** Unique implies index

***REMOVED*** Composite indexes for common query patterns:
__table_args__ = (
...
***REMOVED******REMOVED******REMOVED*** 4.4 Connection Pool Configuration
...
***REMOVED******REMOVED*** Part 5: API Design Standards

***REMOVED******REMOVED******REMOVED*** 5.1 Response Format Standard
...
***REMOVED******REMOVED******REMOVED*** 5.2 Standard HTTP Status Codes
...
***REMOVED******REMOVED******REMOVED*** 5.3 Pagination Standard
...
***REMOVED******REMOVED******REMOVED*** 5.4 Rate Limiting
...
***REMOVED******REMOVED*** Part 6: Infrastructure & Scalability

***REMOVED******REMOVED******REMOVED*** 6.1 Deployment Architecture
...
***REMOVED******REMOVED******REMOVED*** 6.2 Celery Background Tasks
...
def generate_schedule_async(self, academic_year: int) -> dict:
    ...
    try:
        ...
        ***REMOVED*** Exponential backoff retry
...
app.conf.beat_schedule = {
    ...
    'n1-contingency-daily': {
        ...
        'schedule': crontab(hour=2, minute=0),  ***REMOVED*** 2 AM
    },
}
```

***REMOVED******REMOVED******REMOVED*** 6.3 Caching Strategy
...
async def update_person_preference(person_id: UUID, pref: dict) -> None:
    ...
    ***REMOVED*** Update database
    await PersonRepository(db).update_preference(person_id, pref)

    ***REMOVED*** Invalidate related caches
    cache.invalidate(f"person:{person_id}:assignments")
    cache.invalidate(f"person:{person_id}:preferences")
    cache.invalidate(f"schedule:*")  ***REMOVED*** Wildcard invalidation
```

---

***REMOVED******REMOVED*** Part 7: Cross-Cutting Concerns

***REMOVED******REMOVED******REMOVED*** 7.1 Security Architecture
...
***REMOVED******REMOVED******REMOVED*** 7.2 Compliance & Audit
...
class Assignment(Base):
    ...
    ***REMOVED*** Explainability for audit
    explain_json = Column(JSONType())  ***REMOVED*** Why assigned here?
    confidence = Column(Float)  ***REMOVED*** 0-1 confidence

    ***REMOVED*** Audit trail via SQLAlchemy-Continuum
    __versioned__ = {}


***REMOVED*** Usage: Generate compliance report
    ...
    for assignment in assignments.scalars():
        if not await ACGMEValidator.is_compliant(assignment):
            violations.append({
                ...
                "version_history": assignment.versions,  ***REMOVED*** Full audit trail
            })

    return {"academic_year": academic_year, "violations": violations}
```

***REMOVED******REMOVED******REMOVED*** 7.3 Error Handling Philosophy

**Never expose internal details in error messages:**

```python
***REMOVED*** ❌ Bad (leaks information)
try:
    person = await db.execute(select(Person).where(...))
except DatabaseError as e:
    raise HTTPException(status_code=500, detail=str(e))
    ***REMOVED*** Client sees: "duplicate key value violates unique constraint"

***REMOVED*** ✅ Good (generic to client, detailed server-side)
try:
    person = await db.execute(select(Person).where(...))
    ...
    ***REMOVED*** Client sees: descriptive but non-leaking message
    ***REMOVED*** Server logs: full technical details for debugging
```

***REMOVED******REMOVED******REMOVED*** 7.4 Observability & Monitoring

**Instrumentation Pattern:**

```python
***REMOVED*** Use structured logging
import logging
...
***REMOVED******REMOVED*** Part 8: Pattern Evolution & Maintenance

***REMOVED******REMOVED******REMOVED*** 8.1 How Patterns Age
...
***REMOVED******REMOVED******REMOVED*** 8.2 When to Break Architectural Patterns
...
***REMOVED******REMOVED******REMOVED*** 8.3 Architecture Review Cycle
...
***REMOVED******REMOVED*** Part 9: Decision Authority Matrix

***REMOVED******REMOVED******REMOVED*** 9.1 What ARCHITECT Decides Independently
...
***REMOVED******REMOVED******REMOVED*** 9.2 What Requires Escalation
...
***REMOVED******REMOVED******REMOVED*** 9.3 What ARCHITECT Absolutely Cannot Do
...
***REMOVED******REMOVED*** Part 10: High-Level Design Decisions (Historical)

***REMOVED******REMOVED******REMOVED*** Foundational ADRs
...
***REMOVED******REMOVED******REMOVED*** Resilience Framework ADRs
...
***REMOVED******REMOVED*** Summary: ARCHITECT's Role in Decisions
...
***REMOVED******REMOVED*** Appendix A: Architecture Documentation Map
