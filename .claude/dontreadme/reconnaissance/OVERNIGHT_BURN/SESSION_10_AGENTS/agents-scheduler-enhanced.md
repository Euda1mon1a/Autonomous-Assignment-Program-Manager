***REMOVED*** SCHEDULER Agent - Comprehensive Enhanced Specification
...
1. [Overview & Mission](***REMOVED***overview--mission)
2. [Core Competencies](***REMOVED***core-competencies)
3. [Complete Constraint Catalog](***REMOVED***complete-constraint-catalog)
4. [Algorithm Deep Dive](***REMOVED***algorithm-deep-dive)
5. [Solver Patterns & Selection](***REMOVED***solver-patterns--selection)
6. [Constraint Implementation Patterns](***REMOVED***constraint-implementation-patterns)
7. [Infeasibility Detection & Recovery](***REMOVED***infeasibility-detection--recovery)
8. [Scheduling Philosophy](***REMOVED***scheduling-philosophy)
9. [Advanced Capabilities](***REMOVED***advanced-capabilities)
10. [Integration Patterns](***REMOVED***integration-patterns)
11. [Best Practices & Pitfalls](***REMOVED***best-practices--pitfalls)
12. [Operational Playbooks](***REMOVED***operational-playbooks)

---

***REMOVED******REMOVED*** Overview & Mission

***REMOVED******REMOVED******REMOVED*** The SCHEDULER Agent Role
...
***REMOVED******REMOVED******REMOVED*** Core Mission Statement
...
***REMOVED******REMOVED******REMOVED*** Why This Agent Exists
...
***REMOVED******REMOVED******REMOVED*** Competency 1: Constraint Modeling & Composition
...
***REMOVED******REMOVED******REMOVED*** Competency 2: Multi-Solver Strategy
...
***REMOVED******REMOVED******REMOVED*** Competency 3: ACGME Compliance Expertise
...
***REMOVED******REMOVED******REMOVED*** Competency 4: Resilience Integration
...
***REMOVED*** Post-generation: Validate N-1 compliance
...
***REMOVED******REMOVED******REMOVED*** Competency 5: Swap Management
...
***REMOVED******REMOVED******REMOVED*** Competency 6: Emergency Coverage
...
***REMOVED******REMOVED*** Complete Constraint Catalog

***REMOVED******REMOVED******REMOVED*** Hard Constraints (MUST Satisfy All)

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Availability Constraint (CRITICAL)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. 80-Hour Rule Constraint (CRITICAL)
...
def add_to_cpsat(self, model, variables, context):
    ...
    for start_date in all_possible_start_dates:
        window_end = start_date + timedelta(days=27)  ***REMOVED*** 28 days inclusive
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. One-in-Seven Rule Constraint (CRITICAL)
...
def add_to_cpsat(self, model, variables, context):
    ...
    for start_date in all_seven_day_windows:
        ...
        for resident in context.residents:
            ***REMOVED*** Max consecutive assigned days = 6
            ***REMOVED*** (leaves at least 1 day off in 7-day period)
            assignments = sum(
                x[resident_idx, block_idx] for block_idx in blocks_in_window
            )
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Supervision Ratio Constraint (CRITICAL)
...
def add_to_cpsat(self, model, variables, context):
    ...
    for block in context.blocks:
        ...
        ***REMOVED*** Count faculty assignments
        faculty_count = sum(
            x[faculty_idx, block_idx] for faculty in context.faculty
        )

        ***REMOVED*** Count residents
        pgy1_count = sum(
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Capacity Constraint (CRITICAL)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 6. Overnight Call Generation Constraint (HIGH)
...
def add_to_cpsat(self, model, variables, context):
    ...
    for block in sunday_thursday_blocks:
        ...
        model.Add(faculty_on_call == 1)  ***REMOVED*** Exactly 1
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 7. FMIT Constraints (HIGH)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 8. Post-Call Constraints (HIGH)
...
***REMOVED******REMOVED******REMOVED*** Soft Constraints (Optimize Within Hard Bounds)

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Equity Constraint
...
def get_soft_penalty(assignments, context):
    ...
    return variance  ***REMOVED*** Penalty: lower variance is better
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Continuity Constraint
...
def get_soft_penalty(assignments, context):
    ...
    penalty = 0
    for resident in context.residents:
        ***REMOVED*** Find all rotation sequences for resident
        for sequence in get_assignment_sequences(resident):
            ***REMOVED*** Count "breaks" in sequence (switches to different rotation)
            breaks = count_rotation_changes(sequence)
            penalty += breaks  ***REMOVED*** Higher penalty for more fragmentation
    return penalty
```

**Weight**: 5 (medium priority - nice to have but not critical)

***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Coverage Constraint
...
def get_soft_objective(assignments, context):
    ...
    return sum(assignments)  ***REMOVED*** Primary objective: maximize coverage
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Call Equity Constraints
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Preference Trail Constraint (Stigmergy)
...
def get_soft_penalty(assignments, context):
    ...
    for resident in context.residents:
        ...
        ***REMOVED*** Residents prefer rotations they've done before
        ***REMOVED*** Penalize unfamiliar rotations
        for assignment in assignments_for_resident:
            if assignment.template_id not in pref_trail:
                penalty += 1  ***REMOVED*** Unfamiliar rotation
            else:
                penalty -= 0.5  ***REMOVED*** Familiar rotation preferred
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 6. Resilience Soft Constraints
...
def get_soft_penalty(assignments, context):
    ...
    penalty = 0
    for faculty in context.faculty:
        hub_score = context.hub_scores.get(faculty.id, 0.0)  ***REMOVED*** 0.0-1.0
        ...
        if assignments_count > max_safe_assignments:
            ...
            penalty += overage * hub_score  ***REMOVED*** Penalize more for critical faculty
...
***REMOVED******REMOVED*** Algorithm Deep Dive

***REMOVED******REMOVED******REMOVED*** Algorithm 1: Greedy Heuristic Solver
...
***REMOVED******REMOVED******REMOVED*** Algorithm 2: CP-SAT Constraint Programming Solver
...
***REMOVED******REMOVED******REMOVED*** Algorithm 3: PuLP Linear Programming Solver
...
***REMOVED******REMOVED******REMOVED*** Algorithm 4: Hybrid Solver
...
***REMOVED******REMOVED*** Solver Patterns & Selection

***REMOVED******REMOVED******REMOVED*** Complexity Estimation
...
def estimate_complexity(context) -> dict:
    ...
    ***REMOVED*** Factor 2: Constraint density
    ...
    ***REMOVED*** Factor 3: Availability (sparsity)
    available_slots = sum(
    ...
    sparsity_score = (1.0 - sparsity) * 20  ***REMOVED*** Dense = easier

    ***REMOVED*** Factor 4: Soft constraints (optimization goals)
    ...
    ***REMOVED*** Combine into complexity score
...
***REMOVED*** Step 1: Estimate complexity
...
***REMOVED*** Step 2: Select solver and timeout
if complexity_report['score'] < 50:
    solver = "cp_sat"
    timeout = 30  ***REMOVED*** Seconds
else:
    solver = "hybrid"
    timeout = 120

***REMOVED*** Step 3: Generate with selected solver
result = engine.generate(algorithm=solver, timeout_seconds=timeout)
```

***REMOVED******REMOVED******REMOVED*** Warmstart Pattern (Intelligence Reuse)
...
def generate_with_warmstart(context):
    ...
    ***REMOVED*** Step 1: Run greedy quickly (gets ~70% quality in <1s)
    greedy_solver = GreedySolver()
    greedy_result = greedy_solver.solve(context)

    ***REMOVED*** Step 2: Use greedy as warmstart for CP-SAT
    cp_sat_solver = CPSATSolver(warmstart=greedy_result)
    cp_sat_result = cp_sat_solver.solve(context, timeout=30)

    ***REMOVED*** Step 3: CP-SAT improves on greedy (gets optimal within 30s)
    ***REMOVED*** Typically: greedy quality 70%, warmstart quality 85%+, optimal quality 95%+
...
***REMOVED******REMOVED******REMOVED*** Fallback Chain Pattern
...
def solve_with_fallback(context, timeout=60):
    ...
    ***REMOVED*** Try 1: CP-SAT (best quality)
    try:
    ...
    ***REMOVED*** Try 2: PuLP (fast backup)
    ...
    ***REMOVED*** Try 3: Greedy (always works)
    try:
        ...
        ***REMOVED*** Complete failure - return conflict report
        return generate_conflict_report(context, e)
```

---

***REMOVED******REMOVED*** Constraint Implementation Patterns

***REMOVED******REMOVED******REMOVED*** Pattern 1: Hard Constraint Implementation
...
class MyHardConstraint(HardConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** Add constraint equations
        for resident in context.residents:
            r_i = context.resident_idx[resident.id]
            for block in context.blocks:
                b_i = context.block_idx[block.id]

                ***REMOVED*** Example: Don't assign if unavailable
    ...
    def apply_to_pulp(self, model, variables, context):
        ...
        """
        ***REMOVED*** Most hard constraints only validate, not apply to LP
        pass
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Soft Constraint Implementation
...
class MyEquityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** Calculate workload for each resident
        workload = {}
        for resident in context.residents:
            r_i = context.resident_idx[resident.id]
            workload[r_i] = sum(
    ...
    def validate(self, assignments, context):
        ...
        """
        ***REMOVED*** Calculate fairness metrics
        ...
        return {
            "mean_workload": mean,
            "variance": variance,
            "fairness_score": 1.0 / (1.0 + variance)  ***REMOVED*** Higher is fairer
        }
```

***REMOVED******REMOVED******REMOVED*** Pattern 3: Constraint with Context-Dependent Logic
...
class SundayCallEquityConstraint(SoftConstraint):
    ...
    def add_to_cpsat(self, model, variables, context):
        ...
        ***REMOVED*** Get all Sunday blocks
        sunday_blocks = [b for b in context.blocks if b.date.weekday() == 6]  ***REMOVED*** 6 = Sunday

        ***REMOVED*** Count Sunday calls per faculty
        sunday_calls_per_faculty = {}
        for faculty in context.faculty:
        ...
        ***REMOVED*** Penalize variance in Sunday calls
        if len(sunday_calls_per_faculty) > 0:
            ...
            ***REMOVED*** Penalty in objective
            model.Maximize(model.objective_score - variance * self.weight)
```

---

***REMOVED******REMOVED*** Infeasibility Detection & Recovery

***REMOVED******REMOVED******REMOVED*** Recognizing Infeasible Problems
...
***REMOVED******REMOVED******REMOVED*** Infeasibility Analysis Workflow
...
def analyze_infeasibility(context, constraint_manager):
    ...
    ***REMOVED*** Step 1: Check basic viability
    report = {}

    ***REMOVED*** Can all residents be scheduled at least once?
    ...
    ***REMOVED*** Step 2: Run constraint feasibility checks
    ...
    ***REMOVED*** Step 3: Suggest relaxations
...
***REMOVED******REMOVED******REMOVED*** Recovery Strategies

***REMOVED******REMOVED******REMOVED******REMOVED*** Strategy 1: Soft Constraint Relaxation
...
def relax_soft_constraints(constraint_manager):
    ...
    ***REMOVED*** Identify soft constraints currently enabled
    soft_constraints = [c for c in constraint_manager.get_enabled()
                       if isinstance(c, SoftConstraint)]

    ***REMOVED*** Reduce weights progressively
    relaxations = [
        ("Preferences", 0.5),      ***REMOVED*** 50% of original weight
        ("Continuity", 0.3),       ***REMOVED*** 30% of original weight
        ("Coverage", 0.8),         ***REMOVED*** 80% of original weight
        ("Equity", 0.5),           ***REMOVED*** 50% of original weight
    ]
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Strategy 2: Extend Scheduling Window
...
def progressive_relaxation(context, constraint_manager, target_feasibility=True):
    ...
    relaxation_levels = [
        ***REMOVED*** Level 0: Original constraints (most strict)
        {'name': 'Original', 'changes': []},

        ***REMOVED*** Level 1: Relax soft constraints
        {'name': 'Soft Relaxed', 'changes': [
            {'constraint': 'Equity', 'weight': 0.5},
            {'constraint': 'Continuity', 'weight': 0.0},  ***REMOVED*** Disable
        ]},

        ***REMOVED*** Level 2: Relax medium-priority hard constraints
        {'name': 'Medium Hard Relaxed', 'changes': [
            {'constraint': 'OneInSevenRule', 'days_off': 6},  ***REMOVED*** 1-in-6 instead of 1-in-7
            {'constraint': 'CallSpacing', 'min_spacing': 2},  ***REMOVED*** 2 days instead of 3
        ]},

        ***REMOVED*** Level 3: Relax high-priority but sometimes flexible
        {'name': 'High Hard Relaxed', 'changes': [
            {'constraint': 'EightyHourRule', 'hours_allowed': 85},  ***REMOVED*** 85hr instead of 80hr
        ]},

        ***REMOVED*** Level 4: Block extensions
    ...
    ***REMOVED*** If all relaxations fail, return last attempt with conflict report
    return result, relaxation_levels[-1]
```

---

***REMOVED******REMOVED*** Scheduling Philosophy

***REMOVED******REMOVED******REMOVED*** Design Principles

***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 1: Constraints First
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 2: Transparency & Explainability
...
```python
***REMOVED*** For every assignment, provide explanation
class AssignmentExplanation:
    resident: Person
    block: Block
    template: RotationTemplate

    ***REMOVED*** Why this resident?
    reasons_chosen: list[str]  ***REMOVED*** "Low current workload", "Preferred rotation", etc.

    ***REMOVED*** Why this rotation type?
    template_reasons: list[str]  ***REMOVED*** "Balanced rotation diversity", "Educational requirement", etc.

    ***REMOVED*** What almost happened?
    alternatives_considered: list[Alternative]  ***REMOVED*** Who else could have been assigned

    ***REMOVED*** Confidence score
    confidence: float  ***REMOVED*** 0.0-1.0 (1.0 = obviously best choice)
```

**Usage**: Greedy solver provides explanations for every assignment

***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 3: Resilience by Design
...
```python
***REMOVED*** Before generation: Check can survive 1 faculty loss
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 4: ACGME Compliance Non-Negotiable
...
```python
***REMOVED*** Any solver result with ACGME violations is REJECTED
...
***REMOVED*** Never allow partial schedules if compliance uncertain
assert validation_result.valid, "Schedule validation failed"
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 5: Fairness Bias
...
```python
***REMOVED*** Soft constraint priorities (in order)
SOFT_CONSTRAINT_WEIGHTS = {
    'coverage': 1000,              ***REMOVED*** Ensure all blocks covered
    'equity': 50,                  ***REMOVED*** Fair distribution primary
    'continuity': 20,              ***REMOVED*** Minimize disruption
    'preferences': 15,             ***REMOVED*** Honor requests when possible
    'optimization': 5,             ***REMOVED*** Fine-tuning
}

***REMOVED*** Explicit bias in equity calculations
***REMOVED*** Prefer solutions that reduce variance (fairness) over maximizing individual preferences
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Principle 6: Audit Trail & Accountability
...
```python
***REMOVED*** Every schedule generation creates audit record
...
***REMOVED*** Every assignment traces back to generation
assignment = Assignment(
    person_id=resident_id,
    block_id=block_id,
    template_id=template_id,
    schedule_run_id=schedule_run.id,  ***REMOVED*** <- Links to generation context
    created_at=now,
...
***REMOVED******REMOVED*** Advanced Capabilities

***REMOVED******REMOVED******REMOVED*** Capability 1: Night Float → Post-Call Audit
...
def audit_nf_pc_transitions(assignments, context) -> NFPCAudit:
    ...
    for resident in context.residents:
        ***REMOVED*** Find all NF → PC transitions
        ...
        for i in range(len(sorted_assignments) - 1):
            ...
            if (current.template.name == 'Night Float' and
                ...
                ***REMOVED*** Check: Post-Call day must be lighter duty
                ***REMOVED*** Check: No overnight call within 24 hours after Night Float
                ***REMOVED*** Check: Adequate rest period
...
***REMOVED******REMOVED******REMOVED*** Capability 2: Resilience Health Integration
...
class ResilienceMetrics:
    ***REMOVED*** Tier 1: Core metrics
    utilization_rate: float              ***REMOVED*** Target: < 0.80
    hub_centrality_max: float            ***REMOVED*** Max centrality score (critical faculty)
    n1_compliant: bool                   ***REMOVED*** Can survive 1 faculty loss?
    n2_compliant: bool                   ***REMOVED*** Can survive 2 faculty loss?

    ***REMOVED*** Tier 2: Coverage metrics
    coverage_rate: float                 ***REMOVED*** % blocks assigned
    critical_service_staffing: dict      ***REMOVED*** Minimum staffing per service

    ***REMOVED*** Health status
    status: str                          ***REMOVED*** GREEN, YELLOW, ORANGE, RED, BLACK
    risk_score: float                    ***REMOVED*** 0.0-1.0 (1.0 = highest risk)
```
...
def calculate_health_score(metrics: ResilienceMetrics) -> float:
    ...
    ***REMOVED*** Component 1: Utilization (should be < 80%)
    if metrics.utilization_rate <= 0.70:
        util_score = 1.0
    elif metrics.utilization_rate <= 0.80:
        util_score = 0.8
    else:
        util_score = 0.5  ***REMOVED*** Over-utilized, risky

    ***REMOVED*** Component 2: N-1 compliance (can lose 1 person)
    n1_score = 1.0 if metrics.n1_compliant else 0.5

    ***REMOVED*** Component 3: Coverage (all blocks assigned)
    coverage_score = metrics.coverage_rate

    ***REMOVED*** Component 4: Hub concentration (critical faculty over-relied on?)
...
***REMOVED******REMOVED******REMOVED*** Capability 3: Preference Trail (Stigmergy)
...
def track_preference_trail(resident_id, assignments, context):
    ...
    for assignment in assignments:
        ...
        ***REMOVED*** Increment preference counter for this template
        if template_id not in pref_trail:
            pref_trail[template_id] = {
                'count': 0,
                'satisfaction': 0,  ***REMOVED*** 1.0 = loved it, -1.0 = hated it
                'last_assigned': None
            }
        ...
        ***REMOVED*** Can accumulate satisfaction from post-rotation surveys
        ***REMOVED*** pref_trail[template_id]['satisfaction'] += survey_score
...
def use_preference_trail(context):
    ...
    ***REMOVED*** Add soft constraint: prefer templates in resident's trail
    for resident in context.residents:
        trail = context.preference_trails.get(resident.id, {})
        preferred_templates = [
            tid for tid, info in trail.items()
            if info['satisfaction'] > 0.5  ***REMOVED*** Resident liked this rotation
        ]

        ***REMOVED*** During solving: bonus assignments to preferred templates
        ***REMOVED*** Weight based on how much they liked it
        for template_id in preferred_templates:
            satisfaction = trail[template_id]['satisfaction']
            bonus = satisfaction * 5  ***REMOVED*** Soft constraint weight
            add_preference_bonus(resident_id, template_id, bonus)
```

***REMOVED******REMOVED******REMOVED*** Capability 4: Complexity-Adaptive Solving
...
def generate_adaptive(context, target_quality='balanced'):
    ...
    ***REMOVED*** Step 2: Map complexity → solver + timeout
...
def generate_with_resilience(db, start_date, end_date):
    ...
    ***REMOVED*** Pre-check: System healthy enough to generate?
    pre_health = resilience.get_health_score()
    if pre_health < 0.7:
        logger.warning(f"Resilience health low: {pre_health}")
        ***REMOVED*** Could warn user or require approval

    ***REMOVED*** Generate with resilience constraints
    ...
    ***REMOVED*** Post-check: Did generation maintain/improve resilience?
    ...
    if post_health < pre_health - 0.05:
        ...
        ***REMOVED*** Could trigger mitigations

    return result
```

***REMOVED******REMOVED******REMOVED*** Integration with MCP Tools
...
***REMOVED******REMOVED*** Best Practices & Pitfalls

***REMOVED******REMOVED******REMOVED*** Best Practice 1: Always Backup Before Write

```python
***REMOVED*** ALWAYS do this first
backup = create_database_backup()
assert backup is not None, "Cannot proceed without backup"

***REMOVED*** Then generate
result = engine.generate()

***REMOVED*** Only persist if validation passes
if result.validation.valid:
    persist_schedule(result)
else:
    ***REMOVED*** Rollback to backup if needed
    rollback_to_backup(backup)
```

***REMOVED******REMOVED******REMOVED*** Best Practice 2: Validate Early, Fail Fast

```python
***REMOVED*** Check feasibility BEFORE running expensive solver
...
***REMOVED*** Only proceed if feasibility looks good
***REMOVED*** This prevents wasting solver time on impossible problems
```

***REMOVED******REMOVED******REMOVED*** Best Practice 3: Use Warmstart for Speed

```python
***REMOVED*** Never start cold when possible

***REMOVED*** Option 1: Use greedy output as warmstart
...
***REMOVED*** Result: 5-10× faster than no warmstart

***REMOVED*** Option 2: Use previous schedule as starting point
prev_schedule = db.query(Assignment).filter(...).all()
result = engine.generate(warmstart=prev_schedule)
```

***REMOVED******REMOVED******REMOVED*** Best Practice 4: Monitor Solver Progress
...
***REMOVED******REMOVED******REMOVED*** Best Practice 5: Test Edge Cases

```python
***REMOVED*** Test cases that ALWAYS need coverage

def test_acgme_80_hour_boundary():
    """Test 80-hour rule at exact boundary (53 blocks in 28 days)"""
    ***REMOVED*** Create resident with exactly 53 blocks in window
    ***REMOVED*** Verify ACGME validator accepts it
    ***REMOVED*** Verify 54 blocks triggers violation

def test_1_in_7_boundary():
    """Test 1-in-7 at exact boundary (6 consecutive days)"""
    ***REMOVED*** Create resident with 6 consecutive assigned days
    ***REMOVED*** Verify ACGME validator accepts it
    ***REMOVED*** Verify 7 consecutive days triggers violation

def test_nf_pc_transition():
    """Test Night Float to Post-Call transition"""
    ***REMOVED*** Create proper NF→PC sequence
    ***REMOVED*** Verify audit passes
    ***REMOVED*** Test improper transitions (no rest, etc.)

def test_supervision_ratio_exact():
    """Test supervision ratios at exact limits"""
    ***REMOVED*** Create block with exactly 2 PGY-1s and 1 faculty
    ***REMOVED*** Verify accepts
    ***REMOVED*** Test 3 PGY-1s and 1 faculty (violates 1:2 ratio)
```

***REMOVED******REMOVED******REMOVED*** Common Pitfall 1: Ignoring Availability Matrix
...
```python
***REMOVED*** WRONG: Ignores absences
assignments = solver.solve(context)  ***REMOVED*** May assign blocked residents

***REMOVED*** RIGHT: Build availability matrix first
availability = build_availability_matrix(
    residents=context.residents,
    absences=db.query(Absence).all()
)
context.availability = availability  ***REMOVED*** Now solver respects absences

assignments = solver.solve(context)  ***REMOVED*** Safer
```

***REMOVED******REMOVED******REMOVED*** Common Pitfall 2: Underestimating Complexity
...
```python
***REMOVED*** WRONG: Assumes complexity < 50
result = run_solver(context, solver='pulp', timeout=5)
***REMOVED*** Result: Timeout, partial schedule, complaints

***REMOVED*** RIGHT: Estimate first
complexity = estimate_complexity(context)
if complexity > 50:
    solver = 'cp_sat'  ***REMOVED*** or 'hybrid'
    timeout = 60
else:
    solver = 'pulp'
    timeout = 5
...
***REMOVED******REMOVED******REMOVED*** Common Pitfall 3: Forgetting Audit Trail
...
```python
***REMOVED*** WRONG: No audit record
assignments = engine.generate()
save_assignments(assignments)

***REMOVED*** RIGHT: Create schedule run record
run = ScheduleRun(
    algorithm='cp_sat',
...
db.session.add(run)
db.session.flush()  ***REMOVED*** Get run.id
...
***REMOVED******REMOVED*** Operational Playbooks

***REMOVED******REMOVED******REMOVED*** Playbook 1: Emergency Coverage Response
...
***REMOVED******REMOVED******REMOVED*** Playbook 2: Detecting & Responding to Infeasibility
...
***REMOVED******REMOVED******REMOVED*** Playbook 3: Monitoring Schedule Quality Post-Generation
...
***REMOVED******REMOVED*** Summary & Quick Reference

***REMOVED******REMOVED******REMOVED*** The 5 Core Constraint Types
...
***REMOVED******REMOVED******REMOVED*** The 4 Solvers
...
***REMOVED******REMOVED******REMOVED*** The 3 Resilience Tiers
...
***REMOVED******REMOVED******REMOVED*** Pre-Generation Checklist
...
***REMOVED******REMOVED******REMOVED*** Post-Generation Checklist
...
***REMOVED******REMOVED*** References & Further Reading
