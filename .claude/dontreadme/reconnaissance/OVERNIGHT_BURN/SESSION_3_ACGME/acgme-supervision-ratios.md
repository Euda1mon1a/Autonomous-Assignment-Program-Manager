***REMOVED*** ACGME Supervision Ratio Requirements
***REMOVED******REMOVED*** Comprehensive Enforcement & Audit Documentation
...
1. [Ratio Requirements by PGY Level](***REMOVED***ratio-requirements-by-pgy-level)
2. [Calculation Algorithm](***REMOVED***calculation-algorithm)
3. [Exception Handling](***REMOVED***exception-handling)
4. [Audit Procedures](***REMOVED***audit-procedures)
5. [Implementation Details](***REMOVED***implementation-details)
6. [Violation Detection & Reporting](***REMOVED***violation-detection--reporting)
7. [Edge Cases & Gotchas](***REMOVED***edge-cases--gotchas)

---

***REMOVED******REMOVED*** Ratio Requirements by PGY Level

***REMOVED******REMOVED******REMOVED*** Primary Ratios
...
***REMOVED******REMOVED******REMOVED*** ACGME Authority
...
***REMOVED******REMOVED******REMOVED*** Clinical Rationale
...
***REMOVED******REMOVED*** Calculation Algorithm

***REMOVED******REMOVED******REMOVED*** Step 1: Count Residents by PGY Level per Block
...
***REMOVED******REMOVED******REMOVED*** Step 2: Calculate Required Faculty

The system uses a **fractional load approach** to avoid overcounting in mixed PGY scenarios:

***REMOVED******REMOVED******REMOVED******REMOVED*** Formula A: Fractional Load (Recommended)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Formula B: Separate Ceiling (Legacy/Alternative)
...
def calculate_required_faculty_separate(pgy1_count: int, other_count: int) -> int:
    ...
    """
    from_pgy1 = (pgy1_count + 1) // 2  ***REMOVED*** Ceiling division
    from_other = (other_count + 3) // 4  ***REMOVED*** Ceiling division
...
***REMOVED******REMOVED******REMOVED*** Critical Case: Mixed PGY Levels
...
***REMOVED******REMOVED******REMOVED*** Scenarios Requiring Special Treatment

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Zero Residents per Block
...
if pgy1_count == 0 and other_count == 0:
    return 0  ***REMOVED*** No supervision needed
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. No Faculty Available
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Partial Supervision Coverage
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Excess Faculty (Over-staffing)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Faculty Unable to Supervise (PGY-2/3 as Supervising Faculty)
...
***REMOVED******REMOVED*** Audit Procedures

***REMOVED******REMOVED******REMOVED*** 1. Per-Block Audit
...
***REMOVED******REMOVED******REMOVED*** 2. Period-Wide Supervision Audit
...
def audit_period_supervision(
    ...
    Returns:
        {
            ...
            'compliance_rate': float,  ***REMOVED*** % (0-100)
            'violations': [
                {
                    'block_id': UUID,
                    'block_date': date,
    ...
    """
    ***REMOVED*** 1. Get all assignments in period
...
***REMOVED******REMOVED******REMOVED*** 3. Violation Severity Classification
...
***REMOVED******REMOVED******REMOVED*** 4. Audit Reporting Format
...
```python
{
    'total_blocks': int,                    ***REMOVED*** Total blocks analyzed
    'blocks_with_violations': int,          ***REMOVED*** Blocks with inadequate supervision
    'supervision_violations': [             ***REMOVED*** Detailed violation list
        {
            'block_id': str,
            'block_date': str,              ***REMOVED*** ISO format
            'residents': int,               ***REMOVED*** Total residents
            'pgy1_count': int,              ***REMOVED*** PGY-1 residents
            'faculty': int,                 ***REMOVED*** Actual faculty
            'required_faculty': int,        ***REMOVED*** Minimum required
            'deficit': int,                 ***REMOVED*** Required - Actual
        }
    ],
    'compliance_rate': float,               ***REMOVED*** Percentage (0-100)
}
```

***REMOVED******REMOVED******REMOVED*** 5. Automated Audit Triggers
...
***REMOVED******REMOVED******REMOVED*** Constraint Registration
...
class ACGMEConstraintValidator:
    def __init__(self):
        self.constraints = [
            ...
            SupervisionRatioConstraint(),  ***REMOVED*** ← Registered here
        ]
```

***REMOVED******REMOVED******REMOVED*** Validation Entry Points

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Post-Generation Validation
```python
***REMOVED*** After schedule generation completes
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Assignment Validation
```python
***REMOVED*** Before committing new assignments
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Swap Validation
```python
***REMOVED*** After swap execution
...
***REMOVED******REMOVED*** Violation Detection & Reporting

***REMOVED******REMOVED******REMOVED*** Violation Structure

```python
@dataclass
class ConstraintViolation:
    constraint_name: str        ***REMOVED*** "SupervisionRatio"
    constraint_type: str        ***REMOVED*** "supervision"
    severity: str               ***REMOVED*** "CRITICAL"
    message: str                ***REMOVED*** "Block needs 2 faculty but has 1"
    block_id: UUID | None       ***REMOVED*** Block where violation occurs
    details: dict = {           ***REMOVED*** Rich context
        'residents': 2,
        'pgy1_count': 2,
        'faculty': 1,
        'required': 2,
...
***REMOVED******REMOVED******REMOVED*** Violation Message Examples
...
***REMOVED******REMOVED******REMOVED*** Reporting Integration
...
**MCP Tools Output:**
```python
***REMOVED*** From scheduler_mcp/resources.py
{
...
***REMOVED******REMOVED*** Edge Cases & Gotchas

***REMOVED******REMOVED******REMOVED*** 1. Faculty Type Ambiguity
...
***REMOVED******REMOVED******REMOVED*** 2. Partial Blocks (Multiple Sessions)
...
***REMOVED******REMOVED******REMOVED*** 3. Shift Boundaries
...
***REMOVED******REMOVED******REMOVED*** 4. Off-Service Rotations
...
***REMOVED******REMOVED******REMOVED*** 5. Zero-Resident Blocks
...
```python
if not residents:
    continue  ***REMOVED*** No supervision required for faculty-only blocks
```

***REMOVED******REMOVED******REMOVED*** 6. Rounding & Ceiling Arithmetic
...
***REMOVED******REMOVED******REMOVED*** 7. PGY Level == None
...
class TestSupervisionRatioConstraint:
    ...
    def test_calculate_required_faculty_mixed():
        ...
        ***REMOVED*** 1 PGY-1 + 1 PGY-2 = 0.5 + 0.25 = 0.75 → ceil = 1
        assert constraint.calculate_required_faculty(1, 1) == 1

        ***REMOVED*** 3 PGY-1 + 4 PGY-2/3 = 6 + 4 = 10 units → 10/4 = 2.5 → 3
    ...
    def test_validate_adequate_supervision():
        ...
        ***REMOVED*** Block with 1 PGY-1 + 1 faculty → compliant
        assignments = [
...
***REMOVED******REMOVED*** Enforcement Timeline
...
***REMOVED******REMOVED*** Regulatory Alignment

***REMOVED******REMOVED******REMOVED*** ACGME CPR Reference
...
***REMOVED******REMOVED******REMOVED*** Mapping to Implementation
...
***REMOVED******REMOVED******REMOVED*** Constants
```python
...
***REMOVED******REMOVED******REMOVED*** Calculation Formula (Single Line)
```python
required = (pgy1_count * 2 + other_count + 3) // 4 if (pgy1_count + other_count) > 0 else 0
```

***REMOVED******REMOVED******REMOVED*** Violation Threshold
```python
if actual_faculty < required_faculty:
    ***REMOVED*** VIOLATION DETECTED
```

***REMOVED******REMOVED******REMOVED*** Audit Query (SQL)
```sql
SELECT
    block_id,
