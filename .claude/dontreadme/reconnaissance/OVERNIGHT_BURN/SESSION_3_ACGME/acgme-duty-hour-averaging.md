***REMOVED*** ACGME Duty Hour Averaging - Deep Reconnaissance
...
2. [Averaging Period Architecture](***REMOVED***averaging-period-architecture)
3. [Calculation Algorithms](***REMOVED***calculation-algorithms)
4. [Edge Case Analysis](***REMOVED***edge-case-analysis)
5. [Violation Detection Logic](***REMOVED***violation-detection-logic)
6. [Code Implementation Map](***REMOVED***code-implementation-map)
7. [10-Point Reconnaissance Findings](***REMOVED***10-point-reconnaissance-findings)
...
***REMOVED******REMOVED*** Averaging Period Architecture

***REMOVED******REMOVED******REMOVED*** 1. ACGME Regulatory Definition
...
***REMOVED******REMOVED******REMOVED*** 2. Calendar Implementation
...
***REMOVED******REMOVED******REMOVED*** 3. Window Calculation Formula

```
window_end = window_start + timedelta(days=27)  ***REMOVED*** NOT 28 days offset!
***REMOVED*** Rationale: If start = Jan 1, +27 days = Jan 28 (28 days total: Jan 1-28 inclusive)
...
***REMOVED******REMOVED******REMOVED*** 4. Rolling Average Calculation
...
def calculate_rolling_average(hours_by_date: dict, window_start: date) -> float:
    ...
    total_hours = sum(
        ...
        if window_start <= d <= window_end  ***REMOVED*** Inclusive boundaries
    )

    return total_hours / 4  ***REMOVED*** 4 weeks in averaging window
```
...
***REMOVED******REMOVED*** Calculation Algorithms

***REMOVED******REMOVED******REMOVED*** Algorithm 1: Block-Based Counting (Solver Constraint)

Used during schedule generation (CP-SAT/PuLP models).

```python
***REMOVED*** For each 28-day window and each resident
for window_start in all_possible_dates:
    window_end = window_start + timedelta(days=27)
    window_blocks = [b for b in blocks if window_start <= b.date <= window_end]

    for resident in residents:
        ***REMOVED*** Sum of assigned blocks in window must be <= max_blocks
        block_sum = sum(x[resident_i, block_i] for block_i in window_blocks)
        model.Add(block_sum <= 53)  ***REMOVED*** Hard constraint
```

**Assumption:** Each assigned block = 6 hours
**Result:** Blocks ≤ 53 means hours ≤ 318 (well below 320-hour target)

***REMOVED******REMOVED******REMOVED*** Algorithm 2: Hour-Based Validation (Post-Generation)
...
def validate_80_hour_rule(assignments, context):
    ...
    ***REMOVED*** Step 1: Group assignments by resident
    by_resident = defaultdict(list)
    for assignment in assignments:
        by_resident[assignment.person_id].append(assignment)

    ***REMOVED*** Step 2: For each resident, calculate hours per date
    for resident in context.residents:
        ...
        for assignment in resident_assignments:
            ...
            hours_by_date[block.date] += 6  ***REMOVED*** HOURS_PER_BLOCK = 6

        ***REMOVED*** Step 3: Check EVERY possible 28-day window
        sorted_dates = sorted(hours_by_date.keys())
        for window_start_date in sorted_dates:  ***REMOVED*** <-- KEY: ALL windows
            window_end_date = window_start_date + timedelta(days=27)

            ***REMOVED*** Sum hours in window
            total_hours = sum(
                hours for date, hours in hours_by_date.items()
                if window_start_date <= date <= window_end_date
            )

            ***REMOVED*** Calculate weekly average
            avg_weekly = total_hours / 4

            ***REMOVED*** Check violation
            if avg_weekly > 80.0:
                violations.append({
                ...
                break  ***REMOVED*** Report first violation per resident

    return violations
```

***REMOVED******REMOVED******REMOVED*** Algorithm 3: Daily Roll-Forward (Incremental)
...
def check_80_hour_compliance_incremental(resident_id, check_date):
    ...
    ***REMOVED*** Find all 28-day windows that include check_date
    ***REMOVED*** Window must satisfy: window_start <= check_date <= (window_start + 27)

    ***REMOVED*** Earliest window start
    earliest_window_start = check_date - timedelta(days=27)

    ***REMOVED*** Latest window start (check_date is first day of window)
    latest_window_start = check_date

    ***REMOVED*** Check all windows in range
    for window_start in range(earliest_window_start, latest_window_start + 1):
        ...
        if current_hours + 6 > 320:  ***REMOVED*** Adding 1 block would exceed
            return False
...
***REMOVED******REMOVED*** Edge Case Analysis

***REMOVED******REMOVED******REMOVED*** Edge Case 1: Exactly 80.0 Hours Per Week
...
def test_80_hour_rule_exactly_80_compliant():
    ...
    hours_by_date = {
        date1: 6, date2: 6, ..., date28: 6  ***REMOVED*** Sum = 320
    }
    avg = 320 / 4
    assert avg == 80.0
    assert avg <= 80.0  ***REMOVED*** PASS
```

***REMOVED******REMOVED******REMOVED*** Edge Case 2: Boundary at 28-Day Window
...
***REMOVED******REMOVED******REMOVED*** Edge Case 3: Uneven Assignment Across Window
...
***REMOVED******REMOVED******REMOVED*** Edge Case 4: Sparse Assignment (Month with 4 Days Off)
...
***REMOVED******REMOVED******REMOVED*** Edge Case 5: Leap Year February (29 days)
...
***REMOVED******REMOVED******REMOVED*** Edge Case 6: Window Starting on Unassigned Date

**Scenario:**
```python
assigned_dates = [Jan 5, Jan 6, ..., Jan 31]  ***REMOVED*** No assignments Jan 1-4

***REMOVED*** Validation checks ALL possible windows
***REMOVED*** Window starting Jan 1: includes Jan 1-28
***REMOVED***   - Days 1-4: 0 hours
***REMOVED***   - Days 5-28: X hours
***REMOVED***   - Total: X hours / 4 weeks
```
...
***REMOVED******REMOVED*** Violation Detection Logic

***REMOVED******REMOVED******REMOVED*** Violation Detection Algorithm

```python
***REMOVED*** File: backend/app/services/constraints/acgme.py
    ...
    def validate(self, assignments, context):
        ...
        ***REMOVED*** Group by resident
        ...
        for resident in context.residents:
            ...
            ***REMOVED*** Build hours_by_date
            hours_by_date = defaultdict(int)
            for a in resident_assignments:
                for b in context.blocks:
                    if b.id == a.block_id:
                        hours_by_date[b.date] += 6  ***REMOVED*** HOURS_PER_BLOCK
                        break

            if not hours_by_date:
                continue

            ***REMOVED*** Check EVERY window
            sorted_dates = sorted(hours_by_date.keys())
            for start_date in sorted_dates:  ***REMOVED*** <-- EVERY assigned date
                avg_weekly = self._calculate_rolling_average(
                    hours_by_date, start_date
                )

                if avg_weekly > 80.0:  ***REMOVED*** Strict >
                    ...
                    break  ***REMOVED*** First violation per resident
...
***REMOVED******REMOVED******REMOVED*** Violation Criteria
...
***REMOVED******REMOVED******REMOVED*** Violation Reporting

Each violation includes:

```python
{
    'window_start': date,           ***REMOVED*** First day of 28-day window
    'window_end': date,             ***REMOVED*** Last day (start + 27)
    'average_weekly_hours': float,  ***REMOVED*** Calculated average
    'max_weekly_hours': 80,         ***REMOVED*** Regulatory limit
    'violation_amount': float,      ***REMOVED*** How much over (e.g., 0.25 for 80.25)
}
```

---

***REMOVED******REMOVED*** Code Implementation Map
...
***REMOVED******REMOVED******REMOVED*** Constants Reference

```python
***REMOVED*** backend/app/services/constraints/acgme.py
HOURS_PER_BLOCK = 6          ***REMOVED*** Half-day AM/PM block
MAX_WEEKLY_HOURS = 80        ***REMOVED*** Regulatory limit
ROLLING_WEEKS = 4            ***REMOVED*** Window size in weeks
ROLLING_DAYS = 28            ***REMOVED*** Window size in days (28 = 4*7)
MAX_BLOCKS_PER_WINDOW = 53   ***REMOVED*** (80*4) / 6 = 53.33 -> 53
```

***REMOVED******REMOVED******REMOVED*** Window Calculation Code Snippets

```python
***REMOVED*** Correct window calculation
window_end = window_start + timedelta(days=27)  ***REMOVED*** 28 days total
window_blocks = [
    b for b in context.blocks
    if window_start <= b.date <= window_end  ***REMOVED*** Inclusive both ends
]

***REMOVED*** Hours aggregation
total_hours = sum(
    hours
    for d, hours in hours_by_date.items()
    if window_start <= d <= window_end
)
weekly_avg = total_hours / 4  ***REMOVED*** Divide by 4 weeks
```

---

***REMOVED******REMOVED*** 10-Point Reconnaissance Findings

***REMOVED******REMOVED******REMOVED*** 1. PERCEPTION: Current Averaging Windows in Code
...
***REMOVED******REMOVED******REMOVED*** 2. INVESTIGATION: Rolling Calculation Logic

**Finding:** Every possible 28-day window is checked independently.

```python
***REMOVED*** Correct: Check ALL windows
for start_date in sorted(hours_by_date.keys()):
    avg = calculate_rolling_average(hours_by_date, start_date)
    if avg > 80:
        report_violation()

***REMOVED*** Incorrect (if implemented this way): Only check fixed 4-week blocks
for block_start in [day 0, day 28, day 56, ...]:  ***REMOVED*** WRONG!
...
***REMOVED******REMOVED******REMOVED*** 3. ARCANA: 4-Week Averaging Requirements
...
***REMOVED******REMOVED******REMOVED*** 4. HISTORY: Averaging Rule Changes
...
***REMOVED******REMOVED******REMOVED*** 5. INSIGHT: Why Rolling vs Fixed?
...
***REMOVED******REMOVED******REMOVED*** 6. RELIGION: Are Calculations Accurate?
...
***REMOVED******REMOVED******REMOVED*** 7. NATURE: Over-Complicated Averaging?
...
***REMOVED*** Better: O(N) with sliding window
...
***REMOVED******REMOVED******REMOVED*** 8. MEDICINE: Burnout Prevention
...
***REMOVED******REMOVED******REMOVED*** 9. SURVIVAL: Edge of Compliance Handling
...
***REMOVED******REMOVED******REMOVED*** 10. STEALTH: Averaging Manipulation
