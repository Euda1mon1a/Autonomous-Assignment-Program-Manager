***REMOVED*** ACGME Program Evaluation - Quick Reference

**Quick Navigation for Program Directors & Coordinators**

---

***REMOVED******REMOVED*** Essential Metrics At A Glance
...
***REMOVED******REMOVED*** Key System Locations

***REMOVED******REMOVED******REMOVED*** Compliance Reporting
...
***REMOVED******REMOVED******REMOVED*** Real-Time Dashboards
...
***REMOVED******REMOVED******REMOVED*** Violation Tracking
...
***REMOVED******REMOVED******REMOVED*** Monthly Compliance Review
```bash
***REMOVED*** Generate monthly compliance report
curl -X GET "http://localhost:8000/api/v1/analytics/compliance" \
  -H "Authorization: Bearer TOKEN"

***REMOVED*** Or via Python
...
***REMOVED******REMOVED******REMOVED*** Investigate a Violation
```
...
***REMOVED******REMOVED******REMOVED*** Prepare for Site Visit
```
30 days before:
...
***REMOVED******REMOVED******REMOVED*** Respond to a Citation
```
Within 24 hours:
...
***REMOVED******REMOVED*** Key Reports & Where to Find Them
...
***REMOVED******REMOVED*** Alert Escalation

***REMOVED******REMOVED******REMOVED*** What Triggers Alerts
...
***REMOVED******REMOVED******REMOVED*** Responding to Alerts
...
***REMOVED******REMOVED*** Useful Queries

***REMOVED******REMOVED******REMOVED*** Most Recent Violations
```bash
***REMOVED*** Via API
GET /api/v1/analytics/compliance?violations_only=true

***REMOVED*** Via SQL (if available)
SELECT * FROM violations
ORDER BY detected_date DESC
LIMIT 10;
```

***REMOVED******REMOVED******REMOVED*** Resident with Most Violations
```python
...
***REMOVED******REMOVED******REMOVED*** Blocks Without Supervision Coverage
```python
from app.models.assignment import Assignment
from app.models.block import Block

***REMOVED*** Find blocks with insufficient faculty
blocks_with_issues = identify_supervision_gaps(db)
```

---

***REMOVED******REMOVED*** Dashboard Quick Tips

***REMOVED******REMOVED******REMOVED*** Accessing Dashboards
1. Open browser → `http://localhost:3000`
2. Login (default: admin/admin, change in production)
3. Select dashboard from list

***REMOVED******REMOVED******REMOVED*** Setting Up Alerts
...
***REMOVED******REMOVED******REMOVED*** Exporting Data
...
***REMOVED******REMOVED******REMOVED*** "No data showing in dashboard"
- Check date range (top left)
- Verify metrics are being collected (`/metrics/health`)
- Check Prometheus connection (`localhost:9090`)

***REMOVED******REMOVED******REMOVED*** "Compliance report shows zero violations but PD says there are some"
...
***REMOVED******REMOVED******REMOVED*** "Violation detected but alert not sent"
- Check alert notification channel configuration
- Verify email/Slack credentials
- Check alert rules are enabled
- Review alert logs

***REMOVED******REMOVED******REMOVED*** "Fairness metrics seem wrong"
