***REMOVED*** ACGME Leave and Absence Policies - Comprehensive Audit
...
***REMOVED******REMOVED*** I. PERCEPTION: Current Leave Types in Models/Schemas

***REMOVED******REMOVED******REMOVED*** A. Leave Type Inventory (11 Types)
...
***REMOVED******REMOVED******REMOVED*** B. Blocking Behavior Logic

The system implements smart blocking that adapts to duration:

```python
***REMOVED*** From app/models/absence.py
ABSENCE_TYPES = {
    "vacation": False,                 ***REMOVED*** Non-blocking (tracked but allows assignment)
    "conference": False,               ***REMOVED*** Non-blocking (educational, not blocking)
    "medical": None,                   ***REMOVED*** Duration-based: >7 days blocks
    "sick": None,                      ***REMOVED*** Duration-based: >3 days blocks
    "deployment": True,                ***REMOVED*** Always blocking
    "tdy": True,                       ***REMOVED*** Always blocking
    "family_emergency": True,          ***REMOVED*** Always blocking (Hawaii reality: mainland travel)
    "bereavement": True,               ***REMOVED*** Always blocking (Hawaii reality: mainland travel)
    "emergency_leave": True,           ***REMOVED*** Always blocking
    "maternity_paternity": True,       ***REMOVED*** Always blocking
    "convalescent": True,              ***REMOVED*** Always blocking
}

***REMOVED*** Auto-determination via should_block_assignment property
...
***REMOVED******REMOVED******REMOVED*** C. Data Model Fields
...
***REMOVED******REMOVED******REMOVED*** D. Duration Calculation
...
***REMOVED******REMOVED*** II. INVESTIGATION: Leave → Schedule Impact

***REMOVED******REMOVED******REMOVED*** A. Availability Constraint Integration
...
***REMOVED******REMOVED******REMOVED*** B. Availability Matrix Construction

The system pre-computes availability for all (person, block) pairs:

```python
***REMOVED*** From context.availability dict
availability = {
    person_id: {
...
***REMOVED******REMOVED******REMOVED*** C. Leave Impact on Work Hour Compliance
...
***REMOVED******REMOVED******REMOVED*** D. Non-Blocking Leave Semantics
...
***REMOVED******REMOVED*** III. ARCANA: Educational vs Medical Leave

***REMOVED******REMOVED******REMOVED*** A. Educational Leave Types
...
***REMOVED******REMOVED******REMOVED*** B. Medical Leave Types
...
***REMOVED******REMOVED******REMOVED*** C. Leave Allocation Models
...
***REMOVED******REMOVED*** IV. HISTORY: Leave Policy Evolution

***REMOVED******REMOVED******REMOVED*** A. Hawaii-Specific Adaptations
...
***REMOVED******REMOVED******REMOVED*** B. System Design Patterns
...
***REMOVED******REMOVED*** V. INSIGHT: Resident Wellness Balance

***REMOVED******REMOVED******REMOVED*** A. Days-Off Accounting
...
***REMOVED******REMOVED******REMOVED*** B. Wellness Workflow

**Admin Quick-Block Workflow (Emergency Situations):**

```python
***REMOVED*** From app/schemas/absence.py
class AbsenceCreate(AbsenceBase):
    """For admin quick-block workflow (emergency absences)."""

    is_blocking: bool | None = None        ***REMOVED*** Auto-determined if not set
    return_date_tentative: bool = False    ***REMOVED*** Flag for follow-up
    created_by_id: UUID | None = None      ***REMOVED*** Admin who entered absence
```
...
***REMOVED******REMOVED******REMOVED*** C. Recovery Tracking
...
***REMOVED******REMOVED*** VI. RELIGION: Leave Properly Counted?

***REMOVED******REMOVED******REMOVED*** A. Coverage Requirements Integration

**Schedule Generation:** Absence blocking affects coverage capacity

```python
***REMOVED*** From backend/app/scheduling/engine.py
***REMOVED*** Availability matrix passed to solver
availability_matrix = {
    (resident_id, block_id): True/False  ***REMOVED*** Can resident work block?
}

***REMOVED*** Solver uses availability as hard constraint
***REMOVED*** Result: Only unblocked (resident, block) pairs available for assignment
```
...
***REMOVED******REMOVED******REMOVED*** B. Compliance Tracking Implementation
...
***REMOVED******REMOVED******REMOVED*** C. Audit Trail

**Versioning Enabled:** `__versioned__ = {}`  (SQLAlchemy-Continuum)

```python
***REMOVED*** Access history
absence.versions  ***REMOVED*** List of all changes
...
***REMOVED******REMOVED*** VII. NATURE: Over-Complicated Leave Types?

***REMOVED******REMOVED******REMOVED*** A. Redundancy Analysis
...
***REMOVED******REMOVED******REMOVED*** B. Simplification Opportunities

**Consolidation Option 1: Reduce to 8 Types**
```python
***REMOVED*** Proposed consolidation
ABSENCE_TYPES = {
    "vacation": False,
    "conference": False,
    "medical": None,              ***REMOVED*** Combine sick + medical
    "deployment": True,           ***REMOVED*** Combine TDY here with detail field
    "bereavement": True,
    "emergency": True,            ***REMOVED*** Combine family_emergency + emergency_leave
    "maternity_paternity": True,
    "convalescent": True,         ***REMOVED*** Keep separate (post-surgery specific)
}
```

**Consolidation Option 2: Keep 11, Add Metadata**
```python
***REMOVED*** Current design is actually optimal for:
***REMOVED*** - Hawaii military context (deployment/TDY distinction)
***REMOVED*** - Medical distinctions (sick < 3d, medical > 7d, convalescent post-surgery)
***REMOVED*** - Emergency handling (bereavement + family_emergency + emergency_leave)
```
...
***REMOVED******REMOVED*** VIII. MEDICINE: Recovery Leave Adequacy?

***REMOVED******REMOVED******REMOVED*** A. Post-Surgical Recovery Times
...
***REMOVED******REMOVED******REMOVED*** B. Extended Medical Leave (Chronic Conditions)
...
***REMOVED******REMOVED******REMOVED*** C. Return-to-Work Integration

**Current Workflow:**
```python
***REMOVED*** Flag for uncertain return date
return_date_tentative: bool

***REMOVED*** Coordinator follow-up
***REMOVED*** 1. Absence entered with tentative end_date
***REMOVED*** 2. Flag: return_date_tentative = True
***REMOVED*** 3. Medical clearance received
***REMOVED*** 4. Coordinator updates end_date
***REMOVED*** 5. System regenerates availability matrix
***REMOVED*** 6. Scheduler updates assignments automatically
```
...
***REMOVED******REMOVED*** IX. SURVIVAL: Emergency Leave Handling

***REMOVED******REMOVED******REMOVED*** A. Emergency Absence Types
...
***REMOVED******REMOVED******REMOVED*** B. Quick-Block Workflow (Emergency Scenario)

**Situation:** Resident's parent hospitalized in California

**Workflow:**
```python
***REMOVED*** Step 1: Coordinator enters absence immediately
absence = Absence(
    person_id=resident_id,
    start_date=date.today(),           ***REMOVED*** Leave immediately
    end_date=date.today() + timedelta(days=7),  ***REMOVED*** Estimate 1 week
    absence_type="family_emergency",
    replacement_activity="Emergency leave - mainland",
    return_date_tentative=True,        ***REMOVED*** Flag for follow-up
    created_by_id=coordinator_id,
    notes="Parent hospitalized in San Francisco. Return date TBD.",
)

***REMOVED*** Step 2: System blocks all blocks in period
***REMOVED*** availability[resident_id][block_id] = False for all blocks in 7-day window

***REMOVED*** Step 3: Scheduler runs conflict detection
***REMOVED*** Flag: "Resident coverage gaps detected for Jan 15-21"
***REMOVED*** Coordinator must manually reassign to backup residents or use locum tenens

***REMOVED*** Step 4: Resident returns (actual date)
***REMOVED*** Coordinator updates: absence.end_date = actual_return_date
***REMOVED*** System recalculates availability
***REMOVED*** Next schedule generation: resident re-included in assignments

***REMOVED*** Step 5: Audit trail
***REMOVED*** absence.versions shows:
***REMOVED*** - Created: Jan 14, 2025 by Coordinator
***REMOVED*** - Updated: Jan 20, 2025 - end_date changed from Jan 21 to Jan 22
```

***REMOVED******REMOVED******REMOVED*** C. Emergency Leave Adequacy
...
***REMOVED******REMOVED*** X. STEALTH: Untracked Leave Days?

***REMOVED******REMOVED******REMOVED*** A. Tracking Gaps
...
***REMOVED******REMOVED******REMOVED*** B. Hidden Leave (Not Entered in System)
...
***REMOVED******REMOVED******REMOVED*** C. Detection Mechanism
...
def validate_absence_timing(absence: Absence):
    ...
    if days_since_event > 14:  ***REMOVED*** More than 2 weeks after
        return {
            "warning": "Late entry",
...
***REMOVED******REMOVED*** XI. COMPLIANCE TRACKING SUMMARY

***REMOVED******REMOVED******REMOVED*** A. What System Tracks
...
***REMOVED******REMOVED******REMOVED*** B. Coverage Requirements by Absence Type
...
***REMOVED******REMOVED******REMOVED*** C. ACGME Compliance Enforcement
...
***REMOVED******REMOVED*** XII. SCHEMA AND API SUMMARY

***REMOVED******REMOVED******REMOVED*** A. Pydantic Schemas
...
***REMOVED******REMOVED******REMOVED*** B. API Endpoints
...
***REMOVED******REMOVED******REMOVED*** C. Service Layer
...
***REMOVED******REMOVED*** XIII. TESTING COVERAGE

***REMOVED******REMOVED******REMOVED*** A. Test Files
...
***REMOVED******REMOVED******REMOVED*** B. Coverage Gaps
...
***REMOVED******REMOVED*** XIV. RECOMMENDATIONS AND NEXT STEPS

***REMOVED******REMOVED******REMOVED*** A. High Priority (ACGME Compliance)
...
***REMOVED******REMOVED******REMOVED*** B. Medium Priority (Operational)
...
***REMOVED******REMOVED******REMOVED*** C. Low Priority (Enhancement)
