***REMOVED*** Backend Logging and Observability Patterns Audit
...
***REMOVED******REMOVED*** 1. Logging Configuration Audit

***REMOVED******REMOVED******REMOVED*** 1.1 Core Logging Setup (backend/app/core/logging.py)
...
***REMOVED*** Handler Configuration (Lines 186-217)
...
***REMOVED******REMOVED******REMOVED*** 1.2 Environment Configuration (backend/app/core/config.py)

**Logging Configuration Variables:**

```python
LOG_LEVEL: str = "INFO"           ***REMOVED*** Default minimum level
LOG_FORMAT: str = "text"          ***REMOVED*** "json" for production, "text" for development
LOG_FILE: str = ""                ***REMOVED*** Optional file path (empty = stderr only)
```
...
***REMOVED******REMOVED******REMOVED*** 1.3 Framework Logging Suppression (Lines 223-227)
...
***REMOVED******REMOVED*** 2. PII/Sensitive Data Protection Audit

***REMOVED******REMOVED******REMOVED*** 2.1 Sensitive Data Filter (backend/app/middleware/logging/filters.py)

**Design Pattern:** Multi-strategy filtering with field-based and pattern-based detection

```python
***REMOVED*** Field-Based Sensitivity (Lines 23-51)
DEFAULT_SENSITIVE_FIELDS = {
    ...
    ***REMOVED*** ... 15 fields total
}

***REMOVED*** Pattern-Based Detection (Lines 54-71)
SENSITIVE_PATTERNS = [
    (r"\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}\b", "[REDACTED-CARD]"),  ***REMOVED*** Credit cards
    (r"\b\d{3}-\d{2}-\d{4}\b", "[REDACTED-SSN]"),                        ***REMOVED*** SSN
    (r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}", "[REDACTED-EMAIL]"),  ***REMOVED*** Email
    (r"Bearer\s+[A-Za-z0-9\-._~+/]+=*", "Bearer [REDACTED]"),            ***REMOVED*** Tokens
    (r"\b[a-f0-9]{32,}\b", "[REDACTED-KEY]"),                             ***REMOVED*** API keys
]
```
...
***REMOVED******REMOVED******REMOVED*** 2.2 Data Sanitization Rules (backend/app/core/logging/sanitizers.py)
...
***REMOVED******REMOVED******REMOVED*** 2.3 Privacy Maskers (backend/app/privacy/maskers.py)
...
***REMOVED******REMOVED******REMOVED*** 2.4 Path-Based Body Logging Control (Lines 240-265)
...
***REMOVED******REMOVED*** 3. Structured Logging Implementation

***REMOVED******REMOVED******REMOVED*** 3.1 Request/Response Middleware (backend/app/middleware/logging/)
...
class RequestLoggingConfig:
    ...
    log_body: bool = True
    max_body_size: int = 10 * 1024          ***REMOVED*** 10 KB limit
    log_response: bool = True
    max_response_size: int = 10 * 1024      ***REMOVED*** 10 KB limit
    excluded_paths: set[str] = {"/health", "/metrics", "/docs", "/openapi.json"}
    sample_rate: float = 1.0                ***REMOVED*** Log all requests
    log_levels: dict[str, str] = {}         ***REMOVED*** Per-path custom levels
    storage_backend: LogStorage             ***REMOVED*** Pluggable storage
```
...
```python
{
    "type": "request",
    "timestamp": time.time(),
    "request_id": request_id,               ***REMOVED*** Correlation ID
    "method": request.method,
    "path": request.url.path,
    "query_params": dict(request.query_params),
    "ip": request.client.host,
    "user_agent": request.headers.get("user-agent"),
    "user_id": user_id,                     ***REMOVED*** If authenticated
    "headers": { ... },                     ***REMOVED*** Filtered for sensitive data
    "body": { ... },                        ***REMOVED*** Masked, size-limited
}
```
...
```python
{
    ...
    "duration_ms": float,
    "user_id": user_id,
    "response_headers": { ... },            ***REMOVED*** Filtered
}
```

***REMOVED******REMOVED******REMOVED*** 3.2 Response Logger (backend/app/middleware/logging/response_logger.py)
...
***REMOVED******REMOVED******REMOVED*** 3.3 Observability Metrics (backend/app/core/observability.py)
...
***REMOVED******REMOVED*** 4. OpenTelemetry Integration Status

***REMOVED******REMOVED******REMOVED*** 4.1 Tracing Setup (backend/app/tracing/setup.py)
...
```python
TELEMETRY_ENABLED: bool = False                  ***REMOVED*** Disabled by default
...
TELEMETRY_SAMPLING_RATE: float = 1.0             ***REMOVED*** Trace sampling (0.0-1.0)
TELEMETRY_CONSOLE_EXPORT: bool = False           ***REMOVED*** Console exporter for debugging
...
TELEMETRY_EXPORTER_INSECURE: bool = True         ***REMOVED*** Use insecure (no TLS)
TELEMETRY_TRACE_SQLALCHEMY: bool = True          ***REMOVED*** Database query tracing
TELEMETRY_TRACE_REDIS: bool = True               ***REMOVED*** Cache operation tracing
TELEMETRY_TRACE_HTTP: bool = True                ***REMOVED*** External API tracing
```
...
***REMOVED******REMOVED******REMOVED*** 4.2 Tracing Middleware (backend/app/telemetry/middleware.py)

**Span Lifecycle:**

```python
***REMOVED*** Per HTTP Request:
...
***REMOVED******REMOVED******REMOVED*** 4.3 Database Tracing Helper
...
def add_query_attributes(span, query, params=None, table_name=None):
    ...
    span.set_attribute("db.statement", query[:1000])  ***REMOVED*** Limit length
    span.set_attribute("db.sql.table", table_name)

    ***REMOVED*** Sanitize parameters
    sanitized_params = {
...
***REMOVED******REMOVED******REMOVED*** 4.4 Graceful Degradation
...
***REMOVED******REMOVED*** 5. Logging Context Management

***REMOVED******REMOVED******REMOVED*** 5.1 Context Variables (backend/app/core/logging/context.py)

**ContextVar Tracking:**

```python
request_id_ctx: ContextVar[str | None]     ***REMOVED*** Request correlation
user_id_ctx: ContextVar[str | None]        ***REMOVED*** User identity
session_id_ctx: ContextVar[str | None]     ***REMOVED*** Session tracking
trace_id_ctx: ContextVar[str | None]       ***REMOVED*** Distributed trace ID
span_id_ctx: ContextVar[str | None]        ***REMOVED*** Current span ID
custom_ctx: ContextVar[dict]               ***REMOVED*** Custom fields
```
...
***REMOVED******REMOVED******REMOVED*** 5.2 Helper Functions
...
@with_log_context(**context_fields)  ***REMOVED*** Decorator
def my_function(): ...
```

---

***REMOVED******REMOVED*** 6. Debug Logging in Production

***REMOVED******REMOVED******REMOVED*** 6.1 Log Level Configuration
...
***REMOVED******REMOVED******REMOVED*** 6.2 Third-Party Library Suppression

**Intentional Noise Reduction (backend/app/core/logging.py, Lines 223-227):**

```python
***REMOVED*** These libraries produce verbose DEBUG/INFO logs
logging.getLogger("uvicorn.access").setLevel(logging.WARNING)
logging.getLogger("sqlalchemy.engine").setLevel(logging.WARNING)
logging.getLogger("httpx").setLevel(logging.WARNING)
logging.getLogger("httpcore").setLevel(logging.WARNING)
...
***REMOVED******REMOVED******REMOVED*** 6.3 No Production Debug Code Found
...
***REMOVED******REMOVED*** 7. Log Aggregation and Storage

***REMOVED******REMOVED******REMOVED*** 7.1 Log Storage Backend (backend/app/middleware/logging/storage.py)
...
***REMOVED******REMOVED******REMOVED*** 7.2 Log Retention Policy
...
logger.add(
    log_file,
    rotation="100 MB",              ***REMOVED*** Rotate at 100 MB
    retention="7 days",             ***REMOVED*** Keep 7 days of archives
    compression="gz",               ***REMOVED*** Compress rotated files
    serialize=format_type == "json" ***REMOVED*** JSON serialization
)
```
...
***REMOVED******REMOVED*** 8. Security Validation in Logging

***REMOVED******REMOVED******REMOVED*** 8.1 No Password/Token Logging

**Automatic Redaction:**

```python
***REMOVED*** Example from filter_dict() in SensitiveDataFilter:
if is_sensitive:
    if isinstance(value, str):
        filtered[key] = self._partial_mask(value)  ***REMOVED*** Show only ***
    else:
...
***REMOVED******REMOVED******REMOVED*** 8.2 No PII in Error Messages
...
***REMOVED******REMOVED******REMOVED*** 8.3 Request ID Validation
...
if request_id:
    ...
    if not request_id or len(request_id) > self.MAX_REQUEST_ID_LENGTH:
        request_id = None  ***REMOVED*** Generate new UUID
```
...
***REMOVED******REMOVED*** 9. Integration Points

***REMOVED******REMOVED******REMOVED*** 9.1 Application Startup (backend/app/main.py)
...
***REMOVED******REMOVED******REMOVED*** 9.2 Middleware Registration

**Typical FastAPI app setup:**

```python
app.add_middleware(RequestIDMiddleware)           ***REMOVED*** Generate/validate request IDs
app.add_middleware(RequestLoggingMiddleware)      ***REMOVED*** Log all requests/responses
app.add_middleware(TracingMiddleware)             ***REMOVED*** OpenTelemetry tracing
```

**Order Matters:** RequestIDMiddleware first to populate context

***REMOVED******REMOVED******REMOVED*** 9.3 Service-Level Logging
...
async def create_assignment(db, data):
    logger.info(
        "Creating assignment",
        person_id=person_id,      ***REMOVED*** Structured field
...
***REMOVED******REMOVED*** 10. Risk Assessment

***REMOVED******REMOVED******REMOVED*** 10.1 Identified Issues
...
***REMOVED******REMOVED******REMOVED*** 10.2 Best Practices Compliance
...
***REMOVED******REMOVED******REMOVED*** 11.1 Already Implemented (No Action Needed)
...
***REMOVED******REMOVED******REMOVED*** 11.2 Optional Enhancements
...
***REMOVED******REMOVED******REMOVED*** 11.3 Production Deployment Checklist
...
***REMOVED******REMOVED*** 12. File Inventory

***REMOVED******REMOVED******REMOVED*** Core Logging
...
***REMOVED******REMOVED******REMOVED*** Middleware & Filtering
...
***REMOVED******REMOVED******REMOVED*** Privacy & Masking
...
***REMOVED******REMOVED******REMOVED*** Observability & Tracing
...
***REMOVED******REMOVED*** 13. Code Examples

***REMOVED******REMOVED******REMOVED*** Example 1: Using Structured Logging with Context
...
async def create_resident(request_data):
    ***REMOVED*** Create logging context
    ...
    with LogContextManager(context):
        ...
        ***REMOVED*** request_id and user_id automatically included in log output
```

***REMOVED******REMOVED******REMOVED*** Example 2: Automatic Sensitive Data Filtering

```python
***REMOVED*** Request body: {"username": "john", "password": "secret123"}
***REMOVED*** Logged as: {"username": "john", "password": "********"}

***REMOVED*** Email in error message: "User john@example.com not found"
***REMOVED*** Sanitized as: "User [REDACTED-EMAIL] not found"

***REMOVED*** Request headers: {"Authorization": "Bearer eyJhbGc..."}
***REMOVED*** Logged as: {"Authorization": "Bearer [REDACTED]"}
```

***REMOVED******REMOVED******REMOVED*** Example 3: OpenTelemetry Tracing
