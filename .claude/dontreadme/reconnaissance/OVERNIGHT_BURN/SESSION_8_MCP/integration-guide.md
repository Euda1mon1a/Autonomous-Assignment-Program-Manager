***REMOVED*** MCP Tool Integration Guide
***REMOVED******REMOVED*** Patterns, dependencies, and best practices for using 81 MCP tools
...
***REMOVED******REMOVED*** Architecture Model
...
***REMOVED******REMOVED*** Tool Categories & Dependencies

***REMOVED******REMOVED******REMOVED*** Category 1: Core Scheduling (No Dependencies)
...
***REMOVED******REMOVED******REMOVED*** Category 2: Async Task Management (Depends on: Celery, Redis)
...
**Example:**
```python
***REMOVED*** Step 1: Start task
task = start_background_task_tool(
    task_type=TaskType.RESILIENCE_CONTINGENCY,
    params={"days_ahead": 90}
)
***REMOVED*** Returns: BackgroundTaskResult(task_id="abc123", status="pending")

***REMOVED*** Step 2: Poll until complete
import time
while True:
    status = get_task_status_tool(task.task_id)
    if status.status == TaskStatus.SUCCESS:
        result = status.result  ***REMOVED*** Contingency analysis data
        break
    elif status.status == TaskStatus.FAILURE:
        error = status.error  ***REMOVED*** Error details
        break
    time.sleep(5)  ***REMOVED*** Poll every 5 seconds
```

---

***REMOVED******REMOVED******REMOVED*** Category 3: Resilience Framework (Depends on: API Backend, Analytics)
...
***REMOVED******REMOVED******REMOVED*** Category 4: Early Warning System (Depends on: Schedule History, Analytics)
...
***REMOVED******REMOVED******REMOVED*** Category 5: Deployment Pipeline (Depends on: External Services)
...
***REMOVED******REMOVED******REMOVED*** Category 6: Metrics & Analytics (Depends on: Schedule Data)
...
***REMOVED******REMOVED******REMOVED*** Category 7: Advanced Analytics (Research Tools)
...
***REMOVED******REMOVED******REMOVED*** Workflow 1: Morning Health Check (5 min)

```python
async def daily_health_check():
    """Quick morning system health assessment."""

    ***REMOVED*** 1. Utilization (30ms)
    util = check_utilization_threshold_tool()

    ***REMOVED*** 2. Burnout (300ms)
    precursors = detect_burnout_precursors_tool()

    ***REMOVED*** 3. Fatigue (200ms)
    fatigue = get_fatigue_score_tool()

    ***REMOVED*** 4. Drift (500ms)
    spc = run_spc_analysis_tool(window_days=7)

    ***REMOVED*** 5. Unified index (400ms)
    health = get_unified_critical_index_tool()

    ***REMOVED*** Synthesize
    report = {
        "timestamp": datetime.now(),
        "overall_health": health.score,  ***REMOVED*** 0-1
        "utilization_level": util.level,  ***REMOVED*** GREEN/YELLOW/ORANGE/RED/BLACK
        "burnout_signal": precursors.detected,
        "fatigue_score": fatigue.score,  ***REMOVED*** 0-100
        "drift_detected": spc.out_of_control,
    }

    ***REMOVED*** Alert if problems
    if health.score < 0.3:
...
async def emergency_response():
    ...
    ***REMOVED*** 5. Start background task for detailed analysis
    ...
    return {
        ...
        "task_id": task.task_id  ***REMOVED*** For monitoring
    }
```
        ...
        for person_id in conflict.affected_people:
            ...
            if swaps.candidates:
                fixes.append({
                    ...
                    "swap_option": swaps.candidates[0]  ***REMOVED*** Best option
                })
...
***REMOVED******REMOVED******REMOVED*** Workflow 4: Fairness Audit (20 min)

```python
async def fairness_audit(start_date, end_date):
    """Comprehensive equity analysis with improvement plan."""

    ***REMOVED*** 1. Calculate Shapley values
    shapley = calculate_shapley_workload_tool(
        date_range=(start_date, end_date),
        method="shapley"
    )

    ***REMOVED*** 2. Get equality metrics
    equity = calculate_equity_metrics_tool(
        date_range=(start_date, end_date),
        include_roles=True
    )

    ***REMOVED*** 3. Generate Lorenz curve for visualization
    lorenz = generate_lorenz_curve_tool()

    ***REMOVED*** 4. Identify unfair allocations
    inequitable_people = [
        p for p in shapley.person_contributions
        if p.fairness_score < 0.7  ***REMOVED*** Below 70% fairness
    ]

    ***REMOVED*** 5. Find swap options to improve fairness
    improvement_plan = []
    ...
    return {
        ...
        "gini_coefficient": equity.gini,  ***REMOVED*** 0 = perfect equality
...
***REMOVED******REMOVED******REMOVED*** Workflow 5: Burnout Risk Assessment (10 min)

```python
async def burnout_risk_assessment():
    """Multi-method burnout analysis with interventions."""

    ***REMOVED*** Method 1: STA/LTA early warning
    sta_lta = detect_burnout_precursors_tool()

    ***REMOVED*** Method 2: SIR epidemiology model
    burnout_rt = calculate_burnout_rt_tool()

    ***REMOVED*** Method 3: CFFDRS fire danger index
    fire_danger = calculate_fire_danger_index_tool()

    ***REMOVED*** Method 4: FRMS fatigue assessment
    frms = run_frms_assessment_tool()

    ***REMOVED*** Method 5: Cognitive load
    cognitive = assess_cognitive_load_tool()

    ***REMOVED*** Synthesize consensus
    methods_detecting_risk = sum([
        sta_lta.signal_detected,
        burnout_rt.rt > 1.0,  ***REMOVED*** Spreading
        fire_danger.danger_level in ["EXTREME", "VERY_HIGH"],
        frms.overall_risk in ["HIGH", "CRITICAL"],
        cognitive.load_level in ["HEAVY", "EXTREME"]
    ])

    consensus_level = methods_detecting_risk / 5  ***REMOVED*** Consensus %

    ***REMOVED*** If multiple methods agree, escalate
    if consensus_level >= 0.6:  ***REMOVED*** 3+ methods agree
        ***REMOVED*** Start background contingency analysis
        task = start_background_task_tool(
...
***REMOVED******REMOVED******REMOVED*** Pattern 1: Graceful Degradation
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Timeout Handling
...
***REMOVED******REMOVED******REMOVED*** Pattern 3: Retry Logic
...
***REMOVED******REMOVED******REMOVED*** Optimization 1: Parallel Execution
...
async def parallel_metrics():
    ...
    ***REMOVED*** ~500ms instead of 1.5s sequential
```

***REMOVED******REMOVED******REMOVED*** Optimization 2: Caching
...
***REMOVED******REMOVED******REMOVED*** Optimization 3: Batch Operations
...
***REMOVED******REMOVED*** Tool Interdependencies Map
...
***REMOVED******REMOVED*** Recommendation: Tool Orchestration Framework
...
```python
***REMOVED*** Example: Composite tool (not yet implemented)
@mcp.tool()
