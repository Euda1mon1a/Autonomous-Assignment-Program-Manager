***REMOVED*** 80% Utilization Threshold: Concept, Theory & Implementation
...
***REMOVED******REMOVED******REMOVED*** I. PERCEPTION - Current Utilization Tracking
...
***REMOVED******REMOVED******REMOVED*** II. INVESTIGATION - Calculation Methodology

***REMOVED******REMOVED******REMOVED******REMOVED*** Core Calculation Formula
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Methods in UtilizationMonitor
...
***REMOVED******REMOVED******REMOVED*** III. ARCANA - Queuing Theory Background

***REMOVED******REMOVED******REMOVED******REMOVED*** Mathematical Foundation: M/M/1 Queue
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Historical Precedent: Telephone Networks
...
***REMOVED******REMOVED******REMOVED*** IV. HISTORY - Threshold Evolution
...
***REMOVED******REMOVED******REMOVED*** V. INSIGHT - Why 80% Specifically

***REMOVED******REMOVED******REMOVED******REMOVED*** Three Theoretical Justifications
...
***REMOVED******REMOVED******REMOVED*** VI. RELIGION - Threshold Enforcement
...
safe_capacity = monitor.get_safe_capacity(
    ...
    days_in_period=730  ***REMOVED*** Full year
)

***REMOVED*** Solver can only assign up to safe_capacity blocks
***REMOVED*** This prevents creating infeasible schedules at the source
```

**Level 2: Real-Time Assignment Validation**

```python
***REMOVED*** When adding assignments via API
...
if not is_safe:
    ***REMOVED*** Reject assignment, inform user
...
```python
***REMOVED*** From resilience/tasks.py - runs every 15 minutes
@shared_task
...
***REMOVED******REMOVED******REMOVED*** VII. NATURE - Threshold Appropriateness

***REMOVED******REMOVED******REMOVED******REMOVED*** Validation Against Real-World Systems
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Appropriateness to Medical Context
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Edge Cases Where Threshold Adjusts

**Dynamic Adjustment for Known Risks:**

```python
***REMOVED*** During PCS season (40% faculty available)
available_faculty_count = 6  ***REMOVED*** Out of 10
adjusted_capacity = 6 * 2 * 730 = 8760 blocks
effective_80_percent_threshold = 8760 * 0.8 = 7008

***REMOVED*** This prevents over-scheduling during known shortage periods
```

**Temporary Elevation (Approved by Medical Director):**

```python
***REMOVED*** Emergency escalation for critical shortage
threshold.max_utilization = 0.85  ***REMOVED*** Temporary only
threshold.critical_threshold = 0.95  ***REMOVED*** Escalate alerts sooner

***REMOVED*** Automatic rollback after crisis period
```

---

***REMOVED******REMOVED******REMOVED*** VIII. MEDICINE - Healthcare Utilization Context

***REMOVED******REMOVED******REMOVED******REMOVED*** How Utilization Connects to Patient Outcomes
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ACGME 80-Hour Rule Connection
...
***REMOVED******REMOVED******REMOVED*** IX. SURVIVAL - Threshold Breach Handling

***REMOVED******REMOVED******REMOVED******REMOVED*** Response Protocol by Level
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Automatic Recovery Mechanisms

**Fallback Schedule Activation** (when RED for >2 hours):
```python
***REMOVED*** From static_stability.py
fallback_plan = fallback_scheduler.recommend_fallback(
    current_crisis_metrics
)
***REMOVED*** Switches to pre-computed safe schedule
***REMOVED*** Prioritizes essential coverage only
```
...
***REMOVED******REMOVED******REMOVED*** X. STEALTH - Hidden Utilization Factors

***REMOVED******REMOVED******REMOVED******REMOVED*** Factors NOT Directly Measured
...
forecasts = monitor.forecast_utilization(
    ...
    known_absences=absence_schedule,  ***REMOVED*** {date: [UUIDs]}
    required_coverage_by_date=coverage_needs,  ***REMOVED*** {date: int}
    forecast_days=90,
)
...
```python
***REMOVED*** Constrain solver input
max_schedulable = monitor.get_safe_capacity(
    available_faculty,
    blocks_per_faculty_per_day=2.0,
    days_in_period=period_days,
)
***REMOVED*** Pass max_schedulable to solver as hard constraint
```
...
async def create_assignment(assignment: AssignmentCreate, db: Session):
    ***REMOVED*** Check utilization before accepting
...
def periodic_utilization_check():
    ...
    ***REMOVED*** Get current state
    metrics = monitor.calculate_utilization(...)

    ***REMOVED*** Update metrics in Prometheus
    utilization_gauge.set(metrics.utilization_rate)

    ***REMOVED*** Trigger actions based on level
...
***REMOVED******REMOVED*** Key Formulas Reference
