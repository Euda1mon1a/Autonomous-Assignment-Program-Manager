***REMOVED*** Session 7: Resilience Framework - SPC Monitoring Deep Dive
...
***REMOVED******REMOVED*** 1. PERCEPTION - Current SPC Implementation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** SPCAlert Dataclass
```python
@dataclass
class SPCAlert:
    rule: str                      ***REMOVED*** "Rule 1", "Rule 2", etc.
    severity: str                  ***REMOVED*** "CRITICAL", "WARNING", or "INFO"
    message: str                   ***REMOVED*** Human-readable description
    resident_id: UUID | None       ***REMOVED*** Affected resident (or None for system-wide)
    timestamp: datetime            ***REMOVED*** When alert was generated
    data_points: list[float]       ***REMOVED*** Data that triggered the alert
    control_limits: dict           ***REMOVED*** Relevant control limits
```

***REMOVED******REMOVED******REMOVED******REMOVED*** WorkloadControlChart Class
**Purpose:** Analyzes time-series workload data to detect deviations from normal patterns.

**Initialization Parameters:**
```python
chart = WorkloadControlChart(
    target_hours=60.0,    ***REMOVED*** Target/centerline weekly hours
    sigma=5.0             ***REMOVED*** Process standard deviation
)
```
...
***REMOVED******REMOVED******REMOVED*** Primary Method
...
***REMOVED******REMOVED*** 2. INVESTIGATION - Rule Calculations

***REMOVED******REMOVED******REMOVED*** Rule 1: One Point Beyond 3σ (CRITICAL)
...
def _check_rule_1(self, resident_id: UUID, weekly_hours: list[float]) -> list[SPCAlert]:
    ...
    for i, hours in enumerate(weekly_hours):
        if hours > self.ucl_3sigma:  ***REMOVED*** > 75h
            ***REMOVED*** CRITICAL upper violation
        elif hours < self.lcl_3sigma:  ***REMOVED*** < 45h
            ***REMOVED*** CRITICAL lower violation
    return alerts
```
...
***REMOVED******REMOVED******REMOVED*** Rule 2: 2 of 3 Consecutive Points Beyond 2σ (WARNING)
...
def _check_rule_2(self, resident_id: UUID, weekly_hours: list[float]) -> list[SPCAlert]:
    for i in range(len(weekly_hours) - 2):
        ...
        ***REMOVED*** Check upper 2σ
        upper_violations = [h for h in window if h > self.ucl_2sigma]  ***REMOVED*** > 70h
        if len(upper_violations) >= 2:
            ***REMOVED*** WARNING - process shift detected (overload)
            break  ***REMOVED*** Only report first occurrence

        ***REMOVED*** Check lower 2σ
        lower_violations = [h for h in window if h < self.lcl_2sigma]  ***REMOVED*** < 50h
        if len(lower_violations) >= 2:
            ***REMOVED*** WARNING - process shift detected (underutilization)
            break
```
...
***REMOVED******REMOVED******REMOVED*** Rule 3: 4 of 5 Consecutive Points Beyond 1σ (WARNING)
...
def _check_rule_3(self, resident_id: UUID, weekly_hours: list[float]) -> list[SPCAlert]:
    for i in range(len(weekly_hours) - 4):
        ...
        ***REMOVED*** Check upper 1σ
        upper_violations = [h for h in window if h > self.ucl_1sigma]  ***REMOVED*** > 65h
        if len(upper_violations) >= 4:
            ***REMOVED*** WARNING - process trend (gradual shift)
            break

        ***REMOVED*** Check lower 1σ
        lower_violations = [h for h in window if h < self.lcl_1sigma]  ***REMOVED*** < 55h
        if len(lower_violations) >= 4:
            ***REMOVED*** WARNING - process trend (gradual decrease)
            break
```
...
***REMOVED******REMOVED******REMOVED*** Rule 4: 8 Consecutive Points on Same Side of Centerline (INFO)
...
def _check_rule_4(self, resident_id: UUID, weekly_hours: list[float]) -> list[SPCAlert]:
    for i in range(len(weekly_hours) - 7):
        ...
        ***REMOVED*** Check if all points above centerline
        above_center = [h for h in window if h > self.target_hours]  ***REMOVED*** > 60h
        if len(above_center) == 8:
            mean_hours = statistics.mean(window)
            ***REMOVED*** INFO - sustained shift detected
            break

        ***REMOVED*** Check if all points below centerline
        below_center = [h for h in window if h < self.target_hours]  ***REMOVED*** < 60h
        if len(below_center) == 8:
            mean_hours = statistics.mean(window)
            ***REMOVED*** INFO - sustained shift detected (underutilization)
            break
```
...
***REMOVED******REMOVED*** 3. ARCANA - Statistical Process Control Theory

***REMOVED******REMOVED******REMOVED*** Control Chart Zones
...
***REMOVED******REMOVED******REMOVED*** Western Electric Rules (Historical Context)
...
***REMOVED******REMOVED******REMOVED*** Common Cause vs Special Cause Variation
...
***REMOVED******REMOVED*** 4. HISTORY - SPC Evolution

***REMOVED******REMOVED******REMOVED*** Implementation Timeline
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `calculate_control_limits(data: list[float]) -> dict`
...
***REMOVED*** Returns:
{
    "centerline": 60.125,
    "sigma": 1.808,
    "ucl": 65.549,         ***REMOVED*** Centerline + 3σ
    "lcl": 54.701,         ***REMOVED*** Centerline - 3σ
    "ucl_2sigma": 63.741,
    "lcl_2sigma": 56.509,
    "n": 8
}
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `calculate_process_capability(data, lsl, usl) -> dict`
...
capability = calculate_process_capability(
    ...
    lsl=40,   ***REMOVED*** Lower spec (minimum hours)
    usl=80    ***REMOVED*** Upper spec (ACGME limit)
)
***REMOVED*** Returns:
{
    "cp": 2.206,           ***REMOVED*** Potential capability
    "cpk": 2.206,          ***REMOVED*** Actual capability (centered)
    "pp": 2.206,           ***REMOVED*** Performance (long-term)
    "ppk": 2.206,          ***REMOVED*** Performance actual
    "mean": 60.125,
    "sigma": 1.808,
...
***REMOVED******REMOVED*** 5. INSIGHT - Variation Detection Philosophy

***REMOVED******REMOVED******REMOVED*** The Three Levels of Variation

***REMOVED******REMOVED******REMOVED******REMOVED*** Level 1: Random Variation (No Action)
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Level 2: Detectable Shift (Warning, Investigate)
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Level 3: Out of Control (Critical, Immediate Action)
```
...
***REMOVED******REMOVED******REMOVED*** Why SPC Catches What Histograms Miss
...
***REMOVED******REMOVED*** 6. RELIGION - Rule Implementation Status

***REMOVED******REMOVED******REMOVED*** Complete Checklist
...
***REMOVED******REMOVED*** 7. NATURE - Rule Sensitivity Analysis

***REMOVED******REMOVED******REMOVED*** Rule 1 Sensitivity
...
***REMOVED******REMOVED******REMOVED*** Rule 2 Sensitivity
...
***REMOVED******REMOVED******REMOVED*** Rule 3 Sensitivity
...
***REMOVED******REMOVED******REMOVED*** Rule 4 Sensitivity
...
***REMOVED******REMOVED*** 8. MEDICINE - Healthcare Quality Context

***REMOVED******REMOVED******REMOVED*** ACGME Compliance Connection
...
***REMOVED******REMOVED******REMOVED*** Burnout Risk Stratification
...
***REMOVED******REMOVED******REMOVED*** Integration with Creep/Fatigue Model
...
***REMOVED******REMOVED*** 9. SURVIVAL - Alert Response Procedures

***REMOVED******REMOVED******REMOVED*** CRITICAL Alert Response (Rule 1)
...
***REMOVED******REMOVED******REMOVED*** WARNING Alert Response (Rules 2 & 3)
...
***REMOVED******REMOVED******REMOVED*** INFO Alert Response (Rule 4)
...
***REMOVED******REMOVED*** 10. STEALTH - Hidden Variation Patterns

***REMOVED******REMOVED******REMOVED*** Scenarios SPC CATCHES (That Simple Averages Miss)
...
***REMOVED******REMOVED******REMOVED*** Patterns SPC CANNOT Catch
...
***REMOVED******REMOVED*** Implementation Code Samples
...
***REMOVED*** Create control chart (default: target=60h, sigma=5h)
chart = WorkloadControlChart(target_hours=60.0, sigma=5.0)

***REMOVED*** Weekly hours for a resident over 10 weeks
resident_id = uuid4()
...
***REMOVED*** Process alerts
for alert in alerts:
    print(f"{alert.rule} ({alert.severity}): {alert.message}")
    if alert.severity == "CRITICAL":
        ***REMOVED*** Emergency intervention
        notify_program_director(alert)
    elif alert.severity == "WARNING":
        ***REMOVED*** Schedule review within 48 hours
        schedule_review(alert)
```

***REMOVED******REMOVED******REMOVED*** Advanced: Empirical Control Limits

```python
from app.resilience.spc_monitoring import calculate_control_limits

***REMOVED*** Historical data from last academic year
historical_data = [58, 62, 59, 61, 63, 60, 58, 62, 64, 57, ...]

***REMOVED*** Calculate empirical limits (don't assume target=60, sigma=5)
...
***REMOVED*** Use empirical limits
chart = WorkloadControlChart(
    target_hours=limits['centerline'],
    sigma=limits['sigma']
)

***REMOVED*** Monitor with real baseline, not assumptions
alerts = chart.detect_western_electric_violations(resident_id, current_data)
```

***REMOVED******REMOVED******REMOVED*** Advanced: Process Capability

```python
from app.resilience.spc_monitoring import calculate_process_capability

***REMOVED*** ACGME compliance assessment
weekly_hours = [58, 62, 59, 61, 63, 60, 58, 62, 64, 57]

capability = calculate_process_capability(
    data=weekly_hours,
    lsl=40,   ***REMOVED*** Minimum hours per week
    usl=80    ***REMOVED*** ACGME maximum
)
...
***REMOVED******REMOVED******REMOVED*** With Celery Tasks
...
def calculate_weekly_spc_monitoring(self, resident_id: str):
    ...
    try:
        ...
        ***REMOVED*** Get last 12 weeks of data
...
***REMOVED******REMOVED******REMOVED*** With Creep/Fatigue Model
...
***REMOVED******REMOVED*** Dashboard Metrics

***REMOVED******REMOVED******REMOVED*** Recommended Prometheus Metrics

```python
***REMOVED*** Alert counts by rule and severity
...
***REMOVED*** Current process status
spc_process_status = Gauge(
    'spc_process_status',
    'Current process control status',
    ['resident_id']  ***REMOVED*** Value: 0=in_control, 1=warning, 2=critical
)

***REMOVED*** Workload distribution
...
***REMOVED******REMOVED******REMOVED*** Dashboard Panels (Grafana)
...
***REMOVED******REMOVED******REMOVED*** Unit Tests (Already Implemented)

Run existing comprehensive test suite:
```bash
cd backend
pytest tests/resilience/test_spc_monitoring.py -v
***REMOVED*** 59 tests covering all rules, edge cases, integration scenarios
```

***REMOVED******REMOVED******REMOVED*** Integration Tests (Recommended)

```python
***REMOVED*** Test with real database
async def test_spc_monitoring_end_to_end():
    ***REMOVED*** Create test resident
    ***REMOVED*** Generate realistic weekly hours over 12 weeks
    ***REMOVED*** Trigger each rule in isolation
    ***REMOVED*** Verify alerts generated correctly
    ***REMOVED*** Verify database persistence
    ***REMOVED*** Verify notification queuing

***REMOVED*** Test Creep/SPC Bridge (when implemented)
async def test_creep_spc_bridge():
    ***REMOVED*** Generate damage values (0.0-1.0)
    ***REMOVED*** Map to control chart
    ***REMOVED*** Verify scaling math
    ***REMOVED*** Verify alert thresholds match damage stages
```

***REMOVED******REMOVED******REMOVED*** Load Tests (Recommended)

```bash
***REMOVED*** Monitor performance with 100 residents
***REMOVED*** Weekly job should complete < 5 minutes
pytest tests/performance/test_spc_load.py -v

***REMOVED*** Verify query performance
...
***REMOVED******REMOVED******REMOVED*** Key Parameters
...
***REMOVED******REMOVED******REMOVED*** Control Limits Interpretation
