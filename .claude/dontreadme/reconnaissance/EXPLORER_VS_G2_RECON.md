***REMOVED*** Explorer vs G2_RECON: Understanding the Distinction
...
***REMOVED******REMOVED*** Two Different Layers

***REMOVED******REMOVED******REMOVED*** Layer 1: Technical Infrastructure (subagent_type)
...
Task(
  ...
  subagent_type="Explore"  ***REMOVED*** <-- This is infrastructure configuration
)
```
...
***REMOVED******REMOVED******REMOVED*** Layer 2: Organizational Role (PAI Agent)
...
***REMOVED******REMOVED*** The Relationship
...
Task(
  ...
  ***REMOVED******REMOVED*** Scope
  ...
  """,
  subagent_type="Explore"  ***REMOVED*** The execution mechanism
)
```
...
***REMOVED******REMOVED*** Analogy: Role vs Tool
...
***REMOVED******REMOVED*** When to Use Each

***REMOVED******REMOVED******REMOVED*** Use `Explore` subagent_type when:
...
```python
***REMOVED*** Example: Simple exploration
Task(
...
***REMOVED******REMOVED******REMOVED*** Use G2_RECON (as a persona) when:
...
```python
***REMOVED*** Example: Full reconnaissance with persona
Task(
  prompt="""
  ***REMOVED******REMOVED*** Agent: G2_RECON
  ...
  """,
  subagent_type="Explore"  ***REMOVED*** or "general-purpose" if deeper analysis needed
)
```

***REMOVED******REMOVED******REMOVED*** The Decision Tree
...
***REMOVED******REMOVED******REMOVED*** Mistake 1: Treating them as alternatives
...
***REMOVED******REMOVED******REMOVED*** Mistake 2: Using G2_RECON without specifying subagent_type

```python
***REMOVED*** This doesn't work - G2_RECON is not a subagent_type
Task(
  prompt="Do reconnaissance",
  subagent_type="G2_RECON"  ***REMOVED*** ERROR: Invalid subagent_type
)
```

***REMOVED******REMOVED******REMOVED*** Mistake 3: Expecting Explore to follow G2_RECON protocols

```python
***REMOVED*** Explore won't produce G2 output format unless told
Task(
  prompt="Scout the swap executor",
  subagent_type="Explore"
)
***REMOVED*** Result: Ad-hoc output, no risk matrix, no escalation awareness
```

***REMOVED******REMOVED******REMOVED*** Mistake 4: Over-engineering simple searches

```python
***REMOVED*** Overkill for "where is X?"
Task(
  prompt="""
  ***REMOVED******REMOVED*** Agent: G2_RECON
...
***REMOVED******REMOVED*** For ORCHESTRATOR: Choosing the Right Approach
