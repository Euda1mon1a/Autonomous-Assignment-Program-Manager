***REMOVED*** FastMCP Research Document

***REMOVED******REMOVED*** Table of Contents
1. [What is FastMCP](***REMOVED***what-is-fastmcp)
2. [Installation and Setup](***REMOVED***installation-and-setup)
3. [Core Concepts](***REMOVED***core-concepts)
4. [Code Examples](***REMOVED***code-examples)
5. [FastAPI Integration](***REMOVED***fastapi-integration)
6. [Testing MCP Servers](***REMOVED***testing-mcp-servers)
7. [Deployment Considerations](***REMOVED***deployment-considerations)
8. [Relevance to Scheduling Domain](***REMOVED***relevance-to-scheduling-domain)

---

***REMOVED******REMOVED*** What is FastMCP
...
***REMOVED******REMOVED******REMOVED*** MCP Protocol Overview
...
***REMOVED******REMOVED******REMOVED*** FastMCP Evolution
...
***REMOVED******REMOVED*** Installation and Setup
...
***REMOVED******REMOVED******REMOVED*** Creating Your First Server
...
***REMOVED*** Instantiate the FastMCP server
...
***REMOVED******REMOVED******REMOVED*** 1. Resources (Read-Only Data Exposure)
...
***REMOVED******REMOVED******REMOVED*** 2. Tools (Callable Functions)
...
***REMOVED******REMOVED******REMOVED*** 3. Prompts (Reusable Templates)
...
***REMOVED******REMOVED******REMOVED*** Key Difference: Resources vs Tools
...
***REMOVED******REMOVED******REMOVED*** Complete Server with All Three Primitives
...
async def fetch_user_data(user_id: int) -> dict:
    ...
    ***REMOVED*** Simulated async API call
...
***REMOVED******REMOVED******REMOVED*** Advanced Tool with Type Hints and Pydantic
...
***REMOVED******REMOVED******REMOVED*** Resource Templates (Dynamic URLs)

```python
@mcp.resource("resource://documents/{doc_id}")
def get_document(doc_id: str) -> str:
    """Fetches a document by ID."""
    ***REMOVED*** In real implementation, fetch from database
...
***REMOVED******REMOVED******REMOVED*** Pattern 1: Auto-Generate MCP Server from FastAPI
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Mount MCP Server to FastAPI
...
***REMOVED*** Create MCP HTTP app and mount it
mcp_app = mcp.http_app(path='/mcp')
app.mount("/analytics", mcp_app)

***REMOVED*** Now you can run: uvicorn main:app
```

***REMOVED******REMOVED******REMOVED*** Pattern 3: Route Mapping Configuration
...
***REMOVED*** Configure route mapping
route_map = RouteMap(
    get_to_resource=True,  ***REMOVED*** Map GET requests to Resources
    post_to_tool=True,     ***REMOVED*** Map POST requests to Tools
    put_to_tool=True,      ***REMOVED*** Map PUT requests to Tools
    delete_to_tool=True    ***REMOVED*** Map DELETE requests to Tools
)

mcp = FastMCP.from_fastapi(app=app, route_map=route_map)
```

***REMOVED******REMOVED******REMOVED*** Pattern 4: Using fastapi-mcp Library
...
***REMOVED*** Create FastApiMCP instance
mcp = FastApiMCP(
    app=app,
    name="My API",
...
***REMOVED*** MCP server available at /mcp
***REMOVED*** All FastAPI routes automatically exposed as MCP tools
```

***REMOVED******REMOVED******REMOVED*** Integration Best Practices
...
***REMOVED******REMOVED******REMOVED*** Basic Testing Pattern
...
async def main():
    ***REMOVED*** Connect via in-memory transport
    async with Client(mcp) as client:
        ***REMOVED*** Call a tool
        ...
        print(result.data)  ***REMOVED*** 8
```

***REMOVED******REMOVED******REMOVED*** Pytest Integration
...
***REMOVED******REMOVED******REMOVED*** Parameterized Tests
...
***REMOVED******REMOVED******REMOVED*** Testing Resources
...
***REMOVED******REMOVED******REMOVED*** Testing with Mocks (External APIs)
...
async def test_tool_with_external_api(
    ...
    ***REMOVED*** Mock the external API response
...
***REMOVED******REMOVED******REMOVED*** Recommended Tools
...
***REMOVED******REMOVED******REMOVED*** Additional Testing Resources
...
***REMOVED******REMOVED*** Deployment Considerations

***REMOVED******REMOVED******REMOVED*** Production Requirements
...
```python
***REMOVED*** Production HTTP server
mcp.run(
    transport="http",
    host="0.0.0.0",  ***REMOVED*** Listen on all interfaces
...
***REMOVED******REMOVED******REMOVED*** API Gateway Integration
...
***REMOVED******REMOVED******REMOVED*** Monitoring and Logging
...
***REMOVED******REMOVED*** Relevance to Scheduling Domain
...
***REMOVED******REMOVED******REMOVED*** 1. Schedule Data Exposure (Resources)
...
async def get_employee_schedule(employee_id: str) -> dict:
    ...
    ***REMOVED*** Query database for employee schedule
...
***REMOVED******REMOVED******REMOVED*** 2. Scheduling Operations (Tools)
...
async def auto_schedule_period(
    ...
    ***REMOVED*** Run scheduling algorithm
...
***REMOVED******REMOVED******REMOVED*** 3. AI-Assisted Scheduling (Prompts)
...
***REMOVED******REMOVED******REMOVED*** 4. Integration with Existing FastAPI Application
...
***REMOVED*** Create dedicated MCP server (recommended approach)
mcp = FastMCP("Scheduling MCP Server")

***REMOVED*** Add scheduling-specific tools and resources
...
***REMOVED*** Mount MCP server to FastAPI
mcp_app = mcp.http_app(path='/mcp')
app.mount("/mcp", mcp_app)

***REMOVED*** Both REST API and MCP available:
***REMOVED*** - REST API: http://localhost:8000/api/v1/...
***REMOVED*** - MCP: http://localhost:8000/mcp
```

***REMOVED******REMOVED******REMOVED*** 5. Use Cases for LLM Integration
...
***REMOVED******REMOVED******REMOVED*** 6. Benefits for Our Project
...
***REMOVED******REMOVED******REMOVED*** 7. Example Integration Workflow

```python
***REMOVED*** User interacts with Claude or other LLM:
***REMOVED*** "Show me tomorrow's schedule and identify any coverage gaps"

***REMOVED*** LLM uses MCP to:
***REMOVED*** 1. Read resource: resource://schedules/coverage/2025-12-19
***REMOVED*** 2. Call tool: check_coverage_gaps(date="2025-12-19")
***REMOVED*** 3. Use prompt: coverage_analysis_prompt to structure response
***REMOVED*** 4. Return natural language answer with actionable insights
```
