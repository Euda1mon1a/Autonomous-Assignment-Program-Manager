***REMOVED*** MCP Database Query Tools - Comprehensive Documentation
...
***REMOVED******REMOVED******REMOVED*** PERCEPTION: Current Database Access Patterns
...
***REMOVED******REMOVED******REMOVED*** INVESTIGATION: Query Flow
...
***REMOVED******REMOVED******REMOVED*** Category 1: Direct API-Based Tools (via SchedulerAPIClient)

These tools make HTTP requests to the FastAPI backend.

***REMOVED******REMOVED******REMOVED******REMOVED*** Schedule Management Tools
...
```python
***REMOVED*** Tool invocation example
...
***REMOVED*** Returns: ScheduleValidationResult with violations list
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Swap Management Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Conflict Detection Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Compliance and Metrics Tools
...
***REMOVED******REMOVED******REMOVED*** Category 2: Resilience & Analysis Tools

These tools analyze system resilience and schedule stability.

***REMOVED******REMOVED******REMOVED******REMOVED*** Resilience Framework Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Time Crystal & Anti-Churn Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Early Warning & Burnout Detection Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Fatigue Risk Management (FRMS)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Entropy & Thermodynamics Tools
...
***REMOVED******REMOVED******REMOVED*** Category 3: Advanced Analytics Tools

***REMOVED******REMOVED******REMOVED******REMOVED*** Game Theory Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Ecological Dynamics Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Kalman Filtering Tools
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Fourier Analysis Tools
...
***REMOVED******REMOVED******REMOVED*** Category 4: Validation & Constraint Tools
...
***REMOVED******REMOVED*** Database Access Philosophy

***REMOVED******REMOVED******REMOVED*** ARCANA: SQLAlchemy 2.0 Patterns

The backend uses modern SQLAlchemy patterns:

```python
***REMOVED*** GOOD: Eager loading to prevent N+1 queries
...
***REMOVED*** GOOD: Type-safe selects with select() syntax
...
***REMOVED*** GOOD: Connection pooling with pre-ping
engine = create_engine(
    DATABASE_URL,
    pool_pre_ping=True,  ***REMOVED*** Verify connection validity
    pool_size=10,
    max_overflow=20,
    pool_recycle=1800,  ***REMOVED*** Prevent stale connections
)

***REMOVED*** GOOD: Async/await patterns for background tasks
async def async_task():
    with task_session_scope() as session:
        ***REMOVED*** Database work
        assignments = session.query(Assignment).all()

***REMOVED*** GOOD: Read-only repositories for data access
...
***REMOVED******REMOVED******REMOVED*** INSIGHT: Safe Database Access for MCP Tools

**Rule 1: No Direct Database Access from MCP**
```python
***REMOVED*** WRONG: Direct database access in MCP tool
from app.db.session import SessionLocal

async def bad_tool():
    db = SessionLocal()  ***REMOVED*** ❌ Bypasses API layer
    assignments = db.query(Assignment).all()
    db.close()
    return assignments

***REMOVED*** RIGHT: Use API client
async def good_tool():
    client = await get_api_client()  ***REMOVED*** ✓ Authenticated HTTP
...
async def validate_schedule(request):
    ...
    ***REMOVED*** 1. Authenticate with JWT
    headers = await client._ensure_authenticated()

    ***REMOVED*** 2. Make HTTP request to backend
    result = await client.validate_schedule(
        start_date=request.start_date.isoformat(),
        end_date=request.end_date.isoformat(),
    )

    ***REMOVED*** 3. Transform response for MCP format
...
***REMOVED******REMOVED******REMOVED*** API Client Authentication

The MCP server authenticates with the backend using credentials from environment variables:

**Required Environment Variables:**
```bash
API_BASE_URL=http://backend:8000          ***REMOVED*** Backend URL
API_USERNAME=mcp-user                      ***REMOVED*** Service account username
API_PASSWORD=<secure-password>             ***REMOVED*** Service account password
DATABASE_URL=postgresql://...              ***REMOVED*** For resources only
```
...
***REMOVED******REMOVED******REMOVED*** Access Levels
...
***REMOVED******REMOVED*** Guardrails & Safety Mechanisms

***REMOVED******REMOVED******REMOVED*** RELIGION: Read-Only Where Appropriate
...
***REMOVED******REMOVED******REMOVED*** NATURE: Tool Power Levels
...
***REMOVED******REMOVED******REMOVED*** SURVIVAL: Error Handling & Recovery
...
async def health_check() -> bool:
    ...
    client = await get_api_client()
    return await client.health_check()  ***REMOVED*** GET /health
```
...
***REMOVED******REMOVED******REMOVED*** Example 1: Schedule Validation Workflow
...
***REMOVED*** Validate a specific date range
request = ScheduleValidationRequest(
    start_date=date(2025, 1, 1),
    end_date=date(2025, 1, 31),
    check_work_hours=True,
...
***REMOVED******REMOVED******REMOVED*** Example 2: Swap Analysis
...
***REMOVED*** Find swap candidates for a resident
...
***REMOVED******REMOVED******REMOVED*** Example 3: Contingency Planning
...
***REMOVED*** Simulate faculty absence scenario
request = ContingencyRequest(
    ...
    auto_resolve=False,  ***REMOVED*** Don't auto-apply solutions
)
...
***REMOVED******REMOVED******REMOVED*** Example 4: Resilience Analysis
...
***REMOVED*** Check resilience metrics
...
***REMOVED******REMOVED******REMOVED*** Example 5: Burnout Detection
...
***REMOVED*** Scan for burnout precursors
...
***REMOVED*** Review precursor signals
...
***REMOVED******REMOVED*** Common Pitfalls & How to Avoid Them

***REMOVED******REMOVED******REMOVED*** Pitfall 1: Bypassing the API Client
...
async def bad_mcp_tool():
    ***REMOVED*** ❌ Direct database access - no auth, no audit, no API control
    db = SessionLocal()
...
async def good_mcp_tool():
    ***REMOVED*** ✓ Authenticated API access
    client = await get_api_client()
    result = await client.get_assignments(limit=100)
    return result
```

***REMOVED******REMOVED******REMOVED*** Pitfall 2: Ignoring Authentication Failures

**Wrong:**
```python
async def tool_without_retry():
    client = await get_api_client()
    ***REMOVED*** If token expires, this will fail silently
    result = await client.some_operation()
    return result
```
...
async def tool_with_retry():
    try:
        ...
        if e.response.status_code == 401:
            ***REMOVED*** Token expired, re-authenticate
            client._token = None
            result = await client.some_operation()
            return result
        raise
...
***REMOVED******REMOVED******REMOVED*** Pitfall 3: N+1 Query Problems in Resources
...
async def inefficient_resource():
    db = _get_db_session()
    assignments = db.query(Assignment).all()
    ***REMOVED*** This triggers N queries when accessing assignment.person
    for a in assignments:
        print(a.person.name)  ***REMOVED*** ❌ N queries
    db.close()
```
...
async def efficient_resource():
    ...
    assignments = (
        db.query(Assignment)
        .options(joinedload(Assignment.person))  ***REMOVED*** ✓ Single query batch
        .all()
    )
    for a in assignments:
        print(a.person.name)
...
***REMOVED******REMOVED******REMOVED*** Pitfall 4: Returning Sensitive Data
...
async def unsafe_tool():
    ...
    ***REMOVED*** ❌ Returns full names, emails, PII
    return people
```
...
async def safe_tool():
    ...
    ***REMOVED*** ✓ Filter to role-based identifiers only
    sanitized = [
        {
            "person_id": p["id"],
...
***REMOVED******REMOVED*** Undocumented Query Patterns

***REMOVED******REMOVED******REMOVED*** STEALTH: Hidden Query Capabilities
...
***REMOVED*** Direct constraint validation (lower-level than API)
...
```python
***REMOVED*** Behind the scenes: bulk operations
assignment_repo.delete_by_block_ids(block_ids)  ***REMOVED*** Batch delete
assignment_repo.get_by_block_ids(block_ids)    ***REMOVED*** Batch get
```
...
result = await client.generate_schedule(
    start_date=...,
    end_date=...,
    algorithm="greedy",        ***REMOVED*** Fast, simple
    ***REMOVED*** algorithm="cp_sat",      ***REMOVED*** Google OR-Tools constraint solver
    ***REMOVED*** algorithm="pulp",        ***REMOVED*** Open-source LP solver
    ***REMOVED*** algorithm="hybrid",      ***REMOVED*** Combination approach
    timeout_seconds=120,       ***REMOVED*** Custom timeout (5-300s)
)
```
...
***REMOVED*** Returns: circuit breaker states, health checks, override options
```
...
```python
***REMOVED*** Default: page 1, page_size 100
***REMOVED*** Max page_size: 500
result = await client.get_assignments(limit=100)  ***REMOVED*** API returns paginated
```

**Batch Operations:**
For large data sets, use batch endpoints when available:
```python
***REMOVED*** Better than looping over individual queries
result = await client.get_assignments(start_date=..., end_date=..., limit=500)
```

---

***REMOVED******REMOVED*** Database Tables Accessed
