***REMOVED*** MCP Notification Tools Documentation
...
***REMOVED******REMOVED*** Part I: Channel Architecture
...
***REMOVED******REMOVED******REMOVED*** 1. In-App Channel (`InAppChannel`)
...
***REMOVED*** Result: DeliveryResult(success=True, channel="in_app", ...)
```

---

***REMOVED******REMOVED******REMOVED*** 2. Email Channel (`EmailChannel`)
...
**Configuration:**
```python
EmailChannel(from_address="noreply@schedule.mil")  ***REMOVED*** Customizable sender
```
...
- **HTML Template:** Styled email with color-coded priority indicators
  - `priority-high`: Red left border (***REMOVED***dc3545)
  - `priority-normal`: Blue left border (***REMOVED***007bff)
  - `priority-low`: Gray left border (***REMOVED***6c757d)
...
result = await channel.deliver(payload, db_session)
***REMOVED*** Result: DeliveryResult(
***REMOVED***   success=True,
***REMOVED***   channel="email",
***REMOVED***   metadata={
***REMOVED***     "email_payload": {
***REMOVED***       "from": "noreply@schedule.mil",
***REMOVED***       "to": "user-{id}@example.com",
***REMOVED***       "subject": "New Schedule Published",
***REMOVED***       "body": "...",
***REMOVED***       "html": "<html>...</html>",
...
***REMOVED******REMOVED******REMOVED*** 3. Webhook Channel (`WebhookChannel`)
...
***REMOVED*** Result: DeliveryResult(
***REMOVED***   success=True,
***REMOVED***   channel="webhook",
***REMOVED***   metadata={
***REMOVED***     "webhook_url": "https://hooks.slack.com/services/...",
***REMOVED***     "payload": {...}
***REMOVED***   }
***REMOVED*** )
```
...
***REMOVED******REMOVED*** Part II: Notification Types

***REMOVED******REMOVED******REMOVED*** Standard Notifications (Built-In)

**Location:** `/backend/app/notifications/notification_types.py`

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. SCHEDULE_PUBLISHED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. ASSIGNMENT_CHANGED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. SHIFT_REMINDER_24H
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. SHIFT_REMINDER_1H
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. ACGME_WARNING
...
**Special Handling:** Webhook delivery enables alerting external monitoring systems (PagerDuty, Slack ***REMOVED***alerts, etc.)

---

***REMOVED******REMOVED******REMOVED******REMOVED*** 6. ABSENCE_APPROVED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 7. ABSENCE_REJECTED
...
***REMOVED******REMOVED******REMOVED*** Swap Notifications (Domain-Specific)

**Location:** `/backend/app/services/swap_notification_service.py`

**Enum:** `SwapNotificationType`

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. SWAP_REQUEST_RECEIVED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. SWAP_REQUEST_ACCEPTED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. SWAP_REQUEST_REJECTED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. SWAP_EXECUTED
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 7. MARKETPLACE_MATCH
...
***REMOVED******REMOVED*** Part III: Notification Service

**Location:** `/backend/app/notifications/service.py`

***REMOVED******REMOVED******REMOVED*** Core Service: `NotificationService`

```python
service = NotificationService(db_session)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Method 1: `send_notification()` - Immediate Send
...
results = await service.send_notification(
    ...
    channels=["in_app", "email"]  ***REMOVED*** Optional, uses defaults if omitted
)
```

---

***REMOVED******REMOVED******REMOVED******REMOVED*** Method 2: `send_bulk()` - Broadcast to Multiple Recipients
...
***REMOVED*** Result: {
***REMOVED***   "uuid1": [DeliveryResult(...), DeliveryResult(...)],
***REMOVED***   "uuid2": [DeliveryResult(...)],
***REMOVED***   "uuid3": [DeliveryResult(...)],
***REMOVED*** }
```

---

***REMOVED******REMOVED******REMOVED******REMOVED*** Method 3: `schedule_notification()` - Future Delivery
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 4: `process_scheduled_notifications()` - Background Processing
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 5: `get_pending_notifications()` - Fetch User Inbox
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 6: `mark_as_read()` - Mark Notifications Read
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Method 7: `update_user_preferences()` - Customize Notification Settings
...
service.update_user_preferences(
    ...
    preferences=NotificationPreferences(
        user_id=uuid.UUID("..."),
        enabled_channels=["in_app", "webhook"],  ***REMOVED*** Disable email
        notification_types={
            NotificationType.SHIFT_REMINDER_24H.value: False,  ***REMOVED*** Disable 24h reminders
            NotificationType.SHIFT_REMINDER_1H.value: True,
            NotificationType.ACGME_WARNING.value: True,
        },
        quiet_hours_start=22,  ***REMOVED*** 10 PM
        quiet_hours_end=7,     ***REMOVED*** 7 AM
    )
)
```
...
***REMOVED******REMOVED*** Part IV: Background Tasks (Celery)

**Location:** `/backend/app/notifications/tasks.py`

***REMOVED******REMOVED******REMOVED*** Task 1: `send_email()` - Email Delivery
...
```python
{
    ...
    "subject": "Notification Subject",
    "status": "sent",
    "attempts": 1  ***REMOVED*** Attempt number that succeeded
}
```
...
***REMOVED******REMOVED******REMOVED*** Task 2: `send_webhook()` - Webhook Delivery
...
***REMOVED******REMOVED******REMOVED*** Task 3: `detect_leave_conflicts()` - Post-Approval Conflict Detection
...
***REMOVED******REMOVED*** Part V: Undocumented Alert Triggers

***REMOVED******REMOVED******REMOVED*** STEALTH Lens: Hidden Notification Sources

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. System Alert Notifications
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Secret Rotation Alerts
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Early Warning System Alerts
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Conflict Auto-Detection Alerts
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 5. Compliance Report Delivery
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 6. Marketplace Auto-Matcher Notifications
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 7. Defense-in-Depth Level Transitions
...
***REMOVED******REMOVED*** Part VI: MCP Tool Registration (81 Tools)

**Location:** `/mcp-server/src/scheduler_mcp/server.py`

***REMOVED******REMOVED******REMOVED*** Tool Categories
...
***REMOVED******REMOVED******REMOVED*** Example Tool: `detect_burnout_precursors_tool()`

```python
@mcp.tool()
async def detect_burnout_precursors_tool(
    resident_id: str,
    signal_type: str,  ***REMOVED*** "swap_requests", "sick_calls", etc.
    ...
    "alerts": [
        {
            ...
            "sta_lta_ratio": 3.2,
            "severity": "high",
            "predicted_magnitude": 6.5,  ***REMOVED*** Richter-like scale
...
***REMOVED******REMOVED*** Part VII: Configuration & Integration
...
NOTIFICATION_QUIET_HOURS_START=22  ***REMOVED*** 10 PM
NOTIFICATION_QUIET_HOURS_END=7     ***REMOVED*** 7 AM

***REMOVED*** Celery Configuration
CELERY_BROKER_URL=redis://redis:6379/0
CELERY_RESULT_BACKEND=redis://redis:6379/1
CELERY_TASK_TIME_LIMIT=300  ***REMOVED*** 5 minutes
CELERY_TASK_SOFT_TIME_LIMIT=250  ***REMOVED*** 4 min 10 sec

***REMOVED*** Early Warning System
...
***REMOVED******REMOVED*** Part VIII: Testing
...
***REMOVED******REMOVED*** Part IX: Alert Monitoring Dashboard

***REMOVED******REMOVED******REMOVED*** Metrics Exposed to Prometheus

```prometheus
***REMOVED*** Notification metrics
...
***REMOVED******REMOVED*** Part X: Security & Privacy

***REMOVED******REMOVED******REMOVED*** OPSEC Considerations
...
***REMOVED******REMOVED*** Part XI: Troubleshooting Guide

***REMOVED******REMOVED******REMOVED*** Issue: Notifications Not Sending
...
***REMOVED******REMOVED******REMOVED*** Issue: Webhook Delivery Failures
...
***REMOVED******REMOVED******REMOVED*** Issue: Performance / High Queue Depth
...
***REMOVED******REMOVED*** Part XII: Future Enhancements

***REMOVED******REMOVED******REMOVED*** Planned Features (Commented Out, Ready)
...
***REMOVED******REMOVED*** Summary: Complete Tool Inventory

***REMOVED******REMOVED******REMOVED*** All Delivery Channels
...
***REMOVED******REMOVED******REMOVED*** Notification Types Coverage
...
***REMOVED******REMOVED******REMOVED*** Alert Triggers (7 Sources)
