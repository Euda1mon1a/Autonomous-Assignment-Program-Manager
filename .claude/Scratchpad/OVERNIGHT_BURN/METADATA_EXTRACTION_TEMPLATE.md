***REMOVED*** OVERNIGHT_BURN Metadata Extraction Template
...
***REMOVED******REMOVED******REMOVED*** Complete File Record
...
***REMOVED******REMOVED*** Python Extraction Implementation

***REMOVED******REMOVED******REMOVED*** Script 1: Core Metadata Extractor
...
class MetadataExtractor:
    ...
    def _extract_basic_info(self, content: str, file_path: Path) -> Dict:
        ...
        ***REMOVED*** Extract executive summary or first paragraph
        summary = self._extract_summary(content)

        ***REMOVED*** File metrics
        ...
        ***REMOVED*** Dates
        ...
        return {
            ...
            "created_date": "2025-12-30",  ***REMOVED*** Known OVERNIGHT_BURN date
    ...
    def _extract_summary(self, content: str) -> str:
        ...
        ***REMOVED*** Try to find executive summary section
        exec_match = re.search(
            r'***REMOVED******REMOVED*** Executive Summary\n\n(.+?)(?:\n***REMOVED******REMOVED***|\Z)',
            content,
            re.DOTALL
        )
        if exec_match:
        ...
        ***REMOVED*** Fallback: first paragraph after title
        lines = content.split('\n')
        for i, line in enumerate(lines):
            if line.startswith('***REMOVED*** '):
                ***REMOVED*** Next non-empty line
                for j in range(i+1, len(lines)):
                    if lines[j].strip() and not lines[j].startswith('***REMOVED***'):
    ...
    def _extract_structure(self, content: str) -> Dict:
        ...
        ***REMOVED*** Count headers by level
        h1s = len(re.findall(r'^***REMOVED*** ', content, re.MULTILINE))
        h2s = len(re.findall(r'^***REMOVED******REMOVED*** ', content, re.MULTILINE))
        h3s = len(re.findall(r'^***REMOVED******REMOVED******REMOVED*** ', content, re.MULTILINE))

        ***REMOVED*** Extract header titles
        headers = re.findall(r'^***REMOVED***{1,3} (.+)$', content, re.MULTILINE)

        ***REMOVED*** Count content elements
    ...
    def _extract_key_content(self, content: str) -> Dict:
        ...
        ***REMOVED*** Extract findings
        findings = self._extract_findings(content)

        ***REMOVED*** Extract recommendations
        recommendations = self._extract_recommendations(content)

        ***REMOVED*** Extract action items (TODO, MUST, SHOULD)
    ...
    def _extract_findings(self, content: str) -> List[str]:
        ...
        ***REMOVED*** Look for "Finding" sections
        finding_sections = re.findall(
            r'^***REMOVED***{2,4}\s+.*[Ff]inding.*\n+(.+?)(?=\n***REMOVED******REMOVED***|\Z)',
            content,
            re.DOTALL
        )

        for section in finding_sections:
            ***REMOVED*** Extract bullet points
            bullets = re.findall(r'^[-*]\s+(.+)$', section, re.MULTILINE)
            findings.extend(bullets[:10])  ***REMOVED*** Top 10 findings

        return findings[:10]  ***REMOVED*** Return top 10

    def _extract_recommendations(self, content: str) -> List[str]:
        """Extract recommendations."""
        recs = []

        ***REMOVED*** Look for recommendations sections
        rec_sections = re.findall(
            r'^***REMOVED***{2,4}\s+.*[Rr]ecommendation.*\n+(.+?)(?=\n***REMOVED******REMOVED***|\Z)',
            content,
            re.DOTALL
        )
    ...
    def _extract_action_items(self, content: str) -> List[str]:
        ...
        ***REMOVED*** Lines with TODO, MUST, SHOULD
        items.extend(re.findall(
        ...
        return items[:10]  ***REMOVED*** Top 10 items
    ...
    def _classify_and_tag(self, metadata: Dict) -> Dict:
        ...
        ***REMOVED*** Extract keywords
        keywords = self._extract_keywords(metadata)

        ***REMOVED*** Technology tags
    ...
    def _extract_keywords(self, metadata: Dict) -> List[str]:
        ...
        ***REMOVED*** From headers
        for header in metadata["structure"].get("headers", []):
            ***REMOVED*** Split on common separators
            for word in re.split(r'[:\s-]', header.lower()):
                if len(word) > 3 and word not in ["the", "and", "for", "with"]:
                    keywords.add(word)

        ***REMOVED*** Domain-specific keywords
        ...
        return sorted(list(keywords))[:15]  ***REMOVED*** Top 15 keywords
    ...
    def _calculate_quality_metrics(self, content: str, metadata: Dict) -> Dict:
        ...
        ***REMOVED*** Completeness: % of expected sections present
        expected_sections = [
            "executive summary",
            "findings",
            "recommendations",
        ...
        ***REMOVED*** Clarity: readability metrics
        ***REMOVED*** (simplified - just check for good structure)
        ...
        ***REMOVED*** Actionability: presence of specific recommendations
        actionability = min(
            90,
...
***REMOVED******REMOVED******REMOVED*** Script 2: JSON Registry Generator
...
def generate_registry(metadata: Dict, output_path: Path) -> None:
    ...
    ***REMOVED*** Clean metadata for JSON serialization
    registry = {
    ...
    ***REMOVED*** Build summary statistics
    for file_meta in metadata["files"]:
        ...
        ***REMOVED*** Simplified file entry (omit raw content, full embedding)
        simplified = {
            "id": file_meta.get("id"),
...
***REMOVED******REMOVED******REMOVED*** Script 3: Batch Execution
...
***REMOVED******REMOVED*** File Type Classification Rules

***REMOVED******REMOVED******REMOVED*** Index/Navigation (→ Priority P0)
...
***REMOVED******REMOVED******REMOVED*** Summary/Digest (→ Priority P1-2)
...
***REMOVED******REMOVED******REMOVED*** Pattern/Practice (→ Priority P1)
...
***REMOVED******REMOVED******REMOVED*** Audit/Assessment (→ Priority P1)
...
***REMOVED******REMOVED******REMOVED*** Reconnaissance (→ Priority P1)
...
***REMOVED******REMOVED******REMOVED*** Reference/API (→ Priority P1-2)
