***REMOVED*** G2_RECON: Authorization & RBAC Security Audit
...
***REMOVED******REMOVED*** Part 1: PERCEPTION - Current RBAC Implementation

***REMOVED******REMOVED******REMOVED*** 1.1 Role Definitions (8 Roles)

The system defines **8 user roles** with hierarchical structure:

```python
***REMOVED*** From app/models/user.py and app/auth/permissions/models.py

Roles (by privilege level):
├─ ADMIN           ***REMOVED*** System administrator (full access)
├─ COORDINATOR     ***REMOVED*** Schedule manager
├─ FACULTY         ***REMOVED*** Faculty/attending physicians
├─ RESIDENT        ***REMOVED*** Resident physicians
├─ CLINICAL_STAFF  ***REMOVED*** Base clinical staff role
├─ RN              ***REMOVED*** Registered Nurse (inherits from CLINICAL_STAFF)
├─ LPN             ***REMOVED*** Licensed Practical Nurse (inherits from CLINICAL_STAFF)
└─ MSA             ***REMOVED*** Medical Support Assistant (inherits from CLINICAL_STAFF)
```

***REMOVED******REMOVED******REMOVED*** 1.2 Authentication Mechanism
...
```python
***REMOVED*** From app/core/security.py
...
***REMOVED******REMOVED******REMOVED*** 1.3 Permission Framework
...
- ABSENCE: {CREATE, READ}
- PERSON: {READ}  ***REMOVED*** Own profile only
```
...
***REMOVED******REMOVED******REMOVED*** 1.4 Endpoint Protection Coverage
...
***REMOVED*** General authentication
...
***REMOVED******REMOVED*** Part 2: INVESTIGATION - Permission Propagation

***REMOVED******REMOVED******REMOVED*** 2.1 Permission Inheritance Chain
...
**CRITICAL ISSUE ***REMOVED***1: Hierarchy Mismatch**
...
***REMOVED******REMOVED******REMOVED*** 2.2 Permission Propagation Flow
...
***REMOVED******REMOVED******REMOVED*** 2.3 Context-Aware Permission Logic

**Current Implementation (app/auth/access_matrix.py):**
```python
***REMOVED*** Only two context evaluators registered:
...
**Usage Pattern:**
```python
***REMOVED*** Residents can only update their own absence requests
...
**CRITICAL ISSUE ***REMOVED***2: Incomplete Context Evaluation**
...
***REMOVED******REMOVED******REMOVED*** 2.4 Permission Caching Strategy
...
**POTENTIAL ISSUE ***REMOVED***3: Stale Permission Cache**
...
***REMOVED******REMOVED*** Part 3: ARCANA - Role Hierarchy Design

***REMOVED******REMOVED******REMOVED*** 3.1 Role Hierarchy Correctness
...
**CRITICAL ISSUE ***REMOVED***4: Faculty Over-Privileged**
...
**CRITICAL ISSUE ***REMOVED***5: Resident Over-Privileged**
...
***REMOVED******REMOVED******REMOVED*** 3.2 Principle of Least Privilege Assessment
...
***REMOVED******REMOVED*** Part 4: HISTORY - RBAC Evolution

***REMOVED******REMOVED******REMOVED*** 4.1 Implementation Phases Detected
...
***REMOVED******REMOVED******REMOVED*** 4.2 Missing Evolution Markers
...
***REMOVED******REMOVED*** Part 5: INSIGHT - Least Privilege Principle Analysis

***REMOVED******REMOVED******REMOVED*** 5.1 Over-Privileged Roles

**FACULTY Role Problem:**
```python
***REMOVED*** From access_matrix.py line 369
UserRole.FACULTY: {
    ResourceType.PERSON: {PermissionAction.READ},  ***REMOVED*** ✓ Own profile
    ResourceType.SWAP_REQUEST: {
        PermissionAction.APPROVE,  ***REMOVED*** ✗ Can approve others' swaps!
        PermissionAction.REJECT,   ***REMOVED*** ✗ Should only approve own?
    }
}
```
...
```python
***REMOVED*** From resolver.py line 179
UserRole.COORDINATOR: {
    "person:delete",      ***REMOVED*** Can delete people!
    "person:create",      ***REMOVED*** Can create new identities
}
```
...
```python
***REMOVED*** From access_matrix.py line 459
UserRole.RESIDENT: {
    ResourceType.SWAP_REQUEST: {
        PermissionAction.APPROVE,  ***REMOVED*** ✗ Can approve other residents' swaps?
        PermissionAction.REJECT,   ***REMOVED*** ✗ This doesn't make sense
    }
}
```
...
***REMOVED******REMOVED******REMOVED*** 5.2 Under-Privileged Roles

**CLINICAL_STAFF (RN/LPN/MSA):**
```python
***REMOVED*** Only READ access to most resources
***REMOVED*** Cannot create absence requests? (they might need time off)
```
...
***REMOVED******REMOVED*** Part 6: RELIGION - All Endpoints Protected?

***REMOVED******REMOVED******REMOVED*** 6.1 Unprotected Endpoint Scan

**Public Endpoints (No Auth Required):**
```
POST /api/auth/login              ***REMOVED*** ✓ Correct - no auth needed
POST /api/auth/register           ***REMOVED*** ✓ Correct - public signup
POST /api/auth/refresh            ***REMOVED*** ✓ Correct - refresh flow
GET  /api/health                  ***REMOVED*** ✓ Correct - health check
GET  /api/docs                    ***REMOVED*** ? Depends on settings.DEBUG
GET  /api/openapi.json            ***REMOVED*** ? Depends on settings.DEBUG
```

**Protected Endpoints:**
```
GET  /api/people                  ***REMOVED*** Requires get_current_active_user
GET  /api/schedule                ***REMOVED*** Requires get_current_active_user
POST /api/assignments             ***REMOVED*** Requires get_current_active_user
```

***REMOVED******REMOVED******REMOVED*** 6.2 Protection Pattern Consistency
...
**CRITICAL ISSUE ***REMOVED***6: Inconsistent Protection Patterns**
...
@router.get("/data")
async def get_data(
    current_user: User = Depends(get_admin_user)  ***REMOVED*** Better
):
    ...
```
...
***REMOVED******REMOVED******REMOVED*** 6.3 Missing Permission Checks

**Endpoints using dependency but not verifying permissions:**

Found in testing code (test_rbac_authorization.py):
```python
***REMOVED*** Endpoint exists but doesn't enforce permission
response = client.get("/api/admin/users", headers=coordinator_headers)
***REMOVED*** If response.status_code != 403, permission check is missing!
```

---

***REMOVED******REMOVED*** Part 7: NATURE - Over-Granular Permissions?

***REMOVED******REMOVED******REMOVED*** 7.1 Permission Matrix Size
...
***REMOVED******REMOVED******REMOVED*** 7.2 Action Granularity

**Too Fine-Grained?**
```python
CREATE, READ, UPDATE, DELETE  ***REMOVED*** Standard CRUD
APPROVE, REJECT               ***REMOVED*** Workflow actions
EXECUTE                       ***REMOVED*** System actions
```

**Problem:** MANAGE action is supposed to represent "all CRUD" but:
```python
***REMOVED*** From access_matrix.py line 588
if (
...
***REMOVED******REMOVED******REMOVED*** 7.3 Resource Granularity

**Potential Redundancy:**
```python
ResourceType.SWAP          ***REMOVED*** Generic swap
ResourceType.SWAP_REQUEST  ***REMOVED*** Specific to requests
```
...
**Suggested Refactor:**
```python
***REMOVED*** Instead of separate resources:
...
***REMOVED******REMOVED*** Part 8: MEDICINE - Authorization Performance

***REMOVED******REMOVED******REMOVED*** 8.1 Permission Lookup Performance

**Current Implementation:**
```python
***REMOVED*** From access_matrix.py
def has_permission(...) -> bool:
    ***REMOVED*** 1. Convert strings to enums
    if isinstance(role, str):
        role = UserRole(role)  ***REMOVED*** O(1)

    ***REMOVED*** 2. Check static permissions
    role_permissions = self.PERMISSION_MATRIX.get(role, {})  ***REMOVED*** O(1)
    resource_permissions = role_permissions.get(resource, set())  ***REMOVED*** O(1)

    ***REMOVED*** 3. Check inherited permissions (worst case: 8 roles)
    inherited_roles = RoleHierarchy.get_inherited_roles(role)  ***REMOVED*** O(n)
    for inherited_role in inherited_roles:  ***REMOVED*** O(8)
        if self._check_static_permission(...):  ***REMOVED*** O(1)
            has_static_permission = True

    ***REMOVED*** 4. Audit log (always)
    self._audit_permission_check(...)
```

**Complexity:** O(n) where n = depth of role hierarchy (max 8)

***REMOVED******REMOVED******REMOVED*** 8.2 Resolver Caching Performance

**Redis Cache Hit Path:**
```python
***REMOVED*** From resolver.py
if use_cache:
    cached = await cache.get_role_permissions(role)  ***REMOVED*** O(1) Redis GET
    if cached is not None:
        return cached  ***REMOVED*** Cache hit!
```

**Cache Miss Path:**
```python
all_permissions = set()
hierarchy = self._resolve_role_hierarchy(role)  ***REMOVED*** O(n) graph traversal
for role in hierarchy:  ***REMOVED*** O(n)
    all_permissions.update(DEFAULT_ROLE_PERMISSIONS[role])  ***REMOVED*** O(m)
***REMOVED*** Total: O(n*m) where m = permissions per role
```

**Performance Assessment:** Good with caching, acceptable without

***REMOVED******REMOVED******REMOVED*** 8.3 Audit Logging Performance

**Current Audit Implementation:**
```python
self.audit_log.append(entry)  ***REMOVED*** In-memory list
logger.info(f"Permission check...")  ***REMOVED*** Synchronous logging
```
...
***REMOVED******REMOVED*** Part 9: SURVIVAL - Emergency Access Procedures

***REMOVED******REMOVED******REMOVED*** 9.1 Emergency Escalation Paths
...
***REMOVED******REMOVED******REMOVED*** 9.2 Missing Safeguards
...
***REMOVED******REMOVED******REMOVED*** 9.3 Recovery Procedures
...
def blacklist_token(
    ...
    ***REMOVED*** Can only blacklist one token at a time
    ***REMOVED*** No bulk operations!
```
...
***REMOVED******REMOVED*** Part 10: STEALTH - Privilege Escalation Paths

***REMOVED******REMOVED******REMOVED*** 10.1 Horizontal Escalation (Access other users' data)
...
```python
***REMOVED*** What if context doesn't include resource_owner_id?
context = PermissionContext(
    user_id=uuid4(),
    user_role=UserRole.FACULTY,
    ***REMOVED*** resource_owner_id NOT SET - defaults to None
)
***REMOVED*** is_own_resource() returns False (safe)
***REMOVED*** But what if UPDATE permission checking skips context entirely?
```

***REMOVED******REMOVED******REMOVED*** 10.2 Vertical Escalation (Promote to higher role)

**Attack Vector: User Object Mutation**

```python
***REMOVED*** If session is not properly managed:
user.role = "admin"
db.commit()  ***REMOVED*** Permissions now escalated!

***REMOVED*** Even better: JWT stores role at issue time
***REMOVED*** Change user.role in database
***REMOVED*** Old JWT still has old role (safe)
***REMOVED*** But new JWT from refresh might have new role!
```

**Current Safeguard:**
```python
***REMOVED*** From auth.py - role change requires new login
***REMOVED*** JWT is refreshed with new role
```

**Issue:** No explicit check preventing unauthorized role updates

***REMOVED******REMOVED******REMOVED*** 10.3 Lateral Escalation (Swap another user's permissions)
...
POST /api/swaps
{
    "resident_a_id": "attacker_id",  ***REMOVED*** Inject their own ID
    "resident_b_id": "target_id",
    "block_id": "..."
}

***REMOVED*** If API doesn't verify context:
***REMOVED*** Attacker can create fake swap with target resident
***REMOVED*** Depending on approval flow, might gain unauthorized shifts
```

**Mitigation Check (from context validation):**
```python
***REMOVED*** Should verify:
***REMOVED*** - Current user ID matches one of the swap participants
***REMOVED*** - Current user has authority to create swap
```

***REMOVED******REMOVED******REMOVED*** 10.4 Token Theft & Reuse
...
***REMOVED******REMOVED*** RBAC Matrix Analysis

***REMOVED******REMOVED******REMOVED*** Authorization Matrix (Current State)
...
***REMOVED******REMOVED******REMOVED*** Permission Gap Analysis
...
***REMOVED******REMOVED******REMOVED*** CRITICAL ISSUES (Exploit Possible)
...
***REMOVED******REMOVED******REMOVED*** HIGH PRIORITY ISSUES (Likely Exploitable)
...
***REMOVED******REMOVED******REMOVED*** Immediate Actions (This Sprint)

**R1: Fix Role Hierarchy Mismatch (P1)**
```python
***REMOVED*** backend/app/auth/permissions/models.py
***REMOVED*** Make both systems use same hierarchy
ROLE_HIERARCHY = {
    UserRole.ADMIN: [],
    UserRole.COORDINATOR: [UserRole.ADMIN],
    UserRole.FACULTY: [],  ***REMOVED*** NOT coordinator!
    UserRole.RESIDENT: [],  ***REMOVED*** NOT coordinator!
...
```python
***REMOVED*** backend/app/auth/access_matrix.py line 453
UserRole.RESIDENT: {
    ResourceType.SWAP_REQUEST: {
        ...
        ***REMOVED*** Remove: PermissionAction.APPROVE, PermissionAction.REJECT
    }
}
```
...
def _check_context_permission(...):
    if resource == ResourceType.SWAP_REQUEST:
        if action in {APPROVE, REJECT}:
            ***REMOVED*** Must be coordinator or supervisor of person
            return is_supervisor_of_person(context) or user_is_coordinator
```

**R4: Enforce Cache Invalidation (P2)**
```python
***REMOVED*** backend/app/auth/permissions/cache.py
...
***REMOVED******REMOVED******REMOVED*** Short-Term Actions (Next 2 Weeks)
...
```python
***REMOVED*** New table: permission_audit_log
***REMOVED*** Track: who changed what permission, when, why
***REMOVED*** Enable incident reconstruction
```

**R7: Implement Bulk Token Revocation (P2)**
```python
***REMOVED*** backend/app/models/token_blacklist.py
async def revoke_all_tokens_for_user(user_id: UUID):
    """Revoke all tokens for user (compromised account)."""
    ***REMOVED*** Update all tokens to blacklist
    ***REMOVED*** Add to queue for notification service
```

**R8: Add Session Context Binding (P2)**
```python
***REMOVED*** Store in JWT:
- ip_address (at login)
- device_fingerprint (user-agent)
- session_id (unique per login)

***REMOVED*** Validate on each request:
if request.ip_address != jwt.ip_address:
    log_anomaly()  ***REMOVED*** Might be token theft
```

***REMOVED******REMOVED******REMOVED*** Medium-Term Actions (This Quarter)
...
```python
***REMOVED*** New service: PrivilegeEscalationDetector
***REMOVED*** Alerts on:
...
```python
***REMOVED*** Operations requiring dual approval:
- User deletion
- Role escalation
- Permission changes
...
***REMOVED******REMOVED******REMOVED*** Current Test Coverage
...
***REMOVED******REMOVED******REMOVED*** Recommended New Tests

```python
***REMOVED*** backend/tests/auth/test_privilege_escalation.py

class TestPrivilegeEscalation:
    """Test privilege escalation prevention."""

    async def test_faculty_cannot_delete_person():
        ***REMOVED*** Verify FACULTY doesn't inherit COORDINATOR DELETE

    async def test_resident_cannot_approve_other_swaps():
        ***REMOVED*** Verify RESIDENT can't approve swap not involving them

    async def test_role_change_requires_reauth():
        ***REMOVED*** Verify old token doesn't have new permissions

    async def test_cache_invalidation_on_role_change():
        ***REMOVED*** Verify cached permissions cleared when role changes
```
...
***REMOVED******REMOVED*** Appendix: File References

***REMOVED******REMOVED******REMOVED*** Key Authorization Files
