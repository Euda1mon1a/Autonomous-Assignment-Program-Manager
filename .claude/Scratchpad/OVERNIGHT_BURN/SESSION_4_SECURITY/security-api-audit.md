***REMOVED*** API Security Audit Report
**Date:** 2025-12-30
...
***REMOVED******REMOVED*** 1. Rate Limiting Coverage

***REMOVED******REMOVED******REMOVED*** Current Implementation

***REMOVED******REMOVED******REMOVED******REMOVED*** Sliding Window Algorithm
...
```python
***REMOVED*** Core algorithm: removes old entries, counts current, adds new request
pipe.zremrangebyscore(key, 0, window_start)  ***REMOVED*** Remove outside window
pipe.zcard(key)                              ***REMOVED*** Count current
pipe.zadd(key, {str(current_time): current_time})  ***REMOVED*** Add request
pipe.expire(key, window_seconds + 10)        ***REMOVED*** Cleanup
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Token Bucket Algorithm
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Tiered Rate Limits
**File:** `backend/app/core/rate_limit_tiers.py`
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Endpoint-Specific Limits
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Account Lockout (Additional Layer)
...
***REMOVED******REMOVED******REMOVED*** Rate Limiting Coverage Analysis

***REMOVED******REMOVED******REMOVED******REMOVED*** Well-Protected Endpoints
**Authentication:**
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Coverage Gaps (AREAS OF CONCERN)
...
```
RATE_LIMIT_ENABLED=true              ***REMOVED*** Global switch
RATE_LIMIT_LOGIN_ATTEMPTS=5          ***REMOVED*** Per IP per minute
RATE_LIMIT_LOGIN_WINDOW=60           ***REMOVED*** Seconds
RATE_LIMIT_REGISTER_ATTEMPTS=3       ***REMOVED*** Per IP per minute
RATE_LIMIT_REGISTER_WINDOW=60        ***REMOVED*** Seconds
```

**Trusted Proxies:**
```
TRUSTED_PROXIES=[]                   ***REMOVED*** e.g., ["10.0.0.1", "10.0.0.2/32"]
```
...
***REMOVED******REMOVED*** 2. Authentication Requirements

***REMOVED******REMOVED******REMOVED*** JWT-Based Authentication
**File:** `backend/app/core/security.py`

***REMOVED******REMOVED******REMOVED******REMOVED*** Token Structure
```python
Access Token Claims:
...
```
ACCESS_TOKEN_EXPIRE_MINUTES=15      ***REMOVED*** Short-lived (prevents window if stolen)
REFRESH_TOKEN_EXPIRE_DAYS=7         ***REMOVED*** Longer-lived (allows session extension)
REFRESH_TOKEN_ROTATE=true           ***REMOVED*** Issue new token on each use
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Token Storage
```
...
3. **Type-Based Token Validation**
   ```python
   ***REMOVED*** In verify_token():
   if payload.get("type") == "refresh":
       return None  ***REMOVED*** REJECT refresh tokens used as access tokens

   ***REMOVED*** In verify_refresh_token():
   if payload.get("type") != "refresh":
       return None  ***REMOVED*** REJECT access tokens as refresh tokens
   ```
...
***REMOVED******REMOVED******REMOVED*** Authentication Coverage

***REMOVED******REMOVED******REMOVED******REMOVED*** Protected Endpoints (Sample)
```python
***REMOVED*** All of these require get_current_active_user dependency
- GET  /api/absences
- POST /api/absences
- GET  /api/assignments
- POST /api/people
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Public Endpoints (No Auth Required)
```python
***REMOVED*** In main.py (no dependency on get_current_active_user)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Partially Protected Endpoints
```python
***REMOVED*** oauth2_scheme has auto_error=False (allows unauthenticated)
...
WEAK_PASSWORDS = {
    ...
    "scheduler",  ***REMOVED*** Project-specific default
    "your-secret-key-change-in-production",  ***REMOVED*** From templates
}
```

---

***REMOVED******REMOVED*** 3. CORS Configuration

***REMOVED******REMOVED******REMOVED*** Current Configuration
**File:** `backend/app/main.py` (lines 302-318)

```python
cors_kwargs = {
    "allow_credentials": True,      ***REMOVED*** Allows cookies in cross-origin
    "allow_methods": ["*"],         ***REMOVED*** All HTTP methods
    "allow_headers": ["*"],         ***REMOVED*** All headers
}

***REMOVED*** Explicit origins (production-ready)
CORS_ORIGINS: list[str] = ["http://localhost:3000"]

***REMOVED*** Regex pattern for flexible matching
CORS_ORIGINS_REGEX: str = ""

***REMOVED*** Validation at startup
...
***REMOVED******REMOVED******REMOVED*** CORS Security Analysis

***REMOVED******REMOVED******REMOVED******REMOVED*** Strengths
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Validation Code
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Recommendations
...
***REMOVED******REMOVED*** 4. Security Headers

***REMOVED******REMOVED******REMOVED*** Implemented Headers
...
***REMOVED******REMOVED******REMOVED*** HSTS Configuration
- Only applied in production (DEBUG=false)
- Max-Age: 1 year (31536000 seconds)
- includeSubDomains: enabled
- Preload: disabled (can be enabled)

***REMOVED******REMOVED******REMOVED*** CSP for APIs
```
default-src 'none'          ***REMOVED*** Block all by default
frame-ancestors 'none'      ***REMOVED*** Prevent embedding
base-uri 'none'             ***REMOVED*** Block base URL changes
form-action 'none'          ***REMOVED*** Block form submissions
```

---

***REMOVED******REMOVED*** 5. Authorization & Access Control

***REMOVED******REMOVED******REMOVED*** Role-Based Access Control (RBAC)
**File:** `backend/app/core/security.py`

***REMOVED******REMOVED******REMOVED******REMOVED*** User Roles
```python
User.is_admin              ***REMOVED*** Flag for admin access
User.can_manage_schedules  ***REMOVED*** Flag for schedule management
User.is_active             ***REMOVED*** Active status check
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Role-Based Dependencies
```python
get_current_active_user()      ***REMOVED*** Any authenticated active user
get_admin_user()               ***REMOVED*** Admin-only access
get_scheduler_user()           ***REMOVED*** Schedule managers (admin or coordinator)
```
...
***REMOVED******REMOVED*** 6. Proxy & Forwarding Protection

***REMOVED******REMOVED******REMOVED*** X-Forwarded-For Validation
**File:** `backend/app/core/rate_limit.py` (lines 205-271)

***REMOVED******REMOVED******REMOVED******REMOVED*** Trusted Proxy Implementation
```python
def _is_trusted_proxy(ip: str) -> bool:
    """Check if IP is in TRUSTED_PROXIES list."""
    if not trusted_proxies:
        return False  ***REMOVED*** No proxies configured = no header trust

    ***REMOVED*** Supports both single IPs and CIDR notation
...
def get_client_ip(request: Request) -> str:
    ...
    ***REMOVED*** Only trust X-Forwarded-For if from trusted proxy
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Configuration
```
TRUSTED_PROXIES=[]  ***REMOVED*** Default: empty (all headers untrusted)
```

**Examples:**
```
TRUSTED_PROXIES=["10.0.0.1"]              ***REMOVED*** Single IP
TRUSTED_PROXIES=["10.0.0.0/8"]            ***REMOVED*** CIDR notation
TRUSTED_PROXIES=["10.0.0.1", "172.16.0.0/12"]  ***REMOVED*** Multiple
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Security Assessment
...
***REMOVED******REMOVED*** 7. Secret Management
...
def _validate_security_config() -> None:
    ...
    ***REMOVED*** Check WEBHOOK_SECRET (same validation)

    if errors:
        if not settings.DEBUG:
            raise ValueError("Security configuration errors...")  ***REMOVED*** Fail fast
        else:
            logger.warning("Insecure config in DEBUG mode")  ***REMOVED*** Warn in dev
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Secrets Configuration
```
SECRET_KEY                    ***REMOVED*** For JWT signing (64+ chars)
WEBHOOK_SECRET               ***REMOVED*** For webhook validation (64+ chars)
REDIS_PASSWORD               ***REMOVED*** For Redis auth
DB_PASSWORD                  ***REMOVED*** For database auth (via DATABASE_URL)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Generation Command
```bash
...
***REMOVED******REMOVED*** 8. Metrics Endpoint Protection
...
```python
INTERNAL_NETWORKS = [
    ip_network("127.0.0.0/8"),      ***REMOVED*** Localhost
    ip_network("10.0.0.0/8"),       ***REMOVED*** Private
    ip_network("172.16.0.0/12"),    ***REMOVED*** Private
    ip_network("192.168.0.0/16"),   ***REMOVED*** Private
]
...
***REMOVED******REMOVED*** 9. Error Handling & Information Leakage

***REMOVED******REMOVED******REMOVED*** Global Exception Handlers
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    logger.error("Unhandled exception...", exc_info=True)  ***REMOVED*** Log full error

    if settings.DEBUG:
        return {"detail": str(exc), "type": type(exc).__name__}  ***REMOVED*** Dev details
    else:
        return {"detail": "An internal error occurred. Please try again later."}  ***REMOVED*** Generic
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Best Practices Implemented
...
***REMOVED******REMOVED*** 10. API Routes Security Summary

***REMOVED******REMOVED******REMOVED*** Route Protection Patterns

***REMOVED******REMOVED******REMOVED******REMOVED*** Fully Protected Routes
```python
***REMOVED*** Require authentication + active user
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Partially Protected Routes
```python
***REMOVED*** Require authentication
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Public Routes
```python
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Metrics Routes
```python
***REMOVED*** Restricted by middleware (internal IPs only in production)
GET    /metrics                 ← Prometheus metrics
```

---

***REMOVED******REMOVED*** 11. Vulnerabilities & Recommendations

***REMOVED******REMOVED******REMOVED*** Critical Issues (Fix Immediately)
...
***REMOVED******REMOVED******REMOVED*** High Priority Issues
...
6. **Missing Authorization Checks**
   ...
   - **Fix:** Add route-level authorization:
     ```python
     ***REMOVED*** Check user_id matches current user or user is admin
     if assignment.person_id != current_user.id and not current_user.is_admin:
         raise HTTPException(403, "Forbidden")
     ```

***REMOVED******REMOVED******REMOVED*** Medium Priority Issues
...
7. **CORS Configuration Documentation**
   ...
   - **Fix:** Document in deployment guide:
     ```
     ***REMOVED*** Production: Set explicit origins
     CORS_ORIGINS=["https://app.hospital.org"]

     ***REMOVED*** Multiple subdomains:
...
8. **Trusted Proxies Documentation**
   ...
   - **Fix:** Document for common deployment scenarios:
     ...
     ***REMOVED*** AWS ALB
     TRUSTED_PROXIES=["10.0.0.0/8"]  ***REMOVED*** VPC CIDR
     ```
...
***REMOVED******REMOVED******REMOVED*** Low Priority Issues
...
***REMOVED******REMOVED*** 12. Testing & Validation
...
***REMOVED******REMOVED******REMOVED*** Recommended Test Coverage
```python
***REMOVED*** Add tests for:
...
***REMOVED******REMOVED*** 13. Compliance & Standards
...
***REMOVED******REMOVED******REMOVED*** Security Best Practices Implemented
...
***REMOVED******REMOVED*** 14. Deployment Checklist

***REMOVED******REMOVED******REMOVED*** Before Production Deployment
...
***REMOVED******REMOVED*** 15. Architecture Strengths
...
***REMOVED******REMOVED*** 16. Recommendations Summary

***REMOVED******REMOVED******REMOVED*** Immediate (Critical)
...
***REMOVED******REMOVED******REMOVED*** Long-term (Medium Priority)
...
***REMOVED******REMOVED*** 17. Files Reviewed
