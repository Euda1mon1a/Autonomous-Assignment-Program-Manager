***REMOVED*** Resilience Framework: Sacrifice Hierarchy (Triage Medicine Load Shedding)

***REMOVED******REMOVED*** Document Information
...
***REMOVED******REMOVED*** Triage Level Definitions

***REMOVED******REMOVED******REMOVED*** LoadSheddingLevel Enum (Severity Scale)

```python
class LoadSheddingLevel(IntEnum):
    """Load shedding severity levels (0-5 scale)."""

    NORMAL = 0          ***REMOVED*** No shedding - all activities running
    YELLOW = 1          ***REMOVED*** Suspend optional education only
    ORANGE = 2          ***REMOVED*** Also suspend admin and research
    RED = 3             ***REMOVED*** Also suspend core education
    BLACK = 4           ***REMOVED*** Essential services only (patient safety + ACGME minimum)
    CRITICAL = 5        ***REMOVED*** Patient safety only (absolute extreme)
```

***REMOVED******REMOVED******REMOVED*** Activity Category Hierarchy (Protection Priority)
...
class ActivityCategory(IntEnum):
    ...
    PATIENT_SAFETY = 1         ***REMOVED*** ICU coverage, OR staffing, trauma response
    ACGME_REQUIREMENTS = 2      ***REMOVED*** Minimum case numbers, required rotations
    CONTINUITY_OF_CARE = 3      ***REMOVED*** Clinic follow-ups, chronic disease management
    EDUCATION_CORE = 4          ***REMOVED*** Didactics, simulation labs
    RESEARCH = 5                ***REMOVED*** Studies, publications, grants
    ADMINISTRATION = 6          ***REMOVED*** Meetings, committees, paperwork
    EDUCATION_OPTIONAL = 7      ***REMOVED*** Conferences, electives, enrichment
```
...
***REMOVED******REMOVED*** Triage Decision Framework

***REMOVED******REMOVED******REMOVED*** Load Shedding Decision Tree
...
***REMOVED******REMOVED******REMOVED*** Priority Calculation Algorithm

```python
def calculate_required_level(
    current_capacity: float,      ***REMOVED*** Available faculty-hours
    required_capacity: float,      ***REMOVED*** Required faculty-hours for all activities
) -> LoadSheddingLevel:
    ...
    ***REMOVED*** Iterate through levels, finding first one where demand fits capacity
...
***REMOVED******REMOVED******REMOVED*** Activity Sacrifice Order (Within Category)
...
```python
***REMOVED*** Sorting logic from shed_load() method
sorted_activities = sorted(
    current_demand,
    key=lambda a: (
        a.category.value,        ***REMOVED*** Higher category = less protected
        not a.is_required,        ***REMOVED*** Non-required first
        a.faculty_hours,          ***REMOVED*** Smaller commitments first
    ),
    reverse=True,                 ***REMOVED*** So we pop from end (least protected first)
)
```

---

***REMOVED******REMOVED*** Core Data Structures

***REMOVED******REMOVED******REMOVED*** Activity Class
...
class Activity:
    ...
    id: UUID                        ***REMOVED*** Unique identifier
    name: str                       ***REMOVED*** Human-readable name (e.g., "Morning Clinic")
    category: ActivityCategory      ***REMOVED*** Priority tier
    faculty_hours: float            ***REMOVED*** Required faculty hours
    is_required: bool = False       ***REMOVED*** Can it be deferred?
    min_frequency: str | None       ***REMOVED*** e.g., "weekly", "monthly"
    can_be_deferred: bool = True    ***REMOVED*** Whether deferral is allowed
    deferral_limit_days: int = 30   ***REMOVED*** Max days it can be deferred
```

***REMOVED******REMOVED******REMOVED*** LoadSheddingStatus Class (Current State)
...
class LoadSheddingStatus:
    ...
    level: LoadSheddingLevel                ***REMOVED*** Current level (NORMAL, YELLOW, etc.)
    level_name: str                         ***REMOVED*** Human-readable name
    active_since: datetime | None           ***REMOVED*** When this level was activated
    activities_suspended: list[str]         ***REMOVED*** Names of suspended activities
    activities_protected: list[str]         ***REMOVED*** Names still running
    current_coverage: float                 ***REMOVED*** Faculty-hours allocated
    target_coverage: float                  ***REMOVED*** Faculty-hours needed
    decisions_log: list[SacrificeDecision]  ***REMOVED*** Audit trail (last 10)
```

***REMOVED******REMOVED******REMOVED*** SacrificeDecision Class (Audit Trail)
...
class SacrificeDecision:
    ...
    timestamp: datetime             ***REMOVED*** When decision was made
    level: LoadSheddingLevel        ***REMOVED*** What level was activated
    activities_suspended: list[str] ***REMOVED*** What was cut
    reason: str                     ***REMOVED*** Why this decision was made
    coverage_before: float          ***REMOVED*** Capacity before shedding
    coverage_after: float           ***REMOVED*** Capacity after shedding
    approved_by: str | None         ***REMOVED*** Who authorized this
    notes: str = ""                 ***REMOVED*** Additional context
```

---

***REMOVED******REMOVED*** Method Reference

***REMOVED******REMOVED******REMOVED*** Primary Methods

***REMOVED******REMOVED******REMOVED******REMOVED*** `shed_load()` - Main Load Shedding Function
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `activate_level()` - Escalate Triage Level
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `deactivate_level()` - Reduce Triage Level (Recovery)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `get_recovery_plan()` - Recovery Roadmap
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `calculate_required_level()` - Automatic Level Selection
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `get_status()` - Current State Snapshot
...
***REMOVED******REMOVED******REMOVED******REMOVED*** `get_category_summary()` - Category Breakdown
...
***REMOVED******REMOVED******REMOVED*** Design Philosophy: Removing Guilt and Politics
...
***REMOVED******REMOVED******REMOVED*** Key Ethical Constraints
...
***REMOVED******REMOVED*** Integration with Resilience Service

***REMOVED******REMOVED******REMOVED*** Connection to Defense in Depth

The Sacrifice Hierarchy is the **load shedding component** of Defense in Depth:

```python
***REMOVED*** From resilience/service.py
...
if severity == "minor":
    ...
    ***REMOVED*** Result: Suspend optional education
    ...
    ***REMOVED*** Result: Suspend admin, research, optional education
    ...
    ***REMOVED*** Result: Suspend all non-clinical education
    ...
    ***REMOVED*** Result: Essential services only
```

***REMOVED******REMOVED******REMOVED*** Usage in Crisis Response

```python
***REMOVED*** From resilience/service.py - activate_crisis_response()

def activate_crisis_response(
    self,
    severity: str = "moderate",  ***REMOVED*** "minor", "moderate", "severe", "critical"
    reason: str = "",
) -> dict:
    """Activate crisis response mode with automatic load shedding."""

    ***REMOVED*** Self-determines which triage level based on severity
    ***REMOVED*** Sacrifices activities accordingly
    ***REMOVED*** Logs all decisions
    ***REMOVED*** Returns recovery plan
```

---

***REMOVED******REMOVED*** Difficult Decisions Philosophy

***REMOVED******REMOVED******REMOVED*** The Nature of Triage
...
***REMOVED******REMOVED******REMOVED*** Uncomfortable Truths
...
***REMOVED******REMOVED******REMOVED*** Transparency as Ethical Requirement
...
***REMOVED******REMOVED*** Activation Procedures

***REMOVED******REMOVED******REMOVED*** Manual Activation (Administrator)

```python
hierarchy = SacrificeHierarchy()

***REMOVED*** Register activities
for activity in all_activities:
    hierarchy.register_activity(activity)

***REMOVED*** Activate a level (e.g., during flu outbreak)
hierarchy.activate_level(
    LoadSheddingLevel.ORANGE,
...
***REMOVED******REMOVED******REMOVED*** Automatic Activation (Resilience Service)

```python
***REMOVED*** From resilience/service.py
resilience_service = ResilienceService(db)

***REMOVED*** Service automatically determines level based on capacity/demand
...
***REMOVED*** Get recovery roadmap
recovery = resilience_service.sacrifice.get_recovery_plan()
```

***REMOVED******REMOVED******REMOVED*** Recovery Procedure

```python
***REMOVED*** After capacity returns (new faculty arrives)
hierarchy.deactivate_level(LoadSheddingLevel.NORMAL)

***REMOVED*** Services restored in reverse order:
***REMOVED*** 1. Optional education (highest priority, restored first)
***REMOVED*** 2. Administration
***REMOVED*** 3. Research
***REMOVED*** 4. Core education
***REMOVED*** 5. Everything else
```

---

***REMOVED******REMOVED*** Undocumented Patterns and Edge Cases

***REMOVED******REMOVED******REMOVED*** Hidden Complexity: Deferred vs. Sacrificed

The code distinguishes between activities that are **deferred** (can come back) vs. **sacrificed** (gone for this period):

```python
***REMOVED*** Each Activity has:
can_be_deferred: bool = True        ***REMOVED*** Can this be rescheduled?
deferral_limit_days: int = 30       ***REMOVED*** How long can we defer?
```

Conference attendance might be deferrable (move to next month). Emergency OR coverage is not deferrable.

***REMOVED******REMOVED******REMOVED*** Inter-Category Dependencies
...
***REMOVED******REMOVED******REMOVED*** Temporal Aspects
...
***REMOVED******REMOVED******REMOVED*** Category Conflicts
...
***REMOVED******REMOVED*** Implementation Status and Testing

***REMOVED******REMOVED******REMOVED*** Code Location
...
***REMOVED******REMOVED*** Cross-Disciplinary Foundation

***REMOVED******REMOVED******REMOVED*** Source Domain: Emergency Medicine Triage
...
***REMOVED******REMOVED******REMOVED*** Philosophical Foundation: Utilitarianism Under Constraint
...
***REMOVED******REMOVED******REMOVED*** Q: What if cutting optional education causes residents to fail boards?
...
***REMOVED******REMOVED******REMOVED*** Q: Can we add exceptions to the hierarchy?
...
***REMOVED******REMOVED******REMOVED*** Q: How long can we run at RED/BLACK level?
...
***REMOVED******REMOVED******REMOVED*** Q: What if resident wellbeing requires protecting research time?
...
***REMOVED******REMOVED******REMOVED*** Q: Can we use this to justify cutting resident safety training?
...
***REMOVED******REMOVED*** Recommendations for Enhancement

***REMOVED******REMOVED******REMOVED*** Tier 1: Immediate (High Priority)
...
***REMOVED******REMOVED******REMOVED*** Tier 2: Strategic (Medium Priority)
...
***REMOVED******REMOVED******REMOVED*** Tier 3: Advanced (Lower Priority)
