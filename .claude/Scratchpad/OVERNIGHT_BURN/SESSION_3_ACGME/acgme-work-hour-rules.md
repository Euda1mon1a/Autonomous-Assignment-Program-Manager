***REMOVED*** ACGME Work Hour Rules - Complete Implementation Inventory
...
***REMOVED******REMOVED*** PART 1: CORE ACGME RULES INVENTORY

***REMOVED******REMOVED******REMOVED*** Rule 1: 80-Hour Maximum Work Week
...
***REMOVED******REMOVED******REMOVED*** Rule 2: 1-in-7 Day Off Rule
...
For resident:
  ...
  FOR each adjacent pair of dates:
    ...
    ELSE:
      consecutive_count = 1  ***REMOVED*** Reset streak
...
***REMOVED******REMOVED******REMOVED*** Rule 3: Supervision Ratios
...
***REMOVED******REMOVED******REMOVED*** Rule 4: Availability/Absence Enforcement
...
***REMOVED******REMOVED*** PART 2: CODE LOCATION MAPPING

***REMOVED******REMOVED******REMOVED*** Primary Implementation Files

***REMOVED******REMOVED******REMOVED******REMOVED*** File 1: Core Constraint Classes
...
***REMOVED******REMOVED******REMOVED******REMOVED*** File 2: Backward Compatibility Layer
...
***REMOVED******REMOVED******REMOVED******REMOVED*** File 3: Database Validators
...
***REMOVED******REMOVED******REMOVED*** Solver Integration Points

***REMOVED******REMOVED******REMOVED******REMOVED*** OR-Tools CP-SAT Integration
...
***REMOVED******REMOVED******REMOVED******REMOVED*** PuLP Linear Programming Integration
...
***REMOVED******REMOVED******REMOVED*** Test Coverage Files

***REMOVED******REMOVED******REMOVED******REMOVED*** Primary Test File
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Scenario Tests
...
***REMOVED******REMOVED*** PART 3: IMPLEMENTATION DETAILS

***REMOVED******REMOVED******REMOVED*** 80-Hour Rule Implementation Details

***REMOVED******REMOVED******REMOVED******REMOVED*** Calculation Precision
```python
***REMOVED*** From EightyHourRuleConstraint.__init__()
max_blocks_per_window = (MAX_WEEKLY_HOURS * ROLLING_WEEKS) // HOURS_PER_BLOCK
***REMOVED*** = (80 * 4) // 6
***REMOVED*** = 320 // 6
***REMOVED*** = 53 blocks (integer division, not ceil)

***REMOVED*** Critical: This uses floor division
***REMOVED*** 53 blocks × 6 hours = 318 hours (leaves 2-hour margin)
***REMOVED*** 54 blocks × 6 hours = 324 hours (exceeds limit by 4 hours)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Rolling Window Implementation
```python
def _calculate_rolling_average(
    ...
    total_hours = sum(
        ...
        if window_start <= d <= window_end  ***REMOVED*** INCLUSIVE endpoints
    )

    return total_hours / self.ROLLING_WEEKS  ***REMOVED*** Always divide by 4
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Constraint Generation Strategy
```python
***REMOVED*** From add_to_cpsat():
...
***REMOVED*** This generates O(N_dates × N_residents) constraints
***REMOVED*** For 365-day schedule + 10 residents = ~3,650 constraints
```

---

***REMOVED******REMOVED******REMOVED*** 1-in-7 Rule Implementation Details

***REMOVED******REMOVED******REMOVED******REMOVED*** Simplified vs. Strict Interpretation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Consecutive Day Tracking
```python
...
FOR i in range(1, len(sorted_dates)):
    ...
    ELSE:
        consecutive = 1  ***REMOVED*** Reset on gap
...
***REMOVED******REMOVED******REMOVED*** Supervision Ratio Implementation Details

***REMOVED******REMOVED******REMOVED******REMOVED*** Fractional Load Calculation
```python
def calculate_required_faculty(self, pgy1_count: int, other_count: int) -> int:
    ***REMOVED*** Integer math: PGY-1 = 2 units, PGY-2/3 = 1 unit
    ***REMOVED*** (4 units = 1 faculty member)
    supervision_units = (pgy1_count * 2) + other_count
    return (supervision_units + 3) // 4 if supervision_units > 0 else 0

***REMOVED*** Equivalent to: ceil(supervision_units / 4)
***REMOVED*** (supervision_units + 3) // 4 rounds up correctly
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Per-Block Validation
```python
***REMOVED*** From validate():
...
***REMOVED******REMOVED*** PART 4: EDGE CASES & SPECIAL SCENARIOS

***REMOVED******REMOVED******REMOVED*** Edge Case 1: Midnight Window Crossing
...
***REMOVED******REMOVED******REMOVED*** Edge Case 2: Exactly 80 Hours
...
**Implementation:**
```python
IF average_weekly_hours > MAX_WEEKLY_HOURS:  ***REMOVED*** > 80, NOT >=
    VIOLATION

***REMOVED*** Therefore: 80.0 hours exactly = COMPLIANT ✓
***REMOVED*** 80.1 hours = VIOLATION ✗
```

**Floating Point Precision:**
```python
***REMOVED*** Example calculation:
total_hours = 320  ***REMOVED*** 80 hours/week * 4 weeks
average_weekly = 320 / 4  ***REMOVED*** = 80.0
IF 80.0 > 80:  ***REMOVED*** False, no violation
```

---

***REMOVED******REMOVED******REMOVED*** Edge Case 3: Partial Windows at Schedule Boundaries
...
```python
***REMOVED*** Checks EVERY possible 28-day window within data
***REMOVED*** If no complete 28-day window exists at end, no constraint added
***REMOVED*** Result: Last few days may appear to exceed 80-hour limit temporarily

***REMOVED*** This is ACCEPTABLE because:
***REMOVED*** 1. Next schedule block will re-validate
***REMOVED*** 2. Typically generates 365-day schedules (covers multiple 28-day windows)
```

---

***REMOVED******REMOVED******REMOVED*** Edge Case 4: Leap Year & 365/366 Day Schedules
...
**Current Implementation:**
```python
***REMOVED*** Uses Python's timedelta(days=28) which is calendar-aware
***REMOVED*** Feb 29 is treated like any other date
***REMOVED*** Leap year February = 29 days (correctly included)

***REMOVED*** Validation: For leap year:
***REMOVED*** Schedule period = Jan 1 to Dec 31 next year = 366 days
***REMOVED*** Includes extra day in Feb naturally
***REMOVED*** No special handling needed
```

---

***REMOVED******REMOVED******REMOVED*** Edge Case 5: Moonlighting Hours
...
```python
***REMOVED*** Database field exists: Person.moonlighting_hours
***REMOVED*** NOT currently included in 80-hour calculation
***REMOVED*** TODO: Integrate moonlighting hours into rolling average

***REMOVED*** Current limitation:
***REMOVED*** HOURS_PER_BLOCK = 6 (fixed for in-house duty)
***REMOVED*** Moonlighting must be added separately if tracked
```
...
***REMOVED******REMOVED******REMOVED*** Edge Case 6: Post-Call Day Restrictions (24+4 Rule)
...
```python
***REMOVED*** From validate_24_plus_4_rule():
***REMOVED*** Simplified check: If more than 2 blocks on same day -> violation flag
***REMOVED*** NOT a true 24+4 implementation (would need shift start/end times)

***REMOVED*** Database structure supports it:
***REMOVED*** Shift.start_time, Shift.end_time fields exist
***REMOVED*** But scheduler uses block-based (AM/PM) not hour-granular
```
...
***REMOVED******REMOVED******REMOVED*** Edge Case 7: Absence During Peak Period
...
**Implementation:**
```python
***REMOVED*** Availability enforcement = HARD CONSTRAINT
***REMOVED*** If resident.availability[block.id]["available"] == False
***REMOVED***   => assignment_var == 0 (forced)

***REMOVED*** No override or exception mechanism
***REMOVED*** Vacation blocking is absolute

***REMOVED*** Consequence:
***REMOVED*** Vacation period = unscheduled capacity gap
***REMOVED*** May cause other residents to exceed limits
```
...
***REMOVED******REMOVED******REMOVED*** Edge Case 8: Multiple Overlapping Constraints
...
**Implementation:**
```python
***REMOVED*** All constraints are HARD (high priority)
***REMOVED*** Solver uses constraint hierarchy:
***REMOVED*** 1. Availability (blocking absences) - CRITICAL
***REMOVED*** 2. Supervision (patient safety) - CRITICAL
***REMOVED*** 3. 80-Hour rule (ACGME) - CRITICAL
***REMOVED*** 4. 1-in-7 rule (ACGME) - CRITICAL
***REMOVED*** 5. Other operational constraints - HIGH/MEDIUM

***REMOVED*** If constraints are mutually infeasible:
***REMOVED*** Solver fails (no valid schedule exists)
***REMOVED*** Administrator must:
***REMOVED*** - Hire more faculty
***REMOVED*** - Adjust rotation template hours
***REMOVED*** - Extend schedule period
***REMOVED*** - Request ACGME exception
```
...
***REMOVED******REMOVED*** PART 5: COMPLIANCE VERIFICATION CHECKLIST

***REMOVED******REMOVED******REMOVED*** Pre-Deployment Validation

***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Static Code Analysis
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Unit Test Validation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Edge Case Coverage
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Database Validation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Performance Validation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** ✅ Compliance Documentation
...
***REMOVED******REMOVED*** PART 6: KNOWN LIMITATIONS & GAPS

***REMOVED******REMOVED******REMOVED*** Gap 1: Moonlighting Hours Not Integrated

**Status:** Partially implemented
**Severity:** HIGH (ACGME requirement not enforced)
**Required Fix:**
```python
***REMOVED*** In EightyHourRuleConstraint:
***REMOVED*** Add moonlighting_hours to total_hours calculation
total_hours = sum(hours_by_date[d]) + resident.moonlighting_hours
```

---

***REMOVED******REMOVED******REMOVED*** Gap 2: 24+4 Rule Not Enforced

**Status:** Documented, not enforced
**Severity:** MEDIUM (simplified block model is conservative approximation)
**Required Fix:**
```python
***REMOVED*** Need shift start/end time tracking
***REMOVED*** Current 6-hour blocks insufficient for minute-level precision
***REMOVED*** Alternative: Enforce via post-call day auto-assignment (implemented)
```

---

***REMOVED******REMOVED******REMOVED*** Gap 3: No Backup Call Counting
...
***REMOVED******REMOVED******REMOVED*** Gap 4: Strategic Patient Care Exception
...
***REMOVED******REMOVED******REMOVED*** Gap 5: Handoff Time Precision
...
***REMOVED******REMOVED*** PART 7: REGULATORY EVOLUTION & CONTEXT

***REMOVED******REMOVED******REMOVED*** ACGME Rule History
...
***REMOVED******REMOVED******REMOVED*** PGY-1 Specific Restrictions (Not Fully Implemented)
...
***REMOVED******REMOVED*** SUMMARY TABLE: RULES VS. IMPLEMENTATION STATUS
...
***REMOVED******REMOVED******REMOVED*** Code Files (Absolute Paths)
...
***REMOVED******REMOVED******REMOVED*** Official ACGME References
