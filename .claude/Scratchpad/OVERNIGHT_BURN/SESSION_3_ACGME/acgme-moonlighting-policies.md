***REMOVED*** ACGME Moonlighting Policies: Comprehensive Reference
...
***REMOVED******REMOVED*** 1. MOONLIGHTING CATEGORIES: CLASSIFICATION FRAMEWORK

***REMOVED******REMOVED******REMOVED*** 1.1 Primary Category: Location + Employment Status

***REMOVED******REMOVED******REMOVED******REMOVED*** A. Internal Moonlighting (Institution-Based)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** B. External Moonlighting (Off-Campus)
...
***REMOVED******REMOVED******REMOVED*** 1.2 Secondary Category: Educational Value

***REMOVED******REMOVED******REMOVED******REMOVED*** High-Educational Moonlighting
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Medium-Educational Moonlighting
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Low/Exploitative Moonlighting
...
***REMOVED******REMOVED******REMOVED*** 1.3 Tertiary Category: Reporting Status

***REMOVED******REMOVED******REMOVED******REMOVED*** Reported/Approved Moonlighting
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Informally-Approved Moonlighting
...
***REMOVED******REMOVED*** 2. HOUR COUNTING REQUIREMENTS

***REMOVED******REMOVED******REMOVED*** 2.1 Included in the 80-Hour Maximum
...
***REMOVED******REMOVED******REMOVED*** 2.2 NOT Included in the 80-Hour Maximum
...
```python
***REMOVED*** From backend/app/validators/advanced_acgme.py
def validate_moonlighting_hours(
    person_id: str,
    start_date: date,
    end_date: date,
    external_hours: float = 0.0,  ***REMOVED*** Moonlighting hours
) -> list[Violation]:
    """
    ...
    """
    ***REMOVED*** Calculate internal duty hours from assignments
    internal_hours = len(assignments) * HOURS_PER_HALF_DAY  ***REMOVED*** 6 hours/shift

    ***REMOVED*** Total = internal program hours + external moonlighting
    total_hours = internal_hours + external_hours

    ***REMOVED*** Check weekly average
    weeks = max(1, (end_date - start_date).days / 7)
    avg_weekly_hours = total_hours / weeks

    if avg_weekly_hours > MAX_MOONLIGHTING_HOURS_PER_WEEK:  ***REMOVED*** 80
        ***REMOVED*** VIOLATION
```
...
***REMOVED******REMOVED******REMOVED*** 2.4 Tracking Granularity: What Gets Logged?
...
***REMOVED******REMOVED*** 3. APPROVAL WORKFLOWS: Authorization & Governance

***REMOVED******REMOVED******REMOVED*** 3.1 Standard Approval Process (Recommended)
...
***REMOVED******REMOVED******REMOVED*** 3.2 Approval Governance Model
...
def should_approve_moonlighting_request(request) -> bool:
    ***REMOVED*** Calculate impact
    ...
    if weeks_ahead > 80:
        return False  ***REMOVED*** DENY - would violate 80-hour rule

    if weeks_ahead > 75:
        ***REMOVED*** WARNING - check educational value
        if request.educational_value == "HIGH":
            return True  ***REMOVED*** Conditional approval
        else:
            return False  ***REMOVED*** DENY - no educational justification

    if weeks_ahead <= 75:
        return True  ***REMOVED*** APPROVE

    return False  ***REMOVED*** Default deny
```

---

***REMOVED******REMOVED******REMOVED*** 3.3 Approval Documentation Requirements
...
***REMOVED******REMOVED*** 4. COMPLIANCE MONITORING: Real-Time Detection

***REMOVED******REMOVED******REMOVED*** 4.1 Monitoring Architecture
...
***REMOVED******REMOVED******REMOVED*** 4.2 Validation Logic Implementation
...
def validate_moonlighting_hours(
    ...
    if not person.is_resident:
        return []  ***REMOVED*** Only validate residents

    ***REMOVED*** Step 1: Get internal program hours
    assignments = query_assignments(person_id, start_date, end_date)
    internal_hours = len(assignments) * HOURS_PER_HALF_DAY  ***REMOVED*** 6 hours/shift

    ***REMOVED*** Step 2: Add external moonlighting hours
    total_hours = internal_hours + external_hours

    ***REMOVED*** Step 3: Calculate weekly average
    days_in_period = (end_date - start_date).days
    weeks = max(1, days_in_period / 7)
    avg_weekly_hours = total_hours / weeks

    ***REMOVED*** Step 4: Check against limit
    if avg_weekly_hours > MAX_MOONLIGHTING_HOURS_PER_WEEK:  ***REMOVED*** 80
        return [Violation(
...
***REMOVED******REMOVED******REMOVED*** 4.3 Monitoring Thresholds & Escalation
...
def classify_compliance_status(avg_weekly_hours: float) -> str:
    if avg_weekly_hours > 85:
        return "BLACK"  ***REMOVED*** Escalate
    elif avg_weekly_hours > 80:
        return "RED"    ***REMOVED*** Violation
    elif avg_weekly_hours > 78:
        return "ORANGE" ***REMOVED*** Urgent
    elif avg_weekly_hours > 75:
        return "YELLOW" ***REMOVED*** Warning
    else:
        return "GREEN"  ***REMOVED*** Compliant
```

---

***REMOVED******REMOVED*** 5. INTERNAL vs. EXTERNAL MOONLIGHTING: Key Distinctions

***REMOVED******REMOVED******REMOVED*** 5.1 Internal Moonlighting (Institutional Control)
...
**Tracking:**
```python
***REMOVED*** Internal = assignments in same institution
internal_hours = sum(
...
***REMOVED******REMOVED******REMOVED*** 5.2 External Moonlighting (External Oversight)
...
**Tracking:**
```python
***REMOVED*** External = hours reported from non-primary facilities
external_hours = resident_self_reported_external_hours
***REMOVED*** RISK: Depends on resident honesty!
```
...
***REMOVED******REMOVED******REMOVED*** 5.3 Comparison Matrix
...
***REMOVED******REMOVED*** 6. COMPLIANCE MONITORING: Institutional Policies

***REMOVED******REMOVED******REMOVED*** 6.1 Recommended Core Policy Statements
...
***REMOVED******REMOVED******REMOVED*** 6.2 Approval Request Form (Recommended)
...
***REMOVED******REMOVED*** 7. OPERATIONAL RISK ZONES: Where Things Go Wrong

***REMOVED******REMOVED******REMOVED*** 7.1 High-Risk Scenario: Unreported External Moonlighting
...
***REMOVED******REMOVED******REMOVED*** 7.2 Medium-Risk Scenario: Informal Internal Moonlighting
...
***REMOVED******REMOVED******REMOVED*** 7.3 Critical Risk: Compensatory Moonlighting
...
***REMOVED******REMOVED******REMOVED*** 7.4 Regulatory Risk: External Facility Non-Compliance
...
***REMOVED******REMOVED*** 8. IMPLEMENTATION ROADMAP: Codebase Integration

***REMOVED******REMOVED******REMOVED*** 8.1 Current State Analysis
...
***REMOVED******REMOVED******REMOVED*** 8.2 Proposed Implementation Phases
...
class MoonlightingApproval(Base):
    ...
    facility_name = Column(String(255))  ***REMOVED*** Where will work
    location_type = Column(String(50))   ***REMOVED*** "internal" or "external"
    ...
    status = Column(String(50))  ***REMOVED*** "pending", "approved", "denied", "expired"
...
```python
***REMOVED*** Extend validator with alerting
...
```python
***REMOVED*** Quarterly compliance report
class MoonlightingComplianceReport:
    """Quarterly PD-facing report"""

    residents_moonlighting: int  ***REMOVED*** How many actively
    total_moonlighting_hours: float  ***REMOVED*** Aggregate
    hours_by_type: dict  ***REMOVED*** internal vs external
    violations: list  ***REMOVED*** >80h cases
    at_risk: list  ***REMOVED*** 75-80h cases
    trends: dict  ***REMOVED*** increasing/decreasing
```

---

***REMOVED******REMOVED******REMOVED*** 8.3 API Endpoints (Proposed)

```python
***REMOVED*** POST /api/v1/moonlighting/approvals
***REMOVED*** Submit moonlighting request
request = {
...
***REMOVED*** GET /api/v1/moonlighting/approvals/{approval_id}
***REMOVED*** View approval status

***REMOVED*** POST /api/v1/moonlighting/hour-logs
***REMOVED*** Submit weekly hours
log = {
    "approval_id": "<uuid>",
...
***REMOVED*** GET /api/v1/moonlighting/compliance/{resident_id}
***REMOVED*** Check resident's current moonlighting status
response = {
    "status": "YELLOW",
...
***REMOVED*** GET /api/v1/reports/moonlighting-quarterly
***REMOVED*** PD-facing quarterly report
```

---

***REMOVED******REMOVED*** 9. ACGME AUDIT CHECKLIST: What Auditors Ask
...
***REMOVED******REMOVED*** 10. SUMMARY: Key Implementation Requirements
...
***REMOVED******REMOVED*** 11. MILITARY MEDICAL CONTEXT: Special Considerations

***REMOVED******REMOVED******REMOVED*** 11.1 TDY/Deployment Recovery
...
***REMOVED******REMOVED******REMOVED*** 11.2 Locum Tenens at Other Military Facilities
...
***REMOVED******REMOVED******REMOVED*** 11.3 Humanitarian/Deployment Moonlighting
...
***REMOVED******REMOVED******REMOVED*** Appendix A: Regulatory References
...
***REMOVED******REMOVED******REMOVED*** Appendix B: Related ACGME Rules (Context)
...
***REMOVED******REMOVED******REMOVED*** Appendix C: Financial Best Practices
...
***REMOVED******REMOVED******REMOVED*** Appendix D: Fatigue Screening Questions
```
