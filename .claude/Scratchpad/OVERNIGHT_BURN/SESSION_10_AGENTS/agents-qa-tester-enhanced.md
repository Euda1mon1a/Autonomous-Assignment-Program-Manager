***REMOVED*** QA_TESTER Agent - Enhanced Specification
...
1. [Testing Infrastructure Overview](***REMOVED***testing-infrastructure-overview)
2. [Backend Testing (pytest)](***REMOVED***backend-testing-pytest)
3. [Frontend Testing (Jest)](***REMOVED***frontend-testing-jest)
4. [Integration Testing](***REMOVED***integration-testing)
5. [Performance Testing](***REMOVED***performance-testing)
6. [Edge Case Discovery Framework](***REMOVED***edge-case-discovery-framework)
7. [Test Patterns & Best Practices](***REMOVED***test-patterns--best-practices)
8. [Coverage Targets & Metrics](***REMOVED***coverage-targets--metrics)
9. [Fixture & Factory Strategies](***REMOVED***fixture--factory-strategies)
10. [Quality Gates & Escalation](***REMOVED***quality-gates--escalation)
11. [Advanced Workflows](***REMOVED***advanced-workflows)

---

***REMOVED******REMOVED*** Testing Infrastructure Overview
...
backend/
├── tests/
│   ├── conftest.py               ***REMOVED*** Global fixtures & configuration
│   ├── factories.py              ***REMOVED*** FactoryBoy test data generators
│   ├── pytest.ini                ***REMOVED*** pytest configuration
│   ├── unit/                     ***REMOVED*** Unit tests by domain
│   │   └── services/
│   │       └── test_constraint_service.py
│   ├── integration/              ***REMOVED*** Multi-component workflows
│   │   ├── conftest.py          ***REMOVED*** Integration-specific fixtures
│   │   ├── test_scheduling_flow.py
│   │   ├── test_swap_workflow.py
│   │   └── test_resilience_integration.py
│   ├── performance/              ***REMOVED*** Load & stress testing
│   │   ├── conftest.py          ***REMOVED*** Performance fixtures
│   │   ├── test_acgme_load.py
...
│   │   └── test_spin_glass.py
│   ├── validators/               ***REMOVED*** ACGME validator tests
│   │   └── test_acgme_comprehensive.py
│   ├── resilience/               ***REMOVED*** Resilience framework tests
│   │   └── test_resilience_integration.py
│   └── [80+ other test files]    ***REMOVED*** Routes, auth, notifications, etc.

frontend/
├── jest.config.js                ***REMOVED*** Jest configuration
├── tsconfig.jest.json            ***REMOVED*** TypeScript Jest config
├── __tests__/
│   ├── setup.ts                  ***REMOVED*** Test environment setup
│   ├── features/
│   │   ├── analytics/            ***REMOVED*** Analytics component tests
│   │   ├── audit/                ***REMOVED*** Audit log tests
│   │   └── swap-marketplace/     ***REMOVED*** Swap feature tests
│   └── pages/
│       └── settings.test.tsx

load-tests/
├── scenarios/                     ***REMOVED*** k6 load test scenarios
│   ├── api-baseline.js
│   ├── concurrent-users.js
│   ├── schedule-generation.js
│   └── rate-limit-attack.js
└── jest.config.js                ***REMOVED*** k6 test configuration
```
...
***REMOVED******REMOVED*** Backend Testing (pytest)
...
***REMOVED*** Asyncio mode for async tests
asyncio_mode = auto
```

***REMOVED******REMOVED******REMOVED*** Global Fixtures (`conftest.py`)

The main test configuration provides 13+ pytest fixtures:

***REMOVED******REMOVED******REMOVED******REMOVED*** Database Fixtures

```python
@pytest.fixture(scope="function")
def db() -> Generator[Session, None, None]:
    """Create fresh database for each test."""
    ***REMOVED*** Creates all tables, yields session, drops all tables
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Authentication Fixtures
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Data Fixtures
...
***REMOVED******REMOVED******REMOVED*** Running pytest

***REMOVED******REMOVED******REMOVED******REMOVED*** Common Commands
...
***REMOVED*** Run specific test function
pytest tests/test_swap_executor.py::TestSwapExecutor::test_execute_one_to_one_swap

***REMOVED*** Run by marker
pytest -m acgme                    ***REMOVED*** Only ACGME compliance tests
pytest -m unit                     ***REMOVED*** Only unit tests
pytest -m integration              ***REMOVED*** Only integration tests
pytest -m "not slow"               ***REMOVED*** Exclude slow tests

***REMOVED*** Run with coverage
pytest --cov=app --cov-report=html

***REMOVED*** Run with detailed output
pytest -v -s                       ***REMOVED*** Verbose + show print statements

***REMOVED*** Run with timing
pytest --durations=10              ***REMOVED*** Show 10 slowest tests

***REMOVED*** Run in parallel (install pytest-xdist)
pytest -n auto                     ***REMOVED*** Use all CPU cores
```

***REMOVED******REMOVED******REMOVED******REMOVED*** Coverage Reports

```bash
***REMOVED*** Generate coverage report
pytest --cov=app --cov-report=html

***REMOVED*** View coverage by module
pytest --cov=app --cov-report=term-missing

***REMOVED*** Check specific module coverage
pytest --cov=app.services --cov-report=html tests/services/
```

***REMOVED******REMOVED******REMOVED*** Test Pattern Examples

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 1: Unit Test with Fixtures
...
class TestSwapValidator:
    ...
    def test_validate_one_to_one_swap_success(
        ...
        ***REMOVED*** Setup: Create assignments
    ...
    def test_validate_swap_fails_with_acgme_violation(
        ...
        ***REMOVED*** Setup: Create assignments that would exceed 80-hour rule
        for _ in range(20):  ***REMOVED*** Create many assignments
            create_assignment(
                db, person=sample_resident, date=date.today()
            )

        ***REMOVED*** Execute: Try to validate additional swap
        result = validator.validate_swap(
            initiator_id=sample_resident.id,
            partner_id=uuid4(),
            initiator_date=date.today(),
        ...
        ***REMOVED*** Assert: Swap is invalid due to ACGME violation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 2: Integration Test with API Client
...
class TestSwapWorkflow:
    ...
    def test_swap_request_creation_and_execution(
        ...
        ***REMOVED*** Step 1: Create test data
        ...
        ***REMOVED*** Step 2: Create swap request via API
        swap_request = {
        ...
        ***REMOVED*** Step 3: Verify swap request created
        assert response.status_code == 201
        swap_id = response.json()["id"]

        ***REMOVED*** Step 4: Approve swap
        response = client.post(
        ...
        ***REMOVED*** Step 5: Verify swap executed
        response = client.get(f"/api/swaps/{swap_id}", headers=auth_headers)
        assert response.json()["status"] == "completed"

        ***REMOVED*** Step 6: Verify assignments swapped
        response = client.get(
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 3: Parametrized Test
...
***REMOVED*** Test data: (hours, expected_valid, description)
...
class TestACGME80HourRule:
    ...
    def test_80_hour_rule_validation(
        ...
        ***REMOVED*** Create assignments totaling 'hours' hours
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 4: Async Test
...
***REMOVED******REMOVED*** Frontend Testing (Jest)
...
***REMOVED******REMOVED******REMOVED*** Running Jest
...
***REMOVED*** Run with different environment
npm test -- --env=jsdom
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 1: Component Unit Test
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern 2: Hook Test
...
integration/
├── conftest.py                    ***REMOVED*** Integration-specific fixtures
├── test_scheduling_flow.py        ***REMOVED*** Schedule generation workflow
├── test_swap_workflow.py          ***REMOVED*** Swap request workflow
├── test_leave_workflow.py         ***REMOVED*** Leave request workflow
├── test_fmit_swap_workflow.py     ***REMOVED*** FMIT rotation swaps
└── test_resilience_integration.py ***REMOVED*** Resilience features
```

***REMOVED******REMOVED******REMOVED*** Integration Test Example
...
class TestSwapWorkflowIntegration:
    ...
    def test_swap_workflow_complete(
        ...
        """
        ***REMOVED*** Setup: Create residents
        ...
        ***REMOVED*** Verify assignments swapped
        response = client.get(
        ...
        ***REMOVED*** Verify ACGME still compliant
        response = client.get(
...
***REMOVED******REMOVED******REMOVED*** Configuration & Fixtures
...
***REMOVED******REMOVED******REMOVED*** Performance Test Example
...
class TestACGMEPerformance:
    ...
    def test_concurrent_validation(self, db, large_resident_dataset):
        ...
        ***REMOVED*** All results should be identical (consistent validation)
        assert all(r == results[0] for r in results)

        ***REMOVED*** Should complete in reasonable time
...
***REMOVED*** Run standard load test (50 VUs, 5 minutes)
npm run test:load

***REMOVED*** Run stress test (200 VUs, 10 minutes)
npm run test:stress

***REMOVED*** Run custom scenario
k6 run --vus 100 --duration 5m scenarios/schedule-generation.js
```

---

***REMOVED******REMOVED*** Edge Case Discovery Framework

***REMOVED******REMOVED******REMOVED*** Comprehensive Edge Case Catalog

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Temporal Edge Cases
...
class TestTemporalEdgeCases:
    ...
    def test_acgme_window_boundaries(
        ...
        ***REMOVED*** Create assignments up to target date
        for i in range(4 * 7):  ***REMOVED*** 4 weeks
            create_assignment(
                db,
                person=resident,
        ...
        ***REMOVED*** Verify validation at boundary
    ...
    def test_daylight_saving_time_transition(self, db):
        ...
        ***REMOVED*** Fall back: 2:00 AM -> 1:00 AM (gain 1 hour)
        ***REMOVED*** Spring forward: 2:00 AM -> 3:00 AM (lose 1 hour)

        dst_transition = datetime(2025, 3, 9, 2, 0)  ***REMOVED*** Spring forward
        ...
        ***REMOVED*** Duration calculation should handle DST correctly
        expected_hours = 11.0  ***REMOVED*** Not 12 (lose 1 hour)
        assert assignment.duration_hours == expected_hours
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Concurrency Edge Cases
...
class TestConcurrencyEdgeCases:
    ...
    def test_simultaneous_swap_same_resident(self, db):
        ...
        ***REMOVED*** Both swaps should succeed (different assignments)
    ...
    def test_race_condition_schedule_generation(self, db):
        ...
        ***REMOVED*** All schedules should be valid
        for schedule in schedules:
            assert validate_schedule(schedule)

        ***REMOVED*** Database should be consistent
        assert verify_database_integrity(db)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Data Edge Cases
...
class TestDataEdgeCases:
    ...
    @pytest.mark.parametrize("value", [
        ...
        "",
        "   ",
        "a" * 1000,  ***REMOVED*** Very long string
        "\n\t\r",    ***REMOVED*** Whitespace only
        "'; DROP TABLE--",  ***REMOVED*** SQL injection attempt
        "<script>alert('xss')</script>",  ***REMOVED*** XSS attempt
    ])
    def test_resident_name_edge_cases(self, db, value):
        ...
        try:
            ...
            ***REMOVED*** If created, verify it's sanitized correctly
            ...
            ***REMOVED*** Some values might be rejected, which is acceptable
            pass
    ...
    def test_zero_and_negative_hours(self, db):
        ...
        ***REMOVED*** Zero hours should be valid (resident off)
        assignment = create_assignment(db, person=resident, hours=0.0)
        assert validate_assignment(assignment)

        ***REMOVED*** Negative hours should fail validation
    ...
    def test_empty_schedule(self, db):
        ...
        ***REMOVED*** No residents, no assignments
        assert len(db.query(Person).all()) == 0

        result = validate_schedule()
        assert result.is_valid
        assert result.warnings  ***REMOVED*** Should warn about empty schedule
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 4. Business Logic Edge Cases
...
class TestBusinessLogicEdgeCases:
    ...
    def test_exactly_80_hours_in_rolling_window(self, db):
        ...
        ***REMOVED*** Create assignments totaling exactly 80 hours
        for i in range(8):  ***REMOVED*** 8 * 10 = 80
            create_assignment(
                db,
                person=resident,
        ...
        ***REMOVED*** Should be valid (exactly at limit)
        result = validator.validate_80_hour_rule(resident.id)
        assert result.is_valid
    ...
    def test_swap_into_own_shift(self, db):
        ...
        ***REMOVED*** Try to swap with themselves (no-op)
        result = validate_swap(
            initiator_id=resident.id,
            partner_id=resident.id,  ***REMOVED*** Same person
            initiator_assignment_id=assignment.id,
            partner_assignment_id=assignment.id,  ***REMOVED*** Same assignment
        )
    ...
    def test_credential_expires_during_assignment(self, db):
        ...
        ***REMOVED*** Credential expires on assignment date
        credential = create_credential(
            db,
            person=resident,
            type="BLS",
            expires_at=date(2025, 12, 15)  ***REMOVED*** Same day
        )

        ***REMOVED*** Should be invalid (credential expired on that date)
...
***REMOVED******REMOVED*** Test Patterns & Best Practices

***REMOVED******REMOVED******REMOVED*** Pattern Library

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern A: Fixture Composition
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern B: Factory Pattern
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern C: Mocking External Services
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern D: Parametrized Tests

```python
"""Test multiple scenarios with single test."""

@pytest.mark.parametrize("pgy,expected_ratio", [
    (1, 2),    ***REMOVED*** PGY-1 needs 1:2 supervision
    (2, 4),    ***REMOVED*** PGY-2 needs 1:4 supervision
    (3, 4),    ***REMOVED*** PGY-3 needs 1:4 supervision
])
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern E: Context Manager Testing
...
def test_transaction_rollback(db):
    ...
    try:
        with db.begin_nested():
            ...
            ***REMOVED*** Force an error
            raise ValueError("Test error")
    except ValueError:
        pass

    ***REMOVED*** Assignment should be rolled back
...
```python
***REMOVED*** ANTI-PATTERN 1: Hidden test dependencies
...
***REMOVED*** BETTER: Use fixtures
def test_resident_creation(db):
    """Clear dependency on fixture."""
    resident = create_resident(db)
    assert resident is not None

***REMOVED*** ANTI-PATTERN 2: Test too large/unfocused
def test_everything():
    """Tests multiple unrelated things."""
    resident = create_resident(db)
    assignment = create_assignment(db, person=resident)
    faculty = create_faculty(db)
    ***REMOVED*** Test 10 different things...

***REMOVED*** BETTER: Focus on one thing
def test_resident_creation(db):
    """Test only resident creation."""
    resident = create_resident(db)
    assert resident.id is not None

***REMOVED*** ANTI-PATTERN 3: Test assertions on strings
...
***REMOVED*** BETTER: Assert on error type and pattern
def test_error_type():
...
***REMOVED******REMOVED*** Coverage Targets & Metrics

***REMOVED******REMOVED******REMOVED*** Target Coverage
...
***REMOVED******REMOVED******REMOVED*** Measuring Coverage

```bash
***REMOVED*** Generate coverage report
pytest --cov=app --cov-report=html

***REMOVED*** View coverage by file
pytest --cov=app --cov-report=term-missing | head -30

***REMOVED*** Check specific module
pytest --cov=app.scheduling --cov-report=html tests/scheduling/

***REMOVED*** Generate badge
...
1. **Identify untested paths:**
   ```bash
   pytest --cov=app --cov-report=html
   ***REMOVED*** Open htmlcov/index.html and look for red lines
   ```
...
2. **Write targeted tests:**
   ...
   def test_error_handling_in_xyz():
       ...
       ***REMOVED*** Test the specific scenario
   ```
...
***REMOVED******REMOVED*** Fixture & Factory Strategies

***REMOVED******REMOVED******REMOVED*** Fixture Scope

```python
***REMOVED*** Function scope (default) - Fresh for each test
@pytest.fixture(scope="function")
def resident(db):
    return create_resident(db)

***REMOVED*** Module scope - Reused across all tests in module
@pytest.fixture(scope="module")
def large_dataset():
    return create_1000_residents()

***REMOVED*** Session scope - Reused across all tests
@pytest.fixture(scope="session")
def db_url():
    return "postgresql://localhost/test_db"
```

***REMOVED******REMOVED******REMOVED*** Fixture Best Practices

```python
***REMOVED*** GOOD: Fixture names describe what they create
@pytest.fixture
...
***REMOVED*** GOOD: Fixtures are composable
@pytest.fixture
...
***REMOVED*** GOOD: Use cleanup for side effects
@pytest.fixture
def temporary_file():
    """Fixture that cleans up after itself."""
    path = "/tmp/test_schedule.json"
    yield path
    os.remove(path)  ***REMOVED*** Cleanup after test

***REMOVED*** BAD: Unclear fixture names
@pytest.fixture
def data(db):  ***REMOVED*** What data?
    return create_something(db)

***REMOVED*** BAD: Fixtures with side effects
@pytest.fixture
def send_email(db):  ***REMOVED*** Fixtures shouldn't send things
    send_notification()  ***REMOVED*** This runs for every test
    return
```

***REMOVED******REMOVED******REMOVED*** FactoryBoy Patterns
...
class ResidentFactory(factory.Factory):
    ...
    ***REMOVED*** Basic attributes
    id = factory.LazyFunction(uuid4)
    name = factory.Faker("name")

    ***REMOVED*** Traits for variants
    class Params:
        high_call_burden = factory.Trait(
            ***REMOVED*** ... create with many call assignments
        )
        on_leave = factory.Trait(
            ***REMOVED*** ... create with active absence
        )

    ***REMOVED*** Post-generation hooks
    @factory.post_generation
    def add_assignments(obj, create, extracted, **kwargs):
        if not create:
            return

        for _ in range(4 * 7):  ***REMOVED*** Create 4 weeks of assignments
            AssignmentFactory(person=obj)

***REMOVED*** Usage
resident = ResidentFactory()  ***REMOVED*** Basic
resident = ResidentFactory(high_call_burden=True)  ***REMOVED*** With trait
residents = ResidentFactory.create_batch(100)  ***REMOVED*** Many residents
```

---

***REMOVED******REMOVED*** Quality Gates & Escalation

***REMOVED******REMOVED******REMOVED*** Pre-Commit Gates

Before any commit, verify:

```bash
***REMOVED*** 1. All tests pass
pytest --tb=short

***REMOVED*** 2. Coverage acceptable
pytest --cov=app --cov-report=term-missing | grep "TOTAL"
***REMOVED*** Should show >= 80%

***REMOVED*** 3. No lint errors
ruff check app/
ruff format --check app/

***REMOVED*** 4. Type checking passes
mypy app/

***REMOVED*** 5. ACGME tests pass
pytest -m acgme
```

***REMOVED******REMOVED******REMOVED*** Escalation Decision Tree
...
***REMOVED******REMOVED******REMOVED*** Bug Report Template (from QA_TESTER.md)

```markdown
***REMOVED******REMOVED*** QA Escalation: [Bug Title]
...
***REMOVED******REMOVED******REMOVED*** Reproduction
```python
@pytest.mark.skip(reason="Reproduce bug")
def test_bug_reproduction(db):
    ***REMOVED*** Minimal reproducible example
    pass
```

***REMOVED******REMOVED******REMOVED*** Stack Trace
[Full error message if applicable]

***REMOVED******REMOVED******REMOVED*** Impact
[Who is affected? How does it break functionality?]

***REMOVED******REMOVED******REMOVED*** Recommended Fix
[Suggestion if known, or analysis]
```

---

***REMOVED******REMOVED*** Advanced Workflows

***REMOVED******REMOVED******REMOVED*** Workflow 1: Adding New Feature with TDD

```
1. Write failing test
   pytest tests/test_new_feature.py -v  ***REMOVED*** RED

2. Write minimal implementation
   ***REMOVED*** Just enough to make test pass

3. Run test
   pytest tests/test_new_feature.py -v  ***REMOVED*** GREEN

4. Refactor and improve
   ***REMOVED*** Clean up code while tests still pass

5. Add edge case tests
   ***REMOVED*** Test boundary conditions, error cases
...
***REMOVED******REMOVED******REMOVED*** Workflow 2: Debugging a Flaky Test
...
3. Collect failures
   ***REMOVED*** Note which runs failed, which succeeded

4. Add logging
   ***REMOVED*** Add print statements or logging to understand state

5. Identify timing issues
   ***REMOVED*** Look for sleep/timeout issues

6. Check for isolation problems
   ***REMOVED*** Verify fixtures clean up properly

7. Add delay if needed
   ***REMOVED*** Sometimes small sleep prevents race conditions
...
***REMOVED******REMOVED******REMOVED*** Workflow 3: Regression Testing After Fix

```
1. Write failing test that reproduces bug
   def test_bug_issue_247():
       ***REMOVED*** Setup for bug reproduction
       ***REMOVED*** Should fail before fix

2. Verify test fails
   pytest tests/test_regression.py::test_bug_issue_247 -v
   ***REMOVED*** Should show red failure

3. Apply fix
   ***REMOVED*** Fix the code in source

4. Verify test passes
   pytest tests/test_regression.py::test_bug_issue_247 -v
   ***REMOVED*** Should show green
...
8. Document in test docstring
   def test_bug_issue_247():
       """
       Regression test for issue ***REMOVED***247.
...
***REMOVED******REMOVED*** Reference: Complete Test Infrastructure Map

***REMOVED******REMOVED******REMOVED*** Pytest Plugins & Tools
...
```python
***REMOVED*** In-memory SQLite for tests (conftest.py)
TEST_DATABASE_URL = "sqlite:///:memory:"

***REMOVED*** Key advantages:
***REMOVED*** - No setup/teardown overhead
***REMOVED*** - Parallel test execution safe
***REMOVED*** - Fast test execution
***REMOVED*** - Isolated from production
```
...
***REMOVED******REMOVED******REMOVED*** Key Test Files & Responsibilities
...
***REMOVED******REMOVED*** Conclusion & Next Steps

***REMOVED******REMOVED******REMOVED*** For QA_TESTER Implementation
...
***REMOVED******REMOVED******REMOVED*** For Effective Agent Delegation
...
***REMOVED******REMOVED******REMOVED*** Document History
