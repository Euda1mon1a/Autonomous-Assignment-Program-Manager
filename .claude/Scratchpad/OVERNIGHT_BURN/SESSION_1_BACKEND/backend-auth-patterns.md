***REMOVED*** Backend Authentication & Authorization Patterns
...
***REMOVED******REMOVED*** Part 1: Authentication Flow

***REMOVED******REMOVED******REMOVED*** 1.1 High-Level Flow
...
***REMOVED******REMOVED******REMOVED*** 1.2 Authentication Endpoints

***REMOVED******REMOVED******REMOVED******REMOVED*** Login (Form-Based)
```python
POST /api/auth/login
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Login (JSON)
```python
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Get Current User
```python
GET /api/auth/me
Authorization: Bearer <access_token>
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** User Registration
```python
...
***REMOVED******REMOVED******REMOVED******REMOVED*** List Users (Admin Only)
```python
GET /api/auth/users
Authorization: Bearer <access_token>
```
...
***REMOVED******REMOVED*** Part 2: Token Architecture

***REMOVED******REMOVED******REMOVED*** 2.1 JWT Token Structure
...
***REMOVED******REMOVED******REMOVED*** 2.2 Token Verification Logic
...
***REMOVED******REMOVED******REMOVED*** 2.3 Token Blacklisting

**TokenBlacklist Model:**
```python
class TokenBlacklist(Base):
    jti: str                          ***REMOVED*** Unique token identifier
    token_type: str = "access"        ***REMOVED*** "access" or "refresh"
    user_id: UUID                     ***REMOVED*** User who owned token
    blacklisted_at: datetime          ***REMOVED*** When blacklisted
    expires_at: datetime              ***REMOVED*** When token expires (for cleanup)
    reason: str                       ***REMOVED*** "logout", "refresh_rotation", etc.
...
***REMOVED******REMOVED*** Part 3: Rate Limiting & Account Lockout

***REMOVED******REMOVED******REMOVED*** 3.1 IP-Based Rate Limiting (Redis)
...
class RateLimiter:
    ...
    def is_rate_limited(
        self,
        key: str,                    ***REMOVED*** e.g., "login:192.168.1.1"
        max_requests: int,           ***REMOVED*** e.g., 5
        window_seconds: int          ***REMOVED*** e.g., 60
    ) -> tuple[bool, dict]:
        """
...
***REMOVED******REMOVED******REMOVED*** 3.2 Account Lockout (Redis)
...
class AccountLockout:
    ...
    MAX_FAILED_ATTEMPTS = 5              ***REMOVED*** Lock after 5 failures
    BASE_LOCKOUT_SECONDS = 60            ***REMOVED*** Initial: 1 min
    MAX_LOCKOUT_SECONDS = 3600           ***REMOVED*** Cap: 1 hour
    BACKOFF_MULTIPLIER = 2.0             ***REMOVED*** Double on each failure
...
***REMOVED******REMOVED*** Part 4: Role-Based Access Control (RBAC)

***REMOVED******REMOVED******REMOVED*** 4.1 User Roles

```python
class UserRole(Enum):
    """8 user roles with hierarchical structure"""

    ADMIN          = "admin"              ***REMOVED*** Full system access
    COORDINATOR    = "coordinator"        ***REMOVED*** Schedule management
    FACULTY        = "faculty"            ***REMOVED*** Limited + self-management
    CLINICAL_STAFF = "clinical_staff"     ***REMOVED*** Clinical view (parent role)
    RN             = "rn"                 ***REMOVED*** Registered Nurse
    LPN            = "lpn"                ***REMOVED*** Licensed Practical Nurse
    MSA            = "msa"                ***REMOVED*** Medical Support Assistant
    RESIDENT       = "resident"           ***REMOVED*** Basic view + self-management
```

***REMOVED******REMOVED******REMOVED*** 4.2 Role Hierarchy
...
***REMOVED******REMOVED******REMOVED*** 4.3 User Model

```python
class User(Base):
    """User model with role and permission properties"""

    id: UUID                    ***REMOVED*** Primary key
    username: str               ***REMOVED*** Unique username
    email: str                  ***REMOVED*** Unique email
    hashed_password: str        ***REMOVED*** bcrypt hash
    role: str                   ***REMOVED*** One of 8 roles
    is_active: bool             ***REMOVED*** True = can log in
    created_at: datetime
    updated_at: datetime
    last_login: datetime        ***REMOVED*** Tracks login times

    ***REMOVED*** Properties for permission checking
    @property
...
***REMOVED******REMOVED******REMOVED*** 4.4 Access Control Matrix (ResourceType × PermissionAction)
...
class ResourceType(Enum):
    ...
    ***REMOVED*** Procedures & Credentials
    PROCEDURE = "procedure"
...
class PermissionAction(Enum):
    ...
    EXPORT = "export"
    IMPORT = "import"
    MANAGE = "manage"     ***REMOVED*** Includes all CRUD
```

***REMOVED******REMOVED******REMOVED*** 4.5 Permission Matrix Summary
...
***REMOVED******REMOVED******REMOVED*** 4.6 Context-Aware Permissions

```python
class PermissionContext(BaseModel):
    """Context for evaluating permissions dynamically"""

    user_id: UUID                         ***REMOVED*** Who is accessing
    user_role: UserRole                   ***REMOVED*** Their role
    resource_owner_id: UUID | None        ***REMOVED*** Who owns resource
    resource_metadata: dict               ***REMOVED*** Additional context
    ip_address: str | None                ***REMOVED*** For audit
    timestamp: datetime                   ***REMOVED*** When accessed
...
if (role == UserRole.FACULTY and
    ...
    ***REMOVED*** Can approve swaps involving them, not others
    return context.resource_owner_id == context.user_id
```

***REMOVED******REMOVED******REMOVED*** 4.7 Accessing Permissions in Routes
...
***REMOVED******REMOVED*** Part 5: Security Implementation Details

***REMOVED******REMOVED******REMOVED*** 5.1 Password Security
...
***REMOVED*** Hashing (in registration)
hashed = pwd_context.hash(plaintext_password)

***REMOVED*** Verification (in login)
...
def validate_password_strength(cls, v: str) -> str:
    ***REMOVED*** Minimum 12 chars, max 128
    if len(v) < 12 or len(v) > 128:
        raise ValueError("Invalid length")

    ***REMOVED*** Must have 3+ of: lowercase, uppercase, digit, special
    has_lower = bool(re.search(r"[a-z]", v))
    has_upper = bool(re.search(r"[A-Z]", v))
    has_digit = bool(re.search(r"\d", v))
    has_special = bool(re.search(r'[!@***REMOVED***$%^&*(),.?":{}|<>]', v))

    if sum([has_lower, has_upper, has_digit, has_special]) < 3:
        raise ValueError("Insufficient complexity")

    ***REMOVED*** Reject common passwords
...
***REMOVED******REMOVED******REMOVED*** 5.2 JWT Configuration

**Settings (from config):**
```python
***REMOVED*** Token expiration times
ACCESS_TOKEN_EXPIRE_MINUTES = 30        ***REMOVED*** Short-lived (minutes)
REFRESH_TOKEN_EXPIRE_DAYS = 7           ***REMOVED*** Long-lived (days)

***REMOVED*** Token rotation
REFRESH_TOKEN_ROTATE = True             ***REMOVED*** Rotate on refresh
                                        ***REMOVED*** If True: old token blacklisted immediately

***REMOVED*** Signing
SECRET_KEY = "<32+ char random>"        ***REMOVED*** Must be 32+ chars
ALGORITHM = "HS256"                     ***REMOVED*** HMAC-SHA256
```
...
***REMOVED******REMOVED******REMOVED*** 5.3 httpOnly Cookie Security
...
response.set_cookie(
    ...
    httponly=True,                    ***REMOVED*** Cannot access via JavaScript
    secure=not DEBUG,                 ***REMOVED*** HTTPS only in production
...
```python
***REMOVED*** Refresh token NOT in cookie (deliberate!)
***REMOVED*** Returned in response body only
return TokenWithRefresh(
    access_token=access_token,
    refresh_token=refresh_token,    ***REMOVED*** Client must store securely
    token_type="bearer"
)
```
...
***REMOVED******REMOVED******REMOVED*** 5.4 Error Handling & Information Leakage

**Good Practice (Generic Error):**
```python
***REMOVED*** GOOD: Doesn't reveal if user exists
raise HTTPException(
    status_code=401,
    detail="Incorrect username or password"  ***REMOVED*** Same for both errors
)
```

**Bad Practice (Information Leakage):**
```python
***REMOVED*** BAD: Reveals if user exists
if not user:
...
**Server-Side Logging:**
```python
***REMOVED*** Detailed logs only server-side (not returned to client)
logger.error(
...
***REMOVED******REMOVED******REMOVED*** 5.5 Audit Logging

**PermissionAuditEntry Model:**
```python
class PermissionAuditEntry(BaseModel):
    id: UUID
    timestamp: datetime              ***REMOVED*** When checked
    action: str                      ***REMOVED*** "checked", "granted", "denied"
    role: UserRole
    resource: ResourceType
    permission: PermissionAction
    user_id: UUID | None             ***REMOVED*** Who performed action
    context: dict                    ***REMOVED*** Additional context
    result: bool                     ***REMOVED*** True = allowed, False = denied
    reason: str | None               ***REMOVED*** Why denied
```
...
***REMOVED*** Audit is automatic
if acm.has_permission(user.role, resource, action):
    ***REMOVED*** Permission check logged automatically
    pass

***REMOVED*** Access audit log
...
***REMOVED******REMOVED*** Part 6: Dependencies & Injection Points

***REMOVED******REMOVED******REMOVED*** 6.1 Security Dependencies

**Core Dependency Chain:**
```python
***REMOVED*** deps.py / api/deps.py
from app.core.security import (
    get_current_user,              ***REMOVED*** Returns User | None
    get_current_active_user,       ***REMOVED*** Returns User (403 if not auth)
    get_admin_user,                ***REMOVED*** Returns User (403 if not admin)
    get_scheduler_user,            ***REMOVED*** Returns User (403 if can't manage)
)
...
***REMOVED******REMOVED******REMOVED*** 6.2 Role-Filter Dependencies

**Role-Based Filtering:**
```python
from app.api.dependencies.role_filter import (
    require_resource_access,       ***REMOVED*** Decorator-style check
    require_admin,                 ***REMOVED*** Admin-only check
    apply_role_filter,             ***REMOVED*** Auto-filter response
    filter_response,               ***REMOVED*** Decorator for filtering
)

@router.get("/dashboard")
@filter_response()                ***REMOVED*** Auto-filters response by role
...
***REMOVED******REMOVED******REMOVED*** 6.3 Database Session Injection
...
async def register(
    user_in: UserCreate,
    db: Session = Depends(get_db)  ***REMOVED*** Auto-managed transaction
):
...
***REMOVED******REMOVED*** Part 7: Security Compliance Checklist

***REMOVED******REMOVED******REMOVED*** Authentication Compliance
...
***REMOVED******REMOVED******REMOVED*** Authorization Compliance
...
***REMOVED******REMOVED******REMOVED*** Operational Security
...
***REMOVED******REMOVED*** Part 8: Common Integration Patterns

***REMOVED******REMOVED******REMOVED*** 8.1 Adding a Permission Check to Existing Route
...
async def create_schedule(
    schedule_in: ScheduleCreate,
    current_user: User = Depends(get_scheduler_user),  ***REMOVED*** ← Added
    db: Session = Depends(get_db)
):
    ***REMOVED*** Now requires admin or coordinator role
    pass
```

***REMOVED******REMOVED******REMOVED*** 8.2 Custom Role Check
...
async def special_op(
    ...
    ***REMOVED*** Proceed (403 if no permission)
    pass
```

***REMOVED******REMOVED******REMOVED*** 8.3 Filtering Results by Role
...
***REMOVED******REMOVED*** Part 9: Improving the Auth System

***REMOVED******REMOVED******REMOVED*** 9.1 Missing Pieces
...
***REMOVED******REMOVED******REMOVED*** 9.2 Potential Vulnerabilities
...
***REMOVED******REMOVED******REMOVED*** 9.3 Recommended Improvements
...
***REMOVED******REMOVED*** Part 10: Testing & Verification

***REMOVED******REMOVED******REMOVED*** 10.1 Auth Test Coverage
...
***REMOVED******REMOVED******REMOVED*** 10.2 Running Tests

```bash
***REMOVED*** All auth tests
cd backend
pytest tests/test_auth_routes.py -v

***REMOVED*** RBAC specific
pytest tests/auth/test_rbac_authorization.py -v

***REMOVED*** Access matrix
pytest tests/test_access_matrix.py -v

***REMOVED*** Integration (slow)
...
***REMOVED******REMOVED*** Part 11: Configuration Reference

***REMOVED******REMOVED******REMOVED*** Environment Variables

```bash
***REMOVED*** JWT Configuration
SECRET_KEY=<32+ chars random>           ***REMOVED*** REQUIRED: HS256 key
ACCESS_TOKEN_EXPIRE_MINUTES=30
REFRESH_TOKEN_EXPIRE_DAYS=7
REFRESH_TOKEN_ROTATE=true               ***REMOVED*** Blacklist old refresh on rotate

***REMOVED*** Rate Limiting
RATE_LIMIT_ENABLED=true
RATE_LIMIT_LOGIN_ATTEMPTS=5
RATE_LIMIT_LOGIN_WINDOW=60              ***REMOVED*** seconds
RATE_LIMIT_REGISTER_ATTEMPTS=10
RATE_LIMIT_REGISTER_WINDOW=3600

***REMOVED*** Redis (for rate limit, account lockout, blacklist)
REDIS_URL=redis://localhost:6379/0
REDIS_PASSWORD=<optional>

***REMOVED*** HTTPS/Security
DEBUG=false                             ***REMOVED*** Set to false in production
SECURE_COOKIES=true                    ***REMOVED*** Force HTTPS for cookies

***REMOVED*** Trusted Proxies (for rate limiting)
TRUSTED_PROXIES=["127.0.0.1", "10.0.0.0/8"]
```

---

***REMOVED******REMOVED*** Summary: Key Takeaways
...
***REMOVED******REMOVED******REMOVED*** Areas for Enhancement
