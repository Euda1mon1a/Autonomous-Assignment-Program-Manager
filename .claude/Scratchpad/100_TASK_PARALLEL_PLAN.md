***REMOVED*** 100-Task Parallel Execution Plan
...
***REMOVED******REMOVED*** Context Files for CCW Instances

***REMOVED******REMOVED******REMOVED*** Files to Share (All Safe)

**Core Understanding:**
```
CLAUDE.md                                    ***REMOVED*** Project guidelines
docs/architecture/SOLVER_ALGORITHM.md        ***REMOVED*** Scheduling engine
docs/architecture/cross-disciplinary-resilience.md  ***REMOVED*** Resilience framework
```

**For MCP Work (Stream A):**
```
mcp-server/src/scheduler_mcp/server.py       ***REMOVED*** 81 tools, 4411 lines
mcp-server/src/scheduler_mcp/api_client.py   ***REMOVED*** API client pattern
mcp-server/src/scheduler_mcp/tools/          ***REMOVED*** Tool modules
backend/app/api/routes/                      ***REMOVED*** Backend endpoints to call
```

**For Backend Tests (Stream B):**
```
backend/app/services/                        ***REMOVED*** 47 services (21% tested)
backend/app/scheduling/constraints/          ***REMOVED*** 18 constraints (61% tested)
backend/app/resilience/                      ***REMOVED*** 44 modules (39% tested)
backend/tests/conftest.py                    ***REMOVED*** Test fixtures
```

**For Frontend Tests (Stream C):**
```
frontend/src/components/                     ***REMOVED*** 80 components
frontend/src/lib/                            ***REMOVED*** Utilities (auth.ts, validation.ts)
frontend/__tests__/                          ***REMOVED*** Existing test patterns
```

**For Security (Stream E):**
```
docs/security/DATA_SECURITY_POLICY.md        ***REMOVED*** OPSEC/PERSEC policy
docs/security/SECURITY_PATTERN_AUDIT.md      ***REMOVED*** Security architecture
backend/app/api/routes/auth.py               ***REMOVED*** Auth endpoints
backend/app/core/security.py                 ***REMOVED*** Security utilities
```

***REMOVED******REMOVED******REMOVED*** Files to NEVER Share

```
.env                                         ***REMOVED*** Real credentials
docs/data/*.json                             ***REMOVED*** Real schedule data (gitignored)
.sanitize/                                   ***REMOVED*** PII mapping files
```

***REMOVED******REMOVED******REMOVED*** RAG Knowledge Base (Requires Sanitization)

**Location:** `docs/rag-knowledge/` (10 documents, ~150KB total)

**Before sharing with CCW, run sanitization:**
```bash
***REMOVED*** Follow the SOP in docs/security/PII_SANITIZATION_SOP.md
python scripts/sanitize_pii.py --input docs/rag-knowledge/ --output /tmp/rag-sanitized/

***REMOVED*** Or manually review each file for:
***REMOVED*** - Real names → Replace with PGY1-01, FAC-PD format
***REMOVED*** - Real dates → Replace with YYYY-MM-DD placeholders
***REMOVED*** - Internal URLs → Remove or replace with example.com
```
...
***REMOVED******REMOVED*** Database & Container Context for CCW

***REMOVED******REMOVED******REMOVED*** Database Schema (42 SQLAlchemy Models)
...
```
backend/app/models/                          ***REMOVED*** All 42 models
backend/app/db/session.py                    ***REMOVED*** Connection pooling
backend/alembic/versions/                    ***REMOVED*** 40 migrations
```

***REMOVED******REMOVED******REMOVED*** Container Architecture
...
```
docker-compose.yml                           ***REMOVED*** All service definitions
docker-compose.dev.yml                       ***REMOVED*** Dev overrides
.env.example                                 ***REMOVED*** Required env vars (no secrets)
```

---

***REMOVED******REMOVED*** Vector DB / RAG Status

***REMOVED******REMOVED******REMOVED*** Current State: OPERATIONAL
...
***REMOVED******REMOVED******REMOVED*** Key RAG Files for CCW Context
```
backend/app/services/rag_service.py          ***REMOVED*** RAG operations (700+ lines)
backend/app/services/embedding_service.py    ***REMOVED*** Embedding generation
backend/app/models/rag_document.py           ***REMOVED*** Vector storage model
backend/app/api/routes/rag.py                ***REMOVED*** API endpoints
scripts/init_rag_embeddings.py               ***REMOVED*** CLI for embedding docs
```

***REMOVED******REMOVED******REMOVED*** RAG Capabilities
...
***REMOVED******REMOVED*** Celery Background Task Context for CCW

***REMOVED******REMOVED******REMOVED*** Task Queues (6 dedicated + 1 default)
...
***REMOVED******REMOVED******REMOVED*** Periodic Tasks (Celery Beat)
...
***REMOVED******REMOVED******REMOVED*** Task Modules (50+ tasks across 11 modules)

```
backend/app/tasks/
├── schedule_metrics_tasks.py    ***REMOVED*** Fairness, coverage, compliance
├── cleanup_tasks.py             ***REMOVED*** Idempotency, token cleanup
├── audit_tasks.py               ***REMOVED*** Audit log archival
├── backup_tasks.py              ***REMOVED*** Database backups
├── compliance_report_tasks.py   ***REMOVED*** ACGME compliance reports
├── ml_tasks.py                  ***REMOVED*** ML model training
├── rag_tasks.py                 ***REMOVED*** RAG embedding refresh
└── periodic_tasks.py            ***REMOVED*** Beat schedule configuration

backend/app/resilience/tasks.py   ***REMOVED*** Health checks, contingency
backend/app/notifications/tasks.py ***REMOVED*** Email, webhooks
backend/app/security/rotation_tasks.py ***REMOVED*** Secret rotation
backend/app/exports/jobs.py       ***REMOVED*** Export execution
```

***REMOVED******REMOVED******REMOVED*** Worker Configuration
- **Concurrency:** 4 workers
- **Task Timeout:** 10 min hard / 9 min soft
- **Result Retention:** 1 hour (24 hours for metrics)
- **Broker:** Redis (redis://localhost:6379/0)

***REMOVED******REMOVED******REMOVED*** Files to share for Celery context
```
backend/app/core/celery_app.py               ***REMOVED*** Main configuration
backend/app/tasks/periodic_tasks.py          ***REMOVED*** Beat schedule
backend/app/tasks/                           ***REMOVED*** All task modules
backend/app/resilience/tasks.py              ***REMOVED*** Resilience tasks
```

---

***REMOVED******REMOVED*** Stream A: MCP Tool Completion (30 Tasks)
...
| ***REMOVED*** | Task | File | Status → Target |
|---|------|------|-----------------|
...
| ***REMOVED*** | Task | Backend Endpoint Needed |
|---|------|------------------------|
...
***REMOVED******REMOVED*** Stream B: Backend Service Tests (25 Tasks)
...
| ***REMOVED*** | Service | Test File to Create |
|---|---------|-------------------|
...
| ***REMOVED*** | Module | Test File to Create |
|---|--------|-------------------|
...
***REMOVED******REMOVED*** Stream C: Frontend Component Tests (15 Tasks)
...
| ***REMOVED*** | Component/Feature | Test File to Create |
|---|------------------|-------------------|
...
***REMOVED******REMOVED*** Stream D: Integration/E2E Scenarios (10 Tasks)

**Terminal 9 | Goal: End-to-end user journey tests**

***REMOVED******REMOVED******REMOVED*** Task Breakdown

| ***REMOVED*** | Scenario | Type |
|---|----------|------|
...
***REMOVED******REMOVED*** Stream E: Security Sweep (8 Tasks)

**Terminal 10 | Goal: OWASP + HIPAA + OPSEC audit**

***REMOVED******REMOVED******REMOVED*** Task Breakdown

| ***REMOVED*** | Audit Area | Files to Review |
|---|-----------|----------------|
...
***REMOVED******REMOVED*** Stream F: Overflow Tasks (12 Tasks)

**Distributed across available capacity**

***REMOVED******REMOVED******REMOVED*** Constraint Registration (5 tasks)
| ***REMOVED*** | Task |
|---|------|
...
***REMOVED******REMOVED******REMOVED*** Database Optimization (3 tasks)
| ***REMOVED*** | Task |
|---|------|
...
***REMOVED******REMOVED******REMOVED*** Documentation Sync (4 tasks)
| ***REMOVED*** | Task |
|---|------|
...
***REMOVED******REMOVED*** Execution Protocol

***REMOVED******REMOVED******REMOVED*** Pre-Execution Checklist

1. **Verify repo is clean:**
   ```bash
   git status --ignored | grep -E "\.env|data.*\.json"
   ***REMOVED*** Should show only gitignored files
   ```
...
***REMOVED******REMOVED******REMOVED*** Parallel Execution Pattern
...
***REMOVED******REMOVED******REMOVED*** Synthesis Protocol
...
***REMOVED******REMOVED*** Files Modified by This Plan

***REMOVED******REMOVED******REMOVED*** Stream A (MCP)
...
***REMOVED******REMOVED******REMOVED*** Stream B (Backend Tests)
...
***REMOVED******REMOVED******REMOVED*** Stream C (Frontend Tests)
...
***REMOVED******REMOVED******REMOVED*** Stream D (E2E)
- NEW: `backend/tests/e2e/test_*.py` (10 files)

***REMOVED******REMOVED******REMOVED*** Stream E (Security)
