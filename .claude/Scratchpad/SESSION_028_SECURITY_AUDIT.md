***REMOVED*** Security Audit Report - Session 028
**Date:** 2025-12-30
...
***REMOVED******REMOVED*** 1. Hardcoded Credentials & Secrets

***REMOVED******REMOVED******REMOVED*** Status: ✅ PASS (with minor issues)

**Finding:** No production secrets hardcoded. All API keys and passwords properly use environment variables.

**Minor Issues Found:**
```python
***REMOVED*** scripts/seed_inpatient_rotations.py:42
password = os.getenv("SEED_ADMIN_PASSWORD", "admin123")

***REMOVED*** scripts/seed_rotation_templates.py:111
password = os.getenv("SEED_ADMIN_PASSWORD", "admin123")

***REMOVED*** scripts/seed_people.py:12
...
***REMOVED******REMOVED*** 2. SQL Injection Vulnerabilities

***REMOVED******REMOVED******REMOVED*** Status: ⚠️ MEDIUM RISK

**Finding:** Limited use of raw SQL with f-string interpolation for table names.

**Affected Files:**
```python
***REMOVED*** backend/app/db/pool/health.py:254
session.execute(text(f"SELECT pg_sleep({timeout_seconds + 1})"))

***REMOVED*** backend/app/backup/strategies.py:111
query = text(f"SELECT COUNT(*) FROM {table}")

***REMOVED*** backend/app/backup/strategies.py:256
query = text(f"SELECT * FROM {table}")

***REMOVED*** backend/app/backup/strategies.py:436
query = text(f"SELECT * FROM {table}")

***REMOVED*** backend/app/health/checks/database.py:194
...
***REMOVED******REMOVED*** 3. Authentication & Authorization Coverage

***REMOVED******REMOVED******REMOVED*** Status: ⚠️ NEEDS IMPROVEMENT
...
***REMOVED******REMOVED*** 4. CORS Configuration

***REMOVED******REMOVED******REMOVED*** Status: ✅ PASS
...
***REMOVED******REMOVED*** 5. Rate Limiting Implementation

***REMOVED******REMOVED******REMOVED*** Status: ⚠️ CRITICAL (Currently Disabled)

**Finding:** Rate limiting is **DISABLED** in current deployment.

**Evidence:**
```python
***REMOVED*** backend/app/main.py:300
logger.info("Slowapi rate limiting middleware DISABLED")

***REMOVED*** backend/app/core/slowapi_limiter.py:69
limiter = Limiter(
    key_func=get_client_identifier,
    storage_uri=get_redis_url(),
    default_limits=["200/minute", "1000/hour"],
    enabled=settings.RATE_LIMIT_ENABLED,  ***REMOVED*** ← Currently False
)
```

**Configuration:**
```python
***REMOVED*** backend/app/core/config.py:119
RATE_LIMIT_ENABLED: bool = True  ***REMOVED*** Default is True
```
...
***REMOVED******REMOVED*** 6. Input Validation Coverage

***REMOVED******REMOVED******REMOVED*** Status: ✅ EXCELLENT
...
***REMOVED******REMOVED*** 7. Password Hashing & JWT Security
...
**JWT Configuration:**
```python
ALGORITHM = "HS256"  ***REMOVED*** Secure symmetric algorithm

***REMOVED*** Token includes:
...
```python
***REMOVED*** Exponential backoff for failed login attempts
...
***REMOVED******REMOVED*** 8. Cookie Security
...
response.set_cookie(
    ...
    httponly=True,           ***REMOVED*** ✅ XSS protection
...
***REMOVED******REMOVED*** 9. File Upload Security
...
class FileValidator:
    ...
    ***REMOVED*** MIME type whitelist
    ALLOWED_IMAGE_TYPES = {"image/jpeg", "image/png", ...}
    ALLOWED_DOCUMENT_TYPES = {"application/pdf", ...}

    ***REMOVED*** Extension validation
    ALLOWED_EXTENSIONS_MAP = {
        "image/jpeg": {".jpg", ".jpeg"},
        ...
    }

    ***REMOVED*** Magic byte verification
    MAGIC_SIGNATURES = {
...
***REMOVED******REMOVED*** 10. Open Redirect Vulnerability

***REMOVED******REMOVED******REMOVED*** Status: 🔴 CRITICAL
...
```python
***REMOVED*** Redirect to relay state or default dashboard
redirect_url = relay_state if relay_state else "/dashboard"

***REMOVED*** Return HTML that redirects (since this is a POST response)
html_content = f"""
<!DOCTYPE html>
<html>
<head>
...
def validate_redirect_url(url: str, allowed_hosts: list[str]) -> str:
    ...
    ***REMOVED*** Only allow relative URLs or whitelisted domains
    if url.startswith('/'):
        return url  ***REMOVED*** Relative URL is safe
    ...
    return "/dashboard"  ***REMOVED*** Safe default
...
***REMOVED******REMOVED*** 11. XML External Entity (XXE) Vulnerability

***REMOVED******REMOVED******REMOVED*** Status: 🔴 HIGH RISK
...
if self._lxml_available:
    root = self._etree.fromstring(data)  ***REMOVED*** ← No XXE protection
else:
    root = self._etree.fromstring(data)  ***REMOVED*** ← No XXE protection
```
...
```python
***REMOVED*** Option 1: Use defusedxml (RECOMMENDED)
pip install defusedxml

from defusedxml import ElementTree as ET

root = ET.fromstring(data)  ***REMOVED*** Safe by default

***REMOVED*** Option 2: Configure lxml securely
from lxml import etree

parser = etree.XMLParser(
    resolve_entities=False,  ***REMOVED*** Disable external entities
    no_network=True,         ***REMOVED*** Disable network access
    dtd_validation=False,    ***REMOVED*** Disable DTD validation
    load_dtd=False           ***REMOVED*** Don't load DTD
)
root = etree.fromstring(data, parser=parser)

***REMOVED*** Option 3: Configure xml.etree securely
import xml.etree.ElementTree as ET

***REMOVED*** Monkey-patch to disable entities (Python 3.8+)
ET.XMLParser.entity = {}
...
**Recommended Fix:**
```python
***REMOVED*** backend/app/middleware/content/parsers.py

def __init__(self):
    """Initialize XML parser with XXE protection."""
    try:
        ***REMOVED*** Use defusedxml if available (RECOMMENDED)
        ...
        try:
            from lxml import etree
            ***REMOVED*** Configure secure parser
...
***REMOVED******REMOVED*** 12. Security Headers
...
***REMOVED******REMOVED*** 13. Secret Management
...
***REMOVED******REMOVED*** 14. .gitignore Coverage
...
***REMOVED*** Database dumps
*.dump
*.sql
!backend/alembic/**/*.sql

***REMOVED*** PII data (OPSEC/PERSEC)
...
***REMOVED******REMOVED*** Priority Remediation Roadmap
...
***REMOVED******REMOVED******REMOVED*** HIGH PRIORITY (Within 1 Week)
...
***REMOVED******REMOVED******REMOVED*** MEDIUM PRIORITY (Within 1 Month)
...
***REMOVED******REMOVED******REMOVED*** 1. Automated Security Testing

```bash
***REMOVED*** Install security tools
pip install bandit safety pip-audit

***REMOVED*** Run static analysis
bandit -r backend/app -f json -o bandit_report.json

***REMOVED*** Check for known vulnerabilities
safety check --json

***REMOVED*** Audit dependencies
pip-audit
```
...
**Test Cases:**
```python
***REMOVED*** Test rate limiting
for i in range(10):
    requests.post("/api/auth/login", json={"username": "admin", "password": "wrong"})
***REMOVED*** Expected: 429 Too Many Requests after 5 attempts

***REMOVED*** Test open redirect (SHOULD BE BLOCKED AFTER FIX)
requests.get("/api/sso/saml/acs?relay_state=https://evil.com")
***REMOVED*** Expected: Redirect to /dashboard, not evil.com

***REMOVED*** Test XXE (SHOULD BE BLOCKED AFTER FIX)
xxe_payload = """<?xml version="1.0"?>
<!DOCTYPE foo [<!ENTITY xxe SYSTEM "file:///etc/passwd">]>
<root>&xxe;</root>"""
requests.post("/api/endpoint", data=xxe_payload, headers={"Content-Type": "application/xml"})
***REMOVED*** Expected: ParsingError, not file contents

***REMOVED*** Test SQL injection (should already be safe)
requests.get("/api/backup?table=users; DROP TABLE users--")
***REMOVED*** Expected: Validation error or safe handling
```

***REMOVED******REMOVED******REMOVED*** 3. Continuous Monitoring
...
***REMOVED******REMOVED******REMOVED*** OWASP Top 10 (2021)
...
***REMOVED******REMOVED******REMOVED*** Security Score: **7.5/10** → **9.5/10** (after remediation)
