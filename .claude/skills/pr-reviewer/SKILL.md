...
***REMOVED******REMOVED******REMOVED*** Example 1: Standard PR Review (Feature Addition)

**Context:** PR ***REMOVED***523 adds resident work hour summary endpoint
...
2. **Check CI status:**
   ```bash
   gh pr checks 523
   ***REMOVED*** ✓ Tests: passing
   ***REMOVED*** ✓ Lint: passing
   ***REMOVED*** ✗ Coverage: 65% (below 70% threshold)
   ```
3. **Review code:**
...
4. **Run quality gates locally:**
   ...
   ***REMOVED*** FAIL: 65% coverage
   ```

**Output:**
```markdown
***REMOVED******REMOVED*** PR Review: ***REMOVED***523 - Add work hour summary endpoint
...
***REMOVED******REMOVED******REMOVED*** Approval Checklist
...
***REMOVED******REMOVED******REMOVED*** Example 2: Security-Critical PR Review

**Context:** PR ***REMOVED***478 modifies authentication middleware

**Input:**
```python
***REMOVED*** Changed file: backend/app/api/deps.py
def get_current_user(token: str = Depends(oauth2_scheme)):
    ***REMOVED*** CHANGE: Removed JWT expiration check
...
2. **Compare with previous version:**
   ...
   + ***REMOVED*** Expiration check removed
   ```
...
**Output:**
```markdown
***REMOVED******REMOVED*** PR Review: ***REMOVED***478 - Update auth middleware

**Decision:** :red_circle: BLOCK MERGE - CRITICAL SECURITY ISSUE

***REMOVED******REMOVED******REMOVED*** Security Finding
...
***REMOVED******REMOVED******REMOVED*** Action Required
- [ ] Revert to include expiration check
- [ ] Add test: `test_expired_token_rejected()`
- [ ] Human security review before re-submit
```

***REMOVED******REMOVED******REMOVED*** Example 3: Database Migration PR Review

**Context:** PR ***REMOVED***501 adds `email_verified` field to User model

**Input:**
```bash
gh pr diff 501 --name-only
***REMOVED*** backend/app/models/user.py
***REMOVED*** backend/alembic/versions/abc123_add_email_verified.py
```
...
4. **Verify locally:**
   ...
   ***REMOVED*** ERROR: column "email_verified" contains null values
   ```

**Output:**
```markdown
***REMOVED******REMOVED*** PR Review: ***REMOVED***501 - Add email_verified field

**Decision:** REQUEST CHANGES

***REMOVED******REMOVED******REMOVED*** Database Migration Issue
...
```python
***REMOVED*** Migration 1: Add as nullable
op.add_column('users', sa.Column('email_verified', sa.Boolean(), nullable=True))

***REMOVED*** Migration 2: Backfill
op.execute("UPDATE users SET email_verified = false WHERE email_verified IS NULL")

***REMOVED*** Migration 3: Make non-nullable
...
| **False Positive Security Alert** | Bandit flags safe code as vulnerable | Static analysis limitation | 1. Manual review confirms false positive<br>2. Add `***REMOVED*** nosec` comment with justification<br>3. Document in review |
