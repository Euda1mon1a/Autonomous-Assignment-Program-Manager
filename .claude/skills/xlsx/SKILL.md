...
***REMOVED*** Excel Spreadsheet Skill
...
```python
***REMOVED*** For data analysis and basic operations
import pandas as pd

***REMOVED*** For Excel-specific features (formulas, formatting)
...
***REMOVED******REMOVED******REMOVED*** 1. Use Formulas, Not Hardcoded Values

```python
***REMOVED*** BAD - Hardcoding calculated values
ws['C2'] = 45  ***REMOVED*** Total hours

***REMOVED*** GOOD - Use Excel formulas
ws['C2'] = '=SUM(A2:B2)'
```

***REMOVED******REMOVED******REMOVED*** 2. Pandas for Analysis, openpyxl for Excel Features

```python
***REMOVED*** Use pandas for data manipulation
df = pd.read_excel('schedule.xlsx')
summary = df.groupby('rotation').sum()

***REMOVED*** Use openpyxl for formatting and formulas
wb = load_workbook('schedule.xlsx')
ws = wb.active
ws['A1'].font = Font(bold=True)
```

***REMOVED******REMOVED******REMOVED*** 3. Cell Indices Are 1-Based

```python
***REMOVED*** openpyxl uses 1-based indexing
ws.cell(row=1, column=1, value="Header")  ***REMOVED*** A1, not A0
```

***REMOVED******REMOVED*** Schedule Export Patterns

***REMOVED******REMOVED******REMOVED*** Weekly Schedule Export
...
def export_weekly_schedule(assignments: list, start_date: date) -> Workbook:
    ...
    ***REMOVED*** Data rows
    for row_idx, assignment in enumerate(assignments, 2):
        ...
        ***REMOVED*** ... populate daily assignments

    ***REMOVED*** Auto-adjust column widths
...
***REMOVED******REMOVED*** ACGME Compliance Report
...
def create_compliance_report(residents: list, period_start: date, period_end: date) -> Workbook:
    ...
    ***REMOVED*** Compliance status formatting
    ...
    for row, resident in enumerate(residents, 2):
        ...
        ***REMOVED*** ... additional compliance checks

    return wb
```

***REMOVED******REMOVED*** Reading Excel Files

```python
***REMOVED*** Read with pandas for analysis
df = pd.read_excel('schedule.xlsx', sheet_name='Sheet1')

***REMOVED*** Read with openpyxl for formulas
wb = load_workbook('schedule.xlsx')
ws = wb.active

***REMOVED*** WARNING: data_only=True reads calculated values but loses formulas
***REMOVED*** Only use for read-only analysis
wb_values = load_workbook('schedule.xlsx', data_only=True)
```

***REMOVED******REMOVED*** Schedule Import Patterns

***REMOVED******REMOVED******REMOVED*** Bulk Resident Import
...
async def import_residents_from_excel(
    ...
    ***REMOVED*** Normalize column names
    ...
    for idx, row in df.iterrows():
        try:
            ...
            errors.append({
                'row': idx + 2,  ***REMOVED*** Excel row (1-indexed + header)
                'data': row.to_dict(),
                'error': str(e)
            })
...
***REMOVED******REMOVED******REMOVED*** Schedule Assignment Import
...
async def import_schedule_from_excel(
    ...
    ***REMOVED*** Parse header row for dates
    dates = []
    ...
    ***REMOVED*** Parse data rows
    for row in range(2, ws.max_row + 1):
        ...
        ***REMOVED*** Parse assignments for each date
...
***REMOVED******REMOVED******REMOVED*** Import Validation Helpers
...
***REMOVED******REMOVED******REMOVED*** Import Endpoint Pattern
    ...
    ***REMOVED*** Save to temp file
    ...
    try:
        ...
        return {
            ...
            "errors": len(errors),
            "error_details": errors[:10]  ***REMOVED*** First 10 errors
        }
    finally:
        os.unlink(tmp_path)
```

***REMOVED******REMOVED*** Formula Error Checking

Always verify no formula errors exist:

```python
ERROR_PATTERNS = ['***REMOVED***REF!', '***REMOVED***DIV/0!', '***REMOVED***VALUE!', '***REMOVED***N/A', '***REMOVED***NAME?', '***REMOVED***NULL!']
...
***REMOVED******REMOVED******REMOVED*** Color Coding (Financial Model Standard)
...
***REMOVED******REMOVED******REMOVED*** Number Formatting
...
***REMOVED*** Currency
cell.number_format = '"$"***REMOVED***,***REMOVED******REMOVED***0.00'
...
***REMOVED******REMOVED******REMOVED*** Export Endpoint Pattern
...
- [ ] All formulas calculate correctly (no ***REMOVED***REF!, ***REMOVED***DIV/0!, etc.)
