...
***REMOVED*** Pre-PR Checklist Skill
...
***REMOVED******REMOVED*** Documentation Requirements Matrix
...
***REMOVED******REMOVED******REMOVED*** Complexity Indicators
...
***REMOVED******REMOVED*** Pre-Flight Verification Steps

***REMOVED******REMOVED******REMOVED*** Step 1: Classify the Change

```bash
***REMOVED*** Get list of changed files
git diff --cached --name-only
git diff origin/main...HEAD --name-only

***REMOVED*** Count files changed
git diff origin/main...HEAD --stat | tail -1

***REMOVED*** Check for database changes
git diff origin/main...HEAD --name-only | grep -E "(alembic|models)"

***REMOVED*** Check for frontend/backend span
...
***REMOVED******REMOVED******REMOVED*** Step 2: Check Scratchpad Entry
...
```bash
***REMOVED*** Check for recent scratchpad entries
find docs/development -name "*.md" -mtime -1 | head -10

***REMOVED*** Check for session handoffs
find docs/development -name "SESSION_*.md" -mtime -1

***REMOVED*** Check for planning docs
...
***REMOVED******REMOVED******REMOVED*** Step 3: Check CHANGELOG Update

**Location:** `CHANGELOG.md` (project root)

```bash
***REMOVED*** Check if CHANGELOG was modified
git diff origin/main...HEAD --name-only | grep "CHANGELOG.md"

***REMOVED*** View CHANGELOG additions
git diff origin/main...HEAD -- CHANGELOG.md | grep "^+" | head -20
```

**CHANGELOG entry must:**
- [ ] Be under `***REMOVED******REMOVED*** [Unreleased]` section
...
***REMOVED******REMOVED******REMOVED*** Step 4: Check Test Coverage

**Backend tests:** `backend/tests/`
**Frontend tests:** `frontend/__tests__/` or `*.test.ts(x)`

```bash
***REMOVED*** Check for new/modified tests
...
***REMOVED******REMOVED******REMOVED*** Step 5: Check Plan File (If Complex)
...
***REMOVED******REMOVED*** Quick Validation Script

Run this before creating a PR:

```bash
***REMOVED***!/bin/bash
***REMOVED*** Pre-PR Checklist Validation

echo "============================================================"
echo "PRE-PR CHECKLIST VALIDATION"
echo "============================================================"

***REMOVED*** Get changed files
...
***REMOVED*** Scratchpad/Plan check (if complex)
echo "============================================================"
...
***REMOVED******REMOVED*** Checklist by Change Type

***REMOVED******REMOVED******REMOVED*** New Feature Checklist

- [ ] **Scratchpad entry** - Document motivation, approach, decisions
- [ ] **CHANGELOG entry** - Under `***REMOVED******REMOVED******REMOVED*** Added` with user-friendly description
...
***REMOVED******REMOVED******REMOVED*** Bug Fix Checklist

- [ ] **Scratchpad entry** - If debugging was complex
- [ ] **CHANGELOG entry** - Under `***REMOVED******REMOVED******REMOVED*** Fixed` with what was broken
- [ ] **Regression test** - Test that reproduces the bug
- [ ] **Root cause documented** - In commit message or scratchpad

***REMOVED******REMOVED******REMOVED*** Code Change (Enhancement) Checklist

- [ ] **Scratchpad entry** - If complex
- [ ] **CHANGELOG entry** - Under `***REMOVED******REMOVED******REMOVED*** Changed` if user-facing
- [ ] **Tests updated** - Reflect new behavior
- [ ] **Existing tests pass** - No regressions

***REMOVED******REMOVED******REMOVED*** Documentation Only Checklist

- [ ] **CHANGELOG entry** - Under `***REMOVED******REMOVED******REMOVED*** Changed` if significant
...
```markdown
***REMOVED******REMOVED*** Pre-PR Checklist Results

***REMOVED******REMOVED******REMOVED*** Change Classification
- **Type:** [New Feature / Bug Fix / Code Change / Documentation / Refactoring]
- **Complexity:** [Simple / Complex]
- **Files Changed:** [count]

***REMOVED******REMOVED******REMOVED*** Required Documentation Status
