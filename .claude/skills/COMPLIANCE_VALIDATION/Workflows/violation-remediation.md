***REMOVED*** Workflow: Violation Remediation
...
***REMOVED******REMOVED*** Critical Safety Rule

**NEVER execute fixes without database backup:**

```bash
***REMOVED*** Create backup before ANY remediation
cd backend
alembic downgrade -1  ***REMOVED*** Test rollback capability
alembic upgrade head   ***REMOVED*** Restore
...
***REMOVED******REMOVED******REMOVED*** Step 1: Prioritize Violations

```python
from collections import defaultdict

***REMOVED*** Assume violations loaded from audit
tier1_violations = [...]  ***REMOVED*** From audit
tier2_violations = [...]  ***REMOVED*** From audit

***REMOVED*** Sort by priority (Tier 1 first, then by person affected)
all_violations = []
...
***REMOVED******REMOVED******REMOVED*** Step 2: Analyze Each Violation
...
for i, item in enumerate(sorted_violations, 1):
    ...
    ***REMOVED*** Determine strategy based on constraint type
...
***REMOVED******REMOVED******REMOVED*** Step 3: Execute Remediation (Tier 1 First)
        ...
        try:
            ***REMOVED*** Start transaction
            ...
            if plan['strategy'] == "REMOVE_ASSIGNMENT":
                ***REMOVED*** Find and remove the offending assignment
                ...
                ***REMOVED*** Use swap system to redistribute assignments
                ***REMOVED*** (Simplified - real implementation would find specific assignments)
                print(f"  🔄 Finding swap candidates...")

                ***REMOVED*** Get assignments in violation window
                window_start = v.details.get('window_start')
                window_end = v.details.get('window_end')

                ***REMOVED*** Find assignments to swap
                assignments_to_swap = db.query(Assignment).join(Block).filter(
                    Assignment.person_id == v.person_id,
                    Block.date >= window_start,
                    Block.date <= window_end
                ).limit(3).all()  ***REMOVED*** Swap first 3 to reduce load

                for assignment in assignments_to_swap:
                    ***REMOVED*** Find swap candidate
                    ***REMOVED*** (Real implementation would use get_swap_candidates MCP tool)
                ...
                ***REMOVED*** Remove assignments on a specific day to create day off
                consecutive_dates = v.details.get('consecutive_dates', [])
                if consecutive_dates:
                    ***REMOVED*** Pick middle day
...
***REMOVED******REMOVED******REMOVED*** Step 4: Verify Fixes
...
***REMOVED*** Re-run audit (use audit-current-schedule.md workflow)
***REMOVED*** This is CRITICAL - never assume fixes worked without verification
...
***REMOVED******REMOVED******REMOVED*** Step 5: Impact Assessment
...
***REMOVED*** (Simplified - real implementation would calculate comprehensive metrics)

before_metrics = {
    "total_assignments": len(all_assignments),  ***REMOVED*** Original count
    "avg_hours_per_week": 45.2,  ***REMOVED*** Example
    "coverage_rate": 92.3,       ***REMOVED*** Example
}

after_metrics = {
    "total_assignments": len(assignments),  ***REMOVED*** After remediation
    "avg_hours_per_week": 43.8,  ***REMOVED*** Recalculated
    "coverage_rate": 91.1,       ***REMOVED*** Recalculated
}
...
***REMOVED*** Check for unintended consequences
...
***REMOVED******REMOVED******REMOVED*** Step 6: Generate Remediation Report
...
with open(report_filename, 'w') as f:
    f.write("***REMOVED*** Violation Remediation Report\n\n")
    ...
    f.write("***REMOVED******REMOVED*** Remediation Actions\n\n")
    for i, plan in enumerate(remediation_plans, 1):
        v = plan['violation']
        f.write(f"***REMOVED******REMOVED******REMOVED*** {i}. {v.constraint_type}\n\n")
    ...
    f.write("***REMOVED******REMOVED*** Impact Assessment\n\n")
    ...
    if len(new_violations) > 0:
        f.write("***REMOVED******REMOVED*** Remaining Violations\n\n")
        for v in new_violations:
...
***REMOVED******REMOVED******REMOVED*** Step 7: Document Changes (Audit Trail)

```python
***REMOVED*** Log remediation to audit trail table
from app.models.audit_log import AuditLog  ***REMOVED*** If exists
...
***REMOVED******REMOVED*** Remediation Strategies Reference
...
***REMOVED******REMOVED*** Safety Checklist
...
***REMOVED******REMOVED******REMOVED*** Fix Doesn't Resolve Violation
```
...
***REMOVED******REMOVED******REMOVED*** New Violations Introduced
```
...
***REMOVED******REMOVED******REMOVED*** Cannot Find Swap Candidates
```
