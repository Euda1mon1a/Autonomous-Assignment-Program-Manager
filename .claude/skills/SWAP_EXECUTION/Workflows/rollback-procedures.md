...
***REMOVED******REMOVED*** Rollback Window
...
class SwapSnapshot:
    ...
    ***REMOVED*** Original assignments for source week
    ...
    [
        {
            ...
            "person_id": "uuid-john",  ***REMOVED*** Original person
            "rotation": "FMIT",
            "date": "2025-02-03",
            "session": "AM"
        },
        ***REMOVED*** ... 13 more (7 days × 2 sessions)
    ]
    """

    ***REMOVED*** Original assignments for target week (if one-to-one swap)
    target_week_assignments: list[dict] | None

    ***REMOVED*** Original call assignments (Friday/Saturday)
    ...
    [
        {
            ...
            "person_id": "uuid-john",  ***REMOVED*** Original person on call
            "date": "2025-02-07"  ***REMOVED*** Friday
        },
        {
            "call_assignment_id": "uuid-call-2",
            "person_id": "uuid-john",
            "date": "2025-02-08"  ***REMOVED*** Saturday
        }
    ]
    """
...
def create_swap_snapshot(db: Session, swap_request: StructuredSwapRequest) -> SwapSnapshot:
    ...
    ***REMOVED*** Calculate week boundaries
    source_week_end = swap_request.source_week + timedelta(days=6)

    ***REMOVED*** Snapshot source week assignments
    source_blocks = (
        db.query(Block)
    ...
    ***REMOVED*** Snapshot target week assignments (if one-to-one)
    ...
    ***REMOVED*** Persist snapshot to database or cache
    ***REMOVED*** (Could use Redis for 24h TTL, or dedicated snapshot table)
...
***REMOVED******REMOVED*** Rollback Triggers

Rollback can be triggered by:

***REMOVED******REMOVED******REMOVED*** 1. Manual Request (Most Common)
...
***REMOVED******REMOVED******REMOVED*** 2. Automatic Detection (System-Initiated)
...
def check_post_execution_compliance(swap_id: UUID) -> None:
    ...
    ***REMOVED*** Re-run full ACGME validation
    ...
    if result.has_violations:
        ...
        ***REMOVED*** Auto-rollback critical violations
...
***REMOVED******REMOVED******REMOVED*** 3. Dispute Resolution
...
def initiate_dispute_rollback(swap_id: UUID, initiated_by: User) -> None:
    ...
    if rollback_result.success:
        ***REMOVED*** Escalate to coordinator for mediation
...
***REMOVED******REMOVED*** State Restoration Process

***REMOVED******REMOVED******REMOVED*** Step 1: Verify Eligibility
...
***REMOVED******REMOVED******REMOVED*** Step 2: Load Snapshot
...
***REMOVED******REMOVED******REMOVED*** Step 3: Restore Database State
...
def restore_from_snapshot(db: Session, snapshot: SwapSnapshot) -> None:
    ...
    try:
        ***REMOVED*** Restore source week assignments
        ...
        ***REMOVED*** Restore target week assignments (if one-to-one)
        ...
        ***REMOVED*** Restore call assignments
...
***REMOVED******REMOVED******REMOVED*** Step 4: Update SwapRecord Status
...
***REMOVED******REMOVED*** Post-Rollback Validation
...
def validate_rollback(snapshot: SwapSnapshot) -> bool:
    ...
    ***REMOVED*** Check source week assignments
    for assignment_data in snapshot.source_week_assignments:
        assignment = db.query(Assignment).filter(
            Assignment.id == assignment_data["assignment_id"]
        ).first()
    ...
    ***REMOVED*** Similar checks for target week and call assignments...
...
***REMOVED******REMOVED*** Notification After Rollback
...
def notify_rollback(swap: SwapRecord) -> None:
    ...
    ***REMOVED*** Notify coordinator (for awareness)
...
***REMOVED******REMOVED*** Rollback Failure Handling
...
def handle_rollback_failure(
    ...
    ***REMOVED*** 1. Flag swap as in error state
    swap = db.query(SwapRecord).filter(SwapRecord.id == swap_id).first()
    swap.status = SwapStatus.PENDING  ***REMOVED*** Revert to pending for manual fix
    swap.notes = f"ROLLBACK FAILED: {error}. Manual intervention required."
    db.commit()

    ***REMOVED*** 2. Create incident ticket
    create_incident(
        severity="CRITICAL",
    ...
    ***REMOVED*** 3. Alert on-call engineer
    ...
    ***REMOVED*** 4. Freeze schedule modifications (prevent further damage)
    enable_read_only_mode(reason=f"Swap rollback failure: {swap_id}")
```

---

***REMOVED******REMOVED*** Common Rollback Scenarios

***REMOVED******REMOVED******REMOVED*** Scenario 1: Requestor Changed Mind
...
***REMOVED******REMOVED******REMOVED*** Scenario 2: Discovered Conflict Post-Execution

**Trigger:** System detects ACGME violation 5 minutes after execution

```python
***REMOVED*** Automated check
result = check_post_execution_compliance(swap_id)

***REMOVED*** Auto-rollback triggered
{
    "success": true,
    "swap_id": "uuid",
...
***REMOVED******REMOVED******REMOVED*** Scenario 3: Data Entry Error
...
***REMOVED*** After rollback, submit corrected request
POST /api/swap/request
{
    "source_faculty_id": "uuid-a",
    "source_week": "2025-02-10",  ***REMOVED*** Corrected week
...
***REMOVED******REMOVED*** Rollback Rate Monitoring
...
***REMOVED*** Example output:
{
    ...
    "quality_status": "GOOD"  ***REMOVED*** <5% rollback rate is healthy
}
```
...
***REMOVED******REMOVED******REMOVED*** Rollback Window

```python
ROLLBACK_WINDOW_HOURS = 24
SNAPSHOT_TTL_HOURS = 25  ***REMOVED*** Keep slightly longer than rollback window
```

***REMOVED******REMOVED******REMOVED*** Status Transitions
