***REMOVED*** Safety Checks Workflow
...
***REMOVED******REMOVED*** Three-Tier Validation
...
***REMOVED******REMOVED******REMOVED*** Decision Logic
...
def evaluate_swap_safety(request: StructuredSwapRequest) -> Decision:
    ...
    if tier1_result.has_violations:
        return Decision.REJECT  ***REMOVED*** Hard stop

    tier2_result = check_tier2_soft_constraints(request)
    if tier2_result.has_critical_warnings:
        return Decision.FLAG  ***REMOVED*** Escalate for approval

    tier3_result = check_tier3_resilience_impact(request)
    if tier3_result.degradation > 0.15:  ***REMOVED*** >15% degradation
        return Decision.FLAG  ***REMOVED*** Escalate for approval

    return Decision.PROCEED  ***REMOVED*** Execute immediately
```

---

***REMOVED******REMOVED*** Tier 1: Hard Constraints (MUST PASS)

These are **ACGME compliance rules** that cannot be violated. Any failure = instant rejection.

***REMOVED******REMOVED******REMOVED*** 1.1 80-Hour Work Week Rule
...
***REMOVED*** Calculate impact for BOTH parties
...
source_hours_after = calculator.get_4week_average_after_swap(
    person_id=request.source_faculty.id,
    week_ending=request.source_week,
    swap_delta=-FMIT_WEEK_HOURS  ***REMOVED*** Giving away week
)
...
target_hours_after = calculator.get_4week_average_after_swap(
    person_id=request.target_faculty.id,
    week_ending=request.source_week,
    swap_delta=+FMIT_WEEK_HOURS  ***REMOVED*** Taking on week
)

***REMOVED*** Check violations
MAX_HOURS = 80.0
WARN_THRESHOLD = 75.0
...
***REMOVED******REMOVED******REMOVED*** 1.2 1-in-7 Day Off Rule
...
***REMOVED*** Check if target faculty loses their day off
...
***REMOVED******REMOVED******REMOVED*** 1.3 Supervision Ratios
...
***REMOVED*** Calculate supervision ratios for the affected week
ratio_before = validator.get_supervision_ratio(
    date=request.source_week,
    rotation="inpatient"
)

***REMOVED*** Simulate ratio after swap (source faculty removed, target added)
...
***REMOVED******REMOVED******REMOVED*** 1.4 Past Date Check
...
***REMOVED******REMOVED*** Tier 2: Soft Constraints (WARN but may proceed)

These are **fairness and quality** rules. Violations trigger warnings and may require approval.

***REMOVED******REMOVED******REMOVED*** 2.1 Back-to-Back FMIT Weeks
...
***REMOVED******REMOVED******REMOVED*** 2.2 External Conflicts (Absences, TDY, Leave)
...
***REMOVED*** Check target faculty absences during source week
...
conflict = db.query(Absence).filter(
    ...
    Absence.is_blocking == True  ***REMOVED*** Only blocking absences
).first()
...
***REMOVED******REMOVED******REMOVED*** 2.3 Coverage Gaps
...
***REMOVED*** Check if source week has backup coverage
...
if source_coverage.faculty_count == 1 and request.swap_type == SwapType.ABSORB:
    ***REMOVED*** Source is only person covering, and they're giving it away
    return ValidationError(
        code="COVERAGE_GAP",
        severity="error",
...
***REMOVED******REMOVED******REMOVED*** 2.4 Same-Day Conflicts (Call Cascade)
...
***REMOVED*** Check if source week contains Fri/Sat call
has_weekend_call = validator.has_weekend_call_in_week(request.source_week)

if has_weekend_call:
    ***REMOVED*** Get current call assignments for that weekend
...
***REMOVED******REMOVED******REMOVED*** 2.5 Imminent Swap Warning
...
***REMOVED******REMOVED*** Tier 3: Resilience Impact Assessment

Measure how swap affects **system-wide resilience metrics**.

***REMOVED******REMOVED******REMOVED*** 3.1 Utilization Impact
...
***REMOVED******REMOVED******REMOVED*** 3.2 N-1 Contingency Check
...
***REMOVED*** Run N-1 analysis for affected week
...
***REMOVED******REMOVED******REMOVED*** 3.3 Blast Radius Assessment
...
blast_radius = calculate_blast_radius(
    ...
    max_depth=3  ***REMOVED*** Check 3 levels of dependencies
)
...
class SafetyCheckResult:
    ...
    decision: Decision  ***REMOVED*** REJECT | FLAG | PROCEED
...
def make_safety_decision(
    ...
    Returns:
        Decision: REJECT | FLAG | PROCEED
    """
    ***REMOVED*** Tier 1: Hard stop on violations
    if any(e.severity == "error" for e in tier1):
        return Decision.REJECT

    ***REMOVED*** Tier 2: Escalate critical warnings
    critical_tier2 = [
        "BACK_TO_BACK_FMIT",
        "COVERAGE_GAP",
        "EXTERNAL_CONFLICT"
    ...
    ***REMOVED*** Tier 3: Escalate significant degradation
    if tier3.get("utilization_delta", 0) > 0.15:  ***REMOVED*** >15% increase
        return Decision.FLAG
...
```python
{
    "decision": "PROCEED",  ***REMOVED*** or "REJECT" or "FLAG"
...
***REMOVED******REMOVED******REMOVED*** Key Validators
...
***REMOVED******REMOVED******REMOVED*** Constraint Weights
...
TIER_3_THRESHOLDS = {
    "utilization_delta": 0.15,  ***REMOVED*** 15% increase
    "n1_failures": 0,           ***REMOVED*** Zero tolerance
    "blast_radius": 10          ***REMOVED*** Max 10 affected assignments
}
```
