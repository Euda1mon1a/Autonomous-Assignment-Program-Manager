***REMOVED*** Institutional Rules Reference
...
***REMOVED******REMOVED*** Military-Specific Requirements

***REMOVED******REMOVED******REMOVED*** 1. Deployment and TDY (Temporary Duty) Scheduling

***REMOVED******REMOVED******REMOVED******REMOVED*** Deployment Status
...
**Implementation:**
```python
***REMOVED*** Deployment blocks ALL scheduling
if absence.type == 'DEPLOYMENT':
    availability_matrix[person.id][date_range] = 'BLOCKED'
    ***REMOVED*** Cannot override, even for emergencies
```

***REMOVED******REMOVED******REMOVED******REMOVED*** TDY (Temporary Duty) Assignments
...
**Implementation:**
```python
***REMOVED*** TDY reduces capacity but may allow some duties
if absence.type == 'TDY':
    if tdy.allows_remote_work:
        availability_matrix[person.id][date_range] = 'REDUCED'
        ***REMOVED*** Can still do telehealth clinic
    else:
        availability_matrix[person.id][date_range] = 'BLOCKED'
```

***REMOVED******REMOVED******REMOVED*** 2. Duty Restrictions

***REMOVED******REMOVED******REMOVED******REMOVED*** Security Clearance Requirements
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Physical Fitness Standards
...
***REMOVED******REMOVED*** Program Curriculum Requirements

***REMOVED******REMOVED******REMOVED*** 1. FMIT (Family Medicine Inpatient Training) Sequencing

***REMOVED******REMOVED******REMOVED******REMOVED*** Requirement
...
def validate_fmit_sequencing(schedule):
    ...
    for resident in get_pgy1_residents():
        fmit_assignments = [
            ...
            and a.date <= academic_year_start + timedelta(days=180)  ***REMOVED*** 6 months
        ]
...
***REMOVED******REMOVED******REMOVED*** 2. Night Float (NF) Rotation Pattern
...
class NightFloatPostCallConstraint:
    ...
    def validate(self, assignments):
        for block in blocks:
            ...
            ***REMOVED*** Check exactly 1 resident on NF per half-block
...
***REMOVED******REMOVED******REMOVED*** 3. Post-Call (PC) Day Requirements
...
def assign_continuity_clinic(resident, week):
    ...
    ***REMOVED*** Check if on blocking rotation
    ...
    if current_rotation.name not in blocking_rotations:
        ***REMOVED*** Assign continuity clinic
        clinic_day = resident.continuity_clinic_day  ***REMOVED*** e.g., Wednesday
...
***REMOVED******REMOVED******REMOVED*** 5. Procedure Rotation Requirements
...
***REMOVED******REMOVED******REMOVED*** 1. Minimum Staffing Levels

***REMOVED******REMOVED******REMOVED******REMOVED*** Rotation-Specific Minimums
...
***REMOVED******REMOVED******REMOVED*** 2. Holiday Coverage
...
def assign_holiday_coverage(holiday, year):
    ...
    ***REMOVED*** Get residents who worked this holiday last year
    worked_last_year = get_holiday_workers(holiday, year - 1)

    ***REMOVED*** Deprioritize them this year
    candidates = [
        r for r in residents
        if r not in worked_last_year
        and get_holiday_count(r, year) < 2  ***REMOVED*** Haven't worked 2 holidays yet
    ]

    ***REMOVED*** Select from candidates
    selected = select_by_fairness(candidates)
    assign(selected, holiday, 'Holiday_Coverage')

    ***REMOVED*** Log for next year
    log_holiday_assignment(selected, holiday, year)
```

***REMOVED******REMOVED******REMOVED*** 3. Weekend Coverage
...
def assign_weekend_coverage(month):
    ...
    for weekend in weekends:
        ***REMOVED*** Find residents with fewest weekends this month
        candidates = sorted(residents, key=lambda r: get_weekend_count(r, month))

        ***REMOVED*** Exclude those who worked last weekend
        candidates = [c for c in candidates if not worked_last_weekend(c)]

        ***REMOVED*** Assign from candidates
...
***REMOVED******REMOVED*** Leave and Absence Policies

***REMOVED******REMOVED******REMOVED*** 1. Vacation Leave

***REMOVED******REMOVED******REMOVED******REMOVED*** Allowance
...
***REMOVED******REMOVED******REMOVED*** 3. Sick Leave

***REMOVED******REMOVED******REMOVED******REMOVED*** Policy
...
***REMOVED******REMOVED******REMOVED*** 4. Parental Leave
...
def schedule_parental_leave(resident, estimated_date, duration_weeks):
    ...
    ***REMOVED*** Block scheduling around due date (±2 weeks)
    ...
    ***REMOVED*** Adjust rotation schedule
    pause_rotations(resident, buffer_start, buffer_end)

    ***REMOVED*** May extend graduation date
...
***REMOVED******REMOVED*** Call Schedule Requirements

***REMOVED******REMOVED******REMOVED*** 1. Call Equity
...
***REMOVED******REMOVED******REMOVED*** 2. Call Frequency

***REMOVED******REMOVED******REMOVED******REMOVED*** Limits
...
***REMOVED******REMOVED******REMOVED*** Academic Year Structure
...
***REMOVED******REMOVED*** Special Rotation Rules

***REMOVED******REMOVED******REMOVED*** 1. Sports Medicine Rotation
...
***REMOVED******REMOVED******REMOVED*** 2. International Health Rotation
...
***REMOVED******REMOVED*** Monitoring and Compliance

***REMOVED******REMOVED******REMOVED*** Weekly Checks
...
***REMOVED******REMOVED*** Policy Override Authority
