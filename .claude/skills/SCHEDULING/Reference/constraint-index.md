...
***REMOVED******REMOVED*** Tier 1: Absolute Constraints (ACGME)

Cannot be violated under any circumstances. Schedule is invalid if any Tier 1 constraint fails.

***REMOVED******REMOVED******REMOVED*** 1.1 Work Hour Constraint (80-Hour Rule)
...
***REMOVED******REMOVED******REMOVED*** 1.2 Days Off Constraint (1-in-7 Rule)
...
rolling_window_weeks = 4
day_off_hours = 24  ***REMOVED*** Consecutive hours
```
...
for four_week_period in all_periods:
    ...
    if days_off < 4:  ***REMOVED*** Less than 1 per week
...
***REMOVED******REMOVED******REMOVED*** 1.3 Supervision Ratio Constraint (PGY-1)
...
supervision_type = 'direct'  ***REMOVED*** Direct supervision required
```
...
***REMOVED******REMOVED******REMOVED*** 1.4 Supervision Ratio Constraint (PGY-2/3)
...
***REMOVED******REMOVED******REMOVED*** 1.5 Duty Period Limit Constraint
...
transition_hours = 4  ***REMOVED*** For handoff/education
min_rest_after_24h = 14  ***REMOVED*** Hours
min_rest_standard = 8   ***REMOVED*** Hours
```
...
***REMOVED******REMOVED******REMOVED*** 1.6 Night Float Limit Constraint
...
min_rest_after_nf = 24  ***REMOVED*** Hours
```
...
***REMOVED******REMOVED******REMOVED*** 1.7 On-Call Frequency Constraint
...
**Parameters:**
```python
min_days_between_call = 2  ***REMOVED*** Every 3rd night = 2 days between
rolling_window_weeks = 4
```
...
for four_week_period in all_periods:
    ...
    if len(call_nights) > 9:  ***REMOVED*** 28 days / 3 ≈ 9
...
***REMOVED******REMOVED*** Tier 2: Institutional Constraints (Policy)

Require Program Director approval to override. Violations documented as exceptions.

***REMOVED******REMOVED******REMOVED*** 2.1 Deployment Blocking Constraint
...
blocking_type = 'absolute'  ***REMOVED*** No exceptions
```
...
***REMOVED******REMOVED******REMOVED*** 2.2 FMIT Sequencing Constraint
...
rotation_name = 'FMIT'
pgy_level = 1
deadline = academic_year_start + timedelta(days=180)  ***REMOVED*** 6 months
```
...
***REMOVED******REMOVED******REMOVED*** 2.3 Night Float Post-Call Constraint
...
pc_day_offset = 1  ***REMOVED*** Day after NF ends
```
...
***REMOVED******REMOVED******REMOVED*** 2.4 Hard Preference Block Constraint
...
***REMOVED******REMOVED******REMOVED*** 2.5 Minimum Coverage Constraint
...
***REMOVED******REMOVED******REMOVED*** 2.6 Continuity Clinic Constraint
...
***REMOVED******REMOVED******REMOVED*** 2.7 Qualification Constraint
...
***REMOVED******REMOVED*** Tier 3: Optimization Constraints (Preferences)

Best-effort satisfaction. Can be relaxed without approval. Tracked for quality metrics.

***REMOVED******REMOVED******REMOVED*** 3.1 Fairness (Workload Equity) Constraint
...
if gini > 0.15:
    penalty = (gini - 0.15) * 1000  ***REMOVED*** Soft penalty
...
***REMOVED******REMOVED******REMOVED*** 3.2 Call Equity Constraint
...
weight_holiday = 1.5  ***REMOVED*** Holidays count more
```
...
***REMOVED******REMOVED******REMOVED*** 3.3 Shift Preference Constraint
...
satisfaction_weight = 0.20  ***REMOVED*** In objective function
...
***REMOVED******REMOVED******REMOVED*** 3.4 Rotation Preference Constraint
...
***REMOVED******REMOVED******REMOVED*** 3.5 Continuity (Minimize Handoffs) Constraint
...
***REMOVED******REMOVED******REMOVED*** 3.6 Efficiency (Minimize Gaps) Constraint
...
***REMOVED******REMOVED******REMOVED*** 3.7 Resilience (80% Utilization) Constraint
...
for person in all_people:
    capacity = calculate_max_capacity(person)  ***REMOVED*** e.g., 80 hours/week
    scheduled = calculate_scheduled_hours(person)
    utilization = scheduled / capacity

    if utilization > 0.80:
        penalty += (utilization - 0.80) * 100 * 20  ***REMOVED*** 20 per percent over
...
***REMOVED******REMOVED*** Constraint Interaction Matrix
...
***REMOVED******REMOVED*** Constraint Relaxation Strategy
...
***REMOVED******REMOVED*** Implementation Reference

***REMOVED******REMOVED******REMOVED*** Constraint System Architecture

**Base Class:**
```python
***REMOVED*** backend/app/scheduling/constraints/base.py
class BaseConstraint:
    tier: int  ***REMOVED*** 1=ACGME, 2=Institutional, 3=Optimization
    priority: int  ***REMOVED*** 0-2 (lower = higher priority)
    type: str  ***REMOVED*** 'hard' or 'soft'
...
***REMOVED******REMOVED******REMOVED*** Constraint Files
...
***REMOVED******REMOVED*** Quick Reference: Constraint Priorities
