***REMOVED*** Generate Schedule Workflow
...
***REMOVED******REMOVED*** Phase 1: Requirements Gathering

***REMOVED******REMOVED******REMOVED*** Step 1.1: Define Scheduling Horizon

**Determine the time period to schedule:**

```bash
***REMOVED*** For academic year (typical)
START_DATE="2026-07-01"
END_DATE="2027-06-30"

***REMOVED*** For single block (4 weeks)
START_DATE="2026-07-01"
END_DATE="2026-07-28"

***REMOVED*** For quarter (13 weeks)
START_DATE="2026-07-01"
END_DATE="2026-09-30"
```
...
```python
***REMOVED*** Each block is 4 weeks (28 days)
from datetime import datetime
start = datetime.fromisoformat("2026-07-01")
end = datetime.fromisoformat("2027-06-30")
days = (end - start).days
blocks = days // 28  ***REMOVED*** Typically 13 blocks per year
```
...
***REMOVED******REMOVED******REMOVED*** Step 1.2: Gather Personnel Data
...
***REMOVED******REMOVED******REMOVED*** Step 1.3: Collect Absence Data

**Get resident absences:**

```bash
***REMOVED*** Absences in scheduling horizon
curl -s -H "Authorization: Bearer $TOKEN" \
...
***REMOVED******REMOVED******REMOVED*** Step 1.4: Document Rotation Requirements
...
```bash
***REMOVED*** Check minimum coverage per rotation
curl -s -H "Authorization: Bearer $TOKEN" \
  "http://localhost:8000/api/v1/rotation-templates" \
...
***REMOVED******REMOVED******REMOVED*** Step 1.5: Capture Preferences

**Collect preference data:**

```bash
***REMOVED*** Get submitted preferences
curl -s -H "Authorization: Bearer $TOKEN" \
...
***REMOVED******REMOVED******REMOVED*** Step 1.6: Review Institutional Policies
...
***REMOVED******REMOVED******REMOVED*** Step 1.7: Create Scheduling Context

**Tool invocation:**

```bash
***REMOVED*** MCP tool (if available)
...
***REMOVED******REMOVED*** Phase 2: Constraint Propagation
...
***REMOVED******REMOVED******REMOVED*** Step 2.1: Apply ACGME Constraints

**Run ACGME constraint check:**

```bash
***REMOVED*** Pre-validate ACGME constraints with current data
...
***REMOVED******REMOVED******REMOVED*** Step 2.2: Apply Availability Constraints

**Check availability matrix:**

```bash
***REMOVED*** Verify absences don't over-constrain
curl -s -H "Authorization: Bearer $TOKEN" \
...
***REMOVED******REMOVED******REMOVED*** Step 2.3: Apply Qualification Constraints

**Verify personnel qualifications:**

```bash
***REMOVED*** Check qualification mapping
curl -s -H "Authorization: Bearer $TOKEN" \
...
***REMOVED******REMOVED******REMOVED*** Step 2.4: Identify Early Conflicts

**Run feasibility analysis:**

```bash
***REMOVED*** Check if problem is solvable
...
***REMOVED******REMOVED*** Phase 3: Optimization

**Purpose:** Generate high-quality schedules using solver algorithms

***REMOVED******REMOVED******REMOVED*** Step 3.1: Select Solver Algorithm

**Algorithm selection guide:**

```python
***REMOVED*** Calculate problem complexity score
residents = 24
blocks = 13
rotations = 18
absences = 45
...
if complexity < 20:
    solver = "greedy"  ***REMOVED*** Fast, explainable
elif complexity < 50:
    solver = "pulp"    ***REMOVED*** Fast, scalable
elif complexity < 75:
    solver = "cp_sat"  ***REMOVED*** Optimal, moderate time
else:
    solver = "hybrid"  ***REMOVED*** Reliable, longer time
```
...
***REMOVED******REMOVED******REMOVED*** Step 3.2: Configure Optimization Objectives
...
***REMOVED******REMOVED******REMOVED*** Step 3.3: Run Solver with Monitoring
...
```bash
***REMOVED*** Watch backend logs for solver progress
...
***REMOVED******REMOVED******REMOVED*** Step 3.4: Evaluate Solution Quality
...
***REMOVED******REMOVED******REMOVED*** Step 3.5: Review Alternative Solutions
...
***REMOVED******REMOVED******REMOVED*** Step 4.2: Attempt Automated Resolution

**For soft violations, try constraint relaxation:**

```bash
***REMOVED*** If fairness slightly violated, regenerate with relaxed fairness
curl -X POST http://localhost:8000/api/v1/schedule/generate \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** Step 4.3: Manual Conflict Resolution
...
***REMOVED******REMOVED*** Phase 5: Validation & Deployment

**Purpose:** Final verification and safe deployment

***REMOVED******REMOVED******REMOVED*** Step 5.1: Comprehensive ACGME Validation
...
***REMOVED******REMOVED******REMOVED*** Step 5.2: Verify Coverage Requirements
...
***REMOVED******REMOVED******REMOVED*** Step 5.3: Run Resilience Checks
...
***REMOVED******REMOVED******REMOVED*** Step 5.4: Create Final Backup

**Mandatory before deployment:**

```bash
***REMOVED*** Create pre-deployment backup
./scripts/backup-db.sh --docker
...
***REMOVED******REMOVED******REMOVED*** Step 5.5: Deploy Schedule
...
***REMOVED******REMOVED******REMOVED*** Step 5.6: Generate Reports
...
***REMOVED******REMOVED******REMOVED*** Adjustments
...
***REMOVED******REMOVED******REMOVED*** Solver Times Out
...
***REMOVED******REMOVED******REMOVED*** No Feasible Solution
...
***REMOVED******REMOVED******REMOVED*** Poor Quality Score
...
***REMOVED******REMOVED******REMOVED*** ACGME Violations in Result
