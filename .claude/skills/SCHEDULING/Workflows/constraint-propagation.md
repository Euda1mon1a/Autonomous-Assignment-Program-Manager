***REMOVED*** Constraint Propagation Workflow
...
***REMOVED******REMOVED*** Constraint Hierarchy

***REMOVED******REMOVED******REMOVED*** Tier 1: Absolute Constraints (ACGME Regulatory)
...
***REMOVED******REMOVED******REMOVED*** Tier 2: Institutional Constraints (Policy-Based)
...
***REMOVED******REMOVED******REMOVED*** Tier 3: Optimization Constraints (Preferences)
...
***REMOVED******REMOVED*** Propagation Algorithm

***REMOVED******REMOVED******REMOVED*** Step 1: Gather All Constraints
...
```python
***REMOVED*** Query absences in scheduling horizon
absences = db.query(Absence).filter(
    Absence.start_date >= schedule_start,
    Absence.end_date <= schedule_end
).all()

***REMOVED*** Create blocking constraints
for absence in absences:
    if absence.type in ['DEPLOYMENT', 'EXTENDED_LEAVE']:
        ***REMOVED*** Complete block - person cannot be assigned
        ...
        ***REMOVED*** Partial block - reduces capacity but may allow some assignments
...
```python
***REMOVED*** Each template defines required qualifications
...
**From preferences:**
```python
***REMOVED*** Preferences become soft constraints with penalties
for pref in preferences:
    if pref.type == 'HARD_BLOCK':
        ***REMOVED*** Treat as Tier 2 constraint
        hard_constraints.append(
            PreferenceBlockConstraint(person=pref.person, dates=pref.dates)
        )
    else:
        ***REMOVED*** Soft constraint with penalty
        soft_constraints.append(
            PreferenceSoftConstraint(person=pref.person, details=pref.details, weight=pref.priority)
        )
```

***REMOVED******REMOVED******REMOVED*** Step 2: Build Availability Matrix
...
def build_availability_matrix(people, blocks, absences):
    ...
    for person in people:
        for block in blocks:
            ***REMOVED*** Default to available
            matrix[person.id][block.id] = "AVAILABLE"

            ***REMOVED*** Check for blocking absences
            for absence in absences:
                if absence.person_id != person.id:
                    continue

                ***REMOVED*** Check if absence overlaps block
                if overlaps(absence.date_range, block.date_range):
                    if absence.type in ['DEPLOYMENT', 'EXTENDED_LEAVE']:
                        matrix[person.id][block.id] = "BLOCKED"
                        break  ***REMOVED*** Blocked is strongest status
                    elif absence.type in ['CONFERENCE', 'VACATION']:
                        ***REMOVED*** Only reduce if not already blocked
...
def check_coverage_risks(availability_matrix, rotation_templates):
    ...
    for rotation in rotation_templates:
        for block in blocks:
            ***REMOVED*** Count available people for this rotation
            available_count = sum(
...
***REMOVED******REMOVED******REMOVED*** Step 3: Apply Domain Reduction
...
def can_do_activity_type(person, activity_type):
    ...
    ***REMOVED*** Example restrictions
    if activity_type == 'night_float' and person.pgy_level == 1:
        ***REMOVED*** Policy: PGY-1 cannot do solo night float
        return False
    return True
```

***REMOVED******REMOVED******REMOVED*** Step 4: Constraint Propagation Iterations
...
def propagate_constraints(valid_assignments, constraints, max_iterations=10):
    ...
    while changed and iteration < max_iterations:
        ...
        for constraint in constraints:
            ***REMOVED*** Apply constraint and get eliminated assignments
            ...
            if eliminations:
                ...
                for (person_id, block_id, rotation_id) in eliminations:
                    if rotation_id in valid_assignments[(person_id, block_id)]:
                        ...
                        ***REMOVED*** Check if domain became empty
...
class WorkHourConstraint:
    ...
    def propagate(self, valid_assignments):
        ...
        for person_id in people:
            ***REMOVED*** Calculate current work hours by week
            weekly_hours = calculate_weekly_hours(person_id, current_assignments)

            for week in weeks:
                if weekly_hours[week] > 60:  ***REMOVED*** Getting close to limit
                    ***REMOVED*** Eliminate high-hour rotations in this week
                    for block in blocks_in_week(week):
                        for rotation_id in valid_assignments[(person_id, block.id)]:
                            rotation = get_rotation(rotation_id)
                            if rotation.weekly_hours > 10:  ***REMOVED*** Would push over limit
                                eliminations.append((person_id, block.id, rotation_id))

        return eliminations
```

***REMOVED******REMOVED******REMOVED*** Step 5: Detect Conflicts
...
def are_conflicting(c1, c2):
    ...
    ***REMOVED*** Example: Hard block preference conflicts with coverage requirement
...
def classify_severity(c1, c2):
    ...
    if tier1 == 1 or tier2 == 1:
        return 'CRITICAL'  ***REMOVED*** ACGME conflict
    elif tier1 == 2 or tier2 == 2:
        return 'HIGH'      ***REMOVED*** Institutional conflict
    else:
        return 'MEDIUM'    ***REMOVED*** Preference conflict
```

---

***REMOVED******REMOVED*** Backtracking Strategies

***REMOVED******REMOVED******REMOVED*** When to Backtrack
...
***REMOVED******REMOVED******REMOVED*** Backtracking Approaches

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Constraint Relaxation
...
def backtrack_with_relaxation(constraints, valid_assignments):
    ...
    ***REMOVED*** Try with all constraints
    try:
    ...
    ***REMOVED*** Remove Tier 3 constraints and retry
    ...
    ***REMOVED*** Remove some Tier 2 constraints (requires approval)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Coverage Adjustment
    ...
    for rotation in rotation_templates:
        ...
        ***REMOVED*** Try reducing by 1
        ...
        try:
            ...
            ***REMOVED*** Restore and try next rotation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Temporal Decomposition
...
def backtrack_with_decomposition(blocks, constraints):
    ...
    ***REMOVED*** Divide into quarters (13 weeks / 4 = ~3-4 blocks each)
    ...
    for quarter_num, quarter_blocks in enumerate(quarters):
        ...
        ***REMOVED*** Use previous quarter's end state as starting point
        ...
        try:
            ...
            ***REMOVED*** Try relaxing constraints for this quarter only
...
***REMOVED******REMOVED******REMOVED*** Types of Conflicts

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. Hard Conflicts (Cannot Both Be True)

**Example: Deployment vs. Coverage Requirement**

```python
***REMOVED*** Person A is deployed (unavailable)
deployment_constraint = BlockingConstraint(
    person='PGY2-01',
    dates='2026-03-01 to 2026-03-28'
)

***REMOVED*** But coverage requires Person A for critical rotation
...
***REMOVED*** CONFLICT: Person cannot be both unavailable and required
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Soft Conflicts (Can Coexist But Suboptimal)

**Example: Preference vs. Fairness**

```python
***REMOVED*** Resident prefers AM shifts
preference_constraint = ShiftPreferenceConstraint(
    person='PGY1-05',
    preferred_session='AM'
)

***REMOVED*** But fairness requires PM shift to balance workload
...
***REMOVED*** SOFT CONFLICT: Can assign PM but violates preference
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Cascading Conflicts (One Violation Causes Others)

**Example: Post-Call Violation Chain**

```python
***REMOVED*** Resident does 24-hour call on Friday
call_assignment = Assignment(
    person='PGY3-02',
    rotation='Inpatient Call',
    date='2026-03-15',  ***REMOVED*** Friday
    session='24-hour'
)

***REMOVED*** Post-call constraint requires Saturday off
post_call_constraint = PostCallReliefConstraint(
    person='PGY3-02',
    required_off_date='2026-03-16'  ***REMOVED*** Saturday
)

***REMOVED*** But coverage requires this person Saturday
...
***REMOVED*** And they're scheduled for clinic Sunday
sunday_assignment = Assignment(
    person='PGY3-02',
    rotation='Continuity Clinic',
    date='2026-03-17',  ***REMOVED*** Sunday
    session='AM'
)

***REMOVED*** CASCADING: Post-call violation + 1-in-7 violation (no day off in 7 days)
```
...
***REMOVED******REMOVED******REMOVED*** Conflict Detection Algorithm
...
def detect_all_conflicts(assignments, constraints):
    ...
    conflicts = {
        'hard': [],      ***REMOVED*** Must be resolved
        'soft': [],      ***REMOVED*** Should be resolved
        'cascading': []  ***REMOVED*** Chain reactions
    }

    ***REMOVED*** Check each constraint against current assignments
    for constraint in constraints:
        ...
        for violation in violations:
            ...
            ***REMOVED*** Classify conflict type
            if constraint.tier == 1:  ***REMOVED*** ACGME
                conflicts['hard'].append(conflict)
            elif constraint.tier == 2:  ***REMOVED*** Institutional
                ***REMOVED*** Check if it triggers cascade
                if triggers_cascade(conflict, constraints):
                    conflicts['cascading'].append(conflict)
                else:
                    conflicts['hard'].append(conflict)
            else:  ***REMOVED*** Preference
                conflicts['soft'].append(conflict)

    ***REMOVED*** Identify conflict chains
...
def triggers_cascade(conflict, all_constraints):
    ...
    ***REMOVED*** Simulate removing the conflicting assignment
    test_assignments = remove_assignment(conflict.violation.assignment)

    ***REMOVED*** Check if this creates new violations
    new_violations = []
...
***REMOVED******REMOVED*** Propagation Performance
...
def check_qualification(person_id, rotation_id):
    ...
    ***REMOVED*** Expensive database lookup cached
    pass
```

**2. Index data structures:**
```python
***REMOVED*** Index absences by person for O(1) lookup
...
def propagate_constraints(valid_assignments, constraints):
    for constraint in sorted(constraints, key=lambda c: c.selectivity, reverse=True):
        ***REMOVED*** Apply most selective constraints first
        ***REMOVED*** Stop if domain becomes empty
...
***REMOVED******REMOVED*** Diagnostic Output

***REMOVED******REMOVED******REMOVED*** Propagation Report Format
