...
***REMOVED******REMOVED******REMOVED*** Example 1: Adding Nullable Column with Backfill

**Context:** Adding `middle_name` field to Person model

**Input:**
```python
***REMOVED*** Requirement: Add optional middle name field to Person model
***REMOVED*** Model: backend/app/models/person.py
class Person(Base):
    __tablename__ = "persons"
    id = Column(String, primary_key=True)
    first_name = Column(String, nullable=False)
    last_name = Column(String, nullable=False)
    ***REMOVED*** Need to add: middle_name
```
...
4. Test migration:
   ```bash
   alembic upgrade head
   alembic current  ***REMOVED*** Verify applied
   alembic downgrade -1  ***REMOVED*** Test rollback
   alembic upgrade head  ***REMOVED*** Re-apply
   ```
...
***REMOVED******REMOVED******REMOVED*** Example 2: Adding Non-Nullable Column (Three-Phase Migration)

**Context:** Adding required `email_verified` boolean to User model

**Input:**
```python
***REMOVED*** Requirement: All users must have email_verified status (non-nullable)
***REMOVED*** Current state: Field doesn't exist
***REMOVED*** Problem: Can't add non-nullable column to table with existing rows
```

**Process:**
1. **Phase 1: Add as nullable**
   ```python
   ***REMOVED*** Migration 1: abc123_add_email_verified_nullable.py
   def upgrade() -> None:
...
2. **Phase 2: Backfill data**
   ```python
   ***REMOVED*** Migration 2: abc124_backfill_email_verified.py
   def upgrade() -> None:
       ***REMOVED*** Set default value for existing rows
       op.execute("UPDATE users SET email_verified = false WHERE email_verified IS NULL")
   ```

3. **Phase 3: Make non-nullable**
   ```python
   ***REMOVED*** Migration 3: abc125_make_email_verified_required.py
   def upgrade() -> None:
...
***REMOVED******REMOVED******REMOVED*** Example 3: Foreign Key Migration with Data Integrity

**Context:** Linking Assignment to Rotation via foreign key

**Input:**
```python
***REMOVED*** Requirement: Assignments must reference valid Rotations
***REMOVED*** Current: assignment.rotation_name (string, no FK constraint)
***REMOVED*** Target: assignment.rotation_id (FK to rotations.id)
```
...
2. **Backfill from existing data**
   ```python
   def upgrade() -> None:
       ***REMOVED*** Map rotation names to IDs
       op.execute("""
       ...
       ***REMOVED*** Identify orphaned records
       op.execute("""
...
3. **Add foreign key constraint**
   ```python
   def upgrade() -> None:
       ***REMOVED*** Make non-null
       op.alter_column('assignments', 'rotation_id', nullable=False)

       ***REMOVED*** Add FK constraint
       op.create_foreign_key(
           'fk_assignment_rotation',
           'assignments', 'rotations',
           ['rotation_id'], ['id'],
           ondelete='RESTRICT'  ***REMOVED*** Prevent deletion of rotations in use
       )

       ***REMOVED*** Drop old column after verification
...
```python
***REMOVED*** Example migration test
def test_migration_adds_middle_name():
    ***REMOVED*** Downgrade to before migration
    alembic_downgrade()

    ***REMOVED*** Verify column doesn't exist
...
│  ┌────────────────────────────────────────────────────┐         │
│  │ alembic upgrade head    ***REMOVED*** Apply migration          │         │
...
***REMOVED******REMOVED*** Concrete Usage Example: Adding Credential Tracking

**Scenario:** Add credential tracking fields to Person model for HIPAA/training compliance.

***REMOVED******REMOVED******REMOVED*** Complete Migration Walkthrough
...
class Person(Base):
    ...
    ***REMOVED*** NEW FIELDS for credential tracking
    credentials = Column(JSON, nullable=True)  ***REMOVED*** {credential_type: expiry_date}
...
***REMOVED*** Output:
***REMOVED*** INFO  [alembic.autogenerate.compare] Detected added column 'persons.credentials'
***REMOVED*** INFO  [alembic.autogenerate.compare] Detected added column 'persons.last_training_date'
***REMOVED*** INFO  [alembic.autogenerate.compare] Detected added column 'persons.hipaa_certified'
***REMOVED*** INFO  [alembic.autogenerate.compare] Detected added column 'persons.cyber_awareness_date'
***REMOVED*** Generating /backend/alembic/versions/a1b2c3d4e5f6_add_credential_tracking.py...done
```

**Step 3: Review and Enhance Generated Migration**

```python
***REMOVED*** backend/alembic/versions/a1b2c3d4e5f6_add_credential_tracking.py
...
def upgrade() -> None:
    ***REMOVED*** Add new columns (all nullable initially)
    ...
    ***REMOVED*** Add boolean with default - must do in two steps for existing rows
    op.add_column('persons', sa.Column('hipaa_certified', sa.Boolean(), nullable=True))

    ***REMOVED*** ENHANCEMENT: Backfill existing rows with defaults
    op.execute("""
        UPDATE persons
    ...
    ***REMOVED*** Now make hipaa_certified non-nullable
...
def downgrade() -> None:
    ***REMOVED*** Remove columns in reverse order
...
***REMOVED*** Check current state
alembic current
***REMOVED*** Output: 9z8y7x6w5v4u (previous migration)

***REMOVED*** Apply migration
alembic upgrade head
***REMOVED*** Output:
***REMOVED*** INFO  [alembic.runtime.migration] Running upgrade 9z8y7x6w5v4u -> a1b2c3d4e5f6, Add credential tracking

***REMOVED*** Verify schema changed
docker-compose exec db psql -U scheduler -d residency_scheduler -c "\d persons"
***REMOVED*** Should show new columns: credentials, last_training_date, hipaa_certified, cyber_awareness_date

***REMOVED*** TEST ROLLBACK (critical to verify downgrade works!)
alembic downgrade -1
***REMOVED*** Output:
***REMOVED*** INFO  [alembic.runtime.migration] Running downgrade a1b2c3d4e5f6 -> 9z8y7x6w5v4u

***REMOVED*** Verify columns removed
docker-compose exec db psql -U scheduler -d residency_scheduler -c "\d persons"
***REMOVED*** Should NOT show credential tracking columns

***REMOVED*** Re-apply migration
alembic upgrade head
***REMOVED*** Output:
***REMOVED*** INFO  [alembic.runtime.migration] Running upgrade 9z8y7x6w5v4u -> a1b2c3d4e5f6
```
...
***REMOVED*** Should return: {"status":"healthy","database":"connected"}

***REMOVED*** Run tests
pytest

***REMOVED*** Check for any breakage
pytest tests/test_person_model.py -v
pytest tests/api/test_people.py -v

***REMOVED*** All should PASS
```

**Step 6: Commit Both Files**

```bash
***REMOVED*** Stage both model and migration
git add backend/app/models/person.py
git add backend/alembic/versions/a1b2c3d4e5f6_add_credential_tracking.py

***REMOVED*** Commit together
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 1: Migration Fails Mid-Execution
...
```bash
***REMOVED*** 1. Check current revision (might be partially applied)
alembic current
***REMOVED*** Output: might show old or new revision

***REMOVED*** 2. Check actual database state
docker-compose exec db psql -U scheduler -d residency_scheduler -c "\d persons"
***REMOVED*** See if column was partially added

***REMOVED*** 3. Stamp to previous revision (mark as not applied)
alembic downgrade -1
***REMOVED*** Or if that fails, manually stamp:
alembic stamp 9z8y7x6w5v4u

***REMOVED*** 4. Fix the migration
***REMOVED*** Edit migration file to add column as nullable first, then backfill, then make NOT NULL
***REMOVED*** (See Step 3 example above)

***REMOVED*** 5. Re-run migration
alembic upgrade head
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 2: Autogenerate Missed a Change
...
```bash
***REMOVED*** 1. Verify model is imported in env.py
grep -r "from app.models" backend/alembic/env.py

***REMOVED*** 2. Clear Python cache
find backend -type d -name __pycache__ -exec rm -rf {} +
find backend -type f -name "*.pyc" -delete

***REMOVED*** 3. Restart Python environment
deactivate && source venv/bin/activate  ***REMOVED*** If using venv

***REMOVED*** 4. Try again
alembic revision --autogenerate -m "Add field" --sql
***REMOVED*** --sql shows what would be generated without creating file

***REMOVED*** 5. If still failing, create manual migration
alembic revision -m "Add field"
***REMOVED*** Then manually write upgrade() and downgrade()
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 3: Can't Rollback - Data Would Be Lost
...
```bash
***REMOVED*** 1. STOP - do not proceed with destructive downgrade

***REMOVED*** 2. If you need to rollback, export data first:
...
***REMOVED*** 3. Then proceed with downgrade if necessary:
alembic downgrade -1

***REMOVED*** 4. To restore data (if column is re-added later):
***REMOVED*** Import backup_data.csv back into database
```
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 4: Multiple Migration Heads (Merge Conflict)
...
```bash
***REMOVED*** 1. Check heads
alembic heads
***REMOVED*** Output:
***REMOVED*** a1b2c3d4e5f6 (head)
***REMOVED*** x9y8z7w6v5u4 (head)

***REMOVED*** 2. Create merge migration
alembic merge -m "Merge migration heads" a1b2c3d4e5f6 x9y8z7w6v5u4

***REMOVED*** Output: Creates merge migration that depends on both heads
***REMOVED*** Generating /backend/alembic/versions/m1e2r3g4e5d6_merge_heads.py

***REMOVED*** 3. Review merge migration (usually auto-generated correctly)
cat backend/alembic/versions/m1e2r3g4e5d6_merge_heads.py

***REMOVED*** 4. Apply merge
alembic upgrade head
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 5: Foreign Key Constraint Violation
...
```python
***REMOVED*** Fix the migration to handle orphaned data:

def upgrade() -> None:
    ***REMOVED*** 1. Add column as nullable first
    op.add_column('assignments', sa.Column('person_id', sa.String(), nullable=True))

    ***REMOVED*** 2. Identify and handle orphaned records
    op.execute("""
    ...
    ***REMOVED*** 3. Now add the foreign key
    op.create_foreign_key(
    ...
    ***REMOVED*** 4. Make non-nullable if needed
...
***REMOVED******REMOVED******REMOVED*** Pre-Migration Checklist
...
***REMOVED******REMOVED******REMOVED*** Migration Generation Checklist
...
***REMOVED******REMOVED******REMOVED*** Migration Review Checklist
...
***REMOVED******REMOVED******REMOVED*** Data Safety Checklist
