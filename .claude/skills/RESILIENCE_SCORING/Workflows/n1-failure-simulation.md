***REMOVED*** Workflow: N-1 Failure Simulation
...
***REMOVED******REMOVED******REMOVED*** Step 1: Define Simulation Parameters

```python
simulation_params = {
    "duration_days": 7,  ***REMOVED*** 1-week absence (typical illness)
    "start_date": date.today(),  ***REMOVED*** When does absence start
    "residents_to_test": "all",  ***REMOVED*** Or specific subset: ["PGY2-03", "PGY3-01"]
    "coverage_threshold": 1.0  ***REMOVED*** Rotation fails if < 100% of min_residents
}
```

***REMOVED******REMOVED******REMOVED*** Step 2: Baseline Assessment

Before simulating failures, establish baseline coverage:

```python
***REMOVED*** Get current schedule state
baseline = {
    "rotations": {},
    "work_hours": {},
    "continuity": {}
...
***REMOVED******REMOVED******REMOVED*** Step 3: Simulate Each Resident Absent
...
for resident in active_residents:
    ***REMOVED*** Simulate removing this resident for duration_days
    ...
    ***REMOVED*** Analyze impact
    ...
    results[resident.id] = {
        "classification": classify_resident(impact),  ***REMOVED*** "critical", "high-impact", "low-impact"
...
***REMOVED******REMOVED******REMOVED*** Step 4: Impact Analysis
...
def analyze_impact(baseline, simulation):
    ...
    ***REMOVED*** Check rotation coverage failures
    for rotation_id, rotation_data in simulation["rotations"].items():
        ...
        if sim_margin < 0:  ***REMOVED*** Understaffed
    ...
    ***REMOVED*** Check residents who need reassignment to cover gap
    for resident_id, work_hours in simulation["work_hours"].items():
        ...
        if added_hours > 0:
            ...
            ***REMOVED*** Check if added hours push into ACGME violation
            if work_hours > 80:
    ...
    ***REMOVED*** Estimate recovery time
    if impact["failed_rotations"]:
        ***REMOVED*** Complex recovery (need to find replacement)
        impact["recovery_days"] = max([r["days_affected"] for r in impact["failed_rotations"]])
    elif impact["residents_needing_reassignment"]:
        ***REMOVED*** Simple recovery (internal shuffle)
        impact["recovery_days"] = 1
    else:
        ***REMOVED*** No recovery needed (fully absorbed)
        impact["recovery_days"] = 0

    return impact
```

***REMOVED******REMOVED******REMOVED*** Step 5: Classify Residents
...
def classify_resident(impact):
    ...
    ***REMOVED*** Critical: Absence causes coverage failure
    if impact["failed_rotations"]:
        return "critical"

    ***REMOVED*** High-impact: Absence causes significant burden but no hard failure
    if impact["residents_needing_reassignment"]:
        max_added_hours = max([r["added_hours"] for r in impact["residents_needing_reassignment"]])
        if max_added_hours > 10:  ***REMOVED*** Adding >10 hours/week to someone
            return "high-impact"

    ***REMOVED*** Low-impact: Absence absorbed with minimal redistribution
    return "low-impact"
```

***REMOVED******REMOVED******REMOVED*** Step 6: Generate N-1 Vulnerability Matrix

```python
***REMOVED*** Categorize results
...
***REMOVED******REMOVED******REMOVED*** Step 7: Generate Recommendations
...
def generate_n1_recommendations(results):
    ...
    ***REMOVED*** Identify rotation-specific vulnerabilities
    ...
    ***REMOVED*** Check for PGY-year imbalance
    pgy_critical_count = {}
...
```python
***REMOVED*** backend/app/resilience/n_minus_analysis.py
...
***REMOVED******REMOVED******REMOVED*** Example 1: Run Full N-1 Analysis
```bash
...
***REMOVED******REMOVED******REMOVED*** Example 2: Test Specific Residents
```bash
curl -X POST http://localhost:8000/api/resilience/n-minus-analysis \
  -H "Content-Type: application/json" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** Example 3: Programmatic Check
```python
...
***REMOVED******REMOVED******REMOVED*** Critical Residents
...
***REMOVED******REMOVED******REMOVED*** High-Impact Residents
...
***REMOVED******REMOVED******REMOVED*** Low-Impact Residents
...
***REMOVED******REMOVED******REMOVED*** Issue: All residents classified as "critical"
**Cause**: Schedule is over-constrained (too tight)
**Fix**: Reduce rotation minimum staffing or add more residents to pool

***REMOVED******REMOVED******REMOVED*** Issue: No residents classified as "critical" (seems too good)
...
***REMOVED******REMOVED******REMOVED*** Issue: Simulation times out
