***REMOVED*** Workflow: Multi-Failure Scenarios
...
```python
simulation_config = {
    "trials": 10000,  ***REMOVED*** Number of Monte Carlo trials
    "min_failures": 2,  ***REMOVED*** Minimum simultaneous absences
    "max_failures": 3,  ***REMOVED*** Maximum simultaneous absences
    "duration_days": 7,  ***REMOVED*** How long absences last
    "start_date": date.today(),
    "failure_mode": "random"  ***REMOVED*** "random" or "worst_case" or "seasonal"
}
```
...
***REMOVED******REMOVED******REMOVED*** Step 2: Run Monte Carlo Simulation
...
def run_monte_carlo_simulation(config):
    ...
    for trial in range(config["trials"]):
        ***REMOVED*** Randomly select 2-3 residents to be absent
        ...
        ***REMOVED*** Simulate their simultaneous absence
        ...
        ***REMOVED*** Assess impact
        impact = assess_scenario_impact(scenario)

        ***REMOVED*** Record results
        ...
        ***REMOVED*** Track rotation failures
        ...
        ***REMOVED*** Track worst-case scenario
    ...
    ***REMOVED*** Calculate statistics
    ...
    if results["time_to_failure"]:
        ...
        results["p95_time_to_failure"] = statistics.quantiles(results["time_to_failure"], n=20)[18]  ***REMOVED*** 95th percentile
    else:
        results["median_time_to_failure"] = None
        results["p95_time_to_failure"] = None

    ***REMOVED*** Rank rotations by fragility
...
***REMOVED******REMOVED******REMOVED*** Step 3: Assess Scenario Impact
...
def assess_scenario_impact(scenario):
    ...
    ***REMOVED*** Check rotation coverage failures
    for rotation_id, coverage_data in scenario["rotations"].items():
        if coverage_data["actual_residents"] < coverage_data["required_residents"]:
            ...
            ***REMOVED*** Add to impact score
            impact["score"] += shortfall * coverage_data["days_affected"] * 10

    ***REMOVED*** Check work hour violations
    ...
    ***REMOVED*** Determine time to failure (first day where any rotation fails)
    ...
    ***REMOVED*** Check for cascading failures (>3 rotations affected)
    if len(impact["failed_rotations"]) > 3:
        impact["score"] *= 1.5  ***REMOVED*** Multiply by 1.5 for cascading failures
        impact["cascading"] = True

    return impact
```

***REMOVED******REMOVED******REMOVED*** Step 4: Worst-Case Analysis
...
def find_worst_case_scenario(config):
    ...
    """
    critical_residents = get_critical_residents()  ***REMOVED*** From N-1 analysis

    worst_case = None
    worst_score = 0

    ***REMOVED*** Test all pairs of critical residents
    ...
    ***REMOVED*** Test PGY-year groupings
...
***REMOVED******REMOVED******REMOVED*** Step 5: Rank Fragile Rotations
...
def rank_fragile_rotations(monte_carlo_results):
    ...
    ***REMOVED*** Sort by failure probability descending
    fragile_rotations.sort(key=lambda x: x["failure_probability"], reverse=True)

    return fragile_rotations
```

***REMOVED******REMOVED******REMOVED*** Step 6: Generate Mitigation Recommendations
...
def generate_mitigation_recommendations(monte_carlo_results, worst_case):
    ...
    ***REMOVED*** Check collapse probability
    ...
    ***REMOVED*** Check median time to failure
    ...
    ***REMOVED*** Check fragile rotations
    ...
    ***REMOVED*** Analyze worst-case scenario
    if worst_case:
        recommendations.append({
            "priority": "MEDIUM",
...
```python
***REMOVED*** backend/app/resilience/monte_carlo_sim.py
...
***REMOVED******REMOVED******REMOVED*** Example 1: Standard Monte Carlo (10,000 trials)
```bash
...
***REMOVED******REMOVED******REMOVED*** Example 2: Quick Assessment (1,000 trials)
```bash
curl -X POST http://localhost:8000/api/resilience/simulate-failure \
  -H "Content-Type: application/json" \
  -d '{
...
***REMOVED******REMOVED******REMOVED*** Example 3: Worst-Case Analysis
```bash
...
***REMOVED******REMOVED******REMOVED*** Collapse Probability
...
***REMOVED******REMOVED******REMOVED*** Time-to-Failure
...
***REMOVED******REMOVED******REMOVED*** Fragile Rotation Severity
...
***REMOVED******REMOVED******REMOVED*** Issue: Collapse probability = 0 (seems unrealistic)
...
***REMOVED******REMOVED******REMOVED*** Issue: All rotations marked as fragile
