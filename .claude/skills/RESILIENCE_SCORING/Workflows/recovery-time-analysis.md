***REMOVED*** Workflow: Recovery Time Analysis
...
***REMOVED******REMOVED******REMOVED*** Step 1: Define Recovery Scenario

```python
recovery_scenario = {
    "disruption": {
        "absent_residents": ["PGY2-03", "PGY3-01"],  ***REMOVED*** Who is absent
        "start_date": date(2025, 1, 15),
        "duration_days": 14  ***REMOVED*** How long will they be gone
    },
    "available_mitigations": {
        "supplemental_staff": ["FAC-TEMP-01", "FAC-TEMP-02"],  ***REMOVED*** Available temps
        "cross_trained_residents": ["PGY2-05", "PGY3-04"],  ***REMOVED*** Can cover multiple rotations
        "overtime_budget": 40  ***REMOVED*** Max additional hours per week allowed
    },
    "recovery_goal": {
        "target_coverage": 1.0,  ***REMOVED*** 100% of rotation minimums
        "max_acgme_violations": 0,  ***REMOVED*** No violations allowed
        "max_recovery_time_hours": 48  ***REMOVED*** Must restore in 48 hours
    }
}
```

***REMOVED******REMOVED******REMOVED*** Step 2: Baseline Impact Assessment
...
def assess_baseline_impact(scenario):
    ...
    ***REMOVED*** Simulate removing absent residents
    ...
    ***REMOVED*** Identify coverage gaps
    ...
    ***REMOVED*** Identify residents who would need reassignment (naive internal shuffle)
    for resident_id, work_hours in schedule_state["work_hours"].items():
        if work_hours > 80:  ***REMOVED*** Would violate ACGME if we just redistribute
...
***REMOVED******REMOVED******REMOVED*** Step 3: Model Recovery Strategies
...
def model_recovery_strategies(scenario, baseline):
    ...
    ***REMOVED*** Strategy 1: Internal Redistribution
    ...
    strategies.append({
        ...
        "cost": 0,  ***REMOVED*** No additional cost
        "acgme_violations": strategy_1["acgme_violations"],
        "details": strategy_1
    })

    ***REMOVED*** Strategy 2: Supplemental Staff
    if scenario["available_mitigations"]["supplemental_staff"]:
        ...
        strategies.append({
            ...
            "cost": strategy_2["supplemental_staff_count"] * 1000,  ***REMOVED*** $1000/week per temp
            "acgme_violations": 0,  ***REMOVED*** No violations (we add staff, not burden existing)
            "details": strategy_2
        })

    ***REMOVED*** Strategy 3: Cross-Trained Residents
    if scenario["available_mitigations"]["cross_trained_residents"]:
        ...
        strategies.append({
            ...
            "cost": 0,  ***REMOVED*** Assuming residents already trained
            "acgme_violations": strategy_3["acgme_violations"],
            "details": strategy_3
        })

    ***REMOVED*** Strategy 4: Hybrid
    ...
    ***REMOVED*** Find optimal strategy (fastest feasible strategy)
...
***REMOVED******REMOVED******REMOVED*** Step 4: Internal Redistribution Model
...
def model_internal_redistribution(scenario, baseline):
    ...
    ***REMOVED*** Get residents with slack capacity
    residents_with_slack = []
    for resident in active_residents:
        current_hours = get_4week_rolling_hours(resident)
        if current_hours < 70:  ***REMOVED*** Leave 10-hour buffer below 80-hour limit
    ...
    for gap in baseline["coverage_gaps"]:
        ...
        ***REMOVED*** Find residents who can cover this rotation
        ...
        if len(eligible_residents) >= shortfall:
            ***REMOVED*** We can cover this gap
            for i in range(int(shortfall)):
                resident = eligible_residents[i]
                recovery["reassignments"].append({
                    "resident_id": resident["resident_id"],
                    "rotation_id": rotation_id,
                    "hours_added": 40  ***REMOVED*** Assuming full-time rotation
                })
                resident["slack_hours"] -= 40

            ***REMOVED*** Recovery time = time to notify and reassign (assume 24 hours)
            recovery["recovery_time_hours"] = 24
        else:
            ***REMOVED*** Can't cover this gap with internal redistribution
            recovery["feasible"] = False
            return recovery

    ***REMOVED*** Check if any reassignments cause ACGME violations
...
***REMOVED******REMOVED******REMOVED*** Step 5: Supplemental Staff Model
...
def model_supplemental_staff(scenario, baseline):
    ...
    ***REMOVED*** Calculate how many supplemental staff needed
    ...
    if len(available_supplemental) >= total_shortfall:
        ...
        ***REMOVED*** Assign supplemental staff to gaps
        staff_idx = 0
        for gap in baseline["coverage_gaps"]:
        ...
        ***REMOVED*** Recovery time = credentialing + orientation
        ***REMOVED*** Assume: Credentialing (24 hours) + Orientation (8 hours) = 32 hours
        recovery["recovery_time_hours"] = 32
        recovery["feasible"] = True
    else:
        ***REMOVED*** Not enough supplemental staff available
        recovery["feasible"] = False

    return recovery
```

***REMOVED******REMOVED******REMOVED*** Step 6: Identify Bottlenecks
...
def identify_recovery_bottlenecks(strategies):
    ...
    ***REMOVED*** Check if supplemental staff is delayed by credentialing
    ...
    ***REMOVED*** Check if internal redistribution fails due to work hour constraints
    ...
    ***REMOVED*** Check if cross-training would help but isn't available
...
***REMOVED******REMOVED******REMOVED*** Step 7: Generate Recovery Plan
...
def generate_recovery_plan(scenario, strategies, bottlenecks):
    ...
    ***REMOVED*** Build step-by-step plan
    recovery_plan = {
    ...
    ***REMOVED*** Add specific steps based on strategy
...
```python
***REMOVED*** backend/app/resilience/recovery_planner.py
...
***REMOVED******REMOVED******REMOVED*** Example 1: Plan for Known Absence
```bash
...
***REMOVED******REMOVED******REMOVED*** Example 2: Emergency Response (Immediate)
```bash
curl -X POST http://localhost:8000/api/resilience/recovery-plan \
  -H "Content-Type: application/json" \
  -d '{
