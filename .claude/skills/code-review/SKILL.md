...
***REMOVED******REMOVED******REMOVED*** Example 1: N+1 Query Detection

**Context:** Reviewing PR that adds resident work hour summary endpoint

**Input:**
```python
***REMOVED*** backend/app/api/routes/residents.py
...
async def get_resident_work_hours(db: AsyncSession = Depends(get_db)):
    ...
    for resident in residents.scalars():
        ***REMOVED*** Query for each resident - N+1 problem!
...
**Output:**
```markdown
***REMOVED******REMOVED*** Code Review Finding
...
residents = await db.execute(select(Person).where(Person.role == "RESIDENT"))
for resident in residents.scalars():
    assignments = await db.execute(  ***REMOVED*** Separate query per resident
        select(Assignment).where(Assignment.person_id == resident.id)
    )
```
...
residents = await db.execute(
    ...
    .options(selectinload(Person.assignments))  ***REMOVED*** Load in single query
)

result = []
for resident in residents.scalars():
    hours = sum(a.duration for a in resident.assignments)  ***REMOVED*** No additional query
    result.append({"resident": resident.name, "hours": hours})
```
...
***REMOVED******REMOVED******REMOVED*** Example 2: Missing Authentication Check

**Context:** Security review of new admin endpoint

**Input:**
```python
***REMOVED*** backend/app/api/routes/admin.py
...
***REMOVED******REMOVED******REMOVED*** Example 3: Async/Await Missing
...
async def create_assignment(
    ...
    db.add(assignment)  ***REMOVED*** Missing await!
    db.commit()         ***REMOVED*** Missing await!
    return assignment
```
...
```python
db.add(assignment)  ***REMOVED*** Returns coroutine, doesn't execute
db.commit()         ***REMOVED*** Returns coroutine, doesn't execute
```
...
async def create_assignment(
    ...
    assignment = Assignment(**assignment_data.dict())
    db.add(assignment)
    await db.commit()  ***REMOVED*** Add await
    await db.refresh(assignment)  ***REMOVED*** Refresh to get generated ID
    return assignment
```
