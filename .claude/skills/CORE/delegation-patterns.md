***REMOVED*** Delegation Patterns - Subagent Spawning Rules
...
***REMOVED******REMOVED*** II. DELEGATION DECISION FRAMEWORK

***REMOVED******REMOVED******REMOVED*** A. When to Spawn Subagents
...
***REMOVED******REMOVED******REMOVED*** B. Decision Tree
...
***REMOVED******REMOVED*** III. PARALLEL VS SEQUENTIAL EXECUTION

***REMOVED******REMOVED******REMOVED*** A. Parallel Execution (Scatter-Gather)
...
async def validate_schedule_parallel(schedule_id):
    ...
    ***REMOVED*** All run in parallel
    results = await parallel_execute(tasks)

    ***REMOVED*** Synthesize (any failure = overall failure)
...
***REMOVED******REMOVED******REMOVED*** B. Sequential Execution (Pipeline)
...
async def generate_schedule_sequential(params):
    ***REMOVED*** Step 1: Backup database (must complete first)
    ...
    ***REMOVED*** Step 2: Generate schedule (needs backup complete)
    ...
    ***REMOVED*** Step 3: Validate (needs generated schedule)
    ...
    ***REMOVED*** Step 4: Human verification (needs validated schedule)
...
***REMOVED******REMOVED******REMOVED*** C. Hybrid Execution (Pipeline of Parallel Stages)
...
async def optimize_schedule_hybrid(schedule):
    ***REMOVED*** Stage 1: Parallel pre-validation (must all pass)
    validation_tasks = [
    ...
    ***REMOVED*** Stage 2: Parallel optimization attempts
    optimization_tasks = [
    ...
    ***REMOVED*** Select best result (Pareto-optimal)
    best_schedule = pareto_select(optimizations)

    ***REMOVED*** Stage 3: Final validation (sequential, on best result)
...
***REMOVED******REMOVED*** IV. DELEGATION MECHANISMS

***REMOVED******REMOVED******REMOVED*** A. Fire-and-Forget Delegation
...
**Pattern:**
```python
***REMOVED*** Delegate and get future reference
...
***REMOVED******REMOVED******REMOVED*** B. Blocking Delegation
...
***REMOVED*** Use result immediately
...
***REMOVED******REMOVED******REMOVED*** C. Streaming Delegation
...
***REMOVED*** Process results as they arrive
...
***REMOVED******REMOVED*** V. RESULT SYNTHESIS PATTERNS

***REMOVED******REMOVED******REMOVED*** A. All-or-Nothing (AND Logic)
...
**Example:**
```python
***REMOVED*** ACGME compliance: ALL rules must pass
...
***REMOVED*** Any violation = schedule rejected
return synthesize_all_or_nothing(validation_results)
```

---

***REMOVED******REMOVED******REMOVED*** B. Best-Effort (OR Logic)
...
**Example:**
```python
***REMOVED*** Try multiple swap candidates (any match works)
...
***REMOVED*** Return first successful swap
return synthesize_best_effort(swap_attempts)
```

---

***REMOVED******REMOVED******REMOVED*** C. Majority Vote (Consensus)
...
**Example:**
```python
***REMOVED*** Multiple agents review code quality
...
***REMOVED*** Majority "approve" = approved
return synthesize_majority_vote(reviews)
```

---

***REMOVED******REMOVED******REMOVED*** D. Weighted Aggregation
...
**Example:**
```python
***REMOVED*** Multi-objective schedule scoring
objectives = await parallel_execute([
    evaluate_fairness(schedule),      ***REMOVED*** Weight: 0.4
    evaluate_coverage(schedule),      ***REMOVED*** Weight: 0.3
    evaluate_continuity(schedule)     ***REMOVED*** Weight: 0.3
])
...
***REMOVED******REMOVED******REMOVED*** E. Merge-and-Deduplicate
...
**Example:**
```python
***REMOVED*** Find all scheduling conflicts from multiple detectors
...
***REMOVED*** Merge, removing duplicates
return synthesize_merge_dedupe(conflict_scans)
```

---

***REMOVED******REMOVED*** VI. ERROR PROPAGATION

***REMOVED******REMOVED******REMOVED*** A. Fail-Fast Strategy
...
**Example:**
```python
***REMOVED*** Schedule generation pipeline (sequential fail-fast)
steps = [
...
***REMOVED*** Any step failure = abort entire pipeline
return await execute_fail_fast(steps)
```

---

***REMOVED******REMOVED******REMOVED*** B. Fail-Tolerant Strategy
...
**Example:**
```python
***REMOVED*** Generate multiple report formats (any success is good)
reports = [
...
***REMOVED*** Continue even if some formats fail
return await execute_fail_tolerant(reports)
```

---

***REMOVED******REMOVED******REMOVED*** C. Retry-with-Backoff Strategy
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                raise  ***REMOVED*** Final attempt failed

            delay = base_delay * (2 ** attempt)  ***REMOVED*** Exponential backoff
...
**Example:**
```python
***REMOVED*** Database-dependent task (may have transient connection issues)
...
***REMOVED******REMOVED******REMOVED*** D. Circuit-Breaker Strategy
...
**Example:**
```python
***REMOVED*** Protect against failing external service
...
try:
    ...
    ***REMOVED*** Use cached data or fallback
    result = get_cached_data()
```

---

***REMOVED******REMOVED*** VII. RESOURCE MANAGEMENT

***REMOVED******REMOVED******REMOVED*** A. Concurrency Limits
...
***REMOVED*** Global semaphore (max 10 concurrent agents)
...
**Example:**
```python
***REMOVED*** Process 100 schedules, but only 10 at a time
...
***REMOVED******REMOVED******REMOVED*** B. Timeout Management
...
***REMOVED******REMOVED******REMOVED*** C. Memory Management
...
async def delegate_large_dataset(task):
    ***REMOVED*** Stream results instead of loading all into memory
    async for chunk in delegate_streaming(**task):
        process_chunk(chunk)
        ***REMOVED*** Chunk processed and can be garbage collected
```

---

***REMOVED******REMOVED*** VIII. MONITORING & OBSERVABILITY

***REMOVED******REMOVED******REMOVED*** A. Delegation Metrics
...
***REMOVED******REMOVED******REMOVED*** B. Tracing Delegation Chains
...
***REMOVED******REMOVED*** IX. BEST PRACTICES

***REMOVED******REMOVED******REMOVED*** A. Delegation Hygiene
...
***REMOVED******REMOVED******REMOVED*** B. Context Transfer
...
***REMOVED******REMOVED******REMOVED*** C. Error Handling
...
***REMOVED******REMOVED*** X. ANTI-PATTERNS

***REMOVED******REMOVED******REMOVED*** A. Over-Delegation

**Problem:** Delegating trivial tasks, adding overhead without benefit.

**Example:**
```python
***REMOVED*** WRONG - delegation overhead > task time
...
***REMOVED*** RIGHT - do simple calculations directly
result = 2 + 3
```

---

***REMOVED******REMOVED******REMOVED*** B. Delegation Ping-Pong
...
***REMOVED******REMOVED******REMOVED*** C. Context Loss

**Problem:** Important context lost during handoff.

**Example:**
```python
***REMOVED*** WRONG - incomplete context
await delegate_to_agent(
    agent="acgme-compliance",
    action="validate",
    params={"schedule_id": "uuid"}
    ***REMOVED*** Missing: which rules to check, validation depth, etc.
)

***REMOVED*** RIGHT - complete context
await delegate_to_agent(
    agent="acgme-compliance",
    action="validate",
    params={
...
***REMOVED******REMOVED******REMOVED*** D. Unbounded Parallelism

**Problem:** Spawning unlimited parallel agents.

**Example:**
```python
***REMOVED*** WRONG - could spawn thousands of agents
tasks = [
    delegate_to_agent(agent="validator", action="check", params={"id": id})
    for id in all_schedule_ids  ***REMOVED*** Could be 10,000+
]
results = await asyncio.gather(*tasks)  ***REMOVED*** Resource exhaustion!

***REMOVED*** RIGHT - batch with concurrency limit
...
***REMOVED******REMOVED*** XI. QUICK REFERENCE

***REMOVED******REMOVED******REMOVED*** A. Delegation Patterns Cheat Sheet
...
***REMOVED******REMOVED******REMOVED*** B. Synthesis Patterns Cheat Sheet
...
***REMOVED******REMOVED******REMOVED*** C. Error Strategies Cheat Sheet
...
***REMOVED******REMOVED*** XII. DECISION FLOWCHART
