...
***REMOVED*** Schedule Optimization Skill
...
***REMOVED******REMOVED*** Solver Status (2025-12-24, Updated 2025-12-26)
...
PR ***REMOVED***442 was not merged because this issue was caught during evaluation. The correct filter
...
***REMOVED******REMOVED*** Architecture: Block vs Half-Day Scheduling
...
***REMOVED******REMOVED******REMOVED*** Night Float (NF) Half-Block Mirrored Pairing
...
***REMOVED******REMOVED******REMOVED*** Primary Objectives (Hard Constraints)
...
***REMOVED******REMOVED******REMOVED*** Secondary Objectives (Soft Constraints)
...
***REMOVED******REMOVED*** Solver Architecture

***REMOVED******REMOVED******REMOVED*** Google OR-Tools CP-SAT
Primary constraint programming solver:

```python
***REMOVED*** Located in: backend/app/scheduling/engine.py
from ortools.sat.python import cp_model

model = cp_model.CpModel()
***REMOVED*** Define variables, constraints, objectives
...
***REMOVED******REMOVED*** Optimization Strategies

***REMOVED******REMOVED******REMOVED*** 1. Pareto Optimization
...
***REMOVED******REMOVED******REMOVED*** 2. Iterative Improvement
...
***REMOVED******REMOVED******REMOVED*** 3. Decomposition
...
***REMOVED******REMOVED*** Coverage Optimization

***REMOVED******REMOVED******REMOVED*** Target Coverage Levels
...
***REMOVED******REMOVED******REMOVED*** Coverage Gap Resolution
...
***REMOVED******REMOVED*** Workload Balancing
...
***REMOVED******REMOVED******REMOVED*** Balancing Algorithm
    ...
    while gini_coefficient(assignments) > 0.15:
        ...
        ***REMOVED*** Find swappable shift
        ...
        if shift and is_valid_transfer(shift):
            ...
            break  ***REMOVED*** No valid transfers available
```

***REMOVED******REMOVED*** Preference Handling

***REMOVED******REMOVED******REMOVED*** Preference Types
...
***REMOVED******REMOVED******REMOVED*** Preference Satisfaction
Aim for:
- 100% hard blocks honored
- 80%+ soft preferences
- 70%+ historical patterns

***REMOVED******REMOVED*** Resilience Integration

***REMOVED******REMOVED******REMOVED*** 80% Utilization Rule
...
***REMOVED******REMOVED******REMOVED*** Static Fallbacks
...
***REMOVED******REMOVED*** Optimization Workflow

***REMOVED******REMOVED******REMOVED*** New Schedule Generation
...
***REMOVED*** Returns Pareto frontier of valid schedules
```
...
***REMOVED******REMOVED******REMOVED*** Existing Schedule Optimization
...
***REMOVED******REMOVED******REMOVED*** Scenario: New Block Schedule
...
***REMOVED******REMOVED******REMOVED*** Scenario: Coverage Emergency
...
***REMOVED******REMOVED******REMOVED*** Scenario: Fairness Complaint
...
***REMOVED******REMOVED******REMOVED*** Solver Performance
...
***REMOVED******REMOVED******REMOVED*** Schedule Quality
...
***REMOVED******REMOVED*** REQUIRED: Documentation After Each Step
...
***REMOVED******REMOVED******REMOVED*** Documentation Checkpoint Protocol
...
***REMOVED******REMOVED******REMOVED*** Where to Document
...
***REMOVED******REMOVED******REMOVED*** Planning Template
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Investigation
- [ ] Explore current state
- [ ] Document findings in [location]

***REMOVED******REMOVED******REMOVED*** Phase 2: Implementation
- [ ] Make changes
- [ ] Document what changed in commit message

***REMOVED******REMOVED******REMOVED*** Phase 3: Verification
- [ ] Test the changes
- [ ] Document results (success/failure)

***REMOVED******REMOVED******REMOVED*** Phase 4: Documentation Update
- [ ] Update skill if new knowledge gained
- [ ] Update code comments if behavior clarified
- [ ] Update HUMAN_TODO.md if manual work needed
```

***REMOVED******REMOVED******REMOVED*** Anti-Pattern: Silent Failures
...
***REMOVED******REMOVED******REMOVED*** End-to-End: Generating Block 10 Schedule
...
***REMOVED*** Check available residents
python -c "
...
***REMOVED*** Check rotation templates
python -c "
from app.db.session import SessionLocal
...
**Step 2: Initialize Solver**
```python
***REMOVED*** In backend/app/scheduling/engine.py or interactive shell
...
***REMOVED*** Create engine with Block 10 constraints
engine = SchedulingEngine(
    db=db,
    block_number=10,
    start_date=date(2025, 1, 6),  ***REMOVED*** Block 10 start
    end_date=date(2025, 4, 6),     ***REMOVED*** Block 10 end (13 weeks)
...
```python
***REMOVED*** Check call distribution
...
***REMOVED*** Calculate Gini coefficient
...
```python
***REMOVED*** Try decomposition approach
...
for month_start, month_end in decompose_by_month(date(2025, 1, 6), date(2025, 4, 6)):
    month_engine = SchedulingEngine(
        ...
        end_date=month_end,
        solver_type="or-tools",
        timeout_seconds=60  ***REMOVED*** Shorter timeout for smaller problem
    )
    month_result = month_engine.solve()
    monthly_solutions.append(month_result)

***REMOVED*** Combine monthly solutions
combined_solution = combine_solutions(monthly_solutions)
```

**Issue: Infeasible constraints**
```python
***REMOVED*** Run pre-solver validation to detect issues early
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 1: Solver Assigns Everyone to Same Rotation
...
**Detection:**
```python
***REMOVED*** Check assigned templates
assigned_templates = set(a.rotation_template_name for a in result.assignments)
print(f"Templates used: {assigned_templates}")
***REMOVED*** Output: {'Night Float', 'FMIT'} ← WRONG! Should be outpatient electives
```

**Fix:**
```python
***REMOVED*** In engine.py, _get_rotation_templates()
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 2: Solver Times Out
...
1. **Increase timeout:**
   ```python
   engine = SchedulingEngine(..., timeout_seconds=600)  ***REMOVED*** 10 minutes
   ```

2. **Decompose problem:**
   ```python
   ***REMOVED*** Break 13 weeks into 4 smaller problems (3-4 weeks each)
   ```

3. **Relax soft constraints:**
   ```python
   ***REMOVED*** Reduce weight of less important constraints
   constraint_manager.get("TuesdayCallPreference").weight = 1.0  ***REMOVED*** Lower priority
   ```

***REMOVED******REMOVED******REMOVED*** Failure Mode 3: No Template Balance
...
***REMOVED*** If ratio > 1.3, balance is poor
```

**Fix:**
```python
***REMOVED*** In solver, add template balance penalty to objective
template_counts = {}  ***REMOVED*** person_id -> template_name -> count
...
for person in template_counts:
    ...
    objective_terms.append(-5 * balance_penalty)  ***REMOVED*** Penalize imbalance
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 4: ACGME Violations Post-Solve
...
**Fix:**
```python
***REMOVED*** Ensure 80-hour constraint is in solver
    ...
    def add_to_cpsat(self, model, variables, context):
        for person in context.persons:
            for week_start in context.weeks:
                ...
                ***REMOVED*** Each assignment is ~10-12 hours, enforce max 80
                model.Add(sum(week_assignments) * 10 <= 80)
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 5: Forgetting to Document
