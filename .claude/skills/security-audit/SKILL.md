...
***REMOVED******REMOVED******REMOVED*** Example 1: OPSEC/PERSEC Data Leak Detection

**Context:** Reviewing PR that adds schedule export feature

**Input:**
```python
***REMOVED*** backend/app/api/routes/export.py
...
async def export_schedule(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "schedule": [
            {
                "resident_name": a.person.name,  ***REMOVED*** PERSEC VIOLATION
                "rotation": a.rotation.name,
                "date": a.date,
                "deployment_status": a.person.deployment_status  ***REMOVED*** OPSEC VIOLATION
            }
...
**Output:**
```markdown
***REMOVED******REMOVED*** Security Audit Finding
...
async def export_schedule(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_roles(["admin"]))  ***REMOVED*** Auth required
):
    ...
    return {
        "schedule": [
            {
                "resident_id": a.person.id,  ***REMOVED*** Use ID, not name
                "rotation": a.rotation.name,
                "date": a.date,
                ***REMOVED*** DO NOT include deployment_status
            }
            for a in assignments.scalars()
        ]
    }
...
async def search_residents(
    name: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)  ***REMOVED*** Add auth
):
    """Search residents by name."""
    result = await db.execute(
        select(Person)
        .where(Person.name.ilike(f"%{name}%"))  ***REMOVED*** Safe - parameterized
...
**Testing:**
```python
***REMOVED*** Add test to verify exploit is blocked
def test_sql_injection_blocked():
    response = client.get("/search/residents?name='; DROP TABLE persons; --")
    ***REMOVED*** Should return empty list, not execute DROP
...
***REMOVED******REMOVED******REMOVED*** Example 3: Missing Authentication Check
...
async def get_all_schedules(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)  ***REMOVED*** Require auth
):
...
```bash
***REMOVED*** Find all routes missing authentication
...
|--------------|---------|------------|----------------|
| **Bandit False Positive** | Security scan flags safe code | Static analysis can't understand context | 1. Manual review confirms safety<br>2. Add `***REMOVED*** nosec` with justification<br>3. Document in security log |
