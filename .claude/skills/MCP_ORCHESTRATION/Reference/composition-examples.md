***REMOVED*** Tool Composition Examples
...
***REMOVED******REMOVED*** Example 1: Schedule Generation Chain
...
async def safe_schedule_generation(
    ...
    ***REMOVED*** Step 1: Pre-flight capacity check
    logger.info("Step 1/5: Checking utilization capacity")

    try:
        utilization = await call_tool_with_retry(
            "check_utilization_threshold_tool",
            available_faculty=10,  ***REMOVED*** Get from DB in production
            required_blocks=730,   ***REMOVED*** Calculate based on date range
        )
    ...
    ***REMOVED*** Step 2: Generate schedule
    logger.info("Step 2/5: Generating schedule")

    try:
        gen_result = await call_tool_with_retry(
            "generate_schedule",  ***REMOVED*** This calls backend API
    ...
    ***REMOVED*** Step 3: Validate generated schedule
    ...
    ***REMOVED*** Step 4: Detect conflicts
    ...
    ***REMOVED*** Step 5: Run contingency analysis
    logger.info("Step 5/5: Running N-1/N-2 contingency analysis")

    try:
        ***REMOVED*** Start as background task (takes 2-5 minutes)
        ...
        ***REMOVED*** Poll for completion (with timeout)
        contingency = None
        for _ in range(60):  ***REMOVED*** 5 minutes max
...
```python
{
    "status": "success",  ***REMOVED*** or "needs_review", "failed", "blocked"
    "generation": {...},   ***REMOVED*** Generation details
    "validation": {...},   ***REMOVED*** Validation results
    "conflicts": {...},    ***REMOVED*** Conflict detection
    "contingency": {...},  ***REMOVED*** N-1/N-2 analysis
    "utilization": {...},  ***REMOVED*** Capacity check
    "issues": [...],       ***REMOVED*** List of issues found
    "recommendations": [...],  ***REMOVED*** Actionable recommendations
}
```

---

***REMOVED******REMOVED*** Example 2: Swap Validation Chain
...
async def validate_swap_safety(
    ...
    ***REMOVED*** Step 1: Analyze swap candidates to get details
    ...
    ***REMOVED*** Find specific candidate
    ...
    ***REMOVED*** Step 2: Simulate swap (call backend API)
    logger.info("Step 2/5: Simulating swap in database")

    try:
        ***REMOVED*** This would be a backend API call to simulate the swap
        ***REMOVED*** For now, we'll use the assignment date range
    ...
    ***REMOVED*** Step 3: Validate simulated schedule
    ...
    try:
        ...
        logger.error(f"Validation failed: {e}")
        ***REMOVED*** Conservative - reject swap if validation fails
        return {
            "safe": False,
        ...
        if conflicts.total_conflicts > 0:
            ...
            return {
                ...
                "conflict_details": [
                    ...
                    for c in conflicts.conflicts[:5]  ***REMOVED*** Top 5
                ],
            }

    except Exception as e:
        logger.warning(f"Conflict detection failed: {e}")
        ***REMOVED*** Non-blocking - continue without conflict check

    ***REMOVED*** Step 5: Check utilization impact
    logger.info("Step 5/5: Checking utilization impact")

    try:
        ***REMOVED*** Get current faculty count (would query DB in production)
        ...
        if utilization.level in ["red", "black"]:
            ...
            ***REMOVED*** Soft warning - don't block swap
            ***REMOVED*** But flag for approval
...
***REMOVED******REMOVED*** Example 3: Resilience Analysis Chain
...
async def comprehensive_resilience_analysis() -> dict:
    ...
    ***REMOVED*** Fan-out: Run all checks in parallel
    results = await asyncio.gather(
        ***REMOVED*** Tier 1: Critical resilience
        ...
        ***REMOVED*** N-1/N-2 via background task
        _run_contingency_background(),

        ***REMOVED*** Tier 2: Strategic resilience
        call_tool_with_retry("analyze_hub_centrality_tool"),

        ***REMOVED*** Tier 1: Static stability
        call_tool_with_retry("get_static_fallbacks_tool"),

        ***REMOVED*** Military compliance
        call_tool_with_retry(
    ...
    ***REMOVED*** Unpack results (handle exceptions)
    utilization, contingency, hub_analysis, fallbacks, mtf_compliance = results

    ***REMOVED*** Handle partial failures
    ...
    ***REMOVED*** Synthesize health score (0-100)
    health_score = 100.0
    issues = []
    recommendations = []

    ***REMOVED*** Utilization (30 points)
    if utilization:
        if utilization.level == "green":
            pass  ***REMOVED*** Full points
        ...
        health_score -= 15  ***REMOVED*** Penalty for missing data

    ***REMOVED*** Contingency (40 points)
    if contingency:
        ...
        health_score -= 20  ***REMOVED*** Penalty for missing data

    ***REMOVED*** Hub analysis (15 points)
    if hub_analysis:
        critical_hubs = [
    ...
    ***REMOVED*** MTF compliance (5 points)
    if mtf_compliance:
...
async def _run_contingency_background() -> dict:
    ...
    ***REMOVED*** Poll for completion (max 5 minutes)
    for _ in range(60):
        await asyncio.sleep(5)
...
***REMOVED******REMOVED*** Example 4: Deployment Pipeline Chain
...
async def safe_production_deployment(
    ...
    try:
        ...
        ***REMOVED*** Step 3: Smoke tests on staging
        ...
        ***REMOVED*** Step 5: Monitor deployment
        ...
        for attempt in range(20):  ***REMOVED*** 10 minutes max
            ...
            if status.deployment.status == "success":
                ...
                if auto_rollback:
                    ***REMOVED*** Step 6: Automatic rollback
...
async def _execute_rollback(deployment_id: str, failed_version: str):
    ...
    try:
        rollback = await call_tool_with_retry(
            ...
            target_version=None,  ***REMOVED*** Rollback to previous
            dry_run=False,
        )
...
***REMOVED******REMOVED*** Example 5: Performance Optimization Chain
...
async def identify_code_to_remove(
    ...
    ***REMOVED*** Phase 1: Parallel benchmarking (map)
    benchmark_results = await asyncio.gather(
        ...
        call_tool_with_retry(
            ...
            modules=None,  ***REMOVED*** All modules
        ),
        call_tool_with_retry(
    ...
    ***REMOVED*** Phase 2: Identify removal candidates (reduce)
    removal_candidates = []

    ***REMOVED*** From solver benchmarks
    if not isinstance(solver_bench, Exception):
        ***REMOVED*** Remove solvers that consistently lose
        for solver, metrics in solver_bench.results_by_solver.items():
            if metrics.failures > metrics.successes * 0.5:
                removal_candidates.append({
                    ...
                    "estimated_lines": 200,  ***REMOVED*** Estimate
                    "confidence": "high",
                })

    ***REMOVED*** From constraint benchmarks
    ...
    ***REMOVED*** From resilience benchmarks
    ...
    ***REMOVED*** Phase 3: Ablation study for high-value candidates (sequential)
    verified_removals = []

    for candidate in removal_candidates:
        if candidate["estimated_lines"] < min_removal_threshold_lines:
            continue  ***REMOVED*** Skip small modules
...
***REMOVED******REMOVED*** Best Practices from Examples
