...
***REMOVED******REMOVED******REMOVED*** 1. Connection Timeout Errors
...
```python
***REMOVED*** 1. Check backend health first
docker-compose exec mcp-server curl -s http://backend:8000/health

***REMOVED*** 2. Increase timeout for large date ranges
***REMOVED*** Backend operations scale with date range size
***REMOVED*** Small range (1 week): 5-10s
***REMOVED*** Medium range (1 month): 10-30s
***REMOVED*** Large range (1 year): 30-120s

***REMOVED*** 3. Retry with exponential backoff
...
***REMOVED*** 4. Use cached data for read-only operations
...
***REMOVED******REMOVED******REMOVED*** 2. Celery Worker Not Available
...
```bash
***REMOVED*** 1. Check worker status
docker-compose ps celery-worker

***REMOVED*** 2. Check Redis connectivity
docker-compose exec mcp-server redis-cli ping
***REMOVED*** Expected: PONG

***REMOVED*** 3. Check active workers
docker-compose exec mcp-server celery -A app.core.celery_app inspect active

***REMOVED*** 4. Restart workers if stuck
docker-compose restart celery-worker celery-beat

***REMOVED*** 5. Fallback to synchronous execution (if <30s)
try:
    ...
    ***REMOVED*** Call backend API directly for synchronous execution
...
***REMOVED******REMOVED******REMOVED*** 3. Database Lock/Deadlock
...
```python
***REMOVED*** 1. Retry with exponential backoff (database locks are transient)
result = await retry_with_backoff(
    generate_schedule,
    max_retries=5,
    base_delay=0.5,  ***REMOVED*** Short delay for lock contention
    start_date="2025-01-01",
    end_date="2025-01-31"
)

***REMOVED*** 2. Serialize write operations
***REMOVED*** Don't run multiple schedule generations concurrently
write_lock = asyncio.Lock()

async with write_lock:
    result = await generate_schedule(...)

***REMOVED*** 3. Check for active long-running queries
***REMOVED*** SELECT * FROM pg_stat_activity WHERE state = 'active' AND query_start < NOW() - INTERVAL '5 minutes';
```
...
***REMOVED******REMOVED******REMOVED*** 4. Validation Schema Errors
...
```python
***REMOVED*** 1. Validate inputs before calling tool
...
***REMOVED*** 2. Use enums for valid values
...
***REMOVED******REMOVED******REMOVED*** 5. Rate Limit Exceeded (Future)
...
```python
***REMOVED*** 1. Respect Retry-After header
import time
...
***REMOVED*** 2. Implement token bucket rate limiter
...
class RateLimiter:
    ...
    async def acquire(self):
        ...
        ***REMOVED*** Remove old calls outside time window
        while self.calls and self.calls[0] < now - self.time_window:
            self.calls.popleft()

        if len(self.calls) >= self.max_calls:
            ***REMOVED*** Wait until oldest call expires
...
***REMOVED*** 3. Cache API responses to reduce calls
GITHUB_CACHE = TTLCache(maxsize=100, ttl=300)  ***REMOVED*** 5-minute cache
...
***REMOVED******REMOVED******REMOVED*** 6. Task Stuck/Timeout
...
```python
***REMOVED*** 1. Implement task timeout with cancellation
import asyncio
...
***REMOVED*** 2. Check worker logs for stuck tasks
docker-compose logs celery-worker | grep "Task.*started"

***REMOVED*** 3. Restart worker to clear stuck tasks (last resort)
...
***REMOVED******REMOVED******REMOVED*** 7. Deployment Validation Failures
...
```python
***REMOVED*** 1. Analyze blockers before aborting
...
if not validation.valid:
    ***REMOVED*** Check if blockers are critical
    critical_blockers = [
...
for attempt in range(3):
    ...
    ***REMOVED*** Check which tests failed
...
***REMOVED*** 3. Run security scan with thresholds
...
***REMOVED******REMOVED******REMOVED*** 8. Resource Not Found
...
```python
***REMOVED*** 1. Validate ID exists before operations
async def safe_validate_schedule(schedule_id: str):
    """Validate schedule with existence check."""

    ***REMOVED*** Check if schedule exists first
    try:
        status = await schedule_status_resource(
            start_date="2025-01-01",
            end_date="2025-01-31"
        )
        ***REMOVED*** Verify schedule_id in active schedules
        ***REMOVED*** (implementation depends on API)

    except HTTPError as e:
        if e.status_code == 404:
            raise ValueError(f"Schedule {schedule_id} not found")

    ***REMOVED*** Proceed with validation
    return await validate_schedule_by_id_tool(schedule_id=schedule_id)

***REMOVED*** 2. Handle 404s gracefully
try:
...
***REMOVED*** 3. Provide suggestions for typos (fuzzy matching)
...
***REMOVED******REMOVED*** Tool-Specific Error Patterns

***REMOVED******REMOVED******REMOVED*** validate_schedule_tool
...
***REMOVED******REMOVED******REMOVED*** detect_conflicts_tool
...
***REMOVED******REMOVED******REMOVED*** start_background_task_tool
...
***REMOVED******REMOVED******REMOVED*** benchmark_solvers_tool
...
***REMOVED******REMOVED*** Error Recovery Strategies by Severity

***REMOVED******REMOVED******REMOVED*** Low Severity (Log and Continue)
- Validation warnings
- Non-critical conflicts
- Partial results available
- Cache misses

***REMOVED******REMOVED******REMOVED*** Medium Severity (Retry with Backoff)
- Connection timeouts
- Database locks
- Rate limits
- Worker busy

***REMOVED******REMOVED******REMOVED*** High Severity (Escalate to Human)
- Security vulnerabilities (critical)
- Deployment validation failures
- Data integrity violations
- All retries exhausted

***REMOVED******REMOVED******REMOVED*** Emergency Severity (Alert and Rollback)
...
***REMOVED******REMOVED*** Monitoring Recommendations
...
***REMOVED******REMOVED******REMOVED*** Logging Best Practices

```python
***REMOVED*** Structured logging for errors
logger.error(
    "Tool error",
    extra={
...
***REMOVED******REMOVED*** Known Issues Tracking
...
***REMOVED******REMOVED******REMOVED*** Unit Tests for Error Handling
