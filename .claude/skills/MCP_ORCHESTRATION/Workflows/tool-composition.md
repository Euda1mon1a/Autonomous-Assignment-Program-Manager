***REMOVED*** Tool Composition Workflow
...
***REMOVED******REMOVED*** DAG Patterns

***REMOVED******REMOVED******REMOVED*** Pattern 1: Linear Chain
...
async def schedule_generation_pipeline(
    ...
    ***REMOVED*** Step 1: Generate schedule
    ...
    ***REMOVED*** Step 2: Validate generated schedule
    ...
    ***REMOVED*** Step 3: Detect conflicts
    ...
    ***REMOVED*** Step 4: Check utilization
...
async def comprehensive_schedule_analysis(
    ...
    ***REMOVED*** Fan-out: Execute all analysis tools concurrently
    tasks = [
    ...
    ***REMOVED*** Gather results (wait for all to complete)
    ...
    ***REMOVED*** Aggregate results (even with partial failures)
    return {
...
async def find_best_swaps_for_team(
    ...
    ***REMOVED*** Map: Analyze swaps for each faculty member in parallel
    swap_tasks = [
        call_tool_with_retry(
    ...
    ***REMOVED*** Reduce: Aggregate and rank all candidates
    all_candidates = []
...
async def smart_deployment_workflow(
    ...
    ***REMOVED*** Conditional routing based on validation
    if not validation.valid:
        logger.error(f"Deployment validation failed: {validation.blockers}")

        ***REMOVED*** Route to rollback (even though nothing deployed yet)
        return {
            "status": "blocked",
    ...
    ***REMOVED*** Conditional routing based on security
    ...
    ***REMOVED*** Conditional routing based on smoke tests
    ...
    ***REMOVED*** All checks passed - promote to production
...
***REMOVED******REMOVED*** Data Transformation Between Tools

***REMOVED******REMOVED******REMOVED*** Type Mapping
...
***REMOVED******REMOVED******REMOVED*** Schema Adapter Pattern
...
class ToolAdapter:
    ...
    async def execute_chain(self, **source_params):
        ...
        ***REMOVED*** Execute source tool
        source_result = await call_tool_with_retry(
            self.source_tool,
            **source_params
        )

        ***REMOVED*** Transform result to target input
        target_params = self.transform_fn(source_result)

        ***REMOVED*** Execute target tool
...
***REMOVED******REMOVED*** Error Propagation in Chains

***REMOVED******REMOVED******REMOVED*** Fail-Fast Strategy
...
async def fail_fast_chain(*tool_calls):
    ...
    for tool_name, params in tool_calls:
        try:
            ...
            raise  ***REMOVED*** Propagate error, abort chain
...
async def graceful_chain(*tool_calls):
    ...
    for tool_name, params in tool_calls:
        try:
            ...
            results.append(None)  ***REMOVED*** Placeholder
...
***REMOVED******REMOVED******REMOVED*** Retry Chain Strategy
...
***REMOVED******REMOVED*** Result Synthesis

***REMOVED******REMOVED******REMOVED*** Aggregation Strategies

***REMOVED******REMOVED******REMOVED******REMOVED*** Sum/Count Aggregation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Max/Min Aggregation
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Merge Aggregation
...
***REMOVED******REMOVED******REMOVED*** Complex Synthesis Example
...
async def synthesize_schedule_health_report(start_date: str, end_date: str):
    ...
    ***REMOVED*** Synthesize overall health score
    health_score = 100.0

    ***REMOVED*** Validation: -10 per critical issue
    if not isinstance(validation, Exception):
        health_score -= validation.critical_issues * 10

    ***REMOVED*** Conflicts: -5 per conflict
    if not isinstance(conflicts, Exception):
        health_score -= conflicts.total_conflicts * 5

    ***REMOVED*** Contingency: -20 if N-1 fails, -50 if N-2 fails
    ...
    ***REMOVED*** Utilization: -30 if in red zone
    ...
    health_score = max(0, health_score)  ***REMOVED*** Floor at 0

    ***REMOVED*** Synthesize severity
    if health_score >= 80:
        severity = "healthy"
...
***REMOVED******REMOVED*** Example: Check Swap Safety Chain
...
async def check_swap_safety(
    ...
    ***REMOVED*** Step 1: Analyze swap candidates to get swap details
    ...
    ***REMOVED*** Find the specific candidate
    ...
    ***REMOVED*** Step 2: Simulate swap (update database in memory)
    ***REMOVED*** This would call a backend endpoint to simulate the swap

    ***REMOVED*** Step 3: Validate the simulated schedule
    validation = await call_tool_with_retry(
        "validate_schedule_tool",
    ...
    ***REMOVED*** Step 4: Check utilization impact
    ***REMOVED*** (Simplified - would need actual faculty counts)
    ...
    ***REMOVED*** Step 5: Detect conflicts in swapped schedule
