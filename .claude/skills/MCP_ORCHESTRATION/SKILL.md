...
***REMOVED*** MCP Orchestration Skill
...
***REMOVED******REMOVED*** Key Orchestration Phases
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Planning
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Execution
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Recovery
...
***REMOVED******REMOVED*** Orchestration Patterns

***REMOVED******REMOVED******REMOVED*** Pattern 1: Sequential Chain
```
...
***REMOVED******REMOVED******REMOVED*** Pattern 2: Parallel Fan-Out
```
       → Tool B
Tool A → Tool C
       → Tool D
...
***REMOVED******REMOVED******REMOVED*** Pattern 3: Map-Reduce
```
...
***REMOVED******REMOVED******REMOVED*** Pattern 4: Conditional Routing
```
...
***REMOVED******REMOVED*** Error Handling Strategy
...
***REMOVED******REMOVED******REMOVED*** API Connectivity Test
```bash
...
***REMOVED******REMOVED******REMOVED*** 1. Schedule Safety Check
...
***REMOVED******REMOVED******REMOVED*** 2. Emergency Coverage Response
...
***REMOVED******REMOVED******REMOVED*** 3. Deployment Pipeline
...
***REMOVED******REMOVED******REMOVED*** 4. Performance Optimization
...
***REMOVED******REMOVED*** Concrete Implementation Examples

The workflows above are high-level. This section provides detailed, runnable code examples.

***REMOVED******REMOVED******REMOVED*** Example 1: Parallel Safety Check with Error Handling
...
async def comprehensive_safety_check(
    ...
    """
    ***REMOVED*** Phase 1: Define all checks with timeout wrapper
    ...
    ***REMOVED*** Phase 2: Execute checks in parallel
    checks = [
    ...
    ***REMOVED*** Phase 3: Aggregate and analyze results
    report = {
...
***REMOVED******REMOVED******REMOVED*** Example 2: Emergency Coverage with Tiered Strategies
...
async def handle_emergency_absence(
    ...
    ***REMOVED*** TIER 1: Try swap-based coverage
    print("Tier 1: Attempting swap matching...")
    try:
        ***REMOVED*** Get affected assignments
        ...
        ***REMOVED*** Try to find swaps for each affected assignment
        for assignment_id in affected[:10]:  ***REMOVED*** Limit to first 10
            try:
        ...
        ***REMOVED*** If we can cover >= 80% via swaps, use this strategy
    ...
    ***REMOVED*** TIER 2: Try sacrifice hierarchy
    ...
    ***REMOVED*** TIER 3: Try static fallback
    ...
    ***REMOVED*** TIER 4: Manual escalation
...
***REMOVED******REMOVED*** Common Failure Modes and Solutions

Real-world orchestration failures and how to handle them.

***REMOVED******REMOVED******REMOVED*** Failure Mode 1: MCP Server Unresponsive
...
async def check_mcp_health() -> bool:
    ...
    try:
        ***REMOVED*** Simple health check with short timeout
...
**Recovery Steps:**
```bash
***REMOVED*** 1. Check container status
docker-compose ps mcp-server

***REMOVED*** 2. Check logs for errors
docker-compose logs --tail=50 mcp-server

***REMOVED*** 3. Restart container if needed
docker-compose restart mcp-server

***REMOVED*** 4. Verify backend connectivity from MCP
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 2: Partial Results from Parallel Fan-Out
...
**Example Scenario:**
```python
***REMOVED*** 3 out of 4 checks succeeded - what do we do?
results = await asyncio.gather(*checks, return_exceptions=True)
***REMOVED*** [
***REMOVED***   {"data": {...}},           ***REMOVED*** Success
***REMOVED***   TimeoutError(),             ***REMOVED*** Failed
***REMOVED***   {"data": {...}},           ***REMOVED*** Success
***REMOVED***   ConnectionError()           ***REMOVED*** Failed
***REMOVED*** ]
```
...
***REMOVED******REMOVED******REMOVED*** Failure Mode 3: Tool Output Schema Mismatch
...
**Example:**
```python
***REMOVED*** Expected schema
expected = {"is_valid": bool, "violations": list}

***REMOVED*** Actual response (missing field)
actual = {"is_valid": True}  ***REMOVED*** Where's violations?

***REMOVED*** Code crashes
violations = result["violations"]  ***REMOVED*** KeyError!
```
...
def safe_extract(raw_result: Dict[str, Any]) -> ValidationResult:
    ...
    try:
        ...
        ***REMOVED*** Return safe default
        return ValidationResult(is_valid=False, violations=["Schema parse error"])
```

***REMOVED******REMOVED******REMOVED*** Failure Mode 4: Dependency Chain Breaks Mid-Execution
...
**Example:**
```python
***REMOVED*** Sequential dependency: A → B → C
result_a = await tool_a()  ***REMOVED*** Succeeds
result_b = await tool_b(result_a["value"])  ***REMOVED*** FAILS - what about C?
result_c = await tool_c(result_b["value"])  ***REMOVED*** Can't run - result_b doesn't exist
```
...
async def execute_dependency_chain(
    ...
    for tool_name, tool_func in tools:
        try:
            ***REMOVED*** Execute with previous results as context
            result = await tool_func(results)
            results[tool_name] = result
            executed_tools.append(tool_name)

        except Exception as e:
            ***REMOVED*** Chain broken - decide rollback
            report = {
                "status": "FAILED",
                "failed_at": tool_name,
            ...
            if rollback_on_failure:
                ...
                for completed_tool in reversed(executed_tools):
                    try:
                        ***REMOVED*** Call rollback if available
...
***REMOVED******REMOVED******REMOVED*** From FastAPI Endpoint
...
async def api_safety_check(
    ...
    """
    try:
        ***REMOVED*** Quick check - run synchronously
        if (end_date - start_date).days <= 7:
            report = await comprehensive_safety_check(start_date, end_date)
            return report

        ***REMOVED*** Long check - run in background
        else:
...
***REMOVED******REMOVED******REMOVED*** From Celery Task
...
def celery_emergency_coverage(
    ...
    """
    try:
        ***REMOVED*** Celery tasks aren't async - use asyncio.run
        result = asyncio.run(
        ...
        ***REMOVED*** Retry on transient errors
        if "timeout" in str(e).lower() or "connection" in str(e).lower():
            self.retry(exc=e, countdown=30)  ***REMOVED*** Retry after 30s
        else:
...
***REMOVED******REMOVED******REMOVED*** From CLI Script
...
async def main():
    ...
    ***REMOVED*** Exit code based on status
...
***REMOVED******REMOVED******REMOVED*** MCP Server Not Responding
```bash
...
***REMOVED******REMOVED******REMOVED*** Backend API Unreachable from MCP
```bash
***REMOVED*** Test connectivity from MCP container
docker-compose exec mcp-server curl -s http://backend:8000/health
...
***REMOVED******REMOVED******REMOVED*** Tool Returns Unexpected Result
...
***REMOVED******REMOVED*** Real-World Scenario: Multi-Skill Integration

Complete end-to-end example showing MCP orchestration integrated with other skills.

***REMOVED******REMOVED******REMOVED*** Scenario: Pre-Deployment Safety Validation
...
async def validate_block_10_deployment():
    ...
    ***REMOVED*** Use MCP tools for parallel checks
    ...
    if safety_report["status"] == "FAIL":
        ...
        ***REMOVED*** Systematic debugging of failures
        for issue in safety_report["blocking_issues"]:
            print(f"  Investigating: {issue}")
            ***REMOVED*** Debugger skill would explore root causes
    ...
    ***REMOVED*** Additional validation via other skills
    print("\n✓ MCP safety checks passed")
    print("Step 2: Running constraint-preflight...")

    ***REMOVED*** Ensure all constraints are properly registered
    ...
    ***REMOVED*** Detailed ACGME compliance check
    ...
    ***REMOVED*** Safe schedule generation - verify backup
    ...
    ***REMOVED*** All checks passed - approve deployment
