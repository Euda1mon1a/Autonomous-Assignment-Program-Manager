...
- [Complete System Health Check](***REMOVED***complete-system-health-check) - Start here if unsure
- [Backend API Issues](***REMOVED***backend-api-issues) - 500 errors, slow responses
- [MCP Tool Failures](***REMOVED***mcp-tool-failures) - Tool timeouts, connection errors
- [Database Issues](***REMOVED***database-issues) - Slow queries, deadlocks, connection errors
- [Schedule Generation Issues](***REMOVED***schedule-generation-issues) - INFEASIBLE, validation failures
- [Celery Task Issues](***REMOVED***celery-task-issues) - Task failures, queue backlogs
- [Agent Workflow Issues](***REMOVED***agent-workflow-issues) - Multi-agent coordination failures

---

***REMOVED******REMOVED*** Complete System Health Check

Use this comprehensive checklist when starting an investigation.

***REMOVED******REMOVED******REMOVED*** 1. Service Availability
...
***REMOVED*** Expected output: all services "Up"
***REMOVED*** backend        Up (healthy)   8000/tcp
***REMOVED*** db             Up             5432/tcp
***REMOVED*** redis          Up             6379/tcp
***REMOVED*** mcp-server     Up (healthy)   8080/tcp
...
***REMOVED******REMOVED******REMOVED*** 2. Basic Connectivity

Test that services can communicate:

```bash
***REMOVED*** Test backend API
...
asyncio.run(test())
"
***REMOVED*** Expected: "Database connected"

***REMOVED*** Test MCP → backend
...
***REMOVED******REMOVED******REMOVED*** 3. Database Health
...
**If failed:**
- [ ] High connection count → See [Connection Pool Exhausted](common-failure-patterns.md***REMOVED***pattern-1-connection-pool-exhausted)
- [ ] Long-running queries → See [Slow Queries](common-failure-patterns.md***REMOVED***pattern-12-slow-queries)
- [ ] Locks detected → See [Deadlocks](common-failure-patterns.md***REMOVED***pattern-2-deadlocks)

---

***REMOVED******REMOVED******REMOVED*** 4. Redis Health
...
***REMOVED*** Expected: Reasonable usage (< 1GB for typical workload)

***REMOVED*** Check Celery queue depth
LLEN celery
***REMOVED*** Expected: < 100 (unless heavy load)

***REMOVED*** Check for failed tasks
LLEN celery-failed
***REMOVED*** Expected: 0 (or low number)
```
...
***REMOVED******REMOVED******REMOVED*** 5. Celery Workers
...
***REMOVED*** Expected: List of active tasks (or empty if idle)

***REMOVED*** Check registered tasks
docker-compose exec celery-worker celery -A app.core.celery_app inspect registered
***REMOVED*** Expected: List of registered tasks

***REMOVED*** Check worker stats
docker-compose exec celery-worker celery -A app.core.celery_app inspect stats
***REMOVED*** Expected: Worker info, pool stats
```
...
***REMOVED******REMOVED*** Backend API Issues
...
```bash
curl -s http://localhost:8000/health | jq
***REMOVED*** Expected: {"status": "healthy", "database": "connected", "redis": "connected"}
```
☐ Health check passes
...
```bash
***REMOVED*** Extract response times from logs
docker-compose logs backend | grep "completed in" | tail -20
***REMOVED*** Look for patterns: sudden spikes, consistently slow endpoints
```
☐ Response times normal (< 1s for most endpoints)

**4. Check Database Connection Pool**
```python
***REMOVED*** Run this in backend container
docker-compose exec backend python -c "
...
```bash
***REMOVED*** Test the failing endpoint
curl -v -X POST http://localhost:8000/api/[endpoint] \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"test": "data"}'

***REMOVED*** Check response code, headers, body
```
...
***REMOVED******REMOVED*** MCP Tool Failures
...
for tool in sorted([t.name for t in mcp.tools]):
    print(f'  - {tool}')
"
***REMOVED*** Expected: 29+ tools listed
```
...
```bash
***REMOVED*** From MCP server container, test a tool
docker-compose exec mcp-server python -c "
from scheduler_mcp.server import mcp
import asyncio

async def test():
    ***REMOVED*** Example: test health check tool
...
**If issues persist:**
- [ ] Check specific timeout pattern in [Common Failure Patterns](common-failure-patterns.md***REMOVED***pattern-3-validate_schedule-timeout)
...
***REMOVED******REMOVED*** Database Issues
...
**If issues persist:**
- [ ] Connection pool exhausted → See [Pattern 1](common-failure-patterns.md***REMOVED***pattern-1-connection-pool-exhausted)
- [ ] Slow queries → See [Pattern 12](common-failure-patterns.md***REMOVED***pattern-12-slow-queries)
- [ ] Deadlocks → See [Pattern 2](common-failure-patterns.md***REMOVED***pattern-2-deadlocks)
...
```bash
***REMOVED*** Look for log entries about problem size
docker-compose logs backend | grep -i "variables\|constraints" | tail -10

***REMOVED*** Example output:
***REMOVED*** Creating model with 2500 variables, 5000 constraints
```
...
```python
***REMOVED*** Temporarily disable optional constraints
***REMOVED*** In backend/app/scheduling/engine.py

***REMOVED*** Comment out optional constraints to isolate issue
***REMOVED*** solver.Add(optional_constraint_1)  ***REMOVED*** ← Commented out
***REMOVED*** solver.Add(optional_constraint_2)  ***REMOVED*** ← Commented out

***REMOVED*** Keep only hard constraints (ACGME, coverage)
...
```bash
***REMOVED*** Test validation endpoint
...
***REMOVED*** Expected: Detailed validation results
```
☐ Validation works independently

**If issues persist:**
- [ ] INFEASIBLE → See [Pattern 5](common-failure-patterns.md***REMOVED***pattern-5-infeasible---no-solution-found)
- [ ] Timeout → See [Pattern 6](common-failure-patterns.md***REMOVED***pattern-6-solver-timeout)
- [ ] Validation fails → See [Pattern 8](common-failure-patterns.md***REMOVED***pattern-8-acgme-validation-fails-after-generation)
- [ ] Analyze constraint interactions in detail

---

***REMOVED******REMOVED*** Celery Task Issues
...
***REMOVED*** Expected: < 100 (depends on load)
```
☐ Queue depth reasonable

**4. Check for Failed Tasks**
```bash
***REMOVED*** Check failed queue
docker-compose exec redis redis-cli LLEN celery-failed

***REMOVED*** If failed tasks exist, inspect them
...
```bash
***REMOVED*** Check that periodic tasks are scheduled
...
```python
***REMOVED*** From backend container
docker-compose exec backend python -c "
...
***REMOVED*** Wait for result (max 30s)
try:
...
**If issues persist:**
- [ ] Task serialization error → See [Pattern 9](common-failure-patterns.md***REMOVED***pattern-9-task-serialization-error)
...
***REMOVED******REMOVED*** Agent Workflow Issues
...
```bash
***REMOVED*** Check Redis queues for each agent
docker-compose exec redis redis-cli KEYS "agent:*:queue"

***REMOVED*** Check queue depths
...
```bash
***REMOVED*** Check for unacknowledged messages
...
**4. Review Agent Logs**
```bash
***REMOVED*** If agents log to separate files/streams
...
```python
***REMOVED*** Test sending a message between agents
***REMOVED*** (Implementation depends on your agent framework)
```
☐ Communication test passes

**If issues persist:**
- [ ] See [Pattern 7](common-failure-patterns.md***REMOVED***pattern-7-multi-agent-coordination-failure)
...
***REMOVED******REMOVED*** Debugging Decision Tree
...
***REMOVED******REMOVED******REMOVED*** Essential Health Checks

```bash
***REMOVED*** All services status
docker-compose ps

***REMOVED*** All service logs
...
***REMOVED*** Celery workers
docker-compose exec celery-worker celery -A app.core.celery_app inspect active
```

***REMOVED******REMOVED******REMOVED*** Common Diagnostic Queries
