...
***REMOVED******REMOVED*** Pattern Index

- [Database Connection Failures](***REMOVED***database-connection-failures)
- [MCP Tool Timeouts](***REMOVED***mcp-tool-timeouts)
- [Constraint Engine Errors](***REMOVED***constraint-engine-errors)
- [Agent Communication Failures](***REMOVED***agent-communication-failures)
- [Schedule Generation Failures](***REMOVED***schedule-generation-failures)
- [Celery Task Failures](***REMOVED***celery-task-failures)
- [ACGME Compliance Validation Errors](***REMOVED***acgme-compliance-validation-errors)
- [API Authentication Failures](***REMOVED***api-authentication-failures)
- [Performance Degradation](***REMOVED***performance-degradation)

---

***REMOVED******REMOVED*** Database Connection Failures

***REMOVED******REMOVED******REMOVED*** Pattern 1: Connection Pool Exhausted
...
***REMOVED*** Check for long-running queries
docker-compose exec db psql -U scheduler -d residency_scheduler -c "
...
***REMOVED*** Expected: active_connections near max_connections
***REMOVED*** Expected: Multiple queries in 'active' state for extended time
```
...
**Immediate:**
```bash
***REMOVED*** Restart backend to clear connection pool
docker-compose restart backend

***REMOVED*** Terminate long-running queries
docker-compose exec db psql -U scheduler -d residency_scheduler -c "
...
***REMOVED*** Ensure connections are properly closed

***REMOVED*** BAD - no guarantee of cleanup
async def get_db():
    db = AsyncSession()
    return db  ***REMOVED*** ← Connection may leak on error

***REMOVED*** GOOD - always cleanup
async def get_db():
    db = AsyncSession()
    try:
        yield db
    finally:
        await db.close()  ***REMOVED*** ← Always cleanup

***REMOVED*** Increase pool size if needed
***REMOVED*** backend/app/core/config.py
SQLALCHEMY_POOL_SIZE = 20  ***REMOVED*** up from 10
SQLALCHEMY_MAX_OVERFLOW = 10  ***REMOVED*** up from 5
```
...
**Related Patterns:**
- [Pattern 2: Deadlocks](***REMOVED***pattern-2-deadlocks)
- [Performance Degradation: Slow Queries](***REMOVED***slow-queries)

---

***REMOVED******REMOVED******REMOVED*** Pattern 2: Deadlocks
...
**Immediate:**
```python
***REMOVED*** Add explicit row-level locking
...
***REMOVED*** GOOD - explicit lock prevents deadlock
person = await db.execute(
    select(Person)
    .where(Person.id == id)
    .with_for_update()  ***REMOVED*** ← Explicit row lock
)
person.status = "busy"
await db.commit()
```
...
**Permanent:**
```python
***REMOVED*** Ensure consistent resource ordering
***REMOVED*** backend/app/services/schedule_generator.py

***REMOVED*** BAD - variable ordering
for person in people:
    for block in blocks:
        lock_resources(person, block)  ***REMOVED*** ← Order depends on iteration

***REMOVED*** GOOD - consistent ordering
for person in sorted(people, key=lambda p: p.id):  ***REMOVED*** ← Always same order
    for block in sorted(blocks, key=lambda b: b.id):
        lock_resources(person, block)

***REMOVED*** Add retry logic with exponential backoff
...
**Related Patterns:**
- [Pattern 1: Connection Pool Exhausted](***REMOVED***pattern-1-connection-pool-exhausted)

---

***REMOVED******REMOVED*** MCP Tool Timeouts

***REMOVED******REMOVED******REMOVED*** Pattern 3: validate_schedule Timeout
...
**Immediate:**
```python
***REMOVED*** Increase MCP timeout
***REMOVED*** mcp-server/src/scheduler_mcp/config.py
DEFAULT_TIMEOUT = 120  ***REMOVED*** up from 30
```

**Permanent:**
```python
***REMOVED*** Optimize ACGME validator
***REMOVED*** backend/app/scheduling/acgme_validator.py

***REMOVED*** BAD - serial validation
def validate_schedule(schedule):
    for day in range(365):
        validate_day(schedule, day)  ***REMOVED*** ← Sequential, 365 iterations

***REMOVED*** GOOD - batch validation
def validate_schedule(schedule):
    ***REMOVED*** Validate weekly constraints (52 weeks, not 365 days)
    for week in range(52):
        validate_week(schedule, week)  ***REMOVED*** ← 85% fewer iterations

    ***REMOVED*** Parallel validation for independent checks
...
def validate_schedule_cached(schedule_id: str, schedule_hash: str):
    ***REMOVED*** Cache based on schedule hash
...
**Related Patterns:**
- [Performance Degradation: Slow API Endpoints](***REMOVED***slow-api-endpoints)

---

***REMOVED******REMOVED******REMOVED*** Pattern 4: MCP Server Connection Refused
...
***REMOVED*** Check MCP server logs
docker-compose logs mcp-server --tail=50

***REMOVED*** Test connectivity
curl -s http://localhost:8080/health

***REMOVED*** From within Docker network
...
***REMOVED*** Check startup errors
docker-compose logs mcp-server

***REMOVED*** Verify port mapping
docker-compose ps mcp-server
***REMOVED*** Should show: 0.0.0.0:8080->8080/tcp
```

**Permanent:**
```yaml
***REMOVED*** docker-compose.yml - Add health check
mcp-server:
  ...
  healthcheck:
    ...
    retries: 3
    start_period: 40s
  restart: unless-stopped  ***REMOVED*** Auto-restart on crash
```
...
***REMOVED******REMOVED*** Constraint Engine Errors

***REMOVED******REMOVED******REMOVED*** Pattern 5: INFEASIBLE - No Solution Found
...
**Diagnosis:**
```bash
***REMOVED*** Check solver logs
docker-compose logs backend | grep -i "solver\|infeasible"

***REMOVED*** Enable constraint debugging
***REMOVED*** In backend/app/scheduling/engine.py
solver.EnableOutput()  ***REMOVED*** Prints detailed constraint violations

***REMOVED*** Test with minimal constraints
***REMOVED*** Disable optional constraints one by one to find conflict
```
...
if solver.Solve() == cp_model.INFEASIBLE:
    ...
    ***REMOVED*** Try relaxing soft constraints
...
**Permanent:**
```python
***REMOVED*** Add constraint validation
***REMOVED*** backend/app/scheduling/constraints/validator.py

def validate_constraint_feasibility(persons, blocks, constraints):
    """Pre-validate constraints before solving."""

    ***REMOVED*** Check resource capacity
    ...
    ***REMOVED*** Check for known conflicting constraint pairs
...
***REMOVED******REMOVED******REMOVED*** Pattern 6: Solver Timeout
...
**Diagnosis:**
```bash
***REMOVED*** Check solver logs for timeout
docker-compose logs backend | grep "solver.*timeout\|UNKNOWN"

***REMOVED*** Check problem size
docker-compose logs backend | grep "variables\|constraints"
***REMOVED*** Look for: "Creating model with 5000 variables, 10000 constraints"
```
...
**Immediate:**
```python
***REMOVED*** Increase solver timeout
***REMOVED*** backend/app/scheduling/engine.py
solver.parameters.max_time_in_seconds = 600  ***REMOVED*** up from 300
```

**Permanent:**
```python
***REMOVED*** Add search strategies to guide solver
from ortools.sat.python import cp_model

solver = cp_model.CpSolver()
solver.parameters.max_time_in_seconds = 600

***REMOVED*** Add search strategy (helps solver make better choices)
solver.parameters.search_branching = cp_model.FIXED_SEARCH
solver.parameters.preferred_variable_order = cp_model.CHOOSE_FIRST

***REMOVED*** Add hints from previous solutions
if previous_schedule:
    for person, block, value in previous_schedule:
        solver.AddHint(assignments[(person, block)], value)

***REMOVED*** Add solution callback to return best-so-far if timeout
...
***REMOVED******REMOVED*** Agent Communication Failures

***REMOVED******REMOVED******REMOVED*** Pattern 7: Multi-Agent Coordination Failure
...
**Diagnosis:**
```bash
***REMOVED*** Check agent logs (if available)
***REMOVED*** Look for task handoff messages

***REMOVED*** Check for orphaned tasks
docker-compose exec redis redis-cli LRANGE agent:queue:pending 0 -1

***REMOVED*** Check for deadlocks in workflow state
docker-compose exec db psql -U scheduler -d residency_scheduler -c "
...
**Permanent:**
```python
***REMOVED*** Add task acknowledgment pattern
***REMOVED*** backend/app/agents/coordinator.py
...
***REMOVED******REMOVED*** Schedule Generation Failures

***REMOVED******REMOVED******REMOVED*** Pattern 8: ACGME Validation Fails After Generation
...
**Immediate:**
```python
***REMOVED*** Add pre-validation to generator
***REMOVED*** backend/app/scheduling/engine.py
...
def generate_schedule(persons, blocks, constraints):
    ...
    ***REMOVED*** ALWAYS validate before returning
...
**Permanent:**
```python
***REMOVED*** Unify constraint logic between generator and validator
***REMOVED*** backend/app/scheduling/constraints/acgme.py
...
***REMOVED******REMOVED*** Celery Task Failures

***REMOVED******REMOVED******REMOVED*** Pattern 9: Task Serialization Error
...
**Diagnosis:**
```bash
***REMOVED*** Check celery logs
docker-compose logs celery-worker | grep -i "serializ"

***REMOVED*** Check task arguments
...
**Immediate:**
```python
***REMOVED*** Don't pass model objects to tasks
***REMOVED*** backend/app/tasks/schedule.py

***REMOVED*** BAD - passing model object
person = await db.get(Person, person_id)
generate_schedule_task.delay(person)  ***REMOVED*** ← Will fail to serialize

***REMOVED*** GOOD - pass primitive types only
generate_schedule_task.delay(person_id=person.id)  ***REMOVED*** ← Serializable

***REMOVED*** In task, reload from database
@celery.task
...
**Permanent:**
```python
***REMOVED*** Add task argument validation
***REMOVED*** backend/app/tasks/base.py
...
class ValidatedTask(Task):
    ...
    def apply_async(self, args=None, kwargs=None, **options):
        ***REMOVED*** Validate all arguments are JSON-serializable
        try:
...
***REMOVED*** Use as base class for all tasks
...
***REMOVED******REMOVED*** ACGME Compliance Validation Errors

***REMOVED******REMOVED******REMOVED*** Pattern 10: False Positive 1-in-7 Violation
...
**Immediate:**
```python
***REMOVED*** Fix 1-in-7 validation logic
***REMOVED*** backend/app/scheduling/acgme_validator.py

***REMOVED*** BAD - checks calendar days
def check_1_in_7(assignments, start_date):
    for day in range(7):
        date = start_date + timedelta(days=day)
        if not has_assignment_on_date(assignments, date):
            return True  ***REMOVED*** Found a day off
    return False  ***REMOVED*** No full day off found

***REMOVED*** GOOD - checks 24-hour periods
def check_1_in_7(assignments, start_date):
    ***REMOVED*** Check for ANY 24-hour period without work
    ...
    for hour in range(7 * 24):
        ...
        ***REMOVED*** Check if this 24-hour period has no assignments
        has_work = any(
        ...
        if not has_work:
            return True  ***REMOVED*** Found 24-hour period off
...
**Permanent:**
```python
***REMOVED*** Add comprehensive ACGME tests
***REMOVED*** backend/tests/test_acgme_validator.py
...
class TestACGME1in7Rule:
    ...
    async def test_1in7_24_hour_period_off(self):
        ...
        ***REMOVED*** Off from noon Monday to noon Tuesday
    ...
    async def test_1in7_violation(self):
        ...
        ***REMOVED*** Work every day, alternating AM/PM
    ...
    async def test_1in7_timezone_handling(self):
        ...
        ***REMOVED*** Test with HST timezone
...
***REMOVED******REMOVED*** API Authentication Failures

***REMOVED******REMOVED******REMOVED*** Pattern 11: JWT Token Expired
...
***REMOVED*** Check token claims
***REMOVED*** Decode JWT at jwt.io or:
...
**Permanent:**
```python
***REMOVED*** Implement token refresh
***REMOVED*** backend/app/api/routes/auth.py
...
***REMOVED*** Add auto-refresh to MCP client
***REMOVED*** mcp-server/src/scheduler_mcp/client.py
...
class SchedulerAPIClient:
    ...
    async def get_token(self):
        ...
        ***REMOVED*** Refresh if token expires in < 5 minutes
...
***REMOVED******REMOVED*** Performance Degradation

***REMOVED******REMOVED******REMOVED*** Pattern 12: Slow Queries
...
**Permanent:**
```python
***REMOVED*** Add eager loading to prevent N+1
***REMOVED*** backend/app/api/routes/assignments.py

***REMOVED*** BAD - N+1 query
assignments = await db.execute(select(Assignment))
for assignment in assignments.scalars():
    person = assignment.person  ***REMOVED*** ← Separate query for each!
...
assignments = await db.execute(
    select(Assignment)
    .options(selectinload(Assignment.person))  ***REMOVED*** ← Load all persons in one query
)
for assignment in assignments.scalars():
    person = assignment.person  ***REMOVED*** ← No additional query
    print(f"{person.name}: {assignment.date}")

***REMOVED*** Add query performance monitoring
***REMOVED*** backend/app/db/session.py
...
def receive_after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    ...
    if total > 1.0:  ***REMOVED*** Log queries > 1 second
...
**Related Patterns:**
- [Pattern 1: Connection Pool Exhausted](***REMOVED***pattern-1-connection-pool-exhausted)
- [Pattern 3: validate_schedule Timeout](***REMOVED***pattern-3-validate_schedule-timeout)

---

***REMOVED******REMOVED*** Pattern Template

Use this template when adding new patterns:

```markdown
***REMOVED******REMOVED******REMOVED*** Pattern [N]: [Name]
...
**Diagnosis:**
```bash
***REMOVED*** Commands to confirm this pattern
```
...
**Immediate:**
```bash
***REMOVED*** Quick fix to restore service
```

**Permanent:**
```python
***REMOVED*** Code changes to prevent recurrence
```
