...
***REMOVED******REMOVED*** Log Locations

***REMOVED******REMOVED******REMOVED*** Backend API Logs
...
***REMOVED*** Uvicorn access logs
tail -f backend/logs/access.log

***REMOVED*** Error logs only
tail -f backend/logs/error.log
```

***REMOVED******REMOVED******REMOVED*** MCP Server Logs

**Docker Environment**
```bash
***REMOVED*** Real-time MCP logs
docker-compose logs mcp-server --tail=100 --follow

***REMOVED*** Tool invocations
...
**Direct Access**
```bash
***REMOVED*** If running MCP server directly
tail -f mcp-server/logs/mcp.log
```

***REMOVED******REMOVED******REMOVED*** Database Logs
...
***REMOVED*** Enable query logging (for current session)
SET log_statement = 'all';
SET log_duration = on;

***REMOVED*** View PostgreSQL logs
...
***REMOVED******REMOVED******REMOVED*** Celery Worker Logs
...
***REMOVED*** Beat scheduler logs
docker-compose logs celery-beat --tail=50 --follow

***REMOVED*** Filter for specific task
docker-compose logs celery-worker | grep "task_name"
```

**Task Status**
```bash
***REMOVED*** Check Redis for task queues
docker-compose exec redis redis-cli

***REMOVED*** Inside Redis CLI:
LLEN celery              ***REMOVED*** Queue length
LRANGE celery 0 10       ***REMOVED*** First 10 tasks
KEYS celery-task-*       ***REMOVED*** All task results
```

***REMOVED******REMOVED******REMOVED*** Redis Logs

```bash
***REMOVED*** Redis server logs
docker-compose logs redis --tail=50

***REMOVED*** Monitor Redis commands in real-time
docker-compose exec redis redis-cli MONITOR
```

***REMOVED******REMOVED*** Log Analysis Phases

***REMOVED******REMOVED******REMOVED*** Phase 1: Log Location Discovery
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Error Pattern Extraction

***REMOVED******REMOVED******REMOVED******REMOVED*** Common Error Patterns

**1. HTTP 500 Errors**
```bash
***REMOVED*** Find all 500 errors with context
docker-compose logs backend | grep -B 5 -A 5 "500"

***REMOVED*** Count 500 errors by endpoint
...
**2. MCP Tool Timeouts**
```bash
***REMOVED*** Find timeout errors
docker-compose logs mcp-server | grep -i "timeout\|timed out"

***REMOVED*** Identify which tools are timing out
...
```bash
***REMOVED*** Find deadlock errors
docker-compose logs db | grep -i "deadlock"

***REMOVED*** Context around deadlock
...
```bash
***REMOVED*** Find solver errors
docker-compose logs backend | grep -i "solver\|constraint\|infeasible"

***REMOVED*** Identify specific constraint failures
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Cross-Service Correlation

Trace a single request across multiple services using request IDs or timestamps.

***REMOVED******REMOVED******REMOVED******REMOVED*** Using Request IDs (Recommended)
...
***REMOVED*** In routes, use:
logger.info(f"[{request_id}] Processing schedule generation")
```

**Correlation Example**
```bash
***REMOVED*** Find request ID in backend
REQUEST_ID="abc-123-def"

***REMOVED*** Trace across services
echo "=== Backend Logs ==="
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Using Timestamps (When Request IDs Not Available)
...
```bash
***REMOVED*** Merge and sort all logs by timestamp
cat backend_window.log mcp_window.log db_window.log celery_window.log \
  | sort > combined_timeline.log

***REMOVED*** Analyze timeline
less combined_timeline.log
```

***REMOVED******REMOVED******REMOVED*** Phase 4: Timeline Reconstruction
...
```markdown
***REMOVED******REMOVED*** Timeline: [Incident Description]
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Automated Timeline Generation

**Using grep and awk**
```bash
***REMOVED***!/bin/bash
***REMOVED*** Extract timeline from logs
...
***REMOVED*** Sort by timestamp (requires further processing)
```

***REMOVED******REMOVED******REMOVED*** Phase 5: Anomaly Detection

Identify patterns that deviate from normal behavior.

***REMOVED******REMOVED******REMOVED******REMOVED*** Frequency Analysis

**1. Error Rate Over Time**
```bash
***REMOVED*** Count errors per minute
...
***REMOVED*** Output example:
***REMOVED***   5 2025-12-26 14:30  ← Normal
***REMOVED***  45 2025-12-26 14:31  ← Spike!
***REMOVED***   3 2025-12-26 14:32  ← Recovered
```

**2. Response Time Distribution**
```bash
***REMOVED*** Extract response times from uvicorn logs
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern Recognition

**Recurring Errors**
```bash
***REMOVED*** Find most common error messages
docker-compose logs backend \
  | grep ERROR \
  | awk -F'ERROR' '{print $2}' \
  | sort \
...
***REMOVED*** Output shows top 10 most frequent errors
```

**Cascading Failures**
```bash
***REMOVED*** Look for error propagation
docker-compose logs backend --since 2025-12-26T14:30:00 \
  | grep -E "ERROR|CRITICAL" \
  | awk '{print $1 " " $2 " - " $0}'

***REMOVED*** Visual inspection will show if errors cluster together (cascade)
```

***REMOVED******REMOVED*** Diagnostic Queries

***REMOVED******REMOVED******REMOVED*** Backend Health Check

```bash
***REMOVED*** Check if backend is responding
...
***REMOVED***   "redis": "connected"
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED*** Database Connection Status
...
***REMOVED******REMOVED******REMOVED*** MCP Server Connectivity

```bash
***REMOVED*** Test from host
curl -s http://localhost:8080/health

***REMOVED*** Test from within Docker network
docker-compose exec mcp-server curl -s http://backend:8000/health

***REMOVED*** Check tool availability
...
***REMOVED******REMOVED******REMOVED*** Celery Queue Status
...
***REMOVED*** Pending tasks
LRANGE celery 0 -1

***REMOVED*** Task results (successful)
KEYS celery-task-meta-*

***REMOVED*** Failed tasks
LRANGE celery-failed 0 -1
```

***REMOVED******REMOVED*** Log Analysis Checklist
...
***REMOVED******REMOVED******REMOVED*** Regular Expression Patterns
...
***REMOVED******REMOVED******REMOVED*** Performance Analysis

**Identify slow endpoints**
```bash
***REMOVED*** Extract all request durations
docker-compose logs backend \
  | grep "completed in" \
  | awk '{
      endpoint = $(NF-3)
...
***REMOVED******REMOVED*** Example: Complete Log Analysis Session

```bash
***REMOVED***!/bin/bash
***REMOVED*** Log analysis for incident INC-20251226-001
