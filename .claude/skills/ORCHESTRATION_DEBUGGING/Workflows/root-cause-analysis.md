***REMOVED*** Root Cause Analysis Workflow
...
***REMOVED******REMOVED*** The 5-Whys Methodology
...
***REMOVED******REMOVED******REMOVED*** Important Rules
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Problem Statement Definition

Start with a clear, specific problem statement.

***REMOVED******REMOVED******REMOVED******REMOVED*** Problem Statement Template
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Criteria for Good Problem Statement
...
***REMOVED******REMOVED******REMOVED*** Phase 2: Iterative "Why" Questioning

Ask why the problem occurred, then why that happened, and so on.

***REMOVED******REMOVED******REMOVED******REMOVED*** Question Framework

For each why, use this framework:

```markdown
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***[N]: [Previous answer/problem]
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Example: Complete 5-Whys

```markdown
***REMOVED******REMOVED*** 5-Whys Analysis: Schedule Generation 500 Errors
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***1: Why did the API return 500 errors?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***2: Why did database operations timeout?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***3: Why were queries running for >30 seconds?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***4: Why were queries doing sequential scans?
...
***REMOVED******REMOVED******REMOVED*** ROOT CAUSE: Why wasn't the migration applied?

**Evidence**:
```bash
***REMOVED*** Check deployment script
$ cat scripts/deploy.sh
***REMOVED***!/bin/bash
git pull origin main
docker-compose build
docker-compose up -d
***REMOVED*** ← Missing: alembic upgrade head

***REMOVED*** Check CI/CD pipeline
$ cat .github/workflows/deploy.yml
***REMOVED*** ← No migration verification step
```
...
***REMOVED******REMOVED*** Root Cause Summary
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Evidence Gathering

For each "why", gather concrete evidence.

***REMOVED******REMOVED******REMOVED******REMOVED*** Types of Evidence
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Evidence Collection Commands

**Logs**
```bash
***REMOVED*** Backend errors in last hour
docker-compose logs backend --since 1h | grep ERROR

***REMOVED*** MCP tool calls
...
**Code**
```bash
***REMOVED*** Recent changes to relevant files
git log -10 --oneline -- backend/app/api/routes/schedule.py

***REMOVED*** Diff of recent change
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Root Cause Identification

Identify when you've reached the root cause.

***REMOVED******REMOVED******REMOVED******REMOVED*** Root Cause Checklist
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Common False Roots
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Contributing Factors
...
***REMOVED******REMOVED*** Examples from Scheduling System

***REMOVED******REMOVED******REMOVED*** Example 1: MCP Tool Timeout

```markdown
***REMOVED******REMOVED*** Problem: MCP validate_schedule tool times out

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***1: Why does the tool timeout?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***2: Why does the API call take >30 seconds?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***3: Why does validation process days serially?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***4: Why wasn't bulk validation implemented?
**Evidence**: Original PR comments
**Answer**: MVP focused on correctness, not performance
**Verification**: PR ***REMOVED***123 comments: "Optimization deferred to v2"

***REMOVED******REMOVED******REMOVED*** ROOT CAUSE: ACGME validator not optimized for full-year validation
...
***REMOVED******REMOVED******REMOVED*** Example 2: Schedule Generation Infeasible

```markdown
***REMOVED******REMOVED*** Problem: Solver returns INFEASIBLE status

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***1: Why is the schedule infeasible?
**Evidence**: Solver logs show "No solution found"
**Answer**: Constraints are conflicting
**Verification**: Solver diagnostic output

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***2: Why are constraints conflicting?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***3: Why is dual coverage required?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***4: Why are all residents PGY-1?
...
***REMOVED******REMOVED******REMOVED*** ROOT CAUSE: Test environment seed data has unrealistic resident mix
...
***REMOVED******REMOVED******REMOVED*** Example 3: Celery Task Failure

```markdown
***REMOVED******REMOVED*** Problem: Scheduled resilience health check fails

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***1: Why does the health check task fail?
**Evidence**: Celery logs show exception
**Answer**: Task raises `KeyError: 'defense_level'`
**Verification**: Stack trace in worker logs

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***2: Why is 'defense_level' missing?
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***3: Why did API format change without updating Celery task?
**Evidence**: Git log shows API change in PR ***REMOVED***456
**Answer**: PR changed API but didn't update Celery consumer
**Verification**: PR ***REMOVED***456 only modified backend/app/api/ not backend/app/tasks/

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***4: Why wasn't the Celery task updated in the same PR?
...
***REMOVED******REMOVED******REMOVED*** ROOT CAUSE: API and consumer code not co-located; no dependency tracking
...
***REMOVED******REMOVED*** Root Cause Analysis Template

Use this template for systematic analysis:

```markdown
***REMOVED*** Root Cause Analysis: [Problem Title]
...
***REMOVED******REMOVED*** 5-Whys Analysis

***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***1: [Problem statement]
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***2: [Previous answer]
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***3: [Previous answer]
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***4: [Previous answer]
...
***REMOVED******REMOVED******REMOVED*** Why ***REMOVED***5: [Previous answer]
...
***REMOVED******REMOVED******REMOVED*** Root Cause Fix
- [What to change to prevent recurrence]

***REMOVED******REMOVED******REMOVED*** Contributing Factor Mitigation
...
***REMOVED******REMOVED******REMOVED*** 1. Stopping Too Soon
...
***REMOVED******REMOVED******REMOVED*** 2. Blaming People
...
***REMOVED******REMOVED******REMOVED*** 3. Accepting Vague Answers
...
***REMOVED******REMOVED*** Integration with Other Workflows

***REMOVED******REMOVED******REMOVED*** With incident-review.md
Root cause analysis is Phase 4 of incident review.
Use this workflow to complete the "Root Cause Analysis" section.

***REMOVED******REMOVED******REMOVED*** With log-analysis.md
Use log analysis to gather evidence for each "why".

***REMOVED******REMOVED******REMOVED*** With debugging-checklist.md
