***REMOVED*** Anti-Patterns for AI Agents
...
***REMOVED******REMOVED*** Safety Anti-Patterns

These anti-patterns can cause **ACGME violations, data loss, or security breaches**. Never do these.

***REMOVED******REMOVED******REMOVED*** ❌ AP-S01: Silently Relaxing Safety Constraints

**What It Looks Like:**

```python
***REMOVED*** BAD: Commenting out ACGME validation to make tests pass
async def create_assignment(db, assignment_data):
    ***REMOVED*** validation.validate_acgme_compliance(assignment_data)  ***REMOVED*** TODO: Fix later
...
```python
***REMOVED*** GOOD: Fix the root cause of validation failure
async def create_assignment(db, assignment_data):
    ***REMOVED*** Always validate before creating
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-S02: Inventing Residents or Rotations

**What It Looks Like:**

```python
***REMOVED*** BAD: Creating synthetic data to fill gaps
if not resident:
    resident = Person(
        id="PLACEHOLDER_001",
        name="TBD Resident",
...
```python
***REMOVED*** GOOD: Fail loudly if data is missing
if not resident:
    raise ValueError(
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-S03: Skipping ACGME Validation

**What It Looks Like:**

```python
***REMOVED*** BAD: Direct database insert without validation
...
```python
***REMOVED*** GOOD: Use service layer with built-in validation
swap_executor = SwapExecutor()
result = await swap_executor.execute_swap(
    db=db,
    swap_request=swap_request,
    validate_acgme=True  ***REMOVED*** Always True in production
)
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-S04: Bypassing Backup Requirements

**What It Looks Like:**

```python
***REMOVED*** BAD: Modifying schedule without backup
async def regenerate_block_schedule(block_id: str):
    ***REMOVED*** Delete existing assignments
    await db.execute(delete(Assignment).where(Assignment.block_id == block_id))
    ***REMOVED*** Generate new assignments
...
```python
***REMOVED*** GOOD: Use safe-schedule-generation skill with mandatory backup
from app.services.schedule_backup import create_backup, restore_backup

async def regenerate_block_schedule(block_id: str):
    ***REMOVED*** MANDATORY: Backup before any write operation
    backup_id = await create_backup(db, scope="block", scope_id=block_id)

    try:
        ***REMOVED*** Delete and regenerate
        await db.execute(delete(Assignment).where(Assignment.block_id == block_id))
        new_assignments = generate_assignments(block_id)
        db.add_all(new_assignments)
        await db.commit()
        ...
        ***REMOVED*** Rollback to backup on any failure
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-S05: Exposing Sensitive Data in Logs

**What It Looks Like:**

```python
***REMOVED*** BAD: Logging full person records
logger.info(f"Processing swap for person: {person}")  ***REMOVED*** Logs name, email, SSN, etc.
```
...
```python
***REMOVED*** GOOD: Log only non-sensitive identifiers
logger.info(f"Processing swap for person_id: {person.id}")

***REMOVED*** If you need to debug, use sanitized representation
...
***REMOVED******REMOVED*** Quality Anti-Patterns

These anti-patterns create **technical debt, bugs, or maintenance burdens**.

***REMOVED******REMOVED******REMOVED*** ❌ AP-Q01: Committing Without Tests

**What It Looks Like:**

```bash
***REMOVED*** BAD: Committing code without running tests
...
```bash
***REMOVED*** GOOD: Always test before committing
cd backend
pytest tests/test_new_feature.py  ***REMOVED*** Run relevant tests
pytest  ***REMOVED*** Run full test suite
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-Q02: Ignoring Linter Warnings

**What It Looks Like:**

```python
***REMOVED*** Code with linter warnings
def calculateHours(assignments):  ***REMOVED*** Naming violation
    total = 0
    for a in assignments:
        total += a.hours  ***REMOVED*** Unused variable
    return sum([a.hours for a in assignments])  ***REMOVED*** Duplication
```
...
```bash
***REMOVED*** GOOD: Fix linter warnings before committing
cd backend
ruff check . --fix  ***REMOVED*** Auto-fix what's possible
ruff format .       ***REMOVED*** Format code

***REMOVED*** Then review remaining warnings manually
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-Q03: Hardcoding Values

**What It Looks Like:**

```python
***REMOVED*** BAD: Hardcoded URLs, limits, credentials
...
```python
***REMOVED*** GOOD: Use configuration and environment variables
from app.core.config import settings

API_URL = settings.API_BASE_URL  ***REMOVED*** From .env
MAX_HOURS_PER_WEEK = settings.ACGME_MAX_HOURS_PER_WEEK
REDIS_PASSWORD = settings.REDIS_PASSWORD  ***REMOVED*** From .env, never committed
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-Q04: Duplicating Code Instead of Refactoring

**What It Looks Like:**

```python
***REMOVED*** BAD: Copy-pasting similar logic
...
```python
***REMOVED*** GOOD: Extract common logic
...
***REMOVED*** Specialized versions just call the common function
async def get_resident_hours_this_week(db, resident_id):
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-Q05: Missing Error Handling

**What It Looks Like:**

```python
***REMOVED*** BAD: No error handling
async def get_person(db, person_id: str):
    result = await db.execute(select(Person).where(Person.id == person_id))
    return result.scalar_one()  ***REMOVED*** Raises if not found, but caller doesn't know
```
...
```python
***REMOVED*** GOOD: Explicit error handling with custom exceptions
...
***REMOVED******REMOVED*** Process Anti-Patterns

These anti-patterns violate **workflow, git, or CI/CD best practices**.

***REMOVED******REMOVED******REMOVED*** ❌ AP-P01: Pushing Directly to Main

**What It Looks Like:**

```bash
***REMOVED*** BAD: Committing directly to main branch
git checkout main
git add .
...
```bash
***REMOVED*** GOOD: Create feature branch, PR workflow
git checkout -b fix/quick-bug-fix
git add .
git commit -m "fix: Quick bug fix"
git push origin fix/quick-bug-fix

***REMOVED*** Then create PR via gh CLI
gh pr create --title "Fix quick bug" --body "Fixes issue ***REMOVED***123"
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-P02: Force Pushing Without Confirmation

**What It Looks Like:**

```bash
***REMOVED*** BAD: Force pushing without checking
...
```bash
***REMOVED*** GOOD: Use force-with-lease and only on feature branches
git branch --show-current  ***REMOVED*** Confirm you're NOT on main
git push --force-with-lease origin feature-branch  ***REMOVED*** Safer than --force
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-P03: Skipping PR Review

**What It Looks Like:**

```bash
***REMOVED*** BAD: Merging PR immediately without review
gh pr create --title "Feature" --body "New feature"
gh pr merge --auto --merge  ***REMOVED*** Merges without approval
```
...
```bash
***REMOVED*** GOOD: Wait for PR review and CI
gh pr create --title "Feature" --body "New feature"
***REMOVED*** Wait for:
***REMOVED*** - CI checks to pass
***REMOVED*** - Human reviewer approval (if required)
***REMOVED*** - Then merge
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-P04: Amending Others' Commits

**What It Looks Like:**

```bash
***REMOVED*** BAD: Amending a commit you didn't create
git log -1 --format='%an %ae'
***REMOVED*** Output: John Doe <john@example.com>  (not you!)

git commit --amend -m "Updated message"  ***REMOVED*** DON'T DO THIS
```
...
```bash
***REMOVED*** GOOD: Check authorship before amending
git log -1 --format='%an %ae'

***REMOVED*** If not your commit, create new commit instead
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-P05: Modifying Database Without Migration

**What It Looks Like:**

```python
***REMOVED*** BAD: Changing model without migration
class Person(Base):
    __tablename__ = "persons"
    id = Column(String, primary_key=True)
    name = Column(String)
    middle_name = Column(String)  ***REMOVED*** ADDED - but no migration!
```
...
```bash
***REMOVED*** GOOD: Always create migration for model changes
***REMOVED*** 1. Modify model
***REMOVED*** 2. Create migration
cd backend
alembic revision --autogenerate -m "Add middle_name to Person"

***REMOVED*** 3. Review migration (autogenerate isn't perfect)
vim alembic/versions/xxx_add_middle_name_to_person.py

***REMOVED*** 4. Test migration
alembic upgrade head
alembic downgrade -1
alembic upgrade head

***REMOVED*** 5. Commit both model and migration
...
***REMOVED******REMOVED*** Communication Anti-Patterns

These anti-patterns cause **confusion, miscommunication, or incomplete work**.

***REMOVED******REMOVED******REMOVED*** ❌ AP-C01: Assuming Context
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-C02: Making Up Data
...
```python
***REMOVED*** GOOD: Query database before answering
...
***REMOVED*** Then respond with actual data
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-C03: Hiding Errors

**What It Looks Like:**

```python
***REMOVED*** BAD: Silently catching and ignoring errors
try:
    result = await validate_acgme_compliance(assignment)
except Exception:
    pass  ***REMOVED*** Hope it works out
```
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-C04: Overcommitting Scope
...
***REMOVED******REMOVED******REMOVED*** ❌ AP-C05: Vague Commit Messages

**What It Looks Like:**

```bash
***REMOVED*** BAD: Vague or unhelpful commit messages
...
```bash
***REMOVED*** GOOD: Descriptive commit messages following conventional commits
...
***REMOVED******REMOVED*** Detection Checklist
...
***REMOVED******REMOVED*** When You Catch Yourself
...
***REMOVED******REMOVED*** Contributing New Anti-Patterns

If you discover a new anti-pattern:

```markdown
***REMOVED******REMOVED******REMOVED*** ❌ AP-X0N: [Name of Anti-Pattern]

**What It Looks Like:**

```code
***REMOVED*** Example of the anti-pattern
```
...
```code
***REMOVED*** Example of the correct way
```
