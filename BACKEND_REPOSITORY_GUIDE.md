***REMOVED*** Backend Repository Guide
***REMOVED******REMOVED*** SESSION 8 BURN: Data Access Layer Best Practices
...
***REMOVED******REMOVED*** SECTION 1: ASYNC MIGRATION STRATEGY

***REMOVED******REMOVED******REMOVED*** Current Problem

```python
***REMOVED*** CURRENT (Synchronous - blocks event loop)
...
class AssignmentRepository:
    ...
    def get_by_id(self, id: UUID) -> Assignment | None:
        ...
        ***REMOVED*** ↑ This blocks FastAPI async routes!
```

***REMOVED******REMOVED******REMOVED*** Migration Approach

**Phase 1 (Immediate):** Add AsyncBaseRepository alongside existing sync

```python
***REMOVED*** backend/app/repositories/async_base.py
...
***REMOVED******REMOVED******REMOVED*** Phase 2 (Week 2): Create Async Variants of Critical Repos

```python
***REMOVED*** backend/app/repositories/async_assignment.py
from sqlalchemy import select
...
***REMOVED******REMOVED******REMOVED*** Phase 3 (Week 3): Update Database Session

```python
***REMOVED*** backend/app/db/session.py (UPDATED)
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession, async_sessionmaker
from sqlalchemy.orm import Session, sessionmaker
from sqlalchemy import create_engine

***REMOVED*** Keep synchronous engine for backward compatibility
...
***REMOVED*** Add async engine
async_engine = create_async_engine(
    settings.ASYNC_DATABASE_URL,  ***REMOVED*** postgresql+asyncpg://...
    pool_size=settings.DB_POOL_SIZE,
    max_overflow=settings.DB_POOL_MAX_OVERFLOW,
)
...
***REMOVED*** Sync dependency (existing)
...
***REMOVED******REMOVED*** SECTION 2: EAGER LOADING PATTERNS

***REMOVED******REMOVED******REMOVED*** Problem: N+1 Queries

```python
***REMOVED*** ANTI-PATTERN (N+1 Query Risk)
assignments = repo.list_residents()  ***REMOVED*** Query 1: Load all residents
for assignment in assignments:
    print(assignment.person.name)  ***REMOVED*** Query 2..N: Load person for each assignment
```

***REMOVED******REMOVED******REMOVED*** Solution: Eager Loading

```python
***REMOVED*** PATTERN 1: joinedload (Single Query with Join)
...
***REMOVED*** PATTERN 2: selectinload (Multiple Queries but Efficient)
from sqlalchemy.orm import selectinload
...
***REMOVED*** PATTERN 3: Nested eager loading
...
***REMOVED******REMOVED******REMOVED*** Standardization Rule
...
***REMOVED******REMOVED*** SECTION 3: TRANSACTION MANAGEMENT STANDARDIZATION

***REMOVED******REMOVED******REMOVED*** Problem: Inconsistent Commit Patterns

```python
***REMOVED*** ANTI-PATTERN 1: Repository commits (incorrect)
class SwapRepository:
    def create(self, **kwargs):
        swap = SwapRecord(**kwargs)
        self.db.add(swap)
        self.db.commit()  ***REMOVED*** ← Repository shouldn't commit!
        return swap

***REMOVED*** ANTI-PATTERN 2: Implicit commits (unclear)
class AssignmentService:
    def create_assignment(self, ...):
        assignment = self.assignment_repo.create(...)
        ***REMOVED*** When does transaction commit? Unclear!
```

***REMOVED******REMOVED******REMOVED*** Solution: Unit of Work Pattern

```python
***REMOVED*** backend/app/repositories/unit_of_work.py
...
class AssignmentService:
    ...
    def create_assignment_with_cascade(self, ...):
        ...
        with self.uow as uow:
            ***REMOVED*** All operations in this block are in same transaction
            assignment = uow.assignment.create(...)
            uow.person.update(person_obj, updated_at=now())
            uow.block.update(block_obj, last_modified=now())

            ***REMOVED*** Automatically commits on exit, rolls back on exception
```

---

***REMOVED******REMOVED*** SECTION 4: ASYNC QUERY BEST PRACTICES

***REMOVED******REMOVED******REMOVED*** Using SQLAlchemy 2.0 select() Style

```python
***REMOVED*** backend/app/repositories/async_query_examples.py
from sqlalchemy import select, and_, or_
from sqlalchemy.orm import selectinload

***REMOVED*** PATTERN 1: Simple filter
...
***REMOVED*** PATTERN 2: OR conditions
...
***REMOVED******REMOVED*** SECTION 5: REPOSITORY TESTING GUIDE

***REMOVED******REMOVED******REMOVED*** Unit Testing with Mocks

```python
***REMOVED*** backend/tests/repositories/test_assignment_repository.py
import pytest
...
async def test_get_by_id(mock_db):
    ...
    ***REMOVED*** Mock the query execution
...
***REMOVED******REMOVED******REMOVED*** Integration Testing with Test Database

```python
***REMOVED*** backend/tests/repositories/test_assignment_integration.py
...
***REMOVED******REMOVED*** SECTION 6: REPOSITORY ERROR HANDLING

***REMOVED******REMOVED******REMOVED*** Add Repository-Specific Exceptions

```python
***REMOVED*** backend/app/repositories/exceptions.py
...
***REMOVED******REMOVED******REMOVED*** Use Exceptions in Repositories
...
***REMOVED******REMOVED*** SECTION 7: PAGINATION BEST PRACTICES

***REMOVED******REMOVED******REMOVED*** Cursor-Based Pagination (Recommended for Large Data Sets)
...
async def list_with_cursor(
    ...
    if cursor:
        ***REMOVED*** Decode cursor to get last_created_at and last_id
...
***REMOVED******REMOVED******REMOVED*** Offset-Based Pagination (Simple, Current Standard)
...
***REMOVED******REMOVED*** SECTION 8: BULK OPERATIONS

***REMOVED******REMOVED******REMOVED*** Bulk Insert
...
***REMOVED******REMOVED******REMOVED*** Bulk Update
...
***REMOVED******REMOVED*** SECTION 10: CHECKLIST FOR NEW REPOSITORIES
