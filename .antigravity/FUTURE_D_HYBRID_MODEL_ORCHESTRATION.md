***REMOVED*** Hybrid Model Orchestration (FUTURE - CONCEPT ONLY)
...
***REMOVED******REMOVED*** Critical Constraint: Skill Compatibility

**Qwen cannot use `.claude/skills/`** - this is the key architectural constraint.

***REMOVED******REMOVED******REMOVED*** Skill-Required Tasks (Opus Only)
...
***REMOVED******REMOVED******REMOVED*** Skill-Free Tasks (Qwen Squad)
...
***REMOVED******REMOVED*** Task Routing Logic
...
***REMOVED******REMOVED******REMOVED*** Parallel Test Execution

```
Opus Tactician: "Run backend and frontend tests in parallel"

→ Qwen ***REMOVED***1: cd backend && pytest -v
→ Qwen ***REMOVED***2: cd frontend && npm test
→ Qwen ***REMOVED***3: cd frontend && npm run type-check
→ Qwen ***REMOVED***4: cd frontend && npm run lint

← Opus synthesizes results, applies code-review skill if failures
```

***REMOVED******REMOVED******REMOVED*** Bulk Linting/Formatting

```
Opus Tactician: "Format and lint the entire codebase"

→ Qwen ***REMOVED***1: ruff check backend/ --fix
→ Qwen ***REMOVED***2: ruff format backend/
→ Qwen ***REMOVED***3: cd frontend && npm run lint:fix
→ Qwen ***REMOVED***4: mypy backend/app/ --ignore-missing-imports

← Opus reviews output, commits if clean
```

***REMOVED******REMOVED******REMOVED*** Exploration with Synthesis

```
Opus Tactician: "Find all ACGME validation usages and assess coverage"

→ Qwen ***REMOVED***1: grep -r "ACGMEValidator" backend/
→ Qwen ***REMOVED***2: grep -r "acgme" backend/tests/
→ Qwen ***REMOVED***3: grep -r "80.hour\|1.in.7" backend/
...
***REMOVED******REMOVED*** Qwen Code Setup (Reference)
...
***REMOVED*** - Qwen OAuth: 2,000 free requests/day
***REMOVED*** - Or: OPENAI_API_KEY with custom base URL

***REMOVED*** Config location
~/.qwen/settings.json
```
...
***REMOVED******REMOVED*** Prerequisites Before Implementation
